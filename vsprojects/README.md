...
#C/C++ Test Dependencies
   * gtest isn't available as a git repo like the other dependencies.  download it and add it to `/third_party/gtest/` (the folder will end up with `/build-aux/`, `/cmake/`, `/codegear/`, etc. folders in it).  
    ...
    * build all
   * build protobuf solution in `/third_party/protobuf/vsprojects`
   ...
   *  gflags is automatically imported as a git submodule but it comes needing cmake to be ready for a specific platform
...
#C/C++ Test Build Steps
   ...
   * add `"debug": true,` to the top of build.json.  This is the base file for all build tracking.  everything eles is auto-generated by various scripts.
   ...
   * run `/templates/vsprojects/generate_debug_projects.sh`.  This runs a regular visual studio buildgen process, which creates the .sln file with all of the new debug projects, then uses git diff to find the new project names from the .sln that need templates added.  A basic git version of grpc only has templates for non-test items.  It builds the new templates based on the diff, then re-runs the visual studio buildgen, which builds the vs projects for each of the new debug targets
   ...
   * run `/templates/vsprojects/build_test_protos.sh`
    * this builds all `.proto` files in `/test/` in-place.  there might be a better place to put them that mirrors what happens in the linux build process
