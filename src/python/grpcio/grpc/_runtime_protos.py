...
def _is_grpc_tools_importable() -> bool:
    try:
        import grpc_tools  # pylint: disable=unused-import # pytype: disable=import-error # pyright: ignore[reportMissingImports, reportMissingTypeStubs, reportUnusedImport]
...
def _call_with_lazy_import(
    fn_name: Literal["protos", "services"], protobuf_path: str
) -> types.ModuleType: ...


@overload
def _call_with_lazy_import(
    fn_name: Literal["protos_and_services"], protobuf_path: str
...
def _call_with_lazy_import(
    ...
    import grpc_tools.protoc  # pytype: disable=import-error # pyright: ignore[reportMissingImports, reportMissingTypeStubs]

    if _has_runtime_proto_symbols(grpc_tools.protoc):  # pyright: ignore[reportUnknownArgumentType, reportUnknownMemberType]
        fn = getattr(grpc_tools.protoc, "_" + fn_name)  # pyright: ignore[reportUnknownArgumentType, reportUnknownMemberType]
        return fn(protobuf_path)  # pyright: ignore[reportUnknownVariableType]
    raise NotImplementedError(_UNINSTALLED_TEMPLATE.format(fn_name))
