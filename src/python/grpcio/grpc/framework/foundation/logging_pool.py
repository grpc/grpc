...
def _wrap(behavior: Callable) -> Callable:
    """Wraps an arbitrary callable behavior in exception-logging."""

    def _wrapping(*args: Any, **kwargs: Any) -> Any:
        try:
...
class _LoggingPool(object):
    ...
    def submit(self, fn: Callable, *args: Any, **kwargs: Any) -> futures.Future:
        return self._backing_pool.submit(_wrap(fn), *args, **kwargs)

    def map(self, func: Callable, *iterables: Any, **kwargs: Any) -> Any:
