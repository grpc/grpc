...
from grpc import (
    ...
    _interceptor,  # pytype: disable=pyi-error
...
def _get_initial_metadata_operation(
    ...
    return cygrpc.SendInitialMetadataOperation(
        _get_initial_metadata(state, metadata),
        _EMPTY_FLAGS,
    )
...
class _RequestIterator:
    _state: _RPCState
    _call: cygrpc.Call
    _request_deserializer: DeserializingFunction | None
            ...
            while True:
                ...
                if state.request is None:
                    if state.client is _CLOSED:
                        details = f'"{rpc_event.call_details.method}" requires exactly one request message.'
                        _abort(
                            state,
                            rpc_event.call,
...
def _serialize_response(
    ...
    state: _RPCState,
    response: Any,
    response_serializer: SerializingFunction | None,
) -> bytes | None:
...
def _status(
    rpc_event: cygrpc.BaseEvent,
    state: _RPCState,
    serialized_response: bytes | None,
) -> None:
    with state.condition:
        if state.client is not _CANCELLED:
...
def _unary_response_in_pool(
    ...
    request_deserializer: SerializingFunction | None,
    response_serializer: SerializingFunction | None,
) -> None:
...
def _select_thread_pool_for_behavior(
    ...
    if hasattr(behavior, "experimental_thread_pool") and isinstance(
        behavior.experimental_thread_pool,
        futures.ThreadPoolExecutor,
    ):
...
) -> tuple[_RPCState | None, futures.Future | None]:
...
def _add_generic_handlers(
    state: _ServerState,
    generic_handlers: Iterable[grpc.GenericRpcHandler],
) -> None:
    with state.lock:
...
def _stop(state: _ServerState, grace: float | None) -> threading.Event:
    with state.lock:
...
class _Server(grpc.Server):
    ...
    def add_insecure_port(self, address: str) -> int:
        return _common.validate_port_binding_result(
            address,
            _add_insecure_port(self._state, _common.encode(address)),
        )
