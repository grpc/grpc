...
def _unknown_code_details(
    unknown_cygrpc_code: Optional[grpc.StatusCode],
...
def _consume_request_iterator( # noqa: C901
    ...
    def consume_request_iterator() -> None:  # noqa: C901 # pylint: disable=too-many-branches
...
class _InactiveRpcError(grpc.RpcError, grpc.Call, grpc.Future):
    ...
    def result(
        self, timeout: Optional[float] = None, # noqa: ARG002
    ) -> Any:  # pylint: disable=unused-argument
        """See grpc.Future.result."""
        raise self

    def exception(
        self, timeout: Optional[float] = None,  # noqa: ARG002 # pylint: disable=unused-argument
    ) -> Optional[Exception]:
...
class _SingleThreadedRendezvous(
    ...
    def trailing_metadata(self) -> Optional[MetadataType]:
        ...
        with self._state.condition:
            if self._state.trailing_metadata is None:
                msg = "Cannot get trailing metadata until RPC is completed."
                raise grpc.experimental.UsageError(
                    msg,
                )
            return self._state.trailing_metadata
    ...
    def code(self) -> Optional[grpc.StatusCode]:
        ...
        with self._state.condition:
            if self._state.code is None:
                msg = "Cannot get code until RPC is completed."
                raise grpc.experimental.UsageError(
                    msg,
                )
            return self._state.code
    ...
    def details(self) -> Optional[str]:
        ...
        with self._state.condition:
            if self._state.details is None:
                msg = "Cannot get details until RPC is completed."
                raise grpc.experimental.UsageError(
                    msg,
                )
            return _common.decode(self._state.details)
    ...
    def debug_error_string(self) -> Optional[str]:
        with self._state.condition:
            if self._state.debug_error_string is None:
                msg = "Cannot get debug error string until RPC is completed."
                raise grpc.experimental.UsageError(
                    msg,
                )
            return _common.decode(self._state.debug_error_string)
