...
class OpenTelemetryLabelInjector:
  ...
  def get_labels_for_exchange(self) -> Dict[str, AnyStr]:
    ...
    Returns:
      A dict of labels, with a string as key representing label name, string or
      bytes
  ...
  def get_additional_labels(
    ...
    Args:
      include_exchange_labels: Whether to add additional metadata exchange
        related labels.
  ...
  def deserialize_labels(self, labels: Dict[str, AnyStr]) -> Dict[str, AnyStr]:
    ...
    If this injector added labels for metadata exchange, this method will be
    called to
    deserialize the exchanged labels.

    For example, if this injector added xds_peer_metadata_label for exchange:

        labels: {"labelA": b"valueA", "xds_peer_metadata_label":
        b"exchanged_bytes"}
...
class OpenTelemetryPlugin:
  ...
  def __init__(
    ...
    enabled for this OpenTelemetryPlugin.
      meter_provider: A MeterProvider which will be used to collect telemetry
      data,
    or None which means no metrics will be collected.
      target_attribute_filter: [DEPRECATED] This attribute is deprecated and
      should
    not be used.
    Once provided, this will be called per channel to decide whether to record
    the
    target attribute on client or to replace it with "other".
    This helps reduce the cardinality on metrics in cases where many channels
    are created with different targets in the same binary (which might happen
    for example, if the channel target string uses IP addresses directly).
    Return True means the original target string will be used, False means
    target string
    will be replaced with "other".
      generic_method_attribute_filter: Once provided, this will be called with a
      generic
    method type to decide whether to record the method name or to replace it
    with
    "other". Note that pre-registered methods will always be recorded no matter
    what
    this function returns.
    Return True means the original method name will be used, False means method
    name will
    ...
    self.generic_method_attribute_filter = generic_method_attribute_filter or (
        lambda target: False
    )
    self._plugins = [_open_telemetry_observability._OpenTelemetryPlugin(self)]
