...
def _configure_maintenance_server(server: grpc.Server, maintenance_port: int) -> None:
...
def _configure_test_server(server: grpc.Server, port: int, secure_mode: bool, server_id: str) -> None:
    test_pb2_grpc.add_TestServiceServicer_to_server(TestService(server_id, socket.gethostname()), server)
...
def _run(port: int, maintenance_port: int, secure_mode: bool, server_id: str) -> None:
    if port == maintenance_port:
        ...
        test_server = grpc.server(futures.ThreadPoolExecutor(max_workers=_THREAD_POOL_SIZE), xds=secure_mode)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Python xDS interop server.")
    ...
    parser.add_argument("--secure_mode",
                        type=str,
                        default="False",
    ...
    if secure_mode and args.port == args.maintenance_port:
        raise ValueError("--port and --maintenance_port must not be the same when --secure_mode is set.")
