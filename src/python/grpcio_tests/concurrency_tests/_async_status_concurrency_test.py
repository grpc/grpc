...
class AsyncStatusConcurrencyTest(AsyncConcurrencyTestCase):
    async def setUp(self):
        await super().setUp()
        # status uses a generic handler + raw channel (no generated stub)
        _, self._channel = await self.start_server(
            None,
    ...
    async def _status_ok(self, unused_index):
    ...
    async def _error_details(self, unused_index):
        for _ in range(ITERATIONS_PER_TASK):
    ...
    async def test_concurrent_error_details(self):
        await self.run_workers(self._error_details)

    async def test_concurrent_ok_and_error(self):
        await self.run_workers(self._status_ok, self._error_details)
