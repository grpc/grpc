...
def initial_call():
    ...
    response = stub.UnaryCall(test_pb2.SimpleRequest(), timeout=5)
...
class FunctionsFrameworkForkTest(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            if self._port is None:
                ...
                self._streams[0].seek(0)
                sys.stderr.write(
                    "Server STDOUT:\n{}\n".format(self._streams[0].read().decode("utf-8", "ignore"))
                )
                self._streams[1].seek(0)
                sys.stderr.write(
                    "Server STDERR:\n{}\n".format(self._streams[1].read().decode("utf-8", "ignore"))
                )
                sys.stderr.flush()
    ...
    def _verifyTestCase(self, fork_support, initial_call):
        script = _CLIENT_FORK_SCRIPT_TEMPLATE % self._port
        ...
        process = subprocess.Popen(
            [sys.executable, "-m", "functions_framework", "--target", "hello", "--source", script_path, "--port", "8080"],
