...
class ChannelzServicerTest(unittest.TestCase):
    def _send_successful_unary_unary(self, idx):
        _, r = (
            self._pairs[idx]
            .channel.unary_unary(_SUCCESSFUL_UNARY_UNARY, _registered_method=True)
            .with_call(_REQUEST)
        )
    ...
    def _send_failed_unary_unary(self, idx):
        try:
            self._pairs[idx].channel.unary_unary(_FAILED_UNARY_UNARY, _registered_method=True).with_call(
                _REQUEST
            )
    ...
    def _send_successful_stream_stream(self, idx):
        response_iterator = (
            self._pairs[idx]
            .channel.stream_stream(_SUCCESSFUL_STREAM_STREAM, _registered_method=True)
