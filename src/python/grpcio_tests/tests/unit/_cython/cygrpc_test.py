...
class ServerClientMixin:
    ...
    def test_benchmark_memoryview(self):
        # Benchmark large message reception
        PAYLOAD_SIZE = 10 * 1024 * 1024  # 10MB
        ITERATIONS = 5  # Reduced iterations to keep test time reasonable
        DEADLINE = time.time() + 60
        METHOD = b"benchmark"
        REQUEST = b"x" * PAYLOAD_SIZE
        RESPONSE = b"y" * PAYLOAD_SIZE
        ...
        for _ in range(ITERATIONS):
            ...
            server_call.start_server_batch(
                [
                    ...
                    cygrpc.ReceiveCloseOnServerOperation(_EMPTY_FLAGS),
                    cygrpc.SendStatusFromServerOperation(
                        (), cygrpc.StatusCode.ok, b"", _EMPTY_FLAGS
                    ),
                ],
                server_call_tag,
            )
        ...
        end_time = time.time()
        print(f"\n[BENCHMARK] {self.__class__.__name__} Time for {ITERATIONS} iterations of {PAYLOAD_SIZE} bytes: {end_time - start_time:.4f} seconds")
