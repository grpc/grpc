...
def _unary_unary_client(channel, multicallable_kwargs, message):
    ...
    response = multi_callable(message, wait_for_ready=True, timeout=15, **multicallable_kwargs)
    if response != message:
        raise RuntimeError(
...
def _unary_stream_client(channel, multicallable_kwargs, message):
    ...
    response_iterator = multi_callable(message, wait_for_ready=True, timeout=15, **multicallable_kwargs)
...
def _stream_unary_client(channel, multicallable_kwargs, message):
    ...
    response = multi_callable(requests, wait_for_ready=True, timeout=15, **multicallable_kwargs)
    if response != message:
        raise RuntimeError(
            "Request '{}' != Response '{}'".format(message, response)
        )
...
def _stream_stream_client(channel, multicallable_kwargs, message):
    ...
    response_iterator = multi_callable(requests, wait_for_ready=True, timeout=15, **multicallable_kwargs)
