...
php_grpc_zend_object create_wrapped_grpc_channel(
...
int php_grpc_read_args_array(zval* args_array,
  ...
  if (!array_hash) {
    zend_throw_exception(spl_ce_InvalidArgumentException, "array_hash is NULL",
                         1 TSRMLS_CC);
    return FAILURE;
  }
...
bool php_grpc_persistent_list_delete_unused_channel(
    char* target, target_bound_le_t* target_bound_status TSRMLS_DC) {
  zval* data;
  ...
  if (strcmp(le->channel->target, target) == 0) {
    ...
    if (le->channel->ref_count == 1) {
      php_grpc_delete_persistent_list_entry(le->channel->key,
                                            strlen(le->channel->key) TSRMLS_CC);
...
target_bound_le_t* update_and_get_target_upper_bound(char* target, int bound) {
  ...
  if (!(PHP_GRPC_PERSISTENT_LIST_FIND(&grpc_target_upper_bound_map, target,
    ...
    // Init current_count with 1. It should be add 1 when the channel is
    // successfully created and minus 1 when it is removed from the persistent
    // list.
    ...
    PHP_GRPC_PERSISTENT_LIST_UPDATE(&grpc_target_upper_bound_map, target,
...
void create_and_add_channel_to_persistent_list(
    wrapped_grpc_channel* channel, char* target, grpc_channel_args args,
    wrapped_grpc_channel_credentials* creds, char* key, php_grpc_int key_len,
  ...
  if (target_bound_status->current_count >= target_bound_status->upper_bound) {
    if (!php_grpc_persistent_list_delete_unused_channel(
      ...
      gpr_log(GPR_INFO,
              "[Warning] The number of channel for the"
...
PHP_METHOD(Channel, __construct) {
  ...
  if (php_grpc_zend_hash_find(array_hash, "grpc_target_persist_bound",
    ...
    if (Z_TYPE_P(force_new_obj) != IS_LONG) {
      zend_throw_exception(spl_ce_InvalidArgumentException,
                           "plist_bound must be a number", 1 TSRMLS_CC);
    }
  ...
  for (int i = 0; i < args.num_args; i++) {
    PHP_GRPC_SHA1Update(&context, args.args[i].key,
    ...
    switch (args.args[i].type) {
      ...
      case GRPC_ARG_STRING:
        PHP_GRPC_SHA1Update(&context, args.args[i].value.string,
                            strlen(args.args[i].value.string) + 1);
        break;
      default:
        zend_throw_exception(spl_ce_InvalidArgumentException,
                             "args values must be int or string", 1 TSRMLS_CC);
  ...
  if (force_new || (creds != NULL && creds->has_call_creds)) {
    ...
    create_channel(channel, target, args, creds);
                                             ...
                                             key_len, rsrc))) {
    create_and_add_channel_to_persistent_list(channel, target, args, creds, key,
                                              key_len,
                                              target_upper_bound TSRMLS_CC);
  } else {
    ...
    if (strcmp(target, le->channel->target) != 0 ||
      ...
      create_and_add_channel_to_persistent_list(channel, target, args, creds,
                                                key, key_len,
                                                target_upper_bound TSRMLS_CC);
    } else {
      efree(args.args);
...
PHP_METHOD(Channel, watchConnectivityState) {
  ...
  /* "lO" == 1 long 1 object */
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lO", &last_state,
  ...
  grpc_channel_watch_connectivity_state(
      channel->wrapper->wrapped, (grpc_connectivity_state)last_state,
      deadline->wrapped, completion_queue, NULL);
  grpc_event event = grpc_completion_queue_pluck(
      completion_queue, NULL, gpr_inf_future(GPR_CLOCK_REALTIME), NULL);
  gpr_mu_unlock(&channel->wrapper->mu);
...
void php_grpc_delete_persistent_list_entry(char* key,
  ...
  php_grpc_zend_resource* rsrc;
  gpr_mu_lock(&global_persistent_list_mu);
  if (PHP_GRPC_PERSISTENT_LIST_FIND(&grpc_persistent_list, key, key_len,
                                    rsrc)) {
...
static void php_grpc_channel_plink_dtor(
...
PHP_METHOD(Channel, getChannelInfo) {
  ...
  PHP_GRPC_ADD_STRING_TO_ARRAY(return_value, "target", sizeof("target"),
                               channel->wrapper->target, true);
  ...
  PHP_GRPC_ADD_LONG_TO_ARRAY(return_value, "target_upper_bound",
                             sizeof("target_upper_bound"),
                             target_bound_status->upper_bound);
  PHP_GRPC_ADD_LONG_TO_ARRAY(return_value, "target_current_size",
                             sizeof("target_current_size"),
                             target_bound_status->current_count);
  // Info about key
  PHP_GRPC_ADD_STRING_TO_ARRAY(return_value, "key", sizeof("key"),
                               channel->wrapper->key, true);
  // Info about persistent channel ref_count
  PHP_GRPC_ADD_LONG_TO_ARRAY(return_value, "ref_count", sizeof("ref_count"),
  ...
  PHP_GRPC_ADD_STRING_TO_ARRAY(return_value, "ob", sizeof("ob"),
                               grpc_connectivity_state_name(state), true);
  // Info about the channel is closed or not
  PHP_GRPC_ADD_BOOL_TO_ARRAY(return_value, "is_valid", sizeof("is_valid"),
                             (channel->wrapper == NULL));
}
...
PHP_METHOD(Channel, getPersistentList) {
  ...
  array_init(ret_arr);
  // Info about the target
  PHP_GRPC_ADD_STRING_TO_ARRAY(ret_arr, "target", sizeof("target"),
                               le->channel->target, true);
  // Info about the upper bound for the target
  target_bound_le_t* target_bound_status =
  ...
  PHP_GRPC_ADD_LONG_TO_ARRAY(ret_arr, "target_current_size",
                             ...
                             target_bound_status->current_count);
  // Info about key
  PHP_GRPC_ADD_STRING_TO_ARRAY(ret_arr, "key", sizeof("key"), le->channel->key,
                               true);
  // Info about persistent channel ref_count
  PHP_GRPC_ADD_LONG_TO_ARRAY(ret_arr, "ref_count", sizeof("ref_count"),
                             le->channel->ref_count);
  // Info about connectivity status
  int state =
  ...
  PHP_GRPC_ADD_LONG_TO_ARRAY(ret_arr, "connectivity_status",
                             sizeof("connectivity_status"), state);
  PHP_GRPC_ADD_STRING_TO_ARRAY(ret_arr, "ob", sizeof("ob"),
                               grpc_connectivity_state_name(state), true);
...
static zend_function_entry channel_methods[] = {
    PHP_ME(Channel, __construct, arginfo_construct,
        ...
        PHP_ME(Channel, getTarget, arginfo_getTarget, ZEND_ACC_PUBLIC)
            PHP_ME(Channel, getConnectivityState, arginfo_getConnectivityState,
                   ZEND_ACC_PUBLIC) PHP_ME(Channel, watchConnectivityState,
                                           arginfo_watchConnectivityState,
                                           ZEND_ACC_PUBLIC)
                PHP_ME(Channel, close, arginfo_close, ZEND_ACC_PUBLIC)
#ifdef GRPC_PHP_DEBUG
                    PHP_ME(Channel, getChannelInfo, arginfo_getChannelInfo,
                           ZEND_ACC_PUBLIC)
                        PHP_ME(Channel, cleanPersistentList,
                               arginfo_cleanPersistentList, ZEND_ACC_PUBLIC)
                            PHP_ME(Channel, getPersistentList,
                                   arginfo_getPersistentList, ZEND_ACC_PUBLIC)
#endif
                                PHP_FE_END};
...
GRPC_STARTUP_FUNCTION(channel) {
  ...
  le_bound = zend_register_list_destructors_ex(NULL, php_grpc_target_bound_dtor,
                                               "Target Bound", module_number);
  ZEND_HASH_INIT(&grpc_target_upper_bound_map, 20,
