...
zval* grpc_php_wrap_call_credentials(grpc_call_credentials* wrapped TSRMLS_DC) {
  ...
  wrapped_grpc_call_credentials* credentials = PHP_GRPC_GET_WRAPPED_OBJECT(
      wrapped_grpc_call_credentials, credentials_object);
...
int plugin_get_metadata(
  ...
  *error_details = NULL;
  ...
  if (metadata.count > GRPC_METADATA_CREDENTIALS_PLUGIN_SYNC_MAX) {
    ...
    *error_details =
        gpr_strdup("PHP plugin credentials returned too many metadata entries");
