...
namespace Grpc.Core
{
    ...
    {
        protected virtual object BeforeCall<TRequest, TResponse>(ref Method<TRequest, TResponse> method, ref string host, ref CallOptions options, TRequest request) { return null; }

        protected virtual void AfterCall<TResponse>(object state, TResponse response) { }
		...
		{
			var state = BeforeCall(ref method, ref host, ref options, request);
			var response = next(method, host, options, request);
			AfterCall(state, response);
			return response;
		}
		...
		{
			var state = BeforeCall(ref method, ref host, ref options, default(TRequest));
			var response = next(method, host, options);
			AfterCall(state, response);
			return response;
		}
