...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_metadata_array_set(
    grpcsharp_metadata_array *array, size_t index, const char *key,
    const char *value, size_t value_length) {
  GPR_ASSERT(index < array->count);
  array->metadata[index].key = grpc_slice_from_copied_string(key);
  array->metadata[index].value =
...
GPR_EXPORT const char *GPR_CALLTYPE grpcsharp_metadata_array_get_key(
    grpcsharp_metadata_array *array, size_t index, size_t *key_length) {
  GPR_ASSERT(index < array->count);
...
GPR_EXPORT const char *GPR_CALLTYPE grpcsharp_metadata_array_get_value(
    grpcsharp_metadata_array *array, size_t index, size_t *value_length) {
  GPR_ASSERT(index < array->count);
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_start_unary(
  ...
  ops[3].data.recv_initial_metadata.count = &(ctx->recv_initial_metadata.count);
  ops[3].flags = 0;
  ops[3].reserved = NULL;
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_start_client_streaming(
  ...
  ops[1].data.recv_initial_metadata.initial_metadata =
      &(ctx->recv_initial_metadata.metadata);
  ops[1].data.recv_initial_metadata.count = &(ctx->recv_initial_metadata.count);
  ops[1].flags = 0;
  ops[1].reserved = NULL;
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_recv_initial_metadata(
  ...
  ops[0].data.recv_initial_metadata.initial_metadata =
      &(ctx->recv_initial_metadata.metadata);
  ops[0].data.recv_initial_metadata.count = &(ctx->recv_initial_metadata.count);
  ops[0].flags = 0;
  ops[0].reserved = NULL;
...
grpcsharp_server_request_call(grpc_server *server, grpc_completion_queue *cq,
  ...
  return grpc_server_request_call(server, &(ctx->call), &(ctx->call_details),
                                  ...
                                  &(ctx->request_metadata.count), cq, cq, ctx);
}
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_metadata_credentials_notify_from_plugin(
    ...
    grpcsharp_metadata_array *metadata, grpc_status_code status,
