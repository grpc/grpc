...
static grpc_byte_buffer* grpcsharp_create_byte_buffer_from_stolen_slices(
  ...
  grpc_byte_buffer* bb = (grpc_byte_buffer*)gpr_zalloc(sizeof(grpc_byte_buffer));
  bb->type = GRPC_BB_RAW;
 ...
 */
void grpcsharp_metadata_array_destroy_metadata_only(grpc_metadata_array* array) {
 ...
 * The array pointer itself is not freed.
 */
void grpcsharp_metadata_array_destroy_metadata_including_entries(grpc_metadata_array* array) {
  size_t i;
  if (array->metadata) {
...
GPR_EXPORT grpc_metadata_array* GPR_CALLTYPE grpcsharp_metadata_array_create(size_t capacity) {
  grpc_metadata_array* array = (grpc_metadata_array*)gpr_malloc(sizeof(grpc_metadata_array));
  grpc_metadata_array_init(array);
  array->capacity = capacity;
  array->count = 0;
  if (capacity > 0) {
    array->metadata = (grpc_metadata*)gpr_malloc(sizeof(grpc_metadata) * capacity);
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_metadata_array_add(grpc_metadata_array* array,
...
GPR_EXPORT intptr_t GPR_CALLTYPE grpcsharp_metadata_array_count(grpc_metadata_array* array) {
  return (intptr_t)array->count;
}

GPR_EXPORT const char* GPR_CALLTYPE grpcsharp_metadata_array_get_key(grpc_metadata_array* array,
                                                                     size_t index,
                                                                     size_t* key_length) {
...
GPR_EXPORT const char* GPR_CALLTYPE grpcsharp_metadata_array_get_value(grpc_metadata_array* array,
                                                                       size_t index,
                                                                       size_t* value_length) {
  GPR_ASSERT(index < array->count);
...
void grpcsharp_metadata_array_move(grpc_metadata_array* dest, grpc_metadata_array* src) {
  if (!src) {
    dest->capacity = 0;
    dest->count = 0;
    dest->metadata = NULL;
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_batch_context_reset(grpcsharp_batch_context* ctx) {
  grpcsharp_metadata_array_destroy_metadata_including_entries(&(ctx->send_initial_metadata));
  ...
  grpcsharp_metadata_array_destroy_metadata_only(&(ctx->recv_status_on_client.trailing_metadata));
...
grpcsharp_batch_context_recv_initial_metadata(const grpcsharp_batch_context* ctx) {
  return &(ctx->recv_initial_metadata);
}

GPR_EXPORT intptr_t GPR_CALLTYPE
grpcsharp_batch_context_recv_message_length(const grpcsharp_batch_context* ctx) {
 ...
 */
GPR_EXPORT int GPR_CALLTYPE grpcsharp_batch_context_recv_message_next_slice_peek(
  ...
  *slice_data_ptr = NULL;
  ...
  if (!ctx->recv_message_reader) {
    ...
    GPR_ASSERT(grpc_byte_buffer_reader_init(ctx->recv_message_reader, ctx->recv_message));
  }
...
GPR_EXPORT const char* GPR_CALLTYPE grpcsharp_batch_context_recv_status_on_client_details(
...
grpcsharp_batch_context_recv_status_on_client_error_string(const grpcsharp_batch_context* ctx) {
...
grpcsharp_request_call_context_deadline(const grpcsharp_request_call_context* ctx) {
  return ctx->call_details.deadline;
}

GPR_EXPORT const grpc_metadata_array* GPR_CALLTYPE
grpcsharp_request_call_context_request_metadata(const grpcsharp_request_call_context* ctx) {
  return &(ctx->request_metadata);
}

GPR_EXPORT int32_t GPR_CALLTYPE
grpcsharp_batch_context_recv_close_on_server_cancelled(const grpcsharp_batch_context* ctx) {
...
GPR_EXPORT grpc_completion_queue* GPR_CALLTYPE grpcsharp_completion_queue_create_async(void) {
  return grpc_completion_queue_create_for_next(NULL);
}

GPR_EXPORT grpc_completion_queue* GPR_CALLTYPE grpcsharp_completion_queue_create_sync(void) {
...
GPR_EXPORT grpc_event GPR_CALLTYPE grpcsharp_completion_queue_next(grpc_completion_queue* cq) {
  return grpc_completion_queue_next(cq, gpr_inf_future(GPR_CLOCK_REALTIME), NULL);
}

GPR_EXPORT grpc_event GPR_CALLTYPE grpcsharp_completion_queue_pluck(grpc_completion_queue* cq,
                                                                    void* tag) {
  return grpc_completion_queue_pluck(cq, tag, gpr_inf_future(GPR_CLOCK_REALTIME), NULL);
}
...
GPR_EXPORT grpc_call* GPR_CALLTYPE grpcsharp_channel_create_call(
    ...
    grpc_completion_queue* cq, const char* method, const char* host, gpr_timespec deadline) {
  ...
  grpc_call* ret = grpc_channel_create_call(channel, parent_call, propagation_mask, cq,
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_channel_watch_connectivity_state(
    grpc_channel* channel, grpc_connectivity_state last_observed_state, gpr_timespec deadline,
    grpc_completion_queue* cq, grpcsharp_batch_context* ctx) {
  grpc_channel_watch_connectivity_state(channel, last_observed_state, deadline, cq, ctx);
}
...
GPR_EXPORT grpc_channel_args* GPR_CALLTYPE grpcsharp_channel_args_create(size_t num_args) {
  grpc_channel_args* args = (grpc_channel_args*)gpr_malloc(sizeof(grpc_channel_args));
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_channel_args_set_string(grpc_channel_args* args,
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_channel_args_set_integer(grpc_channel_args* args,
                                                                size_t index, const char* key,
                                                                int value) {
  GPR_ASSERT(args);
  GPR_ASSERT(index < args->num_args);
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_channel_args_destroy(grpc_channel_args* args) {
  size_t i;
  if (args) {
...
GPR_EXPORT gpr_timespec GPR_CALLTYPE gprsharp_convert_clock_type(gpr_timespec t,
                                                                 gpr_clock_type target_clock) {
  return gpr_convert_clock_type(t, target_clock);
}

GPR_EXPORT int32_t GPR_CALLTYPE gprsharp_sizeof_timespec(void) { return sizeof(gpr_timespec); }
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_cancel_with_status(grpc_call* call,
                                                                          grpc_status_code status,
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_call_destroy(grpc_call* call) { grpc_call_unref(call); }

typedef grpc_call_error (*grpcsharp_call_start_batch_func)(grpc_call* call, const grpc_op* ops,
                                                           size_t nops, void* tag, void* reserved);

/* Only for testing */
static grpc_call_error grpcsharp_call_start_batch_nop(grpc_call* call, const grpc_op* ops,
                                                      size_t nops, void* tag, void* reserved) {
  (void)call;
  (void)ops;
  (void)nops;
...
static grpc_call_error grpcsharp_call_start_batch_default(grpc_call* call, const grpc_op* ops,
                                                          size_t nops, void* tag, void* reserved) {
...
static grpc_call_error grpcsharp_call_start_batch(grpc_call* call, const grpc_op* ops, size_t nops,
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_start_unary(
    ...
    uint32_t write_flags, grpc_metadata_array* initial_metadata, uint32_t initial_metadata_flags) {
  /* TODO: don't use magic number */
  grpc_op ops[6];
  memset(ops, 0, sizeof(ops));
  ops[0].op = GRPC_OP_SEND_INITIAL_METADATA;
  grpcsharp_metadata_array_move(&(ctx->send_initial_metadata), initial_metadata);
  ...
  ops[3].data.recv_initial_metadata.recv_initial_metadata = &(ctx->recv_initial_metadata);
  ops[3].flags = 0;
  ops[3].reserved = NULL;
  ...
  ops[5].data.recv_status_on_client.error_string = &(ctx->recv_status_on_client.error_string);
  ops[5].flags = 0;
  ops[5].reserved = NULL;

  return grpcsharp_call_start_batch(call, ops, sizeof(ops) / sizeof(ops[0]), ctx, NULL);
}
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_test_call_start_unary_echo(
  ...
  grpc_byte_buffer* send_message = grpcsharp_create_byte_buffer_from_stolen_slices(send_buffer);
  ...
  grpcsharp_metadata_array_move(&(ctx->recv_status_on_client.trailing_metadata), initial_metadata);
  return GRPC_CALL_OK;
}

GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_start_client_streaming(
    grpc_call* call, grpcsharp_batch_context* ctx, grpc_metadata_array* initial_metadata,
  ...
  ops[1].data.recv_initial_metadata.recv_initial_metadata = &(ctx->recv_initial_metadata);
  ops[1].flags = 0;
  ops[1].reserved = NULL;
  ...
  ops[3].data.recv_status_on_client.error_string = &(ctx->recv_status_on_client.error_string);
  ops[3].flags = 0;
  ops[3].reserved = NULL;
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_start_duplex_streaming(
  ...
  ops[1].data.recv_status_on_client.trailing_metadata =
      &(ctx->recv_status_on_client.trailing_metadata);
  ops[1].data.recv_status_on_client.status = &(ctx->recv_status_on_client.status);
  ops[1].data.recv_status_on_client.status_details = &(ctx->recv_status_on_client.status_details);
  ops[1].data.recv_status_on_client.error_string = &(ctx->recv_status_on_client.error_string);
  ops[1].flags = 0;
  ops[1].reserved = NULL;
...
grpcsharp_call_recv_initial_metadata(grpc_call* call, grpcsharp_batch_context* ctx) {
  /* TODO: don't use magic number */
  grpc_op ops[1];
  ops[0].op = GRPC_OP_RECV_INITIAL_METADATA;
  ops[0].data.recv_initial_metadata.recv_initial_metadata = &(ctx->recv_initial_metadata);
  ops[0].flags = 0;
  ops[0].reserved = NULL;
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_send_message(
    grpc_call* call, grpcsharp_batch_context* ctx, grpc_slice_buffer* send_buffer,
    uint32_t write_flags, int32_t send_empty_initial_metadata) {
  /* TODO: don't use magic number */
  grpc_op ops[2];
  memset(ops, 0, sizeof(ops));
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE
grpcsharp_call_send_close_from_client(grpc_call* call, grpcsharp_batch_context* ctx) {
  /* TODO: don't use magic number */
  grpc_op ops[1];
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_call_send_status_from_server(
    ...
    const char* status_details, size_t status_details_len, grpc_metadata_array* trailing_metadata,
    int32_t send_empty_initial_metadata, grpc_slice_buffer* optional_send_buffer,
  ...
  if (optional_send_buffer) {
    ...
    ctx->send_message = grpcsharp_create_byte_buffer_from_stolen_slices(optional_send_buffer);
...
grpcsharp_call_start_serverside(grpc_call* call, grpcsharp_batch_context* ctx) {
  ...
  ops[0].data.recv_close_on_server.cancelled = (&ctx->recv_close_on_server_cancelled);
  ops[0].flags = 0;
  ops[0].reserved = NULL;
...
GPR_EXPORT grpc_server* GPR_CALLTYPE grpcsharp_server_create(const grpc_channel_args* args) {
  return grpc_server_create(args, NULL);
}

GPR_EXPORT void GPR_CALLTYPE grpcsharp_server_register_completion_queue(grpc_server* server,
                                                                        grpc_completion_queue* cq) {
  grpc_server_register_completion_queue(server, cq, NULL);
}

GPR_EXPORT int32_t GPR_CALLTYPE grpcsharp_server_add_insecure_http2_port(grpc_server* server,
                                                                         const char* addr) {
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_server_shutdown_and_notify_callback(
    grpc_server* server, grpc_completion_queue* cq, grpcsharp_batch_context* ctx) {
...
GPR_EXPORT grpc_call_error GPR_CALLTYPE grpcsharp_server_request_call(
    grpc_server* server, grpc_completion_queue* cq, grpcsharp_request_call_context* ctx) {
...
typedef int(GPR_CALLTYPE* grpcsharp_native_callback_dispatcher_func)(void* tag, void* arg0,
                                                                     void* arg1, void* arg2,
                                                                     void* arg3, void* arg4,
                                                                     void* arg5);
...
GPR_EXPORT void GPR_CALLTYPE
grpcsharp_native_callback_dispatcher_init(grpcsharp_native_callback_dispatcher_func func) {
  GPR_ASSERT(func);
...
static grpc_ssl_roots_override_result override_ssl_roots_handler(char** pem_root_certs) {
...
static int grpcsharp_verify_peer_handler(const char* target_name, const char* peer_pem,
                                         void* userdata) {
  return native_callback_dispatcher(userdata, (void*)target_name, (void*)peer_pem, (void*)0, NULL,
                                    NULL, NULL);
}

GPR_EXPORT grpc_channel_credentials* GPR_CALLTYPE grpcsharp_ssl_credentials_create(
    const char* pem_root_certs, const char* key_cert_pair_cert_chain,
    const char* key_cert_pair_private_key, void* verify_peer_callback_tag) {
  ...
  return grpc_ssl_credentials_create(pem_root_certs, key_cert_pair_ptr, verify_options_ptr, NULL);
}
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_call_credentials_release(grpc_call_credentials* creds) {
  grpc_call_credentials_release(creds);
}

GPR_EXPORT grpc_channel* GPR_CALLTYPE grpcsharp_secure_channel_create(
    grpc_channel_credentials* creds, const char* target, const grpc_channel_args* args) {
  return grpc_secure_channel_create(creds, target, args, NULL);
}

GPR_EXPORT grpc_server_credentials* GPR_CALLTYPE grpcsharp_ssl_server_credentials_create(
  ...
  for (i = 0; i < num_key_cert_pairs; i++) {
    if (key_cert_pair_cert_chain_array[i] || key_cert_pair_private_key_array[i]) {
  ...
  creds = grpc_ssl_server_credentials_create_ex(pem_root_certs, key_cert_pairs, num_key_cert_pairs,
                                                client_request_type, NULL);
  gpr_free(key_cert_pairs);
  return creds;
}

GPR_EXPORT void GPR_CALLTYPE grpcsharp_server_credentials_release(grpc_server_credentials* creds) {
...
static int grpcsharp_get_metadata_handler(
  ...
  native_callback_dispatcher(state, (void*)context.service_url, (void*)context.method_name, cb,
                             user_data, (void*)0, NULL);
...
GPR_EXPORT const char* GPR_CALLTYPE
grpcsharp_auth_context_peer_identity_property_name(const grpc_auth_context* ctx) {
...
typedef void(GPR_CALLTYPE* grpcsharp_log_func)(const char* file, int32_t line, uint64_t thd_id,
                                               const char* severity_string, const char* msg);
...
static void grpcsharp_log_handler(gpr_log_func_args* args) {
  log_func(args->file, args->line, gpr_thd_currentid(), gpr_log_severity_string(args->severity),
           args->message);
}
...
GPR_EXPORT void GPR_CALLTYPE grpcsharp_slice_buffer_reset_and_unref(grpc_slice_buffer* buffer) {
  grpc_slice_buffer_reset_and_unref(buffer);
}

GPR_EXPORT void GPR_CALLTYPE grpcsharp_slice_buffer_destroy(grpc_slice_buffer* buffer) {
  grpc_slice_buffer_destroy(buffer);
  gpr_free(buffer);
}

GPR_EXPORT size_t GPR_CALLTYPE grpcsharp_slice_buffer_slice_count(grpc_slice_buffer* buffer) {
  return buffer->count;
}

GPR_EXPORT void GPR_CALLTYPE grpcsharp_slice_buffer_slice_peek(grpc_slice_buffer* buffer,
                                                               size_t index, size_t* slice_len,
...
GPR_EXPORT void* GPR_CALLTYPE grpcsharp_slice_buffer_adjust_tail_space(
    grpc_slice_buffer* buffer, size_t available_tail_space, size_t requested_tail_space) {
...
/* Version info */
GPR_EXPORT const char* GPR_CALLTYPE grpcsharp_version_string() { return grpc_version_string(); }

/* For testing */
GPR_EXPORT void GPR_CALLTYPE grpcsharp_test_callback(test_callback_funcptr callback) {
  callback(1);
}
...
/* For testing */
GPR_EXPORT int32_t GPR_CALLTYPE grpcsharp_sizeof_grpc_event(void) { return sizeof(grpc_event); }
