...
inline std::string ClassName(
    const grpc::protobuf::Descriptor* descriptor, bool qualified) {
  ...
  std::string inner_name =
      descriptor->full_name().substr(outer_name.size());

  if (qualified) {
    return "::" + DotsToColons(outer_name) +
           DotsToUnderscores(inner_name);
  } else {
...
inline std::string GetCppComments(const DescriptorType* desc,
                                  bool leading) {
