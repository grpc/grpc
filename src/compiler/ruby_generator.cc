...
// The following functions are copied directly from the source for the
// protoc ruby generator to ensure compatibility (with the exception of
// int and string type changes). See
// https://github.com/google/protobuf/blob/master/src/google/protobuf/compiler/ruby/ruby_generator.cc#L250
// TODO: keep up to date with protoc code generation, though this
// behavior isn't expected to change
bool IsLower(char ch) { return ch >= 'a' && ch <= 'z'; }

char ToUpper(char ch) { return IsLower(ch) ? (ch - 'a' + 'A') : ch; }

// Package names in protobuf are snake_case by convention, but Ruby
// module names must be PascalCased.
//
...
std::string GetServices(const FileDescriptor* file) {
  std::string output;
  {
    ...
    // Don't write out any output if there no services, to avoid empty
    // service files being generated for proto files that don't declare
    // any.
    ...
    std::map<std::string, std::string> header_comment_vars =
        ListToDict({
            "file.name",
            file->name(),
            "file.package",
    ...
    // Write out require statemment to import the separately generated
    // file that defines the messages used by the service. This is
    // generated by the main ruby plugin.
