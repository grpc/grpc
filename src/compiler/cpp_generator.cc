...
std::string GetHeaderPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(vars, "\n");
    printer->Print(
        vars, "#include \"$filename_base$$message_header_ext$\"\n");
...
// Convert from "a/b/c.proto" to "#include
// \"a/b/c$message_header_ext$\"\n"
...
std::string GetHeaderIncludes(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    if (!params.additional_header_includes.empty()) {
      PrintIncludes(printer.get(), params.additional_header_includes,
                    false, "");
    }
    ...
    std::vector<std::string> headers(headers_strs,
...
void PrintHeaderClientMethodInterfaces(
  ...
  if (is_public) {
    if (method->NoStreaming()) {
      printer->Print(
          *vars,
          "virtual ::grpc::Status $Method$(::grpc::ClientContext* "
          "context, "
      ...
      printer->Print(*vars,
                     "return std::unique_ptr< "
                     "::grpc::ClientReaderWriterInterface< $Request$, "
                     "$Response$>>("
                     "$Method$Raw(context));\n");
      printer->Outdent();
      printer->Print("}\n");
...
void PrintHeaderClientMethodCallbackInterfacesStart(
    grpc_generator::Printer* printer,
    std::map<std::string, std::string>* /*vars*/) {
  // This declares the interface for the callback-based API. The
  // components are pure; even though this is new (post-1.0) API, it can
  // be pure because it is an entirely new interface that happens to be
  // scoped within StubInterface, not new additions to StubInterface
  // itself
  printer->Print("class async_interface {\n");
  // All methods in this new interface are public. There is no need for
  // private "Raw" methods since the callback-based API returns unowned
  // raw pointers
...
void PrintHeaderClientMethodCallbackInterfacesEnd(
  ...
  printer->Outdent();
  printer->Print("};\n");
  // TODO: Remove typedef when all uses of experimental_async are
  // migrated off.
  printer->Print(
      "typedef class async_interface experimental_async_interface;\n");

  // Declare a function to give the async stub contents. It can't be
  // pure since this is a new API in StubInterface, but it is
  // meaningless by default (since any stub that wants to use it must
  // have its own implementation of the callback functions therein), so
  // make the default return value nullptr. Intentionally include the
  // word "class" to avoid possible shadowing.
  // TODO: Remove experimental_async call when possible, replace with
  // nullptr.
  printer->Print(
...
void PrintHeaderClientMethodData(
    grpc_generator::Printer* printer,
    const grpc_generator::Method* method,
    std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
...
// Helper generator. Disables the sync API for Request and Response,
// then adds in an async API for RealRequest and RealResponse types.
// This is to be used to generate async and raw async APIs.
  ...
  if (method->NoStreaming()) {
    printer->Print(
        *vars,
        ...
        "::grpc::Status $Method$("
        "::grpc::ServerContext* /*context*/, const $Request$* "
        "/*request*/, "
    ...
    printer->Print(*vars,
                   "void Request$Method$("
                   "::grpc::ServerContext* context, "
                   "::grpc::ServerAsyncReader< $RealResponse$, "
                   "$RealRequest$>* reader, "
                   "::grpc::CompletionQueue* new_call_cq, "
    ...
    printer->Print(
        *vars,
        ...
        "context, request, writer, new_call_cq, notification_cq, "
        "tag);\n");
    printer->Print("}\n");
  } else if (method->BidiStreaming()) {
    printer->Print(
    ...
    printer->Print(*vars,
                   "void Request$Method$("
                   "::grpc::ServerContext* context, "
                   "::grpc::ServerAsyncReaderWriter< $RealResponse$, "
                   "$RealRequest$>* "
                   "stream, "
                   "::grpc::CompletionQueue* new_call_cq, "
                   "::grpc::ServerCompletionQueue* notification_cq, "
...
void PrintHeaderServerMethodAsync(
  ...
  printer->Print(
      *vars, "class WithAsyncMethod_$Method$ : public BaseClass {\n");
  printer->Print(
      " private:\n"
...
// Helper generator. Disables the sync API for Request and Response,
// then adds in a callback API for RealRequest and RealResponse types.
// This is to be used to generate callback and raw callback APIs.
  ...
  if (method->NoStreaming()) {
    ...
    printer->Print(
        *vars,
        ...
        "}\n");
    printer->Print(*vars,
                   "virtual ::grpc::ServerBidiReactor< $RealRequest$, "
                   "$RealResponse$>* "
                   "$Method$(\n"
                   "  ::grpc::CallbackServerContext* /*context*/)\n"
                   "  { return nullptr; }\n");
...
void PrintHeaderServerMethodGeneric(
  ...
  printer->Print(*vars, "template <class BaseClass>\n");
  printer->Print(
      *vars, "class WithGenericMethod_$Method$ : public BaseClass {\n");
  printer->Print(
      " private:\n"
      "  void BaseClassMustBeDerivedFromService(const Service* "
      "/*service*/) "
...
void PrintHeaderService(grpc_generator::Printer* printer,
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    printer->Print(
        service->method(i)->GetLeadingComments("//").c_str());
    PrintHeaderClientMethodInterfaces(printer, service->method(i).get(),
                                      vars, true);
    printer->Print(
        service->method(i)->GetTrailingComments("//").c_str());
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    PrintHeaderClientMethodCallback(printer, service->method(i).get(),
                                    vars);
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    PrintHeaderClientMethodData(printer, service->method(i).get(),
                                vars);
  }
  printer->Outdent();
  printer->Print("};\n");
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    PrintHeaderServerMethodSync(printer, service->method(i).get(),
                                vars);
  }
  printer->Outdent();
  printer->Print("};\n");
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    ...
    PrintHeaderServerMethodAsync(printer, service->method(i).get(),
                                 vars);
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["Idx"] = as_string(i);
    PrintHeaderServerMethodCallback(printer, service->method(i).get(),
                                    vars);
  }
  ...
  // TODO: Remove following typedef once all uses of
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["Idx"] = as_string(i);
    PrintHeaderServerMethodGeneric(printer, service->method(i).get(),
                                   vars);
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["Idx"] = as_string(i);
    PrintHeaderServerMethodRawCallback(printer,
                                       service->method(i).get(), vars);
  }
  ...
  // Server side - typedef for controlled both unary and server-side
  // streaming
  printer->Print("typedef ");
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["method_name"] = service->method(i)->name();
...
std::string GetHeaderServices(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    auto printer = file->CreatePrinter(&output);
    std::map<std::string, std::string> vars;
    // Package string is empty or ends with a dot. It is used to fully
...
std::string GetSourcePrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(
        vars, "#include \"$filename_base$$message_header_ext$\"\n");
    printer->Print(
        vars, "#include \"$filename_base$$service_header_ext$\"\n");
    printer->Print(vars, "\n");
  }
  return output;
}
...
void PrintSourceClientMethod(grpc_generator::Printer* printer,
  ...
  if (method->NoStreaming()) {
    ...
    printer->Print(
        *vars,
        "::grpc::ClientReaderWriter< $Request$, $Response$>* "
        "$ns$$Service$::Stub::$Method$Raw(::grpc::ClientContext* "
        "context) {\n");
    printer->Print(
        *vars,
...
void PrintSourceService(grpc_generator::Printer* printer,
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    ...
    if (method->NoStreaming()) {
      ...
      // NOTE: There is no reason to consider streamed-unary as a
      // separate category here since this part is setting up the
      // client-side stub and this appears as a NORMAL_RPC from the
      // client-side.
...
std::string GetMockIncludes(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    if (params.gmock_search_path.empty()) {
      ...
      PrintIncludes(printer.get(), gmock_header,
                    params.use_system_headers, params.grpc_search_path);
    } else {
...
void PrintMockClientMethods(grpc_generator::Printer* printer,
  ...
  if (method->NoStreaming()) {
    printer->Print(
        *vars,
        "MOCK_METHOD3($Method$, ::grpc::Status(::grpc::ClientContext* "
        "context, "
        "const $Request$& request, $Response$* response));\n");
    for (auto async_prefix : async_prefixes) {
      (*vars)["AsyncPrefix"] = async_prefix.prefix;
