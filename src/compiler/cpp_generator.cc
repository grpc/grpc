...
void PrintIncludes(grpc_generator::Printer* printer, const std::vector<std::string>& headers,
...
std::string ImportInludeFromProtoName(const std::string& proto_name) {
  return std::string("#include \"") + proto_name.substr(0, proto_name.size() - 6) +
...
std::string GetHeaderIncludes(grpc_generator::File* file, const Parameters& params) {
  std::string output;
  {
    ...
    PrintIncludes(printer.get(), headers, params.use_system_headers, params.grpc_search_path);
    printer->Print(vars, "\n");
...
void PrintHeaderClientMethodInterfaces(grpc_generator::Printer* printer,
  ...
  } async_prefixes[] = {{"Async", ", void* tag", ", tag"}, {"PrepareAsync", "", ""}};
...
void PrintHeaderClientMethod(grpc_generator::Printer* printer, const grpc_generator::Method* method,
                             std::map<std::string, std::string>* vars, bool is_public) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
  (*vars)["Response"] = method->output_type_name();
...
void PrintHeaderClientMethodCallbackInterfacesStart(grpc_generator::Printer* printer,
                                                    std::map<std::string, std::string>* /*vars*/) {
...
void PrintHeaderClientMethodCallbackInterfaces(grpc_generator::Printer* printer,
                                               const grpc_generator::Method* method,
                                               std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
...
void PrintHeaderClientMethodCallbackInterfacesEnd(grpc_generator::Printer* printer,
                                                  std::map<std::string, std::string>* /*vars*/) {
  printer->Outdent();
  printer->Print("};\n");
  // TODO: Remove typedef when all uses of experimental_async are migrated off.
  printer->Print("typedef class async_interface experimental_async_interface;\n");
  ...
  printer->Print("virtual class async_interface* async() { return nullptr; }\n");

  // TODO: Remove experimental_async call when possible.
  printer->Print("class async_interface* experimental_async() { return async(); }\n");
}

void PrintHeaderClientMethodCallbackStart(grpc_generator::Printer* printer,
                                          std::map<std::string, std::string>* /*vars*/) {
...
void PrintHeaderClientMethodCallbackEnd(grpc_generator::Printer* printer,
                                        std::map<std::string, std::string>* /*vars*/) {
  printer->Outdent();
...
void PrintHeaderServerAsyncMethodsHelper(grpc_generator::Printer* printer,
                                         const grpc_generator::Method* method,
                                         std::map<std::string, std::string>* vars) {
  if (method->NoStreaming()) {
    printer->Print(*vars,
...
void PrintHeaderServerCallbackMethodsHelper(grpc_generator::Printer* printer,
                                            const grpc_generator::Method* method,
                                            std::map<std::string, std::string>* vars) {
  if (method->NoStreaming()) {
    printer->Print(*vars,
...
void PrintHeaderServerMethodRawCallback(grpc_generator::Printer* printer,
                                        const grpc_generator::Method* method,
                                        std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  // These will be disabled
...
void PrintHeaderServerMethodStreamedUnary(grpc_generator::Printer* printer,
                                          const grpc_generator::Method* method,
                                          std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
...
void PrintHeaderServerMethodSplitStreaming(grpc_generator::Printer* printer,
                                           const grpc_generator::Method* method,
                                           std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
...
void PrintHeaderService(grpc_generator::Printer* printer, const grpc_generator::Service* service,
                        std::map<std::string, std::string>* vars) {
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    printer->Print(service->method(i)->GetLeadingComments("//").c_str());
    PrintHeaderClientMethodCallbackInterfaces(printer, service->method(i).get(), vars);
    printer->Print(service->method(i)->GetTrailingComments("//").c_str());
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["Idx"] = as_string(i);
    PrintHeaderServerMethodStreamedUnary(printer, service->method(i).get(), vars);
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    (*vars)["Idx"] = as_string(i);
    PrintHeaderServerMethodSplitStreaming(printer, service->method(i).get(), vars);
  }
  ...
  for (int i = 0; i < service->method_count(); ++i) {
    auto method = service->method(i);
    if (service->method(i)->NoStreaming() || ServerOnlyStreaming(method.get())) {
      printer->Print(" >");
    }
  }
...
std::string GetSourceIncludes(grpc_generator::File* file, const Parameters& params) {
  std::string output;
  {
    ...
    auto printer = file->CreatePrinter(&output);
    std::map<std::string, std::string> vars;
    static const char* headers_strs[] = {"functional",
                                         "grpcpp/impl/codegen/async_stream.h",
                                         "grpcpp/impl/codegen/async_unary_call.h",
...
void PrintSourceClientMethod(grpc_generator::Printer* printer, const grpc_generator::Method* method,
                             std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
  (*vars)["Response"] = method->output_type_name();
...
void PrintSourceServerMethod(grpc_generator::Printer* printer, const grpc_generator::Method* method,
                             std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
  (*vars)["Response"] = method->output_type_name();
...
void PrintSourceService(grpc_generator::Printer* printer, const grpc_generator::Service* service,
                        std::map<std::string, std::string>* vars) {
  (*vars)["Service"] = service->name();
...
std::string GetMockIncludes(grpc_generator::File* file, const Parameters& params) {
  std::string output;
  {
    ...
    if (params.gmock_search_path.empty()) {
      ...
      PrintIncludes(printer.get(), gmock_header, false, params.gmock_search_path);
    }
...
void PrintMockClientMethods(grpc_generator::Printer* printer, const grpc_generator::Method* method,
                            std::map<std::string, std::string>* vars) {
  (*vars)["Method"] = method->name();
  (*vars)["Request"] = method->input_type_name();
  (*vars)["Response"] = method->output_type_name();
  ...
  if (method->NoStreaming()) {
    ...
    for (auto async_prefix : async_prefixes) {
      (*vars)["AsyncPrefix"] = async_prefix.prefix;
      (*vars)["AsyncMethodParams"] = async_prefix.method_params;
      (*vars)["MockArgs"] = std::to_string(3 + async_prefix.extra_method_param_count);
      printer->Print(*vars,
    ...
    for (auto async_prefix : async_prefixes) {
      (*vars)["AsyncPrefix"] = async_prefix.prefix;
      (*vars)["AsyncMethodParams"] = async_prefix.method_params;
      (*vars)["MockArgs"] = std::to_string(2 + async_prefix.extra_method_param_count);
      printer->Print(*vars,
                     "MOCK_METHOD$MockArgs$($AsyncPrefix$$Method$Raw, "
...
void PrintMockService(grpc_generator::Printer* printer, const grpc_generator::Service* service,
                      std::map<std::string, std::string>* vars) {
  (*vars)["Service"] = service->name();
