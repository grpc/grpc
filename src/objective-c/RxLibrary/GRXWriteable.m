...
+ (instancetype)writeableWithSingleHandler:(GRXSingleHandler)handler {
  ...
  // We nilify this variable when the block is invoked, so that handler is only
  // invoked once even if the writer tries to write multiple values.
  __block GRXEventHandler eventHandler = ^(BOOL done, id value, NSError *error) {
    // Nillify eventHandler before invoking handler, in case the latter
    // causes the former to be executed recursively. Because blocks can be
    // deallocated even during execution, we have to first retain handler
    // locally to guarantee it's valid.
    // TODO(jcanizales): Just turn this craziness into a simple subclass of
    // GRXWriteable.
    ...
    if (value) {
      ...
      // Even though RxLibrary is independent of gRPC, the domain and code
      // here are, for the moment, set to the values of kGRPCErrorDomain and
      // GRPCErrorCodeInternal. This way, the error formed is the one user
      // of gRPC would expect if the server failed to produce a response.
      //
      // TODO(jcanizales): Figure out a way to keep errors of RxLibrary
      // generic without making users of gRPC take care of two different
      // error domains and error code enums. A possibility is to add error
      // handling to GRXWriters or GRXWriteables, and use them to translate
      // errors between the two domains.
