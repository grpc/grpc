...
@implementation GRXNSFastEnumerator {
  ...
  NSUInteger _index;
  // A "buffer of one element," for the containers that enumerate their elements
  // one by one. Those will set _state.itemsPtr to point to this. The
  // NSFastEnumeration protocol requires it to be __unsafe_unretained, but
  // that's alright because the only use we'd make of its value is to return it
  // immediately as the result of nextObject.
  __unsafe_unretained id _bufferValue;
  // Neither NSEnumerator nor NSFastEnumeration instances are required to work
  // correctly when the underlying container is mutated during iteration. The
  // expectation is that an exception is thrown when that happens. So we check
  // for mutations.
