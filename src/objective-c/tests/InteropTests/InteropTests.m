...
- (void)testEmptyUnaryRPCTest {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiter) {
    ...
    waiter(self, @[ expectation ], TEST_TIMEOUT);
  });
}
    ...
    GRPCUnaryProtoCall *call = [service
        emptyCallWithMessage:request
             responseHandler:[[InteropTestsBlockCallbacks alloc] initWithInitialMetadataCallback:nil
                 ...
                 callOptions:options];
    [call start];
    waiterBlock(self, @[ expectReceive, expectComplete ], TEST_TIMEOUT);
  });
}
...
- (void)testAsyncDispatchWithV2API {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    waiterBlock(self, @[ receiveExpect, closeExpect ], TEST_TIMEOUT);
...
- (void)testLargeUnaryRPC {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    waiterBlock(self, @[ expectation ], TEST_TIMEOUT);
  });
}
...
- (void)testUnaryResponseHandler {
  ...
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    waiterBlock(self, @[ expectComplete, expectCompleteMainQueue ], TEST_TIMEOUT);
  });
}
...
- (void)testConcurrentRPCsWithErrorsWithV2API {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter _Nonnull waiterBlock) {
    ...
    waiterBlock(self, completeExpectations, TEST_TIMEOUT);
  });
}
...
- (void)testCancelAfterFirstResponseRPCWithV2API {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    [call start];
    [call writeMessage:request];
    waiterBlock(self, @[ completionExpectation, responseExpectation ], TEST_TIMEOUT);
  });
}
...
- (void)testCancelAfterFirstRequestWithV2API {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    [call writeMessage:request];
    [call cancel];
    waiterBlock(self, @[ completionExpectation ], TEST_TIMEOUT);
  });
}
...
- (void)testHijackingInterceptor {
  GRPCTestRunWithFlakeRepeats(^(GRPCTestWaiter waiterBlock) {
    ...
    waiterBlock(self, @[ expectUserCallComplete ], TEST_TIMEOUT);
