 ...
 * Caller can obtain a mutable copy of type \b GRPCMutableCallOptions by calling
 * [option mutableCopy]
 */
 ...
 *
 * Note: This property does not have effect on Cronet transport and will be
 * ignored. Note: This property cannot be used to validate a self-signed server
 * certificate. It control the :authority header field of the call and performs
 * an extra check that server's certificate matches the :authority header.
 */
@property(nonatomic, copy, readonly, nullable) NSString *serverAuthority;

/**
 * The timeout for the RPC call in seconds. If set to 0, the call will not
 * timeout. If set to positive, the gRPC call returns with status
 * GRPCErrorCodeDeadlineExceeded if it is not completed within \a timeout
 * seconds. A negative value is not allowed.
 */
@property(nonatomic, readonly) NSTimeInterval timeout;

/**
 * Enable flow control of a gRPC call. The option is default to NO. If set to
 * YES, writeData: method should only be called at most once before a
 * didWriteData callback is issued, and receiveNextMessage: must be called each
 * time before gRPC call issues a didReceiveMessage callback.
 */
@property(nonatomic, readonly) BOOL flowControlEnabled;

/**
 * An array of interceptor factories. When a call starts, interceptors are
 * created by these factories and chained together with the same order as the
 * factories in this array. This parameter should not be modified by any
 * interceptor and will not take effect if done so.
 */
@property(nonatomic, copy, readonly) NSArray<id<GRPCInterceptorFactory>> *interceptorFactories;

// OAuth2 parameters. Users of gRPC may specify one of the following two
// parameters.

/**
 * The OAuth2 access token string. The string is prefixed with "Bearer " then
 * used as value of the request's "authorization" header field. This parameter
 * should not be used simultaneously with \a authTokenProvider.
 */
@property(nonatomic, copy, readonly, nullable) NSString *oauth2AccessToken;

/**
 * The interface to get the OAuth2 access token string. gRPC will attempt to
 * acquire token when initiating the call. This parameter should not be used
 * simultaneously with \a oauth2AccessToken.
 */
 ...
 * Dictionary key is of type NSString, value should be either NSString or NSData
 * containing binary bytes data.
 */
...
/**
 * Custom string that is prefixed to a request's user-agent header field before
 * gRPC's internal user-agent string.
 */
@property(nonatomic, copy, readonly, nullable) NSString *userAgentPrefix;

/**
 * Custom string that is suffixed to a request's user-agent header field after
 * gRPC's internal user-agent string.
 */
@property(nonatomic, copy, readonly, nullable) NSString *userAgentSuffix;

/**
 * The size limit for the response received from server. If it is exceeded, an
 * error with status code GRPCErrorCodeResourceExhausted is returned.
 */
@property(nonatomic, readonly) NSUInteger responseSizeLimit;

/**
 * The compression algorithm to be used by the gRPC call. For more details refer
 * to https://github.com/grpc/grpc/blob/master/doc/compression.md
 */
@property(nonatomic, readonly) GRPCCompressionAlgorithm compressionAlgorithm;

/**
 * Enable/Disable gRPC call's retry feature. The default is enabled. For details
 * of this feature refer to
...
// HTTP/2 keep-alive feature. The parameter \a keepaliveInterval specifies the
// interval between two PING frames. The parameter \a keepaliveTimeout specifies
// the length of the period for which the call should wait for PING ACK. If PING
// ACK is not received after this period, the call fails. Negative values are
// not allowed.
...
/**
 * Specify channel args to be used for this call. For a list of channel args
 * available, see grpc/grpc_types.h
 */
...
/**
 * PEM format root certifications that is trusted. If set to nil, gRPC uses a
 * list of default root certificates.
 */
...
/**
 * PEM format certificate chain for client authentication, if required by the
 * server.
 */
...
/**
 * A string that specify the domain where channel is being cached. Channels with
 * different domains will not get cached to the same connection.
 */
@property(nonatomic, copy, readonly, nullable) NSString *channelPoolDomain;

/**
 * Channel id allows control of channel caching within a channelPoolDomain. A
 * call with a unique channelID will create a new channel (connection) instead
 * of reusing an existing one. Multiple calls in the same channelPoolDomain
 * using identical channelID are allowed to share connection if other channel
 ...
 * Caller can obtain an immutable copy of type \b GRPCCallOptions by calling
 * [option copy]
 */
 ...
 * timeout. If set to positive, the gRPC call returns with status
 * GRPCErrorCodeDeadlineExceeded if it is not completed within \a timeout
 * seconds. Negative value is invalid; setting the parameter to negative value
 ...
 * YES, writeData: method should only be called at most once before a
 * didWriteData callback is issued, and receiveNextMessage: must be called each
 * time before gRPC call can issue a didReceiveMessage callback.
 *
 * If writeData: method is called more than once before issuance of a
 * didWriteData callback, gRPC will continue to queue the message and write them
 * to gRPC core in order. However, the user assumes their own responsibility of
 * flow control by keeping tracking of the pending writes in the call.
 */
...
/**
 * Maximum interval in seconds between two consecutive retries. Pass 0 to use
 * default. Internal-only property used for GTMSessionFetcher transport retry
 * policy.
 */
@property(nonatomic, readwrite) NSTimeInterval maxRetryInterval;

/**
 * Minimum interval in seconds between two consecutive retries. Pass 0 to use
 * default. Internal-only property used for GTMSessionFetcher transport retry
 * policy.
 */
...
/**
 * Multiplier used to increase the interval between retries. Pass 0 to use
 * default. Internal-only property used for GTMSessionFetcher transport retry
 * policy.
 */
...
// the length of the period for which the call should wait for PING ACK. If PING
// ACK is not received after this period, the call fails. Negative values are
// invalid; setting these parameters to negative value will reset the
...
// Parameters for connection backoff. Negative value is invalid; setting the
// parameters to negative value will reset corresponding parameter to 0. For
// details of gRPC's backoff behavior, refer to
// https://github.com/grpc/grpc/blob/master/doc/connection-backoff.md
...
/**
 * A string that specify the domain where channel is being cached. Channels with
 * different domains will not get cached to the same channel. For example, a
 * gRPC example app may use the channel pool domain 'io.grpc.example' so that
 * its calls do not reuse the channel created by other modules in the same
 * process.
 */
@property(nonatomic, copy, readwrite, nullable) NSString *channelPoolDomain;

/**
 * Channel id allows a call to force creating a new channel (connection) rather
 * than using a cached channel. Calls using distinct channelID's will not get
 * cached to the same channel.
 */
