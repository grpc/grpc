...
class DiscardOutput final : public Output {
 public:
  void Mark(absl::string_view, int64_t, int64_t, absl::string_view) override {}
...
class JsonOutput final : public Output {
 public:
  ...
  void Mark(absl::string_view name, int64_t tid, int64_t timestamp,
            absl::string_view json_args) override;
...
struct Bin : public MultiProducerSingleConsumerQueue::Node {
  ...
  static constexpr size_t kEventsPerBin = kMaxBinSize / sizeof(Event) - 1;
  size_t used_space = 0;
  gpr_thd_id thd_id = gpr_thd_currentid();
  alignas(Event) uint8_t events[sizeof(Event) * kEventsPerBin];
};
