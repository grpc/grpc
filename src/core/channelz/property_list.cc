...
size_t PropertyGrid::GetIndex(std::vector<absl::string_view>& vec,
...
Json::Object PropertyGrid::TakeJsonObject() {
  ...
  for (size_t r = 0; r < rows_.size(); ++r) {
    ...
    for (size_t c = 0; c < columns_.size(); ++c) {
      auto it = grid_.find(std::make_pair(c, r));
      if (it != grid_.end()) {
...
void PropertyGrid::SetInternal(absl::string_view column, absl::string_view row,
  ...
  if (value.has_value()) {
    grid_[std::make_pair(c, r)] = *std::move(value);
  } else {
    grid_.erase(std::make_pair(c, r));
  }
}
...
PropertyGrid& PropertyGrid::SetColumn(absl::string_view column,
  ...
  for (auto& [key, value] : values.TakeJsonObject()) {
    grid_[std::make_pair(c, GetIndex(rows_, key))] = std::move(value);
  }
  return *this;
}
...
PropertyGrid& PropertyGrid::SetRow(absl::string_view row, PropertyList values) {
  int r = GetIndex(rows_, row);
  for (auto& [key, value] : values.TakeJsonObject()) {
    grid_[std::make_pair(GetIndex(columns_, key), r)] = std::move(value);
  }
  return *this;
}
