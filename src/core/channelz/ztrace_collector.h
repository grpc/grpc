...
void AppendResults(const Collection<T>& data,
  ...
  for (const auto& value : data) {
    ...
    const gpr_timespec gpr_ts = gpr_cycle_counter_to_time(value.first);
  ...
  void Append(X producer_or_value) {
    GRPC_TRACE_LOG(ztrace, INFO) << "ZTRACE[" << this << "]: " << [&]() {
      ...
      return TextEncode(reinterpret_cast<upb_Message*>(any),
                        google_protobuf_Any_getmsgdef(nullptr));
    }();
  ...
  class Instance : public RefCounted<Instance> {
   ...
   private:
    ...
    void QueueCallbackReady() {
      event_engine_->Run(
          [data = std::move(data_),
           items_matched = std::exchange(items_matched_, 0),
  ...
  class ZTraceImpl final : public ZTrace {
   public:
    ...
    void Run(ZTrace::Args args,
      ...
      impl->instances.insert(instance_);
      NextCallback(std::make_shared<Callback>(std::move(callback)), impl,
                   instance_);
    }
