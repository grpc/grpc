...
static tsi_result
tsi_fake_handshake_message_from_string (const char *msg_string,
...
static void
store32_little_endian (uint32_t value, unsigned char *buf)
{
...
static uint32_t
read_frame_size (const grpc_slice_buffer * sb)
{
...
static tsi_result
tsi_fake_frame_decode (const unsigned char *incoming_bytes,
...
static tsi_result
tsi_fake_frame_encode (unsigned char *outgoing_bytes,
		       size_t * outgoing_bytes_size, tsi_fake_frame * frame)
{
...
static tsi_result
tsi_fake_frame_set_data (unsigned char *data, size_t data_size,
...
static tsi_result
fake_protector_protect (tsi_frame_protector * self,
  ...
  if (frame->size == 0)
    {
      ...
      result =
	tsi_fake_frame_decode (frame_header, &written_in_frame_size, frame);
      if (result != TSI_INCOMPLETE_DATA)
	{
...
static tsi_result
fake_protector_protect_flush (tsi_frame_protector * self,
			      unsigned char *protected_output_frames,
			      size_t * protected_output_frames_size,
  ...
  if (!frame->needs_draining)
    {
      ...
      store32_little_endian ((uint32_t) frame->size, frame->data);	/* Overwrite header. */
    }
...
static tsi_result
fake_protector_unprotect (tsi_frame_protector * self,
...
static tsi_result
fake_zero_copy_grpc_protector_protect (tsi_zero_copy_grpc_protector * self,
...
static tsi_result
fake_zero_copy_grpc_protector_unprotect (tsi_zero_copy_grpc_protector * self,
					 grpc_slice_buffer * protected_slices,
					 grpc_slice_buffer *
					 unprotected_slices)
{
  ...
  while (impl->protected_sb.length >= TSI_FAKE_FRAME_HEADER_SIZE)
      ...
      if (impl->parsed_frame_size == 0)
	{
      ...
      if (impl->protected_sb.length < impl->parsed_frame_size)
	break;
      /* Strips header bytes. */
...
static void
fake_zero_copy_grpc_protector_destroy (tsi_zero_copy_grpc_protector * self)
{
  if (self == nullptr)
    return;
  tsi_fake_zero_copy_grpc_protector *impl =
...
static tsi_result
fake_handshaker_result_extract_peer (const tsi_handshaker_result * self,
				     tsi_peer * peer)
{
...
static tsi_result
fake_handshaker_result_create_zero_copy_grpc_protector (const
...
static tsi_result
fake_handshaker_result_create_frame_protector (const tsi_handshaker_result *
					       self,
					       size_t *
					       max_output_protected_frame_size,
					       tsi_frame_protector **
...
static tsi_result
fake_handshaker_result_get_unused_bytes (const tsi_handshaker_result * self,
...
static void
fake_handshaker_result_destroy (tsi_handshaker_result * self)
{
  fake_handshaker_result *result = (fake_handshaker_result *) self;
...
static tsi_result
fake_handshaker_result_create (const unsigned char *unused_bytes,
			       size_t unused_bytes_size,
...
static tsi_result
fake_handshaker_get_bytes_to_send_to_peer (tsi_handshaker * self,
					   unsigned char *bytes,
					   size_t * bytes_size)
{
  ...
  if (!impl->outgoing_frame.needs_draining)
      ...
      if (tsi_tracing_enabled.enabled ())
	{
	  gpr_log (GPR_INFO, "%s prepared %s.",
		   ...
		   tsi_fake_handshake_message_to_string (impl->
...
static tsi_result
fake_handshaker_process_bytes_from_peer (tsi_handshaker * self,
					 const unsigned char *bytes,
					 size_t * bytes_size)
{
  tsi_result result = TSI_OK;
  ...
  result = tsi_fake_handshake_message_from_string ((const char *) impl->
						   incoming_frame.data +
						   TSI_FAKE_FRAME_HEADER_SIZE,
						   &received_msg);
  if (result != TSI_OK)
...
static tsi_result
fake_handshaker_next (tsi_handshaker * self,
		      ...
		      tsi_handshaker_on_next_done_cb cb, void *user_data)
{
  ...
  do
	...
	handshaker->outgoing_bytes_buffer_size - offset;
      result =
	fake_handshaker_get_bytes_to_send_to_peer (self,
						   handshaker->
...
tsi_create_fake_handshaker (int is_client)
{
  tsi_fake_handshaker *impl =
    (tsi_fake_handshaker *) gpr_zalloc (sizeof (*impl));
...
tsi_frame_protector *
tsi_create_fake_frame_protector (size_t * max_protected_frame_size)
{
  ...
  impl->max_frame_size = (max_protected_frame_size == nullptr)
    ? TSI_FAKE_DEFAULT_FRAME_SIZE : *max_protected_frame_size;
  impl->base.vtable = &frame_protector_vtable;
  return &impl->base;
}

tsi_zero_copy_grpc_protector *
tsi_create_fake_zero_copy_grpc_protector (size_t * max_protected_frame_size)
{
  tsi_fake_zero_copy_grpc_protector *impl =
