...
static void tsi_fake_frame_ensure_size(tsi_fake_frame* frame) {
  if (frame->data == nullptr) {
    ...
    frame->data =
        reinterpret_cast<unsigned char*>(gpr_malloc(frame->allocated_size));
  } else if (frame->size > frame->allocated_size) {
    unsigned char* new_data =
        reinterpret_cast<unsigned char*>(gpr_realloc(frame->data, frame->size));
    frame->data = new_data;
    frame->allocated_size = frame->size;
  }
}
...
static tsi_result fake_handshaker_result_create(
  ...
  fake_handshaker_result* result =
      reinterpret_cast<fake_handshaker_result*>(gpr_zalloc(sizeof(*result)));
  result->base.vtable = &handshaker_result_vtable;
  if (unused_bytes_size > 0) {
    result->unused_bytes =
        reinterpret_cast<unsigned char*>(gpr_malloc(unused_bytes_size));
...
static tsi_result fake_handshaker_next(
  ...
  do {
    ...
    if (result == TSI_INCOMPLETE_DATA) {
      ...
      handshaker->outgoing_bytes_buffer = reinterpret_cast<unsigned char*>(
...
tsi_handshaker* tsi_create_fake_handshaker(int is_client) {
  tsi_fake_handshaker* impl =
      reinterpret_cast<tsi_fake_handshaker*>(gpr_zalloc(sizeof(*impl)));
  impl->base.vtable = &handshaker_vtable;
  impl->is_client = is_client;
  impl->result = TSI_HANDSHAKE_IN_PROGRESS;
  impl->outgoing_bytes_buffer_size =
      TSI_FAKE_HANDSHAKER_OUTGOING_BUFFER_INITIAL_SIZE;
  impl->outgoing_bytes_buffer = reinterpret_cast<unsigned char*>(
...
tsi_frame_protector* tsi_create_fake_frame_protector(
  ...
  tsi_fake_frame_protector* impl =
      reinterpret_cast<tsi_fake_frame_protector*>(gpr_zalloc(sizeof(*impl)));
...
tsi_zero_copy_grpc_protector* tsi_create_fake_zero_copy_grpc_protector(
    size_t* max_protected_frame_size) {
  tsi_fake_zero_copy_grpc_protector* impl =
      reinterpret_cast<tsi_fake_zero_copy_grpc_protector*>(
