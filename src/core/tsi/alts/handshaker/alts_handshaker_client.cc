...
static void alts_grpc_handshaker_client_unref(alts_grpc_handshaker_client* client) {
  if (gpr_unref(&client->refs)) {
    if (client->base.vtable != nullptr && client->base.vtable->destruct != nullptr) {
...
static void maybe_complete_tsi_next(alts_grpc_handshaker_client* client,
  ...
  recv_message_result* r;
  {
    ...
    const bool have_final_result = client->pending_recv_message_result->result != nullptr ||
  ...
  client->cb(r->status, client->user_data, r->bytes_to_send, r->bytes_to_send_size, r->result);
  gpr_free(r);
}

static void handle_response_done(alts_grpc_handshaker_client* client, tsi_result status,
                                 const unsigned char* bytes_to_send, size_t bytes_to_send_size,
                                 tsi_handshaker_result* result) {
  recv_message_result* p = static_cast<recv_message_result*>(gpr_zalloc(sizeof(*p)));
  p->status = status;
...
void alts_handshaker_client_handle_response(alts_handshaker_client* c, bool is_ok) {
  ...
  if (client->cb == nullptr) {
    gpr_log(GPR_ERROR, "client->cb is nullptr in alts_tsi_handshaker_handle_response()");
    return;
  }
  if (handshaker == nullptr) {
    gpr_log(GPR_ERROR, "handshaker is nullptr in alts_tsi_handshaker_handle_response()");
  ...
  if (recv_buffer == nullptr) {
    gpr_log(GPR_ERROR, "recv_buffer is nullptr in alts_tsi_handshaker_handle_response()");
    handle_response_done(client, TSI_INTERNAL_ERROR, nullptr, 0, nullptr);
    return;
  }
  upb::Arena arena;
  grpc_gcp_HandshakerResp* resp = alts_tsi_utils_deserialize_response(recv_buffer, arena.ptr());
  grpc_byte_buffer_destroy(client->recv_buffer);
  client->recv_buffer = nullptr;
  ...
  const grpc_gcp_HandshakerStatus* resp_status = grpc_gcp_HandshakerResp_status(resp);
  ...
  if (out_frames.size > 0) {
    ...
    while (bytes_to_send_size > client->buffer_size) {
      ...
      client->buffer =
          static_cast<unsigned char*>(gpr_realloc(client->buffer, client->buffer_size));
    }
  ...
  grpc_status_code code =
      static_cast<grpc_status_code>(grpc_gcp_HandshakerStatus_code(resp_status));
  ...
  // a performance issue.
  handle_response_done(client, alts_tsi_utils_convert_to_tsi_result(code), bytes_to_send,
...
static tsi_result continue_make_grpc_call(alts_grpc_handshaker_client* client, bool is_start) {
  ...
  if (is_start) {
    ...
    op->data.recv_status_on_client.status_details = &client->handshake_status_details;
    op->flags = 0;
    op->reserved = nullptr;
    op++;
    GPR_ASSERT(op - ops <= kHandshakerClientOpNum);
    gpr_ref(&client->refs);
    grpc_call_error call_error = client->grpc_caller(
        client->call, ops, static_cast<size_t>(op - ops), &client->on_status_received);
    ...
    op->data.recv_initial_metadata.recv_initial_metadata = &client->recv_initial_metadata;
    op++;
    GPR_ASSERT(op - ops <= kHandshakerClientOpNum);
  }
...
void DoHandshakeQueuesInit(void) {
  ...
  g_client_handshake_queue = new HandshakeQueue(per_queue_max_outstanding_handshakes);
  g_server_handshake_queue = new HandshakeQueue(per_queue_max_outstanding_handshakes);
}
...
static void on_status_received(void* arg, grpc_error_handle error) {
  alts_grpc_handshaker_client* client = static_cast<alts_grpc_handshaker_client*>(arg);
...
static grpc_byte_buffer* get_serialized_start_client(alts_handshaker_client* c) {
  ...
  grpc_gcp_StartClientHandshakeReq_set_handshake_security_protocol(start_client, grpc_gcp_ALTS);
  ...
  grpc_gcp_RpcProtocolVersions* client_version =
      grpc_gcp_StartClientHandshakeReq_mutable_rpc_versions(start_client, arena.ptr());
  ...
  grpc_gcp_StartClientHandshakeReq_set_target_name(
      start_client,
      upb_strview_make(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(client->target_name)),
  ...
  while (ptr != nullptr) {
    grpc_gcp_Identity* target_identity =
        grpc_gcp_StartClientHandshakeReq_add_target_identities(start_client, arena.ptr());
    grpc_gcp_Identity_set_service_account(target_identity, upb_strview_makez(ptr->data));
    ptr = ptr->next;
  }
...
static grpc_byte_buffer* get_serialized_start_server(alts_handshaker_client* c,
  ...
  grpc_gcp_ServerHandshakeParameters* value = grpc_gcp_ServerHandshakeParameters_new(arena.ptr());
  grpc_gcp_ServerHandshakeParameters_add_record_protocols(
      value, upb_strview_makez(ALTS_RECORD_PROTOCOL), arena.ptr());
  grpc_gcp_StartServerHandshakeReq_handshake_parameters_set(start_server, grpc_gcp_ALTS, value,
                                                            arena.ptr());
  grpc_gcp_StartServerHandshakeReq_set_in_bytes(
      start_server,
      upb_strview_make(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(*bytes_received)),
                       GRPC_SLICE_LENGTH(*bytes_received)));
  grpc_gcp_RpcProtocolVersions* server_version =
      grpc_gcp_StartServerHandshakeReq_mutable_rpc_versions(start_server, arena.ptr());
...
static grpc_byte_buffer* get_serialized_next(grpc_slice* bytes_received) {
  ...
  upb::Arena arena;
  grpc_gcp_HandshakerReq* req = grpc_gcp_HandshakerReq_new(arena.ptr());
  grpc_gcp_NextHandshakeMessageReq* next = grpc_gcp_HandshakerReq_mutable_next(req, arena.ptr());
  grpc_gcp_NextHandshakeMessageReq_set_in_bytes(
      next, upb_strview_make(reinterpret_cast<const char*> GRPC_SLICE_START_PTR(*bytes_received),
                             GRPC_SLICE_LENGTH(*bytes_received)));
  return get_serialized_handshaker_req(req, arena.ptr());
}
  ...
  if (client->call != nullptr) {
    ...
    if (grpc_core::ExecCtx::Get() == nullptr) {
      ...
      grpc_core::ExecCtx::Run(
          DEBUG_LOCATION,
          GRPC_CLOSURE_CREATE(handshaker_call_unref, client->call, grpc_schedule_on_exec_ctx),
          GRPC_ERROR_NONE);
    }
  }
}
...
static const alts_handshaker_client_vtable vtable = {
    handshaker_client_start_client, handshaker_client_start_server, handshaker_client_next,
    handshaker_client_shutdown, handshaker_client_destruct};
...
alts_handshaker_client* alts_grpc_handshaker_client_create(
  ...
  client->base.vtable = vtable_for_testing == nullptr ? &vtable : vtable_for_testing;
  ...
  client->call = strcmp(handshaker_service_url, ALTS_HANDSHAKER_SERVICE_URL_FOR_TESTING) == 0
                     ? nullptr
...
void alts_handshaker_client_set_grpc_caller_for_testing(alts_handshaker_client* c,
...
void alts_handshaker_client_set_recv_bytes_for_testing(alts_handshaker_client* c,
                                                       grpc_slice* recv_bytes) {
  GPR_ASSERT(c != nullptr);
  alts_grpc_handshaker_client* client = reinterpret_cast<alts_grpc_handshaker_client*>(c);
...
void alts_handshaker_client_set_fields_for_testing(alts_handshaker_client* c,
                                                   alts_tsi_handshaker* handshaker,
                                                   tsi_handshaker_on_next_done_cb cb,
                                                   void* user_data, grpc_byte_buffer* recv_buffer,
...
void alts_handshaker_client_check_fields_for_testing(alts_handshaker_client* c,
                                                     tsi_handshaker_on_next_done_cb cb,
  ...
  GPR_ASSERT(alts_tsi_handshaker_get_has_sent_start_message_for_testing(client->handshaker) ==
             has_sent_start_message);
}

void alts_handshaker_client_set_vtable_for_testing(alts_handshaker_client* c,
...
void alts_handshaker_client_set_cb_for_testing(alts_handshaker_client* c,
                                               tsi_handshaker_on_next_done_cb cb) {
  GPR_ASSERT(c != nullptr);
  alts_grpc_handshaker_client* client = reinterpret_cast<alts_grpc_handshaker_client*>(c);
  client->cb = cb;
...
void alts_handshaker_client_on_status_received_for_testing(alts_handshaker_client* c,
                                                           grpc_status_code status,
...
tsi_result alts_handshaker_client_start_client(alts_handshaker_client* client) {
  if (client != nullptr && client->vtable != nullptr && client->vtable->client_start != nullptr) {
    return client->vtable->client_start(client);
  }
  gpr_log(GPR_ERROR, "client or client->vtable has not been initialized properly");
  return TSI_INVALID_ARGUMENT;
}

tsi_result alts_handshaker_client_start_server(alts_handshaker_client* client,
                                               grpc_slice* bytes_received) {
  if (client != nullptr && client->vtable != nullptr && client->vtable->server_start != nullptr) {
...
tsi_result alts_handshaker_client_next(alts_handshaker_client* client, grpc_slice* bytes_received) {
  if (client != nullptr && client->vtable != nullptr && client->vtable->next != nullptr) {
...
void alts_handshaker_client_shutdown(alts_handshaker_client* client) {
  if (client != nullptr && client->vtable != nullptr && client->vtable->shutdown != nullptr) {
