 ...
 * ALTS handshaker service by scheduling a handshaker request that could
 * be one of client_start, server_start, and next handshaker requests.
 * All APIs in the header are thread-compatible.
 */
...
typedef grpc_call_error (*alts_grpc_caller)(grpc_call* call,
                                            const grpc_op* ops,
                                            size_t nops,
                                            grpc_closure* tag);
...
/**
 * This method schedules a client_start handshaker request to ALTS
 * handshaker service.
 *
...
/**
 * This method schedules a server_start handshaker request to ALTS
 * handshaker service.
 *
 * - client: ALTS handshaker client instance.
 * - bytes_received: bytes in out_frames returned from the peer's
 * handshaker response.
 *
 * It returns TSI_OK on success and an error status code on failure.
 */
tsi_result alts_handshaker_client_start_server(
 ...
 * requests to ALTS handshaker service. After this operation, the
 * handshake will be shutdown, and no more handshaker requests will get
 * scheduled.
 *
 * - client: ALTS handshaker client instance.
 */
 ...
 *
 * - handshaker: ALTS TSI handshaker to which the created handshaker
 * client belongs to.
 ...
 *   testing purpose.
 * - is_client: a boolean value indicating if the created handshaker
 * client is used at the client (is_client = true) or server (is_client
 * = false) side.
 * - max_frame_size: Maximum frame size used by frame protector (User
 * specified maximum frame size if present or default max frame size).
 *
 ...
 * service. Note that the only reason the API is exposed is that it is
 * used in alts_shared_resources.cc.
 *
 * - client: an ALTS handshaker client instance.
 * - is_ok: a boolean value indicating if the handshaker response is ok
 * to read.
 */
void alts_handshaker_client_handle_response(
