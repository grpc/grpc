...
bool grpc_gcp_rpc_protocol_versions_encode(const grpc_gcp_rpc_protocol_versions* versions,
  ...
  upb::Arena arena;
  grpc_gcp_RpcProtocolVersions* versions_msg = grpc_gcp_RpcProtocolVersions_new(arena.ptr());
  grpc_gcp_RpcProtocolVersions_assign_from_struct(versions_msg, arena.ptr(), versions);
  return grpc_gcp_rpc_protocol_versions_encode(versions_msg, arena.ptr(), slice);
}
...
bool grpc_gcp_rpc_protocol_versions_encode(const grpc_gcp_RpcProtocolVersions* versions,
  ...
  size_t buf_length;
  char* buf = grpc_gcp_RpcProtocolVersions_serialize(versions, arena, &buf_length);
  if (buf == nullptr) {
    return false;
  }
...
bool grpc_gcp_rpc_protocol_versions_decode(const grpc_slice& slice,
  ...
  grpc_gcp_RpcProtocolVersions* versions_msg =
      grpc_gcp_RpcProtocolVersions_parse(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(slice)),
...
void grpc_gcp_rpc_protocol_versions_assign_from_upb(grpc_gcp_rpc_protocol_versions* versions,
  ...
  if (max_version_msg != nullptr) {
    versions->max_rpc_version.major = grpc_gcp_RpcProtocolVersions_Version_major(max_version_msg);
    versions->max_rpc_version.minor = grpc_gcp_RpcProtocolVersions_Version_minor(max_version_msg);
  } else {
  ...
  if (min_version_msg != nullptr) {
    versions->min_rpc_version.major = grpc_gcp_RpcProtocolVersions_Version_major(min_version_msg);
    versions->min_rpc_version.minor = grpc_gcp_RpcProtocolVersions_Version_minor(min_version_msg);
  } else {
...
void grpc_gcp_RpcProtocolVersions_assign_from_struct(grpc_gcp_RpcProtocolVersions* versions,
  ...
  grpc_gcp_RpcProtocolVersions_Version_set_major(max_version_msg, value->max_rpc_version.major);
  grpc_gcp_RpcProtocolVersions_Version_set_minor(max_version_msg, value->max_rpc_version.minor);
  grpc_gcp_RpcProtocolVersions_Version* min_version_msg =
      grpc_gcp_RpcProtocolVersions_mutable_min_rpc_version(versions, arena);
  grpc_gcp_RpcProtocolVersions_Version_set_major(min_version_msg, value->min_rpc_version.major);
  grpc_gcp_RpcProtocolVersions_Version_set_minor(min_version_msg, value->min_rpc_version.minor);
}

bool grpc_gcp_rpc_protocol_versions_copy(const grpc_gcp_rpc_protocol_versions* src,
                                         grpc_gcp_rpc_protocol_versions* dst) {
  if ((src == nullptr && dst != nullptr) || (src != nullptr && dst == nullptr)) {
    gpr_log(GPR_ERROR,
...
bool grpc_gcp_rpc_protocol_versions_check(
  ...
  bool result = grpc_core::internal::grpc_gcp_rpc_protocol_version_compare(max_common_version,
