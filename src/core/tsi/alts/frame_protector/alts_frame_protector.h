 ...
 * - is_client: a flag indicating if the frame protector will be used at
 * client (is_client = true) or server (is_client = false) side.
 * - is_rekey: a flag indicating if the frame protector will use an AEAD
 * with rekeying.
 * - max_protected_frame_size: an in/out parameter indicating max frame
 * size to be used by the frame protector. If it is nullptr, the default
 ...
 *
 * This method returns TSI_OK on success and TSI_INTERNAL_ERROR
 * otherwise.
 */
