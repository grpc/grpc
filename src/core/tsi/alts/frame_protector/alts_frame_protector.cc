...
static tsi_result seal(alts_frame_protector* impl) {
  ...
  grpc_status_code status = alts_crypter_process_in_place(
      impl->seal_crypter, impl->in_place_protect_buffer, impl->max_protected_frame_size,
      impl->in_place_protect_bytes_buffered, &output_size, &error_details);
...
static tsi_result alts_protect_flush(tsi_frame_protector* self,
  ...
  if (!alts_write_frame_bytes(impl->writer, protected_output_frames, &written_frame_bytes)) {
...
static tsi_result alts_protect(tsi_frame_protector* self, const unsigned char* unprotected_bytes,
                               size_t* unprotected_bytes_size,
                               unsigned char* protected_output_frames,
                               size_t* protected_output_frames_size) {
  if (self == nullptr || unprotected_bytes == nullptr || unprotected_bytes_size == nullptr ||
      protected_output_frames == nullptr || protected_output_frames_size == nullptr) {
  ...
  if (impl->in_place_protect_bytes_buffered + impl->overhead_length <
    ...
    size_t bytes_to_buffer = GPR_MIN(
    ...
    *unprotected_bytes_size = bytes_to_buffer;
    if (bytes_to_buffer > 0) {
      memcpy(impl->in_place_protect_buffer + impl->in_place_protect_bytes_buffered,
  ...
  if (max_encrypted_payload_bytes(impl) ==
          impl->in_place_protect_bytes_buffered + impl->overhead_length ||
      max_encrypted_payload_bytes(impl) == impl->in_place_protect_bytes_buffered) {
    size_t still_pending_size = 0;
    return alts_protect_flush(self, protected_output_frames, protected_output_frames_size,
                              &still_pending_size);
  } else {
    *protected_output_frames_size = 0;
    return TSI_OK;
...
static tsi_result unseal(alts_frame_protector* impl) {
  ...
  size_t output_size = 0;
  grpc_status_code status = alts_crypter_process_in_place(
      impl->unseal_crypter, impl->in_place_unprotect_buffer, impl->max_unprotected_frame_size,
...
static void ensure_buffer_size(alts_frame_protector* impl) {
  ...
  size_t buffer_space_remaining =
      impl->max_unprotected_frame_size - alts_get_output_bytes_read(impl->reader);
  /**
  ...
  if (buffer_space_remaining < alts_get_reader_bytes_remaining(impl->reader)) {
    size_t buffer_len =
        alts_get_output_bytes_read(impl->reader) + alts_get_reader_bytes_remaining(impl->reader);
    unsigned char* buffer = static_cast<unsigned char*>(gpr_malloc(buffer_len));
    memcpy(buffer, impl->in_place_unprotect_buffer, alts_get_output_bytes_read(impl->reader));
    impl->max_unprotected_frame_size = buffer_len;
    gpr_free(impl->in_place_unprotect_buffer);
    impl->in_place_unprotect_buffer = buffer;
    alts_reset_reader_output_buffer(impl->reader,
                                    buffer + alts_get_output_bytes_read(impl->reader));
  }
}
...
static tsi_result alts_unprotect(tsi_frame_protector* self,
  ...
  if (alts_is_frame_reader_done(impl->reader) &&
    ...
    if (!alts_reset_frame_reader(impl->reader, impl->in_place_unprotect_buffer)) {
  ...
  if (!alts_is_frame_reader_done(impl->reader)) {
    ...
    *protected_frames_bytes_size =
        GPR_MIN(impl->max_unprotected_frame_size - alts_get_output_bytes_read(impl->reader),
                *protected_frames_bytes_size);
    size_t read_frames_bytes_size = *protected_frames_bytes_size;
    if (!alts_read_frame_bytes(impl->reader, protected_frames_bytes, &read_frames_bytes_size)) {
  ...
  if (alts_is_frame_reader_done(impl->reader)) {
    ...
    size_t bytes_to_write =
        GPR_MIN(*unprotected_bytes_size, alts_get_output_bytes_read(impl->reader) -
                                             impl->in_place_unprotect_bytes_processed -
                                             impl->overhead_length);
    if (bytes_to_write > 0) {
      memcpy(unprotected_bytes,
             impl->in_place_unprotect_buffer + impl->in_place_unprotect_bytes_processed,
             bytes_to_write);
    }
...
static grpc_status_code create_alts_crypters(const uint8_t* key, size_t key_size, bool is_client,
                                             bool is_rekey, alts_frame_protector* impl,
                                             char** error_details) {
  grpc_status_code status;
  gsec_aead_crypter* aead_crypter_seal = nullptr;
  gsec_aead_crypter* aead_crypter_unseal = nullptr;
  status = gsec_aes_gcm_aead_crypter_create(key, key_size, kAesGcmNonceLength, kAesGcmTagLength,
  ...
  size_t overflow_size =
      is_rekey ? kAltsRecordProtocolRekeyFrameLimit : kAltsRecordProtocolFrameLimit;
  status = alts_seal_crypter_create(aead_crypter_seal, is_client, overflow_size,
                                    &impl->seal_crypter, error_details);
  if (status != GRPC_STATUS_OK) {
    return status;
  }
  status = alts_unseal_crypter_create(aead_crypter_unseal, is_client, overflow_size,
                                      &impl->unseal_crypter, error_details);
  return status;
}

tsi_result alts_create_frame_protector(const uint8_t* key, size_t key_size, bool is_client,
                                       bool is_rekey, size_t* max_protected_frame_size,
                                       tsi_frame_protector** self) {
  if (key == nullptr || self == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to alts_create_frame_protector().");
    return TSI_INTERNAL_ERROR;
  }
  char* error_details = nullptr;
  alts_frame_protector* impl = static_cast<alts_frame_protector*>(gpr_zalloc(sizeof(*impl)));
  grpc_status_code status =
      create_alts_crypters(key, key_size, is_client, is_rekey, impl, &error_details);
  if (status != GRPC_STATUS_OK) {
  ...
  if (max_protected_frame_size != nullptr) {
    *max_protected_frame_size = GPR_MIN(*max_protected_frame_size, kMaxFrameLength);
    *max_protected_frame_size = GPR_MAX(*max_protected_frame_size, kMinFrameLength);
