...
static tsi_result alts_protect_flush(
  ...
  if (self == nullptr || protected_output_frames == nullptr ||
    ...
    gpr_log(GPR_ERROR,
            "Invalid nullptr arguments to alts_protect_flush().");
    return TSI_INVALID_ARGUMENT;
  }
  alts_frame_protector* impl =
      reinterpret_cast<alts_frame_protector*>(self);
  /**
   * If there's nothing to flush (i.e., in_place_protect_buffer is
   * empty), we're done.
   */
  ...
  /**
   * If a new frame can start being processed, we encrypt the payload
   * and reset the frame writer to point to in_place_protect_buffer that
   * holds the newly sealed frame.
   */
  if (alts_is_frame_writer_done(impl->writer)) {
    ...
    if (!alts_reset_frame_writer(
            impl->writer, impl->in_place_protect_buffer,
  ...
  /**
   * Write the sealed frame as much as possible to
   * protected_output_frames. It's possible a frame will not be written
   * out completely by a single flush (i.e., still_pending_size != 0),
   * in which case the flush should be called iteratively until a
   * complete frame has been written out.
   */
  ...
  *still_pending_size =
      alts_get_num_writer_bytes_remaining(impl->writer);
  /**
...
static tsi_result alts_protect(tsi_frame_protector* self,
  ...
  /**
   * If a full frame has been buffered, we output it. If the first
   * condition holds, then there exists an unencrypted full frame. If
   * the second condition holds, then there exists a full frame that has
...
static tsi_result unseal(alts_frame_protector* impl) {
  ...
  grpc_status_code status = alts_crypter_process_in_place(
      ...
      alts_get_output_bytes_read(impl->reader), &output_size,
      &error_details);
  if (status != GRPC_STATUS_OK) {
    gpr_log(GPR_ERROR, "%s", error_details);
    gpr_free(error_details);
...
static void ensure_buffer_size(alts_frame_protector* impl) {
  ...
  /**
   * Check if we need to resize in_place_unprotect_buffer in order to
   * hold remaining bytes of a full frame.
   */
  if (buffer_space_remaining <
...
static tsi_result alts_unprotect(
  ...
  if (self == nullptr || protected_frames_bytes == nullptr ||
      ...
      unprotected_bytes_size == nullptr) {
    gpr_log(GPR_ERROR,
            "Invalid nullptr arguments to alts_unprotect().");
    return TSI_INVALID_ARGUMENT;
  }
  alts_frame_protector* impl =
      reinterpret_cast<alts_frame_protector*>(self);
  /**
   * If a new frame can start being processed, we reset the frame reader
   * to point to in_place_unprotect_buffer that will be used to hold
   * deframed result.
   */
   ...
   * protected_frames_bytes until a full frame has been read. We also
   * need to make sure in_place_unprotect_buffer is large enough to hold
   * a complete frame.
   */
  if (!alts_is_frame_reader_done(impl->reader)) {
...
static grpc_status_code create_alts_crypters(
    const uint8_t* key, size_t key_size, bool is_client, bool is_rekey,
    alts_frame_protector* impl, char** error_details) {
  ...
  status = alts_seal_crypter_create(aead_crypter_seal, is_client,
                                    overflow_size, &impl->seal_crypter,
                                    error_details);
  if (status != GRPC_STATUS_OK) {
    return status;
  }
  status = alts_unseal_crypter_create(
      aead_crypter_unseal, is_client, overflow_size,
...
tsi_result alts_create_frame_protector(const uint8_t* key,
  ...
  if (status != GRPC_STATUS_OK) {
    gpr_log(GPR_ERROR, "Failed to create ALTS crypters, %s.",
            error_details);
    gpr_free(error_details);
    return TSI_INTERNAL_ERROR;
  }
  /**
   * Set maximum frame size to be used by a frame protector. If it is
   * nullptr, a default frame size will be used. Otherwise, the provided
   * frame size will be adjusted (if not falling into a valid frame
   * range) and used.
   */
  ...
  impl->in_place_protect_bytes_buffered = 0;
  impl->in_place_unprotect_bytes_processed = 0;
  impl->in_place_protect_buffer =
      static_cast<unsigned char*>(gpr_malloc(
          sizeof(unsigned char) * max_protected_frame_size_to_set));
  impl->in_place_unprotect_buffer =
      static_cast<unsigned char*>(gpr_malloc(
          sizeof(unsigned char) * max_protected_frame_size_to_set));
  impl->overhead_length =
      alts_crypter_num_overhead_bytes(impl->seal_crypter);
