...
grpc_status_code increment_counter(alts_record_protocol_crypter* rp_crypter, char** error_details) {
  ...
  grpc_status_code status = alts_counter_increment(rp_crypter->ctr, &is_overflow, error_details);
...
size_t alts_record_protocol_crypter_num_overhead_bytes(const alts_crypter* c) {
  if (c != nullptr) {
    ...
    grpc_status_code status =
        gsec_aead_crypter_tag_length(rp_crypter->crypter, &num_overhead_bytes, &error_details);
    if (status == GRPC_STATUS_OK) {
...
alts_record_protocol_crypter* alts_crypter_create_common(gsec_aead_crypter* crypter, bool is_client,
  ...
  if (crypter != nullptr) {
    ...
    grpc_status_code status = gsec_aead_crypter_nonce_length(crypter, &counter_size, error_details);
    if (status != GRPC_STATUS_OK) {
      return nullptr;
    }
    /* Create a counter. */
    status = alts_counter_create(is_client, counter_size, overflow_size, &rp_crypter->ctr,
                                 error_details);
    if (status != GRPC_STATUS_OK) {
      return nullptr;
    }
