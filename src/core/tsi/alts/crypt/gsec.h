...
//
//- key: buffer containing a key which is binded with AEAD crypter instance. The
//  buffer may be mutated if it's directly used with copy_key set to false.
//- key_length: length of a key in bytes, which should be 44 if rekeying is
//  enabled and 16 or 32 otherwise.
//- copy_key: whether the given key buffer should be used directly or copied.
