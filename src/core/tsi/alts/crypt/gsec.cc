...
grpc_status_code gsec_aead_crypter_encrypt(gsec_aead_crypter* crypter, const uint8_t* nonce,
                                           size_t nonce_length, const uint8_t* aad,
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
    ...
    struct iovec plaintext_vec = {const_cast<uint8_t*>(plaintext), plaintext_length};
    struct iovec ciphertext_vec = {ciphertext_and_tag, ciphertext_and_tag_length};
    return crypter->vtable->encrypt_iovec(crypter, nonce, nonce_length, &aad_vec, 1, &plaintext_vec,
                                          1, ciphertext_vec, bytes_written, error_details);
  }
  /* An error occurred. */
...
grpc_status_code gsec_aead_crypter_encrypt_iovec(
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
      crypter->vtable->encrypt_iovec != nullptr) {
    return crypter->vtable->encrypt_iovec(crypter, nonce, nonce_length, aad_vec, aad_vec_length,
...
grpc_status_code gsec_aead_crypter_decrypt(gsec_aead_crypter* crypter, const uint8_t* nonce,
                                           size_t nonce_length, const uint8_t* aad,
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
    ...
    return crypter->vtable->decrypt_iovec(crypter, nonce, nonce_length, &aad_vec, 1,
                                          &ciphertext_vec, 1, plaintext_vec, bytes_written,
                                          error_details);
  }
  /* An error occurred. */
  maybe_copy_error_msg(vtable_error_msg, error_details);
...
grpc_status_code gsec_aead_crypter_decrypt_iovec(
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
      crypter->vtable->encrypt_iovec != nullptr) {
    return crypter->vtable->decrypt_iovec(crypter, nonce, nonce_length, aad_vec, aad_vec_length,
...
grpc_status_code gsec_aead_crypter_max_ciphertext_and_tag_length(
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
    ...
    return crypter->vtable->max_ciphertext_and_tag_length(
        crypter, plaintext_length, max_ciphertext_and_tag_length_to_return, error_details);
  }
  /* An error occurred. */
  maybe_copy_error_msg(vtable_error_msg, error_details);
  return GRPC_STATUS_INVALID_ARGUMENT;
...
grpc_status_code gsec_aead_crypter_max_plaintext_length(const gsec_aead_crypter* crypter,
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
    ...
    return crypter->vtable->max_plaintext_length(crypter, ciphertext_and_tag_length,
...
grpc_status_code gsec_aead_crypter_nonce_length(const gsec_aead_crypter* crypter,
  ...
  if (crypter != nullptr && crypter->vtable != nullptr &&
    ...
    return crypter->vtable->nonce_length(crypter, nonce_length_to_return, error_details);
  }
  /* An error occurred. */
  maybe_copy_error_msg(vtable_error_msg, error_details);
  return GRPC_STATUS_INVALID_ARGUMENT;
...
grpc_status_code gsec_aead_crypter_key_length(const gsec_aead_crypter* crypter,
                                              size_t* key_length_to_return, char** error_details) {
  if (crypter != nullptr && crypter->vtable != nullptr && crypter->vtable->key_length != nullptr) {
    return crypter->vtable->key_length(crypter, key_length_to_return, error_details);
  }
  /* An error occurred */
  maybe_copy_error_msg(vtable_error_msg, error_details);
  return GRPC_STATUS_INVALID_ARGUMENT;
...
grpc_status_code gsec_aead_crypter_tag_length(const gsec_aead_crypter* crypter,
                                              size_t* tag_length_to_return, char** error_details) {
  if (crypter != nullptr && crypter->vtable != nullptr && crypter->vtable->tag_length != nullptr) {
    return crypter->vtable->tag_length(crypter, tag_length_to_return, error_details);
  }
  /* An error occurred. */
  maybe_copy_error_msg(vtable_error_msg, error_details);
  return GRPC_STATUS_INVALID_ARGUMENT;
