...
static void ensure_iovec_buf_size(alts_grpc_record_protocol* rp,
  ...
  rp->iovec_buf = static_cast<iovec_t*>(gpr_realloc(
      rp->iovec_buf, rp->iovec_buf_length * sizeof(iovec_t)));
}

/* --- Implementation of methods defined in
 * tsi_grpc_record_protocol_common.h.
 * --- */
...
iovec_t alts_grpc_record_protocol_get_header_iovec(
  ...
  if (rp->header_sb.count == 1) {
    header_iovec.iov_base =
        GRPC_SLICE_START_PTR(rp->header_sb.slices[0]);
  } else {
    /* Frame header is in multiple slices, copies the header bytes from
     * slice buffer to a single flat buffer.  */
    alts_grpc_record_protocol_copy_slice_buffer(&rp->header_sb,
                                                rp->header_buf);
...
tsi_result alts_grpc_record_protocol_init(alts_grpc_record_protocol* rp,
  ...
  if (status != GRPC_STATUS_OK) {
    gpr_log(GPR_ERROR,
            "Failed to create alts_iovec_record_protocol, %s.",
            error_details);
  ...
  rp->header_buf =
      static_cast<unsigned char*>(gpr_malloc(rp->header_length));
  rp->tag_length =
...
/* Integrity-only and privacy-integrity share the same implementation.
 * No need to call vtable.  */
