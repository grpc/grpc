...
static tsi_result alts_grpc_integrity_only_extra_copy_protect(alts_grpc_record_protocol* rp,
  ...
  size_t protected_frame_size = unprotected_slices->length + rp->header_length + rp->tag_length;
  ...
  iovec_t header_iovec = {GRPC_SLICE_START_PTR(protected_slice), rp->header_length};
  iovec_t tag_iovec = {GRPC_SLICE_START_PTR(protected_slice) + rp->header_length + data_length,
                       rp->tag_length};
  rp->iovec_buf[0].iov_base = GRPC_SLICE_START_PTR(protected_slice) + rp->header_length;
...
static tsi_result alts_grpc_integrity_only_protect(alts_grpc_record_protocol* rp,
                                                   ...
                                                   grpc_slice_buffer* protected_slices) {
  /* Input sanity check.  */
  if (rp == nullptr || unprotected_slices == nullptr || protected_slices == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to alts_grpc_record_protocol protect.");
  ...
  if (integrity_only_record_protocol->enable_extra_copy) {
    return alts_grpc_integrity_only_extra_copy_protect(rp, unprotected_slices, protected_slices);
  }
  ...
  /* Calls alts_iovec_record_protocol protect.  */
  char* error_details = nullptr;
  iovec_t header_iovec = {GRPC_SLICE_START_PTR(header_slice), GRPC_SLICE_LENGTH(header_slice)};
  iovec_t tag_iovec = {GRPC_SLICE_START_PTR(tag_slice), GRPC_SLICE_LENGTH(tag_slice)};
  alts_grpc_record_protocol_convert_slice_buffer_to_iovec(rp, unprotected_slices);
  grpc_status_code status = alts_iovec_record_protocol_integrity_only_protect(
      rp->iovec_rp, rp->iovec_buf, unprotected_slices->count, header_iovec, tag_iovec,
      &error_details);
  if (status != GRPC_STATUS_OK) {
    gpr_log(GPR_ERROR, "Failed to protect, %s", error_details);
    gpr_free(error_details);
...
static tsi_result alts_grpc_integrity_only_unprotect(alts_grpc_record_protocol* rp,
                                                     ...
                                                     grpc_slice_buffer* unprotected_slices) {
  /* Input sanity check.  */
  if (rp == nullptr || protected_slices == nullptr || unprotected_slices == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to alts_grpc_record_protocol unprotect.");
    return TSI_INVALID_ARGUMENT;
  }
  ...
  grpc_slice_buffer_move_first(protected_slices, rp->header_length, &rp->header_sb);
  ...
  grpc_slice_buffer_reset_and_unref_internal(&integrity_only_record_protocol->data_sb);
  grpc_slice_buffer_move_first(protected_slices, protected_slices->length - rp->tag_length,
  ...
  if (protected_slices->count == 1) {
    ...
    alts_grpc_record_protocol_copy_slice_buffer(protected_slices,
                                                integrity_only_record_protocol->tag_buf);
  ...
  grpc_status_code status = alts_iovec_record_protocol_integrity_only_unprotect(
      rp->iovec_rp, rp->iovec_buf, integrity_only_record_protocol->data_sb.count, header_iovec,
  ...
  grpc_slice_buffer_move_into(&integrity_only_record_protocol->data_sb, unprotected_slices);
  return TSI_OK;
}
...
tsi_result alts_grpc_integrity_only_record_protocol_create(gsec_aead_crypter* crypter,
                                                           ...
                                                           bool is_protect, bool enable_extra_copy,
                                                           alts_grpc_record_protocol** rp) {
  if (crypter == nullptr || rp == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to alts_grpc_record_protocol create.");
    return TSI_INVALID_ARGUMENT;
  }
  ...
  tsi_result result = alts_grpc_record_protocol_init(&impl->base, crypter, overflow_size, is_client,
  ...
  impl->tag_buf = static_cast<unsigned char*>(gpr_malloc(impl->base.tag_length));
