...
static bool read_frame_size(const grpc_slice_buffer* sb, uint32_t* total_frame_size) {
  ...
  *total_frame_size = static_cast<uint32_t>(frame_size + kZeroCopyFrameLengthFieldSize);
  return true;
}
 ...
 */
static tsi_result create_alts_grpc_record_protocol(const uint8_t* key, size_t key_size,
                                                   bool is_rekey, bool is_client,
  ...
  tsi_result result =
      is_integrity_only
          ? alts_grpc_integrity_only_record_protocol_create(
                crypter, overflow_limit, is_client, is_protect, enable_extra_copy, record_protocol)
          : alts_grpc_privacy_integrity_record_protocol_create(crypter, overflow_limit, is_client,
                                                               is_protect, record_protocol);
  if (result != TSI_OK) {
...
static tsi_result alts_zero_copy_grpc_protector_protect(tsi_zero_copy_grpc_protector* self,
  ...
  alts_zero_copy_grpc_protector* protector = reinterpret_cast<alts_zero_copy_grpc_protector*>(self);
  /* Calls alts_grpc_record_protocol protect repeatly.  */
  while (unprotected_slices->length > protector->max_unprotected_data_size) {
    grpc_slice_buffer_move_first(unprotected_slices, protector->max_unprotected_data_size,
                                 &protector->unprotected_staging_sb);
    tsi_result status = alts_grpc_record_protocol_protect(
        protector->record_protocol, &protector->unprotected_staging_sb, protected_slices);
    if (status != TSI_OK) {
      return status;
    }
  }
  return alts_grpc_record_protocol_protect(protector->record_protocol, unprotected_slices,
                                           protected_slices);
}
...
static tsi_result alts_zero_copy_grpc_protector_unprotect(tsi_zero_copy_grpc_protector* self,
                                                          ...
                                                          grpc_slice_buffer* unprotected_slices) {
  if (self == nullptr || unprotected_slices == nullptr || protected_slices == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to zero-copy grpc unprotect.");
    return TSI_INVALID_ARGUMENT;
  }
  alts_zero_copy_grpc_protector* protector = reinterpret_cast<alts_zero_copy_grpc_protector*>(self);
  ...
  while (protector->protected_sb.length >= kZeroCopyFrameLengthFieldSize) {
    if (protector->parsed_frame_size == 0) {
      ...
      if (!read_frame_size(&protector->protected_sb, &protector->parsed_frame_size)) {
    ...
    if (protector->protected_sb.length == protector->parsed_frame_size) {
      status = alts_grpc_record_protocol_unprotect(protector->unrecord_protocol,
                                                   &protector->protected_sb, unprotected_slices);
    } else {
      grpc_slice_buffer_move_first(&protector->protected_sb, protector->parsed_frame_size,
                                   &protector->protected_staging_sb);
      status = alts_grpc_record_protocol_unprotect(
          protector->unrecord_protocol, &protector->protected_staging_sb, unprotected_slices);
    }
...
static const tsi_zero_copy_grpc_protector_vtable alts_zero_copy_grpc_protector_vtable = {
    alts_zero_copy_grpc_protector_protect, alts_zero_copy_grpc_protector_unprotect,
    alts_zero_copy_grpc_protector_destroy, alts_zero_copy_grpc_protector_max_frame_size};

tsi_result alts_zero_copy_grpc_protector_create(const uint8_t* key, size_t key_size, bool is_rekey,
                                                bool is_client, bool is_integrity_only,
                                                bool enable_extra_copy,
                                                size_t* max_protected_frame_size,
                                                tsi_zero_copy_grpc_protector** protector) {
  if (grpc_core::ExecCtx::Get() == nullptr || key == nullptr || protector == nullptr) {
    gpr_log(GPR_ERROR, "Invalid nullptr arguments to alts_zero_copy_grpc_protector create.");
    return TSI_INVALID_ARGUMENT;
  }
  /* Creates alts_zero_copy_protector.  */
  alts_zero_copy_grpc_protector* impl = static_cast<alts_zero_copy_grpc_protector*>(
  ...
  if (status == TSI_OK) {
    status = create_alts_grpc_record_protocol(key, key_size, is_rekey, is_client, is_integrity_only,
    ...
    if (status == TSI_OK) {
      ...
      impl->max_unprotected_data_size = alts_grpc_record_protocol_max_unprotected_data_size(
