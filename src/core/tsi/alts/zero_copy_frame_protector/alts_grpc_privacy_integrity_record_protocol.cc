...
static tsi_result alts_grpc_privacy_integrity_protect(alts_grpc_record_protocol* rp,
  ...
  size_t protected_frame_size = unprotected_slices->length + rp->header_length +
  ...
  grpc_status_code status = alts_iovec_record_protocol_privacy_integrity_protect(
      rp->iovec_rp, rp->iovec_buf, unprotected_slices->count, protected_iovec, &error_details);
...
static tsi_result alts_grpc_privacy_integrity_unprotect(alts_grpc_record_protocol* rp,
                                                        grpc_slice_buffer* protected_slices,
                                                        grpc_slice_buffer* unprotected_slices) {
  /* Input sanity check.  */
  ...
  size_t unprotected_frame_size = protected_slices->length - rp->header_length - rp->tag_length;
  ...
  grpc_status_code status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      rp->iovec_rp, header_iovec, rp->iovec_buf, protected_slices->count, unprotected_iovec,
      &error_details);
  if (status != GRPC_STATUS_OK) {
...
static const alts_grpc_record_protocol_vtable alts_grpc_privacy_integrity_record_protocol_vtable = {
    alts_grpc_privacy_integrity_protect, alts_grpc_privacy_integrity_unprotect, nullptr};
...
tsi_result alts_grpc_privacy_integrity_record_protocol_create(gsec_aead_crypter* crypter,
  ...
  auto* impl =
      static_cast<alts_grpc_record_protocol*>(gpr_zalloc(sizeof(alts_grpc_record_protocol)));
  /* Calls alts_grpc_record_protocol init.  */
  tsi_result result = alts_grpc_record_protocol_init(impl, crypter, overflow_size, is_client,
