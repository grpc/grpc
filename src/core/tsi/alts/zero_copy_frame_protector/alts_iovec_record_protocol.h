...
// Limit k on number of frames such that at most 2^(8 * k) frames can be
// sent.
constexpr size_t kAltsRecordProtocolRekeyFrameLimit = 8;
constexpr size_t kAltsRecordProtocolFrameLimit = 5;

/* An implementation of alts record protocol. The API is
 * thread-compatible. */
 ...
 *
 * On success, the method returns the length of record protocol frame
 * tag. Otherwise, it returns zero.
 */
size_t alts_iovec_record_protocol_get_tag_length(
    const alts_iovec_record_protocol* rp);

/**
 * This method returns maximum allowed unprotected data size, given
 * maximum protected frame size.
 *
 * - rp: an alts_iovec_record_protocol instance.
 * - max_protected_frame_size: maximum protected frame size.
 *
 * On success, the method returns the maximum allowed unprotected data
 * size. Otherwise, it returns zero.
 */
 ...
 * alts_iovec_record_protocol instance, i.e., compute frame header and
 * tag. The caller needs to allocate the memory for header and tag prior
 ...
 * does not function correctly. It is OK to pass nullptr into
 * error_details.
 *
 * On success, the method returns GRPC_STATUS_OK. Otherwise, it returns
 * an error status code along with its details specified in
 ...
 * alts_iovec_record_protocol instance, i.e., verify frame header and
 * tag.
 *
 * - rp: an alts_iovec_record_protocol instance.
 ...
 * alts_iovec_record_protocol instance, i.e., compute a protected frame.
 * The caller needs to allocate the memory for the protected frame prior
 * to calling this method.
 *
 * - rp: an alts_iovec_record_protocol instance.
 * - unprotected_vec: an iovec array containing unprotected data.
 ...
 * alts_iovec_record_protocol instance given a full protected frame,
 * i.e., compute the unprotected data. The caller needs to allocated the
 * memory for the unprotected data prior to calling this method.
 *
 * - rp: an alts_iovec_record_protocol instance.
 * - header: an iovec containing the frame header.
 * - protected_vec: an iovec array containing protected data including
 * the tag.
 * - protected_vec_length: the array length of protected_vec.
 ...
 * gsec_aead_crypter instance, a flag indicating if the created instance
 * will be used at the client or server side, and a flag indicating if
 * the created instance will be used for integrity-only mode or
 * privacy-integrity mode. The ownership of gsec_aead_crypter instance
 * is transferred to this new object.
 *
 * - crypter: a gsec_aead_crypter instance used to perform AEAD
 * decryption.
 * - overflow_size: overflow size of counter in bytes.
 * - is_client: a flag indicating if the alts_iovec_record_protocol
 * instance will be used at the client or server side.
 * - is_integrity_only: a flag indicating if the
 * alts_iovec_record_protocol instance will be used for integrity-only
 * or privacy-integrity mode.
 * - is_protect: a flag indicating if the alts_grpc_record_protocol
 * instance will be used for protect or unprotect.
...
/**
 * This method destroys an alts_iovec_record_protocol instance by
 * de-allocating all of its occupied memory. A gsec_aead_crypter
 * instance passed in at gsec_alts_crypter instance creation time will
 * be destroyed in this method.
 */
