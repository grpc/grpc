...
tsi_result tsi_handshaker_result_create_frame_protector (const
...
tsi_result tsi_handshaker_result_get_unused_bytes (const tsi_handshaker_result
						   * self,
						   const unsigned char
						   **bytes,
						   size_t * byte_size);
...
/* Callback function definition for tsi_handshaker_next.
   ...
   - handshaker_result is the result of handshake when the handshake completes,
     is NULL otherwise.  */
typedef void (*tsi_handshaker_on_next_done_cb) (tsi_result status,
						void *user_data,
						const unsigned char
						*bytes_to_send,
