...
tsi_result tsi_zero_copy_grpc_protector_unprotect_and_get_min_progress_size(
  ...
  if (self == nullptr || self->vtable == nullptr ||
      protected_slices == nullptr || unprotected_slices == nullptr ||
      min_progress_size == nullptr) {
    return TSI_INVALID_ARGUMENT;
  }
  if (self->vtable->unprotect_and_get_min_progress_size == nullptr) {
    return TSI_UNIMPLEMENTED;
  }
  return self->vtable->unprotect_and_get_min_progress_size(
      self, protected_slices, unprotected_slices, min_progress_size);
}
