...
init_openssl (void)
{
  ...
  openssl_mutexes =
    (gpr_mu *) gpr_malloc ((size_t) num_locks * sizeof (gpr_mu));
...
static tsi_result
ssl_get_x509_common_name (X509 * cert, unsigned char **utf8,
			  size_t * utf8_size)
{
...
static tsi_result
peer_property_from_x509_common_name (X509 * cert,
				     tsi_peer_property * property)
{
  ...
  result =
    ...
    (TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY,
     common_name == nullptr ? "" : (const char *) common_name,
...
static tsi_result
add_pem_certificate (X509 * cert, tsi_peer_property * property)
{
...
static tsi_result
add_subject_alt_names_properties_to_peer (tsi_peer * peer,
					  ...
					  size_t subject_alt_name_count)
{
  ...
  for (i = 0; i < subject_alt_name_count; i++)
      ...
      else if (subject_alt_name->type == GEN_IPADD)
	{
	  ...
	  const char *name =
	    inet_ntop (af, subject_alt_name->d.iPAddress->data,
...
peer_from_x509 (X509 * cert, int include_certificate_type, tsi_peer * peer)
{
  ...
  GENERAL_NAMES *subject_alt_names =
    (GENERAL_NAMES *) X509_get_ext_d2i (cert, NID_subject_alt_name, nullptr,
					nullptr);
  int subject_alt_name_count =
    (subject_alt_names !=
     nullptr) ? (int) sk_GENERAL_NAME_num (subject_alt_names) : 0;
  ...
  do
      ...
      if (include_certificate_type)
	{
      ...
      result =
	peer_property_from_x509_common_name (cert,
					     &peer->
					     properties
					     [include_certificate_type ? 1 :
					      0]);
      if (result != TSI_OK)
	break;

      result =
	add_pem_certificate (cert,
			     &peer->
			     properties[include_certificate_type ? 2 : 1]);
      if (result != TSI_OK)
	break;
	...
	{
	  result =
	    add_subject_alt_names_properties_to_peer (peer, subject_alt_names,
						      (size_t)
...
static tsi_result
do_ssl_read (SSL * ssl, unsigned char *unprotected_bytes,
...
static tsi_result
do_ssl_write (SSL * ssl, unsigned char *unprotected_bytes,
	      size_t unprotected_bytes_size)
{
...
static tsi_result
ssl_ctx_use_certificate_chain (SSL_CTX * context,
			       ...
			       size_t pem_cert_chain_size)
{
  ...
  do
    {
      certificate =
	PEM_read_bio_X509_AUX (pem, nullptr, nullptr, (void *) "");
...
static tsi_result
ssl_ctx_use_private_key (SSL_CTX * context, const char *pem_key,
			 size_t pem_key_size)
{
  tsi_result result = TSI_OK;
  ...
  do
    {
      private_key =
	PEM_read_bio_PrivateKey (pem, nullptr, nullptr, (void *) "");
...
static tsi_result
ssl_ctx_load_verification_certs (SSL_CTX * context,
...
static tsi_result
populate_ssl_context (SSL_CTX * context,
		      ...
		      const char *cipher_list)
{
  ...
  if (key_cert_pair != nullptr)
	...
	{
	  result =
	    ssl_ctx_use_certificate_chain (context, key_cert_pair->cert_chain,
					   strlen (key_cert_pair->
						   cert_chain));
	  if (result != TSI_OK)
	    {
...
static tsi_result
extract_x509_subject_names_from_pem_cert (const char *pem_cert,
					  tsi_peer * peer)
{
  tsi_result result = TSI_OK;
  X509 *cert = nullptr;
...
static tsi_result
build_alpn_protocol_name_list (const char **alpn_protocols,
			       ...
			       size_t * protocol_name_list_length)
{
  ...
  *protocol_name_list =
    (unsigned char *) gpr_malloc (*protocol_name_list_length);
  if (*protocol_name_list == nullptr)
    return TSI_OUT_OF_RESOURCES;
...
static int
NullVerifyCallback (int preverify_ok, X509_STORE_CTX * ctx)
{
  return 1;
}
...
static tsi_result
ssl_protector_protect (tsi_frame_protector * self,
...
static tsi_result
ssl_protector_protect_flush (tsi_frame_protector * self,
			     unsigned char *protected_output_frames,
			     size_t * protected_output_frames_size,
...
static tsi_result
ssl_protector_unprotect (tsi_frame_protector * self,
...
static tsi_ssl_handshaker_factory *
tsi_ssl_handshaker_factory_ref (tsi_ssl_handshaker_factory * self)
{
  if (self == nullptr)
    return nullptr;
...
static void
tsi_ssl_handshaker_factory_unref (tsi_ssl_handshaker_factory * self)
{
  if (self == nullptr)
    return;
...
static void
tsi_ssl_handshaker_factory_init (tsi_ssl_handshaker_factory * factory)
{
...
static tsi_result
ssl_handshaker_get_bytes_to_send_to_peer (tsi_handshaker * self,
					  unsigned char *bytes,
					  size_t * bytes_size)
{
...
static tsi_result
ssl_handshaker_process_bytes_from_peer (tsi_handshaker * self,
...
ssl_handshaker_extract_peer (tsi_handshaker * self, tsi_peer * peer)
{
  ...
  if (alpn_selected != nullptr)
	...
	(tsi_peer_property *) gpr_zalloc (sizeof (*new_properties) *
					  (peer->property_count + 1));
      for (i = 0; i < peer->property_count; i++)
	{
	  new_properties[i] = peer->properties[i];
	}
      result =
	tsi_construct_string_peer_property (TSI_SSL_ALPN_SELECTED_PROTOCOL,
...
static tsi_result
ssl_handshaker_create_frame_protector (tsi_handshaker * self,
				       size_t *
...
static tsi_result
create_tsi_ssl_handshaker (SSL_CTX * ctx, int is_client,
...
static int
select_protocol_list (const unsigned char **out,
...
{
  return create_tsi_ssl_handshaker (self->ssl_context, 1,
				    server_name_indication, &self->base,
				    handshaker);
}

void
tsi_ssl_client_handshaker_factory_unref (tsi_ssl_client_handshaker_factory *
					 self)
{
  if (self == nullptr)
    return;
...
static void
tsi_ssl_client_handshaker_factory_destroy (tsi_ssl_handshaker_factory *
					   factory)
{
  if (factory == nullptr)
    return;
...
static int
client_handshaker_factory_npn_callback (SSL * ssl, unsigned char **out,
					unsigned char *outlen,
...
void
tsi_ssl_server_handshaker_factory_unref (tsi_ssl_server_handshaker_factory *
					 self)
{
  if (self == nullptr)
    return;
...
static void
tsi_ssl_server_handshaker_factory_destroy (tsi_ssl_handshaker_factory *
					   factory)
{
  if (factory == nullptr)
    return;
  ...
  if (self->ssl_contexts != nullptr)
    gpr_free (self->ssl_contexts);
...
static int
does_entry_match_name (const char *entry, size_t entry_length,
		       const char *name)
{
  const char *dot;
...
static int
ssl_server_handshaker_factory_servername_callback (SSL * ssl, int *ap,
						   void *arg)
{
...
#if TSI_OPENSSL_ALPN_SUPPORT
static int
server_handshaker_factory_alpn_callback (SSL * ssl, const unsigned char **out,
					 unsigned char *outlen,
					 const unsigned char *in,
					 unsigned int inlen, void *arg)
{
...
static int
server_handshaker_factory_npn_advertised_callback (SSL * ssl,
...
tsi_result
tsi_create_ssl_client_handshaker_factory (const tsi_ssl_pem_key_cert_pair *
					  pem_key_cert_pair,
					  ...
					  factory)
{
  ...
  *factory = nullptr;
  ...
  do
	...
	{
	  result =
	    build_alpn_protocol_name_list (alpn_protocols, num_alpn_protocols,
...
#endif /* TSI_OPENSSL_ALPN_SUPPORT */
	  SSL_CTX_set_next_proto_select_cb (ssl_context,
...
tsi_result
tsi_create_ssl_server_handshaker_factory (const tsi_ssl_pem_key_cert_pair *
					  pem_key_cert_pairs,
					  ...
					  factory)
{
  return tsi_create_ssl_server_handshaker_factory_ex (pem_key_cert_pairs,
						      num_key_cert_pairs,
...
tsi_result
tsi_create_ssl_server_handshaker_factory_ex (const tsi_ssl_pem_key_cert_pair *
					     pem_key_cert_pairs,
					     size_t num_key_cert_pairs,
					     const char
					     *pem_client_root_certs,
  ...
  *factory = nullptr;
  ...
  for (i = 0; i < num_key_cert_pairs; i++)
      ...
      do
	{
	  ...
	  if (pem_client_root_certs != nullptr)
	    {
	      ...
	      result =
		ssl_ctx_load_verification_certs (impl->ssl_contexts[i],
						 pem_client_root_certs,
						 strlen
						 (pem_client_root_certs),
						 &root_names);
	  ...
	  result =
	    extract_x509_subject_names_from_pem_cert (pem_key_cert_pairs[i].
						      cert_chain,
						      &impl->
	  ...
	  SSL_CTX_set_alpn_select_cb (impl->ssl_contexts[i],
				      ...
				      impl);
#endif /* TSI_OPENSSL_ALPN_SUPPORT */
	  SSL_CTX_set_next_protos_advertised_cb (impl->ssl_contexts[i],
	  ...
	  if (!like_ip && does_entry_match_name (property->value.data,
						 ...
						 name))
	    {
	      return 1;
		   ...
		   strncmp (name, property->value.data,
