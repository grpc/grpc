...
static void init_openssl(void) {
  ...
  if (!CRYPTO_get_locking_callback()) {
    ...
    g_openssl_mutexes =
        static_cast<gpr_mu*>(gpr_malloc(static_cast<size_t>(num_locks) * sizeof(gpr_mu)));
...
#endif
  g_ssl_ctx_ex_factory_index = SSL_CTX_get_ex_new_index(0, nullptr, nullptr, nullptr, nullptr);
...
static tsi_result add_pem_certificate(X509* cert, tsi_peer_property* property) {
  ...
  tsi_result result = tsi_construct_string_peer_property(TSI_X509_PEM_CERT_PROPERTY, contents,
...
static tsi_result add_subject_alt_names_properties_to_peer(tsi_peer* peer,
  ...
  for (i = 0; i < subject_alt_name_count; i++) {
    GENERAL_NAME* subject_alt_name = sk_GENERAL_NAME_value(subject_alt_names, TSI_SIZE_AS_SIZE(i));
    if (subject_alt_name->type == GEN_DNS || subject_alt_name->type == GEN_EMAIL ||
      ...
      if (subject_alt_name->type == GEN_DNS) {
        ...
        name_size = ASN1_STRING_to_UTF8(&name, subject_alt_name->d.uniformResourceIdentifier);
      ...
      result = tsi_construct_string_peer_property(
          ...
          static_cast<size_t>(name_size), &peer->properties[(*current_insert_index)++]);
      if (result != TSI_OK) {
        OPENSSL_free(name);
        break;
      }
      ...
      const char* name =
          inet_ntop(af, subject_alt_name->d.iPAddress->data, ntop_buf, INET6_ADDRSTRLEN);
      if (name == nullptr) {
      ...
      result = tsi_construct_string_peer_property_from_cstring(
          TSI_X509_IP_PEER_PROPERTY, name, &peer->properties[(*current_insert_index)++]);
    } else {
      result = tsi_construct_string_peer_property_from_cstring(
...
static tsi_result peer_from_x509(X509* cert, int include_certificate_type, tsi_peer* peer) {
  ...
  int subject_alt_name_count =
      (subject_alt_names != nullptr) ? static_cast<int>(sk_GENERAL_NAME_num(subject_alt_names)) : 0;
  size_t property_count;
  tsi_result result;
  GPR_ASSERT(subject_alt_name_count >= 0);
  property_count = (include_certificate_type ? static_cast<size_t>(1) : 0) +
                   2 /* common name, certificate */ + static_cast<size_t>(subject_alt_name_count);
  for (int i = 0; i < subject_alt_name_count; i++) {
    ...
    if (subject_alt_name->type == GEN_URI || subject_alt_name->type == GEN_DNS ||
        subject_alt_name->type == GEN_EMAIL || subject_alt_name->type == GEN_IPADD) {
      property_count += 1;
    }
  }
  ...
  do {
    ...
    result = peer_property_from_x509_common_name(cert, &peer->properties[current_insert_index++]);
    if (result != TSI_OK) break;

    result = add_pem_certificate(cert, &peer->properties[current_insert_index++]);
    if (result != TSI_OK) break;
...
static tsi_result do_ssl_read(SSL* ssl, unsigned char* unprotected_bytes,
  ...
  read_from_ssl = SSL_read(ssl, unprotected_bytes, static_cast<int>(*unprotected_bytes_size));
  if (read_from_ssl <= 0) {
...
static tsi_result do_ssl_write(SSL* ssl, unsigned char* unprotected_bytes,
  ...
  GPR_ASSERT(unprotected_bytes_size <= INT_MAX);
  ssl_write_result = SSL_write(ssl, unprotected_bytes, static_cast<int>(unprotected_bytes_size));
...
static tsi_result ssl_ctx_use_certificate_chain(SSL_CTX* context, const char* pem_cert_chain,
  ...
  do {
    certificate = PEM_read_bio_X509_AUX(pem, nullptr, nullptr, const_cast<char*>(""));
    ...
    while (true) {
      X509* certificate_authority = PEM_read_bio_X509(pem, nullptr, nullptr, const_cast<char*>(""));
...
#if !defined(OPENSSL_IS_BORINGSSL) && !defined(OPENSSL_NO_ENGINE)
static tsi_result ssl_ctx_use_engine_private_key(SSL_CTX* context, const char* pem_key,
                                                 size_t pem_key_size) {
  tsi_result result = TSI_OK;
...
static tsi_result ssl_ctx_use_pem_private_key(SSL_CTX* context, const char* pem_key,
                                              size_t pem_key_size) {
  tsi_result result = TSI_OK;
  EVP_PKEY* private_key = nullptr;
  BIO* pem;
  ...
  if (pem == nullptr) return TSI_OUT_OF_RESOURCES;
  do {
    private_key = PEM_read_bio_PrivateKey(pem, nullptr, nullptr, const_cast<char*>(""));
    if (private_key == nullptr) {
      result = TSI_INVALID_ARGUMENT;
      break;
    }
...
static tsi_result x509_store_load_certs(X509_STORE* cert_store, const char* pem_roots,
                                        size_t pem_roots_size, STACK_OF(X509_NAME) * *root_names) {
  tsi_result result = TSI_OK;
  size_t num_roots = 0;
  X509* root = nullptr;
...
static tsi_result ssl_ctx_load_verification_certs(SSL_CTX* context, const char* pem_roots,
  ...
  return x509_store_load_certs(cert_store, pem_roots, pem_roots_size, root_name);
}
...
static tsi_result build_alpn_protocol_name_list(const char** alpn_protocols,
  ...
  *protocol_name_list = static_cast<unsigned char*>(gpr_malloc(*protocol_name_list_length));
  ...
  if ((current < *protocol_name_list) ||
      (static_cast<uintptr_t>(current - *protocol_name_list) != *protocol_name_list_length)) {
    return TSI_INTERNAL_ERROR;
  }
  return TSI_OK;
}
...
static int NullVerifyCallback(int /*preverify_ok*/, X509_STORE_CTX* /*ctx*/) { return 1; }
...
static tsi_result tsi_set_min_and_max_tls_versions(SSL_CTX* ssl_context,
  ...
  if (ssl_context == nullptr) {
    gpr_log(GPR_INFO, "Invalid nullptr argument to |tsi_set_min_and_max_tls_versions|.");
    return TSI_INVALID_ARGUMENT;
  }
#if OPENSSL_VERSION_NUMBER >= 0x10100000
...
tsi_ssl_root_certs_store* tsi_ssl_root_certs_store_create(const char* pem_roots) {
  ...
  tsi_ssl_root_certs_store* root_store =
      static_cast<tsi_ssl_root_certs_store*>(gpr_zalloc(sizeof(tsi_ssl_root_certs_store)));
...
tsi_result tsi_ssl_get_cert_chain_contents(STACK_OF(X509) * peer_chain,
  ...
  tsi_result result = tsi_construct_string_peer_property(TSI_X509_PEM_CERT_CHAIN_PROPERTY, contents,
                                                         static_cast<size_t>(len), property);
  BIO_free(bio);
  return result;
}
...
static tsi_result ssl_handshaker_result_extract_peer(const tsi_handshaker_result* self,
  ...
  const tsi_ssl_handshaker_result* impl = reinterpret_cast<const tsi_ssl_handshaker_result*>(self);
  ...
  tsi_peer_property* new_properties =
      static_cast<tsi_peer_property*>(gpr_zalloc(sizeof(*new_properties) * new_property_count));
  ...
  if (peer_chain != nullptr) {
    result = tsi_ssl_get_cert_chain_contents(peer_chain, &peer->properties[peer->property_count]);
    if (result == TSI_OK) peer->property_count++;
  }
  if (alpn_selected != nullptr) {
    result = tsi_construct_string_peer_property(
        TSI_SSL_ALPN_SELECTED_PROTOCOL, reinterpret_cast<const char*>(alpn_selected),
        alpn_selected_len, &peer->properties[peer->property_count]);
    if (result != TSI_OK) return result;
    peer->property_count++;
  }
  // Add security_level peer property.
  result = tsi_construct_string_peer_property_from_cstring(
      TSI_SECURITY_LEVEL_PEER_PROPERTY, tsi_security_level_to_string(TSI_PRIVACY_AND_INTEGRITY),
      &peer->properties[peer->property_count]);
  if (result != TSI_OK) return result;
  peer->property_count++;

  const char* session_reused = SSL_session_reused(impl->ssl) ? "true" : "false";
  result = tsi_construct_string_peer_property_from_cstring(TSI_SSL_SESSION_REUSED_PEER_PROPERTY,
                                                           session_reused,
                                                           &peer->properties[peer->property_count]);
  if (result != TSI_OK) return result;
  peer->property_count++;
...
static tsi_result ssl_handshaker_result_create_frame_protector(
  ...
  tsi_ssl_handshaker_result* impl =
      reinterpret_cast<tsi_ssl_handshaker_result*>(const_cast<tsi_handshaker_result*>(self));
  ...
  protector_impl->buffer = static_cast<unsigned char*>(gpr_malloc(protector_impl->buffer_size));
...
static void ssl_handshaker_result_destroy(tsi_handshaker_result* self) {
  tsi_ssl_handshaker_result* impl = reinterpret_cast<tsi_ssl_handshaker_result*>(self);
  SSL_free(impl->ssl);
...
static tsi_result ssl_handshaker_get_bytes_to_send_to_peer(tsi_ssl_handshaker* impl,
  ...
  if (bytes == nullptr || bytes_size == nullptr || *bytes_size == 0 || *bytes_size > INT_MAX) {
    return TSI_INVALID_ARGUMENT;
  }
...
static tsi_result ssl_handshaker_process_bytes_from_peer(tsi_ssl_handshaker* impl,
  ...
  GPR_ASSERT(*bytes_size <= INT_MAX);
  bytes_written_into_ssl_size = BIO_write(impl->network_io, bytes, static_cast<int>(*bytes_size));
...
// |bytes_remaining|.
static tsi_result ssl_bytes_remaining(tsi_ssl_handshaker* impl, unsigned char** bytes_remaining,
  ...
  *bytes_remaining = static_cast<uint8_t*>(gpr_malloc(bytes_in_ssl));
  int bytes_read =
      BIO_read(SSL_get_rbio(impl->ssl), *bytes_remaining, static_cast<int>(bytes_in_ssl));
  ...
  if (bytes_read < 0 || static_cast<size_t>(bytes_read) != bytes_in_ssl) {
    gpr_log(GPR_ERROR, "Failed to read the expected number of bytes from SSL object.");
...
static tsi_result ssl_handshaker_next(tsi_handshaker* self, const unsigned char* received_bytes,
  ...
  if (received_bytes_size > 0) {
    status = ssl_handshaker_process_bytes_from_peer(impl, received_bytes, &bytes_consumed);
  ...
  do {
    ...
    status = ssl_handshaker_get_bytes_to_send_to_peer(impl, impl->outgoing_bytes_buffer + offset,
                                                      &to_send_size);
    offset += to_send_size;
    if (status == TSI_INCOMPLETE_DATA) {
      impl->outgoing_bytes_buffer_size *= 2;
      impl->outgoing_bytes_buffer = static_cast<unsigned char*>(
          gpr_realloc(impl->outgoing_bytes_buffer, impl->outgoing_bytes_buffer_size));
    }
  ...
  *bytes_to_send_size = offset;
  ...
  if (ssl_handshaker_get_result(impl) == TSI_HANDSHAKE_IN_PROGRESS) {
    *handshaker_result = nullptr;
    ...
    status = ssl_handshaker_result_create(impl, unused_bytes, unused_bytes_size, handshaker_result);
    if (status == TSI_OK) {
  ...
  *handshaker = nullptr;
  ...
  if (is_client) {
    ...
    if (client_factory->session_cache != nullptr) {
      tsi_ssl_handshaker_resume_session(ssl, client_factory->session_cache.get());
    }
    ssl_result = SSL_do_handshake(ssl);
    ssl_result = SSL_get_error(ssl, ssl_result);
    if (ssl_result != SSL_ERROR_WANT_READ) {
      gpr_log(GPR_ERROR, "Unexpected error received from first SSL_do_handshake call: %s",
  ...
  impl->outgoing_bytes_buffer_size = TSI_SSL_HANDSHAKER_OUTGOING_BUFFER_INITIAL_SIZE;
...
static int select_protocol_list(const unsigned char** out, unsigned char* outlen,
...
tsi_result tsi_ssl_client_handshaker_factory_create_handshaker(
    ...
    tsi_handshaker** handshaker) {
  return create_tsi_ssl_handshaker(factory->ssl_context, 1, server_name_indication, &factory->base,
                                   handshaker);
}
...
static int client_handshaker_factory_npn_callback(SSL* /*ssl*/, unsigned char** out,
  ...
  tsi_ssl_client_handshaker_factory* factory = static_cast<tsi_ssl_client_handshaker_factory*>(arg);
  return select_protocol_list(const_cast<const unsigned char**>(out), outlen,
                              factory->alpn_protocol_list, factory->alpn_protocol_list_length, in,
                              inlen);
}
...
tsi_result tsi_ssl_server_handshaker_factory_create_handshaker(
  ...
  return create_tsi_ssl_handshaker(factory->ssl_contexts[0], 0, nullptr, &factory->base,
                                   handshaker);
}
...
static int does_entry_match_name(absl::string_view entry, absl::string_view name) {
  ...
  if (dot == absl::string_view::npos || dot == name_subdomain.size() - 1) {
    gpr_log(GPR_ERROR, "Invalid toplevel subdomain: %s", std::string(name_subdomain).c_str());
    return 0;
  }
...
static int ssl_server_handshaker_factory_servername_callback(SSL* ssl, int* /*ap*/, void* arg) {
  tsi_ssl_server_handshaker_factory* impl = static_cast<tsi_ssl_server_handshaker_factory*>(arg);
  size_t i = 0;
...
#if TSI_OPENSSL_ALPN_SUPPORT
static int server_handshaker_factory_alpn_callback(SSL* /*ssl*/, const unsigned char** out,
                                                   unsigned char* outlen, const unsigned char* in,
                                                   unsigned int inlen, void* arg) {
  tsi_ssl_server_handshaker_factory* factory = static_cast<tsi_ssl_server_handshaker_factory*>(arg);
  return select_protocol_list(out, outlen, in, inlen, factory->alpn_protocol_list,
                              factory->alpn_protocol_list_length);
}
#endif /* TSI_OPENSSL_ALPN_SUPPORT */

static int server_handshaker_factory_npn_advertised_callback(SSL* /*ssl*/,
...
tsi_result tsi_create_ssl_client_handshaker_factory(
  ...
  return tsi_create_ssl_client_handshaker_factory_with_options(&options, factory);
}
...
tsi_result tsi_create_ssl_client_handshaker_factory_with_options(
  ...
  *factory = nullptr;
  ...
  result = tsi_set_min_and_max_tls_versions(ssl_context, options->min_tls_version,
  ...
  if (options->session_cache != nullptr) {
    ...
    impl->session_cache = reinterpret_cast<tsi::SslSessionLRUCache*>(options->session_cache)->Ref();
    SSL_CTX_set_ex_data(ssl_context, g_ssl_ctx_ex_factory_index, impl);
    SSL_CTX_sess_set_new_cb(ssl_context, server_handshaker_factory_new_session_callback);
    SSL_CTX_set_session_cache_mode(ssl_context, SSL_SESS_CACHE_CLIENT);
  }

  do {
    result = populate_ssl_context(ssl_context, options->pem_key_cert_pair, options->cipher_suites);
    if (result != TSI_OK) break;
    ...
    if (OPENSSL_VERSION_NUMBER < 0x10100000 || options->root_store == nullptr) {
      result = ssl_ctx_load_verification_certs(ssl_context, options->pem_root_certs,
    ...
    if (options->num_alpn_protocols != 0) {
      result = build_alpn_protocol_name_list(options->alpn_protocols, options->num_alpn_protocols,
...
tsi_result tsi_create_ssl_server_handshaker_factory_ex(
  ...
  options.alpn_protocols = alpn_protocols;
  options.num_alpn_protocols = num_alpn_protocols;
  return tsi_create_ssl_server_handshaker_factory_with_options(&options, factory);
}
...
tsi_result tsi_create_ssl_server_handshaker_factory_with_options(
  ...
  if (factory == nullptr) return TSI_INVALID_ARGUMENT;
  *factory = nullptr;
  if (options->num_key_cert_pairs == 0 || options->pem_key_cert_pairs == nullptr) {
    return TSI_INVALID_ARGUMENT;
  }
  ...
  impl->ssl_contexts =
      static_cast<SSL_CTX**>(gpr_zalloc(options->num_key_cert_pairs * sizeof(SSL_CTX*)));
  impl->ssl_context_x509_subject_names =
      static_cast<tsi_peer*>(gpr_zalloc(options->num_key_cert_pairs * sizeof(tsi_peer)));
  ...
  if (options->num_alpn_protocols > 0) {
    result =
        build_alpn_protocol_name_list(options->alpn_protocols, options->num_alpn_protocols,
  ...
  for (i = 0; i < options->num_key_cert_pairs; i++) {
    do {
      ...
      result = tsi_set_min_and_max_tls_versions(impl->ssl_contexts[i], options->min_tls_version,
                                                options->max_tls_version);
      if (result != TSI_OK) return result;

      result = populate_ssl_context(impl->ssl_contexts[i], &options->pem_key_cert_pairs[i],
      ...
      int set_sid_ctx_result = SSL_CTX_set_session_id_context(
          impl->ssl_contexts[i], kSslSessionIdContext, GPR_ARRAY_SIZE(kSslSessionIdContext));
      ...
      if (options->session_ticket_key != nullptr) {
        if (SSL_CTX_set_tlsext_ticket_keys(impl->ssl_contexts[i],
      ...
      if (options->pem_client_root_certs != nullptr) {
        ...
        result =
            ssl_ctx_load_verification_certs(impl->ssl_contexts[i], options->pem_client_root_certs,
      ...
      switch (options->client_certificate_request) {
        ...
        case TSI_REQUEST_CLIENT_CERTIFICATE_BUT_DONT_VERIFY:
          SSL_CTX_set_verify(impl->ssl_contexts[i], SSL_VERIFY_PEER, NullVerifyCallback);
          break;
        ...
        case TSI_REQUEST_AND_REQUIRE_CLIENT_CERTIFICATE_BUT_DONT_VERIFY:
          SSL_CTX_set_verify(impl->ssl_contexts[i],
                             SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NullVerifyCallback);
          break;
      ...
      result = tsi_ssl_extract_x509_subject_names_from_pem_cert(
          options->pem_key_cert_pairs[i].cert_chain, &impl->ssl_context_x509_subject_names[i]);
      if (result != TSI_OK) break;

      SSL_CTX_set_tlsext_servername_callback(impl->ssl_contexts[i],
                                             ssl_server_handshaker_factory_servername_callback);
      SSL_CTX_set_tlsext_servername_arg(impl->ssl_contexts[i], impl);
#if TSI_OPENSSL_ALPN_SUPPORT
      SSL_CTX_set_alpn_select_cb(impl->ssl_contexts[i], server_handshaker_factory_alpn_callback,
                                 impl);
#endif /* TSI_OPENSSL_ALPN_SUPPORT */
      SSL_CTX_set_next_protos_advertised_cb(
          impl->ssl_contexts[i], server_handshaker_factory_npn_advertised_callback, impl);
    } while (false);
...
int tsi_ssl_peer_matches_name(const tsi_peer* peer, absl::string_view name) {
  ...
  if (san_count == 0 && cn_property != nullptr && !like_ip) {
    if (does_entry_match_name(absl::string_view(cn_property->value.data, cn_property->value.length),
                              name)) {
      return 1;
    }
  }
