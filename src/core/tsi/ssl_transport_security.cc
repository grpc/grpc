...
static SslHandshakeResult ssl_handshaker_next_impl(tsi_ssl_handshaker* self)
  ...
  if (!self->handshaker_next_args->received_bytes.empty()) {
    ...
    while (remaining_bytes_to_write_to_openssl_size > 0 &&
           (handshake_result.tsi_result == TSI_OK ||
            handshake_result.tsi_result == TSI_INCOMPLETE_DATA) &&
      ...
      // the peer.
      while (handshake_result.tsi_result == TSI_DRAIN_BUFFER) {
        tsi_result status =
...
ssl_handshaker_next_async(tsi_ssl_handshaker* self)
  ...
  if (handshake_result.tsi_result != TSI_ASYNC) {
    ...
    return {handshake_result.tsi_result, std::move(args)};
  }
...
static tsi_result ssl_handshaker_next(
  ...
  return handshake_result.tsi_result;
}
