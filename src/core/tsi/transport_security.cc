...
tsi_result tsi_frame_protector_protect_flush(tsi_frame_protector* self,
  ...
  return self->vtable->protect_flush(self, protected_output_frames, protected_output_frames_size,
                                     still_pending_size);
}
...
tsi_result tsi_frame_protector_unprotect(tsi_frame_protector* self,
  ...
  return self->vtable->unprotect(self, protected_frames_bytes, protected_frames_bytes_size,
...
tsi_result tsi_handshaker_get_bytes_to_send_to_peer(tsi_handshaker* self, unsigned char* bytes,
                                                    size_t* bytes_size) {
  if (self == nullptr || self->vtable == nullptr || bytes == nullptr || bytes_size == nullptr) {
    return TSI_INVALID_ARGUMENT;
  }
...
tsi_result tsi_handshaker_create_frame_protector(tsi_handshaker* self,
  ...
  result = self->vtable->create_frame_protector(self, max_output_protected_frame_size, protector);
  if (result == TSI_OK) {
...
tsi_result tsi_handshaker_next(tsi_handshaker* self, const unsigned char* received_bytes,
  ...
  return self->vtable->next(self, received_bytes, received_bytes_size, bytes_to_send,
...
tsi_result tsi_handshaker_result_create_frame_protector(const tsi_handshaker_result* self,
  ...
  if (self->vtable->create_frame_protector == nullptr) return TSI_UNIMPLEMENTED;
  return self->vtable->create_frame_protector(self, max_output_protected_frame_size, protector);
}
...
tsi_result tsi_construct_allocated_string_peer_property(const char* name, size_t value_length,
                                                        tsi_peer_property* property) {
...
tsi_result tsi_construct_string_peer_property_from_cstring(const char* name, const char* value,
                                                           tsi_peer_property* property) {
  return tsi_construct_string_peer_property(name, value, strlen(value), property);
}

tsi_result tsi_construct_string_peer_property(const char* name, const char* value,
                                              size_t value_length, tsi_peer_property* property) {
...
tsi_result tsi_construct_peer(size_t property_count, tsi_peer* peer) {
  ...
  if (property_count > 0) {
    peer->properties =
        static_cast<tsi_peer_property*>(gpr_zalloc(property_count * sizeof(tsi_peer_property)));
...
const tsi_peer_property* tsi_peer_get_property_by_name(const tsi_peer* peer, const char* name) {
  ...
  for (i = 0; i < peer->property_count; i++) {
    ...
    if (name != nullptr && property->name != nullptr && strcmp(property->name, name) == 0) {
      return property;
    }
  }
  return nullptr;
