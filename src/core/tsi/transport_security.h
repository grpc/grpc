...
{
  const tsi_frame_protector_vtable *
    vtable;
};
...
typedef struct
{
  tsi_result (*extract_peer) (const tsi_handshaker_result * self,
			      tsi_peer * peer);
  tsi_result (*create_zero_copy_grpc_protector) (const tsi_handshaker_result *
						 self,
						 size_t *
...
{
  const tsi_handshaker_result_vtable *
    vtable;
};
...
/* Utils. */
char *
tsi_strdup (const char *src);	/* Sadly, no strdup in C89. */
