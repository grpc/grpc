...
static tsi_result ssl_handshaker_result_extract_peer(
  ...
  /* when session is reused, there will not exist peer's certificate chain, and
   * we will return leaf certificate. */
  int cert_len = 0;
  /*
  const char* pem_cert =
      SSL_session_reused(impl->ssl)
          ? get_cert_ASN1(SSL_get_peer_certificate(impl->ssl), &cert_len)
          : get_cert_chain_ASN1(SSL_get_peer_cert_chain(impl->ssl));
  */
  const char* pem_cert = nullptr;
  if (SSL_session_reused(impl->ssl)) {
...
static void on_credential_reloaded(void* arg, grpc_error* error) {
  ...
  result = ssl_handshaker_next(
      ...
      handshaker->handshaker_next_arg.received_bytes_size,
      handshaker->handshaker_next_arg.bytes_to_send,
      handshaker->handshaker_next_arg.bytes_to_send_size,
      handshaker->handshaker_next_arg.handshaker_result,
      handshaker->handshaker_next_arg.cb,
      handshaker->handshaker_next_arg.user_data);
  cb(result, user_data, *(handshaker->handshaker_next_arg.bytes_to_send),
     *(handshaker->handshaker_next_arg.bytes_to_send_size),
     *(handshaker->handshaker_next_arg.handshaker_result));
  return;
}
...
static tsi_result tls_handshaker_next(
  ...
  if (!handshaker->credential_reloaded) {
    ...
    handshaker->handshaker_next_arg.bytes_to_send = bytes_to_send;
    handshaker->handshaker_next_arg.bytes_to_send_size = bytes_to_send_size;
    handshaker->handshaker_next_arg.handshaker_result = handshaker_result;
...
tsi_result tls_tsi_handshaker_create(const char* server_name_indication,
  ...
  impl->alpn_protocols =
      grpc_fill_alpn_protocol_strings(&impl->num_alpn_protocols);
  impl->pem_root_certs = grpc_core::DefaultSslRootStore::GetPemRootCerts();
  impl->root_store = grpc_core::DefaultSslRootStore::GetRootStore();
...
static char* get_cert_chain_ASN1_server(X509* leaf_cert,
                                        STACK_OF(X509) * cert_chain) {
  if (leaf_cert == nullptr && cert_chain == nullptr) {
    return nullptr;
  }
...
static tsi_result update_ssl_handshaker(tsi_ssl_handshaker* handshaker,
  ...
  if (is_client) {
    ...
    if (has_key_cert_pair) {
      cert_pairs = grpc_convert_grpc_to_tsi_cert_pairs(
