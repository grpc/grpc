...
TlsSessionKeyLoggerCache
  ::TlsSessionKeyLogger::TlsSessionKeyLogger(
    std::string tls_session_key_log_file_path,
    TlsSessionKeyLoggerCache* cache) : fd_(nullptr),
...
TlsSessionKeyLoggerCache
  ...
  {
    grpc_core::MutexLock lock(g_tls_session_key_log_cache_mu);
    cache_->tls_session_key_logger_map_.erase(
      tls_session_key_log_file_path_);
  }
  cache_->Unref();
}

void TlsSessionKeyLoggerCache
  ::TlsSessionKeyLogger::LogSessionKeys(SSL_CTX* /* ssl_context */,
...
grpc_core::RefCountedPtr<TlsSessionKeyLogger>
  ...
  // specified file path.
  auto it = tls_session_key_logger_map_.find(
    tls_session_key_log_file_path);
