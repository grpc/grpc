...
// BoringSSL allows SSL_SESSION to outlive SSL and SSL_CTX objects which
// are re-created by gRPC on every certificate rotation or subchannel
// creation. BoringSSL guarantees that SSL_SESSION is immutable so it's
// safe to share the same original session object between different
