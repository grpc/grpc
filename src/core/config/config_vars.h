...
class GPR_DLL ConfigVars {
 public:
  ...
  // EXPERIMENTAL. Only respected when there is a dependency on :grpc++_reflection. If true, no reflection server will be automatically added.
  bool CppExperimentalDisableReflection() const { return cpp_experimental_disable_reflection_; }
  // EXPERIMENTAL: If non-zero, extend the lifetime of channelz nodes past the underlying object lifetime, up to this many nodes. The value may be adjusted slightly to account for implementation limits.
  int32_t ChannelzMaxOrphanedNodes() const { return channelz_max_orphaned_nodes_; }
 private:
