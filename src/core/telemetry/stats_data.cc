...
const absl::string_view
    GlobalStats::counter_name[static_cast<int>(Counter::COUNT)] = {
        ...
        "http2_stream_stalls",
        "http2_hpack_used_entries",
        "http2_hpack_unused_entries",
        "cq_pluck_creates",
        "cq_next_creates",
        "cq_callback_creates",
        "wrr_updates",
...
const absl::string_view GlobalStats::counter_doc[static_cast<int>(
    ...
    "window",
    "Number of HPACK cache adds that were subsequently used by another request",
    "Number of HPACK cache adds that were never used by another request",
...
GlobalStats::GlobalStats()
    : client_calls_created{0},
      ...
      http2_hpack_used_entries{0},
      http2_hpack_unused_entries{0},
      cq_pluck_creates{0},
      cq_next_creates{0},
...
std::unique_ptr<GlobalStats> GlobalStatsCollector::Collect() const {
  ...
  for (const auto& data : data_) {
    ...
    result->http2_hpack_used_entries +=
        data.http2_hpack_used_entries.load(std::memory_order_relaxed);
    result->http2_hpack_unused_entries +=
        data.http2_hpack_unused_entries.load(std::memory_order_relaxed);
...
std::unique_ptr<GlobalStats> GlobalStats::Diff(const GlobalStats& other) const {
  ...
  result->http2_hpack_used_entries =
      http2_hpack_used_entries - other.http2_hpack_used_entries;
  result->http2_hpack_unused_entries =
      http2_hpack_unused_entries - other.http2_hpack_unused_entries;
