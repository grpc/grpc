...
json_writer_output_check (void *userdata, size_t needed)
{
  ...
  state->output =
    (char *) gpr_realloc (state->output, state->allocated + needed);
...
static void
json_writer_output_string_with_len (void *userdata, const char *str,
				    size_t len)
{
  json_writer_userdata *state = (json_writer_userdata *) userdata;
...
static void
json_reader_string_add_char (void *userdata, uint32_t c)
{
  json_reader_userdata *state = (json_reader_userdata *) userdata;
  GPR_ASSERT (state->string_ptr < state->input);
  GPR_ASSERT (c <= 0xff);
 ...
 */
static void
json_reader_string_add_utf32 (void *userdata, uint32_t c)
{
  if (c <= 0x7f)
    {
...
static void
json_dump_recursive (grpc_json_writer * writer, grpc_json * json,
		     int in_object)
{
  while (json)
    {
