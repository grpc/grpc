...
static void
json_reader_string_add_char (grpc_json_reader * reader, uint32_t c)
{
  reader->vtable->string_add_char (reader->userdata, c);
}

static void
json_reader_string_add_utf32 (grpc_json_reader * reader, uint32_t utf32)
{
...
void
grpc_json_reader_init (grpc_json_reader * reader,
	  ...
	  switch (reader->state)
		...
		case GRPC_JSON_STATE_STRING_ESCAPE_U4:
		  ...
		  if ((reader->unicode_char & 0xfc00) == 0xd800)
			...
			return GRPC_JSON_PARSE_ERROR;
		      utf32 = 0x10000;
		      utf32 += (uint32_t) ((reader->unicode_high_surrogate -
					    0xd800) * 0x400);
		  ...
		  else
		    {
		      ...
		      if (reader->unicode_high_surrogate != 0)
			return GRPC_JSON_PARSE_ERROR;
		      json_reader_string_add_utf32 (reader,
						    reader->unicode_char);
		    }
		  if (reader->escaped_string_was_key)
		    {
