...
SliceAllocator::SliceAllocator(grpc_resource_user* user) : resource_user_(user) {
...
SliceAllocatorFactory::SliceAllocatorFactory(grpc_resource_quota* quota) : resource_quota_(quota) {
...
SliceAllocatorFactory::SliceAllocatorFactory(SliceAllocatorFactory&& other) noexcept
    : resource_quota_(other.resource_quota_) {
  other.resource_quota_ = nullptr;
}

SliceAllocatorFactory& SliceAllocatorFactory::operator=(SliceAllocatorFactory&& other) noexcept {
  resource_quota_ = other.resource_quota_;
  other.resource_quota_ = nullptr;
  return *this;
}

SliceAllocator SliceAllocatorFactory::CreateSliceAllocator(absl::string_view peer_name) {
  return SliceAllocator(grpc_resource_user_create(resource_quota_, peer_name.data()));
}
