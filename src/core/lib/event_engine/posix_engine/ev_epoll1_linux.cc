...
void Epoll1EventHandle::OrphanHandle(PosixEngineClosure* on_done,
  ...
  auto& fds = poller_->file_descriptors_;
...
void Epoll1EventHandle::HandleShutdownInternal(absl::Status why,
  ...
  if (read_closure_->SetShutdown(why)) {
    if (releasing_fd) {
      auto result = poller_->file_descriptors_.EpollCtlDel(
...
Epoll1Poller::Epoll1Poller(Scheduler* scheduler)
  ...
  g_epoll_set_.epfd = file_descriptors_.EpollCreateAndCloexec().fd();
  wakeup_fd_ = *CreateWakeupFd(&file_descriptors_);
  CHECK(wakeup_fd_ != nullptr);
  CHECK(g_epoll_set_.epfd.ready());
  GRPC_TRACE_LOG(event_engine_poller, INFO)
      << "grpc epoll fd: " << g_epoll_set_.epfd;
  CHECK_OK(file_descriptors_
...
void Epoll1Poller::Close() {
  ...
  if (g_epoll_set_.epfd.ready()) {
    file_descriptors_.Close(g_epoll_set_.epfd);
...
EventHandle* Epoll1Poller::CreateHandle(FileDescriptor fd,
  ...
  auto result = file_descriptors_.EpollCtlAdd(
...
int Epoll1Poller::DoEpollWait(EventEngine::Duration timeout) {
  auto fd = file_descriptors_.GetFdForPolling(g_epoll_set_.epfd);
  if (!fd.has_value()) {
...
void Epoll1Poller::AdvanceGeneration() {
  file_descriptors_.AdvanceGeneration();
  {
    grpc_core::MutexLock lock(&mu_);
...
void Epoll1Poller::ResetKickState() {
  ...
  file_descriptors_.EpollCtlDel(g_epoll_set_.epfd, wakeup_fd_->ReadFd());
  wakeup_fd_ = *CreateWakeupFd(&file_descriptors_);
  auto status = file_descriptors_.EpollCtlAdd(
