...
void Epoll1EventHandle::HandleShutdownInternal(absl::Status why,
  ...
  if (read_closure_->SetShutdown(why)) {
    if (releasing_fd) {
      auto result = poller_->posix_interface_.EpollCtlDel(
...
void Epoll1Poller::Close() {
  ...
  if (g_epoll_set_.epfd.ready()) {
    posix_interface_.Close(g_epoll_set_.epfd);
...
int Epoll1Poller::DoEpollWait(EventEngine::Duration timeout) {
  auto fd = posix_interface_.GetFdForPolling(g_epoll_set_.epfd);
  if (!fd.has_value()) {
...
void Epoll1Poller::ResetKickState() {
  ...
  posix_interface_.EpollCtlDel(g_epoll_set_.epfd, wakeup_fd_->ReadFd());
  wakeup_fd_ = *CreateWakeupFd(&posix_interface_);
