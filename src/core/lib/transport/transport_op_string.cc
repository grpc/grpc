...
put_metadata (gpr_strvec * b, grpc_mdelem md)
{
  ...
  gpr_strvec_add (b,
		  grpc_dump_slice (GRPC_MDKEY (md),
				   GPR_DUMP_HEX | GPR_DUMP_ASCII));

  gpr_strvec_add (b, gpr_strdup (" value="));
  gpr_strvec_add (b,
		  grpc_dump_slice (GRPC_MDVALUE (md),
				   GPR_DUMP_HEX | GPR_DUMP_ASCII));
}
...
grpc_transport_op_string (grpc_transport_op * op)
{
  ...
  if (op->on_connectivity_state_change != nullptr)
      ...
      if (op->connectivity_state != nullptr)
	{
	  gpr_asprintf (&tmp, "ON_CONNECTIVITY_STATE_CHANGE:p=%p:from=%s",
			...
			grpc_connectivity_state_name (*op->
						      connectivity_state));
	  gpr_strvec_add (&b, tmp);
	}
      else
  ...
  if (op->set_accept_stream)
	...
	gpr_strvec_add (&b, gpr_strdup (" "));
      first = false;
      gpr_asprintf (&tmp, "SET_ACCEPT_STREAM:%p(%p,...)",
...
void
grpc_call_log_op (const char *file, int line, gpr_log_severity severity,
