...
static int
is_mdelem_static (grpc_mdelem e)
{
...
static void
ref_md_locked (mdtab_shard * shard, interned_metadata * md DEBUG_ARGS)
{
#ifndef NDEBUG
...
grow_mdtab (mdtab_shard * shard)
{
  ...
  mdtab =
    (interned_metadata **) gpr_zalloc (sizeof (interned_metadata *) *
				       capacity);
...
static void
rehash_mdtab (mdtab_shard * shard)
{
...
grpc_mdelem
grpc_mdelem_create (grpc_slice key, grpc_slice value,
		    grpc_mdelem_data * compatible_external_backing_store)
{
  if (!grpc_slice_is_interned (key) || !grpc_slice_is_interned (value))
      ...
      if (grpc_trace_metadata.enabled ())
	{
	  ...
	  gpr_log (GPR_DEBUG, "ELM ALLOC:%p:%" PRIdPTR ": '%s' = '%s'",
		   (void *) allocated,
		   gpr_atm_no_barrier_load (&allocated->refcnt), key_str,
		   value_str);
	  gpr_free (key_str);
	  gpr_free (value_str);
	}
  ...
  if (GRPC_IS_STATIC_METADATA_STRING (key) &&
	...
	grpc_static_mdelem_for_static_strings (GRPC_STATIC_METADATA_INDEX
					       (key),
  ...
  if (grpc_trace_metadata.enabled ())
    {
      char *key_str = grpc_slice_to_c_string (md->key);
      char *value_str = grpc_slice_to_c_string (md->value);
      gpr_log (GPR_DEBUG, "ELM   NEW:%p:%" PRIdPTR ": '%s' = '%s'",
	       (void *) md, gpr_atm_no_barrier_load (&md->refcnt), key_str,
	       value_str);
      gpr_free (key_str);
      gpr_free (value_str);
    }
...
grpc_mdelem
grpc_mdelem_from_slices (grpc_slice key, grpc_slice value)
{
...
grpc_mdelem
grpc_mdelem_from_grpc_metadata (grpc_metadata * metadata)
{
  bool changed = false;
  ...
  return grpc_mdelem_create (key_slice, value_slice,
			     changed ? nullptr : (grpc_mdelem_data *)
			     metadata);
}

static size_t
get_base64_encoded_size (size_t raw_length)
{
...
size_t
grpc_mdelem_get_size_in_hpack_table (grpc_mdelem elem,
...
grpc_mdelem_ref (grpc_mdelem gmd DEBUG_ARGS)
{
  switch (GRPC_MDELEM_STORAGE (gmd))
	...
	if (grpc_trace_metadata.enabled ())
	  {
	    ...
	    gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
		     "ELM   REF:%p:%" PRIdPTR "->%" PRIdPTR ": '%s' = '%s'",
		     (void *) md, gpr_atm_no_barrier_load (&md->refcnt),
		     gpr_atm_no_barrier_load (&md->refcnt) + 1, key_str,
		     value_str);
	    gpr_free (key_str);
	    gpr_free (value_str);
	  }
	...
	allocated_metadata *md =
	  (allocated_metadata *) GRPC_MDELEM_DATA (gmd);
#ifndef NDEBUG
	if (grpc_trace_metadata.enabled ())
	  {
	    char *key_str = grpc_slice_to_c_string (md->key);
...
grpc_mdelem_unref (grpc_mdelem gmd DEBUG_ARGS)
{
  switch (GRPC_MDELEM_STORAGE (gmd))
	...
	uint32_t hash = GRPC_MDSTR_KV_HASH (grpc_slice_hash (md->key),
					    grpc_slice_hash (md->value));
	const gpr_atm prev_refcount =
	  gpr_atm_full_fetch_add (&md->refcnt, -1);
...
void *
grpc_mdelem_get_user_data (grpc_mdelem md, void (*destroy_func) (void *))
{
  switch (GRPC_MDELEM_STORAGE (md))
	...
	void *result;
	if (gpr_atm_acq_load (&im->destroy_user_data) ==
...
bool
grpc_mdelem_eq (grpc_mdelem a, grpc_mdelem b)
{
  if (a.payload == b.payload)
    return true;
  if (GRPC_MDELEM_IS_INTERNED (a) && GRPC_MDELEM_IS_INTERNED (b))
    return false;
  if (GRPC_MDISNULL (a) || GRPC_MDISNULL (b))
    return false;
