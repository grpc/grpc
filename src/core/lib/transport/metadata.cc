...
void grpc_core::RefcountedMdBase::TraceAtStart(const char* tag) {
  if (grpc_trace_metadata.enabled()) {
    char* key_str = grpc_slice_to_c_string(key());
    char* value_str = grpc_slice_to_c_string(value());
    gpr_log(GPR_DEBUG, "ELM   %s:%p:%" PRIdPTR ": '%s' = '%s'", tag, this,
...
void grpc_validate_grpc_mdelem_static_key_extern_val(const grpc_slice& key,
                                                     const grpc_slice& value) {
  GPR_DEBUG_ASSERT(key.refcount == &grpc_core::kNoopRefcount ||
                   grpc_slice_is_interned(key));
  GPR_DEBUG_ASSERT(!grpc_slice_is_interned(value));
}
#endif
...
static grpc_mdelem md_create(
  ...
  bool key_is_interned = key_definitely_static || grpc_slice_is_interned(key);
  bool value_is_interned =
...
grpc_mdelem grpc_mdelem_create_key_static(
    const grpc_slice& key, const grpc_slice& value,
    grpc_mdelem_data* compatible_external_backing_store) {
...
grpc_mdelem grpc_mdelem_from_slices_key_static(const grpc_slice& key,
                                               const grpc_slice& value) {
  return md_from_slices</*key_definitely_static=*/true>(key, value);
}

grpc_mdelem grpc_mdelem_from_slices_key_static_val_static(
    const grpc_slice& key, const grpc_slice& value) {
  grpc_mdelem out = md_create_maybe_static<true, true>(key, value);
  return out;
}

grpc_mdelem grpc_mdelem_from_slices_key_static_val_interned(
    const grpc_slice& key, const grpc_slice& value) {
...
grpc_mdelem grpc_mdelem_from_slices_key_interned_val_interned(
    const grpc_slice& key, const grpc_slice& value) {
