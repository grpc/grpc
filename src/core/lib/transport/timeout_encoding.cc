...
static int64_t
round_up (int64_t x, int64_t divisor)
{
...
static int64_t
round_up_to_three_sig_figs (int64_t x)
{
  if (x < 1000)
    return x;
  if (x < 10000)
...
static void
enc_ext (char *buffer, int64_t value, char ext)
{
...
static void
enc_seconds (char *buffer, int64_t sec)
{
  if (sec % 3600 == 0)
    {
...
static void
enc_millis (char *buffer, int64_t x)
{
...
void
grpc_http2_encode_timeout (grpc_millis timeout, char *buffer)
{
  if (timeout <= 0)
    {
      enc_tiny (buffer);
    ...
    {
      enc_seconds (buffer,
		   timeout / GPR_MS_PER_SEC + (timeout % GPR_MS_PER_SEC !=
					       0));
    }
}

static int
is_all_whitespace (const char *p, const char *end)
{
  while (p != end && *p == ' ')
    p++;
  return p == end;
...
int
grpc_http2_decode_timeout (grpc_slice text, grpc_millis * timeout)
{
  grpc_millis x = 0;
