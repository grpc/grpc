...
grpc_service_config *
grpc_service_config_create (const char *json_string)
{
...
void
grpc_service_config_destroy (grpc_service_config * service_config)
{
...
void
grpc_service_config_parse_global_params (const grpc_service_config *
					 service_config,
					 void (*process_json) (const grpc_json
							       * json,
							       void *arg),
					 void *arg)
{
  const grpc_json *json = service_config->json_tree;
  if (json->type != GRPC_JSON_OBJECT || json->key != nullptr)
    return;
...
const char *
grpc_service_config_get_lb_policy_name (const grpc_service_config *
					service_config)
{
  const grpc_json *json = service_config->json_tree;
  if (json->type != GRPC_JSON_OBJECT || json->key != nullptr)
...
static size_t
count_names_in_method_config_json (grpc_json * json)
{
  size_t num_names = 0;
  for (grpc_json * field = json->child; field != nullptr; field = field->next)
    {
      if (field->key != nullptr && strcmp (field->key, "name") == 0)
	{
	  if (field->type != GRPC_JSON_ARRAY)
	    return -1;
	  for (grpc_json * name = field->child; name != nullptr;
	       name = name->next)
	    {
	      if (name->type != GRPC_JSON_OBJECT)
		return -1;
	      ++num_names;
	    }
	}
...
static char *
parse_json_method_name (grpc_json * json)
{
...
static bool
parse_json_method_config (grpc_json * json,
			  void *(*create_value) (const grpc_json *
						 method_config_json),
  ...
  for (grpc_json * child = json->child; child != nullptr; child = child->next)
      ...
      if (strcmp (child->key, "name") == 0)
	{
	  if (child->type != GRPC_JSON_ARRAY)
	    goto done;
	  for (grpc_json * name = child->child; name != nullptr;
	       name = name->next)
	    {
...
grpc_service_config_create_method_config_table (const grpc_service_config *
						service_config,
								     ...
								     *value))
{
  ...
  for (grpc_json * field = json->child; field != nullptr; field = field->next)
      ...
      if (strcmp (field->key, "methodConfig") == 0)
	{
	  ...
	  entries =
	    (grpc_slice_hash_table_entry *) gpr_malloc (num_entries *
							sizeof
  ...
  if (entries != nullptr)
    {
      method_config_table =
	grpc_slice_hash_table_create (num_entries, entries, unref_value,
				      nullptr);
      gpr_free (entries);
    }
...
void *
grpc_method_config_table_get (const grpc_slice_hash_table * table,
			      grpc_slice path)
{
