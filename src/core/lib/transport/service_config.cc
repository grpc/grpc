...
grpc_service_config* grpc_service_config_create(const char* json_string) {
  grpc_service_config* service_config = reinterpret_cast<grpc_service_config*>(
      gpr_malloc(sizeof(*service_config)));
...
grpc_slice_hash_table* grpc_service_config_create_method_config_table(
  ...
  for (grpc_json* field = json->child; field != nullptr; field = field->next) {
    ...
    if (strcmp(field->key, "methodConfig") == 0) {
      ...
      entries = reinterpret_cast<grpc_slice_hash_table_entry*>(
...
void* grpc_method_config_table_get(grpc_exec_ctx* exec_ctx,
  ...
  if (value == nullptr) {
    ...
    char* buf = reinterpret_cast<char*>(gpr_malloc(len + 2));  // '*' and NUL
