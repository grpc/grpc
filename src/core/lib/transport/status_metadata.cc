...
grpc_status_code grpc_get_status_code_from_metadata(grpc_mdelem md) {
  ...
  if (user_data != nullptr) {
    return static_cast<grpc_status_code>(reinterpret_cast<intptr_t>(user_data) - STATUS_OFFSET);
  }
  uint32_t status;
  if (!grpc_parse_slice_to_uint32(GRPC_MDVALUE(md), &status)) {
    status = GRPC_STATUS_UNKNOWN; /* could not parse status code */
  }
  grpc_mdelem_set_user_data(md, destroy_status, reinterpret_cast<void*>(status + STATUS_OFFSET));
...
grpc_mdelem grpc_get_reffed_status_elem_slowpath(int status_code) {
  ...
  return grpc_mdelem_from_slices(GRPC_MDSTR_GRPC_STATUS, grpc_core::UnmanagedMemorySlice(tmp));
}
