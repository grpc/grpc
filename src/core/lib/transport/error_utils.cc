...
void grpc_error_get_status(grpc_error_handle error,
  ...
  // Start with the parent error and recurse through the tree of
  // children until we find the first one that has a status code.
  ...
  if (found_error == GRPC_ERROR_NONE) {
    /// If no grpc-status exists, retry through the tree to find a http2
    /// error code
    found_error = recursively_find_error_with_field(
        error, GRPC_ERROR_INT_HTTP2_ERROR);
  }
  ...
  intptr_t integer;
  if (grpc_error_get_int(found_error, GRPC_ERROR_INT_GRPC_STATUS,
                         &integer)) {
  ...
  if (http_error != nullptr) {
    if (grpc_error_get_int(found_error, GRPC_ERROR_INT_HTTP2_ERROR,
                           &integer)) {
      *http_error = static_cast<grpc_http2_error_code>(integer);
    } else if (grpc_error_get_int(
                   found_error, GRPC_ERROR_INT_GRPC_STATUS, &integer)) {
      *http_error = grpc_status_to_http2_error(
          static_cast<grpc_status_code>(integer));
    } else {
      *http_error = found_error == GRPC_ERROR_NONE
                        ? GRPC_HTTP2_NO_ERROR
...
absl::Status grpc_error_to_absl_status(grpc_error_handle error) {
  ...
  // TODO(yashykt): This should be updated once we decide on how to use
  // the absl::Status payload to capture all the contents of grpc_error.
  grpc_slice message;
  ...
  return absl::Status(
      ...
      absl::string_view(
          reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(message)),
