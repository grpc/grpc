...
static grpc_error *
recursively_find_error_with_field (grpc_error * error, grpc_error_ints which)
{
  ...
  if (grpc_error_is_special (error))
    return nullptr;
...
void
grpc_error_get_status (grpc_error * error, grpc_millis deadline,
		       ...
		       const char **error_string)
{
  ...
  if (grpc_error_get_int (found_error, GRPC_ERROR_INT_GRPC_STATUS, &integer))
    ...
    {
      status =
	grpc_http2_error_to_grpc_status ((grpc_http2_error_code) integer,
					 deadline);
    }
  if (code != nullptr)
    *code = status;
  ...
  if (http_error != nullptr)
    {
	  ...
	  (found_error, GRPC_ERROR_INT_HTTP2_ERROR, &integer))
	{
	...
	{
	  *http_error =
	    grpc_status_to_http2_error ((grpc_status_code) integer);
	}
      else
	{
  ...
  if (slice != nullptr)
    {
	  ...
	  (found_error, GRPC_ERROR_STR_GRPC_MESSAGE, slice))
	{
	  if (!grpc_error_get_str
	      (found_error, GRPC_ERROR_STR_DESCRIPTION, slice))
	    {
...
bool
grpc_error_has_clear_grpc_status (grpc_error * error)
{
