...
static void
static_ref (void *unused)
{
}

static void
static_unref (void *unused)
{
}
...
grpc_mdelem
grpc_static_mdelem_for_static_strings (int a, int b)
{
  if (a == -1 || b == -1)
    return GRPC_MDNULL;
  ...
  return h < GPR_ARRAY_SIZE (elem_keys) && elem_keys[h] == k &&
    ...
    ? GRPC_MAKE_MDELEM (&grpc_static_mdelem_table[elem_idxs[h]],
			GRPC_MDELEM_STORAGE_STATIC) : GRPC_MDNULL;
}
