...
class AsyncConnectivityStateWatcherInterface::Notifier {
 ...
 private:
  ...
  GRPC_TRACE_LOG(connectivity_state, INFO)  << "watcher " << self->watcher_.get()                 << ": delivering async notification for "                 << ConnectivityStateName(self->state_) << " ("                 << self->status_.ToString() << ")";
...
ConnectivityStateTracker::~ConnectivityStateTracker() {
  ...
  GRPC_TRACE_LOG(connectivity_state, INFO)  << "ConnectivityStateTracker " << name_ << "[" << this                 << "]: notifying watcher " << p.first << ": "                 << ConnectivityStateName(current_state) << " -> "                 << ConnectivityStateName(GRPC_CHANNEL_SHUTDOWN);
...
void ConnectivityStateTracker::AddWatcher(
  ...
  GRPC_TRACE_LOG(connectivity_state, INFO)  << "ConnectivityStateTracker " << name_ << "[" << this                 << "]: notifying watcher " << watcher.get() << ": "                 << ConnectivityStateName(initial_state) << " -> "                 << ConnectivityStateName(current_state);
...
GRPC_TRACE_LOG(connectivity_state, INFO)  << "ConnectivityStateTracker " << name_ << "[" << this               << "]: " << ConnectivityStateName(current_state) << " -> "               << ConnectivityStateName(state) << " (" << reason << ", "               << status.ToString() << ")";
  state_.store(state, std::memory_order_relaxed);
  status_ = status;
  for (const auto& p : watchers_) {
  GRPC_TRACE_LOG(connectivity_state, INFO)  << "ConnectivityStateTracker " << name_ << "[" << this                 << "]: notifying watcher " << p.first << ": "                 << ConnectivityStateName(current_state) << " -> "                 << ConnectivityStateName(state);
