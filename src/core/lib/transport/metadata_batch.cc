...
static void
assert_valid_callouts (grpc_metadata_batch * batch)
{
#ifndef NDEBUG
...
grpc_error *
grpc_attach_md_to_error (grpc_error * src, grpc_mdelem md)
{
  grpc_error *out =
    grpc_error_set_str (grpc_error_set_str (src, GRPC_ERROR_STR_KEY,
...
static grpc_error *
maybe_link_callout (grpc_metadata_batch * batch, grpc_linked_mdelem * storage)
					    ...
					    grpc_linked_mdelem * storage)
{
  ...
  if (batch->idx.array[idx] == nullptr)
    {
      if (grpc_static_callout_is_default[idx])
  ...
  return
    grpc_attach_md_to_error (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			     ("Unallowed duplicate metadata"), storage->md);
}

static void
maybe_unlink_callout (grpc_metadata_batch * batch,
		      grpc_linked_mdelem * storage)
{
  grpc_metadata_batch_callouts_index idx =
    GRPC_BATCH_INDEX_OF (GRPC_MDKEY (storage->md));
...
static void
unlink_storage (grpc_mdelem_list * list, grpc_linked_mdelem * storage)
{
  assert_valid_list (list);
...
void
grpc_metadata_batch_set_value (grpc_linked_mdelem * storage, grpc_slice value)
{
  grpc_mdelem old_mdelem = storage->md;
  grpc_mdelem new_mdelem =
    grpc_mdelem_from_slices (grpc_slice_ref_internal
			     (GRPC_MDKEY (old_mdelem)), value);
  storage->md = new_mdelem;
  GRPC_MDELEM_UNREF (old_mdelem);
}

grpc_error *
grpc_metadata_batch_substitute (grpc_metadata_batch * batch,
				grpc_linked_mdelem * storage,
				grpc_mdelem new_mdelem)
{
  assert_valid_callouts (batch);
...
void
grpc_metadata_batch_clear (grpc_metadata_batch * batch)
{
...
bool
grpc_metadata_batch_is_empty (grpc_metadata_batch * batch)
{
...
size_t
grpc_metadata_batch_size (grpc_metadata_batch * batch)
{
  size_t size = 0;
...
static void
add_error (grpc_error ** composite, grpc_error * error,
	   const char *composite_error_string)
{
  ...
  if (*composite == GRPC_ERROR_NONE)
    {
      *composite =
	GRPC_ERROR_CREATE_FROM_COPIED_STRING (composite_error_string);
    }
