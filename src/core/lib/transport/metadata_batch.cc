...
static void assert_valid_list(grpc_mdelem_list* list) {
  ...
  GPR_ASSERT((list->head == list->tail) ==
...
#ifndef NDEBUG
  for (grpc_linked_mdelem* l = batch->list.head; l != nullptr;
       l = l->next) {
...
error_with_md(grpc_mdelem md) {
  return grpc_attach_md_to_error(GRPC_ERROR_CREATE_FROM_STATIC_STRING(
                                     "Unallowed duplicate metadata"),
                                 md);
}

static grpc_error_handle link_callout(
...
static grpc_error_handle maybe_link_callout(
    grpc_metadata_batch* batch, grpc_linked_mdelem* storage) {
  grpc_metadata_batch_callouts_index idx =
...
static void link_head(grpc_mdelem_list* list,
                      grpc_linked_mdelem* storage) {
...
// TODO(arjunroy): Need to revisit this and see what guarantees exist
// between C-core and the internal-metadata subsystem. E.g. can we
// ensure a particular metadata is never added twice, even in the
// presence of user supplied data?
grpc_error_handle grpc_metadata_batch_link_head(
    grpc_metadata_batch* batch, grpc_linked_mdelem* storage,
    grpc_metadata_batch_callouts_index idx) {
...
static void link_tail(grpc_mdelem_list* list,
                      grpc_linked_mdelem* storage) {
  assert_valid_list(list);
  GPR_DEBUG_ASSERT(!GRPC_MDISNULL(storage->md));
...
static void add_error(grpc_error_handle* composite,
