...
namespace grpc_core
{
  ...
  BdpEstimator::BdpEstimator (const char *name):ping_state_ (PingState::UNSCHEDULED), accumulator_ (0), estimate_ (65536), ping_start_time_ (gpr_time_0 (GPR_CLOCK_MONOTONIC)), inter_ping_delay_ (100.0),	// start at 100ms
    stable_estimate_count_ (0), bw_est_ (0), name_ (name)
  {
  }
  ...
  grpc_millis BdpEstimator::CompletePing ()
  {
	...
	if (stable_estimate_count_ >= 2)
	  {
	    inter_ping_delay_ += 100 + (int) (rand () * 100.0 / RAND_MAX);	// if the ping estimate is steady,
