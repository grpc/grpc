...
/* This file provides a mechanism for tracking metadata through the grpc
   stack. It's not intended for consumption outside of the library.

   Metadata is tracked in the context of a sharded global grpc_mdctx.
   The context tracks unique strings (grpc_mdstr) and pairs of strings
   (grpc_mdelem). Any of these objects can be checked for equality by
   comparing their pointers. These objects are reference counted.

   grpc_mdelem can additionally store a (non-NULL) user data pointer.
   This pointer is intended to be used to cache semantic meaning of a
   metadata element. For example, an OAuth token may cache the
   credentials it represents and the time at which it expires in the
   mdelem user data.

   Combining this metadata cache and the hpack compression table allows
   us to simply lookup complete preparsed objects quickly, incurring a
   few atomic ops per metadata element on the fast path.

   grpc_mdelem instances MAY live longer than their refcount implies,
   and are garbage collected periodically, meaning cached data can
   easily outlive a single request.

   STATIC METADATA: in static_metadata.h we declare a set of static
   metadata. These mdelems and mdstrs are available via pre-declared
   code generated macros and are available to code anywhere between
   grpc_init() and grpc_shutdown(). They are not refcounted, but can be
   passed to _ref and _unref functions declared here - in which case
   those functions are effectively no-ops. */
...
/* GRPC_MDELEM_STORAGE_* enum values that can be treated as interned
   always have this bit set in their integer value */
#define GRPC_MDELEM_STORAGE_INTERNED_BIT 1

/* External and static storage metadata has no refcount to ref/unref.
 * Allocated and interned metadata do have a refcount. Metadata ref and
 * unref methods use a switch statement on this enum to determine which
 * behaviour to execute. Keeping the no-ref cases together and the
 * ref-cases together leads to slightly better code generation (9
 * inlined instructions rather than 10). */
typedef enum {
  /* memory pointed to by grpc_mdelem::payload is owned by an external
     system */
  ...
  /* memory pointed to by grpc_mdelem::payload is allocated by the
     metadata system */
  GRPC_MDELEM_STORAGE_ALLOCATED = 2,
  /* memory pointed to by grpc_mdelem::payload is interned by the
     metadata system */
  GRPC_MDELEM_STORAGE_INTERNED = 2 | GRPC_MDELEM_STORAGE_INTERNED_BIT,
} grpc_mdelem_data_storage;

struct grpc_mdelem {
  /* a grpc_mdelem_data* generally, with the two lower bits signalling
     memory ownership as per grpc_mdelem_data_storage */
  uintptr_t payload;
};
...
#define GRPC_MDELEM_IS_INTERNED(md) \
  ((grpc_mdelem_data_storage)(      \
      (md).payload & (uintptr_t)GRPC_MDELEM_STORAGE_INTERNED_BIT))

/* Given arbitrary input slices, create a grpc_mdelem object. The caller
 * refs the input slices; we unref them. This method is always safe to
 * call; however, if we know data about the slices in question (e.g. if
 * we knew our key was
 * static) we can call specializations that save on cycle count. */
grpc_mdelem grpc_mdelem_from_slices(const grpc_slice& key,
                                    const grpc_slice& value);

/* Like grpc_mdelem_from_slices, but we know that key is a static slice.
   This saves us a few branches and a no-op call to md_unref() for the
   key. */
...
/* Cheaply convert a grpc_metadata to a grpc_mdelem; may use the
   grpc_metadata object as backing storage (so lifetimes should align)
 */
grpc_mdelem grpc_mdelem_from_grpc_metadata(grpc_metadata* metadata);

/* Does not unref the slices; if a new non-interned mdelem is needed,
   allocates one if compatible_external_backing_store is NULL, or uses
   compatible_external_backing_store if it is non-NULL (in which case
   it's the users responsibility to ensure that it outlives usage) */
...
/* Often we compare metadata where we know a-priori that the second
 * parameter is static, and that the keys match. This most commonly
 * happens when processing metadata batch callouts in initial/trailing
 * filters. In this case, fastpath
 * grpc_mdelem_eq and remove unnecessary checks. */
inline bool grpc_mdelem_static_value_eq(grpc_mdelem a,
...
/* Mutator and accessor for grpc_mdelem user data. The destructor
   function
...
class RefcountedMdBase {
 public:
  ...
  void Ref() {
    /* we can assume the ref count is >= 1 as the application is calling
       this function - meaning that no adjustment to mdtab_free is
       necessary, simplifying the logic here to be just an atomic
       increment */
 ...
 protected:
  ...
  bool AllRefsDropped() {
    return refcnt_.Load(MemoryOrder::ACQUIRE) == 0;
  }
  bool FirstRef() {
    return refcnt_.FetchAdd(1, MemoryOrder::RELAXED) == 0;
  }
...
inline grpc_mdelem grpc_mdelem_ref(grpc_mdelem gmd) {
  ...
  switch (GRPC_MDELEM_STORAGE(gmd)) {
    ...
    case GRPC_MDELEM_STORAGE_INTERNED: {
      auto* md = reinterpret_cast<grpc_core::InternedMetadata*>
          GRPC_MDELEM_DATA(gmd);
    ...
    case GRPC_MDELEM_STORAGE_ALLOCATED: {
      auto* md = reinterpret_cast<grpc_core::AllocatedMetadata*>
          GRPC_MDELEM_DATA(gmd);
#ifndef NDEBUG
      md->Ref(file, line);
#else
...
void grpc_mdelem_on_final_unref(grpc_mdelem_data_storage storage,
                                ...
                                const char* file, int line);
inline void grpc_mdelem_unref(grpc_mdelem gmd, const char* file,
                              int line) {
#else
...
inline void grpc_mdelem_unref(grpc_mdelem gmd) {
  ...
  switch (storage) {
    ...
    case GRPC_MDELEM_STORAGE_INTERNED:
    case GRPC_MDELEM_STORAGE_ALLOCATED:
      auto* md = reinterpret_cast<grpc_core::RefcountedMdBase*>
          GRPC_MDELEM_DATA(gmd);
      /* once the refcount hits zero, some other thread can come along
         and free an interned md at any time: it's unsafe from this
         point on to access it so we read the hash now. */
...
#define GRPC_MDELEM_LENGTH(e)           \
  (GRPC_SLICE_LENGTH(GRPC_MDKEY((e))) + \
   GRPC_SLICE_LENGTH(GRPC_MDVALUE((e))) + 32)
...
/* Like grpc_mdelem_from_slices, but we know that key is a static or
   interned slice and value is not static or interned. This gives us an
   inlinable fastpath - we know we must allocate metadata now, and that
   we do not need to unref the value (rather, we just transfer the ref).
   We can avoid a ref since: 1) the key slice is passed in already ref'd
