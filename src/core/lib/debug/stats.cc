...
void
grpc_stats_shutdown (void)
{
  gpr_free (grpc_stats_per_cpu_storage);
}

void
grpc_stats_collect (grpc_stats_data * output)
{
  ...
  for (size_t core = 0; core < g_num_cores; core++)
	...
	{
	  output->counters[i] +=
	    gpr_atm_no_barrier_load (&grpc_stats_per_cpu_storage[core].
				     counters[i]);
	}
...
void
grpc_stats_diff (const grpc_stats_data * b, const grpc_stats_data * a,
		 grpc_stats_data * c)
{
  for (size_t i = 0; i < GRPC_STATS_COUNTER_COUNT; i++)
    {
...
int
grpc_stats_histo_find_bucket_slow (int value, const int *table,
				   int table_size)
{
...
size_t
grpc_stats_histo_count (const grpc_stats_data * stats,
			grpc_stats_histograms histogram)
{
  ...
  for (int i = 0; i < grpc_stats_histo_buckets[histogram]; i++)
    {
      sum +=
	(size_t) stats->histograms[grpc_stats_histo_start[histogram] + i];
    }
  return sum;
}

static double
threshold_for_count_below (const gpr_atm * bucket_counts,
  ...
  if (count_so_far == count_below)
      ...
      for (upper_idx = lower_idx + 1; upper_idx < num_buckets; upper_idx++)
	{
      ...
      return (bucket_boundaries[lower_idx] +
	      bucket_boundaries[upper_idx]) / 2.0;
    }
  else
    {
	...
	(count_so_far - count_below) / (double) bucket_counts[lower_idx];
    }
}

double
grpc_stats_histo_percentile (const grpc_stats_data * stats,
			     grpc_stats_histograms histogram,
			     double percentile)
{
  ...
  if (count == 0)
    return 0.0;
  return threshold_for_count_below (stats->histograms +
				    grpc_stats_histo_start[histogram],
				    grpc_stats_histo_bucket_boundaries
				    [histogram],
				    grpc_stats_histo_buckets[histogram],
				    (double) count * percentile / 100.0);
}

char *
grpc_stats_data_as_json (const grpc_stats_data * data)
{
  gpr_strvec v;
  char *tmp;
  bool is_first = true;
