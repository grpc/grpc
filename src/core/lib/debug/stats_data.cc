...
Histogram_32768_24 operator-(const Histogram_32768_24& left, const Histogram_32768_24& right) {
...
namespace {
const int kStatsTable0[25] = {0,1,2,4,7,11,17,26,40,61,93,142,216,329,500,760,1155,1755,2667,4052,6155,9350,14203,21574,32768};
const uint8_t kStatsTable1[27] = {3,3,4,5,6,6,7,8,9,10,11,11,12,13,14,15,16,16,17,18,19,20,20,21,22,23,24};
const int kStatsTable2[21] = {0,1,3,8,19,45,106,250,588,1383,3252,7646,17976,42262,99359,233593,549177,1291113,3035402,7136218,16777216};
const uint8_t kStatsTable3[23] = {2,3,3,4,5,6,7,8,8,9,10,11,12,12,13,14,15,16,16,17,18,19,20};
const int kStatsTable4[11] = {0,1,2,4,7,11,17,26,38,56,80};
const uint8_t kStatsTable5[9] = {3,3,4,5,6,6,7,8,9};
}  // namespace
int Histogram_32768_24::BucketFor(int value) {if (value < 3) {
if (value < 0) {
return 0;
} else {
return value;
...
DblUint val;
val.dbl = value;
const int bucket = kStatsTable3[((val.uint - 4611686018427387904ull) >> 52)];
...
DblUint val;
val.dbl = value;
const int bucket = kStatsTable5[((val.uint - 4613937818241073152ull) >> 51)];
...
GlobalStats::GlobalStats() : client_calls_created{0},server_calls_created{0},client_channels_created{0},client_subchannels_created{0},server_channels_created{0},insecure_connections_created{0},syscall_write{0},syscall_read{0},tcp_read_alloc_8k{0},tcp_read_alloc_64k{0},http2_settings_writes{0},http2_pings_sent{0},http2_writes_begun{0},http2_transport_stalls{0},http2_stream_stalls{0},cq_pluck_creates{0},cq_next_creates{0},cq_callback_creates{0} {}
HistogramView GlobalStats::histogram(Histogram which) const {
  switch (which) {
    ...
    case Histogram::kCallInitialSize:
      return HistogramView{&Histogram_32768_24::BucketFor, kStatsTable0, 24, call_initial_size.buckets()};
    case Histogram::kTcpWriteSize:
      return HistogramView{&Histogram_16777216_20::BucketFor, kStatsTable2, 20, tcp_write_size.buckets()};
    case Histogram::kTcpWriteIovSize:
      return HistogramView{&Histogram_80_10::BucketFor, kStatsTable4, 10, tcp_write_iov_size.buckets()};
    case Histogram::kTcpReadSize:
      return HistogramView{&Histogram_16777216_20::BucketFor, kStatsTable2, 20, tcp_read_size.buckets()};
    case Histogram::kTcpReadOffer:
      return HistogramView{&Histogram_16777216_20::BucketFor, kStatsTable2, 20, tcp_read_offer.buckets()};
    case Histogram::kTcpReadOfferIovSize:
      return HistogramView{&Histogram_80_10::BucketFor, kStatsTable4, 10, tcp_read_offer_iov_size.buckets()};
    case Histogram::kHttp2SendMessageSize:
      return HistogramView{&Histogram_16777216_20::BucketFor, kStatsTable2, 20, http2_send_message_size.buckets()};
  }
}
