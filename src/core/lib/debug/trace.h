...
#define GRPC_RING_HASH_LB_LOG(level)         \
  (ring_hash_lb_trace.enabled() ? LOG(level) \
                                : NoDestructSingleton<LogSink>::Get())
...
#define GRPC_TIMER_CHECK_LOG(level)         \
  (timer_check_trace.enabled() ? LOG(level) \
                               : NoDestructSingleton<LogSink>::Get())
