...
static gpr_timespec to_seconds_from_sub_second_time(
    int64_t time_in_units, int64_t units_per_sec, gpr_clock_type type) {
  gpr_timespec out;
  if (time_in_units == INT64_MAX) {
    ...
    if (time_in_units >= 0) {
      ...
      out.tv_sec =
          (-((units_per_sec - 1) - (time_in_units + units_per_sec)) /
           units_per_sec) -
          1;
    }
    out.tv_nsec = static_cast<int32_t>(
        (time_in_units - out.tv_sec * units_per_sec) * GPR_NS_PER_SEC /
        units_per_sec);
    out.clock_type = type;
  }
  return out;
...
static gpr_timespec to_seconds_from_above_second_time(
    int64_t time_in_units, int64_t secs_per_unit, gpr_clock_type type) {
  gpr_timespec out;
...
gpr_timespec gpr_time_add(gpr_timespec a, gpr_timespec b) {
  ...
  // tv_nsec in a timespan is always +ve. -ve timespan is represented as
  // (-ve tv_sec, +ve tv_nsec). For example, timespan = -2.5 seconds is
  // represented as {-3, 5e8, GPR_TIMESPAN}
...
gpr_timespec gpr_time_sub(gpr_timespec a, gpr_timespec b) {
  ...
  if (b.clock_type == GPR_TIMESPAN) {
    ...
    // tv_nsec in a timespan is always +ve. -ve timespan is represented
    // as (-ve tv_sec, +ve tv_nsec). For example, timespan = -2.5
    // seconds is represented as {-3, 5e8, GPR_TIMESPAN}
    GPR_ASSERT(b.tv_nsec >= 0);
  } else {
    GPR_ASSERT(a.clock_type == b.clock_type);
...
int32_t gpr_time_to_millis(gpr_timespec t) {
  if (t.tv_sec >= 2147483) {
    ...
    /* TODO(ctiller): correct handling here (it's so far in the past do
       we care?) */
    return -2147483647;
  } else {
...
double gpr_timespec_to_micros(gpr_timespec t) {
  return static_cast<double>(t.tv_sec) * GPR_US_PER_SEC +
         t.tv_nsec * 1e-3;
}
...
gpr_timespec gpr_convert_clock_type(gpr_timespec t,
  ...
  // If the given input hits this code, the same result is not
  // guaranteed for the same input because it relies on `gpr_now` to
  // calculate the difference between two different clocks. Please be
  // careful when you want to use this function in unit tests. (e.g.
  // https://github.com/grpc/grpc/pull/22655)
