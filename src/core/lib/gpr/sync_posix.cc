...
void gpr_mu_init(gpr_mu* mu) {
  ...
  mu->leak_checker =
      static_cast<int*>(malloc(sizeof(*mu->leak_checker)));
...
void gpr_cv_init(gpr_cv* cv) {
  ...
  cv->leak_checker =
      static_cast<int*>(malloc(sizeof(*cv->leak_checker)));
...
#if GPR_LINUX
    abs_deadline =
        gpr_convert_clock_type(abs_deadline, GPR_CLOCK_MONOTONIC);
#else
    abs_deadline =
...
#ifdef GRPC_ASAN_ENABLED
    err = pthread_cond_timedwait(&cv->cond_var, &mu->mutex,
                                 &abs_deadline_ts);
#else
