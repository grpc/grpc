...
char *
gpr_dump (const char *buf, size_t len, uint32_t flags)
{
...
int
gpr_parse_bytes_to_uint32 (const char *buf, size_t len, uint32_t * result)
{
  uint32_t out = 0;
  uint32_t new_val;
  size_t i;
...
int
int64_ttoa (int64_t value, char *string)
{
  int64_t sign;
  int i = 0;
...
int
gpr_parse_nonnegative_int (const char *value)
{
  char *end;
  long result = strtol (value, &end, 0);
  if (*end != '\0' || result < 0 || result > INT_MAX)
    return -1;
  return (int) result;
}

char *
gpr_leftpad (const char *str, char flag, size_t length)
{
...
char *
gpr_strjoin_sep (const char **strs, size_t nstrs, const char *sep,
		 size_t * final_length)
{
  ...
  if (sv->count == sv->capacity)
    {
      ...
      sv->strs =
	(char **) gpr_realloc (sv->strs, sizeof (char *) * sv->capacity);
    }
...
int
gpr_stricmp (const char *a, const char *b)
{
  int ca, cb;
  do
    {
...
static void
add_string_to_split (const char *beg, const char *end, char ***strs,
...
void
gpr_string_split (const char *input, const char *sep, char ***strs,
		  size_t * nstrs)
{
  const char *next;
  *strs = nullptr;
...
void *
gpr_memrchr (const void *s, int c, size_t n)
{
  if (s == nullptr)
    return nullptr;
  char *b = (char *) s;
