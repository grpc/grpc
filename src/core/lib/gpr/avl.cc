...
gpr_avl
gpr_avl_create (const gpr_avl_vtable * vtable)
{
  gpr_avl out;
  out.vtable = vtable;
  out.root = nullptr;
...
static void
unref_node (const gpr_avl_vtable * vtable, gpr_avl_node * node,
	    void *user_data)
{
  if (node == nullptr)
    {
...
#ifndef NDEBUG
static long
calculate_height (gpr_avl_node * node)
{
...
static gpr_avl_node *
assert_invariants (gpr_avl_node * n)
{
  if (n == nullptr)
    return nullptr;
...
static gpr_avl_node *
get (const gpr_avl_vtable * vtable, gpr_avl_node * node,
...
void *
gpr_avl_get (gpr_avl avl, void *key, void *user_data)
{
...
int
gpr_avl_maybe_get (gpr_avl avl, void *key, void **value, void *user_data)
{
  gpr_avl_node *node = get (avl.vtable, avl.root, key, user_data);
  if (node != nullptr)
    {
...
static gpr_avl_node *
rotate_left (const gpr_avl_vtable * vtable, void *key,
...
static gpr_avl_node *
rotate_right (const gpr_avl_vtable * vtable, void *key,
	      void *value, gpr_avl_node * left,
	      gpr_avl_node * right, void *user_data)
{
  gpr_avl_node *n = new_node (vtable->copy_key (left->key, user_data),
			      vtable->copy_value (left->value, user_data),
			      ref_node (left->left),
			      new_node (key, value, ref_node (left->right),
					right));
...
static gpr_avl_node *
rotate_left_right (const gpr_avl_vtable * vtable, void *key,
		   void *value, gpr_avl_node * left,
		   gpr_avl_node * right, void *user_data)
{
  /* rotate_right(..., rotate_left(left), right) */
  gpr_avl_node *n = new_node (vtable->copy_key (left->right->key, user_data),
			      vtable->copy_value (left->right->value,
						  user_data),
			      new_node (vtable->
					copy_key (left->key, user_data),
					vtable->copy_value (left->value,
							    user_data),
					ref_node (left->left),
					ref_node (left->right->left)),
			      new_node (key, value,
...
static gpr_avl_node *
rotate_right_left (const gpr_avl_vtable * vtable, void *key,
		   void *value, gpr_avl_node * left,
		   gpr_avl_node * right, void *user_data)
{
  /* rotate_left(..., left, rotate_right(right)) */
  gpr_avl_node *n = new_node (vtable->copy_key (right->left->key, user_data),
			      vtable->copy_value (right->left->value,
						  user_data),
			      new_node (key, value, left,
					ref_node (right->left->left)),
			      new_node (vtable->
					copy_key (right->key, user_data),
					vtable->copy_value (right->value,
							    user_data),
					ref_node (right->left->right),
					ref_node (right->right)));
  unref_node (vtable, right, user_data);
  return n;
}
...
static gpr_avl_node *
rebalance (const gpr_avl_vtable * vtable, void *key,
	   void *value, gpr_avl_node * left,
	   gpr_avl_node * right, void *user_data)
{
  switch (node_height (left) - node_height (right))
	...
	{
	  return
	    assert_invariants (rotate_right
			       (vtable, key, value, left, right, user_data));
	}
    case -2:
...
static gpr_avl_node *
add_key (const gpr_avl_vtable * vtable, gpr_avl_node * node,
	 void *key, void *value, void *user_data)
{
  ...
  if (cmp == 0)
  ...
  else
    {
      return rebalance (vtable, vtable->copy_key (node->key, user_data),
			vtable->copy_value (node->value, user_data),
			ref_node (node->left), add_key (vtable, node->right,
							key, value,
							user_data),
			user_data);
    }
...
gpr_avl
gpr_avl_add (gpr_avl avl, void *key, void *value, void *user_data)
{
...
static gpr_avl_node *
in_order_head (gpr_avl_node * node)
{
...
static gpr_avl_node *
in_order_tail (gpr_avl_node * node)
{
...
static gpr_avl_node *
remove_key (const gpr_avl_vtable * vtable,
	    gpr_avl_node * node, void *key, void *user_data)
{
  long cmp;
  if (node == nullptr)
    {
  ...
  if (cmp == 0)
      ...
      else if (node->left->height < node->right->height)
	{
	  ...
	  return rebalance (vtable, vtable->copy_key (h->key, user_data),
			    ...
			    ref_node (node->left), remove_key (vtable,
							       node->right,
							       h->key,
							       user_data),
			    user_data);
      ...
      else
	{
	  ...
	  return rebalance (vtable, vtable->copy_key (h->key, user_data),
			    vtable->copy_value (h->value, user_data),
			    remove_key (vtable, node->left, h->key,
					user_data), ref_node (node->right),
			    user_data);
	}
    }
  else if (cmp > 0)
...
gpr_avl
gpr_avl_remove (gpr_avl avl, void *key, void *user_data)
{
  gpr_avl_node *old_root = avl.root;
...
gpr_avl
gpr_avl_ref (gpr_avl avl, void *user_data)
{
  ref_node (avl.root);
  return avl;
}
...
void
gpr_avl_unref (gpr_avl avl, void *user_data)
{
  unref_node (avl.vtable, avl.root, user_data);
}

int
gpr_avl_is_empty (gpr_avl avl)
{
  return avl.root == nullptr;
}
