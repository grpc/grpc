...
gpr_arena *
gpr_arena_create (size_t initial_size)
{
...
size_t
gpr_arena_destroy (gpr_arena * arena)
{
...
void *
gpr_arena_alloc (gpr_arena * arena, size_t size)
{
  ...
  while (start > z->size_end)
	...
	{
	  size_t next_z_size =
	    (size_t) gpr_atm_no_barrier_load (&arena->size_so_far);
	  next_z = (zone *) gpr_zalloc (sizeof (zone) + next_z_size);
	  next_z->size_begin = z->size_end;
	  next_z->size_end = z->size_end + next_z_size;
	  if (!gpr_atm_rel_cas
	      (&z->next_atm, (gpr_atm) NULL, (gpr_atm) next_z))
	    {
	      gpr_free (next_z);
