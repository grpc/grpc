...
static constexpr gpr_atm GPR_LOG_SEVERITY_UNSET =
    GPR_LOG_SEVERITY_ERROR + 10;
static constexpr gpr_atm GPR_LOG_SEVERITY_NONE =
    GPR_LOG_SEVERITY_ERROR + 11;
...
void gpr_log_message(const char* file, int line,
  ...
  if ((gpr_atm_no_barrier_load(&g_min_severity_to_print_stacktrace)) ==
    ...
    if (strlen(stacktrace_minloglevel.get()) > 0) {
      min_severity_to_print_stacktrace =
          parse_log_severity(stacktrace_minloglevel.get(),
                             min_severity_to_print_stacktrace);
    }
...
void gpr_set_log_function(gpr_log_func f) {
  gpr_atm_no_barrier_store(&g_log_func,
                           (gpr_atm)(f ? f : gpr_default_log));
}
