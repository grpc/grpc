...
static void plaintext_handshake(void* arg, grpc_endpoint* endpoint, const char* /*host*/,
                                grpc_millis /*deadline*/,
                                void (*on_done)(void* arg, grpc_endpoint* endpoint)) {
  on_done(arg, endpoint);
}

const grpc_httpcli_handshaker grpc_httpcli_plaintext = {"http", plaintext_handshake};
...
static void finish(internal_request* req, grpc_error_handle error) {
  grpc_polling_entity_del_from_pollset_set(req->pollent, req->context->pollset_set);
...
static void append_error(internal_request* req, grpc_error_handle error) {
  if (req->overall_error == GRPC_ERROR_NONE) {
    req->overall_error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("Failed HTTP/1 client request");
  }
  grpc_resolved_address* addr = &req->addresses->addrs[req->next_address - 1];
  std::string addr_text = grpc_sockaddr_to_uri(addr);
  req->overall_error = grpc_error_add_child(
      req->overall_error, grpc_error_set_str(error, GRPC_ERROR_STR_TARGET_ADDRESS,
...
static void on_handshake_done(void* arg, grpc_endpoint* ep) {
  ...
  if (!ep) {
    next_address(req, GRPC_ERROR_CREATE_FROM_STATIC_STRING("Unexplained handshake failure"));
    return;
  }
...
static void on_connected(void* arg, grpc_error_handle error) {
  ...
  req->handshaker->handshake(req, req->ep,
...
static void next_address(internal_request* req, grpc_error_handle error) {
  ...
  if (req->next_address == req->addresses->naddrs) {
    finish(req, GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING(
                    "Failed HTTP requests to all targets", &req->overall_error, 1));
    return;
  }
  addr = &req->addresses->addrs[req->next_address++];
  GRPC_CLOSURE_INIT(&req->connected, on_connected, req, grpc_schedule_on_exec_ctx);
  grpc_arg arg =
      grpc_channel_arg_pointer_create(const_cast<char*>(GRPC_ARG_RESOURCE_QUOTA),
                                      req->resource_quota, grpc_resource_quota_arg_vtable());
  grpc_channel_args args = {1, &arg};
  grpc_tcp_client_connect(&req->connected, &req->ep, req->context->pollset_set, &args, addr,
                          req->deadline);
}
...
static void internal_request_begin(grpc_httpcli_context* context, grpc_polling_entity* pollent,
                                   grpc_resource_quota* resource_quota,
                                   const grpc_httpcli_request* request, grpc_millis deadline,
                                   grpc_closure* on_done, grpc_httpcli_response* response,
                                   const char* name, const grpc_slice& request_text) {
  internal_request* req = static_cast<internal_request*>(gpr_malloc(sizeof(internal_request)));
  ...
  GRPC_CLOSURE_INIT(&req->done_write, done_write, req, grpc_schedule_on_exec_ctx);
  ...
  GPR_ASSERT(pollent);
  grpc_polling_entity_add_to_pollset_set(req->pollent, req->context->pollset_set);
  grpc_resolve_address(request->host, req->handshaker->default_port, req->context->pollset_set,
...
void grpc_httpcli_get(grpc_httpcli_context* context, grpc_polling_entity* pollent,
  ...
  std::string name = absl::StrFormat("HTTP:GET:%s:%s", request->host, request->http.path);
  internal_request_begin(context, pollent, resource_quota, request, deadline, on_done, response,
                         name.c_str(), grpc_httpcli_format_get_request(request));
}
...
void grpc_httpcli_post(grpc_httpcli_context* context, grpc_polling_entity* pollent,
  ...
  std::string name = absl::StrFormat("HTTP:POST:%s:%s", request->host, request->http.path);
  internal_request_begin(context, pollent, resource_quota, request, deadline, on_done, response,
                         name.c_str(),
