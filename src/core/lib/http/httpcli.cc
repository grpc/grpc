...
static void
plaintext_handshake (void *arg, grpc_endpoint * endpoint,
...
static void
finish (internal_request * req, grpc_error * error)
{
...
static void
append_error (internal_request * req, grpc_error * error)
{
  ...
  req->overall_error = grpc_error_add_child (req->overall_error,
...
static void
do_read (internal_request * req)
{
  grpc_endpoint_read (req->ep, &req->incoming, &req->on_read);
}

static void
on_read (void *user_data, grpc_error * error)
{
...
static void
on_written (internal_request * req)
{
  do_read (req);
}
...
static void
start_write (internal_request * req)
{
...
static void
on_handshake_done (void *arg, grpc_endpoint * ep)
{
  internal_request *req = (internal_request *) arg;
...
static void
on_connected (void *arg, grpc_error * error)
{
  internal_request *req = (internal_request *) arg;
  ...
  req->handshaker->handshake (req, req->ep,
			      req->ssl_host_override ? req->
			      ssl_host_override : req->host, req->deadline,
			      on_handshake_done);
}

static void
next_address (internal_request * req, grpc_error * error)
{
  ...
  if (req->next_address == req->addresses->naddrs)
    {
      finish (req,
	      ...
	      ("Failed HTTP requests to all targets", &req->overall_error,
	       1));
      return;
    }
  ...
  grpc_arg arg = grpc_channel_arg_pointer_create ((char *)
...
static void
on_resolved (void *arg, grpc_error * error)
{
  internal_request *req = (internal_request *) arg;
  if (error != GRPC_ERROR_NONE)
    {
...
static void
internal_request_begin (grpc_httpcli_context * context,
			grpc_polling_entity * pollent,
			grpc_resource_quota * resource_quota,
			...
			const char *name, grpc_slice request_text)
{
  ...
  grpc_resolve_address (request->host, req->handshaker->default_port,
			req->context->pollset_set,
			GRPC_CLOSURE_CREATE (on_resolved, req,
					     grpc_schedule_on_exec_ctx),
			&req->addresses);
}

void
grpc_httpcli_get (grpc_httpcli_context * context,
		  grpc_polling_entity * pollent,
		  grpc_resource_quota * resource_quota,
...
void
grpc_httpcli_post (grpc_httpcli_context * context,
		   grpc_polling_entity * pollent,
		   grpc_resource_quota * resource_quota,
		   const grpc_httpcli_request * request,
		   ...
		   grpc_httpcli_response * response)
{
  ...
  internal_request_begin (context, pollent, resource_quota, request, deadline,
			  on_done, response, name,
			  grpc_httpcli_format_post_request (request,
							    body_bytes,
							    body_size));
  gpr_free (name);
}
...
void
grpc_httpcli_set_override (grpc_httpcli_get_override get,
