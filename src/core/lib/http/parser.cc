...
static grpc_error *
handle_response_line (grpc_http_parser * parser)
{
...
static grpc_error *
handle_request_line (grpc_http_parser * parser)
{
  uint8_t *beg = parser->cur_line;
  uint8_t *cur = beg;
  uint8_t *end = beg + parser->cur_line_length;
  ...
  if (cur == end)
    return
      GRPC_ERROR_CREATE_FROM_STATIC_STRING ("No method on HTTP request line");
  ...
  if (cur == end)
    return
      GRPC_ERROR_CREATE_FROM_STATIC_STRING ("No path on HTTP request line");
...
static grpc_error *
handle_first_line (grpc_http_parser * parser)
{
  switch (parser->type)
    {
...
static grpc_error *
add_header (grpc_http_parser * parser)
{
  uint8_t *beg = parser->cur_line;
  uint8_t *cur = beg;
  uint8_t *end = beg + parser->cur_line_length;
  ...
  hdr.value =
    buf2str (cur, (size_t) (end - cur) - parser->cur_line_end_length);
  ...
  if (*hdr_count == parser->hdr_capacity)
	...
	GPR_MAX (parser->hdr_capacity + 1, parser->hdr_capacity * 3 / 2);
      *hdrs =
	(grpc_http_header *) gpr_realloc (*hdrs,
					  parser->hdr_capacity *
					  sizeof (**hdrs));
    }
  (*hdrs)[(*hdr_count)++] = hdr;
...
static grpc_error *
finish_line (grpc_http_parser * parser, bool * found_body_start)
{
  grpc_error *err;
  switch (parser->state)
    {
...
static grpc_error *
addbyte_body (grpc_http_parser * parser, uint8_t byte)
{
...
static bool
check_line (grpc_http_parser * parser)
{
...
static grpc_error *
addbyte (grpc_http_parser * parser, uint8_t byte, bool * found_body_start)
{
  switch (parser->state)
    {
    case GRPC_HTTP_FIRST_LINE:
...
void
grpc_http_parser_init (grpc_http_parser * parser, grpc_http_type type,
...
void
grpc_http_parser_destroy (grpc_http_parser * parser)
{
}

void
grpc_http_request_destroy (grpc_http_request * request)
{
  size_t i;
...
void
grpc_http_response_destroy (grpc_http_response * response)
{
  size_t i;
...
grpc_error *
grpc_http_parser_parse (grpc_http_parser * parser, grpc_slice slice,
...
grpc_error *
grpc_http_parser_eof (grpc_http_parser * parser)
{
