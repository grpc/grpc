...
static grpc_error_handle handle_response_line(grpc_http_parser* parser) {
  ...
  if (cur == end || *cur < '0' || *cur++ > '1') {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Expected HTTP/1.0 or HTTP/1.1");
  }
  if (cur == end || *cur++ != ' ') {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Expected ' '");
  }
  ...
  parser->http.response->status = (cur[-3] - '0') * 100 + (cur[-2] - '0') * 10 + (cur[-1] - '0');
  if (cur == end || *cur++ != ' ') {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Expected ' '");
  }
...
static grpc_error_handle handle_request_line(grpc_http_parser* parser) {
  ...
  if (cur == end) {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("No method on HTTP request line");
  }
  ...
  ++cur;
  if (cur == end) {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("End of line in HTTP version string");
  }
  ...
  if (vers_major == 1) {
    ...
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Expected one of HTTP/1.0, HTTP/1.1, or HTTP/2.0");
  }
...
static grpc_error_handle handle_first_line(grpc_http_parser* parser) {
  ...
  GPR_UNREACHABLE_CODE(return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Should never reach here"));
}
...
static grpc_error_handle add_header(grpc_http_parser* parser) {
  ...
  if (*cur == ' ' || *cur == '\t') {
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("Continued header lines not supported yet");
    goto done;
  }
  ...
  if (cur == end) {
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("Didn't find ':' in header string");
    goto done;
  }
  GPR_ASSERT(cur >= beg);
  ...
  if (*hdr_count == parser->hdr_capacity) {
    ...
    *hdrs =
        static_cast<grpc_http_header*>(gpr_realloc(*hdrs, parser->hdr_capacity * sizeof(**hdrs)));
  }
  (*hdrs)[(*hdr_count)++] = hdr;
...
static bool check_line(grpc_http_parser* parser) {
  if (parser->cur_line_length >= 2 && parser->cur_line[parser->cur_line_length - 2] == '\r' &&
      parser->cur_line[parser->cur_line_length - 1] == '\n') {
    return true;
  }

  // HTTP request with \n\r line termiantors.
  else if (parser->cur_line_length >= 2 && parser->cur_line[parser->cur_line_length - 2] == '\n' &&
           parser->cur_line[parser->cur_line_length - 1] == '\r') {
    return true;
  }

  // HTTP request with only \n line terminators.
  else if (parser->cur_line_length >= 1 && parser->cur_line[parser->cur_line_length - 1] == '\n') {
  ...
  switch (parser->state) {
    ...
    case GRPC_HTTP_HEADERS:
      if (parser->cur_line_length >= GRPC_HTTP_PARSER_MAX_HEADER_LENGTH) {
        ...
        return GRPC_ERROR_CREATE_FROM_STATIC_STRING("HTTP header max line length exceeded");
      }
...
grpc_error_handle grpc_http_parser_parse(grpc_http_parser* parser, const grpc_slice& slice,
  ...
  for (size_t i = 0; i < GRPC_SLICE_LENGTH(slice); i++) {
    ...
    grpc_error_handle err = addbyte(parser, GRPC_SLICE_START_PTR(slice)[i], &found_body_start);
    if (err != GRPC_ERROR_NONE) return err;
