...
class grpc_httpcli_ssl_channel_security_connector final : public grpc_channel_security_connector {
 public:
  ...
  tsi_result InitHandshakerFactory(const char* pem_root_certs,
    ...
    return tsi_create_ssl_client_handshaker_factory_with_options(&options, &handshaker_factory_);
  }
  ...
  void add_handshakers(const grpc_channel_args* args, grpc_pollset_set* /*interested_parties*/,
    ...
    handshake_mgr->Add(grpc_core::SecurityHandshakerCreate(handshaker, this, args));
  }

  tsi_ssl_client_handshaker_factory* handshaker_factory() const { return handshaker_factory_; }
  ...
  void check_peer(tsi_peer peer, grpc_endpoint* /*ep*/,
    ...
    /* Check the peer name. */
    if (secure_peer_name_ != nullptr && !tsi_ssl_peer_matches_name(&peer, secure_peer_name_)) {
      error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
          absl::StrCat("Peer name ", secure_peer_name_, " is not in peer certificate").c_str());
    }
  ...
  int cmp(const grpc_security_connector* other_sc) const override {
    auto* other = reinterpret_cast<const grpc_httpcli_ssl_channel_security_connector*>(other_sc);
...
httpcli_ssl_channel_security_connector_create(const char* pem_root_certs,
                                              const tsi_ssl_root_certs_store* root_store,
...
static void on_handshake_done(void* arg, grpc_error_handle error) {
  ...
  if (error != GRPC_ERROR_NONE) {
    gpr_log(GPR_ERROR, "Secure transport setup failed: %s", grpc_error_std_string(error).c_str());
...
static void ssl_handshake(void* arg, grpc_endpoint* tcp, const char* host, grpc_millis deadline,
                          void (*on_done)(void* arg, grpc_endpoint* endpoint)) {
  auto* c = new on_done_closure();
  const char* pem_root_certs = grpc_core::DefaultSslRootStore::GetPemRootCerts();
  const tsi_ssl_root_certs_store* root_store = grpc_core::DefaultSslRootStore::GetRootStore();
  ...
  grpc_core::RefCountedPtr<grpc_channel_security_connector> sc =
      httpcli_ssl_channel_security_connector_create(
          pem_root_certs, root_store, host, static_cast<grpc_core::HandshakerArgs*>(arg)->args);
  ...
  grpc_core::HandshakerRegistry::AddHandshakers(grpc_core::HANDSHAKER_CLIENT, &args,
