 ...
 public:
  ...
  class Builder {
   public:
    ...
    static void SetDefaultPrefix(std::string default_prefix);
  ...
  static std::string AddDefaultPrefixIfNeeded(absl::string_view target);

  /// Returns the resolver factory for \a scheme.
  /// Caller does NOT own the return value.
  static ResolverFactory* LookupResolverFactory(absl::string_view scheme);

 private:
  static ResolverFactory* FindResolverFactory(absl::string_view target,
                                              URI* uri);
};
