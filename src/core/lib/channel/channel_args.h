...
// Channel args are intentionally immutable, to avoid the need for
// locking.

/** Copy the arguments in \a src into a new instance */
grpc_channel_args* grpc_channel_args_copy(const grpc_channel_args* src);

/** Copy the arguments in \a src into a new instance, stably sorting
 * keys */
grpc_channel_args* grpc_channel_args_normalize(
    const grpc_channel_args* src);

/** Copy the arguments in \a src and append \a to_add. If \a to_add is
 * NULL, it is equivalent to calling \a grpc_channel_args_copy. */
...
/** Returns the value of argument \a name from \a args, or NULL if not
 * found. */
...
typedef struct grpc_integer_options {
  int default_value;  // Return this if value is outside of expected
                      // bounds.
  int min_value;
  int max_value;
} grpc_integer_options;

/** Returns the value of \a arg, subject to the constraints in \a
 * options. */
int grpc_channel_arg_get_integer(const grpc_arg* arg,
                                 const grpc_integer_options options);
/** Similar to the above, but needs to find the arg from \a args by the
 * name first. */
...
char* grpc_channel_args_find_string(const grpc_channel_args* args,
                                    const char* name);
/** If \a arg is of type GRPC_ARG_INTEGER, returns true if it's
 * non-zero. Returns \a default_value if \a arg is of other types. */
