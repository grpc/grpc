...
static int cmp_arg(const grpc_arg* a, const grpc_arg* b) {
  int c = grpc_core::icmp(a->type, b->type);
  if (c != 0) return c;
  ...
  switch (a->type) {
    ...
    case GRPC_ARG_INTEGER:
      return grpc_core::icmp(a->value.integer, b->value.integer);
    case GRPC_ARG_POINTER:
      c = grpc_core::icmp(a->value.pointer.p, b->value.pointer.p);
      if (c != 0) {
        c = grpc_core::icmp(a->value.pointer.vtable, b->value.pointer.vtable);
        if (c == 0) {
...
int grpc_channel_args_compare(const grpc_channel_args* a,
  ...
  int c = grpc_core::icmp(a->num_args, b->num_args);
  if (c != 0) return c;
