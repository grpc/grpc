...
grpc_channel_args *
grpc_channel_args_copy_and_remove (const grpc_channel_args * src,
				   const char **to_remove,
				   size_t num_to_remove)
{
  return grpc_channel_args_copy_and_add_and_remove (src, to_remove,
						    num_to_remove, nullptr,
						    0);
}

static bool
should_remove_arg (const grpc_arg * arg, const char **to_remove,
		   size_t num_to_remove)
{
  for (size_t i = 0; i < num_to_remove; ++i)
    {
      if (strcmp (arg->key, to_remove[i]) == 0)
	return true;
    }
  return false;
}
...
grpc_channel_args *
grpc_channel_args_copy_and_add_and_remove (const grpc_channel_args * src,
					   const char **to_remove,
					   size_t num_to_remove,
...
grpc_channel_args *
grpc_channel_args_union (const grpc_channel_args * a,
...
static int
cmp_arg (const grpc_arg * a, const grpc_arg * b)
{
...
static int
cmp_key_stable (const void *ap, const void *bp)
{
...
grpc_channel_args *
grpc_channel_args_normalize (const grpc_channel_args * a)
{
  grpc_arg **args =
    (grpc_arg **) gpr_malloc (sizeof (grpc_arg *) * a->num_args);
  for (size_t i = 0; i < a->num_args; i++)
    {
      args[i] = &a->args[i];
    }
...
grpc_channel_args_destroy (grpc_channel_args * a)
{
  ...
  for (i = 0; i < a->num_args; i++)
	...
	case GRPC_ARG_INTEGER:
	  break;
	case GRPC_ARG_POINTER:
	  a->args[i].value.pointer.vtable->destroy (a->args[i].value.pointer.
						    p);
	  break;
	}
 ...
 * 0. */
static int
find_stream_compression_algorithm_states_bitset (const grpc_channel_args * a,
						 int **states_arg)
{
  if (a != nullptr)
    {
  ...
  if (grpc_channel_args_get_compression_algorithm (*a) == algorithm &&
      state == 0)
    {
      ...
      gpr_log (GPR_ERROR,
	       ...
	       "operation has been ignored.", algo_name);
    }
							  ...
							  int state)
{
  ...
  if (grpc_channel_args_get_stream_compression_algorithm (*a) == algorithm &&
  ...
  else
    {
      ...
      tmp.type = GRPC_ARG_INTEGER;
      tmp.key =
	(char *) GRPC_STREAM_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET;
      /* all enabled by default */
...
uint32_t
grpc_channel_args_stream_compression_algorithm_get_states (const
							   grpc_channel_args *
							   a)
{
  int *states_arg;
...
grpc_channel_args *
grpc_channel_args_set_socket_mutator (grpc_channel_args * a,
...
int
grpc_channel_args_compare (const grpc_channel_args * a,
			   const grpc_channel_args * b)
{
...
const grpc_arg *
grpc_channel_args_find (const grpc_channel_args * args, const char *name)
{
  if (args != nullptr)
    {
...
int
grpc_channel_arg_get_integer (const grpc_arg * arg,
...
bool
grpc_channel_arg_get_bool (const grpc_arg * arg, bool default_value)
{
  if (arg == nullptr)
...
bool
grpc_channel_args_want_minimal_stack (const grpc_channel_args * args)
{
  return
    grpc_channel_arg_get_bool (grpc_channel_args_find
			       (args, GRPC_ARG_MINIMAL_STACK), false);
}

grpc_arg
grpc_channel_arg_string_create (char *name, char *value)
{
  grpc_arg arg;
...
grpc_arg
grpc_channel_arg_integer_create (char *name, int value)
{
  grpc_arg arg;
...
grpc_arg
grpc_channel_arg_pointer_create (char *name, void *value,
