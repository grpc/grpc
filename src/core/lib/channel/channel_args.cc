...
grpc_channel_args* grpc_channel_args_copy_and_add_and_remove(
  ...
  // Create result.
  grpc_channel_args* dst = reinterpret_cast<grpc_channel_args*>(
  ...
  dst->args =
      reinterpret_cast<grpc_arg*>(gpr_malloc(sizeof(grpc_arg) * dst->num_args));
...
grpc_channel_args* grpc_channel_args_union(const grpc_channel_args* a,
  ...
  grpc_arg* uniques =
      reinterpret_cast<grpc_arg*>(gpr_malloc(sizeof(*uniques) * max_out));
...
static int cmp_key_stable(const void* ap, const void* bp) {
  const grpc_arg* const* a = reinterpret_cast<const grpc_arg* const*>(ap);
  const grpc_arg* const* b = reinterpret_cast<const grpc_arg* const*>(bp);
...
grpc_channel_args* grpc_channel_args_normalize(const grpc_channel_args* a) {
  grpc_arg** args =
      reinterpret_cast<grpc_arg**>(gpr_malloc(sizeof(grpc_arg*) * a->num_args));
  ...
  grpc_channel_args* b = reinterpret_cast<grpc_channel_args*>(
      gpr_malloc(sizeof(grpc_channel_args)));
  b->num_args = a->num_args;
  b->args =
      reinterpret_cast<grpc_arg*>(gpr_malloc(sizeof(grpc_arg) * b->num_args));
  for (size_t i = 0; i < a->num_args; i++) {
  ...
  for (i = 0; i < a->num_args; ++i) {
    if (a->args[i].type == GRPC_ARG_INTEGER &&
        !strcmp(GRPC_STREAM_COMPRESSION_CHANNEL_DEFAULT_ALGORITHM,
                a->args[i].key)) {
      return static_cast<grpc_stream_compression_algorithm>(
