...
grpc_channel_stack_builder* grpc_channel_stack_builder_create(void) {
  grpc_channel_stack_builder* b = static_cast<grpc_channel_stack_builder*>(gpr_zalloc(sizeof(*b)));
...
grpc_channel_stack_builder_iterator* grpc_channel_stack_builder_create_iterator_at_first(
    grpc_channel_stack_builder* builder) {
  return create_iterator_at_filter_node(builder, &builder->begin);
}

grpc_channel_stack_builder_iterator* grpc_channel_stack_builder_create_iterator_at_last(
    grpc_channel_stack_builder* builder) {
...
grpc_channel_stack_builder_iterator* grpc_channel_stack_builder_iterator_find(
  ...
  while (grpc_channel_stack_builder_move_next(it)) {
    ...
    const char* filter_name_at_it = grpc_channel_stack_builder_iterator_filter_name(it);
...
bool grpc_channel_stack_builder_append_filter(grpc_channel_stack_builder* builder,
  ...
  bool ok = grpc_channel_stack_builder_add_filter_before(it, filter, post_init_func, user_data);
...
bool grpc_channel_stack_builder_prepend_filter(grpc_channel_stack_builder* builder,
  ...
  grpc_channel_stack_builder_iterator* it =
      grpc_channel_stack_builder_create_iterator_at_first(builder);
  bool ok = grpc_channel_stack_builder_add_filter_after(it, filter, post_init_func, user_data);
  grpc_channel_stack_builder_iterator_destroy(it);
  return ok;
}
...
grpc_error_handle grpc_channel_stack_builder_finish(grpc_channel_stack_builder* builder,
  ...
  *result = gpr_zalloc(prefix_bytes + channel_stack_size);
  ...
  grpc_error_handle error = grpc_channel_stack_init(
      initial_refs, destroy, destroy_arg == nullptr ? *result : destroy_arg, filters, num_filters,
  ...
  if (error != GRPC_ERROR_NONE) {
    ...
    *result = nullptr;
    ...
    for (filter_node* p = builder->begin.next; p != &builder->end; p = p->next) {
      if (p->init != nullptr) {
        p->init(channel_stack, grpc_channel_stack_element(channel_stack, i), p->init_arg);
      }
      i++;
    }
  }
