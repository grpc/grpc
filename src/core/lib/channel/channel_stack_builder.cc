...
grpc_channel_stack_builder_create (void)
{
...
void
grpc_channel_stack_builder_set_target (grpc_channel_stack_builder * b,
				       const char *target)
{
  gpr_free (b->target);
...
const char *
grpc_channel_stack_builder_get_target (grpc_channel_stack_builder * b)
{
  return b->target;
}
...
grpc_channel_stack_builder_iterator *
grpc_channel_stack_builder_create_iterator_at_last (grpc_channel_stack_builder
						    * builder)
{
...
grpc_channel_stack_builder_iterator_filter_name
  (grpc_channel_stack_builder_iterator * iterator)
{
  if (iterator->node->filter == nullptr)
    return nullptr;
  return iterator->node->filter->name;
}

bool
grpc_channel_stack_builder_move_next (grpc_channel_stack_builder_iterator *
				      iterator)
{
  if (iterator->node == &iterator->builder->end)
    return false;
...
bool
grpc_channel_stack_builder_move_prev (grpc_channel_stack_builder_iterator *
				      iterator)
{
  if (iterator->node == &iterator->builder->begin)
    return false;
...
grpc_channel_stack_builder_iterator *
grpc_channel_stack_builder_iterator_find (grpc_channel_stack_builder *
					  builder, const char *filter_name)
{
  ...
  while (grpc_channel_stack_builder_move_next (it))
	...
	grpc_channel_stack_builder_iterator_filter_name (it);
      if (strcmp (filter_name, filter_name_at_it) == 0)
	break;
    }
  return it;
}

bool grpc_channel_stack_builder_move_prev (grpc_channel_stack_builder_iterator
					   * iterator);

void
grpc_channel_stack_builder_set_name (grpc_channel_stack_builder * builder,
				     const char *name)
{
...
void
grpc_channel_stack_builder_set_channel_arguments (grpc_channel_stack_builder *
						  builder,
...
void
grpc_channel_stack_builder_set_transport (grpc_channel_stack_builder *
					  builder, grpc_transport * transport)
{
...
grpc_transport *
grpc_channel_stack_builder_get_transport (grpc_channel_stack_builder *
					  builder)
{
...
bool
grpc_channel_stack_builder_append_filter (grpc_channel_stack_builder *
					  builder,
					  const grpc_channel_filter * filter,
					  grpc_post_filter_create_init_func
					  post_init_func, void *user_data)
{
  grpc_channel_stack_builder_iterator *it =
    grpc_channel_stack_builder_create_iterator_at_last (builder);
  bool ok =
    grpc_channel_stack_builder_add_filter_before (it, filter, post_init_func,
						  user_data);
...
bool
grpc_channel_stack_builder_remove_filter (grpc_channel_stack_builder *
					  builder, const char *filter_name)
{
  grpc_channel_stack_builder_iterator *it =
...
bool
grpc_channel_stack_builder_prepend_filter (grpc_channel_stack_builder *
					   builder,
					   const grpc_channel_filter * filter,
					   grpc_post_filter_create_init_func
					   post_init_func, void *user_data)
{
  grpc_channel_stack_builder_iterator *it =
    grpc_channel_stack_builder_create_iterator_at_first (builder);
  bool ok =
    grpc_channel_stack_builder_add_filter_after (it, filter, post_init_func,
						 user_data);
  grpc_channel_stack_builder_iterator_destroy (it);
  return ok;
}
...
static void
add_after (filter_node * before, const grpc_channel_filter * filter,
...
void
grpc_channel_stack_builder_destroy (grpc_channel_stack_builder * builder)
{
...
grpc_error *
grpc_channel_stack_builder_finish (grpc_channel_stack_builder * builder,
				   ...
				   void *destroy_arg, void **result)
{
  ...
  const grpc_channel_filter **filters =
    (const grpc_channel_filter **) gpr_malloc (sizeof (*filters) *
					       num_filters);
  size_t i = 0;
  for (filter_node * p = builder->begin.next; p != &builder->end; p = p->next)
    {
  ...
  *result = gpr_zalloc (prefix_bytes + channel_stack_size);
  ...
  // and initialize it
  grpc_error *error =
    grpc_channel_stack_init (initial_refs, destroy,
			     destroy_arg == nullptr ? *result : destroy_arg,
			     filters, num_filters, builder->args,
			     builder->transport, builder->name,
			     channel_stack);
