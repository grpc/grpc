...
size_t
grpc_channel_stack_size (const grpc_channel_filter ** filters,
			 size_t filter_count)
{
...
grpc_error *
grpc_channel_stack_init (int initial_refs, grpc_iomgr_cb_func destroy,
			 void *destroy_arg,
			 ...
			 const char *name, grpc_channel_stack * stack)
{
  ...
  for (i = 0; i < filter_count; i++)
      ...
      if (error != GRPC_ERROR_NONE)
	{
      ...
      user_data +=
	ROUND_UP_TO_ALIGNMENT_SIZE (filters[i]->sizeof_channel_data);
...
void
grpc_channel_stack_destroy (grpc_channel_stack * stack)
{
...
grpc_error *
grpc_call_stack_init (grpc_channel_stack * channel_stack,
		      int initial_refs, grpc_iomgr_cb_func destroy,
		      void *destroy_arg,
		      const grpc_call_element_args * elem_args)
{
  grpc_channel_element *channel_elems =
  ...
  GRPC_STREAM_REF_INIT (&elem_args->call_stack->refcount, initial_refs,
			destroy, destroy_arg, "CALL_STACK");
...
void
grpc_call_stack_set_pollset_or_pollset_set (grpc_call_stack * call_stack,
					    grpc_polling_entity * pollent)
{
  ...
  for (i = 0; i < count; i++)
    {
      call_elems[i].filter->set_pollset_or_pollset_set (&call_elems[i],
							pollent);
      user_data +=
	ROUND_UP_TO_ALIGNMENT_SIZE (call_elems[i].filter->sizeof_call_data);
    }
...
void
grpc_call_stack_ignore_set_pollset_or_pollset_set (grpc_call_element * elem,
						   grpc_polling_entity *
						   pollent)
{
}
...
void
grpc_call_stack_destroy (grpc_call_stack * stack,
  ...
  for (i = 0; i < count; i++)
    {
      elems[i].filter->destroy_call_elem (&elems[i], final_info,
					  i ==
					  count -
					  1 ? then_schedule_closure :
					  nullptr);
    }
}

void
grpc_call_next_op (grpc_call_element * elem,
...
void
grpc_channel_next_get_info (grpc_channel_element * elem,
...
grpc_channel_stack_from_top_element (grpc_channel_element * elem)
{
  return (grpc_channel_stack *) ((char *) (elem) -
				 ROUND_UP_TO_ALIGNMENT_SIZE (sizeof
							     (grpc_channel_stack)));
}
...
{
  return (grpc_call_stack *) ((char *) (elem) -
			      ROUND_UP_TO_ALIGNMENT_SIZE (sizeof
							  (grpc_call_stack)));
}
