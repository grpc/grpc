...
grpc_error_handle grpc_channel_stack_init(
  ...
  if (grpc_trace_channel_stack.enabled()) {
    ...
    for (size_t i = 0; i < filter_count; i++) {
      gpr_log(
          GPR_INFO, "CHANNEL_STACK:   filter %s%s", filters[i]->name,
          filters[i]->init_call_promise != nullptr ? " [promise-capable]" : "");
    }
  }
...
void grpc_channel_stack::InitClientCallPromise(
    const grpc_core::CallArgs& call_args) {
  for (size_t i = 0; i < this->count; i++) {
    auto* elem = grpc_channel_stack_element(this, i);
    elem->filter->init_call_promise(elem, call_args);
  }
}

void grpc_channel_stack::InitServerCallPromise(
    const grpc_core::CallArgs& call_args) {
  for (size_t i = this->count - 1; i-- > 0;) {
    auto* elem = grpc_channel_stack_element(this, i);
    elem->filter->init_call_promise(elem, call_args);
  }
}
