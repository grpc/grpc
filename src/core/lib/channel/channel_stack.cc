...
size_t grpc_channel_stack_size(const grpc_channel_filter** filters, size_t filter_count) {
  ...
  size_t size = GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_channel_stack)) +
                GPR_ROUND_UP_TO_ALIGNMENT_SIZE(filter_count * sizeof(grpc_channel_element));
  size_t i;
...
#define CHANNEL_ELEMS_FROM_STACK(stk)     \
  ((grpc_channel_element*)((char*)(stk) + \
                           GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_channel_stack))))

#define CALL_ELEMS_FROM_STACK(stk) \
  ((grpc_call_element*)((char*)(stk) + GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_call_stack))))
...
size_t grpc_channel_stack_filter_instance_number(grpc_channel_stack* channel_stack,
  ...
  for (size_t i = 0; i < channel_stack->count; ++i) {
    grpc_channel_element* element = grpc_channel_stack_element(channel_stack, i);
...
grpc_error_handle grpc_channel_stack_init(int initial_refs, grpc_iomgr_cb_func destroy,
  ...
  GRPC_STREAM_REF_INIT(&stack->refcount, initial_refs, destroy, destroy_arg, name);
  ...
  for (i = 0; i < filter_count; i++) {
    ...
    grpc_error_handle error = elems[i].filter->init_channel_elem(&elems[i], &args);
    ...
    user_data += GPR_ROUND_UP_TO_ALIGNMENT_SIZE(filters[i]->sizeof_channel_data);
...
grpc_error_handle grpc_call_stack_init(grpc_channel_stack* channel_stack, int initial_refs,
  ...
  GRPC_STREAM_REF_INIT(&elem_args->call_stack->refcount, initial_refs, destroy, destroy_arg,
                       "CALL_STACK");
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    user_data += GPR_ROUND_UP_TO_ALIGNMENT_SIZE(call_elems[i].filter->sizeof_call_data);
  }
  for (size_t i = 0; i < count; i++) {
    grpc_error_handle error = call_elems[i].filter->init_call_elem(&call_elems[i], elem_args);
    if (error != GRPC_ERROR_NONE) {
      if (first_error == GRPC_ERROR_NONE) {
        first_error = error;
      } else {
...
void grpc_call_stack_destroy(grpc_call_stack* stack, const grpc_call_final_info* final_info,
  ...
  /* destroy per-filter data */
  for (i = 0; i < count; i++) {
    elems[i].filter->destroy_call_elem(&elems[i], final_info,
...
grpc_channel_stack* grpc_channel_stack_from_top_element(grpc_channel_element* elem) {
  return reinterpret_cast<grpc_channel_stack*>(
      reinterpret_cast<char*>(elem) - GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_channel_stack)));
}

grpc_call_stack* grpc_call_stack_from_top_element(grpc_call_element* elem) {
  return reinterpret_cast<grpc_call_stack*>(
      reinterpret_cast<char*>(elem) - GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_call_stack)));
}
