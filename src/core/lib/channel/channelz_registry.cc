  ...
  RefCountedPtr<BaseNode> node_after_pagination_limit;
  {
    ...
    for (auto it = node_map_.lower_bound(start_channel_id);
      ...
      if (node->type() == BaseNode::EntityType::kTopLevelChannel &&
        ...
        // Check if we are over pagination limit to determine if we need
        // to set the "end" element. If we don't go through this block,
        // we know that when the loop terminates, we have <= to
        // kPaginationLimit. Note that because we have already increased
        // this node's refcount, we need to decrease it, but we can't
        // unref while holding the lock, because this may lead to a
        // deadlock.
...
std::string ChannelzRegistry::InternalGetServers(
    intptr_t start_server_id) {
...
char* grpc_channelz_get_server_sockets(intptr_t server_id,
  ...
  // This cast is ok since we have just checked to make sure base_node
  // is actually a server node.
...
char* grpc_channelz_get_channel(intptr_t channel_id) {
  ...
  if (channel_node == nullptr ||
      (channel_node->type() != grpc_core::channelz::BaseNode::
...
char* grpc_channelz_get_subchannel(intptr_t subchannel_id) {
  grpc_core::ApplicationCallbackExecCtx callback_exec_ctx;
  grpc_core::ExecCtx exec_ctx;
  grpc_core::RefCountedPtr<grpc_core::channelz::BaseNode>
      subchannel_node =
