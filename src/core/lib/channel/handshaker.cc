...
void
grpc_handshaker_init (const grpc_handshaker_vtable * vtable,
...
void
grpc_handshaker_destroy (grpc_handshaker * handshaker)
{
  handshaker->vtable->destroy (handshaker);
}

void
grpc_handshaker_shutdown (grpc_handshaker * handshaker, grpc_error * why)
{
  handshaker->vtable->shutdown (handshaker, why);
}

void
grpc_handshaker_do_handshake (grpc_handshaker * handshaker,
...
void
grpc_handshake_manager_pending_list_add (grpc_handshake_manager ** head,
					 grpc_handshake_manager * mgr)
{
...
void
grpc_handshake_manager_pending_list_remove (grpc_handshake_manager ** head,
					    grpc_handshake_manager * mgr)
{
  if (mgr->next != nullptr)
    {
...
void
grpc_handshake_manager_pending_list_shutdown_all (grpc_handshake_manager *
						  head, grpc_error * why)
{
  while (head != nullptr)
    {
...
void
grpc_handshake_manager_add (grpc_handshake_manager * mgr,
			    grpc_handshaker * handshaker)
{
  gpr_mu_lock (&mgr->mu);
  ...
  if (realloc_count > 0)
    {
      mgr->handshakers =
	(grpc_handshaker **) gpr_realloc (mgr->handshakers,
					  realloc_count *
					  sizeof (grpc_handshaker *));
    }
...
static void
grpc_handshake_manager_unref (grpc_handshake_manager * mgr)
{
...
void
grpc_handshake_manager_destroy (grpc_handshake_manager * mgr)
{
  grpc_handshake_manager_unref (mgr);
}

void
grpc_handshake_manager_shutdown (grpc_handshake_manager * mgr,
				 grpc_error * why)
{
  gpr_mu_lock (&mgr->mu);
...
static bool
call_next_handshaker_locked (grpc_handshake_manager * mgr, grpc_error * error)
{
  ...
  if (error != GRPC_ERROR_NONE || mgr->shutdown || mgr->args.exit_early ||
    ...
    {
      grpc_handshaker_do_handshake (mgr->handshakers[mgr->index],
...
static void
call_next_handshaker (void *arg, grpc_error * error)
{
...
on_timeout (void *arg, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
    {				// Timer fired, rather than being cancelled.
      grpc_handshake_manager_shutdown (mgr,
...
void
grpc_handshake_manager_do_handshake (grpc_handshake_manager * mgr,
