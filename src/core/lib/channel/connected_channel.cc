...
static void
run_in_call_combiner (void *arg, grpc_error * error)
{
...
static void
run_cancel_in_call_combiner (void *arg, grpc_error * error)
{
...
static void
intercept_callback (call_data * calld, callback_state * state,
		    ...
		    grpc_closure ** original_closure)
{
  ...
  state->reason = reason;
  *original_closure = GRPC_CLOSURE_INIT (&state->closure,
					 free_when_done ?
					 run_cancel_in_call_combiner :
					 run_in_call_combiner, state,
					 grpc_schedule_on_exec_ctx);
}

static callback_state *
get_state_for_batch (call_data * calld,
		     grpc_transport_stream_op_batch * batch)
{
  if (batch->send_initial_metadata)
    return &calld->on_complete[0];
  if (batch->send_message)
    return &calld->on_complete[1];
  if (batch->send_trailing_metadata)
    return &calld->on_complete[2];
  if (batch->recv_initial_metadata)
    return &calld->on_complete[3];
  if (batch->recv_message)
    return &calld->on_complete[4];
  if (batch->recv_trailing_metadata)
    return &calld->on_complete[5];
...
static void
con_start_transport_stream_op_batch (grpc_call_element * elem,
				     grpc_transport_stream_op_batch * batch)
{
...
init_call_elem (grpc_call_element * elem, const grpc_call_element_args * args)
{
  ...
  int r =
    grpc_transport_init_stream (chand->transport,
				TRANSPORT_STREAM_FROM_CALL_DATA (calld),
...
static void
set_pollset_or_pollset_set (grpc_call_element * elem,
...
/* No-op. */
static void
con_get_channel_info (grpc_channel_element * elem,
...
static void
bind_transport (grpc_channel_stack * channel_stack,
...
bool
grpc_add_connected_filter (grpc_channel_stack_builder * builder,
...
grpc_stream *
grpc_connected_channel_get_stream (grpc_call_element * elem)
{
  call_data *calld = (call_data *) elem->call_data;
