...
char* grpc_base64_encode(const void* vdata, size_t data_size, int url_safe, int multiline) {
  size_t result_projected_size = grpc_base64_estimate_encoded_size(data_size, multiline);
...
size_t grpc_base64_estimate_encoded_size(size_t data_size, int multiline) {
  size_t result_projected_size =
      4 * ((data_size + 3) / 3) +
      2 * (multiline ? (data_size / (3 * GRPC_BASE64_MULTILINE_NUM_BLOCKS)) : 0) + 1;
  return result_projected_size;
}

void grpc_base64_encode_core(char* result, const void* vdata, size_t data_size, int url_safe,
                             int multiline) {
  ...
  const size_t result_projected_size = grpc_base64_estimate_encoded_size(data_size, multiline);
...
static void decode_two_chars(const unsigned char* codes, unsigned char* result,
  ...
  uint32_t packed = (static_cast<uint32_t>(codes[0]) << 10) |
                    (static_cast<uint32_t>(codes[1]) << 4) | (static_cast<uint32_t>(codes[2]) >> 2);
  result[(*result_offset)++] = static_cast<unsigned char>(packed >> 8);
  result[(*result_offset)++] = static_cast<unsigned char>(packed);
}

static int decode_group(const unsigned char* codes, size_t num_codes, unsigned char* result,
                        size_t* result_offset) {
