...
char *
grpc_base64_encode (const void *vdata, size_t data_size, int url_safe,
		    int multiline)
{
...
size_t
grpc_base64_estimate_encoded_size (size_t data_size, int url_safe,
				   int multiline)
{
  size_t result_projected_size =
    4 * ((data_size + 3) / 3) +
...
void
grpc_base64_encode_core (char *result, const void *vdata, size_t data_size,
...
static void
decode_one_char (const unsigned char *codes, unsigned char *result,
...
static void
decode_two_chars (const unsigned char *codes, unsigned char *result,
		  size_t * result_offset)
{
...
static int
decode_group (const unsigned char *codes, size_t num_codes,
	      unsigned char *result, size_t * result_offset)
{
  ...
  if (codes[2] == GRPC_BASE64_PAD_BYTE)
  ...
  else
    {
      /* No padding. */
      uint32_t packed =
	((uint32_t) codes[0] << 18) | ((uint32_t) codes[1] << 12) |
