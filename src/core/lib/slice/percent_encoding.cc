...
static bool
is_unreserved_character (uint8_t c, const uint8_t * unreserved_bytes)
{
  return ((unreserved_bytes[c / 8] >> (c % 8)) & 1) != 0;
}

grpc_slice
grpc_percent_encode_slice (grpc_slice slice, const uint8_t * unreserved_bytes)
{
...
static bool
valid_hex (const uint8_t * p, const uint8_t * end)
{
  if (p >= end)
    return false;
...
bool
grpc_strict_percent_decode_slice (grpc_slice slice_in,
...
grpc_slice
grpc_permissive_percent_decode_slice (grpc_slice slice_in)
{
  const uint8_t *p = GRPC_SLICE_START_PTR (slice_in);
  const uint8_t *in_end = GRPC_SLICE_END_PTR (slice_in);
  size_t out_length = 0;
