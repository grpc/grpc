...
char* grpc_slice_to_c_string(grpc_slice slice) {
  char* out = reinterpret_cast<char*>(gpr_malloc(GRPC_SLICE_LENGTH(slice) + 1));
...
static void new_slice_ref(void* p) {
  new_slice_refcount* r = reinterpret_cast<new_slice_refcount*>(p);
  gpr_ref(&r->refs);
}
...
grpc_slice grpc_slice_new_with_user_data(void* p, size_t len,
                                         ...
                                         void* user_data) {
  grpc_slice slice;
  new_slice_refcount* rc = reinterpret_cast<new_slice_refcount*>(
      gpr_malloc(sizeof(new_slice_refcount)));
  ...
  slice.data.refcounted.bytes = reinterpret_cast<uint8_t*>(p);
  slice.data.refcounted.length = len;
  return slice;
}
...
static void new_with_len_ref(void* p) {
  new_with_len_slice_refcount* r =
      reinterpret_cast<new_with_len_slice_refcount*>(p);
  gpr_ref(&r->refs);
}
...
grpc_slice grpc_slice_new_with_len(void* p, size_t len,
  ...
  new_with_len_slice_refcount* rc =
      reinterpret_cast<new_with_len_slice_refcount*>(
...
static void malloc_ref(void* p) {
  malloc_refcount* r = reinterpret_cast<malloc_refcount*>(p);
  gpr_ref(&r->refs);
}
...
grpc_slice grpc_slice_malloc_large(size_t length) {
  ...
  malloc_refcount* rc = reinterpret_cast<malloc_refcount*>(
...
int grpc_slice_chr(grpc_slice s, char c) {
  const char* b = reinterpret_cast<const char*> GRPC_SLICE_START_PTR(s);
  const char* p =
      reinterpret_cast<const char*>(memchr(b, c, GRPC_SLICE_LENGTH(s)));
