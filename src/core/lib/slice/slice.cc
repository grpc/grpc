...
char *
grpc_slice_to_c_string (grpc_slice slice)
{
...
grpc_slice
grpc_slice_ref_internal (grpc_slice slice)
{
  if (slice.refcount)
    {
...
void
grpc_slice_unref_internal (grpc_slice slice)
{
  if (slice.refcount)
    {
...
/* Public API */
grpc_slice
grpc_slice_ref (grpc_slice slice)
{
...
/* Public API */
void
grpc_slice_unref (grpc_slice slice)
{
...
/* grpc_slice_from_static_string support structure - a refcount that does
   nothing */
static void
noop_ref (void *unused)
{
}

static void
noop_unref (void *unused)
{
}
...
grpc_slice
grpc_slice_from_static_buffer (const void *s, size_t len)
{
  grpc_slice slice;
...
grpc_slice
grpc_slice_from_static_string (const char *s)
{
...
grpc_slice
grpc_slice_from_copied_buffer (const char *source, size_t length)
{
  if (length == 0)
...
grpc_slice
grpc_slice_malloc_large (size_t length)
{
  grpc_slice slice;
...
grpc_slice
grpc_slice_malloc (size_t length)
{
  grpc_slice slice;
...
grpc_slice
grpc_slice_sub_no_ref (grpc_slice source, size_t begin, size_t end)
{
  grpc_slice subset;
...
grpc_slice
grpc_slice_sub (grpc_slice source, size_t begin, size_t end)
{
  grpc_slice subset;
  ...
  if (end - begin <= sizeof (subset.data.inlined.bytes))
    {
      subset.refcount = nullptr;
      subset.data.inlined.length = (uint8_t) (end - begin);
      memcpy (subset.data.inlined.bytes,
	      GRPC_SLICE_START_PTR (source) + begin, end - begin);
    }
  else
    {
...
grpc_slice
grpc_slice_split_tail_maybe_ref (grpc_slice * source, size_t split,
				 grpc_slice_ref_whom ref_whom)
{
  ...
  if (source->refcount == nullptr)
	  ...
	  ref_whom != GRPC_SLICE_REF_TAIL)
	{
	  ...
	  memcpy (tail.data.inlined.bytes,
		  source->data.refcounted.bytes + split, tail_length);
...
grpc_slice
grpc_slice_split_tail (grpc_slice * source, size_t split)
{
  return grpc_slice_split_tail_maybe_ref (source, split, GRPC_SLICE_REF_BOTH);
}

grpc_slice
grpc_slice_split_head (grpc_slice * source, size_t split)
{
  grpc_slice head;
...
int
grpc_slice_default_eq_impl (grpc_slice a, grpc_slice b)
{
  if (GRPC_SLICE_LENGTH (a) != GRPC_SLICE_LENGTH (b))
    return false;
  if (GRPC_SLICE_LENGTH (a) == 0)
    return true;
...
int
grpc_slice_eq (grpc_slice a, grpc_slice b)
{
...
int
grpc_slice_cmp (grpc_slice a, grpc_slice b)
{
...
int
grpc_slice_str_cmp (grpc_slice a, const char *b)
{
...
int
grpc_slice_is_equivalent (grpc_slice a, grpc_slice b)
{
...
int
grpc_slice_buf_start_eq (grpc_slice a, const void *b, size_t len)
{
  if (GRPC_SLICE_LENGTH (a) < len)
    return 0;
  return 0 == memcmp (GRPC_SLICE_START_PTR (a), b, len);
}

int
grpc_slice_rchr (grpc_slice s, char c)
{
...
int
grpc_slice_chr (grpc_slice s, char c)
{
  const char *b = (const char *) GRPC_SLICE_START_PTR (s);
...
int
grpc_slice_slice (grpc_slice haystack, grpc_slice needle)
{
  ...
  if (needle_len == 1)
    return grpc_slice_chr (haystack, (char) *needle_bytes);
