...
#ifndef GRPC_CORE_LIB_SLICE_SLICE_BUFFER_INTERNAL_H
#define GRPC_CORE_LIB_SLICE_SLICE_BUFFER_INTERNAL_H
...
#define GROW_INTERNAL(x) (3 * (x) / 2)

inline void grpc_slice_buffer_maybe_embiggen(grpc_slice_buffer* sb) {
  ...
  if (slice_count == sb->capacity) {
    if (sb->base_slices != sb->slices) {
      ...
      sb->capacity = GROW_INTERNAL(sb->capacity);
...
inline void grpc_slice_buffer_init_internal(grpc_slice_buffer* sb) {
  sb->count = 0;
  sb->length = 0;
...
inline uint8_t* grpc_slice_buffer_tiny_add_internal(grpc_slice_buffer* sb,
                                                    size_t n) {
  grpc_slice* back;
  uint8_t* out;
...
add_new:
  grpc_slice_buffer_maybe_embiggen(sb);
...
inline size_t grpc_slice_buffer_add_indexed_internal(grpc_slice_buffer* sb,
                                                     grpc_slice s) {
  size_t out = sb->count;
  grpc_slice_buffer_maybe_embiggen(sb);
  sb->slices[out] = s;
...
inline void grpc_slice_buffer_add_internal(grpc_slice_buffer* sb,
                                           grpc_slice s) {
  size_t n = sb->count;
...
inline void grpc_slice_buffer_addn_internal(grpc_slice_buffer* sb,
                                            grpc_slice* s, size_t n) {
  size_t i;
  for (i = 0; i < n; i++) {
...
inline void grpc_slice_buffer_pop_internal(grpc_slice_buffer* sb) {
  if (sb->count != 0) {
...
inline void grpc_slice_buffer_swap_internal(grpc_slice_buffer* a,
                                            grpc_slice_buffer* b) {
...
//           src, grpc_slice_split_tail_maybe_ref(&slice, n,
//           GRPC_SLICE_REF_BOTH));
...
inline void grpc_slice_buffer_undo_take_first_internal(grpc_slice_buffer* sb,
                                                       grpc_slice slice) {
  sb->slices--;
  sb->slices[0] = slice;
  sb->count++;
...
#endif /* GRPC_CORE_LIB_SLICE_SLICE_BUFFER_INTERNAL_H */
