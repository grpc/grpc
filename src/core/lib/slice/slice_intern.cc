...
static void
interned_slice_destroy (interned_slice_refcount * s)
{
...
static void
interned_slice_sub_ref (void *p)
{
  interned_slice_ref (((char *) p) - offsetof (interned_slice_refcount, sub));
}

static void
interned_slice_sub_unref (void *p)
{
  interned_slice_unref (((char *) p) -
			offsetof (interned_slice_refcount, sub));
}

static uint32_t
interned_slice_hash (grpc_slice slice)
{
...
static int
interned_slice_eq (grpc_slice a, grpc_slice b)
{
...
static grpc_slice
materialize (interned_slice_refcount * s)
{
  grpc_slice slice;
...
uint32_t
grpc_slice_default_hash_impl (grpc_slice s)
{
...
grpc_slice
grpc_slice_maybe_static_intern (grpc_slice slice,
				bool * returned_slice_is_different)
{
  ...
  for (uint32_t i = 0; i <= max_static_metadata_hash_probe; i++)
	...
	static_metadata_hash[(hash +
			      i) % GPR_ARRAY_SIZE (static_metadata_hash)];
...
bool
grpc_slice_is_interned (grpc_slice slice)
{
...
grpc_slice
grpc_slice_intern (grpc_slice slice)
{
...
void
grpc_test_only_set_slice_hash_seed (uint32_t seed)
{
  g_hash_seed = seed;
  ...
  for (size_t i = 0; i < SHARD_COUNT; i++)
    {
      ...
      shard->strs =
	(interned_slice_refcount **) gpr_zalloc (sizeof (*shard->strs) *
						 shard->capacity);
    }
...
void
grpc_slice_intern_shutdown (void)
{
  for (size_t i = 0; i < SHARD_COUNT; i++)
    {
      slice_shard *shard = &g_shards[i];
