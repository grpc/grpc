...
static void
maybe_embiggen (grpc_slice_buffer * sb)
{
  ...
  if (slice_count == sb->capacity)
      ...
      if (sb->base_slices != sb->slices)
	{
	  ...
	  memmove (sb->base_slices, sb->slices,
		   sb->count * sizeof (grpc_slice));
      ...
      else
	{
	  ...
	  if (sb->base_slices == sb->inlined)
	    {
	      sb->base_slices =
		(grpc_slice *) gpr_malloc (sb->capacity *
					   sizeof (grpc_slice));
	      memcpy (sb->base_slices, sb->inlined,
		      slice_count * sizeof (grpc_slice));
	    }
	  else
	    {
	      sb->base_slices =
		(grpc_slice *) gpr_realloc (sb->base_slices,
					    sb->capacity *
					    sizeof (grpc_slice));
	    }
...
void
grpc_slice_buffer_init (grpc_slice_buffer * sb)
{
  sb->count = 0;
  sb->length = 0;
...
void
grpc_slice_buffer_destroy_internal (grpc_slice_buffer * sb)
{
...
void
grpc_slice_buffer_destroy (grpc_slice_buffer * sb)
{
...
uint8_t *
grpc_slice_buffer_tiny_add (grpc_slice_buffer * sb, size_t n)
{
  grpc_slice *back;
  uint8_t *out;
...
size_t
grpc_slice_buffer_add_indexed (grpc_slice_buffer * sb, grpc_slice s)
{
  size_t out = sb->count;
  maybe_embiggen (sb);
  sb->slices[out] = s;
...
void
grpc_slice_buffer_add (grpc_slice_buffer * sb, grpc_slice s)
{
  size_t n = sb->count;
  ...
  if (!s.refcount && n)
	  ...
	  back->data.inlined.length < GRPC_SLICE_INLINED_SIZE)
	{
	  if (s.data.inlined.length + back->data.inlined.length <=
	    ...
	    {
	      size_t cp1 =
		GRPC_SLICE_INLINED_SIZE - back->data.inlined.length;
	      memcpy (back->data.inlined.bytes + back->data.inlined.length,
	      ...
	      back->refcount = nullptr;
	      back->data.inlined.length =
		(uint8_t) (s.data.inlined.length - cp1);
...
void
grpc_slice_buffer_addn (grpc_slice_buffer * sb, grpc_slice * s, size_t n)
{
  size_t i;
  for (i = 0; i < n; i++)
    {
...
void
grpc_slice_buffer_pop (grpc_slice_buffer * sb)
{
  if (sb->count != 0)
    {
...
void
grpc_slice_buffer_reset_and_unref_internal (grpc_slice_buffer * sb)
{
  size_t i;
...
void
grpc_slice_buffer_reset_and_unref (grpc_slice_buffer * sb)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_slice_buffer_reset_and_unref_internal (sb);
}
...
void
grpc_slice_buffer_swap (grpc_slice_buffer * a, grpc_slice_buffer * b)
{
  ...
  if (a->base_slices == a->inlined)
      ...
      if (b->base_slices == b->inlined)
	{
	  ...
	  memcpy (a->base_slices, b->base_slices,
		  b_count * sizeof (grpc_slice));
...
void
grpc_slice_buffer_move_into (grpc_slice_buffer * src, grpc_slice_buffer * dst)
{
  /* anything to move? */
  if (src->count == 0)
    {
...
static void
slice_buffer_move_first_maybe_ref (grpc_slice_buffer * src, size_t n,
				   grpc_slice_buffer * dst, bool incref)
{
  ...
  while (src->count > 0)
	...
	{			/* n < slice_len */
	  grpc_slice_buffer_undo_take_first (src,
...
void
grpc_slice_buffer_move_first (grpc_slice_buffer * src, size_t n,
...
void
grpc_slice_buffer_move_first_no_ref (grpc_slice_buffer * src, size_t n,
				     grpc_slice_buffer * dst)
{
...
void
grpc_slice_buffer_move_first_into_buffer (grpc_slice_buffer * src, size_t n,
					  void *dst)
{
  char *dstp = (char *) dst;
  GPR_ASSERT (src->length >= n);
...
void
grpc_slice_buffer_trim_end (grpc_slice_buffer * sb, size_t n,
...
grpc_slice
grpc_slice_buffer_take_first (grpc_slice_buffer * sb)
{
  grpc_slice slice;
...
void
grpc_slice_buffer_undo_take_first (grpc_slice_buffer * sb, grpc_slice slice)
{
  sb->slices--;
  sb->slices[0] = slice;
  sb->count++;
