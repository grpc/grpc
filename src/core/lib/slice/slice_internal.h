...
// Interned slices have specific fast-path operations for hashing. To
// inline these operations, we need to forward declare them here.
extern uint32_t
    grpc_static_metadata_hash_values[GRPC_STATIC_MDSTR_COUNT];

// grpc_slice_refcount : A reference count for grpc_slice.
//
// Non-inlined grpc_slice objects are refcounted. Historically this was
// implemented via grpc_slice_refcount, a C-style polymorphic class
// using a manually managed vtable of operations. Subclasses would
// define their own vtable; the 'virtual' methods (ref, unref, equals
// and hash) would simply call the function pointers in the vtable as
// necessary.
//
// Unfortunately, this leads to some inefficiencies in the generated
// code that can be improved upon. For example, equality checking for
// interned slices is a simple equality check on the refcount pointer.
// With the vtable approach, this would translate to roughly the
...
//   ret retval
//
// This leads to a function call for a function defined in another
// translation unit, which imposes memory barriers, which reduces the
// compiler's ability to optimize (in addition to the added overhead of
// call/ret). Additionally, it may be harder to reason about branch
// prediction when we're jumping to essentially arbitrarily provided
// function pointers.
//
...
// Ref()/Unref().
//
// Instead, grpc_slice_refcount provides the same functionality as the
// C-style virtual class, but in a de-virtualized manner - Eq(), Hash(),
// Ref() and Unref() are provided within this header file. Fastpaths for
// Eq()/Hash() (interned and static metadata slices), as well as the
// Ref() operation, can all be inlined without any memory barriers.
//
// It does this by:
// 1. Using grpc_core::RefCount<> (header-only) for Ref/Unref. Two
// special cases
//    need support: No-op ref/unref (eg. static metadata slices) and
//    stream slice references (where all the slices share the
//    streamref). This is in addition to the normal case of '1 slice, 1
//    ref'. To support these cases, we explicitly track a nullable
//    pointer to the underlying RefCount<>. No-op ref/unref is used by
//    checking the pointer for null, and doing nothing if it is. Both
//    stream slice refs and 'normal' slices use the same path for
//    Ref/Unref (by targeting the non-null pointer).
//
// 2. introducing the notion of grpc_slice_refcount::Type. This
// describes if a
//    slice ref is used by a static metadata slice, an interned slice,
//    or other slices. We switch on the slice ref type in order to
//    provide fastpaths for Equals() and Hash().
//
// In total, this saves us roughly 1-2% latency for unary calls, with
// smaller calls benefitting. The effect is present, but not as useful,
// for larger calls where the cost of sending the data dominates.
// TODO(arjunroy): Investigate if this can be removed with strongly
// typed grpc_slices.
struct grpc_slice_refcount {
 public:
  ...
  explicit grpc_slice_refcount(grpc_slice_refcount* sub)
      : sub_refcount_(sub) {}
  ...
  //
  //  2. RefCount* ref
  //  The pointer to the actual underlying grpc_core::RefCount. Rather
  //  than performing struct offset computations as in the original
  //  implementation to get to the refcount, which requires a virtual
  //  method, we devirtualize by using a nullable pointer to allow a
  //  single pair of Ref/Unref methods.
  //
  //  3. DestroyerFn destroyer_fn
  //  Called when the refcount goes to 0, with destroyer_arg as
  //  parameter.
  //
  ...
  grpc_slice_refcount(grpc_slice_refcount::Type type,
...
struct InternedSliceRefcount {
  ...
  InternedSliceRefcount(size_t length, uint32_t hash,
      ...
      : base(grpc_slice_refcount::Type::INTERNED, &refcnt, Destroy,
             this, &sub),
        sub(grpc_slice_refcount::Type::REGULAR, &refcnt, Destroy, this,
            &sub),
        length(length),
        hash(hash),
...
inline const uint8_t* grpc_refcounted_slice_data(
    const grpc_slice& slice) {
  GPR_DEBUG_ASSERT(slice.refcount != nullptr);
  return slice.data.refcounted.bytes;
}

inline int grpc_slice_refcount::Eq(const grpc_slice& a,
                                   const grpc_slice& b) {
  GPR_DEBUG_ASSERT(a.refcount != nullptr);
  GPR_DEBUG_ASSERT(a.refcount == this);
  switch (ref_type_) {
    case Type::STATIC:
      GPR_DEBUG_ASSERT((GRPC_STATIC_METADATA_INDEX(a) ==
                        GRPC_STATIC_METADATA_INDEX(b)) ==
  ...
  if (grpc_refcounted_slice_length(a) != GRPC_SLICE_LENGTH(b))
    return false;
  if (grpc_refcounted_slice_length(a) == 0) return true;
  return 0 == memcmp(grpc_refcounted_slice_data(a),
                     GRPC_SLICE_START_PTR(b),
...
inline uint32_t grpc_slice_refcount::Hash(const grpc_slice& slice) {
  ...
  switch (ref_type_) {
    case Type::STATIC:
      return ::grpc_static_metadata_hash_values
          [GRPC_STATIC_METADATA_INDEX(slice)];
    case Type::INTERNED:
      return reinterpret_cast<grpc_core::InternedSliceRefcount*>(
                 slice.refcount)
          ->hash;
    case Type::NOP:
    case Type::REGULAR:
...
inline const grpc_slice& grpc_slice_ref_internal(
    const grpc_slice& slice) {
  if (slice.refcount) {
    slice.refcount->Ref();
  }
...
// Returns a pointer to the first slice in the slice buffer without
// giving ownership to or a reference count on that slice.
...
inline bool grpc_slice_is_interned(const grpc_slice& slice) {
  return (slice.refcount && (slice.refcount->GetType() ==
                                 grpc_slice_refcount::Type::INTERNED ||
                             slice.refcount->GetType() ==
                                 grpc_slice_refcount::Type::STATIC));
}

inline bool grpc_slice_static_interned_equal(const grpc_slice& a,
                                             const grpc_slice& b) {
  GPR_DEBUG_ASSERT(grpc_slice_is_interned(a) &&
                   grpc_slice_is_interned(b));
  return a.refcount == b.refcount;
}
...
// used for surface boundaries where we might receive an un-interned
// static string
...
// Returns the memory used by this slice, not counting the slice
// structure itself. This means that inlined and slices from static
// strings will return 0. All other slices will return the size of the
// allocated chars.
size_t grpc_slice_memory_usage(grpc_slice s);

grpc_core::UnmanagedMemorySlice grpc_slice_sub_no_ref(
    const grpc_core::UnmanagedMemorySlice& source, size_t begin,
    size_t end);
