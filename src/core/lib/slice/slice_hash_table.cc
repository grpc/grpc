...
static bool
is_empty (grpc_slice_hash_table_entry * entry)
{
  return entry->value == nullptr;
}

static void
grpc_slice_hash_table_add (grpc_slice_hash_table * table,
			   grpc_slice key, void *value)
{
  ...
  for (size_t offset = 0; offset < table->size; ++offset)
      ...
      if (is_empty (&table->entries[idx]))
	{
	  ...
	  if (offset > table->max_num_probes)
	    table->max_num_probes = offset;
	  return;
	}
    }
...
grpc_slice_hash_table_create (size_t num_entries,
			      grpc_slice_hash_table_entry * entries,
			      void (*destroy_value) (void *value),
			      int (*value_cmp) (void *a, void *b))
{
  ...
  const size_t entry_size =
    sizeof (grpc_slice_hash_table_entry) * table->size;
...
grpc_slice_hash_table *
grpc_slice_hash_table_ref (grpc_slice_hash_table * table)
{
  if (table != nullptr)
    gpr_ref (&table->refs);
  return table;
...
void
grpc_slice_hash_table_unref (grpc_slice_hash_table * table)
{
...
void *
grpc_slice_hash_table_get (const grpc_slice_hash_table * table,
...
static int
pointer_cmp (void *a, void *b)
{
  return GPR_ICMP (a, b);
}

int
grpc_slice_hash_table_cmp (const grpc_slice_hash_table * a,
			   const grpc_slice_hash_table * b)
{
  ...
  for (size_t i = 0; i < a->size; ++i)
      ...
      else if (is_empty (&b->entries[i]))
	{
      ...
      const int key_cmp =
	grpc_slice_cmp (a->entries[i].key, b->entries[i].key);
      if (key_cmp != 0)
	return key_cmp;
      const int value_cmp =
