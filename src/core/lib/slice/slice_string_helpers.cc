...
grpc_slice grpc_dump_slice_to_slice(const grpc_slice& s,
  ...
  grpc_core::UniquePtr<char> ptr(gpr_dump_return_len(
      reinterpret_cast<const char*> GRPC_SLICE_START_PTR(s),
      GRPC_SLICE_LENGTH(s), flags, &len));
  return grpc_slice_from_moved_buffer(std::move(ptr), len);
}

/** Finds the initial (\a begin) and final (\a end) offsets of the next
 * substring from \a str + \a read_offset until the next \a sep or the
 * end of \a str.
 *
 * Returns 1 and updates \a begin and \a end. Returns 0 otherwise. */
static int slice_find_separator_offset(const grpc_slice str,
                                       const char* sep,
...
static void grpc_slice_split_inner(grpc_slice str, const char* sep,
  ...
  if (slice_find_separator_offset(str, sep, 0, &begin, &end) != 0) {
    ...
    } while (slice_find_separator_offset(str, sep, sep_pos + sep_len,
                                         &begin, &end) != 0);
