...
char* gpr_strdup(const char* src) {
  ...
  len = strlen(src) + 1;
  dst = reinterpret_cast<char*>(gpr_malloc(len));
...
static void dump_out_append(dump_out* out, char c) {
  if (out->length == out->capacity) {
    ...
    out->data = reinterpret_cast<char*>(gpr_realloc(out->data, out->capacity));
  }
...
char* gpr_leftpad(const char* str, char flag, size_t length) {
  ...
  char* out = reinterpret_cast<char*>(gpr_malloc(out_length + 1));
...
char* gpr_strjoin_sep(const char** strs, size_t nstrs, const char* sep,
  ...
  out = reinterpret_cast<char*>(gpr_malloc(out_length));
  out_length = 0;
  for (i = 0; i < nstrs; i++) {
...
static void add_string_to_split(const char* beg, const char* end, char*** strs,
  ...
  char* out =
      reinterpret_cast<char*>(gpr_malloc(static_cast<size_t>(end - beg) + 1));
  memcpy(out, beg, static_cast<size_t>(end - beg));
  out[end - beg] = 0;
  if (*nstrs == *capstrs) {
    *capstrs = GPR_MAX(8, 2 * *capstrs);
    *strs =
        reinterpret_cast<char**>(gpr_realloc(*strs, sizeof(*strs) * *capstrs));
  }
  (*strs)[*nstrs] = out;
  ++*nstrs;
}
