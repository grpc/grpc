...
auto TupleCallMemberImpl(C* p, R (C::*f)(Args...), Tuple<Types...>& args,
  ...
  return (p->*f)(get<ints>(args)...);
}

template <typename F, typename... Types, int... ints>
auto TupleCallImpl(F&& f, const Tuple<Types...>& args, IntSeq<ints...>)
    -> decltype(f(tuple_impl::Example<Types>()...)) {
  return f(get<ints>(args)...);
}

template <class C, typename R, typename... Args, typename... Types, int... ints>
auto TupleCallMemberImpl(C* p, R (C::*f)(Args...), const Tuple<Types...>& args,
                         IntSeq<ints...>)
    -> decltype((p->*f)(tuple_impl::Example<Types>()...)) {
  return (p->*f)(get<ints>(args)...);
}
