...
gpr_cmdline* gpr_cmdline_create(const char* description) {
  gpr_cmdline* cl =
      reinterpret_cast<gpr_cmdline*>(gpr_zalloc(sizeof(gpr_cmdline)));
...
static void add_arg(gpr_cmdline* cl, const char* name, const char* help,
  ...
  a = reinterpret_cast<arg*>(gpr_zalloc(sizeof(arg)));
  a->name = name;
  a->help = help;
  a->type = type;
  a->value = value;
...
static int value_state(gpr_cmdline* cl, char* str) {
  ...
  switch (cl->cur_arg->type) {
    case ARGTYPE_INT:
      ...
      *reinterpret_cast<int*>(cl->cur_arg->value) = static_cast<int>(intval);
      break;
    case ARGTYPE_BOOL:
      if (0 == strcmp(str, "1") || 0 == strcmp(str, "true")) {
        *reinterpret_cast<int*>(cl->cur_arg->value) = 1;
      } else if (0 == strcmp(str, "0") || 0 == strcmp(str, "false")) {
        *reinterpret_cast<int*>(cl->cur_arg->value) = 0;
      } else {
      ...
      break;
    case ARGTYPE_STRING:
      *reinterpret_cast<char**>(cl->cur_arg->value) = str;
      break;
  }
...
static int normal_state(gpr_cmdline* cl, char* str) {
  ...
  if (str[0] == '-') {
    ...
    if (eq != nullptr) {
      ...
      tmp = arg_name = reinterpret_cast<char*>(
