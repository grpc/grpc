...
typedef enum
{ UNKICKED, KICKED, DESIGNATED_POLLER } kick_state;

static const char *
kick_state_string (kick_state st)
{
  switch (st)
    {
    case UNKICKED:
...
static void
fd_shutdown_internal (grpc_fd * fd, grpc_error * why, bool releasing_fd)
{
...
static bool
worker_insert (grpc_pollset * pollset, grpc_pollset_worker * worker)
{
...
typedef enum
{ EMPTIED, NEW_ROOT, REMOVED } worker_remove_result;

static worker_remove_result
worker_remove (grpc_pollset * pollset, grpc_pollset_worker * worker)
{
...
pollset_global_init (void)
{
  ...
  g_neighborhoods =
    (pollset_neighborhood *) gpr_zalloc (sizeof (*g_neighborhoods) *
					 g_num_neighborhoods);
...
static grpc_error *
process_epoll_events (grpc_pollset * pollset)
{
...
static grpc_error *
do_epoll_wait (grpc_pollset * ps, grpc_millis deadline)
{
...
static bool
check_neighborhood_for_available_poller (pollset_neighborhood * neighborhood)
{
	    ...
	    grpc_pollset_worker ** worker_hdl)
{
  ...
  if (gpr_atm_no_barrier_load (&g_active_poller) == (gpr_atm) worker)
      ...
      if (worker->next != worker && worker->next->state == UNKICKED)
	{
	  if (grpc_polling_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG, " .. choose next poller to be peer %p",
		       worker);
	    }
...
pollset_kick (grpc_pollset * pollset, grpc_pollset_worker * specific_worker)
{
  ...
  if (grpc_polling_trace.enabled ())
    {
      ...
      gpr_strvec_init (&log);
      char *tmp;
      gpr_asprintf (&tmp, "PS:%p KICK:%p curps=%p curworker=%p root=%p",
  ...
  if (specific_worker == nullptr)
      ...
      if (gpr_tls_get (&g_current_thread_pollset) != (intptr_t) pollset)
	{
	  ...
	  if (root_worker->state == KICKED)
	  ...
	  else if (root_worker == next_worker &&	// only try and wake up a poller if
	      ...
	      else
		{
		  ...
		  if (grpc_polling_trace.enabled ())
		    {
		      gpr_log (GPR_ERROR, " .. non-root poller %p (root=%p)",
...
grpc_init_epoll1_linux (bool explicit_request)
{
