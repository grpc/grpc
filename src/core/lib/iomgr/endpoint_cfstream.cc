...
#ifndef NDEBUG
#define EP_REF(ep, reason) \
  CFStreamRef((ep), (reason), __FILE__, __LINE__)
#define EP_UNREF(ep, reason) \
  CFStreamUnref((ep), (reason), __FILE__, __LINE__)
static void CFStreamUnref(CFStreamEndpoint* ep, const char* reason,
  ...
  if (grpc_tcp_trace.enabled()) {
    ...
    gpr_log(file, line, GPR_LOG_SEVERITY_DEBUG,
            "CFStream endpoint unref %p : %s %" PRIdPTR " -> %" PRIdPTR,
            ep, reason, val, val - 1);
  }
...
static void CFStreamRef(CFStreamEndpoint* ep, const char* reason,
  ...
  if (grpc_tcp_trace.enabled()) {
    gpr_atm val = gpr_atm_no_barrier_load(&ep->refcount.count);
    gpr_log(file, line, GPR_LOG_SEVERITY_DEBUG,
            "CFStream endpoint ref %p : %s %" PRIdPTR " -> %" PRIdPTR,
            ep, reason, val, val + 1);
  }
  gpr_ref(&ep->refcount);
}
...
static void CFStreamRef(CFStreamEndpoint* ep) {
  gpr_ref(&ep->refcount);
}
#endif

static grpc_error_handle CFStreamAnnotateError(
    grpc_error_handle src_error, CFStreamEndpoint* ep) {
...
static void CallReadCb(CFStreamEndpoint* ep, grpc_error_handle error) {
  if (grpc_tcp_trace.enabled()) {
    ...
    for (i = 0; i < ep->read_slices->count; i++) {
      ...
      gpr_log(GPR_DEBUG, "READ %p (peer=%s): %s", ep,
              ep->peer_string.c_str(), dump);
      gpr_free(dump);
    }
  }
...
static void CallWriteCb(CFStreamEndpoint* ep, grpc_error_handle error) {
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream endpoint:%p call_write_cb %p %p:%p",
...
static void ReadAction(void* arg, grpc_error_handle error) {
  ...
  CFIndex read_size = CFReadStreamRead(
      ep->read_stream, GRPC_SLICE_START_PTR(slice), len);
  if (read_size == -1) {
    grpc_slice_buffer_reset_and_unref_internal(ep->read_slices);
    ...
    if (read_size < static_cast<CFIndex>(len)) {
      grpc_slice_buffer_trim_end(ep->read_slices, len - read_size,
                                 nullptr);
    }
...
static void WriteAction(void* arg, grpc_error_handle error) {
  ...
  if (write_size == -1) {
    ...
    if (grpc_tcp_trace.enabled()) {
      ...
      char* dump =
          grpc_dump_slice(trace_slice, GPR_DUMP_HEX | GPR_DUMP_ASCII);
      gpr_log(GPR_DEBUG, "WRITE %p (peer=%s): %s", ep,
              ep->peer_string.c_str(), dump);
      gpr_free(dump);
...
static void CFStreamRead(grpc_endpoint* ep, grpc_slice_buffer* slices,
  ...
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream endpoint:%p read (%p, %p) length:%zu",
...
void CFStreamShutdown(grpc_endpoint* ep, grpc_error_handle why) {
  CFStreamEndpoint* ep_impl = reinterpret_cast<CFStreamEndpoint*>(ep);
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream endpoint:%p shutdown (%p)", ep_impl,
            why);
  }
  ...
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream endpoint:%p shutdown DONE (%p)",
            ep_impl, why);
  }
}
...
grpc_endpoint* grpc_cfstream_endpoint_create(
  ...
  CFDataRef native_handle =
      static_cast<CFDataRef>(CFReadStreamCopyProperty(
          ep_impl->read_stream, kCFStreamPropertySocketNativeHandle));
  CFSocketNativeHandle sockfd;
  CFDataGetBytes(native_handle,
