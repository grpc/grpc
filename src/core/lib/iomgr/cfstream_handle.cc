...
CFStreamHandle* CFStreamHandle::CreateStreamHandle(CFReadStreamRef read_stream,
                                                   CFWriteStreamRef write_stream) {
  return new CFStreamHandle(read_stream, write_stream);
}

void CFStreamHandle::ReadCallback(CFReadStreamRef stream, CFStreamEventType type,
  ...
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream ReadCallback (%p, %p, %lu, %p)", handle, stream, type,
  ...
  switch (type) {
    ...
    case kCFStreamEventErrorOccurred:
      ...
      error = grpc_error_set_int(GRPC_ERROR_CREATE_FROM_CFERROR(stream_error, "read error"),
...
void CFStreamHandle::WriteCallback(CFWriteStreamRef stream, CFStreamEventType type,
  ...
  if (grpc_tcp_trace.enabled()) {
    gpr_log(GPR_DEBUG, "CFStream WriteCallback (%p, %p, %lu, %p)", handle, stream, type,
            clientCallBackInfo);
  }
  switch (type) {
    case kCFStreamEventOpenCompleted:
    ...
    case kCFStreamEventErrorOccurred:
      ...
      error = grpc_error_set_int(GRPC_ERROR_CREATE_FROM_CFERROR(stream_error, "write error"),
                                 GRPC_ERROR_INT_GRPC_STATUS, GRPC_STATUS_UNAVAILABLE);
      CFRelease(stream_error);
      handle->open_event_.SetShutdown(GRPC_ERROR_REF(error));
      handle->write_event_.SetShutdown(GRPC_ERROR_REF(error));
...
CFStreamHandle::CFStreamHandle(CFReadStreamRef read_stream, CFWriteStreamRef write_stream) {
  ...
  CFStreamClientContext ctx = {0, static_cast<void*>(this), CFStreamHandle::Retain,
...
void CFStreamHandle::NotifyOnOpen(grpc_closure* closure) { open_event_.NotifyOn(closure); }

void CFStreamHandle::NotifyOnRead(grpc_closure* closure) { read_event_.NotifyOn(closure); }

void CFStreamHandle::NotifyOnWrite(grpc_closure* closure) { write_event_.NotifyOn(closure); }
...
void CFStreamHandle::Unref(const char* file, int line, const char* reason) {
  if (grpc_tcp_trace.enabled()) {
    gpr_atm val = gpr_atm_no_barrier_load(&refcount_.count);
    gpr_log(GPR_DEBUG, "CFStream Handle unref %p : %s %" PRIdPTR " -> %" PRIdPTR, this, reason, val,
            val - 1);
  }
