...
void
grpc_timer_init (grpc_timer * timer, grpc_millis deadline,
...
void
grpc_timer_init_unset (grpc_timer * timer)
{
  timer->pending = 0;
}

void
grpc_timer_cancel (grpc_timer * timer)
{
  GRPC_UV_ASSERT_SAME_THREAD ();
  if (timer->pending)
    {
...
grpc_timer_check (grpc_millis * next)
{
