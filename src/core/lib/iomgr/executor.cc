...
static size_t
run_closures (grpc_closure_list list)
{
  size_t n = 0;
...
void
grpc_executor_set_threading (bool threading)
{
...
static void
executor_push (grpc_closure * closure, grpc_error * error, bool is_short)
{
  bool retry_push;
  if (is_short)
    {
  ...
  do
	...
	(size_t) gpr_atm_no_barrier_load (&g_cur_threads);
...
#ifndef NDEBUG
	      gpr_log (GPR_DEBUG,
		       "EXECUTOR: schedule %p (created %s:%d) inline",
		       closure, closure->file_created, closure->line_created);
#else
	      gpr_log (GPR_DEBUG, "EXECUTOR: schedule %p inline", closure);
#endif
	    }
	  grpc_closure_list_append (grpc_core::ExecCtx::Get ()->
				    closure_list (), closure, error);
	  return;
	}
...
#else
	      gpr_log (GPR_DEBUG,
		       "EXECUTOR: try to schedule %p (%s) to thread %d",
		       closure, is_short ? "short" : "long",
		       (int) (ts - g_thread_state));
#endif
	    }
...
static void
executor_push_short (grpc_closure * closure, grpc_error * error)
{
  executor_push (closure, error, true);
}

static void
executor_push_long (grpc_closure * closure, grpc_error * error)
{
...
static grpc_closure_scheduler executor_scheduler_long =
  { &executor_vtable_long };

grpc_closure_scheduler *
grpc_executor_scheduler (grpc_executor_job_length length)
{
