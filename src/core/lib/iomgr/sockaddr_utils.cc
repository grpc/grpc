...
int
grpc_sockaddr_is_v4mapped (const grpc_resolved_address * resolved_addr,
			   grpc_resolved_address * resolved_addr4_out)
{
  ...
  struct sockaddr_in *addr4_out =
    ...
    ? nullptr : (struct sockaddr_in *) resolved_addr4_out->addr;
...
int
grpc_sockaddr_to_v4mapped (const grpc_resolved_address * resolved_addr,
...
int
grpc_sockaddr_is_wildcard (const grpc_resolved_address * resolved_addr,
			   int *port_out)
{
...
void
grpc_sockaddr_make_wildcards (int port, grpc_resolved_address * wild4_out,
...
void
grpc_sockaddr_make_wildcard4 (int port,
			      grpc_resolved_address * resolved_wild_out)
{
  struct sockaddr_in *wild_out =
    (struct sockaddr_in *) resolved_wild_out->addr;
...
void
grpc_sockaddr_make_wildcard6 (int port,
			      grpc_resolved_address * resolved_wild_out)
{
...
int
grpc_sockaddr_to_string (char **out,
  ...
  *out = nullptr;
  ...
  if (ip != nullptr && grpc_inet_ntop (addr->sa_family, ip, ntop_buf,
      ...
      if (sin6_scope_id != 0)
	{
	  ...
	  gpr_asprintf (&host_with_scope, "%s%%25%" PRIu32, ntop_buf,
			sin6_scope_id);
...
char *
grpc_sockaddr_to_uri (const grpc_resolved_address * resolved_addr)
{
  grpc_resolved_address addr_normalized;
...
const char *
grpc_sockaddr_get_uri_scheme (const grpc_resolved_address * resolved_addr)
{
  const struct sockaddr *addr = (const struct sockaddr *) resolved_addr->addr;
...
int
grpc_sockaddr_get_family (const grpc_resolved_address * resolved_addr)
{
  const struct sockaddr *addr = (const struct sockaddr *) resolved_addr->addr;
...
int
grpc_sockaddr_get_port (const grpc_resolved_address * resolved_addr)
{
  const struct sockaddr *addr = (const struct sockaddr *) resolved_addr->addr;
  switch (addr->sa_family)
    {
...
int
grpc_sockaddr_set_port (const grpc_resolved_address * resolved_addr, int port)
{
  const struct sockaddr *addr = (const struct sockaddr *) resolved_addr->addr;
  switch (addr->sa_family)
    {
