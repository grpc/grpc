...
set_non_block (SOCKET sock)
{
  ...
  DWORD ret;
  status =
    WSAIoctl (sock, GRPC_FIONBIO, &param, sizeof (param), NULL, 0, &ret, NULL,
	      NULL);
...
static void
on_read (void *tcpp, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
      ...
      else
	{
	  if (info->bytes_transfered != 0 && !tcp->shutting_down)
		...
		("TCP stream shutting down", &tcp->shutdown_error,
		 1) :
...
static void
win_read (grpc_endpoint * ep, grpc_slice_buffer * read_slices,
	  grpc_closure * cb)
{
...
/* Asynchronous callback from the IOCP, or the background thread. */
static void
on_write (void *tcpp, grpc_error * error)
{
  grpc_tcp *tcp = (grpc_tcp *) tcpp;
  grpc_winsocket *handle = tcp->socket;
...
/* Initiates a write. */
static void
win_write (grpc_endpoint * ep, grpc_slice_buffer * slices, grpc_closure * cb)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
  grpc_winsocket *socket = tcp->socket;
...
static void
win_delete_from_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pss)
{
}
...
static void
win_shutdown (grpc_endpoint * ep, grpc_error * why)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
  gpr_mu_lock (&tcp->mu);
...
win_get_resource_user (grpc_endpoint * ep)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
...
grpc_endpoint *
grpc_tcp_create (grpc_winsocket * socket,
		 grpc_channel_args * channel_args, const char *peer_string)
{
