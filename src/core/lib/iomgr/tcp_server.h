...
typedef struct grpc_tcp_server_acceptor {
  /* grpc_tcp_server_cb functions share a ref on from_server that is
...
namespace grpc_core {
// An interface for a handler to take a externally connected fd as a
// internal connection.
...
typedef struct grpc_tcp_server_vtable {
  ...
  grpc_error_handle (*add_port)(grpc_tcp_server* s,
                                ...
                                int* out_port);
  grpc_core::TcpServerFdHandler* (*create_fd_handler)(
      grpc_tcp_server* s);
...
/* Create a server, initially not bound to any ports. The caller owns
   one ref. If shutdown_complete is not NULL, it will be used by
...
/* Add a port to the server, returning the newly allocated port on
   success, or -1 on failure.

   The :: and 0.0.0.0 wildcard addresses are treated identically,
   accepting both IPv4 and IPv6 connections, but :: is the preferred
   style.  This usually creates one socket, but possibly two on systems
   which support IPv6, but not dualstack sockets. */
/* TODO(ctiller): deprecate this, and make grpc_tcp_server_add_ports to
   handle all of the multiple socket port matching logic in one place */
grpc_error_handle grpc_tcp_server_add_port(
    grpc_tcp_server* s, const grpc_resolved_address* addr,
    int* out_port);

/* Create and return a TcpServerFdHandler so that it can be used by
   upper layer to hand over an externally connected fd to the grpc
   server. */
...
/* Returns the file descriptor of the Mth (fd_index) listening socket of
   the Nth (port_index) call to add_port() on this server, or -1 if the
   indices are out of bounds. The file descriptor remains owned by the
   server, and will be cleaned up when the ref count reaches zero. */
...
/* shutdown_starting is called when ref count has reached zero and the
   server is about to be destroyed. The server will be deleted after it
   returns. Calling grpc_tcp_server_ref() from it has no effect. */
