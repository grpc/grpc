...
static grpc_error *
pollset_kick_ext (grpc_pollset * p,
		  grpc_pollset_worker * specific_worker, uint32_t flags)
...
static grpc_error *
pollset_kick_locked (grpc_fd_watcher * watcher)
{
...
fd_shutdown_error (grpc_fd * fd)
{
  if (!fd->shutdown)
    ...
    {
      return GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING ("FD shutdown",
							       &fd->
							       shutdown_error,
							       1);
    }
...
static void
notify_on_locked (grpc_fd * fd, grpc_closure ** st, grpc_closure * closure)
{
...
static int
set_ready_locked (grpc_fd * fd, grpc_closure ** st)
{
  if (*st == CLOSURE_READY)
    {
...
static void
set_read_notifier_pollset_locked (grpc_fd * fd,
				  grpc_pollset * read_notifier_pollset)
{
...
static uint32_t
fd_begin_poll (grpc_fd * fd, grpc_pollset * pollset,
...
static void
fd_end_poll (grpc_fd_watcher * watcher, int got_read, int got_write,
	     grpc_pollset * read_notifier_pollset)
{
  int was_polling = 0;
  int kick = 0;
...
static bool
pollset_in_pollset_sets (grpc_pollset * p)
{
  return p->pollset_set_count;
}

static bool
pollset_has_observers (grpc_pollset * p)
{
...
static void
kick_append_error (grpc_error ** composite, grpc_error * error)
{
	  ...
	  for (specific_worker = p->root_worker.next;
	       ...
	       specific_worker = specific_worker->next)
	    {
	      kick_append_error (&error,
				 grpc_wakeup_fd_wakeup (&specific_worker->
							wakeup_fd->fd));
	    }
...
pollset_add_fd (grpc_pollset * pollset, grpc_fd * fd)
{
  ...
  if (pollset->fd_count == pollset->fd_capacity)
	...
	GPR_MAX (pollset->fd_capacity + 8, pollset->fd_count * 3 / 2);
      pollset->fds =
	(grpc_fd **) gpr_realloc (pollset->fds,
				  sizeof (grpc_fd *) * pollset->fd_capacity);
    }
...
static void
work_combine_error (grpc_error ** composite, grpc_error * error)
{
  if (error == GRPC_ERROR_NONE)
    return;
  if (*composite == GRPC_ERROR_NONE)
...
#define POLLIN_CHECK (POLLIN | POLLHUP | POLLERR)
	  ...
	  if (pollset->fd_count + 2 <= inline_elements)
		...
		sizeof (*watchers) * (pollset->fd_count + 2);
	      ...
	      watchers =
		(grpc_fd_watcher *) (void *) ((char *) buf + pfd_size);
	    }
	  ...
	  for (i = 1; i < pfd_count; i++)
	    {
	      ...
	      pfds[i].events =
		(short) fd_begin_poll (fd, pollset, &worker, POLLIN, POLLOUT,
				       &watchers[i]);
	  ...
	  if (r < 0)
	      ...
	      if (pfds[0].revents & POLLIN_CHECK)
		{
		  ...
		  work_combine_error (&error,
				      grpc_wakeup_fd_consume_wakeup (&worker.
								     wakeup_fd->
								     fd));
		}
	      for (i = 1; i < pfd_count; i++)
		{
		  if (watchers[i].fd == nullptr)
			...
			{
			  gpr_log (GPR_DEBUG,
				   "%p got_event: %d r:%d w:%d [%d]", pollset,
				   pfds[i].fd,
			...
			{
			  gpr_atm_no_barrier_store (&watchers[i].fd->pollhup,
						    1);
			}
		      fd_end_poll (&watchers[i],
				   pfds[i].revents & POLLIN_CHECK,
...
static int
poll_deadline_to_millis_timeout (grpc_millis deadline)
{
...
static void
pollset_set_destroy (grpc_pollset_set * pollset_set)
{
  size_t i;
...
static void
pollset_set_add_pollset (grpc_pollset_set * pollset_set,
			 grpc_pollset * pollset)
{
  size_t i, j;
  gpr_mu_lock (&pollset->mu);
  ...
  if (pollset_set->pollset_count == pollset_set->pollset_capacity)
	...
	(grpc_pollset **) gpr_realloc (pollset_set->pollsets,
				       pollset_set->pollset_capacity *
...
static void
pollset_set_del_pollset (grpc_pollset_set * pollset_set,
			 grpc_pollset * pollset)
{
  size_t i;
  gpr_mu_lock (&pollset_set->mu);
  ...
  if (bag->pollset_set_count == bag->pollset_set_capacity)
    {
      ...
      bag->pollset_sets =
	(grpc_pollset_set **) gpr_realloc (bag->pollset_sets,
...
static void
pollset_set_add_fd (grpc_pollset_set * pollset_set, grpc_fd * fd)
{
  size_t i;
  gpr_mu_lock (&pollset_set->mu);
  if (pollset_set->fd_count == pollset_set->fd_capacity)
    {
      ...
      pollset_set->fds =
	(grpc_fd **) gpr_realloc (pollset_set->fds,
...
static void
pollset_set_del_fd (grpc_pollset_set * pollset_set, grpc_fd * fd)
{
  size_t i;
  gpr_mu_lock (&pollset_set->mu);
  for (i = 0; i < pollset_set->fd_count; i++)
...
static void
cache_insert_locked (poll_args * args)
{
  uint32_t key =
    gpr_murmur_hash3 (args->fds, args->nfds * sizeof (struct pollfd),
		      0xDEADBEEF);
...
static poll_args *
get_poller_locked (struct pollfd *fds, nfds_t count)
{
  uint32_t key =
...
static void
cache_delete_locked (poll_args * args)
{
  if (!args->prev)
    {
      uint32_t key =
...
static void
cache_poller_locked (poll_args * args)
{
...
static void
cache_destroy_locked (poll_args * args)
{
  if (args->next)
    {
      args->next->prev = args->prev;
...
static void
decref_poll_result (poll_result * res)
{
...
void
remove_cvn (grpc_cv_node ** head, grpc_cv_node * target)
{
  if (target->next)
    {
  ...
  while (1)
    {
      ...
      int retval =
	g_cvfds.poll (result->fds, result->nfds, CV_POLL_PERIOD_MS);
...
static int
cvfd_poll (struct pollfd *fds, nfds_t nfds, int timeout)
{
  unsigned int i;
  int res, idx;
  grpc_cv_node *pollcv;
  ...
  pollcv->cv = &pollcv_cv;
  grpc_cv_node *fd_cvs =
    (grpc_cv_node *) gpr_malloc (nfds * sizeof (grpc_cv_node));
  ...
  for (i = 0; i < nfds; i++)
    {
      if (fds[i].fd < 0 && (fds[i].events & POLLIN))
	{
	  remove_cvn (&g_cvfds.cvfds[GRPC_FD_TO_IDX (fds[i].fd)].cvs,
		      &(fd_cvs[i]));
...
global_cv_fd_table_init ()
{
  ...
  g_cvfds.cvfds =
    (grpc_fd_node *) gpr_malloc (sizeof (grpc_fd_node) *
				 CV_DEFAULT_TABLE_SIZE);
  ...
  poll_cache.active_pollers =
    (poll_args **) gpr_malloc (sizeof (void *) * 32);
  for (unsigned int i = 0; i < poll_cache.size; i++)
    {
      poll_cache.active_pollers[i] = nullptr;
    }
...
grpc_init_poll_posix (bool explicit_request)
{
...
const grpc_event_engine_vtable *
grpc_init_poll_cv_posix (bool explicit_request)
{
  global_cv_fd_table_init ();
