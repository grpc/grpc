...
static grpc_fd* fd_create(int fd, const char* name) {
  grpc_fd* r = reinterpret_cast<grpc_fd*>(gpr_malloc(sizeof(*r)));
  gpr_mu_init(&r->mu);
...
static void pollset_add_fd(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset,
  ...
  if (pollset->fd_count == pollset->fd_capacity) {
    ...
    pollset->fds = reinterpret_cast<grpc_fd**>(
...
static grpc_error* pollset_work(grpc_exec_ctx* exec_ctx, grpc_pollset* pollset,
  ...
  if (pollset->local_wakeup_cache != nullptr) {
    ...
    worker.wakeup_fd = reinterpret_cast<grpc_cached_wakeup_fd*>(
...
#define POLLIN_CHECK (POLLIN | POLLHUP | POLLERR)
      ...
      if (pollset->fd_count + 2 <= inline_elements) {
        ...
        pfds = reinterpret_cast<struct pollfd*>(buf);
        watchers = reinterpret_cast<grpc_fd_watcher*>(
            reinterpret_cast<void*>(reinterpret_cast<char*>(buf) + pfd_size));
      }
...
static grpc_pollset_set* pollset_set_create(void) {
  grpc_pollset_set* pollset_set =
      reinterpret_cast<grpc_pollset_set*>(gpr_zalloc(sizeof(*pollset_set)));
...
static void pollset_set_add_pollset(grpc_exec_ctx* exec_ctx,
  ...
  if (pollset_set->pollset_count == pollset_set->pollset_capacity) {
    ...
    pollset_set->pollsets = reinterpret_cast<grpc_pollset**>(gpr_realloc(
...
static void pollset_set_add_pollset_set(grpc_exec_ctx* exec_ctx,
  ...
  if (bag->pollset_set_count == bag->pollset_set_capacity) {
    ...
    bag->pollset_sets = reinterpret_cast<grpc_pollset_set**>(
...
static void pollset_set_add_fd(grpc_exec_ctx* exec_ctx,
  ...
  if (pollset_set->fd_count == pollset_set->fd_capacity) {
    ...
    pollset_set->fds = reinterpret_cast<grpc_fd**>(
...
static void init_result(poll_args* pargs) {
  pargs->result =
      reinterpret_cast<poll_result*>(gpr_malloc(sizeof(poll_result)));
  gpr_ref_init(&pargs->result->refcount, 1);
  pargs->result->watchers = nullptr;
  pargs->result->watchcount = 0;
  pargs->result->fds = reinterpret_cast<struct pollfd*>(
...
static poll_args* get_poller_locked(struct pollfd* fds, nfds_t count) {
  ...
  poll_args* pargs =
      reinterpret_cast<poll_args*>(gpr_malloc(sizeof(struct poll_args)));
...
static void cache_poller_locked(poll_args* args) {
  if (poll_cache.count + 1 > poll_cache.size / 2) {
    ...
    poll_cache.count = 0;
    poll_cache.active_pollers = reinterpret_cast<poll_args**>(
        gpr_malloc(sizeof(void*) * poll_cache.size));
...
static int cvfd_poll(struct pollfd* fds, nfds_t nfds, int timeout) {
  ...
  gpr_mu_lock(&g_cvfds.mu);
  pollcv = reinterpret_cast<cv_node*>(gpr_malloc(sizeof(cv_node)));
  pollcv->next = nullptr;
  gpr_cv pollcv_cv;
  gpr_cv_init(&pollcv_cv);
  pollcv->cv = &pollcv_cv;
  cv_node* fd_cvs =
      reinterpret_cast<cv_node*>(gpr_malloc(nfds * sizeof(cv_node)));
  ...
  if (!skip_poll && nsockfds > 0) {
    struct pollfd* pollfds = reinterpret_cast<struct pollfd*>(
...
static void global_cv_fd_table_init() {
  ...
  g_cvfds.cvfds = reinterpret_cast<fd_node*>(
      gpr_malloc(sizeof(fd_node) * CV_DEFAULT_TABLE_SIZE));
  ...
  poll_cache.active_pollers =
      reinterpret_cast<poll_args**>(gpr_malloc(sizeof(void*) * 32));
  for (unsigned int i = 0; i < poll_cache.size; i++) {
    poll_cache.active_pollers[i] = nullptr;
  }
