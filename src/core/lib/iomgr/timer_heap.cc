...
/* Adjusts a heap so as to move a hole at position i closer to the root,
   ...
   its argument. */
static void
adjust_upwards (grpc_timer ** first, uint32_t i, grpc_timer * t)
{
  while (i > 0)
    {
      uint32_t parent = (uint32_t) (((int) i - 1) / 2);
      if (first[parent]->deadline <= t->deadline)
	break;
...
/* Adjusts a heap so as to move a hole at position i farther away from the root,
   ...
   position. */
static void
adjust_downwards (grpc_timer ** first, uint32_t i, uint32_t length,
		  grpc_timer * t)
{
  for (;;)
    {
	...
	first[right_child]->deadline ? right_child : left_child;
      if (t->deadline <= first[next_i]->deadline)
	break;
...
static void
maybe_shrink (grpc_timer_heap * heap)
{
  if (heap->timer_count >= 8 &&
      heap->timer_count <= heap->timer_capacity / SHRINK_FULLNESS_FACTOR / 2)
    {
      ...
      heap->timers =
	(grpc_timer **) gpr_realloc (heap->timers,
				     heap->timer_capacity *
				     sizeof (grpc_timer *));
    }
}
...
static void
note_changed_priority (grpc_timer_heap * heap, grpc_timer * timer)
{
...
void
grpc_timer_heap_init (grpc_timer_heap * heap)
{
  memset (heap, 0, sizeof (*heap));
}

void
grpc_timer_heap_destroy (grpc_timer_heap * heap)
{
  gpr_free (heap->timers);
}

int
grpc_timer_heap_add (grpc_timer_heap * heap, grpc_timer * timer)
{
...
void
grpc_timer_heap_remove (grpc_timer_heap * heap, grpc_timer * timer)
{
  uint32_t i = timer->heap_index;
...
int
grpc_timer_heap_is_empty (grpc_timer_heap * heap)
{
  return heap->timer_count == 0;
}

grpc_timer *
grpc_timer_heap_top (grpc_timer_heap * heap)
{
  return heap->timers[0];
}

void
grpc_timer_heap_pop (grpc_timer_heap * heap)
{
