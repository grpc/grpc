...
static int
retry_named_port_failure (int status, request * r,
      ...
      for (size_t i = 0; i < GPR_ARRAY_SIZE (svc); i++)
	{
	  if (strcmp (r->port, svc[i][0]) == 0)
		...
		(uv_getaddrinfo_t *) gpr_malloc (sizeof (uv_getaddrinfo_t));
	      ...
	      retry_status =
		uv_getaddrinfo (uv_default_loop (), req, getaddrinfo_cb,
...
static grpc_error *
handle_addrinfo_result (int status, struct addrinfo *result,
			grpc_resolved_addresses ** addresses)
{
...
static void
getaddrinfo_callback (uv_getaddrinfo_t * req, int status,
		      struct addrinfo *res)
{
...
static grpc_error *
try_split_host_port (const char *name,
		     const char *default_port, char **host, char **port)
{
		      ...
		      grpc_resolved_addresses ** addrs)
{
  ...
  s =
    uv_getaddrinfo (uv_default_loop (), req, getaddrinfo_callback, host, port,
		    hints);
