...
namespace grpc_core {
// TODO(yashkt) : Remove this class and replace it with a class that
// does not use ExecCtx
class Combiner {
 public:
  ...
  // either:
  // a pointer to the initiating exec ctx if that is the only exec_ctx
  // that has ever queued to this combiner, or NULL. If this is
  // non-null, it's not dereferencable (since the initiating exec_ctx
  // may have gone out of scope)
...
// thread. The actual thread executing actions may change over time (but
...
#define GRPC_COMBINER_DEBUG_ARGS
#define GRPC_COMBINER_REF(combiner, reason) \
  grpc_combiner_ref((combiner))
#define GRPC_COMBINER_UNREF(combiner, reason) \
  grpc_combiner_unref((combiner))
#endif
