...
static void combiner_finally_exec (grpc_closure * closure,
				   grpc_error * error);
...
static void
really_destroy (grpc_combiner * lock)
{
...
static void
start_destroy (grpc_combiner * lock)
{
  gpr_atm old_state =
    gpr_atm_full_fetch_add (&lock->state, -STATE_UNORPHANED);
  GRPC_COMBINER_TRACE (gpr_log
		       (GPR_DEBUG, "C:%p really_destroy old_state=%" PRIdPTR,
			lock, old_state));
  if (old_state == 1)
    {
      really_destroy (lock);
...
void
grpc_combiner_unref (grpc_combiner * lock GRPC_COMBINER_DEBUG_ARGS)
{
...
grpc_combiner *
grpc_combiner_ref (grpc_combiner * lock GRPC_COMBINER_DEBUG_ARGS)
{
...
static void
push_last_on_exec_ctx (grpc_combiner * lock)
{
...
static void
push_first_on_exec_ctx (grpc_combiner * lock)
{
...
static void
combiner_exec (grpc_closure * cl, grpc_error * error)
{
...
static void
offload (void *arg, grpc_error * error)
{
...
static void
queue_offload (grpc_combiner * lock)
{
...
grpc_combiner_continue_exec_ctx ()
{
  ...
  if (!lock->time_to_execute_final_list ||
      ...
      (gpr_atm_acq_load (&lock->state) >> 1) > 1)
    {
      ...
      GRPC_COMBINER_TRACE (gpr_log
			   (GPR_DEBUG, "C:%p maybe_finish_one n=%p", lock,
			    n));
      if (n == nullptr)
	{
      ...
      while (c != nullptr)
	{
	  ...
	  GRPC_COMBINER_TRACE (gpr_log
			       (GPR_DEBUG, "C:%p execute_final[%d] c=%p",
				lock, loops, c));
  ...
  gpr_atm old_state =
    gpr_atm_full_fetch_add (&lock->state, -STATE_ELEM_COUNT_LOW_BIT);
  GRPC_COMBINER_TRACE (gpr_log
		       (GPR_DEBUG, "C:%p finish old_state=%" PRIdPTR, lock,
			old_state));
...
static void
combiner_finally_exec (grpc_closure * closure, grpc_error * error)
{
...
static void
enqueue_finally (void *closure, grpc_error * error)
{
  combiner_finally_exec ((grpc_closure *) closure, GRPC_ERROR_REF (error));
}

grpc_closure_scheduler *
grpc_combiner_scheduler (grpc_combiner * combiner)
{
  return &combiner->scheduler;
}

grpc_closure_scheduler *
grpc_combiner_finally_scheduler (grpc_combiner * combiner)
{
