  ...
  grpc_core
{
  ...
  void
  LockfreeEvent::NotifyOn (grpc_closure * closure)
  {
    while (true)
      {
	gpr_atm
	...
	if (grpc_polling_trace.enabled ())
	  {
	    gpr_log (GPR_ERROR,
		     "LockfreeEvent::NotifyOn: %p curr=%p closure=%p", this,
		     (void *) curr, closure);
	  }
	switch (curr)
	  {
	  case kClosureNotReady:
	    {
	      /* kClosureNotReady -> <closure>.
	         ...
	         barrier. */
	      if (gpr_atm_rel_cas
		  (&state_, kClosureNotReady, (gpr_atm) closure))
		{
	  ...
	  case kClosureReady:
	    {
		  ...
		  (&state_, kClosureReady, kClosureNotReady))
		{
	  ...
	  default:
		...
		{
		  grpc_error *
		    shutdown_err = (grpc_error *) (curr & ~kShutdownBit);
  ...
  bool
  LockfreeEvent::SetShutdown (grpc_error * shutdown_err)
  {
    gpr_atm
      new_state = (gpr_atm) shutdown_err | kShutdownBit;
	...
	if (grpc_polling_trace.enabled ())
	  {
	    gpr_log (GPR_ERROR,
		     "LockfreeEvent::SetShutdown: %p curr=%p err=%s", &state_,
  ...
  LockfreeEvent::SetReady ()
  {
	...
	if (grpc_polling_trace.enabled ())
	  {
	    gpr_log (GPR_ERROR, "LockfreeEvent::SetReady: %p curr=%p",
		     &state_, (void *) curr);
	  }
	  ...
	  case kClosureNotReady:
	    {
	      /* No barrier required as we're transitioning to a state that does not
	         involve a closure */
	      if (gpr_atm_no_barrier_cas
		  (&state_, kClosureNotReady, kClosureReady))
		{
		  return;	/* early out */
		}
	      break;		/* retry */
