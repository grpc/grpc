...
class CallCombinerClosureList {
 public:
  ...
  GRPC_TRACE_LOG(call_combiner, INFO)  << "CallCombinerClosureList executing closure while already "                    "holding call_combiner "                 << call_combiner                 << ": closure=" << closures_[0].closure->DebugString()                 << " error=" << StatusToString(closures_[0].error)                 << " reason=" << closures_[0].reason;
