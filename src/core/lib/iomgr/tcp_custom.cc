...
static void tcp_free(grpc_custom_socket* s) {
  custom_tcp_endpoint* tcp = reinterpret_cast<custom_tcp_endpoint*>(s->endpoint);
...
static void tcp_unref(custom_tcp_endpoint* tcp, const char* reason, const char* file, int line) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_atm val = gpr_atm_no_barrier_load(&tcp->refcount.count);
    gpr_log(file, line, GPR_LOG_SEVERITY_ERROR, "TCP unref %p : %s %" PRIdPTR " -> %" PRIdPTR,
            tcp->socket, reason, val, val - 1);
  }
...
static void tcp_ref(custom_tcp_endpoint* tcp, const char* reason, const char* file, int line) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_atm val = gpr_atm_no_barrier_load(&tcp->refcount.count);
    gpr_log(file, line, GPR_LOG_SEVERITY_ERROR, "TCP   ref %p : %s %" PRIdPTR " -> %" PRIdPTR,
            tcp->socket, reason, val, val + 1);
  }
...
static void call_read_cb(custom_tcp_endpoint* tcp, grpc_error_handle error) {
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO, "TCP:%p call_cb %p %p:%p", tcp->socket, cb, cb->cb, cb->cb_arg);
    size_t i;
    ...
    for (i = 0; i < tcp->read_slices->count; i++) {
      ...
      gpr_log(GPR_INFO, "READ %p (peer=%s): %s", tcp, tcp->peer_string.c_str(), dump);
      gpr_free(dump);
    }
  }
  TCP_UNREF(tcp, "read");
...
static void custom_read_callback(grpc_custom_socket* socket, size_t nread,
  ...
  custom_tcp_endpoint* tcp = reinterpret_cast<custom_tcp_endpoint*>(socket->endpoint);
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (nread < tcp->read_slices->length) {
      ...
      grpc_slice_buffer_trim_end(tcp->read_slices, tcp->read_slices->length - nread, &garbage);
...
static void tcp_read_allocation_done(void* tcpp, grpc_error_handle error) {
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    char* buffer = reinterpret_cast<char*>(GRPC_SLICE_START_PTR(tcp->read_slices->slices[0]));
    size_t len = GRPC_SLICE_LENGTH(tcp->read_slices->slices[0]);
    grpc_custom_socket_vtable->read(tcp->socket, buffer, len, custom_read_callback);
  } else {
    grpc_slice_buffer_reset_and_unref_internal(tcp->read_slices);
...
static void endpoint_read(grpc_endpoint* ep, grpc_slice_buffer* read_slices, grpc_closure* cb,
                          bool /*urgent*/) {
  ...
  TCP_REF(tcp, "read");
  if (grpc_resource_user_alloc_slices(&tcp->slice_allocator, GRPC_TCP_DEFAULT_READ_SLICE_SIZE, 1,
                                      tcp->read_slices)) {
    tcp_read_allocation_done(tcp, GRPC_ERROR_NONE);
  }
}

static void custom_write_callback(grpc_custom_socket* socket, grpc_error_handle error) {
  grpc_core::ApplicationCallbackExecCtx callback_exec_ctx;
  grpc_core::ExecCtx exec_ctx;
  custom_tcp_endpoint* tcp = reinterpret_cast<custom_tcp_endpoint*>(socket->endpoint);
...
static void endpoint_write(grpc_endpoint* ep, grpc_slice_buffer* write_slices, grpc_closure* cb,
                           void* /*arg*/) {
  custom_tcp_endpoint* tcp = reinterpret_cast<custom_tcp_endpoint*>(ep);
  GRPC_CUSTOM_IOMGR_ASSERT_SAME_THREAD();
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    ...
    for (j = 0; j < write_slices->count; j++) {
      ...
      gpr_log(GPR_INFO, "WRITE %p (peer=%s): %s", tcp->socket, tcp->peer_string.c_str(), data);
      gpr_free(data);
    }
  }
  ...
  tcp->write_cb = cb;
  TCP_REF(tcp, "write");
  grpc_custom_socket_vtable->write(tcp->socket, tcp->write_slices, custom_write_callback);
}
...
static void endpoint_add_to_pollset_set(grpc_endpoint* ep, grpc_pollset_set* pollset) {
  // No-op. We're ignoring pollsets currently
  (void)ep;
  (void)pollset;
}

static void endpoint_delete_from_pollset_set(grpc_endpoint* ep, grpc_pollset_set* pollset) {
  // No-op. We're ignoring pollsets currently
  (void)ep;
  (void)pollset;
}
...
static void endpoint_shutdown(grpc_endpoint* ep, grpc_error_handle why) {
  ...
  if (!tcp->shutting_down) {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
      gpr_log(GPR_INFO, "TCP %p shutdown why=%s", tcp->socket, grpc_error_std_string(why).c_str());
    }
...
grpc_endpoint* custom_tcp_endpoint_create(grpc_custom_socket* socket,
  ...
  if (grpc_custom_socket_vtable->getsockname(
          ...
          reinterpret_cast<int*>(&resolved_local_addr.len)) != GRPC_ERROR_NONE) {
    tcp->local_address = "";
  } else {
