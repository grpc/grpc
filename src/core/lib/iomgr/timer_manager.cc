...
static void
timer_thread (void *completed_thread_ptr);

static void
gc_completed_threads (void)
{
  if (g_completed_threads != nullptr)
    {
      completed_thread *
	to_gc = g_completed_threads;
...
static void
start_timer_thread_and_unlock (void)
{
  ...
  completed_thread *
    ct = (completed_thread *) gpr_malloc (sizeof (*ct));
  ...
  if (!g_kicked)
    {
      ...
      // g_timed_waiter_generation
      uint64_t
	my_timed_waiter_generation = g_timed_waiter_generation - 1;
      ...
      if (next != GRPC_MILLIS_INF_FUTURE)
	{
	  if (!g_has_timed_waiter || (next < g_timed_waiter_deadline))
		...
		{
		  grpc_millis
		    wait_time = next - grpc_core::ExecCtx::Get ()->Now ();
...
static void
timer_thread_cleanup (completed_thread * ct)
{
  gpr_mu_lock (&g_mu);
...
void
grpc_timer_manager_shutdown (void)
{
  stop_threads ();
...
void
grpc_timer_manager_set_threading (bool threaded)
{
  if (threaded)
    {
      start_threads ();
