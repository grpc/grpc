			...
			grpc_tcp_server ** server)
{
  ...
  grpc_tcp_server *s =
    (grpc_tcp_server *) gpr_zalloc (sizeof (grpc_tcp_server));
      ...
      if (0 == strcmp (GRPC_ARG_ALLOW_REUSEPORT, args->args[i].key))
	{
	  if (args->args[i].type == GRPC_ARG_INTEGER)
	  ...
	  else
	    {
	      gpr_free (s);
	      return
		GRPC_ERROR_CREATE_FROM_STATIC_STRING (GRPC_ARG_ALLOW_REUSEPORT
						      " must be an integer");
	    }
	}
      else if (0 ==
	       strcmp (GRPC_ARG_EXPAND_WILDCARD_ADDRS, args->args[i].key))
	{
	  if (args->args[i].type == GRPC_ARG_INTEGER)
	    {
...
/* called when all listening endpoints have been shutdown, so no further
   ...
   things */
static void
deactivated_all_ports (grpc_tcp_server * s)
{
...
static void
on_read (void *arg, grpc_error * err)
{
  ...
  read_notifier_pollset =
    sp->server->
    pollsets[(size_t)
	     gpr_atm_no_barrier_fetch_add (&sp->server->
					   next_pollset_to_assign,
					   1) % sp->server->pollset_count];
  ...
  for (;;)
	...
	{
	  gpr_log (GPR_DEBUG, "SERVER_CONNECT: incoming connection: %s",
		   addr_str);
	}
			      ...
			      int requested_port, int *out_port)
{
  ...
  *out_port = -1;
  ...
  if (grpc_tcp_server_have_ifaddrs () && s->expand_wildcard_addrs)
    {
      return grpc_tcp_server_add_all_local_addrs (s, port_index,
						  requested_port, out_port);
    }
...
static grpc_error *
clone_port (grpc_tcp_listener * listener, unsigned count)
{
  grpc_tcp_listener *sp = nullptr;
  char *addr_str;
  char *name;
  ...
  for (unsigned i = 0; i < count; i++)
    {
      ...
      grpc_dualstack_mode dsmode;
      err =
	grpc_create_dualstack_socket (&listener->addr, SOCK_STREAM, 0,
				      &dsmode, &fd);
      if (err != GRPC_ERROR_NONE)
	return err;
			  ...
			  const grpc_resolved_address * addr, int *out_port)
{
  ...
  *out_port = -1;
  ...
  if ((err =
       grpc_tcp_server_add_addr (s, addr, port_index, 0, &dsmode,
				 &sp)) == GRPC_ERROR_NONE)
    {
      *out_port = sp->port;
    }
...
get_port_index (grpc_tcp_server * s, unsigned port_index)
{
...
unsigned
grpc_tcp_server_port_fd_count (grpc_tcp_server * s, unsigned port_index)
{
  unsigned num_fds = 0;
  gpr_mu_lock (&s->mu);
...
int
grpc_tcp_server_port_fd (grpc_tcp_server * s, unsigned port_index,
			 unsigned fd_index)
{
  gpr_mu_lock (&s->mu);
  grpc_tcp_listener *sp = get_port_index (s, port_index);
...
void
grpc_tcp_server_start (grpc_tcp_server * s, grpc_pollset ** pollsets,
  ...
  while (sp != nullptr)
	  ...
	  pollset_count > 1)
	{
	  GPR_ASSERT (GRPC_LOG_IF_ERROR
		      ("clone_port",
		       clone_port (sp, (unsigned) (pollset_count - 1))));
