...
grpc_error* grpc_tcp_server_create(grpc_exec_ctx* exec_ctx,
  ...
  grpc_tcp_server* s =
      reinterpret_cast<grpc_tcp_server*>(gpr_zalloc(sizeof(grpc_tcp_server)));
...
static void destroyed_port(grpc_exec_ctx* exec_ctx, void* server,
                           grpc_error* error) {
  grpc_tcp_server* s = reinterpret_cast<grpc_tcp_server*>(server);
  gpr_mu_lock(&s->mu);
  s->destroyed_ports++;
...
static void on_read(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* err) {
  ...
  for (;;) {
    ...
    grpc_tcp_server_acceptor* acceptor =
        reinterpret_cast<grpc_tcp_server_acceptor*>(
...
static grpc_error* clone_port(grpc_tcp_listener* listener, unsigned count) {
  ...
  for (unsigned i = 0; i < count; i++) {
    ...
    sp = reinterpret_cast<grpc_tcp_listener*>(
