...
const char *
poll_obj_string (poll_obj_type po_type)
{
  switch (po_type)
    {
    case POLL_OBJ_FD:
...
#ifndef NDEBUG
static void
pi_add_ref_dbg (polling_island * pi, const char *reason,
...
static void
pi_unref_dbg (polling_island * pi, const char *reason,
	      const char *file, int line)
{
  if (grpc_polling_trace.enabled ())
    {
...
static void
pi_add_ref (polling_island * pi)
{
  gpr_atm_no_barrier_fetch_add (&pi->ref_count, 1);
}

static void
pi_unref (polling_island * pi)
{
  ...
  if (1 == gpr_atm_full_fetch_add (&pi->ref_count, -1))
    {
      polling_island *next =
	(polling_island *) gpr_atm_acq_load (&pi->merged_to);
...
static void
polling_island_add_fds_locked (polling_island * pi, grpc_fd ** fds,
			       ...
			       grpc_error ** error)
{
  ...
  for (i = 0; i < fd_count; i++)
      ...
      if (pi->fd_cnt == pi->fd_capacity)
	{
	  ...
	  pi->fds =
	    (grpc_fd **) gpr_realloc (pi->fds,
				      sizeof (grpc_fd *) * pi->fd_capacity);
	}
...
static void
polling_island_add_wakeup_fd_locked (polling_island * pi,
  ...
  if (err < 0 && errno != EEXIST)
    {
      gpr_asprintf (&err_msg,
		    ...
		    "error: %d (%s)",
		    pi->epoll_fd, GRPC_WAKEUP_FD_GET_READ_FD (wakeup_fd),
...
/* The caller is expected to hold pi->mu lock before calling this function */
static void
polling_island_remove_all_fds_locked (polling_island * pi,
				      bool remove_fd_refs,
				      grpc_error ** error)
{
  int err;
  ...
  for (i = 0; i < pi->fd_cnt; i++)
      ...
      if (err < 0 && errno != ENOENT)
	{
	  gpr_asprintf (&err_msg,
			...
			"error: %d (%s)",
			pi->epoll_fd, i, pi->fds[i]->fd, errno,
			strerror (errno));
	  append_error (error, GRPC_OS_ERROR (errno, err_msg), err_desc);
	  gpr_free (err_msg);
	}
...
/* The caller is expected to hold pi->mu lock before calling this function */
static void
polling_island_remove_fd_locked (polling_island * pi, grpc_fd * fd,
...
polling_island_create (grpc_fd * initial_fd, grpc_error ** error)
{
...
static void
polling_island_delete (polling_island * pi)
{
...
static polling_island *
polling_island_maybe_get_latest (polling_island * pi)
{
...
static polling_island *
polling_island_lock (polling_island * pi)
{
...
*/
static void
polling_island_lock_pair (polling_island ** p, polling_island ** q)
{
...
static void
polling_island_unlock_pair (polling_island * p, polling_island * q)
{
  if (p == q)
    {
      gpr_mu_unlock (&p->mu);
...
static polling_island *
polling_island_merge (polling_island * p,
		      polling_island * q, grpc_error ** error)
{
...
#ifndef NDEBUG
static void
fd_ref (grpc_fd * fd, const char *reason, const char *file, int line)
{
  ref_by (fd, 2, reason, file, line);
}

static void
fd_unref (grpc_fd * fd, const char *reason, const char *file, int line)
{
...
poller_kick_init ()
{
  signal (grpc_wakeup_signal, sig_handler);
}
...
static grpc_error *
pollset_worker_kick (grpc_pollset_worker * worker)
{
 ...
 * be locked) */
static int
pollset_has_workers (grpc_pollset * p)
{
...
static grpc_error *
pollset_kick (grpc_pollset * p, grpc_pollset_worker * specific_worker)
{
	  ...
	  if (pollset_has_workers (p))
		   ...
		   worker = worker->next)
		{
		  if (gpr_tls_get (&g_current_thread_worker) !=
		      (intptr_t) worker)
		    {
		      append_error (&error, pollset_worker_kick (worker),
				    err_desc);
		    }
		}
...
pollset_init (grpc_pollset * pollset, gpr_mu ** mu)
{
  ...
  *mu = &pollset->po.mu;
  ...
  pollset->root_worker.next = pollset->root_worker.prev =
    &pollset->root_worker;
...
static void
pollset_release_polling_island (grpc_pollset * ps, const char *reason)
{
  if (ps->po.pi != nullptr)
    {
...
static void
finish_shutdown_locked (grpc_pollset * pollset)
{
...
static void
pollset_work_and_unlock (grpc_pollset * pollset,
			 ...
			 sigset_t * sig_mask, grpc_error ** error)
{
  ...
  ep_rv =
    epoll_pwait (epoll_fd, ep_ev, GRPC_EPOLL_MAX_EVENTS, timeout_ms,
		 sig_mask);
      ...
      if (data_ptr == &polling_island_wakeup_fd)
	{
	  GRPC_POLLING_TRACE
	    ("pollset_work: pollset: %p, worker: %p polling island (epoll_fd: "
	     "%d) got merged", (void *) pollset, (void *) worker, epoll_fd);
...
static void
add_poll_object (poll_obj * bag, poll_obj_type bag_type,
...
retry:
  ...
  if (item->pi == bag->pi)
      ...
      if (pi_new == nullptr)
	{
	      ...
	      if (item->pi != nullptr)
		{
		  GRPC_POLLING_TRACE
		    ("add_poll_object: Raced creating new polling island. pi_new: %p "
		     "(fd: %d, %s: %p)", (void *) pi_new,
		     FD_FROM_PO (item)->fd, poll_obj_string (bag_type),
		     (void *) bag);
	  ...
	  GRPC_POLLING_TRACE
	    ("add_poll_object: Created new polling island. pi_new: %p (%s: %p, "
	     "%s: %p)", (void *) pi_new, poll_obj_string (item_type),
	     (void *) item, poll_obj_string (bag_type), (void *) bag);
	}
      else
	{
	  GRPC_POLLING_TRACE
	...
	("add_poll_obj: item->pi was NULL. pi_new: %p (item(%s): %p, "
	 "bag(%s): %p)", (void *) pi_new, poll_obj_string (item_type),
	 (void *) item, poll_obj_string (bag_type), (void *) bag);
    }
...
static void
pollset_set_destroy (grpc_pollset_set * pss)
{
...
static void
pollset_set_add_pollset_set (grpc_pollset_set * bag, grpc_pollset_set * item)
{
 ...
 * */
void *
grpc_fd_get_polling_island (grpc_fd * fd)
{
  polling_island *pi;
...
void *
grpc_pollset_get_polling_island (grpc_pollset * ps)
{
  polling_island *pi;
...
bool
grpc_are_polling_islands_equal (void *p, void *q)
{
...
grpc_init_epollsig_linux (bool explicit_request)
{
  /* If use of signals is disabled, we cannot use epoll engine */
  if (is_grpc_wakeup_signal_initialized && grpc_wakeup_signal < 0)
    {
      gpr_log (GPR_ERROR,
	       "Skipping epollsig because use of signals is disabled.");
      return nullptr;
    }
