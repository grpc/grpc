...
static void fd_destroy(grpc_exec_ctx* exec_ctx, void* arg, grpc_error* error) {
  grpc_fd* fd = reinterpret_cast<grpc_fd*>(arg);
...
static grpc_error* pollable_create(pollable_type type, pollable** p) {
  ...
  *p = reinterpret_cast<pollable*>(gpr_malloc(sizeof(**p)));
...
static grpc_pollset_set* pollset_set_create(void) {
  grpc_pollset_set* pss =
      reinterpret_cast<grpc_pollset_set*>(gpr_zalloc(sizeof(*pss)));
  gpr_mu_init(&pss->mu);
...
static void pollset_set_add_fd(grpc_exec_ctx* exec_ctx, grpc_pollset_set* pss,
  ...
  if (pss->fd_count == pss->fd_capacity) {
    ...
    pss->fds = reinterpret_cast<grpc_fd**>(
...
static void pollset_set_add_pollset(grpc_exec_ctx* exec_ctx,
  ...
  if (pss->pollset_count == pss->pollset_capacity) {
    ...
    pss->pollsets = reinterpret_cast<grpc_pollset**>(gpr_realloc(
...
static void pollset_set_add_pollset_set(grpc_exec_ctx* exec_ctx,
  ...
  if (a->fd_capacity < a->fd_count + b->fd_count) {
    ...
    a->fds = reinterpret_cast<grpc_fd**>(
  ...
  if (a->pollset_capacity < a->pollset_count + b->pollset_count) {
    ...
    a->pollsets = reinterpret_cast<grpc_pollset**>(
