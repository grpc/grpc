...
typedef enum
{ PO_MULTI, PO_FD, PO_EMPTY } pollable_type;
...
static const char *
pollable_type_string (pollable_type t)
{
  switch (t)
    {
    case PO_MULTI:
...
static char *
pollable_desc (pollable * p)
{
  char *out;
  gpr_asprintf (&out, "type=%s epfd=%d wakeup=%d",
		pollable_type_string (p->type), p->epfd, p->wakeup.read_fd);
  return out;
}
...
typedef enum
{ PWLINK_POLLABLE = 0, PWLINK_POLLSET, PWLINK_COUNT } pwlinks;
...
static bool
append_error (grpc_error ** composite, grpc_error * error, const char *desc)
{
...
static void
fd_destroy (void *arg, grpc_error * error)
{
  ...
  if (old == n)
    {
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE
			  (fd_destroy, fd, grpc_schedule_on_exec_ctx),
			  GRPC_ERROR_NONE);
    }
  else
    {
...
static grpc_fd *
fd_create (int fd, const char *name)
{
...
static void
fd_orphan (grpc_fd * fd, grpc_closure * on_done, int *release_fd,
...
static grpc_pollset *
fd_get_read_notifier_pollset (grpc_fd * fd)
{
...
static void
fd_shutdown (grpc_fd * fd, grpc_error * why)
{
...
static void
fd_notify_on_read (grpc_fd * fd, grpc_closure * closure)
{
  fd->read_closure->NotifyOn (closure);
}

static void
fd_notify_on_write (grpc_fd * fd, grpc_closure * closure)
{
...
static grpc_error *
pollable_create (pollable_type type, pollable ** p)
{
  *p = nullptr;
...
#ifdef NDEBUG
static pollable *
pollable_ref (pollable * p)
{
#else
static pollable *
pollable_ref (pollable * p, int line, const char *reason)
{
...
#ifdef NDEBUG
static void
pollable_unref (pollable * p)
{
#else
static void
pollable_unref (pollable * p, int line, const char *reason)
{
  if (p == nullptr)
    return;
  if (grpc_trace_pollable_refcount.enabled ())
...
static grpc_error *
pollable_add_fd (pollable * p, grpc_fd * fd)
{
...
static void
pollset_maybe_finish_shutdown (grpc_pollset * pollset)
{
  if (grpc_polling_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
 ...
 * held */
static grpc_error *
kick_one_worker (grpc_pollset_worker * specific_worker)
{
...
static grpc_error *
pollset_kick (grpc_pollset * pollset, grpc_pollset_worker * specific_worker)
{
	  ...
	  if (pollset->root_worker == nullptr)
	    {
	      if (grpc_polling_trace.enabled ())
		{
		  gpr_log (GPR_DEBUG, "PS:%p kicked_any_without_poller",
			   pollset);
		}
	  ...
	  else
	    {
	      ...
	      return kick_one_worker (pollset->root_worker->
				      links[PWLINK_POLLSET].next);
	    }
	}
      else
	{
...
static grpc_error *
pollset_kick_all (grpc_pollset * pollset)
{
  grpc_error *error = GRPC_ERROR_NONE;
...
static void
pollset_init (grpc_pollset * pollset, gpr_mu ** mu)
{
...
static int
poll_deadline_to_millis_timeout (grpc_millis millis)
{
...
static void
fd_become_readable (grpc_fd * fd, grpc_pollset * notifier)
{
...
static void
fd_become_writable (grpc_fd * fd)
{
  fd->write_closure->SetReady ();
}

static grpc_error *
fd_get_or_become_pollable (grpc_fd * fd, pollable ** p)
{
...
static void
pollset_shutdown (grpc_pollset * pollset, grpc_closure * closure)
{
...
static grpc_error *
pollable_process_events (grpc_pollset * pollset,
			 pollable * pollable_obj, bool drain)
{
  static const char *err_desc = "pollset_process_events";
  grpc_error *error = GRPC_ERROR_NONE;
  for (int i = 0; (drain || i < MAX_EPOLL_EVENTS_HANDLED_EACH_POLL_CALL) &&
       pollable_obj->event_cursor != pollable_obj->event_count; i++)
    {
      ...
      if (1 & (intptr_t) data_ptr)
	{
	  if (grpc_polling_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG, "PS:%p got pollset_wakeup %p", pollset,
		       data_ptr);
	    }
	  append_error (&error,
			grpc_wakeup_fd_consume_wakeup ((grpc_wakeup_fd
							*) ((~(intptr_t) 1) &
							    (intptr_t)
							    data_ptr)),
			err_desc);
	  ...
	  if (grpc_polling_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG,
		       ...
		       "write=%d", pollset, fd, cancel, read_ev, write_ev);
	    }
	  if (read_ev || cancel)
	    {
...
static void
pollset_destroy (grpc_pollset * pollset)
{
...
static grpc_error *
pollable_epoll (pollable * p, grpc_millis deadline)
{
...
static bool
worker_insert (grpc_pollset_worker ** root_worker,
...
typedef enum
{ WRR_NEW_ROOT, WRR_EMPTIED, WRR_REMOVED } worker_remove_result;

static worker_remove_result
worker_remove (grpc_pollset_worker ** root_worker,
	       grpc_pollset_worker * worker, pwlinks link)
{
...
static bool
begin_worker (grpc_pollset * pollset, grpc_pollset_worker * worker,
...
static void
end_worker (grpc_pollset * pollset, grpc_pollset_worker * worker,
...
static grpc_error *
pollset_work (grpc_pollset * pollset,
	      grpc_pollset_worker ** worker_hdl, grpc_millis deadline)
{
...
#define WORKER_PTR (&worker)
  ...
  if (grpc_polling_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "PS:%p work hdl=%p worker=%p now=%" PRIdPTR " deadline=%"
	       PRIdPTR " kwp=%d pollable=%p", pollset, worker_hdl, WORKER_PTR,
  ...
  if (pollset->kicked_without_poller)
      ...
      if (begin_worker (pollset, WORKER_PTR, worker_hdl, deadline))
	{
	  ...
	  if (WORKER_PTR->pollable_obj->event_cursor ==
	    ...
	    {
	      append_error (&error,
			    pollable_epoll (WORKER_PTR->pollable_obj,
					    deadline), err_desc);
	    }
	  append_error (&error,
			pollable_process_events (pollset,
						 WORKER_PTR->pollable_obj,
						 false), err_desc);
...
static grpc_error *
pollset_transition_pollable_from_empty_to_fd_locked (grpc_pollset * pollset,
						     grpc_fd * fd)
{
  ...
  POLLABLE_UNREF (pollset->active_pollable, "pollset");
  append_error (&error,
		fd_get_or_become_pollable (fd, &pollset->active_pollable),
		err_desc);
  return error;
}

static grpc_error *
pollset_transition_pollable_from_fd_to_multi_locked (grpc_pollset * pollset,
						     grpc_fd * and_add_fd)
{
  static const char *err_desc =
  ...
  if (append_error
	...
	{
	  append_error (&error,
			pollable_add_fd (pollset->active_pollable,
					 and_add_fd), err_desc);
	}
    }
  return error;
...
static grpc_error *
pollset_add_fd_locked (grpc_pollset * pollset, grpc_fd * fd)
{
  grpc_error *error = GRPC_ERROR_NONE;
  ...
  switch (pollset->active_pollable->type)
    ...
    case PO_FD:
      ...
      if ((gpr_atm_no_barrier_load
	   (&pollset->active_pollable->owner_fd->refst) & 1) == 0)
	{
	  error =
	    pollset_transition_pollable_from_empty_to_fd_locked (pollset, fd);
	}
...
static grpc_error *
pollset_as_multipollable_locked (grpc_pollset * pollset,
				 pollable ** pollable_obj)
{
  grpc_error *error = GRPC_ERROR_NONE;
  pollable *po_at_start =
  ...
  switch (pollset->active_pollable->type)
	...
	{
	  error =
	    pollset_transition_pollable_from_fd_to_multi_locked (pollset,
								 nullptr);
	}
      gpr_mu_unlock (&po_at_start->owner_fd->orphan_mu);
      break;
...
static void
pollset_add_fd (grpc_pollset * pollset, grpc_fd * fd)
{
  gpr_mu_lock (&pollset->mu);
...
pss_lock_adam (grpc_pollset_set * pss)
{
  gpr_mu_lock (&pss->mu);
...
static void
pollset_set_unref (grpc_pollset_set * pss)
{
  if (pss == nullptr)
    return;
  if (!gpr_unref (&pss->refs))
    return;
...
static void
pollset_set_add_fd (grpc_pollset_set * pss, grpc_fd * fd)
{
  if (grpc_polling_trace.enabled ())
    {
  ...
  if (pss->fd_count == pss->fd_capacity)
    {
      ...
      pss->fds =
	(grpc_fd **) gpr_realloc (pss->fds,
				  pss->fd_capacity * sizeof (*pss->fds));
    }
...
static void
pollset_set_del_fd (grpc_pollset_set * pss, grpc_fd * fd)
{
  if (grpc_polling_trace.enabled ())
    {
...
static void
pollset_set_del_pollset (grpc_pollset_set * pss, grpc_pollset * ps)
{
  if (grpc_polling_trace.enabled ())
    {
...
static grpc_error *
add_fds_to_pollsets (grpc_fd ** fds, size_t fd_count,
		     grpc_pollset ** pollsets,
		     size_t pollset_count,
  ...
  for (size_t i = 0; i < fd_count; i++)
      ...
      else
	{
	  for (size_t j = 0; j < pollset_count; j++)
	    {
	      append_error (&error,
			    pollable_add_fd (pollsets[j]->active_pollable,
					     fds[i]), err_desc);
	    }
	  gpr_mu_unlock (&fds[i]->orphan_mu);
...
static void
pollset_set_add_pollset (grpc_pollset_set * pss, grpc_pollset * ps)
{
  if (grpc_polling_trace.enabled ())
    {
  ...
  if (!GRPC_LOG_IF_ERROR (err_desc,
			  pollset_as_multipollable_locked (ps,
							   &pollable_obj)))
    {
  ...
  if (pss->pollset_count == pss->pollset_capacity)
    {
      ...
      pss->pollsets =
	(grpc_pollset **) gpr_realloc (pss->pollsets,
...
static void
pollset_set_add_pollset_set (grpc_pollset_set * a, grpc_pollset_set * b)
{
  if (grpc_polling_trace.enabled ())
    {
...
static void
pollset_set_del_pollset_set (grpc_pollset_set * bag, grpc_pollset_set * item)
{
}
...
grpc_init_epollex_linux (bool explicitly_requested)
{
