...
static void
validate_non_pending_timer (grpc_timer * t)
{
...
static gpr_atm
compute_min_deadline (timer_shard * shard)
{
...
{
  size_t i;
  run_some_expired_timers (GPR_ATM_MAX, nullptr,
...
static void
note_deadline_change (timer_shard * shard)
{
		 ...
		 grpc_closure * closure)
{
  ...
  if (grpc_timer_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "TIMER %p: SET %" PRIdPTR " now %" PRIdPTR " call %p[%p]",
...
static int
refill_heap (timer_shard * shard, gpr_atm now)
{
...
/* This pops the next non-cancelled timer with deadline <= now from the
   ...
   REQUIRES: shard->mu locked */
static grpc_timer *
pop_one (timer_shard * shard, gpr_atm now)
{
  grpc_timer *timer;
  for (;;)
    {
	...
	{
	  gpr_log (GPR_DEBUG,
		   "TIMER %p: FIRE %" PRIdPTR "ms late via %s scheduler",
...
static size_t
pop_timers (timer_shard * shard, gpr_atm now,
...
static grpc_timer_check_result
run_some_expired_timers (gpr_atm now, gpr_atm * next, grpc_error * error)
{
...
grpc_timer_check (grpc_millis * next)
{
  ...
  if (now < min_timer)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "TIMER CHECK SKIP: now=%" PRIdPTR " min_timer=%" PRIdPTR,
		   now, min_timer);
	}
      return GRPC_TIMERS_CHECKED_AND_EMPTY;
    }
