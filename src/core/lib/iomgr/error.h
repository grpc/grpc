...
const char *
grpc_error_string (grpc_error * error);

/// Create an error - but use GRPC_ERROR_CREATE instead
grpc_error *
grpc_error_create (const char *file, int line, grpc_slice desc,
...
#ifndef NDEBUG
grpc_error *
grpc_error_ref (grpc_error * err, const char *file, int line);
void
grpc_error_unref (grpc_error * err, const char *file, int line);
...
#else
grpc_error *
grpc_error_ref (grpc_error * err);
void
grpc_error_unref (grpc_error * err);
#define GRPC_ERROR_REF(err) grpc_error_ref(err)
#define GRPC_ERROR_UNREF(err) grpc_error_unref(err)
#endif

grpc_error *
grpc_error_set_int (grpc_error * src, grpc_error_ints which, intptr_t value)
  GRPC_MUST_USE_RESULT;
     bool
     grpc_error_get_int (grpc_error * error, grpc_error_ints which,
			 intptr_t * p);
/// This call takes ownership of the slice; the error is responsible for
/// eventually unref-ing it.
     grpc_error *
     grpc_error_set_str (grpc_error * src, grpc_error_strs which,
			 grpc_slice str)
  GRPC_MUST_USE_RESULT;
/// Returns false if the specified string is not set.
/// Caller does NOT own the slice.
     bool
     grpc_error_get_str (grpc_error * error, grpc_error_strs which,
			 grpc_slice * s);
...
/// child error.
     grpc_error *
     grpc_error_add_child (grpc_error * src, grpc_error * child)
  GRPC_MUST_USE_RESULT;
     grpc_error *
     grpc_os_error (const char *file, int line, int err,
		    const char *call_name)
  GRPC_MUST_USE_RESULT;

     inline grpc_error *
     grpc_assert_never_ok (grpc_error * error)
{
...
grpc_error *
grpc_wsa_error (const char *file, int line, int err, const char *call_name)
  GRPC_MUST_USE_RESULT;
...
#define GRPC_WSA_ERROR(err, call_name) \
  grpc_wsa_error(__FILE__, __LINE__, err, call_name)
     ...
     bool
     grpc_log_if_error (const char *what, grpc_error * error,
