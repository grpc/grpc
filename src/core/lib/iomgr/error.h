...
/// See https://github.com/grpc/grpc/blob/master/doc/core/grpc-error.md
/// for a full write up of this object.
...
typedef enum {
  ...
  GRPC_ERROR_INT_INDEX =
      static_cast<int>(grpc_core::StatusIntProperty::kIndex),
  /// context sensitive size associated with the error
  GRPC_ERROR_INT_SIZE =
      static_cast<int>(grpc_core::StatusIntProperty::kSize),
  ...
  GRPC_ERROR_INT_FD =
      static_cast<int>(grpc_core::StatusIntProperty::kFd),
  /// HTTP status (i.e. 404)
  GRPC_ERROR_INT_HTTP_STATUS =
      static_cast<int>(grpc_core::StatusIntProperty::kHttpStatus),
  /// chttp2: did the error occur while a write was in progress
  GRPC_ERROR_INT_OCCURRED_DURING_WRITE = static_cast<int>(
      grpc_core::StatusIntProperty::kOccurredDuringWrite),
  /// channel connectivity state associated with the error
  GRPC_ERROR_INT_CHANNEL_CONNECTIVITY_STATE = static_cast<int>(
...
typedef enum {
  ...
  GRPC_ERROR_STR_FILE =
      static_cast<int>(grpc_core::StatusStrProperty::kFile),
  ...
  GRPC_ERROR_STR_KEY =
      static_cast<int>(grpc_core::StatusStrProperty::kKey),
  /// value associated with the error
  GRPC_ERROR_STR_VALUE =
      static_cast<int>(grpc_core::StatusStrProperty::kValue),
...
// debug only toggles that allow for a sanity to check that ensures we
// will never create any errors in the per-RPC hotpath.
...
// Create an error that references some other errors. This function adds
// a reference to each error in errs - it does not consume an existing
// reference
#define GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING(desc, errs,   \
                                                         count)        \
  grpc_status_create(absl::StatusCode::kUnknown, desc, DEBUG_LOCATION, \
                     count, errs)
#define GRPC_ERROR_CREATE_REFERENCING_FROM_COPIED_STRING(desc, errs,   \
                                                         count)        \
  grpc_status_create(absl::StatusCode::kUnknown, desc, DEBUG_LOCATION, \
                     count, errs)

// Consumes all the errors in the vector and forms a referencing error
// from them. If the vector is empty, return GRPC_ERROR_NONE.
...
static absl::Status grpc_status_create_from_vector(
  ...
  if (error_list->size() != 0) {
    error = grpc_status_create(absl::StatusCode::kUnknown, desc,
                               DEBUG_LOCATION, error_list->size(),
                               error_list->data());
    error_list->clear();
  }
  return error;
...
absl::Status grpc_os_error(const grpc_core::DebugLocation& location,
                           int err,
...
/// create an error associated with errno!=0 (an 'operating system'
/// error)
#define GRPC_OS_ERROR(err, call_name) \
  grpc_assert_never_ok(grpc_os_error(DEBUG_LOCATION, err, call_name))

absl::Status grpc_wsa_error(const grpc_core::DebugLocation& location,
                            int err,
                            const char* call_name) GRPC_MUST_USE_RESULT;
...
/// The following "special" errors can be propagated without allocating
/// memory. They are always even so that other code (particularly
/// combiner locks, polling engines) can safely use the lower bit for
/// themselves.
...
void grpc_error_do_unref(grpc_error_handle err, const char* file,
                         int line);
...
/// Create an error (this is the preferred way of generating an error
/// that is
///   not due to a system call - for system calls, use GRPC_OS_ERROR or
///   GRPC_WSA_ERROR as appropriate)
/// \a referencing is an array of num_referencing elements indicating
/// one or more errors that are believed to have contributed to this one
...
#define GRPC_ERROR_CREATE_FROM_STRING_VIEW(desc)                   \
  grpc_error_create(                                               \
      __FILE__, __LINE__,                                          \
      grpc_slice_from_copied_buffer((desc).data(), (desc).size()), \
      NULL, 0)
...
grpc_error_handle grpc_error_set_int(
    grpc_error_handle src, grpc_error_ints which,
    intptr_t value) GRPC_MUST_USE_RESULT;
/// It is an error to pass nullptr as `p`. Caller should allocate a
/// phony intptr_t for `p`, even if the value of `p` is not used.
...
/// Add a child error: an error that is believed to have contributed to
/// this error occurring. Allows root causing high level errors from
/// lower level errors that contributed to them. The src error takes
/// ownership of the child error.
///
/// Edge Conditions -
/// 1) If either of \a src or \a child is GRPC_ERROR_NONE, returns a
/// reference to the other argument. 2) If both \a src and \a child are
/// GRPC_ERROR_NONE, returns GRPC_ERROR_NONE. 3) If \a src and \a child
/// point to the same error, returns a single reference. (Note that, 2
/// references should have been received to the error in this case.)
grpc_error_handle grpc_error_add_child(grpc_error_handle src,
                                       grpc_error_handle child)
    GRPC_MUST_USE_RESULT;

bool grpc_log_error(const char* what, grpc_error_handle error,
                    const char* file, int line);
inline bool grpc_log_if_error(const char* what, grpc_error_handle error,
                              const char* file, int line) {
  return error == GRPC_ERROR_NONE
             ? true
