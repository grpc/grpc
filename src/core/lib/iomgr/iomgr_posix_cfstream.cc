...
/// CFStream is build-enabled on iOS by default and disabled by default
/// on other platforms (see port_platform.h). To enable CFStream build
/// on another platform, the users need to define macro
/// "GRPC_CFSTREAM=1" when building gRPC.
///
/// When CFStream is to be built (either by default on iOS or by macro
/// on other platforms), the users can disable CFStream with environment
/// variable "grpc_cfstream=0". This will let gRPC to fallback to use
/// POSIX sockets. In addition, the users may choose to use an
/// alternative CFRunLoop based pollset "ev_apple" by setting
/// environment variable "GRPC_CFSTREAM_RUN_LOOP=1". This pollset
/// resolves a bug from Apple when CFStream streams dispatch events to
/// dispatch queues. The caveat of this pollset is that users may not be
/// able to run a gRPC server in the same process.
...
static void apple_iomgr_platform_init(void) {
...
CFStreamEnv ParseEnvForCFStream() {
  ...
  char* enable_cfstream_run_loop_str =
  ...
  env.enable_cfstream_run_loop =
      enable_cfstream_run_loop_str != nullptr &&
...
void grpc_set_default_iomgr_platform() {
  ...
  if (!env.enable_cfstream) {
    ...
    // Use CFStream with dispatch queue for client; use POSIX sockets
    // for server
