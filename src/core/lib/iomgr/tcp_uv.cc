...
{
  return grpc_error_set_str (grpc_error_set_int (src_error,
						 /* All tcp errors are marked with UNAVAILABLE so that application may
						  * choose to retry. */
						 GRPC_ERROR_INT_GRPC_STATUS,
						 GRPC_STATUS_UNAVAILABLE),
			     GRPC_ERROR_STR_TARGET_ADDRESS,
			     grpc_slice_from_copied_string (tcp->
							    peer_string));
}
...
static void
uv_close_callback (uv_handle_t * handle)
{
...
static void
alloc_uv_buf (uv_handle_t * handle, size_t suggested_size, uv_buf_t * buf)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_tcp *tcp = (grpc_tcp *) handle->data;
  (void) suggested_size;
...
static void
read_callback (uv_stream_t * stream, ssize_t nread, const uv_buf_t * buf)
{
  grpc_error *error;
  grpc_core::ExecCtx exec_ctx;
...
tcp_read_allocation_done (void *tcpp, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
    {
      status =
	uv_read_start ((uv_stream_t *) tcp->handle, alloc_uv_buf,
		       read_callback);
      if (status != 0)
	{
	  error =
	    tcp_annotate_error (GRPC_ERROR_CREATE_FROM_STATIC_STRING
				("TCP Read failed at start"), tcp);
	  error =
...
static void
uv_endpoint_read (grpc_endpoint * ep, grpc_slice_buffer * read_slices,
		  grpc_closure * cb)
{
...
static void
uv_endpoint_write (grpc_endpoint * ep,
		   grpc_slice_buffer * write_slices, grpc_closure * cb)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
  uv_buf_t *buffers;
  ...
  if (tcp->shutting_down)
    {
      GRPC_CLOSURE_SCHED (cb,
			  ...
			  (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			   ("TCP socket is shutting down"), tcp));
      return;
    }
...
static void
uv_add_to_pollset (grpc_endpoint * ep, grpc_pollset * pollset)
{
...
static void
uv_add_to_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset)
{
  // No-op. We're ignoring pollsets currently
  (void) ep;
  (void) pollset;
...
static void
uv_delete_from_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset)
{
  // No-op. We're ignoring pollsets currently
  (void) ep;
  (void) pollset;
...
static void
shutdown_callback (uv_shutdown_t * req, int status)
{
}

static void
uv_endpoint_shutdown (grpc_endpoint * ep, grpc_error * why)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
  if (!tcp->shutting_down)
    {
...
uv_get_resource_user (grpc_endpoint * ep)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
