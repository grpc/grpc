...
static grpc_error_handle tcp_error_create(const char* desc, int status) {
   ...
   * choose to retry. */
  error = grpc_error_set_int(error, GRPC_ERROR_INT_GRPC_STATUS, GRPC_STATUS_UNAVAILABLE);
...
static void alloc_uv_buf(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf) {
  uv_socket_t* uv_socket = (uv_socket_t*)((grpc_custom_socket*)handle->data)->impl;
  (void)suggested_size;
...
static void uv_socket_read(grpc_custom_socket* socket, char* buffer, size_t length,
                           grpc_custom_read_callback read_cb) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  int status;
  ...
  // TODO(murgatroid99): figure out what the return value here means
  status = uv_read_start((uv_stream_t*)uv_socket->handle, (uv_alloc_cb)alloc_uv_buf,
...
void uv_socket_write(grpc_custom_socket* socket, grpc_slice_buffer* write_slices,
                     grpc_custom_write_callback write_cb) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  ...
  // TODO(murgatroid99): figure out what the return value here means
  uv_write(write_req, (uv_stream_t*)uv_socket->handle, uv_buffers, write_slices->count,
           uv_write_callback);
}
...
static void uv_socket_close(grpc_custom_socket* socket, grpc_custom_close_callback close_cb) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  uv_socket->close_cb = close_cb;
  uv_close((uv_handle_t*)uv_socket->handle, uv_close_callback);
}

static grpc_error_handle uv_socket_init_helper(uv_socket_t* uv_socket, int domain) {
...
static grpc_error_handle uv_socket_init(grpc_custom_socket* socket, int domain) {
...
static grpc_error_handle uv_socket_getpeername(grpc_custom_socket* socket,
                                               const grpc_sockaddr* addr, int* addr_len) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  int err = uv_tcp_getpeername(uv_socket->handle, (struct sockaddr*)IGNORE_CONST(addr), addr_len);
...
static grpc_error_handle uv_socket_getsockname(grpc_custom_socket* socket,
                                               const grpc_sockaddr* addr, int* addr_len) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  int err = uv_tcp_getsockname(uv_socket->handle, (struct sockaddr*)IGNORE_CONST(addr), addr_len);
...
static void accept_new_connection(grpc_custom_socket* socket) {
  ...
  if (uv_socket->accept_error != GRPC_ERROR_NONE) {
    ...
    GPR_ASSERT(uv_accept((uv_stream_t*)uv_socket->handle, (uv_stream_t*)uv_new_socket->handle) ==
               0);
...
void uv_socket_accept(grpc_custom_socket* socket, grpc_custom_socket* new_socket,
                      grpc_custom_accept_callback accept_cb) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
...
static grpc_error_handle uv_socket_bind(grpc_custom_socket* socket, const grpc_sockaddr* addr,
                                        size_t len, int flags) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  int status = uv_tcp_bind((uv_tcp_t*)uv_socket->handle, (struct sockaddr*)addr, 0);
  return tcp_error_create("Failed to bind to port", status);
}

static grpc_error_handle uv_socket_listen(grpc_custom_socket* socket) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  int status = uv_listen((uv_stream_t*)uv_socket->handle, SOMAXCONN, uv_on_connect);
...
static void uv_socket_connect(grpc_custom_socket* socket, const grpc_sockaddr* addr, size_t len,
                              grpc_custom_connect_callback connect_cb) {
  uv_socket_t* uv_socket = (uv_socket_t*)socket->impl;
  uv_socket->connect_cb = connect_cb;
  uv_socket->connect_req.data = socket;
  int status = uv_tcp_connect(&uv_socket->connect_req, uv_socket->handle, (struct sockaddr*)addr,
                              uv_tc_on_connect);
  if (status != 0) {
...
static grpc_resolved_addresses* handle_addrinfo_result(struct addrinfo* result) {
...
static void uv_resolve_callback(uv_getaddrinfo_t* req, int status, struct addrinfo* res) {
  ...
  grpc_custom_resolve_callback(r, result, tcp_error_create("getaddrinfo failed", status));
}
...
static void uv_resolve_async(grpc_custom_resolver* r, const char* host, const char* port) {
  int status;
  uv_getaddrinfo_t* req = (uv_getaddrinfo_t*)gpr_malloc(sizeof(uv_getaddrinfo_t));
  req->data = r;
  struct addrinfo hints;
  memset(&hints, 0, sizeof(struct addrinfo));
  ...
  status = uv_getaddrinfo(uv_default_loop(), req, uv_resolve_callback, host, port, &hints);
  if (status != 0) {
    gpr_free(req);
...
grpc_socket_vtable grpc_uv_socket_vtable = {
    uv_socket_init,        uv_socket_connect, uv_socket_destroy, uv_socket_shutdown,
    uv_socket_close,       uv_socket_write,   uv_socket_read,    uv_socket_getpeername,
    uv_socket_getsockname, uv_socket_bind,    uv_socket_listen,  uv_socket_accept};
