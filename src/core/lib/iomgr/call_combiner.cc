...
static grpc_error *
decode_cancel_state_error (gpr_atm cancel_state)
{
  if (cancel_state & 1)
    {
...
static gpr_atm
encode_cancel_state_error (grpc_error * error)
{
...
grpc_call_combiner_start (grpc_call_combiner * call_combiner,
			  ...
			  grpc_error * error DEBUG_ARGS, const char *reason)
{
  GPR_TIMER_BEGIN ("call_combiner_start", 0);
  if (grpc_call_combiner_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "==> grpc_call_combiner_start() [%p] closure=%p ["
	       DEBUG_FMT_STR "%s] error=%s", call_combiner,
	       closure DEBUG_FMT_ARGS, reason, grpc_error_string (error));
    }
  size_t prev_size =
...
void
grpc_call_combiner_stop (grpc_call_combiner * call_combiner DEBUG_ARGS,
			 const char *reason)
{
  ...
  if (prev_size > 1)
      ...
      while (true)
	{
	  ...
	  bool empty;
	  grpc_closure *closure =
	    (grpc_closure *) gpr_mpscq_pop_and_check_end (&call_combiner->
							  queue, &empty);
	  if (closure == nullptr)
	    {
	  ...
	  if (grpc_call_combiner_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG,
		       "  EXECUTING FROM QUEUE: closure=%p error=%s", closure,
		       grpc_error_string (closure->error_data.error));
	    }
  ...
  while (true)
    {
      ...
      gpr_atm original_state =
	gpr_atm_acq_load (&call_combiner->cancel_state);
      ...
      else
	{
	  if (gpr_atm_full_cas (&call_combiner->cancel_state, original_state,
		...
		{
		  gpr_log (GPR_DEBUG,
			   "call_combiner=%p: setting notify_on_cancel=%p",
			   call_combiner, closure);
		}
