...
static grpc_error* blocking_resolve_address_impl(
  ...
  *addresses = reinterpret_cast<grpc_resolved_addresses*>(
      gpr_malloc(sizeof(grpc_resolved_addresses)));
  (*addresses)->naddrs = 0;
  for (resp = result; resp != nullptr; resp = resp->ai_next) {
    (*addresses)->naddrs++;
  }
  (*addresses)->addrs = reinterpret_cast<grpc_resolved_address*>(
...
static void do_request_thread(grpc_exec_ctx* exec_ctx, void* rp,
                              grpc_error* error) {
  request* r = reinterpret_cast<request*>(rp);
  GRPC_CLOSURE_SCHED(
      exec_ctx, r->on_done,
...
static void resolve_address_impl(grpc_exec_ctx* exec_ctx, const char* name,
  ...
  request* r = reinterpret_cast<request*>(gpr_malloc(sizeof(request)));
