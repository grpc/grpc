...
get_socket_factory (const grpc_channel_args * args)
{
  if (args)
    {
      const grpc_arg *arg =
...
grpc_udp_server *
grpc_udp_server_create (const grpc_channel_args * args)
{
  grpc_udp_server *s =
    (grpc_udp_server *) gpr_malloc (sizeof (grpc_udp_server));
  gpr_mu_init (&s->mu);
...
static void
shutdown_fd (void *args, grpc_error * error)
{
...
static void
dummy_cb (void *arg, grpc_error * error)
{
  // No-op.
}
...
static void
destroyed_port (void *server, grpc_error * error)
{
	  ...
	  if (!sp->orphan_notified)
	    {
	      ...
	      sp->orphan_cb (sp->emfd, &sp->orphan_fd_closure,
			     sp->server->user_data);
	    }
...
void
grpc_udp_server_destroy (grpc_udp_server * s, grpc_closure * on_done)
{
  grpc_udp_listener *sp;
  gpr_mu_lock (&s->mu);
...
static int
bind_socket (grpc_socket_factory * socket_factory, int sockfd,
...
static int
prepare_socket (grpc_socket_factory * socket_factory, int fd,
...
static void
do_read (void *arg, grpc_error * error)
{
...
static void
on_read (void *arg, grpc_error * error)
{
...
void
fd_notify_on_write_wrapper (void *arg, grpc_error * error)
{
  grpc_udp_listener *sp = reinterpret_cast < grpc_udp_listener * >(arg);
  gpr_mu_lock (&sp->server->mu);
  if (!sp->notify_on_write_armed)
...
static void
do_write (void *arg, grpc_error * error)
{
  grpc_udp_listener *sp = reinterpret_cast < grpc_udp_listener * >(arg);
  gpr_mu_lock (&sp->server->mu);
  if (sp->already_shutdown)
  ...
  else
    {
      ...
      GRPC_CLOSURE_INIT (&sp->notify_on_write_closure,
...
static void
on_write (void *arg, grpc_error * error)
{
  grpc_udp_listener *sp = (grpc_udp_listener *) arg;
...
int
grpc_udp_server_add_port (grpc_udp_server * s,
			  const grpc_resolved_address * addr,
			  int rcv_buf_size, int snd_buf_size,
			  grpc_udp_server_start_cb start_cb,
			  grpc_udp_server_read_cb read_cb,
			  ...
			  grpc_udp_server_orphan_cb orphan_cb)
{
  ...
  if (grpc_sockaddr_get_port (addr) == 0)
      ...
      for (sp = s->head; sp; sp = sp->next)
	{
	  sockname_temp.len = sizeof (struct sockaddr_storage);
	  if (0 ==
	      getsockname (sp->fd, (struct sockaddr *) sockname_temp.addr,
  ...
  if (grpc_sockaddr_is_wildcard (addr, &port))
    {
      ...
      GRPC_ERROR_UNREF (grpc_create_dualstack_socket_using_factory
			(s->socket_factory, addr, SOCK_DGRAM, IPPROTO_UDP,
			 &dsmode, &fd));
      allocated_port1 =
	add_socket_to_server (s, fd, addr, rcv_buf_size, snd_buf_size,
			      start_cb, read_cb, write_cb, orphan_cb);
...
int
grpc_udp_server_get_fd (grpc_udp_server * s, unsigned port_index)
{
  grpc_udp_listener *sp;
