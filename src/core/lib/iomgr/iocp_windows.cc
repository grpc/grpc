...
static DWORD
deadline_to_millis_timeout (grpc_millis deadline)
{
  ...
  if (timeout > std::numeric_limits < DWORD >::max ())
    return INFINITE;
  return static_cast < DWORD > (deadline - now);
}

grpc_iocp_work_status
grpc_iocp_work (grpc_millis deadline)
{
  BOOL success;
  DWORD bytes = 0;
  DWORD flags = 0;
...
grpc_iocp_kick (void)
{
  ...
  gpr_atm_full_fetch_add (&g_custom_events, 1);
  success =
    PostQueuedCompletionStatus (g_iocp, 0, (ULONG_PTR) & g_iocp_kick_token,
				&g_iocp_custom_overlap);
  GPR_ASSERT (success);
}

void
grpc_iocp_flush (void)
{
