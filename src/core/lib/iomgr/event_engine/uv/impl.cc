...
class uvEndpoint final
 ...
 public:
  ...
  virtual void Read(
      ...
      grpc_event_engine::experimental::SliceBuffer* buffer) override final;

  virtual void Write(
      grpc_event_engine::experimental::EventEngine::Callback on_writable,
      grpc_event_engine::experimental::SliceBuffer* data) override final;
...
void uvEndpoint::Read(
  ...
  tcp->slice_allocator_->Allocate(
      DEFAULT_BUFFER_SIZE, tcp->read_sb_, [this, tcp](absl::Status status) {
        gpr_log(GPR_DEBUG, "allocate cb status: %s", status.ToString().c_str());
        this->getEngine()->schedule([tcp](uvEngine* engine) {
          uv_read_start(
