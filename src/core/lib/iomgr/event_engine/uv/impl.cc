...
absl::StatusOr<int> uvListener::Bind(
  ...
  engine_->schedule([this, &p, &addr](uvEngine* engine) {
    ...
    switch (r) {
      case UV_EINVAL:
        p.set_value(-1 /*util::Status(util::error::INVALID_ARGUMENT, "uv_tcp_bind said we passed an invalid argument to it")*/);
        return;
      case 0:
        break;
      default:
        p.set_value(-1 /*util::Status(util::error::UNKNOWN, "uv_tcp_bind returned an error code we don't know about")*/);
        return;
    }
    ...
    switch (r) {
      case UV_EINVAL:
        p.set_value(-1 /*util::Status(util::error::INVALID_ARGUMENT, "uv_tcp_getsockname said we passed an invalid argument to it")*/);
        return;
      case 0:
        break;
      default:
        p.set_value(-1 /*util::Status(util::error::UNKNOWN, "uv_tcp_getsockname returned an error code we don't know about")*/);
        return;
    }
    switch (boundAddr.ss_family) {
      ...
      default:
        p.set_value(-1 /*util::Status(util::error::INVALID_ARGUMENT, "returned socket address in :Bind is neither IPv4 nor IPv6")*/);
        break;
    }
  });
