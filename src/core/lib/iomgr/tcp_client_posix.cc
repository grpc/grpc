...
void grpc_tcp_client_create_from_prepared_fd(
    ...
    grpc_closure* closure, grpc_fd* fdobj,
    const grpc_channel_args* channel_args, const grpc_resolved_address* addr,
    grpc_millis deadline, grpc_endpoint** ep) {
  const int fd = grpc_fd_wrapped_fd(fdobj);
  gpr_log(GPR_INFO, "Connecting fd %d fdobj %p", fd, fdobj);//TODO
  int err;
  async_connect* ac;
  do {
...
static void tcp_client_connect_impl(grpc_exec_ctx* exec_ctx,
  ...
  *ep = nullptr;
  ...
  grpc_tcp_client_create_from_prepared_fd(exec_ctx, interested_parties, closure,
                                          fdobj, channel_args, &mapped_addr,
                                          deadline, ep);
}
