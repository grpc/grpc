...
static grpc_error *
prepare_socket (const grpc_resolved_address * addr, int fd,
		const grpc_channel_args * channel_args)
{
  ...
  if (channel_args)
	...
	{
	  if (0 ==
	      strcmp (channel_args->args[i].key, GRPC_ARG_SOCKET_MUTATOR))
	    {
...
static void
tc_on_alarm (void *acp, grpc_error * error)
{
  int
    done;
  async_connect *
    ac = (async_connect *) acp;
...
grpc_endpoint *
grpc_tcp_client_create_from_fd (grpc_fd * fd,
				const grpc_channel_args * channel_args,
				const char *addr_str)
{
...
static void
on_writable (void *acp, grpc_error * error)
{
  async_connect *
    ac = (async_connect *) acp;
  int
  ...
  do
    {
      ...
      err =
	getsockopt (grpc_fd_wrapped_fd (fd), SOL_SOCKET, SO_ERROR, &so_error,
		    &so_error_size);
    }
  ...
  switch (so_error)
    ...
    case 0:
      ...
      *ep =
	grpc_tcp_client_create_from_fd (fd, ac->channel_args, ac->addr_str);
      fd = nullptr;
      break;
    case ENOBUFS:
...
finish:
  ...
  const grpc_slice
    addr_str_slice = grpc_slice_from_copied_string (ac->addr_str);
  gpr_mu_unlock (&ac->mu);
  if (error != GRPC_ERROR_NONE)
    {
      char *
	...
	str;
      bool
	ret = grpc_error_get_str (error, GRPC_ERROR_STR_DESCRIPTION, &str);
      GPR_ASSERT (ret);
      char *
	desc = grpc_slice_to_c_string (str);
...
grpc_error *
grpc_tcp_client_prepare_fd (const grpc_channel_args * channel_args,
...
void
grpc_tcp_client_create_from_prepared_fd (grpc_pollset_set *
					 interested_parties,
  ...
  do
    {
      ...
      err =
	connect (fd, (const struct sockaddr *) addr->addr,
		 (socklen_t) addr->len);
    }
  while (err < 0 && errno == EINTR);
  if (err >= 0)
