...
static grpc_tcp_listener* find_listener_with_addr(
...
grpc_error_handle grpc_tcp_server_add_all_local_addrs(
  ...
  if (requested_port == 0) {
    /* Note: There could be a race where some local addrs can listen on
       the selected port and some can't. The sane way to handle this
       would be to retry by recreating the whole grpc_tcp_server.
       Backing out individual
