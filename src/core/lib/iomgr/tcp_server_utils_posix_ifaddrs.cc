...
find_listener_with_addr (grpc_tcp_server * s, grpc_resolved_address * addr)
{
  grpc_tcp_listener *l;
  gpr_mu_lock (&s->mu);
				     ...
				     int requested_port, int *out_port)
{
  ...
  if (requested_port == 0)
	...
	{
	  return
	    GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Bad get_unused_port()");
	}
  ...
  for (ifa_it = ifa; ifa_it != nullptr; ifa_it = ifa_it->ifa_next)
      ...
      if (find_listener_with_addr (s, &addr) != nullptr)
	{
      ...
      if ((err =
	   grpc_tcp_server_add_addr (s, &addr, port_index, fd_index, &dsmode,
...
bool
grpc_tcp_server_have_ifaddrs (void)
{
  return true;
}
