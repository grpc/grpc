...
/* Resource users are kept in (potentially) several intrusive linked
   lists at once. These are the list names. */
typedef enum {
  /* Resource users that are waiting for an allocation */
  GRPC_RULIST_AWAITING_ALLOCATION,
  /* Resource users that have free memory available for internal
     reclamation */
  GRPC_RULIST_NON_EMPTY_FREE_POOL,
  /* Resource users that have published a benign reclamation is
     available */
...
struct grpc_resource_user {
  ...
  /* Closure to schedule an allocation under the resource quota combiner
   * lock */
  grpc_closure allocate_closure;
  /* Closure to publish a non empty free pool under the resource quota
     combiner lock */
  grpc_closure add_to_free_pool_closure;

  /* one ref for each ref call (released by grpc_resource_user_unref),
     and one ref for each byte allocated (released by
     grpc_resource_user_free) */
  gpr_atm refs;
  /* is this resource user unlocked? starts at 0, increases for each
     shutdown call */
  gpr_atm shutdown;

  gpr_mu mu;
  /* The amount of memory (in bytes) this user has cached for its own
     use: to avoid quota contention, each resource user can keep some
     memory in addition to what it is immediately using (e.g., for
     caching), and the quota can pull it back under memory pressure.
     This value can become negative if more memory has been requested
     than existed in the free pool, at which point the quota is
     consulted to bring this value non-negative (asynchronously). */
  int64_t free_pool;
  /* A list of closures to call once free_pool becomes non-negative - ie
     when all outstanding allocations have been granted. */
  grpc_closure_list on_allocated;
  /* True if we are currently trying to allocate from the quota, false
   * if not */
  bool allocating;
  /* The amount of memory (in bytes) that has been requested from this
   * user asynchronously but hasn't been granted yet. */
  int64_t outstanding_allocations;
  /* True if we are currently trying to add ourselves to the non-free
     quota list, false otherwise */
  bool added_to_free_pool;

  /* The number of threads currently allocated to this resource user */
  gpr_atm num_threads_allocated;

  /* Reclaimers: index 0 is the benign reclaimer, 1 is the destructive
   * reclaimer
   */
  grpc_closure* reclaimers[2];
  /* Reclaimers just posted: once we're in the combiner lock, we'll move
     them to the array above */
  grpc_closure* new_reclaimers[2];
  /* Trampoline closures to finish reclamation and re-enter the quota
     combiner lock */
  grpc_closure post_reclaimer_closure[2];

  /* Closure to execute under the quota combiner to de-register and
     shutdown the resource user */
...
struct grpc_resource_quota {
  ...
  /* Main combiner lock: all activity on a quota executes under this
   * combiner (so no mutex is needed for this data structure) */
  grpc_core::Combiner* combiner;
  /* Size of the resource quota */
  int64_t size;
  /* Amount of free memory in the resource quota */
  int64_t free_pool;
  /* Used size of memory in the resource quota. Updated as soon as the
   * resource users start to allocate or free the memory. */
  gpr_atm used;

  gpr_atm last_size;

  /* Mutex to protect max_threads and num_threads_allocated */
  /* Note: We could have used gpr_atm for max_threads and
   * num_threads_allocated and avoid having this mutex; but in that
   * case, each invocation of the function
   * grpc_resource_user_allocate_threads() would have had to do at least
   * two atomic loads (for max_threads and num_threads_allocated)
   * followed by a CAS (on num_threads_allocated). Moreover, we expect
   * grpc_resource_user_allocate_threads() to be often called
   * concurrently thereby increasing the chances of failing the CAS
   * operation. This additional complexity is not worth the tiny perf
   * gain we may (or may not) have by using atomics */
  gpr_mu thread_count_mu;

  /* Max number of threads allowed */
  int max_threads;

  /* Number of threads currently allocated via this resource_quota
   * object */
  ...
  /* This is only really usable for debugging: it's always a stale
     pointer, but a stale pointer that might just be fresh enough to
     guide us to where the reclamation system is stuck */
...
static void ru_unref_by(grpc_resource_user* resource_user,
                        gpr_atm amount);
...
static grpc_resource_user* rulist_pop_head(
    grpc_resource_quota* resource_quota, grpc_rulist list) {
  grpc_resource_user** root = &resource_quota->roots[list];
...
static void rulist_remove(grpc_resource_user* resource_user,
                          grpc_rulist list) {
...
static bool rq_reclaim_from_per_user_free_pool(
    grpc_resource_quota* resource_quota);
static bool rq_reclaim(grpc_resource_quota* resource_quota,
                       bool destructive);

static void rq_step(void* rq, grpc_error_handle /*error*/) {
  grpc_resource_quota* resource_quota =
      static_cast<grpc_resource_quota*>(rq);
...
/* update the atomically available resource estimate - use no barriers
   since timeliness of delivery really doesn't matter much */
...
static bool rq_alloc(grpc_resource_quota* resource_quota) {
  ...
  while ((resource_user = rulist_pop_head(
              resource_quota, GRPC_RULIST_AWAITING_ALLOCATION))) {
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
      gpr_log(GPR_INFO,
              ...
              resource_user,
              gpr_atm_no_barrier_load(&resource_user->shutdown),
              resource_user->free_pool,
              resource_user->outstanding_allocations);
    }
    if (gpr_atm_no_barrier_load(&resource_user->shutdown)) {
      resource_user->allocating = false;
      grpc_closure_list_fail_all(&resource_user->on_allocated,
                                 GRPC_ERROR_CREATE_FROM_STATIC_STRING(
                                     "Resource user shutdown"));
      int64_t aborted_allocations =
          resource_user->outstanding_allocations;
      ...
      if (aborted_allocations > 0) {
        ru_unref_by(resource_user,
                    static_cast<gpr_atm>(aborted_allocations));
      }
      continue;
    }
    if (resource_user->free_pool < 0 &&
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
        gpr_log(GPR_INFO,
                ...
                resource_user->name.c_str(), amt,
                resource_quota->free_pool);
      }
    } else if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace) &&
               resource_user->free_pool >= 0) {
      gpr_log(
          GPR_INFO, "RQ %s %s: discard already satisfied alloc request",
...
static bool rq_reclaim_from_per_user_free_pool(
  ...
  grpc_resource_user* resource_user;
  while ((resource_user = rulist_pop_head(
              resource_quota, GRPC_RULIST_NON_EMPTY_FREE_POOL))) {
    gpr_mu_lock(&resource_user->mu);
    ...
    if (resource_user->free_pool > 0) {
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
        gpr_log(GPR_INFO,
                ...
                resource_quota->name.c_str(),
                resource_user->name.c_str(), resource_user->free_pool,
                resource_quota->free_pool);
      }
      gpr_mu_unlock(&resource_user->mu);
    }
...
static bool rq_reclaim(grpc_resource_quota* resource_quota,
  ...
  grpc_rulist list = destructive ? GRPC_RULIST_RECLAIMER_DESTRUCTIVE
                                 : GRPC_RULIST_RECLAIMER_BENIGN;
  grpc_resource_user* resource_user =
      rulist_pop_head(resource_quota, list);
  ...
  GPR_ASSERT(c);
  resource_quota->debug_only_last_reclaimer_resource_user =
      resource_user;
...
/*******************************************************************************
 * ru_slice: a slice implementation that is backed by a
 * grpc_resource_user
 */
...
/*******************************************************************************
 * grpc_resource_quota internal implementation: resource user
 * manipulation under the combiner
 */

static void ru_allocate(void* ru, grpc_error_handle /*error*/) {
  grpc_resource_user* resource_user =
      static_cast<grpc_resource_user*>(ru);
...
static void ru_destroy(void* ru, grpc_error_handle /*error*/) {
  ...
  grpc_resource_user_free_threads(
      resource_user, static_cast<int>(gpr_atm_no_barrier_load(
  ...
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, resource_user->reclaimers[1],
                          GRPC_ERROR_CANCELLED);
  if (resource_user->free_pool != 0) {
    resource_user->resource_quota->free_pool +=
        resource_user->free_pool;
    rq_step_sched(resource_user->resource_quota);
  }
...
static void ru_alloc_slices(
  ...
  for (size_t i = 0; i < slice_allocator->count; i++) {
    grpc_slice_buffer_add_indexed(
        ...
        ru_slice_create(slice_allocator->resource_user,
...
/*******************************************************************************
 * grpc_resource_quota internal implementation: quota manipulation under
 * the combiner
 */
...
grpc_resource_quota* grpc_resource_quota_create(const char* name) {
  ...
  GRPC_CLOSURE_INIT(&resource_quota->rq_step_closure, rq_step,
...
/* Public API */
void grpc_resource_quota_set_max_threads(
...
void grpc_resource_quota_resize(grpc_resource_quota* resource_quota,
  ...
  grpc_core::ExecCtx exec_ctx;
  rq_resize_args* a =
      static_cast<rq_resize_args*>(gpr_malloc(sizeof(*a)));
  ...
  for (size_t i = 0; i < channel_args->num_args; i++) {
    if (0 ==
      ...
      if (channel_args->args[i].type == GRPC_ARG_POINTER) {
        ...
        gpr_log(GPR_DEBUG,
                GRPC_ARG_RESOURCE_QUOTA " should be a pointer");
      }
    }
  }
...
const grpc_arg_pointer_vtable* grpc_resource_quota_arg_vtable(void) {
  static const grpc_arg_pointer_vtable vtable = {rq_copy, rq_destroy,
                                                 rq_cmp};
  return &vtable;
}
...
grpc_resource_user* grpc_resource_user_create(
  ...
  if (name != nullptr) {
    ...
    resource_user->name =
        absl::StrCat("anonymous_resource_user_",
                     reinterpret_cast<intptr_t>(resource_user));
  }
  return resource_user;
}
...
static void ru_ref_by(grpc_resource_user* resource_user,
                      gpr_atm amount) {
  GPR_ASSERT(amount > 0);
  GPR_ASSERT(
      gpr_atm_no_barrier_fetch_add(&resource_user->refs, amount) != 0);
}
...
void grpc_resource_user_free_threads(grpc_resource_user* resource_user,
  ...
  if (old_count < thread_count || rq->num_threads_allocated < 0) {
    gpr_log(GPR_ERROR,
            "Releasing more threads (%d) than currently allocated (rq "
            "threads: "
            "%d, ru threads: %d)",
            thread_count, rq->num_threads_allocated + thread_count,
            old_count);
    abort();
  }
  gpr_mu_unlock(&resource_user->resource_quota->thread_count_mu);
...
static bool resource_user_alloc_locked(
  ...
  if (optional_on_done != nullptr) {
    ...
    grpc_closure_list_append(&resource_user->on_allocated,
                             optional_on_done, GRPC_ERROR_NONE);
  }
  if (!resource_user->allocating) {
...
bool grpc_resource_user_safe_alloc(grpc_resource_user* resource_user,
  ...
  do {
    ...
    cas_success =
        gpr_atm_full_cas(&resource_quota->used, used, new_used);
  } while (!cas_success);
...
bool grpc_resource_user_alloc(grpc_resource_user* resource_user,
                              size_t size,
                              grpc_closure* optional_on_done) {
  // TODO(juanlishen): Maybe return immediately if shutting down.
  // Deferring this because some tests become flaky after the change.
  gpr_mu_lock(&resource_user->mu);
  grpc_resource_quota* resource_quota = resource_user->resource_quota;
...
bool grpc_resource_user_alloc_slices(
  ...
  if (GPR_UNLIKELY(gpr_atm_no_barrier_load(
          &slice_allocator->resource_user->shutdown))) {
  ...
  const bool ret = grpc_resource_user_alloc(
      slice_allocator->resource_user, count * length,
