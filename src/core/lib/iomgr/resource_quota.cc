...
static void rulist_add_head(grpc_resource_user* resource_user, grpc_rulist list) {
...
static void rulist_add_tail(grpc_resource_user* resource_user, grpc_rulist list) {
  grpc_resource_quota* resource_quota = resource_user->resource_quota;
  grpc_resource_user** root = &resource_quota->roots[list];
  if (*root == nullptr) {
    *root = resource_user;
...
static bool rulist_empty(grpc_resource_quota* resource_quota, grpc_rulist list) {
  return resource_quota->roots[list] == nullptr;
}

static grpc_resource_user* rulist_pop_head(grpc_resource_quota* resource_quota, grpc_rulist list) {
  grpc_resource_user** root = &resource_quota->roots[list];
  ...
  if (resource_user->links[list].next == resource_user) {
    *root = nullptr;
    ...
    resource_user->links[list].next->links[list].prev = resource_user->links[list].prev;
    resource_user->links[list].prev->links[list].next = resource_user->links[list].next;
...
static bool rq_reclaim_from_per_user_free_pool(grpc_resource_quota* resource_quota);
...
static void rq_step_sched(grpc_resource_quota* resource_quota) {
  ...
  resource_quota->combiner->FinallyRun(&resource_quota->rq_step_closure, GRPC_ERROR_NONE);
}
...
static void rq_update_estimate(grpc_resource_quota* resource_quota) {
  ...
  if (resource_quota->size != 0) {
    memory_usage_estimation = GPR_CLAMP(
        (gpr_atm)((1.0 - ((double)resource_quota->free_pool) / ((double)resource_quota->size)) *
                  MEMORY_USAGE_ESTIMATION_MAX),
        0, MEMORY_USAGE_ESTIMATION_MAX);
  }
  gpr_atm_no_barrier_store(&resource_quota->memory_usage_estimation, memory_usage_estimation);
}

/* returns true if all allocations are completed */
static bool rq_alloc(grpc_resource_quota* resource_quota) {
  grpc_resource_user* resource_user;
  while ((resource_user = rulist_pop_head(resource_quota, GRPC_RULIST_AWAITING_ALLOCATION))) {
    gpr_mu_lock(&resource_user->mu);
    if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
      gpr_log(GPR_INFO,
              "RQ: check allocation for user %p shutdown=%" PRIdPTR " free_pool=%" PRId64
              " outstanding_allocations=%" PRId64,
    ...
    if (gpr_atm_no_barrier_load(&resource_user->shutdown)) {
      ...
      grpc_closure_list_fail_all(&resource_user->on_allocated,
    ...
    if (resource_user->free_pool < 0 && -resource_user->free_pool <= resource_quota->free_pool) {
      int64_t amt = -resource_user->free_pool;
      resource_user->free_pool = 0;
      resource_quota->free_pool -= amt;
      rq_update_estimate(resource_quota);
      if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
        gpr_log(GPR_INFO, "RQ %s %s: grant alloc %" PRId64 " bytes; rq_free_pool -> %" PRId64,
...
static bool rq_reclaim_from_per_user_free_pool(grpc_resource_quota* resource_quota) {
  grpc_resource_user* resource_user;
  while ((resource_user = rulist_pop_head(resource_quota, GRPC_RULIST_NON_EMPTY_FREE_POOL))) {
    gpr_mu_lock(&resource_user->mu);
    ...
    if (resource_user->free_pool > 0) {
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
        gpr_log(GPR_INFO,
                ...
                resource_quota->name.c_str(), resource_user->name.c_str(), resource_user->free_pool,
                resource_quota->free_pool);
      }
      gpr_mu_unlock(&resource_user->mu);
    }
...
static bool rq_reclaim(grpc_resource_quota* resource_quota, bool destructive) {
  ...
  grpc_rulist list = destructive ? GRPC_RULIST_RECLAIMER_DESTRUCTIVE : GRPC_RULIST_RECLAIMER_BENIGN;
  grpc_resource_user* resource_user = rulist_pop_head(resource_quota, list);
  if (resource_user == nullptr) return false;
  if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
    gpr_log(GPR_INFO, "RQ %s %s: initiate %s reclamation", resource_quota->name.c_str(),
            resource_user->name.c_str(), destructive ? "destructive" : "benign");
  }
...
static grpc_slice ru_slice_create(grpc_resource_user* resource_user, size_t size) {
...
static void ru_allocate(void* ru, grpc_error_handle /*error*/) {
  ...
  if (rulist_empty(resource_user->resource_quota, GRPC_RULIST_AWAITING_ALLOCATION)) {
...
static void ru_add_to_free_pool(void* ru, grpc_error_handle /*error*/) {
  grpc_resource_user* resource_user = static_cast<grpc_resource_user*>(ru);
  if (!rulist_empty(resource_user->resource_quota, GRPC_RULIST_AWAITING_ALLOCATION) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_NON_EMPTY_FREE_POOL)) {
    rq_step_sched(resource_user->resource_quota);
  }
  rulist_add_tail(resource_user, GRPC_RULIST_NON_EMPTY_FREE_POOL);
}

static bool ru_post_reclaimer(grpc_resource_user* resource_user, bool destructive) {
...
static void ru_post_benign_reclaimer(void* ru, grpc_error_handle /*error*/) {
  ...
  if (!rulist_empty(resource_user->resource_quota, GRPC_RULIST_AWAITING_ALLOCATION) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_NON_EMPTY_FREE_POOL) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_RECLAIMER_BENIGN)) {
    rq_step_sched(resource_user->resource_quota);
  }
  rulist_add_tail(resource_user, GRPC_RULIST_RECLAIMER_BENIGN);
}

static void ru_post_destructive_reclaimer(void* ru, grpc_error_handle /*error*/) {
  grpc_resource_user* resource_user = static_cast<grpc_resource_user*>(ru);
  if (!ru_post_reclaimer(resource_user, true)) return;
  if (!rulist_empty(resource_user->resource_quota, GRPC_RULIST_AWAITING_ALLOCATION) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_NON_EMPTY_FREE_POOL) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_RECLAIMER_BENIGN) &&
      rulist_empty(resource_user->resource_quota, GRPC_RULIST_RECLAIMER_DESTRUCTIVE)) {
    rq_step_sched(resource_user->resource_quota);
  }
...
static void ru_shutdown(void* ru, grpc_error_handle /*error*/) {
  ...
  grpc_resource_user* resource_user = static_cast<grpc_resource_user*>(ru);
  gpr_mu_lock(&resource_user->mu);
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, resource_user->reclaimers[0], GRPC_ERROR_CANCELLED);
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, resource_user->reclaimers[1], GRPC_ERROR_CANCELLED);
...
static void ru_destroy(void* ru, grpc_error_handle /*error*/) {
  ...
  grpc_resource_user_free_threads(
      resource_user,
      static_cast<int>(gpr_atm_no_barrier_load(&resource_user->num_threads_allocated)));
...
static void ru_alloc_slices(grpc_resource_user_slice_allocator* slice_allocator) {
  for (size_t i = 0; i < slice_allocator->count; i++) {
    grpc_slice_buffer_add_indexed(
        slice_allocator->dest,
        ru_slice_create(slice_allocator->resource_user, slice_allocator->length));
  }
}
...
static void ru_allocated_slices(void* arg, grpc_error_handle error) {
  ...
  grpc_core::Closure::Run(DEBUG_LOCATION, &slice_allocator->on_done, GRPC_ERROR_REF(error));
}
...
grpc_resource_quota* grpc_resource_quota_create(const char* name) {
  ...
  if (name != nullptr) {
    ...
    resource_quota->name =
        absl::StrCat("anonymous_pool_", reinterpret_cast<intptr_t>(resource_quota));
  }
  GRPC_CLOSURE_INIT(&resource_quota->rq_step_closure, rq_step, resource_quota, nullptr);
  GRPC_CLOSURE_INIT(&resource_quota->rq_reclamation_done_closure, rq_reclamation_done,
...
double grpc_resource_quota_get_memory_pressure(grpc_resource_quota* resource_quota) {
  return (static_cast<double>(gpr_atm_no_barrier_load(&resource_quota->memory_usage_estimation))) /
...
void grpc_resource_quota_resize(grpc_resource_quota* resource_quota, size_t size) {
  ...
  gpr_atm_no_barrier_store(&resource_quota->last_size, (gpr_atm)GPR_MIN((size_t)GPR_ATM_MAX, size));
...
grpc_resource_user* grpc_resource_user_create(grpc_resource_quota* resource_quota,
  ...
  resource_user->resource_quota = grpc_resource_quota_ref_internal(resource_quota);
  GRPC_CLOSURE_INIT(&resource_user->allocate_closure, &ru_allocate, resource_user, nullptr);
  GRPC_CLOSURE_INIT(&resource_user->add_to_free_pool_closure, &ru_add_to_free_pool, resource_user,
                    nullptr);
  GRPC_CLOSURE_INIT(&resource_user->post_reclaimer_closure[0], &ru_post_benign_reclaimer,
                    resource_user, nullptr);
  GRPC_CLOSURE_INIT(&resource_user->post_reclaimer_closure[1], &ru_post_destructive_reclaimer,
                    resource_user, nullptr);
  GRPC_CLOSURE_INIT(&resource_user->destroy_closure, &ru_destroy, resource_user, nullptr);
  ...
  if (name != nullptr) {
    ...
    resource_user->name =
        absl::StrCat("anonymous_resource_user_", reinterpret_cast<intptr_t>(resource_user));
  }
  return resource_user;
}
...
static void ru_unref_by(grpc_resource_user* resource_user, gpr_atm amount) {
  ...
  if (old == amount) {
    resource_user->resource_quota->combiner->Run(&resource_user->destroy_closure, GRPC_ERROR_NONE);
  }
}

void grpc_resource_user_ref(grpc_resource_user* resource_user) { ru_ref_by(resource_user, 1); }

void grpc_resource_user_unref(grpc_resource_user* resource_user) { ru_unref_by(resource_user, 1); }

void grpc_resource_user_shutdown(grpc_resource_user* resource_user) {
  if (gpr_atm_full_fetch_add(&resource_user->shutdown, 1) == 0) {
    resource_user->resource_quota->combiner->Run(
        GRPC_CLOSURE_CREATE(ru_shutdown, resource_user, nullptr), GRPC_ERROR_NONE);
  }
}
...
bool grpc_resource_user_allocate_threads(grpc_resource_user* resource_user, int thread_count) {
  ...
  if (rq->num_threads_allocated + thread_count <= rq->max_threads) {
    ...
    gpr_atm_no_barrier_fetch_add(&resource_user->num_threads_allocated, thread_count);
    is_success = true;
  }
...
static bool resource_user_alloc_locked(grpc_resource_user* resource_user, size_t size,
                                       grpc_closure* optional_on_done) {
  ru_ref_by(resource_user, static_cast<gpr_atm>(size));
  resource_user->free_pool -= static_cast<int64_t>(size);
  if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
    gpr_log(GPR_INFO, "RQ %s %s: alloc %" PRIdPTR "; free_pool -> %" PRId64,
            resource_user->resource_quota->name.c_str(), resource_user->name.c_str(), size,
  ...
  if (optional_on_done != nullptr) {
    ...
    grpc_closure_list_append(&resource_user->on_allocated, optional_on_done, GRPC_ERROR_NONE);
  }
  if (!resource_user->allocating) {
    resource_user->allocating = true;
    resource_user->resource_quota->combiner->Run(&resource_user->allocate_closure, GRPC_ERROR_NONE);
  }
  return false;
}
...
bool grpc_resource_user_safe_alloc(grpc_resource_user* resource_user, size_t size) {
  ...
  do {
    ...
    if (static_cast<size_t>(new_used) > grpc_resource_quota_peek_size(resource_quota)) {
      gpr_mu_unlock(&resource_user->mu);
      return false;
    }
...
bool grpc_resource_user_alloc(grpc_resource_user* resource_user, size_t size,
  ...
  const bool ret = resource_user_alloc_locked(resource_user, size, optional_on_done);
  gpr_mu_unlock(&resource_user->mu);
  return ret;
}
...
void grpc_resource_user_free(grpc_resource_user* resource_user, size_t size) {
  ...
  if (is_bigger_than_zero && was_zero_or_negative && !resource_user->added_to_free_pool) {
    resource_user->added_to_free_pool = true;
    resource_quota->combiner->Run(&resource_user->add_to_free_pool_closure, GRPC_ERROR_NONE);
  }
  gpr_mu_unlock(&resource_user->mu);
...
void grpc_resource_user_post_reclaimer(grpc_resource_user* resource_user, bool destructive,
  ...
  resource_user->resource_quota->combiner->Run(&resource_user->post_reclaimer_closure[destructive],
                                               GRPC_ERROR_NONE);
}

void grpc_resource_user_finish_reclamation(grpc_resource_user* resource_user) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
    gpr_log(GPR_INFO, "RQ %s %s: reclamation complete", resource_user->resource_quota->name.c_str(),
...
void grpc_resource_user_slice_allocator_init(grpc_resource_user_slice_allocator* slice_allocator,
  ...
  GRPC_CLOSURE_INIT(&slice_allocator->on_allocated, ru_allocated_slices, slice_allocator,
                    grpc_schedule_on_exec_ctx);
  GRPC_CLOSURE_INIT(&slice_allocator->on_done, cb, p, grpc_schedule_on_exec_ctx);
  slice_allocator->resource_user = resource_user;
}

bool grpc_resource_user_alloc_slices(grpc_resource_user_slice_allocator* slice_allocator,
                                     size_t length, size_t count, grpc_slice_buffer* dest) {
  if (GPR_UNLIKELY(gpr_atm_no_barrier_load(&slice_allocator->resource_user->shutdown))) {
    grpc_core::ExecCtx::Run(DEBUG_LOCATION, &slice_allocator->on_allocated,
                            GRPC_ERROR_CREATE_FROM_STATIC_STRING("Resource user shutdown"));
    return false;
  }
  ...
  const bool ret = grpc_resource_user_alloc(slice_allocator->resource_user, count * length,
