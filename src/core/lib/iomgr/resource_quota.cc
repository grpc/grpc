...
static void
rulist_add_head (grpc_resource_user * resource_user, grpc_rulist list)
{
...
static void
rulist_add_tail (grpc_resource_user * resource_user, grpc_rulist list)
{
  grpc_resource_quota *resource_quota = resource_user->resource_quota;
  grpc_resource_user **root = &resource_quota->roots[list];
  if (*root == nullptr)
...
static bool
rulist_empty (grpc_resource_quota * resource_quota, grpc_rulist list)
{
  return resource_quota->roots[list] == nullptr;
}

static grpc_resource_user *
rulist_pop_head (grpc_resource_quota * resource_quota, grpc_rulist list)
{
  grpc_resource_user **root = &resource_quota->roots[list];
...
static void
rulist_remove (grpc_resource_user * resource_user, grpc_rulist list)
{
  if (resource_user->links[list].next == nullptr)
    return;
  grpc_resource_quota *resource_quota = resource_user->resource_quota;
...
static bool rq_reclaim_from_per_user_free_pool (grpc_resource_quota *
						resource_quota);
static bool rq_reclaim (grpc_resource_quota * resource_quota,
			bool destructive);

static void
rq_step (void *rq, grpc_error * error)
{
  grpc_resource_quota *resource_quota = (grpc_resource_quota *) rq;
  resource_quota->step_scheduled = false;
  do
    {
      if (rq_alloc (resource_quota))
	goto done;
    }
...
static void
rq_step_sched (grpc_resource_quota * resource_quota)
{
  if (resource_quota->step_scheduled)
    return;
...
static void
rq_update_estimate (grpc_resource_quota * resource_quota)
{
...
static bool
rq_alloc (grpc_resource_quota * resource_quota)
{
  ...
  while ((resource_user = rulist_pop_head (resource_quota,
	...
	{
	  gpr_log (GPR_DEBUG,
		   "RQ %s %s: discard already satisfied alloc request",
...
static bool
rq_reclaim_from_per_user_free_pool (grpc_resource_quota * resource_quota)
{
  grpc_resource_user *resource_user;
  while ((resource_user = rulist_pop_head (resource_quota,
					   GRPC_RULIST_NON_EMPTY_FREE_POOL)))
...
static bool
rq_reclaim (grpc_resource_quota * resource_quota, bool destructive)
{
  if (resource_quota->reclaiming)
    return true;
...
static grpc_slice
ru_slice_create (grpc_resource_user * resource_user, size_t size)
{
...
static void
ru_allocate (void *ru, grpc_error * error)
{
...
static void
ru_add_to_free_pool (void *ru, grpc_error * error)
{
  grpc_resource_user *resource_user = (grpc_resource_user *) ru;
  if (!rulist_empty (resource_user->resource_quota,
		     GRPC_RULIST_AWAITING_ALLOCATION) &&
...
static bool
ru_post_reclaimer (grpc_resource_user * resource_user, bool destructive)
{
...
static void
ru_post_benign_reclaimer (void *ru, grpc_error * error)
{
  grpc_resource_user *resource_user = (grpc_resource_user *) ru;
  if (!ru_post_reclaimer (resource_user, false))
    return;
  if (!rulist_empty (resource_user->resource_quota,
		     GRPC_RULIST_AWAITING_ALLOCATION) &&
      rulist_empty (resource_user->resource_quota,
...
static void
ru_post_destructive_reclaimer (void *ru, grpc_error * error)
{
  grpc_resource_user *resource_user = (grpc_resource_user *) ru;
  if (!ru_post_reclaimer (resource_user, true))
    return;
  if (!rulist_empty (resource_user->resource_quota,
		     GRPC_RULIST_AWAITING_ALLOCATION) &&
      rulist_empty (resource_user->resource_quota,
...
static void
ru_shutdown (void *ru, grpc_error * error)
{
  if (grpc_resource_quota_trace.enabled ())
    {
...
static void
ru_destroy (void *ru, grpc_error * error)
{
  grpc_resource_user *resource_user = (grpc_resource_user *) ru;
...
static void
ru_allocated_slices (void *arg, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
	...
	{
	  grpc_slice_buffer_add_indexed (slice_allocator->dest,
					 ru_slice_create (slice_allocator->
							  resource_user,
							  slice_allocator->
							  length));
	}
...
static void
rq_resize (void *args, grpc_error * error)
{
...
static void
rq_reclamation_done (void *rq, grpc_error * error)
{
  grpc_resource_quota *resource_quota = (grpc_resource_quota *) rq;
...
/* Public API */
grpc_resource_quota *
grpc_resource_quota_create (const char *name)
{
  grpc_resource_quota *resource_quota =
  ...
  GRPC_CLOSURE_INIT (&resource_quota->rq_step_closure, rq_step,
		     resource_quota,
		     grpc_combiner_finally_scheduler (resource_quota->
						      combiner));
...
/* Public API */
void
grpc_resource_quota_unref (grpc_resource_quota * resource_quota)
{
...
/* Public API */
void
grpc_resource_quota_ref (grpc_resource_quota * resource_quota)
{
  grpc_resource_quota_ref_internal (resource_quota);
}

double
grpc_resource_quota_get_memory_pressure (grpc_resource_quota * resource_quota)
{
  return ((double)
...
/* Public API */
void
grpc_resource_quota_resize (grpc_resource_quota * resource_quota, size_t size)
{
  grpc_core::ExecCtx exec_ctx;
...
grpc_resource_quota *
grpc_resource_quota_from_channel_args (const grpc_channel_args * channel_args)
{
  for (size_t i = 0; i < channel_args->num_args; i++)
      ...
      if (0 == strcmp (channel_args->args[i].key, GRPC_ARG_RESOURCE_QUOTA))
	{
	  if (channel_args->args[i].type == GRPC_ARG_POINTER)
	    {
	      return grpc_resource_quota_ref_internal ((grpc_resource_quota *)
						       channel_args->args[i].
						       value.pointer.p);
	    }
	  else
...
static void
ru_ref_by (grpc_resource_user * resource_user, gpr_atm amount)
{
  GPR_ASSERT (amount > 0);
  GPR_ASSERT (gpr_atm_no_barrier_fetch_add (&resource_user->refs, amount) !=
	      0);
}

static void
ru_unref_by (grpc_resource_user * resource_user, gpr_atm amount)
{
  GPR_ASSERT (amount > 0);
...
grpc_resource_user_shutdown (grpc_resource_user * resource_user)
{
  if (gpr_atm_full_fetch_add (&resource_user->shutdown, 1) == 0)
    {
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE (ru_shutdown, resource_user,
					       grpc_combiner_scheduler
					       (resource_user->
						resource_quota->combiner)),
			  GRPC_ERROR_NONE);
    }
}
			  ...
			  grpc_closure * optional_on_done)
{
  ...
  if (grpc_resource_quota_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "RQ %s %s: alloc %" PRIdPTR "; free_pool -> %" PRId64,
  ...
  if (resource_user->free_pool < 0)
      ...
      if (!resource_user->allocating)
	{
	  ...
	  GRPC_CLOSURE_SCHED (&resource_user->allocate_closure,
			      GRPC_ERROR_NONE);
	}
    }
  else
...
grpc_resource_user_free (grpc_resource_user * resource_user, size_t size)
{
  ...
  if (grpc_resource_quota_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "RQ %s %s: free %" PRIdPTR "; free_pool -> %" PRId64,
	       resource_user->resource_quota->name, resource_user->name, size,
	       resource_user->free_pool);
    }
...
grpc_resource_user_finish_reclamation (grpc_resource_user * resource_user)
{
  ...
  GRPC_CLOSURE_SCHED (&resource_user->resource_quota->
		      rq_reclamation_done_closure, GRPC_ERROR_NONE);
}
