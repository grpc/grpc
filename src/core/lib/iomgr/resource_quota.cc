...
static void rq_step(grpc_exec_ctx* exec_ctx, void* rq, grpc_error* error) {
  grpc_resource_quota* resource_quota =
      reinterpret_cast<grpc_resource_quota*>(rq);
...
static void ru_slice_ref(void* p) {
  ru_slice_refcount* rc = reinterpret_cast<ru_slice_refcount*>(p);
  gpr_ref(&rc->refs);
}
...
static grpc_slice ru_slice_create(grpc_resource_user* resource_user,
                                  size_t size) {
  ru_slice_refcount* rc = reinterpret_cast<ru_slice_refcount*>(
...
static void ru_allocate(grpc_exec_ctx* exec_ctx, void* ru, grpc_error* error) {
  grpc_resource_user* resource_user = reinterpret_cast<grpc_resource_user*>(ru);
...
static void ru_allocated_slices(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  grpc_resource_user_slice_allocator* slice_allocator =
      reinterpret_cast<grpc_resource_user_slice_allocator*>(arg);
...
static void rq_resize(grpc_exec_ctx* exec_ctx, void* args, grpc_error* error) {
  rq_resize_args* a = reinterpret_cast<rq_resize_args*>(args);
...
grpc_resource_quota* grpc_resource_quota_create(const char* name) {
  grpc_resource_quota* resource_quota = reinterpret_cast<grpc_resource_quota*>(
      gpr_malloc(sizeof(*resource_quota)));
...
void grpc_resource_quota_resize(grpc_resource_quota* resource_quota,
                                size_t size) {
  grpc_exec_ctx exec_ctx = GRPC_EXEC_CTX_INIT;
  rq_resize_args* a = reinterpret_cast<rq_resize_args*>(gpr_malloc(sizeof(*a)));
...
static void* rq_copy(void* rq) {
  grpc_resource_quota_ref(reinterpret_cast<grpc_resource_quota*>(rq));
  return rq;
}

static void rq_destroy(grpc_exec_ctx* exec_ctx, void* rq) {
  grpc_resource_quota_unref_internal(
      exec_ctx, reinterpret_cast<grpc_resource_quota*>(rq));
}
...
grpc_resource_user* grpc_resource_user_create(
  ...
  grpc_resource_user* resource_user =
      reinterpret_cast<grpc_resource_user*>(gpr_malloc(sizeof(*resource_user)));
...
void grpc_resource_user_free(grpc_exec_ctx* exec_ctx,
  ...
  gpr_mu_unlock(&resource_user->mu);
  ru_unref_by(exec_ctx, resource_user, static_cast<gpr_atm>(size));
}
