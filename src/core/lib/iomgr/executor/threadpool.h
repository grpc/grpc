...
// Threadpool is an executor that maintains a pool of threads sitting
// around and waiting for closures. A threadpool also maintains a queue
// of pending closures, when closures appearing in the queue, the
// threads in pool will pull them out and execute them.
class ThreadPoolInterface {
 public:
  // Waits for all pending closures to complete, then shuts down thread
  // pool.
  virtual ~ThreadPoolInterface() {}

  // Schedules a given closure for execution later.
  // Depending on specific subclass implementation, this routine might
  // cause current thread to be blocked (in case of unable to schedule).
  // Closure should contain a function pointer and arguments it will
  // take, more details for closure struct at
...
// Worker thread for threadpool. Executes closures in the queue, until
// getting a NULL closure.
class ThreadPoolWorker {
 ...
 private:
  ...
  MPMCQueueInterface*
      queue_;             // Queue in thread pool to pull closures from
...
// A fixed size thread pool implementation of abstract thread pool
// interface. In this implementation, the number of threads in pool is
// fixed, but the capacity of closure queue is unlimited.
class ThreadPool : public ThreadPoolInterface {
 public:
  // Creates a thread pool with size of "num_threads", with default
  // thread name "ThreadPoolWorker" and all thread options set to
  // default. If the given size is 0 or less, there will be 1 worker
  // thread created inside pool.
  explicit ThreadPool(int num_threads);

  // Same as ThreadPool(int num_threads) constructor, except
  // that it also sets "thd_name" as the name of all threads in the
  // thread pool.
  ThreadPool(int num_threads, const char* thd_name);

  // Same as ThreadPool(const char *thd_name, int num_threads)
  // constructor, except that is also set thread_options for threads.
  // Notes for stack size:
  // If the stack size field of the passed in Thread::Options is set to
  // default value 0, default ThreadPool stack size will be used. The
  // current default stack size of this implementation is 1952K for
  // mobile platform and 64K for all others.
  ThreadPool(int num_threads, const char* thd_name,
  ...
  // Adds given closure into pending queue immediately. Since closure
  // queue has infinite length, this routine will not block.
 ...
 private:
  ...
  Atomic<bool> shut_down_{
      false};  // Destructor has been called if set to true

  void SharedThreadPoolConstructor();
  // For ThreadPool, default stack size for mobile platform is 1952K.
  // for other platforms is 64K.
