...
static void
async_connect_unlock_and_cleanup (async_connect * ac, grpc_winsocket * socket)
{
  int done = (--ac->refs == 0);
  gpr_mu_unlock (&ac->mu);
  if (done)
...
static void
on_alarm (void *acp, grpc_error * error)
{
...
static void
on_connect (void *acp, grpc_error * error)
{
  async_connect *ac = (async_connect *) acp;
  ...
  if (error == GRPC_ERROR_NONE)
      ...
      if (socket != NULL)
	{
	  ...
	  DWORD flags;
	  BOOL wsa_success =
	    WSAGetOverlappedResult (socket->socket,
...
static void
tcp_client_connect_impl (grpc_closure * on_done,
			 grpc_endpoint ** endpoint,
			 ...
			 grpc_millis deadline)
{
  ...
  *endpoint = NULL;
  ...
  status =
    bind (sock, (struct sockaddr *) &local_address.addr,
  ...
  success =
    ConnectEx (sock, (struct sockaddr *) &addr->addr, (int) addr->len, NULL,
...
failure:
  ...
  grpc_error *final_error =
    grpc_error_set_str (GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING
			...
			grpc_slice_from_copied_string (target_uri));
...
void (*grpc_tcp_client_connect_impl) (grpc_closure * closure,
				      grpc_endpoint ** ep,
				      grpc_pollset_set * interested_parties,
				      const grpc_channel_args * channel_args,
				      const grpc_resolved_address * addr,
				      grpc_millis deadline) =
  tcp_client_connect_impl;

void
grpc_tcp_client_connect (grpc_closure * closure, grpc_endpoint ** ep,
			 grpc_pollset_set * interested_parties,
			 const grpc_channel_args * channel_args,
			 const grpc_resolved_address * addr,
			 grpc_millis deadline)
