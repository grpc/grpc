...
class Executor {
 public:
  ...
  /* Enable/disable threading - must be called after Init and
   * Shutdown(). Never call SetThreading(false) in the middle of an
   * application */
  void SetThreading(bool threading);

  /** Shutdown the executor, running all pending work as part of the
   * call */
  void Shutdown();

  /** Enqueue the closure onto the executor. is_short is true if the
   * closure is a short job (i.e expected to not block and complete
   * quickly) */
  void Enqueue(grpc_closure* closure, grpc_error_handle error,
               bool is_short);

  // TODO(sreek): Currently we have two executors (available globally):
  // The default executor and the resolver executor.
  //
  // Some of the functions below operate on the DEFAULT executor only
  // while some operate of ALL the executors. This is a bit confusing
  // and should be cleaned up in future (where we make all the following
  // functions take ExecutorType and/or JobType)
 ...
 private:
  static size_t RunClosures(const char* executor_name,
