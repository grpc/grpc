...
static int retry_named_port_failure(grpc_custom_resolver* r, grpc_resolved_addresses** res) {
...
void grpc_custom_resolve_callback(grpc_custom_resolver* r, grpc_resolved_addresses* result,
                                  grpc_error_handle error) {
...
static grpc_error_handle try_split_host_port(const char* name, const char* default_port,
...
static grpc_error_handle blocking_resolve_address_impl(const char* name, const char* default_port,
                                                       grpc_resolved_addresses** addresses) {
  GRPC_CUSTOM_IOMGR_ASSERT_SAME_THREAD();

  grpc_custom_resolver resolver;
  grpc_error_handle err = try_split_host_port(name, default_port, &resolver.host, &resolver.port);
  ...
  err = resolve_address_vtable->resolve(resolver.host.c_str(), resolver.port.c_str(), &addrs);
  if (err != GRPC_ERROR_NONE) {
...
static grpc_address_resolver_vtable custom_resolver_vtable = {resolve_address_impl,
