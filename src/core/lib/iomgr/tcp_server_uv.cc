...
grpc_error *
grpc_tcp_server_create (grpc_closure * shutdown_complete,
			const grpc_channel_args * args,
			grpc_tcp_server ** server)
{
  grpc_tcp_server *s =
    (grpc_tcp_server *) gpr_malloc (sizeof (grpc_tcp_server));
	  ...
	  if (args->args[i].type == GRPC_ARG_POINTER)
	    {
	      ...
	      s->resource_quota = grpc_resource_quota_ref_internal ((grpc_resource_quota *) args->args[i].value.pointer.p);
	    }
	  else
	    {
	      grpc_resource_quota_unref_internal (s->resource_quota);
	      gpr_free (s);
	      return
		GRPC_ERROR_CREATE_FROM_STATIC_STRING (GRPC_ARG_RESOURCE_QUOTA
...
grpc_tcp_server *
grpc_tcp_server_ref (grpc_tcp_server * s)
{
...
void
grpc_tcp_server_shutdown_starting_add (grpc_tcp_server * s,
...
static void
handle_close_callback (uv_handle_t * handle)
{
...
static void
close_listener (grpc_tcp_listener * sp)
{
  if (!sp->closed)
    {
      sp->closed = true;
...
static void
tcp_server_destroy (grpc_tcp_server * s)
{
...
void
grpc_tcp_server_unref (grpc_tcp_server * s)
{
  GRPC_UV_ASSERT_SAME_THREAD ();
  if (gpr_unref (&s->refs))
    {
...
static void
finish_accept (grpc_tcp_listener * sp)
{
  ...
  if (grpc_tcp_trace.enabled ())
	...
	{
	  gpr_log (GPR_DEBUG, "SERVER_CONNECT: %p accepted connection",
		   sp->server);
	}
    }
...
on_connect (uv_stream_t * server, int status)
{
  ...
  if (grpc_tcp_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "SERVER_CONNECT: %p incoming connection",
	       sp->server);
    }
...
static grpc_error *
add_addr_to_server (grpc_tcp_server * s,
		    ...
		    unsigned port_index, grpc_tcp_listener ** listener)
{
  ...
  if (status != 0)
	...
	grpc_error_set_str (error, GRPC_ERROR_STR_OS_ERROR,
			    grpc_slice_from_static_string (uv_strerror
							   (status)));
      return error;
    }
  ...
  if (status != 0)
    {
      error =
	GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Failed to listen to port");
      error =
	grpc_error_set_str (error, GRPC_ERROR_STR_OS_ERROR,
			    grpc_slice_from_static_string (uv_strerror
							   (status)));
  ...
  status =
    uv_tcp_getsockname (handle, (struct sockaddr *) &sockname_temp.addr,
...
static grpc_error *
add_wildcard_addrs_to_server (grpc_tcp_server * s,
			      unsigned port_index,
			      int requested_port,
...
grpc_tcp_server_add_port (grpc_tcp_server * s,
			  const grpc_resolved_address * addr, int *port)
{
  ...
  if (grpc_tcp_trace.enabled ())
    {
      ...
      if (port_string)
	{
	  gpr_log (GPR_DEBUG, "SERVER %p add_port %s error=%s", s,
		   port_string, str);
...
void
grpc_tcp_server_start (grpc_tcp_server * server, grpc_pollset ** pollsets,
...
void
grpc_tcp_server_shutdown_listeners (grpc_tcp_server * s)
{
}
