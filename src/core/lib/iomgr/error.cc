...
static const char *
error_int_name (grpc_error_ints key)
{
  switch (key)
    {
...
static const char *
error_str_name (grpc_error_strs key)
{
  switch (key)
    {
...
static const char *
error_time_name (grpc_error_times key)
{
  switch (key)
    {
...
bool
grpc_error_is_special (grpc_error * err)
{
  ...
  if (grpc_trace_error_refcount.enabled ())
    {
      gpr_log (GPR_DEBUG, "%p: %" PRIdPTR " -> %" PRIdPTR " [%s:%d]", err,
	       ...
	       gpr_atm_no_barrier_load (&err->atomics.refs.count) + 1, file,
	       line);
    }
...
static void
unref_slice (grpc_slice slice)
{
  grpc_slice_unref_internal (slice);
}

static void
unref_strs (grpc_error * err)
{
...
static uint8_t
get_placement (grpc_error ** err, size_t size)
{
  GPR_ASSERT (*err);
...
static void
internal_set_int (grpc_error ** err, grpc_error_ints which, intptr_t value)
{
  ...
  if (slot == UINT8_MAX)
	...
	{
	  gpr_log (GPR_ERROR,
		   "Error %p is full, dropping int {\"%s\":%" PRIiPTR "}",
...
static void
internal_set_str (grpc_error ** err, grpc_error_strs which, grpc_slice value)
{
  uint8_t slot = (*err)->strs[which];
  if (slot == UINT8_MAX)
    {
      ...
      if (slot == UINT8_MAX)
	{
	  ...
	  gpr_log (GPR_ERROR,
		   "Error %p is full, dropping string {\"%s\":\"%s\"}", *err,
...
static void
internal_set_time (grpc_error ** err, grpc_error_times which,
		   gpr_timespec value)
{
  ...
  if (slot == UINT8_MAX)
      ...
      if (slot == UINT8_MAX)
	{
	  ...
	  gpr_log (GPR_ERROR, "Error %p is full, dropping \"%s\":\"%s\"}",
		   *err, error_time_name (which), time_str);
...
static void
internal_add_error (grpc_error ** err, grpc_error * new_err)
{
		   ...
		   grpc_error ** referencing, size_t num_referencing)
{
  ...
  uint8_t initial_arena_capacity = (uint8_t) (DEFAULT_ERROR_CAPACITY +
					      (uint8_t) (num_referencing *
							 SLOTS_PER_LINKED_ERROR)
					      + SURPLUS_CAPACITY);
  grpc_error *err =
    (grpc_error *) gpr_malloc (sizeof (*err) +
			       initial_arena_capacity * sizeof (intptr_t));
  if (err == nullptr)
    {				// TODO(ctiller): make gpr_malloc return NULL
  ...
  for (size_t i = 0; i < num_referencing; ++i)
    {
      if (referencing[i] == GRPC_ERROR_NONE)
	continue;
      internal_add_error (&err, GRPC_ERROR_REF (referencing[i]));	// TODO(ncteisen), change ownership semantics
    }

  internal_set_time (&err, GRPC_ERROR_TIME_CREATED,
...
static grpc_error *
copy_error_and_unref (grpc_error * in)
{
  ...
  if (grpc_error_is_special (in))
      ...
      else if (in == GRPC_ERROR_CANCELLED)
	{
	  ...
	  internal_set_int (&out, GRPC_ERROR_INT_GRPC_STATUS,
      ...
      if (grpc_trace_error_refcount.enabled ())
	{
      ...
      memcpy ((void *) ((uintptr_t) out + skip),
...
bool
grpc_error_get_int (grpc_error * err, grpc_error_ints which, intptr_t * p)
{
...
grpc_error *
grpc_error_set_str (grpc_error * src, grpc_error_strs which, grpc_slice str)
{
...
bool
grpc_error_get_str (grpc_error * err, grpc_error_strs which, grpc_slice * str)
{
  if (grpc_error_is_special (err))
    {
      if (which == GRPC_ERROR_STR_GRPC_MESSAGE)
	{
	  for (size_t i = 0; i < GPR_ARRAY_SIZE (error_status_map); i++)
		...
		{
		  *str =
		    grpc_slice_from_static_string (error_status_map[i].msg);
		  return true;
		}
	    }
	}
...
static void
append_chr (char c, char **s, size_t * sz, size_t * cap)
{
  if (*sz == *cap)
    {
...
static void
append_str (const char *str, char **s, size_t * sz, size_t * cap)
{
...
static void
append_esc_str (const uint8_t * str, size_t len, char **s, size_t * sz,
		size_t * cap)
{
...
static void
append_kv (kv_pairs * kvs, char *key, char *value)
{
...
static void
collect_ints_kvs (grpc_error * err, kv_pairs * kvs)
{
...
static char *
key_str (grpc_error_strs which)
{
  return gpr_strdup (error_str_name (which));
}

static char *
fmt_str (grpc_slice slice)
{
  char *s = nullptr;
  size_t sz = 0;
  size_t cap = 0;
...
static void
collect_strs_kvs (grpc_error * err, kv_pairs * kvs)
{
  for (size_t which = 0; which < GRPC_ERROR_STR_MAX; ++which)
    {
      uint8_t slot = err->strs[which];
...
static char *
key_time (grpc_error_times which)
{
  return gpr_strdup (error_time_name (which));
}

static char *
fmt_time (gpr_timespec tm)
{
  char *out;
  const char *pfx = "!!";
...
static void
collect_times_kvs (grpc_error * err, kv_pairs * kvs)
{
...
static void
add_errs (grpc_error * err, char **s, size_t * sz, size_t * cap)
{
  uint8_t slot = err->first_err;
  bool first = true;
  while (slot != UINT8_MAX)
...
static int
cmp_kvs (const void *a, const void *b)
{
  ...
  for (size_t i = 0; i < kvs->num_kvs; i++)
    {
      if (i != 0)
	append_chr (',', &s, &sz, &cap);
      append_esc_str ((const uint8_t *) kvs->kvs[i].key,
...
const char *
grpc_error_string (grpc_error * err)
{
  GPR_TIMER_BEGIN ("grpc_error_string", 0);
  if (err == GRPC_ERROR_NONE)
    return no_error_string;
  if (err == GRPC_ERROR_OOM)
    return oom_error_string;
  if (err == GRPC_ERROR_CANCELLED)
    return cancelled_error_string;
...
grpc_error *
grpc_os_error (const char *file, int line, int err, const char *call_name)
{
  return
			...
			(grpc_error_set_int
			 (grpc_error_create
			  (file, line,
			   ...
			   nullptr, 0), GRPC_ERROR_INT_ERRNO, err),
...
grpc_wsa_error (const char *file, int line, int err, const char *call_name)
{
  ...
  grpc_error *error =
    grpc_error_set_str (grpc_error_set_str
			(grpc_error_set_int (grpc_error_create (file, line,
			 ...
			 GRPC_ERROR_STR_OS_ERROR,
			 grpc_slice_from_copied_string (utf8_message)),
			GRPC_ERROR_STR_SYSCALL,
			grpc_slice_from_static_string (call_name));
...
bool
grpc_log_if_error (const char *what, grpc_error * error, const char *file,
		   int line)
{
