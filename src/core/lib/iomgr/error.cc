...
absl::Status grpc_os_error(const grpc_core::DebugLocation& location,
                           int err, const char* call_name) {
  absl::Status s = StatusCreate(absl::StatusCode::kUnknown, "OS Error",
                                location, {});
  grpc_core::StatusSetInt(&s, grpc_core::StatusIntProperty::kErrorNo,
                          err);
...
absl::Status grpc_wsa_error(const grpc_core::DebugLocation& location,
  ...
  absl::Status s = StatusCreate(absl::StatusCode::kUnknown, "WSA Error",
                                location, {});
...
static void error_destroy(grpc_error_handle err) {
  ...
  unref_errs(err);
  unref_strs(err);
  gpr_free(reinterpret_cast<void*>(
      gpr_atm_acq_load(&err->atomics.error_string)));
  gpr_free(err);
}
...
static uint8_t get_placement(grpc_error_handle* err, size_t size) {
  ...
  if ((*err)->arena_size + slots > (*err)->arena_capacity) {
    ...
    *err = static_cast<grpc_error_handle>(
        gpr_realloc(*err, sizeof(grpc_error) + (*err)->arena_capacity *
                                                   sizeof(intptr_t)));
#ifndef NDEBUG
    if (grpc_trace_error_refcount.enabled()) {
      if (*err != orig) {
...
static void internal_add_error(grpc_error_handle* err,
  ...
  if ((*err)->first_err == UINT8_MAX) {
    ...
    GPR_ASSERT((*err)->last_err != UINT8_MAX);
    grpc_linked_error* old_last = reinterpret_cast<grpc_linked_error*>(
        (*err)->arena + (*err)->last_err);
    old_last->next = slot;
    (*err)->last_err = slot;
  }
...
#define SLOTS_PER_LINKED_ERROR \
  (sizeof(grpc_linked_error) / sizeof(intptr_t))

// size of storing one int and two slices and a timespec. For line,
// desc, file, and time created
...
grpc_error_handle grpc_error_create(const char* file, int line,
  ...
  uint8_t initial_arena_capacity = static_cast<uint8_t>(
      ...
      SURPLUS_CAPACITY);
  grpc_error_handle err = static_cast<grpc_error_handle>(gpr_malloc(
  ...
  if (!gpr_atm_no_barrier_load(&g_error_creation_allowed)) {
    gpr_log(GPR_ERROR,
            "Error creation occurred when error creation was disabled "
            "[%s:%d]",
            file, line);
    abort();
  }
  ...
  for (size_t i = 0; i < num_referencing; ++i) {
    ...
    internal_add_error(
        &err,
        GRPC_ERROR_REF(referencing[i]));  // TODO(ncteisen), change
                                          // ownership semantics
  }
...
static grpc_error_handle copy_error_and_unref(grpc_error_handle in) {
  ...
  if (grpc_error_is_special(in)) {
    ...
    // the returned err will be added to, so we ensure this is room to
    ...
    out = static_cast<grpc_error_handle>(gpr_malloc(
        sizeof(*in) + new_arena_capacity * sizeof(intptr_t)));
#ifndef NDEBUG
    if (grpc_trace_error_refcount.enabled()) {
    ...
    memcpy(
        reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(out) +
                                skip),
...
grpc_error_handle grpc_error_add_child(grpc_error_handle src,
  ...
  if (src != GRPC_ERROR_NONE) {
    if (child == GRPC_ERROR_NONE) {
      ...
      /* \a src and \a child are the same. Drop one of the references
...
static void append_str(const char* str, char** s, size_t* sz,
                       size_t* cap) {
...
static void append_esc_str(const uint8_t* str, size_t len, char** s,
...
static char* fmt_str(const grpc_slice& slice) {
  ...
  size_t sz = 0;
  size_t cap = 0;
  append_esc_str(GRPC_SLICE_START_PTR(slice), GRPC_SLICE_LENGTH(slice),
                 &s, &sz, &cap);
  append_chr(0, &s, &sz, &cap);
  return s;
}
...
static void collect_strs_kvs(grpc_error_handle err, kv_pairs* kvs) {
  for (size_t which = 0; which < GRPC_ERROR_STR_MAX; ++which) {
    ...
    if (slot != UINT8_MAX) {
      append_kv(
          kvs, key_str(static_cast<grpc_error_strs>(which)),
...
static char* fmt_time(gpr_timespec tm) {
  ...
  gpr_asprintf(&out, "\"%s%" PRId64 ".%09d\"", pfx, tm.tv_sec,
               tm.tv_nsec);
  return out;
}
  ...
  for (size_t which = 0; which < GRPC_ERROR_TIME_MAX; ++which) {
    ...
    if (slot != UINT8_MAX) {
      append_kv(kvs, key_time(static_cast<grpc_error_times>(which)),
                fmt_time(*reinterpret_cast<gpr_timespec*>(err->arena +
                                                          slot)));
    }
  }
}

static void add_errs(grpc_error_handle err, char** s, size_t* sz,
                     size_t* cap) {
  uint8_t slot = err->first_err;
  bool first = true;
  while (slot != UINT8_MAX) {
  ...
  return grpc_error_set_str(
      grpc_error_set_str(
          grpc_error_set_int(
              grpc_error_create(
                  file, line,
                  grpc_slice_from_static_string(strerror(err)), nullptr,
                  0),
...
grpc_error_handle grpc_wsa_error(const char* file, int line, int err,
  ...
  grpc_error_handle error = grpc_error_set_str(
      grpc_error_set_str(
          grpc_error_set_int(
              grpc_error_create(
                  file, line, grpc_slice_from_static_string("OS Error"),
                  NULL, 0),
