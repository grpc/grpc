...
void grpc_disable_error_creation() { gpr_atm_no_barrier_store(&g_error_creation_allowed, false); }

void grpc_enable_error_creation() { gpr_atm_no_barrier_store(&g_error_creation_allowed, true); }
...
absl::Status grpc_os_error(const grpc_core::DebugLocation& location, int err,
  ...
  absl::Status s = StatusCreate(absl::StatusCode::kUnknown, "OS Error", location, {});
  grpc_core::StatusSetInt(&s, grpc_core::StatusIntProperty::kErrorNo, err);
  grpc_core::StatusSetStr(&s, grpc_core::StatusStrProperty::kOsError, strerror(err));
  grpc_core::StatusSetStr(&s, grpc_core::StatusStrProperty::kSyscall, call_name);
  return s;
}
...
absl::Status grpc_wsa_error(const grpc_core::DebugLocation& location, int err,
  ...
  absl::Status s = StatusCreate(absl::StatusCode::kUnknown, "WSA Error", location, {});
...
grpc_error_handle grpc_error_set_int(grpc_error_handle src, grpc_error_ints which, intptr_t value) {
  grpc_core::StatusSetInt(&src, static_cast<grpc_core::StatusIntProperty>(which), value);
  return src;
}

bool grpc_error_get_int(grpc_error_handle error, grpc_error_ints which, intptr_t* p) {
  absl::optional<intptr_t> value =
      grpc_core::StatusGetInt(error, static_cast<grpc_core::StatusIntProperty>(which));
  if (value.has_value()) {
    *p = *value;
    return true;
  } else {
...
grpc_error_handle grpc_error_set_str(grpc_error_handle src, grpc_error_strs which,
  ...
  grpc_core::StatusSetStr(&src, static_cast<grpc_core::StatusStrProperty>(which),
...
bool grpc_error_get_str(grpc_error_handle error, grpc_error_strs which, grpc_slice* s) {
  absl::optional<std::string> value =
      grpc_core::StatusGetStr(error, static_cast<grpc_core::StatusStrProperty>(which));
  if (value.has_value()) {
...
static void unref_errs(grpc_error_handle err) {
  ...
  while (slot != UINT8_MAX) {
    ...
    GPR_ASSERT(err->last_err == slot ? lerr->next == UINT8_MAX : lerr->next != UINT8_MAX);
    slot = lerr->next;
  }
}
...
static void unref_strs(grpc_error_handle err) {
  for (size_t which = 0; which < GRPC_ERROR_STR_MAX; ++which) {
    ...
    if (slot != UINT8_MAX) {
      grpc_slice_unref_internal(*reinterpret_cast<grpc_slice*>(err->arena + slot));
    }
  }
}
...
static void error_destroy(grpc_error_handle err) {
  ...
  unref_errs(err);
  unref_strs(err);
  gpr_free(reinterpret_cast<void*>(gpr_atm_acq_load(&err->atomics.error_string)));
  gpr_free(err);
}
...
#endif
    *err = static_cast<grpc_error_handle>(
        gpr_realloc(*err, sizeof(grpc_error) + (*err)->arena_capacity * sizeof(intptr_t)));
#ifndef NDEBUG
    if (grpc_trace_error_refcount.enabled()) {
      if (*err != orig) {
...
static void internal_set_str(grpc_error_handle* err, grpc_error_strs which,
  ...
  if (slot == UINT8_MAX) {
    ...
    if (slot == UINT8_MAX) {
      ...
      gpr_log(GPR_ERROR, "Error %p is full, dropping string {\"%s\":\"%s\"}", *err,
              error_str_name(which), str);
      gpr_free(str);
      return;
    }
  } else {
...
static void internal_set_time(grpc_error_handle* err, grpc_error_times which, gpr_timespec value) {
  ...
  if (slot == UINT8_MAX) {
    ...
    if (slot == UINT8_MAX) {
      ...
      gpr_log(GPR_ERROR, "Error %p is full, dropping \"%s\":\"%s\"}", *err, error_time_name(which),
              time_str);
      gpr_free(time_str);
      return;
    }
...
static void internal_add_error(grpc_error_handle* err, grpc_error_handle new_err) {
  ...
  if (slot == UINT8_MAX) {
    gpr_log(GPR_ERROR, "Error %p is full, dropping error %p = %s", *err, new_err,
            grpc_error_string(new_err));
...
grpc_error_handle grpc_error_create(const char* file, int line, const grpc_slice& desc,
  ...
  uint8_t initial_arena_capacity = static_cast<uint8_t>(
      DEFAULT_ERROR_CAPACITY + static_cast<uint8_t>(num_referencing * SLOTS_PER_LINKED_ERROR) +
      SURPLUS_CAPACITY);
  ...
  if (!gpr_atm_no_barrier_load(&g_error_creation_allowed)) {
    gpr_log(GPR_ERROR, "Error creation occurred when error creation was disabled [%s:%d]", file,
            line);
    abort();
  }
  if (grpc_trace_error_refcount.enabled()) {
  ...
  internal_set_str(&err, GRPC_ERROR_STR_FILE, grpc_slice_from_static_string(file));
  ...
  for (size_t i = 0; i < num_referencing; ++i) {
    ...
    internal_add_error(
        &err,
        GRPC_ERROR_REF(referencing[i]));  // TODO(ncteisen), change ownership semantics
  }
...
static grpc_error_handle copy_error_and_unref(grpc_error_handle in) {
  ...
  if (grpc_error_is_special(in)) {
    ...
    if (in == GRPC_ERROR_NONE) {
      internal_set_str(&out, GRPC_ERROR_STR_DESCRIPTION, grpc_slice_from_static_string("no error"));
      internal_set_int(&out, GRPC_ERROR_INT_GRPC_STATUS, GRPC_STATUS_OK);
    } else if (in == GRPC_ERROR_OOM) {
      internal_set_str(&out, GRPC_ERROR_STR_DESCRIPTION, grpc_slice_from_static_string("oom"));
...
const special_error_status_map error_status_map[] = {
    ...
    {GRPC_STATUS_RESOURCE_EXHAUSTED, "Out of memory", strlen("Out of memory")},  // GRPC_ERROR_OOM
    {GRPC_STATUS_INVALID_ARGUMENT, "", 0},                      // GRPC_ERROR_RESERVED_2
    {GRPC_STATUS_CANCELLED, "Cancelled", strlen("Cancelled")},  // GRPC_ERROR_CANCELLED
};
...
bool grpc_error_get_str(grpc_error_handle err, grpc_error_strs which, grpc_slice* str) {
  if (grpc_error_is_special(err)) {
    ...
    const special_error_status_map& msg = error_status_map[reinterpret_cast<size_t>(err)];
    str->refcount = &grpc_core::kNoopRefcount;
    str->data.refcounted.bytes = reinterpret_cast<uint8_t*>(const_cast<char*>(msg.msg));
...
static void append_kv(kv_pairs* kvs, char* key, char* value) {
  if (kvs->num_kvs == kvs->cap_kvs) {
    ...
    kvs->kvs = static_cast<kv_pair*>(gpr_realloc(kvs->kvs, sizeof(*kvs->kvs) * kvs->cap_kvs));
  }
...
static char* key_int(grpc_error_ints which) { return gpr_strdup(error_int_name(which)); }
...
static void collect_ints_kvs(grpc_error_handle err, kv_pairs* kvs) {
  for (size_t which = 0; which < GRPC_ERROR_INT_MAX; ++which) {
    uint8_t slot = err->ints[which];
    if (slot != UINT8_MAX) {
      append_kv(kvs, key_int(static_cast<grpc_error_ints>(which)), fmt_int(err->arena[slot]));
    }
  }
}

static char* key_str(grpc_error_strs which) { return gpr_strdup(error_str_name(which)); }

static char* fmt_str(const grpc_slice& slice) {
  char* s = nullptr;
  size_t sz = 0;
  size_t cap = 0;
  append_esc_str(GRPC_SLICE_START_PTR(slice), GRPC_SLICE_LENGTH(slice), &s, &sz, &cap);
  append_chr(0, &s, &sz, &cap);
  return s;
}
...
static char* key_time(grpc_error_times which) { return gpr_strdup(error_time_name(which)); }
...
const char* grpc_error_string(grpc_error_handle err) {
  ...
  void* p = reinterpret_cast<void*>(gpr_atm_acq_load(&err->atomics.error_string));
  if (p != nullptr) {
  ...
  if (!gpr_atm_rel_cas(&err->atomics.error_string, 0, reinterpret_cast<gpr_atm>(out))) {
    gpr_free(out);
...
grpc_error_handle grpc_os_error(const char* file, int line, int err, const char* call_name) {
  return grpc_error_set_str(
      grpc_error_set_str(
          grpc_error_set_int(
              grpc_error_create(file, line, grpc_slice_from_static_string(strerror(err)), nullptr,
                                0),
...
#ifdef GPR_WINDOWS
grpc_error_handle grpc_wsa_error(const char* file, int line, int err, const char* call_name) {
  char* utf8_message = gpr_format_message(err);
  grpc_error_handle error = grpc_error_set_str(
      grpc_error_set_str(
          grpc_error_set_int(
              grpc_error_create(file, line, grpc_slice_from_static_string("OS Error"), NULL, 0),
