...
static uint8_t get_placement(grpc_error** err, size_t size) {
  ...
  if ((*err)->arena_size + slots > (*err)->arena_capacity) {
    ...
    grpc_error* orig = *err;
#endif
    *err = reinterpret_cast<grpc_error*>(gpr_realloc(
...
static void internal_set_time(grpc_error** err, grpc_error_times which,
  ...
  if (slot == UINT8_MAX) {
    ...
    if (slot == UINT8_MAX) {
      ...
      gpr_free(const_cast<char*>(time_str));
      return;
    }
  }
...
grpc_error* grpc_error_create(const char* file, int line, grpc_slice desc,
  ...
  uint8_t initial_arena_capacity = static_cast<uint8_t>(
      ...
      SURPLUS_CAPACITY);
  grpc_error* err = reinterpret_cast<grpc_error*>(
...
static grpc_error* copy_error_and_unref(grpc_error* in) {
  ...
  if (grpc_error_is_special(in)) {
    ...
    out = reinterpret_cast<grpc_error*>(
    ...
    memcpy(reinterpret_cast<void*>((uintptr_t)out + skip),
           reinterpret_cast<void*>((uintptr_t)in + skip),
...
static void append_chr(char c, char** s, size_t* sz, size_t* cap) {
  if (*sz == *cap) {
    ...
    *s = reinterpret_cast<char*>(gpr_realloc(*s, *cap));
  }
  (*s)[(*sz)++] = c;
}
...
static void append_kv(kv_pairs* kvs, char* key, char* value) {
  if (kvs->num_kvs == kvs->cap_kvs) {
    ...
    kvs->kvs = reinterpret_cast<kv_pair*>(
...
static int cmp_kvs(const void* a, const void* b) {
  const kv_pair* ka = reinterpret_cast<const kv_pair*>(a);
  const kv_pair* kb = reinterpret_cast<const kv_pair*>(b);
