...
static void
destroy_server (void *arg, grpc_error * error)
{
...
static void
finish_shutdown_locked (grpc_tcp_server * s)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE
		      (destroy_server, s, grpc_schedule_on_exec_ctx),
		      GRPC_ERROR_NONE);
}
...
static grpc_error *
prepare_socket (SOCKET sock, const grpc_resolved_address * addr, int *port)
{
...
failure:
  ...
  grpc_error_set_int (grpc_error_set_str
...
static void
decrement_active_ports_and_notify_locked (grpc_tcp_listener * sp)
{
  sp->shutting_down = 0;
...
static grpc_error *
start_accept_locked (grpc_tcp_listener * port)
{
...
static void
on_accept (void *arg, grpc_error * error)
{
  ...
  if (!wsa_success)
      ...
      if (!sp->shutting_down)
	{
	  ...
	  err =
	    getpeername (sock, (struct sockaddr *) peer_name.addr,
			 &peer_name_len);
  ...
  for (sp = s->head; sp; sp = sp->next)
    {
      GPR_ASSERT (GRPC_LOG_IF_ERROR
		  ("start_accept", start_accept_locked (sp)));
      s->active_ports++;
    }
  gpr_mu_unlock (&s->mu);
}
