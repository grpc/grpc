...
struct grpc_tcp_listener {
  /* This seemingly magic number comes from AcceptEx's documentation.
     each address buffer needs to have at least 16 more bytes at their
     end. */
...
static void destroy_server(void* arg, grpc_error_handle error) {
  ...
  /* Now that the accepts have been aborted, we can destroy the sockets.
     The IOCP won't get notified on these, so we can flag them as
...
static grpc_error_handle prepare_socket(
    SOCKET sock, const grpc_resolved_address* addr, int* port) {
...
/* In order to do an async accept, we need to create a socket first
   which will be the one assigned to the new incoming connection. */
static grpc_error_handle start_accept_locked(grpc_tcp_listener* port) {
  ...
  success = port->AcceptEx(port->socket->socket, sock, port->addresses,
                           0, addrlen, addrlen, &bytes_received,
                           &port->socket->read_info.overlapped);

  /* It is possible to get an accept immediately without delay. However,
     we
     will still get an IOCP notification for it. So let's just ignore
     it. */
  if (!success) {
  ...
  /* We're ready to do the accept. Calling grpc_socket_notify_on_read
     may immediately process an accept that happened in the meantime. */
...
static void on_accept(void* arg, grpc_error_handle error) {
  ...
  /* The general mechanism for shutting down is to queue abortion calls.
     While this is necessary in the read/write case, it's useless for
     the accept
  ...
  /* The IOCP notified us of a completed operation. Let's grab the
     results, and act accordingly. */
  transfered_bytes = 0;
  wsa_success = WSAGetOverlappedResult(
      sock, &info->overlapped, &transfered_bytes, FALSE, &flags);
  if (!wsa_success) {
    if (!sp->shutting_down) {
    ...
    if (!sp->shutting_down) {
      ...
      err = getpeername(sock, (grpc_sockaddr*)peer_name.addr,
                        &peer_name_len);
      ...
      std::string fd_name =
          absl::StrCat("tcp_server:", peer_name_string);
  ...
  /* As we were notified from the IOCP of one and exactly one accept,
     the former socked we created has now either been destroy or
     assigned to the new connection. We need to create a new one for the
     next connection. */
  GPR_ASSERT(
      GRPC_LOG_IF_ERROR("start_accept", start_accept_locked(sp)));
...
static grpc_error_handle add_socket_to_server(
    grpc_tcp_server* s, SOCKET sock, const grpc_resolved_address* addr,
  ...
  status = WSAIoctl(sock, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid,
                    sizeof(guid), &AcceptEx, sizeof(AcceptEx),
...
static void tcp_server_start(
    grpc_tcp_server* s, const std::vector<grpc_pollset*>* /*pollsets*/,
