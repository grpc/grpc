...
grpc_error_handle grpc_resolve_unix_domain_address(const char* /* name */,
  ...
  *addresses = NULL;
  return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Unix domain sockets are not supported on Windows");
}

grpc_error_handle grpc_resolve_unix_abstract_domain_address(absl::string_view,
                                                            grpc_resolved_addresses** addresses) {
  *addresses = NULL;
  return GRPC_ERROR_CREATE_FROM_STATIC_STRING("Unix domain sockets are not supported on Windows");
}
...
std::string grpc_sockaddr_to_uri_unix_if_possible(const grpc_resolved_address* /* addr */) {
  return "";
}
