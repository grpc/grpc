  ...
  do {
    ...
    if (read_bytes <= 0) {
      ...
      if (read_bytes == 0) {
        ...
        *error = tcp_annotate_error(absl::InternalError(absl::StrCat(
    ...
    if (!tcp->memory_owner.is_valid() && error.ok()) {
      tcp_read_error =
          tcp_annotate_error(absl::InternalError(absl::StrCat(
                                 "Socket closed to ", tcp->peer_string)),
                             tcp);
    } else {
      ...
      if (saved_errno == EAGAIN || saved_errno == ENOBUFS) {
        ...
        *error = tcp_annotate_error(
            grpc_core::StatusCreate(
                ...
                absl::StrCat("sendmsg: ", grpc_core::StrError(saved_errno),
                             " (", saved_errno,
                             "), peer_address=", tcp->peer_string),
                DEBUG_LOCATION, {}),
            tcp);
  ...
  if (buf->length == 0) {
    grpc_core::Closure::Run(
        ...
        grpc_fd_is_shutdown(tcp->em_fd)
            ? tcp_annotate_error(GRPC_ERROR_CREATE(absl::StrCat(
                                     "EOF from ", tcp->peer_string)),
                                 tcp)
            : absl::OkStatus());
    tcp_shutdown_buffer_list(tcp);
    return;
