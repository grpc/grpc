...
static void
done_poller (void *bp, grpc_error * error_ignored)
{
...
static void
run_poller (void *bp, grpc_error * error_ignored)
{
  backup_poller *p = (backup_poller *) bp;
  if (grpc_tcp_trace.enabled ())
    {
  ...
  grpc_millis deadline =
    grpc_core::ExecCtx::Get ()->Now () + 13 * GPR_MS_PER_SEC;
  GRPC_STATS_INC_TCP_BACKUP_POLLER_POLLS ();
  GRPC_LOG_IF_ERROR ("backup_poller:pollset_work",
		     grpc_pollset_work (BACKUP_POLLER_POLLSET (p), nullptr,
					deadline));
  ...
  if (gpr_atm_no_barrier_load (&g_uncovered_notifications_pending) == 1 &&
      ...
      if (grpc_tcp_trace.enabled ())
	{
      ...
      grpc_pollset_shutdown (BACKUP_POLLER_POLLSET (p),
			     GRPC_CLOSURE_INIT (&p->run_poller, done_poller,
						p,
  ...
  if (grpc_tcp_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "BACKUP_POLLER:%p uncover cnt %d->%d", p,
	       (int) old_count, (int) old_count - 1);
    }
  GPR_ASSERT (old_count != 1);
}

static void
cover_self (grpc_tcp * tcp)
{
  backup_poller *p;
  gpr_atm old_count =
...
static void
tcp_drop_uncovered_then_handle_write (void *arg, grpc_error * error)
{
  if (grpc_tcp_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "TCP:%p got_write: %s", arg,
...
static void
add_to_estimate (grpc_tcp * tcp, size_t bytes)
{
  tcp->bytes_read_this_round += (double) bytes;
}

static void
finish_estimate (grpc_tcp * tcp)
{
...
static size_t
get_target_read_size (grpc_tcp * tcp)
{
...
static grpc_error *
tcp_annotate_error (grpc_error * src_error, grpc_tcp * tcp)
{
  return
    grpc_error_set_str (grpc_error_set_int
			(grpc_error_set_int
			 (src_error, GRPC_ERROR_INT_FD, tcp->fd),
...
static void
tcp_shutdown (grpc_endpoint * ep, grpc_error * why)
{
  ...
  if (grpc_tcp_trace.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "TCP unref %p : %s %" PRIdPTR " -> %" PRIdPTR, tcp, reason,
	       val, val - 1);
    }
  ...
  if (grpc_tcp_trace.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&tcp->refcount.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "TCP   ref %p : %s %" PRIdPTR " -> %" PRIdPTR, tcp, reason,
	       val, val + 1);
    }
...
static void
call_read_cb (grpc_tcp * tcp, grpc_error * error)
{
  grpc_closure *cb = tcp->read_cb;

  if (grpc_tcp_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "TCP:%p call_cb %p %p:%p", tcp, cb, cb->cb,
	       cb->cb_arg);
      size_t i;
...
tcp_do_read (grpc_tcp * tcp)
{
  ...
  if (read_bytes < 0)
      ...
      else
	{
	  ...
	  call_read_cb (tcp,
			tcp_annotate_error (GRPC_OS_ERROR (errno, "recvmsg"),
					    tcp));
	  TCP_UNREF (tcp, "read");
	}
    }
...
static void
tcp_read_allocation_done (void *tcpp, grpc_error * error)
{
...
static void
tcp_continue_read (grpc_tcp * tcp)
{
...
static void
tcp_handle_read (void *arg /* grpc_tcp */ , grpc_error * error)
{
  grpc_tcp *tcp = (grpc_tcp *) arg;
  GPR_ASSERT (!tcp->finished_edge);
  if (grpc_tcp_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "TCP:%p got_read: %s", tcp,
	       grpc_error_string (error));
    }
...
static void
tcp_read (grpc_endpoint * ep, grpc_slice_buffer * incoming_buffer,
	  grpc_closure * cb)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
...
static bool
tcp_flush (grpc_tcp * tcp, grpc_error ** error)
{
  struct msghdr msg;
  ...
  for (;;)
    {
      ...
      for (iov_size = 0; outgoing_slice_idx != tcp->outgoing_buffer->count &&
	   iov_size != MAX_WRITE_IOVEC; iov_size++)
	{
	  iov[iov_size].iov_base =
	    GRPC_SLICE_START_PTR (tcp->outgoing_buffer->
				  slices[outgoing_slice_idx]) +
	    tcp->outgoing_byte_idx;
	  iov[iov_size].iov_len =
	    GRPC_SLICE_LENGTH (tcp->outgoing_buffer->
			       slices[outgoing_slice_idx]) -
	    tcp->outgoing_byte_idx;
      ...
      if (sent_length < 0)
	{
	  if (errno == EAGAIN)
		...
		{
		  grpc_slice_unref_internal (grpc_slice_buffer_take_first
					     (tcp->outgoing_buffer));
		}
	      return false;
	    }
		...
		tcp_annotate_error (GRPC_OS_ERROR (errno, "sendmsg"), tcp);
	      grpc_slice_buffer_reset_and_unref_internal (tcp->
							  outgoing_buffer);
	      return true;
	    }
	  else
...
static void
tcp_handle_write (void *arg /* grpc_tcp */ , grpc_error * error)
{
  grpc_tcp *tcp = (grpc_tcp *) arg;
  grpc_closure *cb;
...
static void
tcp_write (grpc_endpoint * ep, grpc_slice_buffer * buf, grpc_closure * cb)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
  ...
  if (grpc_tcp_trace.enabled ())
      ...
      for (i = 0; i < buf->count; i++)
	{
	  ...
	  gpr_log (GPR_DEBUG, "WRITE %p (peer=%s): %s", tcp, tcp->peer_string,
		   data);
	  gpr_free (data);
	}
    }
  ...
  if (buf->length == 0)
    {
      ...
      GRPC_CLOSURE_SCHED (cb, grpc_fd_is_shutdown (tcp->em_fd)
			  ?
			  tcp_annotate_error
...
static void
tcp_add_to_pollset (grpc_endpoint * ep, grpc_pollset * pollset)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
...
static void
tcp_add_to_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset_set)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
...
static void
tcp_delete_from_pollset_set (grpc_endpoint * ep,
...
tcp_get_resource_user (grpc_endpoint * ep)
{
  grpc_tcp *tcp = (grpc_tcp *) ep;
      ...
      for (size_t i = 0; i < channel_args->num_args; i++)
	{
	  if (0 ==
			   ...
			   GRPC_ARG_RESOURCE_QUOTA))
	    {
	      ...
	      resource_quota = grpc_resource_quota_ref_internal ((grpc_resource_quota *) channel_args->args[i].value.pointer.p);
	    }
	}
    }
...
void
grpc_tcp_destroy_and_release_fd (grpc_endpoint * ep, int *fd,
				 grpc_closure * done)
{
  grpc_network_status_unregister_endpoint (ep);
  grpc_tcp *tcp = (grpc_tcp *) ep;
