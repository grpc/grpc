...
static void maybe_make_read_slices(grpc_tcp* tcp)
  ...
  if (tcp->incoming_buffer->length <
    ...
    // If memory usage begins to be high drop down to just one entry in the iovec, so as to reduce the possibility of stranding more memory than we otherwise need.
    ...
    while (tcp->incoming_buffer->length <
      ...
      grpc_slice_buffer_add_indexed(tcp->incoming_buffer,
                                    tcp->memory_owner.MakeSlice(alloc_size));
