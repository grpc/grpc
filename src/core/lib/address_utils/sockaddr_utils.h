...
/* Returns true if addr is an IPv4-mapped IPv6 address within the
   ...
   If addr4_out is non-NULL, the inner IPv4 address will be copied here
   when returning true. */
int grpc_sockaddr_is_v4mapped(const grpc_resolved_address* addr,
                              grpc_resolved_address* addr4_out);

/* If addr is an AF_INET address, writes the corresponding
   ::ffff:0.0.0.0/96 address to addr6_out and returns true.  Otherwise
   returns false. */
...
int grpc_sockaddr_is_wildcard(const grpc_resolved_address* addr,
                              int* port_out);
...
void grpc_sockaddr_make_wildcard4(int port,
                                  grpc_resolved_address* wild_out);
...
// If the normalize flag is enabled, ::ffff:0.0.0.0/96 IPv6 addresses
// are displayed as plain IPv4.
std::string grpc_sockaddr_to_string(const grpc_resolved_address* addr,
                                    bool normalize)
    GRPC_MUST_USE_RESULT;

// Newer form of grpc_string_to_sockaddr which returns an error instead
// of crashing if \a addr is not IPv6/IPv6
...
int grpc_sockaddr_get_family(
    const grpc_resolved_address* resolved_addr);

std::string grpc_sockaddr_get_packed_host(
    const grpc_resolved_address* resolved_addr);

// Applies a mask of \a mask_bits to IPv4/IPv6 addresses. Has no effect
// if the address type is not IPv4/IPv6.
void grpc_sockaddr_mask_bits(grpc_resolved_address* address,
                             uint32_t mask_bits);

// If \a address is IPv4/IPv6, checks if the IP address falls in the
// CIDR specified by \a subnet_address and \a mask_bits. Returns false
// if \a address is not an IPv4/IPv6 address. The ports (if set) are
// ignored for matching purposes. Note that, \a subnet_address should be
// normalized, i.e., `grpc_sockaddr_mask_bits` should have been called
// on it if necessary.
bool grpc_sockaddr_match_subnet(
    const grpc_resolved_address* address,
    const grpc_resolved_address* subnet_address, uint32_t mask_bits);

namespace grpc_event_engine {
namespace experimental {

std::string ResolvedAddressToURI(
