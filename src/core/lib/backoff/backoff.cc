 ...
 * rand() modifies a global variable we have no control over. */
	...
	return a;
      if (a > b)
	GPR_SWAP (double, a, b);	// make sure a < b
...
BackOff::BackOff (const Options & options):options_ (options),
    rng_state_ (static_cast < uint32_t >
		(gpr_now (GPR_CLOCK_REALTIME).tv_nsec))
  {
    Reset ();
  }
  ...
  grpc_millis BackOff::NextAttemptTime ()
  {
    ...
    const double jitter =
      generate_uniform_random_number_between (&rng_state_,
					      -options_.jitter () *
					      current_backoff_,
					      options_.jitter () *
					      current_backoff_);
    const grpc_millis next_timeout =
      (grpc_millis) (current_backoff_ + jitter);
