...
double generate_uniform_random_number_between(uint32_t* rng_state, double a, double b) {
  if (a == b) return a;
...
BackOff::BackOff(const Options& options)
    : options_(options), rng_state_(static_cast<uint32_t>(gpr_now(GPR_CLOCK_REALTIME).tv_nsec)) {
  Reset();
}
...
grpc_millis BackOff::NextAttemptTime() {
  ...
  current_backoff_ = static_cast<grpc_millis>(std::min(
      current_backoff_ * options_.multiplier(), static_cast<double>(options_.max_backoff())));
  const double jitter = generate_uniform_random_number_between(
      &rng_state_, -options_.jitter() * current_backoff_, options_.jitter() * current_backoff_);
