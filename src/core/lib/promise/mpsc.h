...
class Mpsc {
 ...
 private:
  class SendPoller {
   public:
    ...
    SendPoller(SendPoller&& other)
        : node_(std::exchange(other.node_, nullptr)) {}
    SendPoller& operator=(SendPoller&& other) {
  ...
  class NextPoller {
   public:
    ...
    NextPoller(NextPoller&& other)
        : mpsc_(std::exchange(other.mpsc_, nullptr)) {}
    NextPoller& operator=(NextPoller&& other) {
 ...
 private:
  ...
  void ReleaseTokensAndClose(bool wakeup, Node* node);
  Poll<Node*> Dequeue();
  ...
  void DrainMpscFrom(bool wakeup, Node* node);
  void PushStub();
  void ReleaseActiveTokens(uint64_t tokens);
  Poll<ValueOrFailure<Node*>> PollNext();
  Json PollNextJson() const;

  static constexpr uint64_t kActiveTokensWakerBit = 1ull << 63;
  static constexpr uint64_t kActiveTokensMask = kActiveTokensWakerBit - 1;
  const uint64_t max_queued_;

  std::atomic<uint64_t> queued_tokens_{0};
  std::atomic<Waker*> active_tokens_waker_ = nullptr;
