...
/*
 * Automatically generated by tools/codegen/core/gen_try_join_switch.py
 */
    ...
    if ((state_ & 1) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 1;
          Destruct(&pending0_);
          Construct(&ready0_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    }
...
class TryJoin<F0, F1> {
 ...
 public:
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 2) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 2;
          Destruct(&pending1_);
          Construct(&ready1_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_)));
  }
};
template <typename F0, typename F1, typename F2>
class TryJoin<F0, F1, F2> {
 private:
  [[no_unique_address]] uint8_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
    ...
    if ((state_ & 4) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 4;
          Destruct(&pending2_);
          Construct(&ready2_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3> {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
    ...
    if ((state_ & 8) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 8;
          Destruct(&pending3_);
          Construct(&ready3_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3, F4> {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
    ...
    if ((state_ & 16) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 16;
          Destruct(&pending4_);
          Construct(&ready4_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5>
class TryJoin<F0, F1, F2, F3, F4, F5> {
 private:
  [[no_unique_address]] uint8_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 32) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 32;
          Destruct(&pending5_);
          Construct(&ready5_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(Result(absl::in_place, std::move(ready0_), std::move(ready1_),
                        std::move(ready2_), std::move(ready3_),
                        std::move(ready4_), std::move(ready5_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6>
class TryJoin<F0, F1, F2, F3, F4, F5, F6> {
 private:
  [[no_unique_address]] uint8_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5, F6 f6)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
    ...
    if ((state_ & 64) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 64;
          Destruct(&pending6_);
          Construct(&ready6_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7> {
 private:
  [[no_unique_address]] uint8_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5, F6 f6, F7 f7)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
    ...
    if ((state_ & 128) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 128;
          Destruct(&pending7_);
          Construct(&ready7_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8> {
 private:
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5, F6 f6, F7 f7, F8 f8)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
    ...
    if ((state_ & 256) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 256;
          Destruct(&pending8_);
          Construct(&ready8_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5, F6 f6, F7 f7, F8 f8, F9 f9)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
  ...
  using Result =
      absl::StatusOr<std::tuple<R0, R1, R2, R3, R4, R5, R6, R7, R8, R9>>;
  Poll<Result> operator()() {
    if ((state_ & 1) == 0) {
      auto r = pending0_();
      if (auto* p = r.get_ready()) {
    ...
    if ((state_ & 512) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 512;
          Destruct(&pending9_);
          Construct(&ready9_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(Result(absl::in_place, std::move(ready0_), std::move(ready1_),
                        ...
                        std::move(ready4_), std::move(ready5_),
                        std::move(ready6_), std::move(ready7_),
                        std::move(ready8_), std::move(ready9_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
          typename F10>
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  ...
  using Result =
      absl::StatusOr<std::tuple<R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10>>;
  Poll<Result> operator()() {
    if ((state_ & 1) == 0) {
      auto r = pending0_();
      if (auto* p = r.get_ready()) {
    ...
    if ((state_ & 1024) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 1024;
          Destruct(&pending10_);
          Construct(&ready10_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               std::move(ready2_), std::move(ready3_), std::move(ready4_),
               std::move(ready5_), std::move(ready6_), std::move(ready7_),
               std::move(ready8_), std::move(ready9_), std::move(ready10_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
          typename F10, typename F11>
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
 ...
 public:
  ...
  using Result = absl::StatusOr<
      std::tuple<R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11>>;
  Poll<Result> operator()() {
    if ((state_ & 1) == 0) {
      auto r = pending0_();
      if (auto* p = r.get_ready()) {
    ...
    if ((state_ & 2048) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 2048;
          Destruct(&pending11_);
          Construct(&ready11_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(Result(absl::in_place, std::move(ready0_), std::move(ready1_),
                        ...
                        std::move(ready6_), std::move(ready7_),
                        std::move(ready8_), std::move(ready9_),
                        std::move(ready10_), std::move(ready11_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
          typename F10, typename F11, typename F12>
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 4096) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 4096;
          Destruct(&pending12_);
          Construct(&ready12_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready5_), std::move(ready6_), std::move(ready7_),
               std::move(ready8_), std::move(ready9_), std::move(ready10_),
               std::move(ready11_), std::move(ready12_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
          typename F10, typename F11, typename F12, typename F13>
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 8192) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 8192;
          Destruct(&pending13_);
          Construct(&ready13_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready5_), std::move(ready6_), std::move(ready7_),
               std::move(ready8_), std::move(ready9_), std::move(ready10_),
               std::move(ready11_), std::move(ready12_), std::move(ready13_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
          typename F10, typename F11, typename F12, typename F13, typename F14>
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14> {
 private:
  [[no_unique_address]] uint16_t state_ = 0;
  using R0 = decltype(IntoResult(std::declval<F0>()().get_ready()));
  union {
    ...
    if ((state_ & 16384) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 16384;
          Destruct(&pending14_);
          Construct(&ready14_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 32768) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 32768;
          Destruct(&pending15_);
          Construct(&ready15_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready8_), std::move(ready9_), std::move(ready10_),
               std::move(ready11_), std::move(ready12_), std::move(ready13_),
               std::move(ready14_), std::move(ready15_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 65536) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 65536;
          Destruct(&pending16_);
          Construct(&ready16_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready8_), std::move(ready9_), std::move(ready10_),
               std::move(ready11_), std::move(ready12_), std::move(ready13_),
               std::move(ready14_), std::move(ready15_), std::move(ready16_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
    ...
    if ((state_ & 131072) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 131072;
          Destruct(&pending17_);
          Construct(&ready17_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 262144) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 262144;
          Destruct(&pending18_);
          Construct(&ready18_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready11_), std::move(ready12_), std::move(ready13_),
               std::move(ready14_), std::move(ready15_), std::move(ready16_),
               std::move(ready17_), std::move(ready18_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 524288) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 524288;
          Destruct(&pending19_);
          Construct(&ready19_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready11_), std::move(ready12_), std::move(ready13_),
               std::move(ready14_), std::move(ready15_), std::move(ready16_),
               std::move(ready17_), std::move(ready18_), std::move(ready19_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
    ...
    if ((state_ & 1048576) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 1048576;
          Destruct(&pending20_);
          Construct(&ready20_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
...
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14,
 ...
 public:
  TryJoin(F0 f0, F1 f1, F2 f2, F3 f3, F4 f4, F5 f5, F6 f6, F7 f7, F8 f8, F9 f9,
          F10 f10, F11 f11, F12 f12, F13 f13, F14 f14, F15 f15, F16 f16,
          F17 f17, F18 f18, F19 f19, F20 f20, F21 f21)
      : pending0_(std::move(f0)),
        pending1_(std::move(f1)),
        pending2_(std::move(f2)),
        pending3_(std::move(f3)),
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 2097152) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 2097152;
          Destruct(&pending21_);
          Construct(&ready21_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready14_), std::move(ready15_), std::move(ready16_),
               std::move(ready17_), std::move(ready18_), std::move(ready19_),
               std::move(ready20_), std::move(ready21_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 4194304) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 4194304;
          Destruct(&pending22_);
          Construct(&ready22_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready14_), std::move(ready15_), std::move(ready16_),
               std::move(ready17_), std::move(ready18_), std::move(ready19_),
               std::move(ready20_), std::move(ready21_), std::move(ready22_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
    ...
    if ((state_ & 8388608) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 8388608;
          Destruct(&pending23_);
          Construct(&ready23_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 16777216) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 16777216;
          Destruct(&pending24_);
          Construct(&ready24_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready17_), std::move(ready18_), std::move(ready19_),
               std::move(ready20_), std::move(ready21_), std::move(ready22_),
               std::move(ready23_), std::move(ready24_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 33554432) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 33554432;
          Destruct(&pending25_);
          Construct(&ready25_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready17_), std::move(ready18_), std::move(ready19_),
               std::move(ready20_), std::move(ready21_), std::move(ready22_),
               std::move(ready23_), std::move(ready24_), std::move(ready25_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
    ...
    if ((state_ & 67108864) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 67108864;
          Destruct(&pending26_);
          Construct(&ready26_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 134217728) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 134217728;
          Destruct(&pending27_);
          Construct(&ready27_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready20_), std::move(ready21_), std::move(ready22_),
               std::move(ready23_), std::move(ready24_), std::move(ready25_),
               std::move(ready26_), std::move(ready27_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 268435456) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 268435456;
          Destruct(&pending28_);
          Construct(&ready28_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready20_), std::move(ready21_), std::move(ready22_),
               std::move(ready23_), std::move(ready24_), std::move(ready25_),
               std::move(ready26_), std::move(ready27_), std::move(ready28_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
...
class TryJoin<F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14,
 ...
 public:
  ...
  using Result = absl::StatusOr<std::tuple<
    ...
    if ((state_ & 536870912) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 536870912;
          Destruct(&pending29_);
          Construct(&ready29_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 1073741824) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 1073741824;
          Destruct(&pending30_);
          Construct(&ready30_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready23_), std::move(ready24_), std::move(ready25_),
               std::move(ready26_), std::move(ready27_), std::move(ready28_),
               std::move(ready29_), std::move(ready30_)));
  }
};
template <typename F0, typename F1, typename F2, typename F3, typename F4,
          typename F5, typename F6, typename F7, typename F8, typename F9,
  ...
  Poll<Result> operator()() {
    ...
    if ((state_ & 2147483648) == 0) {
      ...
      if (auto* p = r.get_ready()) {
        if (p->ok()) {
          state_ |= 2147483648;
          Destruct(&pending31_);
          Construct(&ready31_, IntoResult(p));
        } else {
          return ready(Result(IntoStatus(p)));
        }
      }
    ...
    return ready(
        Result(absl::in_place, std::move(ready0_), std::move(ready1_),
               ...
               std::move(ready23_), std::move(ready24_), std::move(ready25_),
               std::move(ready26_), std::move(ready27_), std::move(ready28_),
               std::move(ready29_), std::move(ready30_), std::move(ready31_)));
  }
};
