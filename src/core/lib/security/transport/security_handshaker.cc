...
static size_t
move_read_buffer_into_handshake_buffer (security_handshaker * h)
{
  ...
  while (h->args->read_buffer->count > 0)
	...
	grpc_slice_buffer_take_first (h->args->read_buffer);
...
static void
security_handshaker_unref (security_handshaker * h)
{
  if (gpr_unref (&h->refs))
    {
      gpr_mu_destroy (&h->mu);
...
// later destruction.
static void
cleanup_args_for_failure_locked (security_handshaker * h)
{
...
static void
security_handshake_failed_locked (security_handshaker * h, grpc_error * error)
{
...
static void
on_peer_checked_inner (security_handshaker * h, grpc_error * error)
{
  ...
  tsi_result result =
    tsi_handshaker_result_create_zero_copy_grpc_protector (h->
							   handshaker_result,
							   nullptr,
							   &zero_copy_protector);
  ...
  if (zero_copy_protector == nullptr)
    {
      result =
	tsi_handshaker_result_create_frame_protector (h->handshaker_result,
						      nullptr, &protector);
      if (result != TSI_OK)
	{
	  error =
  ...
  result =
    tsi_handshaker_result_get_unused_bytes (h->handshaker_result,
					    &unused_bytes,
					    &unused_bytes_size);
  ...
  if (unused_bytes_size > 0)
    {
      grpc_slice slice =
	grpc_slice_from_copied_buffer ((char *) unused_bytes,
				       unused_bytes_size);
      h->args->endpoint =
	grpc_secure_endpoint_create (protector, zero_copy_protector,
				     h->args->endpoint, &slice, 1);
...
static void
on_peer_checked (void *arg, grpc_error * error)
{
...
static grpc_error *
check_peer_locked (security_handshaker * h)
{
  tsi_peer peer;
  tsi_result result =
...
static grpc_error *
on_handshake_next_done_locked (security_handshaker * h, tsi_result result,
  ...
  if (bytes_to_send_size > 0)
    {
      ...
      grpc_slice to_send = grpc_slice_from_copied_buffer ((const char *)
							  bytes_to_send,
							  bytes_to_send_size);
...
static void
on_handshake_next_done_grpc_wrapper (tsi_result result, void *user_data,
				     const unsigned char *bytes_to_send,
				     size_t bytes_to_send_size,
				     ...
				     handshaker_result)
{
  ...
  gpr_mu_lock (&h->mu);
  grpc_error *error =
    on_handshake_next_done_locked (h, result, bytes_to_send,
...
static grpc_error *
do_handshaker_next_locked (security_handshaker * h,
			   ...
			   size_t bytes_received_size)
{
  ...
  tsi_result result =
    tsi_handshaker_next (h->handshaker, bytes_received, bytes_received_size,
			 &bytes_to_send,
			 &bytes_to_send_size, &handshaker_result,
...
static void
on_handshake_data_received_from_peer (void *arg, grpc_error * error)
{
  security_handshaker *h = (security_handshaker *) arg;
  gpr_mu_lock (&h->mu);
  if (error != GRPC_ERROR_NONE || h->shutdown)
    {
      security_handshake_failed_locked (h,
...
static void
on_handshake_data_sent_to_peer (void *arg, grpc_error * error)
{
  security_handshaker *h = (security_handshaker *) arg;
  gpr_mu_lock (&h->mu);
  if (error != GRPC_ERROR_NONE || h->shutdown)
...
static void
security_handshaker_destroy (grpc_handshaker * handshaker)
{
...
static void
security_handshaker_shutdown (grpc_handshaker * handshaker, grpc_error * why)
{
  security_handshaker *h = (security_handshaker *) handshaker;
  gpr_mu_lock (&h->mu);
  if (!h->shutdown)
...
static void
security_handshaker_do_handshake (grpc_handshaker * handshaker,
...
security_handshaker_create (tsi_handshaker * handshaker,
			    grpc_security_connector * connector)
{
...
static void
fail_handshaker_destroy (grpc_handshaker * handshaker)
{
  gpr_free (handshaker);
}

static void
fail_handshaker_shutdown (grpc_handshaker * handshaker, grpc_error * why)
{
  GRPC_ERROR_UNREF (why);
}

static void
fail_handshaker_do_handshake (grpc_handshaker * handshaker,
			      grpc_tcp_server_acceptor * acceptor,
			      grpc_closure * on_handshake_done,
			      grpc_handshaker_args * args)
{
...
static void
client_handshaker_factory_add_handshakers (grpc_handshaker_factory *
					   handshaker_factory,
...
static void
server_handshaker_factory_add_handshakers (grpc_handshaker_factory * hf,
					   const grpc_channel_args * args,
					   grpc_handshake_manager *
					   handshake_mgr)
{
...
static void
handshaker_factory_destroy (grpc_handshaker_factory * handshaker_factory)
{
}
...
grpc_handshaker *
grpc_security_handshaker_create (tsi_handshaker * handshaker,
				 grpc_security_connector * connector)
{
