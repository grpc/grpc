...
static void
destroy (secure_endpoint * secure_ep)
{
...
static void
secure_endpoint_unref (secure_endpoint * ep, const char *reason,
  ...
  if (grpc_trace_secure_endpoint.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "SECENDP unref %p : %s %" PRIdPTR " -> %" PRIdPTR, ep, reason,
	       val, val - 1);
    }
  if (gpr_unref (&ep->ref))
    {
...
static void
secure_endpoint_ref (secure_endpoint * ep, const char *reason,
		     const char *file, int line)
{
  if (grpc_trace_secure_endpoint.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&ep->ref.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "SECENDP   ref %p : %s %" PRIdPTR " -> %" PRIdPTR, ep, reason,
	       val, val + 1);
    }
  gpr_ref (&ep->ref);
}
...
static void
secure_endpoint_unref (secure_endpoint * ep)
{
  if (gpr_unref (&ep->ref))
    {
      destroy (ep);
...
static void
flush_read_staging_buffer (secure_endpoint * ep, uint8_t ** cur,
			   uint8_t ** end)
{
...
static void
call_read_cb (secure_endpoint * ep, grpc_error * error)
{
  if (grpc_trace_secure_endpoint.enabled ())
    {
      size_t i;
  ...
  if (ep->zero_copy_protector != nullptr)
    {
      ...
      result =
	tsi_zero_copy_grpc_protector_unprotect (ep->zero_copy_protector,
						&ep->source_buffer,
						ep->read_buffer);
    }
  else
	  ...
	  while (message_size > 0 || keep_looping)
	    {
	      ...
	      result =
		tsi_frame_protector_unprotect (ep->protector, message_bytes,
	...
	{
	  grpc_slice_buffer_add (ep->read_buffer,
				 grpc_slice_split_head (&ep->
							read_staging_buffer,
							(size_t) (cur -
  ...
  if (result != TSI_OK)
    {
      ...
      call_read_cb (ep,
		    ...
		    (GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Unwrap failed"),
		     result));
      return;
    }
...
static void
endpoint_read (grpc_endpoint * secure_ep, grpc_slice_buffer * slices,
	       grpc_closure * cb)
{
...
static void
flush_write_staging_buffer (secure_endpoint * ep, uint8_t ** cur,
			    uint8_t ** end)
{
...
static void
endpoint_write (grpc_endpoint * secure_ep, grpc_slice_buffer * slices,
		grpc_closure * cb)
{
  ...
  if (grpc_trace_secure_endpoint.enabled ())
	...
	{
	  char *data =
	    grpc_dump_slice (slices->slices[i],
			     GPR_DUMP_HEX | GPR_DUMP_ASCII);
  ...
  if (ep->zero_copy_protector != nullptr)
	  ...
	  while (message_size > 0)
	    {
	      ...
	      gpr_mu_lock (&ep->protector_mu);
	      result =
		tsi_frame_protector_protect (ep->protector, message_bytes,
					     &processed_message_size, cur,
      ...
      if (result == TSI_OK)
	{
	  ...
	  do
	    {
	      ...
	      gpr_mu_lock (&ep->protector_mu);
	      result =
		tsi_frame_protector_protect_flush (ep->protector, cur,
						   &protected_buffer_size_to_send,
						   &still_pending_size);
	      gpr_mu_unlock (&ep->protector_mu);
	      if (result != TSI_OK)
	  ...
	  if (cur != GRPC_SLICE_START_PTR (ep->write_staging_buffer))
	    {
	      grpc_slice_buffer_add (&ep->output_buffer,
				     grpc_slice_split_head (&ep->
...
static void
endpoint_shutdown (grpc_endpoint * secure_ep, grpc_error * why)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
static void
endpoint_destroy (grpc_endpoint * secure_ep)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
static void
endpoint_add_to_pollset (grpc_endpoint * secure_ep, grpc_pollset * pollset)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
static void
endpoint_add_to_pollset_set (grpc_endpoint * secure_ep,
...
static void
endpoint_delete_from_pollset_set (grpc_endpoint * secure_ep,
				  grpc_pollset_set * pollset_set)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
static int
endpoint_get_fd (grpc_endpoint * secure_ep)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
endpoint_get_resource_user (grpc_endpoint * secure_ep)
{
  secure_endpoint *ep = (secure_endpoint *) secure_ep;
...
grpc_secure_endpoint_create (struct tsi_frame_protector *protector,
			     ...
			     *zero_copy_protector, grpc_endpoint * transport,
			     grpc_slice * leftover_slices,
			     size_t leftover_nslices)
{
  size_t i;
  secure_endpoint *ep =
    (secure_endpoint *) gpr_malloc (sizeof (secure_endpoint));
