...
static int connector_cmp(void* a, void* b) {
  return grpc_security_connector_cmp(
      reinterpret_cast<grpc_security_connector*>(a),
      reinterpret_cast<grpc_security_connector*>(b));
}
...
grpc_security_connector* grpc_security_connector_from_arg(const grpc_arg* arg) {
  ...
  return reinterpret_cast<grpc_security_connector*>(arg->value.pointer.p);
}
...
grpc_channel_security_connector* grpc_fake_channel_security_connector_create(
    ...
    const grpc_channel_args* args) {
  grpc_fake_channel_security_connector* c =
      reinterpret_cast<grpc_fake_channel_security_connector*>(
          gpr_zalloc(sizeof(*c)));
...
static const char** fill_alpn_protocol_strings(size_t* num_alpn_protocols) {
  ...
  *num_alpn_protocols = grpc_chttp2_num_alpn_versions();
  const char** alpn_protocol_strings = reinterpret_cast<const char**>(
...
static bool try_replace_server_handshaker_factory(
  ...
  gpr_free(cert_pairs);
  gpr_free(reinterpret_cast<void*>(alpn_protocol_strings));
...
tsi_peer tsi_shallow_peer_from_ssl_auth_context(
  ...
  if (max_num_props > 0) {
    peer.properties = reinterpret_cast<tsi_peer_property*>(
...
grpc_security_status grpc_ssl_channel_security_connector_create(
  ...
  c = reinterpret_cast<grpc_ssl_channel_security_connector*>(
...
grpc_ssl_server_security_connector_initialize(
    grpc_server_credentials* server_creds) {
  grpc_ssl_server_security_connector* c =
      reinterpret_cast<grpc_ssl_server_security_connector*>(
