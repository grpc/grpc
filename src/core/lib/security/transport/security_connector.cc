...
void
grpc_set_ssl_roots_override_callback (grpc_ssl_roots_override_callback cb)
{
...
void
grpc_security_connector_check_peer (grpc_security_connector * sc,
				    tsi_peer peer,
...
int
grpc_security_connector_cmp (grpc_security_connector * sc,
			     grpc_security_connector * other)
{
  if (sc == nullptr || other == nullptr)
    return GPR_ICMP (sc, other);
...
int
grpc_channel_security_connector_cmp (grpc_channel_security_connector * sc1,
				     grpc_channel_security_connector * sc2)
{
  ...
  if (c != 0)
    return c;
  return GPR_ICMP ((void *) sc1->add_handshakers,
		   (void *) sc2->add_handshakers);
}
...
grpc_security_connector_ref (grpc_security_connector * sc, const char *file,
  ...
  if (grpc_trace_security_connector_refcount.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "SECURITY_CONNECTOR:%p   ref %" PRIdPTR " -> %" PRIdPTR " %s",
	       sc, val, val + 1, reason);
    }
#else
grpc_security_connector *
  ...
  if (grpc_trace_security_connector_refcount.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&sc->refcount.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "SECURITY_CONNECTOR:%p unref %" PRIdPTR " -> %" PRIdPTR " %s",
	       sc, val, val - 1, reason);
    }
#else
void
...
grpc_security_connector_to_arg (grpc_security_connector * sc)
{
  return grpc_channel_arg_pointer_create ((char *)
					  GRPC_ARG_SECURITY_CONNECTOR, sc,
...
{
  if (strcmp (arg->key, GRPC_ARG_SECURITY_CONNECTOR))
    return nullptr;
...
grpc_security_connector *
grpc_security_connector_find_in_args (const grpc_channel_args * args)
{
  size_t i;
  if (args == nullptr)
    return nullptr;
...
static bool
fake_check_target (const char *target_type, const char *target,
		   const char *set_str)
{
  ...
  for (size_t i = 0; i < set_size; ++i)
    {
      if (set[i] != nullptr && strcmp (target, set[i]) == 0)
	found = true;
    }
  for (size_t i = 0; i < set_size; ++i)
    {
...
static void
fake_secure_name_check (const char *target,
			const char *expected_targets, bool is_lb_channel)
{
  ...
  if (is_lb_channel)
	...
	{
	  gpr_log (GPR_ERROR,
		   "Backend target '%s' not found in expected set '%s'",
...
static void
fake_check_peer (grpc_security_connector * sc, tsi_peer peer,
		 grpc_auth_context ** auth_context,
		 grpc_closure * on_peer_checked)
{
  ...
  *auth_context = grpc_auth_context_create (nullptr);
  grpc_auth_context_add_cstring_property (*auth_context,
...
static void
fake_channel_check_peer (grpc_security_connector * sc, tsi_peer peer,
			 grpc_auth_context ** auth_context,
			 grpc_closure * on_peer_checked)
{
...
static void
fake_server_check_peer (grpc_security_connector * sc, tsi_peer peer,
			grpc_auth_context ** auth_context,
			grpc_closure * on_peer_checked)
{
  fake_check_peer (sc, peer, auth_context, on_peer_checked);
...
static int
fake_channel_cmp (grpc_security_connector * sc1,
...
static int
fake_server_cmp (grpc_security_connector * sc1, grpc_security_connector * sc2)
{
  return grpc_server_security_connector_cmp ((grpc_server_security_connector
					      *) sc1,
...
static bool
fake_channel_check_call_host (grpc_channel_security_connector * sc,
			      const char *host,
			      grpc_auth_context * auth_context,
...
static void
fake_channel_cancel_check_call_host (grpc_channel_security_connector * sc,
				     grpc_closure * on_call_host_checked,
				     grpc_error * error)
{
  GRPC_ERROR_UNREF (error);
...
static void
fake_channel_add_handshakers (grpc_channel_security_connector * sc,
			      grpc_handshake_manager * handshake_mgr)
{
  grpc_handshake_manager_add (handshake_mgr,
			      grpc_security_handshaker_create
			      (tsi_create_fake_handshaker
			       (true /* is_client */ ), &sc->base));
}

static void
fake_server_add_handshakers (grpc_server_security_connector * sc,
			     grpc_handshake_manager * handshake_mgr)
{
  grpc_handshake_manager_add (handshake_mgr,
			      grpc_security_handshaker_create
			      (tsi_create_fake_handshaker
			       (false /* is_client */ ), &sc->base));
}
...
grpc_fake_channel_security_connector_create (grpc_channel_credentials *
					     channel_creds,
					     ...
					     const grpc_channel_args * args)
{
  ...
  const char *expected_targets =
    grpc_fake_transport_get_expected_targets (args);
...
grpc_server_security_connector *
grpc_fake_server_security_connector_create (grpc_server_credentials *
					    server_creds)
{
...
static bool
server_connector_has_cert_config_fetcher (grpc_ssl_server_security_connector *
					  c)
{
...
static void
ssl_channel_add_handshakers (grpc_channel_security_connector * sc,
			     grpc_handshake_manager * handshake_mgr)
{
  grpc_ssl_channel_security_connector *c =
    (grpc_ssl_channel_security_connector *) sc;
  ...
  tsi_result result =
    tsi_ssl_client_handshaker_factory_create_handshaker (c->
  ...
  grpc_handshake_manager_add (handshake_mgr,
			      grpc_security_handshaker_create
			      (tsi_create_adapter_handshaker (tsi_hs),
			       &sc->base));
}

static const char **
fill_alpn_protocol_strings (size_t * num_alpn_protocols)
{
  GPR_ASSERT (num_alpn_protocols != nullptr);
  *num_alpn_protocols = grpc_chttp2_num_alpn_versions ();
  const char **alpn_protocol_strings =
    (const char **) gpr_malloc (sizeof (const char *) *
				(*num_alpn_protocols));
 ...
 * factory created). */
static bool
try_replace_server_handshaker_factory (grpc_ssl_server_security_connector *
				       sc,
				       const
				       ...
				       config)
{
  ...
  tsi_result result =
    tsi_create_ssl_server_handshaker_factory_ex (cert_pairs,
						 config->num_key_cert_pairs,
...
static bool
try_fetch_ssl_server_credentials (grpc_ssl_server_security_connector * sc)
{
  grpc_ssl_server_certificate_config *certificate_config = nullptr;
  bool status;

  GPR_ASSERT (sc != nullptr);
  if (!server_connector_has_cert_config_fetcher (sc))
    return false;

  grpc_ssl_server_credentials *server_creds =
    (grpc_ssl_server_credentials *) sc->base.server_creds;
  grpc_ssl_certificate_config_reload_status cb_result =
    server_creds->certificate_config_fetcher.cb (server_creds->
...
static void
ssl_server_add_handshakers (grpc_server_security_connector * sc,
			    grpc_handshake_manager * handshake_mgr)
{
  grpc_ssl_server_security_connector *c =
    (grpc_ssl_server_security_connector *) sc;
  ...
  tsi_handshaker *tsi_hs = nullptr;
  tsi_result result =
    tsi_ssl_server_handshaker_factory_create_handshaker (c->
...
tsi_ssl_peer_to_auth_context (const tsi_peer * peer)
{
  ...
  grpc_auth_context_add_cstring_property (ctx,
					  GRPC_TRANSPORT_SECURITY_TYPE_PROPERTY_NAME,
  ...
  for (i = 0; i < peer->property_count; i++)
	...
	continue;
      if (strcmp (prop->name, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY) ==
	  0)
	{
...
static grpc_error *
ssl_check_peer (grpc_security_connector * sc,
...
static void
ssl_channel_check_peer (grpc_security_connector * sc, tsi_peer peer,
			grpc_auth_context ** auth_context,
			grpc_closure * on_peer_checked)
{
  grpc_ssl_channel_security_connector *c =
...
static void
ssl_server_check_peer (grpc_security_connector * sc, tsi_peer peer,
		       grpc_auth_context ** auth_context,
		       grpc_closure * on_peer_checked)
{
...
static int
ssl_channel_cmp (grpc_security_connector * sc1, grpc_security_connector * sc2)
{
...
static int
ssl_server_cmp (grpc_security_connector * sc1, grpc_security_connector * sc2)
{
  return grpc_server_security_connector_cmp ((grpc_server_security_connector
					      *) sc1,
					     (grpc_server_security_connector
...
static void
add_shallow_auth_property_to_peer (tsi_peer * peer,
...
tsi_peer
tsi_shallow_peer_from_ssl_auth_context (const grpc_auth_context *
					auth_context)
{
...
void
tsi_shallow_peer_destruct (tsi_peer * peer)
{
  if (peer->properties != nullptr)
    gpr_free (peer->properties);
}

static bool
ssl_channel_check_call_host (grpc_channel_security_connector * sc,
			     const char *host,
			     grpc_auth_context * auth_context,
			     grpc_closure * on_call_host_checked,
			     grpc_error ** error)
{
  ...
  if (ssl_host_matches_name (&peer, host))
...
static void
ssl_channel_cancel_check_call_host (grpc_channel_security_connector * sc,
				    grpc_closure * on_call_host_checked,
				    grpc_error * error)
{
  GRPC_ERROR_UNREF (error);
...
static grpc_slice
compute_default_pem_root_certs_once (void)
{
  ...
  if (default_root_certs_path != nullptr)
    {
      GRPC_LOG_IF_ERROR ("load_file",
			 grpc_load_file (default_root_certs_path, 1,
					 &result));
  ...
  if (GRPC_SLICE_IS_EMPTY (result) && ssl_roots_override_cb != nullptr)
      ...
      if (ovrd_res == GRPC_SSL_ROOTS_OVERRIDE_OK)
	{
	  ...
	  result = grpc_slice_from_copied_buffer (pem_root_certs, strlen (pem_root_certs) + 1);	// NULL terminator.
	}
...
grpc_slice
grpc_get_default_ssl_roots_for_testing (void)
{
  return compute_default_pem_root_certs_once ();
}

const char *
grpc_get_default_ssl_roots (void)
{
...
grpc_ssl_channel_security_connector_create (grpc_channel_credentials *
					    channel_creds,
					    grpc_call_credentials *
					    request_metadata_creds,
					    ...
					    sc)
{
  ...
  result =
    tsi_create_ssl_client_handshaker_factory (has_key_cert_pair ? config->
...
grpc_ssl_server_security_connector_initialize (grpc_server_credentials *
					       server_creds)
{
  grpc_ssl_server_security_connector *c =
    (grpc_ssl_server_security_connector *)
...
grpc_security_status
grpc_ssl_server_security_connector_create (grpc_server_credentials * gsc,
					   grpc_server_security_connector **
					   sc)
{
  tsi_result result = TSI_OK;
  ...
  if (server_connector_has_cert_config_fetcher (c))
	...
	fill_alpn_protocol_strings (&num_alpn_protocols);
      result =
	tsi_create_ssl_server_handshaker_factory_ex (server_credentials->
						     config.
						     pem_key_cert_pairs,
						     server_credentials->
						     config.
