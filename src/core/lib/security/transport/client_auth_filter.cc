...
void
grpc_auth_metadata_context_reset (grpc_auth_metadata_context *
				  auth_md_context)
{
  ...
  GRPC_AUTH_CONTEXT_UNREF ((grpc_auth_context *) auth_md_context->
...
static void
add_error (grpc_error ** combined, grpc_error * error)
{
...
static void
on_credentials_metadata (void *arg, grpc_error * input_error)
{
...
void
grpc_auth_metadata_context_build (const char *url_scheme,
...
static void
cancel_get_request_metadata (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  if (error != GRPC_ERROR_NONE)
    {
      grpc_call_credentials_cancel_get_request_metadata (calld->creds,
							 &calld->md_array,
							 GRPC_ERROR_REF
							 (error));
    }
			...
			grpc_transport_stream_op_batch * batch)
{
  ...
  grpc_client_security_context *ctx =
    ...
    context[GRPC_CONTEXT_SECURITY].value;
  ...
  if (channel_call_creds != nullptr && call_creds_has_md)
    ...
    {
      calld->creds =
	grpc_call_credentials_ref (call_creds_has_md ? ctx->
				   creds : channel_call_creds);
    }

  grpc_auth_metadata_context_build (chand->security_connector->base.
				    url_scheme, calld->host, calld->method,
				    chand->auth_context,
  ...
  if (grpc_call_credentials_get_request_metadata
      (calld->creds, calld->pollent, calld->auth_md_context, &calld->md_array,
  ...
  else
    {
      ...
      grpc_call_combiner_set_notify_on_cancel (calld->call_combiner,
					       GRPC_CLOSURE_INIT (&calld->
								  get_request_metadata_cancel_closure,
...
static void
on_host_checked (void *arg, grpc_error * error)
{
  grpc_transport_stream_op_batch *batch =
    (grpc_transport_stream_op_batch *) arg;
  grpc_call_element *elem =
...
static void
cancel_check_call_host (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  channel_data *chand = (channel_data *) elem->channel_data;
  if (error != GRPC_ERROR_NONE)
    {
      grpc_channel_security_connector_cancel_check_call_host (chand->
							      security_connector,
							      &calld->
							      async_result_closure,
							      GRPC_ERROR_REF
							      (error));
    }
				      ...
				      grpc_transport_stream_op_batch * batch)
{
  ...
  if (batch->send_initial_metadata)
    {
      for (grpc_linked_mdelem * l =
	   batch->payload->send_initial_metadata.send_initial_metadata->list.
	   head; l != nullptr; l = l->next)
	{
	  grpc_mdelem md = l->md;
	  ...
	  if (grpc_channel_security_connector_check_call_host
	  ...
	  else
	    {
	      ...
	      grpc_call_combiner_set_notify_on_cancel (calld->call_combiner,
						       GRPC_CLOSURE_INIT
						       (&calld->
							check_call_host_cancel_closure,
							cancel_check_call_host,
							elem,
							grpc_schedule_on_exec_ctx));
	    }
