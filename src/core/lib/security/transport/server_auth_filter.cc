...
metadata_batch_to_md_array (const grpc_metadata_batch * batch)
{
  ...
  for (l = batch->list.head; l != nullptr; l = l->next)
      ...
      if (result.count == result.capacity)
	{
	  ...
	  result.metadata =
	    (grpc_metadata *) gpr_realloc (result.metadata,
					   result.capacity *
...
static void
on_md_processing_done_inner (grpc_call_element * elem,
			     const grpc_metadata * consumed_md,
			     size_t num_consumed_md,
			     const grpc_metadata * response_md,
			     size_t num_response_md, grpc_error * error)
{
  call_data *calld = (call_data *) elem->call_data;
  ...
  if (error == GRPC_ERROR_NONE)
    {
      ...
      error =
	grpc_metadata_batch_filter (batch->payload->recv_initial_metadata.
				    recv_initial_metadata, remove_consumed_md,
				    elem,
...
static void
cancel_call (void *arg, grpc_error * error)
{
...
recv_initial_metadata_ready (void *arg, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
	  ...
	  && chand->creds->processor.process != nullptr)
	{
	  ...
	  calld->md =
	    metadata_batch_to_md_array (batch->payload->recv_initial_metadata.
...
static void
auth_start_transport_stream_op_batch (grpc_call_element * elem,
...
init_call_elem (grpc_call_element * elem, const grpc_call_element_args * args)
{
  ...
  if (args->context[GRPC_CONTEXT_SECURITY].value != nullptr)
    {
      args->context[GRPC_CONTEXT_SECURITY].destroy (args->
						    context
