...
TlsChannelSecurityConnector::TlsChannelSecurityConnector(
  ...
  // Create a watcher.
  auto watcher_ptr =
      absl::make_unique<TlsChannelCertificateWatcher>(this);
  ...
  // We will use the root certs stored in system default locations if
  // not watching root certs on the client side. We will handle this
  // case differently here, because "watching a default roots without
  // the identity certs" is a valid case(and hence we will need to call
  // OnCertificatesChanged), but it requires nothing from the provider,
  // and hence no need to register the watcher.
...
void TlsChannelSecurityConnector::check_peer(
  ...
  *auth_context = grpc_ssl_peer_to_auth_context(
      &peer, GRPC_TLS_TRANSPORT_SECURITY_TYPE);
  ...
  if (config != nullptr) {
    ...
    if (p == nullptr) {
      ...
      char* peer_pem =
          static_cast<char*>(gpr_zalloc(p->value.length + 1));
      ...
      // TODO(zhenlian) - This should be cleaned up as part of the
      // custom verification changes. Fill in the subject alternative
      // names
      ...
      if (check_arg_->subject_alternative_names != nullptr) {
        for (size_t i = 0;
             i < check_arg_->subject_alternative_names_size; ++i) {
...
int TlsChannelSecurityConnector::cmp(
  ...
  auto* other =
      reinterpret_cast<const TlsChannelSecurityConnector*>(other_sc);
...
void TlsChannelSecurityConnector::TlsChannelCertificateWatcher::
  ...
  if (key_cert_pairs.has_value()) {
    security_connector_->pem_key_cert_pair_list_ =
        std::move(key_cert_pairs);
  }
  const bool root_ready =
      !security_connector_->options_->watch_root_cert() ||
...
// TODO(ZhenLian): implement the logic to signal waiting handshakers
...
TlsChannelSecurityConnector::UpdateHandshakerFactoryLocked() {
  ...
  if (pem_root_certs_.has_value()) {
    // TODO(ZhenLian): update the underlying TSI layer to use C++ types
    // like std::string and absl::string_view to avoid making another
    // copy here.
  ...
  grpc_security_status status =
      grpc_ssl_tsi_client_handshaker_factory_init(
          pem_key_cert_pair,
          pem_root_certs.empty() || use_default_roots
              ? nullptr
              : pem_root_certs.c_str(),
          skip_server_certificate_verification,
          grpc_get_tsi_tls_version(options_->min_tls_version()),
          grpc_get_tsi_tls_version(options_->max_tls_version()),
          ssl_session_cache_, &client_handshaker_factory_);
  /* Free memory. */
...
TlsChannelSecurityConnector::ProcessServerAuthorizationCheckResult(
  ...
  if (arg->status == GRPC_STATUS_CANCELLED) {
    error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
        absl::StrCat(
            "Server authorization check is cancelled by the caller "
            "with error: ",
            arg->error_details->error_details())
            .c_str());
  } else if (arg->status == GRPC_STATUS_OK) {
    /* Server authorization check completed successfully but returned
     * check failure. */
    if (!arg->success) {
      error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
    ...
    error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
        absl::StrCat("Server authorization check did not finish "
                     "correctly with error: ",
                     arg->error_details->error_details())
            .c_str());
  }
  return error;
...
TlsServerSecurityConnector::TlsServerSecurityConnector(
  ...
  // Create a watcher.
  auto watcher_ptr =
      absl::make_unique<TlsServerCertificateWatcher>(this);
  certificate_watcher_ = watcher_ptr.get();
  // Register the watcher with the distributor.
  grpc_tls_certificate_distributor* distributor =
      options_->certificate_distributor();
...
void TlsServerSecurityConnector::TlsServerCertificateWatcher::
  ...
  if ((root_being_watched && root_has_value && identity_being_watched &&
       identity_has_value) ||
      (root_being_watched && root_has_value &&
