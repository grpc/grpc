...
TlsChannelSecurityConnector::TlsChannelSecurityConnector(
  ...
  gpr_log(GPR_ERROR, "TlsChannelSecurityConnector::TlsChannelSecurityConnector is called");
...
void TlsChannelSecurityConnector::check_peer(
  ...
  *auth_context =
  ...
  gpr_log(GPR_ERROR, "before calling pending_request->Start();");
...
TlsChannelSecurityConnector::ChannelPendingVerifierRequest::
  ...
  gpr_log(GPR_ERROR, "TlsChannelSecurityConnector::ChannelPendingVerifierRequest::~ChannelPendingVerifierRequest() is called");
}

void TlsChannelSecurityConnector::ChannelPendingVerifierRequest::Start() {
  gpr_log(GPR_ERROR, "inside TlsChannelSecurityConnector::ChannelPendingVerifierRequest::Start()");
  absl::Status sync_status;
  grpc_tls_certificate_verifier* verifier =
      security_connector_->options_->certificate_verifier();
  gpr_log(GPR_ERROR, "before calling verifier->Verify");
  bool is_done = verifier->Verify(
      &request_,
      absl::bind_front(&ChannelPendingVerifierRequest::OnVerifyDone, this,
                       true),
      &sync_status);
  gpr_log(GPR_ERROR, "before deciding is_done");
  if (is_done) {
    OnVerifyDone(false, sync_status);
  }
  gpr_log(GPR_ERROR, "out  TlsChannelSecurityConnector::ChannelPendingVerifierRequest::Start()");
}

void TlsChannelSecurityConnector::ChannelPendingVerifierRequest::OnVerifyDone(
    bool run_callback_inline, absl::Status status) {
  gpr_log(GPR_ERROR, "inside TlsChannelSecurityConnector::ChannelPendingVerifierRequest::OnVerifyDone");
  {
  ...
  if (run_callback_inline) {
    ...
    gpr_log(GPR_ERROR, "calling ExecCtx::Run(DEBUG_LOCATION, on_peer_checked_, error)");
    ExecCtx::Run(DEBUG_LOCATION, on_peer_checked_, error);
  }
  delete this;
  gpr_log(GPR_ERROR, "out TlsChannelSecurityConnector::ChannelPendingVerifierRequest::OnVerifyDone");
}
...
void TlsServerSecurityConnector::ServerPendingVerifierRequest::Start() {
  gpr_log(GPR_ERROR, "inside TlsServerSecurityConnector::ServerPendingVerifierRequest::Start");
  absl::Status sync_status;
  grpc_tls_certificate_verifier* verifier =
      security_connector_->options_->certificate_verifier();
  gpr_log(GPR_ERROR, "1");
