...
namespace {
grpc_error* ssl_check_peer(const char* peer_name, const tsi_peer* peer,
 ...
 public:
  ...
  int cmp(const grpc_security_connector* other) const override {
    return grpc_server_security_connector_cmp(
        this, reinterpret_cast<const grpc_server_security_connector*>(other));
  }
 ...
 private:
  ...
  bool try_fetch_ssl_server_credentials() {
    ...
    grpc_ssl_server_credentials* server_creds =
        reinterpret_cast<grpc_ssl_server_credentials*>(this->server_creds());
...
grpc_security_status grpc_ssl_channel_security_connector_create(
  ...
  *sc = nullptr;
  ...
  grpc_ssl_channel_security_connector* c =
      grpc_core::New<grpc_ssl_channel_security_connector>(
...
grpc_security_status grpc_ssl_server_security_connector_create(
  ...
  grpc_ssl_server_security_connector* c =
      grpc_core::New<grpc_ssl_server_security_connector>(server_credentials);
  const grpc_security_status retval = c->InitializeHandshakerFactory();
  if (retval == GRPC_SECURITY_OK) {
    *sc = c;
  } else {
    if (c != nullptr) c->Unref();
