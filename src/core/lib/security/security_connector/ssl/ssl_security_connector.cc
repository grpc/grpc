...
namespace {
grpc_error_handle ssl_check_peer(const char* peer_name, const tsi_peer* peer,
  ...
  *auth_context = grpc_ssl_peer_to_auth_context(peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
...
class grpc_ssl_channel_security_connector final : public grpc_channel_security_connector {
 public:
  ...
  grpc_security_status InitializeHandshakerFactory(const grpc_ssl_config* config,
    ...
    const tsi_result result = tsi_create_ssl_client_handshaker_factory_with_options(
  ...
  void check_peer(tsi_peer peer, grpc_endpoint* /*ep*/,
    ...
    if (error == GRPC_ERROR_NONE && verify_options_->verify_peer_callback != nullptr) {
      ...
      if (p == nullptr) {
        error =
            GRPC_ERROR_CREATE_FROM_STATIC_STRING("Cannot check peer: missing pem cert property.");
      } else {
        char* peer_pem = static_cast<char*>(gpr_malloc(p->value.length + 1));
        memcpy(peer_pem, p->value.data, p->value.length);
        peer_pem[p->value.length] = '\0';
        int callback_status = verify_options_->verify_peer_callback(
            target_name, peer_pem, verify_options_->verify_peer_callback_userdata);
        gpr_free(peer_pem);
        if (callback_status) {
          error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
              absl::StrFormat("Verify peer callback returned a failure (%d)", callback_status)
                  .c_str());
        }
      }
    }
  ...
  int cmp(const grpc_security_connector* other_sc) const override {
    auto* other = reinterpret_cast<const grpc_ssl_channel_security_connector*>(other_sc);
...
class grpc_ssl_server_security_connector : public grpc_server_security_connector {
 public:
  ...
  grpc_security_status InitializeHandshakerFactory() {
    if (has_cert_config_fetcher()) {
      ...
      auto* server_credentials = static_cast<const grpc_ssl_server_credentials*>(server_creds());
      ...
      options.client_certificate_request = grpc_get_tsi_client_certificate_request_type(
          server_credentials->config().client_certificate_request);
      options.cipher_suites = grpc_get_ssl_cipher_suites();
      options.alpn_protocols = alpn_protocol_strings;
      options.num_alpn_protocols = static_cast<uint16_t>(num_alpn_protocols);
      options.min_tls_version =
          grpc_get_tsi_tls_version(server_credentials->config().min_tls_version);
      options.max_tls_version =
          grpc_get_tsi_tls_version(server_credentials->config().max_tls_version);
      const tsi_result result = tsi_create_ssl_server_handshaker_factory_with_options(
 ...
 private:
  ...
  bool try_replace_server_handshaker_factory(const grpc_ssl_server_certificate_config* config) {
    if (config == nullptr) {
      gpr_log(GPR_ERROR,
    ...
    options.num_alpn_protocols = static_cast<uint16_t>(num_alpn_protocols);
    tsi_result result =
        tsi_create_ssl_server_handshaker_factory_with_options(&options, &new_handshaker_factory);
  ...
  void set_server_handshaker_factory(tsi_ssl_server_handshaker_factory* new_factory) {
...
grpc_ssl_channel_security_connector_create(
  ...
  grpc_core::RefCountedPtr<grpc_ssl_channel_security_connector> c =
      grpc_core::MakeRefCounted<grpc_ssl_channel_security_connector>(
          std::move(channel_creds), std::move(request_metadata_creds), config, target_name,
          overridden_target_name);
  const grpc_security_status result =
      c->InitializeHandshakerFactory(config, pem_root_certs, root_store, ssl_session_cache);
...
grpc_core::RefCountedPtr<grpc_server_security_connector> grpc_ssl_server_security_connector_create(
  ...
  grpc_core::RefCountedPtr<grpc_ssl_server_security_connector> c =
      grpc_core::MakeRefCounted<grpc_ssl_server_security_connector>(std::move(server_credentials));
