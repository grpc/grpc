...
const char* linux_cert_files[] = {
    "/etc/ssl/certs/ca-certificates.crt", "/etc/pki/tls/certs/ca-bundle.crt",
    "/etc/ssl/ca-bundle.pem", "/etc/pki/tls/cacert.pem",
    "/etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem"};
const char* linux_cert_directories[] = {
...
grpc_slice GetSystemRootCerts() {
  ...
  size_t num_cert_files_ = GPR_ARRAY_SIZE(linux_cert_files);
  for (size_t i = 0; i < num_cert_files_; i++) {
    grpc_error* error =
        grpc_load_file(linux_cert_files[i], 1, &valid_bundle_slice);
...
grpc_slice LoadSystemRootCerts() {
  ...
  if (GRPC_SLICE_IS_EMPTY(result)) {
    for (size_t i = 0;
         i < GPR_ARRAY_SIZE(linux_cert_directories); i++) {
      result = CreateRootCertsBundle(
          linux_cert_directories[i]);
      if (!GRPC_SLICE_IS_EMPTY(result)) {
        break;
      }
    }
