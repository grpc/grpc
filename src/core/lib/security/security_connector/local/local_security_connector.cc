...
grpc_core::RefCountedPtr<grpc_auth_context> local_auth_context_create(const tsi_peer* peer) {
  /* Create auth context. */
...
void local_check_peer(tsi_peer peer, grpc_endpoint* ep,
                      ...
                      grpc_closure* on_peer_checked, grpc_local_connect_type type) {
  ...
  if (!uri.ok() || !grpc_parse_uri(*uri, &resolved_addr)) {
    ...
    grpc_resolved_address* addr = grpc_sockaddr_is_v4mapped(&resolved_addr, &addr_normalized)
                                      ? &addr_normalized
                                      : &resolved_addr;
    ...
    if (type == UDS && grpc_is_unix_socket(addr)) {
      ...
      const grpc_sockaddr_in* addr4 = reinterpret_cast<const grpc_sockaddr_in*>(sock_addr);
      if (grpc_htonl(addr4->sin_addr.s_addr) == INADDR_LOOPBACK) {
        is_endpoint_local = true;
      }
      // IPv6
    } else if (type == LOCAL_TCP && sock_addr->sa_family == GRPC_AF_INET6) {
      const grpc_sockaddr_in6* addr6 = reinterpret_cast<const grpc_sockaddr_in6*>(addr);
      if (memcmp(&addr6->sin6_addr, &in6addr_loopback, sizeof(in6addr_loopback)) == 0) {
        is_endpoint_local = true;
      }
    }
  }
  ...
  if (!is_endpoint_local) {
    error =
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("Endpoint is neither UDS or TCP loopback address.");
  ...
  const char* security_level = tsi_security_level_to_string(TSI_PRIVACY_AND_INTEGRITY);
  tsi_result result = tsi_construct_string_peer_property_from_cstring(
      TSI_SECURITY_LEVEL_PEER_PROPERTY, security_level, &peer.properties[peer.property_count]);
  ...
  *auth_context = local_auth_context_create(&peer);
  ...
  error = *auth_context != nullptr
              ? GRPC_ERROR_NONE
              : GRPC_ERROR_CREATE_FROM_STATIC_STRING("Could not create local auth context");
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, on_peer_checked, error);
}

class grpc_local_channel_security_connector final : public grpc_channel_security_connector {
 public:
  ...
  void add_handshakers(const grpc_channel_args* args, grpc_pollset_set* /*interested_parties*/,
    ...
    GPR_ASSERT(tsi_local_handshaker_create(true /* is_client */, &handshaker) == TSI_OK);
    handshake_manager->Add(grpc_core::SecurityHandshakerCreate(handshaker, this, args));
  }

  int cmp(const grpc_security_connector* other_sc) const override {
    auto* other = reinterpret_cast<const grpc_local_channel_security_connector*>(other_sc);
  ...
  void check_peer(tsi_peer peer, grpc_endpoint* ep,
    ...
    local_check_peer(peer, ep, auth_context, on_peer_checked, creds->connect_type());
  }
  ...
  bool check_call_host(absl::string_view host, grpc_auth_context* /*auth_context*/,
    ...
    if (host.empty() || host != target_name_) {
      *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("local call host does not match target name");
    }
    return true;
  }
...
class grpc_local_server_security_connector final : public grpc_server_security_connector {
 public:
  ...
  void add_handshakers(const grpc_channel_args* args, grpc_pollset_set* /*interested_parties*/,
                       grpc_core::HandshakeManager* handshake_manager) override {
    tsi_handshaker* handshaker = nullptr;
    GPR_ASSERT(tsi_local_handshaker_create(false /* is_client */, &handshaker) == TSI_OK);
    handshake_manager->Add(grpc_core::SecurityHandshakerCreate(handshaker, this, args));
  }
...
grpc_local_channel_security_connector_create(
  ...
  if (channel_creds == nullptr || target_name == nullptr) {
    gpr_log(GPR_ERROR, "Invalid arguments to grpc_local_channel_security_connector_create()");
    return nullptr;
  }
  // Perform sanity check on UDS address. For TCP local connection, the check
  // will be done during check_peer procedure.
  grpc_local_credentials* creds = static_cast<grpc_local_credentials*>(channel_creds.get());
  const grpc_arg* server_uri_arg = grpc_channel_args_find(args, GRPC_ARG_SERVER_URI);
  const char* server_uri_str = grpc_channel_arg_get_string(server_uri_arg);
  if (creds->connect_type() == UDS &&
      strncmp(GRPC_UDS_URI_PATTERN, server_uri_str, strlen(GRPC_UDS_URI_PATTERN)) != 0) {
    gpr_log(GPR_ERROR,
...
grpc_local_server_security_connector_create(
  ...
  if (server_creds == nullptr) {
    gpr_log(GPR_ERROR, "Invalid arguments to grpc_local_server_security_connector_create()");
    return nullptr;
  }
  return grpc_core::MakeRefCounted<grpc_local_server_security_connector>(std::move(server_creds));
}
