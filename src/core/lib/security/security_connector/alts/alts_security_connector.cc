 ...
 public:
  grpc_alts_channel_security_connector(
      ...
      : grpc_channel_security_connector(
            GRPC_ALTS_URL_SCHEME, std::move(channel_creds),
  ...
  int cmp(const grpc_security_connector* other_sc) const override {
    auto* other =
        reinterpret_cast<const grpc_alts_channel_security_connector*>(
            other_sc);
 ...
 public:
  ...
  void add_handshakers(
    ...
    const grpc_alts_server_credentials* creds =
        static_cast<const grpc_alts_server_credentials*>(
            server_creds());
    size_t user_specified_max_frame_size = 0;
    const grpc_arg* arg =
        grpc_channel_args_find(args, GRPC_ARG_TSI_MAX_FRAME_SIZE);
...
grpc_alts_auth_context_from_tsi_peer(const tsi_peer* peer) {
  ...
  if (!check_result) {
    gpr_log(GPR_ERROR,
            "Mismatch of local and peer rpc protocol versions.");
    return nullptr;
  }
  ...
  for (i = 0; i < peer->property_count; i++) {
    ...
    if (strcmp(tsi_prop->name,
               TSI_ALTS_SERVICE_ACCOUNT_PEER_PROPERTY) == 0) {
