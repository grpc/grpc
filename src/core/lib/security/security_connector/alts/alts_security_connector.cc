...
void alts_check_peer(tsi_peer peer, grpc_core::RefCountedPtr<grpc_auth_context>* auth_context,
                     grpc_closure* on_peer_checked) {
  *auth_context = grpc_core::internal::grpc_alts_auth_context_from_tsi_peer(&peer);
  tsi_peer_destruct(&peer);
  grpc_error_handle error =
      *auth_context != nullptr
          ? GRPC_ERROR_NONE
          : GRPC_ERROR_CREATE_FROM_STATIC_STRING("Could not get ALTS auth context from TSI peer");
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, on_peer_checked, error);
}

class grpc_alts_channel_security_connector final : public grpc_channel_security_connector {
 public:
  grpc_alts_channel_security_connector(
      grpc_core::RefCountedPtr<grpc_channel_credentials> channel_creds,
      grpc_core::RefCountedPtr<grpc_call_credentials> request_metadata_creds,
      const char* target_name)
      : grpc_channel_security_connector(GRPC_ALTS_URL_SCHEME, std::move(channel_creds),
  ...
  void add_handshakers(const grpc_channel_args* args, grpc_pollset_set* interested_parties,
    ...
    const grpc_alts_credentials* creds = static_cast<const grpc_alts_credentials*>(channel_creds());
    size_t user_specified_max_frame_size = 0;
    const grpc_arg* arg = grpc_channel_args_find(args, GRPC_ARG_TSI_MAX_FRAME_SIZE);
    if (arg != nullptr && arg->type == GRPC_ARG_INTEGER) {
      user_specified_max_frame_size =
          grpc_channel_arg_get_integer(arg, {0, 0, std::numeric_limits<int>::max()});
    }
  ...
  int cmp(const grpc_security_connector* other_sc) const override {
    auto* other = reinterpret_cast<const grpc_alts_channel_security_connector*>(other_sc);
  ...
  bool check_call_host(absl::string_view host, grpc_auth_context* /*auth_context*/,
    ...
    if (host.empty() || host != target_name_) {
      *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("ALTS call host does not match target name");
    }
    return true;
  }
...
class grpc_alts_server_security_connector final : public grpc_server_security_connector {
 public:
  explicit grpc_alts_server_security_connector(
      grpc_core::RefCountedPtr<grpc_server_credentials> server_creds)
      : grpc_server_security_connector(GRPC_ALTS_URL_SCHEME, std::move(server_creds)) {}
  ...
  void add_handshakers(const grpc_channel_args* args, grpc_pollset_set* interested_parties,
    ...
    GPR_ASSERT(alts_tsi_handshaker_create(
                   ...
                   interested_parties, &handshaker, user_specified_max_frame_size) == TSI_OK);
    handshake_manager->Add(grpc_core::SecurityHandshakerCreate(handshaker, this, args));
  }
...
grpc_core::RefCountedPtr<grpc_auth_context> grpc_alts_auth_context_from_tsi_peer(
    const tsi_peer* peer) {
  if (peer == nullptr) {
    gpr_log(GPR_ERROR, "Invalid arguments to grpc_alts_auth_context_from_tsi_peer()");
    return nullptr;
  }
  /* Validate certificate type. */
  const tsi_peer_property* cert_type_prop =
      tsi_peer_get_property_by_name(peer, TSI_CERTIFICATE_TYPE_PEER_PROPERTY);
  if (cert_type_prop == nullptr || strncmp(cert_type_prop->value.data, TSI_ALTS_CERTIFICATE_TYPE,
  ...
  grpc_slice slice =
      grpc_slice_from_copied_buffer(rpc_versions_prop->value.data, rpc_versions_prop->value.length);
  bool decode_result = grpc_gcp_rpc_protocol_versions_decode(slice, &peer_versions);
  ...
  bool check_result =
      grpc_gcp_rpc_protocol_versions_check(&local_versions, &peer_versions, nullptr);
  if (!check_result) {
  ...
  for (i = 0; i < peer->property_count; i++) {
    ...
    if (strcmp(tsi_prop->name, TSI_ALTS_SERVICE_ACCOUNT_PEER_PROPERTY) == 0) {
      grpc_auth_context_add_property(ctx.get(), TSI_ALTS_SERVICE_ACCOUNT_PEER_PROPERTY,
    ...
    if (strcmp(tsi_prop->name, TSI_ALTS_CONTEXT) == 0) {
      grpc_auth_context_add_property(ctx.get(), TSI_ALTS_CONTEXT, tsi_prop->value.data,
...
grpc_alts_channel_security_connector_create(
  ...
  if (channel_creds == nullptr || target_name == nullptr) {
    gpr_log(GPR_ERROR, "Invalid arguments to grpc_alts_channel_security_connector_create()");
    return nullptr;
  }
...
grpc_core::RefCountedPtr<grpc_server_security_connector> grpc_alts_server_security_connector_create(
  ...
  if (server_creds == nullptr) {
    gpr_log(GPR_ERROR, "Invalid arguments to grpc_alts_server_security_connector_create()");
    return nullptr;
  }
  return grpc_core::MakeRefCounted<grpc_alts_server_security_connector>(std::move(server_creds));
}
