...
grpc_call_error grpc_call_set_credentials(
...
static grpc_auth_property_iterator empty_iterator = {nullptr, 0,
                                                     nullptr};
...
int grpc_auth_context_set_peer_identity_property_name(
  ...
  GRPC_API_TRACE(
      "grpc_auth_context_set_peer_identity_property_name(ctx=%p, "
      "name=%s)",
      2, (ctx, name));
  if (prop == nullptr) {
...
int grpc_auth_context_peer_is_authenticated(
    const grpc_auth_context* ctx) {
  GRPC_API_TRACE("grpc_auth_context_peer_is_authenticated(ctx=%p)", 1,
                 (ctx));
...
grpc_auth_property_iterator grpc_auth_context_property_iterator(
  ...
  GRPC_API_TRACE("grpc_auth_context_property_iterator(ctx=%p)", 1,
                 (ctx));
...
grpc_auth_property_iterator grpc_auth_context_find_properties_by_name(
  ...
  grpc_auth_property_iterator it = empty_iterator;
  GRPC_API_TRACE(
      "grpc_auth_context_find_properties_by_name(ctx=%p, name=%s)", 2,
      (ctx, name));
...
void grpc_auth_context::ensure_capacity() {
  if (properties_.count == properties_.capacity) {
    ...
    properties_.array = static_cast<grpc_auth_property*>(
        gpr_realloc(properties_.array,
                    properties_.capacity * sizeof(grpc_auth_property)));
  }
}
...
void grpc_auth_context_add_cstring_property(grpc_auth_context* ctx,
                                            ...
                                            const char* value) {
  GRPC_API_TRACE(
      "grpc_auth_context_add_cstring_property(ctx=%p, name=%s, "
      "value=%s)",
      3, (ctx, name, value));
...
static void auth_context_pointer_arg_destroy(void* p) {
  if (p != nullptr) {
    static_cast<grpc_auth_context*>(p)->Unref(
        DEBUG_LOCATION, "auth_context_pointer_arg");
  }
}
...
static void* auth_context_pointer_arg_copy(void* p) {
  ...
  return ctx == nullptr
             ? nullptr
             : ctx->Ref(DEBUG_LOCATION, "auth_context_pointer_arg")
                   .release();
}
