...
{
  return (grpc_server_security_context *)
  ...
  if (grpc_trace_auth_context_refcount.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "AUTH_CONTEXT:%p   ref %" PRIdPTR " -> %" PRIdPTR " %s", ctx,
	       val, val + 1, reason);
    }
#else
grpc_auth_context *
  ...
  if (grpc_trace_auth_context_refcount.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&ctx->refcount.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "AUTH_CONTEXT:%p unref %" PRIdPTR " -> %" PRIdPTR " %s", ctx,
	       val, val - 1, reason);
    }
#else
void
...
ensure_auth_context_capacity (grpc_auth_context * ctx)
{
  if (ctx->properties.count == ctx->properties.capacity)
	...
	(grpc_auth_property *) gpr_realloc (ctx->properties.array,
					    ctx->properties.capacity *
					    sizeof (grpc_auth_property));
    }
}
				...
				const char *value, size_t value_length)
{
  ...
  GRPC_API_TRACE
    ("grpc_auth_context_add_property(ctx=%p, name=%s, value=%*.*s, "
     "value_length=%lu)", 6, (ctx, name, (int) value_length,
...
static void
auth_context_pointer_arg_destroy (void *p)
{
  GRPC_AUTH_CONTEXT_UNREF ((grpc_auth_context *) p,
			   "auth_context_pointer_arg");
}

static void *
auth_context_pointer_arg_copy (void *p)
{
...
static int
auth_context_pointer_cmp (void *a, void *b)
{
  return GPR_ICMP (a, b);
}
...
grpc_arg
grpc_auth_context_to_arg (grpc_auth_context * p)
{
...
{
  if (strcmp (arg->key, GRPC_AUTH_CONTEXT_ARG) != 0)
    return nullptr;
