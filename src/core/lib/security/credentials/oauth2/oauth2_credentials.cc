...
grpc_auth_refresh_token grpc_auth_refresh_token_create_from_json(const Json& json) {
  ...
  if (prop_value == nullptr || strcmp(prop_value, GRPC_AUTH_JSON_TYPE_AUTHORIZED_USER) != 0) {
    goto end;
  }
  result.type = GRPC_AUTH_JSON_TYPE_AUTHORIZED_USER;

  if (!grpc_copy_json_string_property(json, "client_secret", &result.client_secret) ||
      !grpc_copy_json_string_property(json, "client_id", &result.client_id) ||
      !grpc_copy_json_string_property(json, "refresh_token", &result.refresh_token)) {
    goto end;
  }
  success = 1;
...
grpc_auth_refresh_token grpc_auth_refresh_token_create_from_string(const char* json_string) {
  ...
  if (error != GRPC_ERROR_NONE) {
    gpr_log(GPR_ERROR, "JSON parsing failed: %s", grpc_error_std_string(error).c_str());
...
grpc_credentials_status grpc_oauth2_token_fetcher_credentials_parse_server_response(
    const grpc_http_response* response, grpc_mdelem* token_md, grpc_millis* token_lifetime) {
  ...
  if (response->status != 200) {
    gpr_log(GPR_ERROR, "Call to http server ended with error %d [%s].", response->status,
    ...
    if (error != GRPC_ERROR_NONE) {
      gpr_log(GPR_ERROR, "Could not parse JSON from %s: %s", null_terminated_body,
    ...
    if (it == json.object_value().end() || it->second.type() != Json::Type::STRING) {
    ...
    if (it == json.object_value().end() || it->second.type() != Json::Type::NUMBER) {
    ...
    *token_md = grpc_mdelem_from_slices(
        ...
        grpc_slice_from_cpp_string(absl::StrCat(token_type, " ", access_token)));
...
static void on_oauth2_token_fetcher_http_response(void* user_data, grpc_error_handle error) {
  ...
  grpc_credentials_metadata_request* r = static_cast<grpc_credentials_metadata_request*>(user_data);
...
void grpc_oauth2_token_fetcher_credentials::on_http_response(grpc_credentials_metadata_request* r,
  ...
  grpc_credentials_status status =
      error == GRPC_ERROR_NONE ? grpc_oauth2_token_fetcher_credentials_parse_server_response(
  ...
  while (pending_request != nullptr) {
    ...
    if (status == GRPC_CREDENTIALS_OK) {
      grpc_credentials_mdelem_array_add(pending_request->md_array, access_token_md);
    } else {
      new_error = GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING(
          "Error occurred when fetching oauth2 token.", &error, 1);
    }
    grpc_core::ExecCtx::Run(DEBUG_LOCATION, pending_request->on_request_metadata, new_error);
    grpc_polling_entity_del_from_pollset_set(pending_request->pollent,
...
bool grpc_oauth2_token_fetcher_credentials::get_request_metadata(
  ...
  grpc_millis refresh_threshold = GRPC_SECURE_TOKEN_REFRESH_THRESHOLD_SECS * GPR_MS_PER_SEC;
  grpc_mdelem cached_access_token_md = GRPC_MDNULL;
  gpr_mu_lock(&mu_);
  if (!GRPC_MDISNULL(access_token_md_) &&
      gpr_time_cmp(gpr_time_sub(token_expiration_, gpr_now(GPR_CLOCK_MONOTONIC)),
  ...
  grpc_oauth2_pending_get_request_metadata* pending_request =
      static_cast<grpc_oauth2_pending_get_request_metadata*>(gpr_malloc(sizeof(*pending_request)));
  pending_request->md_array = md_array;
  pending_request->on_request_metadata = on_request_metadata;
  pending_request->pollent = pollent;
  grpc_polling_entity_add_to_pollset_set(pollent, grpc_polling_entity_pollset_set(&pollent_));
  ...
  if (start_fetch) {
    Ref().release();
    fetch_oauth2(grpc_credentials_metadata_request_create(this->Ref()), &httpcli_context_,
...
grpc_oauth2_token_fetcher_credentials::grpc_oauth2_token_fetcher_credentials()
    : grpc_call_credentials(GRPC_CALL_CREDENTIALS_TYPE_OAUTH2),
      ...
      pollent_(grpc_polling_entity_create_from_pollset_set(grpc_pollset_set_create())) {
  gpr_mu_init(&mu_);
...
class grpc_compute_engine_token_fetcher_credentials : public grpc_oauth2_token_fetcher_credentials {
 public:
 ...
 protected:
  void fetch_oauth2(grpc_credentials_metadata_request* metadata_req,
                    grpc_httpcli_context* http_context, grpc_polling_entity* pollent,
                    grpc_iomgr_cb_func response_cb, grpc_millis deadline) override {
    grpc_http_header header = {const_cast<char*>("Metadata-Flavor"), const_cast<char*>("Google")};
    grpc_httpcli_request request;
    memset(&request, 0, sizeof(grpc_httpcli_request));
    request.host = const_cast<char*>(GRPC_COMPUTE_ENGINE_METADATA_HOST);
    request.http.path = const_cast<char*>(GRPC_COMPUTE_ENGINE_METADATA_TOKEN_PATH);
  ...
  std::string debug_string() override {
    return absl::StrFormat("GoogleComputeEngineTokenFetcherCredentials{%s}",
...
grpc_call_credentials* grpc_google_compute_engine_credentials_create(void* reserved) {
  GRPC_API_TRACE("grpc_compute_engine_credentials_create(reserved=%p)", 1, (reserved));
  GPR_ASSERT(reserved == nullptr);
  return grpc_core::MakeRefCounted<grpc_compute_engine_token_fetcher_credentials>().release();
}
...
void grpc_google_refresh_token_credentials::fetch_oauth2(
    grpc_credentials_metadata_request* metadata_req, grpc_httpcli_context* httpcli_context,
    grpc_polling_entity* pollent, grpc_iomgr_cb_func response_cb, grpc_millis deadline) {
  grpc_http_header header = {const_cast<char*>("Content-Type"),
                             const_cast<char*>("application/x-www-form-urlencoded")};
  grpc_httpcli_request request;
  std::string body =
      absl::StrFormat(GRPC_REFRESH_TOKEN_POST_BODY_FORMAT_STRING, refresh_token_.client_id,
  ...
  /* TODO(ctiller): Carry the resource_quota in ctx and share it with the host
     channel. This would allow us to cancel an authentication query when under
     extreme memory pressure. */
  grpc_resource_quota* resource_quota = grpc_resource_quota_create("oauth2_credentials_refresh");
  grpc_httpcli_post(httpcli_context, pollent, resource_quota, &request, body.c_str(), body.size(),
                    deadline,
...
grpc_refresh_token_credentials_create_from_auth_refresh_token(
  ...
  return grpc_core::MakeRefCounted<grpc_google_refresh_token_credentials>(refresh_token);
}

std::string grpc_google_refresh_token_credentials::debug_string() {
  return absl::StrFormat("GoogleRefreshToken{ClientID:%s,%s}", refresh_token_.client_id,
                         grpc_oauth2_token_fetcher_credentials::debug_string());
}
...
grpc_call_credentials* grpc_google_refresh_token_credentials_create(const char* json_refresh_token,
                                                                    void* reserved) {
  grpc_auth_refresh_token token = grpc_auth_refresh_token_create_from_string(json_refresh_token);
  ...
  GPR_ASSERT(reserved == nullptr);
  return grpc_refresh_token_credentials_create_from_auth_refresh_token(token).release();
}
...
void MaybeAddToBody(const char* field_name, const char* field, std::vector<std::string>* body) {
...
class StsTokenFetcherCredentials : public grpc_oauth2_token_fetcher_credentials {
 public:
  StsTokenFetcherCredentials(URI sts_url, const grpc_sts_credentials_options* options)
  ...
  std::string debug_string() override {
    return absl::StrFormat("StsTokenFetcherCredentials{Path:%s,Authority:%s,%s}", sts_url_.path(),
                           sts_url_.authority(),
                           grpc_oauth2_token_fetcher_credentials::debug_string());
  }
 ...
 private:
  void fetch_oauth2(grpc_credentials_metadata_request* metadata_req,
    ...
    request.http.hdrs = &header;
    request.handshaker =
        (sts_url_.scheme() == "https") ? &grpc_httpcli_ssl : &grpc_httpcli_plaintext;
    /* TODO(ctiller): Carry the resource_quota in ctx and share it with the host
       channel. This would allow us to cancel an authentication query when under
       extreme memory pressure. */
    grpc_resource_quota* resource_quota = grpc_resource_quota_create("oauth2_credentials_refresh");
    grpc_httpcli_post(http_context, pollent, resource_quota, &request, body, body_length, deadline,
                      GRPC_CLOSURE_INIT(&http_post_cb_closure_, response_cb, metadata_req,
                                        grpc_schedule_on_exec_ctx),
                      &metadata_req->response);
    grpc_resource_quota_unref_internal(resource_quota);
  ...
  grpc_error_handle FillBody(char** body, size_t* body_length) {
    *body = nullptr;
    ...
    auto cleanup = [&body, &body_length, &body_parts, &subject_token, &actor_token, &err]() {
      if (err == GRPC_ERROR_NONE) {
    ...
    body_parts.push_back(
        absl::StrFormat(GRPC_STS_POST_MINIMAL_BODY_FORMAT_STRING,
    ...
    MaybeAddToBody("requested_token_type", requested_token_type_.get(), &body_parts);
    if ((actor_token_path_ != nullptr) && *actor_token_path_ != '\0') {
      err = LoadTokenFile(actor_token_path_.get(), &actor_token);
      if (err != GRPC_ERROR_NONE) return cleanup();
      MaybeAddToBody("actor_token",
                     reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(actor_token)), &body_parts);
...
absl::StatusOr<URI> ValidateStsCredentialsOptions(const grpc_sts_credentials_options* options) {
  ...
  absl::StatusOr<URI> sts_url = URI::Parse(
      options->token_exchange_service_uri == nullptr ? "" : options->token_exchange_service_uri);
  if (!sts_url.ok()) {
    ...
    error_list.push_back(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("Invalid URI scheme, must be https to http."));
  }
  if (options->subject_token_path == nullptr || strlen(options->subject_token_path) == 0) {
    error_list.push_back(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("subject_token needs to be specified"));
  }
  if (options->subject_token_type == nullptr || strlen(options->subject_token_type) == 0) {
    error_list.push_back(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("subject_token_type needs to be specified"));
  }
  if (error_list.empty()) {
    return sts_url;
  }
  auto grpc_error_vec =
      GRPC_ERROR_CREATE_FROM_VECTOR("Invalid STS Credentials Options", &error_list);
  auto retval = absl::InvalidArgumentError(grpc_error_std_string(grpc_error_vec));
...
grpc_call_credentials* grpc_sts_credentials_create(const grpc_sts_credentials_options* options,
                                                   void* reserved) {
  GPR_ASSERT(reserved == nullptr);
  absl::StatusOr<grpc_core::URI> sts_url = grpc_core::ValidateStsCredentialsOptions(options);
  if (!sts_url.ok()) {
    gpr_log(GPR_ERROR, "STS Credentials creation failed. Error: %s.",
            sts_url.status().ToString().c_str());
    return nullptr;
  }
  return grpc_core::MakeRefCounted<grpc_core::StsTokenFetcherCredentials>(std::move(*sts_url),
                                                                          options)
      .release();
}
...
bool grpc_access_token_credentials::get_request_metadata(grpc_polling_entity* /*pollent*/,
...
grpc_access_token_credentials::grpc_access_token_credentials(const char* access_token)
    : grpc_call_credentials(GRPC_CALL_CREDENTIALS_TYPE_OAUTH2) {
  grpc_core::ExecCtx exec_ctx;
  access_token_md_ =
      grpc_mdelem_from_slices(grpc_core::ExternallyManagedSlice(GRPC_AUTHORIZATION_METADATA_KEY),
...
grpc_call_credentials* grpc_access_token_credentials_create(const char* access_token,
  ...
  GRPC_API_TRACE(
      ...
      1, (reserved));
  GPR_ASSERT(reserved == nullptr);
  return grpc_core::MakeRefCounted<grpc_access_token_credentials>(access_token).release();
}
