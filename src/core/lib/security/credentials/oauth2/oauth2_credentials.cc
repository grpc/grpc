...
static void
oauth2_token_fetcher_destruct (grpc_call_credentials * creds)
{
							     ...
							     token_lifetime)
{
  ...
  if (response->status != 200)
      ...
      if (expires_in == nullptr || expires_in->type != GRPC_JSON_NUMBER)
	{
      ...
      gpr_asprintf (&new_access_token, "%s %s", token_type->value,
		    access_token->value);
      *token_lifetime =
	strtol (expires_in->value, nullptr, 10) * GPR_MS_PER_SEC;
      if (!GRPC_MDISNULL (*token_md))
  ...
  if (null_terminated_body != nullptr)
...
static void
on_oauth2_token_fetcher_http_response (void *user_data, grpc_error * error)
{
  ...
  grpc_credentials_status status =
    grpc_oauth2_token_fetcher_credentials_parse_server_response (&r->response,
								 &access_token_md,
								 &token_lifetime);
  ...
  c->token_expiration = status == GRPC_CREDENTIALS_OK
    ? grpc_core::ExecCtx::Get ()->Now () + token_lifetime : 0;
...
static bool
oauth2_token_fetcher_get_request_metadata (grpc_call_credentials * creds,
...
static void
oauth2_token_fetcher_cancel_get_request_metadata (grpc_call_credentials *
						  creds,
						  grpc_credentials_mdelem_array
						  * md_array,
						  grpc_error * error)
			     ...
			     grpc_millis deadline)
{
  ...
  grpc_httpcli_get (httpcli_context, pollent, resource_quota, &request,
...
static void
refresh_token_destruct (grpc_call_credentials * creds)
{
...
static char *
create_loggable_refresh_token (grpc_auth_refresh_token * token)
{
  ...
  if (grpc_api_trace.enabled ())
    {
      ...
      gpr_log (GPR_INFO,
	       ...
	       "reserved=%p)", loggable_token, reserved);
...
static void
access_token_destruct (grpc_call_credentials * creds)
{
...
static bool
access_token_get_request_metadata (grpc_call_credentials * creds,
				   grpc_polling_entity * pollent,
				   grpc_auth_metadata_context context,
...
static void
access_token_cancel_get_request_metadata (grpc_call_credentials * c,
					  grpc_credentials_mdelem_array *
