...
void grpc_tls_certificate_distributor::SetKeyMaterials(
  ...
  GPR_ASSERT(pem_root_certs.has_value() ||
    ...
    for (auto* watcher_ptr : cert_info.root_cert_watchers) {
      ...
      if (pem_key_cert_pairs.has_value() &&
        ...
        auto& identity_cert_info =
            certificate_info_map_[*watcher_it->second
                                       .identity_cert_name];
  ...
  if (pem_key_cert_pairs.has_value()) {
    ...
    for (const auto watcher_ptr : cert_info.identity_cert_watchers) {
      ...
      if (pem_root_certs.has_value() &&
        ...
        // In this case, We've already sent the credential updates at
        // the time when checking pem_root_certs, so we will skip here.
        continue;
...
void grpc_tls_certificate_distributor::SetErrorForCert(
  ...
  GPR_ASSERT(root_cert_error.has_value() ||
             identity_cert_error.has_value());
  grpc_core::MutexLock lock(&mu_);
  ...
  if (root_cert_error.has_value()) {
    for (auto* watcher_ptr : cert_info.root_cert_watchers) {
      ...
      const auto watcher_it = watchers_.find(watcher_ptr);
      GPR_ASSERT(watcher_it != watchers_.end());
      // identity_cert_error_to_report is the error of the identity cert
      // this watcher is watching, if there is any.
      ...
      if (identity_cert_error.has_value() &&
        ...
        identity_cert_error_to_report =
            identity_cert_info.identity_cert_error;
      }
      watcher_ptr->OnError(
  ...
  if (identity_cert_error.has_value()) {
    for (auto* watcher_ptr : cert_info.identity_cert_watchers) {
      ...
      const auto watcher_it = watchers_.find(watcher_ptr);
      GPR_ASSERT(watcher_it != watchers_.end());
      // root_cert_error_to_report is the error of the root cert this
      // watcher is watching, if there is any.
      grpc_error_handle root_cert_error_to_report = GRPC_ERROR_NONE;
      if (root_cert_error.has_value() &&
          watcher_it->second.root_cert_name == cert_name) {
        // In this case, We've already sent the error updates at the
        // time when checking root_cert_error, so we will skip here.
        continue;
      } else if (watcher_it->second.root_cert_name.has_value()) {
        auto& root_cert_info =
            certificate_info_map_[*watcher_it->second.root_cert_name];
...
void grpc_tls_certificate_distributor::SetError(
  ...
  for (const auto& watcher : watchers_) {
    ...
    watcher_ptr->OnError(watcher_info.root_cert_name.has_value()
...
void grpc_tls_certificate_distributor::WatchTlsCertificates(
  ...
  GPR_ASSERT(root_cert_name.has_value() ||
             identity_cert_name.has_value());
  ...
  // Update watchers_ and certificate_info_map_.
  {
    grpc_core::MutexLock lock(&mu_);
    const auto watcher_it = watchers_.find(watcher_ptr);
    // The caller needs to cancel the watcher first if it wants to
    ...
    if (root_cert_name.has_value()) {
      CertificateInfo& cert_info =
          certificate_info_map_[*root_cert_name];
    ...
    // contents. Note that an *_cert_error in cert_info only indicates
    // error occurred while trying to fetch the latest cert, but the
    // updated_*_certs should always be valid. So we will send the
    // updates regardless of
    // *_cert_error.
    if (updated_root_certs.has_value() ||
        updated_identity_pairs.has_value()) {
      watcher_ptr->OnCertificatesChanged(
          updated_root_certs, std::move(updated_identity_pairs));
    }
    // Notify this watcher if the certs it is watching already had some
    // errors.
    if (root_error != GRPC_ERROR_NONE ||
        identity_error != GRPC_ERROR_NONE) {
  ...
  // Invoke watch status callback if needed.
  {
    ...
    if (watch_status_callback_ != nullptr) {
      if (root_cert_name == identity_cert_name &&
        ...
        watch_status_callback_(*root_cert_name,
                               start_watching_root_cert,
                               start_watching_identity_cert);
      } else {
        if (start_watching_root_cert) {
          watch_status_callback_(
              *root_cert_name, true,
              already_watching_identity_for_root_cert);
        }
  ...
  // Invoke watch status callback if needed.
  {
    ...
    if (watch_status_callback_ != nullptr) {
      if (root_cert_name == identity_cert_name &&
        ...
        watch_status_callback_(*root_cert_name,
                               !stop_watching_root_cert,
                               !stop_watching_identity_cert);
      } else {
        if (stop_watching_root_cert) {
          watch_status_callback_(
              *root_cert_name, false,
              already_watching_identity_for_root_cert);
        }
        if (stop_watching_identity_cert) {
          watch_status_callback_(
