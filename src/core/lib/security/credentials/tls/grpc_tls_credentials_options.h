...
struct grpc_tls_credentials_options
 ...
 public:
  ...
  // otherwise.
  TlsCertificateDistributor* certificate_distributor() {
  ...
  void set_watch_root_cert(bool watch_root_cert) {
    watch_root_cert_ = watch_root_cert;
  }
  ...
  // tls_credentials, it should always be set to true unless the identity
  // key-cert pairs are not needed.
  void set_watch_identity_pair(bool watch_identity_pair) {
    watch_identity_pair_ = watch_identity_pair;
  }
  // Sets the name of identity key-cert pairs being watched, if
  // |set_watch_identity_pair| is called. If not set, an empty string will be
  // used as the name.
  ...
  void set_crl_directory(std::string crl_directory) {
    crl_directory_ = std::move(crl_directory);
  }
  void set_crl_provider(
  ...
  bool operator==(const grpc_tls_credentials_options& other) const {
    return cert_request_type_ == other.cert_request_type_ &&
           ...
           (certificate_verifier_ == other.certificate_verifier_ ||
            (certificate_verifier_ != nullptr &&
             other.certificate_verifier_ != nullptr &&
             certificate_verifier_->Compare(
                 other.certificate_verifier_.get()) == 0)) &&
