...
struct grpc_tls_certificate_distributor
 ...
 private:
  ...
  // is changed.
  std::function<void(std::string, bool, bool)> watch_status_callback_ ABSL_GUARDED_BY(callback_mu_);
  ...
  std::map<std::string, CertificateInfo> certificate_info_map_ ABSL_GUARDED_BY(mu_);
};
