...
#include "src/core/lib/security/credentials/tls/grpc_tls_static_file_certificate_provider.h"
...
StaticFileCertificateProvider::StaticFileCertificateProvider(
  ...
  GPR_ASSERT(GRPC_LOG_IF_ERROR(
      "load_file", grpc_load_file(root_certificate_file_name, 1, &root_slice)));
  std::string root_cert = std::string(
      reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(root_slice)),
      GRPC_SLICE_LENGTH(root_slice));
  // FileWatcherCertificateProvider uses the default cert name "" for root
  // certificates.
  ...
  GPR_ASSERT(GRPC_LOG_IF_ERROR(
      "load_file",
      grpc_load_file(identity_certificate_file_name, 1, &cert_slice)));
  GPR_ASSERT(GRPC_LOG_IF_ERROR(
      "load_file", grpc_load_file(private_key_file_name, 1, &key_slice)));
  ...
  // FileWatcherCertificateProvider uses the default cert name "" for identity
  // certificates.
...
grpc_tls_certificate_provider* grpc_tls_certificate_provider_file_static_create(
  ...
  return new grpc_core::StaticFileCertificateProvider(
      private_key_file_name, identity_certificate_file_name,
