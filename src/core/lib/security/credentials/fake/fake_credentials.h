...
/* Creates a fake server transport security credentials object for
 * testing. */
 ...
 *
 * That is to say, LB channels have a heading list of LB targets
 * separated from the list of backend targets by a semicolon. For non-LB
 * channels, only the latter is present. */
grpc_arg grpc_fake_transport_expected_targets_arg(
    char* expected_targets);

/* Return the value associated with the expected targets channel arg or
 * NULL */
