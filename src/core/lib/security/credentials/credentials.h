...
typedef struct
{
  ...
  grpc_channel_credentials
    *(*duplicate_without_call_credentials) (grpc_channel_credentials * c);
...
grpc_channel_credentials
  *grpc_channel_credentials_ref (grpc_channel_credentials * creds);
...
grpc_channel_credentials_create_security_connector (grpc_channel_credentials *
						    creds, const char *target,
...
grpc_channel_credentials *grpc_channel_credentials_from_arg (const grpc_arg *
							     arg);

/* Util to find the channel credentials from channel args. */
grpc_channel_credentials *grpc_channel_credentials_find_in_args (const
								 grpc_channel_args
								 * args);
...
void grpc_credentials_mdelem_array_append (grpc_credentials_mdelem_array *
					   dst,
					   grpc_credentials_mdelem_array *
					   src);

void grpc_credentials_mdelem_array_destroy (grpc_credentials_mdelem_array *
					    list);
...
grpc_call_credentials *grpc_call_credentials_ref (grpc_call_credentials *
						  creds);
...
bool grpc_call_credentials_get_request_metadata (grpc_call_credentials *
						 creds,
						 grpc_polling_entity *
						 pollent,
...
grpc_call_credentials *grpc_md_only_test_credentials_create (const char
							     *md_key,
							     const char
							     *md_value,
							     bool is_async);
...
typedef struct
{
  void (*destruct) (grpc_server_credentials * c);
    grpc_security_status (*create_security_connector) (grpc_server_credentials
						       * c,
...
grpc_security_status
grpc_server_credentials_create_security_connector (grpc_server_credentials *
						   creds,
						   grpc_server_security_connector
						   ** sc);

grpc_server_credentials *grpc_server_credentials_ref (grpc_server_credentials
						      * creds);
...
grpc_server_credentials *grpc_server_credentials_from_arg (const grpc_arg *
							   arg);
grpc_server_credentials *grpc_find_server_credentials_in_args (const
							       grpc_channel_args
							       * args);
...
void
grpc_credentials_metadata_request_destroy (grpc_credentials_metadata_request *
					   r);
