...
static void
composite_call_destruct (grpc_call_credentials * creds)
{
  grpc_composite_call_credentials *c =
    (grpc_composite_call_credentials *) creds;
...
static void
composite_call_metadata_cb (void *arg, grpc_error * error)
{
...
static bool
composite_call_get_request_metadata (grpc_call_credentials * creds,
				     grpc_polling_entity * pollent,
...
static void
composite_call_cancel_get_request_metadata (grpc_call_credentials * creds,
					    ...
					    md_array, grpc_error * error)
{
  ...
  for (size_t i = 0; i < c->inner.num_creds; ++i)
    {
      grpc_call_credentials_cancel_get_request_metadata (c->inner.
							 creds_array[i],
							 md_array,
							 GRPC_ERROR_REF
							 (error));
...
static grpc_call_credentials_array
get_creds_array (grpc_call_credentials ** creds_addr)
{
					...
					void *reserved)
{
  ...
  GRPC_API_TRACE
    ("grpc_composite_call_credentials_create(creds1=%p, creds2=%p, "
     "reserved=%p)", 3, (creds1, creds2, reserved));
						 ...
						 creds)
{
  ...
  GPR_ASSERT (strcmp (creds->type, GRPC_CALL_CREDENTIALS_TYPE_COMPOSITE) ==
	      0);
  return &c->inner;
}

grpc_call_credentials *
grpc_credentials_contains_type (grpc_call_credentials * creds,
				const char *type,
				grpc_call_credentials ** composite_creds)
{
  ...
  if (strcmp (creds->type, type) == 0)
    {
      if (composite_creds != nullptr)
	*composite_creds = nullptr;
      return creds;
    }
...
static void
composite_channel_destruct (grpc_channel_credentials * creds)
{
...
composite_channel_create_security_connector (grpc_channel_credentials * creds,
					     grpc_call_credentials *
					     call_creds, const char *target,
					     ...
					     grpc_channel_args ** new_args)
{
  ...
  if (call_creds != nullptr)
	...
	grpc_composite_call_credentials_create (c->call_creds, call_creds,
						nullptr);
      status =
	c->inner_creds->vtable->create_security_connector (c->inner_creds,
...
composite_channel_duplicate_without_call_credentials (grpc_channel_credentials
						      * creds)
{
  grpc_composite_channel_credentials *c =
    (grpc_composite_channel_credentials *) creds;
					   ...
					   void *reserved)
{
  ...
  GRPC_API_TRACE
    ("grpc_composite_channel_credentials_create(channel_creds=%p, "
     "call_creds=%p, reserved=%p)", 3, (channel_creds, call_creds, reserved));
