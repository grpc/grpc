...
grpc_security_status grpc_ssl_credentials::create_security_connector(
  ...
  status = grpc_ssl_channel_security_connector_create(
      this, call_creds, &config_, target, overridden_target_name,
...
void grpc_ssl_credentials::build_config(
    const char* pem_root_certs, grpc_ssl_pem_key_cert_pair* pem_key_cert_pair,
    const verify_peer_options* verify_options) {
  config_.pem_root_certs =
      pem_root_certs ? gpr_strdup(pem_root_certs) : nullptr;
...
grpc_security_status grpc_ssl_server_credentials::create_security_connector(
    grpc_server_security_connector** sc) {
  return grpc_ssl_server_security_connector_create(this, sc);
}
