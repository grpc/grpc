...
static void ssl_build_config(const char* pem_root_certs,
  ...
  if (pem_key_cert_pair != nullptr) {
    ...
    config->pem_key_cert_pair = reinterpret_cast<tsi_ssl_pem_key_cert_pair*>(
...
grpc_channel_credentials* grpc_ssl_credentials_create(
    ...
    void* reserved) {
  grpc_ssl_credentials* c = reinterpret_cast<grpc_ssl_credentials*>(
...
tsi_ssl_pem_key_cert_pair* grpc_convert_grpc_to_tsi_cert_pairs(
  ...
  if (num_key_cert_pairs > 0) {
    ...
    tsi_pairs = reinterpret_cast<tsi_ssl_pem_key_cert_pair*>(
...
grpc_ssl_server_certificate_config* grpc_ssl_server_certificate_config_create(
  ...
  grpc_ssl_server_certificate_config* config =
      reinterpret_cast<grpc_ssl_server_certificate_config*>(
  ...
  if (num_key_cert_pairs > 0) {
    GPR_ASSERT(pem_key_cert_pairs != nullptr);
    config->pem_key_cert_pairs = reinterpret_cast<grpc_ssl_pem_key_cert_pair*>(
...
grpc_ssl_server_credentials_create_options_using_config(
  ...
  options = reinterpret_cast<grpc_ssl_server_credentials_options*>(
...
grpc_ssl_server_credentials_create_options_using_config_fetcher(
  ...
  grpc_ssl_server_certificate_config_fetcher* fetcher =
      reinterpret_cast<grpc_ssl_server_certificate_config_fetcher*>(
          gpr_zalloc(sizeof(grpc_ssl_server_certificate_config_fetcher)));
  fetcher->cb = cb;
  fetcher->user_data = user_data;

  grpc_ssl_server_credentials_options* options =
      reinterpret_cast<grpc_ssl_server_credentials_options*>(
          gpr_zalloc(sizeof(grpc_ssl_server_credentials_options)));
  options->client_certificate_request = client_certificate_request;
...
grpc_server_credentials* grpc_ssl_server_credentials_create_with_options(
  ...
  c = reinterpret_cast<grpc_ssl_server_credentials*>(
