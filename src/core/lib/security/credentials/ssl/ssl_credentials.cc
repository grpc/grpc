...
void
grpc_tsi_ssl_pem_key_cert_pairs_destroy (tsi_ssl_pem_key_cert_pair * kp,
...
static void
ssl_destruct (grpc_channel_credentials * creds)
{
...
ssl_create_security_connector (grpc_channel_credentials * creds,
			       ...
			       grpc_channel_args ** new_args)
{
  ...
  status =
    grpc_ssl_channel_security_connector_create (creds, call_creds, &c->config,
						target,
  ...
  grpc_arg new_arg = grpc_channel_arg_string_create ((char *)
...
tsi_ssl_pem_key_cert_pair *
grpc_convert_grpc_to_tsi_cert_pairs (const grpc_ssl_pem_key_cert_pair *
				     pem_key_cert_pairs,
				     size_t num_key_cert_pairs)
{
  ...
  if (num_key_cert_pairs > 0)
    {
      ...
      tsi_pairs =
	(tsi_ssl_pem_key_cert_pair *) gpr_zalloc (num_key_cert_pairs *
						  sizeof
						  (tsi_ssl_pem_key_cert_pair));
    }
  for (size_t i = 0; i < num_key_cert_pairs; i++)
			 ...
			 grpc_ssl_server_config * config)
{
  ...
  config->pem_key_cert_pairs =
    grpc_convert_grpc_to_tsi_cert_pairs (pem_key_cert_pairs,
					 num_key_cert_pairs);
...
grpc_ssl_server_certificate_config_create (const char *pem_root_certs,
					   const grpc_ssl_pem_key_cert_pair *
					   pem_key_cert_pairs,
					   size_t num_key_cert_pairs)
{
  ...
  if (num_key_cert_pairs > 0)
    {
      GPR_ASSERT (pem_key_cert_pairs != nullptr);
      config->pem_key_cert_pairs =
	(grpc_ssl_pem_key_cert_pair *) gpr_zalloc (num_key_cert_pairs *
						   sizeof
...
void
grpc_ssl_server_certificate_config_destroy (grpc_ssl_server_certificate_config
					    * config)
{
  if (config == nullptr)
    return;
...
{
  return grpc_ssl_server_credentials_create_ex (pem_root_certs,
						pem_key_cert_pairs,
						num_key_cert_pairs,
						force_client_auth ?
...
grpc_ssl_server_credentials_create_ex (const char *pem_root_certs,
				       ...
				       void *reserved)
{
  GRPC_API_TRACE ("grpc_ssl_server_credentials_create_ex("
  ...
  grpc_ssl_server_certificate_config *cert_config =
    grpc_ssl_server_certificate_config_create (pem_root_certs,
					       pem_key_cert_pairs,
					       num_key_cert_pairs);
  grpc_ssl_server_credentials_options *options =
    grpc_ssl_server_credentials_create_options_using_config
  ...
  (grpc_ssl_server_credentials_options * options)
{
  ...
  if (options->certificate_config_fetcher != nullptr)
    {
      c->config.client_certificate_request =
