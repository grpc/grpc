...
grpc_ssl_credentials::create_security_connector(
  ...
  grpc_core::RefCountedPtr<grpc_channel_security_connector> sc =
      grpc_ssl_channel_security_connector_create(this->Ref(), std::move(call_creds), &config_,
                                                 target, overridden_target_name, ssl_session_cache);
  if (sc == nullptr) {
    return sc;
  }
...
void grpc_ssl_credentials::build_config(const char* pem_root_certs,
                                        grpc_ssl_pem_key_cert_pair* pem_key_cert_pair,
                                        const grpc_ssl_verify_peer_options* verify_options) {
  ...
  if (pem_key_cert_pair != nullptr) {
    ...
    config_.pem_key_cert_pair =
        static_cast<tsi_ssl_pem_key_cert_pair*>(gpr_zalloc(sizeof(tsi_ssl_pem_key_cert_pair)));
    config_.pem_key_cert_pair->cert_chain = gpr_strdup(pem_key_cert_pair->cert_chain);
    config_.pem_key_cert_pair->private_key = gpr_strdup(pem_key_cert_pair->private_key);
  } else {
    config_.pem_key_cert_pair = nullptr;
  }
  if (verify_options != nullptr) {
    memcpy(&config_.verify_options, verify_options, sizeof(verify_peer_options));
  } else {
...
grpc_ssl_server_credentials::grpc_ssl_server_credentials(
  ...
  if (options.certificate_config_fetcher != nullptr) {
    ...
    build_config(
        options.certificate_config->pem_root_certs, options.certificate_config->pem_key_cert_pairs,
        options.certificate_config->num_key_cert_pairs, options.client_certificate_request);
  }
}

grpc_ssl_server_credentials::~grpc_ssl_server_credentials() {
  grpc_tsi_ssl_pem_key_cert_pairs_destroy(config_.pem_key_cert_pairs, config_.num_key_cert_pairs);
  gpr_free(config_.pem_root_certs);
}
grpc_core::RefCountedPtr<grpc_server_security_connector>
grpc_ssl_server_credentials::create_security_connector(const grpc_channel_args* /* args */) {
...
void grpc_ssl_server_credentials::build_config(
  ...
  config_.pem_root_certs = gpr_strdup(pem_root_certs);
  config_.pem_key_cert_pairs =
      grpc_convert_grpc_to_tsi_cert_pairs(pem_key_cert_pairs, num_key_cert_pairs);
...
grpc_ssl_server_certificate_config* grpc_ssl_server_certificate_config_create(
    ...
    size_t num_key_cert_pairs) {
  grpc_ssl_server_certificate_config* config = static_cast<grpc_ssl_server_certificate_config*>(
  ...
  for (size_t i = 0; i < num_key_cert_pairs; i++) {
    GPR_ASSERT(pem_key_cert_pairs[i].private_key != nullptr);
    GPR_ASSERT(pem_key_cert_pairs[i].cert_chain != nullptr);
    config->pem_key_cert_pairs[i].cert_chain = gpr_strdup(pem_key_cert_pairs[i].cert_chain);
    config->pem_key_cert_pairs[i].private_key = gpr_strdup(pem_key_cert_pairs[i].private_key);
  }
  return config;
}
...
grpc_ssl_server_credentials_create_options_using_config_fetcher(
  ...
  grpc_ssl_server_credentials_options* options = static_cast<grpc_ssl_server_credentials_options*>(
      gpr_zalloc(sizeof(grpc_ssl_server_credentials_options)));
  options->client_certificate_request = client_certificate_request;
...
grpc_server_credentials* grpc_ssl_server_credentials_create(
  ...
  return grpc_ssl_server_credentials_create_ex(
      ...
      force_client_auth ? GRPC_SSL_REQUEST_AND_REQUIRE_CLIENT_CERTIFICATE_AND_VERIFY
...
grpc_server_credentials* grpc_ssl_server_credentials_create_ex(
  ...
  grpc_ssl_server_certificate_config* cert_config = grpc_ssl_server_certificate_config_create(
      pem_root_certs, pem_key_cert_pairs, num_key_cert_pairs);
  grpc_ssl_server_credentials_options* options =
      grpc_ssl_server_credentials_create_options_using_config(client_certificate_request,
                                                              cert_config);
...
grpc_server_credentials* grpc_ssl_server_credentials_create_with_options(
  ...
  if (options == nullptr) {
    gpr_log(GPR_ERROR, "Invalid options trying to create SSL server credentials.");
    goto done;
  }

  if (options->certificate_config == nullptr && options->certificate_config_fetcher == nullptr) {
    gpr_log(GPR_ERROR,
