...
static target_service_account* target_service_account_create(const char* service_account) {
  ...
  auto* sa = static_cast<target_service_account*>(gpr_zalloc(sizeof(target_service_account)));
...
static const grpc_alts_credentials_options_vtable vtable = {alts_client_options_copy,
...
static grpc_alts_credentials_options* alts_client_options_copy(
  ...
  grpc_alts_credentials_options* new_options = grpc_alts_credentials_client_options_create();
  ...
  auto node = (reinterpret_cast<const grpc_alts_credentials_client_options*>(options))
                  ->target_account_list_head;
  while (node != nullptr) {
    target_service_account* new_node = target_service_account_create(node->data);
    if (prev == nullptr) {
  ...
  grpc_gcp_rpc_protocol_versions_copy(&options->rpc_versions, &new_options->rpc_versions);
  return new_options;
}
