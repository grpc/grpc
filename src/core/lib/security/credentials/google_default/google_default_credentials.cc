...
static void
on_compute_engine_detection_http_response (void *user_data,
					   grpc_error * error)
{
  ...
  GRPC_LOG_IF_ERROR ("Pollset kick",
		     grpc_pollset_kick (grpc_polling_entity_pollset
					(&detector->pollent), nullptr));
  gpr_mu_unlock (g_polling_mu);
}

static void
destroy_pollset (void *p, grpc_error * e)
{
...
is_stack_running_on_compute_engine ()
{
  ...
  grpc_httpcli_get (&context, &detector.pollent, resource_quota, &request,
		    ...
		    (on_compute_engine_detection_http_response, &detector,
		     grpc_schedule_on_exec_ctx), &detector.response);
				...
				grpc_call_credentials ** creds)
{
  ...
  json = grpc_json_parse_string_with_len ((char *)
					  GRPC_SLICE_START_PTR (creds_data),
  ...
  if (json == nullptr)
	...
	grpc_error_set_str (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			    ...
			    grpc_slice_ref_internal (creds_data));
      goto end;
    }
...
grpc_google_default_credentials_create (void)
{
  ...
  err =
    create_default_creds_from_path (gpr_getenv
...
{
  if (creds_path_getter != nullptr)
    return creds_path_getter ();
