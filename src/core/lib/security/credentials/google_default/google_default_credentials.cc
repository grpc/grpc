...
static void init_default_credentials(void) { g_state_mu = new grpc_core::Mutex(); }
...
grpc_google_default_channel_credentials::create_security_connector(
  ...
  const bool is_grpclb_load_balancer =
      grpc_channel_args_find_bool(args, GRPC_ARG_ADDRESS_IS_GRPCLB_LOAD_BALANCER, false);
  ...
  const char* xds_cluster = grpc_channel_args_find_string(args, GRPC_ARG_XDS_CLUSTER_NAME);
  const bool is_xds_non_cfe_cluster =
      xds_cluster != nullptr && !absl::StartsWith(xds_cluster, "google_cfe_");
  const bool use_alts =
      is_grpclb_load_balancer || is_backend_from_grpclb_load_balancer || is_xds_non_cfe_cluster;
  ...
  grpc_core::RefCountedPtr<grpc_channel_security_connector> sc =
      use_alts ? alts_creds_->create_security_connector(call_creds, target, args, new_args)
               : ssl_creds_->create_security_connector(call_creds, target, args, new_args);
...
grpc_channel_args* grpc_google_default_channel_credentials::update_arguments(
  ...
  if (grpc_channel_args_find(args, GRPC_ARG_DNS_ENABLE_SRV_QUERIES) == nullptr) {
    grpc_arg new_srv_arg =
        grpc_channel_arg_integer_create(const_cast<char*>(GRPC_ARG_DNS_ENABLE_SRV_QUERIES), true);
...
static void on_metadata_server_detection_http_response(void* user_data, grpc_error_handle error) {
  metadata_server_detector* detector = static_cast<metadata_server_detector*>(user_data);
  ...
  detector->is_done = 1;
  GRPC_LOG_IF_ERROR("Pollset kick",
                    grpc_pollset_kick(grpc_polling_entity_pollset(&detector->pollent), nullptr));
...
static int is_metadata_server_reachable() {
  ...
  grpc_resource_quota* resource_quota = grpc_resource_quota_create("google_default_credentials");
  grpc_httpcli_get(&context, &detector.pollent, resource_quota, &request,
  ...
  GRPC_CLOSURE_INIT(&destroy_closure, destroy_pollset,
                    grpc_polling_entity_pollset(&detector.pollent), grpc_schedule_on_exec_ctx);
  grpc_pollset_shutdown(grpc_polling_entity_pollset(&detector.pollent), &destroy_closure);
...
static grpc_error_handle create_default_creds_from_path(
    const std::string& creds_path, grpc_core::RefCountedPtr<grpc_call_credentials>* creds) {
  ...
  if (json.type() != Json::Type::OBJECT) {
    error = grpc_error_set_str(GRPC_ERROR_CREATE_FROM_STATIC_STRING("Failed to parse JSON"),
...
static grpc_core::RefCountedPtr<grpc_call_credentials> make_default_call_creds(
  ...
  err = create_default_creds_from_path(grpc_get_well_known_google_credentials_file_path(),
                                       &call_creds);
  if (err == GRPC_ERROR_NONE) return call_creds;
  *error = grpc_error_add_child(*error, err);
  ...
  if (metadata_server_available()) {
    ...
    if (call_creds == nullptr) {
      ...
      *error = grpc_error_add_child(
          *error, GRPC_ERROR_CREATE_FROM_STATIC_STRING("Failed to get credentials from network"));
    }
  }
...
grpc_channel_credentials* grpc_google_default_credentials_create(
  ...
  if (call_creds != nullptr) {
    ...
    grpc_alts_credentials_options* options = grpc_alts_credentials_client_options_create();
    grpc_channel_credentials* alts_creds = grpc_alts_credentials_create(options);
    grpc_alts_credentials_options_destroy(options);
    auto creds = grpc_core::MakeRefCounted<grpc_google_default_channel_credentials>(
        grpc_core::RefCountedPtr<grpc_channel_credentials>(alts_creds),
        grpc_core::RefCountedPtr<grpc_channel_credentials>(ssl_creds));
    result = grpc_composite_channel_credentials_create(creds.get(), call_creds.get(), nullptr);
