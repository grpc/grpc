...
grpc_core::TraceFlag grpc_plugin_credentials_trace (false,
						    "plugin_credentials");

static void
plugin_destruct (grpc_call_credentials * creds)
{
...
static void
pending_request_remove_locked (grpc_plugin_credentials * c,
...
static void
pending_request_complete (grpc_plugin_credentials_pending_request * r)
{
  gpr_mu_lock (&r->creds->mu);
  if (!r->cancelled)
    pending_request_remove_locked (r->creds, r);
...
static grpc_error *
process_plugin_result (grpc_plugin_credentials_pending_request * r,
  ...
  if (status != GRPC_STATUS_OK)
      ...
      for (size_t i = 0; i < num_md; ++i)
	{
	  if (!GRPC_LOG_IF_ERROR ("validate_metadata_from_plugin",
				  grpc_validate_header_key_is_legal (md[i].
								     key)))
	    {
...
static void
plugin_md_request_metadata_ready (void *request,
...
static bool
plugin_get_request_metadata (grpc_call_credentials * creds,
			     ...
			     grpc_error ** error)
{
  ...
  if (c->plugin.get_metadata != nullptr)
	...
	{
	  gpr_log (GPR_INFO,
		   "plugin_credentials[%p]: request %p: invoking plugin", c,
		   pending_request);
	}
	  ...
	  if (grpc_plugin_credentials_trace.enabled ())
	    {
	      gpr_log (GPR_INFO,
		       ...
		       "asynchronously", c, pending_request);
	    }
	  ...
	  if (grpc_plugin_credentials_trace.enabled ())
	    {
	      gpr_log (GPR_INFO,
		       ...
		       "will be returned asynchronously", c, pending_request);
	    }
	  retval = false;
	}
      else
	  ...
	  if (grpc_plugin_credentials_trace.enabled ())
	    {
	      gpr_log (GPR_INFO,
		       "plugin_credentials[%p]: request %p: plugin returned "
		       "synchronously", c, pending_request);
	    }
	  *error =
	    process_plugin_result (pending_request, creds_md, num_creds_md,
				   status, error_details);
	}
      // Clean up.
...
static void
plugin_cancel_get_request_metadata (grpc_call_credentials * creds,
				    grpc_credentials_mdelem_array * md_array,
				    grpc_error * error)
{
  grpc_plugin_credentials *c = (grpc_plugin_credentials *) creds;
  ...
  for (grpc_plugin_credentials_pending_request * pending_request =
      ...
      if (pending_request->md_array == md_array)
	{
	  if (grpc_plugin_credentials_trace.enabled ())
	    {
	      gpr_log (GPR_INFO,
		       "plugin_credentials[%p]: cancelling request %p", c,
		       pending_request);
	    }
...
grpc_metadata_credentials_create_from_plugin (grpc_metadata_credentials_plugin
					      plugin, void *reserved)
{
  grpc_plugin_credentials *c =
    (grpc_plugin_credentials *) gpr_zalloc (sizeof (*c));
  GRPC_API_TRACE ("grpc_metadata_credentials_create_from_plugin(reserved=%p)",
		  1, (reserved));
