...
bool grpc_service_account_jwt_access_credentials::get_request_metadata(
  ...
  gpr_timespec refresh_threshold =
      gpr_time_from_seconds(GRPC_SECURE_TOKEN_REFRESH_THRESHOLD_SECS, GPR_TIMESPAN);

  /* See if we can return a cached jwt. */
  grpc_mdelem jwt_md = GRPC_MDNULL;
  {
    gpr_mu_lock(&cache_mu_);
    if (cached_.service_url != nullptr && strcmp(cached_.service_url, context.service_url) == 0 &&
        !GRPC_MDISNULL(cached_.jwt_md) &&
        (gpr_time_cmp(gpr_time_sub(cached_.jwt_expiration, gpr_now(GPR_CLOCK_REALTIME)),
                      refresh_threshold) > 0)) {
  ...
  if (GRPC_MDISNULL(jwt_md)) {
    ...
    if (jwt != nullptr) {
      ...
      gpr_free(jwt);
      cached_.jwt_expiration = gpr_time_add(gpr_now(GPR_CLOCK_REALTIME), jwt_lifetime_);
      cached_.service_url = gpr_strdup(context.service_url);
      cached_.jwt_md =
          grpc_mdelem_from_slices(grpc_slice_from_static_string(GRPC_AUTHORIZATION_METADATA_KEY),
...
grpc_service_account_jwt_access_credentials::grpc_service_account_jwt_access_credentials(
  ...
  if (gpr_time_cmp(token_lifetime, max_token_lifetime) > 0) {
    gpr_log(GPR_INFO, "Cropping token lifetime to maximum allowed value (%d secs).",
...
grpc_service_account_jwt_access_credentials_create_from_auth_json_key(grpc_auth_json_key key,
  ...
  return grpc_core::MakeRefCounted<grpc_service_account_jwt_access_credentials>(key,
                                                                                token_lifetime);
}
