...
bool grpc_service_account_jwt_access_credentials::get_request_metadata(
  ...
  grpc_mdelem jwt_md = GRPC_MDNULL;
  {
    ...
    // 2) scope is not emtpy AND should_clear_audience_ is a non-zero value.
    if (cached_.service_url != nullptr &&
        (strcmp(cached_.service_url, context.service_url) == 0 ||
         (!scope.empty() && should_clear_audience_)) &&
...
grpc_service_account_jwt_access_credentials_create_from_auth_json_key(
    ...
    std::string user_provided_scope, int should_clear_audience) {
...
grpc_call_credentials* grpc_service_account_jwt_access_credentials_create(
    ...
    const char* user_provided_scope, int should_clear_audience) {
  ...
  return grpc_service_account_jwt_access_credentials_create_from_auth_json_key(
             ...
             user_provided_scope, should_clear_audience)
      .release();
}
