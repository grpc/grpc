...
static void
jwt_destruct (grpc_call_credentials * creds)
{
...
static bool
jwt_get_request_metadata (grpc_call_credentials * creds,
			  ...
			  grpc_error ** error)
{
  ...
  if (GRPC_MDISNULL (jwt_md))
      ...
      if (jwt != nullptr)
	{
	  ...
	  c->cached.jwt_md =
	    grpc_mdelem_from_slices (grpc_slice_from_static_string
...
static void
jwt_cancel_get_request_metadata (grpc_call_credentials * c,
				 grpc_credentials_mdelem_array * md_array,
				 grpc_error * error)
{
...
static char *
redact_private_key (const char *json_key)
{
