...
static grpc_json *
parse_json_part_from_jwt (const char *str, size_t len, grpc_slice * buffer)
{
  grpc_json *json;
  ...
  *buffer = grpc_base64_decode_with_len (str, len, 1);
  ...
  json =
    grpc_json_parse_string_with_len ((char *) GRPC_SLICE_START_PTR (*buffer),
...
static jose_header *
jose_header_from_json (grpc_json * json, grpc_slice buffer)
{
  grpc_json *cur;
...
grpc_jwt_claims *
grpc_jwt_claims_from_json (grpc_json * json, grpc_slice buffer)
{
  grpc_json *cur;
...
verifier_cb_ctx_create (grpc_jwt_verifier * verifier, grpc_pollset * pollset,
			...
			const char *audience, grpc_slice signature,
...
{
  if (ctx->audience != nullptr)
    gpr_free (ctx->audience);
  if (ctx->claims != nullptr)
...
static void
on_keys_retrieved (void *user_data, grpc_error * error)
{
...
static void
on_openid_config_retrieved (void *user_data, grpc_error * error)
{
  const grpc_json *cur;
  verifier_cb_ctx *ctx = (verifier_cb_ctx *) user_data;
...
/* Very non-sophisticated way to detect an email address. Should be good
   enough for now... */
const char *
grpc_jwt_issuer_email_domain (const char *issuer)
{
  ...
  if (dot == nullptr || dot == email_domain)
    return email_domain;
...
retrieve_key_and_verify (verifier_cb_ctx * ctx)
{
  ...
  grpc_httpcli_get (&ctx->verifier->http_ctx, &ctx->pollent, resource_quota,
		    &req,
		    grpc_core::ExecCtx::Get ()->Now () +
		    grpc_jwt_verifier_max_delay, http_cb,
			  ...
			  grpc_jwt_verification_done_cb cb, void *user_data)
{
  ...
  if (GRPC_SLICE_IS_EMPTY (signature))
    goto error;
