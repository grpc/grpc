...
static jose_header* jose_header_from_json(grpc_exec_ctx* exec_ctx,
  ...
  grpc_json* cur;
  jose_header* h =
      reinterpret_cast<jose_header*>(gpr_zalloc(sizeof(jose_header)));
  h->buffer = buffer;
...
grpc_jwt_claims* grpc_jwt_claims_from_json(grpc_exec_ctx* exec_ctx,
  ...
  grpc_jwt_claims* claims =
      reinterpret_cast<grpc_jwt_claims*>(gpr_malloc(sizeof(grpc_jwt_claims)));
...
static verifier_cb_ctx* verifier_cb_ctx_create(
  ...
  verifier_cb_ctx* ctx =
      reinterpret_cast<verifier_cb_ctx*>(gpr_zalloc(sizeof(verifier_cb_ctx)));
...
static void on_keys_retrieved(grpc_exec_ctx* exec_ctx, void* user_data,
                              grpc_error* error) {
  verifier_cb_ctx* ctx = reinterpret_cast<verifier_cb_ctx*>(user_data);
...
const char* grpc_jwt_issuer_email_domain(const char* issuer) {
  ...
  dot = reinterpret_cast<const char*>(gpr_memrchr(
...
void grpc_jwt_verifier_verify(grpc_exec_ctx* exec_ctx,
  ...
  json = parse_json_part_from_jwt(exec_ctx, cur, static_cast<size_t>(dot - cur),
                                  &header_buffer);
  if (json == nullptr) goto error;
...
grpc_jwt_verifier* grpc_jwt_verifier_create(
    ...
    size_t num_mappings) {
  grpc_jwt_verifier* v = reinterpret_cast<grpc_jwt_verifier*>(
      gpr_zalloc(sizeof(grpc_jwt_verifier)));
  grpc_httpcli_context_init(&v->http_ctx);

  /* We know at least of one mapping. */
  v->allocated_mappings = 1 + num_mappings;
  v->mappings = reinterpret_cast<email_key_mapping*>(
