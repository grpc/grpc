...
void
grpc_channel_credentials_unref (grpc_channel_credentials * creds)
{
  if (creds == nullptr)
    return;
...
void
grpc_channel_credentials_release (grpc_channel_credentials * creds)
{
...
grpc_call_credentials_ref (grpc_call_credentials * creds)
{
  if (creds == nullptr)
    return nullptr;
  gpr_ref (&creds->refcount);
...
void
grpc_call_credentials_unref (grpc_call_credentials * creds)
{
  if (creds == nullptr)
    return;
  if (gpr_unref (&creds->refcount))
...
void
grpc_call_credentials_release (grpc_call_credentials * creds)
{
...
bool
grpc_call_credentials_get_request_metadata (grpc_call_credentials * creds,
					    ...
					    grpc_error ** error)
{
  ...
  return creds->vtable->get_request_metadata (creds, pollent, context,
					      md_array, on_request_metadata,
					      error);
}

void
grpc_call_credentials_cancel_get_request_metadata (grpc_call_credentials *
						   creds,
						   grpc_credentials_mdelem_array
						   * md_array,
						   grpc_error * error)
						    ...
						    new_args)
{
  *new_args = nullptr;
  ...
  return channel_creds->vtable->create_security_connector (channel_creds,
							   nullptr, target,
							   args, sc,
							   new_args);
}
  ...
  (grpc_channel_credentials * channel_creds)
{
  if (channel_creds != nullptr && channel_creds->vtable != nullptr &&
    ...
    {
      return channel_creds->vtable->
	duplicate_without_call_credentials (channel_creds);
    }
  else
    {
...
grpc_arg
grpc_channel_credentials_to_arg (grpc_channel_credentials * credentials)
{
...
grpc_channel_credentials *
grpc_channel_credentials_from_arg (const grpc_arg * arg)
{
  if (strcmp (arg->key, GRPC_ARG_CHANNEL_CREDENTIALS))
    return nullptr;
...
grpc_channel_credentials *
grpc_channel_credentials_find_in_args (const grpc_channel_args * args)
{
  size_t i;
  if (args == nullptr)
    return nullptr;
...
grpc_server_credentials_set_auth_metadata_processor (grpc_server_credentials *
						     ...
						     processor)
{
  GRPC_API_TRACE ("grpc_server_credentials_set_auth_metadata_processor("
		  "creds=%p, "
		  "processor=grpc_auth_metadata_processor { process: %p, state: %p })",
		  3, (creds, (void *) (intptr_t) processor.process,
		      processor.state));
  if (creds == nullptr)
    return;
  if (creds->processor.destroy != nullptr &&
...
static void
server_credentials_pointer_arg_destroy (void *p)
{
  grpc_server_credentials_unref ((grpc_server_credentials *) p);
}

static void *
server_credentials_pointer_arg_copy (void *p)
{
  return grpc_server_credentials_ref ((grpc_server_credentials *) p);
}

static int
server_credentials_pointer_cmp (void *a, void *b)
{
  return GPR_ICMP (a, b);
}
...
grpc_arg
grpc_server_credentials_to_arg (grpc_server_credentials * p)
{
  return grpc_channel_arg_pointer_create ((char *)
					  GRPC_SERVER_CREDENTIALS_ARG, p,
					  &cred_ptr_vtable);
}

grpc_server_credentials *
grpc_server_credentials_from_arg (const grpc_arg * arg)
{
  if (strcmp (arg->key, GRPC_SERVER_CREDENTIALS_ARG) != 0)
    return nullptr;
  if (arg->type != GRPC_ARG_POINTER)
    {
      gpr_log (GPR_ERROR, "Invalid type %d for arg %s", arg->type,
...
grpc_server_credentials *
grpc_find_server_credentials_in_args (const grpc_channel_args * args)
{
  size_t i;
  if (args == nullptr)
    return nullptr;
