...
StringMatcher::StringMatcher(Type type, const std::string& matcher,
                             bool case_sensitive,
                             bool use_case_sensitive_in_regex)
    : type_(type),
      case_sensitive_(case_sensitive),
      use_case_sensitive_in_regex_(use_case_sensitive_in_regex) {
  if (type_ == Type::SAFE_REGEX) {
    RE2::Options options;
    if (use_case_sensitive_in_regex) {
...
StringMatcher::StringMatcher(const StringMatcher& other)
  ...
  switch (type_) {
    case Type::SAFE_REGEX: {
      RE2::Options options;
      if (other.use_case_sensitive_in_regex_) {
...
bool StringMatcher::Match(absl::string_view value) const {
  switch (type_) {
    ...
    case StringMatcher::Type::SAFE_REGEX:
      // case_sensitive_ is ignored for SAFE_REGEX except for path_specifier.
