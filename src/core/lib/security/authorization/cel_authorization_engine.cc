...
CelAuthorizationEngine::CreateCelAuthorizationEngine(
  ...
  if (rbac_policies.empty() || rbac_policies.size() > 2) {
    gpr_log(GPR_ERROR,
            "Invalid rbac policies vector. Must contain either one or "
            "two rbac "
            "policies.");
    return nullptr;
  } else if (rbac_policies.size() == 2 &&
             (envoy_config_rbac_v3_RBAC_action(rbac_policies[0]) !=
                  kDeny ||
              envoy_config_rbac_v3_RBAC_action(rbac_policies[1]) !=
                  kAllow)) {
    gpr_log(GPR_ERROR,
...
CelAuthorizationEngine::CelAuthorizationEngine(
  ...
  for (const auto& rbac_policy : rbac_policies) {
    // Extract array of policies and store their condition fields in
    // either allow_if_matched_ or deny_if_matched_, depending on the
    // policy action.
    ...
    while ((policy_entry = envoy_config_rbac_v3_RBAC_policies_next(
      ...
      // Parse condition to make a pointer tied to the lifetime of
      // arena_.
      size_t serial_len;
      ...
      if (envoy_config_rbac_v3_RBAC_action(rbac_policy) == kAllow) {
        allow_if_matched_.insert(
            std::make_pair(policy_name, parsed_condition));
      } else {
        deny_if_matched_.insert(
            std::make_pair(policy_name, parsed_condition));
      }
    }
  }
...
std::unique_ptr<mock_cel::Activation>
CelAuthorizationEngine::CreateActivation(const EvaluateArgs& args) {
    ...
    if (elem == kUrlPath) {
      ...
      if (!url_path.empty()) {
        activation->InsertValue(
            kUrlPath, mock_cel::CelValue::CreateStringView(url_path));
      }
    } else if (elem == kHost) {
      absl::string_view host(args.GetHost());
      if (!host.empty()) {
        activation->InsertValue(
            kHost, mock_cel::CelValue::CreateStringView(host));
      }
    } else if (elem == kMethod) {
      absl::string_view method(args.GetMethod());
      if (!method.empty()) {
        activation->InsertValue(
            kMethod, mock_cel::CelValue::CreateStringView(method));
      }
        ...
        if (header_item != headers.end()) {
          header_items.push_back(
              std::pair<mock_cel::CelValue, mock_cel::CelValue>(
                  ...
                  mock_cel::CelValue::CreateStringView(
                      header_item->second)));
        }
      }
      headers_ = mock_cel::ContainerBackedMapImpl::Create(
          absl::Span<std::pair<mock_cel::CelValue, mock_cel::CelValue>>(
              header_items));
      activation->InsertValue(
          kHeaders, mock_cel::CelValue::CreateMap(headers_.get()));
      ...
      activation->InsertValue(
          kSourcePort,
          mock_cel::CelValue::CreateInt64(args.GetPeerPort()));
    } else if (elem == kDestinationAddress) {
      absl::string_view destination_address(
          args.GetLocalAddressString());
