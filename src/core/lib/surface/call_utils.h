...
class OpHandlerImpl {
 ...
 public:
  void ToProto(grpc_channelz_v2_Promise* promise_proto,
    ...
    char* type_ptr = static_cast<char*>(upb_Arena_Malloc(arena, type.size()));
    memcpy(type_ptr, type.data(), type.size());
    grpc_channelz_v2_Promise_Custom_set_type(
        custom_promise, upb_StringView_FromDataAndSize(type_ptr, type.size()));
 ...
 public:
  void ToProto(grpc_channelz_v2_Promise* promise_proto,
    ...
    auto* custom_promise =
        grpc_channelz_v2_Promise_mutable_custom_promise(promise_proto, arena);
    grpc_channelz_v2_Promise_Custom_set_type(
        custom_promise, StdStringToUpbString("WaitForCqEndOp"));
    channelz::PropertyList properties;
    std::visit(
