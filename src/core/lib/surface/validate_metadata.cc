...
static grpc_error *
conforms_to (grpc_slice slice, const uint8_t * legal_bits,
	     const char *err_desc)
{
      ...
      if ((legal_bits[byte] & (1 << bit)) == 0)
	{
	  ...
	  grpc_error *error =
	    grpc_error_set_str (grpc_error_set_int
				...
				grpc_slice_from_copied_string (dump));
	  gpr_free (dump);
	  return error;
	}
    }
...
grpc_error *
grpc_validate_header_key_is_legal (grpc_slice slice)
{
...
int
grpc_header_key_is_legal (grpc_slice slice)
{
  return error2int (grpc_validate_header_key_is_legal (slice));
}

grpc_error *
grpc_validate_header_nonbin_value_is_legal (grpc_slice slice)
{
  static const uint8_t legal_header_bits[256 / 8] = {
...
int
grpc_header_nonbin_value_is_legal (grpc_slice slice)
{
  return error2int (grpc_validate_header_nonbin_value_is_legal (slice));
}

int
grpc_is_binary_header (grpc_slice slice)
{
  if (GRPC_SLICE_LENGTH (slice) < 5)
    return 0;
