...
int
grpc_byte_buffer_reader_init (grpc_byte_buffer_reader * reader,
  ...
  switch (reader->buffer_in->type)
      ...
      if (is_compressed (reader->buffer_in))
	{
	  if (grpc_msg_decompress (reader->buffer_in->data.raw.compression,
	    ...
	    {			/* all fine */
	      reader->buffer_out =
		grpc_raw_byte_buffer_create (decompressed_slices_buffer.
					     slices,
...
int
grpc_byte_buffer_reader_next (grpc_byte_buffer_reader * reader,
			      grpc_slice * slice)
{
  switch (reader->buffer_in->type)
    {
	...
	if (reader->current.index < slice_buffer->count)
	  {
	    *slice =
	      grpc_slice_ref_internal (slice_buffer->
				       slices[reader->current.index]);
