...
static void fail_call (grpc_server * server, size_t cq_idx,
...
static void
shutdown_cleanup (void *arg, grpc_error * error)
{
...
static void
send_shutdown (grpc_channel * channel, bool send_goaway,
	       grpc_error * send_disconnect)
{
  ...
  op->goaway_error =
    ...
    grpc_error_set_int (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			("Server shutdown"), GRPC_ERROR_INT_GRPC_STATUS,
...
static void
channel_broadcaster_shutdown (channel_broadcaster * cb,
			      bool send_goaway, grpc_error * force_disconnect)
{
  size_t i;
...
static void
request_matcher_init (request_matcher * rm, grpc_server * server)
{
  memset (rm, 0, sizeof (*rm));
  rm->server = server;
  rm->requests_per_cq =
    (gpr_locked_mpscq *) gpr_malloc (sizeof (*rm->requests_per_cq) *
				     server->cq_count);
...
static void
request_matcher_destroy (request_matcher * rm)
{
...
static void
kill_zombie (void *elem, grpc_error * error)
{
  grpc_call_unref (grpc_call_from_top_element ((grpc_call_element *) elem));
}

static void
request_matcher_zombify_all_pending_calls (request_matcher * rm)
{
  while (rm->pending_head)
    {
      ...
      GRPC_CLOSURE_INIT (&calld->kill_zombie_closure, kill_zombie,
			 grpc_call_stack_element (grpc_call_get_call_stack
						  (calld->call), 0),
			 grpc_schedule_on_exec_ctx);
...
static void
request_matcher_kill_requests (grpc_server * server,
			       request_matcher * rm, grpc_error * error)
{
  requested_call *rc;
  for (size_t i = 0; i < server->cq_count; i++)
    {
      while ((rc =
	      (requested_call *) gpr_locked_mpscq_pop (&rm->
						       requests_per_cq[i])) !=
	     nullptr)
	{
...
static void
server_delete (grpc_server * server)
{
...
static void
finish_destroy_channel (void *cd, grpc_error * error)
{
...
static void
destroy_channel (channel_data * chand, grpc_error * error)
{
...
static void
done_request_event (void *req, grpc_cq_completion * c)
{
  gpr_free (req);
}

static void
publish_call (grpc_server * server, call_data * calld, size_t cq_idx,
	      requested_call * rc)
{
...
static void
publish_new_rpc (void *arg, grpc_error * error)
{
  ...
  for (size_t i = 0; i < server->cq_count; i++)
    {
      ...
      requested_call *rc =
	(requested_call *) gpr_locked_mpscq_try_pop (&rm->
						     requests_per_cq[cq_idx]);
      if (rc == nullptr)
	{
	  continue;
	}
...
static void
finish_start_new_rpc (grpc_server * server, grpc_call_element * elem,
		      request_matcher * rm,
		      ...
		      payload_handling)
{
  ...
  switch (payload_handling)
	...
	GRPC_CLOSURE_INIT (&calld->publish, publish_new_rpc, elem,
			   grpc_schedule_on_exec_ctx);
	grpc_call_start_batch_and_execute (calld->call, &op, 1,
					   &calld->publish);
	break;
      }
    }
...
static void
start_new_rpc (grpc_call_element * elem)
{
  ...
  if (chand->registered_methods && calld->path_set && calld->host_set)
      ...
      for (i = 0; i <= chand->registered_method_max_probes; i++)
	{
	  ...
	  if (!rm->has_host)
	    continue;
	  if (!grpc_slice_eq (rm->host, calld->host))
	    continue;
	  if (!grpc_slice_eq (rm->method, calld->path))
	    continue;
...
static void
done_shutdown_event (void *server, grpc_cq_completion * completion)
{
  server_unref ((grpc_server *) server);
}
...
static void
kill_pending_work_locked (grpc_server * server, grpc_error * error)
{
  if (server->started)
    {
      request_matcher_kill_requests (server,
				     &server->unregistered_request_matcher,
				     GRPC_ERROR_REF (error));
      request_matcher_zombify_all_pending_calls (&server->
...
static void
maybe_finish_shutdown (grpc_server * server)
{
  size_t i;
  if (!gpr_atm_acq_load (&server->shutdown_flag)
...
static void
server_on_recv_initial_metadata (void *ptr, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
	...
	grpc_slice_ref_internal (GRPC_MDVALUE
				 (calld->recv_initial_metadata->idx.named.
				  path->md));
      calld->host =
	grpc_slice_ref_internal (GRPC_MDVALUE
				 (calld->recv_initial_metadata->idx.named.
...
static void
server_mutate_op (grpc_call_element * elem,
...
static void
server_start_transport_stream_op_batch (grpc_call_element * elem,
					grpc_transport_stream_op_batch * op)
{
...
static void
got_initial_metadata (void *ptr, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) ptr;
  call_data *calld = (call_data *) elem->call_data;
  if (error == GRPC_ERROR_NONE)
...
static void
accept_stream (void *cd, grpc_transport * transport,
	       const void *transport_server_data)
{
  ...
  GRPC_CLOSURE_INIT (&calld->got_initial_metadata, got_initial_metadata, elem,
		     grpc_schedule_on_exec_ctx);
  grpc_call_start_batch_and_execute (call, &op, 1,
				     &calld->got_initial_metadata);
}

static void
channel_connectivity_changed (void *cd, grpc_error * error)
{
  channel_data *chand = (channel_data *) cd;
  grpc_server *server = chand->server;
  if (chand->connectivity_state != GRPC_CHANNEL_SHUTDOWN)
    {
      ...
      grpc_channel_next_op (grpc_channel_stack_element
			    (grpc_channel_get_channel_stack (chand->channel),
			     0), op);
    }
  else
    {
...
static void
register_completion_queue (grpc_server * server,
			   grpc_completion_queue * cq, void *reserved)
{
  ...
  n = server->cq_count++;
  server->cqs =
    (grpc_completion_queue **) gpr_realloc (server->cqs,
					    server->cq_count *
...
void
grpc_server_register_completion_queue (grpc_server * server,
				       grpc_completion_queue * cq,
				       void *reserved)
{
  GRPC_API_TRACE
    ("grpc_server_register_completion_queue(server=%p, cq=%p, reserved=%p)",
...
void *
grpc_server_register_method (grpc_server * server, const char *method,
			     const char *host,
			     grpc_server_register_method_payload_handling
			     payload_handling, uint32_t flags)
{
  registered_method *m;
  GRPC_API_TRACE
    ("grpc_server_register_method(server=%p, method=%s, host=%s, "
     "flags=0x%08x)", 4, (server, method, host, flags));
  if (!method)
    {
      gpr_log (GPR_ERROR,
...
static void
start_listeners (void *s, grpc_error * error)
{
...
grpc_server_start (grpc_server * server)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE (start_listeners, server,
...
void
grpc_server_get_pollsets (grpc_server * server, grpc_pollset *** pollsets,
			  size_t * pollset_count)
{
...
void
grpc_server_setup_transport (grpc_server * s, grpc_transport * transport,
			     ...
			     const grpc_channel_args * args)
{
  ...
  chand =
    (channel_data *)
    grpc_channel_stack_element (grpc_channel_get_channel_stack (channel),
				0)->channel_data;
  chand->server = s;
  server_ref (s);
  ...
  for (cq_idx = 0; cq_idx < s->cq_count; cq_idx++)
    {
      if (grpc_cq_pollset (s->cqs[cq_idx]) == accepting_pollset)
	break;
    }
  ...
  if (num_registered_methods > 0)
	...
	(channel_registered_method *) gpr_zalloc (alloc);
      for (rm = s->registered_methods; rm; rm = rm->next)
	{
	  grpc_slice host;
	  bool has_host;
	  ...
	  if (rm->host != nullptr)
	    {
	      host =
		grpc_slice_intern (grpc_slice_from_static_string (rm->host));
	      has_host = true;
	    }
	  else
	    {
	  ...
	  method =
	    grpc_slice_intern (grpc_slice_from_static_string (rm->method));
	  hash =
	    GRPC_MDSTR_KV_HASH (has_host ? grpc_slice_hash (host) : 0,
				grpc_slice_hash (method));
	  for (probes = 0;
	       chand->registered_methods[(hash + probes) %
					 slots].server_registered_method !=
	       nullptr; probes++)
	    ;
...
void
done_published_shutdown (void *done_arg, grpc_cq_completion * storage)
{
  (void) done_arg;
  gpr_free (storage);
}
...
static void
listener_destroy_done (void *s, grpc_error * error)
{
  grpc_server *server = (grpc_server *) s;
  gpr_mu_lock (&server->mu_global);
...
void
grpc_server_shutdown_and_notify (grpc_server * server,
				 grpc_completion_queue * cq, void *tag)
{
  ...
  GRPC_API_TRACE ("grpc_server_shutdown_and_notify(server=%p, cq=%p, tag=%p)",
		  3, (server, cq, tag));
  ...
  if (server->shutdown_published)
    {
      grpc_cq_end_op (cq, tag, GRPC_ERROR_NONE, done_published_shutdown,
		      nullptr,
		      (grpc_cq_completion *)
  ...
  server->shutdown_tags = (shutdown_tag *) gpr_realloc (server->shutdown_tags,
							sizeof (shutdown_tag)
							*
							(server->
							 num_shutdown_tags +
...
grpc_server_cancel_all_calls (grpc_server * server)
{
  ...
  channel_broadcaster_shutdown (&broadcaster, false /* send_goaway */ ,
				GRPC_ERROR_CREATE_FROM_STATIC_STRING
...
void
grpc_server_add_listener (grpc_server * server, void *arg,
...
queue_call_request (grpc_server * server, size_t cq_idx, requested_call * rc)
{
	  ...
	  if (!gpr_atm_full_cas (&calld->state, PENDING, ACTIVATED))
	    {
	      ...
	      GRPC_CLOSURE_INIT (&calld->kill_zombie_closure, kill_zombie,
				 ...
				 (grpc_call_get_call_stack (calld->call), 0),
				 grpc_schedule_on_exec_ctx);
	      GRPC_CLOSURE_SCHED (&calld->kill_zombie_closure,
				  GRPC_ERROR_NONE);
	    }
			  ...
			  void *tag)
{
  ...
  GRPC_API_TRACE ("grpc_server_request_call("
...
grpc_call_error
grpc_server_request_registered_call (grpc_server * server, void *rmp,
				     grpc_call ** call,
				     gpr_timespec * deadline,
				     grpc_metadata_array * initial_metadata,
				     grpc_byte_buffer ** optional_payload,
				     ...
				     cq_for_notification, void *tag)
{
  ...
  GRPC_STATS_INC_SERVER_REQUESTED_CALLS ();
  GRPC_API_TRACE ("grpc_server_request_registered_call("
		  "server=%p, rmp=%p, call=%p, deadline=%p, initial_metadata=%p, "
		  "optional_payload=%p, cq_bound_to_call=%p, cq_for_notification=%p, "
		  "tag=%p)",
		  9,
		  (server, rmp, call, deadline, initial_metadata,
...
static void
fail_call (grpc_server * server, size_t cq_idx, requested_call * rc,
	   grpc_error * error)
{
  *rc->call = nullptr;
