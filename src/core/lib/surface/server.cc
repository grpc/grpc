...
// RPCs that come in from the transport must be matched against RPC
// requests from the application. An incoming request from the
// application can be matched to an RPC that has already arrived or can
// be queued up for later use. Likewise, an RPC coming in from the
// transport can either be matched to a request that already arrived
// from the application or can be queued up for later use (marked
// pending). If there is a match, the request's tag is posted on the
// request's notification CQ.
//
// RequestMatcherInterface is the base class to provide this
// functionality.
class Server::RequestMatcherInterface {
 public:
  virtual ~RequestMatcherInterface() {}

  // Unref the calls associated with any incoming RPCs in the pending
  // queue (not yet matched to an application-requested RPC).
  virtual void ZombifyPending() = 0;

  // Mark all application-requested RPCs failed if they have not been
  // matched to an incoming RPC. The error parameter indicates why the
  // RPCs are being failed (always server shutdown in all current
  // implementations).
  virtual void KillRequests(grpc_error_handle error) = 0;

  // How many request queues are supported by this matcher. This is an
  // abstract concept that essentially maps to gRPC completion queues.
  virtual size_t request_queue_count() const = 0;

  // This function is invoked when the application requests a new RPC
  // whose information is in the call parameter. The request_queue_index
  // marks the queue onto which to place this RPC, and is typically
  // associated with a gRPC CQ. If there are pending RPCs waiting to be
  // matched, publish one (match it and notify the CQ).
  virtual void RequestCallWithPossiblePublish(
      size_t request_queue_index, RequestedCall* call) = 0;

  // This function is invoked on an incoming RPC, represented by the
  // calld object. The RequestMatcher will try to match it against an
  // application-requested RPC if possible or will place it in the
  // pending queue otherwise. To enable some measure of fairness between
  // server CQs, the match is done starting at the
  // start_request_queue_index parameter in a cyclic order rather than
...
// The RealRequestMatcher is an implementation of
// RequestMatcherInterface that actually uses all the features of
// RequestMatcherInterface: expecting the application to explicitly
// request RPCs and then matching those to incoming RPCs, along with a
// slow path by which incoming RPCs are put on a locked pending list if
// they aren't able to be matched to an application request.
class Server::RealRequestMatcher : public RequestMatcherInterface {
 public:
  ...
  ~RealRequestMatcher() override {
    for (LockedMultiProducerSingleConsumerQueue& queue :
         requests_per_cq_) {
    ...
    if (requests_per_cq_[request_queue_index].Push(&call->mpscq_node)) {
      ...
      while (true) {
        ...
        if (!next_pending.calld->MaybeActivate()) {
          ...
          next_pending.calld->Publish(request_queue_index,
                                      next_pending.rc);
        }
      }
    }
  ...
  void MatchOrQueue(size_t start_request_queue_index,
    ...
    for (size_t i = 0; i < requests_per_cq_.size(); i++) {
      ...
      RequestedCall* rc = reinterpret_cast<RequestedCall*>(
          requests_per_cq_[cq_idx].TryPop());
      if (rc != nullptr) {
    ...
    // We need to ensure that all the queues are empty.  We do this
    // under the server mu_call_ lock to ensure that if something is
    // added to an empty request queue, it will block until the call is
    // actually added to the pending list.
    RequestedCall* rc = nullptr;
    size_t cq_idx = 0;
    size_t loop_count;
    {
      MutexLock lock(&server_->mu_call_);
      for (loop_count = 0; loop_count < requests_per_cq_.size();
           loop_count++) {
        cq_idx = (start_request_queue_index + loop_count) %
                 requests_per_cq_.size();
        rc = reinterpret_cast<RequestedCall*>(
            requests_per_cq_[cq_idx].Pop());
        if (rc != nullptr) {
          break;
        }
      }
...
// AllocatingRequestMatchers don't allow the application to request an
// RPC in advance or queue up any incoming RPC for later match. Instead,
// MatchOrQueue will call out to an allocation function passed in at the
// construction of the object. These request matchers are designed for
// the C++ callback API, so they only support 1 completion queue (passed
// in at the constructor). They are also used for the sync API.
class Server::AllocatingRequestMatcherBase
    : public RequestMatcherInterface {
 public:
  AllocatingRequestMatcherBase(Server* server,
...
// An allocating request matcher for non-registered methods (used for
// generic API and unimplemented RPCs).
class Server::AllocatingRequestMatcherBatch
    : public AllocatingRequestMatcherBase {
 public:
  AllocatingRequestMatcherBatch(
      Server* server, grpc_completion_queue* cq,
  ...
  void MatchOrQueue(size_t /*start_request_queue_index*/,
    ...
    if (server()->ShutdownRefOnRequest()) {
      ...
      RequestedCall* rc = new RequestedCall(
          static_cast<void*>(call_info.tag), call_info.cq,
          call_info.call, call_info.initial_metadata,
          call_info.details);
      calld->SetState(CallData::CallState::ACTIVATED);
      calld->Publish(cq_idx(), rc);
    } else {
...
class ChannelBroadcaster {
 public:
  // This can have an empty constructor and destructor since we want to
  // control when the actual setup and shutdown broadcast take place.
...
grpc_resource_user* CreateDefaultResourceUser(
  ...
  if (started_ && config_fetcher_ != nullptr &&
    ...
    for (grpc_pollset* pollset : pollsets_) {
      grpc_pollset_set_del_pollset(
          config_fetcher_->interested_parties(), pollset);
    }
  }
...
void Server::Start() {
  ...
  if (unregistered_request_matcher_ == nullptr) {
    unregistered_request_matcher_ =
        absl::make_unique<RealRequestMatcher>(this);
  }
  ...
  // Register the interested parties from the config fetcher to the cq
  // pollsets before starting listeners so that config fetcher is being
  // polled when the listeners start watch the fetcher.
...
grpc_error_handle Server::SetupTransport(
  ...
  if (cq_idx == cqs_.size()) {
    // Completion queue not found.  Pick a random one to publish new
    // calls to.
...
Server::RegisteredMethod* Server::RegisterMethod(
  ...
  if ((flags & ~GRPC_INITIAL_METADATA_USED_MASK) != 0) {
    gpr_log(GPR_ERROR,
            "grpc_server_register_method invalid flags 0x%08x", flags);
    return nullptr;
  }
...
void DonePublishedShutdown(void* /*done_arg*/,
...
// - Kills all pending requests-for-incoming-RPC-calls (i.e., the
// requests made
//   via grpc_server_request_call() and
//   grpc_server_request_registered_call() will now be cancelled). See
//   KillPendingWorkLocked().
//
// - Shuts down the listeners (i.e., the server will no longer listen on
// the
//   port for new incoming channels).
//
// - Iterates through all channels on the server and sends shutdown msg
// (see
//   ChannelBroadcaster::BroadcastShutdown() for details) to the clients
//   via the transport layer. The transport layer then guarantees the
//   following:
//    -- Sends shutdown to the client (e.g., HTTP2 transport sends
//    GOAWAY).
//    -- If the server has outstanding calls that are in the process,
//    the
//       connection is NOT closed until the server is done with all
//       those calls.
//    -- Once there are no more calls in progress, the channel is
//    closed.
void Server::ShutdownAndNotify(grpc_completion_queue* cq, void* tag) {
  ChannelBroadcaster broadcaster;
  {
    ...
    if (shutdown_published_) {
      grpc_cq_end_op(cq, tag, GRPC_ERROR_NONE, DonePublishedShutdown,
...
void Server::Orphan() {
  ...
  if (default_resource_user_ != nullptr) {
    grpc_resource_quota_unref(
        grpc_resource_user_quota(default_resource_user_));
...
grpc_call_error Server::ValidateServerRequestAndCq(
    size_t* cq_idx, grpc_completion_queue* cq_for_notification,
  ...
  grpc_call_error error = ValidateServerRequest(
      cq_for_notification, tag, optional_payload, rm);
...
grpc_call_error Server::QueueRequestedCall(size_t cq_idx,
                                           RequestedCall* rc) {
  if (ShutdownCalled()) {
    FailCall(cq_idx, rc,
             GRPC_ERROR_CREATE_FROM_STATIC_STRING("Server Shutdown"));
...
grpc_call_error Server::RequestCall(
  ...
  RequestedCall* rc = new RequestedCall(tag, cq_bound_to_call, call,
...
grpc_call_error Server::RequestRegisteredCall(
  ...
  RequestedCall* rc = new RequestedCall(tag_new, cq_bound_to_call, call,
                                        request_metadata, rm, deadline,
                                        optional_payload);
  return QueueRequestedCall(cq_idx, rc);
}
...
Server::ChannelData::~ChannelData() {
  ...
  if (server_ != nullptr) {
    if (server_->channelz_node_ != nullptr &&
        channelz_socket_uuid_ != 0) {
...
void Server::ChannelData::InitTransport(RefCountedPtr<Server> server,
  ...
  // Build a lookup table phrased in terms of mdstr's in this channels
  // context to quickly find registered methods.
  ...
  if (num_registered_methods > 0) {
    ...
    for (std::unique_ptr<RegisteredMethod>& rm :
  ...
  op->start_connectivity_watch =
      MakeOrphanable<ConnectivityWatcher>(this);
...
Server::ChannelRegisteredMethod*
Server::ChannelData::GetRegisteredMethod(const grpc_slice& host,
...
void Server::ChannelData::AcceptStream(
    void* arg, grpc_transport* /*transport*/,
...
void Server::ChannelData::Destroy() {
  ...
  server_->Ref().release();
  server_->MaybeFinishShutdown();
  GRPC_CLOSURE_INIT(&finish_destroy_channel_closure_, FinishDestroy,
  ...
  grpc_channel_next_op(grpc_channel_stack_element(
                           grpc_channel_get_channel_stack(channel_), 0),
                       op);
}
...
void Server::ChannelData::DestroyChannelElement(
    grpc_channel_element* elem) {
...
void Server::CallData::Start(grpc_call_element* elem) {
  ...
  grpc_call_start_batch_and_execute(
      call_, &op, 1, &recv_initial_metadata_batch_complete_);
}
...
void Server::CallData::Publish(size_t cq_idx, RequestedCall* rc) {
  ...
  *rc->call = call_;
  ...
  GPR_SWAP(grpc_metadata_array, *rc->initial_metadata,
           initial_metadata_);
  switch (rc->type) {
    case RequestedCall::Type::BATCH_CALL:
...
void Server::CallData::PublishNewRpc(void* arg,
  ...
  auto* chand =
      static_cast<Server::ChannelData*>(call_elem->channel_data);
...
void Server::CallData::StartTransportStreamOpBatchImpl(
  ...
  if (batch->recv_initial_metadata) {
    GPR_ASSERT(batch->payload->recv_initial_metadata.recv_flags ==
               nullptr);
  ...
  if (batch->recv_trailing_metadata) {
    ...
    batch->payload->recv_trailing_metadata
        .recv_trailing_metadata_ready = &recv_trailing_metadata_ready_;
  }
...
void Server::CallData::RecvInitialMetadataReady(
  ...
  if (error == GRPC_ERROR_NONE) {
    GPR_DEBUG_ASSERT(calld->recv_initial_metadata_->idx.named.path !=
                     nullptr);
    GPR_DEBUG_ASSERT(
        calld->recv_initial_metadata_->idx.named.authority != nullptr);
    calld->path_.emplace(grpc_slice_ref_internal(GRPC_MDVALUE(
        calld->recv_initial_metadata_->idx.named.path->md)));
    calld->host_.emplace(grpc_slice_ref_internal(GRPC_MDVALUE(
        calld->recv_initial_metadata_->idx.named.authority->md)));
  ...
  if (calld->seen_recv_trailing_metadata_ready_) {
    GRPC_CALL_COMBINER_START(
        calld->call_combiner_, &calld->recv_trailing_metadata_ready_,
...
void Server::CallData::RecvTrailingMetadataReady(
    void* arg, grpc_error_handle error) {
  grpc_call_element* elem = static_cast<grpc_call_element*>(arg);
  CallData* calld = static_cast<CallData*>(elem->call_data);
...
grpc_server* grpc_server_create(const grpc_channel_args* args,
  ...
  c_server->core_server =
      grpc_core::MakeOrphanable<grpc_core::Server>(args);
  return c_server;
}
...
void grpc_server_register_completion_queue(grpc_server* server,
                                           ...
                                           void* reserved) {
  GRPC_API_TRACE(
      "grpc_server_register_completion_queue(server=%p, cq=%p, "
      "reserved=%p)",
  ...
  if (cq_type != GRPC_CQ_NEXT && cq_type != GRPC_CQ_CALLBACK) {
    ...
    /* Ideally we should log an error and abort but
       ruby-wrapped-language API
...
void* grpc_server_register_method(
  ...
  return server->core_server->RegisterMethod(method, host,
                                             payload_handling, flags);
}
...
void grpc_server_cancel_all_calls(grpc_server* server) {
  grpc_core::ApplicationCallbackExecCtx callback_exec_ctx;
  grpc_core::ExecCtx exec_ctx;
  GRPC_API_TRACE("grpc_server_cancel_all_calls(server=%p)", 1,
                 (server));
...
grpc_call_error grpc_server_request_call(
  ...
  return server->core_server->RequestCall(
      call, details, request_metadata, cq_bound_to_call,
      cq_for_notification, tag);
}
...
grpc_call_error grpc_server_request_registered_call(
  ...
  grpc_core::ExecCtx exec_ctx;
  GRPC_STATS_INC_SERVER_REQUESTED_CALLS();
  auto* rm = static_cast<grpc_core::Server::RegisteredMethod*>(
      registered_method);
  GRPC_API_TRACE(
      "grpc_server_request_registered_call("
      "server=%p, registered_method=%p, call=%p, deadline=%p, "
      "request_metadata=%p, "
      "optional_payload=%p, cq_bound_to_call=%p, "
      "cq_for_notification=%p, "
      "tag=%p)",
      9,
      (server, registered_method, call, deadline, request_metadata,
       optional_payload, cq_bound_to_call, cq_for_notification,
       tag_new));
  return server->core_server->RequestRegisteredCall(
      rm, call, deadline, request_metadata, optional_payload,
...
void grpc_server_set_config_fetcher(
  ...
  grpc_core::ExecCtx exec_ctx;
  GRPC_API_TRACE(
      "grpc_server_set_config_fetcher(server=%p, config_fetcher=%p)", 2,
