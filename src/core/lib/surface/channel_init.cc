...
void
grpc_channel_init_register_stage (grpc_channel_stack_type type,
				  int priority,
  ...
  if (g_slots[type].cap_slots == g_slots[type].num_slots)
    {
      ...
      g_slots[type].slots = (stage_slot *) gpr_realloc (g_slots[type].slots,
							g_slots[type].
							cap_slots *
							sizeof (*g_slots
								[type].
...
void
grpc_channel_init_finalize (void)
{
  GPR_ASSERT (!g_finalized);
  for (int i = 0; i < GRPC_NUM_CHANNEL_STACK_TYPES; i++)
    {
      qsort (g_slots[i].slots, g_slots[i].num_slots,
	     sizeof (*g_slots[i].slots), compare_slots);
    }
  g_finalized = true;
}

void
grpc_channel_init_shutdown (void)
{
  for (int i = 0; i < GRPC_NUM_CHANNEL_STACK_TYPES; i++)
    {
...
bool
grpc_channel_init_create_stack (grpc_channel_stack_builder * builder,
