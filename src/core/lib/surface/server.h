...
class Server : public InternallyRefCounted<Server> {
 public:
  ...
  // approaches here.
  class ConfigFetcherInterface : public RefCounted<ConfigFetcherInterface> {
   public:
    ...
    virtual ~ConfigFetcherInterface() = default;

    virtual void StartWatch(const grpc_channel_args& args,
    ...
    // TODO(roth): We currently need to know that all the watchers are
    // registered before we actually start fetching, due to limitations
    // of the way that we're using xDS for the gRPC server.  Once we've
    // switched to the new xDS naming scheme, this method can go away.
  ...
  ConfigFetcherInterface* config_fetcher() const {
    return config_fetcher_.get();
  }
  void set_config_fetcher(
      RefCountedPtr<ConfigFetcherInterface> config_fetcher) {
 ...
 private:
  ...
  RefCountedPtr<ConfigFetcherInterface> config_fetcher_;
...
struct grpc_server_config_fetcher {
  grpc_core::RefCountedPtr<grpc_core::Server::ConfigFetcherInterface> impl;
};
