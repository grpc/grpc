...
class Server : public InternallyRefCounted<Server> {
 public:
  ...
  /// Implementations must override the Orphan() method, which should
  /// stop listening and initiate destruction of the listener.
  class ListenerInterface : public Orphanable {
   public:
    ~ListenerInterface() override = default;

    /// Starts listening. This listener may refer to the pollset object
    /// beyond this call, so it is a pointer rather than a reference.
    ...
    /// Sets a closure to be invoked by the listener when its
  ...
  // vector itself is immutable, but the pollsets inside are mutable.
  // The result is valid for the lifetime of the server.
  ...
  // Adds a listener to the server.  When the server starts, it will
  // call the listener's Start() method, and when it shuts down, it will
  // orphan the listener.
  ...
  // Sets up a transport.  Creates a channel stack and binds the
  // transport to the server.  Called from the listener when a new
  ...
  // unregistered collection should use a specific allocator for request
  // matching.
  void SetRegisteredMethodAllocator(
      grpc_completion_queue* cq, void* method_tag,
      std::function<RegisteredCallAllocation()> allocator);
  void SetBatchMethodAllocator(
      grpc_completion_queue* cq,
 ...
 private:
  ...
  class ChannelData {
   ...
   private:
    ...
    // The index into Server::cqs_ of the CQ used as a starting point
    // for where to publish new incoming calls.
    size_t cq_idx_;
    ...
    // TODO(vjpai): Convert this to an STL map type as opposed to a
    // direct bucket implementation. (Consider performance impact, hash
    // function to use, etc.)
  ...
  class CallData {
   public:
    ...
    CallData(grpc_call_element* elem,
   ...
   private:
    ...
    static void RecvInitialMetadataBatchComplete(
    ...
    void StartTransportStreamOpBatchImpl(
        grpc_call_element* elem, grpc_transport_stream_op_batch* batch);
  ...
  static void DoneRequestEvent(void* req,
                               grpc_cq_completion* completion);
  ...
  // Take a shutdown ref for a request (increment by 2) and return if
  ...
  // Decrement the shutdown ref counter by either 1 (for shutdown call)
  // or 2 (for in-flight request) and possibly call MaybeFinishShutdown
  // if appropriate.
  ...
  // Returns whether there are no more shutdown refs, which means that
  // shutdown has been called and all accepted requests have been
  // published if using an AllocatingRequestMatcher.
  ...
  // mu_global_ controls access to non-call-related state (e.g., channel
  // state). mu_call_ controls access to call-related state (e.g., the
  // call lists).
  //
  ...
  // startup synchronization: flag is protected by mu_global_, signals
  // whether we are doing the listener start routine or not.
  ...
  std::unique_ptr<RequestMatcherInterface>
      unregistered_request_matcher_;

  // The shutdown refs counter tracks whether or not shutdown has been
  // called and whether there are any AllocatingRequestMatcher requests
  // that have been accepted but not yet started (+2 on each one). If
  // shutdown has been called, the lowest bit will be 0 (defaults to 1)
  // and the counter will be even. The server should not notify on
  // shutdown until the counter is 0 (shutdown is called and there are
  // no requests that are accepted but not started).
...
// TODO(roth): Eventually, will need a way to modify configuration even
// after a connection is established (e.g., to change things like L7
// rate limiting, RBAC, and fault injection configs).  One possible
// option would be to do something like ServiceConfig and
// ConfigSelector, but that might add unnecessary per-call overhead.
// Need to consider other approaches here.
struct grpc_server_config_fetcher {
 public:
  class ConnectionManager
      : public grpc_core::RefCounted<ConnectionManager> {
   public:
    // Ownership of \a args is transfered.
    virtual absl::StatusOr<grpc_channel_args*>
  ...
  class WatcherInterface {
   public:
    ...
    // UpdateConnectionManager() is invoked by the config fetcher when a
    // new config is available. Implementations should update the
    // connection manager and start serving if not already serving.
    virtual void UpdateConnectionManager(
        grpc_core::RefCountedPtr<ConnectionManager> manager) = 0;
    // Implementations should stop serving when this is called. Serving
    // should only resume when UpdateConfig() is invoked.
  ...
  virtual void StartWatch(
      std::string listening_address, grpc_channel_args* args,
