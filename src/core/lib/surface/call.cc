...
struct batch_control {
  batch_control() { grpc_core::RefInit(&steps_to_complete, 0); }
...
void grpc_call_ref(grpc_call* c) { grpc_core::Ref(&c->ext_ref); }

void grpc_call_unref(grpc_call* c) {
  if (!grpc_core::Unref(&c->ext_ref)) return;
...
static void finish_batch_step(batch_control* bctl) {
  if (grpc_core::Unref(&bctl->steps_to_complete)) {
...
static grpc_call_error call_start_batch(grpc_call* call, const grpc_op* ops,
  ...
  grpc_core::RefInit(&bctl->steps_to_complete,
                     (has_send_ops ? 1 : 0) + num_recv_ops);
