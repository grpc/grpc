...
struct grpc_call {
  ...
  ~grpc_call() { gpr_free(static_cast<void*>(const_cast<char*>(final_info.error_string))); }
...
#define CALL_STACK_FROM_CALL(call) \
  (grpc_call_stack*)((char*)(call) + GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_call)))
#define CALL_FROM_CALL_STACK(call_stack) \
  (grpc_call*)(((char*)(call_stack)) - GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_call)))

#define CALL_ELEM_FROM_CALL(call, idx) grpc_call_stack_element(CALL_STACK_FROM_CALL(call), idx)
#define CALL_FROM_TOP_ELEM(top_elem) \
  CALL_FROM_CALL_STACK(grpc_call_stack_from_top_element(top_elem))

static void execute_batch(grpc_call* call, grpc_transport_stream_op_batch* batch,
                          grpc_closure* start_batch_closure);

static void cancel_with_status(grpc_call* c, grpc_status_code status, const char* description);
...
static void add_init_error(grpc_error_handle* composite, grpc_error_handle new_err) {
...
void* grpc_call_arena_alloc(grpc_call* call, size_t size) { return call->arena->Alloc(size); }

static parent_call* get_or_create_parent_call(grpc_call* call) {
  parent_call* p = reinterpret_cast<parent_call*>(gpr_atm_acq_load(&call->parent_call_atm));
  if (p == nullptr) {
    p = call->arena->New<parent_call>();
    if (!gpr_atm_rel_cas(&call->parent_call_atm, reinterpret_cast<gpr_atm>(nullptr),
                         reinterpret_cast<gpr_atm>(p))) {
      p->~parent_call();
      p = reinterpret_cast<parent_call*>(gpr_atm_acq_load(&call->parent_call_atm));
    }
  }
  return p;
}
...
static parent_call* get_parent_call(grpc_call* call) {
  return reinterpret_cast<parent_call*>(gpr_atm_acq_load(&call->parent_call_atm));
}
...
grpc_error_handle grpc_call_create(const grpc_call_create_args* args, grpc_call** out_call) {
  ...
  grpc_channel_stack* channel_stack = grpc_channel_get_channel_stack(args->channel);
  size_t initial_size = grpc_channel_get_call_size_estimate(args->channel);
  GRPC_STATS_INC_CALL_INITIAL_SIZE(initial_size);
  size_t call_and_stack_size =
      GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(grpc_call)) + channel_stack->call_stack_size;
  size_t call_alloc_size = call_and_stack_size + (args->parent ? sizeof(child_call) : 0);
  ...
  *out_call = call;
  ...
  if (call->is_client) {
    ...
    GPR_ASSERT(args->add_initial_metadata_count < MAX_SEND_EXTRA_METADATA_COUNT);
    for (size_t i = 0; i < args->add_initial_metadata_count; i++) {
      call->send_extra_metadata[i].md = args->add_initial_metadata[i];
      if (grpc_slice_eq_static_interned(GRPC_MDKEY(args->add_initial_metadata[i]),
                                        GRPC_MDSTR_PATH)) {
        path = grpc_slice_ref_internal(GRPC_MDVALUE(args->add_initial_metadata[i]));
      }
    }
  ...
  if (args->parent != nullptr) {
    call->child = new (reinterpret_cast<char*>(arena_with_call.second) + call_and_stack_size)
    ...
    if (args->propagation_mask & GRPC_PROPAGATE_CENSUS_TRACING_CONTEXT) {
      if (0 == (args->propagation_mask & GRPC_PROPAGATE_CENSUS_STATS_CONTEXT)) {
        add_init_error(&error,
                       GRPC_ERROR_CREATE_FROM_STATIC_STRING("Census tracing propagation requested "
      ...
      add_init_error(&error,
                     GRPC_ERROR_CREATE_FROM_STATIC_STRING("Census context propagation requested "
  ...
  grpc_call_element_args call_args = {CALL_STACK_FROM_CALL(call),
                                      ...
                                      call->arena,
                                      &call->call_combiner};
  add_init_error(&error, grpc_call_stack_init(channel_stack, 1, destroy_call, call, &call_args));
  ...
  if (args->parent != nullptr) {
    ...
    if (pc->first_child == nullptr) {
      ...
      cc->sibling_next->child->sibling_prev = cc->sibling_prev->child->sibling_next = call;
    }
  ...
  if (args->cq != nullptr) {
    ...
    call->pollent = grpc_polling_entity_create_from_pollset(grpc_cq_pollset(args->cq));
  }
  if (args->pollset_set_alternative != nullptr) {
    call->pollent = grpc_polling_entity_create_from_pollset_set(args->pollset_set_alternative);
  }
  if (!grpc_polling_entity_is_empty(&call->pollent)) {
    grpc_call_stack_set_pollset_or_pollset_set(CALL_STACK_FROM_CALL(call), &call->pollent);
  }
...
static void destroy_call(void* call, grpc_error_handle /*error*/) {
  ...
  grpc_call* c = static_cast<grpc_call*>(call);
  for (i = 0; i < 2; i++) {
    grpc_metadata_batch_destroy(&c->metadata_batch[1 /* is_receiving */][i /* is_initial */]);
  }
  ...
  grpc_error_get_status(status_error, c->send_deadline, &c->final_info.final_status, nullptr,
                        nullptr, &(c->final_info.error_string));
  GRPC_ERROR_UNREF(status_error);
  c->final_info.stats.latency = gpr_cycle_counter_sub(gpr_get_cycle_counter(), c->start_time);
  grpc_call_stack_destroy(
      CALL_STACK_FROM_CALL(c), &c->final_info,
      GRPC_CLOSURE_INIT(&c->release_call, release_call, c, grpc_schedule_on_exec_ctx));
}
...
// the filter stack.
static void execute_batch_in_call_combiner(void* arg, grpc_error_handle /*ignored*/) {
...
static void execute_batch(grpc_call* call, grpc_transport_stream_op_batch* batch,
  ...
  GRPC_CALL_COMBINER_START(&call->call_combiner, start_batch_closure, GRPC_ERROR_NONE,
                           "executing batch");
}

char* grpc_call_get_peer(grpc_call* call) {
  char* peer_string = reinterpret_cast<char*>(gpr_atm_acq_load(&call->peer_string));
...
grpc_call_error grpc_call_cancel_with_status(grpc_call* c, grpc_status_code status,
...
static void done_termination(void* arg, grpc_error_handle /*error*/) {
  ...
  GRPC_CALL_COMBINER_STOP(&state->call->call_combiner, "on_complete for cancel_stream op");
...
static void cancel_with_error(grpc_call* c, grpc_error_handle error) {
  ...
  state->call = c;
  GRPC_CLOSURE_INIT(&state->finish_batch, done_termination, state, grpc_schedule_on_exec_ctx);
  grpc_transport_stream_op_batch* op = grpc_make_transport_stream_op(&state->finish_batch);
  op->cancel_stream = true;
  op->payload->cancel_stream.cancel_error = error;
  execute_batch(c, op, &state->start_batch);
}

void grpc_call_cancel_internal(grpc_call* call) { cancel_with_error(call, GRPC_ERROR_CANCELLED); }

static grpc_error_handle error_from_status(grpc_status_code status, const char* description) {
...
static void set_final_status(grpc_call* call, grpc_error_handle error) {
  ...
  if (call->is_client) {
    grpc_error_get_status(error, call->send_deadline, call->final_op.client.status,
                          call->final_op.client.status_details, nullptr,
    ...
    *call->final_op.server.cancelled =
    ...
    grpc_core::channelz::ServerNode* channelz_node =
        call->final_op.server.core_server->channelz_node();
    if (channelz_node != nullptr) {
      if (*call->final_op.server.cancelled || reinterpret_cast<grpc_error_handle>(gpr_atm_acq_load(
...
static void set_incoming_message_compression_algorithm(grpc_call* call,
                                                       grpc_message_compression_algorithm algo) {
  GPR_ASSERT(algo < GRPC_MESSAGE_COMPRESS_ALGORITHMS_COUNT);
  call->incoming_message_compression_algorithm = algo;
}

static void set_incoming_stream_compression_algorithm(grpc_call* call,
                                                      grpc_stream_compression_algorithm algo) {
...
static grpc_compression_algorithm compression_algorithm_for_level_locked(
  ...
  return grpc_compression_algorithm_for_level(level, call->encodings_accepted_by_peer);
}
...
static void set_encodings_accepted_by_peer(grpc_call* /*call*/, grpc_mdelem mdel,
  ...
  accepted_user_data = grpc_mdelem_get_user_data(mdel, destroy_encodings_accepted_by_peer);
  if (accepted_user_data != nullptr) {
    *encodings_accepted_by_peer =
        static_cast<uint32_t>(reinterpret_cast<uintptr_t>(accepted_user_data) - 1);
    return;
  }
  ...
  *encodings_accepted_by_peer = 0;
  ...
  for (i = 0; i < accept_encoding_parts.count; i++) {
    ...
    if (r) {
      ...
      char* accept_encoding_entry_str = grpc_slice_to_c_string(accept_encoding_entry_slice);
      gpr_log(GPR_DEBUG, "Unknown entry in accept encoding metadata: '%s'. Ignoring.",
  ...
  grpc_mdelem_set_user_data(
      ...
      reinterpret_cast<void*>(static_cast<uintptr_t>(*encodings_accepted_by_peer) + 1));
}
...
grpc_stream_compression_algorithm grpc_call_test_only_get_incoming_stream_encodings(
    grpc_call* call) {
...
static grpc_metadata* get_md_elem(grpc_metadata* metadata, grpc_metadata* additional_metadata,
                                  int i, int count) {
  grpc_metadata* res = i < count ? &metadata[i] : &additional_metadata[i - count];
  GPR_ASSERT(res);
  return res;
}

static int prepare_application_metadata(grpc_call* call, int count, grpc_metadata* metadata,
                                        int is_trailing, int prepend_extra_metadata,
                                        grpc_metadata* additional_metadata,
                                        int additional_metadata_count) {
  int total_count = count + additional_metadata_count;
  int i;
  grpc_metadata_batch* batch = &call->metadata_batch[0 /* is_receiving */][is_trailing];
  for (i = 0; i < total_count; i++) {
    grpc_metadata* md = get_md_elem(metadata, additional_metadata, i, count);
    grpc_linked_mdelem* l = linked_from_md(md);
    GPR_ASSERT(sizeof(grpc_linked_mdelem) == sizeof(md->internal_data));
    if (!GRPC_LOG_IF_ERROR("validate_metadata", grpc_validate_header_key_is_legal(md->key))) {
      break;
    } else if (!grpc_is_binary_header_internal(md->key) &&
               !GRPC_LOG_IF_ERROR("validate_metadata",
    ...
    if (call->send_extra_metadata_count == 0) {
      ...
      for (i = 0; i < call->send_extra_metadata_count; i++) {
        GRPC_LOG_IF_ERROR("prepare_application_metadata",
                          grpc_metadata_batch_link_tail(batch, &call->send_extra_metadata[i]));
      }
    }
  }
  for (i = 0; i < total_count; i++) {
...
static void publish_app_metadata(grpc_call* call, grpc_metadata_batch* b, int is_trailing) {
  ...
  if (dest->count + b->list.count > dest->capacity) {
    dest->capacity = GPR_MAX(dest->capacity + b->list.count, dest->capacity * 3 / 2);
...
static void recv_initial_filter(grpc_call* call, grpc_metadata_batch* b) {
  ...
  call->encodings_accepted_by_peer = grpc_compression_bitset_from_message_stream_compression_bitset(
      message_encodings_accepted_by_peer, stream_encodings_accepted_by_peer);
...
static void recv_trailing_filter(void* args, grpc_metadata_batch* b,
  ...
  if (batch_error != GRPC_ERROR_NONE) {
    ...
    grpc_status_code status_code = grpc_get_status_code_from_metadata(b->idx.named.grpc_status->md);
    grpc_error_handle error = GRPC_ERROR_NONE;
    ...
    gpr_log(GPR_DEBUG, "Received trailing metadata with no error and no status");
    set_final_status(call,
                     grpc_error_set_int(GRPC_ERROR_CREATE_FROM_STATIC_STRING("No status received"),
...
grpc_call_stack* grpc_call_get_call_stack(grpc_call* call) { return CALL_STACK_FROM_CALL(call); }
...
static bool are_write_flags_valid(uint32_t flags) {
  ...
  const uint32_t allowed_write_positions = (GRPC_WRITE_USED_MASK | GRPC_WRITE_INTERNAL_USED_MASK);
...
static batch_control* reuse_or_allocate_batch_control(grpc_call* call, const grpc_op* ops) {
...
static void finish_batch_completion(void* user_data, grpc_cq_completion* /*storage*/) {
...
static void reset_batch_errors(batch_control* bctl) {
  GRPC_ERROR_UNREF(reinterpret_cast<grpc_error_handle>(gpr_atm_acq_load(&bctl->batch_error)));
  gpr_atm_rel_store(&bctl->batch_error, reinterpret_cast<gpr_atm>(GRPC_ERROR_NONE));
}
...
static void post_batch_completion(batch_control* bctl) {
  ...
  grpc_error_handle error =
      GRPC_ERROR_REF(reinterpret_cast<grpc_error_handle>(gpr_atm_acq_load(&bctl->batch_error)));

  if (bctl->op.send_initial_metadata) {
    grpc_metadata_batch_destroy(&call->metadata_batch[0 /* is_receiving */][0 /* is_trailing */]);
  }
  if (bctl->op.send_message) {
    if (bctl->op.payload->send_message.stream_write_closed && error == GRPC_ERROR_NONE) {
  ...
  if (bctl->op.send_trailing_metadata) {
    grpc_metadata_batch_destroy(&call->metadata_batch[0 /* is_receiving */][1 /* is_trailing */]);
  }
  ...
  if (error != GRPC_ERROR_NONE && bctl->op.recv_message && *call->receiving_buffer != nullptr) {
  ...
  if (bctl->completion_data.notify_tag.is_closure) {
    ...
    grpc_core::Closure::Run(
        DEBUG_LOCATION, static_cast<grpc_closure*>(bctl->completion_data.notify_tag.tag), error);
    GRPC_CALL_INTERNAL_UNREF(call, "completion");
  } else {
    /* unrefs error */
...
static void continue_receiving_slices(batch_control* bctl) {
  ...
  for (;;) {
    size_t remaining =
        call->receiving_stream->length() - (*call->receiving_buffer)->data.raw.slice_buffer.length;
    if (remaining == 0) {
...
static void receiving_slice_ready(void* bctlp, grpc_error_handle error) {
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (error == GRPC_ERROR_NONE) {
      grpc_slice_buffer_add(&(*call->receiving_buffer)->data.raw.slice_buffer, slice);
  ...
  if (call->receiving_stream == nullptr) {
    *call->receiving_buffer = nullptr;
    ...
    if ((call->receiving_stream->flags() & GRPC_WRITE_INTERNAL_COMPRESS) &&
      ...
      *call->receiving_buffer = grpc_raw_compressed_byte_buffer_create(nullptr, 0, algo);
    } else {
...
static void receiving_stream_ready(void* bctlp, grpc_error_handle error) {
  ...
  if (error != GRPC_ERROR_NONE) {
    call->receiving_stream.reset();
    if (reinterpret_cast<grpc_error_handle>(gpr_atm_acq_load(&bctl->batch_error)) ==
        GRPC_ERROR_NONE) {
      gpr_atm_rel_store(&bctl->batch_error, reinterpret_cast<gpr_atm>(GRPC_ERROR_REF(error)));
    }
    cancel_with_error(call, GRPC_ERROR_REF(error));
  }
  ...
  if (error != GRPC_ERROR_NONE || call->receiving_stream == nullptr ||
      !gpr_atm_rel_cas(&call->recv_state, RECV_NONE, reinterpret_cast<gpr_atm>(bctlp))) {
...
static void receiving_stream_ready_in_call_combiner(void* bctlp, grpc_error_handle error) {
  batch_control* bctl = static_cast<batch_control*>(bctlp);
  grpc_call* call = bctl->call;
...
static void GPR_ATTRIBUTE_NOINLINE handle_both_stream_and_msg_compression_set(grpc_call* call) {
...
static void GPR_ATTRIBUTE_NOINLINE handle_error_parsing_compression_algorithm(grpc_call* call) {
  std::string error_msg = absl::StrFormat(
...
handle_invalid_compression(grpc_call* call, grpc_compression_algorithm compression_algorithm) {
  std::string error_msg =
      absl::StrFormat("Invalid compression algorithm value '%d'.", compression_algorithm);
  gpr_log(GPR_ERROR, "%s", error_msg.c_str());
...
static void GPR_ATTRIBUTE_NOINLINE handle_compression_algorithm_disabled(
  ...
  std::string error_msg = absl::StrFormat("Compression algorithm '%s' is disabled.", algo_name);
  gpr_log(GPR_ERROR, "%s", error_msg.c_str());
  cancel_with_status(call, GRPC_STATUS_UNIMPLEMENTED, error_msg.c_str());
}
...
static void validate_filtered_metadata(batch_control* bctl) {
  ...
  grpc_call* call = bctl->call;
  if (GPR_UNLIKELY(call->incoming_stream_compression_algorithm != GRPC_STREAM_COMPRESS_NONE &&
                   call->incoming_message_compression_algorithm != GRPC_MESSAGE_COMPRESS_NONE)) {
    handle_both_stream_and_msg_compression_set(call);
  } else if (GPR_UNLIKELY(grpc_compression_algorithm_from_message_stream_compression_algorithm(
    ...
    } else if (GPR_UNLIKELY(grpc_compression_options_is_algorithm_enabled_internal(
    ...
    if (GPR_UNLIKELY(!GPR_BITGET(call->encodings_accepted_by_peer, compression_algorithm))) {
...
static void receiving_initial_metadata_ready(void* bctlp, grpc_error_handle error) {
  batch_control* bctl = static_cast<batch_control*>(bctlp);
  grpc_call* call = bctl->call;
  ...
  if (error == GRPC_ERROR_NONE) {
    grpc_metadata_batch* md = &call->metadata_batch[1 /* is_receiving */][0 /* is_trailing */];
  ...
  while (true) {
    ...
    if (rsr_bctlp == 0) {
       ...
       * received first. */
      if (gpr_atm_no_barrier_cas(&call->recv_state, RECV_NONE, RECV_INITIAL_METADATA_FIRST)) {
        break;
      }
    } else {
      /* Already received messages */
      saved_rsr_closure = GRPC_CLOSURE_CREATE(receiving_stream_ready, (batch_control*)rsr_bctlp,
                                              grpc_schedule_on_exec_ctx);
  ...
  if (saved_rsr_closure != nullptr) {
    grpc_core::Closure::Run(DEBUG_LOCATION, saved_rsr_closure, GRPC_ERROR_REF(error));
  }

  finish_batch_step(bctl);
}

static void receiving_trailing_metadata_ready(void* bctlp, grpc_error_handle error) {
  batch_control* bctl = static_cast<batch_control*>(bctlp);
  grpc_call* call = bctl->call;
...
static grpc_call_error call_start_batch(grpc_call* call, const grpc_op* ops, size_t nops,
                                        void* notify_tag, int is_notify_tag_closure) {
  ...
  if (nops == 0) {
    if (!is_notify_tag_closure) {
      ...
      grpc_cq_end_op(call->cq, notify_tag, GRPC_ERROR_NONE, free_no_op_completion, nullptr,
                     static_cast<grpc_cq_completion*>(gpr_malloc(sizeof(grpc_cq_completion))));
    } else {
      grpc_core::Closure::Run(DEBUG_LOCATION, static_cast<grpc_closure*>(notify_tag),
                              GRPC_ERROR_NONE);
    }
    error = GRPC_CALL_OK;
    goto done;
  ...
  bctl->completion_data.notify_tag.is_closure = static_cast<uint8_t>(is_notify_tag_closure != 0);
    ...
    switch (op->op) {
      case GRPC_OP_SEND_INITIAL_METADATA: {
        ...
        if (op->data.send_initial_metadata.maybe_compression_level.is_set) {
          ...
          const grpc_compression_options copts = grpc_channel_compression_options(call->channel);
        ...
        if (level_set && !call->is_client) {
          const grpc_compression_algorithm calgo =
              compression_algorithm_for_level_locked(call, effective_compression_level);
        ...
        if (op->data.send_initial_metadata.count + additional_metadata_count > INT_MAX) {
        ...
        if (!prepare_application_metadata(
                ...
                op->data.send_initial_metadata.metadata, 0, call->is_client, &compression_md,
        ...
        stream_op_payload->send_initial_metadata.send_initial_metadata_flags = op->flags;
        if (call->is_client) {
          stream_op_payload->send_initial_metadata.peer_string = &call->peer_string;
        }
        has_send_ops = true;
        break;
      }
      case GRPC_OP_SEND_MESSAGE: {
        ...
        if (op->data.send_message.send_message->data.raw.compression > GRPC_COMPRESS_NONE) {
          flags |= GRPC_WRITE_INTERNAL_COMPRESS;
        }
        stream_op->send_message = true;
        call->sending_message = true;
        call->sending_stream.Init(&op->data.send_message.send_message->data.raw.slice_buffer,
                                  flags);
        stream_op_payload->send_message.send_message.reset(call->sending_stream.get());
        has_send_ops = true;
        break;
      }
      case GRPC_OP_SEND_CLOSE_FROM_CLIENT: {
      ...
      case GRPC_OP_SEND_STATUS_FROM_SERVER: {
        ...
        if (op->data.send_status_from_server.trailing_metadata_count > INT_MAX) {
          error = GRPC_CALL_ERROR_INVALID_METADATA;
          goto done_with_error;
        }
        stream_op->send_trailing_metadata = true;
        ...
        call->send_extra_metadata[0].md =
            grpc_get_reffed_status_elem(op->data.send_status_from_server.status);
        grpc_error_handle status_error =
            op->data.send_status_from_server.status == GRPC_STATUS_OK
                ? GRPC_ERROR_NONE
                : grpc_error_set_int(
                      GRPC_ERROR_CREATE_FROM_STATIC_STRING("Server returned error"),
                      GRPC_ERROR_INT_GRPC_STATUS,
                      static_cast<intptr_t>(op->data.send_status_from_server.status));
        if (op->data.send_status_from_server.status_details != nullptr) {
          call->send_extra_metadata[1].md = grpc_mdelem_from_slices(
              GRPC_MDSTR_GRPC_MESSAGE,
              grpc_slice_ref_internal(*op->data.send_status_from_server.status_details));
          call->send_extra_metadata_count++;
          if (status_error != GRPC_ERROR_NONE) {
            char* msg = grpc_slice_to_c_string(GRPC_MDVALUE(call->send_extra_metadata[1].md));
            status_error = grpc_error_set_str(status_error, GRPC_ERROR_STR_GRPC_MESSAGE,
                                              grpc_slice_from_copied_string(msg));
            gpr_free(msg);
          }
        }

        gpr_atm_rel_store(&call->status_error, reinterpret_cast<gpr_atm>(status_error));
        if (!prepare_application_metadata(
                call, static_cast<int>(op->data.send_status_from_server.trailing_metadata_count),
                op->data.send_status_from_server.trailing_metadata, 1, 1, nullptr, 0)) {
        ...
        stream_op_payload->send_trailing_metadata.send_trailing_metadata =
            &call->metadata_batch[0 /* is_receiving */][1 /* is_trailing */];
        stream_op_payload->send_trailing_metadata.sent = &call->sent_server_trailing_metadata;
        has_send_ops = true;
        break;
      }
      case GRPC_OP_RECV_INITIAL_METADATA: {
        ...
        call->buffered_metadata[0] = op->data.recv_initial_metadata.recv_initial_metadata;
        GRPC_CLOSURE_INIT(&call->receiving_initial_metadata_ready, receiving_initial_metadata_ready,
        ...
        if (call->is_client) {
          ...
          stream_op_payload->recv_initial_metadata.peer_string = &call->peer_string;
        }
        ++num_recv_ops;
        break;
      }
      case GRPC_OP_RECV_MESSAGE: {
        ...
        GRPC_CLOSURE_INIT(&call->receiving_stream_ready, receiving_stream_ready_in_call_combiner,
                          bctl, grpc_schedule_on_exec_ctx);
        stream_op_payload->recv_message.recv_message_ready = &call->receiving_stream_ready;
        ++num_recv_ops;
        break;
      }
      case GRPC_OP_RECV_STATUS_ON_CLIENT: {
        ...
        call->buffered_metadata[1] = op->data.recv_status_on_client.trailing_metadata;
        call->final_op.client.status = op->data.recv_status_on_client.status;
        call->final_op.client.status_details = op->data.recv_status_on_client.status_details;
        call->final_op.client.error_string = op->data.recv_status_on_client.error_string;
      ...
      case GRPC_OP_RECV_CLOSE_ON_SERVER: {
        ...
        call->requested_final_op = true;
        call->final_op.server.cancelled = op->data.recv_close_on_server.cancelled;
        stream_op->recv_trailing_metadata = true;
        stream_op_payload->recv_trailing_metadata.recv_trailing_metadata =
            &call->metadata_batch[1 /* is_receiving */][1 /* is_trailing */];
        stream_op_payload->recv_trailing_metadata.collect_stats =
  ...
  if (has_send_ops) {
    GRPC_CLOSURE_INIT(&bctl->finish_batch, finish_batch, bctl, grpc_schedule_on_exec_ctx);
...
grpc_compression_algorithm grpc_call_compression_for_level(grpc_call* call,
  ...
  grpc_compression_algorithm algo = compression_algorithm_for_level_locked(call, level);
  return algo;
}

bool grpc_call_is_trailers_only(const grpc_call* call) {
  bool result = call->is_trailers_only;
  GPR_DEBUG_ASSERT(!result ||
                   call->metadata_batch[1 /* is_receiving */][0 /* is_trailing */].list.count == 0);
  return result;
}
