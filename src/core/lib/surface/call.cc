...
static gpr_atm
pack_received_status (received_status r)
{
  return r.is_set ? (1 | (gpr_atm) r.error) : 0;
}

static received_status
unpack_received_status (gpr_atm atm)
{
  if ((atm & 1) == 0)
    {
      return
      {
      false, GRPC_ERROR_NONE};
    }
  else
    {
      return
      {
      true, (grpc_error *) (atm & ~(gpr_atm) 1)};
    }
}
...
static void
add_init_error (grpc_error ** composite, grpc_error * new_err)
{
...
void *
grpc_call_arena_alloc (grpc_call * call, size_t size)
{
  return gpr_arena_alloc (call->arena, size);
}

static parent_call *
get_or_create_parent_call (grpc_call * call)
{
...
static parent_call *
get_parent_call (grpc_call * call)
{
  return (parent_call *) gpr_atm_acq_load (&call->parent_call_atm);
}

grpc_error *
grpc_call_create (const grpc_call_create_args * args, grpc_call ** out_call)
{
  size_t i, j;
  ...
  call =
    (grpc_call *) gpr_arena_alloc (arena,
				   sizeof (grpc_call) +
  ...
  *out_call = call;
      ...
      for (i = 0; i < args->add_initial_metadata_count; i++)
	{
	  ...
	  if (grpc_slice_eq (GRPC_MDKEY (args->add_initial_metadata[i]),
		...
		grpc_slice_ref_internal (GRPC_MDVALUE
					 (args->add_initial_metadata[i]));
	    }
	}
      call->send_extra_metadata_count =
	(int) args->add_initial_metadata_count;
    }
  else
    {
  ...
  if (args->parent != nullptr)
      ...
      if (args->propagation_mask & GRPC_PROPAGATE_CENSUS_TRACING_CONTEXT)
	{
	  ...
	  grpc_call_context_set (call, GRPC_CONTEXT_TRACING,
				 args->parent->context[GRPC_CONTEXT_TRACING].
				 value, nullptr);
	}
  ...
  add_init_error (&error,
		  grpc_call_stack_init (channel_stack, 1, destroy_call, call,
					&call_args));
  ...
  if (immediately_cancel)
    {
      cancel_with_error (call, STATUS_FROM_API_OVERRIDE,
  ...
  if (args->pollset_set_alternative != nullptr)
    {
      call->pollent =
	grpc_polling_entity_create_from_pollset_set (args->
...
void
grpc_call_set_completion_queue (grpc_call * call, grpc_completion_queue * cq)
{
  GPR_ASSERT (cq);
...
#endif
void
grpc_call_internal_ref (grpc_call * c REF_ARG)
{
  GRPC_CALL_STACK_REF (CALL_STACK_FROM_CALL (c), REF_REASON);
}

void
grpc_call_internal_unref (grpc_call * c REF_ARG)
{
  GRPC_CALL_STACK_UNREF (CALL_STACK_FROM_CALL (c), REF_REASON);
}

static void
release_call (void *call, grpc_error * error)
{
...
static void set_status_value_directly (grpc_status_code status, void *dest);
static void
destroy_call (void *call, grpc_error * error)
{
  size_t i;
  int ii;
  grpc_call *c = (grpc_call *) call;
  ...
  for (i = 0; i < 2; i++)
    {
      grpc_metadata_batch_destroy (&c->
				   metadata_batch[1 /* is_receiving */ ][i
									 /* is_initial */
									 ]);
    }
  ...
  for (i = 0; i < STATUS_SOURCE_COUNT; i++)
    {
      GRPC_ERROR_UNREF (unpack_received_status
			(gpr_atm_acq_load (&c->status[i])).error);
    }

  grpc_call_stack_destroy (CALL_STACK_FROM_CALL (c), &c->final_info,
			   GRPC_CLOSURE_INIT (&c->release_call, release_call,
					      c, grpc_schedule_on_exec_ctx));
  GPR_TIMER_END ("destroy_call", 0);
}

void
grpc_call_ref (grpc_call * c)
{
  gpr_ref (&c->ext_ref);
}

void
grpc_call_unref (grpc_call * c)
{
  if (!gpr_unref (&c->ext_ref))
    return;
...
// the filter stack.
static void
execute_batch_in_call_combiner (void *arg, grpc_error * ignored)
{
...
grpc_call_error
grpc_call_cancel_with_status (grpc_call * c,
			      grpc_status_code status,
			      const char *description, void *reserved)
{
  grpc_core::ExecCtx exec_ctx;
  GRPC_API_TRACE ("grpc_call_cancel_with_status("
...
static void
done_termination (void *arg, grpc_error * error)
{
...
static void
cancel_with_error (grpc_call * c, status_source source, grpc_error * error)
{
...
static grpc_error *
error_from_status (grpc_status_code status, const char *description)
{
...
static void
cancel_with_status (grpc_call * c, status_source source,
...
static bool
get_final_status_from (grpc_call * call, grpc_error * error,
...
static void
get_final_status (grpc_call * call,
		  ...
		  const char **error_string)
{
  ...
  for (i = 0; i < STATUS_SOURCE_COUNT; i++)
    {
      status[i] =
	unpack_received_status (gpr_atm_acq_load (&call->status[i]));
    }
  if (grpc_call_error_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "get_final_status %s",
	  ...
	  if (status[i].is_set)
	    {
	      gpr_log (GPR_DEBUG, "  %d: %s", i,
		       grpc_error_string (status[i].error));
	    }
	}
    }
  ...
  for (int allow_ok_status = 0; allow_ok_status < 2; allow_ok_status++)
      ...
      for (i = 0; i < STATUS_SOURCE_COUNT; i++)
	{
	  if (status[i].is_set &&
	    ...
	    {
	      if (get_final_status_from
		  (call, status[i].error, allow_ok_status != 0, set_value,
...
static void
set_status_from_error (grpc_call * call, status_source source,
		       grpc_error * error)
{
  if (!gpr_atm_rel_cas (&call->status[source], pack_received_status (
								      {
								      false,
								      GRPC_ERROR_NONE}
			), pack_received_status (
						  {
						  true, error}
			)))
    {
...
static void
set_incoming_compression_algorithm (grpc_call * call,
...
static void
set_incoming_stream_compression_algorithm (grpc_call * call,
...
stream_compression_algorithm_for_level_locked (grpc_call * call,
...
uint32_t
grpc_call_test_only_get_message_flags (grpc_call * call)
{
  uint32_t flags;
...
static void
destroy_encodings_accepted_by_peer (void *p)
{
  return;
}

static void
set_encodings_accepted_by_peer (grpc_call * call, grpc_mdelem mdel)
{
  size_t i;
  grpc_compression_algorithm algorithm;
  ...
  grpc_mdelem_set_user_data (mdel, destroy_encodings_accepted_by_peer,
			     (void
			      *) (((uintptr_t) call->
...
static void
set_stream_encodings_accepted_by_peer (grpc_call * call, grpc_mdelem mdel)
{
  size_t i;
  ...
  grpc_mdelem_set_user_data (mdel, destroy_encodings_accepted_by_peer,
			     (void
			      *) (((uintptr_t) call->
				   stream_encodings_accepted_by_peer) + 1));
}

uint32_t
grpc_call_test_only_get_encodings_accepted_by_peer (grpc_call * call)
{
  uint32_t encodings_accepted_by_peer;
...
uint32_t
grpc_call_test_only_get_stream_encodings_accepted_by_peer (grpc_call * call)
{
  uint32_t stream_encodings_accepted_by_peer;
...
static grpc_linked_mdelem *
linked_from_md (const grpc_metadata * md)
{
  return (grpc_linked_mdelem *) & md->internal_data;
}

static grpc_metadata *
get_md_elem (grpc_metadata * metadata,
	     grpc_metadata * additional_metadata, int i, int count)
{
  grpc_metadata *res =
...
static int
prepare_application_metadata (grpc_call * call, int count,
	  ...
	  for (i = 0; i < call->send_extra_metadata_count; i++)
	    {
	      GRPC_LOG_IF_ERROR ("prepare_application_metadata",
				 grpc_metadata_batch_link_tail (batch,
								&call->
								send_extra_metadata
								[i]));
	    }
  ...
  if (algorithm == GRPC_COMPRESS_ALGORITHMS_COUNT)
    {
      ...
      gpr_log (GPR_ERROR,
	       ...
	       "incoming data as uncompressed.", md_c_str);
      gpr_free (md_c_str);
...
static grpc_stream_compression_algorithm
decode_stream_compression (grpc_mdelem md)
{
  grpc_stream_compression_algorithm algorithm =
...
static void
publish_app_metadata (grpc_call * call, grpc_metadata_batch * b,
		      int is_trailing)
{
  if (b->list.count == 0)
    return;
  ...
  if (dest->count + b->list.count > dest->capacity)
	...
	GPR_MAX (dest->capacity + b->list.count, dest->capacity * 3 / 2);
      dest->metadata =
	(grpc_metadata *) gpr_realloc (dest->metadata,
...
static void
recv_initial_filter (grpc_call * call, grpc_metadata_batch * b)
{
  if (b->idx.named.content_encoding != nullptr)
      ...
      if (b->idx.named.grpc_encoding != nullptr)
	{
      ...
      set_incoming_stream_compression_algorithm (call,
						 decode_stream_compression
						 (b->idx.named.
						  content_encoding->md));
  ...
  else if (b->idx.named.grpc_encoding != nullptr)
    {
      ...
      set_incoming_compression_algorithm (call,
					  decode_compression (b->idx.named.
							      grpc_encoding->
							      md));
...
static void
recv_trailing_filter (void *args, grpc_metadata_batch * b)
{
...
static bool
are_write_flags_valid (uint32_t flags)
{
...
static bool
are_initial_metadata_flags_valid (uint32_t flags, bool is_client)
{
  /* check that only bits in GRPC_WRITE_(INTERNAL?)_USED_MASK are set */
...
static int
batch_slot_for_op (grpc_op_type type)
{
  switch (type)
    {
...
allocate_batch_control (grpc_call * call, const grpc_op * ops, size_t num_ops)
{
  ...
  if (*pslot == nullptr)
    {
      *pslot =
	(batch_control *) gpr_arena_alloc (call->arena,
					   sizeof (batch_control));
    }
...
static void
finish_batch_completion (void *user_data, grpc_cq_completion * storage)
{
...
static grpc_error *
consolidate_batch_errors (batch_control * bctl)
{
  ...
  if (n == 0)
    ...
    {
      grpc_error *error =
	GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING ("Call batch failed",
							  bctl->errors, n);
...
static void
post_batch_completion (batch_control * bctl)
{
  ...
  if (bctl->op.send_initial_metadata)
    {
      grpc_metadata_batch_destroy (&call->
				   metadata_batch[0 /* is_receiving */ ][0
									 /* is_trailing */
									 ]);
    }
  ...
  if (bctl->op.send_trailing_metadata)
    {
      grpc_metadata_batch_destroy (&call->
				   metadata_batch[0 /* is_receiving */ ][1
									 /* is_trailing */
									 ]);
    }
...
static void
finish_batch_step (batch_control * bctl)
{
...
static void
continue_receiving_slices (batch_control * bctl)
{
  grpc_error *error;
  grpc_call *call = bctl->call;
  for (;;)
	...
	{
	  error =
	    grpc_byte_stream_pull (call->receiving_stream,
				   &call->receiving_slice);
	  if (error == GRPC_ERROR_NONE)
	    {
	      grpc_slice_buffer_add (&(*call->receiving_buffer)->data.raw.
				     slice_buffer, call->receiving_slice);
	    }
	  else
	    {
	      grpc_byte_stream_destroy (call->receiving_stream);
...
static void
receiving_slice_ready (void *bctlp, grpc_error * error)
{
...
static void
process_data_after_md (batch_control * bctl)
{
  grpc_call *call = bctl->call;
  if (call->receiving_stream == nullptr)
	  ...
	  (call->incoming_compression_algorithm > GRPC_COMPRESS_NONE))
	{
	  *call->receiving_buffer =
	    grpc_raw_compressed_byte_buffer_create (nullptr, 0,
						    call->
...
static void
receiving_stream_ready (void *bctlp, grpc_error * error)
{
  batch_control *bctl = (batch_control *) bctlp;
  grpc_call *call = bctl->call;
  if (error != GRPC_ERROR_NONE)
...
static void
receiving_stream_ready_in_call_combiner (void *bctlp, grpc_error * error)
{
  batch_control *bctl = (batch_control *) bctlp;
  grpc_call *call = bctl->call;
...
static void
validate_filtered_metadata (batch_control * bctl)
{
  grpc_call *call = bctl->call;
  ...
  if (call->incoming_stream_compression_algorithm !=
      ...
      if (algo >= GRPC_STREAM_COMPRESS_ALGORITHMS_COUNT)
	{
	  gpr_asprintf (&error_msg,
			"Invalid stream compression algorithm value '%d'.",
			algo);
	  gpr_log (GPR_ERROR, "%s", error_msg);
	  cancel_with_status (call, STATUS_FROM_SURFACE,
			      GRPC_STATUS_UNIMPLEMENTED, error_msg);
	}
      else
	if (grpc_compression_options_is_stream_compression_algorithm_enabled
	    (&compression_options, algo) == 0)
	{
	  ...
	  gpr_asprintf (&error_msg,
			"Stream compression algorithm '%s' is disabled.",
			algo_name);
	  gpr_log (GPR_ERROR, "%s", error_msg);
	  cancel_with_status (call, STATUS_FROM_SURFACE,
			      GRPC_STATUS_UNIMPLEMENTED, error_msg);
		       ...
		       call->incoming_stream_compression_algorithm))
	{
	  if (grpc_compression_trace.enabled ())
	    {
	      const char *algo_name = nullptr;
	      grpc_stream_compression_algorithm_name (call->
	...
	{
	  gpr_asprintf (&error_msg,
			"Invalid compression algorithm value '%d'.", algo);
	  gpr_log (GPR_ERROR, "%s", error_msg);
	  cancel_with_status (call, STATUS_FROM_SURFACE,
			      GRPC_STATUS_UNIMPLEMENTED, error_msg);
	}
		       ...
		       call->incoming_compression_algorithm))
	{
	  if (grpc_compression_trace.enabled ())
	    {
	      const char *algo_name = nullptr;
	      grpc_compression_algorithm_name (call->
					       incoming_compression_algorithm,
					       &algo_name);
	      gpr_log (GPR_ERROR,
...
static void
add_batch_error (batch_control * bctl, grpc_error * error, bool has_cancelled)
{
  if (error == GRPC_ERROR_NONE)
    return;
...
static void
receiving_initial_metadata_ready (void *bctlp, grpc_error * error)
{
  batch_control *bctl = (batch_control *) bctlp;
  grpc_call *call = bctl->call;
  ...
  while (true)
      ...
      else
	{
	  ...
	  saved_rsr_closure =
	    GRPC_CLOSURE_CREATE (receiving_stream_ready,
...
static void
finish_batch (void *bctlp, grpc_error * error)
{
  batch_control *bctl = (batch_control *) bctlp;
  grpc_call *call = bctl->call;
...
static void
free_no_op_completion (void *p, grpc_cq_completion * completion)
{
  gpr_free (completion);
}

static grpc_call_error
call_start_batch (grpc_call * call, const grpc_op * ops,
		  size_t nops, void *notify_tag, int is_notify_tag_closure)
{
  size_t i;
  const grpc_op *op;
  batch_control *bctl;
	...
	case GRPC_OP_SEND_INITIAL_METADATA:
	  {
	    ...
	    if (!are_initial_metadata_flags_valid
		(op->flags, call->is_client))
	      {
		...
		effective_stream_compression_level =
		  ...
		  maybe_stream_compression_level.level;
		level_set = true;
		...
		if (stream_compression)
		...
		else
		  {
		    const grpc_compression_algorithm calgo =
		      compression_algorithm_for_level_locked (call,
							      effective_compression_level);
		...
		additional_metadata_count > INT_MAX)
	      {
	    ...
	    stream_op_payload->send_initial_metadata.send_initial_metadata =
	      &call->
	      metadata_batch[0 /* is_receiving */ ][0 /* is_trailing */ ];
	...
	case GRPC_OP_SEND_MESSAGE:
	  {
	    ...
	    grpc_slice_buffer_stream_init (&call->sending_stream,
					   ...
					   send_message->data.raw.
					   slice_buffer, op->flags);
		...
		GRPC_COMPRESS_NONE)
	      {
		call->sending_stream.base.flags |=
	...
	case GRPC_OP_SEND_CLOSE_FROM_CLIENT:
	  {
	    ...
	    stream_op_payload->send_trailing_metadata.send_trailing_metadata =
	      &call->
	      metadata_batch[0 /* is_receiving */ ][1 /* is_trailing */ ];
	    break;
	  }
	case GRPC_OP_SEND_STATUS_FROM_SERVER:
	  {
		...
		{
		  override_error =
		    error_from_status (op->data.send_status_from_server.
				       status, "Returned non-ok status");
		}
	      if (op->data.send_status_from_server.status_details != nullptr)
		{
		  call->send_extra_metadata[1].md =
		    grpc_mdelem_from_slices (GRPC_MDSTR_GRPC_MESSAGE,
					     grpc_slice_ref_internal (*op->
								      data.
								      send_status_from_server.
								      status_details));
		  call->send_extra_metadata_count++;
		  char *msg =
		    grpc_slice_to_c_string (GRPC_MDVALUE
					    (call->send_extra_metadata[1].
					     md));
		  override_error =
		    grpc_error_set_str (override_error,
					GRPC_ERROR_STR_GRPC_MESSAGE,
					grpc_slice_from_copied_string (msg));
		  gpr_free (msg);
		}
	      set_status_from_error (call, STATUS_FROM_API_OVERRIDE,
				     override_error);
	    }
	    if (!prepare_application_metadata (call,
					       (int) op->data.
					       send_status_from_server.
					       trailing_metadata_count,
					       op->data.
					       send_status_from_server.
					       trailing_metadata, 1, 1,
					       nullptr, 0))
	      {
	...
	case GRPC_OP_RECV_INITIAL_METADATA:
	  {
	    ...
	    stream_op_payload->recv_initial_metadata.recv_initial_metadata =
	      &call->
	      metadata_batch[1 /* is_receiving */ ][0 /* is_trailing */ ];
	...
	case GRPC_OP_RECV_STATUS_ON_CLIENT:
	  {
	    ...
	    stream_op_payload->recv_trailing_metadata.recv_trailing_metadata =
	      &call->
	      metadata_batch[1 /* is_receiving */ ][1 /* is_trailing */ ];
...
grpc_call_error
grpc_call_start_batch (grpc_call * call, const grpc_op * ops,
		       size_t nops, void *tag, void *reserved)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_call_error err;

  GRPC_API_TRACE ("grpc_call_start_batch(call=%p, ops=%p, nops=%lu, tag=%p, "
		  "reserved=%p)",
...
grpc_call_error
grpc_call_start_batch_and_execute (grpc_call * call,
				   const grpc_op * ops,
...
void
grpc_call_context_set (grpc_call * call, grpc_context_index elem,
...
grpc_compression_algorithm
grpc_call_compression_for_level (grpc_call * call,
				 grpc_compression_level level)
{
  grpc_compression_algorithm algo =
...
const char *
grpc_call_error_to_string (grpc_call_error error)
{
  switch (error)
    {
