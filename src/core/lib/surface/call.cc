...
grpc_error* grpc_call_create(grpc_exec_ctx* exec_ctx,
  ...
  *out_call = call;
  ...
  if (immediately_cancel) {
    cancel_with_error(exec_ctx, call, GRPC_ERROR_CANCELLED);
  }
  if (args->cq != nullptr) {
    GPR_ASSERT(
...
void grpc_call_unref(grpc_call* c) {
  ...
  if (cancel) {
    cancel_with_error(&exec_ctx, c, GRPC_ERROR_CANCELLED);
  } else {
...
grpc_call_error grpc_call_cancel_with_status(grpc_call* c,
  ...
  cancel_with_status(&exec_ctx, c, status, description);
  grpc_exec_ctx_finish(&exec_ctx);
  return GRPC_CALL_OK;
}
...
static void cancel_with_status(grpc_exec_ctx* exec_ctx, grpc_call* c,
                               grpc_status_code status,
                               const char* description) {
  cancel_with_error(exec_ctx, c, error_from_status(status, description));
}
...
static grpc_error* aggregate_batch_errors(batch_control* bctl) {
  grpc_error* errors[4];
  int count = 0;
  memset(errors, 0, sizeof(errors));
  if (bctl->batch_error != GRPC_ERROR_NONE) {
    errors[count] = bctl->batch_error;
    count++;
  }
  if (bctl->rsr_error != GRPC_ERROR_NONE) {
    errors[count] = bctl->rsr_error;
    count++;
  }
  if (bctl->rimr_error != GRPC_ERROR_NONE) {
...
static void reset_batch_errors(batch_control* bctl) {
  ...
  GRPC_ERROR_UNREF(bctl->rsr_error);
  GRPC_ERROR_UNREF(bctl->rimr_error);
  bctl->batch_error = GRPC_ERROR_NONE;
  bctl->rsr_error = GRPC_ERROR_NONE;
  bctl->rimr_error = GRPC_ERROR_NONE;
}
...
static void post_batch_completion(grpc_exec_ctx* exec_ctx,
  ...
  grpc_call* call = bctl->call;
  grpc_error* error = aggregate_batch_errors(bctl);
  ...
  if (bctl->op.recv_trailing_metadata) {
    ...
    recv_trailing_filter(exec_ctx, call, md, GRPC_ERROR_REF(bctl->batch_error));
      ...
      if (child != nullptr) {
        do {
          ...
          if (child->cancellation_is_inherited) {
            ...
            cancel_with_error(exec_ctx, child, GRPC_ERROR_CANCELLED);
...
static void receiving_initial_metadata_ready(grpc_exec_ctx* exec_ctx,
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    bctl->rimr_error = GRPC_ERROR_REF(error);
    cancel_with_error(exec_ctx, call, GRPC_ERROR_REF(error));
  }
