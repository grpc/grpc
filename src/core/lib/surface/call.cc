...
static parent_call* get_or_create_parent_call(grpc_call* call) {
  ...
  if (p == nullptr) {
    p = reinterpret_cast<parent_call*>(
        gpr_arena_alloc(call->arena, sizeof(*p)));
...
grpc_error* grpc_call_create(grpc_exec_ctx* exec_ctx,
  ...
  call = reinterpret_cast<grpc_call*>(gpr_arena_alloc(
  ...
  *out_call = call;
  ...
  if (args->parent != nullptr) {
    child_call* cc = call->child = reinterpret_cast<child_call*>(
...
static void release_call(grpc_exec_ctx* exec_ctx, void* call,
                         grpc_error* error) {
  grpc_call* c = reinterpret_cast<grpc_call*>(call);
...
static void execute_batch_in_call_combiner(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  grpc_transport_stream_op_batch* batch =
      reinterpret_cast<grpc_transport_stream_op_batch*>(arg);
  grpc_call* call =
      reinterpret_cast<grpc_call*>(batch->handler_private.extra_arg);
...
static void done_termination(grpc_exec_ctx* exec_ctx, void* arg,
                             grpc_error* error) {
  cancel_state* state = reinterpret_cast<cancel_state*>(arg);
...
static void cancel_with_error(grpc_exec_ctx* exec_ctx, grpc_call* c,
  ...
  cancel_state* state =
      reinterpret_cast<cancel_state*>(gpr_malloc(sizeof(*state)));
  state->call = c;
...
static void set_encodings_accepted_by_peer(grpc_exec_ctx* exec_ctx,
  ...
  grpc_mdelem_set_user_data(
      ...
      reinterpret_cast<void*>((static_cast<uintptr_t>(call->encodings_accepted_by_peer)) + 1));
}
...
static int prepare_application_metadata(
  ...
  for (i = 0; i < total_count; i++) {
    ...
    l->md = grpc_mdelem_from_grpc_metadata(exec_ctx,
                                           const_cast<grpc_metadata*>(md));
  }
  if (i != total_count) {
    for (int j = 0; j < i; j++) {
      const grpc_metadata* md =
...
static void publish_app_metadata(grpc_call* call, grpc_metadata_batch* b,
  ...
  if (dest->count + b->list.count > dest->capacity) {
    ...
    dest->metadata = reinterpret_cast<grpc_metadata*>(
...
static void recv_trailing_filter(grpc_exec_ctx* exec_ctx, void* args,
                                 grpc_metadata_batch* b) {
  grpc_call* call = reinterpret_cast<grpc_call*>(args);
...
static void set_status_value_directly(grpc_status_code status, void* dest) {
  *reinterpret_cast<grpc_status_code*>(dest) = status;
}

static void set_cancelled_value(grpc_status_code status, void* dest) {
  *reinterpret_cast<int*>(dest) = (status != GRPC_STATUS_OK);
}
...
static batch_control* allocate_batch_control(grpc_call* call,
  ...
  if (*pslot == nullptr) {
    *pslot = reinterpret_cast<batch_control*>(
...
static void finish_batch_completion(grpc_exec_ctx* exec_ctx, void* user_data,
  ...
  batch_control* bctl = reinterpret_cast<batch_control*>(user_data);
...
static void receiving_slice_ready(grpc_exec_ctx* exec_ctx, void* bctlp,
                                  grpc_error* error) {
  batch_control* bctl = reinterpret_cast<batch_control*>(bctlp);
  grpc_call* call = bctl->call;
