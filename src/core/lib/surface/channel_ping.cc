...
static void
ping_destroy (void *arg, grpc_cq_completion * storage)
{
  gpr_free (arg);
}

static void
ping_done (void *arg, grpc_error * error)
{
...
void
grpc_channel_ping (grpc_channel * channel, grpc_completion_queue * cq,
		   void *tag, void *reserved)
{
  GRPC_API_TRACE ("grpc_channel_ping(channel=%p, cq=%p, tag=%p, reserved=%p)",
		  4, (channel, cq, tag, reserved));
