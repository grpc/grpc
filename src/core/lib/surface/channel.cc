...
grpc_channel *
grpc_channel_create_with_builder (grpc_channel_stack_builder * builder,
				  grpc_channel_stack_type channel_stack_type)
{
  char *target = gpr_strdup (grpc_channel_stack_builder_get_target (builder));
  grpc_channel_args *args =
    grpc_channel_args_copy (grpc_channel_stack_builder_get_channel_arguments
			    (builder));
  ...
  grpc_error *error =
    grpc_channel_stack_builder_finish (builder, sizeof (grpc_channel), 1,
				       destroy_channel, nullptr,
				       (void **) &channel);
  ...
  gpr_atm_no_barrier_store (&channel->call_size_estimate,
			    (gpr_atm) CHANNEL_STACK_FROM_CHANNEL (channel)->
			    call_stack_size);
  ...
  for (size_t i = 0; i < args->num_args; i++)
			    ...
			    GRPC_COMPRESSION_CHANNEL_DEFAULT_LEVEL))
	{
	  ...
	  channel->compression_options.default_level.level =
	    (grpc_compression_level) grpc_channel_arg_get_integer (&args->
								   args[i],
								   {
	...
	{
	  channel->compression_options.default_stream_compression_level.
	    is_set = true;
	  channel->compression_options.default_stream_compression_level.
	    level =
			    ...
			    GRPC_COMPRESSION_CHANNEL_DEFAULT_ALGORITHM))
	{
	  ...
	  channel->compression_options.default_algorithm.algorithm =
	    (grpc_compression_algorithm) grpc_channel_arg_get_integer (&args->
								       args
								       [i],
								       {
								       GRPC_COMPRESS_NONE,
	...
	{
	  channel->compression_options.enabled_algorithms_bitset = (uint32_t) args->args[i].value.integer | 0x1;	/* always support no compression */
	}
      else if (0 ==
	       strcmp (args->args[i].key,
		       GRPC_STREAM_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET))
	{
	  channel->compression_options.enabled_stream_compression_algorithms_bitset = (uint32_t) args->args[i].value.integer | 0x1;	/* always support no compression */
	}
    }
...
size_t
grpc_channel_get_call_size_estimate (grpc_channel * channel)
{
#define ROUND_UP_SIZE 256
  ...
  return ((size_t) gpr_atm_no_barrier_load (&channel->call_size_estimate) +
	  2 * ROUND_UP_SIZE) & ~(size_t) (ROUND_UP_SIZE - 1);
}

void
grpc_channel_update_call_size_estimate (grpc_channel * channel, size_t size)
{
  size_t cur =
    (size_t) gpr_atm_no_barrier_load (&channel->call_size_estimate);
  if (cur < size)
    {
...
void
grpc_channel_get_info (grpc_channel * channel,
			  ...
			  gpr_timespec deadline, void *reserved)
{
  ...
  grpc_core::ExecCtx exec_ctx;
  grpc_call *call =
    grpc_channel_create_call_internal (channel, parent_call, propagation_mask,
				       cq, nullptr,
...
grpc_call *
grpc_channel_create_pollset_set_call (grpc_channel * channel,
				      grpc_call * parent_call,
				      uint32_t propagation_mask,
...
{
  registered_call *rc =
    (registered_call *) gpr_malloc (sizeof (registered_call));
  GRPC_API_TRACE
  ...
  rc->authority =
    host ? grpc_mdelem_from_slices (GRPC_MDSTR_AUTHORITY,
				    grpc_slice_intern
				    (grpc_slice_from_static_string (host))) :
    GRPC_MDNULL;
				     ...
				     gpr_timespec deadline, void *reserved)
{
  ...
  GRPC_API_TRACE ("grpc_channel_create_registered_call("
		  ...
		  (channel, parent_call, (unsigned) propagation_mask,
		   completion_queue, registered_call_handle, deadline.tv_sec,
...
#endif
void
grpc_channel_internal_ref (grpc_channel * c REF_ARG)
{
  GRPC_CHANNEL_STACK_REF (CHANNEL_STACK_FROM_CHANNEL (c), REF_REASON);
}

void
grpc_channel_internal_unref (grpc_channel * c REF_ARG)
{
  GRPC_CHANNEL_STACK_UNREF (CHANNEL_STACK_FROM_CHANNEL (c), REF_REASON);
}

static void
destroy_channel (void *arg, grpc_error * error)
{
...
grpc_channel_compression_options (const grpc_channel * channel)
{
