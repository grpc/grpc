...
grpc_channel* grpc_channel_create_with_builder(grpc_channel_stack_builder* builder,
  ...
  grpc_channel_args* args =
      grpc_channel_args_copy(grpc_channel_stack_builder_get_channel_arguments(builder));
  grpc_resource_user* resource_user = grpc_channel_stack_builder_get_resource_user(builder);
  ...
  grpc_error_handle builder_error =
      grpc_channel_stack_builder_finish(builder, sizeof(grpc_channel), 1, destroy_channel, nullptr,
  ...
  for (size_t i = 0; i < args->num_args; i++) {
    if (0 == strcmp(args->args[i].key, GRPC_COMPRESSION_CHANNEL_DEFAULT_LEVEL)) {
      channel->compression_options.default_level.is_set = true;
      channel->compression_options.default_level.level =
          static_cast<grpc_compression_level>(grpc_channel_arg_get_integer(
              &args->args[i],
              {GRPC_COMPRESS_LEVEL_NONE, GRPC_COMPRESS_LEVEL_NONE, GRPC_COMPRESS_LEVEL_COUNT - 1}));
    } else if (0 == strcmp(args->args[i].key, GRPC_COMPRESSION_CHANNEL_DEFAULT_ALGORITHM)) {
      channel->compression_options.default_algorithm.is_set = true;
      channel->compression_options.default_algorithm.algorithm =
          static_cast<grpc_compression_algorithm>(grpc_channel_arg_get_integer(
              &args->args[i],
              {GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE, GRPC_COMPRESS_ALGORITHMS_COUNT - 1}));
    } else if (0 == strcmp(args->args[i].key, GRPC_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET)) {
      ...
      if (args->args[i].type == GRPC_ARG_POINTER) {
        ...
        channel->channelz_node =
            static_cast<grpc_core::channelz::ChannelNode*>(args->args[i].value.pointer.p)->Ref();
      } else {
        gpr_log(GPR_DEBUG, GRPC_ARG_CHANNELZ_CHANNEL_NODE " should be a pointer");
      }
    }
  }
...
static grpc_core::UniquePtr<char> get_default_authority(const grpc_channel_args* input_args) {
  ...
  for (size_t i = 0; i < num_args; ++i) {
    ...
    } else if (0 == strcmp(input_args->args[i].key, GRPC_SSL_TARGET_NAME_OVERRIDE_ARG)) {
...
static grpc_channel_args* build_channel_args(const grpc_channel_args* input_args,
...
void* channelz_node_copy(void* p) {
  grpc_core::channelz::ChannelNode* node = static_cast<grpc_core::channelz::ChannelNode*>(p);
  node->Ref().release();
  return p;
}
...
const grpc_arg_pointer_vtable channelz_node_arg_vtable = {channelz_node_copy, channelz_node_destroy,
                                                          channelz_node_cmp};
...
void CreateChannelzNode(grpc_channel_stack_builder* builder) {
  ...
  const bool channelz_enabled =
      grpc_channel_args_find_bool(args, GRPC_ARG_ENABLE_CHANNELZ, GRPC_ENABLE_CHANNELZ_DEFAULT);
  ...
  const bool is_internal_channel =
      grpc_channel_args_find_bool(args, GRPC_ARG_CHANNELZ_IS_INTERNAL_CHANNEL, false);
  // Create the channelz node.
  const char* target = grpc_channel_stack_builder_get_target(builder);
  grpc_core::RefCountedPtr<grpc_core::channelz::ChannelNode> channelz_node =
      grpc_core::MakeRefCounted<grpc_core::channelz::ChannelNode>(
          target != nullptr ? target : "", channel_tracer_max_memory, is_internal_channel);
  channelz_node->AddTraceEvent(grpc_core::channelz::ChannelTrace::Severity::Info,
                               grpc_slice_from_static_string("Channel created"));
  // Add channelz node to channel args.
  // We remove the is_internal_channel arg, since we no longer need it.
  grpc_arg new_arg =
      grpc_channel_arg_pointer_create(const_cast<char*>(GRPC_ARG_CHANNELZ_CHANNEL_NODE),
...
grpc_channel* grpc_channel_create(const char* target, const grpc_channel_args* input_args,
                                  grpc_channel_stack_type channel_stack_type,
                                  grpc_transport* optional_transport,
                                  grpc_resource_user* resource_user, grpc_error_handle* error) {
  ...
  const grpc_core::UniquePtr<char> default_authority = get_default_authority(input_args);
  grpc_channel_args* args = build_channel_args(input_args, default_authority.get());
  if (grpc_channel_stack_type_is_client(channel_stack_type)) {
    auto channel_args_mutator = grpc_channel_args_get_client_channel_creation_mutator();
  ...
  grpc_channel* channel = grpc_channel_create_with_builder(builder, channel_stack_type, error);
  if (channel == nullptr) {
    grpc_shutdown();  // Since we won't call destroy_channel().
  }
  return channel;
...
void grpc_channel_reset_connect_backoff(grpc_channel* channel) {
  grpc_core::ApplicationCallbackExecCtx callback_exec_ctx;
  grpc_core::ExecCtx exec_ctx;
  GRPC_API_TRACE("grpc_channel_reset_connect_backoff(channel=%p)", 1, (channel));
...
static grpc_call* grpc_channel_create_call_internal(
    ...
    grpc_completion_queue* cq, grpc_pollset_set* pollset_set_alternative, grpc_mdelem path_mdelem,
...
grpc_call* grpc_channel_create_call(grpc_channel* channel, grpc_call* parent_call,
  ...
  grpc_call* call = grpc_channel_create_call_internal(
      ...
      host != nullptr ? grpc_mdelem_create(GRPC_MDSTR_AUTHORITY, *host, nullptr) : GRPC_MDNULL,
...
RegisteredCall::RegisteredCall(const char* method_arg, const char* host_arg)
    : method(method_arg != nullptr ? method_arg : ""),
      ...
      path(grpc_mdelem_from_slices(GRPC_MDSTR_PATH,
                                   grpc_core::ExternallyManagedSlice(method.c_str()))),
...
void* grpc_channel_register_call(grpc_channel* channel, const char* method, const char* host,
                                 void* reserved) {
  GRPC_API_TRACE("grpc_channel_register_call(channel=%p, method=%s, host=%s, reserved=%p)", 4,
...
grpc_call* grpc_channel_create_registered_call(grpc_channel* channel, grpc_call* parent_call,
                                               ...
                                               void* reserved) {
  grpc_core::RegisteredCall* rc = static_cast<grpc_core::RegisteredCall*>(registered_call_handle);
  GRPC_API_TRACE(
      ...
      "reserved=%p)",
      9,
      (channel, parent_call, (unsigned)propagation_mask, completion_queue, registered_call_handle,
       deadline.tv_sec, deadline.tv_nsec, (int)deadline.clock_type, reserved));
  GPR_ASSERT(!reserved);
  grpc_core::ApplicationCallbackExecCtx callback_exec_ctx;
  grpc_core::ExecCtx exec_ctx;
  grpc_call* call = grpc_channel_create_call_internal(
      channel, parent_call, propagation_mask, completion_queue, nullptr, GRPC_MDELEM_REF(rc->path),
      GRPC_MDELEM_REF(rc->authority), grpc_timespec_to_millis_round_up(deadline));
...
static void destroy_channel(void* arg, grpc_error_handle /*error*/) {
  ...
  if (channel->channelz_node != nullptr) {
    channel->channelz_node->AddTraceEvent(grpc_core::channelz::ChannelTrace::Severity::Info,
  ...
  if (channel->resource_user != nullptr) {
    grpc_resource_user_free(channel->resource_user, GRPC_RESOURCE_QUOTA_CHANNEL_SIZE);
  }
...
void grpc_channel_destroy_internal(grpc_channel* channel) {
  ...
  op->disconnect_with_error = GRPC_ERROR_CREATE_FROM_STATIC_STRING("Channel Destroyed");
