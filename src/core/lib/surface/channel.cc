  ...
  for (size_t i = 0; i < args->num_args; i++) {
    if (0 == strcmp(args->args[i].key, GRPC_ARG_DEFAULT_AUTHORITY)) {
      ...
      channel->compression_options.default_stream_compression_level.level =
          static_cast<grpc_stream_compression_level>(
              grpc_channel_arg_get_integer(
                  &args->args[i], {GRPC_STREAM_COMPRESS_LEVEL_NONE,
      ...
      channel->compression_options.default_stream_compression_algorithm
          .algorithm = static_cast<grpc_stream_compression_algorithm>(
          grpc_channel_arg_get_integer(
              &args->args[i],
...
void* grpc_channel_register_call(grpc_channel* channel, const char* method,
  ...
  registered_call* rc =
      reinterpret_cast<registered_call*>(gpr_malloc(sizeof(registered_call)));
  GRPC_API_TRACE(
...
grpc_call* grpc_channel_create_registered_call(
    ...
    gpr_timespec deadline, void* reserved) {
  registered_call* rc =
      reinterpret_cast<registered_call*>(registered_call_handle);
  GRPC_API_TRACE(
...
static void destroy_channel(grpc_exec_ctx* exec_ctx, void* arg,
                            grpc_error* error) {
  grpc_channel* channel = reinterpret_cast<grpc_channel*>(arg);
