...
namespace grpc_core
{
  ...
  namespace
  {
    ...
    static void fill_metadata (grpc_call_element * elem,
			       grpc_metadata_batch * mdb)
    {
      CallData *calld = reinterpret_cast < CallData * >(elem->call_data);
      bool expected = false;
      if (!calld->filled_metadata.
				   ...
				   grpc_core::memory_order_relaxed))
	{
      ...
      ChannelData *chand =
	reinterpret_cast < ChannelData * >(elem->channel_data);
	...
	grpc_mdelem_from_slices (GRPC_MDSTR_GRPC_MESSAGE,
				 grpc_slice_from_copied_string (chand->
								error_message));
    ...
    static void lame_start_transport_stream_op_batch (grpc_call_element *
						      elem,
      ...
      else if (op->recv_trailing_metadata)
	{
      ...
      grpc_transport_stream_op_batch_finish_with_failure (op,
...
grpc_channel *
grpc_lame_client_channel_create (const char *target,
				 grpc_status_code error_code,
				 const char *error_message)
{
  ...
  GRPC_API_TRACE ("grpc_lame_client_channel_create(target=%s, error_code=%d, "
		  "error_message=%s)", 3, (target, (int) error_code,
					   error_message));
  GPR_ASSERT (elem->filter == &grpc_lame_filter);
  auto chand =
    reinterpret_cast < grpc_core::ChannelData * >(elem->channel_data);
