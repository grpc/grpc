...
char* FilterStackCall::GetLocalAddress() {
  ...
  if (!local_address_slice.empty()) {
    absl::string_view local_address_string_view = local_address_slice.as_string_view();
    char* local_address_string =
        static_cast<char*>(gpr_malloc(local_address_string_view.size() + 1));
    memcpy(local_address_string, local_address_string_view.data(), local_address_string_view.size());
