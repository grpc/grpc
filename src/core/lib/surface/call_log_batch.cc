...
grpc_op_string (const grpc_op * op)
{
  ...
  switch (op->op)
	...
	{
	  gpr_strvec_add (&b,
			  grpc_dump_slice (*op->data.send_status_from_server.
					   status_details, GPR_DUMP_ASCII));
	}
      else
	{
...
void
grpc_call_log_batch (const char *file, int line, gpr_log_severity severity,
