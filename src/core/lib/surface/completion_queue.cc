...
static size_t
non_polling_poller_size (void)
{
  return sizeof (non_polling_poller);
}

static void
non_polling_poller_init (grpc_pollset * pollset, gpr_mu ** mu)
{
...
static void
non_polling_poller_destroy (grpc_pollset * pollset)
{
  non_polling_poller *npp = (non_polling_poller *) pollset;
...
static grpc_error *
non_polling_poller_work (grpc_pollset * pollset,
...
static grpc_error *
non_polling_poller_kick (grpc_pollset * pollset,
			 grpc_pollset_worker * specific_worker)
{
...
static void
non_polling_poller_shutdown (grpc_pollset * pollset, grpc_closure * closure)
{
  non_polling_poller *p = (non_polling_poller *) pollset;
...
void
grpc_completion_queue_thread_local_cache_init (grpc_completion_queue * cq)
{
...
int
grpc_completion_queue_thread_local_cache_flush (grpc_completion_queue * cq,
						void **tag, int *ok)
{
  grpc_cq_completion *storage =
...
static void
cq_event_queue_init (grpc_cq_event_queue * q)
{
...
static void
cq_event_queue_destroy (grpc_cq_event_queue * q)
{
  gpr_mpscq_destroy (&q->queue);
}

static bool
cq_event_queue_push (grpc_cq_event_queue * q, grpc_cq_completion * c)
{
...
cq_event_queue_pop (grpc_cq_event_queue * q)
{
  ...
  if (gpr_spinlock_trylock (&q->queue_lock))
    {
      ...
      bool is_empty = false;
      c =
	(grpc_cq_completion *) gpr_mpscq_pop_and_check_end (&q->queue,
							    &is_empty);
...
static long
cq_event_queue_num_items (grpc_cq_event_queue * q)
{
  return (long) gpr_atm_no_barrier_load (&q->num_queue_items);
}

grpc_completion_queue *
grpc_completion_queue_create_internal (grpc_cq_completion_type
				       completion_type,
				       grpc_cq_polling_type polling_type)
{
  ...
  GRPC_API_TRACE ("grpc_completion_queue_create_internal(completion_type=%d, "
		  "polling_type=%d)", 2, (completion_type, polling_type));
...
grpc_get_cq_completion_type (grpc_completion_queue * cq)
{
  return cq->vtable->cq_completion_type;
}

int
grpc_get_cq_poll_num (grpc_completion_queue * cq)
{
  int cur_num_polls;
  gpr_mu_lock (cq->mu);
...
#ifndef NDEBUG
void
grpc_cq_internal_ref (grpc_completion_queue * cq, const char *reason,
  ...
  if (grpc_trace_cq_refcount.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "CQ:%p   ref %" PRIdPTR " -> %" PRIdPTR " %s", cq, val,
	       val + 1, reason);
    }
#else
void
grpc_cq_internal_ref (grpc_completion_queue * cq)
{
#endif
...
static void
on_pollset_shutdown_done (void *arg, grpc_error * error)
{
...
#ifndef NDEBUG
void
grpc_cq_internal_unref (grpc_completion_queue * cq, const char *reason,
			const char *file, int line)
{
  if (grpc_trace_cq_refcount.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&cq->owning_refs.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "CQ:%p unref %" PRIdPTR " -> %" PRIdPTR " %s", cq, val,
	       val - 1, reason);
    }
#else
void
grpc_cq_internal_unref (grpc_completion_queue * cq)
{
#endif
...
#ifndef NDEBUG
static void
cq_check_tag (grpc_completion_queue * cq, void *tag, bool lock_cq)
{
  int found = 0;
  if (lock_cq)
    {
...
static bool
atm_inc_if_nonzero (gpr_atm * counter)
{
  while (true)
    {
...
static bool
cq_begin_op_for_next (grpc_completion_queue * cq, void *tag)
{
  cq_next_data *cqd = (cq_next_data *) DATA_FROM_CQ (cq);
...
static bool
cq_begin_op_for_pluck (grpc_completion_queue * cq, void *tag)
{
...
bool
grpc_cq_begin_op (grpc_completion_queue * cq, void *tag)
{
#ifndef NDEBUG
  gpr_mu_lock (cq->mu);
  if (cq->outstanding_tag_count == cq->outstanding_tag_capacity)
	...
	GPR_MAX (4, 2 * cq->outstanding_tag_capacity);
      cq->outstanding_tags =
	(void **) gpr_realloc (cq->outstanding_tags,
			       sizeof (*cq->outstanding_tags) *
			       cq->outstanding_tag_capacity);
    }
...
static void
cq_end_op_for_next (grpc_completion_queue * cq, void *tag,
		    grpc_error * error,
		    void (*done) (void *done_arg,
				  grpc_cq_completion * storage),
		    void *done_arg, grpc_cq_completion * storage)
{
  ...
  if (grpc_api_trace.enabled () ||
      (grpc_trace_operation_failures.enabled () && error != GRPC_ERROR_NONE))
    {
      ...
      GRPC_API_TRACE ("cq_end_op_for_next(cq=%p, tag=%p, error=%s, "
  ...
  if ((grpc_completion_queue *) gpr_tls_get (&g_cached_cq) == cq &&
	...
	gpr_atm_acq_load (&cqd->pending_events) == 1;
...
static void
cq_end_op_for_pluck (grpc_completion_queue * cq, void *tag,
		     grpc_error * error,
		     void (*done) (void *done_arg,
				   grpc_cq_completion * storage),
		     void *done_arg, grpc_cq_completion * storage)
{
  ...
  if (grpc_api_trace.enabled () ||
    ...
    {
      const char *errmsg = grpc_error_string (error);
      GRPC_API_TRACE ("cq_end_op_for_pluck(cq=%p, tag=%p, error=%s, "
		      "done=%p, done_arg=%p, storage=%p)",
		      6, (cq, tag, errmsg, done, done_arg, storage));
      if (grpc_trace_operation_failures.enabled ()
	  && error != GRPC_ERROR_NONE)
  ...
  storage->done_arg = done_arg;
  storage->next =
    ((uintptr_t) & cqd->completed_head) | ((uintptr_t) (is_success));
...
void
grpc_cq_end_op (grpc_completion_queue * cq, void *tag, grpc_error * error,
		void (*done) (void *done_arg, grpc_cq_completion * storage),
		void *done_arg, grpc_cq_completion * storage)
{
...
public:
  ...
  {
    cq_is_finished_arg *
      a = (cq_is_finished_arg *) check_ready_to_finish_arg_;
    grpc_completion_queue *
      cq = a->cq;
    cq_next_data *
      cqd = (cq_next_data *) DATA_FROM_CQ (cq);
    GPR_ASSERT (a->stolen_completion == nullptr);

    gpr_atm
      current_last_seen_things_queued_ever =
...
#ifndef NDEBUG
static void
dump_pending_tags (grpc_completion_queue * cq)
{
  if (!grpc_trace_pending_tags.enabled ())
    return;
  ...
  gpr_mu_unlock (cq->mu);
  char *
    out = gpr_strvec_flatten (&v, nullptr);
...
static grpc_event
cq_next (grpc_completion_queue * cq, gpr_timespec deadline, void *reserved)
{
  grpc_event
    ret;
  cq_next_data *
  ...
  GRPC_API_TRACE ("grpc_completion_queue_next("
		  "cq=%p, "
		  "deadline=gpr_timespec { tv_sec: %" PRId64
		  ", tv_nsec: %d, clock_type: %d }, "
		  "reserved=%p)",
		  5,
		  (cq, deadline.tv_sec, deadline.tv_nsec,
  ...
  grpc_millis
    deadline_millis = grpc_timespec_to_millis_round_up (deadline);
  cq_is_finished_arg
    is_finished_arg = {
    gpr_atm_no_barrier_load (&cqd->things_queued_ever),
    cq,
  ...
  for (;;)
    {
      grpc_millis
	iteration_deadline = deadline_millis;
      ...
      if (is_finished_arg.stolen_completion != nullptr)
	{
      ...
      grpc_cq_completion *
	c = cq_event_queue_pop (&cqd->queue);
	  ...
	  grpc_core::ExecCtx::Get ()->Now () >= deadline_millis)
	{
      ...
      cq->num_polls++;
      grpc_error *
	err = cq->poller_vtable->work (POLLSET_FROM_CQ (cq), nullptr,
				       iteration_deadline);
      gpr_mu_unlock (cq->mu);
...
/* Finishes the completion queue shutdown. This means that there are no more
   ...
   this function */
static void
cq_finish_shutdown_next (grpc_completion_queue * cq)
{
  cq_next_data *
    cqd = (cq_next_data *) DATA_FROM_CQ (cq);
...
static void
cq_shutdown_next (grpc_completion_queue * cq)
{
  cq_next_data *
    cqd = (cq_next_data *) DATA_FROM_CQ (cq);
...
grpc_event
grpc_completion_queue_next (grpc_completion_queue * cq,
			    gpr_timespec deadline, void *reserved)
{
...
static int
add_plucker (grpc_completion_queue * cq, void *tag,
	     grpc_pollset_worker ** worker)
{
  cq_pluck_data *
    cqd = (cq_pluck_data *) DATA_FROM_CQ (cq);
...
static void
del_plucker (grpc_completion_queue * cq, void *tag,
	     grpc_pollset_worker ** worker)
{
  cq_pluck_data *
    cqd = (cq_pluck_data *) DATA_FROM_CQ (cq);
	...
	while ((c = (grpc_cq_completion *) (prev->next & ~(uintptr_t) 1)) !=
	       &cqd->completed_head)
	  {
	    if (c->tag == a->tag)
	      {
		prev->next =
		  (prev->next & (uintptr_t) 1) | (c->next & ~(uintptr_t) 1);
...
static grpc_event
cq_pluck (grpc_completion_queue * cq, void *tag,
	  gpr_timespec deadline, void *reserved)
{
  grpc_event ret;
  grpc_cq_completion *c;
  ...
  if (grpc_cq_pluck_trace.enabled ())
    {
      GRPC_API_TRACE ("grpc_completion_queue_pluck("
		      "cq=%p, tag=%p, "
		      "deadline=gpr_timespec { tv_sec: %" PRId64
		      ", tv_nsec: %d, clock_type: %d }, "
		      "reserved=%p)",
  ...
  for (;;)
	  ...
	  grpc_core::ExecCtx::Get ()->Now () >= deadline_millis)
	{
      ...
      cq->num_polls++;
      grpc_error *err =
	cq->poller_vtable->work (POLLSET_FROM_CQ (cq), &worker,
				 deadline_millis);
      if (err != GRPC_ERROR_NONE)
	{
	  del_plucker (cq, tag, &worker);
...
static void
cq_finish_shutdown_pluck (grpc_completion_queue * cq)
{
  cq_pluck_data *cqd = (cq_pluck_data *) DATA_FROM_CQ (cq);
...
static void
cq_shutdown_pluck (grpc_completion_queue * cq)
{
  cq_pluck_data *cqd = (cq_pluck_data *) DATA_FROM_CQ (cq);
...
void
grpc_completion_queue_shutdown (grpc_completion_queue * cq)
{
  grpc_core::ExecCtx exec_ctx;
...
void
grpc_completion_queue_destroy (grpc_completion_queue * cq)
{
...
grpc_pollset *
grpc_cq_pollset (grpc_completion_queue * cq)
{
  return cq->poller_vtable->can_get_pollset ? POLLSET_FROM_CQ (cq) : nullptr;
}

bool
grpc_cq_can_listen (grpc_completion_queue * cq)
{
