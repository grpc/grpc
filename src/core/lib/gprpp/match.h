...
/// Match on a variant.
/// Given variant \a value, and a set of callables \a fs, call the
/// appropriate callable based on the type contained in \a value.
///
...
auto Match(const absl::variant<T0, Ts...>& value, Fs... fs) -> decltype(
    std::declval<OverloadType<Fs...>>()(std::declval<T0>())) {
  return absl::visit(Overload(std::move(fs)...), value);
}

/// A version of Match that takes a mutable pointer to a variant and
/// calls its overload callables with a mutable pointer to the current
/// variant value.
///
/// Example:
///   variant<int, string> v = 42;
