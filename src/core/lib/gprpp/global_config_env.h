...
#define GPR_GLOBAL_CONFIG_DEFINE_BOOL(name, default_value, help)                         \
  ...
  static ::grpc_core::GlobalConfigEnvBool g_env_##name(g_env_str_##name, default_value); \
...
#define GPR_GLOBAL_CONFIG_DEFINE_INT32(name, default_value, help)                         \
  static char g_env_str_##name[] = #name;                                                 \
  static ::grpc_core::GlobalConfigEnvInt32 g_env_##name(g_env_str_##name, default_value); \
...
#define GPR_GLOBAL_CONFIG_DEFINE_STRING(name, default_value, help)                           \
  static char g_env_str_##name[] = #name;                                                    \
  static ::grpc_core::GlobalConfigEnvString g_env_##name(g_env_str_##name, default_value);   \
  ::grpc_core::UniquePtr<char> gpr_global_config_get_##name() { return g_env_##name.Get(); } \
  void gpr_global_config_set_##name(const char* value) { g_env_##name.Set(value); }
