...
int grpc_compression_algorithm_is_message(grpc_compression_algorithm algorithm) {
  return (algorithm >= GRPC_COMPRESS_DEFLATE && algorithm <= GRPC_COMPRESS_GZIP) ? 1 : 0;
}
...
int grpc_compression_algorithm_parse(grpc_slice name, grpc_compression_algorithm* algorithm) {
  if (grpc_slice_eq_static_interned(name, GRPC_MDSTR_IDENTITY)) {
    ...
    *algorithm = GRPC_COMPRESS_GZIP;
    return 1;
  } else if (grpc_slice_eq_static_interned(name, GRPC_MDSTR_STREAM_SLASH_GZIP)) {
  ...
  if (level == GRPC_COMPRESS_LEVEL_NONE) {
    ...
    if (!grpc_compression_algorithm_from_message_stream_compression_algorithm(
            ...
            grpc_message_compression_algorithm_for_level(
                level, grpc_compression_bitset_to_message_bitset(accepted_encodings)),
...
int grpc_compression_options_is_algorithm_enabled(const grpc_compression_options* opts,
                                                  grpc_compression_algorithm algorithm) {
  return grpc_compression_options_is_algorithm_enabled_internal(opts, algorithm);
}
