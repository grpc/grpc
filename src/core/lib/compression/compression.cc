...
grpc_compression_algorithm grpc_compression_algorithm_for_level(
  ...
  if (level == GRPC_COMPRESS_LEVEL_NONE) {
    ...
    // TODO(mxyan): Design algorithm to select from all algorithms,
    // including stream compression algorithm
    if (!grpc_compression_algorithm_from_message_stream_compression_algorithm(
            &algo,
            grpc_message_compression_algorithm_for_level(
                level, grpc_compression_bitset_to_message_bitset(
                           accepted_encodings)),
...
int grpc_compression_options_is_algorithm_enabled(
    ...
    grpc_compression_algorithm algorithm) {
  return grpc_compression_options_is_algorithm_enabled_internal(
      opts, algorithm);
}
...
grpc_compression_algorithm grpc_compression_algorithm_from_slice(
  ...
  if (grpc_slice_eq_static_interned(str,
