...
int
grpc_compression_algorithm_parse (grpc_slice name,
...
int
grpc_stream_compression_algorithm_parse (grpc_slice name,
...
grpc_compression_algorithm_name (grpc_compression_algorithm algorithm,
				 const char **name)
{
  GRPC_API_TRACE ("grpc_compression_algorithm_parse(algorithm=%d, name=%p)",
...
int
grpc_stream_compression_algorithm_name (grpc_stream_compression_algorithm
...
grpc_compression_algorithm_from_slice (grpc_slice str)
{
  if (grpc_slice_eq (str, GRPC_MDSTR_IDENTITY))
    return GRPC_COMPRESS_NONE;
  if (grpc_slice_eq (str, GRPC_MDSTR_DEFLATE))
    return GRPC_COMPRESS_DEFLATE;
  if (grpc_slice_eq (str, GRPC_MDSTR_GZIP))
    return GRPC_COMPRESS_GZIP;
  return GRPC_COMPRESS_ALGORITHMS_COUNT;
}

grpc_stream_compression_algorithm
grpc_stream_compression_algorithm_from_slice (grpc_slice str)
{
  if (grpc_slice_eq (str, GRPC_MDSTR_IDENTITY))
...
grpc_slice
grpc_compression_algorithm_slice (grpc_compression_algorithm algorithm)
{
  switch (algorithm)
    {
    case GRPC_COMPRESS_NONE:
...
grpc_slice
grpc_stream_compression_algorithm_slice (grpc_stream_compression_algorithm
					 algorithm)
{
  switch (algorithm)
    {
...
grpc_mdelem
grpc_compression_encoding_mdelem (grpc_compression_algorithm algorithm)
{
  switch (algorithm)
    {
    case GRPC_COMPRESS_NONE:
...
grpc_mdelem
grpc_stream_compression_encoding_mdelem (grpc_stream_compression_algorithm
					 algorithm)
{
  switch (algorithm)
    {
...
void
grpc_compression_options_enable_algorithm (grpc_compression_options * opts,
					   grpc_compression_algorithm
					   algorithm)
{
...
void
grpc_compression_options_disable_algorithm (grpc_compression_options * opts,
					    grpc_compression_algorithm
					    algorithm)
{
...
int
grpc_compression_options_is_algorithm_enabled (const grpc_compression_options
					       * opts,
					       grpc_compression_algorithm
					       algorithm)
{
...
int
grpc_compression_options_is_stream_compression_algorithm_enabled (const
				      ...
				      uint32_t accepted_encodings)
{
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (algos_ranking); i++)
      ...
      for (size_t j = 0; j < num_supported; j++)
	{
      ...
      if (algos_supported_idx == num_supported)
	break;
    }
...
grpc_stream_compression_algorithm_for_level (grpc_stream_compression_level
					     level,
					     uint32_t
					     accepted_stream_encodings)
{
