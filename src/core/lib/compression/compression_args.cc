...
grpc_channel_args* grpc_channel_args_compression_algorithm_set_state(
  ...
  if (grpc_channel_args_get_channel_default_compression_algorithm(*a) ==
    ...
    if (state != 0) {
      grpc_core::SetBit((unsigned*)states_arg, algorithm);
    } else if (algorithm != GRPC_COMPRESS_NONE) {
      grpc_core::ClearBit((unsigned*)states_arg, algorithm);
    }
  } else {
    /* create a new arg */
    grpc_arg tmp;
    ...
    if (state != 0) {
      grpc_core::SetBit((unsigned*)&tmp.value.integer, algorithm);
    } else if (algorithm != GRPC_COMPRESS_NONE) {
      grpc_core::ClearBit((unsigned*)&tmp.value.integer, algorithm);
    }
