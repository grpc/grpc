...
/** Returns 1 if the argument for compression algorithm's enabled states
 * bitset was found in \a a, returning the arg's value in \a states.
 * Otherwise, returns 0. */
  ...
  if (a != nullptr) {
    ...
    for (i = 0; i < a->num_args; ++i) {
      if (a->args[i].type == GRPC_ARG_INTEGER &&
        ...
        **states_arg =
            (**states_arg &
             ((1 << GRPC_COMPRESS_ALGORITHMS_COUNT) - 1)) |
...
uint32_t grpc_channel_args_compression_algorithm_get_states(
  ...
  if (find_compression_algorithm_states_bitset(a, &states_arg)) {
    ...
    return (1u << GRPC_COMPRESS_ALGORITHMS_COUNT) -
           1; /* All algs. enabled */
  }
}
