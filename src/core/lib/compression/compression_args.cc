...
grpc_compression_algorithm grpc_channel_args_get_channel_default_compression_algorithm(
  ...
  for (i = 0; i < a->num_args; ++i) {
    if (a->args[i].type == GRPC_ARG_INTEGER &&
      ...
      return default_algorithm < GRPC_COMPRESS_ALGORITHMS_COUNT ? default_algorithm
                                                                : GRPC_COMPRESS_NONE;
    }
  }
...
static int find_compression_algorithm_states_bitset(const grpc_channel_args* a, int** states_arg) {
  if (a != nullptr) {
    ...
    for (i = 0; i < a->num_args; ++i) {
      if (a->args[i].type == GRPC_ARG_INTEGER &&
          !strcmp(GRPC_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET, a->args[i].key)) {
        *states_arg = &a->args[i].value.integer;
        **states_arg = (**states_arg & ((1 << GRPC_COMPRESS_ALGORITHMS_COUNT) - 1)) |
...
grpc_channel_args* grpc_channel_args_compression_algorithm_set_state(
  ...
  if (grpc_channel_args_get_channel_default_compression_algorithm(*a) == algorithm && state == 0) {
    ...
    grpc_arg tmp;
    tmp.type = GRPC_ARG_INTEGER;
    tmp.key = const_cast<char*>(GRPC_COMPRESSION_CHANNEL_ENABLED_ALGORITHMS_BITSET);
