...
static bool
gzip_flate (grpc_stream_compression_context_gzip * ctx,
  ...
  while (max_output_size > 0 && (in->length > 0 || flush) && !eoc)
    {
      size_t slice_size =
	max_output_size <
	OUTPUT_BLOCK_SIZE ? max_output_size : OUTPUT_BLOCK_SIZE;
      ...
      while (ctx->zs.avail_out > 0 && in->length > 0 && !eoc)
	{
	  ...
	  if (ctx->zs.avail_in > 0)
	    {
	      grpc_slice_buffer_undo_take_first (in,
...
static bool
grpc_stream_compress_gzip (grpc_stream_compression_context * ctx,
...
static bool
grpc_stream_decompress_gzip (grpc_stream_compression_context * ctx,
			     grpc_slice_buffer * in,
			     grpc_slice_buffer * out,
			     size_t * output_size,
...
grpc_stream_compression_context_create_gzip (grpc_stream_compression_method
					     method)
{
  ...
  if (method == GRPC_STREAM_COMPRESSION_GZIP_DECOMPRESS)
    ...
    {
      r =
	deflateInit2 (&gzip_ctx->zs, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 0x1F,
...
static void
grpc_stream_compression_context_destroy_gzip (grpc_stream_compression_context
					      * ctx)
{
  if (ctx == nullptr)
    {
