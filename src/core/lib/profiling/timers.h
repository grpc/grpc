...
#if !(defined(GRPC_STAP_PROFILER) + defined(GRPC_BASIC_PROFILER) + defined(GRPC_CUSTOM_PROFILER))
...
#define GPR_TIMER_MARK(tag, important) gpr_timer_add_mark(tag, important, __FILE__, __LINE__);
...
class ProfileScope {
 public:
  ProfileScope(const char* desc, bool important, const char* file, int line) : desc_(desc) {
...
#define GPR_TIMER_SCOPE_NAME(prefix, line) GPR_TIMER_SCOPE_NAME_INTERNAL(prefix, line)
#define GPR_TIMER_SCOPE(tag, important)                                                    \
  ::grpc::ProfileScope GPR_TIMER_SCOPE_NAME(_profile_scope_, __LINE__)((tag), (important), \
                                                                       __FILE__, __LINE__)
