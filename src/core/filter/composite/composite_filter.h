...
class CompositeFilter final : public V3InterceptorToV2Bridge<CompositeFilter> {
 public:
  ...
  struct Config final : public FilterConfig {
    ...
    // use (which may include additions from the blackboard).
    // Note: This is populated using the blackboard but is not actually part
    // of the identity of the config (i.e., does not matter for Equals()).
    absl::flat_hash_map<
        const XdsMatcher::Action*,
        std::vector<RefCountedPtr<const FilterConfig>>> merged_config_map;
  };
