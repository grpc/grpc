    ...
    // FilterEnabled
    {
      ...
      // HeaderMutationRules
      {
        ...
        if (header_mutation_rules_proto == nullptr) {
          ...
          auto header_mutation_rules = ParseHeaderMutationRules(
              context, header_mutation_rules_proto, errors);
          auto header_mutation_rules_json =
              JsonParse(header_mutation_rules.ToJsonString());
          if (header_mutation_rules_json.ok()) {
            ext_authz_config["decoder_header_mutation_rules"] =
                *header_mutation_rules_json;
          }
        }
      }
    }
...
absl::StatusOr<XdsHttpFilterImpl::ServiceConfigJsonEntry>
XdsExtAuthzFilter::GenerateServiceConfig(const Json& hcm_filter_config) const {
  return ServiceConfigJsonEntry{"ext_authz", JsonDump(hcm_filter_config)};
}
...
RefCountedPtr<const FilterConfig> XdsExtAuthzFilter::ParseTopLevelConfig(
  ...
  // XdsGrpcService
  {
    ...
    if (grpc_service_proto == nullptr) {
      ...
      config->ext_authz.xds_grpc_service =
          ParseXdsGrpcService(context, grpc_service_proto, errors);
    }
  }
  // FilterEnabled
  {
    ...
    if (filter_enabled_proto == nullptr) {
      ...
      if (default_value == nullptr) {
        ...
        config->ext_authz.filter_enabled = {numerator, denom_val};
      }
    }
  }
  // deny_at_disable
  {
    ...
    if (deny_at_disable_proto != nullptr) {
      auto* default_value =
          envoy_config_core_v3_RuntimeFeatureFlag_default_value(
              deny_at_disable_proto);
      config->ext_authz.deny_at_disable = ParseBoolValue(default_value);
    }
  }
  // failure_mode_allow
  config->ext_authz.failure_mode_allow =
      envoy_extensions_filters_http_ext_authz_v3_ExtAuthz_failure_mode_allow(
          ext_authz);
  // failure_mode_allow_header_add
  config->ext_authz.failure_mode_allow_header_add =
      envoy_extensions_filters_http_ext_authz_v3_ExtAuthz_failure_mode_allow_header_add(
          ext_authz);
  // status_on_error
  {
    ...
    if (status_on_error_proto == nullptr) {
      ...
      config->ext_authz.status_on_error = static_cast<grpc_status_code>(
          envoy_type_v3_HttpStatus_code(status_on_error_proto));
    }
  }
  // include_peer_certificate
  config->ext_authz.include_peer_certificate =
      envoy_extensions_filters_http_ext_authz_v3_ExtAuthz_include_peer_certificate(
          ext_authz);
  // allowed_headers
  {
    ...
    if (allowed_headers_proto != nullptr) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        config->ext_authz.allowed_headers.push_back(
  ...
  // disallowed_headers
  {
    ...
    if (disallowed_headers_proto != nullptr) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        config->ext_authz.disallowed_headers.push_back(
            {StringMatcherParse(context, patterns[i], errors)});
      }
    }
  }
  // HeaderMutationRules
  {
    const auto* header_mutation_rules_proto =
        envoy_extensions_filters_http_ext_authz_v3_ExtAuthz_decoder_header_mutation_rules(
            ext_authz);
    if (header_mutation_rules_proto != nullptr) {
      config->ext_authz.decoder_header_mutation_rules =
          ParseHeaderMutationRules(context, header_mutation_rules_proto,
                                   errors);
    }
  }
