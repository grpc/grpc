...
class SessionEndpointImpl {
 public:
  ...
  bool Read(absl::AnyInvocable<void(absl::Status)> on_read,
    ...
    GRPC_CHECK(!read_in_progress_.exchange(true, std::memory_order_acquire))
    ...
    if (err != GRPC_CALL_OK) {
      auto cb = std::move(read_tag_.callback);
  ...
  bool Write(absl::AnyInvocable<void(absl::Status)> on_writable,
    ...
    GRPC_CHECK(!write_in_progress_.exchange(true, std::memory_order_acquire))
    ...
    write_tag_.callback = [this, cb = std::move(on_writable),
      ...
      if (ok) {
        ...
        cb(absl::UnavailableError("Write failed"));
      }
      Unref();
    };
    ...
    if (err != GRPC_CALL_OK) {
      auto cb = std::move(write_tag_.callback);
