...
void RetryFetchRegionalAccessBoundary(void* arg, grpc_error_handle error) {
  ...
  if (error.ok()) {
    ...
    gpr_mu_lock(&req->creds->regional_access_boundary_fetcher_->cache_mu_);
    req->creds->regional_access_boundary_fetcher_->fetch_in_flight_ = false;
    req->creds->regional_access_boundary_fetcher_->cooldown_deadline_ = gpr_time_add(
        gpr_now(GPR_CLOCK_REALTIME),
        gpr_time_from_seconds(
            GRPC_REGIONAL_ACCESS_BOUNDARY_BASE_COOLDOWN_DURATION_SECS *
                req->creds->regional_access_boundary_fetcher_->cooldown_multiplier_,
            GPR_TIMESPAN));
    if (req->creds->regional_access_boundary_fetcher_->cooldown_multiplier_ *
            GRPC_REGIONAL_ACCESS_BOUNDARY_BASE_COOLDOWN_DURATION_SECS <
        GRPC_REGIONAL_ACCESS_BOUNDARY_MAX_COOLDOWN_DURATION_SECS) {
      req->creds->regional_access_boundary_fetcher_->cooldown_multiplier_ *= 2;
    }
    gpr_mu_unlock(&req->creds->regional_access_boundary_fetcher_->cache_mu_);
  }
}
...
void OnRegionalAccessBoundaryResponse(void* arg, grpc_error_handle error) {
  ...
  if (error.ok() && req->response.status == 200) {
    ...
    if (json.ok() && json->type() == grpc_core::Json::Type::kObject) {
      ...
      if (!encoded_locations.empty()) {
        ...
        gpr_mu_lock(&creds->regional_access_boundary_fetcher_->cache_mu_);
        creds->regional_access_boundary_fetcher_->cache_ = {
            std::move(encoded_locations),
            std::move(locations),
            gpr_time_add(gpr_now(GPR_CLOCK_REALTIME), ttl)};

        // On success, reset the cooldown multiplier.
        req->creds->regional_access_boundary_fetcher_->cooldown_multiplier_ = 1;
        gpr_mu_unlock(&creds->regional_access_boundary_fetcher_->cache_mu_);
        success = true;
      }
    }
  }
  ...
  if (should_retry) {
    ...
    gpr_mu_lock(&creds->regional_access_boundary_fetcher_->cache_mu_);
    creds->regional_access_boundary_fetcher_->fetch_in_flight_ = false;
...
void StartRegionalAccessBoundaryFetch(
  ...
  gpr_mu_lock(&req->creds->regional_access_boundary_fetcher_->cache_mu_);
  req->creds->regional_access_boundary_fetcher_->fetch_in_flight_ = true;
  gpr_mu_unlock(&req->creds->regional_access_boundary_fetcher_->cache_mu_);
...
ArenaPromise<absl::StatusOr<ClientMetadataHandle>> RegionalAccessBoundaryFetcher::Fetch(
...

  {
    ...
    if (hasValidCache || fetch_in_flight_ ||
