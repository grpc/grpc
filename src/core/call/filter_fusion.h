...
template <auto... Ts>
constexpr bool IsNoInterceptorFilter() {
  if constexpr (AllNoInterceptor<decltype(Ts)...>) {
    return true;
  } else {
    return false;
...
class AdaptOnServerTrailingMetadataMethod<
 ...
 public:
  ...
  void operator()(T& x) {
    ...
    if (!status.ok()) {
      x = ServerMetadataFromStatus(std::move(status));
    }
  }
...
using FuseOnServerTrailingMetadata = FuseImplOnServerTrailingMetadata<
    MethodVariantForFilters<&Filters::Call::OnServerTrailingMetadata...>(),
    Derived, Filters...>;
...
using FuseOnClientToServerHalfClose = FuseImplOnClientToServerHalfClose<
    IsNoInterceptorFilter<&Filters::Call::OnClientToServerHalfClose...>(),
    Derived, Filters...>;
...
class FusedFilter : public ImplementChannelFilter<FusedFilter<Filters...>>,
 ...
 public:
  ...
  static absl::StatusOr<std::unique_ptr<FusedFilter<Filters...>>> Create(
      const grpc_core::ChannelArgs& args, grpc_core::ChannelFilter::Args) {
