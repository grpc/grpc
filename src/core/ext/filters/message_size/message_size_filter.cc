...
static void *
refcounted_message_size_limits_ref (void *value)
{
...
static void
refcounted_message_size_limits_unref (void *value)
{
  refcounted_message_size_limits *limits =
    (refcounted_message_size_limits *) value;
...
static void *
refcounted_message_size_limits_create_from_json (const grpc_json * json)
{
  ...
  for (grpc_json * field = json->child; field != nullptr; field = field->next)
      ...
      if (strcmp (field->key, "maxRequestMessageBytes") == 0)
	{
	  ...
	  max_request_message_bytes =
	    gpr_parse_nonnegative_int (field->value);
...
static void
recv_message_ready (void *user_data, grpc_error * error)
{
...
static int
default_size (const grpc_channel_args * args, int without_minimal_stack)
{
  ...
  if (channel_arg != nullptr)
      ...
      if (service_config != nullptr)
	{
	  chand->method_limit_table =
	    grpc_service_config_create_method_config_table (service_config,
...
static bool
maybe_add_message_size_filter (grpc_channel_stack_builder * builder,
			       void *arg)
{
...
void
grpc_message_size_filter_init (void)
{
...
void
grpc_message_size_filter_shutdown (void)
{
}
