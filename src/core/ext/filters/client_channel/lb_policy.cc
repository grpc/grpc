...
grpc_core::DebugOnlyTraceFlag grpc_trace_lb_policy_refcount (false,
							     "lb_policy_refcount");

void
grpc_lb_policy_init (grpc_lb_policy * policy,
...
#ifndef NDEBUG
void
grpc_lb_policy_ref (grpc_lb_policy * lb_policy, const char *file, int line,
		    const char *reason)
{
  if (grpc_trace_lb_policy_refcount.enabled ())
    {
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "LB_POLICY:%p   ref %" PRIdPTR " -> %" PRIdPTR " %s",
	       lb_policy, old_refs, old_refs + 1, reason);
    }
#else
void
grpc_lb_policy_ref (grpc_lb_policy * lb_policy)
{
#endif
...
#ifndef NDEBUG
void
grpc_lb_policy_unref (grpc_lb_policy * lb_policy, const char *file, int line,
		      const char *reason)
{
  if (grpc_trace_lb_policy_refcount.enabled ())
    {
      gpr_atm old_refs = gpr_atm_no_barrier_load (&lb_policy->refs.count);
      gpr_log (file, line, GPR_LOG_SEVERITY_DEBUG,
	       "LB_POLICY:%p unref %" PRIdPTR " -> %" PRIdPTR " %s",
	       lb_policy, old_refs, old_refs - 1, reason);
    }
#else
void
grpc_lb_policy_unref (grpc_lb_policy * lb_policy)
{
#endif
...
void
grpc_lb_policy_shutdown_locked (grpc_lb_policy * policy,
...
int
grpc_lb_policy_pick_locked (grpc_lb_policy * policy,
...
void
grpc_lb_policy_cancel_pick_locked (grpc_lb_policy * policy,
				   grpc_lb_policy_pick_state * pick,
				   grpc_error * error)
{
...
void
grpc_lb_policy_cancel_picks_locked (grpc_lb_policy * policy,
...
void
grpc_lb_policy_exit_idle_locked (grpc_lb_policy * policy)
{
  policy->vtable->exit_idle_locked (policy);
}

void
grpc_lb_policy_ping_one_locked (grpc_lb_policy * policy,
...
void
grpc_lb_policy_notify_on_state_change_locked (grpc_lb_policy * policy,
...
grpc_lb_policy_check_connectivity_locked (grpc_lb_policy * policy,
					  grpc_error ** connectivity_error)
{
  return policy->vtable->check_connectivity_locked (policy,
						    connectivity_error);
}

void
grpc_lb_policy_update_locked (grpc_lb_policy * policy,
...
void
grpc_lb_policy_set_reresolve_closure_locked (grpc_lb_policy * policy,
					     grpc_closure *
...
void
grpc_lb_policy_try_reresolve (grpc_lb_policy * policy,
