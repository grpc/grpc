...
static grpc_server_retry_throttle_data* grpc_server_retry_throttle_data_create(
  ...
  grpc_server_retry_throttle_data* throttle_data =
      reinterpret_cast<grpc_server_retry_throttle_data*>(
...
static void* copy_server_name(void* key, void* unused) {
  return gpr_strdup(reinterpret_cast<const char*>(key));
}

static long compare_server_name(void* key1, void* key2, void* unused) {
  return strcmp(reinterpret_cast<const char*>(key1),
                reinterpret_cast<const char*>(key2));
}

static void destroy_server_retry_throttle_data(void* value, void* unused) {
  grpc_server_retry_throttle_data* throttle_data =
      reinterpret_cast<grpc_server_retry_throttle_data*>(value);
...
grpc_server_retry_throttle_data* grpc_retry_throttle_map_get_data_for_server(
  ...
  grpc_server_retry_throttle_data* throttle_data =
      reinterpret_cast<grpc_server_retry_throttle_data*>(
          gpr_avl_get(g_avl, const_cast<char*>(server_name), nullptr));
  if (throttle_data == nullptr) {
    // Entry not found.  Create a new one.
    throttle_data = grpc_server_retry_throttle_data_create(
        max_milli_tokens, milli_token_ratio, nullptr);
    g_avl = gpr_avl_add(g_avl, const_cast<char*>(server_name), throttle_data,
                        nullptr);
  } else {
