...
class XdsClient : public InternallyRefCounted<XdsClient> {
 ...
 private:
  ...
  // After an update is processed and service config created, update
  // weighted_cluster_child_name_map_ to reflect all the actions in the service
  // config and remove old actions from the previous update/service config.
  void UpdateFinalActions(const WeightedClusterChildNameMap& actions_to_remove);
