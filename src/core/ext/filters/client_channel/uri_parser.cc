...
static grpc_uri *
bad_uri (const char *uri_text, size_t pos, const char *section,
...
static char *
decode_and_copy_component (const char *src, size_t begin, size_t end)
{
...
static void
parse_query_parts (grpc_uri * uri)
{
...
grpc_uri *
grpc_uri_parse (const char *uri_text, bool suppress_errors)
{
  grpc_uri *uri;
  ...
  if (uri_text[i] == '#')
    {
      ...
      if (!parse_fragment_or_query (uri_text, &i))
	{
	  return bad_uri (uri_text, i - fragment_end, "fragment",
			  suppress_errors);
	}
  ...
  uri->scheme =
    decode_and_copy_component (uri_text, scheme_begin, scheme_end);
  uri->authority =
...
const char *
grpc_uri_get_query_arg (const grpc_uri * uri, const char *key)
{
