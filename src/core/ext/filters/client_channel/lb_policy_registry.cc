...
class RegistryState {
 public:
  ...
  void RegisterLoadBalancingPolicyFactory(std::unique_ptr<LoadBalancingPolicyFactory> factory) {
    gpr_log(GPR_DEBUG, "registering LB policy factory for \"%s\"", factory->name());
  ...
  LoadBalancingPolicyFactory* GetLoadBalancingPolicyFactory(const char* name) const {
    for (size_t i = 0; i < factories_.size(); ++i) {
 ...
 private:
  absl::InlinedVector<std::unique_ptr<LoadBalancingPolicyFactory>, 10> factories_;
};
...
OrphanablePtr<LoadBalancingPolicy> LoadBalancingPolicyRegistry::CreateLoadBalancingPolicy(
  ...
  // Find factory.
  LoadBalancingPolicyFactory* factory = g_state->GetLoadBalancingPolicyFactory(name);
...
bool LoadBalancingPolicyRegistry::LoadBalancingPolicyExists(const char* name,
  ...
  if (requires_config != nullptr) {
    ...
    *requires_config = factory->ParseLoadBalancingConfig(Json(), &error) == nullptr;
...
grpc_error_handle ParseLoadBalancingConfigHelper(const Json& lb_config_array,
                                                 Json::Object::const_iterator* result) {
  ...
  for (const Json& lb_config : lb_config_array.array_value()) {
    if (lb_config.type() != Json::Type::OBJECT) {
      return GRPC_ERROR_CREATE_FROM_STATIC_STRING("child entry should be of type object");
    }
    if (lb_config.object_value().empty()) {
      return GRPC_ERROR_CREATE_FROM_STATIC_STRING("no policy found in child entry");
    }
    ...
    if (LoadBalancingPolicyRegistry::LoadBalancingPolicyExists(it->first.c_str(), nullptr)) {
      *result = it;
  ...
  return GRPC_ERROR_CREATE_FROM_COPIED_STRING(
      absl::StrCat("No known policies in list: ", absl::StrJoin(policies_tried, " ")).c_str());
}

}  // namespace

RefCountedPtr<LoadBalancingPolicy::Config> LoadBalancingPolicyRegistry::ParseLoadBalancingConfig(
  ...
  *error = ParseLoadBalancingConfigHelper(json, &policy);
  ...
  if (factory == nullptr) {
    *error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
        absl::StrFormat("Factory not found for policy \"%s\"", policy->first).c_str());
    return nullptr;
  }
