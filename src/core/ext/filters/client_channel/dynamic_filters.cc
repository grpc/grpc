...
#define CALL_TO_CALL_STACK(call) \
  (grpc_call_stack*)((char*)(call) + GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(DynamicFilters::Call)))
#define CALL_STACK_TO_CALL(callstack)             \
  (DynamicFilters::Call*)(((char*)(call_stack)) - \
                          GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(DynamicFilters::Call)))
...
DynamicFilters::Call::Call(Args args, grpc_error_handle* error)
  ...
  *error = grpc_call_stack_init(channel_stack_->channel_stack_, 1, Destroy, this, &call_args);
...
void DynamicFilters::Call::StartTransportStreamOpBatch(grpc_transport_stream_op_batch* batch) {
  grpc_call_stack* call_stack = CALL_TO_CALL_STACK(this);
...
void DynamicFilters::Call::Unref() { GRPC_CALL_STACK_UNREF(CALL_TO_CALL_STACK(this), ""); }

void DynamicFilters::Call::Unref(const DebugLocation& /*location*/, const char* reason) {
...
void DynamicFilters::Call::Destroy(void* arg, grpc_error_handle /*error*/) {
  ...
  grpc_call_stack_destroy(CALL_TO_CALL_STACK(self), nullptr, after_call_stack_destroy);
...
void DynamicFilters::Call::IncrementRefCount(const grpc_core::DebugLocation& /*location*/,
                                             const char* reason) {
...
std::pair<grpc_channel_stack*, grpc_error_handle> CreateChannelStack(
    const grpc_channel_args* args, std::vector<const grpc_channel_filter*> filters) {
  // Allocate memory for channel stack.
  const size_t channel_stack_size = grpc_channel_stack_size(filters.data(), filters.size());
  grpc_channel_stack* channel_stack =
      reinterpret_cast<grpc_channel_stack*>(gpr_zalloc(channel_stack_size));
  // Initialize stack.
  grpc_error_handle error = grpc_channel_stack_init(
      /*initial_refs=*/1, DestroyChannelStack, channel_stack, filters.data(), filters.size(), args,
...
DynamicFilters::~DynamicFilters() { GRPC_CHANNEL_STACK_UNREF(channel_stack_, "~DynamicFilters"); }

RefCountedPtr<DynamicFilters::Call> DynamicFilters::CreateCall(DynamicFilters::Call::Args args,
                                                               grpc_error_handle* error) {
  size_t allocation_size =
      GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(Call)) + channel_stack_->call_stack_size;
