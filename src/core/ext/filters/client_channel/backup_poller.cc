  ...
  if (env != nullptr)
      ...
      if (poll_interval_ms == -1)
	{
	  gpr_log (GPR_ERROR,
		   ...
		   "default value %d will be used.", env, g_poll_interval_ms);
	}
      else
	{
...
static void
backup_poller_shutdown_unref (backup_poller * p)
{
...
static void
done_poller (void *arg, grpc_error * error)
{
  ...
  if (gpr_unref (&g_poller->refs))
    {
      ...
      grpc_pollset_shutdown (p->pollset,
			     GRPC_CLOSURE_INIT (&p->shutdown_closure,
						done_poller, p,
...
static void
run_poller (void *arg, grpc_error * error)
{
...
void
grpc_client_channel_start_backup_polling (grpc_pollset_set *
					  interested_parties)
{
  ...
  if (g_poller == nullptr)
    {
      ...
      grpc_timer_init (&g_poller->polling_timer,
		       grpc_core::ExecCtx::Get ()->Now () +
		       g_poll_interval_ms, &g_poller->run_poller_closure);
    }
...
void
grpc_client_channel_stop_backup_polling (grpc_pollset_set *
					 interested_parties)
{
  if (g_poll_interval_ms == 0)
    {
