				       ...
				       int try_to_connect)
{
  ...
  if (client_channel_elem->filter == &grpc_client_channel_filter)
    {
      state =
	grpc_client_channel_check_connectivity_state (client_channel_elem,
						      try_to_connect);
...
static void
partly_done (state_watcher * w, bool due_to_completion, grpc_error * error)
{
...
static void
watch_complete (void *pw, grpc_error * error)
{
  partly_done ((state_watcher *) pw, true, GRPC_ERROR_REF (error));
}

static void
timeout_complete (void *pw, grpc_error * error)
{
  partly_done ((state_watcher *) pw, false, GRPC_ERROR_REF (error));
}

int
grpc_channel_num_external_connectivity_watchers (grpc_channel * channel)
{
  grpc_channel_element *client_channel_elem =
    grpc_channel_stack_last_element (grpc_channel_get_channel_stack
				     (channel));
...
static void
watcher_timer_init (void *arg, grpc_error * error_ignored)
{
  watcher_timer_init_arg *wa = (watcher_timer_init_arg *) arg;

  grpc_timer_init (&wa->w->alarm,
...
int
grpc_channel_support_connectivity_watcher (grpc_channel * channel)
{
  grpc_channel_element *client_channel_elem =
    grpc_channel_stack_last_element (grpc_channel_get_channel_stack
				     (channel));
				       ...
				       grpc_completion_queue * cq, void *tag)
{
  ...
  GRPC_API_TRACE ("grpc_channel_watch_connectivity_state("
