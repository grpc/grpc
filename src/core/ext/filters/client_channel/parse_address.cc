...
bool
grpc_parse_unix (const grpc_uri * uri, grpc_resolved_address * resolved_addr)
{
...
bool
grpc_parse_ipv4_hostport (const char *hostport, grpc_resolved_address * addr,
			  bool log_errors)
{
  bool success = false;
  // Split host and port.
...
bool
grpc_parse_ipv4 (const grpc_uri * uri, grpc_resolved_address * resolved_addr)
{
...
bool
grpc_parse_ipv6_hostport (const char *hostport, grpc_resolved_address * addr,
			  bool log_errors)
{
  bool success = false;
  // Split host and port.
  ...
  if (host_end != nullptr)
      ...
      if (inet_pton (AF_INET6, host_without_scope, &in6->sin6_addr) == 0)
	{
      ...
      if (gpr_parse_bytes_to_uint32 (host_end + 1,
				     strlen (host) - host_without_scope_len -
				     1, &sin6_scope_id) == 0)
	{
...
bool
grpc_parse_ipv6 (const grpc_uri * uri, grpc_resolved_address * resolved_addr)
{
...
bool
grpc_parse_uri (const grpc_uri * uri, grpc_resolved_address * resolved_addr)
{
  if (strcmp ("unix", uri->scheme) == 0)
    {
