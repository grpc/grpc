...
class ConnectedSubchannel : public RefCounted<ConnectedSubchannel> {
 ...
 private:
  ...
  // ref counted pointer to the channelz node in this connected
  // subchannel's owning subchannel.
...
class SubchannelCall {
 public:
  ...
  // stack, but does NOT free the memory because it's in the call arena.
  void Unref();
 ...
 private:
  ...
  // If channelz is enabled, intercepts recv_trailing so that we may
  // check the status and associate it to a subchannel.
  ...
  // State needed to support channelz interception of recv trailing
  // metadata.
...
// A subchannel that knows how to connect to exactly one target address.
// It provides a target for load balancing.
//
 ...
 public:
  class ConnectivityStateWatcherInterface
   ...
   public:
    ...
    // Implementations should call PopConnectivityStateChange to get the
    // next connectivity state change.
    ...
    // TODO(yashkt): This is currently needed to send the state updates
    // in the right order when asynchronously notifying. This will no
    // longer be necessary when we have access to EventManager.
   ...
   private:
    ...
    // Keeps track of the updates that the watcher instance must be
    // notified of.
    // TODO(yashkt): This is currently needed to send the state updates
    // in the right order when asynchronously notifying. This will no
    // longer be necessary when we have access to EventManager.
  ...
  // Throttles keepalive time to \a new_keepalive_time iff \a
  // new_keepalive_time is larger than the subchannel's current
  // keepalive time. The updated value will have an affect when the
  // subchannel creates a new ConnectedSubchannel.
  ...
  // service name.
  // If the return value is GRPC_CHANNEL_READY, also sets
  ...
  // TODO(roth): Move connection backoff out of subchannels and up into
  // LB policy code (probably by adding a SubchannelGroup between
  ...
  static grpc_arg CreateSubchannelAddressArg(
  ...
  static void GetAddressFromSubchannelAddressArg(
      const grpc_channel_args* args, grpc_resolved_address* addr);

 private:
  // A linked list of ConnectivityStateWatcherInterfaces that are
  // monitoring the subchannel's state.
  class ConnectivityStateWatcherList {
   public:
    ...
    void NotifyLocked(Subchannel* subchannel,
                      grpc_connectivity_state state,
   ...
   private:
    // TODO(roth): Once we can use C++-14 heterogeneous lookups, this
  ...
  // A map that tracks ConnectivityStateWatcherInterfaces using a
  // particular health check service name.
  //
  ...
  // Subchannel key that identifies this subchannel in the subchannel
  // pool.
