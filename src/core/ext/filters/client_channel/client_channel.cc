...
static method_parameters *
method_parameters_ref (method_parameters * method_params)
{
...
static void
method_parameters_unref (method_parameters * method_params)
{
...
static void *
method_parameters_ref_wrapper (void *value)
{
  return method_parameters_ref ((method_parameters *) value);
}

static void
method_parameters_unref_wrapper (void *value)
{
  method_parameters_unref ((method_parameters *) value);
}

static bool
parse_wait_for_ready (grpc_json * field,
		      wait_for_ready_value * wait_for_ready)
{
...
static bool
parse_timeout (grpc_json * field, grpc_millis * timeout)
{
  if (field->type != GRPC_JSON_STRING)
    return false;
  size_t len = strlen (field->value);
  if (field->value[len - 1] != 's')
    return false;
...
static void *
method_parameters_create_from_json (const grpc_json * json)
{
  ...
  for (grpc_json * field = json->child; field != nullptr; field = field->next)
	...
	{
	  if (wait_for_ready != WAIT_FOR_READY_UNSET)
	    return nullptr;	// Duplicate.
	  if (!parse_wait_for_ready (field, &wait_for_ready))
	    return nullptr;
	}
      else if (strcmp (field->key, "timeout") == 0)
	{
	  if (timeout > 0)
	    return nullptr;	// Duplicate.
	  if (!parse_timeout (field, &timeout))
	    return nullptr;
	}
    }
...
typedef struct client_channel_channel_data
{
  ...
  struct external_connectivity_watcher
    *external_connectivity_watcher_list_head;
...
static void
set_channel_connectivity_state_locked (channel_data * chand,
				       grpc_connectivity_state state,
...
static void
on_lb_policy_state_changed_locked (void *arg, grpc_error * error)
{
...
static void
watch_lb_policy_locked (channel_data * chand,
			grpc_lb_policy * lb_policy,
			grpc_connectivity_state current_state)
{
...
static void
start_resolving_locked (channel_data * chand)
{
  if (grpc_client_channel_trace.enabled ())
    {
...
static void
parse_retry_throttle_params (const grpc_json * field, void *arg)
{
  service_config_parsing_state *parsing_state =
    (service_config_parsing_state *) arg;
  if (strcmp (field->key, "retryThrottling") == 0)
    {
      if (parsing_state->retry_throttle_data != nullptr)
	return;			// Duplicate.
	  ...
	  if (strcmp (sub_field->key, "maxTokens") == 0)
	      ...
	      if (decimal_point != nullptr)
		{
		  ...
		  if (!gpr_parse_bytes_to_uint32
		      (decimal_point + 1, decimal_len, &decimal_value))
		    {
		      return;
		    }
					      ...
					      &whole_value))
		{
	      ...
	      milli_token_ratio =
		(int) ((whole_value * multiplier) + decimal_value);
	      if (milli_token_ratio <= 0)
		return;
	    }
	}
      parsing_state->retry_throttle_data =
	grpc_retry_throttle_map_get_data_for_server (parsing_state->
						     server_name,
						     max_milli_tokens,
						     milli_token_ratio);
    }
...
static void
request_reresolution_locked (void *arg, grpc_error * error)
{
  ...
  grpc_lb_policy_set_reresolve_closure_locked (chand->lb_policy,
					       &args->closure);
}

static void
on_resolver_result_changed_locked (void *arg, grpc_error * error)
{
  ...
  if (chand->resolver_result != nullptr)
      ...
      if (chand->resolver != nullptr)
	{
	  ...
	  const grpc_arg *channel_arg =
	    grpc_channel_args_find (chand->resolver_result,
	  ...
	  // above.
	  if (lb_policy_name == nullptr)
	    lb_policy_name = "pick_first";
	  grpc_lb_policy_args lb_policy_args;
	  lb_policy_args.args = chand->resolver_result;
	  lb_policy_args.client_channel_factory =
	  ...
	  if (chand->lb_policy != nullptr && !lb_policy_name_changed)
	    {
	      ...
	      grpc_lb_policy_update_locked (chand->lb_policy,
					    &lb_policy_args);
	    }
	  else
	    {
	      ...
	      new_lb_policy =
		grpc_lb_policy_create (lb_policy_name, &lb_policy_args);
	      ...
	      else
		{
		  ...
		  GRPC_CLOSURE_INIT (&args->closure,
				     request_reresolution_locked, args,
				     grpc_combiner_scheduler (chand->
							      combiner));
	  ...
	  if (channel_arg != nullptr)
	      ...
	      if (service_config != nullptr)
		{
		  ...
		  GPR_ASSERT (channel_arg->type == GRPC_ARG_STRING);
		  grpc_uri *uri =
		    grpc_uri_parse (channel_arg->value.string, true);
		  GPR_ASSERT (uri->path[0] != '\0');
		  service_config_parsing_state parsing_state;
		  memset (&parsing_state, 0, sizeof (parsing_state));
		  parsing_state.server_name =
		    uri->path[0] == '/' ? uri->path + 1 : uri->path;
		  grpc_service_config_parse_global_params (service_config,
							   parse_retry_throttle_params,
							   &parsing_state);
  ...
  if (grpc_client_channel_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       ...
	       lb_policy_name_changed ? " (changed)" : "",
	       service_config_json);
    }
  ...
  if (new_lb_policy != nullptr || error != GRPC_ERROR_NONE ||
      ...
      if (chand->lb_policy != nullptr)
	{
	  ...
	  grpc_pollset_set_del_pollset_set (chand->lb_policy->
					    interested_parties,
  ...
  if (error != GRPC_ERROR_NONE || chand->resolver == nullptr)
      ...
      if (chand->resolver != nullptr)
	{
      ...
      set_channel_connectivity_state_locked (chand, GRPC_CHANNEL_SHUTDOWN,
					     GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING
					     ("Got resolver result after disconnection",
					      &error, 1), "resolver_gone");
      GRPC_CHANNEL_STACK_UNREF (chand->owning_stack, "resolver");
      grpc_closure_list_fail_all (&chand->
				  waiting_for_resolver_result_closures,
				  GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING
      ...
      if (new_lb_policy != nullptr)
	{
	  if (grpc_client_channel_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG, "chand=%p: initializing new LB policy",
		       chand);
	    }
	  GRPC_ERROR_UNREF (state_error);
	  state =
	    grpc_lb_policy_check_connectivity_locked (new_lb_policy,
						      &state_error);
	  grpc_pollset_set_add_pollset_set (new_lb_policy->interested_parties,
					    chand->interested_parties);
	  GRPC_CLOSURE_LIST_SCHED (&chand->
				   waiting_for_resolver_result_closures);
	...
	{
	  set_channel_connectivity_state_locked (chand, state,
...
static void
start_transport_op_locked (void *arg, grpc_error * error_ignored)
{
  ...
  if (op->on_connectivity_state_change != nullptr)
    {
      grpc_connectivity_state_notify_on_state_change (&chand->state_tracker,
						      op->connectivity_state,
						      op->
  ...
  if (op->send_ping.on_initiate != nullptr || op->send_ping.on_ack != nullptr)
    {
      if (chand->lb_policy == nullptr)
	{
	  GRPC_CLOSURE_SCHED (op->send_ping.on_initiate,
			      GRPC_ERROR_CREATE_FROM_STATIC_STRING
			      ("Ping with no load balancing"));
	  GRPC_CLOSURE_SCHED (op->send_ping.on_ack,
			      GRPC_ERROR_CREATE_FROM_STATIC_STRING
			      ("Ping with no load balancing"));
	}
      else
	{
	  grpc_lb_policy_ping_one_locked (chand->lb_policy,
...
static void
cc_start_transport_op (grpc_channel_element * elem, grpc_transport_op * op)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  ...
  if (op->bind_pollset != nullptr)
    {
      grpc_pollset_set_add_pollset (chand->interested_parties,
				    op->bind_pollset);
    }
...
static void
cc_get_channel_info (grpc_channel_element * elem,
		     const grpc_channel_info * info)
{
  ...
  if (info->lb_policy_name != nullptr)
	...
	? nullptr : gpr_strdup (chand->info_lb_policy_name);
    }
  if (info->service_config_json != nullptr)
    {
      *info->service_config_json =
	chand->info_service_config_json == nullptr
	? nullptr : gpr_strdup (chand->info_service_config_json);
    }
  gpr_mu_unlock (&chand->info_mu);
}
...
static grpc_error *
cc_init_channel_elem (grpc_channel_element * elem,
		      grpc_channel_element_args * args)
{
  ...
  grpc_client_channel_factory_ref ((grpc_client_channel_factory *) arg->value.
				   pointer.p);
  chand->client_channel_factory =
  ...
  chand->resolver =
    grpc_resolver_create (proxy_name !=
			  nullptr ? proxy_name : arg->value.string,
...
static void
shutdown_resolver_locked (void *arg, grpc_error * error)
{
...
static void
cc_destroy_channel_elem (grpc_channel_element * elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  if (chand->resolver != nullptr)
    {
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE
			  (shutdown_resolver_locked, chand->resolver,
			   grpc_combiner_scheduler (chand->combiner)),
			  GRPC_ERROR_NONE);
    }
...
typedef struct client_channel_call_data
{
  ...
  grpc_transport_stream_op_batch
    *waiting_for_pick_batches[MAX_WAITING_BATCHES];
...
static void
waiting_for_pick_batches_add (call_data * calld,
  ...
  if (batch->send_initial_metadata)
  ...
  else
    {
      GPR_ASSERT (calld->waiting_for_pick_batches_count <
		  MAX_WAITING_BATCHES);
      calld->waiting_for_pick_batches[calld->
				      waiting_for_pick_batches_count++] =
	batch;
    }
}
...
// This is called via the call combiner, so access to calld is synchronized.
static void
fail_pending_batch_in_call_combiner (void *arg, grpc_error * error)
{
  call_data *calld = (call_data *) arg;
  if (calld->waiting_for_pick_batches_count > 0)
    {
      --calld->waiting_for_pick_batches_count;
      grpc_transport_stream_op_batch_finish_with_failure (calld->
							  waiting_for_pick_batches
							  [calld->
							   waiting_for_pick_batches_count],
							  GRPC_ERROR_REF
							  (error),
...
// This is called via the call combiner, so access to calld is synchronized.
static void
waiting_for_pick_batches_fail (grpc_call_element * elem, grpc_error * error)
{
  call_data *calld = (call_data *) elem->call_data;
  if (grpc_client_channel_trace.enabled ())
    {
  ...
  for (size_t i = 0; i < calld->waiting_for_pick_batches_count; ++i)
    {
      ...
      GRPC_CALL_COMBINER_START (calld->call_combiner,
				&calld->
				handle_pending_batch_in_call_combiner[i],
				GRPC_ERROR_REF (error),
...
// This is called via the call combiner, so access to calld is synchronized.
static void
run_pending_batch_in_call_combiner (void *arg, grpc_error * ignored)
{
  call_data *calld = (call_data *) arg;
  if (calld->waiting_for_pick_batches_count > 0)
    {
...
// This is called via the call combiner, so access to calld is synchronized.
static void
waiting_for_pick_batches_resume (grpc_call_element * elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
  if (grpc_client_channel_trace.enabled ())
...
static void
apply_service_config_to_call_locked (grpc_call_element * elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
  if (grpc_client_channel_trace.enabled ())
  ...
  if (chand->method_params_table != nullptr)
    {
      calld->method_params =
	(method_parameters *) grpc_method_config_table_get (chand->
							    method_params_table,
							    calld->path);
...
static void
create_subchannel_call_locked (grpc_call_element * elem, grpc_error * error)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
  ...
  grpc_error *new_error =
    calld->pick.connected_subchannel->CreateCall (call_args,
						  &calld->subchannel_call);
  if (grpc_client_channel_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "chand=%p calld=%p: create subchannel_call=%p: error=%s",
...
static void
pick_done_locked (grpc_call_element * elem, grpc_error * error)
{
  call_data *calld = (call_data *) elem->call_data;
  channel_data *chand = (channel_data *) elem->channel_data;
...
static void
async_pick_done_locked (grpc_call_element * elem, grpc_error * error)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
...
static void
pick_callback_cancel_locked (void *arg, grpc_error * error)
{
  ...
  if (error != GRPC_ERROR_NONE && chand->lb_policy != nullptr)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "chand=%p calld=%p: cancelling pick from LB policy %p",
...
static void
pick_callback_done_locked (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
...
// returns true.
static bool
pick_callback_start_locked (grpc_call_element * elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
  if (grpc_client_channel_trace.enabled ())
  ...
  if (pick_done)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "chand=%p calld=%p: pick completed synchronously", chand,
		   calld);
	}
    }
  else
    {
      ...
      grpc_call_combiner_set_notify_on_cancel (calld->call_combiner,
					       GRPC_CLOSURE_INIT (&calld->
								  lb_pick_cancel_closure,
								  pick_callback_cancel_locked,
								  elem,
...
// holding the call combiner.
static void
pick_after_resolver_result_cancel_locked (void *arg, grpc_error * error)
{
  ...
  async_pick_done_locked (elem,
			  GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING
			  ("Pick cancelled", &error, 1));
}

static void pick_after_resolver_result_start_locked (grpc_call_element *
						     elem);

static void
pick_after_resolver_result_done_locked (void *arg, grpc_error * error)
{
  pick_after_resolver_result_args *args =
    (pick_after_resolver_result_args *) arg;
  if (args->finished)
  ...
  if (error != GRPC_ERROR_NONE)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "chand=%p calld=%p: resolver failed to return data", chand,
		   calld);
	}
      async_pick_done_locked (elem, GRPC_ERROR_REF (error));
    }
	...
	{
	  gpr_log (GPR_DEBUG,
		   "chand=%p calld=%p: resolver returned, doing pick", chand,
		   calld);
	}
  ...
  // right way to deal with it.
  ...
  else
    {
      if (grpc_client_channel_trace.enabled ())
	{
	  gpr_log (GPR_DEBUG, "chand=%p calld=%p: resolver disconnected",
		   chand, calld);
	}
      async_pick_done_locked (elem,
			      GRPC_ERROR_CREATE_FROM_STATIC_STRING
...
static void
pick_after_resolver_result_start_locked (grpc_call_element * elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  call_data *calld = (call_data *) elem->call_data;
  if (grpc_client_channel_trace.enabled ())
...
static void
start_pick_locked (void *arg, grpc_error * ignored)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  channel_data *chand = (channel_data *) elem->channel_data;
...
static void
on_complete (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  if (calld->retry_throttle_data != nullptr)
    {
      if (error == GRPC_ERROR_NONE)
	{
	  grpc_server_retry_throttle_data_record_success (calld->
							  retry_throttle_data);
	}
      else
	{
	  ...
	  grpc_server_retry_throttle_data_record_failure (calld->
							  retry_throttle_data);
	}
    }
...
static void
cc_start_transport_stream_op_batch (grpc_call_element * elem,
				    grpc_transport_stream_op_batch * batch)
{
  call_data *calld = (call_data *) elem->call_data;
  channel_data *chand = (channel_data *) elem->channel_data;
  ...
  if (calld->error != GRPC_ERROR_NONE)
      ...
      if (grpc_client_channel_trace.enabled ())
	{
      ...
      grpc_transport_stream_op_batch_finish_with_failure (batch,
							  GRPC_ERROR_REF
							  (calld->error),
							  calld->
							  call_combiner);
  ...
  if (batch->send_initial_metadata)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "chand=%p calld=%p: entering client_channel combiner",
		   chand, calld);
	}
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT
...
static grpc_error *
cc_init_call_elem (grpc_call_element * elem,
...
static void
cc_destroy_call_elem (grpc_call_element * elem,
		      ...
		      grpc_closure * then_schedule_closure)
{
  ...
  for (size_t i = 0; i < GRPC_CONTEXT_COUNT; ++i)
	...
	{
	  calld->pick.subchannel_call_context[i].destroy (calld->pick.
							  subchannel_call_context
							  [i].value);
	}
    }
...
static void
cc_set_pollset_or_pollset_set (grpc_call_element * elem,
			       grpc_polling_entity * pollent)
{
  call_data *calld = (call_data *) elem->call_data;
  calld->pollent = pollent;
...
static void
try_to_connect_locked (void *arg, grpc_error * error_ignored)
{
  channel_data *chand = (channel_data *) arg;
  if (chand->lb_policy != nullptr)
    {
...
grpc_client_channel_check_connectivity_state (grpc_channel_element * elem,
					      int try_to_connect)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  ...
  if (out == GRPC_CHANNEL_IDLE && try_to_connect)
    {
      ...
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE (try_to_connect_locked, chand,
					       grpc_combiner_scheduler
					       (chand->combiner)),
			  GRPC_ERROR_NONE);
    }
...
static external_connectivity_watcher *
lookup_external_connectivity_watcher (channel_data * chand,
				      grpc_closure * on_complete)
{
  gpr_mu_lock (&chand->external_connectivity_watcher_list_mu);
...
static void
external_connectivity_watcher_list_append (channel_data * chand,
					   external_connectivity_watcher * w)
{
...
static void
external_connectivity_watcher_list_remove (channel_data * chand,
					   external_connectivity_watcher *
					   too_remove)
{
  GPR_ASSERT (lookup_external_connectivity_watcher
...
int
grpc_client_channel_num_external_connectivity_watchers (grpc_channel_element *
							elem)
{
  channel_data *chand = (channel_data *) elem->channel_data;
  int count = 0;
...
static void
on_external_watch_complete_locked (void *arg, grpc_error * error)
{
...
static void
watch_connectivity_state_locked (void *arg, grpc_error * error_ignored)
{
  external_connectivity_watcher *w = (external_connectivity_watcher *) arg;
  ...
  if (w->state != nullptr)
    {
      ...
      grpc_connectivity_state_notify_on_state_change (&w->chand->
						      state_tracker, w->state,
						      &w->my_closure);
    }
  else
    {
      ...
      if (found)
	{
	  ...
	  grpc_connectivity_state_notify_on_state_change (&found->chand->
							  state_tracker,
							  nullptr,
							  &found->my_closure);
	}
...
void
grpc_client_channel_watch_connectivity_state (grpc_channel_element * elem,
					      grpc_polling_entity pollent,
					      grpc_connectivity_state * state,
					      grpc_closure * closure,
					      grpc_closure *
					      watcher_timer_init)
{
  ...
  GRPC_CHANNEL_STACK_REF (w->chand->owning_stack,
			  "external_connectivity_watcher");
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT
		      (&w->my_closure, watch_connectivity_state_locked, w,
		       grpc_combiner_scheduler (chand->combiner)),
		      GRPC_ERROR_NONE);
}
