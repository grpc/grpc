...
RetryFilter::CallData::CallAttempt::CallAttempt(CallData* calld)
  ...
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: create lb_call=%p",
...
RetryFilter::CallData::CallAttempt::~CallAttempt() {
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: destroying call attempt",
...
void RetryFilter::CallData::CallAttempt::AddClosureForBatch(
    ...
    CallCombinerClosureList* closures) {
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: adding batch (%s): %s",
...
bool RetryFilter::CallData::CallAttempt::ShouldRetry(
  ...
  if (status.has_value()) {
    if (GPR_LIKELY(*status == GRPC_STATUS_OK)) {
      ...
      grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: call succeeded",
                calld_->chand_, calld_, this);
      return false;
    }
  ...
  if (calld_->retry_throttle_data_ != nullptr &&
    ...
    grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: retries throttled",
              calld_->chand_, calld_, this);
    return false;
  }
...
RetryFilter::CallData::CallAttempt::BatchData::BatchData(
  ...
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: creating batch %p",
...
RetryFilter::CallData::CallAttempt::BatchData::~BatchData() {
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: destroying batch %p",
            call_attempt_->calld_->chand_, call_attempt_->calld_,
            call_attempt_.get(), this);
...
void RetryFilter::CallData::CallAttempt::BatchData::OnComplete(
  ...
  if (GPR_UNLIKELY(!calld->retry_committed_ && error != GRPC_ERROR_NONE &&
                   !call_attempt->completed_recv_trailing_metadata_)) {
    grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p attempt=%p: deferring on_complete",
              calld->chand_, calld, call_attempt);
...
void RetryFilter::CallData::StartTransportStreamOpBatch(
  ...
  if (GPR_UNLIKELY(batch->cancel_stream)) {
    ...
    grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: cancelled from surface: %s", chand_,
    ...
    if (retry_timer_pending_) {
      grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: cancelling retry timer", chand_,
                this);
  ...
  if (call_attempt_ == nullptr) {
    ...
    // pending batches.
    grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: creating call attempt", chand_,
              this);
    CreateCallAttempt();
    return;
  }
  // Send batches to call attempt.
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: starting batch on attempt=%p", chand_,
...
void RetryFilter::CallData::FreeCachedSendInitialMetadata() {
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: destroying send_initial_metadata",
            chand_, this);
...
void RetryFilter::CallData::FreeCachedSendTrailingMetadata() {
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: destroying send_trailing_metadata",
            chand_, this);
  ...
  if (batch->on_complete == nullptr &&
      ...
      (!batch->recv_trailing_metadata ||
       batch->payload->recv_trailing_metadata.recv_trailing_metadata_ready ==
           nullptr)) {
    grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: clearing pending batch", chand_,
              this);
    PendingBatchClear(pending);
  }
}
...
void RetryFilter::CallData::RetryCommit(CallAttempt* call_attempt) {
  ...
  grpc_retry_trace.Log(GPR_INFO, "chand=%p calld=%p: committing retries", chand_, this);
