 ...
 public:
  explicit XdsResolver(ResolverArgs args)
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_resolver_trace)) {
      gpr_log(GPR_INFO, "[xds_resolver %p] created for server name %s", this, server_name_.c_str());
    }
  }
 ...
 private:
  ...
  class ListenerWatcher : public XdsClient::ListenerWatcherInterface {
   public:
    ...
    void OnError(grpc_error_handle error) override { new Notifier(resolver_, error); }
  ...
  class ClusterState : public DualRefCounted<ClusterState> {
   public:
    using ClusterStateMap = std::map<std::string, WeakRefCountedPtr<ClusterState>>;

    ClusterState(RefCountedPtr<XdsResolver> resolver, const std::string& cluster_name)
        : resolver_(std::move(resolver)),
          it_(resolver_->cluster_state_map_.emplace(cluster_name, WeakRef()).first) {}
  ...
  // call is committed.
  class XdsCallDispatchController : public ConfigSelector::CallDispatchController {
   public:
    explicit XdsCallDispatchController(RefCountedPtr<ClusterState> cluster_state)
  ...
  class XdsConfigSelector : public ConfigSelector {
   public:
    XdsConfigSelector(RefCountedPtr<XdsResolver> resolver, grpc_error_handle* error);
    ...
    bool Equals(const ConfigSelector* other) const override {
      ...
      return route_table_ == other_xds->route_table_ && clusters_ == other_xds->clusters_;
    }

    CallConfig GetCallConfig(GetCallConfigArgs args) override;

    std::vector<const grpc_channel_filter*> GetFilters() override { return filters_; }
   ...
   private:
    ...
    grpc_error_handle CreateMethodConfig(const XdsApi::Route& route,
  ...
  grpc_error_handle CreateServiceConfig(RefCountedPtr<ServiceConfig>* service_config);
  void GenerateResult();
...
XdsResolver::Notifier::Notifier(RefCountedPtr<XdsResolver> resolver, XdsApi::LdsUpdate update)
    : resolver_(std::move(resolver)), update_(std::move(update)), type_(kLdsUpdate) {
  GRPC_CLOSURE_INIT(&closure_, &RunInExecCtx, this, nullptr);
  ExecCtx::Run(DEBUG_LOCATION, &closure_, GRPC_ERROR_NONE);
}

XdsResolver::Notifier::Notifier(RefCountedPtr<XdsResolver> resolver, XdsApi::RdsUpdate update)
...
XdsResolver::Notifier::Notifier(RefCountedPtr<XdsResolver> resolver, grpc_error_handle error)
...
void XdsResolver::Notifier::RunInExecCtx(void* arg, grpc_error_handle error) {
  ...
  self->resolver_->work_serializer_->Run([self, error]() { self->RunInWorkSerializer(error); },
                                         DEBUG_LOCATION);
}
...
void XdsResolver::Notifier::RunInWorkSerializer(grpc_error_handle error) {
  ...
  switch (type_) {
    case kLdsUpdate:
      ...
      break;
    case kRdsUpdate:
      resolver_->OnRouteConfigUpdate(std::move(*update_.http_connection_manager.rds_update));
      break;
    case kError:
...
bool XdsResolver::XdsConfigSelector::Route::operator==(const Route& other) const {
  return route == other.route && weighted_cluster_state == other.weighted_cluster_state &&
         MethodConfigsEqual(method_config.get(), other.method_config.get());
}
...
XdsResolver::XdsConfigSelector::XdsConfigSelector(RefCountedPtr<XdsResolver> resolver,
                                                  grpc_error_handle* error)
    : resolver_(std::move(resolver)) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_resolver_trace)) {
    gpr_log(GPR_INFO, "[xds_resolver %p] creating XdsConfigSelector %p", resolver_.get(), this);
  }
  ...
  for (auto& route : resolver_->current_virtual_host_.routes) {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_resolver_trace)) {
      gpr_log(GPR_INFO, "[xds_resolver %p] XdsConfigSelector %p: route: %s", resolver_.get(), this,
    ...
    if (!route.max_stream_duration.has_value()) {
      route_entry.route.max_stream_duration =
          resolver_->current_listener_.http_connection_manager.http_max_stream_duration;
    }
    if (route.weighted_clusters.empty()) {
      *error = CreateMethodConfig(route_entry.route, nullptr, &route_entry.method_config);
      ...
      for (const auto& weighted_cluster : route_entry.route.weighted_clusters) {
        ...
        *error = CreateMethodConfig(route_entry.route, &weighted_cluster,
        ...
        route_entry.weighted_cluster_state.push_back(std::move(cluster_weight_state));
  ...
  for (const auto& http_filter :
    ...
    if (http_filter.config.config_proto_type_name == kXdsHttpRouterFilterConfigName) {
      found_router = true;
      break;
    }
    ...
    const XdsHttpFilterImpl* filter_impl =
        XdsHttpFilterRegistry::GetFilterForType(http_filter.config.config_proto_type_name);
  ...
  if (!found_router) {
    filter_error_ = grpc_error_set_int(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("no xDS HTTP router filter configured"),
...
XdsResolver::XdsConfigSelector::~XdsConfigSelector() {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_resolver_trace)) {
    gpr_log(GPR_INFO, "[xds_resolver %p] destroying XdsConfigSelector %p", resolver_.get(), this);
  }
  clusters_.clear();
...
const XdsHttpFilterImpl::FilterConfig* FindFilterConfigOverride(
    const std::string& instance_name, const XdsApi::RdsUpdate::VirtualHost& vhost,
    const XdsApi::Route& route, const XdsApi::Route::ClusterWeight* cluster_weight) {
...
grpc_error_handle XdsResolver::XdsConfigSelector::CreateMethodConfig(
  ...
  // Set timeout.
  if (route.max_stream_duration.has_value() &&
      (route.max_stream_duration->seconds != 0 || route.max_stream_duration->nanos != 0)) {
  ...
  for (const auto& http_filter :
    ...
    const XdsHttpFilterImpl::FilterConfig* config_override = FindFilterConfigOverride(
        http_filter.name, resolver_->current_virtual_host_, route, cluster_weight);
    ...
    if (!method_config_field.ok()) {
      return GRPC_ERROR_CREATE_FROM_COPIED_STRING(
          absl::StrCat("failed to generate method config for HTTP filter ", http_filter.name, ": ",
                       method_config_field.status().ToString())
              .c_str());
    }
    per_filter_configs[method_config_field->service_config_field_name].push_back(
        method_config_field->element);
  }
...
grpc_channel_args* XdsResolver::XdsConfigSelector::ModifyChannelArgs(grpc_channel_args* args) {
  if (filter_error_ == GRPC_ERROR_NONE) return args;
  grpc_arg error_arg = MakeLameClientErrorArg(filter_error_);
  grpc_channel_args* new_args = grpc_channel_args_copy_and_add(args, &error_arg, 1);
  grpc_channel_args_destroy(args);
  return new_args;
}
...
absl::optional<absl::string_view> GetHeaderValue(grpc_metadata_batch* initial_metadata,
  ...
  return grpc_metadata_batch_get_value(initial_metadata, header_name, concatenated_value);
}
...
bool HeadersMatch(const std::vector<HeaderMatcher>& header_matchers,
  ...
  for (const auto& header_matcher : header_matchers) {
    std::string concatenated_value;
    if (!header_matcher.Match(
            GetHeaderValue(initial_metadata, header_matcher.name(), &concatenated_value))) {
      return false;
    }
  }
  return true;
...
absl::optional<uint64_t> HeaderHashHelper(const XdsApi::Route::HashPolicy& policy,
                                          grpc_metadata_batch* initial_metadata) {
...
ConfigSelector::CallConfig XdsResolver::XdsConfigSelector::GetCallConfig(GetCallConfigArgs args) {
  for (const auto& entry : route_table_) {
    // Path matching.
    if (!entry.route.matchers.path_matcher.Match(StringViewFromSlice(*args.path))) {
      continue;
    }
    // Header Matching.
    if (!HeadersMatch(entry.route.matchers.header_matchers, args.initial_metadata)) {
      continue;
    }
    ...
    if (entry.route.weighted_clusters.empty()) {
      ...
      const uint32_t key =
          rand() % entry.weighted_cluster_state[entry.weighted_cluster_state.size() - 1].range_end;
    ...
    for (const auto& hash_policy : entry.route.hash_policies) {
      ...
      switch (hash_policy.type) {
        ...
        case XdsApi::Route::HashPolicy::CHANNEL_ID:
          new_hash = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(resolver_.get()));
          break;
        default:
          GPR_ASSERT(0);
      }
    ...
    if (method_config != nullptr) {
      call_config.method_configs = method_config->GetMethodParsedConfigVector(grpc_empty_slice());
      call_config.service_config = std::move(method_config);
    }
    call_config.call_attributes[kXdsClusterAttribute] = it->first;
    std::string hash_string = absl::StrCat(hash.value());
    char* hash_value = static_cast<char*>(args.arena->Alloc(hash_string.size() + 1));
...
void XdsResolver::StartLocked() {
  ...
  grpc_pollset_set_add_pollset_set(xds_client_->interested_parties(), interested_parties_);
  channelz::ChannelNode* parent_channelz_node =
      grpc_channel_args_find_pointer<channelz::ChannelNode>(args_, GRPC_ARG_CHANNELZ_CHANNEL_NODE);
...
void XdsResolver::ShutdownLocked() {
  ...
  if (xds_client_ != nullptr) {
    ...
    channelz::ChannelNode* parent_channelz_node =
        grpc_channel_args_find_pointer<channelz::ChannelNode>(args_,
                                                              GRPC_ARG_CHANNELZ_CHANNEL_NODE);
    if (parent_channelz_node != nullptr) {
      xds_client_->RemoveChannelzLinkage(parent_channelz_node);
    }
    grpc_pollset_set_del_pollset_set(xds_client_->interested_parties(), interested_parties_);
    xds_client_.reset();
  }
}
...
void XdsResolver::OnListenerUpdate(XdsApi::LdsUpdate listener) {
  ...
  if (listener.http_connection_manager.route_config_name != route_config_name_) {
    if (route_config_watcher_ != nullptr) {
  ...
  if (route_config_name_.empty()) {
    GPR_ASSERT(current_listener_.http_connection_manager.rds_update.has_value());
    OnRouteConfigUpdate(std::move(*current_listener_.http_connection_manager.rds_update));
  } else {
...
void XdsResolver::OnRouteConfigUpdate(XdsApi::RdsUpdate rds_update) {
  ...
  XdsApi::RdsUpdate::VirtualHost* vhost = rds_update.FindVirtualHostForDomain(server_name_);
  if (vhost == nullptr) {
    OnError(GRPC_ERROR_CREATE_FROM_COPIED_STRING(
        absl::StrCat("could not find VirtualHost for ", server_name_, " in RouteConfiguration")
            .c_str()));
    return;
  }
...
void XdsResolver::OnError(grpc_error_handle error) {
  gpr_log(GPR_ERROR, "[xds_resolver %p] received error from XdsClient: %s", this,
          grpc_error_std_string(error).c_str());
  Result result;
...
void XdsResolver::OnResourceDoesNotExist() {
  ...
  current_virtual_host_.routes.clear();
  Result result;
  result.service_config = ServiceConfig::Create(args_, "{}", &result.service_config_error);
  GPR_ASSERT(result.service_config != nullptr);
  result.args = grpc_channel_args_copy(args_);
  result_handler_->ReturnResult(std::move(result));
}

grpc_error_handle XdsResolver::CreateServiceConfig(RefCountedPtr<ServiceConfig>* service_config) {
...
void XdsResolver::GenerateResult() {
  ...
  result.args = grpc_channel_args_copy_and_add(args_, new_args, GPR_ARRAY_SIZE(new_args));
  result_handler_->ReturnResult(std::move(result));
}
