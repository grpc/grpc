  ...
  class XdsConfigSelector : public ConfigSelector {
   public:
    XdsConfigSelector(RefCountedPtr<XdsResolver> resolver,
      ...
      for (auto& route : route_table_.routes) {
        if (route.weighted_clusters.empty()) {
          ...
          route.weighted_clusters.empty()
              ? route.cluster_name
              //? absl::StrFormat("cds:%s", route.cluster_name)
              : absl::StrFormat("weighted:%s", WeightedClustersActionName(
      ...
      gpr_log(GPR_INFO, "DONNAAA: RDS update copied to route_table %s",
              route_table_.ToString().c_str());
      {
        MutexLock lock(&resolver_->cluster_state_map_mu_);
    ...
    void UpdateWeightedClusterIndexMap(const XdsApi::RdsUpdate& rds_update) {
      ...
      // weighted target policy (same clusters and weights) appears in the old
      // map, then that old action name is taken again and should be moved to
      // the new map; any other action names from the old set of actions are
      // candidates for reuse.
      WeightedClusterIndexMap new_weighted_cluster_index_map;
      ...
      // Second pass of all remaining unique WeightedCluster actions: if
      // clusters for a new action are the same as an old unused action, reuse
      // the name.  If clusters differ, use a brand new name.
