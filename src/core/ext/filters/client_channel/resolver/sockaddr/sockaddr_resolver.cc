...
static void
sockaddr_shutdown_locked (grpc_resolver * resolver)
{
...
static void
sockaddr_channel_saw_error_locked (grpc_resolver * resolver)
{
  sockaddr_resolver *r = (sockaddr_resolver *) resolver;
  r->published = false;
...
static void
sockaddr_next_locked (grpc_resolver * resolver,
		      grpc_channel_args ** target_result,
		      grpc_closure * on_complete)
{
  sockaddr_resolver *r = (sockaddr_resolver *) resolver;
...
static void
sockaddr_maybe_finish_next_locked (sockaddr_resolver * r)
{
...
static void
sockaddr_destroy (grpc_resolver * gr)
{
...
sockaddr_create (grpc_resolver_args * args,
			     ...
			     grpc_resolved_address * dst))
{
  if (0 != strcmp (args->uri->authority, ""))
    {
      gpr_log (GPR_ERROR,
	       "authority based uri's not supported by the %s scheme",
	       args->uri->scheme);
      return nullptr;
    }
  /* Construct addresses. */
  ...
  grpc_lb_addresses *addresses =
    grpc_lb_addresses_create (path_parts.count,
			      nullptr /* user_data_vtable */ );
...
void
grpc_resolver_sockaddr_init (void)
{
...
void
grpc_resolver_sockaddr_shutdown (void)
{
}
