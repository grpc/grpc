...
static void dns_next_locked (grpc_resolver * r,
			     grpc_channel_args ** target_result,
...
static void
dns_shutdown_locked (grpc_resolver * resolver)
{
...
static void
dns_channel_saw_error_locked (grpc_resolver * resolver)
{
  dns_resolver *r = (dns_resolver *) resolver;
  if (!r->resolving)
    {
...
static void
dns_next_locked (grpc_resolver * resolver,
		 grpc_channel_args ** target_result,
		 grpc_closure * on_complete)
{
  dns_resolver *r = (dns_resolver *) resolver;
...
static void
dns_on_retry_timer_locked (void *arg, grpc_error * error)
{
...
static void
dns_on_resolved_locked (void *arg, grpc_error * error)
{
  dns_resolver *r = (dns_resolver *) arg;
  ...
  error = grpc_error_set_str (error, GRPC_ERROR_STR_TARGET_ADDRESS,
			      grpc_slice_from_copied_string (r->
							     name_to_resolve));
  if (r->addresses != nullptr)
	...
	{
	  grpc_lb_addresses_set_address (addresses, i,
...
dns_start_resolving_locked (dns_resolver * r)
{
  ...
  r->resolving = true;
  r->addresses = nullptr;
  grpc_resolve_address (r->name_to_resolve, r->default_port,
			r->interested_parties,
			GRPC_CLOSURE_CREATE (dns_on_resolved_locked, r,
					     grpc_combiner_scheduler (r->base.
								      combiner)),
			&r->addresses);
}
...
dns_create (grpc_resolver_args * args, const char *default_port)
{
...
dns_resolver_factory_create ()
{
  return &dns_resolver_factory;
}

void
grpc_resolver_dns_native_init (void)
{
...
void
grpc_resolver_dns_native_shutdown (void)
{
}
