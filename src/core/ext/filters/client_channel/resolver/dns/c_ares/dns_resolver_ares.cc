...
static void
dns_ares_shutdown_locked (grpc_resolver * resolver)
{
  ...
  if (r->next_completion != nullptr)
    {
      *r->target_result = nullptr;
      GRPC_CLOSURE_SCHED (r->next_completion,
...
static void
dns_ares_channel_saw_error_locked (grpc_resolver * resolver)
{
  ares_dns_resolver *r = (ares_dns_resolver *) resolver;
  if (!r->resolving)
    {
...
static void
dns_ares_on_retry_timer_locked (void *arg, grpc_error * error)
{
...
static bool
value_in_json_array (grpc_json * array, const char *value)
{
  for (grpc_json * entry = array->child; entry != nullptr;
       entry = entry->next)
    {
      if (entry->type == GRPC_JSON_STRING
...
static char *
choose_service_config (char *service_config_choice_json)
{
  grpc_json *choices_json =
    grpc_json_parse_string (service_config_choice_json);
...
static void
dns_ares_on_resolved_locked (void *arg, grpc_error * error)
{
  ares_dns_resolver *r = (ares_dns_resolver *) arg;
  ...
  if (r->lb_addresses != nullptr)
      ...
      if (r->service_config_json != nullptr)
	{
	  service_config_string =
	    choose_service_config (r->service_config_json);
	  ...
	  if (service_config_string != nullptr)
	    {
	      ...
	      new_args[num_args_to_add++] = grpc_channel_arg_string_create ((char *) GRPC_ARG_SERVICE_CONFIG, service_config_string);
	      service_config =
		grpc_service_config_create (service_config_string);
	      if (service_config != nullptr)
		{
		  ...
		  if (lb_policy_name != nullptr)
			...
			GRPC_ARG_LB_POLICY_NAME;
		      new_args[num_args_to_add++] =
			grpc_channel_arg_string_create ((char *)
							GRPC_ARG_LB_POLICY_NAME,
							(char *)
							lb_policy_name);
		    }
      ...
      result =
	grpc_channel_args_copy_and_add_and_remove (r->channel_args,
						   args_to_remove,
						   num_args_to_remove,
						   new_args, num_args_to_add);
      if (service_config != nullptr)
  ...
  else
    {
      ...
      if (timeout > 0)
	{
	  gpr_log (GPR_DEBUG, "retrying in %" PRIdPTR " milliseconds",
		   timeout);
	}
      else
	{
...
static void
dns_ares_next_locked (grpc_resolver * resolver,
		      grpc_channel_args ** target_result,
		      grpc_closure * on_complete)
{
...
static void
dns_ares_start_resolving_locked (ares_dns_resolver * r)
{
  ...
  r->pending_request =
    grpc_dns_lookup_ares (r->dns_server, r->name_to_resolve, r->default_port,
			  r->interested_parties,
			  &r->dns_ares_on_resolved_locked, &r->lb_addresses,
			  true /* check_grpclb */ ,
			  r->request_service_config ? &r->
			  service_config_json : nullptr);
}

static void
dns_ares_maybe_finish_next_locked (ares_dns_resolver * r)
{
  if (r->next_completion != nullptr &&
      r->resolved_version != r->published_version)
    {
      *r->target_result = r->resolved_result == nullptr
	? nullptr : grpc_channel_args_copy (r->resolved_result);
...
static void
dns_ares_destroy (grpc_resolver * gr)
{
...
dns_ares_create (grpc_resolver_args * args, const char *default_port)
{
  /* Get name from args. */
  ...
  const grpc_arg *arg =
    grpc_channel_args_find (r->channel_args,
			    GRPC_ARG_SERVICE_CONFIG_DISABLE_RESOLUTION);
  r->request_service_config =
    !grpc_channel_arg_get_integer (arg, (grpc_integer_options)
				   {
				   false, false, true});
  r->interested_parties = grpc_pollset_set_create ();
  if (args->pollset_set != nullptr)
    {
      grpc_pollset_set_add_pollset_set (r->interested_parties,
					args->pollset_set);
    }
...
static void
dns_ares_factory_ref (grpc_resolver_factory * factory)
{
}

static void
dns_ares_factory_unref (grpc_resolver_factory * factory)
{
}

static grpc_resolver *
dns_factory_create_resolver (grpc_resolver_factory * factory,
...
static char *
dns_ares_factory_get_default_host_name (grpc_resolver_factory * factory,
					grpc_uri * uri)
{
...
static grpc_resolver_factory dns_resolver_factory =
  { &dns_ares_factory_vtable };

static grpc_resolver_factory *
dns_ares_resolver_factory_create ()
{
  return &dns_resolver_factory;
}

void
grpc_resolver_dns_ares_init (void)
{
...
void
grpc_resolver_dns_ares_shutdown (void)
{
  char *resolver = gpr_getenv ("GRPC_DNS_RESOLVER");
  if (resolver != nullptr && gpr_stricmp (resolver, "ares") == 0)
    {
