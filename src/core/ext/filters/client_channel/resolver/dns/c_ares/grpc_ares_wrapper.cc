...
static void
grpc_ares_request_ref (grpc_ares_request * r)
{
  gpr_ref (&r->pending_queries);
}

static void
grpc_ares_request_unref (grpc_ares_request * r)
{
...
create_hostbyname_request (grpc_ares_request * parent_request, char *host,
...
static void
destroy_hostbyname_request (grpc_ares_hostbyname_request * hr)
{
...
static void
on_hostbyname_done_cb (void *arg, int status, int timeouts,
		       struct hostent *hostent)
{
  ...
  if (status == ARES_SUCCESS)
      ...
      for (i = 0; hostent->h_addr_list[i] != nullptr; i++)
	{
      ...
      (*lb_addresses)->addresses = (grpc_lb_address *) gpr_realloc ((*lb_addresses)->addresses, sizeof (grpc_lb_address) * (*lb_addresses)->num_addresses);
      for (i = prev_naddr; i < (*lb_addresses)->num_addresses; i++)
	{
	  switch (hostent->h_addrtype)
		...
		grpc_lb_addresses_set_address (*lb_addresses, i, &addr,
					       addr_len,
					       hr->
...
static void
on_srv_query_done_cb (void *arg, int status, int timeouts,
  ...
  if (status == ARES_SUCCESS)
      ...
      if (parse_status == ARES_SUCCESS)
	{
	  ares_channel *channel =
	    grpc_ares_ev_driver_get_channel (r->ev_driver);
	  for (struct ares_srv_reply * srv_it = reply; srv_it != nullptr;
		...
		{
		  grpc_ares_hostbyname_request *hr =
		    create_hostbyname_request (r, srv_it->host,
					       htons (srv_it->port),
					       true /* is_balancer */ );
...
static void
on_txt_done_cb (void *arg, int status, int timeouts,
		unsigned char *buf, int len)
{
  ...
  if (result != nullptr)
	...
	(char *) gpr_malloc (service_config_len + 1);
	...
	{
	  *r->service_config_json_out =
	    (char *) gpr_realloc (*r->service_config_json_out,
      ...
      gpr_log (GPR_INFO, "found service config: %s",
	       *r->service_config_json_out);
    }
  // Clean up.
  ares_free_data (reply);
  ...
  if (dns_server != nullptr)
    {
	  ...
	  (dns_server, &addr, false /* log_errors */ ))
	{
  ...
  if (service_config_json != nullptr)
    {
      ...
      ares_search (*channel, config_name, ns_c_in, ns_t_txt, on_txt_done_cb,
		   r);
...
static void
on_dns_lookup_done_cb (void *arg, grpc_error * error)
{
				...
				grpc_resolved_addresses ** addrs)
{
  ...
  grpc_dns_lookup_ares (nullptr /* dns_server */ , name, default_port,
			interested_parties, &r->on_dns_lookup_done,
			&r->lb_addrs, false /* check_grpclb */ ,
