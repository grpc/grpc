...
/* A wrapped fd that integrates with the grpc iomgr of the current
 * platform. A GrpcPolledFd knows how to create grpc platform-specific
 * iomgr endpoints from "ares_socket_t" sockets, and then sign up for
 * readability/writeability with that poller, and do shutdown and
 * destruction. */
class GrpcPolledFd {
 public:
  virtual ~GrpcPolledFd() {}
  /* Called when c-ares library is interested and there's no pending
   * callback */
  virtual void RegisterForOnReadableLocked(
      grpc_closure* read_closure) = 0;
  /* Called when c-ares library is interested and there's no pending
   * callback */
  virtual void RegisterForOnWriteableLocked(
      grpc_closure* write_closure) = 0;
