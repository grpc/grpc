...
static void grpc_ares_notify_on_event_locked (grpc_ares_ev_driver *
					      ev_driver);
...
{
  *ev_driver =
    (grpc_ares_ev_driver *) gpr_malloc (sizeof (grpc_ares_ev_driver));
...
void
grpc_ares_ev_driver_shutdown (grpc_ares_ev_driver * ev_driver)
{
  gpr_mu_lock (&ev_driver->mu);
  ev_driver->shutting_down = true;
...
static bool
grpc_ares_is_fd_still_readable (grpc_ares_ev_driver * ev_driver, int fd)
{
...
static void
on_readable_cb (void *arg, grpc_error * error)
{
...
static void
on_writable_cb (void *arg, grpc_error * error)
{
  fd_node *fdn = (fd_node *) arg;
  grpc_ares_ev_driver *ev_driver = fdn->ev_driver;
  gpr_mu_lock (&fdn->mu);
...
static void
grpc_ares_notify_on_event_locked (grpc_ares_ev_driver * ev_driver)
{
...
void
grpc_ares_ev_driver_start (grpc_ares_ev_driver * ev_driver)
{
  gpr_mu_lock (&ev_driver->mu);
