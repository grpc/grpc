 ...
 public:
  ...
  GrpcPolledFdWindows(ares_socket_t as, grpc_combiner* combiner)
      : read_buf_(grpc_empty_slice()),
        ...
        gotten_into_request_list_(false) {
  ...
  bool gotten_into_request_list() const { return gotten_into_request_list_; }
  void set_gotten_into_request_list() { gotten_into_request_list_ = true; }
  ...
  char* name_ = nullptr;
  bool gotten_into_request_list_;
};
 ...
 * with a GrpcPolledFdWindows factory and event request */
class SockToPolledFdMap {
 public:
  ...
  static int CloseSocket(SOCKET s, void* user_data) {
    ...
    map->RemoveEntry(s);
    // If a gRPC polled fd has not made it in to the request's list yet, then
    // the request has not and will never see this socket.
    if (!polled_fd->gotten_into_request_list()) {
...
class GrpcPolledFdFactoryWindows : public GrpcPolledFdFactory {
 public:
  ...
  GrpcPolledFd* NewGrpcPolledFdLocked(ares_socket_t as,
    ...
    // doesn't need to call ShutdownLocked, since now the request will.
    polled_fd->set_gotten_into_request_list();
    return polled_fd;
  }
