...
create_key (const grpc_subchannel_args * args,
	    grpc_channel_args *
	    (*copy_channel_args) (const grpc_channel_args * args))
{
  ...
  if (k->args.filter_count > 0)
    {
      k->args.filters =
	(const grpc_channel_filter **) gpr_malloc (sizeof (*k->args.filters) *
						   k->args.filter_count);
...
grpc_subchannel_key *
grpc_subchannel_key_create (const grpc_subchannel_args * args)
{
  return create_key (args, grpc_channel_args_normalize);
}

static grpc_subchannel_key *
subchannel_key_copy (grpc_subchannel_key * k)
{
  return create_key (&k->args, grpc_channel_args_copy);
}

int
grpc_subchannel_key_compare (const grpc_subchannel_key * a,
...
void
grpc_subchannel_key_destroy (grpc_subchannel_key * k)
{
...
static void
sck_avl_destroy (void *p, void *user_data)
{
  grpc_subchannel_key_destroy ((grpc_subchannel_key *) p);
}

static void *
sck_avl_copy (void *p, void *unused)
{
  return subchannel_key_copy ((grpc_subchannel_key *) p);
}

static long
sck_avl_compare (void *a, void *b, void *unused)
{
...
static void
scv_avl_destroy (void *p, void *user_data)
{
  GRPC_SUBCHANNEL_WEAK_UNREF ((grpc_subchannel *) p, "subchannel_index");
}

static void *
scv_avl_copy (void *p, void *unused)
{
...
void
grpc_subchannel_index_init (void)
{
...
void
grpc_subchannel_index_shutdown (void)
{
...
void
grpc_subchannel_index_unref (void)
{
...
void
grpc_subchannel_index_ref (void)
{
  gpr_ref_non_zero (&g_refcount);
}

grpc_subchannel *
grpc_subchannel_index_find (grpc_subchannel_key * key)
{
  // Lock, and take a reference to the subchannel index.
  // We don't need to do the search under a lock as avl's are immutable.
  gpr_mu_lock (&g_mu);
  gpr_avl index =
    gpr_avl_ref (g_subchannel_index, grpc_core::ExecCtx::Get ());
  gpr_mu_unlock (&g_mu);

  grpc_subchannel *c = GRPC_SUBCHANNEL_REF_FROM_WEAK_REF ((grpc_subchannel *)
							  gpr_avl_get (index,
								       key,
								       grpc_core::
								       ExecCtx::
...
grpc_subchannel *
grpc_subchannel_index_register (grpc_subchannel_key * key,
...
void
grpc_subchannel_index_unregister (grpc_subchannel_key * key,
				  grpc_subchannel * constructed)
{
  bool done = false;
  while (!done)
...
void
grpc_subchannel_index_test_only_set_force_creation (bool force_creation)
{
