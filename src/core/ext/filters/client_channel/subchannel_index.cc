...
static grpc_subchannel_key* create_key(
  ...
  grpc_subchannel_key* k =
      reinterpret_cast<grpc_subchannel_key*>(gpr_malloc(sizeof(*k)));
  k->args.filter_count = args->filter_count;
  if (k->args.filter_count > 0) {
    k->args.filters = reinterpret_cast<const grpc_channel_filter**>(
...
void grpc_subchannel_key_destroy(grpc_exec_ctx* exec_ctx,
  ...
  grpc_channel_args_destroy(exec_ctx,
                            const_cast<grpc_channel_args*>(k->args.args));
  gpr_free(k);
}

static void sck_avl_destroy(void* p, void* user_data) {
  grpc_exec_ctx* exec_ctx = reinterpret_cast<grpc_exec_ctx*>(user_data);
  grpc_subchannel_key_destroy(exec_ctx,
                              reinterpret_cast<grpc_subchannel_key*>(p));
}

static void* sck_avl_copy(void* p, void* unused) {
  return subchannel_key_copy(reinterpret_cast<grpc_subchannel_key*>(p));
}

static long sck_avl_compare(void* a, void* b, void* unused) {
  return grpc_subchannel_key_compare(reinterpret_cast<grpc_subchannel_key*>(a),
                                     reinterpret_cast<grpc_subchannel_key*>(b));
}
...
grpc_subchannel* grpc_subchannel_index_register(grpc_exec_ctx* exec_ctx,
  ...
  while (c == nullptr) {
    ...
    c = reinterpret_cast<grpc_subchannel*>(gpr_avl_get(index, key, exec_ctx));
    if (c != nullptr) {
...
void grpc_subchannel_index_unregister(grpc_exec_ctx* exec_ctx,
  ...
  while (!done) {
    ...
    grpc_subchannel* c =
        reinterpret_cast<grpc_subchannel*>(gpr_avl_get(index, key, exec_ctx));
    if (c != constructed) {
      gpr_avl_unref(index, exec_ctx);
      break;
    }
