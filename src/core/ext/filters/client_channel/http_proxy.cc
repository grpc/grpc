 ...
 */
static char *
get_http_proxy_server (char **user_cred)
{
...
static bool
proxy_mapper_map_name (grpc_proxy_mapper * mapper,
		       ...
		       char **name_to_resolve, grpc_channel_args ** new_args)
{
  ...
  *name_to_resolve = get_http_proxy_server (&user_cred);
  ...
  if (uri == nullptr || uri->path[0] == '\0')
    {
      gpr_log (GPR_ERROR,
	       ...
	       "parse server URI '%s' -- not using proxy", server_uri);
      goto no_use_proxy;
    }
	  ...
	  for (size_t i = 0; i < num_no_proxy_hosts; i++)
	    {
		  ...
		  gpr_stricmp (no_proxy_entry,
			       &server_host[uri_len - no_proxy_len]) == 0)
		{
		  gpr_log (GPR_INFO,
  ...
  args_to_add[0] = grpc_channel_arg_string_create ((char *)
  ...
  if (user_cred != nullptr)
	...
	grpc_base64_encode (user_cred, strlen (user_cred), 0, 0);
      char *header;
      gpr_asprintf (&header, "Proxy-Authorization:Basic %s",
		    encoded_user_cred);
      gpr_free (encoded_user_cred);
      args_to_add[1] = grpc_channel_arg_string_create ((char *)
...
static bool
proxy_mapper_map_address (grpc_proxy_mapper * mapper,
...
static void
proxy_mapper_destroy (grpc_proxy_mapper * mapper)
{
}
