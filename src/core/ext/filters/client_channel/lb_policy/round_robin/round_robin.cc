...
static size_t
get_next_ready_subchannel_index_locked (const round_robin_lb_policy * p)
{
  ...
  for (size_t i = 0; i < p->subchannel_list->num_subchannels; ++i)
      ...
      if (grpc_lb_round_robin_trace.enabled ())
	{
	  gpr_log (GPR_DEBUG,
		   ...
		   grpc_connectivity_state_name (p->subchannel_list->
						 subchannels[index].
	  ...
	  if (grpc_lb_round_robin_trace.enabled ())
	    {
	      gpr_log (GPR_DEBUG,
		       ...
		       "subchannel_list %p",
		       (void *) p,
		       (void *) p->subchannel_list->subchannels[index].
		       subchannel, (unsigned long) index,
  ...
  if (grpc_lb_round_robin_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "[RR %p] no subchannels in ready state",
	       (void *) p);
    }
...
static void
update_last_ready_subchannel_index_locked (round_robin_lb_policy * p,
...
static void
rr_shutdown_locked (grpc_lb_policy * pol, grpc_lb_policy * new_policy)
{
  round_robin_lb_policy *p = (round_robin_lb_policy *) pol;
  grpc_error *error =
...
static void
rr_cancel_pick_locked (grpc_lb_policy * pol,
...
static void
rr_cancel_picks_locked (grpc_lb_policy * pol,
...
static void
start_picking_locked (round_robin_lb_policy * p)
{
...
static void
rr_exit_idle_locked (grpc_lb_policy * pol)
{
  round_robin_lb_policy *p = (round_robin_lb_policy *) pol;
...
static int
rr_pick_locked (grpc_lb_policy * pol, grpc_lb_policy_pick_state * pick)
{
  round_robin_lb_policy *p = (round_robin_lb_policy *) pol;
  if (grpc_lb_round_robin_trace.enabled ())
    {
  ...
  if (p->subchannel_list != nullptr)
	...
	get_next_ready_subchannel_index_locked (p);
...
static void
update_state_counters_locked (grpc_lb_subchannel_data * sd)
{
...
static void
update_lb_connectivity_status_locked (grpc_lb_subchannel_data * sd,
				      grpc_error * error)
{
  ...
  round_robin_lb_policy *p =
    (round_robin_lb_policy *) subchannel_list->policy;
  ...
  else if (subchannel_list->num_shutdown == subchannel_list->num_subchannels)
    {
...
static void
rr_connectivity_changed_locked (void *arg, grpc_error * error)
{
	...
	if (sd->subchannel_list != p->subchannel_list)
	  {
		...
		const unsigned long num_subchannels =
		  p->subchannel_list != nullptr
		  ? (unsigned long) p->subchannel_list->num_subchannels : 0;
		gpr_log (GPR_DEBUG,
			 "[RR %p] phasing out subchannel list %p (size %lu) in favor "
			 "of %p (size %lu)",
			 (void *) p, (void *) p->subchannel_list,
			 num_subchannels, (void *) sd->subchannel_list,
			 num_subchannels);
	      }
	    if (p->subchannel_list != nullptr)
	      {
...
rr_check_connectivity_locked (grpc_lb_policy * pol, grpc_error ** error)
{
  round_robin_lb_policy *p = (round_robin_lb_policy *) pol;
...
static void
rr_notify_on_state_change_locked (grpc_lb_policy * pol,
...
static void
rr_ping_one_locked (grpc_lb_policy * pol, grpc_closure * on_initiate,
		    grpc_closure * on_ack)
{
  round_robin_lb_policy *p = (round_robin_lb_policy *) pol;
...
static void
rr_update_locked (grpc_lb_policy * policy, const grpc_lb_policy_args * args)
{
  ...
  if (arg == nullptr || arg->type != GRPC_ARG_POINTER)
    {
      gpr_log (GPR_ERROR, "[RR %p] update provided no addresses; ignoring",
	       p);
  ...
  if (grpc_lb_round_robin_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "[RR %p] received update with %" PRIuPTR " addresses", p,
	       addresses->num_addresses);
    }
  grpc_lb_subchannel_list *subchannel_list =
    grpc_lb_subchannel_list_create (&p->base, &grpc_lb_round_robin_trace,
				    addresses, args,
...
static void
rr_set_reresolve_closure_locked (grpc_lb_policy * policy,
...
round_robin_create (grpc_lb_policy_factory * factory,
		    grpc_lb_policy_args * args)
{
  GPR_ASSERT (args->client_channel_factory != nullptr);
  round_robin_lb_policy *p =
    (round_robin_lb_policy *) gpr_zalloc (sizeof (*p));
