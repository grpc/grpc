...
void RoundRobin::RoundRobinSubchannelData::UpdateConnectivityStateLocked(
  ...
  // READY, then we report the state change as-is for aggregation purposes.
  // However, once we do see a failure, we report TRANSIENT_FAILURE for
  // aggregation purposes and do not change the aggregation state until
  // we go back into state READY.
