 ...
 * used upon policy transition to RERESOLVE OR TRANSIENT_FAILURE. */
static void update_lb_connectivity_status_locked(grpc_exec_ctx *exec_ctx,
   ...
   *
   * 3) RULE: ALL subchannels are SHUTDOWN => policy is RERESOLVE.
  ...
  if (subchannel_list->num_ready > 0) { /* 1) READY */
    ...
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker,
             ...
             p->subchannel_list->num_subchannels) { /* 3) RERESOLVE */
    grpc_connectivity_state_set(exec_ctx, &p->state_tracker,
                                GRPC_CHANNEL_RERESOLVE, GRPC_ERROR_REF(error),
                                "rr_reresolve");
