...
static void
on_complete_for_send (void *arg, grpc_error * error)
{
  ...
  GRPC_CLOSURE_RUN (calld->original_on_complete_for_send,
...
init_call_elem (grpc_call_element * elem, const grpc_call_element_args * args)
{
  ...
  calld->client_stats = grpc_grpclb_client_stats_ref ((grpc_grpclb_client_stats *) args->context[GRPC_GRPCLB_CLIENT_STATS].value);
  // Record call started.
		   ...
		   grpc_closure * ignored)
{
  ...
  // received.
  grpc_grpclb_client_stats_add_call_finished (!calld->
					      send_initial_metadata_succeeded
					      /* client_failed_to_send */ ,
					      calld->
					      recv_initial_metadata_succeeded
					      /* known_received */ ,
					      calld->client_stats);
