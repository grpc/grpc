 ...
 * serviced by two callbacks, \a lb_on_server_status_received_ and \a
 * lb_on_response_received_. The former will be called when the call to the LB
...
class GlbLbCallData final : public grpc_core::InternallyRefCountedWithTracing {
 public:
  ...
  void StartQuery();
  // TODO(roth): should be removed when glb is C++-ized and a fiend class of
  // GlbLbCallData.
 ...
 private:
  ...
  // TODO(roth): should be private when glb is C++-ized and a fiend class of
  // GlbLbCallData.
  ...
  static void OnReceivedBalancerMessageLocked(void* arg, grpc_error* error);
...
GlbLbCallData::GlbLbCallData(glb_lb_policy* glb_policy)
  ...
  GRPC_CLOSURE_INIT(&lb_on_sent_initial_request_, OnSentInitialRequestLocked,
                    this, grpc_combiner_scheduler(glb_policy_->base.combiner));
  GRPC_CLOSURE_INIT(&lb_on_response_received_, OnReceivedBalancerMessageLocked,
                    this, grpc_combiner_scheduler(glb_policy_->base.combiner));
...
void GlbLbCallData::Orphan() {
  ...
  // todo move to caller
  glb_policy_->lb_calld = nullptr;
}

void GlbLbCallData::StartQuery() {
  GPR_ASSERT(lb_call_ != nullptr);
  ...
  op->reserved = nullptr;
  op++;
  Ref(DEBUG_LOCATION, "OnReceivedBalancerMessageLocked");
  call_error = grpc_call_start_batch_and_execute(
...
grpc_grpclb_client_stats* GlbLbCallData::ClientStats() { return client_stats_; }

void GlbLbCallData::OnSentInitialRequestLocked(void* arg, grpc_error* error) {
  ...
  lb_calld->Unref(DEBUG_LOCATION, "OnSentInitialRequestLocked");
}

void GlbLbCallData::OnReceivedBalancerMessageLocked(void* arg,
                                                    grpc_error* error) {
  GlbLbCallData* lb_calld = (GlbLbCallData*)arg;
  ...
  if (lb_calld != glb_policy->lb_calld ||
    ...
    lb_calld->Unref(DEBUG_LOCATION, "OnReceivedBalancerMessageLocked");
    return;
  }
  grpc_op ops[2];
  memset(ops, 0, sizeof(ops));
  ...
  if (!glb_policy->shutting_down) {
    ...
    op->reserved = nullptr;
    op++;
    // Reuse the "OnReceivedBalancerMessageLocked" ref taken in
    // query_for_backends_locked().
    const grpc_call_error call_error = grpc_call_start_batch_and_execute(
        lb_calld->lb_call_, ops, (size_t)(op - ops),
        &lb_calld->lb_on_response_received_);
    GPR_ASSERT(GRPC_CALL_OK == call_error);
  } else {
    lb_calld->Unref(DEBUG_LOCATION,
                    "OnReceivedBalancerMessageLocked+glb_shutdown");
  }
}

void GlbLbCallData::OnBalancerStatusReceivedLocked(void* arg,
                                                   grpc_error* error) {
  GlbLbCallData* lb_calld = (GlbLbCallData*)arg;
  glb_lb_policy* glb_policy = lb_calld->glb_policy_;
  GPR_ASSERT(lb_calld->lb_call_ != nullptr);
  ...
  if (lb_calld == glb_policy->lb_calld) {
    ...
    if (lb_calld->client_load_report_timer_callback_pending_) {
      grpc_timer_cancel(&lb_calld->client_load_report_timer_);
    }
...
void GlbLbCallData::SendClientLoadReportLocked() {
  ...
  GRPC_CLOSURE_INIT(&client_load_report_closure_, ClientLoadReportDoneLocked,
                    this, grpc_combiner_scheduler(glb_policy->base.combiner));
...
void GlbLbCallData::MaybeSendClientLoadReportLocked(void* arg,
  ...
  // been sent to send this (see OnSentInitialRequestLocked()).
...
void GlbLbCallData::ScheduleNextClientLoadReportLocked() {
...
void GlbLbCallData::ClientLoadReportDoneLocked(void* arg, grpc_error* error) {
  GlbLbCallData* lb_calld = (GlbLbCallData*)arg;
  glb_lb_policy* glb_policy = lb_calld->glb_policy_;
  grpc_byte_buffer_destroy(lb_calld->send_message_payload_);
  lb_calld->send_message_payload_ = nullptr;
...
bool GlbLbCallData::LoadReportCountersAreZero(grpc_grpclb_request* request) {
...
static bool pick_from_internal_rr_locked(glb_lb_policy* glb_policy,
  ...
  if (glb_policy->serverlist != nullptr) {
    ...
    if (server->drop) {
      ...
      if (glb_policy->lb_calld != nullptr &&
          glb_policy->lb_calld->ClientStats() != nullptr) {
        grpc_grpclb_client_stats_add_call_dropped_locked(
            server->load_balance_token, glb_policy->lb_calld->ClientStats());
      }
      if (force_async) {
  ...
  if (glb_policy->lb_calld != nullptr &&
      glb_policy->lb_calld->ClientStats() != nullptr) {
    pp->client_stats =
        grpc_grpclb_client_stats_ref(glb_policy->lb_calld->ClientStats());
  }
...
static void query_for_backends_locked(glb_lb_policy* glb_policy) {
  ...
  glb_policy->lb_calld = grpc_core::New<GlbLbCallData>(glb_policy);
  if (grpc_lb_glb_trace.enabled()) {
    gpr_log(GPR_INFO,
