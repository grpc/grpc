...
static void wrapped_rr_closure(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  if (wc_arg->rr_policy != NULL) {
    ...
    if (*wc_arg->target != NULL) {
      if (!GRPC_MDISNULL(wc_arg->lb_token)) {
        ...
        gpr_log(GPR_ERROR,
                ...
                "instance %p).",
                *wc_arg->target, wc_arg->rr_policy);
        abort();
      }
    ...
    if (GRPC_TRACER_ON(grpc_lb_glb_trace)) {
      gpr_log(GPR_INFO, "Unreffing RR %p", wc_arg->rr_policy);
    }
...
static bool pick_from_internal_rr_locked(
  ...
  if (glb_policy->serverlist != NULL) {
    ...
    if (server->drop) {
      ...
      if (GRPC_TRACER_ON(grpc_lb_glb_trace)) {
        gpr_log(GPR_INFO, "[grpclb %p] Unreffing RR for drop (0x%" PRIxPTR ")",
                glb_policy, (intptr_t)wc_arg->rr_policy);
      }
  ...
  if (pick_done) {
    ...
    if (GRPC_TRACER_ON(grpc_lb_glb_trace)) {
      gpr_log(GPR_INFO, "[grpclb %p] Unreffing RR (0x%" PRIxPTR ")", glb_policy,
