...
struct glb_lb_policy
{
  ...
  grpc_metadata_array lb_initial_metadata_recv;	/* initial MD from LB server */
    grpc_metadata_array lb_trailing_metadata_recv;	/* trailing MD from LB server */
 ...
 * metadata */
static grpc_error *
initial_metadata_add_lb_token (grpc_metadata_batch * initial_metadata,
			       grpc_linked_mdelem * lb_token_mdelem_storage,
			       grpc_mdelem lb_token)
{
  GPR_ASSERT (lb_token_mdelem_storage != nullptr);
  GPR_ASSERT (!GRPC_MDISNULL (lb_token));
  return grpc_metadata_batch_add_tail (initial_metadata,
				       lb_token_mdelem_storage, lb_token);
}

static void
destroy_client_stats (void *arg)
{
  grpc_grpclb_client_stats_unref ((grpc_grpclb_client_stats *) arg);
}

static void
pending_pick_set_metadata_and_context (pending_pick * pp)
{
...
static void
pending_pick_complete (void *arg, grpc_error * error)
{
...
static pending_pick *
pending_pick_create (glb_lb_policy * glb_policy,
		     grpc_lb_policy_pick_state * pick)
{
...
static void
pending_pick_add (pending_pick ** root, pending_pick * new_pp)
{
  new_pp->next = *root;
  *root = new_pp;
}
...
static void
pending_ping_add (pending_ping ** root, grpc_closure * on_initiate,
		  grpc_closure * on_ack)
{
...
static bool
is_server_valid (const grpc_grpclb_server * server, size_t idx, bool log)
{
  if (server->drop)
    return false;
  ...
  if (ip->size != 4 && ip->size != 16)
      ...
      if (log)
	{
	  gpr_log (GPR_ERROR,
		   ...
		   "serverlist. Ignoring", ip->size, (unsigned long) idx);
	}
      return false;
    }
  return true;
...
static void *
lb_token_copy (void *token)
{
  return token == nullptr ? nullptr : (void *) GRPC_MDELEM_REF (grpc_mdelem
								{
								(uintptr_t)
								token}).
    payload;
...
static void
lb_token_destroy (void *token)
{
  if (token != nullptr)
    {
      GRPC_MDELEM_UNREF (grpc_mdelem
			 {
			 (uintptr_t) token}
      );
    }
...
static int
lb_token_cmp (void *token1, void *token2)
{
  if (token1 > token2)
    return 1;
  if (token1 < token2)
...
static void
parse_server (const grpc_grpclb_server * server, grpc_resolved_address * addr)
{
...
process_serverlist_locked (const grpc_grpclb_serverlist * serverlist)
{
  size_t num_valid = 0;
  ...
  for (size_t i = 0; i < serverlist->num_servers; ++i)
    {
      if (is_server_valid (serverlist->servers[i], i, true))
	++num_valid;
    }
  ...
  for (size_t sl_idx = 0; sl_idx < serverlist->num_servers; ++sl_idx)
    {
      ...
      if (!is_server_valid (serverlist->servers[sl_idx], sl_idx, false))
	continue;
      ...
      if (server->has_load_balance_token)
	{
	  ...
	  grpc_slice lb_token_mdstr =
	    grpc_slice_from_copied_buffer (server->load_balance_token,
					   lb_token_length);
	  user_data =
	    (void *) grpc_mdelem_from_slices (GRPC_MDSTR_LB_TOKEN,
					      lb_token_mdstr).payload;
	}
      else
	{
...
static grpc_lb_addresses *
extract_backend_addresses_locked (const grpc_lb_addresses * addresses)
{
  ...
  for (size_t i = 0; i < addresses->num_addresses; ++i)
	...
	continue;
      ...
      grpc_lb_addresses_set_address (backend_addresses, num_copied,
				     &addr->addr, addr->len,
				     false /* is_balancer */ ,
				     nullptr /* balancer_name */ ,
				     (void *) GRPC_MDELEM_LB_TOKEN_EMPTY.
				     payload);
      ++num_copied;
    }
...
static void
update_lb_connectivity_status_locked (glb_lb_policy * glb_policy,
...
static bool
pick_from_internal_rr_locked (glb_lb_policy * glb_policy,
			      bool force_async, pending_pick * pp)
{
  ...
  if (glb_policy->serverlist != nullptr)
      ...
      if (server->drop)
	{
	  ...
	  grpc_grpclb_client_stats_add_call_dropped_locked (server->
							    load_balance_token,
							    glb_policy->
							    client_stats);
	  if (force_async)
  ...
  bool pick_done =
    grpc_lb_policy_pick_locked (glb_policy->rr_policy, pp->pick);
  if (pick_done)
    {
      pending_pick_set_metadata_and_context (pp);
      if (force_async)
...
lb_policy_args_create (glb_lb_policy * glb_policy)
{
  ...
  if (glb_policy->serverlist != nullptr)
  ...
  else
    {
      ...
      addresses =
	grpc_lb_addresses_copy (glb_policy->fallback_backend_addresses);
    }
  GPR_ASSERT (addresses != nullptr);
  grpc_lb_policy_args *args =
    (grpc_lb_policy_args *) gpr_zalloc (sizeof (*args));
  ...
  args->args =
    grpc_channel_args_copy_and_add_and_remove (glb_policy->args,
					       keys_to_remove,
					       GPR_ARRAY_SIZE
					       (keys_to_remove), &arg, 1);
  grpc_lb_addresses_destroy (addresses);
  return args;
}

static void
lb_policy_args_destroy (grpc_lb_policy_args * args)
{
...
static void
create_rr_locked (glb_lb_policy * glb_policy, grpc_lb_policy_args * args)
{
  ...
  grpc_lb_policy_notify_on_state_change_locked (glb_policy->rr_policy,
						&glb_policy->
						rr_connectivity_state,
						&glb_policy->
						on_rr_connectivity_changed);
  ...
  while ((pping = glb_policy->pending_pings))
	...
	{
	  gpr_log (GPR_INFO,
		   "[grpclb %p] Pending ping about to PING from RR %p",
		   glb_policy, glb_policy->rr_policy);
	}
...
static void
rr_handover_locked (glb_lb_policy * glb_policy)
{
  ...
  if (glb_policy->rr_policy != nullptr)
  ...
  else
    {
      ...
      if (grpc_lb_glb_trace.enabled ())
	{
	  gpr_log (GPR_DEBUG, "[grpclb %p] Created new RR policy %p",
		   glb_policy, glb_policy->rr_policy);
	}
    }
...
static void
on_rr_connectivity_changed_locked (void *arg, grpc_error * error)
{
  ...
  if (glb_policy->rr_connectivity_state == GRPC_CHANNEL_SHUTDOWN)
    {
      ...
      GRPC_LB_POLICY_UNREF (glb_policy->rr_policy,
...
static void
destroy_balancer_name (void *balancer_name)
{
  gpr_free (balancer_name);
}

static grpc_slice_hash_table_entry
targets_info_entry_create (const char *address, const char *balancer_name)
{
...
static int
balancer_name_cmp_fn (void *a, void *b)
{
...
build_lb_channel_args (const grpc_lb_addresses * addresses,
		       grpc_fake_resolver_response_generator *
		       response_generator, const grpc_channel_args * args)
{
  ...
  for (size_t i = 0; i < addresses->num_addresses; ++i)
	...
	targets_info_entry_create (addr_str,
				   addresses->addresses[i].balancer_name);
      gpr_free (addr_str);

      grpc_lb_addresses_set_address (lb_addresses, lb_addresses_idx++,
				     addresses->addresses[i].address.addr,
  ...
  grpc_channel_args *result =
    grpc_channel_args_copy_and_add (lb_channel_args,
				    &lb_channel_addresses_arg, 1);
...
static void
glb_destroy (grpc_lb_policy * pol)
{
  ...
  grpc_fake_resolver_response_generator_unref (glb_policy->
					       response_generator);
...
static void
glb_shutdown_locked (grpc_lb_policy * pol, grpc_lb_policy * new_policy)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) pol;
  grpc_error *error =
  ...
  if (glb_policy->rr_policy != nullptr)
    ...
    {
      grpc_lb_policy_try_reresolve (pol, &grpc_lb_glb_trace,
...
static void
glb_cancel_pick_locked (grpc_lb_policy * pol,
...
//   we invoke the completion closure and set *target to nullptr right here.
static void
glb_cancel_picks_locked (grpc_lb_policy * pol,
			 ...
			 grpc_error * error)
{
  ...
  if (glb_policy->rr_policy != nullptr)
    {
      grpc_lb_policy_cancel_picks_locked (glb_policy->rr_policy,
...
static void
start_picking_locked (glb_lb_policy * glb_policy)
{
  ...
  if (glb_policy->lb_fallback_timeout_ms > 0 &&
	...
	glb_policy->lb_fallback_timeout_ms;
      GRPC_LB_POLICY_REF (&glb_policy->base, "grpclb_fallback_timer");
      GRPC_CLOSURE_INIT (&glb_policy->lb_on_fallback,
			 lb_on_fallback_timer_locked, glb_policy,
			 grpc_combiner_scheduler (glb_policy->base.combiner));
...
static void
glb_exit_idle_locked (grpc_lb_policy * pol)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) pol;
...
static int
glb_pick_locked (grpc_lb_policy * pol, grpc_lb_policy_pick_state * pick)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) pol;
  ...
  if (glb_policy->rr_policy != nullptr)
	...
	{			// RR not in shutdown
	  ...
	  GPR_ASSERT (glb_policy->client_stats != nullptr);
	  pick_done =
	    pick_from_internal_rr_locked (glb_policy,
					  false /* force_async */ , pp);
	}
    }
  else
...
glb_check_connectivity_locked (grpc_lb_policy * pol,
...
static void
glb_ping_one_locked (grpc_lb_policy * pol, grpc_closure * on_initiate,
		     grpc_closure * on_ack)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) pol;
  if (glb_policy->rr_policy)
    {
      grpc_lb_policy_ping_one_locked (glb_policy->rr_policy, on_initiate,
				      on_ack);
    }
  else
    {
...
static void
glb_notify_on_state_change_locked (grpc_lb_policy * pol,
...
static void
lb_call_on_retry_timer_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
...
static void
maybe_restart_lb_call (glb_lb_policy * glb_policy)
{
  if (glb_policy->started_picking && glb_policy->updating_lb_call)
      ...
      if (grpc_lb_glb_trace.enabled ())
	{
	  ...
	  if (timeout > 0)
	    ...
	    {
	      gpr_log (GPR_DEBUG,
		       "[grpclb %p] ... retry LB call immediately.",
		       glb_policy);
	    }
	}
...
static void
schedule_next_client_load_report (glb_lb_policy * glb_policy)
{
...
static void
client_load_report_done_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
...
static void
do_send_client_load_report_locked (glb_lb_policy * glb_policy)
{
  grpc_op op;
  ...
  grpc_call_error call_error =
    grpc_call_start_batch_and_execute (glb_policy->lb_call, &op, 1,
				       &glb_policy->
				       client_load_report_closure);
  if (call_error != GRPC_CALL_OK)
    {
      gpr_log (GPR_ERROR, "[grpclb %p] call_error=%d", glb_policy,
	       call_error);
...
static bool
load_report_counters_are_zero (grpc_grpclb_request * request)
{
...
static void
send_client_load_report_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
...
static void lb_on_server_status_received_locked (void *arg,
						 grpc_error * error);
static void lb_on_response_received_locked (void *arg, grpc_error * error);
static void
lb_call_init_locked (glb_lb_policy * glb_policy)
{
  ...
  glb_policy->lb_call =
    grpc_channel_create_pollset_set_call (glb_policy->lb_channel, nullptr,
...
static void
lb_call_destroy_locked (glb_lb_policy * glb_policy)
{
  GPR_ASSERT (glb_policy->lb_call != nullptr);
 ...
 */
static void
query_for_backends_locked (glb_lb_policy * glb_policy)
{
  ...
  call_error =
    grpc_call_start_batch_and_execute (glb_policy->lb_call, ops,
				       (size_t) (op - ops),
				       &glb_policy->
				       lb_on_sent_initial_request);
  GPR_ASSERT (GRPC_CALL_OK == call_error);
  ...
  GRPC_LB_POLICY_REF (&glb_policy->base,
		      "lb_on_server_status_received_locked");
  call_error =
    grpc_call_start_batch_and_execute (glb_policy->lb_call, ops,
				       (size_t) (op - ops),
...
static void
lb_on_sent_initial_request_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
...
static void
lb_on_response_received_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
  grpc_op ops[2];
  memset (ops, 0, sizeof (ops));
  ...
  if (glb_policy->lb_response_payload != nullptr)
	  ...
	  nullptr)
	{
	  if (response->has_client_stats_report_interval)
		...
		GPR_MAX (GPR_MS_PER_SEC,
			 grpc_grpclb_duration_to_millis (&response->
							 client_stats_report_interval));
	      if (grpc_lb_glb_trace.enabled ())
		{
		  gpr_log (GPR_INFO,
			   "[grpclb %p] Received initial LB response message; "
			   "client load reporting interval = %" PRIdPTR
			   " milliseconds", glb_policy,
			   glb_policy->client_stats_report_interval);
		}
	  ...
	  else if (grpc_lb_glb_trace.enabled ())
	    {
	      gpr_log (GPR_INFO,
		       ...
		       "reporting NOT enabled", glb_policy);
	    }
	  ...
	  if (serverlist != nullptr)
	      ...
	      if (serverlist->num_servers > 0)
		{
		  if (grpc_grpclb_serverlist_equals (glb_policy->serverlist,
		      ...
		      if (glb_policy->serverlist != nullptr)
			{
			  ...
			  grpc_grpclb_destroy_serverlist (glb_policy->
							  serverlist);
			}
		      else
			{
			  /* or dispose of the fallback */
			  grpc_lb_addresses_destroy (glb_policy->
						     fallback_backend_addresses);
			  glb_policy->fallback_backend_addresses = nullptr;
			  if (glb_policy->fallback_timer_callback_pending)
			    {
			      grpc_timer_cancel (&glb_policy->
						 lb_fallback_timer);
			    }
			}
	    ...
	    {			/* serverlist == nullptr */
	      gpr_log (GPR_ERROR,
		       ...
		       glb_policy,
		       grpc_dump_slice (response_slice,
      ...
      if (!glb_policy->shutting_down)
	{
	  ...
	  op->data.recv_message.recv_message =
	    &glb_policy->lb_response_payload;
	  op->flags = 0;
	  op->reserved = nullptr;
	  op++;
...
static void
lb_on_fallback_timer_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
...
static void
lb_on_server_status_received_locked (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
  GPR_ASSERT (glb_policy->lb_call != nullptr);
  if (grpc_lb_glb_trace.enabled ())
...
static void
fallback_update_locked (glb_lb_policy * glb_policy,
...
static void
glb_update_locked (grpc_lb_policy * policy, const grpc_lb_policy_args * args)
{
  ...
  // resolver.
  grpc_channel_args *lb_channel_args =
    build_lb_channel_args (addresses, glb_policy->response_generator,
			   args->args);
  grpc_fake_resolver_response_generator_set_response (glb_policy->
						      response_generator,
						      lb_channel_args);
  grpc_channel_args_destroy (lb_channel_args);
  ...
  if (!glb_policy->watching_lb_channel)
	...
	grpc_channel_check_connectivity_state (glb_policy->lb_channel,
					       true /* try to connect */ );
	...
	grpc_channel_stack_last_element (grpc_channel_get_channel_stack
      ...
      grpc_client_channel_watch_connectivity_state (client_channel_elem,
						    ...
						    (glb_policy->base.
						     interested_parties),
						    &glb_policy->
						    lb_channel_connectivity,
						    &glb_policy->
						    lb_channel_on_connectivity_changed,
						    nullptr);
    }
}
...
static void
glb_lb_channel_on_connectivity_changed_cb (void *arg, grpc_error * error)
{
  glb_lb_policy *glb_policy = (glb_lb_policy *) arg;
  if (glb_policy->shutting_down)
    goto done;
...
static void
glb_set_reresolve_closure_locked (grpc_lb_policy * policy,
...
glb_create (grpc_lb_policy_factory * factory, grpc_lb_policy_args * args)
{
  ...
  if (num_grpclb_addrs == 0)
    return nullptr;

  glb_lb_policy *glb_policy =
    (glb_lb_policy *) gpr_zalloc (sizeof (*glb_policy));
  ...
  glb_policy->lb_call_timeout_ms = grpc_channel_arg_get_integer (arg,
								 {
								 0, 0, INT_MAX}
  );

  arg =
    grpc_channel_args_find (args->args, GRPC_ARG_GRPCLB_FALLBACK_TIMEOUT_MS);
  glb_policy->lb_fallback_timeout_ms = grpc_channel_arg_get_integer (arg,
								     {
  ...
  glb_policy->args =
    grpc_channel_args_copy_and_add_and_remove (args->args, args_to_remove,
					       GPR_ARRAY_SIZE
					       (args_to_remove), &new_arg, 1);
  ...
  glb_policy->lb_channel =
    grpc_lb_policy_grpclb_create_lb_channel (uri_str,
...
static void
glb_factory_ref (grpc_lb_policy_factory * factory)
{
}

static void
glb_factory_unref (grpc_lb_policy_factory * factory)
{
}
...
static bool
maybe_add_client_load_reporting_filter (grpc_channel_stack_builder * builder,
					void *arg)
{
