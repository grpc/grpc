...
grpc_grpclb_client_stats_create ()
{
...
void
grpc_grpclb_client_stats_unref (grpc_grpclb_client_stats * client_stats)
{
  if (gpr_unref (&client_stats->refs))
    {
      grpc_grpclb_dropped_call_counts_destroy (client_stats->
					       drop_token_counts);
...
void
grpc_grpclb_client_stats_add_call_started (grpc_grpclb_client_stats *
					   client_stats)
{
...
void
grpc_grpclb_client_stats_add_call_finished (bool
					    ...
					    client_stats)
{
  ...
  if (finished_known_received)
    {
      gpr_atm_full_fetch_add (&client_stats->
			      num_calls_finished_known_received, (gpr_atm) 1);
    }
}

void
grpc_grpclb_client_stats_add_call_dropped_locked (char *token,
						  grpc_grpclb_client_stats *
						  client_stats)
{
  ...
  drop_token_counts->token_counts =
    (grpc_grpclb_drop_token_count *) gpr_realloc (drop_token_counts->
						  token_counts,
						  new_num_entries *
						  sizeof
...
static void
atomic_get_and_reset_counter (int64_t * value, gpr_atm * counter)
{
...
void
grpc_grpclb_client_stats_get_locked (grpc_grpclb_client_stats * client_stats,
				     ...
				     drop_token_counts)
{
  ...
  atomic_get_and_reset_counter (num_calls_finished_with_client_failed_to_send,
				&client_stats->
				num_calls_finished_with_client_failed_to_send);
  atomic_get_and_reset_counter (num_calls_finished_known_received,
				&client_stats->
				num_calls_finished_known_received);
...
void
grpc_grpclb_dropped_call_counts_destroy (grpc_grpclb_dropped_call_counts *
					 drop_entries)
{
