...
void GrpcLbClientStats::AddCallFinished(bool finished_with_client_failed_to_send,
                                        bool finished_known_received) {
  gpr_atm_full_fetch_add(&num_calls_finished_, (gpr_atm)1);
  if (finished_with_client_failed_to_send) {
    gpr_atm_full_fetch_add(&num_calls_finished_with_client_failed_to_send_, (gpr_atm)1);
  }
...
void GrpcLbClientStats::AddCallDropped(const char* token) {
  ...
  drop_token_counts_->emplace_back(grpc_core::UniquePtr<char>(gpr_strdup(token)), 1);
}
...
void GrpcLbClientStats::Get(int64_t* num_calls_started, int64_t* num_calls_finished,
  ...
  AtomicGetAndResetCounter(num_calls_finished_known_received, &num_calls_finished_known_received_);
  MutexLock lock(&drop_count_mu_);
