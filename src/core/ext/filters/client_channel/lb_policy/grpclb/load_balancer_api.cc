...
bool GrpcLbServer::operator==(const GrpcLbServer& other) const {
  ...
  r = strncmp(load_balance_token, other.load_balance_token, sizeof(load_balance_token));
  if (r != 0) return false;
...
grpc_slice grpc_grpclb_request_encode(const grpc_lb_v1_LoadBalanceRequest* request,
                                      upb_arena* arena) {
  size_t buf_length;
  char* buf = grpc_lb_v1_LoadBalanceRequest_serialize(request, arena, &buf_length);
...
grpc_slice GrpcLbRequestCreate(const char* lb_service_name, upb_arena* arena) {
  ...
  size_t name_len = GPR_MIN(strlen(lb_service_name), GRPC_GRPCLB_SERVICE_NAME_MAX_LENGTH);
  grpc_lb_v1_InitialLoadBalanceRequest_set_name(initial_request,
                                                upb_strview_make(lb_service_name, name_len));
...
grpc_slice GrpcLbLoadReportRequestCreate(
  ...
  google_protobuf_Timestamp_assign(grpc_lb_v1_ClientStats_mutable_timestamp(req_stats, arena),
  ...
  grpc_lb_v1_ClientStats_set_num_calls_finished_known_received(req_stats,
  ...
  if (drop_token_counts != nullptr) {
    for (size_t i = 0; i < drop_token_counts->size(); ++i) {
      ...
      grpc_lb_v1_ClientStatsPerToken_set_load_balance_token(cur_msg,
                                                            upb_strview_make(token, token_len));
  ...
  if (server_count > 0) {
    ...
    for (size_t i = 0; i < server_count; ++i) {
      ...
      if (token.size == 0) {
        ...
        gpr_log(GPR_ERROR, "grpc_lb_v1_LoadBalanceResponse has too long token. len=%zu",
                token.size);
      }
...
grpc_millis grpc_grpclb_duration_to_millis(const google_protobuf_Duration* duration_pb) {
  return static_cast<grpc_millis>((google_protobuf_Duration_seconds(duration_pb) * GPR_MS_PER_SEC) +
...
bool GrpcLbResponseParse(const grpc_slice& serialized_response, upb_arena* arena,
  ...
  grpc_lb_v1_LoadBalanceResponse* response = grpc_lb_v1_LoadBalanceResponse_parse(
      reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(serialized_response)),
  ...
  auto* initial_response = grpc_lb_v1_LoadBalanceResponse_initial_response(response);
  if (initial_response != nullptr) {
    result->type = result->INITIAL;
    const google_protobuf_Duration* client_stats_report_interval =
        grpc_lb_v1_InitialLoadBalanceResponse_client_stats_report_interval(initial_response);
