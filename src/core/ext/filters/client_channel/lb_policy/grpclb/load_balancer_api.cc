...
static bool count_serverlist(pb_istream_t* stream, const pb_field_t* field,
                             void** arg) {
  grpc_grpclb_serverlist* sl = reinterpret_cast<grpc_grpclb_serverlist*>(*arg);
...
static bool decode_serverlist(pb_istream_t* stream, const pb_field_t* field,
                              void** arg) {
  decode_serverlist_arg* dec_arg =
      reinterpret_cast<decode_serverlist_arg*>(*arg);
  GPR_ASSERT(dec_arg->serverlist->num_servers >= dec_arg->decoding_idx);
  grpc_grpclb_server* server = reinterpret_cast<grpc_grpclb_server*>(
...
grpc_grpclb_request* grpc_grpclb_request_create(const char* lb_service_name) {
  grpc_grpclb_request* req = reinterpret_cast<grpc_grpclb_request*>(
...
static bool encode_drops(pb_ostream_t* stream, const pb_field_t* field,
  ...
  grpc_grpclb_dropped_call_counts* drop_entries =
      reinterpret_cast<grpc_grpclb_dropped_call_counts*>(*arg);
...
grpc_grpclb_initial_response* grpc_grpclb_initial_response_parse(
  ...
  grpc_grpclb_initial_response* initial_res =
      reinterpret_cast<grpc_grpclb_initial_response*>(
...
grpc_grpclb_serverlist* grpc_grpclb_response_parse_serverlist(
  ...
  grpc_grpclb_serverlist* sl = reinterpret_cast<grpc_grpclb_serverlist*>(
  ...
  if (sl->num_servers > 0) {
    sl->servers = reinterpret_cast<grpc_grpclb_server**>(
...
grpc_grpclb_serverlist* grpc_grpclb_serverlist_copy(
  ...
  grpc_grpclb_serverlist* copy = reinterpret_cast<grpc_grpclb_serverlist*>(
      gpr_zalloc(sizeof(grpc_grpclb_serverlist)));
  copy->num_servers = sl->num_servers;
  memcpy(&copy->expiration_interval, &sl->expiration_interval,
         sizeof(grpc_grpclb_duration));
  copy->servers = reinterpret_cast<grpc_grpclb_server**>(
      gpr_malloc(sizeof(grpc_grpclb_server*) * sl->num_servers));
  for (size_t i = 0; i < sl->num_servers; i++) {
    copy->servers[i] = reinterpret_cast<grpc_grpclb_server*>(
