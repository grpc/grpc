...
static bool
count_serverlist (pb_istream_t * stream, const pb_field_t * field, void **arg)
{
...
/* invoked once for every Server in ServerList */
static bool
decode_serverlist (pb_istream_t * stream, const pb_field_t * field,
		   void **arg)
{
...
grpc_grpclb_request *
grpc_grpclb_request_create (const char *lb_service_name)
{
...
static void
populate_timestamp (gpr_timespec timestamp,
...
static bool
encode_string (pb_ostream_t * stream, const pb_field_t * field,
	       void *const *arg)
{
  char *str = (char *) *arg;
...
static bool
encode_drops (pb_ostream_t * stream, const pb_field_t * field,
	      void *const *arg)
{
  ...
  for (size_t i = 0; i < drop_entries->num_entries; ++i)
	...
	return false;
      ...
      drop_message.load_balance_token.arg =
	drop_entries->token_counts[i].token;
      drop_message.has_num_calls = true;
      drop_message.num_calls = drop_entries->token_counts[i].count;
      if (!pb_encode_submessage
	  (stream, grpc_lb_v1_ClientStatsPerToken_fields, &drop_message))
	{
	  return false;
	}
    }
...
grpc_grpclb_request *
grpc_grpclb_load_report_request_create_locked (grpc_grpclb_client_stats *
					       client_stats)
{
  grpc_grpclb_request *req =
  ...
  populate_timestamp (gpr_now (GPR_CLOCK_REALTIME),
		      &req->client_stats.timestamp);
  ...
  grpc_grpclb_client_stats_get_locked (client_stats,
...
grpc_slice
grpc_grpclb_request_encode (const grpc_grpclb_request * request)
{
...
void
grpc_grpclb_request_destroy (grpc_grpclb_request * request)
{
...
grpc_grpclb_initial_response_parse (grpc_slice encoded_grpc_grpclb_response)
{
  pb_istream_t stream =
    pb_istream_from_buffer (GRPC_SLICE_START_PTR
			    (encoded_grpc_grpclb_response),
...
grpc_grpclb_response_parse_serverlist (grpc_slice
				       encoded_grpc_grpclb_response)
{
  pb_istream_t stream =
    pb_istream_from_buffer (GRPC_SLICE_START_PTR
  ...
  if (sl->num_servers > 0)
	...
	(grpc_grpclb_server **) gpr_zalloc (sizeof (grpc_grpclb_server *) *
      ...
      status =
	pb_decode (&stream_at_start, grpc_lb_v1_LoadBalanceResponse_fields,
		   &res);
      if (!status)
	{
...
void
grpc_grpclb_destroy_serverlist (grpc_grpclb_serverlist * serverlist)
{
...
grpc_grpclb_serverlist *
grpc_grpclb_serverlist_copy (const grpc_grpclb_serverlist * sl)
{
...
bool
grpc_grpclb_serverlist_equals (const grpc_grpclb_serverlist * lhs,
...
bool
grpc_grpclb_server_equals (const grpc_grpclb_server * lhs,
...
int
grpc_grpclb_duration_compare (const grpc_grpclb_duration * lhs,
...
grpc_millis
grpc_grpclb_duration_to_millis (grpc_grpclb_duration * duration_pb)
{
  return (grpc_millis) ((duration_pb->has_seconds ? duration_pb->
			 seconds : 0) * GPR_MS_PER_SEC +
			(duration_pb->has_nanos ? duration_pb->nanos : 0) /
			GPR_NS_PER_MS);
}

void
grpc_grpclb_initial_response_destroy (grpc_grpclb_initial_response * response)
{
  gpr_free (response);
}
