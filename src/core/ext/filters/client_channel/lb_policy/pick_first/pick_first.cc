...
static void
pf_shutdown_locked (grpc_lb_policy * pol, grpc_lb_policy * new_policy)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
  grpc_error *error =
  ...
  if (p->latest_pending_subchannel_list != nullptr)
    {
      grpc_lb_subchannel_list_shutdown_and_unref (p->
						  latest_pending_subchannel_list,
						  "pf_shutdown");
...
static void
pf_cancel_pick_locked (grpc_lb_policy * pol,
		       grpc_lb_policy_pick_state * pick, grpc_error * error)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
...
static void
pf_cancel_picks_locked (grpc_lb_policy * pol,
...
static void
start_picking_locked (pick_first_lb_policy * p)
{
  ...
  if (p->subchannel_list != nullptr &&
      ...
      for (size_t i = 0; i < p->subchannel_list->num_subchannels; ++i)
	{
	  if (p->subchannel_list->subchannels[i].subchannel != nullptr)
	    {
	      grpc_lb_subchannel_list_ref_for_connectivity_watch (p->
								  subchannel_list,
								  "connectivity_watch+start_picking");
	      grpc_lb_subchannel_data_start_connectivity_watch (&p->
								subchannel_list->
								subchannels
								[i]);
	      break;
...
static void
pf_exit_idle_locked (grpc_lb_policy * pol)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
...
static int
pf_pick_locked (grpc_lb_policy * pol, grpc_lb_policy_pick_state * pick)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
...
static void
destroy_unselected_subchannels_locked (pick_first_lb_policy * p)
{
  for (size_t i = 0; i < p->subchannel_list->num_subchannels; ++i)
    {
...
pf_check_connectivity_locked (grpc_lb_policy * pol, grpc_error ** error)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
...
static void
pf_notify_on_state_change_locked (grpc_lb_policy * pol,
...
static void
pf_ping_one_locked (grpc_lb_policy * pol, grpc_closure * on_initiate,
		    grpc_closure * on_ack)
{
  pick_first_lb_policy *p = (pick_first_lb_policy *) pol;
  if (p->selected)
...
static void
pf_update_locked (grpc_lb_policy * policy, const grpc_lb_policy_args * args)
{
  ...
  grpc_lb_subchannel_list *subchannel_list =
    grpc_lb_subchannel_list_create (&p->base, &grpc_lb_pick_first_trace,
				    addresses, args,
	  ...
	  if (sd->subchannel == p->selected->subchannel)
	      ...
	      if (grpc_lb_pick_first_trace.enabled ())
		{
		  gpr_log (GPR_INFO,
			   ...
			   "at update index %" PRIuPTR " of %" PRIuPTR
			   "; update done", p, p->selected->subchannel, i,
  ...
  if (p->started_picking)
    {
      ...
      grpc_lb_subchannel_data_start_connectivity_watch (&subchannel_list->
							subchannels[0]);
    }
}

static void
pf_connectivity_changed_locked (void *arg, grpc_error * error)
{
  grpc_lb_subchannel_data *sd = (grpc_lb_subchannel_data *) arg;
  pick_first_lb_policy *p =
    (pick_first_lb_policy *) sd->subchannel_list->policy;
  if (grpc_lb_pick_first_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       "Pick First %p connectivity changed for subchannel %p (%"
	       PRIuPTR " of %" PRIuPTR
	       "), subchannel_list %p: state=%s p->shutdown=%d "
	       "sd->subchannel_list->shutting_down=%d error=%s", (void *) p,
	       (void *) sd->subchannel,
	       sd->subchannel_list->checking_subchannel,
	       sd->subchannel_list->num_subchannels,
	       (void *) sd->subchannel_list,
	       grpc_connectivity_state_name (sd->
  ...
  if (p->shutdown)
    {
      ...
      grpc_lb_subchannel_list_unref_for_connectivity_watch (sd->
							    subchannel_list,
							    "pf_shutdown");
      return;
    }
	  ...
	  if (sd->curr_connectivity_state == GRPC_CHANNEL_TRANSIENT_FAILURE)
	    {
	      ...
	      grpc_lb_policy_try_reresolve (&p->base,
	      ...
	      grpc_lb_subchannel_data_unref_subchannel (sd, "pf_selected_shutdown");	// Unrefs connected subchannel
	    }
	  else
	    {
	      grpc_connectivity_state_set (&p->state_tracker,
	...
	do
	  {
	    sd->subchannel_list->checking_subchannel =
	      ...
	      sd->subchannel_list->num_subchannels;
	    sd =
	      &sd->subchannel_list->subchannels[sd->subchannel_list->
						checking_subchannel];
	  }
...
static void
pf_set_reresolve_closure_locked (grpc_lb_policy * policy,
...
create_pick_first (grpc_lb_policy_factory * factory,
