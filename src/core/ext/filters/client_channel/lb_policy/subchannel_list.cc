...
void
grpc_lb_subchannel_data_unref_subchannel (grpc_lb_subchannel_data * sd,
					  const char *reason)
{
  if (sd->subchannel != nullptr)
	...
	{
	  gpr_log (GPR_DEBUG,
		   "[%s %p] subchannel list %p index %" PRIuPTR " of %"
		   PRIuPTR " (subchannel %p): unreffing subchannel",
		   sd->subchannel_list->tracer->name (),
...
void
grpc_lb_subchannel_data_start_connectivity_watch (grpc_lb_subchannel_data *
						  sd)
{
  if (sd->subchannel_list->tracer->enabled ())
    {
  ...
  grpc_subchannel_notify_on_state_change (sd->subchannel,
					  sd->subchannel_list->policy->
					  interested_parties,
					  &sd->
...
void
grpc_lb_subchannel_data_stop_connectivity_watch (grpc_lb_subchannel_data * sd)
{
  if (sd->subchannel_list->tracer->enabled ())
    {
      gpr_log (GPR_DEBUG,
...
grpc_lb_subchannel_list_create (grpc_lb_policy * p,
				...
				grpc_iomgr_cb_func connectivity_changed_cb)
{
  ...
  if (tracer->enabled ())
    {
      gpr_log (GPR_DEBUG,
	       ...
	       " subchannels", tracer->name (), p, subchannel_list,
  ...
  subchannel_list->subchannels =
    (grpc_lb_subchannel_data *) gpr_zalloc (sizeof (grpc_lb_subchannel_data) *
					    addresses->num_addresses);
  ...
  for (size_t i = 0; i < addresses->num_addresses; i++)
	...
	grpc_channel_args_copy_and_add_and_remove (args->args, keys_to_remove,
						   GPR_ARRAY_SIZE
						   (keys_to_remove),
						   &addr_arg,
						   1);
	...
	grpc_client_channel_factory_create_subchannel (args->
      ...
      if (tracer->enabled ())
	{
	  ...
	  gpr_log (GPR_DEBUG,
		   ...
		   tracer->name (), p, subchannel_list, subchannel_index,
		   subchannel, address_uri);
	  gpr_free (address_uri);
	}
...
static void
subchannel_list_destroy (grpc_lb_subchannel_list * subchannel_list)
{
...
void
grpc_lb_subchannel_list_ref (grpc_lb_subchannel_list * subchannel_list,
			     const char *reason)
{
  ...
  if (subchannel_list->tracer->enabled ())
    {
      const gpr_atm count =
	gpr_atm_acq_load (&subchannel_list->refcount.count);
...
void
grpc_lb_subchannel_list_unref (grpc_lb_subchannel_list * subchannel_list,
			       const char *reason)
{
...
void
grpc_lb_subchannel_list_ref_for_connectivity_watch (grpc_lb_subchannel_list *
						    subchannel_list,
						    const char *reason)
{
...
void
grpc_lb_subchannel_list_unref_for_connectivity_watch (grpc_lb_subchannel_list
						      * subchannel_list,
						      const char *reason)
{
...
static void
subchannel_data_cancel_connectivity_watch (grpc_lb_subchannel_data * sd,
					   const char *reason)
{
  if (sd->subchannel_list->tracer->enabled ())
    {
...
void
grpc_lb_subchannel_list_shutdown_and_unref (grpc_lb_subchannel_list *
					    subchannel_list,
					    const char *reason)
{
  if (subchannel_list->tracer->enabled ())
