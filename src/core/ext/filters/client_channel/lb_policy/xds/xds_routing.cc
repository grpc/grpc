...
class XdsRoutingLb : public LoadBalancingPolicy {
 ...
 private:
  ...
  class RoutePicker : public SubchannelPicker {
   ...
   private:
    RouteTable route_table_;
    // Take a reference to config so that we can use XdsApi::Route::Matchers
    // from it.
...
bool PathMatch(
  ...
  switch (path_matcher.type) {
    case XdsApi::Route::Matchers::PathMatcher::PathMatcherType::
        PREFIX:
...
bool HeaderMatchHelper(
  ...
  if (!value.has_value()) {
    if (header_matcher.type == XdsApi::Route::Matchers::
  ...
  switch (header_matcher.type) {
    case XdsApi::Route::Matchers::HeaderMatcher::
...
bool HeadersMatch(
    const std::vector<XdsApi::Route::Matchers::HeaderMatcher>&
        header_matchers,
    LoadBalancingPolicy::MetadataInterface* initial_metadata) {
              ...
              if (header_matcher_seen) {
                ...
                if (header_it->second.type() != Json::Type::OBJECT) {
                  ...
                  if (header_matcher.range_end > header_matcher.range_start) {
                    header_matcher.type = XdsApi::Route::
