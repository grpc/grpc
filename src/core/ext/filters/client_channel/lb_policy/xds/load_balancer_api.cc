...
xds_grpclb_request* xds_grpclb_request_create(const char* lb_service_name) {
  xds_grpclb_request* req = static_cast<xds_grpclb_request*>(
      gpr_malloc(sizeof(xds_grpclb_request)));
...
xds_grpclb_request* xds_grpclb_load_report_request_create_locked(
  ...
  xds_grpclb_request* req = static_cast<xds_grpclb_request*>(
      gpr_zalloc(sizeof(xds_grpclb_request)));
...
xds_grpclb_serverlist* xds_grpclb_serverlist_copy(
  ...
  for (size_t i = 0; i < sl->num_servers; i++) {
    copy->servers[i] = static_cast<xds_grpclb_server*>(
        gpr_malloc(sizeof(xds_grpclb_server)));
