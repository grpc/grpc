 ...
 private:
  ...
  class BalancerCallState
   ...
   private:
    ...
    XdsLb* grpclb_policy() const {
      return static_cast<XdsLb*>(grpclb_policy_.get());
    }
...
void ParseServer(const xds_grpclb_server* server,
...
XdsLb::BalancerCallState::BalancerCallState(
  ...
  xds_grpclb_request* request =
      xds_grpclb_request_create(grpclb_policy()->server_name_);
...
void XdsLb::BalancerCallState::MaybeSendClientLoadReportLocked(
  ...
  XdsLb* grpclb_policy = lb_calld->grpclb_policy();
...
void XdsLb::BalancerCallState::OnBalancerMessageReceivedLocked(
  ...
  if (!lb_calld->seen_initial_response_ &&
    ...
    if (serverlist->num_servers > 0) {
      ...
      if (xds_grpclb_serverlist_equals(grpclb_policy->serverlist_,
                                       serverlist)) {
        if (grpc_lb_xds_trace.enabled()) {
          gpr_log(GPR_INFO,
        ...
        if (grpclb_policy->serverlist_ != nullptr) {
          ...
          xds_grpclb_destroy_serverlist(grpclb_policy->serverlist_);
        } else {
...
void XdsLb::BalancerCallState::OnBalancerStatusReceivedLocked(
  ...
  grpclb_policy->TryReresolutionLocked(&grpc_lb_xds_trace, GRPC_ERROR_NONE);
    ...
    : LoadBalancingPolicy(args),
      ...
      lb_call_backoff_(
          BackOff::Options()
              ...
              .set_max_backoff(GRPC_XDS_RECONNECT_MAX_BACKOFF_SECONDS *
                               1000)) {
  // Initialization.
...
void XdsLb::OnFallbackTimerLocked(void* arg, grpc_error* error) {
  XdsLb* grpclb_policy = static_cast<XdsLb*>(arg);
