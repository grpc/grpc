...
const char* kXdsLocalityNameAttributeKey = "io.grpc.xds_locality_name";
...
std::unique_ptr<ServerAddress::AttributeInterface> MakeLocalityNameAttribute(
  ...
  return absl::make_unique<XdsLocalityAttribute>(std::move(locality_name));
}
...
RefCountedPtr<XdsLocalityName> GetLocalityAttribute(
  ...
  return locality_attr->locality_name();
}
