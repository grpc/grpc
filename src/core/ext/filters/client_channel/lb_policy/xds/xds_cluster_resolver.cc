...
XdsClusterResolverLbConfig::DiscoveryMechanism::JsonLoader() {
...
void XdsClusterResolverLbConfig::DiscoveryMechanism::JsonPostLoad(
  ...
  if (it != json.object_value().end()) {
    ...
    if (it->second.type() != Json::Type::OBJECT) {
      ...
      lrs_load_reporting_server.emplace(
  ...
  // Parse "outlierDetection", if enabled.
  if (XdsOutlierDetectionEnabled()) {
    ...
    outlier_detection_lb_config = LoadJsonObjectField<Json::Object>(
        json.object_value(), "outlierDetection", errors, /*required=*/false);
  }
  // Parse "type".
  {
    auto type_field =
        LoadJsonObjectField<std::string>(json.object_value(), "type", errors);
  ...
  if (type == DiscoveryMechanismType::EDS) {
    auto value = LoadJsonObjectField<std::string>(
        json.object_value(), "edsServiceName", errors, /*required=*/false);
      ...
      if (it->second.type() != Json::Type::ARRAY) {
        ...
        for (size_t i = 0; i < array.size(); ++i) {
          ...
          }
          {
            ...
            if (policy_it != policy.end()) {
              LoadFromJson<RingHashConfig>(policy_it->second, errors);
 ...
 public:
  ...
  ParseLoadBalancingConfig(const Json& json) const override {
    ...
    auto config = LoadFromJson<XdsClusterResolverLbConfig>(
        json, "errors validating xds_cluster_resolver LB policy config");
