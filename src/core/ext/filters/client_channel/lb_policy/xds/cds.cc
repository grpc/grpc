...
class CdsLbConfig : public LoadBalancingPolicy::Config {
 public:
  explicit CdsLbConfig(std::string cluster)
...
class CdsLb : public LoadBalancingPolicy {
 ...
 private:
  ...
  class Helper : public ChannelControlHelper {
   public:
    explicit Helper(RefCountedPtr<CdsLb> parent)
...
void CdsLb::ClusterWatcher::Notifier::RunInExecCtx(
...
void CdsLb::Helper::UpdateState(
    grpc_connectivity_state state, const absl::Status& status,
    std::unique_ptr<SubchannelPicker> picker) {
  if (parent_->shutting_down_ || parent_->child_policy_ == nullptr)
    return;
  if (GRPC_TRACE_FLAG_ENABLED(grpc_cds_lb_trace)) {
    gpr_log(GPR_INFO,
            "[cdslb %p] state updated by child: %s message_state: (%s)",
...
void CdsLb::UpdateLocked(UpdateArgs args) {
  ...
  if (old_config == nullptr ||
      old_config->cluster() != config_->cluster()) {
    if (old_config != nullptr) {
      for (auto& watcher : watchers_) {
        if (GRPC_TRACE_FLAG_ENABLED(grpc_cds_lb_trace)) {
          gpr_log(GPR_INFO,
...
// This method will attempt to generate one or multiple entries of
// discovery mechanism recursively: For cluster types EDS or
// LOGICAL_DNS, one discovery mechanism entry may be generated cluster
// name, type and other data from the CdsUpdate inserted into the entry
// and the entry appended to the array of entries. Note, discovery
// mechanism entry can be generated if an CdsUpdate is available;
// otherwise, just return false. For cluster type AGGREGATE, recursively
// call the method for each child cluster.
bool CdsLb::GenerateDiscoveryMechanismForCluster(
  ...
  if (state.watcher == nullptr) {
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_cds_lb_trace)) {
      gpr_log(GPR_INFO, "[cdslb %p] starting watch for cluster %s",
              this, name.c_str());
    }
...
void CdsLb::OnClusterChanged(const std::string& name,
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_cds_lb_trace)) {
    gpr_log(GPR_INFO,
            "[cdslb %p] received CDS update for cluster %s from xds "
            "client %p: %s",
            this, name.c_str(), xds_client_.get(),
            cluster_data.ToString().c_str());
  }
  // Store the update in the map if we are still interested in watching
  // this cluster (i.e., it is not cancelled already). If we've already
  // deleted this entry, then this is an update notification that was
  ...
  // discovery mechanisms. If we don't have some of the data we need
  // (i.e., we just started up and not all watchers have returned data
  // yet), then don't update the child policy at all.
  Json::Array discovery_mechanisms;
  std::set<std::string> clusters_needed;
  if (GenerateDiscoveryMechanismForCluster(config_->cluster(),
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_cds_lb_trace)) {
      ...
      gpr_log(GPR_INFO,
              "[cdslb %p] generated config for child policy: %s", this,
              json_str.c_str());
    }
...
grpc_error_handle CdsLb::UpdateXdsCertificateProvider(
  ...
  if (xds_certificate_provider_ == nullptr) {
    xds_certificate_provider_ =
        MakeRefCounted<XdsCertificateProvider>();
  }
  // Configure root cert.
  ...
  if (!root_provider_instance_name.empty()) {
    new_root_provider = xds_client_->certificate_provider_store()
...
class CdsLbFactory : public LoadBalancingPolicyFactory {
 public:
  OrphanablePtr<LoadBalancingPolicy> CreateLoadBalancingPolicy(
    ...
    return MakeOrphanable<CdsLb>(std::move(xds_client),
                                 std::move(args));
  }
  ...
  RefCountedPtr<LoadBalancingPolicy::Config> ParseLoadBalancingConfig(
    ...
    if (json.type() == Json::Type::JSON_NULL) {
      // xds was mentioned as a policy in the deprecated
      // loadBalancingPolicy field or in the client API.
      *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
          "field:loadBalancingPolicy error:cds policy requires "
          "configuration. "
