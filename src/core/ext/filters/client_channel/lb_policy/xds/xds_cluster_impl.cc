...
class CircuitBreakerCallCounterMap {
 public:
  using Key = std::pair<std::string /*cluster*/, std::string /*eds_service_name*/>;
...
RefCountedPtr<CircuitBreakerCallCounterMap::CallCounter> CircuitBreakerCallCounterMap::GetOrCreate(
...
class XdsClusterImplLbConfig : public LoadBalancingPolicy::Config {
 public:
  XdsClusterImplLbConfig(RefCountedPtr<LoadBalancingPolicy::Config> child_policy,
      ...
      : child_policy_(std::move(child_policy)),
        ...
        lrs_load_reporting_server_name_(std::move(lrs_load_reporting_server_name)),
  ...
  RefCountedPtr<XdsApi::EdsUpdate::DropConfig> drop_config() const { return drop_config_; }
...
class XdsClusterImplLb : public LoadBalancingPolicy {
 ...
 private:
  class StatsSubchannelWrapper : public DelegatingSubchannel {
   public:
    StatsSubchannelWrapper(RefCountedPtr<SubchannelInterface> wrapped_subchannel,
                           RefCountedPtr<XdsClusterLocalityStats> locality_stats)
        : DelegatingSubchannel(std::move(wrapped_subchannel)),
          locality_stats_(std::move(locality_stats)) {}

    XdsClusterLocalityStats* locality_stats() const { return locality_stats_.get(); }
  ...
  class Picker : public SubchannelPicker {
   public:
    Picker(XdsClusterImplLb* xds_cluster_impl_lb, RefCountedPtr<RefCountedPicker> picker);
  ...
  class Helper : public ChannelControlHelper {
   public:
    ...
    ~Helper() override { xds_cluster_impl_policy_.reset(DEBUG_LOCATION, "Helper"); }
  ...
  void UpdateChildPolicyLocked(ServerAddressList addresses, const grpc_channel_args* args);
...
XdsClusterImplLb::Picker::Picker(XdsClusterImplLb* xds_cluster_impl_lb,
    ...
    : call_counter_(xds_cluster_impl_lb->call_counter_),
      max_concurrent_requests_(xds_cluster_impl_lb->config_->max_concurrent_requests()),
      drop_config_(xds_cluster_impl_lb->config_->drop_config()),
      drop_stats_(xds_cluster_impl_lb->drop_stats_),
      picker_(std::move(picker)) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO, "[xds_cluster_impl_lb %p] constructed new picker %p", xds_cluster_impl_lb,
            this);
  }
}

LoadBalancingPolicy::PickResult XdsClusterImplLb::Picker::Pick(LoadBalancingPolicy::PickArgs args) {
  // Handle EDS drops.
  ...
  if (picker_ == nullptr) {  // Should never happen.
    ...
    result.error = grpc_error_set_int(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("xds_cluster_impl picker not given any child picker"),
  ...
  if (result.type == result.PICK_COMPLETE && result.subchannel != nullptr) {
    ...
    if (drop_stats_ != nullptr) {  // If load reporting is enabled.
      auto* subchannel_wrapper = static_cast<StatsSubchannelWrapper*>(result.subchannel.get());
    ...
    auto original_recv_trailing_metadata_ready = result.recv_trailing_metadata_ready;
    result.recv_trailing_metadata_ready =
        // Note: This callback does not run in either the control plane
        // work serializer or in the data plane mutex.
        [locality_stats, original_recv_trailing_metadata_ready, call_counter](
            grpc_error_handle error, MetadataInterface* metadata, CallState* call_state) {
...
XdsClusterImplLb::XdsClusterImplLb(RefCountedPtr<XdsClient> xds_client, Args args)
    : LoadBalancingPolicy(std::move(args)), xds_client_(std::move(xds_client)) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO, "[xds_cluster_impl_lb %p] created -- using xds client %p", this,
            xds_client_.get());
  }
}
...
XdsClusterImplLb::~XdsClusterImplLb() {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO, "[xds_cluster_impl_lb %p] destroying xds_cluster_impl LB policy", this);
  }
}
...
void XdsClusterImplLb::UpdateLocked(UpdateArgs args) {
  ...
  if (is_initial_update) {
    if (config_->lrs_load_reporting_server_name().has_value()) {
      drop_stats_ =
          xds_client_->AddClusterDropStats(config_->lrs_load_reporting_server_name().value(),
                                           config_->cluster_name(), config_->eds_service_name());
    }
    call_counter_ =
        g_call_counter_map->GetOrCreate(config_->cluster_name(), config_->eds_service_name());
  } else {
  ...
  if (is_initial_update ||
      config_->max_concurrent_requests() != old_config->max_concurrent_requests()) {
  ...
  if (picker_ != nullptr) {
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
      gpr_log(GPR_INFO,
              ...
              "status=(%s) "
              "picker=%p",
              this, ConnectivityStateName(state_), status_.ToString().c_str(), drop_picker.get());
    }
    channel_control_helper()->UpdateState(state_, status_, std::move(drop_picker));
  }
}
...
OrphanablePtr<LoadBalancingPolicy> XdsClusterImplLb::CreateChildPolicyLocked(
  ...
  OrphanablePtr<LoadBalancingPolicy> lb_policy = MakeOrphanable<ChildPolicyHandler>(
      std::move(lb_policy_args), &grpc_xds_cluster_impl_lb_trace);
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO, "[xds_cluster_impl_lb %p] Created new child policy handler %p", this,
            lb_policy.get());
  }
...
void XdsClusterImplLb::UpdateChildPolicyLocked(ServerAddressList addresses,
  ...
  grpc_arg cluster_arg =
      grpc_channel_arg_string_create(const_cast<char*>(GRPC_ARG_XDS_CLUSTER_NAME),
                                     const_cast<char*>(config_->cluster_name().c_str()));
  update_args.args = grpc_channel_args_copy_and_add(args, &cluster_arg, 1);
  // Update the policy.
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO, "[xds_cluster_impl_lb %p] Updating child policy handler %p", this,
            child_policy_.get());
  }
...
RefCountedPtr<SubchannelInterface> XdsClusterImplLb::Helper::CreateSubchannel(
  ...
  if (xds_cluster_impl_policy_->config_->lrs_load_reporting_server_name().has_value()) {
    RefCountedPtr<XdsLocalityName> locality_name;
    auto* attribute = address.GetAttribute(kXdsLocalityNameAttributeKey);
    if (attribute != nullptr) {
      const auto* locality_attr = static_cast<const XdsLocalityAttribute*>(attribute);
      locality_name = locality_attr->locality_name();
    }
    RefCountedPtr<XdsClusterLocalityStats> locality_stats =
        xds_cluster_impl_policy_->xds_client_->AddClusterLocalityStats(
            *xds_cluster_impl_policy_->config_->lrs_load_reporting_server_name(),
            xds_cluster_impl_policy_->config_->cluster_name(),
            xds_cluster_impl_policy_->config_->eds_service_name(), std::move(locality_name));
    return MakeRefCounted<StatsSubchannelWrapper>(
        xds_cluster_impl_policy_->channel_control_helper()->CreateSubchannel(std::move(address),
                                                                             args),
        std::move(locality_stats));
  }
  // Load reporting not enabled, so don't wrap the subchannel.
  return xds_cluster_impl_policy_->channel_control_helper()->CreateSubchannel(std::move(address),
                                                                              args);
}

void XdsClusterImplLb::Helper::UpdateState(grpc_connectivity_state state,
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_cluster_impl_lb_trace)) {
    gpr_log(GPR_INFO,
            ...
            "state=%s (%s) "
            "picker=%p",
            xds_cluster_impl_policy_.get(), ConnectivityStateName(state), status.ToString().c_str(),
            picker.get());
  }
  // Save the state and picker.
  xds_cluster_impl_policy_->state_ = state;
  xds_cluster_impl_policy_->status_ = status;
  xds_cluster_impl_policy_->picker_ = MakeRefCounted<RefCountedPicker>(std::move(picker));
...
void XdsClusterImplLb::Helper::AddTraceEvent(TraceSeverity severity, absl::string_view message) {
  if (xds_cluster_impl_policy_->shutting_down_) return;
  xds_cluster_impl_policy_->channel_control_helper()->AddTraceEvent(severity, message);
}
...
class XdsClusterImplLbFactory : public LoadBalancingPolicyFactory {
 public:
  OrphanablePtr<LoadBalancingPolicy> CreateLoadBalancingPolicy(
    ...
    return MakeOrphanable<XdsClusterImplLb>(std::move(xds_client), std::move(args));
  }
  ...
  RefCountedPtr<LoadBalancingPolicy::Config> ParseLoadBalancingConfig(
    ...
    if (it == json.object_value().end()) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("field:childPolicy error:required field missing"));
    } else {
    ...
    if (it != json.object_value().end()) {
      if (it->second.type() != Json::Type::NUMBER) {
        ...
        max_concurrent_requests = gpr_parse_nonnegative_int(it->second.string_value().c_str());
      }
    }
    // Drop config.
    ...
    if (it == json.object_value().end()) {
      ...
      if (!child_errors.empty()) {
        error_list.push_back(GRPC_ERROR_CREATE_FROM_VECTOR("field:dropCategories", &child_errors));
      }
    }
    if (!error_list.empty()) {
      *error = GRPC_ERROR_CREATE_FROM_VECTOR("xds_cluster_impl_experimental LB policy config",
                                             &error_list);
      return nullptr;
    }
    return MakeRefCounted<XdsClusterImplLbConfig>(
        std::move(child_policy), std::move(cluster_name), std::move(eds_service_name),
        std::move(lrs_load_reporting_server_name), max_concurrent_requests, std::move(drop_config));
  }
 ...
 private:
  static std::vector<grpc_error_handle> ParseDropCategories(
    ...
    if (json.type() != Json::Type::ARRAY) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("dropCategories field is not an array"));
      return error_list;
    }
    for (size_t i = 0; i < json.array_value().size(); ++i) {
      const Json& entry = json.array_value()[i];
      std::vector<grpc_error_handle> child_errors = ParseDropCategory(entry, drop_config);
      if (!child_errors.empty()) {
        grpc_error_handle error =
            GRPC_ERROR_CREATE_FROM_COPIED_STRING(absl::StrCat("errors parsing index ", i).c_str());
  ...
  static std::vector<grpc_error_handle> ParseDropCategory(
    ...
    if (json.type() != Json::Type::OBJECT) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("dropCategories entry is not an object"));
      return error_list;
    }
    std::string category;
    auto it = json.object_value().find("category");
    if (it == json.object_value().end()) {
      error_list.push_back(GRPC_ERROR_CREATE_FROM_STATIC_STRING("\"category\" field not present"));
    } else if (it->second.type() != Json::Type::STRING) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("\"category\" field is not a string"));
    } else {
    ...
    if (it == json.object_value().end()) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("\"requests_per_million\" field is not present"));
    } else if (it->second.type() != Json::Type::NUMBER) {
      error_list.push_back(
          GRPC_ERROR_CREATE_FROM_STATIC_STRING("\"requests_per_million\" field is not a number"));
    } else {
      requests_per_million = gpr_parse_nonnegative_int(it->second.string_value().c_str());
    }
    if (error_list.empty()) {
...
void grpc_lb_policy_xds_cluster_impl_shutdown() { delete grpc_core::g_call_counter_map; }
