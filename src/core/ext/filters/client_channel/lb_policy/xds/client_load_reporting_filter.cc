...
struct call_data {
  ...
  grpc_core::XdsLbClientStats::LocalityStats* locality_stats;
...
static void destroy_call_elem(grpc_call_element* elem,
  ...
  if (calld->locality_stats != nullptr) {
    // Record call finished, optionally setting client_failed_to_send and
    // received.
    calld->locality_stats->AddCallFinished(
        !calld->recv_initial_metadata_succeeded);
  }
  calld->~call_data();
}
...
static void start_transport_stream_op_batch(
  ...
  if (batch->send_initial_metadata) {
    ...
    grpc_linked_mdelem* locality_stats_md =
        batch->payload->send_initial_metadata.send_initial_metadata->idx.named
            .locality_stats;
    if (locality_stats_md != nullptr) {
      grpc_core::XdsLbClientStats::LocalityStats* locality_stats =
          static_cast<grpc_core::XdsLbClientStats::LocalityStats*>(
              grpc_mdelem_get_user_data(
                  locality_stats_md->md,
                  grpc_core::XdsLbClientStats::LocalityStats::Destroy));
      if (locality_stats != nullptr) {
        calld->locality_stats = locality_stats;
...
const grpc_channel_filter grpc_xds_client_load_reporting_filter = {
