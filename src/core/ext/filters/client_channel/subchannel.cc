...
RefCountedPtr<SubchannelCall> SubchannelCall::Create(
    RefCountedPtr<ConnectedSubchannel> connected_subchannel, const Args& args,
    grpc_error** error) {
  const size_t allocation_size =
      connected_subchannel->GetInitialCallSizeEstimate(args.parent_data_size);
  return RefCountedPtr<SubchannelCall>(
      new (args.arena->Alloc(allocation_size))
          SubchannelCall(std::move(connected_subchannel), args, error));
}
