...
static void
connection_destroy (void *arg, grpc_error * error)
{
...
static void
subchannel_destroy (void *arg, grpc_error * error)
{
...
static gpr_atm
ref_mutate (grpc_subchannel * c, gpr_atm delta,
...
grpc_subchannel *
grpc_subchannel_weak_ref (grpc_subchannel * c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
  gpr_atm old_refs;
...
grpc_subchannel *
grpc_subchannel_ref_from_weak_ref (grpc_subchannel *
				   c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
  if (!c)
    return nullptr;
  for (;;)
...
void
grpc_subchannel_unref (grpc_subchannel * c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
  gpr_atm old_refs;
...
void
grpc_subchannel_weak_unref (grpc_subchannel *
			    c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
  gpr_atm old_refs;
  old_refs =
    ref_mutate (c, -(gpr_atm) 1, 1 REF_MUTATE_PURPOSE ("WEAK_UNREF"));
  if (old_refs == 1)
    {
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE
			  (subchannel_destroy, c, grpc_schedule_on_exec_ctx),
			  GRPC_ERROR_NONE);
    }
}

static void
parse_args_for_backoff_values (const grpc_channel_args * args,
  ...
  if (args != nullptr)
      ...
      for (size_t i = 0; i < args->num_args; i++)
	{
	  if (0 == strcmp (args->args[i].key,
	    ...
	    {
	      fixed_reconnect_backoff = false;
	      max_backoff_ms = grpc_channel_arg_get_integer (&args->args[i],
							     {
							     static_cast <
							     int
							     >
...
grpc_subchannel *
grpc_subchannel_create (grpc_connector * connector,
			const grpc_subchannel_args * args)
{
  ...
  gpr_free (addr);
  c->args =
    grpc_channel_args_copy_and_add_and_remove (new_args !=
					       nullptr ? new_args : args->
					       args, keys_to_remove,
					       GPR_ARRAY_SIZE
					       (keys_to_remove), &new_arg, 1);
...
static void
continue_connect_locked (grpc_subchannel * c)
{
...
grpc_subchannel_check_connectivity (grpc_subchannel * c, grpc_error ** error)
{
...
static void
on_external_state_watcher_done (void *arg, grpc_error * error)
{
...
static void
on_alarm (void *arg, grpc_error * error)
{
  grpc_subchannel *c = (grpc_subchannel *) arg;
  gpr_mu_lock (&c->mu);
  c->have_alarm = false;
  if (c->disconnected)
    {
      error =
	GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING ("Disconnected",
							  &error, 1);
    }
  else
    {
...
static void
maybe_start_connecting_locked (grpc_subchannel * c)
{
  if (c->disconnected)
    {
  ...
  if (!c->backoff_begun)
	...
	{
	  gpr_log (GPR_INFO,
		   "Subchannel %p: Retry in %" PRIdPTR " milliseconds", c,
		   time_til_next);
	}
      GRPC_CLOSURE_INIT (&c->on_alarm, on_alarm, c,
			 grpc_schedule_on_exec_ctx);
					...
					grpc_closure * notify)
{
  ...
  if (state == nullptr)
	   ...
	   w != &c->root_external_state_watcher; w = w->next)
	{
	  if (w->notify == notify)
	    {
	      grpc_connectivity_state_notify_on_state_change (&c->
							      state_tracker,
							      nullptr,
							      &w->closure);
	    }
...
static void
on_connected_subchannel_connectivity_changed (void *p, grpc_error * error)
{
  ...
  switch (connected_subchannel_watcher->connectivity_state)
	...
	c->connected_subchannel->NotifyOnStateChange (nullptr,
						      &connected_subchannel_watcher->
						      connectivity_state,
						      &connected_subchannel_watcher->
						      closure);
...
static bool
publish_transport_locked (grpc_subchannel * c)
{
  /* construct channel stack */
  grpc_channel_stack_builder *builder = grpc_channel_stack_builder_create ();
  grpc_channel_stack_builder_set_channel_arguments (builder,
						    c->connecting_result.
						    channel_args);
  ...
  grpc_error *error =
    grpc_channel_stack_builder_finish (builder, 0, 1, connection_destroy,
				       nullptr, (void **) &stk);
  if (error != GRPC_ERROR_NONE)
    {
  ...
  /* publish */
  c->connected_subchannel.reset (grpc_core::New <
				 grpc_core::ConnectedSubchannel > (stk));
...
static void
on_subchannel_connected (void *arg, grpc_error * error)
{
  grpc_subchannel *c = (grpc_subchannel *) arg;
...
static void
subchannel_call_destroy (void *call, grpc_error * error)
{
...
void
grpc_subchannel_call_set_cleanup_closure (grpc_subchannel_call * call,
					  grpc_closure * closure)
{
...
void
grpc_subchannel_call_ref (grpc_subchannel_call *
			  c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
...
void
grpc_subchannel_call_unref (grpc_subchannel_call *
			    c GRPC_SUBCHANNEL_REF_EXTRA_ARGS)
{
...
void
grpc_subchannel_call_process_op (grpc_subchannel_call * call,
...
grpc_subchannel_get_connected_subchannel (grpc_subchannel * c)
{
  gpr_mu_lock (&c->mu);
  auto
    copy = c->connected_subchannel;
  gpr_mu_unlock (&c->mu);
  return copy;
}

const grpc_subchannel_key *
grpc_subchannel_get_key (const grpc_subchannel * subchannel)
{
  return subchannel->key;
}

grpc_call_stack *
grpc_subchannel_call_get_call_stack (grpc_subchannel_call * subchannel_call)
{
  return SUBCHANNEL_CALL_TO_CALL_STACK (subchannel_call);
}

static void
grpc_uri_to_sockaddr (const char *uri_str, grpc_resolved_address * addr)
{
  grpc_uri *
    uri = grpc_uri_parse (uri_str, 0 /* suppress_errors */ );
...
void
grpc_get_subchannel_address_arg (const grpc_channel_args * args,
				 grpc_resolved_address * addr)
{
  const char *
    addr_uri_str = grpc_get_subchannel_address_uri_arg (args);
  memset (addr, 0, sizeof (*addr));
  if (*addr_uri_str != '\0')
    {
...
const char *
grpc_get_subchannel_address_uri_arg (const grpc_channel_args * args)
{
  const grpc_arg *
    addr_arg = grpc_channel_args_find (args, GRPC_ARG_SUBCHANNEL_ADDRESS);
...
grpc_arg
grpc_create_subchannel_address_arg (const grpc_resolved_address * addr)
{
  return grpc_channel_arg_string_create ((char *) GRPC_ARG_SUBCHANNEL_ADDRESS,
					 addr->len >
					 0 ? grpc_sockaddr_to_uri (addr) :
					 gpr_strdup (""));
}
...
namespace
  grpc_core
{
  ConnectedSubchannel::ConnectedSubchannel (grpc_channel_stack *
					    channel_stack):
  ...
  void
  ConnectedSubchannel::Ping (grpc_closure * on_initiate,
			     grpc_closure * on_ack)
  {
    grpc_transport_op *
      op = grpc_make_transport_op (nullptr);
  ...
  grpc_error *
  ConnectedSubchannel::CreateCall (const CallArgs & args,
				   grpc_subchannel_call ** call)
  {
    *call = (grpc_subchannel_call *) gpr_arena_alloc (args.arena,
						      sizeof
						      (grpc_subchannel_call) +
						      channel_stack_->
						      call_stack_size);
    grpc_call_stack *
      callstk = SUBCHANNEL_CALL_TO_CALL_STACK (*call);
    ...
    grpc_error *
      error =
      grpc_call_stack_init (channel_stack_, 1, subchannel_call_destroy, *call,
			    &call_args);
    if (error != GRPC_ERROR_NONE)
      {
	const char *
	  error_string = grpc_error_string (error);
