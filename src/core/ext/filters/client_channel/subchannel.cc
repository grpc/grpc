...
#define SUBCHANNEL_CALL_TO_CALL_STACK(call) \
  (grpc_call_stack*)((char*)(call) + GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(SubchannelCall)))
#define CALL_STACK_TO_SUBCHANNEL_CALL(callstack) \
  (SubchannelCall*)(((char*)(call_stack)) - GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(SubchannelCall)))
...
ConnectedSubchannel::ConnectedSubchannel(
    ...
    : RefCounted<ConnectedSubchannel>(GRPC_TRACE_FLAG_ENABLED(grpc_trace_subchannel_refcount)
                                          ? "ConnectedSubchannel"
                                          : nullptr),
...
void ConnectedSubchannel::StartWatch(grpc_pollset_set* interested_parties,
...
size_t ConnectedSubchannel::GetInitialCallSizeEstimate() const {
  return GPR_ROUND_UP_TO_ALIGNMENT_SIZE(sizeof(SubchannelCall)) + channel_stack_->call_stack_size;
}
...
RefCountedPtr<SubchannelCall> SubchannelCall::Create(Args args, grpc_error_handle* error) {
  const size_t allocation_size = args.connected_subchannel->GetInitialCallSizeEstimate();
  Arena* arena = args.arena;
  return RefCountedPtr<SubchannelCall>(new (arena->Alloc(allocation_size))
                                           SubchannelCall(std::move(args), error));
}

SubchannelCall::SubchannelCall(Args args, grpc_error_handle* error)
    : connected_subchannel_(std::move(args.connected_subchannel)), deadline_(args.deadline) {
  ...
  *error = grpc_call_stack_init(connected_subchannel_->channel_stack(), 1, SubchannelCall::Destroy,
                                this, &call_args);
...
void SubchannelCall::StartTransportStreamOpBatch(grpc_transport_stream_op_batch* batch) {
...
grpc_call_stack* SubchannelCall::GetCallStack() { return SUBCHANNEL_CALL_TO_CALL_STACK(this); }
...
RefCountedPtr<SubchannelCall> SubchannelCall::Ref(const grpc_core::DebugLocation& location,
                                                  const char* reason) {
  IncrementRefCount(location, reason);
  return RefCountedPtr<SubchannelCall>(this);
}

void SubchannelCall::Unref() { GRPC_CALL_STACK_UNREF(SUBCHANNEL_CALL_TO_CALL_STACK(this), ""); }
...
void SubchannelCall::Destroy(void* arg, grpc_error_handle /*error*/) {
  ...
  RefCountedPtr<ConnectedSubchannel> connected_subchannel = std::move(self->connected_subchannel_);
  ...
  // call arena.
  grpc_call_stack_destroy(SUBCHANNEL_CALL_TO_CALL_STACK(self), nullptr, after_call_stack_destroy);
...
void SubchannelCall::MaybeInterceptRecvTrailingMetadata(grpc_transport_stream_op_batch* batch) {
  ...
  GRPC_CLOSURE_INIT(&recv_trailing_metadata_ready_, RecvTrailingMetadataReady, this,
...
void GetCallStatus(grpc_status_code* status, grpc_millis deadline, grpc_metadata_batch* md_batch,
                   grpc_error_handle error) {
  if (error != GRPC_ERROR_NONE) {
    grpc_error_get_status(error, deadline, status, nullptr, nullptr, nullptr);
  } else {
    if (md_batch->idx.named.grpc_status != nullptr) {
      *status = grpc_get_status_code_from_metadata(md_batch->idx.named.grpc_status->md);
    } else {
...
void SubchannelCall::RecvTrailingMetadataReady(void* arg, grpc_error_handle error) {
  SubchannelCall* call = static_cast<SubchannelCall*>(arg);
  GPR_ASSERT(call->recv_trailing_metadata_ != nullptr);
  grpc_status_code status = GRPC_STATUS_OK;
  GetCallStatus(&status, call->deadline_, call->recv_trailing_metadata_, GRPC_ERROR_REF(error));
  ...
  Closure::Run(DEBUG_LOCATION, call->original_recv_trailing_metadata_, GRPC_ERROR_REF(error));
}
...
class Subchannel::ConnectedSubchannelStateWatcher : public AsyncConnectivityStateWatcherInterface {
 public:
        ...
        if (!c->disconnected_ && c->connected_subchannel_ != nullptr) {
          if (grpc_trace_subchannel.enabled()) {
            gpr_log(GPR_INFO,
                    ...
                    c->connected_subchannel_.get(), c, ConnectivityStateName(new_state));
          }
          ...
          c->SetConnectivityStateLocked(
              ...
              new_state == GRPC_CHANNEL_SHUTDOWN
                  ? absl::Status(absl::StatusCode::kUnavailable, "Subchannel has disconnected.")
                  : status);
...
class Subchannel::AsyncWatcherNotifierLocked {
 public:
  AsyncWatcherNotifierLocked(RefCountedPtr<Subchannel::ConnectivityStateWatcherInterface> watcher,
    ...
    watcher_->PushConnectivityStateChange({state, status, std::move(connected_subchannel)});
...
void Subchannel::ConnectivityStateWatcherList::NotifyLocked(Subchannel* subchannel,
...
class Subchannel::HealthWatcherMap::HealthWatcher : public AsyncConnectivityStateWatcherInterface {
 public:
  ...
  ~HealthWatcher() override { subchannel_.reset(DEBUG_LOCATION, "health_watcher"); }

  const std::string& health_check_service_name() const { return health_check_service_name_; }
 ...
 private:
  ...
  void StartHealthCheckingLocked() ABSL_EXCLUSIVE_LOCKS_REQUIRED(subchannel_->mu_) {
    ...
    health_check_client_ = MakeOrphanable<HealthCheckClient>(
        health_check_service_name_, subchannel_->connected_subchannel_, subchannel_->pollset_set_,
        subchannel_->channelz_node_, Ref());
  }
...
void Subchannel::HealthWatcherMap::AddWatcherLocked(
  ...
  if (it == map_.end()) {
    auto w = MakeOrphanable<HealthWatcher>(std::move(subchannel), health_check_service_name);
...
void Subchannel::HealthWatcherMap::RemoveWatcherLocked(const std::string& health_check_service_name,
                                                       ConnectivityStateWatcherInterface* watcher) {
  auto it = map_.find(health_check_service_name);
...
grpc_connectivity_state Subchannel::HealthWatcherMap::CheckConnectivityStateLocked(
  ...
  if (it == map_.end()) {
    ...
    return subchannel->state_ == GRPC_CHANNEL_READY ? GRPC_CHANNEL_CONNECTING : subchannel->state_;
  }
...
BackOff::Options ParseArgsForBackoffValues(const grpc_channel_args* args,
  ...
  grpc_millis initial_backoff_ms = GRPC_SUBCHANNEL_INITIAL_CONNECT_BACKOFF_SECONDS * 1000;
  *min_connect_timeout_ms = GRPC_SUBCHANNEL_RECONNECT_MIN_TIMEOUT_SECONDS * 1000;
  grpc_millis max_backoff_ms = GRPC_SUBCHANNEL_RECONNECT_MAX_BACKOFF_SECONDS * 1000;
  bool fixed_reconnect_backoff = false;
  if (args != nullptr) {
    for (size_t i = 0; i < args->num_args; i++) {
      if (0 == strcmp(args->args[i].key, "grpc.testing.fixed_reconnect_backoff_ms")) {
      ...
      } else if (0 == strcmp(args->args[i].key, GRPC_ARG_MIN_RECONNECT_BACKOFF_MS)) {
        fixed_reconnect_backoff = false;
        *min_connect_timeout_ms = grpc_channel_arg_get_integer(
            &args->args[i], {static_cast<int>(*min_connect_timeout_ms), 100, INT_MAX});
      } else if (0 == strcmp(args->args[i].key, GRPC_ARG_MAX_RECONNECT_BACKOFF_MS)) {
        fixed_reconnect_backoff = false;
        max_backoff_ms = grpc_channel_arg_get_integer(
            &args->args[i], {static_cast<int>(max_backoff_ms), 100, INT_MAX});
      } else if (0 == strcmp(args->args[i].key, GRPC_ARG_INITIAL_RECONNECT_BACKOFF_MS)) {
        fixed_reconnect_backoff = false;
        initial_backoff_ms = grpc_channel_arg_get_integer(
            &args->args[i], {static_cast<int>(initial_backoff_ms), 100, INT_MAX});
      }
    }
  }
  return BackOff::Options()
      .set_initial_backoff(initial_backoff_ms)
      .set_multiplier(fixed_reconnect_backoff ? 1.0 : GRPC_SUBCHANNEL_RECONNECT_BACKOFF_MULTIPLIER)
      .set_jitter(fixed_reconnect_backoff ? 0.0 : GRPC_SUBCHANNEL_RECONNECT_JITTER)
...
Subchannel::Subchannel(SubchannelKey key, OrphanablePtr<SubchannelConnector> connector,
  ...
  grpc_resolved_address* addr = static_cast<grpc_resolved_address*>(gpr_malloc(sizeof(*addr)));
  ...
  gpr_free(addr);
  args_ = grpc_channel_args_copy_and_add_and_remove(new_args != nullptr ? new_args : args,
                                                    keys_to_remove, GPR_ARRAY_SIZE(keys_to_remove),
                                                    &new_arg, 1);
  ...
  const bool channelz_enabled = grpc_channel_arg_get_bool(arg, GRPC_ENABLE_CHANNELZ_DEFAULT);
  arg = grpc_channel_args_find(args_, GRPC_ARG_MAX_CHANNEL_TRACE_EVENT_MEMORY_PER_NODE);
  ...
  if (channelz_enabled) {
    channelz_node_ =
        MakeRefCounted<channelz::SubchannelNode>(GetTargetAddress(), channel_tracer_max_memory);
...
RefCountedPtr<Subchannel> Subchannel::Create(OrphanablePtr<SubchannelConnector> connector,
                                             const grpc_channel_args* args) {
  ...
  RefCountedPtr<Subchannel> registered = subchannel_pool->RegisterSubchannel(c->key_, c);
...
void Subchannel::ThrottleKeepaliveTime(int new_keepalive_time) {
  ...
  if (new_keepalive_time > keepalive_time_) {
    ...
    if (grpc_trace_subchannel.enabled()) {
      gpr_log(GPR_INFO, "Subchannel=%p: Throttling keepalive time to %d", this, new_keepalive_time);
    }
    ...
    grpc_channel_args* new_args =
        grpc_channel_args_copy_and_add_and_remove(args_, &arg_to_remove, 1, &arg_to_add, 1);
    grpc_channel_args_destroy(args_);
    args_ = new_args;
  }
}
...
const char* Subchannel::GetTargetAddress() {
  const grpc_arg* addr_arg = grpc_channel_args_find(args_, GRPC_ARG_SUBCHANNEL_ADDRESS);
  const char* addr_str = grpc_channel_arg_get_string(addr_arg);
  GPR_ASSERT(addr_str != nullptr);  // Should have been set by LB policy.
  return addr_str;
}

channelz::SubchannelNode* Subchannel::channelz_node() { return channelz_node_.get(); }
...
grpc_connectivity_state Subchannel::CheckConnectivityState(
  ...
  if (!health_check_service_name.has_value()) {
    ...
    state = health_watcher_map_.CheckConnectivityStateLocked(this, *health_check_service_name);
  }
...
void Subchannel::WatchConnectivityState(
  ...
  if (!health_check_service_name.has_value()) {
    ...
    health_watcher_map_.AddWatcherLocked(WeakRef(DEBUG_LOCATION, "health_watcher"), initial_state,
...
grpc_arg Subchannel::CreateSubchannelAddressArg(const grpc_resolved_address* addr) {
  return grpc_channel_arg_string_create(
      const_cast<char*>(GRPC_ARG_SUBCHANNEL_ADDRESS),
      gpr_strdup(addr->len > 0 ? grpc_sockaddr_to_uri(addr).c_str() : ""));
}

const char* Subchannel::GetUriFromSubchannelAddressArg(const grpc_channel_args* args) {
  const grpc_arg* addr_arg = grpc_channel_args_find(args, GRPC_ARG_SUBCHANNEL_ADDRESS);
  const char* addr_str = grpc_channel_arg_get_string(addr_arg);
  GPR_ASSERT(addr_str != nullptr);  // Should have been set by LB policy.
  return addr_str;
}
...
void Subchannel::GetAddressFromSubchannelAddressArg(const grpc_channel_args* args,
...
// \a state.
const char* SubchannelConnectivityStateChangeString(grpc_connectivity_state state) {
  switch (state) {
...
void Subchannel::SetConnectivityStateLocked(grpc_connectivity_state state,
  ...
  if (channelz_node_ != nullptr) {
    ...
    channelz_node_->AddTraceEvent(
        ...
        grpc_slice_from_static_string(SubchannelConnectivityStateChangeString(state)));
  }
...
void Subchannel::MaybeStartConnectingLocked() {
  ...
  connecting_ = true;
  WeakRef(DEBUG_LOCATION, "connecting").release();  // ref held by pending connect
  if (!backoff_begun_) {
    backoff_begun_ = true;
    ...
    const grpc_millis time_til_next = next_attempt_deadline_ - ExecCtx::Get()->Now();
    if (time_til_next <= 0) {
      gpr_log(GPR_INFO, "Subchannel %p: Retry immediately", this);
    } else {
      gpr_log(GPR_INFO, "Subchannel %p: Retry in %" PRId64 " milliseconds", this, time_til_next);
    }
    GRPC_CLOSURE_INIT(&on_retry_alarm_, OnRetryAlarm, this, grpc_schedule_on_exec_ctx);
...
void Subchannel::OnRetryAlarm(void* arg, grpc_error_handle error) {
  ...
  if (c->disconnected_) {
    error = GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING("Disconnected", &error, 1);
...
void Subchannel::ContinueConnectingLocked() {
  ...
  const grpc_millis min_deadline = min_connect_timeout_ms_ + ExecCtx::Get()->Now();
...
void Subchannel::OnConnectingFinished(void* arg, grpc_error_handle error) {
  WeakRefCountedPtr<Subchannel> c(static_cast<Subchannel*>(arg));
  const grpc_channel_args* delete_channel_args = c->connecting_result_.channel_args;
  {
    MutexLock lock(&c->mu_);
    c->connecting_ = false;
    if (c->connecting_result_.transport != nullptr && c->PublishTransportLocked()) {
      // Do nothing, transport was published.
    } else if (!c->disconnected_) {
      gpr_log(GPR_INFO, "Connect failed: %s", grpc_error_std_string(error).c_str());
...
bool Subchannel::PublishTransportLocked() {
  ...
  grpc_channel_stack_builder_set_channel_arguments(builder, connecting_result_.channel_args);
  grpc_channel_stack_builder_set_transport(builder, connecting_result_.transport);
  ...
  RefCountedPtr<channelz::SocketNode> socket = std::move(connecting_result_.socket_node);
  ...
  // Publish.
  connected_subchannel_.reset(new ConnectedSubchannel(stk, args_, channelz_node_));
  gpr_log(GPR_INFO, "New connected subchannel at %p for subchannel %p", connected_subchannel_.get(),
          this);
  if (channelz_node_ != nullptr) {
    channelz_node_->SetChildSocket(std::move(socket));
  }
  // Start watching connected subchannel.
  connected_subchannel_->StartWatch(pollset_set_, MakeOrphanable<ConnectedSubchannelStateWatcher>(
