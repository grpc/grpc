...
grpc_lb_addresses *
grpc_lb_addresses_create (size_t num_addresses,
...
grpc_lb_addresses *
grpc_lb_addresses_copy (const grpc_lb_addresses * addresses)
{
  ...
  for (size_t i = 0; i < addresses->num_addresses; ++i)
      ...
      if (new_addresses->addresses[i].user_data != nullptr)
	{
	  new_addresses->addresses[i].user_data =
	    addresses->user_data_vtable->copy (new_addresses->addresses[i].
					       user_data);
	}
    }
...
void
grpc_lb_addresses_set_address (grpc_lb_addresses * addresses, size_t index,
			       ...
			       void *user_data)
{
  ...
  if (user_data != nullptr)
    GPR_ASSERT (addresses->user_data_vtable != nullptr);
...
bool
grpc_lb_addresses_set_address_from_uri (grpc_lb_addresses * addresses,
...
int
grpc_lb_addresses_cmp (const grpc_lb_addresses * addresses1,
		       const grpc_lb_addresses * addresses2)
{
  if (addresses1->num_addresses > addresses2->num_addresses)
    return 1;
  if (addresses1->num_addresses < addresses2->num_addresses)
    return -1;
  if (addresses1->user_data_vtable > addresses2->user_data_vtable)
    return 1;
  if (addresses1->user_data_vtable < addresses2->user_data_vtable)
    return -1;
  for (size_t i = 0; i < addresses1->num_addresses; ++i)
    {
      ...
      if (target1->address.len > target2->address.len)
	return 1;
      if (target1->address.len < target2->address.len)
	return -1;
      ...
      if (retval != 0)
	return retval;
      if (target1->is_balancer > target2->is_balancer)
	return 1;
      if (target1->is_balancer < target2->is_balancer)
	return -1;
...
grpc_lb_addresses_destroy (grpc_lb_addresses * addresses)
{
  for (size_t i = 0; i < addresses->num_addresses; ++i)
	...
	{
	  addresses->user_data_vtable->destroy (addresses->addresses[i].
						user_data);
	}
    }
...
static void *
lb_addresses_copy (void *addresses)
{
  return grpc_lb_addresses_copy ((grpc_lb_addresses *) addresses);
}

static void
lb_addresses_destroy (void *addresses)
{
  grpc_lb_addresses_destroy ((grpc_lb_addresses *) addresses);
}

static int
lb_addresses_cmp (void *addresses1, void *addresses2)
{
...
grpc_arg
grpc_lb_addresses_create_channel_arg (const grpc_lb_addresses * addresses)
{
  return grpc_channel_arg_pointer_create ((char *) GRPC_ARG_LB_ADDRESSES,
					  (void *) addresses,
					  &lb_addresses_arg_vtable);
}

grpc_lb_addresses *
grpc_lb_addresses_find_channel_arg (const grpc_channel_args * channel_args)
{
