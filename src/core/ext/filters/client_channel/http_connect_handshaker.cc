...
static void
http_connect_handshaker_unref (http_connect_handshaker * handshaker)
{
  if (gpr_unref (&handshaker->refcount))
	...
	{
	  grpc_slice_buffer_destroy_internal (handshaker->
...
// later destruction.
static void
cleanup_args_for_failure_locked (http_connect_handshaker * handshaker)
{
...
static void
handshake_failed_locked (http_connect_handshaker * handshaker,
			 grpc_error * error)
{
  ...
  if (!handshaker->shutdown)
    {
      ...
      grpc_endpoint_shutdown (handshaker->args->endpoint,
...
static void
on_write_done (void *arg, grpc_error * error)
{
...
static void
on_read_done (void *arg, grpc_error * error)
{
  http_connect_handshaker *handshaker = (http_connect_handshaker *) arg;
  gpr_mu_lock (&handshaker->mu);
  if (error != GRPC_ERROR_NONE || handshaker->shutdown)
	  ...
	  if (handshaker->http_parser.state == GRPC_HTTP_BODY)
	    {
		  ...
		  GRPC_SLICE_LENGTH (handshaker->args->read_buffer->
				     slices[i]))
		{
		  grpc_slice_buffer_add (&tmp_buffer,
	      ...
	      grpc_slice_buffer_addn (&tmp_buffer,
				      &handshaker->args->read_buffer->
				      slices[i + 1],
				      handshaker->args->read_buffer->count -
				      i - 1);
	      grpc_slice_buffer_swap (handshaker->args->read_buffer,
				      &tmp_buffer);
  ...
  if (handshaker->http_parser.state != GRPC_HTTP_BODY)
    {
      grpc_slice_buffer_reset_and_unref_internal (handshaker->args->
						  read_buffer);
      grpc_endpoint_read (handshaker->args->endpoint,
			  handshaker->args->read_buffer,
			  &handshaker->response_read_closure);
...
static void
http_connect_handshaker_destroy (grpc_handshaker * handshaker_in)
{
  http_connect_handshaker *handshaker =
    (http_connect_handshaker *) handshaker_in;
  http_connect_handshaker_unref (handshaker);
}

static void
http_connect_handshaker_shutdown (grpc_handshaker * handshaker_in,
				  grpc_error * why)
{
  http_connect_handshaker *handshaker =
    (http_connect_handshaker *) handshaker_in;
...
static void
http_connect_handshaker_do_handshake (grpc_handshaker * handshaker_in,
  ...
  if (arg != nullptr)
      ...
      for (size_t i = 0; i < num_header_strings; ++i)
	{
	  ...
	  if (sep == nullptr)
	    {
	      gpr_log (GPR_ERROR,
		       "skipping unparseable HTTP CONNECT header: %s",
		       header_strings[i]);
	      continue;
	    }
	  *sep = '\0';
...
static void
handshaker_factory_add_handshakers (grpc_handshaker_factory * factory,
...
static void
handshaker_factory_destroy (grpc_handshaker_factory * factory)
{
}
