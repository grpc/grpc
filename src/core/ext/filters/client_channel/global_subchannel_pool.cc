  ...
  for (int attempt_count = 0; c == nullptr; attempt_count++) {
    ...
    if (c != nullptr) {
      ...
      if (c != nullptr) {
        ...
        if (attempt_count >= GRPC_REGISTER_SUBCHANNEL_CALM_DOWN_AFTER_ATTEMPTS)
        {
          ...
          // gpr does not support thread yield operation, this is the best we can do.
          gpr_sleep_until(gpr_time_add(gpr_now(GPR_CLOCK_REALTIME), gpr_time_from_micros(GRPC_REGISTER_SUBCHANNEL_CALM_DOWN_MICROS, GPR_TIMESPAN)));
        }
      }
    } else {
