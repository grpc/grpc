...
grpc_workaround_user_agent_md* grpc_parse_user_agent(grpc_mdelem md) {
  grpc_workaround_user_agent_md* user_agent_md =
      reinterpret_cast<grpc_workaround_user_agent_md*>(
          grpc_mdelem_get_user_data(md, destroy_user_agent_md));

  if (nullptr != user_agent_md) {
    return user_agent_md;
  }
  user_agent_md = reinterpret_cast<grpc_workaround_user_agent_md*>(
  ...
  grpc_mdelem_set_user_data(md, destroy_user_agent_md, reinterpret_cast<void*>(user_agent_md));
