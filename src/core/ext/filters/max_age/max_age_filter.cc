...
static void
increase_call_count (channel_data * chand)
{
...
static void
decrease_call_count (channel_data * chand)
{
...
static void
start_max_idle_timer_after_init (void *arg, grpc_error * error)
{
...
static void
start_max_age_timer_after_init (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
  ...
  grpc_timer_init (&chand->max_age_timer,
		   grpc_core::ExecCtx::Get ()->Now () +
		   chand->max_connection_age, &chand->close_max_age_channel);
...
static void
start_max_age_grace_timer_after_goaway_op (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
  gpr_mu_lock (&chand->max_age_timer_mu);
...
static void
close_max_idle_channel (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
...
static void
close_max_age_channel (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
  gpr_mu_lock (&chand->max_age_timer_mu);
  ...
  if (error == GRPC_ERROR_NONE)
	...
	grpc_make_transport_op (&chand->
				start_max_age_grace_timer_after_goaway_op);
      op->goaway_error =
...
static void
force_close_max_age_channel (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
  gpr_mu_lock (&chand->max_age_timer_mu);
...
static void
channel_connectivity_changed (void *arg, grpc_error * error)
{
  channel_data *chand = (channel_data *) arg;
...
add_random_max_connection_age_jitter_and_convert_to_grpc_millis (int value)
{
  ...
  return result > ((double) GRPC_MILLIS_INF_FUTURE) - 0.5
    ? GRPC_MILLIS_INF_FUTURE : (grpc_millis) result;
}
		   ...
		   grpc_channel_element_args * args)
{
  ...
  chand->max_connection_age_grace =
    ...
    INT_MAX ? GRPC_MILLIS_INF_FUTURE : DEFAULT_MAX_CONNECTION_AGE_GRACE_MS;
  chand->max_connection_idle =
    DEFAULT_MAX_CONNECTION_IDLE_MS ==
    INT_MAX ? GRPC_MILLIS_INF_FUTURE : DEFAULT_MAX_CONNECTION_IDLE_MS;
  for (size_t i = 0; i < args->channel_args->num_args; ++i)
	...
	{
	  const int value =
	    grpc_channel_arg_get_integer (&args->channel_args->args[i],
  ...
  GRPC_CLOSURE_INIT (&chand->close_max_age_channel, close_max_age_channel,
		     chand, grpc_schedule_on_exec_ctx);
...
static bool
maybe_add_max_age_filter (grpc_channel_stack_builder * builder, void *arg)
{
...
void
grpc_max_age_filter_shutdown (void)
{
}
