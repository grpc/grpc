...
/* If these settings change, make sure that we are not sending a GOAWAY
 * for inproc transport, since a GOAWAY to inproc ends up destroying the
 * transport.
 */
...
struct channel_data {
  ...
  /* Guards access to max_age_timer, max_age_timer_pending,
     max_age_grace_timer and max_age_grace_timer_pending */
  ...
  bool max_age_grace_timer_pending ABSL_GUARDED_BY(max_age_timer_mu) =
      false;
  /* The timer for checking if the channel has reached its max age */
  grpc_timer max_age_timer ABSL_GUARDED_BY(max_age_timer_mu);
  /* The timer for checking if the max-aged channel has uesed up the
     grace period */
  ...
  /* Closure to run when the channel's idle duration reaches
     max_connection_idle and should be closed gracefully */
  grpc_closure max_idle_timer_cb;
  /* Closure to run when the channel reaches its max age and should be
     closed gracefully */
  grpc_closure close_max_age_channel;
  /* Closure to run the channel uses up its max age grace time and
     should be closed forcibly */
  grpc_closure force_close_max_age_channel;
  /* Closure to run when the init fo channel stack is done and the
     max_idle timer should be started */
  grpc_closure start_max_idle_timer_after_init;
  /* Closure to run when the init fo channel stack is done and the
     max_age timer should be started */
  ...
  /* 'idle_state' holds the states of max_idle_timer and channel
     idleness. It can contain one of the following values:
     +--------------------------------+----------------+---------+
     ...
     MAX_IDLE_STATE_INIT: The initial and final state of 'idle_state'.
     The channel has 1 or 1+ active calls, and the timer is not set.
     Note that we may put a virtual call to hold this state at channel
     initialization or shutdown, so that the channel won't enter other
     states.

     MAX_IDLE_STATE_TIMER_SET: The state after the timer is set and no
     calls have arrived after the timer is set. The channel must have 0
     active call in this state. If the timer is fired in this state, we
     will close the channel due to idleness.

     MAX_IDLE_STATE_SEEN_EXIT_IDLE: The state after the timer is set and
     at least one call has arrived after the timer is set. The channel
     must have 1 or 1+ active calls in this state. If the timer is fired
     in this state, we won't reschudle it.

     MAX_IDLE_STATE_SEEN_ENTER_IDLE: The state after the timer is set
     and the at least one call has arrived after the timer is set, BUT
     the channel currently has 0 active calls. If the timer is fired in
     this state, we will reschudle it.

     max_idle_timer will not be cancelled (unless the channel is
     shutting down). If the timer callback is called when the
     max_idle_timer is valid (i.e. idle_state is
     MAX_IDLE_STATE_TIMER_SET), the channel will be closed due to
     idleness, otherwise the channel won't be changed.

     State transitions:
         MAX_IDLE_STATE_INIT <-------3------
     MAX_IDLE_STATE_SEEN_EXIT_IDLE ^    |                              ^
     ^    | |    |                              |     |    | 1    2
     +-----------4------------+     6    7 |    |     | |    | |    v |
     |    v MAX_IDLE_STATE_TIMER_SET <----5------
...
/* Increase the nubmer of active calls. Before the increasement, if
   there are no calls, the max_idle_timer should be cancelled. */
      ...
      switch (idle_state) {
        case MAX_IDLE_STATE_TIMER_SET:
          /* max_idle_timer_cb may have already set idle_state to
             MAX_IDLE_STATE_INIT, in this case, we don't need to set it
             to MAX_IDLE_STATE_SEEN_EXIT_IDLE */
          gpr_atm_rel_cas(&chand->idle_state, MAX_IDLE_STATE_TIMER_SET,
                          MAX_IDLE_STATE_SEEN_EXIT_IDLE);
          return;
        case MAX_IDLE_STATE_SEEN_ENTER_IDLE:
          gpr_atm_rel_store(&chand->idle_state,
                            MAX_IDLE_STATE_SEEN_EXIT_IDLE);
          return;
        default:
          /* try again */
...
/* Decrease the nubmer of active calls. After the decrement, if there
   are no calls, the max_idle_timer should be started. */
    ...
    while (true) {
      ...
      switch (idle_state) {
        ...
        case MAX_IDLE_STATE_SEEN_EXIT_IDLE:
          if (gpr_atm_rel_cas(&chand->idle_state,
                              MAX_IDLE_STATE_SEEN_EXIT_IDLE,
                              MAX_IDLE_STATE_SEEN_ENTER_IDLE)) {
            return;
          }
...
static void start_max_idle_timer_after_init(
  ...
  /* Decrease call_count. If there are no active calls at this time,
     max_idle_timer will start here. If the number of active calls is
     not 0, max_idle_timer will start after all the active calls end. */
...
class ConnectivityWatcher
 ...
 public:
  ...
  ~ConnectivityWatcher() override {
    GRPC_CHANNEL_STACK_UNREF(chand_->channel_stack,
                             "max_age conn_watch");
  }
 ...
 private:
  void OnConnectivityStateChange(
    ...
    /* If there are no active calls, this increasement will cancel
       max_idle_timer, and prevent max_idle_timer from being started in
       the future. */
...
static void start_max_age_timer_after_init(
  ...
  op->start_connectivity_watch.reset(
      new grpc_core::ConnectivityWatcher(chand));
...
static void close_max_idle_channel(channel_data* chand) {
  ...
  op->goaway_error = grpc_error_set_int(
      GRPC_ERROR_CREATE_FROM_STATIC_STRING("max_idle"),
    ...
    while (try_again) {
      ...
      switch (idle_state) {
        case MAX_IDLE_STATE_TIMER_SET:
          ...
          /* This MAX_IDLE_STATE_INIT is a final state, we don't have to
           * check if idle_state has been changed */
        ...
        case MAX_IDLE_STATE_SEEN_ENTER_IDLE:
          ...
          /* idle_state may have already been set to
             MAX_IDLE_STATE_SEEN_EXIT_IDLE by increase_call_count(), in
             this case, we don't need to set it to
             MAX_IDLE_STATE_TIMER_SET */
          gpr_atm_rel_cas(&chand->idle_state,
                          MAX_IDLE_STATE_SEEN_ENTER_IDLE,
                          MAX_IDLE_STATE_TIMER_SET);
          try_again = false;
          break;
...
/* A random jitter of +/-10% will be added to MAX_CONNECTION_AGE to
   spread out connection storms. Note that the MAX_CONNECTION_AGE option
   without jitter would not create connection storms by itself, but if
   there happened to be a connection storm it could cause it to repeat
   at a fixed period. */
static grpc_millis
add_random_max_connection_age_jitter_and_convert_to_grpc_millis(
  ...
  /* INT_MAX - 0.5 converts the value to float, so that result will not
     be cast to int implicitly before the comparison. */
...
static grpc_error_handle max_age_init_channel_elem(
  ...
  GRPC_CLOSURE_INIT(&chand->close_max_age_channel,
                    close_max_age_channel, chand,
                    grpc_schedule_on_exec_ctx);
  ...
  if (chand->max_connection_age != GRPC_MILLIS_INF_FUTURE) {
    /* When the channel reaches its max age, we send down an op with
       goaway_error set.  However, we can't send down any ops until
       after the channel stack is fully initialized.  If we start the
       timer here, we have no guarantee that the timer won't pop before
       channel stack initialization is finished.  To avoid that problem,
       we create a closure to start the timer, and we schedule that
       closure to be run after call stack initialization is done. */
    GRPC_CHANNEL_STACK_REF(chand->channel_stack,
                           "max_age start_max_age_timer_after_init");
  ...
  /* Initialize the number of calls as 1, so that the max_idle_timer
     will not start until start_max_idle_timer_after_init is invoked. */
