...
grpc_millis GetClientIdleTimeout(const grpc_channel_args* args) {
  return GPR_MAX(
      grpc_channel_arg_get_integer(grpc_channel_args_find(args, GRPC_ARG_CLIENT_IDLE_TIMEOUT_MS),
...
class ChannelData {
 ...
 private:
  ...
  static void IdleTransportOpCompleteCallback(void* arg, grpc_error_handle error);
...
grpc_error_handle ChannelData::Init(grpc_channel_element* elem, grpc_channel_element_args* args) {
    ...
    while (true) {
      switch (state) {
        ...
        case TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START:
          ...
          if (state_.CompareExchangeWeak(&state, TIMER_PENDING_CALLS_ACTIVE, MemoryOrder::ACQUIRE,
                                         MemoryOrder::RELAXED)) {
            return;
          }
          break;
    ...
    while (true) {
      switch (state) {
        ...
        case TIMER_PENDING_CALLS_ACTIVE:
          ...
          if (state_.CompareExchangeWeak(&state, TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START,
...
ChannelData::ChannelData(grpc_channel_element* elem, grpc_channel_element_args* args,
                         grpc_error_handle* /*error*/)
    : elem_(elem),
  ...
  GRPC_IDLE_FILTER_LOG("created with max_leisure_time = %" PRId64 " ms", client_idle_timeout_);
  // Initialize the idle timer without setting it.
  grpc_timer_init_unset(&idle_timer_);
  // Initialize the idle timer callback closure.
  GRPC_CLOSURE_INIT(&idle_timer_callback_, IdleTimerCallback, this, grpc_schedule_on_exec_ctx);
  // Initialize the idle transport op complete callback.
  GRPC_CLOSURE_INIT(&idle_transport_op_complete_callback_, IdleTransportOpCompleteCallback, this,
  ...
  while (!finished) {
    switch (state) {
      case TIMER_PENDING:
        ...
        finished = chand->state_.CompareExchangeWeak(&state, PROCESSING, MemoryOrder::ACQUIRE,
                                                     MemoryOrder::RELAXED);
        if (finished) {
          chand->EnterIdle();
        ...
        break;
      case TIMER_PENDING_CALLS_ACTIVE:
        finished = chand->state_.CompareExchangeWeak(&state, CALLS_ACTIVE, MemoryOrder::RELAXED,
                                                     MemoryOrder::RELAXED);
        break;
      case TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START:
...
void ChannelData::IdleTransportOpCompleteCallback(void* arg, grpc_error_handle /*error*/) {
  ChannelData* chand = static_cast<ChannelData*>(arg);
...
void ChannelData::StartIdleTimer() {
  ...
  grpc_timer_init(&idle_timer_, last_idle_time_ + client_idle_timeout_, &idle_timer_callback_);
}
...
void ChannelData::EnterIdle() {
  ...
  idle_transport_op_ = {};
  idle_transport_op_.disconnect_with_error =
      grpc_error_set_int(GRPC_ERROR_CREATE_FROM_STATIC_STRING("enter idle"),
...
grpc_error_handle CallData::Init(grpc_call_element* elem, const grpc_call_element_args* /*args*/) {
  ChannelData* chand = static_cast<ChannelData*>(elem->channel_data);
  chand->IncreaseCallCount();
  return GRPC_ERROR_NONE;
}

void CallData::Destroy(grpc_call_element* elem, const grpc_call_final_info* /*final_info*/,
...
static bool MaybeAddClientIdleFilter(grpc_channel_stack_builder* builder, void* /*arg*/) {
  ...
  if (!grpc_channel_args_want_minimal_stack(channel_args) &&
    ...
    return grpc_channel_stack_builder_prepend_filter(builder, &grpc_client_idle_filter, nullptr,
                                                     nullptr);
  } else {
    return true;
  }
...
void grpc_client_idle_filter_init(void) {
  grpc_channel_init_register_stage(GRPC_CLIENT_CHANNEL, GRPC_CHANNEL_INIT_BUILTIN_PRIORITY,
