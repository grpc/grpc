...
// TODO(juanlishen): The idle filter is disabled in client channel by
// default due to b/143502997. Try to fix the bug and enable the filter
// by default.
...
#define GRPC_IDLE_FILTER_LOG(format, ...)                         \
  do {                                                            \
    if (GRPC_TRACE_FLAG_ENABLED(grpc_trace_client_idle_filter)) { \
      gpr_log(GPR_INFO, "(client idle filter) " format,           \
              ##__VA_ARGS__);                                     \
    }                                                             \
  } while (0)
...
/*
  client_idle_filter maintains a state tracking if there are active
calls in the channel and its internal idle_timer_. The states are
specified as following:
  ...
  IDLE: The initial state of the client_idle_filter, indicating the
channel is in IDLE.

  CALLS_ACTIVE: The channel has 1 or 1+ active calls and the timer is
not set.

  TIMER_PENDING: The state after the timer is set and no calls have
arrived after the timer is set. The channel must have 0 active call in
this state. If the timer is fired in this state, the channel will go
into IDLE state.

  TIMER_PENDING_CALLS_ACTIVE: The state after the timer is set and at
least one call has arrived after the timer is set. The channel must have
1 or 1+ active calls in this state. If the timer is fired in this state,
we won't reschedule it.

  TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START: The state after the timer
is set and at least one call has arrived after the timer is set, BUT the
channel currently has 0 active call. If the timer is fired in this
state, we will reschedule it according to the finish time of the latest
call.

  PROCESSING: The state set to block other threads when the setting
thread is doing some work to keep state consistency.

  idle_timer_ will not be cancelled (unless the channel is shutting
down). If the timer callback is called when the idle_timer_ is valid
(i.e. idle_state is TIMER_PENDING), the channel will enter IDLE,
otherwise the channel won't be changed.
...
class ChannelData {
 ...
 private:
  ChannelData(grpc_channel_element* elem,
              grpc_channel_element_args* args,
  ...
  // the channel goes back into IDLE state.
...
void ChannelData::StartTransportOp(grpc_channel_element* elem,
  ...
  if (op->disconnect_with_error != GRPC_ERROR_NONE) {
    // IncreaseCallCount() introduces a phony call and prevent the timer
    // from being reset by other threads.
    chand->IncreaseCallCount();
    // If the timer has been set, cancel the timer.
    // No synchronization issues here. grpc_timer_cancel() is valid as
    // long as the timer has been init()ed before.
  ...
  if (previous_value == 0) {
    ...
    while (true) {
      switch (state) {
        ...
        case IDLE:
          // In this case, no other threads will modify the state, so we
          // can just store the value.
        ...
        case TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START:
          // At this point, the state may have been switched to IDLE by
          // the idle timer callback. Therefore, use CAS operation to
          // change the state atomically. Use MemoryOrder::ACQUIRE on
          // success to ensure last_idle_time_ has been properly set in
          // DecreaseCallCount().
          if (state_.CompareExchangeWeak(
                  &state, TIMER_PENDING_CALLS_ACTIVE,
                  MemoryOrder::ACQUIRE, MemoryOrder::RELAXED)) {
            return;
          }
          break;
        default:
          // The state has not been switched to desired value yet, try
          // again.
...
void ChannelData::DecreaseCallCount() {
  const intptr_t previous_value =
      call_count_.FetchSub(1, MemoryOrder::RELAXED);
  GRPC_IDLE_FILTER_LOG("call counter has decreased to %" PRIuPTR,
                       previous_value - 1);
  if (previous_value == 1) {
    // This call is the one that makes the channel idle.
    // last_idle_time_ does not need to be Atomic<> because busy-loops
    // in IncreaseCallCount(), DecreaseCallCount() and
    // IdleTimerCallback() will prevent multiple threads from
    // simultaneously accessing this variable.
    ...
    while (true) {
      switch (state) {
        ...
        // TIMER_PENDING
        case CALLS_ACTIVE:
          // Release store here to make other threads see the updated
          // value of last_idle_time_.
          StartIdleTimer();
        ...
        case TIMER_PENDING_CALLS_ACTIVE:
          // At this point, the state may have been switched to
          // CALLS_ACTIVE by the idle timer callback. Therefore, use CAS
          // operation to change the state atomically. Release store
          // here to make the idle timer callback see the updated value
          // of last_idle_time_ to properly reset the idle timer.
          if (state_.CompareExchangeWeak(
...
ChannelData::ChannelData(grpc_channel_element* elem,
  ...
  // If the idle filter is explicitly disabled in channel args, this
  // ctor should not get called.
...
void ChannelData::IdleTimerCallback(void* arg,
  ...
  if (error != GRPC_ERROR_NONE) {
    ...
    GRPC_CHANNEL_STACK_UNREF(chand->channel_stack_,
                             "max idle timer callback");
    return;
  }
  bool finished = false;
  ...
  while (!finished) {
    switch (state) {
      case TIMER_PENDING:
        // Change the state to PROCESSING to block IncreaseCallCout()
        // until the EnterIdle() operation finishes, preventing
        // mistakenly entering IDLE when active RPC exists.
        finished = chand->state_.CompareExchangeWeak(
            &state, PROCESSING, MemoryOrder::ACQUIRE,
            MemoryOrder::RELAXED);
        if (finished) {
          chand->EnterIdle();
      ...
      case TIMER_PENDING_CALLS_ACTIVE:
        finished = chand->state_.CompareExchangeWeak(
            &state, CALLS_ACTIVE, MemoryOrder::RELAXED,
            MemoryOrder::RELAXED);
        break;
      case TIMER_PENDING_CALLS_SEEN_SINCE_TIMER_START:
        // Change the state to PROCESSING to block IncreaseCallCount()
        // until the StartIdleTimer() operation finishes, preventing
        // mistakenly restarting the timer after grpc_timer_cancel()
        // when shutdown.
        finished = chand->state_.CompareExchangeWeak(
            &state, PROCESSING, MemoryOrder::ACQUIRE,
            MemoryOrder::RELAXED);
...
void ChannelData::IdleTransportOpCompleteCallback(
...
void ChannelData::EnterIdle() {
  ...
  idle_transport_op_.on_consumed =
      &idle_transport_op_complete_callback_;
...
grpc_error_handle CallData::Init(
...
static bool MaybeAddClientIdleFilter(
