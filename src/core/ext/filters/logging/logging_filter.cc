...
void CallData::LogClientHeader(bool is_client, CallTracer* tracer,
...
void CallData::LogClientHalfClose(bool is_client, CallTracer* tracer) {
  LoggingSink::Entry entry;
  SetCommonEntryFields(&entry, is_client, tracer,
...
void CallData::LogServerHeader(bool is_client, CallTracer* tracer,
...
void CallData::LogServerTrailer(bool is_client, CallTracer* tracer,
                                const ServerMetadata* metadata) {
  LoggingSink::Entry entry;
  SetCommonEntryFields(&entry, is_client, tracer,
...
void CallData::LogClientMessage(bool is_client, CallTracer* tracer,
...
void CallData::LogServerMessage(bool is_client, CallTracer* tracer,
                                const SliceBuffer* message) {
  LoggingSink::Entry entry;
  SetCommonEntryFields(&entry, is_client, tracer,
...
void CallData::LogCancel(bool is_client, CallTracer* tracer) {
  LoggingSink::Entry entry;
  SetCommonEntryFields(&entry, is_client, tracer,
...
void ClientLoggingFilter::Call::OnClientInitialMetadata(
  ...
  call_data_->LogClientHeader(
      /*is_client=*/true, MaybeGetContext<CallTracer>(), md);
}
...
void ClientLoggingFilter::Call::OnServerTrailingMetadata(ServerMetadata& md) {
  ...
  if (md.get(GrpcCallWasCancelled()).value_or(false) &&
    ...
    call_data_->LogCancel(
        /*is_client=*/true, MaybeGetContext<CallTracer>());
    return;
  }
...
void ClientLoggingFilter::Call::OnClientToServerMessage(
  ...
  call_data_->LogClientMessage(
      /*is_client=*/true, MaybeGetContext<CallTracer>(), message.payload());
}
...
void ServerLoggingFilter::Call::OnClientInitialMetadata(ClientMetadata& md) {
  ...
  call_data_->LogClientHeader(
      /*is_client=*/false, MaybeGetContext<CallTracer>(), md);
}
...
void ServerLoggingFilter::Call::OnServerTrailingMetadata(ServerMetadata& md) {
  ...
  if (md.get(GrpcCallWasCancelled()).value_or(false) &&
      md.get(GrpcStatusMetadata()) == GRPC_STATUS_CANCELLED) {
    call_data_->LogCancel(
        /*is_client=*/false, MaybeGetContext<CallTracer>());
    return;
  }
  call_data_->LogServerTrailer(
      /*is_client=*/false, MaybeGetContext<CallTracer>(), &md);
...
void ServerLoggingFilter::Call::OnClientToServerMessage(
  ...
  if (!call_data_.has_value()) return;
  call_data_->LogClientMessage(
      /*is_client=*/false, MaybeGetContext<CallTracer>(), message.payload());
}
