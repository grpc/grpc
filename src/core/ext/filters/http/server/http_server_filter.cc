...
static void
add_error (const char *error_name, grpc_error ** cumulative,
	   grpc_error * new_err)
{
...
static grpc_error *
server_filter_incoming_metadata (grpc_call_element * elem,
				 grpc_metadata_batch * b)
{
  ...
  else
    {
      add_error (error_name, &error,
		 grpc_error_set_str (GRPC_ERROR_CREATE_FROM_STATIC_STRING
				     ("Missing header"), GRPC_ERROR_STR_KEY,
      ...
      if (offset < path_length)
	{
	  ...
	  grpc_mdelem mdelem_path_without_query =
	    grpc_mdelem_from_slices (GRPC_MDSTR_PATH,
				     grpc_slice_sub (path_slice, 0, offset));
	  ...
	  grpc_slice_buffer_add (&calld->read_slice_buffer,
				 grpc_base64_decode_with_len ((const char *)
							      GRPC_SLICE_START_PTR
							      (query_slice),
							      GRPC_SLICE_LENGTH
							      (query_slice),
  ...
  if (b->idx.named.host != nullptr && b->idx.named.authority == nullptr)
    {
      ...
      add_error (error_name, &error,
		 grpc_metadata_batch_add_head (b, el,
...
static void
hs_on_recv (void *user_data, grpc_error * err)
{
  ...
  if (err == GRPC_ERROR_NONE)
    {
      err =
	server_filter_incoming_metadata (elem, calld->recv_initial_metadata);
    }
  else
    {
      GRPC_ERROR_REF (err);
...
static void
hs_on_complete (void *user_data, grpc_error * err)
{
  grpc_call_element *elem = (grpc_call_element *) user_data;
  call_data *calld = (call_data *) elem->call_data;
  ...
  if (calld->seen_path_with_query && calld->recv_message_ready != nullptr)
	...
	? nullptr : (grpc_byte_stream *) & calld->read_stream;
...
static void
hs_recv_message_ready (void *user_data, grpc_error * err)
{
  grpc_call_element *elem = (grpc_call_element *) user_data;
  call_data *calld = (call_data *) elem->call_data;
...
static grpc_error *
hs_mutate_op (grpc_call_element * elem, grpc_transport_stream_op_batch * op)
{
  ...
  if (op->send_initial_metadata)
    {
      ...
      add_error (error_name, &error,
		 grpc_metadata_batch_add_head (op->payload->
					       send_initial_metadata.
					       send_initial_metadata,
					       &calld->status,
					       GRPC_MDELEM_STATUS_200));
      add_error (error_name, &error,
		 grpc_metadata_batch_add_tail (op->payload->
					       send_initial_metadata.
					       send_initial_metadata,
					       &calld->content_type,
					       GRPC_MDELEM_CONTENT_TYPE_APPLICATION_SLASH_GRPC));
...
static void
hs_start_transport_stream_op_batch (grpc_call_element * elem,
...
init_call_elem (grpc_call_element * elem, const grpc_call_element_args * args)
{
  ...
  GRPC_CLOSURE_INIT (&calld->hs_on_complete, hs_on_complete, elem,
		     grpc_schedule_on_exec_ctx);
  GRPC_CLOSURE_INIT (&calld->hs_recv_message_ready, hs_recv_message_ready,
