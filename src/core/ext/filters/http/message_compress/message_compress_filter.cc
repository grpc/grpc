 ...
 public:
  explicit ChannelData(grpc_channel_element_args* args) {
    ...
    default_compression_algorithm_ =
        grpc_channel_args_get_channel_default_compression_algorithm(args->channel_args);
    // Make sure the default is enabled.
    if (!GPR_BITGET(enabled_compression_algorithms_bitset_, default_compression_algorithm_)) {
      const char* name;
      GPR_ASSERT(grpc_compression_algorithm_name(default_compression_algorithm_, &name) == 1);
      gpr_log(GPR_ERROR, "default compression algorithm %s not enabled: switching to none", name);
      default_compression_algorithm_ = GRPC_COMPRESS_NONE;
    }
    enabled_message_compression_algorithms_bitset_ =
        grpc_compression_bitset_to_message_bitset(enabled_compression_algorithms_bitset_);
    enabled_stream_compression_algorithms_bitset_ =
        grpc_compression_bitset_to_stream_bitset(enabled_compression_algorithms_bitset_);
...
class CallData {
 public:
  CallData(grpc_call_element* elem, const grpc_call_element_args& args)
    ...
    if (GPR_LIKELY(GPR_BITGET(channeld->enabled_compression_algorithms_bitset(),
      ...
      message_compression_algorithm_ = grpc_compression_algorithm_to_message_compression_algorithm(
          channeld->default_compression_algorithm());
    }
    GRPC_CLOSURE_INIT(&start_send_message_batch_in_call_combiner_, StartSendMessageBatch, elem,
  ...
  void CompressStartTransportStreamOpBatch(grpc_call_element* elem,
 ...
 private:
  ...
  grpc_error_handle ProcessSendInitialMetadata(grpc_call_element* elem,
  ...
  static void FailSendMessageBatchInCallCombiner(void* calld_arg, grpc_error_handle error);

  static void SendMessageOnComplete(void* calld_arg, grpc_error_handle error);

  grpc_core::CallCombiner* call_combiner_;
  grpc_message_compression_algorithm message_compression_algorithm_ = GRPC_MESSAGE_COMPRESS_NONE;
  ...
  std::aligned_storage<sizeof(grpc_core::SliceBufferByteStream),
                       alignof(grpc_core::SliceBufferByteStream)>::type replacement_stream_;
...
bool CallData::SkipMessageCompression() {
  ...
  uint32_t flags = send_message_batch_->payload->send_message.send_message->flags();
...
grpc_compression_algorithm FindCompressionAlgorithm(grpc_metadata_batch* initial_metadata,
  ...
  GPR_ASSERT(grpc_compression_algorithm_parse(GRPC_MDVALUE(md), &compression_algorithm));
  // Remove this metadata since it's an internal one (i.e., it won't be
  // transmitted out).
  grpc_metadata_batch_remove(initial_metadata, GRPC_BATCH_GRPC_INTERNAL_ENCODING_REQUEST);
  ...
  if (GPR_LIKELY(
          GPR_BITGET(channeld->enabled_compression_algorithms_bitset(), compression_algorithm))) {
    return compression_algorithm;
  }
  const char* algorithm_name;
  GPR_ASSERT(grpc_compression_algorithm_name(compression_algorithm, &algorithm_name));
  gpr_log(GPR_ERROR,
...
grpc_error_handle CallData::ProcessSendInitialMetadata(grpc_call_element* elem,
                                                       grpc_metadata_batch* initial_metadata) {
  ChannelData* channeld = static_cast<ChannelData*>(elem->channel_data);
  ...
  message_compression_algorithm_ =
      grpc_compression_algorithm_to_message_compression_algorithm(compression_algorithm);
  grpc_stream_compression_algorithm stream_compression_algorithm =
      grpc_compression_algorithm_to_stream_compression_algorithm(compression_algorithm);
  ...
  if (message_compression_algorithm_ != GRPC_MESSAGE_COMPRESS_NONE) {
    ...
    error = grpc_metadata_batch_add_tail(
        ...
        grpc_message_compression_encoding_mdelem(message_compression_algorithm_),
  ...
  error =
      grpc_metadata_batch_add_tail(initial_metadata, &accept_encoding_storage_,
  ...
  if (!initial_metadata->idx.named.accept_encoding) {
    error =
        grpc_metadata_batch_add_tail(initial_metadata, &accept_stream_encoding_storage_,
...
void CallData::SendMessageOnComplete(void* calld_arg, grpc_error_handle error) {
  ...
  grpc_core::Closure::Run(DEBUG_LOCATION, calld->original_send_message_on_complete_,
...
void CallData::FinishSendMessage(grpc_call_element* elem) {
  GPR_DEBUG_ASSERT(message_compression_algorithm_ != GRPC_MESSAGE_COMPRESS_NONE);
  // Compress the data if appropriate.
  grpc_slice_buffer tmp;
  grpc_slice_buffer_init(&tmp);
  uint32_t send_flags = send_message_batch_->payload->send_message.send_message->flags();
  bool did_compress = grpc_msg_compress(message_compression_algorithm_, &slices_, &tmp);
  if (did_compress) {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_compression_trace)) {
      const char* algo_name;
      const size_t before_size = slices_.length;
      const size_t after_size = tmp.length;
      const float savings_ratio =
          1.0f - static_cast<float>(after_size) / static_cast<float>(before_size);
      GPR_ASSERT(
          grpc_message_compression_algorithm_name(message_compression_algorithm_, &algo_name));
      gpr_log(GPR_INFO, "Compressed[%s] %" PRIuPTR " bytes vs. %" PRIuPTR " bytes (%.2f%% savings)",
  ...
  // batch down.
  new (&replacement_stream_) grpc_core::SliceBufferByteStream(&slices_, send_flags);
  send_message_batch_->payload->send_message.send_message.reset(
      reinterpret_cast<grpc_core::SliceBufferByteStream*>(&replacement_stream_));
...
void CallData::FailSendMessageBatchInCallCombiner(void* calld_arg, grpc_error_handle error) {
  CallData* calld = static_cast<CallData*>(calld_arg);
  if (calld->send_message_batch_ != nullptr) {
    grpc_transport_stream_op_batch_finish_with_failure(
        calld->send_message_batch_, GRPC_ERROR_REF(error), calld->call_combiner_);
...
grpc_error_handle CallData::PullSliceFromSendMessage() {
  ...
  grpc_error_handle error =
      send_message_batch_->payload->send_message.send_message->Pull(&incoming_slice);
...
void CallData::ContinueReadingSendMessage(grpc_call_element* elem) {
  if (slices_.length == send_message_batch_->payload->send_message.send_message->length()) {
...
void CallData::OnSendMessageNextDone(void* elem_arg, grpc_error_handle error) {
  ...
  if (calld->slices_.length ==
      calld->send_message_batch_->payload->send_message.send_message->length()) {
...
void CallData::StartSendMessageBatch(void* elem_arg, grpc_error_handle /*unused*/) {
  grpc_call_element* elem = static_cast<grpc_call_element*>(elem_arg);
  CallData* calld = static_cast<CallData*>(elem->call_data);
...
void CallData::CompressStartTransportStreamOpBatch(grpc_call_element* elem,
                                                   grpc_transport_stream_op_batch* batch) {
  ...
  if (batch->send_initial_metadata) {
    ...
    if (error != GRPC_ERROR_NONE) {
      grpc_transport_stream_op_batch_finish_with_failure(batch, error, call_combiner_);
      return;
    }
    ...
    if (send_message_batch_ != nullptr) {
      GRPC_CALL_COMBINER_START(call_combiner_, &start_send_message_batch_in_call_combiner_,
                               GRPC_ERROR_NONE,
  ...
  if (batch->send_message) {
    ...
    if (!seen_initial_metadata_) {
      GRPC_CALL_COMBINER_STOP(call_combiner_, "send_message batch pending send_initial_metadata");
      return;
    }
...
void CompressDestroyCallElem(grpc_call_element* elem, const grpc_call_final_info* /*final_info*/,
