...
void CallData::CompressStartTransportStreamOpBatch(
  ...
  if (GPR_LIKELY(!g_test_only_disable_decompression)) {
...
void TestOnlyDisableDecompression() {
  g_test_only_disable_decompression = true;
}

void TestOnlyEnableDecompression() {
  g_test_only_disable_decompression = false;
}
