...
static bool
skip_compression (grpc_call_element * elem, uint32_t flags,
...
static grpc_error *
process_send_initial_metadata (grpc_call_element * elem,
			       grpc_metadata_batch * initial_metadata,
			       bool * has_compression_algorithm)
  GRPC_MUST_USE_RESULT;
     static grpc_error *process_send_initial_metadata (grpc_call_element *
						       elem,
						       grpc_metadata_batch *
						       initial_metadata,
						       bool *
  ...
  if (!initial_metadata->idx.named.accept_encoding)
    {
      error =
	grpc_metadata_batch_add_tail (initial_metadata,
				      &calld->accept_stream_encoding_storage,
				      GRPC_MDELEM_ACCEPT_STREAM_ENCODING_FOR_ALGORITHMS
				      (channeld->
				       supported_stream_compression_algorithms));
    }
...
static void
send_message_batch_continue (grpc_call_element * elem)
{
  call_data *calld = (call_data *) elem->call_data;
...
static void
finish_send_message (grpc_call_element * elem)
{
  call_data *calld = (call_data *) elem->call_data;
  ...
  if (did_compress)
      ...
      if (grpc_compression_trace.enabled ())
	{
	  ...
	  const float savings_ratio =
	    1.0f - (float) after_size / (float) before_size;
	  GPR_ASSERT (grpc_compression_algorithm_name
		      (calld->compression_algorithm, &algo_name));
	  gpr_log (GPR_DEBUG,
		   "Compressed[%s] %" PRIuPTR " bytes vs. %" PRIuPTR
		   " bytes (%.2f%% savings)", algo_name, before_size,
		   after_size, 100 * savings_ratio);
	}
      ...
      if (grpc_compression_trace.enabled ())
	{
	  ...
	  gpr_log (GPR_DEBUG,
		   ...
		   "%" PRIuPTR, algo_name, calld->slices.length);
	}
    }
  grpc_slice_buffer_destroy_internal (&tmp);
  // Swap out the original byte stream with our new one and send the
  // batch down.
  grpc_byte_stream_destroy (calld->send_message_batch->payload->send_message.
			    send_message);
...
static void
fail_send_message_batch_in_call_combiner (void *arg, grpc_error * error)
{
...
pull_slice_from_send_message (call_data * calld)
{
  ...
  grpc_error *error =
    grpc_byte_stream_pull (calld->send_message_batch->payload->send_message.
			   send_message,
			   &incoming_slice);
  if (error == GRPC_ERROR_NONE)
    {
...
static void
continue_reading_send_message (grpc_call_element * elem)
{
  call_data *calld = (call_data *) elem->call_data;
...
static void
start_send_message_batch (void *arg, grpc_error * unused)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  if (skip_compression (elem,
			calld->send_message_batch->payload->send_message.
			send_message->flags,
...
static void
compress_start_transport_stream_op_batch (grpc_call_element * elem,
					  ...
					  batch)
{
	  ...
	  if (calld->send_initial_metadata_state == INITIAL_METADATA_UNSEEN)
	    {
	      GRPC_CALL_COMBINER_START (calld->call_combiner,
					...
					"failing send_message op");
	    }
	  else
	    {
	      grpc_byte_stream_shutdown (calld->send_message_batch->payload->
					 send_message.send_message,
					 GRPC_ERROR_REF (calld->
							 cancel_error));
	    }
	}
    }
  ...
  if (batch->send_initial_metadata)
    {
      GPR_ASSERT (calld->send_initial_metadata_state ==
		  INITIAL_METADATA_UNSEEN);
      bool has_compression_algorithm;
      grpc_error *error =
	process_send_initial_metadata (elem,
	...
	? HAS_COMPRESSION_ALGORITHM : NO_COMPRESSION_ALGORITHM;
      ...
      if (calld->send_message_batch != nullptr)
	{
	  GRPC_CALL_COMBINER_START (calld->call_combiner,
				    &calld->
				    start_send_message_batch_in_call_combiner,
				    GRPC_ERROR_NONE,
		   ...
		   grpc_channel_element_args * args)
{
  ...
  channeld->supported_compression_algorithms =
    (((1u << GRPC_COMPRESS_ALGORITHMS_COUNT) - 1) &
     channeld->enabled_algorithms_bitset) | 1u;

  /* Configuration for stream compression */
  channeld->enabled_stream_compression_algorithms_bitset =
    grpc_channel_args_stream_compression_algorithm_get_states (args->
							       channel_args);
