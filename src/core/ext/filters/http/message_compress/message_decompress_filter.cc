...
class ChannelData {
 public:
  explicit ChannelData(const grpc_channel_element_args* args)
      : max_recv_size_(
            GetMaxRecvSizeFromChannelArgs(args->channel_args)) {}
...
class CallData {
 public:
  CallData(const grpc_call_element_args& args, const ChannelData* chand)
    ...
    GRPC_CLOSURE_INIT(&on_recv_message_next_done_,
                      OnRecvMessageNextDone, this,
                      grpc_schedule_on_exec_ctx);
 ...
 private:
  ...
  grpc_message_compression_algorithm algorithm_ =
  ...
  // recv_slices_ holds the slices read from the original recv_message
  // stream. It is initialized during construction and reset when a new
  // stream is created using it.
...
void CallData::OnRecvInitialMetadataReady(void* arg,
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (grpc_encoding != nullptr) {
      calld->algorithm_ =
          DecodeMessageCompressionAlgorithm(grpc_encoding->md);
    }
  }
...
void CallData::OnRecvMessageReady(void* arg, grpc_error_handle error) {
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (calld->algorithm_ != GRPC_MESSAGE_COMPRESS_NONE) {
      // recv_message can be NULL if trailing metadata is received
      // instead of message, or it's possible that the message was not
      // compressed.
      if (*calld->recv_message_ == nullptr ||
          (*calld->recv_message_)->length() == 0 ||
          ((*calld->recv_message_)->flags() &
...
void CallData::FinishRecvMessage() {
  ...
  if (grpc_msg_decompress(algorithm_, &recv_slices_,
                          &decompressed_slices) == 0) {
    ...
    // Swap out the original receive byte stream with our new one and
    // send the batch down. Initializing recv_replacement_stream_ with
    // decompressed_slices removes all the slices from
    // decompressed_slices leaving it empty.
    new (&recv_replacement_stream_)
        SliceBufferByteStream(&decompressed_slices, recv_flags);
    recv_message_->reset(reinterpret_cast<SliceBufferByteStream*>(
        &recv_replacement_stream_));
...
void CallData::ContinueRecvMessageReadyCallback(
  ...
  // The surface will clean up the receiving stream if there is an
  // error.
...
void CallData::MaybeResumeOnRecvTrailingMetadataReady() {
  if (seen_recv_trailing_metadata_ready_) {
    ...
    GRPC_CALL_COMBINER_START(call_combiner_,
                             &on_recv_trailing_metadata_ready_, error,
...
void CallData::OnRecvTrailingMetadataReady(void* arg,
  ...
  if (calld->original_recv_initial_metadata_ready_ != nullptr ||
    ...
    calld->on_recv_trailing_metadata_ready_error_ =
        GRPC_ERROR_REF(error);
    GRPC_CALL_COMBINER_STOP(
        calld->call_combiner_,
...
grpc_error_handle DecompressInitCallElem(
...
grpc_error_handle DecompressInitChannelElem(
