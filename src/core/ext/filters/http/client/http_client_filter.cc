...
static grpc_error *
client_filter_incoming_metadata (grpc_call_element * elem,
				 grpc_metadata_batch * b)
{
	    ...
	    grpc_error_set_str (grpc_error_set_int
				(grpc_error_set_str
				 (GRPC_ERROR_CREATE_FROM_STATIC_STRING
				  ...
				  grpc_slice_from_copied_string (val)),
  ...
  if (b->idx.named.grpc_message != nullptr)
	...
	grpc_permissive_percent_decode_slice (GRPC_MDVALUE
					      (b->idx.named.grpc_message->
					       md));
      if (grpc_slice_is_equivalent
	  (pct_decoded_msg, GRPC_MDVALUE (b->idx.named.grpc_message->md)))
	{
  ...
  if (b->idx.named.content_type != nullptr)
			   ...
			   GRPC_MDELEM_CONTENT_TYPE_APPLICATION_SLASH_GRPC))
	{
	  if (grpc_slice_buf_start_eq
	      (GRPC_MDVALUE (b->idx.named.content_type->md),
	      ...
	      (GRPC_SLICE_START_PTR
	       ...
	       GRPC_SLICE_START_PTR (GRPC_MDVALUE
				     (b->idx.named.content_type->
				      md))[EXPECTED_CONTENT_TYPE_LENGTH] ==
	       ';'))
	    {
...
recv_initial_metadata_ready (void *user_data, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE)
    {
      error =
	client_filter_incoming_metadata (elem, calld->recv_initial_metadata);
    }
  else
    {
...
static void
recv_trailing_metadata_on_complete (void *user_data, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) user_data;
  call_data *calld = (call_data *) elem->call_data;
  if (error == GRPC_ERROR_NONE)
  ...
  GRPC_CLOSURE_RUN (calld->original_recv_trailing_metadata_on_complete,
		    error);
}

static void
send_message_on_complete (void *arg, grpc_error * error)
{
...
static grpc_error *
pull_slice_from_send_message (call_data * calld)
{
  grpc_slice incoming_slice;
  grpc_error *error =
    grpc_byte_stream_pull (&calld->send_message_caching_stream.base,
			   &incoming_slice);
  if (error == GRPC_ERROR_NONE)
    {
...
static grpc_error *
read_all_available_send_message_data (call_data * calld)
{
...
static void
on_send_message_next_done (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  call_data *calld = (call_data *) elem->call_data;
  if (error != GRPC_ERROR_NONE)
...
static char *
slice_buffer_to_string (grpc_slice_buffer * slice_buffer)
{
...
static grpc_error *
update_path_for_get (grpc_call_element * elem,
		     grpc_transport_stream_op_batch * batch)
{
  call_data *calld = (call_data *) elem->call_data;
  grpc_slice path_slice =
    GRPC_MDVALUE (batch->payload->send_initial_metadata.
  ...
  estimated_len +=
    grpc_base64_estimate_encoded_size (batch->payload->send_message.
				       send_message->length,
				       true /* url_safe */ ,
				       false /* multi_line */ );
...
static void
remove_if_present (grpc_metadata_batch * batch,
...
static void
hc_start_transport_stream_op_batch (grpc_call_element * elem,
				    grpc_transport_stream_op_batch * batch)
{
  call_data *calld = (call_data *) elem->call_data;
  ...
  if (batch->send_initial_metadata)
	  ...
	  channeld->max_payload_size_for_get)
	{
	  ...
	  if (calld->send_message_bytes_read ==
		...
		goto done;
	      ...
	      grpc_byte_stream_destroy (&calld->send_message_caching_stream.
					base);
	    }
	  else
	    {
	       ...
	       GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST)
	{
      ...
      remove_if_present (batch->payload->send_initial_metadata.
			 send_initial_metadata, GRPC_BATCH_METHOD);
      remove_if_present (batch->payload->send_initial_metadata.
			 send_initial_metadata, GRPC_BATCH_SCHEME);
      remove_if_present (batch->payload->send_initial_metadata.
			 send_initial_metadata, GRPC_BATCH_TE);
      remove_if_present (batch->payload->send_initial_metadata.
			 send_initial_metadata, GRPC_BATCH_CONTENT_TYPE);
      remove_if_present (batch->payload->send_initial_metadata.
			 send_initial_metadata, GRPC_BATCH_USER_AGENT);

      /* Send : prefixed headers, which have to be before any application
         layer headers. */
      error =
	grpc_metadata_batch_add_head (batch->payload->send_initial_metadata.
				      send_initial_metadata, &calld->method,
				      method);
      if (error != GRPC_ERROR_NONE)
	goto done;
      error =
	grpc_metadata_batch_add_head (batch->payload->send_initial_metadata.
				      send_initial_metadata, &calld->scheme,
				      channeld->static_scheme);
      if (error != GRPC_ERROR_NONE)
	goto done;
      error =
	grpc_metadata_batch_add_tail (batch->payload->send_initial_metadata.
  ...
  if (args != nullptr)
      ...
      for (i = 0; i < args->num_args; ++i)
	{
	  if (args->args[i].type == GRPC_ARG_STRING &&
		...
		{
		  if (0 ==
		      grpc_slice_str_cmp (GRPC_MDVALUE (valid_schemes[j]),
...
static size_t
max_payload_size_from_args (const grpc_channel_args * args)
{
  if (args != nullptr)
    {
	...
	{
	  if (0 ==
	      strcmp (args->args[i].key, GRPC_ARG_MAX_PAYLOAD_SIZE_FOR_GET))
	    {
...
static grpc_slice
user_agent_from_args (const grpc_channel_args * args,
		      const char *transport_name)
{
  ...
  for (i = 0; args && i < args->num_args; i++)
    {
      if (0 ==
	  strcmp (args->args[i].key, GRPC_ARG_SECONDARY_USER_AGENT_STRING))
	{
	  if (args->args[i].type != GRPC_ARG_STRING)
	    {
	      gpr_log (GPR_ERROR, "Channel argument '%s' should be a string",
		   ...
		   grpc_channel_element_args * args)
{
  ...
  chand->user_agent = grpc_mdelem_from_slices (GRPC_MDSTR_USER_AGENT,
					       user_agent_from_args (args->
								     channel_args,
								     args->
								     optional_transport->
								     vtable->
