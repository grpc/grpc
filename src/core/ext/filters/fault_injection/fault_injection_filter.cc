...
static_assert(std::is_trivially_destructible<Atomic<uint32_t>>::value,
...
inline bool UnderFraction(const uint32_t numerator, const uint32_t denominator) {
...
class CallData {
 ...
 private:
  ...
  void DelayBatch(grpc_call_element* elem, grpc_transport_stream_op_batch* batch);
  ...
  void FaultInjectionFinished() { g_active_faults.FetchSub(1, MemoryOrder::RELAXED); }
...
ChannelData::ChannelData(grpc_channel_element* elem, grpc_channel_element_args* args)
    : index_(grpc_channel_stack_filter_instance_number(args->channel_stack, elem)) {}
...
grpc_error_handle CallData::Init(grpc_call_element* elem, const grpc_call_element_args* args) {
  ...
  if (calld->fi_policy_ == nullptr) {
    return GRPC_ERROR_CREATE_FROM_STATIC_STRING("failed to find fault injection policy");
  }
  return GRPC_ERROR_NONE;
}
...
void CallData::StartTransportStreamOpBatch(grpc_call_element* elem,
  ...
  if (batch->send_initial_metadata) {
    calld->DecideWhetherToInjectFaults(batch->payload->send_initial_metadata.send_initial_metadata);
    if (GRPC_TRACE_FLAG_ENABLED(grpc_fault_injection_filter_trace)) {
      gpr_log(GPR_INFO, "chand=%p calld=%p: Fault injection triggered delay=%d abort=%d",
              elem->channel_data, calld, calld->delay_request_, calld->abort_request_);
    }
    if (calld->MaybeDelay()) {
    ...
    if (abort_error != GRPC_ERROR_NONE) {
      ...
      grpc_transport_stream_op_batch_finish_with_failure(batch, GRPC_ERROR_REF(calld->abort_error_),
...
CallData::CallData(grpc_call_element* elem, const grpc_call_element_args* args)
    : owning_call_(args->call_stack), arena_(args->arena), call_combiner_(args->call_combiner) {
  auto* chand = static_cast<ChannelData*>(elem->channel_data);
  ...
  GRPC_CLOSURE_INIT(&recv_trailing_metadata_ready_, HijackedRecvTrailingMetadataReady, elem,
...
void CallData::DecideWhetherToInjectFaults(grpc_metadata_batch* initial_metadata) {
  FaultInjectionMethodParsedConfig::FaultInjectionPolicy* copied_policy = nullptr;
  // Update the policy with values in initial metadata.
  if (!fi_policy_->abort_code_header.empty() || !fi_policy_->abort_percentage_header.empty() ||
      !fi_policy_->delay_header.empty() || !fi_policy_->delay_percentage_header.empty()) {
    ...
    auto maybe_copy_policy_func = [this, &copied_policy]() {
      if (copied_policy == nullptr) {
        copied_policy =
            arena_->New<FaultInjectionMethodParsedConfig::FaultInjectionPolicy>(*fi_policy_);
      }
    };
    for (grpc_linked_mdelem* md = initial_metadata->list.head; md != nullptr; md = md->next) {
      ...
      if (!fi_policy_->abort_code_header.empty() &&
          (copied_policy == nullptr || copied_policy->abort_code == GRPC_STATUS_OK) &&
      ...
      if (!fi_policy_->abort_percentage_header.empty() &&
        ...
        copied_policy->abort_percentage_numerator =
            GPR_MIN(GetLinkedMetadatumValueUnsignedInt(md), fi_policy_->abort_percentage_numerator);
      }
      if (!fi_policy_->delay_header.empty() &&
        ...
        maybe_copy_policy_func();
        copied_policy->delay =
            static_cast<grpc_millis>(GPR_MAX(GetLinkedMetadatumValueInt64(md), 0));
      }
      if (!fi_policy_->delay_percentage_header.empty() &&
        ...
        copied_policy->delay_percentage_numerator =
            GPR_MIN(GetLinkedMetadatumValueUnsignedInt(md), fi_policy_->delay_percentage_numerator);
      }
    }
  ...
  // Roll the dice
  delay_request_ =
      fi_policy_->delay != 0 && UnderFraction(fi_policy_->delay_percentage_numerator,
...
void CallData::DelayBatch(grpc_call_element* elem, grpc_transport_stream_op_batch* batch) {
  ...
  GRPC_CLOSURE_INIT(&batch->handler_private.closure, ResumeBatch, elem, grpc_schedule_on_exec_ctx);
...
void CallData::ResumeBatch(void* arg, grpc_error_handle error) {
  ...
  if (error == GRPC_ERROR_CANCELLED || calld->resume_batch_canceller_ == nullptr) {
    return;
  }
  if (GRPC_TRACE_FLAG_ENABLED(grpc_fault_injection_filter_trace)) {
    gpr_log(GPR_INFO, "chand=%p calld=%p: Resuming delayed stream op batch %p", elem->channel_data,
            calld, calld->delayed_batch_);
  }
  // Lame the canceller
  ...
  if (error != GRPC_ERROR_NONE) {
    grpc_transport_stream_op_batch_finish_with_failure(calld->delayed_batch_, error,
                                                       calld->call_combiner_);
    return;
  }
  // Chain to the next filter.
...
void CallData::HijackedRecvTrailingMetadataReady(void* arg, grpc_error_handle error) {
  grpc_call_element* elem = static_cast<grpc_call_element*>(arg);
  auto* calld = static_cast<CallData*>(elem->call_data);
  if (calld->abort_error_ != GRPC_ERROR_NONE) {
    error = grpc_error_add_child(GRPC_ERROR_REF(error), GRPC_ERROR_REF(calld->abort_error_));
  } else {
...
void FaultInjectionFilterInit(void) { grpc_core::FaultInjectionServiceConfigParser::Register(); }
