...
static void
yield_call_combiner (void *arg, grpc_error * ignored)
{
...
// synchronized.
static void
send_cancel_op_in_call_combiner (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  grpc_deadline_state *deadline_state =
    (grpc_deadline_state *) elem->call_data;
...
// Timer callback.
static void
timer_callback (void *arg, grpc_error * error)
{
  grpc_call_element *elem = (grpc_call_element *) arg;
  grpc_deadline_state *deadline_state =
    (grpc_deadline_state *) elem->call_data;
  if (error != GRPC_ERROR_CANCELLED)
	...
	grpc_error_set_int (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			    ("Deadline Exceeded"), GRPC_ERROR_INT_GRPC_STATUS,
...
// synchronized.
static void
start_timer_if_needed (grpc_call_element * elem, grpc_millis deadline)
{
...
// synchronized.
static void
cancel_timer_if_needed (grpc_deadline_state * deadline_state)
{
...
static void
inject_on_complete_cb (grpc_deadline_state * deadline_state,
		       grpc_transport_stream_op_batch * op)
{
  deadline_state->next_on_complete = op->on_complete;
  GRPC_CLOSURE_INIT (&deadline_state->on_complete, on_complete,
...
static void
start_timer_after_init (void *arg, grpc_error * error)
{
...
void
grpc_deadline_state_init (grpc_call_element * elem,
...
void
grpc_deadline_state_destroy (grpc_call_element * elem)
{
  grpc_deadline_state *deadline_state =
    (grpc_deadline_state *) elem->call_data;
  cancel_timer_if_needed (deadline_state);
...
void
grpc_deadline_state_reset (grpc_call_element * elem, grpc_millis new_deadline)
{
  grpc_deadline_state *deadline_state =
    (grpc_deadline_state *) elem->call_data;
  cancel_timer_if_needed (deadline_state);
...
static void
client_start_transport_stream_op_batch (grpc_call_element * elem,
					grpc_transport_stream_op_batch * op)
{
...
recv_initial_metadata_ready (void *arg, grpc_error * error)
{
  ...
  // Invoke the next callback.
  calld->next_recv_initial_metadata_ready->cb (calld->
					       next_recv_initial_metadata_ready->
					       cb_arg, error);
}
...
bool
grpc_deadline_checking_enabled (const grpc_channel_args * channel_args)
{
  return
    grpc_channel_arg_get_bool (grpc_channel_args_find
			       (channel_args,
...
static bool
maybe_add_deadline_filter (grpc_channel_stack_builder * builder, void *arg)
{
  return
    grpc_deadline_checking_enabled
    (grpc_channel_stack_builder_get_channel_arguments (builder)) ?
    grpc_channel_stack_builder_prepend_filter (builder,
					       (const grpc_channel_filter *)
					       arg, nullptr, nullptr) : true;
}

void
grpc_deadline_filter_init (void)
{
...
void
grpc_deadline_filter_shutdown (void)
{
}
