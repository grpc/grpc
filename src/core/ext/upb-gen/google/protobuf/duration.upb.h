...
extern "C" {
#endif
typedef struct google_protobuf_Duration {
...
UPB_INLINE google_protobuf_Duration* google_protobuf_Duration_parse(const char* buf, size_t size,
  ...
  if (!ret) return NULL;
  if (upb_Decode(buf, size, UPB_UPCAST(ret), &google__protobuf__Duration_msg_init, NULL, 0,
                 arena) != kUpb_DecodeStatus_Ok) {
    return NULL;
  }
  return ret;
}
UPB_INLINE google_protobuf_Duration* google_protobuf_Duration_parse_ex(
    const char* buf, size_t size, const upb_ExtensionRegistry* extreg,
    int options, upb_Arena* arena) {
  google_protobuf_Duration* ret = google_protobuf_Duration_new(arena);
  if (!ret) return NULL;
  if (upb_Decode(buf, size, UPB_UPCAST(ret), &google__protobuf__Duration_msg_init, extreg,
