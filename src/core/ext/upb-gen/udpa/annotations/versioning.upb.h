...
extern "C" {
#endif
typedef struct udpa_annotations_VersioningAnnotation {
...
UPB_INLINE udpa_annotations_VersioningAnnotation* udpa_annotations_VersioningAnnotation_parse(const char* buf, size_t size,
  ...
  if (!ret) return NULL;
  if (upb_Decode(buf, size, UPB_UPCAST(ret), &udpa__annotations__VersioningAnnotation_msg_init, NULL, 0,
                 arena) != kUpb_DecodeStatus_Ok) {
    return NULL;
  }
  return ret;
}
UPB_INLINE udpa_annotations_VersioningAnnotation* udpa_annotations_VersioningAnnotation_parse_ex(
    const char* buf, size_t size, const upb_ExtensionRegistry* extreg,
    int options, upb_Arena* arena) {
  udpa_annotations_VersioningAnnotation* ret = udpa_annotations_VersioningAnnotation_new(arena);
  if (!ret) return NULL;
  if (upb_Decode(buf, size, UPB_UPCAST(ret), &udpa__annotations__VersioningAnnotation_msg_init, extreg,
...
UPB_INLINE void udpa_annotations_set_versioning(struct google_protobuf_MessageOptions* msg,
                                        const udpa_annotations_VersioningAnnotation* val,
                                        upb_Arena* arena) {
  const upb_MiniTableExtension* ext = &udpa_annotations_versioning_ext;
  UPB_ASSUME(upb_MiniTableField_IsScalar(&ext->UPB_PRIVATE(field)));
  UPB_ASSUME(UPB_PRIVATE(_upb_MiniTableField_GetRep)(
...
UPB_INLINE struct udpa_annotations_VersioningAnnotation* udpa_annotations_mutable_versioning(
