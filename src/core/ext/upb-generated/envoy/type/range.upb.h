...
UPB_INLINE envoy_type_Int64Range *envoy_type_Int64Range_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_Int64Range_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_Int64Range *envoy_type_Int64Range_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_Int64Range *ret = envoy_type_Int64Range_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_Int64Range_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_Int32Range *envoy_type_Int32Range_new(upb_arena *arena) {
  return (envoy_type_Int32Range *)_upb_msg_new(&envoy_type_Int32Range_msginit, arena);
}
UPB_INLINE envoy_type_Int32Range *envoy_type_Int32Range_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_Int32Range_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_Int32Range *envoy_type_Int32Range_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_Int32Range *ret = envoy_type_Int32Range_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_Int32Range_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_DoubleRange *envoy_type_DoubleRange_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_DoubleRange_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_DoubleRange *envoy_type_DoubleRange_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_DoubleRange *ret = envoy_type_DoubleRange_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_DoubleRange_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_type_range_proto_upb_file_layout;
