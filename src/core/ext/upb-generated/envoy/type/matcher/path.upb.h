...
UPB_INLINE envoy_type_matcher_PathMatcher *envoy_type_matcher_PathMatcher_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_matcher_PathMatcher_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_matcher_PathMatcher *envoy_type_matcher_PathMatcher_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_matcher_PathMatcher *ret = envoy_type_matcher_PathMatcher_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_matcher_PathMatcher_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_type_matcher_path_proto_upb_file_layout;
