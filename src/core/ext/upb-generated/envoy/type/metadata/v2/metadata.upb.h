...
UPB_INLINE envoy_type_metadata_v2_MetadataKey *envoy_type_metadata_v2_MetadataKey_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKey_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKey *envoy_type_metadata_v2_MetadataKey_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKey *ret = envoy_type_metadata_v2_MetadataKey_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKey_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKey_PathSegment** envoy_type_metadata_v2_MetadataKey_resize_path(envoy_type_metadata_v2_MetadataKey *msg, size_t len, upb_arena *arena) {
  return (envoy_type_metadata_v2_MetadataKey_PathSegment**)_upb_array_resize_accessor2(msg, UPB_SIZE(8, 16), len, UPB_SIZE(2, 3), arena);
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKey_PathSegment *envoy_type_metadata_v2_MetadataKey_PathSegment_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKey_PathSegment_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKey_PathSegment *envoy_type_metadata_v2_MetadataKey_PathSegment_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKey_PathSegment *ret = envoy_type_metadata_v2_MetadataKey_PathSegment_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKey_PathSegment_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKind *envoy_type_metadata_v2_MetadataKind_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKind *envoy_type_metadata_v2_MetadataKind_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKind *ret = envoy_type_metadata_v2_MetadataKind_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Request *envoy_type_metadata_v2_MetadataKind_Request_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Request_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Request *envoy_type_metadata_v2_MetadataKind_Request_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKind_Request *ret = envoy_type_metadata_v2_MetadataKind_Request_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Request_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Route *envoy_type_metadata_v2_MetadataKind_Route_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Route_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Route *envoy_type_metadata_v2_MetadataKind_Route_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKind_Route *ret = envoy_type_metadata_v2_MetadataKind_Route_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Route_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Cluster *envoy_type_metadata_v2_MetadataKind_Cluster_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Cluster_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Cluster *envoy_type_metadata_v2_MetadataKind_Cluster_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKind_Cluster *ret = envoy_type_metadata_v2_MetadataKind_Cluster_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Cluster_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Host *envoy_type_metadata_v2_MetadataKind_Host_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Host_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_metadata_v2_MetadataKind_Host *envoy_type_metadata_v2_MetadataKind_Host_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_metadata_v2_MetadataKind_Host *ret = envoy_type_metadata_v2_MetadataKind_Host_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_metadata_v2_MetadataKind_Host_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_type_metadata_v2_metadata_proto_upb_file_layout;
