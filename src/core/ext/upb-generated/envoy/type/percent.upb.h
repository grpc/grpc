...
UPB_INLINE envoy_type_Percent *envoy_type_Percent_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_Percent_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_Percent *envoy_type_Percent_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_Percent *ret = envoy_type_Percent_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_Percent_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE double envoy_type_Percent_value(const envoy_type_Percent *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(0, 0), double); }
...
UPB_INLINE envoy_type_FractionalPercent *envoy_type_FractionalPercent_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_type_FractionalPercent_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_type_FractionalPercent *envoy_type_FractionalPercent_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_type_FractionalPercent *ret = envoy_type_FractionalPercent_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_type_FractionalPercent_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE uint32_t envoy_type_FractionalPercent_numerator(const envoy_type_FractionalPercent *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(4, 4), uint32_t); }
UPB_INLINE int32_t envoy_type_FractionalPercent_denominator(const envoy_type_FractionalPercent *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(0, 0), int32_t); }
...
extern const upb_msglayout_file envoy_type_percent_proto_upb_file_layout;
