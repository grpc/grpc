...
UPB_INLINE envoy_service_discovery_v2_AdsDummy *envoy_service_discovery_v2_AdsDummy_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_service_discovery_v2_AdsDummy_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_service_discovery_v2_AdsDummy *envoy_service_discovery_v2_AdsDummy_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_service_discovery_v2_AdsDummy *ret = envoy_service_discovery_v2_AdsDummy_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_service_discovery_v2_AdsDummy_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_service_discovery_v2_ads_proto_upb_file_layout;
