...
UPB_INLINE envoy_config_listener_v2_ApiListener *envoy_config_listener_v2_ApiListener_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_config_listener_v2_ApiListener_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_config_listener_v2_ApiListener *envoy_config_listener_v2_ApiListener_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_config_listener_v2_ApiListener *ret = envoy_config_listener_v2_ApiListener_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_config_listener_v2_ApiListener_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE bool envoy_config_listener_v2_ApiListener_has_api_listener(const envoy_config_listener_v2_ApiListener *msg) { return _upb_hasbit(msg, 1); }
UPB_INLINE const struct google_protobuf_Any* envoy_config_listener_v2_ApiListener_api_listener(const envoy_config_listener_v2_ApiListener *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(4, 8), const struct google_protobuf_Any*); }
...
extern const upb_msglayout_file envoy_config_listener_v2_api_listener_proto_upb_file_layout;
