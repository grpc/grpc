...
UPB_INLINE envoy_config_trace_v2_Tracing *envoy_config_trace_v2_Tracing_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_config_trace_v2_Tracing_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_config_trace_v2_Tracing *envoy_config_trace_v2_Tracing_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_config_trace_v2_Tracing *ret = envoy_config_trace_v2_Tracing_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_config_trace_v2_Tracing_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE bool envoy_config_trace_v2_Tracing_has_http(const envoy_config_trace_v2_Tracing *msg) { return _upb_hasbit(msg, 1); }
UPB_INLINE const envoy_config_trace_v2_Tracing_Http* envoy_config_trace_v2_Tracing_http(const envoy_config_trace_v2_Tracing *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(4, 8), const envoy_config_trace_v2_Tracing_Http*); }
...
UPB_INLINE void envoy_config_trace_v2_Tracing_set_http(envoy_config_trace_v2_Tracing *msg, envoy_config_trace_v2_Tracing_Http* value) {
  _upb_sethas(msg, 1);
  *UPB_PTR_AT(msg, UPB_SIZE(4, 8), envoy_config_trace_v2_Tracing_Http*) = value;
}
...
UPB_INLINE envoy_config_trace_v2_Tracing_Http *envoy_config_trace_v2_Tracing_Http_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_config_trace_v2_Tracing_Http_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_config_trace_v2_Tracing_Http *envoy_config_trace_v2_Tracing_Http_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_config_trace_v2_Tracing_Http *ret = envoy_config_trace_v2_Tracing_Http_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_config_trace_v2_Tracing_Http_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_config_trace_v2_http_tracer_proto_upb_file_layout;
