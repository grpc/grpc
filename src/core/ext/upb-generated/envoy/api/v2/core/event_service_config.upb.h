...
UPB_INLINE envoy_api_v2_core_EventServiceConfig *envoy_api_v2_core_EventServiceConfig_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_api_v2_core_EventServiceConfig_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_api_v2_core_EventServiceConfig *envoy_api_v2_core_EventServiceConfig_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_api_v2_core_EventServiceConfig *ret = envoy_api_v2_core_EventServiceConfig_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_api_v2_core_EventServiceConfig_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_api_v2_core_event_service_config_proto_upb_file_layout;
