/* This file was generated by upbc (the upb compiler) from the input
 * file:
 *
 *     envoy/api/v2/endpoint.proto
 *
 * Do not edit -- your changes will be discarded when the file is
 * regenerated. */

#include <stddef.h>
#include "upb/msg_internal.h"
#include "envoy/api/v2/endpoint.upb.h"
#include "envoy/api/v2/endpoint/endpoint_components.upb.h"
#include "envoy/type/percent.upb.h"
#include "google/protobuf/duration.upb.h"
#include "google/protobuf/wrappers.upb.h"
#include "udpa/annotations/migrate.upb.h"
#include "udpa/annotations/status.upb.h"
#include "validate/validate.upb.h"

#include "upb/port_def.inc"

static const upb_msglayout_sub envoy_api_v2_ClusterLoadAssignment_submsgs[3] = {
  {.submsg = &envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry_msginit},
  {.submsg = &envoy_api_v2_ClusterLoadAssignment_Policy_msginit},
  {.submsg = &envoy_api_v2_endpoint_LocalityLbEndpoints_msginit},
};

static const upb_msglayout_field envoy_api_v2_ClusterLoadAssignment__fields[4] = {
  {1, UPB_SIZE(4, 8), 0, 0, 9, _UPB_MODE_SCALAR | (_UPB_REP_STRVIEW << _UPB_REP_SHIFT)},
  {2, UPB_SIZE(16, 32), 0, 2, 11, _UPB_MODE_ARRAY | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
  {4, UPB_SIZE(12, 24), 1, 1, 11, _UPB_MODE_SCALAR | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
  {5, UPB_SIZE(20, 40), 0, 0, 11, _UPB_MODE_MAP | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
};

const upb_msglayout envoy_api_v2_ClusterLoadAssignment_msginit = {
  &envoy_api_v2_ClusterLoadAssignment_submsgs[0],
  &envoy_api_v2_ClusterLoadAssignment__fields[0],
  UPB_SIZE(24, 48), 4, _UPB_MSGEXT_NONE, 2, 255,
};

static const upb_msglayout_sub envoy_api_v2_ClusterLoadAssignment_Policy_submsgs[3] = {
  {.submsg = &envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload_msginit},
  {.submsg = &google_protobuf_Duration_msginit},
  {.submsg = &google_protobuf_UInt32Value_msginit},
};

static const upb_msglayout_field envoy_api_v2_ClusterLoadAssignment_Policy__fields[4] = {
  {2, UPB_SIZE(12, 24), 0, 0, 11, _UPB_MODE_ARRAY | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
  {3, UPB_SIZE(4, 8), 1, 2, 11, _UPB_MODE_SCALAR | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
  {4, UPB_SIZE(8, 16), 2, 1, 11, _UPB_MODE_SCALAR | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
  {5, UPB_SIZE(1, 1), 0, 0, 8, _UPB_MODE_SCALAR | (_UPB_REP_1BYTE << _UPB_REP_SHIFT)},
};

const upb_msglayout envoy_api_v2_ClusterLoadAssignment_Policy_msginit = {
  &envoy_api_v2_ClusterLoadAssignment_Policy_submsgs[0],
  &envoy_api_v2_ClusterLoadAssignment_Policy__fields[0],
  UPB_SIZE(16, 32), 4, _UPB_MSGEXT_NONE, 0, 255,
};

static const upb_msglayout_sub envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload_submsgs[1] = {
  {.submsg = &envoy_type_FractionalPercent_msginit},
};

static const upb_msglayout_field envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload__fields[2] = {
  {1, UPB_SIZE(4, 8), 0, 0, 9, _UPB_MODE_SCALAR | (_UPB_REP_STRVIEW << _UPB_REP_SHIFT)},
  {2, UPB_SIZE(12, 24), 1, 0, 11, _UPB_MODE_SCALAR | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
};

const upb_msglayout envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload_msginit = {
  &envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload_submsgs[0],
  &envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload__fields[0],
  UPB_SIZE(16, 32), 2, _UPB_MSGEXT_NONE, 2, 255,
};

static const upb_msglayout_sub envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry_submsgs[1] = {
  {.submsg = &envoy_api_v2_endpoint_Endpoint_msginit},
};

static const upb_msglayout_field envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry__fields[2] = {
  {1, UPB_SIZE(0, 0), 0, 0, 9, _UPB_MODE_SCALAR | (_UPB_REP_STRVIEW << _UPB_REP_SHIFT)},
  {2, UPB_SIZE(8, 16), 0, 0, 11, _UPB_MODE_SCALAR | (_UPB_REP_PTR << _UPB_REP_SHIFT)},
};

const upb_msglayout envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry_msginit = {
  &envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry_submsgs[0],
  &envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry__fields[0],
  UPB_SIZE(16, 32), 2, _UPB_MSGEXT_NONE, 2, 255,
};

static const upb_msglayout *messages_layout[4] = {
  &envoy_api_v2_ClusterLoadAssignment_msginit,
  &envoy_api_v2_ClusterLoadAssignment_Policy_msginit,
  &envoy_api_v2_ClusterLoadAssignment_Policy_DropOverload_msginit,
  &envoy_api_v2_ClusterLoadAssignment_NamedEndpointsEntry_msginit,
};

const upb_msglayout_file envoy_api_v2_endpoint_proto_upb_file_layout = {
  messages_layout,
  NULL,
  4,
  0,
};

#include "upb/port_undef.inc"

