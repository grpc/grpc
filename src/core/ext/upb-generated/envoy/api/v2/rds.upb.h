...
UPB_INLINE envoy_api_v2_RdsDummy *envoy_api_v2_RdsDummy_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_api_v2_RdsDummy_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_api_v2_RdsDummy *envoy_api_v2_RdsDummy_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_api_v2_RdsDummy *ret = envoy_api_v2_RdsDummy_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_api_v2_RdsDummy_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
extern const upb_msglayout_file envoy_api_v2_rds_proto_upb_file_layout;
