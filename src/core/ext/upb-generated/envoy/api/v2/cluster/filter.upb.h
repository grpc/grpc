...
UPB_INLINE envoy_api_v2_cluster_Filter *envoy_api_v2_cluster_Filter_parse(const char *buf, size_t size,
  ...
  if (!ret) return NULL;
  if (!upb_decode(buf, size, ret, &envoy_api_v2_cluster_Filter_msginit, arena)) return NULL;
  return ret;
}
UPB_INLINE envoy_api_v2_cluster_Filter *envoy_api_v2_cluster_Filter_parse_ex(const char *buf, size_t size,
                           const upb_extreg *extreg, int options,
                           upb_arena *arena) {
  envoy_api_v2_cluster_Filter *ret = envoy_api_v2_cluster_Filter_new(arena);
  if (!ret) return NULL;
  if (!_upb_decode(buf, size, ret, &envoy_api_v2_cluster_Filter_msginit, extreg, options, arena)) {
    return NULL;
  }
  return ret;
}
...
UPB_INLINE upb_strview envoy_api_v2_cluster_Filter_name(const envoy_api_v2_cluster_Filter *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(4, 8), upb_strview); }
UPB_INLINE bool envoy_api_v2_cluster_Filter_has_typed_config(const envoy_api_v2_cluster_Filter *msg) { return _upb_hasbit(msg, 1); }
UPB_INLINE const struct google_protobuf_Any* envoy_api_v2_cluster_Filter_typed_config(const envoy_api_v2_cluster_Filter *msg) { return *UPB_PTR_AT(msg, UPB_SIZE(12, 24), const struct google_protobuf_Any*); }
...
extern const upb_msglayout_file envoy_api_v2_cluster_filter_proto_upb_file_layout;
