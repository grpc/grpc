...
GRPCAPI grpc_channel* grpc_cronet_secure_channel_create(void* engine, const char* target,
                                                        ...
                                                        void* reserved) {
  gpr_log(GPR_DEBUG, "grpc_create_cronet_transport: stream_engine = %p, target=%s", engine, target);
  ...
  grpc_channel_args* new_args =
      grpc_channel_args_copy_and_add(args, &disable_client_authority_filter_arg, 1);

  grpc_transport* ct = grpc_create_cronet_transport(engine, target, new_args, reserved);

  grpc_core::ExecCtx exec_ctx;
  grpc_channel* channel = grpc_channel_create(target, new_args, GRPC_CLIENT_DIRECT_CHANNEL, ct);
