  ...
  if (s->state.pending_recv_trailing_metadata && s->state.fail_state) {
    if (!s->state.flush_read && !s->state.rs.read_stream_closed) {
      ...
      s->state.rs.read_buffer =
          reinterpret_cast<char*>(gpr_malloc(GRPC_FLUSH_READ_SIZE));
...
static void add_to_storage(struct stream_obj* s,
  ...
  struct op_and_state* new_op = reinterpret_cast<struct op_and_state*>(
...
static void on_failed(bidirectional_stream* stream, int net_error) {
  ...
  stream_obj* s = reinterpret_cast<stream_obj*>(stream->annotation);
  gpr_mu_lock(&s->mu);
...
static void create_grpc_frame(grpc_exec_ctx* exec_ctx,
  ...
  *p_write_buffer_size = length + GRPC_HEADER_SIZE_IN_BYTES;
  ...
  char* write_buffer =
      reinterpret_cast<char*>(gpr_malloc(length + GRPC_HEADER_SIZE_IN_BYTES));
...
static void convert_metadata_to_cronet_headers(
  ...
  bidirectional_stream_header* headers =
      reinterpret_cast<bidirectional_stream_header*>(gpr_malloc(
...
static void parse_grpc_header(const uint8_t* data, int* length,
  ...
  *length = 0;
  *length |= (static_cast<uint8_t>(*p++)) << 24;
  *length |= (static_cast<uint8_t>(*p++)) << 16;
  *length |= (static_cast<uint8_t>(*p++)) << 8;
  *length |= (static_cast<uint8_t>(*p++));
}
    ...
    if (stream_state->state_op_done[OP_CANCEL_ERROR]) {
      ...
      if (stream_state->rs.received_bytes == GRPC_HEADER_SIZE_IN_BYTES &&
        ...
        if (stream_state->rs.length_field > 0) {
          stream_state->rs.read_buffer = reinterpret_cast<char*>(
...
grpc_transport* grpc_create_cronet_transport(void* engine, const char* target,
                                             ...
                                             void* reserved) {
  grpc_cronet_transport* ct = reinterpret_cast<grpc_cronet_transport*>(
      gpr_malloc(sizeof(grpc_cronet_transport)));
  if (!ct) {
    goto error;
  }
  ct->base.vtable = &grpc_cronet_vtable;
  ct->engine = reinterpret_cast<stream_engine*>(engine);
  ct->host = reinterpret_cast<char*>(gpr_malloc(strlen(target) + 1));
  if (!ct->host) {
    goto error;
  }
