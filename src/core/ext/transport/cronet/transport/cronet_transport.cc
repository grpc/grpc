...
void
grpc_cronet_stream_ref (stream_obj * s, const char *reason)
{
  grpc_stream_ref (s->refcount, reason);
}

void
grpc_cronet_stream_unref (stream_obj * s, const char *reason)
{
...
void
grpc_cronet_stream_ref (stream_obj * s)
{
  grpc_stream_ref (s->refcount);
}

void
grpc_cronet_stream_unref (stream_obj * s)
{
...
*/
static const char *
op_result_string (enum e_op_result i)
{
  switch (i)
    {
...
static void
null_and_maybe_free_read_buffer (stream_obj * s)
{
  ...
  if (s->state.pending_recv_trailing_metadata && s->state.fail_state)
      ...
      if (!s->state.flush_read && !s->state.rs.read_stream_closed)
	{
	  ...
	  s->state.rs.read_buffer =
	    (char *) gpr_malloc (GRPC_FLUSH_READ_SIZE);
...
static grpc_error *
make_error_with_desc (int error_code, const char *desc)
{
...
*/
static void
add_to_storage (struct stream_obj *s, grpc_transport_stream_op_batch * op)
{
...
*/
static void
remove_from_storage (struct stream_obj *s, struct op_and_state *oas)
{
...
*/
static void
execute_from_storage (stream_obj * s)
{
  gpr_mu_lock (&s->mu);
...
*/
static void
on_failed (bidirectional_stream * stream, int net_error)
{
...
*/
static void
on_canceled (bidirectional_stream * stream)
{
...
*/
static void
on_succeeded (bidirectional_stream * stream)
{
...
*/
static void
on_stream_ready (bidirectional_stream * stream)
{
...
*/
static void
on_response_headers_received (bidirectional_stream * stream,
			      const bidirectional_stream_header_array *
			      headers, const char *negotiated_protocol)
{
  grpc_core::ExecCtx exec_ctx;
  ...
  for (size_t i = 0; i < headers->count; i++)
    {
      GRPC_LOG_IF_ERROR ("on_response_headers_received",
			 grpc_chttp2_incoming_metadata_buffer_add (&s->state.
								   rs.
								   initial_metadata,
...
*/
static void
on_write_completed (bidirectional_stream * stream, const char *data)
{
  grpc_core::ExecCtx exec_ctx;
  stream_obj *s = (stream_obj *) stream->annotation;
...
*/
static void
on_read_completed (bidirectional_stream * stream, char *data, int count)
{
  grpc_core::ExecCtx exec_ctx;
  stream_obj *s = (stream_obj *) stream->annotation;
...
*/
static void
on_response_trailers_received (bidirectional_stream * stream,
			       const bidirectional_stream_header_array *
			       trailers)
{
  grpc_core::ExecCtx exec_ctx;
  ...
  for (size_t i = 0; i < trailers->count; i++)
    {
      CRONET_LOG (GPR_DEBUG, "trailer key=%s, value=%s",
...
*/
static void
create_grpc_frame (grpc_slice_buffer * write_slice_buffer,
		   ...
		   size_t * p_write_buffer_size, uint32_t flags)
{
  ...
  *p_write_buffer_size = length + GRPC_HEADER_SIZE_IN_BYTES;
  ...
  char *write_buffer =
    (char *) gpr_malloc (length + GRPC_HEADER_SIZE_IN_BYTES);
...
*/
static void
convert_metadata_to_cronet_headers (grpc_linked_mdelem * head,
				    const char *host, char **pp_url,
...
static void
parse_grpc_header (const uint8_t * data, int *length, bool * compressed)
{
...
static bool
header_has_authority (grpc_linked_mdelem * head)
{
  while (head != nullptr)
    {
...
*/
static bool
op_can_be_run (grpc_transport_stream_op_batch * curr_op,
...
*/
static enum e_op_result
execute_stream_op (struct op_and_state *oas)
{
  ...
  if (stream_op->send_initial_metadata &&
      op_can_be_run (stream_op, s, &oas->state, OP_SEND_INITIAL_METADATA))
    {
      ...
      s->cbs =
	bidirectional_stream_create (t->engine, s->curr_gs,
				     &cronet_callbacks);
      ...
      if (t->use_packet_coalescing)
	{
      ...
      s->header_array.headers = nullptr;
      convert_metadata_to_cronet_headers (stream_op->payload->
					  send_initial_metadata.
					  send_initial_metadata->list.head,
					  t->host, &url,
					  &s->header_array.headers,
      ...
      else
	{
	  ...
	  if (1 !=
	      grpc_byte_stream_next (stream_op->payload->send_message.
				     send_message,
				     stream_op->payload->send_message.
				     send_message->length, nullptr))
	    {
	      /* Should never reach here */
	      GPR_ASSERT (false);
	    }
	  if (GRPC_ERROR_NONE !=
	      grpc_byte_stream_pull (stream_op->payload->send_message.
				     send_message, &slice))
	    {
	      /* Should never reach here */
	      GPR_ASSERT (false);
	  ...
	  if (write_slice_buffer.count > 0)
	    {
	      ...
	      create_grpc_frame (&write_slice_buffer,
	      ...
	      bidirectional_stream_write (s->cbs,
					  stream_state->ws.write_buffer,
		  ...
		  if (!stream_op->send_trailing_metadata)
		    {
		      CRONET_LOG (GPR_DEBUG,
				  "bidirectional_stream_flush (%p)", s->cbs);
		      bidirectional_stream_flush (s->cbs);
		      result = ACTION_TAKEN_WITH_CALLBACK;
		    }
		  else
			  ...
			  OP_RECV_INITIAL_METADATA))
    {
      ...
      if (stream_state->state_op_done[OP_CANCEL_ERROR])
	{
	  GRPC_CLOSURE_SCHED (stream_op->payload->recv_initial_metadata.
			      recv_initial_metadata_ready, GRPC_ERROR_NONE);
	}
	...
	{
	  grpc_chttp2_incoming_metadata_buffer_publish (&oas->s->state.rs.
							initial_metadata,
							stream_op->payload->
      ...
      if (stream_state->state_op_done[OP_CANCEL_ERROR])
	{
	  ...
	  GRPC_CLOSURE_SCHED (stream_op->payload->recv_message.
      ...
      else if (stream_state->rs.length_field_received == false)
	{
	  if (stream_state->rs.received_bytes == GRPC_HEADER_SIZE_IN_BYTES &&
	      stream_state->rs.remaining_bytes == 0)
	    {
	      ...
	      parse_grpc_header ((const uint8_t *) stream_state->rs.
				 read_buffer, &stream_state->rs.length_field,
	      ...
	      if (stream_state->rs.length_field > 0)
		{
		  stream_state->rs.read_buffer =
		    (char *) gpr_malloc ((size_t) stream_state->rs.
					 length_field);
		  ...
		  CRONET_LOG (GPR_DEBUG, "bidirectional_stream_read(%p)",
			      s->cbs);
		  stream_state->state_op_done[OP_READ_REQ_MADE] = true;	/* Indicates that at least one read request has been made */
		  bidirectional_stream_read (s->cbs,
					     stream_state->rs.read_buffer,
					     stream_state->rs.
					     remaining_bytes);
	      ...
	      else
		{
		  ...
		  grpc_slice_buffer_destroy_internal (&stream_state->rs.
						      read_slice_buffer);
		  grpc_slice_buffer_init (&stream_state->rs.
					  read_slice_buffer);
		  ...
		  if (stream_state->rs.compressed)
		    {
		      stream_state->rs.sbs.base.flags |=
			GRPC_WRITE_INTERNAL_COMPRESS;
		    }
		  *((grpc_byte_buffer **) stream_op->payload->recv_message.
		    recv_message) =
      ...
      else if (stream_state->rs.remaining_bytes == 0)
	{
	  ...
	  *((grpc_byte_buffer **) stream_op->payload->recv_message.
	    recv_message) = (grpc_byte_buffer *) & stream_state->rs.sbs;
	  GRPC_CLOSURE_SCHED (stream_op->payload->recv_message.
			      recv_message_ready, GRPC_ERROR_NONE);
	  stream_state->state_op_done[OP_RECV_MESSAGE] = true;
	  oas->state.state_op_done[OP_RECV_MESSAGE] = true;
	     ...
	     gpr_arena * arena)
{
  ...
  s->state.flush_cronet_when_ready = s->state.pending_write_for_trailer =
    false;
...
static void
set_pollset_do_nothing (grpc_transport * gt, grpc_stream * gs,
			grpc_pollset * pollset)
{
}

static void
set_pollset_set_do_nothing (grpc_transport * gt, grpc_stream * gs,
...
static void
perform_op (grpc_transport * gt, grpc_transport_op * op)
{
}
...
grpc_transport *
grpc_create_cronet_transport (void *engine, const char *target,
