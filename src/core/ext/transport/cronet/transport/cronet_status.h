...
enum cronet_net_error_code {
  ...
  // An asynchronous IO operation is not yet complete.  This usually
  // does not
  // indicate a fatal error.  Typically this error will be generated as
  // a
  ...
  // An unexpected error.  This may be caused by a programming mistake
  // or an
  // invalid assumption.
  CRONET_NET_ERROR_UNEXPECTED = -9,

  // Permission to access a resource = other than the network = was
  // denied.
  ...
  // The file upload failed because the file's modification time was
  // different
  ...
  // The request was blocked by the URL block list configured by the
  // domain
  // administrator.
  ...
  // The request was blocked because the forced reenrollment check is
  // still
  ...
  // The upload failed because the upload stream needed to be re-read =
  // due to a
  // retry or a redirect = but the upload stream doesn't support that
  // operation.
  CRONET_NET_ERROR_UPLOAD_STREAM_REWIND_NOT_SUPPORTED = -25,

  // The request failed because the URLRequestContext is shutting down =
  // or has
  // been shut down.
  ...
  // requirements
  // which are not met ('X-Frame-Options' and 'Content-Security-Policy'
  // ancestor
  ...
  // The request was blocked by system policy disallowing some or all
  // cleartext
  ...
  // The request was blocked because it is a private network request
  // coming from
  // an insecure context in a less private IP address space. This is
  // used to
  ...
  // A connection timed out as a result of not receiving an ACK for data
  // sent.
  ...
  // The IP address or port number is invalid (e.g. = cannot connect to
  // the IP
  // address 0 or the port 0,.
  CRONET_NET_ERROR_ADDRESS_INVALID = -108,

  // The IP address is unreachable.  This usually means that there is no
  // route
  // to
  ...
  // The client and server don't support a common SSL protocol version
  // or
  // cipher suite.
  ...
  // The proxy requested authentication (for tunnel establishment, with
  // an
  // unsupported method.
  ...
  // an error.
  //
  // Note: this error is not in the -2xx range so that it won't be
  // handled as a
  // certificate error.
  ...
  // There are too many pending DNS resolves = so a request in the queue
  // was
  // aborted.
  CRONET_NET_ERROR_HOST_RESOLVER_QUEUE_TOO_LARGE = -119,

  // Failed establishing a connection to the SOCKS proxy server for a
  // target
  // host.
  CRONET_NET_ERROR_SOCKS_CONNECTION_FAILED = -120,

  // The SOCKS proxy server failed establishing connection to the target
  // host
  ...
  // Winsock sometimes reports more data written than passed.  This is
  // probably
  // due to a broken LSP.
  CRONET_NET_ERROR_WINSOCK_UNEXPECTED_WRITTEN_BYTES = -124,

  // An SSL peer sent us a fatal decompression_failure alert. This
  // typically
  // occurs when a peer selects DEFLATE compression in the mistaken
  // belief that
  // it supports it.
  CRONET_NET_ERROR_SSL_DECOMPRESSION_FAILURE_ALERT = -125,

  // An SSL peer sent us a fatal bad_record_mac alert. This has been
  // observed
  ...
  // Could not create a connection to the proxy server. An error
  // occurred
  // either in resolving its name = or in connecting a socket to it.
  // Note that this does NOT include failures during the actual
  // "CONNECT" method
  // of an HTTP proxy.
  CRONET_NET_ERROR_PROXY_CONNECTION_FAILED = -130,

  // A mandatory proxy configuration could not be used. Currently this
  // means
  // that a mandatory PAC script could not be fetched = parsed or
  // executed.
  CRONET_NET_ERROR_MANDATORY_PROXY_CONFIGURATION_FAILED = -131,

  // -132 was formerly ERR_ESET_ANTI_VIRUS_SSL_INTERCEPTION

  // We've hit the max socket limit for the socket pool while
  // preconnecting.  We
  // don't bother trying to preconnect more sockets.
  CRONET_NET_ERROR_PRECONNECT_MAX_SOCKET_LIMIT = -133,

  // The permission to use the SSL client certificate's private key was
  // denied.
  ...
  // Permission to access the network was denied. This is used to
  // distinguish
  // errors that were most likely caused by a firewall from other access
  // denied
  ...
  // We were unable to sign the CertificateVerify data of an SSL client
  // auth
  // handshake with the client certificate's private key.
  //
  // Possible causes for this include the user implicitly or explicitly
  // denying access to the private key = the private key may not be
  // valid for
  // signing = the key may be relying on a cached handle which is no
  // longer
  // valid = or the CSP won't allow arbitrary data to be signed.
  CRONET_NET_ERROR_SSL_CLIENT_AUTH_SIGNATURE_FAILED = -141,

  // The message was too large for the transport.  (for example a UDP
  // message
  ...
  // Websocket protocol error. Indicates that we are terminating the
  // connection
  ...
  // The certificate didn't match the built-in public key pins for the
  // host
  // name.
  // The pins are set in net/http/transport_security_state.cc and
  // require that
  // one of a set of public keys exist on the path from the leaf to the
  // root.
  ...
  // An SSL peer sent us a fatal decrypt_error alert. This typically
  // occurs when
  // a peer could not correctly verify a signature (in CertificateVerify
  // or
  // ServerKeyExchange, or validate a Finished message.
  CRONET_NET_ERROR_SSL_DECRYPT_ERROR_ALERT = -153,

  // There are too many pending WebSocketJob instances = so the new job
  // was not
  // pushed to the queue.
  ...
  // Failed to set the socket's send buffer size as requested = despite
  // success
  // return code from setsockopt.
  CRONET_NET_ERROR_SOCKET_SEND_BUFFER_SIZE_UNCHANGEABLE = -163,

  // Failed to import a client certificate from the platform store into
  // the SSL
  // library.
  ...
  // Resolving a hostname to an IP address list included the IPv4
  // address
  // "127.0.53.53". This is a special IP address which ICANN has
  // recommended to
  // indicate there was a name collision = and alert admins to a
  // potential
  // problem.
  ...
  // This is
  // not a certificate error code as no X509Certificate object is
  // available.
  // This
  // error is fatal.
  CRONET_NET_ERROR_SSL_SERVER_CERT_BAD_FORMAT = -167,

  // Certificate Transparency: Received a signed tree head that failed
  // to parse.
  CRONET_NET_ERROR_CT_STH_PARSING_FAILED = -168,

  // Certificate Transparency: Received a signed tree head whose JSON
  // parsing
  // was
  ...
  // failed
  // before the AuthController was used to generate credentials. The
  // caller
  // should
  ...
  // Certificate Transparency: Failed to parse the received consistency
  // proof.
  CRONET_NET_ERROR_CT_CONSISTENCY_PROOF_PARSING_FAILED = -171,

  // The SSL server required an unsupported cipher suite that has since
  // been
  // removed. This error will temporarily be signaled on a fallback for
  // one or
  // two
  // releases immediately following a cipher suite's removal = after
  // which the
  // fallback will be removed.
  CRONET_NET_ERROR_SSL_OBSOLETE_CIPHER = -172,

  // When a WebSocket handshake is done successfully and the connection
  // has been
  // upgraded = the URLRequest is cancelled with this error code.
  CRONET_NET_ERROR_WS_UPGRADE = -173,

  // Socket ReadIfReady support is not implemented. This error should
  // not be
  // user
  ...
  // There were no common signature algorithms between our client
  // certificate
  // private key and the server's preferences.
  CRONET_NET_ERROR_SSL_CLIENT_AUTH_NO_COMMON_ALGORITHMS = -177,

  // TLS 1.3 early data was rejected by the server. This will be
  // received before
  // any data is returned from the socket. The request should be retried
  // with
  // early data disabled.
  ...
  // be
  // received before any data is returned from the socket. The request
  // should be
  ...
  // TLS 1.3 was enabled = but a lower version was negotiated and the
  // server
  // returned a value indicating it supported TLS 1.3. This is part of a
  // security
  // check in TLS 1.3 = but it may also indicate the user is behind a
  // buggy
  // TLS-terminating proxy which implemented TLS 1.2 incorrectly. (See
  // https:  //crbug.com/boringssl/226.,
  CRONET_NET_ERROR_TLS13_DOWNGRADE_DETECTED = -180,

  // The server's certificate has a keyUsage extension incompatible with
  // the
  ...
  // The server responded with a certificate whose common name did not
  // match
  ...
  // The server responded with a certificate that = by our clock =
  // appears to
  ...
  // The server responded with a certificate that is signed by an
  // authority
  ...
  //
  // 3. The server is presenting a self-signed certificate = providing
  // no
  //    defense against active attackers (but foiling passive
  //    attackers,.
  //
  ...
  // We have the capability to ignore this error = but it is probably
  // not the
  // thing to do.
  ...
  // Certificate Transparency was required for this connection = but the
  // server
  ...
  // -216 was QUIC_CERT_ROOT_NOT_KNOWN which has been renumbered to not
  // be in
  // the
  // certificate error range.

  // The certificate is known to be used for interception by an entity
  // other
  // the device owner.
  ...
  //
  // Update the value of CERT_END whenever you add a new certificate
  // error
  // code.
  ...
  // Attempting to load an URL resulted in an unsafe redirect (e.g. = a
  // redirect
  // to file:  // is considered unsafe,.
  CRONET_NET_ERROR_UNSAFE_REDIRECT = -311,

  // Attempting to load an URL with an unsafe port number.  These are
  // port
  // numbers that correspond to services = which are not robust to
  // spurious
  // input
  // that may be constructed as a result of an allowed web construct
  // (e.g. =
  // HTTP
  ...
  // The response was 407 (Proxy Authentication Required, = yet we did
  // not send
  ...
  // The response was 416 (Requested range not satisfiable, and the
  // server
  // cannot
  ...
  // The server sent an FTP directory listing in a format we do not
  // understand.
  CRONET_NET_ERROR_UNRECOGNIZED_FTP_DIRECTORY_LISTING_FORMAT = -334,

  // Obsolete.  Was only logged in NetLog when an HTTP/2 pushed stream
  // expired.
  ...
  // An HTTP Authentication scheme was tried which is not supported on
  // this
  // machine.
  ...
  // An unexpected = but documented = SSPI or GSSAPI status code was
  // returned.
  ...
  // The HTTP response contained multiple distinct Content-Length
  // headers.
  CRONET_NET_ERROR_RESPONSE_HEADERS_MULTIPLE_CONTENT_LENGTH = -346,

  // HTTP/2 headers have been received = but not all of them - status or
  // version
  // headers are missing = so we're expecting additional frames to
  // complete
  // them.
  CRONET_NET_ERROR_INCOMPLETE_HTTP2_HEADERS = -347,

  // No PAC URL configuration could be retrieved from DHCP. This can
  // indicate
  ...
  // HTTP/2 server refused the request without processing = and sent
  // either a
  // GOAWAY frame with error code NO_ERROR and Last-Stream-ID lower than
  // the
  // stream id corresponding to the request indicating that this request
  // has not
  // been processed yet = or a RST_STREAM frame with error code
  // REFUSED_STREAM.
  // Client MAY retry (on a different connection,.  See RFC7540
  // Section 8.1.4.
  ...
  // The HTTP response body transferred fewer bytes than were advertised
  // by the
  ...
  // The QUIC crypto handshake failed.  This means that the server was
  // unable
  ...
  // appear on
  // histograms.
  //   CRONET_NET_ERROR_REQUEST_FOR_SECURE_RESOURCE_OVER_INSECURE_QUIC =
  //   -359,
  ...
  // The server was expected to return an HTTP/1.x response = but did
  // not.
  // Rather
  ...
  // Received HTTP/2 RST_STREAM frame with NO_ERROR error code.  This
  // error
  // should
  // be handled internally by HTTP/2 code = and should not make it above
  // the
  // SpdyStream layer.
  ...
  // A pushed stream was claimed and later reset by the server. When
  // this
  // happens =
  // the request should be retried.
  CRONET_NET_ERROR_HTTP2_CLAIMED_PUSHED_STREAM_RESET_BY_SERVER = -374,

  // An HTTP transaction was retried too many times due for
  // authentication or
  // invalid certificates. This may be due to a bug in the net stack
  // that would
  // otherwise infinite loop = or if the server or proxy continually
  // requests
  // fresh
  ...
  // A pushed HTTP/2 stream was claimed by a request based on matching
  // URL and
  // request headers = but the pushed response headers do not match the
  // request.
  CRONET_NET_ERROR_HTTP2_PUSHED_RESPONSE_DOES_NOT_MATCH = -378,

  // The server returned a non-2xx HTTP response code.
  //
  // Not that this error is only used by certain APIs that interpret the
  // HTTP
  ...
  // The certificate presented on a QUIC connection does not chain to a
  // known
  // root
  // and the origin connected to is not on a list of domains where
  // unknown roots
  // are allowed.
  ...
  // Multiple transactions are racing to create disk cache entries. This
  // is an
  // internal error returned from the HttpCache to the
  // HttpCacheTransaction that
  // tells the transaction to restart the entry-creation logic because
  // the state
  // of the cache has changed.
  CRONET_NET_ERROR_CACHE_RACE = -406,

  // The cache was unable to read a checksum record on an entry. This
  // can be
  // returned from attempts to read from the cache. It is an internal
  // error =
  // returned by the SimpleCache backend = but not by any URLRequest
  // methods
  // or members.
  CRONET_NET_ERROR_CACHE_CHECKSUM_READ_FAILURE = -407,

  // The cache found an entry with an invalid checksum. This can be
  // returned
  // from
  // attempts to read from the cache. It is an internal error = returned
  // by the
  // SimpleCache backend = but not by any URLRequest methods or members.
  CRONET_NET_ERROR_CACHE_CHECKSUM_MISMATCH = -408,

  // Internal error code for the HTTP cache. The cache lock timeout has
  // fired.
  CRONET_NET_ERROR_CACHE_LOCK_TIMEOUT = -409,

  // Received a challenge after the transaction has read some data = and
  // the
  // credentials aren't available.  There isn't a way to get them at
  // that point.
  CRONET_NET_ERROR_CACHE_AUTH_FAILURE_AFTER_READ = -410,

  // Internal not-quite error code for the HTTP cache. In-memory hints
  // suggest
  // that the cache entry would not have been useable with the
  // transaction's
  ...
  // An attempt to import a client certificate failed = as the user's
  // key
  ...
  // A Trust Tokens protocol operation-executing request failed for one
  // of a
  // number of reasons (precondition failure = internal error = bad
  // response,.
  CRONET_NET_ERROR_TRUST_TOKEN_OPERATION_FAILED = -506,

  // When handling a Trust Tokens protocol operation-executing request =
  // the
  // system
  // found that the request's desired Trust Tokens results were already
  // present
  // in
  ...
  // The server cannot fulfill the request at this point. This is a
  // temporary
  // error.
  ...
  // The file is busy = or some other temporary error condition on
  // opening
  // the file.
  ...
  // Server rejected our command because we didn't issue the commands in
  // right
  // order.
  ...
  // Note it's a little weird this is an error but reimporting a PKCS12
  // is ok
  ...
  // The entry was not found in cache or other local sources = for
  // lookups where
