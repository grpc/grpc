...
static grpc_error* fill_in_metadata(grpc_exec_ctx* exec_ctx, inproc_stream* s,
  ...
  for (grpc_linked_mdelem* elem = metadata->list.head;
    ...
    grpc_linked_mdelem* nelem = reinterpret_cast<grpc_linked_mdelem*>(
...
static void fail_helper_locked(grpc_exec_ctx* exec_ctx, inproc_stream* s,
  ...
  if (s->recv_initial_md_op) {
    ...
    if (!s->t->is_client) {
      ...
      grpc_metadata_batch fake_md;
      grpc_metadata_batch_init(&fake_md);
      grpc_linked_mdelem* path_md = reinterpret_cast<grpc_linked_mdelem*>(
      ...
      GPR_ASSERT(grpc_metadata_batch_link_tail(exec_ctx, &fake_md, path_md) ==
                 GRPC_ERROR_NONE);
      grpc_linked_mdelem* auth_md = reinterpret_cast<grpc_linked_mdelem*>(
...
static void op_state_machine(grpc_exec_ctx* exec_ctx, void* arg,
  ...
  inproc_stream* s = reinterpret_cast<inproc_stream*>(arg);
...
static void inproc_transports_create(grpc_exec_ctx* exec_ctx,
  ...
  inproc_transport* st =
      reinterpret_cast<inproc_transport*>(gpr_zalloc(sizeof(*st)));
  inproc_transport* ct =
      reinterpret_cast<inproc_transport*>(gpr_zalloc(sizeof(*ct)));
  // Share one lock between both sides since both sides get affected
  st->mu = ct->mu = reinterpret_cast<shared_mu*>(gpr_malloc(sizeof(*st->mu)));
