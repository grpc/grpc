  ...
  inproc_transport(const grpc_transport_vtable* vtable, shared_mu* mu,
      ...
      : mu(mu),
        ...
        state_tracker(is_client ? "inproc_client" : "inproc_server",
                      GRPC_CHANNEL_READY) {
    base.vtable = vtable;
    // Start each side of transport with 2 refs since they each have a
    // ref to the other
    gpr_ref_init(&refs, 2);
  }
...
struct inproc_stream {
  inproc_stream(inproc_transport* t, grpc_stream_refcount* refcount,
    ...
    if (!server_data) {
      ...
      // Pass the client-side stream address to the server-side for a
      // ref
      ...
      // This is the server-side and is being called through
      // accept_stream_cb
      ...
      // Now we are about to affect the other side, so lock the
      // transport to make sure that it doesn't get destroyed
      gpr_mu_lock(&t->mu->mu);
      cs->other_side = this;
      // Now transfer from the other side's write_buffer if any to the
      // to_read buffer
...
#undef STREAM_UNREF
  ...
  // The following tracks if the server-side only pretends to have
  // received trailing metadata since it no longer cares about the RPC.
  // If that is the case, it is still ok for the client to send trailing
  // metadata (in which case it will be ignored).
...
void log_metadata(const grpc_metadata_batch* md_batch, bool is_client,
  ...
  for (grpc_linked_mdelem* md = md_batch->list.head; md != nullptr;
    ...
    gpr_log(GPR_INFO, "INPROC:%s:%s: %s: %s",
...
grpc_error_handle fill_in_metadata(inproc_stream* s,
  ...
  for (grpc_linked_mdelem* elem = metadata->list.head;
       (elem != nullptr) && (error == GRPC_ERROR_NONE);
       elem = elem->next) {
    grpc_linked_mdelem* nelem = static_cast<grpc_linked_mdelem*>(
        s->arena->Alloc(sizeof(*nelem)));
    nelem->md = grpc_mdelem_from_slices(
        grpc_slice_intern(GRPC_MDKEY(elem->md)),
...
// This function means that we are done talking/listening to the other
// side
...
// stream. This is called when one of the pending operations for the
// stream is done and about to be NULLed out
...
void fail_helper_locked(inproc_stream* s, grpc_error_handle error) {
  ...
  if (s->recv_initial_md_op) {
    ...
    if (!s->t->is_client) {
      ...
      grpc_linked_mdelem* path_md = static_cast<grpc_linked_mdelem*>(
          s->arena->Alloc(sizeof(*path_md)));
      path_md->md =
          grpc_mdelem_from_slices(g_fake_path_key, g_fake_path_value);
      GPR_ASSERT(grpc_metadata_batch_link_tail(&fake_md, path_md) ==
                 GRPC_ERROR_NONE);
      grpc_linked_mdelem* auth_md = static_cast<grpc_linked_mdelem*>(
          s->arena->Alloc(sizeof(*auth_md)));
      auth_md->md =
          grpc_mdelem_from_slices(g_fake_auth_key, g_fake_auth_value);
    ...
    INPROC_LOG(GPR_INFO,
               "fail_helper %p scheduling initial-metadata-ready %s %s",
  ...
  if (s->recv_message_op) {
    INPROC_LOG(GPR_INFO, "fail_helper %p scheduling message-ready %s",
               s, grpc_error_std_string(error).c_str());
  ...
  if (s->recv_trailing_md_op) {
    INPROC_LOG(GPR_INFO,
               "fail_helper %p scheduling trailing-metadata-ready %s",
               s, grpc_error_std_string(error).c_str());
    grpc_core::ExecCtx::Run(
        DEBUG_LOCATION,
        ...
        GRPC_ERROR_REF(error));
    INPROC_LOG(GPR_INFO,
               "fail_helper %p scheduling trailing-md-on-complete %s",
               s, grpc_error_std_string(error).c_str());
    complete_if_batch_end_locked(
...
void message_transfer_locked(inproc_stream* sender,
  ...
  size_t remaining = sender->send_message_op->payload->send_message
                         .send_message->length();
  ...
  do {
    ...
    grpc_closure unused;
    GPR_ASSERT(sender->send_message_op->payload->send_message
                   .send_message->Next(SIZE_MAX, &unused));
    grpc_error_handle error =
  ...
  INPROC_LOG(GPR_INFO,
             "message_transfer_locked %p scheduling message-ready",
             receiver);
  grpc_core::ExecCtx::Run(DEBUG_LOCATION,
                          receiver->recv_message_op->payload
...
void op_state_machine_locked(inproc_stream* s,
                             grpc_error_handle error) {
  // This function gets called when we have contents in the unprocessed
  // reads Get what we want based on our ops wanted Schedule our
  // appropriate closures and then return to ops_needed state if still
  // needed
  ...
  if (s->send_message_op && other) {
    if (other->recv_message_op) {
      ...
      // A server send will never be matched if the server already sent
      // status
      s->send_message_op->payload->send_message.send_message.reset();
      complete_if_batch_end_locked(s, GRPC_ERROR_NONE,
                                   s->send_message_op,
                                   "op_state_machine scheduling "
                                   "send-message-on-complete case 1");
      s->send_message_op = nullptr;
    }
  }
  ...
  if (s->send_trailing_md_op &&
    ...
    if (*destfilled || s->trailing_md_sent) {
      ...
      new_err = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
          "Extra trailing metadata");
      ...
      s->trailing_md_sent = true;
      if (s->send_trailing_md_op->payload->send_trailing_metadata
              .sent) {
      ...
      if (!s->t->is_client && s->trailing_md_recvd &&
        ...
        INPROC_LOG(
            GPR_INFO,
            "op_state_machine %p scheduling trailing-metadata-ready",
            s);
        grpc_core::ExecCtx::Run(
            DEBUG_LOCATION,
            s->recv_trailing_md_op->payload->recv_trailing_metadata
    ...
    complete_if_batch_end_locked(s, GRPC_ERROR_NONE,
                                 ...
                                 "op_state_machine scheduling "
                                 "send-trailing-metadata-on-complete");
    s->send_trailing_md_op = nullptr;
  }
  if (s->recv_initial_md_op) {
    if (s->initial_md_recvd) {
      new_err = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
          "Already recvd initial md");
      INPROC_LOG(GPR_INFO,
                 "op_state_machine %p scheduling on_complete errors "
                 "for already "
                 "recvd initial md %s",
    ...
    if (s->to_read_initial_md_filled) {
      ...
      complete_if_batch_end_locked(s, new_err, s->recv_initial_md_op,
                                   "op_state_machine scheduling "
                                   "recv-initial-metadata-on-complete");
      s->recv_initial_md_op = nullptr;
  ...
  if (s->to_read_trailing_md_filled) {
    ...
    if (s->recv_message_op != nullptr) {
      ...
      *s->recv_message_op->payload->recv_message.recv_message = nullptr;
      INPROC_LOG(GPR_INFO,
                 "op_state_machine %p scheduling message-ready", s);
      grpc_core::ExecCtx::Run(
          DEBUG_LOCATION,
          s->recv_message_op->payload->recv_message.recv_message_ready,
          GRPC_ERROR_NONE);
    ...
    if ((s->trailing_md_sent || s->t->is_client) &&
        s->send_message_op) {
      // Nothing further will try to receive from this stream, so finish
      // off any outstanding send_message op
      s->send_message_op->payload->send_message.send_message.reset();
      s->send_message_op->payload->send_message.stream_write_closed =
          true;
      complete_if_batch_end_locked(s, new_err, s->send_message_op,
                                   "op_state_machine scheduling "
                                   "send-message-on-complete case 2");
      s->send_message_op = nullptr;
    }
    if (s->recv_trailing_md_op != nullptr) {
      ...
      // 2. this stream is the server-side AND has already sent its
      // trailing md
      //    (If the server hasn't already sent its trailing md, it
      //    doesn't have
      ...
      INPROC_LOG(GPR_INFO,
                 "op_state_machine %p has trailing md but not yet "
                 "waiting for it",
                 s);
    }
  }
  if (!s->t->is_client && s->trailing_md_sent &&
      (s->recv_trailing_md_op != nullptr)) {
    // In this case, we don't care to receive the write-close from the
    // client because we have already sent status and the RPC is over as
    // far as we are concerned.
    INPROC_LOG(GPR_INFO,
               "op_state_machine %p scheduling trailing-md-ready %s", s,
               grpc_error_std_string(new_err).c_str());
    grpc_core::ExecCtx::Run(
        DEBUG_LOCATION,
        s->recv_trailing_md_op->payload->recv_trailing_metadata
    ...
    s->trailing_md_recvd = true;
    s->recv_trailing_md_op = nullptr;
    // Since we are only pretending to have received the trailing MD, it
    // would be ok (not an error) if the client actually sends it later.
    s->trailing_md_recvd_implicit_only = true;
  }
  if (s->trailing_md_recvd && s->recv_message_op) {
    // No further message will come on this stream, so finish off the
    // recv_message_op
    INPROC_LOG(GPR_INFO, "op_state_machine %p scheduling message-ready",
               s);
    *s->recv_message_op->payload->recv_message.recv_message = nullptr;
    grpc_core::ExecCtx::Run(
        DEBUG_LOCATION,
  ...
  if (s->send_message_op || s->send_trailing_md_op ||
      s->recv_initial_md_op || s->recv_message_op ||
      s->recv_trailing_md_op) {
    // Didn't get the item we wanted so we still need to get
    // rescheduled
    INPROC_LOG(GPR_INFO,
               "op_state_machine %p still needs closure %p %p %p %p %p",
               s, s->send_message_op, s->send_trailing_md_op,
               s->recv_initial_md_op, s->recv_message_op,
               s->recv_trailing_md_op);
    s->ops_needed = true;
  }
...
bool cancel_stream_locked(inproc_stream* s, grpc_error_handle error) {
  ...
  if (s->cancel_self_error == GRPC_ERROR_NONE) {
    ...
    // Send trailing md to the other side indicating cancellation, even
    // if we already have
    s->trailing_md_sent = true;
...
void perform_stream_op(grpc_transport* gt, grpc_stream* gs,
  ...
  if (on_complete == nullptr) {
    on_complete =
        GRPC_CLOSURE_INIT(&op->handler_private.closure, do_nothing,
                          nullptr, grpc_schedule_on_exec_ctx);
  }

  if (op->cancel_stream) {
    // Call cancel_stream_locked without ref'ing the cancel_error
    // because this function is responsible to make sure that that field
    // gets unref'ed
    ...
    INPROC_LOG(
        GPR_INFO, "perform_stream_op %p %s%s%s%s%s%s%s", s,
  ...
  if (error == GRPC_ERROR_NONE &&
    ...
    // 2. We want to send trailing metadata and there isn't an unmatched
    // send
    ...
    if ((op->send_message && other &&
        ...
        (op->send_trailing_metadata &&
         (!s->send_message_op ||
          (other && other->recv_trailing_md_op))) ||
        (op->recv_initial_metadata && s->to_read_initial_md_filled) ||
        (op->recv_message && other &&
         other->send_message_op != nullptr) ||
    ...
    if (error != GRPC_ERROR_NONE) {
      ...
      if (op->recv_initial_metadata) {
        if (op->payload->recv_initial_metadata
                .trailing_metadata_available != nullptr) {
          // Set to true unconditionally, because we're failing the
          // call, so even if we haven't actually seen the
          // send_trailing_metadata op from the other side, we're going
          // to return trailing metadata anyway.
          *op->payload->recv_initial_metadata
               .trailing_metadata_available = true;
        }
        INPROC_LOG(GPR_INFO,
                   "perform_stream_op error %p scheduling "
                   "initial-metadata-ready %s",
                   s, grpc_error_std_string(error).c_str());
        grpc_core::ExecCtx::Run(DEBUG_LOCATION,
                                op->payload->recv_initial_metadata
                                    .recv_initial_metadata_ready,
      ...
      if (op->recv_message) {
        INPROC_LOG(GPR_INFO,
                   "perform_stream_op error %p scheduling recv "
                   "message-ready %s",
                   s, grpc_error_std_string(error).c_str());
      ...
      if (op->recv_trailing_metadata) {
        INPROC_LOG(GPR_INFO,
                   ...
                   s, grpc_error_std_string(error).c_str());
        grpc_core::ExecCtx::Run(DEBUG_LOCATION,
                                op->payload->recv_trailing_metadata
                                    .recv_trailing_metadata_ready,
                                GRPC_ERROR_REF(error));
      }
    }
    INPROC_LOG(GPR_INFO,
               "perform_stream_op %p scheduling on_complete %s", s,
               grpc_error_std_string(error).c_str());
...
void destroy_stream(grpc_transport* gt, grpc_stream* gs,
  ...
  INPROC_LOG(GPR_INFO, "destroy_stream %p %p", gs,
...
grpc_channel* grpc_inproc_channel_create(grpc_server* server,
  ...
  // Remove max_connection_idle and max_connection_age channel arguments
  // since those do not apply to inproc transports.
  ...
  default_authority_arg.key =
      const_cast<char*>(GRPC_ARG_DEFAULT_AUTHORITY);
  default_authority_arg.value.string =
  ...
  grpc_transport* server_transport;
  grpc_transport* client_transport;
  inproc_transports_create(&server_transport, server_args,
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (error != GRPC_ERROR_NONE) {
      ...
      GRPC_ERROR_UNREF(error);
      // client_transport was destroyed when grpc_channel_create saw an
      // error.
