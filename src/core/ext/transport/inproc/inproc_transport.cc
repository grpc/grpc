...
#undef STREAM_UNREF
  ...
  void fail_helper_locked(inproc_stream * s, grpc_error_handle error) {
    ...
    if (s->recv_trailing_md_op) {
      INPROC_LOG(GPR_INFO,
                 "fail_helper %p scheduling trailing-metadata-ready %s", s,
                 grpc_error_std_string(error).c_str());
      grpc_core::ExecCtx::Run(
          DEBUG_LOCATION,
          ...
          GRPC_ERROR_REF(error));
      INPROC_LOG(GPR_INFO,
                 "fail_helper %p scheduling trailing-md-on-complete %s", s,
                 grpc_error_std_string(error).c_str());
      complete_if_batch_end_locked(
    ...
    if (error == GRPC_ERROR_NONE &&
      ...
      if (error == GRPC_ERROR_NONE && op->send_initial_metadata) {
        ...
        if (*destfilled || s->initial_md_sent) {
          ...
          if (s->t->is_client) {
            grpc_core::Timestamp* dl = (other == nullptr)
  ...
  void set_pollset(
      grpc_transport* /*gt*/, grpc_stream* /*gs*/, grpc_pollset * /*pollset*/) {
    // Nothing to do here
  }

  void set_pollset_set(
      grpc_transport* /*gt*/, grpc_stream* /*gs*/, grpc_pollset_set *
      /*pollset_set*/) {
    // Nothing to do here
  }
