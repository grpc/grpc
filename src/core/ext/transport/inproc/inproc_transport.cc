...
static void
ref_transport (inproc_transport * t)
{
...
static void
really_destroy_transport (inproc_transport * t)
{
...
static void
unref_transport (inproc_transport * t)
{
...
static void
ref_stream (inproc_stream * s, const char *reason)
{
...
static void
unref_stream (inproc_stream * s, const char *reason)
{
...
static void
really_destroy_stream (inproc_stream * s)
{
...
static void
log_metadata (const grpc_metadata_batch * md_batch, bool is_client,
	      bool is_initial)
{
...
static grpc_error *
fill_in_metadata (inproc_stream * s,
...
static int
init_stream (grpc_transport * gt, grpc_stream * gs,
...
static void
close_stream_locked (inproc_stream * s)
{
  if (!s->closed)
    {
...
static void
close_other_side_locked (inproc_stream * s, const char *reason)
{
...
static void
complete_if_batch_end_locked (inproc_stream * s, grpc_error * error,
...
static void
maybe_schedule_op_closure_locked (inproc_stream * s, grpc_error * error)
{
...
static void
fail_helper_locked (inproc_stream * s, grpc_error * error)
{
  ...
  if (!s->trailing_md_sent)
	...
	? &s->write_buffer_trailing_md : &other->to_read_trailing_md;
      bool *destfilled =
	(other ==
	 nullptr) ? &s->write_buffer_trailing_md_filled : &other->
  ...
  if (s->recv_initial_md_op)
      ...
      if (!s->t->is_client)
	{
	  ...
	  grpc_linked_mdelem *path_md =
	    (grpc_linked_mdelem *) gpr_arena_alloc (s->arena,
						    sizeof (*path_md));
	  path_md->md =
	  ...
	  fill_in_metadata (s, &fake_md, 0,
			    s->recv_initial_md_op->payload->
			    recv_initial_metadata.recv_initial_metadata,
			    s->recv_initial_md_op->payload->
			    recv_initial_metadata.recv_flags, nullptr);
	  grpc_metadata_batch_destroy (&fake_md);
	  err = GRPC_ERROR_NONE;
	}
      else
	  ...
	  trailing_metadata_available != nullptr)
	{
	  ...
	  *s->recv_initial_md_op->payload->recv_initial_metadata.
      ...
      INPROC_LOG (GPR_DEBUG,
		  ...
		  error, err);
      GRPC_CLOSURE_SCHED (s->recv_initial_md_op->payload->
  ...
  if (s->recv_message_op)
    {
      INPROC_LOG (GPR_DEBUG, "fail_helper %p scheduling message-ready %p", s,
		  error);
      GRPC_CLOSURE_SCHED (s->recv_message_op->payload->recv_message.
			  recv_message_ready, GRPC_ERROR_REF (error));
...
static void
message_transfer_locked (inproc_stream * sender, inproc_stream * receiver)
{
  size_t remaining =
  ...
  do
    {
      ...
      GPR_ASSERT (grpc_byte_stream_next
		  (sender->send_message_op->payload->send_message.
		   send_message, SIZE_MAX, &unused));
      grpc_error *error =
	grpc_byte_stream_pull (sender->send_message_op->payload->send_message.
			       send_message,
			       &message_slice);
  ...
  grpc_slice_buffer_stream_init (&receiver->recv_stream,
				 &receiver->recv_message, 0);
  *receiver->recv_message_op->payload->recv_message.recv_message =
  ...
  INPROC_LOG (GPR_DEBUG,
	      ...
	      receiver);
  GRPC_CLOSURE_SCHED (receiver->recv_message_op->payload->recv_message.
		      recv_message_ready, GRPC_ERROR_NONE);
  complete_if_batch_end_locked (sender, GRPC_ERROR_NONE,
				sender->send_message_op,
				"message_transfer scheduling sender on_complete");
  complete_if_batch_end_locked (receiver, GRPC_ERROR_NONE,
...
static void
op_state_machine (void *arg, grpc_error * error)
{
  ...
  if (s->send_trailing_md_op &&
      ...
      if (*destfilled || s->trailing_md_sent)
	{
	  ...
	  new_err =
	    GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Extra trailing metadata");
  ...
  if (s->recv_initial_md_op)
      ...
      if (s->to_read_initial_md_filled)
	{
	  ...
	  if (s->recv_initial_md_op->payload->recv_initial_metadata.
	    ...
	    {
	      *s->recv_initial_md_op->payload->recv_initial_metadata.
		trailing_metadata_available = (other != nullptr
					       && other->
					       send_trailing_md_op !=
					       nullptr);
	    }
	  ...
	  s->to_read_initial_md_filled = false;
	  INPROC_LOG (GPR_DEBUG,
		      "op_state_machine %p scheduling initial-metadata-ready %p",
		      s, new_err);
	  GRPC_CLOSURE_SCHED (s->recv_initial_md_op->payload->
	  ...
	  if (new_err != GRPC_ERROR_NONE)
	    {
	      INPROC_LOG (GPR_DEBUG,
			  "op_state_machine %p scheduling on_complete errors2 %p",
			  s, new_err);
	      fail_helper_locked (s, GRPC_ERROR_REF (new_err));
	      goto done;
	    }
	  ...
	  if (s->t->is_client || s->trailing_md_sent)
	  ...
	  else
	    {
	      INPROC_LOG (GPR_DEBUG,
			  ...
			  "trailing-md-on-complete %p", s, new_err);
	    }
	}
      else
	{
	  INPROC_LOG (GPR_DEBUG,
		      "op_state_machine %p has trailing md but not yet waiting for it",
		      s);
	}
    }
  if (s->trailing_md_recvd && s->recv_message_op)
    {
      ...
      // recv_message_op
      INPROC_LOG (GPR_DEBUG, "op_state_machine %p scheduling message-ready",
		  s);
      GRPC_CLOSURE_SCHED (s->recv_message_op->payload->recv_message.
			  recv_message_ready, GRPC_ERROR_NONE);
      complete_if_batch_end_locked (s, new_err, s->recv_message_op,
...
static bool
cancel_stream_locked (inproc_stream * s, grpc_error * error)
{
  ...
  if (s->cancel_self_error == GRPC_ERROR_NONE)
	...
	{
	  complete_if_batch_end_locked (s, s->cancel_self_error,
					s->recv_trailing_md_op,
...
static void
perform_stream_op (grpc_transport * gt, grpc_stream * gs,
		   grpc_transport_stream_op_batch * op)
{
  ...
  if (grpc_inproc_trace.enabled ())
	...
	{
	  log_metadata (op->payload->send_initial_metadata.
			send_initial_metadata, s->t->is_client, true);
	}
      if (op->send_trailing_metadata)
	{
	  log_metadata (op->payload->send_trailing_metadata.
			send_trailing_metadata, s->t->is_client, false);
	}
    }
  grpc_error *error = GRPC_ERROR_NONE;
  ...
  if (error == GRPC_ERROR_NONE &&
	...
	{
	  grpc_metadata_batch *dest = (other == nullptr)
	    ? &s->write_buffer_initial_md : &other->to_read_initial_md;
	  uint32_t *destflags = (other == nullptr)
	    ? &s->write_buffer_initial_md_flags
	    : &other->to_read_initial_md_flags;
	  bool *destfilled =
	    (other ==
	     nullptr) ? &s->write_buffer_initial_md_filled : &other->
	  ...
	  if (*destfilled || s->initial_md_sent)
	      ...
	      if (!other || !other->closed)
		{
		  fill_in_metadata (s,
				    ...
				    op->payload->send_initial_metadata.
				    send_initial_metadata_flags, dest,
	      ...
	      if (s->t->is_client)
		{
		  grpc_millis *dl =
		    (other ==
		     nullptr) ? &s->write_buffer_deadline : &other->deadline;
		  *dl =
		    GPR_MIN (*dl,
			     op->payload->send_initial_metadata.
		  ...
		  trailing_metadata_available != nullptr)
		{
		  ...
		  *op->payload->recv_initial_metadata.
	      ...
	      INPROC_LOG (GPR_DEBUG,
			  ...
			  s, error);
	      GRPC_CLOSURE_SCHED (op->payload->recv_initial_metadata.
				  recv_initial_metadata_ready,
	  ...
	  if (op->recv_message)
	    {
	      INPROC_LOG (GPR_DEBUG,
			  ...
			  s, error);
	      GRPC_CLOSURE_SCHED (op->payload->recv_message.
				  recv_message_ready, GRPC_ERROR_REF (error));
	    }
	}
      INPROC_LOG (GPR_DEBUG, "perform_stream_op %p scheduling on_complete %p",
		  s, error);
...
static void
perform_transport_op (grpc_transport * gt, grpc_transport_op * op)
{
  inproc_transport *t = (inproc_transport *) gt;
  ...
  if (op->on_connectivity_state_change)
    {
      grpc_connectivity_state_notify_on_state_change (&t->connectivity,
...
static void
destroy_stream (grpc_transport * gt, grpc_stream * gs,
...
static void
destroy_transport (grpc_transport * gt)
{
  inproc_transport *t = (inproc_transport *) gt;
...
static void
set_pollset (grpc_transport * gt, grpc_stream * gs, grpc_pollset * pollset)
{
  // Nothing to do here
}

static void
set_pollset_set (grpc_transport * gt, grpc_stream * gs,
 ...
 */
static void
do_nothing (void *arg, grpc_error * error)
{
}

void
grpc_inproc_transport_init (void)
{
 ...
 */
static void
inproc_transports_create (grpc_transport ** server_transport,
...
grpc_channel *
grpc_inproc_channel_create (grpc_server * server,
			    grpc_channel_args * args, void *reserved)
{
...
void
grpc_inproc_transport_shutdown (void)
{
  grpc_core::ExecCtx exec_ctx;
