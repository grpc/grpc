...
auto ChaoticGoodServerListener::DataConnectionListener::Extract(
    absl::string_view id) {
  MutexLock lock(&mu_);
...
void ChaoticGoodServerListener::DataConnectionListener::ConnectionTimeout(
  ...
  if (!pending_connection.empty()) {
    pending_connection.mapped().latch->Set(
...
void ChaoticGoodServerListener::DataConnectionListener::FinishDataConnection(
  ...
  if (pending_connection.mapped().control_endpoint_auth_context != nullptr) {
    ...
    std::optional<bool> is_same_peer = auth_context->CompareAuthContext(
      pending_connection.mapped().control_endpoint_auth_context.get());
  ...
  pending_connection.mapped().latch->Set(std::move(endpoint));
}
