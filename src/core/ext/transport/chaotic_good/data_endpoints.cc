...
auto Endpoint::PullDataPayload(RefCountedPtr<EndpointContext> ctx) {
  return Map(
      ...
      [ctx](
        ...
        SliceBuffer buffer;
        bool force_metrics_update = false;
        ...
        for (size_t i = 0; i < queued_frames->size(); ++i) {
          ...
          if (queued_frame.frame->call_tracer != nullptr &&
              queued_frame.frame->call_tracer->ForceMetricsUpdate()) {
            force_metrics_update = true;
          }
        ...
        return std::make_pair(std::move(buffer), force_metrics_update);
      });
}
  ...
  return Loop([ctx = std::move(ctx),
    ...
    return TrySeq(
        ...
        [ctx, metrics_collector](std::pair<SliceBuffer, bool> result) {
          ...
          bool force_metrics_update = result.second;
