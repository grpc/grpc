...
SendRate::DeliveryData SendRate::GetDeliveryData(uint64_t current_time) const {
  ...
  double last_scheduled_time =
      current_time > timestamps_.last_scheduled_time
          ? -static_cast<double>(current_time - timestamps_.last_scheduled_time)
          : static_cast<double>(timestamps_.last_scheduled_time - current_time);
  double last_reader_dequeued_time =
      current_time > timestamps_.last_reader_dequeued_time
          ? -static_cast<double>(current_time -
                                 timestamps_.last_reader_dequeued_time)
          : static_cast<double>(timestamps_.last_reader_dequeued_time -
                                current_time);
  if (current_rate_ <= 0) {
    return DeliveryData{(start_time + rtt_usec_ * 500.0) * 1e-9, 1e14,
                        queued_bytes_,
                        DeliveryData::RelativeTimestamps{
                            last_scheduled_time, last_reader_dequeued_time}};
    } else {
