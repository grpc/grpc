...
class PickBestScheduler : public SimpleScheduler {
 ...
 private:
  ...
  bool IsChannelEligible(const Channel* c, uint64_t bytes) {
    switch (filter_fn_) {
      case FilterFn::kNone:
        return true;
  ...
  bool Compare(const Channel* a, const Channel* b, uint64_t bytes) {
    switch (comparative_fn_) {
      case ComparativeFn::kOutstandingBytesAndScheduledTime:
        return std::make_tuple(
                   a->delivery_data.queued_bytes.reader_outstanding_bytes +
                       a->delivery_data.queued_bytes
                           .endpoint_outstanding_bytes +
                       scheduled_bytes_per_channel(a->id),
                   a->delivery_data.timestamps.last_scheduled_time) <
               std::make_tuple(
                   b->delivery_data.queued_bytes.reader_outstanding_bytes +
                       b->delivery_data.queued_bytes
                           .endpoint_outstanding_bytes +
                       scheduled_bytes_per_channel(b->id),
                   b->delivery_data.timestamps.last_scheduled_time);
        ...
        return std::make_tuple(
                   a->delivery_data.queued_bytes.reader_outstanding_bytes +
                       a->delivery_data.queued_bytes
                           .endpoint_outstanding_bytes +
                       scheduled_bytes_per_channel(a->id),
                   a->delivery_data.timestamps.last_reader_dequeued_time) <
               std::make_tuple(
                   b->delivery_data.queued_bytes.reader_outstanding_bytes +
                       b->delivery_data.queued_bytes
                           .endpoint_outstanding_bytes +
                       scheduled_bytes_per_channel(b->id),
                   b->delivery_data.timestamps.last_reader_dequeued_time);
    }
    return false;
  }
};
    ...
    ParseConfig(name, value)
        .Var("weight", weight_fn_,
             {{"any_ready", WeightFn::kAnyReady},
              ...
              {"inverse_outstanding_bytes",
               WeightFn::kInverseOutstandingBytes}})
        .Check();
  }
