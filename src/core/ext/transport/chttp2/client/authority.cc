...
grpc_channel_args* grpc_default_authority_add_if_not_present(const grpc_channel_args* args) {
  ...
  if (!has_default_authority) {
    ...
    default_authority = grpc_core::ResolverRegistry::GetDefaultAuthority(server_uri_str);
    new_args[num_new_args++] =
        grpc_channel_arg_string_create(const_cast<char*>(GRPC_ARG_DEFAULT_AUTHORITY),
