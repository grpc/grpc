...
Chttp2Connector::Chttp2Connector() {
  GRPC_CLOSURE_INIT(&connected_, Connected, this,
...
void Chttp2Connector::Connect(const Args& args, Result* result,
  ...
  Subchannel::GetAddressFromSubchannelAddressArg(args.channel_args,
                                                 &addr);
  grpc_endpoint** ep;
  {
    MutexLock lock(&mu_);
  ...
  // grpc_tcp_client_connect() returns, and since the closure requires
  // access to mu_, this can result in a deadlock (see
  // https://github.com/grpc/grpc/issues/16427 for details).
  // grpc_tcp_client_connect() will fill endpoint_ with proper contents,
  // and we make sure that we still exist at that point by taking a ref.
  ...
  bool unref = false;
  {
    ...
    if (error != GRPC_ERROR_NONE || self->shutdown_) {
      if (error == GRPC_ERROR_NONE) {
        error =
            GRPC_ERROR_CREATE_FROM_STATIC_STRING("connector shutdown");
      } else {
...
void Chttp2Connector::StartHandshakeLocked() {
  ...
  handshake_mgr_->DoHandshake(endpoint_, args_.channel_args,
...
namespace {
void NullThenSchedClosure(const DebugLocation& location,
...
void Chttp2Connector::OnHandshakeDone(void* arg,
  ...
  Chttp2Connector* self =
      static_cast<Chttp2Connector*>(args->user_data);
  {
    MutexLock lock(&self->mu_);
    if (error != GRPC_ERROR_NONE || self->shutdown_) {
      if (error == GRPC_ERROR_NONE) {
        error =
            GRPC_ERROR_CREATE_FROM_STATIC_STRING("connector shutdown");
        // We were shut down after handshaking completed successfully,
        // so destroy the endpoint here.
        if (args->endpoint != nullptr) {
          // TODO(ctiller): It is currently necessary to shutdown
          // endpoints before destroying them, even if we know that
          // there are no pending read/write callbacks.  This should be
          // fixed, at which point this can be removed.
  ...
  Chttp2Connector* self = static_cast<Chttp2Connector*>(arg);
  {
    ...
    if (!self->notify_error_.has_value()) {
      grpc_endpoint_delete_from_pollset_set(
          self->endpoint_, self->args_.interested_parties);
      if (error != GRPC_ERROR_NONE) {
      ...
      // OnTimeout() was already invoked. Call Notify() again so that
...
void Chttp2Connector::OnTimeout(void* arg,
  ...
  Chttp2Connector* self = static_cast<Chttp2Connector*>(arg);
  {
    MutexLock lock(&self->mu_);
    if (!self->notify_error_.has_value()) {
      // The transport did not receive the settings frame in time.
      ...
      self->MaybeNotify(GRPC_ERROR_CREATE_FROM_STATIC_STRING(
          "connection attempt timed out before receiving SETTINGS "
          "frame"));
    } else {
      // OnReceiveSettings() was already invoked. Call Notify() again so
      // that notify_ can be invoked.
      self->MaybeNotify(GRPC_ERROR_NONE);
    }
  }
  self->Unref();
