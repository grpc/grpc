...
static void
chttp2_connector_ref (grpc_connector * con)
{
...
static void
chttp2_connector_unref (grpc_connector * con)
{
  chttp2_connector *c = (chttp2_connector *) con;
  if (gpr_unref (&c->refs))
    {
	...
	grpc_endpoint_destroy (c->endpoint);
      gpr_free (c);
    }
}

static void
chttp2_connector_shutdown (grpc_connector * con, grpc_error * why)
{
  chttp2_connector *c = (chttp2_connector *) con;
  gpr_mu_lock (&c->mu);
  c->shutdown = true;
  if (c->handshake_mgr != nullptr)
    {
      grpc_handshake_manager_shutdown (c->handshake_mgr,
				       GRPC_ERROR_REF (why));
    }
...
static void
start_handshake_locked (chttp2_connector * c)
{
  ...
  grpc_handshake_manager_do_handshake (c->handshake_mgr,
				       c->args.interested_parties,
				       c->endpoint, c->args.channel_args,
				       c->args.deadline,
				       nullptr /* acceptor */ ,
				       on_handshake_done, c);
...
static void
connected (void *arg, grpc_error * error)
{
...
static void
chttp2_connector_connect (grpc_connector * con,
			  ...
			  grpc_closure * notify)
{
  ...
  GPR_ASSERT (!c->connecting);
  c->connecting = true;
  grpc_tcp_client_connect (&c->connected, &c->endpoint,
