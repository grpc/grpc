 ...
 private:
  static grpc_channel_args* GetSecureNamingChannelArgs(
    ...
    if (channel_credentials == nullptr) {
      gpr_log(GPR_ERROR,
              "Can't create subchannel: channel credentials missing "
              "for secure "
              "channel.");
      return nullptr;
    }
    ...
    if (grpc_security_connector_find_in_args(args) != nullptr) {
      gpr_log(GPR_ERROR,
              "Can't create subchannel: security connector already "
              "present in "
              "channel args.");
      return nullptr;
    }
    ...
    if (grpc_channel_args_find(args, GRPC_ARG_DEFAULT_AUTHORITY) ==
        nullptr) {
      // If the channel args don't already contain
      // GRPC_ARG_DEFAULT_AUTHORITY, add the arg, setting it to the
      // value just obtained.
    ...
    grpc_channel_args* args_with_authority =
        grpc_channel_args_copy_and_add(args, args_to_add,
                                       num_args_to_add);
    // Create the security connector using the credentials and target
    // name.
    grpc_channel_args* new_args_from_connector = nullptr;
    RefCountedPtr<grpc_channel_security_connector>
        subchannel_security_connector =
            channel_credentials->create_security_connector(
                /*call_creds=*/nullptr, authority.c_str(),
                args_with_authority, &new_args_from_connector);
    ...
    grpc_arg new_security_connector_arg =
        grpc_security_connector_to_arg(
            subchannel_security_connector.get());
    grpc_channel_args* new_args = grpc_channel_args_copy_and_add(
        new_args_from_connector != nullptr ? new_args_from_connector
                                           : args_with_authority,
        &new_security_connector_arg, 1);
    subchannel_security_connector.reset(DEBUG_LOCATION,
                                        "lb_channel_create");
...
grpc_channel* grpc_secure_channel_create(
  ...
  if (creds != nullptr) {
    // Add channel args containing the client channel factory and
    // channel credentials.
    ...
    grpc_channel_args* new_args =
        grpc_channel_args_copy_and_add_and_remove(
            args, &arg_to_remove, 1, args_to_add,
