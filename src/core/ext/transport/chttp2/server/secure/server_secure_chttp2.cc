...
int grpc_server_add_secure_http2_port(grpc_server* server,
  ...
  if (creds == nullptr) {
    err = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "No credentials specified for secure server port "
        "(creds==NULL)");
    goto done;
  }
  // TODO(yashykt): Ideally, we would not want to have different
  // behavior here based on whether a config fetcher is configured or
  // not. Currently, we have a feature for SSL credentials reloading
  // with an application callback that assumes that there is a single
  // security connector. If we delay the creation of the security
  // connector to after the creation of the listener(s), we would have
  // potentially multiple security connectors which breaks the
  // assumption for SSL creds reloading. When the API for SSL creds
  // reloading is rewritten, we would be able to make this workaround go
  // away by removing that assumption. As an immediate drawback of this
  // workaround, config fetchers need to be registered before adding
  // ports to the server.
  if (server->core_server->config_fetcher() != nullptr) {
    ...
    args = grpc_channel_args_copy_and_add(
        server->core_server->channel_args(), &arg_to_add, 1);
  } else {
    sc = creds->create_security_connector(nullptr);
    if (sc == nullptr) {
      err = GRPC_ERROR_CREATE_FROM_COPIED_STRING(
          absl::StrCat("Unable to create secure server with "
                       "credentials of type ",
                       creds->type())
              .c_str());
      goto done;
