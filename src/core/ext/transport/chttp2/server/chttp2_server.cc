...
static void
server_connection_state_unref (server_connection_state * connection_state)
{
...
static void
on_timeout (void *arg, grpc_error * error)
{
...
static void
on_receive_settings (void *arg, grpc_error * error)
{
  server_connection_state *connection_state = (server_connection_state *) arg;
...
static void
on_handshake_done (void *arg, grpc_error * error)
{
  ...
  if (error != GRPC_ERROR_NONE || connection_state->svr_state->shutdown)
      ...
      if (args->endpoint != nullptr)
	{
	  ...
	  grpc_server_setup_transport (connection_state->svr_state->server,
				       transport,
	  ...
	  grpc_timer_init (&connection_state->timer,
			   connection_state->deadline,
			   &connection_state->on_timeout);
	}
    }
  grpc_handshake_manager_pending_list_remove (&connection_state->svr_state->
					      pending_handshake_mgrs,
					      connection_state->
					      handshake_mgr);
...
static void
on_accept (void *arg, grpc_endpoint * tcp,
	   grpc_pollset * accepting_pollset,
	   grpc_tcp_server_acceptor * acceptor)
{
  ...
  const grpc_arg *timeout_arg =
    grpc_channel_args_find (state->args,
  ...
  grpc_handshake_manager_do_handshake (connection_state->handshake_mgr,
...
static void
server_start_listener (grpc_server * server, void *arg,
		       grpc_pollset ** pollsets, size_t pollset_count)
{
  ...
  gpr_mu_unlock (&state->mu);
  grpc_tcp_server_start (state->tcp_server, pollsets, pollset_count,
			 on_accept, state);
}

static void
tcp_server_shutdown_complete (void *arg, grpc_error * error)
{
  server_state *state = (server_state *) arg;
  ...
  grpc_handshake_manager_pending_list_shutdown_all (state->
						    pending_handshake_mgrs,
...
/* Server callback: destroy the tcp listener (so we don't generate further
   callbacks) */
static void
server_destroy_listener (grpc_server * server, void *arg,
...
grpc_error *
grpc_chttp2_server_add_port (grpc_server * server, const char *addr,
			     grpc_channel_args * args, int *port_num)
{
  ...
  *port_num = -1;
  ...
  for (i = 0; i < naddrs; i++)
    {
      errors[i] =
	grpc_tcp_server_add_port (tcp_server, &resolved->addrs[i],
				  &port_temp);
  ...
  if (count == 0)
    {
      ...
      gpr_asprintf (&msg,
		    ...
		    naddrs);
      err =
	GRPC_ERROR_CREATE_REFERENCING_FROM_COPIED_STRING (msg, errors,
							  naddrs);
      gpr_free (msg);
      goto error;
    }
