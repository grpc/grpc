...
grpc_slice
grpc_chttp2_base64_encode (grpc_slice input)
{
...
grpc_slice
grpc_chttp2_huffman_compress (grpc_slice input)
{
  size_t nbits;
  uint8_t *in;
  uint8_t *out;
...
static void
enc_add2 (huff_out * out, uint8_t a, uint8_t b)
{
...
static void
enc_add1 (huff_out * out, uint8_t a)
{
  b64_huff_sym sa = huff_alphabet[a];
...
grpc_slice
grpc_chttp2_base64_encode_and_huffman_compress (grpc_slice input)
{
  size_t input_length = GRPC_SLICE_LENGTH (input);
  size_t input_triplets = input_length / 3;
  size_t tail_case = input_length % 3;
