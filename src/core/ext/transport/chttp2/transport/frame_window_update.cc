...
grpc_slice
grpc_chttp2_window_update_create (uint32_t id, uint32_t window_update,
...
grpc_chttp2_window_update_parser_begin_frame (grpc_chttp2_window_update_parser
					      * parser, uint32_t length,
					      uint8_t flags)
{
  if (flags || length != 4)
    {
      char *msg;
      gpr_asprintf (&msg, "invalid window update: length=%d, flags=%02x",
		    length, flags);
...
grpc_error *
grpc_chttp2_window_update_parser_parse (void *parser,
