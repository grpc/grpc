  ...
  return Loop([self = RefAsSubclass<Http2ClientTransport>()]() {
    ...
    return TrySeq(
          ...
          while (self->max_write_size_ > 0) {
            ...
            if (!stream_id.has_value()) {
              GRPC_HTTP2_CLIENT_DLOG
                  << "Http2ClientTransport MultiplexerLoop "
                     "No writable streams available, max_write_size_ = "
                  << self->max_write_size_;
              break;
            }
            GRPC_HTTP2_CLIENT_DLOG << "Http2ClientTransport MultiplexerLoop "
            ...
            absl::StatusOr<StreamDataQueue<ClientMetadataHandle>::DequeueResult>
                result = stream->DequeueFrames(
                    std::min(
                        /*transport_tokens*/ std::numeric_limits<size_t>::max(),
                        self->max_write_size_),
                    self->settings_.peer().max_frame_size(), self->encoder_);
            if (GPR_LIKELY(result.ok())) {
              ...
              GRPC_HTTP2_CLIENT_DLOG
                  << "Http2ClientTransport MultiplexerLoop "
                     "Failed to dequeue frames for stream "
                  << *stream_id << " with status: " << result.status();
              return self->HandleError(Http2Status::AbslStreamError(
                  result.status().code(),
                  std::string(result.status().message())));
            }

            // Update the max_write_size_ based on the bytes consumed in the
            // current dequeue. We may overshoot the max_write_size_ by
            // at most the size of Metadata being written.
            self->max_write_size_ =
                (self->max_write_size_ >= result->total_bytes_consumed)
                    ? (self->max_write_size_ - result->total_bytes_consumed)
                    : 0;
            GRPC_HTTP2_CLIENT_DLOG << "Http2ClientTransport MultiplexerLoop "
                                      "max_write_size_ after dequeue = "
                                   << self->max_write_size_
        ...
        [self](std::vector<Http2Frame> frames) {
          ...
          GRPC_HTTP2_CLIENT_DLOG
              << "Http2ClientTransport MultiplexerLoop Write "
                 "output_buf.length() = "
              << output_buf.Length();
          return If(
            output_buf.Length() > 0,
...
Http2ClientTransport::Http2ClientTransport(
    ...
    : endpoint_(std::move(endpoint)),
      ...
      target_max_write_size_(/*10 MB*/ 10u * 1024u * 1024u),
