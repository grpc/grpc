...
static char* fmt_int64_diff_str(int64_t old_val, int64_t new_val) {
  ...
  if (old_val != new_val) {
    str =
        absl::StrFormat("%" PRId64 " -> %" PRId64 "", old_val, new_val);
  } else {
...
static char* fmt_uint32_diff_str(uint32_t old_val, uint32_t new_val) {
  ...
  if (old_val != new_val) {
    str =
        absl::StrFormat("%" PRIu32 " -> %" PRIu32 "", old_val, new_val);
  } else {
...
void FlowControlTrace::Finish() {
  ...
  if (sfc_ != nullptr) {
    srw_str =
        fmt_int64_diff_str(remote_window_delta_ + remote_window,
                           sfc_->remote_window_delta() + remote_window);
    slw_str = fmt_int64_diff_str(
  ...
  gpr_log(GPR_DEBUG,
          "%p[%u][%s] | %s | trw:%s, tlw:%s, taw:%s, srw:%s, slw:%s, "
          "saw:%s",
...
TransportFlowControlDisabled::TransportFlowControlDisabled(
  ...
  t->settings[GRPC_ACKED_SETTINGS]
             [GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE] = kFrameSize;
...
TransportFlowControl::TransportFlowControl(
    const grpc_chttp2_transport* t, bool enable_bdp_probe)
    : t_(t),
...
uint32_t TransportFlowControl::MaybeSendUpdate(bool writing_anyway) {
  ...
  if ((writing_anyway ||
       announced_window_ <= target_announced_window / 2) &&
...
grpc_error_handle StreamFlowControl::RecvData(
    int64_t incoming_frame_size) {
  ...
  if (incoming_frame_size > acked_stream_window) {
    if (incoming_frame_size <= sent_stream_window) {
      gpr_log(GPR_ERROR,
              ...
              "This would usually cause a disconnection, but allowing "
              "it due to"
...
void StreamFlowControl::IncomingByteStreamUpdate(size_t max_size_hint,
  ...
  GPR_DEBUG_ASSERT(max_recv_bytes <=
                   kMaxWindowUpdateSize - sent_init_window);
...
// Take in a target and modifies it based on the memory pressure of the
// system
...
FlowControlAction TransportFlowControl::PeriodicUpdate() {
  ...
  if (enable_bdp_probe_) {
    ...
    // target might change based on how much memory pressure we are
    // under
    // TODO(ncteisen): experiment with setting target to be huge under
    // low memory pressure.
    double target = pow(2, SmoothLogBdp(TargetLogBdp()));
    if (g_test_only_transport_target_window_estimates_mocker !=
        nullptr) {
