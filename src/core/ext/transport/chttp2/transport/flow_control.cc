...
namespace grpc_core
{
  namespace chttp2
  {
	...
	if (old_val != new_val)
	  {
	    gpr_asprintf (&str, "%" PRId64 " -> %" PRId64 "", old_val,
			  new_val);
	  }
	else
	  {
	...
	if (new_val > 0 && old_val != new_val)
	  {
	    gpr_asprintf (&str, "%" PRIu32 " -> %" PRIu32 "", old_val,
			  new_val);
	  }
	else
	  {
    ...
    void FlowControlTrace::Init (const char *reason,
				 TransportFlowControl * tfc,
	...
	[GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE];
      char *trw_str =
	fmt_int64_diff_str (remote_window_, tfc_->remote_window ());
      char *tlw_str =
	fmt_int64_diff_str (target_window_, tfc_->target_window ());
      char *taw_str =
      ...
      else
	{
      ...
      gpr_log (GPR_DEBUG,
	       ...
	       tfc_->transport ()->is_client ? "cli" : "svr", reason_,
	       trw_str, tlw_str, taw_str, srw_str, slw_str, saw_str);
      gpr_free (trw_str);
      gpr_free (tlw_str);
      gpr_free (taw_str);
      gpr_free (srw_str);
	...
	kFrameSize;
      t->
	settings[GRPC_PEER_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE]
	= kMaxWindow;
      t->
	...
	[GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE] = kMaxWindow;
    }

    TransportFlowControl::TransportFlowControl (const grpc_chttp2_transport *
						t,
						bool enable_bdp_probe):t_ (t),
      enable_bdp_probe_ (enable_bdp_probe), bdp_estimator_ (t->peer_string),
      pid_controller_ (grpc_core::PidController::Args ().set_gain_p (4).
		       set_gain_i (8).set_gain_d (0).
		       set_initial_control_value (TargetLogBdp ()).
		       set_min_control_value (-1).set_max_control_value (25).
	...
	(const uint32_t) target_window ();
      if ((writing_anyway || announced_window_ <= target_announced_window / 2)
	  && announced_window_ != target_announced_window)
	{
	  const uint32_t announce =
	    (uint32_t) GPR_CLAMP (target_announced_window - announced_window_,
				  0, UINT32_MAX);
    ...
    grpc_error *TransportFlowControl::
  ...
  StreamFlowControl::StreamFlowControl (TransportFlowControl * tfc, const grpc_chttp2_stream * s):tfc_ (tfc),
      s_
      (s)
    {
    }
	...
	announced_window_delta_ + acked_init_window;
	...
	{
	  uint32_t announce =
	    (uint32_t) GPR_CLAMP (local_window_delta_ -
				  announced_window_delta_, 0, UINT32_MAX);
...
// Take in a target and modifies it based on the memory pressure of the system
	...
	grpc_resource_quota_get_memory_pressure (quota);
    ...
    {
      return
	AdjustForMemoryPressure (grpc_resource_user_quota
    ...
    FlowControlAction::Urgency TransportFlowControl::
      DeltaUrgency (int64_t value, grpc_chttp2_setting_id setting_id)
    {
      int64_t delta =
	(int64_t) value -
	(int64_t) t_->settings[GRPC_LOCAL_SETTINGS][setting_id];
      ...
      if (enable_bdp_probe_)
	{
	  ...
	  target_initial_window_size_ =
	    (int32_t) GPR_CLAMP (target, 128, INT32_MAX);

	  action.
	    set_send_initial_window_update (DeltaUrgency
					    (target_initial_window_size_,
					     GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE),
					    (uint32_t)
					    target_initial_window_size_);
	  ...
	  int32_t frame_size =
	    (int32_t)
	    GPR_CLAMP (GPR_MAX
		       ((int32_t) GPR_CLAMP (bw_dbl, 0, INT_MAX) / 1000,
			target_initial_window_size_),
		       16384, 16777215);
	  action.
	    set_send_max_frame_size_update (DeltaUrgency
					    ((int64_t) frame_size,
					     GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE),
					    frame_size);
	}
    ...
    FlowControlAction StreamFlowControl::
      ...
      if (!s_->read_closed)
	{
	  uint32_t sent_init_window =
	    ...
	    [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE];
	  if (local_window_delta_ > announced_window_delta_ &&
	      announced_window_delta_ + sent_init_window <=
	      sent_init_window / 2)
	    {
	      action.
		set_send_stream_update (FlowControlAction::Urgency::
					UPDATE_IMMEDIATELY);
	    }
