...
static uint8_t *
fill_header (uint8_t * out, uint32_t length, uint8_t flags)
{
...
grpc_error *
grpc_chttp2_settings_parser_begin_frame (grpc_chttp2_settings_parser * parser,
...
grpc_error *
grpc_chttp2_settings_parser_parse (void *p, grpc_chttp2_transport * t,
	  ...
	  if (grpc_wire_id_to_setting_id (parser->id, &id))
		  ...
		  || parser->value > sp->max_value)
		{
		  switch (sp->invalid_value_behavior)
		    ...
		    case GRPC_CHTTP2_CLAMP_INVALID_VALUE:
		      parser->value =
			GPR_CLAMP (parser->value, sp->min_value,
				   sp->max_value);
		      break;
		  ...
		  parser->incoming_settings[id] != parser->value)
		{
		  ...
		  if (grpc_http_trace.enabled ()
		    ...
		    {
		      gpr_log (GPR_DEBUG,
			       "%p[%s] adding %d for initial_window change",
