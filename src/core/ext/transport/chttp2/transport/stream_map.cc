...
void grpc_chttp2_stream_map_init(grpc_chttp2_stream_map* map,
  ...
  map->keys = reinterpret_cast<uint32_t*>(
      gpr_malloc(sizeof(uint32_t) * initial_capacity));
  map->values =
      reinterpret_cast<void**>(gpr_malloc(sizeof(void*) * initial_capacity));
  map->count = 0;
  map->free = 0;
...
void grpc_chttp2_stream_map_add(grpc_chttp2_stream_map* map, uint32_t key,
  ...
  if (count == capacity) {
    if (map->free > capacity / 4) {
      ...
      map->keys = keys = reinterpret_cast<uint32_t*>(
          gpr_realloc(keys, capacity * sizeof(uint32_t)));
      map->values = values = reinterpret_cast<void**>(
