...
grpc_error *
grpc_chttp2_goaway_parser_begin_frame (grpc_chttp2_goaway_parser * p,
...
grpc_error *
grpc_chttp2_goaway_parser_parse (void *parser,
				 ...
				 grpc_slice slice, int is_last)
{
  ...
  switch (p->state)
	...
	memcpy (p->debug_data + p->debug_pos, cur, (size_t) (end - cur));
      ...
      if (is_last)
	{
	  grpc_chttp2_add_incoming_goaway (t, (uint32_t) p->error_code,
					   grpc_slice_new (p->debug_data,
							   p->debug_length,
							   gpr_free));
	  p->debug_data = nullptr;
	}
...
void
grpc_chttp2_goaway_append (uint32_t last_stream_id, uint32_t error_code,
