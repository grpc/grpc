...
static void
fill_header (uint8_t * p, uint8_t type, uint32_t id, size_t len,
	     uint8_t flags)
{
...
static void
finish_frame (framer_state * st, int is_header_boundary,
...
/* make sure that the current frame is of the type desired, and has sufficient
   ...
   needed */
static void
ensure_space (framer_state * st, size_t need_bytes)
{
...
static void
inc_filter (uint8_t idx, uint32_t * sum, uint8_t * elems)
{
  elems[idx]++;
  if (elems[idx] < 255)
    {
...
static void
add_header_data (framer_state * st, grpc_slice slice)
{
...
static void
evict_entry (grpc_chttp2_hpack_compressor * c)
{
  c->tail_remote_index++;
...
// table.
static uint32_t
prepare_space_for_new_elem (grpc_chttp2_hpack_compressor * c,
			    size_t elem_size)
{
...
/* dummy function */
static void
add_nothing (grpc_chttp2_hpack_compressor * c, grpc_mdelem elem,
	     size_t elem_size)
{
}
...
// the decoder.
static void
add_key_with_index (grpc_chttp2_hpack_compressor * c,
...
static void
add_elem_with_index (grpc_chttp2_hpack_compressor * c,
		     grpc_mdelem elem, uint32_t new_index)
{
  if (new_index == 0)
    {
...
static void
add_key (grpc_chttp2_hpack_compressor * c, grpc_mdelem elem, size_t elem_size)
{
  uint32_t new_index = prepare_space_for_new_elem (c, elem_size);
  add_key_with_index (c, elem, new_index);
}
...
static void
emit_indexed (grpc_chttp2_hpack_compressor * c, uint32_t elem_index,
	      framer_state * st)
{
...
static wire_value
get_wire_value (grpc_mdelem elem, bool true_binary_enabled)
{
  wire_value wire_val;
  if (grpc_is_binary_header (GRPC_MDKEY (elem)))
      ...
      else
	{
	  ...
	  wire_val.data =
	    grpc_chttp2_base64_encode_and_huffman_compress (GRPC_MDVALUE
							    (elem));
	}
    }
  else
...
static size_t
wire_value_length (wire_value v)
{
  return GPR_SLICE_LENGTH (v.data) + v.insert_null_before_wire_value;
}

static void
add_wire_value (framer_state * st, wire_value v)
{
  if (v.insert_null_before_wire_value)
    *add_tiny_header_data (st, 1) = 0;
  add_header_data (st, v.data);
}

static void
emit_lithdr_incidx (grpc_chttp2_hpack_compressor * c,
		    uint32_t key_index, grpc_mdelem elem, framer_state * st)
{
  ...
  GRPC_CHTTP2_WRITE_VARINT ((uint32_t) len_val, 1, value.huffman_prefix,
			    add_tiny_header_data (st, len_val_len),
			    len_val_len);
  add_wire_value (st, value);
}

static void
emit_lithdr_noidx (grpc_chttp2_hpack_compressor * c,
		   uint32_t key_index, grpc_mdelem elem, framer_state * st)
{
...
static void
emit_lithdr_incidx_v (grpc_chttp2_hpack_compressor * c,
		      ...
		      framer_state * st)
{
  ...
  *add_tiny_header_data (st, 1) = 0x40;
  GRPC_CHTTP2_WRITE_VARINT (len_key, 1, 0x00,
			    add_tiny_header_data (st, len_key_len),
			    len_key_len);
...
static void
emit_lithdr_noidx_v (grpc_chttp2_hpack_compressor * c,
		     uint32_t unused_index, grpc_mdelem elem,
		     framer_state * st)
{
  GPR_ASSERT (unused_index == 0);
...
static void
emit_advertise_table_size_change (grpc_chttp2_hpack_compressor * c,
				  framer_state * st)
{
...
static uint32_t
dynidx (grpc_chttp2_hpack_compressor * c, uint32_t elem_index)
{
...
/* encode an mdelem */
static void
hpack_enc (grpc_chttp2_hpack_compressor * c, grpc_mdelem elem,
	   framer_state * st)
{
  GPR_ASSERT (GRPC_SLICE_LENGTH (GRPC_MDKEY (elem)) > 0);
  if (GRPC_SLICE_START_PTR (GRPC_MDKEY (elem))[0] != ':')
    {				/* regular header */
  ...
  bool key_interned = elem_interned
    || grpc_slice_is_interned (GRPC_MDKEY (elem));
  ...
  if (elem_interned)
    {
      ...
      if (grpc_mdelem_eq (c->entries_elems[HASH_FRAGMENT_2 (elem_hash)], elem)
	  && c->indices_elems[HASH_FRAGMENT_2 (elem_hash)] >
	  c->tail_remote_index)
	{
	  ...
	  emit_indexed (c,
			dynidx (c,
				c->
				indices_elems[HASH_FRAGMENT_2 (elem_hash)]),
			st);
	  return;
	}
	  ...
	  c->tail_remote_index)
	{
	  ...
	  emit_indexed (c,
			dynidx (c,
				c->
				indices_elems[HASH_FRAGMENT_3 (elem_hash)]),
			st);
	  return;
	}
    }
  ...
  if (grpc_slice_eq (c->entries_keys[HASH_FRAGMENT_2 (key_hash)],
		     GRPC_MDKEY (elem)) && indices_key > c->tail_remote_index)
    {
...
static void
deadline_enc (grpc_chttp2_hpack_compressor * c, grpc_millis deadline,
	      framer_state * st)
{
...
static uint32_t
elems_for_bytes (uint32_t bytes)
{
  return (bytes + 31) / 32;
}

void
grpc_chttp2_hpack_compressor_init (grpc_chttp2_hpack_compressor * c)
{
  memset (c, 0, sizeof (*c));
  ...
  c->table_elem_size =
    (uint16_t *) gpr_malloc (sizeof (*c->table_elem_size) *
			     c->cap_table_elems);
...
void
grpc_chttp2_hpack_compressor_set_max_usable_size (grpc_chttp2_hpack_compressor
						  * c,
						  uint32_t max_table_size)
{
  c->max_usable_size = max_table_size;
  grpc_chttp2_hpack_compressor_set_max_table_size (c,
						   GPR_MIN (c->max_table_size,
							    max_table_size));
}

static void
rebuild_elems (grpc_chttp2_hpack_compressor * c, uint32_t new_cap)
{
...
void
grpc_chttp2_hpack_compressor_set_max_table_size (grpc_chttp2_hpack_compressor
						 * c, uint32_t max_table_size)
{
  ...
  if (grpc_http_trace.enabled ())
    {
      gpr_log (GPR_DEBUG, "set max table size from encoder to %d",
	       max_table_size);
    }
}

void
grpc_chttp2_encode_header (grpc_chttp2_hpack_compressor * c,
