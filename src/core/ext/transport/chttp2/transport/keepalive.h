...
#define GRPC_GRPC_KEEPALIVE_LOG VLOG(2)
...
class KeepaliveManager {
 public:
  ...
  void GotData() {
    ...
    GRPC_KEEPALIVE_LOG << "Data received. Setting data_received_ to true";
  ...
  auto WaitForData() {
    return [this]() -> Poll<absl::Status> {
      if (data_received_in_last_cycle_) {
        GRPC_KEEPALIVE_LOG << "WaitForData: Data received. Poll resolved";
        return absl::OkStatus();
      } else {
        GRPC_KEEPALIVE_LOG << "WaitForData: Data not received. Poll pending";
