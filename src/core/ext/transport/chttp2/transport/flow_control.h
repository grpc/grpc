  ...
  grpc
{
  ...
  {
    class
      TrickledCHTTP2;		// to make this a friend
  }				// namespace testing
}				// namespace grpc
...
// transport how quickly the action must take place.
	...
	QUEUE_UPDATE,
      ...
      Urgency
      send_stream_update () const
      {
	return
	  send_stream_update_;
      }
      Urgency
      send_transport_update () const
      {
	return
	  send_transport_update_;
      }
      Urgency
      send_initial_window_update () const
      {
	return
	  send_initial_window_update_;
      }
      Urgency
      send_max_frame_size_update () const
      {
	return
	  send_max_frame_size_update_;
      }
      uint32_t
      initial_window_size () const
      {
	return
	...
	return
      ...
      FlowControlAction &
      set_send_stream_update (Urgency u)
      {
	...
	return *
      ...
      FlowControlAction &
      set_send_transport_update (Urgency u)
      {
	...
	return *
      ...
      FlowControlAction &
      set_send_initial_window_update (Urgency u, uint32_t update)
      {
	...
	return *
      ...
      FlowControlAction &
      set_send_max_frame_size_update (Urgency u, uint32_t update)
      {
	...
	return *this;
      ...
      static const char *
      UrgencyString (Urgency u);
      void
      Trace (grpc_chttp2_transport * t) const;

    private:
      Urgency send_stream_update_ = Urgency::NO_ACTION_NEEDED;
      Urgency
	send_transport_update_ = Urgency::NO_ACTION_NEEDED;
      Urgency
    ...
    private:
      void
      Init (const char *reason, TransportFlowControl * tfc,
	    StreamFlowControl * sfc);
      void
      Finish ();

      const bool
	enabled_ = grpc_flowctl_trace.enabled ();
...
// we abort in any methods which require implementation in the base class.
	...
	abort ();
      ...
      virtual uint32_t
      MaybeSendUpdate (bool writing_anyway)
      {
	abort ();
      }
	...
	abort ();
      ...
      virtual grpc_error *
      RecvData (int64_t incoming_frame_size)
      {
	abort ();
      }
...
// performance.
    class TransportFlowControlDisabled
      final:
      public
      TransportFlowControlBase
    {
...
// to be as performant as possible.
	...
	remote_window_ -= size;
      ...
      grpc_error *
      ValidateRecvData (int64_t incoming_frame_size);
      void
      CommitRecvData (int64_t incoming_frame_size)
      {
	...
	override
      {
	FlowControlTrace
	trace ("  data recv", this, nullptr);
	grpc_error *
	  error = ValidateRecvData (incoming_frame_size);
	...
	override
      {
	FlowControlTrace
	trace ("t updt recv", this, nullptr);
	remote_window_ += size;
      }
	...
	return
      ...
      void
      PreUpdateAnnouncedWindowOverIncomingWindow (int64_t delta)
      {
	if (delta > 0)
	  {
      ...
      void
      PostUpdateAnnouncedWindowOverIncomingWindow (int64_t delta)
      {
	if (delta > 0)
	  {
	    announced_stream_total_over_incoming_window_ += delta;
    ...
    private:
      ...
      TargetLogBdp ();
      double
      SmoothLogBdp (double value);
      FlowControlAction::Urgency DeltaUrgency (int64_t value,
					       grpc_chttp2_setting_id
					       setting_id);

      FlowControlAction
      UpdateAction (FlowControlAction action)
      {
	if (announced_window_ < target_window () / 2)
	  {
	    action.
	      set_send_transport_update (FlowControlAction::Urgency::
					 UPDATE_IMMEDIATELY);
	  }
	return action;
      }

      const grpc_chttp2_transport *const
	t_;
  ...
  /** calculating what we should give for local window:
	...
	announced_stream_total_over_incoming_window_ = 0;
      int64_t
	announced_stream_total_under_incoming_window_ = 0;
...
// we abort in any methods which require implementation in the base class.
	...
	abort ();
      ...
      virtual void
      SentData (int64_t outgoing_frame_size)
      {
	abort ();
      }
	...
	abort ();
      ...
      // ready for more data.
      virtual void
      IncomingByteStreamUpdate (size_t max_size_hint, size_t have_already)
      {
	abort ();
      }
      ...
      remote_window_delta ()
      {
	return remote_window_delta_;
      }
      int64_t
      local_window_delta ()
      {
	return local_window_delta_;
      }
      int64_t
...
// to be as performant as possible.
	...
	override;
      ...
      int64_t
      remote_window_delta () const
      {
	return
	  remote_window_delta_;
      }
      int64_t
      local_window_delta () const
      {
	return
	  local_window_delta_;
      }
      int64_t
      announced_window_delta () const
      {
	return
	  announced_window_delta_;
      }

      const grpc_chttp2_stream *
      stream () const
      {
	return
	  s_;
	...
	tfc_;
      const grpc_chttp2_stream *const
	s_;

      void
      UpdateAnnouncedWindowDelta (TransportFlowControl * tfc, int64_t change)
      {
	tfc->
	  PreUpdateAnnouncedWindowOverIncomingWindow
	  (announced_window_delta_);
