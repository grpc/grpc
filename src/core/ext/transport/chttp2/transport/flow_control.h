...
// Encapsulates a collections of actions the transport needs to take
// with regard to flow control. Each action comes with urgencies that
// tell the transport how quickly the action must take place.
...
class TransportFlowControlBase {
 public:
  ...
  // Is flow control enabled? This is needed in other codepaths like the
  // checks in parsing and in writing.
  virtual bool flow_control_enabled() const = 0;

  // Called to check if the transport needs to send a WINDOW_UPDATE
  // frame
  ...
  // Called to do bookkeeping when a stream owned by this transport
  // sends data on the wire
  virtual void StreamSentData(int64_t /* size */) = 0;

  // Called to do bookkeeping when a stream owned by this transport
  // receives data from the wire. Also does error checking for frame
  // size.
  virtual grpc_error_handle RecvData(
      int64_t /* incoming_frame_size */) = 0;

  // Called to do bookkeeping when we receive a WINDOW_UPDATE frame.
  virtual void RecvUpdate(uint32_t /* size */) = 0;

  // Returns the BdpEstimator held by this object. Caller is responsible
  // for checking for nullptr. TODO(ncteisen): consider fully
  // encapsulating all bdp estimator actions inside TransportFlowControl
  ...
  // Used in certain benchmarks in which we don't want FlowControl to be
  // a factor
...
// Implementation of flow control that does NOTHING. Always returns
// maximum values, never initiates writes, and assumes that the remote
// peer is doing the same. To be used to narrow down on flow control as
// the cause of negative performance.
class TransportFlowControlDisabled final
    : public TransportFlowControlBase {
 public:
  ...
  grpc_error_handle RecvData(
      int64_t /* incoming_frame_size */) override {
    return GRPC_ERROR_NONE;
  }
  void RecvUpdate(uint32_t /* size */) override {}
};

// Implementation of flow control that abides to HTTP/2 spec and
// attempts to be as performant as possible.
class TransportFlowControl final : public TransportFlowControlBase {
 public:
  ...
  // returns an announce if we should send a transport update to our
  // peer, else returns zero; writing_anyway indicates if a write would
  // happen regardless of the send - if it is false and this function
  // returns non-zero, this announce will cause a write to occur
  uint32_t MaybeSendUpdate(bool writing_anyway) override;

  // Reads the flow control data and returns and actionable struct that
  // will tell chttp2 exactly what it needs to do
  FlowControlAction MakeAction() override {
    return UpdateAction(FlowControlAction());
  }

  // Call periodically (at a low-ish rate, 100ms - 10s makes sense)
  // to perform more complex flow control calculations and return an
  // action to let chttp2 change its parameters
  FlowControlAction PeriodicUpdate() override;
  ...
  // See comment above announced_stream_total_over_incoming_window_ for
 ...
 private:
  ...
  /** calculating what we should give for local window:
      ...
      across all streams: this is data that we want to receive right now
     (it has an outstanding read) and the total amount of flow control
     under initial window size across all streams: this is data we've
     read early we want to adjust incoming_window such that:
...
// Fat interface with all methods a stream flow control implementation
// needs to support.
class StreamFlowControlBase {
 public:
  ...
  virtual FlowControlAction UpdateAction(
      FlowControlAction /* action */) {
    abort();
  }

  // Using the protected members, returns an Action for this stream to
  // be taken by the tranport.
  virtual FlowControlAction MakeAction() = 0;

  // Bookkeeping for when data is sent on this stream.
  virtual void SentData(int64_t /* outgoing_frame_size */) = 0;

  // Bookkeeping and error checking for when data is received by this
  // stream.
  virtual grpc_error_handle RecvData(
      int64_t /* incoming_frame_size */) = 0;
  ...
  // Bookkeeping for when a call pulls bytes out of the transport. At
  // this point we consider the data 'used' and can thus let out peer
  // know we are ready for more data.
...
class StreamFlowControl final : public StreamFlowControlBase {
 public:
  ...
  ~StreamFlowControl() override {
    tfc_->PreUpdateAnnouncedWindowOverIncomingWindow(
  ...
  // we have sent data on the wire, we must track this in our
  // bookkeeping for the remote peer's flow control.
  void SentData(int64_t outgoing_frame_size) override {
  ...
  // returns an announce if we should send a stream update to our peer,
  // else returns zero
 ...
 private:
  ...
  void UpdateAnnouncedWindowDelta(TransportFlowControl* tfc,
                                  int64_t change) {
    tfc->PreUpdateAnnouncedWindowOverIncomingWindow(
        announced_window_delta_);
    announced_window_delta_ += change;
    tfc->PostUpdateAnnouncedWindowOverIncomingWindow(
        announced_window_delta_);
  }
};
