...
static bool
input_is_valid (uint8_t * input_ptr, size_t length)
{
  size_t i;
...
bool
grpc_base64_decode_partial (struct grpc_base64_decode_context * ctx)
{
  size_t input_tail;
  ...
  while (ctx->input_end >= ctx->input_cur + 4 &&
    ...
    {
      if (!input_is_valid (ctx->input_cur, 4))
	return false;
  ...
  if (input_tail == 4)
      ...
      if (ctx->input_cur[3] == '=')
	{
	  if (ctx->input_cur[2] == '='
	    ...
	    {
	      if (!input_is_valid (ctx->input_cur, 2))
		return false;
	    ...
	    {
	      if (!input_is_valid (ctx->input_cur, 3))
		return false;
	      *(ctx->output_cur++) = COMPOSE_OUTPUT_BYTE_0 (ctx->input_cur);
	...
	{
	  if (!input_is_valid (ctx->input_cur, input_tail))
	    return false;
	  switch (input_tail)
	    {
	    case 3:
...
grpc_slice
grpc_chttp2_base64_decode (grpc_slice input)
{
...
grpc_slice
grpc_chttp2_base64_decode_with_length (grpc_slice input, size_t output_length)
{
  size_t input_length = GRPC_SLICE_LENGTH (input);
