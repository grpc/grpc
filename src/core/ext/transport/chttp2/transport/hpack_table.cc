...
static uint32_t
entries_for_bytes (uint32_t bytes)
{
...
void
grpc_chttp2_hptbl_init (grpc_chttp2_hptbl * tbl)
{
  size_t
    i;
  ...
  tbl->ents =
    (grpc_mdelem *) gpr_malloc (sizeof (*tbl->ents) * tbl->cap_entries);
...
grpc_mdelem
grpc_chttp2_hptbl_lookup (const grpc_chttp2_hptbl * tbl, uint32_t tbl_index)
{
...
{
  grpc_mdelem
    first_ent = tbl->ents[tbl->first_ent];
  size_t
    elem_bytes = GRPC_SLICE_LENGTH (GRPC_MDKEY (first_ent)) +
...
static void
rebuild_ents (grpc_chttp2_hptbl * tbl, uint32_t new_cap)
{
  grpc_mdelem *
    ents = (grpc_mdelem *) gpr_malloc (sizeof (*ents) * new_cap);
  uint32_t
    i;
...
void
grpc_chttp2_hptbl_set_max_bytes (grpc_chttp2_hptbl * tbl, uint32_t max_bytes)
{
...
grpc_error *
grpc_chttp2_hptbl_set_current_table_size (grpc_chttp2_hptbl * tbl,
					  uint32_t bytes)
{
  ...
  if (tbl->max_entries > tbl->cap_entries)
    ...
    {
      uint32_t
	new_cap = GPR_MAX (tbl->max_entries, 16u);
...
grpc_chttp2_hptbl_add (grpc_chttp2_hptbl * tbl, grpc_mdelem md)
{
  ...
  size_t
    elem_bytes = GRPC_SLICE_LENGTH (GRPC_MDKEY (md)) +
    GRPC_SLICE_LENGTH (GRPC_MDVALUE (md)) + GRPC_CHTTP2_HPACK_ENTRY_OVERHEAD;
...
grpc_chttp2_hptbl_find (const grpc_chttp2_hptbl * tbl, grpc_mdelem md)
{
  ...
  for (i = 0; i < GRPC_CHTTP2_LAST_STATIC_ENTRY; i++)
    {
      grpc_mdelem
	ent = tbl->static_ents[i];
      if (!grpc_slice_eq (GRPC_MDKEY (md), GRPC_MDKEY (ent)))
	continue;
      r.index = i + 1u;
  ...
  for (i = 0; i < tbl->num_ents; i++)
    {
      uint32_t
	idx = (uint32_t) (tbl->num_ents - i + GRPC_CHTTP2_LAST_STATIC_ENTRY);
      grpc_mdelem
