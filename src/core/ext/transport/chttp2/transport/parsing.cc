...
static grpc_error *init_window_update_frame_parser (grpc_chttp2_transport *
						    t);
...
grpc_error *
grpc_chttp2_perform_read (grpc_chttp2_transport * t, grpc_slice slice)
{
  ...
  switch (t->deframe_state)
      ...
      while (cur != end && t->deframe_state != GRPC_DTS_FH_0)
	{
	  if (*cur != GRPC_CHTTP2_CLIENT_CONNECT_STRING[t->deframe_state])
	    {
	      ...
	      gpr_asprintf (&msg,
			    ...
			    "at byte %d",
			    GRPC_CHTTP2_CLIENT_CONNECT_STRING[t->
							      deframe_state],
			    (int) (uint8_t)
			    GRPC_CHTTP2_CLIENT_CONNECT_STRING[t->
							      deframe_state],
	  ...
	  ++cur;
	  t->deframe_state =
	    (grpc_chttp2_deframe_transport_state) (1 +
						   (int) t->deframe_state);
	}
      if (cur == end)
	{
...
static grpc_error *
init_frame_parser (grpc_chttp2_transport * t)
{
  if (t->is_first_frame &&
      t->incoming_frame_type != GRPC_CHTTP2_FRAME_SETTINGS)
    {
      char *msg;
      gpr_asprintf (&msg,
		    "Expected SETTINGS frame as the first frame, got frame type %d",
  ...
  if (t->expect_continuation_stream_id != 0)
      ...
      if (t->incoming_frame_type != GRPC_CHTTP2_FRAME_CONTINUATION)
	{
	  char *msg;
	  gpr_asprintf (&msg,
			"Expected CONTINUATION frame, got frame type %02x",
			t->incoming_frame_type);
	  grpc_error *err = GRPC_ERROR_CREATE_FROM_COPIED_STRING (msg);
	  gpr_free (msg);
	  return err;
      ...
      if (t->expect_continuation_stream_id != t->incoming_stream_id)
	{
	  ...
	  gpr_asprintf (&msg,
			...
			t->expect_continuation_stream_id,
...
static grpc_error *
skip_parser (void *parser, grpc_chttp2_transport * t,
...
static grpc_error *
init_skip_frame_parser (grpc_chttp2_transport * t, int is_header)
{
  if (is_header)
    {
...
void
grpc_chttp2_parsing_become_skip_parser (grpc_chttp2_transport * t)
{
  init_skip_frame_parser (t, t->parser == grpc_chttp2_header_parser_parse);
}

static grpc_error *
init_data_frame_parser (grpc_chttp2_transport * t)
{
  ...
  if (err == GRPC_ERROR_NONE)
    {
      err =
	grpc_chttp2_data_parser_begin_frame (&s->data_parser,
  ...
  if (err == GRPC_ERROR_NONE)
      ...
      if (s != nullptr)
	{
      ...
      grpc_slice_buffer_add (&t->qbuf,
			     grpc_chttp2_rst_stream_create (t->
							    incoming_stream_id,
...
static void
on_initial_header (void *tp, grpc_mdelem md)
{
  ...
  if (grpc_slice_eq (GRPC_MDKEY (md), GRPC_MDSTR_GRPC_TIMEOUT))
	...
	static_cast <
	grpc_millis * >(grpc_mdelem_get_user_data (md, free_timeout));
      grpc_millis timeout;
	  ...
	  if (GRPC_MDELEM_IS_INTERNED (md))
	    {
	      /* store the result */
	      cached_timeout =
		(grpc_millis *) gpr_malloc (sizeof (grpc_millis));
	...
	{
	  grpc_chttp2_incoming_metadata_buffer_set_deadline (&s->
							     metadata_buffer
							     [0],
							     grpc_core::
							     ExecCtx::Get ()->
	...
	s->metadata_buffer[0].size + GRPC_MDELEM_LENGTH (md);
      ...
      if (new_size > metadata_size_limit)
	{
	  gpr_log (GPR_DEBUG,
		   ...
		   " vs. %" PRIuPTR ")", new_size, metadata_size_limit);
	...
	{
	  grpc_error *error =
	    grpc_chttp2_incoming_metadata_buffer_add (&s->metadata_buffer[0],
						      md);
...
static void
on_trailing_header (void *tp, grpc_mdelem md)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) tp;
  grpc_chttp2_stream *s = t->incoming_stream;
  ...
  const size_t new_size =
    s->metadata_buffer[1].size + GRPC_MDELEM_LENGTH (md);
  const size_t metadata_size_limit =
    t->
    settings[GRPC_ACKED_SETTINGS][GRPC_CHTTP2_SETTINGS_MAX_HEADER_LIST_SIZE];
  if (new_size > metadata_size_limit)
    {
      gpr_log (GPR_DEBUG,
...
static grpc_error *
init_header_frame_parser (grpc_chttp2_transport * t, int is_continuation)
{
  uint8_t is_eoh =
  ...
  if (s == nullptr)
    {
      if (is_continuation)
	{
	  GRPC_CHTTP2_IF_TRACING (gpr_log (GPR_ERROR,
      ...
      if (s == nullptr)
	{
	  GRPC_CHTTP2_IF_TRACING (gpr_log
				  (GPR_ERROR,
				   "grpc_chttp2_stream not accepted"));
	  return init_skip_frame_parser (t, 1);
	}
    }
  else
  ...
  switch (s->header_frames_received)
	...
	{
	  GRPC_CHTTP2_IF_TRACING (gpr_log
				  (GPR_INFO, "parsing Trailers-Only"));
    ...
    case 1:
      GRPC_CHTTP2_IF_TRACING (gpr_log
			      (GPR_INFO, "parsing trailing_metadata"));
      t->hpack_parser.on_header = on_trailing_header;
      s->received_trailing_metadata = true;
      break;
    case 2:
...
static grpc_error *
init_window_update_frame_parser (grpc_chttp2_transport * t)
{
  grpc_error *err =
    grpc_chttp2_window_update_parser_begin_frame (&t->simple.window_update,
						  t->incoming_frame_size,
...
static grpc_error *
init_ping_parser (grpc_chttp2_transport * t)
{
  grpc_error *err =
    grpc_chttp2_ping_parser_begin_frame (&t->simple.ping,
					 t->incoming_frame_size,
					 t->incoming_frame_flags);
  if (err != GRPC_ERROR_NONE)
    return err;
...
static grpc_error *
init_rst_stream_parser (grpc_chttp2_transport * t)
{
  grpc_error *err =
    grpc_chttp2_rst_stream_parser_begin_frame (&t->simple.rst_stream,
					       t->incoming_frame_size,
					       t->incoming_frame_flags);
  if (err != GRPC_ERROR_NONE)
    return err;
...
static grpc_error *
init_goaway_parser (grpc_chttp2_transport * t)
{
  grpc_error *err =
    grpc_chttp2_goaway_parser_begin_frame (&t->goaway_parser,
					   t->incoming_frame_size,
					   t->incoming_frame_flags);
  if (err != GRPC_ERROR_NONE)
    return err;
...
static grpc_error *
init_settings_frame_parser (grpc_chttp2_transport * t)
{
  if (t->incoming_stream_id != 0)
    {
      return
  ...
  grpc_error *err =
    grpc_chttp2_settings_parser_begin_frame (&t->simple.settings,
					     t->incoming_frame_size,
					     t->incoming_frame_flags,
  ...
  if (t->incoming_frame_flags & GRPC_CHTTP2_FLAG_ACK)
    {
      memcpy (t->settings[GRPC_ACKED_SETTINGS],
	      t->settings[GRPC_SENT_SETTINGS],
	      GRPC_CHTTP2_NUM_SETTINGS * sizeof (uint32_t));
      grpc_chttp2_hptbl_set_max_bytes (&t->hpack_parser.table,
				       t->
				       settings[GRPC_ACKED_SETTINGS]
...
static grpc_error *
parse_frame_slice (grpc_chttp2_transport * t, grpc_slice slice, int is_last)
{
  grpc_chttp2_stream *s = t->incoming_stream;
