...
// GRPC Header
namespace {
ValueOrHttp2Status<uint32_t> GrpcMessageHeaderFlagsToMessageFlags(
    const uint8_t flags) {
  switch (flags) {
...
uint8_t MessageFlagsToGrpcMessageHeaderFlags(const uint32_t flags) {
...
ValueOrHttp2Status<GrpcMessageHeader> ExtractGrpcHeader(SliceBuffer& payload) {
  ...
  ValueOrHttp2Status<uint32_t> message_flags =
      GrpcMessageHeaderFlagsToMessageFlags(buffer[0]);
...
void AppendGrpcHeaderToSliceBuffer(SliceBuffer& payload, const uint32_t flags,
  ...
  frame_hdr[0] = MessageFlagsToGrpcMessageHeaderFlags(flags);
