...
/* length of a value that needs varint tail encoding (it's bigger than
   can be bitpacked into the opcode byte) - returned value includes the
   length of the opcode byte */
...
/* maximum value that can be bitpacked with the opcode if the opcode has
   a prefix of length prefix_bits */
...
#define GRPC_CHTTP2_WRITE_VARINT(n, prefix_bits, prefix_or, target, \
                                 length)                            \
  do {                                                              \
    uint8_t* tgt = target;                                          \
    if ((length) == 1u) {                                           \
      ...
      (tgt)[0] = (prefix_or) |                                      \
                 (uint8_t)GRPC_CHTTP2_MAX_IN_PREFIX(prefix_bits);   \
