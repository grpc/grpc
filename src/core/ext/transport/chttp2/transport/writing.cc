...
static void
add_to_write_list (grpc_chttp2_write_cb ** list, grpc_chttp2_write_cb * cb)
{
  cb->next = *list;
  *list = cb;
}
...
static void
finish_write_cb (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
...
static void
maybe_initiate_ping (grpc_chttp2_transport * t)
{
  ...
  if (t->ping_state.pings_before_data_required == 0 &&
	...
	{
	  gpr_log (GPR_DEBUG,
		   "%s: Ping delayed [%p]: too many recent pings: %d/%d",
		   t->is_client ? "CLIENT" : "SERVER", t->peer_string,
  ...
  if (next_allowed_ping > now)
      ...
      if (!t->ping_state.is_delayed_ping_timer_set)
	{
	  ...
	  grpc_timer_init (&t->ping_state.delayed_ping_timer,
...
static bool
update_list (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
...
static void
report_stall (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
	      const char *staller)
{
  gpr_log (GPR_DEBUG,
	   ...
	   s->flow_controlled_buffer.length, s->compressed_data_buffer.length,
	   s->flow_controlled_bytes_flowed,
	   t->
	   settings[GRPC_ACKED_SETTINGS]
	   [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
	   t->flow_control->remote_window (), (uint32_t) GPR_MAX (0,
								  s->
								  flow_control->
								  remote_window_delta
								  () +
...
static bool
stream_ref_if_not_destroyed (gpr_refcount * r)
{
  gpr_atm count;
  do
    {
...
static uint32_t
target_write_size (grpc_chttp2_transport * t)
{
  return 1024 * 1024;
}
...
static bool
is_default_initial_metadata (grpc_metadata_batch * initial_metadata)
{
...
namespace
{
  ...
  public:
      ...
      if (t_->dirtied_local_settings && !t_->sent_local_settings)
	{
	  grpc_slice_buffer_add (&t_->outbuf,
				 grpc_chttp2_settings_create (t_->
							      settings
							      [GRPC_SENT_SETTINGS],
							      t_->
							      settings
    ...
    {
      grpc_chttp2_hpack_compressor_set_max_table_size (&t_->hpack_compressor,
						       t_->
						       settings
						       [GRPC_PEER_SETTINGS]
    ...
    void IncInitialMetadataWrites ()
    {
      ++initial_metadata_writes_;
    }
    void IncWindowUpdateWrites ()
    {
      ++flow_control_writes_;
    }
    void IncMessageWrites ()
    {
      ++message_writes_;
    }
    void IncTrailingMetadataWrites ()
    {
      ++trailing_metadata_writes_;
    }

    void NoteScheduledResults ()
    {
      result_.early_results_scheduled = true;
    }

    grpc_chttp2_transport *transport () const
    {
      return t_;
    }
  ...
  public:
    DataSendContext (WriteContext * write_context, grpc_chttp2_transport * t,
		     grpc_chttp2_stream * s):write_context_ (write_context),
      t_ (t), s_ (s), sending_bytes_before_ (s_->sending_bytes)
    {
    }

    uint32_t stream_remote_window () const
    {
      return (uint32_t) GPR_MAX (0, s_->flow_control->remote_window_delta () +
    ...
    {
      return (uint32_t) GPR_MIN (t_->
				 settings[GRPC_PEER_SETTINGS]
				 [GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE],
				 GPR_MIN (stream_remote_window (),
					  t_->flow_control->
					  remote_window ()));
    }

    bool AnyOutgoing () const
    {
	  ...
	  && s_->stream_compression_ctx != nullptr)
	{
	  ...
	  grpc_stream_compression_context_destroy (s_->
	...
	grpc_metadata_batch_is_empty (s_->send_trailing_metadata);
      grpc_chttp2_encode_data (s_->id, &s_->compressed_data_buffer,
			       send_bytes, is_last_frame_,
      ...
      if (s_->stream_compression_ctx == nullptr)
	{
	  s_->stream_compression_ctx =
	    grpc_stream_compression_context_create (s_->
      ...
      if (!grpc_stream_compress (s_->stream_compression_ctx,
				 ...
				 &s_->compressed_data_buffer, nullptr,
				 MAX_SIZE_T,
  ...
  {
  public:
    StreamWriteContext (WriteContext * write_context,
			grpc_chttp2_stream *
			s):write_context_ (write_context),
      t_ (write_context->transport ()), s_ (s)
    {
      GRPC_CHTTP2_IF_TRACING (gpr_log
			      (GPR_DEBUG,
			       "W:%p %s[%d] im-(sent,send)=(%d,%d) announce=%d",
			       t_, t_->is_client ? "CLIENT" : "SERVER", s->id,
    ...
    void FlushInitialMetadata ()
    {
      ...
      if (s_->sent_initial_metadata)
	return;
      if (s_->send_initial_metadata == nullptr)
	return;
      ...
      else
	{
	  grpc_encode_header_options hopt = {
	    s_->id,		// stream_id
	    false,		// is_eof
	    t_->settings[GRPC_PEER_SETTINGS][GRPC_CHTTP2_SETTINGS_GRPC_ALLOW_TRUE_BINARY_METADATA] != 0,	// use_true_binary_metadata
      ...
      write_context_->NoteScheduledResults ();
      grpc_chttp2_complete_closure_step (t_, s_,
					 &s_->send_initial_metadata_finished,
					 GRPC_ERROR_NONE,
	...
	return;
      ...
      grpc_slice_buffer_add (&t_->outbuf,
			     grpc_chttp2_window_update_create (s_->id,
							       stream_announce,
							       &s_->stats.
							       outgoing));
      write_context_->ResetPingRecvClock ();
	...
	return;
      ...
      if (s_->send_trailing_metadata == nullptr)
	return;
      if (s_->fetching_send_message != nullptr)
	return;
      if (s_->flow_controlled_buffer.length != 0)
	return;
      if (s_->compressed_data_buffer.length != 0)
	return;
      ...
      GRPC_CHTTP2_IF_TRACING (gpr_log
			      (GPR_INFO, "sending trailing_metadata"));
      if (grpc_metadata_batch_is_empty (s_->send_trailing_metadata))
	{
	  grpc_chttp2_encode_data (s_->id, &s_->flow_controlled_buffer, 0,
				   true, &s_->stats.outgoing, &t_->outbuf);
	}
      else
	{
	  grpc_encode_header_options hopt = {
	  ...
	  grpc_chttp2_encode_header (&t_->hpack_compressor,
				     ...
				     s_->send_trailing_metadata, &hopt,
				     &t_->outbuf);
	}
      ...
      write_context_->NoteScheduledResults ();
      grpc_chttp2_complete_closure_step (t_, s_,
					 &s_->send_trailing_metadata_finished,
					 GRPC_ERROR_NONE,
					 "send_trailing_metadata_finished");
    }

    bool stream_became_writable ()
    {
  ...
  private:
	...
	{
	  grpc_slice_buffer_add (&t_->outbuf,
				 grpc_chttp2_rst_stream_create (s_->id,
								GRPC_HTTP2_NO_ERROR,
								&s_->stats.
								outgoing));
	}
...
grpc_chttp2_begin_write (grpc_chttp2_transport * t)
{
  WriteContext ctx (t);
  ctx.FlushSettings ();
  ctx.FlushPingAcks ();
...
void
grpc_chttp2_end_write (grpc_chttp2_transport * t, grpc_error * error)
{
  ...
  while (grpc_chttp2_list_pop_writing_stream (t, &s))
	...
	{
	  update_list (t, s, (int64_t) s->sending_bytes,
