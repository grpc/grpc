...
void ContextList::Append(ContextList** head, grpc_chttp2_stream* s) {
  if (get_copied_context_fn_g == nullptr || write_timestamps_callback_g == nullptr) {
    return;
  }
...
void ContextList::Execute(void* arg, grpc_core::Timestamps* ts, grpc_error_handle error) {
...
void grpc_http2_set_write_timestamps_callback(void (*fn)(void*, grpc_core::Timestamps*,
                                                         grpc_error_handle error)) {
  write_timestamps_callback_g = fn;
}

void grpc_http2_set_fn_get_copied_context(void* (*fn)(void*)) { get_copied_context_fn_g = fn; }
} /* namespace grpc_core */
