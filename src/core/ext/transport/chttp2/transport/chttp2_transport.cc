...
void grpc_chttp2_cancel_stream(
  ...
  // We need to destroy the metadata before we potentially destroy the stream
  // (and its arena), which can happen in grpc_chttp2_mark_stream_closed.
...
static void close_from_api(
  ...
  raw_encoder.Encode(grpc_core::GrpcMessageMetadata(),
  ...
  // Capture stream properties before marking it closed, as it might be
  // destroyed by grpc_chttp2_mark_stream_closed.
  bool sent_initial_metadata = s->sent_initial_metadata;
  uint32_t id = s->id;
  auto remove_stream_handle = grpc_chttp2_mark_stream_closed(t, s, 1, 1, error);
  grpc_core::MaybeTarpit(
      t, tarpit,
      [sent_initial_metadata, id,
       remove_stream_handle = std::move(remove_stream_handle),
       raw_encoder = std::move(raw_encoder)](grpc_chttp2_transport* t) mutable {
        grpc_slice hdr;
