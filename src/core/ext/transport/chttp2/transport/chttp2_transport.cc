  ...
  grpc_core::MaybeTarpit(
      ...
      [error = std::move(error),
       ...
       send_trailing_metadata = std::move(send_trailing_metadata)](
          grpc_chttp2_transport* t) mutable {
        grpc_slice hdr;
        ...
        // TODO(akshitpatel): Verify if this holds.
        GRPC_DCHECK(send_trailing_metadata == nullptr ||
                    send_trailing_metadata->get(grpc_core::GrpcStatusMetadata())
                            .value_or(GRPC_STATUS_UNKNOWN) == grpc_status);
        GRPC_DCHECK(send_trailing_metadata == nullptr ||
                    send_trailing_metadata
                            ->get_pointer(grpc_core::GrpcMessageMetadata())
                            ->as_string_view() == message);
