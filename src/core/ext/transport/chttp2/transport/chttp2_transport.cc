...
static void
incoming_byte_stream_publish_error (grpc_chttp2_incoming_byte_stream * bs,
				    grpc_error * error);
static void incoming_byte_stream_unref (grpc_chttp2_incoming_byte_stream *
					bs);
...
static void
destruct_transport (grpc_chttp2_transport * t)
{
  size_t i;
...
#ifndef NDEBUG
void
grpc_chttp2_unref_transport (grpc_chttp2_transport * t, const char *reason,
  ...
  if (grpc_trace_chttp2_refcount.enabled ())
    {
      ...
      gpr_log (GPR_DEBUG,
	       "chttp2:unref:%p %" PRIdPTR "->%" PRIdPTR " %s [%s:%d]", t,
	       val, val - 1, reason, file, line);
    }
  if (!gpr_unref (&t->refs))
    return;
...
void
grpc_chttp2_ref_transport (grpc_chttp2_transport * t, const char *reason,
			   const char *file, int line)
{
  if (grpc_trace_chttp2_refcount.enabled ())
    {
      gpr_atm val = gpr_atm_no_barrier_load (&t->refs.count);
      gpr_log (GPR_DEBUG,
	       "chttp2:  ref:%p %" PRIdPTR "->%" PRIdPTR " %s [%s:%d]", t,
	       val, val + 1, reason, file, line);
    }
  gpr_ref (&t->refs);
}
#else
void
grpc_chttp2_unref_transport (grpc_chttp2_transport * t)
{
  if (!gpr_unref (&t->refs))
    return;
  destruct_transport (t);
...
void
grpc_chttp2_ref_transport (grpc_chttp2_transport * t)
{
  gpr_ref (&t->refs);
}
#endif
...
static void
init_transport (grpc_chttp2_transport * t,
		...
		grpc_endpoint * ep, bool is_client)
{
  ...
  if (is_client)
    {
      ...
      queue_setting_update (t, GRPC_CHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS,
			    0);
    }
  ...
  if (t->is_client)
	...
	? GRPC_MILLIS_INF_FUTURE : g_default_client_keepalive_time_ms;
      t->keepalive_timeout = g_default_client_keepalive_timeout_ms == INT_MAX
	? GRPC_MILLIS_INF_FUTURE : g_default_client_keepalive_timeout_ms;
	...
	? GRPC_MILLIS_INF_FUTURE : g_default_server_keepalive_time_ms;
      t->keepalive_timeout = g_default_server_keepalive_timeout_ms == INT_MAX
	? GRPC_MILLIS_INF_FUTURE : g_default_server_keepalive_timeout_ms;
      t->keepalive_permit_without_calls =
      ...
      for (i = 0; i < channel_args->num_args; i++)
	{
	  if (0 == strcmp (channel_args->args[i].key,
		...
		grpc_channel_arg_get_integer (&channel_args->args[i],
	      ...
	      if (value >= 0)
		{
		  grpc_chttp2_hpack_compressor_set_max_usable_size (&t->
								    hpack_compressor,
								    (uint32_t)
								    value);
		}
	    ...
	    {
	      t->write_buffer_size =
		(uint32_t) grpc_channel_arg_get_integer (&channel_args->
							 args[i],
							 {
							 0, 0,
	      ...
	      if (channel_args->args[i].type != GRPC_ARG_STRING)
		{
		  gpr_log (GPR_ERROR, "%s should be a string",
		       ...
		       strcmp (channel_args->args[i].value.string, "blend"))
		{
		  t->opt_target = GRPC_CHTTP2_OPTIMIZE_FOR_LATENCY;
		}
	      else if (0 ==
		       strcmp (channel_args->args[i].value.string, "latency"))
		{
		  t->opt_target = GRPC_CHTTP2_OPTIMIZE_FOR_LATENCY;
		}
	      else if (0 ==
		       strcmp (channel_args->args[i].value.string,
			       "throughput"))
		{
		...
		{
		  gpr_log (GPR_ERROR,
			   "%s value '%s' unknown, assuming 'blend'",
			   GRPC_ARG_OPTIMIZATION_TARGET,
	      ...
	      for (j = 0; j < (int) GPR_ARRAY_SIZE (settings_map); j++)
		{
		  if (0 == strcmp (channel_args->args[i].key,
			...
			{
			  int value =
			    grpc_channel_arg_get_integer (&channel_args->
							  args[i],
							  settings_map[j].
							  integer_options);
			  if (value >= 0)
...
static void
destroy_transport_locked (void *tp, grpc_error * error)
{
...
destroy_transport (grpc_transport * gt)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE (destroy_transport_locked, t,
					   grpc_combiner_scheduler (t->
								    combiner)),
		      GRPC_ERROR_NONE);
}

static void
close_transport_locked (grpc_chttp2_transport * t, grpc_error * error)
{
  ...
  if (t->closed_with_error == GRPC_ERROR_NONE)
      ...
      if (t->write_state != GRPC_CHTTP2_WRITE_STATE_IDLE)
	{
	  ...
	  t->close_transport_on_writes_finished =
	    grpc_error_add_child (t->close_transport_on_writes_finished,
				  error);
	  return;
	}
...
#ifndef NDEBUG
void
grpc_chttp2_stream_ref (grpc_chttp2_stream * s, const char *reason)
{
  grpc_stream_ref (s->refcount, reason);
}

void
grpc_chttp2_stream_unref (grpc_chttp2_stream * s, const char *reason)
{
  grpc_stream_unref (s->refcount, reason);
}
#else
void
grpc_chttp2_stream_ref (grpc_chttp2_stream * s)
{
  grpc_stream_ref (s->refcount);
}

void
grpc_chttp2_stream_unref (grpc_chttp2_stream * s)
{
	     ...
	     gpr_arena * arena)
{
  ...
  if (t->flow_control->flow_control_enabled ())
	...
	(static_cast <
	 grpc_core::chttp2::TransportFlowControl * >(t->flow_control.get ()),
	 s);
    }
  else
    {
...
static void
destroy_stream_locked (void *sp, grpc_error * error)
{
		...
		grpc_closure * then_schedule_closure)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT
		      (&s->destroy_stream, destroy_stream_locked, s,
		       grpc_combiner_scheduler (t->combiner)),
		      GRPC_ERROR_NONE);
  GPR_TIMER_END ("destroy_stream", 0);
}
...
grpc_chttp2_stream *
grpc_chttp2_parsing_lookup_stream (grpc_chttp2_transport * t, uint32_t id)
{
  return (grpc_chttp2_stream *) grpc_chttp2_stream_map_find (&t->stream_map,
							     id);
}

grpc_chttp2_stream *
grpc_chttp2_parsing_accept_stream (grpc_chttp2_transport * t, uint32_t id)
{
  ...
  t->channel_callback.accept_stream (t->channel_callback.
				     accept_stream_user_data, &t->base,
				     (void *) (uintptr_t) id);
...
static const char *
write_state_name (grpc_chttp2_write_state st)
{
  switch (st)
    {
...
static void
set_write_state (grpc_chttp2_transport * t,
		 grpc_chttp2_write_state st, const char *reason)
{
  GRPC_CHTTP2_IF_TRACING (gpr_log
			  (GPR_DEBUG, "W:%p %s state %s -> %s [%s]", t,
...
static void
inc_initiate_write_reason (grpc_chttp2_initiate_write_reason reason)
{
  switch (reason)
    {
...
void
grpc_chttp2_initiate_write (grpc_chttp2_transport * t,
...
void
grpc_chttp2_mark_stream_writable (grpc_chttp2_transport * t,
				  grpc_chttp2_stream * s)
{
  if (t->closed_with_error == GRPC_ERROR_NONE &&
...
write_scheduler (grpc_chttp2_transport * t,
		 bool early_results_scheduled, bool partial_write)
{
...
static const char *
begin_writing_desc (bool partial, bool inlined)
{
...
static void
write_action_begin_locked (void *gt, grpc_error * error_ignored)
{
      ...
      if (scheduler != grpc_schedule_on_exec_ctx)
	{
      ...
      set_write_state (t,
		       ...
		       begin_writing_desc (r.partial,
					   ...
					   grpc_schedule_on_exec_ctx));
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT
			  (&t->write_action, write_action, t, scheduler),
			  GRPC_ERROR_NONE);
    }
  else
    {
...
static void
write_action_end_locked (void *tp, grpc_error * error)
{
  ...
  switch (t->write_state)
    ...
    case GRPC_CHTTP2_WRITE_STATE_WRITING_WITH_MORE:
      ...
      GRPC_CHTTP2_REF_TRANSPORT (t, "writing");
      GRPC_CLOSURE_RUN (GRPC_CLOSURE_INIT (&t->write_action_begin_locked,
					   write_action_begin_locked, t,
					   grpc_combiner_finally_scheduler
					   (t->combiner)), GRPC_ERROR_NONE);
      break;
...
static void
queue_setting_update (grpc_chttp2_transport * t,
		      grpc_chttp2_setting_id id, uint32_t value)
{
...
void
grpc_chttp2_add_incoming_goaway (grpc_chttp2_transport * t,
				 ...
				 grpc_slice goaway_text)
{
  ...
  t->goaway_error =
    grpc_error_set_str (grpc_error_set_int
			(GRPC_ERROR_CREATE_FROM_STATIC_STRING
			 ("GOAWAY received"), GRPC_ERROR_INT_HTTP2_ERROR,
			 (intptr_t) goaway_error), GRPC_ERROR_STR_RAW_BYTES,
			goaway_text);
  ...
  if (t->is_client && goaway_error == GRPC_HTTP2_ENHANCE_YOUR_CALM &&
	...
	INT_MAX /
	KEEPALIVE_TIME_BACKOFF_MULTIPLIER ? GRPC_MILLIS_INF_FUTURE
...
static void
maybe_start_some_streams (grpc_chttp2_transport * t)
{
  grpc_chttp2_stream *s;
...
static grpc_closure *
add_closure_barrier (grpc_closure * closure)
{
...
static void
null_then_run_closure (grpc_closure ** closure, grpc_error * error)
{
...
void
grpc_chttp2_complete_closure_step (grpc_chttp2_transport * t,
				   grpc_chttp2_stream * s,
  ...
  if (grpc_http_trace.enabled ())
    {
      ...
      gpr_log (GPR_DEBUG,
	       ...
	       "write_state=%s",
	       t, closure,
	       (int) (closure->next_data.scratch /
		      CLOSURE_BARRIER_FIRST_REF_BIT),
	       (int) (closure->next_data.scratch %
		      CLOSURE_BARRIER_FIRST_REF_BIT), desc, errstr,
	       write_state_name (t->write_state));
    }
  if (error != GRPC_ERROR_NONE)
    {
      if (closure->error_data.error == GRPC_ERROR_NONE)
	{
	  ...
	  closure->error_data.error =
	    grpc_error_set_str (closure->error_data.error,
				...
				grpc_slice_from_copied_string (t->
							       peer_string));
	}
      closure->error_data.error =
...
static bool
contains_non_ok_status (grpc_metadata_batch * batch)
{
...
static void
maybe_become_writable_due_to_send_msg (grpc_chttp2_transport * t,
				       grpc_chttp2_stream * s)
{
...
static void
add_fetched_slice_locked (grpc_chttp2_transport * t, grpc_chttp2_stream * s)
{
...
static void
continue_fetching_send_locked (grpc_chttp2_transport * t,
			       grpc_chttp2_stream * s)
{
  for (;;)
    {
      ...
      if (s->fetched_send_message_length == s->fetching_send_message->length)
	{
	  ...
	  if (notify_offset <= s->flow_controlled_bytes_written)
	    {
	      grpc_chttp2_complete_closure_step (t, s,
						 &s->
		...
		? &s->on_write_finished_cbs : &s->on_flow_controlled_cbs;
	      cb->next = *list;
	      *list = cb;
	    }
	...
	{
	  grpc_error *error =
	    grpc_byte_stream_pull (s->fetching_send_message,
				   &s->fetching_slice);
	  if (error != GRPC_ERROR_NONE)
	    {
	      grpc_byte_stream_destroy (s->fetching_send_message);
...
static void
complete_fetch_locked (void *gs, grpc_error * error)
{
  grpc_chttp2_stream *s = (grpc_chttp2_stream *) gs;
  grpc_chttp2_transport *t = s->t;
  if (error == GRPC_ERROR_NONE)
...
static void
log_metadata (const grpc_metadata_batch * md_batch, uint32_t id,
...
static void
perform_stream_op_locked (void *stream_op, grpc_error * error_ignored)
{
  ...
  grpc_chttp2_stream *s =
    (grpc_chttp2_stream *) op->handler_private.extra_arg;
  ...
  if (grpc_http_trace.enabled ())
    {
      ...
      gpr_log (GPR_DEBUG, "perform_stream_op_locked: %s; on_complete = %p",
	       str, op->on_complete);
      gpr_free (str);
      if (op->send_initial_metadata)
	{
	  log_metadata (op_payload->send_initial_metadata.
			send_initial_metadata, s->id, t->is_client, true);
	}
      if (op->send_trailing_metadata)
	{
	  log_metadata (op_payload->send_trailing_metadata.
			send_trailing_metadata, s->id, t->is_client, false);
	}
    }
  ...
  if (op->send_message)
    {
      ...
      GRPC_STATS_INC_HTTP2_SEND_MESSAGE_SIZE (op->payload->send_message.
					      send_message->length);
      on_complete->next_data.scratch |= CLOSURE_BARRIER_MAY_COVER_WRITE;
      ...
      else
	{
	  GPR_ASSERT (s->fetching_send_message == nullptr);
	  uint8_t *frame_hdr =
	    grpc_slice_buffer_tiny_add (&s->flow_controlled_buffer,
	  ...
	  s->next_message_end_offset = s->flow_controlled_bytes_written +
	    (int64_t) s->flow_controlled_buffer.length + (int64_t) len;
      ...
      if (s->id != 0)
	{
	  if (!s->read_closed)
		...
		IncomingByteStreamUpdate (GRPC_HEADER_SIZE_IN_BYTES,
					  already_received);
	      grpc_chttp2_act_on_flowctl_action (s->flow_control->
						 MakeAction (), t, s);
	    }
	}
		   ...
		   grpc_transport_stream_op_batch * op)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT
		      (&op->handler_private.closure, perform_stream_op_locked,
...
static void
cancel_pings (grpc_chttp2_transport * t, grpc_error * error)
{
...
static void
send_ping_locked (grpc_chttp2_transport * t,
...
static void
retry_initiate_ping_locked (void *tp, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) tp;
  t->ping_state.is_delayed_ping_timer_set = false;
  if (error == GRPC_ERROR_NONE)
...
void
grpc_chttp2_ack_ping (grpc_chttp2_transport * t, uint64_t id)
{
  grpc_chttp2_ping_queue *pq = &t->ping_queue;
  if (pq->inflight_id != id)
    {
      char *from = grpc_endpoint_get_peer (t->ep);
      gpr_log (GPR_DEBUG, "Unknown ping response from %s: %" PRIx64, from,
	       id);
      gpr_free (from);
      return;
    }
...
static void
send_goaway (grpc_chttp2_transport * t, grpc_error * error)
{
...
void
grpc_chttp2_add_ping_strike (grpc_chttp2_transport * t)
{
  ...
  if (++t->ping_recv_state.ping_strikes > t->ping_policy.max_ping_strikes &&
      t->ping_policy.max_ping_strikes != 0)
    {
      ...
      close_transport_locked (t,
			      grpc_error_set_int
			      (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			       ("Too many pings"), GRPC_ERROR_INT_GRPC_STATUS,
			       GRPC_STATUS_UNAVAILABLE));
    }
}

static void
perform_transport_op_locked (void *stream_op, grpc_error * error_ignored)
{
  ...
  if (op->on_connectivity_state_change != nullptr)
    {
      grpc_connectivity_state_notify_on_state_change (&t->channel_callback.
						      state_tracker,
...
void
grpc_chttp2_maybe_complete_recv_initial_metadata (grpc_chttp2_transport * t,
						  grpc_chttp2_stream * s)
{
  if (s->recv_initial_metadata_ready != nullptr &&
      ...
      if (s->seen_error)
	{
	  ...
	  if (!s->pending_byte_stream)
	    {
	      grpc_slice_buffer_reset_and_unref_internal (&s->
							  unprocessed_incoming_frames_buffer);
	    }
	}
      grpc_chttp2_incoming_metadata_buffer_publish (&s->metadata_buffer[0],
						    s->recv_initial_metadata);
      null_then_run_closure (&s->recv_initial_metadata_ready,
			     GRPC_ERROR_NONE);
    }
}

void
grpc_chttp2_maybe_complete_recv_message (grpc_chttp2_transport * t,
					 grpc_chttp2_stream * s)
{
	  ...
	  while (s->unprocessed_incoming_frames_buffer.length > 0 ||
		  ...
		  GRPC_STREAM_COMPRESSION_IDENTITY_DECOMPRESS)
		{
		  ...
		  if (!s->stream_decompression_ctx)
			...
			grpc_stream_compression_context_create (s->
		  ...
		  if (!grpc_stream_decompress (s->stream_decompression_ctx,
			  ...
			  GRPC_HEADER_SIZE_IN_BYTES)
			{
		      ...
		      error =
			grpc_deframe_unprocessed_incoming_frames (&s->
								  data_parser,
								  s,
								  &s->
		      ...
		      if (end_of_context)
			{
			  grpc_stream_compression_context_destroy (s->
		...
		{
		  error =
		    grpc_deframe_unprocessed_incoming_frames (&s->data_parser,
							      s,
							      &s->
							      unprocessed_incoming_frames_buffer,
							      nullptr,
...
void
grpc_chttp2_maybe_complete_recv_trailing_metadata (grpc_chttp2_transport * t,
						   grpc_chttp2_stream * s)
{
  grpc_chttp2_maybe_complete_recv_message (t, s);
  if (s->recv_trailing_metadata_finished != nullptr && s->read_closed &&
	...
	{
	  grpc_chttp2_incoming_metadata_buffer_publish (&s->
							metadata_buffer[1],
							s->
...
static void
remove_stream (grpc_chttp2_transport * t, uint32_t id, grpc_error * error)
{
  grpc_chttp2_stream *s =
...
void
grpc_chttp2_cancel_stream (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
...
void
grpc_chttp2_fake_status (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
			 grpc_error * error)
{
...
static void
add_error (grpc_error * error, grpc_error ** refs, size_t * nrefs)
{
  if (error == GRPC_ERROR_NONE)
    return;
...
static grpc_error *
removal_error (grpc_error * extra_error, grpc_chttp2_stream * s,
	       const char *master_error_msg)
{
  ...
  if (nrefs > 0)
    {
      error =
	GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING (master_error_msg,
							  refs, nrefs);
    }
...
static void
flush_write_list (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
...
void
grpc_chttp2_fail_pending_writes (grpc_chttp2_transport * t,
...
void
grpc_chttp2_mark_stream_closed (grpc_chttp2_transport * t,
...
static void
close_from_api (grpc_chttp2_transport * t, grpc_chttp2_stream * s,
		grpc_error * error)
{
  grpc_slice hdr;
...
static void
cancel_stream_cb (void *user_data, uint32_t key, void *stream)
{
...
static void
end_all_the_calls (grpc_chttp2_transport * t, grpc_error * error)
{
...
template < class F > static void
WithUrgency (grpc_chttp2_transport * t,
...
void
grpc_chttp2_act_on_flowctl_action (const grpc_core::chttp2::
...
static grpc_error *
try_http_parsing (grpc_chttp2_transport * t)
{
...
static void
read_action_locked (void *tp, grpc_error * error)
{
  ...
  if (t->closed_with_error == GRPC_ERROR_NONE)
	...
	{
	  grpc_core::BdpEstimator * bdp_est =
	    t->flow_control->bdp_estimator ();
	  if (bdp_est)
	    {
	      bdp_est->AddIncomingBytes ((int64_t)
					 GRPC_SLICE_LENGTH (t->read_buffer.
							    slices[i]));
	    }
  ...
  if (error == GRPC_ERROR_NONE && t->closed_with_error != GRPC_ERROR_NONE)
    {
      error =
	GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING ("Transport closed",
							  &t->
							  closed_with_error,
							  1);
    }
...
static void
schedule_bdp_ping_locked (grpc_chttp2_transport * t)
{
...
static void
start_bdp_ping_locked (void *tp, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) tp;
  if (grpc_http_trace.enabled ())
    {
...
static void
finish_bdp_ping_locked (void *tp, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) tp;
  if (grpc_http_trace.enabled ())
    {
...
static void
next_bdp_ping_timer_expired_locked (void *tp, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) tp;
  GPR_ASSERT (t->have_next_bdp_ping_timer);
...
void
grpc_chttp2_config_default_keepalive_args (grpc_channel_args * args,
					   bool is_client)
{
  size_t i;
  if (args)
      ...
      for (i = 0; i < args->num_args; i++)
	{
	  if (0 == strcmp (args->args[i].key, GRPC_ARG_KEEPALIVE_TIME_MS))
	    ...
	    {
	      const bool value =
		(uint32_t) grpc_channel_arg_get_integer (&args->args[i],
							 { is_client ?
							 g_default_client_keepalive_permit_without_calls
							 :
							 g_default_server_keepalive_timeout_ms,
...
static void
init_keepalive_ping_locked (void *arg, grpc_error * error)
{
  ...
  if (t->destroying || t->closed_with_error != GRPC_ERROR_NONE)
      ...
      else
	{
	  ...
	  grpc_timer_init (&t->keepalive_ping_timer,
			   ...
			   t->keepalive_time, &t->init_keepalive_ping_locked);
	}
    }
...
static void
start_keepalive_ping_locked (void *arg, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) arg;
...
static void
finish_keepalive_ping_locked (void *arg, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) arg;
...
static void
keepalive_watchdog_fired_locked (void *arg, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) arg;
  if (t->keepalive_state == GRPC_CHTTP2_KEEPALIVE_STATE_PINGING)
    {
...
static void
connectivity_state_set (grpc_chttp2_transport * t,
			grpc_connectivity_state state,
			grpc_error * error, const char *reason)
{
  GRPC_CHTTP2_IF_TRACING (gpr_log
			  (GPR_DEBUG, "set connectivity_state=%d", state));
...
static void
reset_byte_stream (void *arg, grpc_error * error)
{
...
static void
incoming_byte_stream_unref (grpc_chttp2_incoming_byte_stream * bs)
{
...
static void
incoming_byte_stream_next_locked (void *argp, grpc_error * error_ignored)
{
  ...
  if (!s->read_closed)
    {
      s->flow_control->IncomingByteStreamUpdate (bs->next_action.
						 max_size_hint, cur_length);
...
static bool
incoming_byte_stream_next (grpc_byte_stream * byte_stream,
			   size_t max_size_hint, grpc_closure * on_complete)
{
  ...
  if (s->unprocessed_incoming_frames_buffer.length > 0)
  ...
  else
    {
      ...
      GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT (&bs->next_action.closure,
					     incoming_byte_stream_next_locked,
					     bs,
					     grpc_combiner_scheduler (bs->
								      transport->
								      combiner)),
			  GRPC_ERROR_NONE);
      GPR_TIMER_END ("incoming_byte_stream_next", 0);
...
static grpc_error *
incoming_byte_stream_pull (grpc_byte_stream * byte_stream, grpc_slice * slice)
{
  ...
  if (s->unprocessed_incoming_frames_buffer.length > 0)
      ...
      if (!s->unprocessed_incoming_frames_decompressed)
	{
      ...
      error =
	grpc_deframe_unprocessed_incoming_frames (&s->data_parser, s,
						  &s->
						  unprocessed_incoming_frames_buffer,
						  slice, nullptr);
      if (error != GRPC_ERROR_NONE)
...
static void
incoming_byte_stream_destroy (grpc_byte_stream * byte_stream)
{
  GPR_TIMER_BEGIN ("incoming_byte_stream_destroy", 0);
  grpc_chttp2_incoming_byte_stream *bs =
    (grpc_chttp2_incoming_byte_stream *) byte_stream;
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_INIT (&bs->destroy_action,
					 incoming_byte_stream_destroy_locked,
					 bs,
					 grpc_combiner_scheduler (bs->
								  transport->
								  combiner)),
				    ...
				    grpc_error * error)
{
  ...
  GRPC_ERROR_UNREF (s->byte_stream_error);
  s->byte_stream_error = GRPC_ERROR_REF (error);
  grpc_chttp2_cancel_stream (bs->transport, bs->stream,
			     GRPC_ERROR_REF (error));
}

grpc_error *
grpc_chttp2_incoming_byte_stream_push (grpc_chttp2_incoming_byte_stream * bs,
				       grpc_slice slice,
...
grpc_error *
grpc_chttp2_incoming_byte_stream_finished (grpc_chttp2_incoming_byte_stream *
					   bs, grpc_error * error,
					   bool reset_on_error)
{
  grpc_chttp2_stream *s = bs->stream;
...
static void
incoming_byte_stream_shutdown (grpc_byte_stream * byte_stream,
			       grpc_error * error)
{
  grpc_chttp2_incoming_byte_stream *bs =
    (grpc_chttp2_incoming_byte_stream *) byte_stream;
...
static void
incoming_byte_stream_destroy_locked (void *byte_stream,
				     grpc_error * error_ignored)
{
  grpc_chttp2_incoming_byte_stream *bs =
    (grpc_chttp2_incoming_byte_stream *) byte_stream;
...
grpc_chttp2_incoming_byte_stream_create (grpc_chttp2_transport * t,
					 grpc_chttp2_stream * s,
					 uint32_t frame_size, uint32_t flags)
{
  ...
  incoming_byte_stream->base.vtable =
    &grpc_chttp2_incoming_byte_stream_vtable;
...
static void
post_benign_reclaimer (grpc_chttp2_transport * t)
{
  if (!t->benign_reclaimer_registered)
    {
      t->benign_reclaimer_registered = true;
      GRPC_CHTTP2_REF_TRANSPORT (t, "benign_reclaimer");
      grpc_resource_user_post_reclaimer (grpc_endpoint_get_resource_user
					 (t->ep), false,
...
static void
post_destructive_reclaimer (grpc_chttp2_transport * t)
{
...
static void
benign_reclaimer_locked (void *arg, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) arg;
  if (error == GRPC_ERROR_NONE &&
      grpc_chttp2_stream_map_size (&t->stream_map) == 0)
  ...
  else if (error == GRPC_ERROR_NONE && grpc_resource_quota_trace.enabled ())
    {
      gpr_log (GPR_DEBUG,
	       ...
	       PRIdPTR " streams", t->peer_string,
	       grpc_chttp2_stream_map_size (&t->stream_map));
    }
  t->benign_reclaimer_registered = false;
  if (error != GRPC_ERROR_CANCELLED)
    {
      grpc_resource_user_finish_reclamation (grpc_endpoint_get_resource_user
					     (t->ep));
    }
...
static void
destructive_reclaimer_locked (void *arg, grpc_error * error)
{
  grpc_chttp2_transport *t = (grpc_chttp2_transport *) arg;
  ...
  if (error == GRPC_ERROR_NONE && n > 0)
	...
	(grpc_chttp2_stream *) grpc_chttp2_stream_map_rand (&t->stream_map);
      if (grpc_resource_quota_trace.enabled ())
	{
	  gpr_log (GPR_DEBUG, "HTTP2: %s - abandon stream id %d",
		   t->peer_string, s->id);
	}
      grpc_chttp2_cancel_stream (t, s,
				 grpc_error_set_int
...
const char *
grpc_chttp2_initiate_write_reason_string (grpc_chttp2_initiate_write_reason
					  reason)
{
  switch (reason)
    {
...
chttp2_get_endpoint (grpc_transport * t)
{
