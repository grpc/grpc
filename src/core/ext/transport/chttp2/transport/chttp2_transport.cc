...
static void destroy_stream_locked(void* sp, grpc_error* error) {
  ...
  GPR_ASSERT(!s->seen_recv_message);
        ...
        if (!s->unprocessed_incoming_frames_decompressed &&
          ...
          if (!grpc_stream_decompress(
            ...
            error = grpc_deframe_unprocessed_incoming_frames(
                ...
                s->recv_payload->recv_message.recv_message);
            if (end_of_context) {
          ...
          error = grpc_deframe_unprocessed_incoming_frames(
              ...
              nullptr, s->recv_payload->recv_message.recv_message);
        }
        if (error != GRPC_ERROR_NONE) {
          s->seen_error = true;
          grpc_slice_buffer_reset_and_unref_internal(&s->frame_storage);
          grpc_slice_buffer_reset_and_unref_internal(
              &s->unprocessed_incoming_frames_buffer);
          break;
        } else if (*s->recv_payload->recv_message.recv_message != nullptr) {
          break;
        }
      }
    }
...
void grpc_chttp2_maybe_complete_recv_trailing_metadata(grpc_chttp2_transport* t,
                                                       grpc_chttp2_stream* s) {
  grpc_chttp2_maybe_complete_recv_message(t, s);
  if (s->seen_recv_trailing_metadata && s->read_closed && s->write_closed) {
    ...
    if (s->read_closed && s->frame_storage.length > 0 && !pending_data &&
        !s->seen_error && s->seen_recv_trailing_metadata) {
...
void grpc_chttp2_fake_status(grpc_chttp2_transport* t, grpc_chttp2_stream* s,
  ...
  /* stream_global->seen_recv_trailing_metadata gives us a
