  ...
  if (is_client) {
    grpc_slice_buffer_note_add(
        ...
        grpc_slice_emplace_static_string(GRPC_CHTTP2_CLIENT_CONNECT_STRING,
                                         grpc_slice_buffer_reserve(&outbuf)));
  }
  ...
  if (t->resource_user != nullptr &&
    ...
    grpc_slice_buffer_note_add(
        &t->qbuf, grpc_chttp2_rst_stream_create(
                      id, static_cast<uint32_t>(GRPC_HTTP2_REFUSED_STREAM),
                      nullptr, grpc_slice_buffer_reserve(&t->qbuf)));
...
static void close_from_api(grpc_chttp2_transport* t, grpc_chttp2_stream* s,
  ...
  grpc_slice* pfx_hdr = grpc_slice_buffer_reserve(&t->qbuf);
  ;
