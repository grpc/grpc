...
void
grpc_chttp2_encode_data (uint32_t id, grpc_slice_buffer * inbuf,
...
grpc_error *
grpc_deframe_unprocessed_incoming_frames (grpc_chttp2_data_parser * p,
					  grpc_chttp2_stream * s,
					  ...
					  grpc_byte_stream ** stream_out)
{
  ...
  while (slices->count > 0)
	...
	case GRPC_CHTTP2_DATA_FH_0:
	  ...
	  switch (p->frame_type)
	    ...
	    case 1:
	      ...
	      break;
	    default:
	      gpr_asprintf (&msg, "Bad GRPC frame type 0x%02x",
			    p->frame_type);
		...
		grpc_error_set_str (p->error, GRPC_ERROR_STR_RAW_BYTES,
	      ...
	      gpr_free (msg);
	      p->error =
		grpc_error_set_int (p->error, GRPC_ERROR_INT_OFFSET,
				    cur - beg);
	      p->state = GRPC_CHTTP2_DATA_ERROR;
	      grpc_slice_unref_internal (slice);
	      return GRPC_ERROR_REF (p->error);
	...
	case GRPC_CHTTP2_DATA_FH_4:
	  ...
	  p->parsing_frame =
	    grpc_chttp2_incoming_byte_stream_create (t, s, p->frame_size,
						     message_flags);
	  *stream_out = &p->parsing_frame->base;
	  ...
	  if (cur != end)
	    {
	      grpc_slice_buffer_undo_take_first (slices,
	...
	case GRPC_CHTTP2_DATA_FRAME:
	  {
		...
		if (GRPC_ERROR_NONE !=
		    (error =
		     grpc_chttp2_incoming_byte_stream_push (p->parsing_frame,
							    grpc_slice_sub
							    (slice,
							     (size_t) (cur -
								       beg),
		...
		if (GRPC_ERROR_NONE !=
		    (error =
		     grpc_chttp2_incoming_byte_stream_finished (p->
								parsing_frame,
								GRPC_ERROR_NONE,
								true)))
		  {
