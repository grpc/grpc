...
class WritableStreams {
 public:
  ...
  absl::Status Enqueue(const RefCountedPtr<Stream> stream,
    ...
    GRPC_WRITABLE_STREAMS_DEBUG << "UnbufferedImmediateEnqueue stream id "
                                << stream->GetStreamId() << " with priority "
                                << GetWritableStreamPriorityString(priority)
                                << " status " << status;
    return (status.ok())
               ? absl::OkStatus()
  ...
  auto Next(const bool transport_tokens_available) {
    ...
    return AssertResultType<absl::StatusOr<RefCountedPtr<Stream>>>(TrySeq(
        ...
        [this, transport_tokens_available](
          ...
          return If(
              stream.has_value(),
              [stream]() -> absl::StatusOr<RefCountedPtr<Stream>> {
              ...
              [this, transport_tokens_available] {
                ...
                return Map(
                    ...
                    [this, transport_tokens_available](
                        ValueOrFailure<
                            std::vector<std::optional<StreamIDAndPriority>>>
                            batch) -> absl::StatusOr<RefCountedPtr<Stream>> {
                      if (batch.ok()) {
  ...
  std::optional<RefCountedPtr<Stream>> ImmediateNext(
 ...
 private:
  class PrioritizedQueue {
   public:
    ...
    void Push(const RefCountedPtr<Stream> stream,
    ...
    std::optional<RefCountedPtr<Stream>> Pop(
        const bool transport_tokens_available) {
      ...
      for (uint8_t i = 0; i < buckets_.size(); ++i) {
        ...
        if (!bucket.empty()) {
          ...
          RefCountedPtr<Stream> stream = bucket.front();
          bucket.pop();
          total_streams_--;
          GRPC_WRITABLE_STREAMS_DEBUG
    ...
    static constexpr uint8_t kWaitForTransportFlowControlIndex =
        static_cast<uint8_t>(
            WritableStreamPriority::kWaitForTransportFlowControl);
    std::vector<std::queue<RefCountedPtr<Stream>>> buckets_;
