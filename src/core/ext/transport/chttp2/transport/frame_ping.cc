...
grpc_slice
grpc_chttp2_ping_create (uint8_t ack, uint64_t opaque_8bytes)
{
...
grpc_error *
grpc_chttp2_ping_parser_begin_frame (grpc_chttp2_ping_parser * parser,
				     uint32_t length, uint8_t flags)
{
  if (flags & 0xfe || length != 8)
    {
      char *msg;
      gpr_asprintf (&msg, "invalid ping: length=%d, flags=%02x", length,
		    flags);
...
grpc_error *
grpc_chttp2_ping_parser_parse (void *parser,
      ...
      else
	{
	  ...
	  if (!g_disable_ping_ack)
	      ...
	      if (t->ping_ack_count == t->ping_ack_capacity)
		{
		  ...
		  t->ping_acks =
		    (uint64_t *) gpr_realloc (t->ping_acks,
					      t->ping_ack_capacity *
					      sizeof (*t->ping_acks));
		}
