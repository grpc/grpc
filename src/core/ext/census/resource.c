...
static bool validate_units_helper(pb_istream_t *stream, int *count,
  ...
  while (stream->bytes_left) {
    ...
    google_census_Resource_BasicUnit *new_bup = (google_census_Resource_BasicUnit *)
...
size_t allocate_resource(void) {
  ...
  if (n_resources == n_defined_resources) {
    ...
    resource **new_resources = (resource **) gpr_malloc(new_n_resources * sizeof(resource *));
    if (n_resources != 0) {
...
int32_t define_resource(const resource *base) {
  ...
  resources[id]->numerators = (google_census_Resource_BasicUnit *) gpr_malloc(len);
  ...
  if (base->n_denominators != 0) {
    ...
    resources[id]->denominators = (google_census_Resource_BasicUnit *) gpr_malloc(len);
