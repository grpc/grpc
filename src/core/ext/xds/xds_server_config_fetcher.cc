 ...
 public:
  ...
  absl::StatusOr<RefCountedPtr<ServerConfigSelector>> Watch(
    ...
    return XdsServerConfigSelector::Create(
        xds_client_->xds_http_filter_registry(), static_resource_.value(),
        http_filters_);
  }
  ...
  if (XdsRbacEnabled()) {
    ...
    for (const auto& http_filter :
      ...
      auto* filter_impl = static_cast<const GrpcXdsHttpFilter*>(
          xds_client_->xds_http_filter_registry().GetFilterForType(
  ...
  for (auto& vhost : rds_update.virtual_hosts) {
    ...
    for (auto& route : vhost.routes) {
      ...
      auto result = XdsRouting::GeneratePerHTTPFilterConfigs(
          xds_http_filter_registry, http_filters, vhost, route, nullptr,
          ChannelArgs());
...
XdsServerConfigFetcher::ListenerWatcher::FilterChainMatchManager::
  ...
  return XdsServerConfigSelector::Create(
      xds_client_->xds_http_filter_registry(), resource.value(), http_filters_);
}
...
void XdsServerConfigFetcher::ListenerWatcher::FilterChainMatchManager::
  ...
  watcher_->OnServerConfigSelectorUpdate(XdsServerConfigSelector::Create(
      xds_client_->xds_http_filter_registry(), *resource_, http_filters_));
}
