...
template <class T>
void ScheduleNotifyWatchersOnErrorInWorkSerializer(
    ...
    for (const auto& p : a.second) {
      ...
      if (lds_update_map.find({authority_name, listener_name}) ==
        ...
        ScheduleNotifyWatchersOnResourceDoesNotExistInWorkSerializer(
            &xds_client()->work_serializer_, listener_state.watchers,
            DEBUG_LOCATION);
      }
    }
  }
  ...
  for (const auto& a : rds_state.subscribed_resources) {
    ...
    for (const auto& p : a.second) {
      ...
      if (rds_resource_names_seen.find(XdsApi::ConstructFullResourceName(
        ...
        ScheduleNotifyWatchersOnResourceDoesNotExistInWorkSerializer(
            &xds_client()->work_serializer_, route_config_state.watchers,
            DEBUG_LOCATION);
      }
    }
  }
  ...
  for (const auto& a : cds_state.subscribed_resources) {
    ...
    for (const auto& p : a.second) {
      ...
      if (cds_update_map.find({authority, cluster_name}) ==
        ...
        ScheduleNotifyWatchersOnResourceDoesNotExistInWorkSerializer(
            &xds_client()->work_serializer_, cluster_state.watchers,
            DEBUG_LOCATION);
      }
    }
  }
  ...
  for (const auto& a : eds_state.subscribed_resources) {
    ...
    for (const auto& p : a.second) {
      ...
      if (eds_resource_names_seen.find(XdsApi::ConstructFullResourceName(
        ...
        ScheduleNotifyWatchersOnResourceDoesNotExistInWorkSerializer(
            &xds_client()->work_serializer_, endpoint_state.watchers,
            DEBUG_LOCATION);
      }
    }
  }
...
void XdsClient::ChannelState::AdsCallState::RejectAdsUpdateHelperLocked(
  ...
  ScheduleNotifyWatchersOnErrorInWorkSerializer(
      &xds_client()->work_serializer_, state.watchers,
      GRPC_ERROR_REF(result.parse_error), DEBUG_LOCATION);
...
void XdsClient::NotifyOnErrorLocked(grpc_error_handle error) {
  std::vector<RefCountedPtr<ListenerWatcherInterface>> listener_watchers;
  std::vector<RefCountedPtr<RouteConfigWatcherInterface>> route_config_watchers;
  std::vector<RefCountedPtr<ClusterWatcherInterface>> cluster_watchers;
  std::vector<RefCountedPtr<EndpointWatcherInterface>> endpoint_watchers;
  for (const auto& a : authority_state_map_) {
    for (const auto& p : a.second.listener_map) {
      ...
      for (const auto& q : listener_state.watchers) {
        listener_watchers.push_back(q.second);
      }
    }
