...
class RootCertificatesWatcher
 ...
 public:
  // Takes a ref to \a parent instead of a raw pointer since the watcher
  // is owned by the root certificate distributor and not by \a parent.
  // Note that presently, the watcher is immediately deleted when
  // CancelTlsCertificatesWatch() is called, but that can potentially
  ...
  void OnCertificatesChanged(
    ...
    if (root_certs.has_value()) {
      parent_->SetKeyMaterials(
          cert_name_, std::string(root_certs.value()), absl::nullopt);
    }
  }
  ...
  void OnError(grpc_error_handle root_cert_error,
    ...
    if (root_cert_error != GRPC_ERROR_NONE) {
      parent_->SetErrorForCert(cert_name_,
                               root_cert_error /* pass the ref */,
                               absl::nullopt);
    }
 ...
 public:
  ...
  void OnCertificatesChanged(
    ...
    if (key_cert_pairs.has_value()) {
      parent_->SetKeyMaterials(cert_name_, absl::nullopt,
                               key_cert_pairs);
    }
  }
...
XdsCertificateProvider::ClusterCertificateState::
  ...
  if (root_cert_watcher_ != nullptr) {
    root_cert_distributor_->CancelTlsCertificatesWatch(
        root_cert_watcher_);
  }
...
bool XdsCertificateProvider::ClusterCertificateState::IsSafeToRemove()
    const {
    ...
    if (root_cert_distributor != nullptr) {
      ...
      xds_certificate_provider_->distributor_->SetErrorForCert(
          ...
          GRPC_ERROR_CREATE_FROM_STATIC_STRING(
              "No certificate provider available for root "
              "certificates"),
          absl::nullopt);
    }
  }
...
void XdsCertificateProvider::ClusterCertificateState::
  ...
  if (watching_identity_certs_) {
    // The identity certificates are being watched. Swap out the
    // watcher.
    ...
    if (identity_cert_distributor != nullptr) {
      UpdateIdentityCertWatcher(cert_name,
                                identity_cert_distributor.get());
    } else {
      identity_cert_watcher_ = nullptr;
      xds_certificate_provider_->distributor_->SetErrorForCert(
          "", absl::nullopt,
          GRPC_ERROR_CREATE_FROM_STATIC_STRING(
              "No certificate provider available for identity "
              "certificates"));
    }
  }
...
void XdsCertificateProvider::ClusterCertificateState::
    WatchStatusCallback(const std::string& cert_name,
                        bool root_being_watched,
                        bool identity_being_watched) {
  // We aren't specially handling the case where root_cert_distributor
  // is same as identity_cert_distributor. Always using two separate
  // watchers irrespective of the fact results in a straightforward
  // design, and using a single watcher does not seem to provide any
  // benefit other than cutting down on the number of callbacks.
  ...
  if (identity_being_watched && !watching_identity_certs_) {
    ...
    if (identity_cert_distributor_ == nullptr) {
      ...
      UpdateIdentityCertWatcher(cert_name,
                                identity_cert_distributor_.get());
    }
...
XdsCertificateProvider::XdsCertificateProvider()
  ...
  distributor_->SetWatchStatusCallback(absl::bind_front(
      &XdsCertificateProvider::WatchStatusCallback, this));
}
...
bool XdsCertificateProvider::ProvidesRootCerts(
    const std::string& cert_name) {
  MutexLock lock(&mu_);
...
void XdsCertificateProvider::UpdateRootCertNameAndDistributor(
  ...
  it->second->UpdateRootCertNameAndDistributor(
      cert_name, root_cert_name, root_cert_distributor);
...
void XdsCertificateProvider::UpdateRequireClientCertificate(
  ...
  it->second->set_require_client_certificate(
...
void XdsCertificateProvider::WatchStatusCallback(
...
XdsCertificateProvider::GetFromChannelArgs(
    const grpc_channel_args* args) {
  XdsCertificateProvider* xds_certificate_provider =
      grpc_channel_args_find_pointer<XdsCertificateProvider>(
          args, GRPC_ARG_XDS_CERTIFICATE_PROVIDER);
  return xds_certificate_provider != nullptr
             ? xds_certificate_provider->Ref()
             : nullptr;
}
