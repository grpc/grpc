...
class XdsTransportFactory : public InternallyRefCounted<XdsTransportFactory> {
 public:
  ...
  // The on_connectivity_change callback will be invoked with a non-OK
  // status whenever there is a connectivity failure on the transport.
  // If the transport then later becomes connected again, it will be
  // invoked again with OK status.
