...
private:
  ...
  absl::InlinedVector<std::unique_ptr<ResolverFactory>, 3> factories_;
}
...
//
// CertificateProviderRegistry::Builder
//

void CertificateProviderRegistry::Builder::InitRegistry() {
  if (g_state == nullptr) g_state = new RegistryState();
}

void CertificateProviderRegistry::Builder::ShutdownRegistry() {
  delete g_state;
  g_state = nullptr;
}
