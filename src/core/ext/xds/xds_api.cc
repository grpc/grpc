...
grpc_error* RoutePathMatchParse(const envoy_config_route_v3_RouteMatch* match,
  ...
  if (case_sensitive != nullptr) {
    ignore_case = !google_protobuf_BoolValue_value(case_sensitive);
  }
...
grpc_error* RouteHeaderMatchersParse(
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (envoy_config_route_v3_HeaderMatcher_has_exact_match(header)) {
      type = HeaderMatcher::HeaderMatcherType::EXACT;
      string_matcher = UpbStringToStdString(
      ...
      GPR_ASSERT(regex_matcher != nullptr);
      type = HeaderMatcher::HeaderMatcherType::SAFE_REGEX;
      string_matcher = UpbStringToStdString(
          envoy_type_matcher_v3_RegexMatcher_regex(regex_matcher));
    } else if (envoy_config_route_v3_HeaderMatcher_has_range_match(header)) {
      type = HeaderMatcher::HeaderMatcherType::RANGE;
      ...
      type = HeaderMatcher::HeaderMatcherType::PRESENT;
      present_match = envoy_config_route_v3_HeaderMatcher_present_match(header);
    } else if (envoy_config_route_v3_HeaderMatcher_has_prefix_match(header)) {
      type = HeaderMatcher::HeaderMatcherType::PREFIX;
      string_matcher = UpbStringToStdString(
          envoy_config_route_v3_HeaderMatcher_prefix_match(header));
    } else if (envoy_config_route_v3_HeaderMatcher_has_suffix_match(header)) {
      type = HeaderMatcher::HeaderMatcherType::SUFFIX;
      string_matcher = UpbStringToStdString(
          envoy_config_route_v3_HeaderMatcher_suffix_match(header));
    } else if (envoy_config_route_v3_HeaderMatcher_has_contains_match(header)) {
      type = HeaderMatcher::HeaderMatcherType::CONTAINS;
      string_matcher = UpbStringToStdString(
    ...
    HeaderMatcher header_matcher(name, type, string_matcher, range_start,
                                 range_end, present_match, invert_match);
    if (type == HeaderMatcher::HeaderMatcherType::SAFE_REGEX) {
      if (!header_matcher.matcher()->regex_matcher()->ok()) {
        return GRPC_ERROR_CREATE_FROM_STATIC_STRING(
