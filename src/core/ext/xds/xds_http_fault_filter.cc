...
XdsHttpFaultFilter::GenerateFilterConfig(upb_strview serialized_filter_config,
  ...
  return XdsHttpFilterImpl::FilterConfig{kXdsHttpFaultFilterConfigName,
...
XdsHttpFaultFilter::GenerateServiceConfig(
    ...
    const XdsHttpFilterImpl::FilterConfig* filter_config_override) const {
  Json policy_json = filter_config_override != nullptr
                         ? filter_config_override->config
                         : hcm_filter_config.config;
  // The policy JSON may be empty, that's allowed.
  return XdsHttpFilterImpl::ServiceConfigJsonEntry{"faultInjectionPolicy",
                                                   policy_json.Dump()};
}
