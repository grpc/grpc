...
CircuitBreakerCallCounterMap::GetOrCreate(const std::string& cluster,
  ...
  auto it = g_call_counter_map->map_.find(key);
  if (it == g_call_counter_map->map_.end()) {
    it = g_call_counter_map->map_.insert({key, nullptr}).first;
  } else {
...
void CircuitBreakerCallCounterMap::Init() {
  g_call_counter_map = new CircuitBreakerCallCounterMap();
}

void CircuitBreakerCallCounterMap::Shutdown() { delete g_call_counter_map; }
