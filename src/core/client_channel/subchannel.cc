      ...
      if (new_state == GRPC_CHANNEL_TRANSIENT_FAILURE ||
        ...
        if (c->stats_plugin_group_ != nullptr) {
          ...
          SubchannelConnectionsDomainOpenConnections::GetStorage(
              scope, c->target_, connected_subchannel->security_level(),
              c->backend_service_, c->locality_)
              ->Decrement(
...
class NewSubchannel::ConnectionStateWatcher final
 ...
 public:
  explicit ConnectionStateWatcher(WeakRefCountedPtr<NewSubchannel> subchannel)
  ...
  void OnDisconnect(absl::Status status,
    ...
    if (connected_subchannel == nullptr) return;
    // Record disconnection metrics
    if (subchannel_->stats_plugin_group_ != nullptr) {
      // Determine disconnect reason from DisconnectInfo
      std::string disconnect_reason;
      if (subchannel_->shutdown_) {
      ...
      auto scope = GlobalCollectionScope();
      SubchannelMetricsDomainDisconnections::GetStorage(
          scope, subchannel_->target_, subchannel_->backend_service_,
          subchannel_->locality_, disconnect_reason)
          ->Increment(SubchannelMetricsDomainDisconnections::kDisconnections);
      SubchannelConnectionsDomainOpenConnections::GetStorage(
          scope, subchannel_->target_, connected_subchannel->security_level(),
...
// Note: Must be called with a state that is different from the current state.
void NewSubchannel::SetConnectivityStateLocked(grpc_connectivity_state state,
                                               const absl::Status& status) {
  state_ = state;
  if (status.ok()) {
    status_ = status;
...
NewSubchannel::GetConnectedSubchannel() {
  MutexLock lock(&mu_);
  return connected_subchannel_;
}
