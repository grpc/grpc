...
static int grpc_rb_channel_create_in_process_add_args_hash_cb(
    VALUE key, VALUE val, VALUE args_obj) {
  const char* the_key;
  ...
  if (args->num_args <= 0) {
    rb_raise(rb_eRuntimeError,
             "hash_cb bug: num_args is %lu for key:%s", args->num_args,
             StringValueCStr(key));
    return ST_STOP;
  }
...
/* channel_convert_params allows the call to
   ...
   exception-handler.  This allows any allocated memory to be freed
   before propagating any exception that occurs */
...
static VALUE grpc_rb_hash_convert_to_channel_args0(VALUE as_value) {
  ...
  VALUE grpc_rb_cChannelArgs =
      rb_define_class("TmpChannelArgs", rb_cObject);
...
void grpc_rb_hash_convert_to_channel_args(VALUE src_hash,
  ...
  params.dst = dst;
  rb_protect(grpc_rb_hash_convert_to_channel_args0, (VALUE)&params,
             &status);
  if (status != 0) {
    if (dst->args != NULL) {
