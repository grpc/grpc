...
static VALUE grpc_rb_call_credentials_callback(VALUE args) {
  ...
  if (gpr_should_log(GPR_LOG_SEVERITY_DEBUG)) {
    ...
    VALUE callback_source_info = rb_funcall(callback_func, rb_intern("source_location"), 0);
    if (callback_source_info != Qnil) {
      ...
      gpr_log(GPR_DEBUG,
              ...
              StringValueCStr(callback_func_str), StringValueCStr(source_filename),
              StringValueCStr(source_line_number), StringValueCStr(callback_args_str));
    } else {
      gpr_log(GPR_DEBUG,
              "GRPC_RUBY: grpc_rb_call_credentials invoking user callback:|%s| "
              "(failed to get source filename and line) with arguments:|%s|",
              StringValueCStr(callback_func_str), StringValueCStr(callback_args_str));
    }
  }
  VALUE metadata = rb_funcall(callback_func, rb_intern("call"), 1, callback_args);
...
static VALUE grpc_rb_call_credentials_callback_rescue(VALUE args, VALUE exception_object) {
  ...
  rb_hash_aset(result, rb_str_new2("status"), INT2NUM(GRPC_STATUS_UNAUTHENTICATED));
...
static void grpc_rb_call_credentials_callback_with_gil(void* param) {
  ...
  params->callback(params->user_data, md_ary.metadata, md_ary.count, status, error_details);
...
static int grpc_rb_call_credentials_plugin_get_metadata(
  ...
  grpc_rb_event_queue_enqueue(grpc_rb_call_credentials_callback_with_gil, (void*)(params));
...
static rb_data_type_t grpc_rb_call_credentials_data_type = {"grpc_call_credentials",
...
VALUE grpc_rb_wrap_call_credentials(grpc_call_credentials* c, VALUE mark) {
  ...
  TypedData_Get_Struct(rb_wrapper, grpc_rb_call_credentials, &grpc_rb_call_credentials_data_type,
                       wrapper);
  wrapper->wrapped = c;
  wrapper->mark = mark;
  return rb_wrapper;
...
static VALUE grpc_rb_call_credentials_init(VALUE self, VALUE proc) {
  ...
  TypedData_Get_Struct(self, grpc_rb_call_credentials, &grpc_rb_call_credentials_data_type,
                       wrapper);
...
void Init_grpc_call_credentials() {
  ...
  rb_define_alloc_func(grpc_rb_cCallCredentials, grpc_rb_call_credentials_alloc);

  /* Provides a ruby constructor and support for dup/clone. */
  rb_define_method(grpc_rb_cCallCredentials, "initialize", grpc_rb_call_credentials_init, 1);
  rb_define_method(grpc_rb_cCallCredentials, "initialize_copy", grpc_rb_cannot_init_copy, 1);
  rb_define_method(grpc_rb_cCallCredentials, "compose", grpc_rb_call_credentials_compose, -1);
...
grpc_call_credentials* grpc_rb_get_wrapped_call_credentials(VALUE v) {
  grpc_rb_call_credentials* wrapper = NULL;
  TypedData_Get_Struct(v, grpc_rb_call_credentials, &grpc_rb_call_credentials_data_type, wrapper);
