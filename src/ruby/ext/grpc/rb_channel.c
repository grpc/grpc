...
static void* run_poll_channels_loop_no_gil(void* arg) {
  ...
  bg_watched_channel* bg = NULL;
  (void)arg;
  grpc_absl_vlog(GPR_LOCATION, "GRPC_RUBY: run_poll_channels_loop_no_gil - begin");
...
static void* run_poll_channels_loop_unblocking_func_wrapper(void* arg) {
  ...
  grpc_absl_vlog_int(GPR_LOCATION, "GRPC_RUBY: cq shutdown on global polling cq. pid: ",
          getpid());
...
void grpc_rb_channel_polling_thread_start() {
  ...
  if (!RTEST(g_channel_polling_thread)) {
    grpc_absl_log_error(GPR_LOCATION,"GRPC_RUBY: failed to spawn channel polling thread");
