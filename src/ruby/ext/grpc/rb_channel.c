...
/* id_channel is the name of the hidden ivar that preserves a reference
 * to the channel on a call, so that calls are not GCed before their
 * channel.  */
static ID id_channel;

/* id_target is the name of the hidden ivar that preserves a reference
 * to the target string used to create the call, preserved so that it
 * does not get GCed before the channel */
static ID id_target;
...
/* Used during the conversion of a hash to channel args during channel
 * setup */
static VALUE grpc_rb_cChannelArgs;

typedef struct bg_watched_channel {
  grpc_channel* channel;
  // these fields must only be accessed under
...
typedef struct grpc_rb_channel {
  ...
  /* The actual channel (protected in a wrapper to tell when it's safe
   * to destroy) */
...
/*
  call-seq:
    ...
    creds = ...
    secure_channel = Channel:new("myhost:443", {'arg1': 'value1'},
  creds)
...
static VALUE grpc_rb_channel_init(int argc, VALUE* argv, VALUE self) {
  ...
  TypedData_Get_Struct(self, grpc_rb_channel, &grpc_channel_data_type,
                       wrapper);
  ...
  if (TYPE(credentials) == T_SYMBOL) {
    ...
    if (grpc_rb_is_channel_credentials(credentials)) {
      ...
      rb_raise(rb_eTypeError,
               "bad creds, want ChannelCredentials or "
  ...
  rb_thread_call_without_gvl(
      channel_init_try_register_connection_polling_without_gil, &stack,
      NULL, NULL);
...
static void* get_state_without_gil(void* arg) {
  ...
  if (stack->bg->channel_destroyed) {
    ...
    stack->out = grpc_channel_check_connectivity_state(
        stack->bg->channel, stack->try_to_connect);
  }
  gpr_mu_unlock(&global_connection_polling_mu);
...
static void* wait_for_watch_state_op_complete_without_gvl(void* arg) {
  ...
  gpr_mu_lock(&global_connection_polling_mu);
  // it's unsafe to do a "watch" after "channel polling abort" because
  // the cq has been shut down.
  ...
  grpc_channel_watch_connectivity_state(
      stack->bg_wrapped->channel, stack->last_state, stack->deadline,
...
static VALUE grpc_rb_channel_watch_connectivity_state(VALUE self,
  ...
  if (!FIXNUM_P(last_state)) {
    rb_raise(rb_eTypeError,
             "bad type for last_state. want a GRPC::Core::ChannelState "
             "constant");
    return Qnil;
  }
...
/* Create a call given a grpc_channel, in order to call method. The
   request is not sent until grpc_call_invoke is called. */
static VALUE grpc_rb_channel_create_call(VALUE self, VALUE parent,
  ...
  grpc_ruby_fork_guard();
  if (host != Qnil) {
    host_slice = grpc_slice_from_copied_buffer(RSTRING_PTR(host),
                                               RSTRING_LEN(host));
  ...
  method_slice = grpc_slice_from_copied_buffer(RSTRING_PTR(method),
                                               RSTRING_LEN(method));
  call = grpc_channel_create_call(
      wrapper->bg_wrapped->channel, parent_call, flags, cq,
  ...
  /* Make this channel an instance attribute of the call so that it is
...
/* Note this is an API-level call; a wrapped channel's finalizer doesn't
 * call this */
...
/* Initialize a grpc_rb_channel's "protected grpc_channel" and try to
 * push it onto the background thread for constant watches. */
static void* channel_init_try_register_connection_polling_without_gil(
    void* arg) {
  channel_init_try_register_stack* stack =
...
static void grpc_rb_channel_try_register_connection_polling(
  ...
  GPR_ASSERT(bg_watched_channel_list_lookup(bg));
  // prevent bg from being free'd by GC while background thread is
  // watching it
  bg->refcount++;
  ...
  grpc_channel_watch_connectivity_state(
      bg->channel, conn_state, gpr_inf_future(GPR_CLOCK_REALTIME),
      channel_polling_cq, op);
}
...
static void* run_poll_channels_loop_no_gil(void* arg) {
    ...
    if (event.type == GRPC_OP_COMPLETE) {
      ...
      if (op->op_type == CONTINUOUS_WATCH) {
        bg = (bg_watched_channel*)
                 op->op.continuous_watch_callback_args.bg;
        bg->refcount--;
  ...
  gpr_log(GPR_DEBUG,
          "GRPC_RUBY: run_poll_channels_loop_no_gil - exit connection "
          "polling "
          "loop");
  return NULL;
}
...
static void run_poll_channels_loop_unblocking_func(void* arg) {
  ...
  gpr_mu_lock(&global_connection_polling_mu);
  gpr_log(GPR_DEBUG,
          "GRPC_RUBY: run_poll_channels_loop_unblocking_func - begin "
          "aborting "
          "connection polling");
  ...
  gpr_mu_unlock(&global_connection_polling_mu);
  gpr_log(GPR_DEBUG,
          "GRPC_RUBY: run_poll_channels_loop_unblocking_func - end "
          "aborting "
          "connection polling");
}

// Poll channel connectivity states in background thread without the
// GIL.
static VALUE run_poll_channels_loop(VALUE arg) {
  (void)arg;
  gpr_log(GPR_DEBUG,
          "GRPC_RUBY: run_poll_channels_loop - create connection "
          "polling thread");
  grpc_ruby_init();
 ...
 * Transports in idle channels can get destroyed. Normally c-core
 * re-connects, but in grpc-ruby core never gets a thread until an RPC
 * is made, because ruby only calls c-core's "completion_queu_pluck"
 * API. This uses a global background thread that calls
 * "completion_queue_next" on registered
 * "watch_channel_connectivity_state" calls - so that c-core can
 * reconnect if needed, when there aren't any RPC's.
 * TODO(apolcyn) remove this when core handles new RPCs on dead
 * connections.
 */
void grpc_rb_channel_polling_thread_start() {
  ...
  if (!RTEST(background_thread)) {
    ...
    rb_thread_call_without_gvl(set_abort_channel_polling_without_gil,
                               NULL, NULL, NULL);
  }
}
...
void Init_grpc_channel() {
  ...
  rb_define_method(grpc_rb_cChannel, "initialize", grpc_rb_channel_init,
                   -1);
  ...
  rb_define_method(grpc_rb_cChannel, "target",
                   grpc_rb_channel_get_target, 0);
  rb_define_method(grpc_rb_cChannel, "destroy", grpc_rb_channel_destroy,
                   0);
  ...
  rb_define_const(
      grpc_rb_cChannel, "MAX_MESSAGE_LENGTH",
...
grpc_channel* grpc_rb_get_wrapped_channel(VALUE v) {
  grpc_rb_channel* wrapper = NULL;
  TypedData_Get_Struct(v, grpc_rb_channel, &grpc_channel_data_type,
                       wrapper);
