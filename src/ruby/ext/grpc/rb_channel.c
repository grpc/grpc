...
static void grpc_rb_channel_watch_connection_state_op_complete(
  ...
  assert(!op->op.api_callback_args.called_back);
...
static void grpc_rb_channel_safe_destroy(bg_watched_channel* bg) {
  ...
  assert(bg_watched_channel_list_lookup(bg));
...
static void bg_watched_channel_list_free_and_remove(
  ...
  assert(bg_watched_channel_list_lookup(target));
  assert(target->channel_destroyed && target->refcount == 0);
...
void grpc_rb_channel_polling_thread_start() {
  ...
  assert(!RTEST(g_channel_polling_thread));
  assert(!g_abort_channel_polling);
  assert(g_channel_polling_cq == NULL);
