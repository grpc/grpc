...
LoadReporterAsyncServiceImpl::LoadReporterAsyncServiceImpl(
    ...
    : cq_(std::move(cq)) {
  thread_ = absl::make_unique<::grpc_core::Thread>("server_load_reporting", Work, this);
...
void LoadReporterAsyncServiceImpl::ScheduleNextFetchAndSample() {
  auto next_fetch_and_sample_time =
      gpr_time_add(gpr_now(GPR_CLOCK_MONOTONIC),
                   gpr_time_from_millis(kFetchAndSampleIntervalSeconds * 1000, GPR_TIMESPAN));
  {
    grpc_core::MutexLock lock(&cq_shutdown_mu_);
    if (shutdown_) return;
    ...
    next_fetch_and_sample_alarm_->Set(cq_.get(), next_fetch_and_sample_time, this);
  }
...
void LoadReporterAsyncServiceImpl::Work(void* arg) {
  LoadReporterAsyncServiceImpl* service = static_cast<LoadReporterAsyncServiceImpl*>(arg);
  ...
  ReportLoadHandler::CreateAndStart(service->cq_.get(), service, service->load_reporter_.get());
  void* tag;
  bool ok;
  while (true) {
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::CreateAndStart(
    ServerCompletionQueue* cq, LoadReporterAsyncServiceImpl* service, LoadReporter* load_reporter) {
  ...
  ReportLoadHandler* p = handler.get();
  {
    ...
    p->on_done_notified_ = CallableTag(std::bind(&ReportLoadHandler::OnDoneNotified, p,
                                                 std::placeholders::_1, std::placeholders::_2),
                                       handler);
    p->next_inbound_ = CallableTag(std::bind(&ReportLoadHandler::OnRequestDelivered, p,
                                             std::placeholders::_1, std::placeholders::_2),
                                   std::move(handler));
    p->ctx_.AsyncNotifyWhenDone(&p->on_done_notified_);
    service->RequestReportLoad(&p->ctx_, &p->stream_, cq, cq, &p->next_inbound_);
  }
}
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::OnRequestDelivered(
  ...
  CreateAndStart(cq_, service_, load_reporter_);
  {
    ...
    next_inbound_ = CallableTag(std::bind(&ReportLoadHandler::OnReadDone, this,
                                          std::placeholders::_1, std::placeholders::_2),
                                std::move(self));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::OnReadDone(
  ...
  if (call_status_ < INITIAL_REQUEST_RECEIVED) {
    if (!request_.has_initial_request()) {
      ...
      load_reporter_->ReportStreamCreated(load_balanced_hostname_, lb_id_, load_key_);
      const auto& load_report_interval = initial_request.load_report_interval();
      load_report_interval_ms_ = static_cast<unsigned long>(load_report_interval.seconds() * 1000 +
                                                            load_report_interval.nanos() / 1000);
      gpr_log(GPR_INFO,
              "[LRS %p] Initial request received. Start load reporting (load "
              "balanced host: %s, interval: %" PRIu64 " ms, lb_id_: %s, handler: %p)...",
              service_, load_balanced_hostname_.c_str(), load_report_interval_ms_, lb_id_.c_str(),
              this);
    ...
    gpr_log(GPR_ERROR, "[LRS %p] Another request received (lb_id_: %s, handler: %p).", service_,
            lb_id_.c_str(), this);
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::ScheduleNextReport(
  ...
  auto next_report_time = gpr_time_add(
      gpr_now(GPR_CLOCK_MONOTONIC), gpr_time_from_millis(load_report_interval_ms_, GPR_TIMESPAN));
  {
    grpc_core::ReleasableMutexLock lock(&service_->cq_shutdown_mu_);
    if (service_->shutdown_) {
      lock.Release();
    ...
    next_outbound_ = CallableTag(std::bind(&ReportLoadHandler::SendReport, this,
                                           std::placeholders::_1, std::placeholders::_2),
                                 std::move(self));
    // TODO(juanlishen): Improve the Alarm implementation to reuse a single
    // instance for multiple events.
  ...
  gpr_log(GPR_DEBUG, "[LRS %p] Next load report scheduled (lb_id_: %s, handler: %p).", service_,
          lb_id_.c_str(), this);
}
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::SendReport(
  ...
  if (call_status_ < INITIAL_RESPONSE_SENT) {
    ...
    initial_response->set_implementation_id(::grpc::lb::v1::InitialLoadReportResponse::CPP);
  ...
  }
  {
    ...
    next_outbound_ = CallableTag(std::bind(&ReportLoadHandler::ScheduleNextReport, this,
                                           std::placeholders::_1, std::placeholders::_2),
                                 std::move(self));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::Shutdown(
  ...
  if (call_status_ >= DELIVERED && call_status_ < FINISH_CALLED) {
    ...
    if (!service_->shutdown_) {
      on_finish_done_ = CallableTag(std::bind(&ReportLoadHandler::OnFinishDone, this,
                                              std::placeholders::_1, std::placeholders::_2),
                                    std::move(self));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::OnFinishDone(
    ...
    std::shared_ptr<ReportLoadHandler> /*self*/, bool ok) {
  if (ok) {
    gpr_log(GPR_INFO, "[LRS %p] Load reporting finished (lb_id_: %s, handler: %p).", service_,
            lb_id_.c_str(), this);
  }
}
