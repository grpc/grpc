...
LoadReporterAsyncServiceImpl::LoadReporterAsyncServiceImpl(
  ...
  thread_ = absl::make_unique<::grpc_core::Thread>(
      "server_load_reporting", Work, this);
  ...
  load_reporter_ = absl::make_unique<LoadReporter>(
      ...
      std::unique_ptr<CensusViewProvider>(
          new CensusViewProviderDefaultImpl()),
...
void LoadReporterAsyncServiceImpl::ScheduleNextFetchAndSample() {
  auto next_fetch_and_sample_time = gpr_time_add(
                           ...
                           GPR_TIMESPAN));
  {
    grpc_core::MutexLock lock(&cq_shutdown_mu_);
    if (shutdown_) return;
    // TODO(juanlishen): Improve the Alarm implementation to reuse a
    // single instance for multiple events.
    next_fetch_and_sample_alarm_ = absl::make_unique<Alarm>();
    next_fetch_and_sample_alarm_->Set(cq_.get(),
                                      next_fetch_and_sample_time, this);
  }
...
void LoadReporterAsyncServiceImpl::Work(void* arg) {
  ...
  // TODO(juanlishen): This is a workaround to wait for the cq to be
  // ready. Need to figure out why cq is not ready after service starts.
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::CreateAndStart(
  ...
  ReportLoadHandler* p = handler.get();
  {
    ...
    p->on_done_notified_ = CallableTag(
        std::bind(&ReportLoadHandler::OnDoneNotified, p,
                  std::placeholders::_1, std::placeholders::_2),
        handler);
    p->next_inbound_ = CallableTag(
        std::bind(&ReportLoadHandler::OnRequestDelivered, p,
                  std::placeholders::_1, std::placeholders::_2),
        std::move(handler));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::
    OnRequestDelivered(std::shared_ptr<ReportLoadHandler> self,
                       bool ok) {
  if (ok) {
  ...
  CreateAndStart(cq_, service_, load_reporter_);
  {
    ...
    next_inbound_ = CallableTag(
        std::bind(&ReportLoadHandler::OnReadDone, this,
                  std::placeholders::_1, std::placeholders::_2),
        std::move(self));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::OnReadDone(
  ...
  if (!ok || shutdown_) {
    if (!ok && call_status_ < INITIAL_REQUEST_RECEIVED) {
      // The client may have half-closed the stream or the stream is
      // broken.
      gpr_log(
          GPR_INFO,
          "[LRS %p] Failed reading the initial request from the stream "
          "(lb_id_: %s, handler: %p, done_notified: %d, is_cancelled: "
          "%d).",
          service_, lb_id_.c_str(), this,
  ...
  if (call_status_ < INITIAL_REQUEST_RECEIVED) {
    if (!request_.has_initial_request()) {
      ...
      load_balanced_hostname_ =
          initial_request.load_balanced_hostname();
      load_key_ = initial_request.load_key();
      load_reporter_->ReportStreamCreated(load_balanced_hostname_,
                                          lb_id_, load_key_);
      const auto& load_report_interval =
          initial_request.load_report_interval();
      load_report_interval_ms_ = static_cast<unsigned long>(
          load_report_interval.seconds() * 1000 +
          load_report_interval.nanos() / 1000);
      gpr_log(GPR_INFO,
              "[LRS %p] Initial request received. Start load reporting "
              "(load "
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::
    ScheduleNextReport(std::shared_ptr<ReportLoadHandler> self,
                       bool ok) {
  if (!ok || shutdown_) {
      ...
      gpr_time_from_millis(load_report_interval_ms_, GPR_TIMESPAN));
  {
    ...
    next_outbound_ = CallableTag(
        std::bind(&ReportLoadHandler::SendReport, this,
                  std::placeholders::_1, std::placeholders::_2),
        std::move(self));
    // TODO(juanlishen): Improve the Alarm implementation to reuse a
    // single instance for multiple events.
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::SendReport(
  ...
  }
  {
    ...
    next_outbound_ = CallableTag(
        std::bind(&ReportLoadHandler::ScheduleNextReport, this,
                  std::placeholders::_1, std::placeholders::_2),
        std::move(self));
...
void LoadReporterAsyncServiceImpl::ReportLoadHandler::Shutdown(
  ...
  if (!shutdown_) {
    ...
    shutdown_ = true;
    if (call_status_ >= INITIAL_REQUEST_RECEIVED) {
      load_reporter_->ReportStreamClosed(load_balanced_hostname_,
                                         lb_id_);
      next_report_alarm_->Cancel();
    }
  }
  // OnRequestDelivered() may be called after OnDoneNotified(), so we
  // need to try to Finish() every time we are in Shutdown().
  if (call_status_ >= DELIVERED && call_status_ < FINISH_CALLED) {
    ...
    if (!service_->shutdown_) {
      on_finish_done_ = CallableTag(
          std::bind(&ReportLoadHandler::OnFinishDone, this,
                    std::placeholders::_1, std::placeholders::_2),
          std::move(self));
      // TODO(juanlishen): Maybe add a message proto for the client to
      // explicitly cancel the stream so that we can return OK status in
      // such cases.
