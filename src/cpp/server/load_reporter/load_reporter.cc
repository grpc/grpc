...
CpuStatsProvider::CpuStatsSample
CpuStatsProviderDefaultImpl::GetCpuStats() {
...
CensusViewProvider::CensusViewProvider()
  ...
  auto vd_start_count =
      ::opencensus::stats::ViewDescriptor()
          ...
          .set_description(
              "Delta count of calls started broken down by <token, "
              "host, "
              "user_id>.");
  ...
  // If this view is set as Count of kMeasureEndBytesSent (in hope of
  // saving one measure), it's infeasible to prepare fake data for
  // testing. That's because the OpenCensus API to make up view data
  // will add the input data as separate measurements instead of setting
  ...
  auto vd_end_bytes_sent =
      ::opencensus::stats::ViewDescriptor()
          ...
          .add_column(tag_key_status_)
          .set_description(
              "Delta sum of bytes sent broken down by <token, host, "
              "user_id, "
              "status>.");
  ::opencensus::stats::SetAggregationWindow(
      ::opencensus::stats::AggregationWindow::Delta(),
      &vd_end_bytes_sent);
  ...
  auto vd_end_bytes_received =
      ::opencensus::stats::ViewDescriptor()
          ...
          .add_column(tag_key_status_)
          .set_description(
              "Delta sum of bytes received broken down by <token, "
              "host, "
              "user_id, status>.");
  ::opencensus::stats::SetAggregationWindow(
      ::opencensus::stats::AggregationWindow::Delta(),
      &vd_end_bytes_received);
  view_descriptor_map_.emplace(kViewEndBytesReceived,
                               vd_end_bytes_received);
  ...
  auto vd_metric_call_count =
      ::opencensus::stats::ViewDescriptor()
          ...
          .set_description(
              "Delta count of calls broken down by <token, host, "
              "user_id, "
              "metric_name>.");
  ::opencensus::stats::SetAggregationWindow(
      ::opencensus::stats::AggregationWindow::Delta(),
      &vd_metric_call_count);
  view_descriptor_map_.emplace(kViewOtherCallMetricCount,
                               vd_metric_call_count);
  ...
  ::opencensus::stats::SetAggregationWindow(
      ::opencensus::stats::AggregationWindow::Delta(),
      &vd_metric_value);
  view_descriptor_map_.emplace(kViewOtherCallMetricValue,
                               vd_metric_value);
}
...
double CensusViewProvider::GetRelatedViewDataRowDouble(
  ...
  auto it_vd =
      view_data_map.find(std::string(view_name, view_name_len));
...
CensusViewProviderDefaultImpl::CensusViewProviderDefaultImpl() {
  for (const auto& p : view_descriptor_map()) {
    ...
    // We need to use pair's piecewise ctor here, otherwise the deleted
    // copy ctor of View will be called.
...
CensusViewProvider::ViewDataMap
CensusViewProviderDefaultImpl::FetchViewData() {
  gpr_log(GPR_DEBUG, "[CVP %p] Starts fetching Census view data.",
          this);
  ...
  for (auto& p : view_map_) {
    ...
    if (view.IsValid()) {
      ...
      gpr_log(GPR_DEBUG,
              "[CVP %p] Can't fetch view data because view is invalid "
              "(view: %s).",
...
std::string LoadReporter::GenerateLbId() {
  while (true) {
    if (next_lb_id_ > UINT32_MAX) {
      gpr_log(GPR_ERROR,
              "[LR %p] The LB ID exceeds the max valid value!", this);
      return "";
    }
    ...
    // The client may send requests with LB ID that has never been
    // allocated by this load reporter. Those IDs are tracked and will
    // be skipped when we generate a new ID.
...
LoadReporter::GenerateLoadBalancingFeedback() {
  ...
  while (std::distance(oldest, newest) > 0 &&
    ...
    // A zero limit means that the system info reading was failed, so
    // these records can't be used to calculate CPU utilization.
  ...
  for (auto p = newest; p != oldest; --p) {
    // Because these two numbers are counters, the oldest record
  ...
  feedback.set_server_utilization(
...
LoadReporter::GenerateLoads(const std::string& hostname,
  ...
  auto assigned_stores =
      load_data_store_.GetAssignedStores(hostname, lb_id);
  ...
  for (PerBalancerStore* per_balancer_store : *assigned_stores) {
    ...
    if (!per_balancer_store->load_record_map().empty()) {
      for (const auto& p : per_balancer_store->load_record_map()) {
        ...
        load->set_num_calls_started(
            static_cast<int64_t>(value.start_count()));
        ...
        load->set_total_bytes_sent(
            static_cast<int64_t>(value.bytes_sent()));
        ...
        load->mutable_total_latency()->set_nanos(
            (static_cast<int32_t>(value.latency_ms()) % 1000) *
            1000000);
        ...
        if (per_balancer_store->lb_id() != lb_id) {
          // This per-balancer store is an orphan assigned to this
          // receiving balancer.
...
void LoadReporter::AttachOrphanLoadId(
  ...
  if (per_balancer_store.lb_id() == kInvalidLbId) {
    ...
    // We shouldn't set load_key_unknown to any value in this case
    // because load_key_unknown and orphaned_load_identifier are under
    // an oneof struct.
...
void LoadReporter::AppendNewFeedbackRecord(uint64_t rpcs,
                                           uint64_t errors) {
  ...
  feedback_records_.emplace_back(std::chrono::system_clock::now(), rpcs,
                                 errors, cpu_stats.first,
                                 cpu_stats.second);
}
...
void LoadReporter::ReportStreamCreated(const std::string& hostname,
  ...
  gpr_log(GPR_INFO,
          "[LR %p] Report stream created (host: %s, LB ID: %s, load "
          "key: %s).",
...
void LoadReporter::ReportStreamClosed(const std::string& hostname,
  ...
  gpr_log(GPR_INFO,
          "[LR %p] Report stream closed (host: %s, LB ID: %s).", this,
...
void LoadReporter::ProcessViewDataCallEnd(
  ...
  if (it != view_data_map.end()) {
    for (const auto& p : it->second.int_data()) {
      ...
      // This is due to a bug reported internally of Java server load
      // reporting implementation.
      // TODO(juanlishen): Check whether this situation happens in OSS
      // C++.
      ...
      const uint64_t bytes_sent =
          CensusViewProvider::GetRelatedViewDataRowInt(
              view_data_map, kViewEndBytesSent,
              sizeof(kViewEndBytesSent) - 1, tag_values);
      ...
      const uint64_t latency_ms =
          CensusViewProvider::GetRelatedViewDataRowInt(
              view_data_map, kViewEndLatencyMs,
              sizeof(kViewEndLatencyMs) - 1, tag_values);
      uint64_t ok_count = 0;
      ...
      LoadRecordValue value =
          LoadRecordValue(0, ok_count, error_count, bytes_sent,
...
void LoadReporter::ProcessViewDataOtherCallMetrics(
  ...
  if (it != view_data_map.end()) {
    for (const auto& p : it->second.int_data()) {
      ...
      LoadRecordValue value =
          LoadRecordValue(metric_name, static_cast<uint64_t>(num_calls),
                          total_metric_value);
      {
        grpc_core::MutexLock lock(&store_mu_);
        load_data_store_.MergeRow(host, key, value);
...
void LoadReporter::FetchAndSample() {
  gpr_log(GPR_DEBUG,
          "[LR %p] Starts fetching Census view data and sampling LB "
          "feedback "
          "record.",
          this);
