...
LoadRecordKey::LoadRecordKey(const std::string& client_ip_and_token,
  ...
  GRPC_CHECK_GE(client_ip_and_token.size(), 2u);
  int ip_hex_size;
  GRPC_CHECK(sscanf(client_ip_and_token.substr(0, 2).c_str(), "%d", &ip_hex_size) ==
        1);
  GRPC_CHECK(ip_hex_size == 0 || ip_hex_size == kIpv4AddressLength ||
...
void PerBalancerStore::MergeRow(const LoadRecordKey& key,
  ...
  GRPC_CHECK(static_cast<int64_t>(num_calls_in_progress_) +
...
void PerHostStore::ReportStreamClosed(const std::string& lb_id) {
  ...
  GRPC_CHECK(it_store_for_gone_lb != per_balancer_stores_.end());
  // Remove this closed stream from our records.
  GRPC_CHECK(UnorderedMapOfSetEraseKeyValue(load_key_to_receiving_lb_ids_,
...
void PerHostStore::AssignOrphanedStore(PerBalancerStore* orphaned_store,
  ...
  GRPC_CHECK(it != assigned_stores_.end());
...
void PerHostStore::SetUpForNewLbId(const std::string& lb_id,
  ...
  GRPC_CHECK(per_balancer_stores_.find(lb_id) == per_balancer_stores_.end());
  GRPC_CHECK(assigned_stores_.find(lb_id) == assigned_stores_.end());
...
void LoadDataStore::ReportStreamClosed(const std::string& hostname,
  ...
  GRPC_CHECK(it_per_host_store != per_host_stores_.end());
