...
namespace grpc
{
  ...
  {
    auto *w =
      reinterpret_cast < AuthMetadataProcessorAyncWrapper * >(wrapper);
    delete w;
  }

  void AuthMetadataProcessorAyncWrapper::Process (void *wrapper,
						  ...
						  cb, void *user_data)
  {
	...
	w->thread_pool_->
	  Add (std::
	       bind (&AuthMetadataProcessorAyncWrapper::InvokeProcessor, w,
  ...
  void AuthMetadataProcessorAyncWrapper::InvokeProcessor (grpc_auth_context *
							  ctx,
							  const grpc_metadata
							  * md, size_t num_md,
							  grpc_process_auth_metadata_done_cb
							  cb, void *user_data)
  {
    ...
    Status status =
      processor_->Process (metadata, &context, &consumed_metadata,
			   &response_metadata);
  ...
  void SecureServerCredentials::SetAuthMetadataProcessor (const std::
							  shared_ptr <
							  AuthMetadataProcessor
							  > &processor)
  {
    auto *wrapper = new AuthMetadataProcessorAyncWrapper (processor);
    grpc_server_credentials_set_auth_metadata_processor (creds_,
							 {
    ...
    SslServerCredentials (const SslServerCredentialsOptions & options)
  {
    ...
    grpc_server_credentials *c_creds =
      grpc_ssl_server_credentials_create_ex (options.pem_root_certs.
					     empty ()? nullptr : options.
					     pem_root_certs.c_str (),
					     pem_key_cert_pairs.
					     empty ()? nullptr :
					     &pem_key_cert_pairs[0],
