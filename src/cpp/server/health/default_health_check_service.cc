...
namespace grpc
{
  ...
  DefaultHealthCheckService::HealthCheckServiceImpl::
    HealthCheckServiceImpl (DefaultHealthCheckService *
			    service):service_ (service), method_ (nullptr)
  {
    internal::MethodHandler * handler =
      new internal::RpcMethodHandler < HealthCheckServiceImpl, ByteBuffer,
      ByteBuffer > (std::mem_fn (&HealthCheckServiceImpl::Check), this);
    method_ =
      new internal::RpcServiceMethod (kHealthCheckMethodName,
				      internal::RpcMethod::NORMAL_RPC,
				      handler);
    AddMethod (method_);
  }

  Status DefaultHealthCheckService::HealthCheckServiceImpl::
    Check (ServerContext * context, const ByteBuffer * request,
	   ByteBuffer * response)
  {
    // Decode request.
	...
	request_bytes =
	  static_cast < uint8_t * >(gpr_malloc (request->Length ()));
	...
	pb_istream_t istream =
	  pb_istream_from_buffer (request_bytes, request_size);
	bool decode_status =
	  pb_decode (&istream, grpc_health_v1_HealthCheckRequest_fields,
		     &request_struct);
    ...
    DefaultHealthCheckService::ServingStatus serving_status =
      service_->GetServingStatus (request_struct.has_service ? request_struct.
				  service : "");
  ...
  void DefaultHealthCheckService::SetServingStatus (const grpc::
						    string & service_name,
						    bool serving)
  {
  ...
  DefaultHealthCheckService::ServingStatus
    DefaultHealthCheckService::GetServingStatus (const grpc::
