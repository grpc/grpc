...
void BindSessionToInnerServer(
    grpc_call* call, grpc::Server* inner_server,
    grpc_core::Transport** out_transport,
    grpc_event_engine::experimental::EventEngine::Endpoint** out_endpoint) {
...
void InitiateSessionGracefulShutdown(
  ...
  grpc_core::ExecCtx exec_ctx;
  if (endpoint != nullptr) {
    auto* session_endpoint = static_cast<grpc_core::SessionEndpoint*>(endpoint);
