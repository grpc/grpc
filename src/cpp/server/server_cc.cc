...
class Server::SyncRequest final : public grpc::internal::CompletionQueueTag {
 public:
  ...
  void ContinueRunAfterInterception() {
    ...
    g_callbacks->PreSynchronousRequest(&ctx_->ctx);
    auto* handler = resources_ ? method_->handler()
                               : server_->resource_exhausted_handler_.get();
    handler->RunHandler(grpc::internal::MethodHandler::HandlerParameter(
        ...
        nullptr, nullptr));
    g_callbacks->PostSynchronousRequest(&ctx_->ctx);
...
Server::Server(
  ...
  g_callbacks->UpdateArguments(args);
...
int Server::AddListeningPort(const std::string& addr,
  ...
  g_callbacks->AddPort(this, addr, creds, port);
  return port;
}
