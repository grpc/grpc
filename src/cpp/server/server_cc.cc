...
// The default value for maximum number of threads that can be created
// in the sync server. This value of INT_MAX is chosen to match the
// default behavior if no ResourceQuota is set. To modify the max number
// of threads in a sync server, pass a custom ResourceQuota object (with
// the desired number of max-threads set) to the server builder.
#define DEFAULT_MAX_SYNC_SERVER_THREADS INT_MAX

// Give a useful status error message if the resource is exhausted
// specifically because the server threadpool is full.
const char* kServerThreadpoolExhausted = "Server Threadpool Exhausted";

// Although we might like to give a useful status error message on
// unimplemented RPCs, it's not always possible since that also would
// need to be added across languages and isn't actually required by the
// spec.
...
class UnimplementedAsyncRequestContext {
 protected:
  UnimplementedAsyncRequestContext()
      : generic_stream_(&server_context_) {}
...
ServerInterface::BaseAsyncRequest::BaseAsyncRequest(
  ...
  /* Set up interception state partially for the receive ops.
   * call_wrapper_ is not filled at this point, but it will be filled
   * before the interceptors are run. */
...
bool ServerInterface::BaseAsyncRequest::FinalizeResult(void** tag,
  ...
  if (call_wrapper_.call() == nullptr) {
    ...
    call_wrapper_ =
        internal::Call(call_, server_, call_cq_,
  ...
  if (*status && call_ && call_wrapper_.server_rpc_info()) {
    ...
    interceptor_methods_.SetRecvInitialMetadata(
    ...
    if (interceptor_methods_.RunInterceptors(
      ...
      // ContinueFinalizeResultAfterInterception will be run when
...
ServerInterface::RegisteredAsyncRequest::RegisteredAsyncRequest(
    ...
    internal::ServerAsyncStreamingInterface* stream,
    CompletionQueue* call_cq, ServerCompletionQueue* notification_cq,
    void* tag, const char* name, internal::RpcMethod::RpcType type)
    : BaseAsyncRequest(server, context, stream, call_cq,
...
ServerInterface::GenericAsyncRequest::GenericAsyncRequest(
    ...
    void* tag, bool delete_on_finalize)
    : BaseAsyncRequest(server, context, stream, call_cq,
                       notification_cq, tag, delete_on_finalize) {
  grpc_call_details_init(&call_details_);
  GPR_ASSERT(notification_cq);
  GPR_ASSERT(call_cq);
  // The following call_start_batch is internally-generated so no need
  // for an explanatory log on failure.
  GPR_ASSERT(grpc_server_request_call(
                 server->server(), &call_, &call_details_,
                 context->client_metadata_.arr(), call_cq->cq(),
                 notification_cq->cq(), this) == GRPC_CALL_OK);
}

bool ServerInterface::GenericAsyncRequest::FinalizeResult(
...
class ShutdownCallback : public grpc_completion_queue_functor {
 public:
  ShutdownCallback() {
    ...
    // Set inlineable to true since this callback is trivial and thus
    // does not need to be run from the executor (triggering a thread
    // hop). This should only be used by internal callbacks like this
...
/// Use private inheritance rather than composition only to establish
/// order of construction, since the public base class should be
/// constructed after the elements belonging to the private base class
/// are constructed. This is not possible using true composition.
class Server::UnimplementedAsyncRequest final
 ...
 public:
  UnimplementedAsyncRequest(ServerInterface* server,
      ...
      : GenericAsyncRequest(server, &server_context_, &generic_stream_,
                            cq, cq, nullptr, false) {}
  ...
  grpc::GenericServerAsyncReaderWriter* stream() {
    return &generic_stream_;
  }
};

/// UnimplementedAsyncResponse should not post user-visible completions
/// to the C++ completion queue, but is generated as a CQ event by the
/// core
class Server::UnimplementedAsyncResponse final
 ...
 public:
  explicit UnimplementedAsyncResponse(
  ...
  bool FinalizeResult(void** tag, bool* status) override {
    if (grpc::internal::CallOpSet<
      ...
      // The tag was swallowed due to interception. We will see it
      // again.
    }
    return false;
  }
...
class Server::SyncRequest final
 ...
 public:
  ...
  ~SyncRequest() override {
    ...
    // constructor, since some paths may or may not actually go through
    // the Run stage where other objects are allocated.
  ...
  void Run(const std::shared_ptr<GlobalCallbacks>& global_callbacks,
    ...
    wrapped_call_.Init(call_, server_, &cq_,
                       server_->max_receive_message_size(),
                       ctx_->ctx.set_server_rpc_info(
                           method_->name(), method_->method_type(),
    ...
    if (has_request_payload_) {
      ...
      auto* handler = resources_
                          ? method_->handler()
                          : server_->resource_exhausted_handler_.get();
      deserialized_request_ = handler->Deserialize(
          call_, request_payload_, &request_status_, nullptr);
      ...
      interceptor_methods_.AddInterceptionHookPoint(
          grpc::experimental::InterceptionHookPoints::
              POST_RECV_MESSAGE);
      interceptor_methods_.SetRecvMessage(deserialized_request_,
                                          nullptr);
    }
    ...
    if (interceptor_methods_.RunInterceptors(
      ...
      // There were interceptors to be run, so
      // ContinueRunAfterInterception will be run when interceptors are
      // done.
    }
  }
  ...
  void ContinueRunAfterInterception() {
    ...
    handler->RunHandler(grpc::internal::MethodHandler::HandlerParameter(
        &*wrapped_call_, &ctx_->ctx, deserialized_request_,
        request_status_, nullptr, nullptr));
    ...
    // Cleanup structures allocated during
    // Run/ContinueRunAfterInterception
 ...
 private:
  ...
  struct ServerContextWrapper {
    ...
    ServerContextWrapper(gpr_timespec deadline,
...
class Server::CallbackRequest final
 ...
 public:
  static_assert(
      std::is_base_of<grpc::CallbackServerContext,
  ...
  // characteristics of the method being requested. For generic
  // services, method is nullptr since these services don't have
  // pre-defined methods.
  CallbackRequest(Server* server,
  ...
  // For generic services, method is nullptr since these services don't
  // have pre-defined methods.
  ...
  ~CallbackRequest() override {
    ...
    if (ctx_alloc_by_default_ ||
        server_->context_allocator() == nullptr) {
      default_ctx_.Destroy();
    }
    server_->UnrefWithPossibleNotify();
  }

  // Needs specialization to account for different processing of
  // metadata in generic API
  bool FinalizeResult(void** tag, bool* status) override;

 private:
  // method_name needs to be specialized between named method and
  // generic
  const char* method_name() const;

  class CallbackCallTag : public grpc_completion_queue_functor {
   public:
    explicit CallbackCallTag(
        Server::CallbackRequest<ServerContextType>* req)
        : req_(req) {
      functor_run = &CallbackCallTag::StaticRun;
      // Set inlineable to true since this callback is
      // internally-controlled without taking any locks, and thus does
      // not need to be run from the executor (which triggers a thread
      // hop). This should only be used by internal callbacks like this
      // and not by user application code. The work here is actually
      // non-trivial, but there is no chance of having user locks
      // conflict with each other so it's ok to run inlined.
      inlineable = true;
    }

    // force_run can not be performed on a tag if operations using this
    // tag have been sent to PerformOpsOnCall. It is intended for error
    // conditions that are detected before the operations are internally
    // processed.
   ...
   private:
    ...
    void Run(bool ok) {
      ...
      call_ = new (grpc_call_arena_alloc(req_->call_,
                                         sizeof(grpc::internal::Call)))
          grpc::internal::Call(
      ...
      if (req_->interceptor_methods_.RunInterceptors(
        ...
        // There were interceptors to be run, so
        // ContinueRunAfterInterception will be run when interceptors
        // are done.
      }
    }
    void ContinueRunAfterInterception() {
...
bool Server::CallbackRequest<
    grpc::CallbackServerContext>::FinalizeResult(void** /*tag*/,
                                                 bool* /*status*/) {
  return false;
}
...
const char* Server::CallbackRequest<
    grpc::CallbackServerContext>::method_name() const {
  return method_->name();
}
...
// Implementation of ThreadManager. Each instance of
// SyncRequestThreadManager manages a pool of threads that poll for
// incoming Sync RPCs and call the appropriate RPC handlers
class Server::SyncRequestThreadManager : public grpc::ThreadManager {
 public:
  SyncRequestThreadManager(
      Server* server, grpc::CompletionQueue* server_cq,
  ...
  WorkStatus PollForWork(void** tag, bool* ok) override {
    *tag = nullptr;
    // TODO(ctiller): workaround for GPR_TIMESPAN based deadlines not
    // working right now
  ...
  void DoWork(void* tag, bool ok, bool resources) override {
    ...
    // Under the AllocatingRequestMatcher model we will never see an
    // invalid tag here.
  ...
  void AddSyncMethod(grpc::internal::RpcServiceMethod* method,
                     void* tag) {
  ...
  void AddUnknownSyncMethod() {
    if (has_sync_method_) {
      unknown_method_ =
          absl::make_unique<grpc::internal::RpcServiceMethod>(
  ...
  void Wait() override {
    ...
    while (server_cq_->Next(&tag, &ok)) {
      // This problem can arise if the server CQ gets a request queued
      // to it before it gets shutdown but then pulls it after shutdown.
  ...
  if (sync_server_cqs_ != nullptr) {
    ...
    if (server_rq == nullptr) {
      ...
      grpc_resource_quota_set_max_threads(
          server_rq, DEFAULT_MAX_SYNC_SERVER_THREADS);
    ...
    if (started_ && !shutdown_) {
      ...
      if (callback_cq != nullptr) {
        if (grpc_iomgr_run_in_background()) {
          // gRPC-core provides the backing needed for the preferred CQ
          // type
...
std::shared_ptr<grpc::Channel> Server::InProcessChannel(
  ...
  return grpc::CreateChannelInternal(
      "inproc",
      grpc_inproc_channel_create(server_, &channel_args, nullptr),
      std::vector<std::unique_ptr<
...
Server::experimental_type::InProcessChannelWithInterceptors(
  ...
  return grpc::CreateChannelInternal(
      "inproc",
      grpc_inproc_channel_create(server_->server_, &channel_args,
                                 nullptr),
      std::move(interceptor_creators));
}

static grpc_server_register_method_payload_handling
PayloadHandlingForMethod(grpc::internal::RpcServiceMethod* method) {
...
bool Server::RegisterService(const std::string* addr,
                             grpc::Service* service) {
  bool has_async_methods = service->has_async_methods();
  if (has_async_methods) {
    GPR_ASSERT(service->server_ == nullptr &&
               "Can only register an asynchronous service against one "
               "server.");
...
void Server::RegisterAsyncGenericService(
...
void Server::RegisterCallbackGenericService(
  ...
  GPR_ASSERT(service->server_ == nullptr &&
             "Can only register a callback generic service against one "
             "server.");
  service->server_ = this;
  ...
  server_->core_server->SetBatchMethodAllocator(cq->cq(), [this, cq] {
    grpc_core::Server::BatchCallAllocation result;
    new CallbackRequest<grpc::GenericCallbackServerContext>(this, cq,
                                                            &result);
    return result;
  });
}
...
void Server::UnrefWithPossibleNotify() {
  if (GPR_UNLIKELY(shutdown_refs_outstanding_.fetch_sub(
    ...
    // No refs outstanding means that shutdown has been initiated and no
    // more callback requests are outstanding.
...
void Server::Start(grpc::ServerCompletionQueue** cqs, size_t num_cqs) {
  ...
  // Only create default health check service when user did not provide
  // an explicit one.
  ...
  if (health_check_service_ == nullptr &&
    ...
    grpc_server_register_completion_queue(
        server_, health_check_cq->cq(), nullptr);
  ...
  for (auto& acceptor : acceptors_) {
    acceptor->GetCredentials()->AddPortToServer(acceptor->name(),
                                                server_);
  }

  // If this server uses callback methods, then create a callback
  // generic service to handle any unimplemented methods using the
  // default reactor creator
  if (has_callback_methods_ && !has_callback_generic_service_) {
    unimplemented_service_ =
        absl::make_unique<grpc::CallbackGenericService>();
  ...
  // Otherwise, we must provide at least one RPC request for an
  // "unimplemented" RPC, which covers any RPC for a method name that
  // isn't matched. If we have a sync service, let it be a sync
  // unimplemented RPC, which must be registered before server start (to
  // initialize an AllocatingRequestMatcher). If we have an
  // AllocatingRequestMatcher, we can't also specify other unimplemented
  // RPCs via explicit async requests, so we won't do so. If we only
  // have async services, we can specify unimplemented RPCs on each
  // async CQ so that some user polling thread will move them along as
  // long as some progress is being made on any RPCs in the system.
  ...
  // If this server has any support for synchronous methods (has any
  // sync server CQs), make sure that we have a ResourceExhausted
  // handler to deal with the case of thread exhaustion
...
void Server::ShutdownInternal(gpr_timespec deadline) {
  ...
  /// The completion queue to use for server shutdown completion
  /// notification
  grpc::CompletionQueue shutdown_cq;
  grpc::ShutdownTag shutdown_tag;  // Phony shutdown tag
  grpc_server_shutdown_and_notify(server_, shutdown_cq.cq(),
                                  &shutdown_tag);
  ...
  // If this timed out, it means we are done with the grace period for a
  // clean shutdown. We should force a shutdown now by cancelling all
  // inflight calls
  ...
  // Shutdown all ThreadManagers. This will try to gracefully stop all
  // the threads in the ThreadManagers (once they process any inflight
  // requests)
  for (const auto& value : sync_req_mgrs_) {
  ...
  // Shutdown the callback CQ. The CQ is owned by its own shutdown tag,
  // so it will delete itself at true shutdown.
  CompletionQueue* callback_cq =
      callback_cq_.load(std::memory_order_relaxed);
  if (callback_cq != nullptr) {
    if (grpc_iomgr_run_in_background()) {
  ...
  // Drain the shutdown queue (if the previous call to AsyncNext() timed
  // out and we didn't remove the tag from the queue yet)
...
#ifndef NDEBUG
  // Unregister this server with the CQs passed into it by the user so
  // that those can be checked for properly-ordered shutdown.
...
bool Server::UnimplementedAsyncRequest::FinalizeResult(void** tag,
  ...
  if (GenericAsyncRequest::FinalizeResult(tag, status)) {
    ...
    if (*status) {
      // Create a new request/response pair using the server and CQ
      // values stored in this object's base class.
...
Server::UnimplementedAsyncResponse::UnimplementedAsyncResponse(
  ...
  grpc::internal::UnknownMethodHandler::FillOps(
      request_->context(), kUnknownRpcMethod, this);
...
grpc::CompletionQueue* Server::CallbackCQ() {
  ...
  CompletionQueue* callback_cq =
      callback_cq_.load(std::memory_order_acquire);
  if (callback_cq != nullptr) {
    return callback_cq;
  }
  // The callback_cq_ wasn't already set, so grab a lock and set it up
  // exactly once for this server.
