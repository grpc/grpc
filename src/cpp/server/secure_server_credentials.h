...
namespace grpc
{
  ...
  public:
    ...
    static void Process (void *wrapper, grpc_auth_context * context,
      ...
      AuthMetadataProcessorAyncWrapper (const std::shared_ptr <
					AuthMetadataProcessor >
					&processor):thread_pool_
      (CreateDefaultThreadPool ()), processor_ (processor)
    {
    }
  ...
  {
  public:
    explicit SecureServerCredentials (grpc_server_credentials *
				      creds):creds_ (creds)
    {
    }
