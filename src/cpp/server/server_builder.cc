...
namespace grpc
{
  ...
  ServerBuilder::ServerBuilder ():max_receive_message_size_ (-1),
    max_send_message_size_ (-1),
    sync_server_settings_ (SyncServerSettings ()),
    resource_quota_ (nullptr), generic_service_ (nullptr)
  {
    ...
    ServerBuilder::AddCompletionQueue (bool is_frequently_polled)
  {
    ServerCompletionQueue *cq =
      new ServerCompletionQueue (is_frequently_polled ?
  ...
  ServerBuilder & ServerBuilder::
    RegisterAsyncGenericService (AsyncGenericService * service)
  {
    if (generic_service_)
      {
	gpr_log (GPR_ERROR,
		 "Adding multiple AsyncGenericService is unsupported for now. "
		 "Dropping the service %p", (void *) service);
      }
    else
      {
  ...
  ServerBuilder & ServerBuilder::SetOption (std::unique_ptr <
					    ServerBuilderOption > option)
  {
    options_.push_back (std::move (option));
    return *this;
  }

  ServerBuilder & ServerBuilder::SetSyncServerOption (ServerBuilder::
						      SyncServerOption option,
						      int val)
  {
    switch (option)
      {
  ...
  ServerBuilder & ServerBuilder::
    SetCompressionAlgorithmSupportStatus (grpc_compression_algorithm
  ...
  ServerBuilder & ServerBuilder::
    SetDefaultCompressionLevel (grpc_compression_level level)
  {
  ...
  ServerBuilder & ServerBuilder::
    SetDefaultCompressionAlgorithm (grpc_compression_algorithm algorithm)
  {
  ...
  ServerBuilder & ServerBuilder::SetResourceQuota (const grpc::
						   ResourceQuota &
						   resource_quota)
  {
    if (resource_quota_ != nullptr)
  ...
  ServerBuilder & ServerBuilder::AddListeningPort (const grpc::
						   string & addr_uri,
						   std::shared_ptr <
  ...
  std::unique_ptr < Server > ServerBuilder::BuildAndStart ()
  {
    ...
    std::shared_ptr < std::vector < std::unique_ptr <
      ServerCompletionQueue >>> sync_server_cqs (std::make_shared <
						 std::vector <
						 std::unique_ptr <
	...
	for (int i = 0; i < sync_server_settings_.num_cqs; i++)
	  {
	    sync_server_cqs->
	      emplace_back (new ServerCompletionQueue (polling_type));
	  }
      }

    std::unique_ptr < Server >
      server (new
	      Server (max_receive_message_size_, &args, sync_server_cqs,
    ...
    for (auto it = sync_server_cqs->begin (); it != sync_server_cqs->end ();
	 ++it)
      {
    ...
    for (auto service = services_.begin (); service != services_.end ();
	 service++)
      {
	if (!server->
	    RegisterService ((*service)->host.get (), (*service)->service))
	  {
	    return nullptr;
	  }
      }
  ...
  void ServerBuilder::InternalAddPluginFactory (std::unique_ptr <
						ServerBuilderPlugin >
						(*CreatePlugin) ())
  {
    gpr_once_init (&once_init_plugin_list, do_plugin_list_init);
