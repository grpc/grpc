...
SecureCallCredentials::SecureCallCredentials(
...
grpc::Status StsCredentialsOptionsFromJson(
  ...
  grpc_core::Json json =
      grpc_core::Json::Parse(json_string.c_str(), &error);
  ...
  value = grpc_json_get_string_property(json, "subject_token_path",
                                        nullptr);
  if (value == nullptr) {
    ClearStsCredentialsOptions(options);
    return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT,
  ...
  value = grpc_json_get_string_property(json, "subject_token_type",
                                        nullptr);
  if (value == nullptr) {
    ClearStsCredentialsOptions(options);
    return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT,
  ...
  value = grpc_json_get_string_property(json, "requested_token_type",
                                        nullptr);
...
grpc::Status StsCredentialsOptionsFromEnv(
...
grpc_sts_credentials_options StsCredentialsCppToCoreOptions(
  ...
  opts.token_exchange_service_uri =
...
std::shared_ptr<CallCredentials> StsCredentials(
  ...
  return WrapCallCredentials(
      grpc_sts_credentials_create(&opts, nullptr));
}
...
std::shared_ptr<ChannelCredentials> AltsCredentials(
  ...
  grpc_channel_credentials* c_creds =
      grpc_alts_credentials_create(c_options);
...
std::shared_ptr<CallCredentials> AccessTokenCredentials(
  ...
  grpc::GrpcLibraryCodegen init;  // To call grpc_init().
  return WrapCallCredentials(grpc_access_token_credentials_create(
...
// Combines one channel credentials and one call credentials into a
...
std::shared_ptr<ChannelCredentials> CompositeChannelCredentials(
  ...
  // channel_creds and call_creds) into
  // grpc_composite_credentials_create will see their refcounts
  // incremented.
  SecureChannelCredentials* s_channel_creds =
      channel_creds->AsSecureCredentials();
  SecureCallCredentials* s_call_creds =
      call_creds->AsSecureCredentials();
...
void MetadataCredentialsPluginWrapper::Destroy(void* wrapper) {
  ...
  grpc_core::Executor::Run(
      GRPC_CLOSURE_CREATE(DeleteWrapper, wrapper, nullptr),
      GRPC_ERROR_NONE);
}
...
int MetadataCredentialsPluginWrapper::GetMetadata(
  ...
  if (w->plugin_->IsBlocking()) {
    // The internals of context may be destroyed if GetMetadata is
    // cancelled. Make a copy for InvokePlugin.
    grpc_auth_metadata_context context_copy =
    ...
    w->thread_pool_->Add([w, context_copy, cb, user_data]() mutable {
      w->MetadataCredentialsPluginWrapper::InvokePlugin(
          context_copy, cb, user_data, nullptr, nullptr, nullptr,
          nullptr);
...
void MetadataCredentialsPluginWrapper::InvokePlugin(
  ...
  // const_cast is safe since the SecureAuthContext only inc/dec the
  // refcount and the object is passed as a const ref to
  // plugin_->GetMetadata.
  ...
  if (creds_md != nullptr) {
    ...
    if (md.size() > GRPC_METADATA_CREDENTIALS_PLUGIN_SYNC_MAX) {
      ...
      *error_details = gpr_strdup(
          "blocking plugin credentials returned too many metadata "
          "keys");
      UnrefMetadata(md);
    } else {
