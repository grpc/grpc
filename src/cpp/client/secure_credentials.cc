...
namespace grpc
{
  ...
  static internal::GrpcLibraryInitializer g_gli_initializer;
    ...
    SecureChannelCredentials (grpc_channel_credentials *
			      c_creds):c_creds_ (c_creds)
  {
    g_gli_initializer.summon ();
  }

  std::shared_ptr < grpc::Channel >
    SecureChannelCredentials::CreateChannel (const string & target,
					     const grpc::
					     ChannelArguments & args)
  {
    grpc_channel_args channel_args;
    args.SetChannelArgs (&channel_args);
    return CreateChannelInternal (args.GetSslTargetNameOverride (),
				  grpc_secure_channel_create (c_creds_,
							      target.c_str (),
							      &channel_args,
							      nullptr));
  }

SecureCallCredentials::SecureCallCredentials (grpc_call_credentials * c_creds):c_creds_
    (c_creds)
  {
    g_gli_initializer.summon ();
  }
  ...
  namespace
  {
    std::shared_ptr < ChannelCredentials >
      WrapChannelCredentials (grpc_channel_credentials * creds)
    {
    ...
    std::shared_ptr < CallCredentials >
      WrapCallCredentials (grpc_call_credentials * creds)
    {
      return creds == nullptr ? nullptr
	: std::shared_ptr < CallCredentials >
...
// Builds SSL Credentials given SSL specific options
    ...
    SslCredentials (const SslCredentialsOptions & options)
  {
    ...
    grpc_channel_credentials *c_creds =
      grpc_ssl_credentials_create (options.pem_root_certs.
				   empty ()? nullptr : options.pem_root_certs.
				   c_str (),
				   options.pem_private_key.
				   empty ()? nullptr : &pem_key_cert_pair,
				   nullptr);
...
// Builds credentials for use when running in GCE
  std::shared_ptr < CallCredentials > GoogleComputeEngineCredentials ()
  {
    GrpcLibraryCodegen init;	// To call grpc_init().
    return
      WrapCallCredentials (grpc_google_compute_engine_credentials_create
			   (nullptr));
  }

// Builds JWT credentials.
  std::shared_ptr < CallCredentials >
    ServiceAccountJWTAccessCredentials (const grpc::string & json_key,
					long token_lifetime_seconds)
  {
    ...
    return
      WrapCallCredentials (grpc_service_account_jwt_access_credentials_create
...
// Builds refresh token credentials.
    ...
    GoogleRefreshTokenCredentials (const grpc::string & json_refresh_token)
  {
    GrpcLibraryCodegen init;	// To call grpc_init().
    return
      WrapCallCredentials (grpc_google_refresh_token_credentials_create
...
// Builds access token credentials.
    ...
    AccessTokenCredentials (const grpc::string & access_token)
  {
    GrpcLibraryCodegen init;	// To call grpc_init().
    return
      WrapCallCredentials (grpc_access_token_credentials_create
			   (access_token.c_str (), nullptr));
  }

// Builds IAM credentials.
  std::shared_ptr < CallCredentials >
    GoogleIAMCredentials (const grpc::string & authorization_token,
			  const grpc::string & authority_selector)
  {
    GrpcLibraryCodegen init;	// To call grpc_init().
    return
      WrapCallCredentials (grpc_google_iam_credentials_create
			   (authorization_token.c_str (),
			    authority_selector.c_str (), nullptr));
  }
...
// composite credentials.
				 ...
				 &call_creds)
  {
	...
	return
	  WrapChannelCredentials (grpc_composite_channel_credentials_create
				  (s_channel_creds->GetRawCreds (),
				   s_call_creds->GetRawCreds (), nullptr));
      }
    return nullptr;
  }

  std::shared_ptr < CallCredentials >
    CompositeCallCredentials (const std::shared_ptr < CallCredentials >
			      &creds1,
			      const std::shared_ptr < CallCredentials >
			      &creds2)
  {
    ...
    if (s_creds1 != nullptr && s_creds2 != nullptr)
      {
	return
	  WrapCallCredentials (grpc_composite_call_credentials_create
			       (s_creds1->GetRawCreds (),
				s_creds2->GetRawCreds (), nullptr));
      }
    return nullptr;
  }
  ...
  int MetadataCredentialsPluginWrapper::GetMetadata (void *wrapper,
						     ...
						     **error_details)
  {
	...
	*error_details = nullptr;
	...
	w->thread_pool_->
	  Add (std::
	       bind (&MetadataCredentialsPluginWrapper::InvokePlugin, w,
		     context, cb, user_data, nullptr, nullptr, nullptr,
		     nullptr));
	return 0;
      }
    else
      {
	// Synchronous return.
	w->InvokePlugin (context, cb, user_data, creds_md, num_creds_md,
  ...
  void MetadataCredentialsPluginWrapper::
    InvokePlugin (grpc_auth_metadata_context context,
		  grpc_credentials_plugin_metadata_cb cb, void *user_data,
		  grpc_metadata creds_md[4], size_t * num_creds_md,
		  grpc_status_code * status_code, const char **error_details)
  {
    ...
    SecureAuthContext cpp_channel_auth_context (const_cast <
						grpc_auth_context *
						>(context.
	...
	if (md.size () > GRPC_METADATA_CREDENTIALS_PLUGIN_SYNC_MAX)
	  ...
	  for (const auto & elem:md)
	    ...
	    *error_details =
	      status.ok ()? nullptr : gpr_strdup (status.error_message ().
						  c_str ());
	  }
      }
    else
...
MetadataCredentialsPluginWrapper::MetadataCredentialsPluginWrapper (std::unique_ptr < MetadataCredentialsPlugin > plugin):thread_pool_ (CreateDefaultThreadPool ()),
    plugin_ (std::
	     move (plugin))
  {
  }
  ...
  std::shared_ptr < CallCredentials >
    MetadataCredentialsFromPlugin (std::unique_ptr <
				   MetadataCredentialsPlugin > plugin)
  {
    GrpcLibraryCodegen init;	// To call grpc_init().
    ...
    return
      WrapCallCredentials (grpc_metadata_credentials_create_from_plugin
			   (c_plugin, nullptr));
  }
