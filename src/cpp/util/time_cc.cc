  ...
  grpc
{
  ...
  void
  Timepoint2Timespec (const system_clock::time_point & from,
		      gpr_timespec * to)
  {
    system_clock::duration deadline = from.time_since_epoch ();
    seconds
      secs = duration_cast < seconds > (deadline);
	...
	*to = gpr_inf_future (GPR_CLOCK_REALTIME);
    ...
    nanoseconds
      nsecs = duration_cast < nanoseconds > (deadline - secs);
  ...
  void
  TimepointHR2Timespec (const high_resolution_clock::time_point & from,
			gpr_timespec * to)
  {
  ...
  system_clock::time_point Timespec2Timepoint (gpr_timespec t)
  {
    ...
    tp +=
      duration_cast < system_clock::time_point::duration >
      (seconds (t.tv_sec));
    tp +=
      duration_cast < system_clock::time_point::duration >
