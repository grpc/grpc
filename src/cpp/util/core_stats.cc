  ...
  grpc
{
  ...
  void
  CoreStatsToProto (const grpc_stats_data & core, Stats * proto)
  {
	...
	Metric *
	  m = proto->add_metrics ();
	m->
	set_name (grpc_stats_counter_name[i]);
	m->
	set_count (core.counters[i]);
      }
  ...
  void
  ProtoToCoreStats (const grpc::core::Stats & proto, grpc_stats_data * core)
  {
