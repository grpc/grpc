...
// AltsContext. Normal users should use GetAltsContextFromAuthContext to
// get AltsContext, instead of constructing their own.
AltsContext::AltsContext(const grpc_gcp_AltsContext* ctx) {
  ...
  if (application_protocol.data != nullptr &&
      application_protocol.size > 0) {
    application_protocol_ = std::string(application_protocol.data,
                                        application_protocol.size);
  }
  upb_strview record_protocol =
      grpc_gcp_AltsContext_record_protocol(ctx);
  ...
  if (peer_service_account.data != nullptr &&
      peer_service_account.size > 0) {
    peer_service_account_ = std::string(peer_service_account.data,
                                        peer_service_account.size);
  }
  upb_strview local_service_account =
      grpc_gcp_AltsContext_local_service_account(ctx);
  if (local_service_account.data != nullptr &&
      local_service_account.size > 0) {
    local_service_account_ = std::string(local_service_account.data,
                                         local_service_account.size);
  }
  ...
  if (versions != nullptr) {
    ...
    if (max_version != nullptr) {
      ...
      peer_rpc_versions_.max_rpc_version.major_version =
          max_version_major;
      peer_rpc_versions_.max_rpc_version.minor_version =
          max_version_minor;
    }
    ...
    if (min_version != nullptr) {
      ...
      peer_rpc_versions_.min_rpc_version.major_version =
          min_version_major;
      peer_rpc_versions_.min_rpc_version.minor_version =
          min_version_minor;
    }
  }
  ...
  if (grpc_gcp_AltsContext_has_peer_attributes(ctx)) {
    ...
    const grpc_gcp_AltsContext_PeerAttributesEntry*
...
std::string AltsContext::record_protocol() const {
...
AltsContext::RpcProtocolVersions AltsContext::peer_rpc_versions()
    const {
  return peer_rpc_versions_;
}

const std::map<std::string, std::string>& AltsContext::peer_attributes()
    const {
