...
const grpc_completion_queue_factory* CoreCodegen::grpc_completion_queue_factory_lookup(
...
grpc_completion_queue* CoreCodegen::grpc_completion_queue_create_for_next(void* reserved) {
  return ::grpc_completion_queue_create_for_next(reserved);
}

grpc_completion_queue* CoreCodegen::grpc_completion_queue_create_for_pluck(void* reserved) {
...
grpc_event CoreCodegen::grpc_completion_queue_pluck(grpc_completion_queue* cq, void* tag,
...
int CoreCodegen::gpr_cv_wait(gpr_cv* cv, gpr_mu* mu, gpr_timespec abs_deadline) {
...
grpc_call_error CoreCodegen::grpc_call_start_batch(grpc_call* call, const grpc_op* ops, size_t nops,
                                                   void* tag, void* reserved) {
  return ::grpc_call_start_batch(call, ops, nops, tag, reserved);
}

grpc_call_error CoreCodegen::grpc_call_cancel_with_status(grpc_call* call, grpc_status_code status,
...
void CoreCodegen::grpc_byte_buffer_reader_destroy(grpc_byte_buffer_reader* reader) {
  ::grpc_byte_buffer_reader_destroy(reader);
}

int CoreCodegen::grpc_byte_buffer_reader_next(grpc_byte_buffer_reader* reader, grpc_slice* slice) {
  return ::grpc_byte_buffer_reader_next(reader, slice);
}

int CoreCodegen::grpc_byte_buffer_reader_peek(grpc_byte_buffer_reader* reader, grpc_slice** slice) {
  return ::grpc_byte_buffer_reader_peek(reader, slice);
}

grpc_byte_buffer* CoreCodegen::grpc_raw_byte_buffer_create(grpc_slice* slice, size_t nslices) {
  return ::grpc_raw_byte_buffer_create(slice, nslices);
}

grpc_slice CoreCodegen::grpc_slice_new_with_user_data(void* p, size_t len, void (*destroy)(void*),
                                                      void* user_data) {
...
grpc_slice CoreCodegen::grpc_slice_malloc(size_t length) { return ::grpc_slice_malloc(length); }

void CoreCodegen::grpc_slice_unref(grpc_slice slice) { ::grpc_slice_unref(slice); }

grpc_slice CoreCodegen::grpc_slice_ref(grpc_slice slice) { return ::grpc_slice_ref(slice); }
...
grpc_slice CoreCodegen::grpc_slice_from_static_buffer(const void* buffer, size_t length) {
  return ::grpc_slice_from_static_buffer(buffer, length);
}

grpc_slice CoreCodegen::grpc_slice_from_copied_buffer(const void* buffer, size_t length) {
  return ::grpc_slice_from_copied_buffer(static_cast<const char*>(buffer), length);
}
...
void CoreCodegen::grpc_slice_buffer_pop(grpc_slice_buffer* sb) { ::grpc_slice_buffer_pop(sb); }
...
gpr_timespec CoreCodegen::gpr_inf_future(gpr_clock_type type) { return ::gpr_inf_future(type); }

gpr_timespec CoreCodegen::gpr_time_0(gpr_clock_type type) { return ::gpr_time_0(type); }

void CoreCodegen::assert_fail(const char* failed_assertion, const char* file, int line) {
  gpr_log(file, line, GPR_LOG_SEVERITY_ERROR, "assertion failed: %s", failed_assertion);
  abort();
}
