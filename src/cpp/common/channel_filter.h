...
/// A C++ wrapper for the \c grpc_metadata_batch struct.
  ...
  public:
    ...
    class const_iterator:public std::iterator <
      std::bidirectional_iterator_tag, const grpc_mdelem >
    {
    public:
...
/// A C++ wrapper for the \c grpc_transport_stream_op_batch struct.
  ...
  public:
    ...
    explicit TransportStreamOpBatch (grpc_transport_stream_op_batch *
				     op):op_ (op),
      send_initial_metadata_ (op->send_initial_metadata ? op->payload->
			      send_initial_metadata.
			      send_initial_metadata : nullptr),
      send_trailing_metadata_ (op->send_trailing_metadata ? op->payload->
			       send_trailing_metadata.
			       send_trailing_metadata : nullptr),
      recv_initial_metadata_ (op->recv_initial_metadata ? op->payload->
			      recv_initial_metadata.
			      recv_initial_metadata : nullptr),
      recv_trailing_metadata_ (op->recv_trailing_metadata ? op->payload->
			       recv_trailing_metadata.
			       recv_trailing_metadata : nullptr)
    {
    }
    ...
    {
      return op_->send_initial_metadata ? &op_->payload->
	send_initial_metadata.send_initial_metadata_flags : nullptr;
    }
    ...
    {
      return (census_context *) op_->payload->context[GRPC_CONTEXT_TRACING].
	value;
    }
...
// grpc_channel_filter struct.
	...
	CallDataType *call_data =
	  reinterpret_cast < CallDataType * >(elem->call_data);
...
/// value is nullptr, the filter will be added unconditionally.
				...
				include_filter)
  {
    ...
    typedef internal::ChannelFilter < ChannelDataType,
      CallDataType > FilterType;
