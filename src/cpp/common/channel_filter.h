...
class MetadataBatch {
 public:
  ...
  grpc_linked_mdelem* AddMetadata(const string& key,
                                  const string& value);
  ...
  const_iterator begin() const {
    return const_iterator(batch_->list.head);
  }
...
class TransportOp {
 public:
  ...
  bool send_goaway() const {
    return op_->goaway_error != GRPC_ERROR_NONE;
  }
...
class TransportStreamOpBatch {
 public:
  ...
  MetadataBatch* send_initial_metadata() {
    return op_->send_initial_metadata ? &send_initial_metadata_
                                      : nullptr;
  }
  MetadataBatch* send_trailing_metadata() {
    return op_->send_trailing_metadata ? &send_trailing_metadata_
                                       : nullptr;
  }
  MetadataBatch* recv_initial_metadata() {
    return op_->recv_initial_metadata ? &recv_initial_metadata_
                                      : nullptr;
  }
  MetadataBatch* recv_trailing_metadata() {
    return op_->recv_trailing_metadata ? &recv_trailing_metadata_
                                       : nullptr;
  }
  ...
  grpc_closure* recv_initial_metadata_ready() const {
    return op_->recv_initial_metadata
               ? op_->payload->recv_initial_metadata
...
class ChannelData {
 public:
  ...
  // TODO(roth): Come up with a more C++-like API for the channel
  // element.
  ...
  virtual void StartTransportOp(grpc_channel_element* elem,
                                TransportOp* op);
...
class CallData {
 public:
  ...
  virtual grpc_error_handle Init(
 ...
 public:
  ...
  static void DestroyCallElement(grpc_call_element* elem,
    ...
    CallDataType* call_data =
        static_cast<CallDataType*>(elem->call_data);
...
/// The \a include_filter argument specifies a function that will be
/// called to determine at run-time whether or not to add the filter. If
/// the value is nullptr, the filter will be added unconditionally. If
/// the channel stack type is GRPC_CLIENT_SUBCHANNEL, the caller should
/// ensure that subchannels with different filter lists will always have
/// different channel args. This requires setting a channel arg in case
/// the registration function relies on some condition other than
/// channel args to decide whether to add a filter or not.
template <typename ChannelDataType, typename CallDataType>
void RegisterChannelFilter(
  ...
  if (internal::channel_filters == nullptr) {
    ...
    internal::channel_filters =
        new std::vector<internal::FilterRecord>();
  }
  ...
  typedef internal::ChannelFilter<ChannelDataType, CallDataType>
      FilterType;
  internal::FilterRecord filter_record = {
      ...
      {FilterType::StartTransportStreamOpBatch,
       ...
       FilterType::DestroyChannelElement, FilterType::GetChannelInfo,
       name}};
