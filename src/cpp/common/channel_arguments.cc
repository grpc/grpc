...
namespace grpc
{
  ...
  ChannelArguments::
    ChannelArguments (const ChannelArguments & other):strings_ (other.
								strings_)
  {
  ...
  void ChannelArguments::
    SetCompressionAlgorithm (grpc_compression_algorithm algorithm)
  {
...
// each with more significant identifier.
  void ChannelArguments::SetUserAgentPrefix (const grpc::
					     string & user_agent_prefix)
  {
  ...
  void ChannelArguments::SetResourceQuota (const grpc::
  ...
  void ChannelArguments::SetLoadBalancingPolicyName (const grpc::
						     string & lb_policy_name)
  {
    SetString (GRPC_ARG_LB_POLICY_NAME, lb_policy_name);
  }

  void ChannelArguments::SetServiceConfigJSON (const grpc::
					       string & service_config_json)
  {
  ...
  void ChannelArguments::SetPointerWithVtable (const grpc::string & key,
					       void *value,
