...
namespace grpc
{
  ...
  ThreadManager::WorkerThread::WorkerThread (ThreadManager *
					     thd_mgr):thd_mgr_ (thd_mgr)
  {
  ...
  ThreadManager::ThreadManager (int min_pollers,
				int max_pollers):shutdown_ (false),
    num_pollers_ (0), min_pollers_ (min_pollers),
    max_pollers_ (max_pollers == -1 ? INT_MAX : max_pollers), num_threads_ (0)
  {
  }
  ...
  for (auto thd:completed_threads)
      delete thd;
  }
