  ...
  grpc
{
  ...
  void
  ProtoServerReflection::SetServiceList (const std::vector < grpc::string >
					 *services)
  {
    services_ = services;
  }
  ...
  Status
  ProtoServerReflection::ServerReflectionInfo (ServerContext * context,
					       ServerReaderWriter <
					       ...
					       *stream)
  {
	...
	switch (request.message_request_case ())
	  ...
	  case ServerReflectionRequest::MessageRequestCase::kAllExtensionNumbersOfType:
	    status =
	      GetAllExtensionNumbers (context,
				      request.
				      all_extension_numbers_of_type (),
				      response.
				      mutable_all_extension_numbers_response
				      ());
	    break;
	...
	response.
	  set_allocated_original_request (new
  ...
  void
  ProtoServerReflection::FillErrorResponse (const Status & status,
  ...
  Status
  ProtoServerReflection::ListService (ServerContext * context,
				      ListServiceResponse * response)
  {
    ...
    for (auto it = services_->begin (); it != services_->end (); ++it)
      {
	ServiceResponse *
	  service_response = response->add_service ();
  ...
  Status
  ProtoServerReflection::GetFileByName (ServerContext * context,
					...
					ServerReflectionResponse * response)
  {
    ...
    const
      ...
      file_desc = descriptor_pool_->FindFileByName (filename);
  ...
  Status
  ProtoServerReflection::GetFileContainingSymbol (ServerContext * context,
						  const grpc::string & symbol,
						  ServerReflectionResponse *
						  response)
  {
    if (descriptor_pool_ == nullptr)
    ...
    const
      protobuf::FileDescriptor *
      file_desc = descriptor_pool_->FindFileContainingSymbol (symbol);
    if (file_desc == nullptr)
      {
  ...
  Status
  ProtoServerReflection::GetFileContainingExtension (ServerContext * context,
						     ...
						     * response)
  {
    ...
    const
      ...
      field_desc = descriptor_pool_->FindExtensionByNumber (desc,
							    request->
							    extension_number
							    ());
  ...
  Status
  ProtoServerReflection::GetAllExtensionNumbers (ServerContext * context,
						 ...
						 response)
  {
    ...
    const
      protobuf::Descriptor *
      desc = descriptor_pool_->FindMessageTypeByName (type);
    if (desc == nullptr)
      {
	return Status (StatusCode::NOT_FOUND, "Type not found.");
      }
  ...
  void
  ProtoServerReflection::FillFileDescriptorResponse (const protobuf::
						     FileDescriptor *
						     file_desc,
						     ServerReflectionResponse
						     * response,
