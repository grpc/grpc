...
class CensusContext {
 public:
  CensusContext()
      : span_(::opencensus::trace::Span::BlankSpan()), tags_({}) {}
...
// Serializes the outgoing stats context.  Field IDs are 1 byte followed
// by field data. A 1 byte version ID is always encoded first. Tags are
// directly serialized into the given grpc_slice.
size_t StatsContextSerialize(size_t max_tags_len, grpc_slice* tags);

// Serialize outgoing server stats. Returns the number of bytes
// serialized.
size_t ServerStatsSerialize(uint64_t server_elapsed_time, char* buf,
                            size_t buf_size);

// Deserialize incoming server stats. Returns the number of bytes
// deserialized.
size_t ServerStatsDeserialize(const char* buf, size_t buf_size,
                              uint64_t* server_elapsed_time);

// Deserialize the incoming SpanContext and generate a new server
// context based on that. This new span will never be a root span. This
// should only be called with a blank CensusContext as it overwrites it.
...
// span automatically becomes a root span. This should only be called
...
// associated with the census_context* stored by grpc. The user will
// need to call this for manual propagation of tracing data.
::opencensus::trace::SpanContext SpanContextFromCensusContext(
    const census_context* ctxt);
::opencensus::trace::Span SpanFromCensusContext(
    const census_context* ctxt);
