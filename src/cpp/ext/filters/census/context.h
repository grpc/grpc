...
class CensusContext {
 public:
  ...
  CensusContext(absl::string_view name, const ::opencensus::trace::SpanContext& parent_ctxt)
      : span_(::opencensus::trace::Span::StartSpanWithRemoteParent(name, parent_ctxt)), tags_({}) {}
...
::opencensus::trace::SpanContext SpanContextFromCensusContext(const census_context* ctxt);
...
inline absl::string_view GetMethod(const grpc_slice* path) {
  ...
  return absl::StripPrefix(
      absl::string_view(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(*path)),
