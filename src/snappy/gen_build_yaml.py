...
try:
    with open('third_party/snappy/CMakeLists.txt') as f:
        cmake = f.read()

    def cmpath(x):
        return 'third_party/snappy/%s' % x.replace('${CMAKE_CURRENT_BINARY_DIR}/',
                                                 '')
    ...
    out['libs'] = [{
        ...
        # TODO(mattbrown): need to generate snappy-stubs-public.h similar to third_party/snappy.BUILD
        'headers':
            sorted([
