...
task 'dlls', [:plat] do |t, args|
  ...
  prepare_ccache_cmd += "  if curl -L -f https://github.com/ccache/ccache/releases/download/v4.10.2/ccache-4.10.2-linux-$ARCH.tar.xz | tar -xJ -C /tmp; then "
  prepare_ccache_cmd += "    export PATH=\"$PATH:/tmp/ccache-4.10.2-linux-$ARCH\"; "
