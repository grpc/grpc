...
/// A thin wrapper around \a grpc_alarm (see / \a / src/core/surface/alarm.h).
  ...
  public:
    ...
    /// here. \endinternal.
    template < typename T >
      Alarm (CompletionQueue * cq, const T & deadline, void *tag):tag_ (tag),
      alarm_ (grpc_alarm_create (nullptr))
    {
      grpc_alarm_set (alarm_, cq->cq (),
  ...
  Alarm (Alarm && rhs):tag_ (rhs.tag_), alarm_ (rhs.alarm_)
    ...
    {
      if (alarm_ != nullptr)
	grpc_alarm_destroy (alarm_, nullptr);
    }
    ...
    {
      if (alarm_ != nullptr)
	grpc_alarm_cancel (alarm_, nullptr);
    }
