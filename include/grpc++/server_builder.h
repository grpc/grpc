...
/// A builder class for the creation and startup of \a grpc::Server instances.
  ...
  public:
    ...
    ServerBuilder & AddListeningPort (const grpc::string & addr_uri,
				      ...
				      creds, int *selected_port = nullptr);
    ...
    /// use cases.
      ...
      AddCompletionQueue (bool is_frequently_polled = true);
    ...
    /// serialization format is unknown
      ServerBuilder & RegisterAsyncGenericService (AsyncGenericService *
						   service);
    ...
    ServerBuilder & SetDefaultCompressionAlgorithm (grpc_compression_algorithm
						    algorithm);

    /// Set the attached buffer pool for this server
    ServerBuilder & SetResourceQuota (const ResourceQuota & resource_quota);

    ServerBuilder & SetOption (std::unique_ptr < ServerBuilderOption >
			       option);
    ...
    /// directly)
    template < class T >
      ServerBuilder & AddChannelArgument (const grpc::string & arg,
					  const T & value)
    {
    ...
    static void InternalAddPluginFactory (std::unique_ptr <
					  ServerBuilderPlugin >
					  (*CreatePlugin) ());
  ...
  private:
    ...
    {
      SyncServerSettings ():num_cqs (1), min_pollers (1), max_pollers (2),
    ...
    struct NamedService
    {
      ...
      NamedService (const grpc::string & h,
		    Service * s):host (new grpc::string (h)), service (s)
      {
      }
      HostString host;
      Service *service;
