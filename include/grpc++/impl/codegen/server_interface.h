...
/// Servers are configured and started via \a grpc::ServerBuilder.
  ...
  public:
    ...
    /// terminated.
    template < class T > void Shutdown (const T & deadline)
    {
    ...
    void Shutdown ()
    {
      ShutdownInternal (g_core_codegen_interface->
  ...
  protected:
    ...
    virtual bool RegisterService (const grpc::string * host,
				  Service * service) = 0;

    /// Register a generic service. This call does not take ownership of the
    /// service. The service must exist for the lifetime of the Server instance.
    virtual void RegisterAsyncGenericService (AsyncGenericService * service) =
      0;
    ...
    public:
      RegisteredAsyncRequest (ServerInterface * server,
			      ...
			      stream, CompletionQueue * call_cq, void *tag);
    ...
    {
    public:
      NoPayloadAsyncRequest (void *registered_method,
			     ServerInterface * server,
			     ServerContext * context,
			     internal::ServerAsyncStreamingInterface * stream,
			     CompletionQueue * call_cq,
			     ServerCompletionQueue * notification_cq,
			     void *tag):RegisteredAsyncRequest (server,
								context,
								stream,
								call_cq, tag)
      {
  ...
  template < class Message > class PayloadAsyncRequest final:public
    ...
    public:
      PayloadAsyncRequest (void *registered_method, ServerInterface * server,
			   ...
			   Message * request):RegisteredAsyncRequest (server,
								      context,
								      stream,
								      call_cq,
								      tag),
	registered_method_ (registered_method), server_ (server),
	context_ (context), stream_ (stream), call_cq_ (call_cq),
	notification_cq_ (notification_cq), tag_ (tag), request_ (request)
      {
	...
	if (*status)
	  {
	    if (payload_ == nullptr ||
		!SerializationTraits < Message >::Deserialize (payload_,
							       request_).
		ok ())
	      {
		...
		// the application.
		g_core_codegen_interface->grpc_call_cancel_with_status (call_,
									GRPC_STATUS_INTERNAL,
									"Unable to parse request",
									nullptr);
		g_core_codegen_interface->grpc_call_unref (call_);
		new PayloadAsyncRequest (registered_method_, server_,
					 context_, stream_, call_cq_,
			     ...
			     void *tag, Message * message)
    {
      ...
      new PayloadAsyncRequest < Message > (method->server_tag (), this,
					   context, stream, call_cq,
