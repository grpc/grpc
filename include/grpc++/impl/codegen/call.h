...
// mess. Make sure it does not happen.
    inline grpc_metadata *FillMetadataArray (const std::multimap <
					     grpc::string,
					     grpc::string > &metadata,
					     size_t * metadata_count,
					     const grpc::
					     string & optional_error_details)
    {
      *metadata_count =
	...
	(grpc_metadata *) (g_core_codegen_interface->gpr_malloc ((*metadata_count) * sizeof (grpc_metadata)));
      size_t i = 0;
      ...
      if (!optional_error_details.empty ())
	{
	  metadata_array[i].key =
	    ...
	    grpc_slice_from_static_buffer (kBinaryErrorDetailsKey,
					   sizeof (kBinaryErrorDetailsKey) -
					   1);
...
/// Per-message write options.
  ...
  public:
    ...
    WriteOptions (const WriteOptions & other):flags_ (other.flags_),
      last_message_ (other.last_message_)
    {
    }
...
/// used for generating multiple names for the same thing.
	...
	maybe_stream_compression_level_.is_set = false;
      ...
      void SendInitialMetadata (const std::multimap < grpc::string,
				grpc::string > &metadata, uint32_t flags)
      {
	maybe_compression_level_.is_set = false;
	maybe_stream_compression_level_.is_set = false;
	send_ = true;
	...
	if (maybe_stream_compression_level_.is_set)
	  {
	    op->data.send_initial_metadata.maybe_stream_compression_level.
	      level = maybe_stream_compression_level_.level;
	  }
      }
      void FinishOp (bool * status)
      {
					     ...
					     WriteOptions options)
    {
      ...
      Status result =
	SerializationTraits < M, void >::Serialize (message,
						    send_buf_.bbuf_ptr (),
						    &own_buf);
      if (!own_buf)
	{
    ...
    {
    public:
    CallOpRecvMessage ():got_message (false),
	message_ (nullptr),
	...
	if (recv_buf_.Valid ())
	  {
		...
		got_message = *status =
		  SerializationTraits <
		  R >::Deserialize (recv_buf_.bbuf_ptr (), message_).ok ();
		recv_buf_.Release ();
	      }
	    else
	      {
  ...
  template < class R > class DeserializeFuncType final:public
      DeserializeFunc
    {
    public:
    ...
    public:
      ...
      void ServerSendStatus (const std::multimap < grpc::string,
			     grpc::string > &trailing_metadata,
			     const Status & status)
      {
	send_error_details_ = status.error_details ();
	trailing_metadata_ =
	  FillMetadataArray (trailing_metadata, &trailing_metadata_count_,
			     send_error_details_);
	...
	*recv_status_ = Status (static_cast < StatusCode > (status_code_),
				grpc::
				string (GRPC_SLICE_START_PTR (error_message_),
...
/// API.
    ...
    public:
      ...
      /// upwards.
      virtual void FillOps (grpc_call * call, grpc_op * ops, size_t * nops) =
	0;
    };
...
/// the template parmeter for CallNoOp is varied by argument position.
  template < class Op1 = CallNoOp < 1 >, class Op2 = CallNoOp < 2 >, class Op3 = CallNoOp < 3 >, class Op4 = CallNoOp < 4 >, class Op5 = CallNoOp < 5 >, class Op6 = CallNoOp < 6 >> class CallOpSet:public CallOpSetInterface,
      public Op1,
      public Op2, public Op3, public Op4, public Op5, public Op6
    {
    public:
...
/// C++ users.
  template < class Op1 = CallNoOp < 1 >, class Op2 = CallNoOp < 2 >, class Op3 = CallNoOp < 3 >, class Op4 = CallNoOp < 4 >, class Op5 = CallNoOp < 5 >, class Op6 = CallNoOp < 6 >> class SneakyCallOpSet:public CallOpSet < Op1, Op2, Op3, Op4, Op5,
      Op6
      >
    {
    public:
...
/// Straightforward wrapping of the C call object
  ...
  /** call is owned by the caller */
      Call (grpc_call * call, CallHook * call_hook,
	    CompletionQueue * cq):call_hook_ (call_hook), cq_ (cq),
	call_ (call), max_receive_message_size_ (-1)
      {
      }

      Call (grpc_call * call, CallHook * call_hook, CompletionQueue * cq,
	    int max_receive_message_size):call_hook_ (call_hook),
	cq_ (cq),
	call_ (call), max_receive_message_size_ (max_receive_message_size)
      {
      }
