  ...
  namespace internal
  {
    ...
    public:
      GrpcBufferWriter (grpc_byte_buffer ** bp, int block_size,
			int total_size):block_size_ (block_size),
	total_size_ (total_size), byte_count_ (0), have_backup_ (false)
      {
	...
	if (have_backup_)
	...
	else
	  {
	    ...
	    size_t remain = total_size_ - byte_count_ > block_size_
	      ? block_size_ : total_size_ - byte_count_;
	    slice_ =
	      g_core_codegen_interface->grpc_slice_malloc (remain >
	...
	*data = GRPC_SLICE_START_PTR (slice_);
	...
	g_core_codegen_interface->grpc_slice_buffer_add (slice_buffer_,
							 slice_);
	return true;
      }
	...
	if ((size_t) count == GRPC_SLICE_LENGTH (slice_))
	  ...
	  {
	    backup_slice_ =
	      g_core_codegen_interface->grpc_slice_split_tail (&slice_,
							       GRPC_SLICE_LENGTH
							       (slice_) -
							       count);
	    g_core_codegen_interface->grpc_slice_buffer_add (slice_buffer_,
    ...
    {
    public:
      explicit GrpcBufferReader (grpc_byte_buffer * buffer):byte_count_ (0),
	backup_count_ (0), status_ ()
      {
	...
	if (backup_count_ > 0)
	  {
	    *data =
	      GRPC_SLICE_START_PTR (slice_) + GRPC_SLICE_LENGTH (slice_) -
	      backup_count_;
...
// BufferWriter must be a subclass of io::ZeroCopyOutputStream.
      ...
      if ((size_t) byte_size <= GRPC_SLICE_INLINED_SIZE)
	{
	  ...
	  GPR_CODEGEN_ASSERT (GRPC_SLICE_END_PTR (slice) ==
			      msg.
			      SerializeWithCachedSizesToArray
			      (GRPC_SLICE_START_PTR (slice)));
	  *bp =
	    g_core_codegen_interface->grpc_raw_byte_buffer_create (&slice, 1);
...
// BufferReader must be a subclass of io::ZeroCopyInputStream.
	...
	if (!decoder.ConsumedEntireMessage ())
	  {
	    result =
	      Status (StatusCode::INTERNAL, "Did not read entire message");
	  }
      }
...
// be found in include/grpc++/impl/codegen/serialization_traits.h.
  ...
  public:
    static Status Serialize (const grpc::protobuf::Message & msg,
			     grpc_byte_buffer ** bp, bool * own_buffer)
    {
      return internal::GenericSerialize < internal::GrpcBufferWriter,
	T > (msg, bp, own_buffer);
    }
    ...
    static Status Deserialize (grpc_byte_buffer * buffer,
			       grpc::protobuf::Message * msg)
    {
      return internal::GenericDeserialize < internal::GrpcBufferReader,
	T > (buffer, msg);
    }
  };
#endif
