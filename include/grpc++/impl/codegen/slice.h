...
/// slice pointing to a subset of another slice.
  ...
  public:
    ...
    /// Specialization of above for common case where buf == user_data
      Slice (void *buf, size_t len, void (*destroy) (void *)):Slice (buf, len,
								     destroy,
								     buf)
    {
    }
  ...
  {
    return grpc::string_ref (reinterpret_cast <
  ...
  {
    return grpc::string (reinterpret_cast <
			 char *>(GRPC_SLICE_START_PTR (slice)),
			 GRPC_SLICE_LENGTH (slice));
  }

  inline grpc_slice SliceReferencingString (const grpc::string & str)
  {
    return g_core_codegen_interface->grpc_slice_from_static_buffer (str.
								    data (),
								    str.
								    length
								    ());
  ...
  {
    return g_core_codegen_interface->grpc_slice_from_copied_buffer (str.
								    data (),
								    str.
								    length
								    ());
