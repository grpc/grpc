...
/** Returns the completion queue factory based on the attributes. MAY return a
  ...
  GRPCAPI const grpc_completion_queue_factory
    *grpc_completion_queue_factory_lookup (const
...
/** Helper function to create a completion queue with grpc_cq_completion_type
  ...
  GRPCAPI grpc_completion_queue *grpc_completion_queue_create_for_next (void
									*reserved);

/** Helper function to create a completion queue with grpc_cq_completion_type
    of GRPC_CQ_PLUCK and grpc_cq_polling_type of GRPC_CQ_DEFAULT_POLLING */
  GRPCAPI grpc_completion_queue *grpc_completion_queue_create_for_pluck (void
									 *reserved);

/** Create a completion queue */
  GRPCAPI grpc_completion_queue *grpc_completion_queue_create (const
 ...
 */
  GRPCAPI void
    grpc_completion_queue_thread_local_cache_init (grpc_completion_queue *
						   cq);
 ...
 */
  GRPCAPI int
    grpc_completion_queue_thread_local_cache_flush (grpc_completion_queue *
						    cq, void **tag, int *ok);
 ...
 * channel.
 * Useful for testing. **/
  GRPCAPI int grpc_channel_num_external_connectivity_watchers (grpc_channel *
							       channel);
...
/** Check whether a grpc channel supports connectivity watcher */
  GRPCAPI int grpc_channel_support_connectivity_watcher (grpc_channel *
							 channel);
...
/** Create a call given a handle returned from grpc_channel_register_call.
  ...
  GRPCAPI grpc_call *grpc_channel_create_registered_call (grpc_channel *
							  channel,
							  grpc_call *
							  parent_call,
							  uint32_t
...
/** Retrieve the calls current census context. */
  GRPCAPI struct census_context *grpc_census_call_get_context (grpc_call *
							       call);
...
/** Request notification of a new pre-registered call. 'cq_for_notification'
  ...
  GRPCAPI grpc_call_error grpc_server_request_registered_call (grpc_server *
							       server,
							       void
							       *registered_method,
							       grpc_call **
...
/** Create a buffer pool */
  GRPCAPI grpc_resource_quota *grpc_resource_quota_create (const char
							   *trace_name);
