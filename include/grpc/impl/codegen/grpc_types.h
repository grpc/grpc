...
/** Updates to a server's configuration from a config fetcher (for example,
 * listener updates from xDS) cause all older connections to be gracefully shut
 * down (i.e., "drained") with a grace period configured by this channel arg.
 * Int valued, milliseconds. Defaults to 10 minutes.*/
