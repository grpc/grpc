...
GPRAPI GPR_DLL void gpr_mu_init(gpr_mu* mu);

/** Cause *mu no longer to be initialized, freeing any memory in use.  Requires:
 *mu initialized; no other concurrent operation on *mu.  */
GPRAPI GPR_DLL void gpr_mu_destroy(gpr_mu* mu);

/** Wait until no thread has a lock on *mu, cause the calling thread to own an
   exclusive lock on *mu, then return.  May block indefinitely or crash if the
   calling thread has a lock on *mu.  Requires:  *mu initialized.  */
GPRAPI GPR_DLL void gpr_mu_lock(gpr_mu* mu);

/** Release an exclusive lock on *mu held by the calling thread.  Requires:  *mu
   initialized; the calling thread holds an exclusive lock on *mu.  */
GPRAPI GPR_DLL void gpr_mu_unlock(gpr_mu* mu);

/** Without blocking, attempt to acquire an exclusive lock on *mu for the
   calling thread, then return non-zero iff success.  Fail, if any thread holds
   the lock; succeeds with high probability if no thread holds the lock.
   Requires:  *mu initialized.  */
GPRAPI GPR_DLL int gpr_mu_trylock(gpr_mu* mu);
...
GPRAPI GPR_DLL void gpr_cv_init(gpr_cv* cv);

/** Cause *cv no longer to be initialized, freeing any memory in use.  Requires:
 *cv initialized; no other concurrent operation on *cv.*/
GPRAPI GPR_DLL void gpr_cv_destroy(gpr_cv* cv);
...
GPRAPI GPR_DLL int gpr_cv_wait(gpr_cv* cv, gpr_mu* mu,
...
GPRAPI GPR_DLL void gpr_cv_signal(gpr_cv* cv);

/** Wake all threads waiting on *cv.  Requires:  *cv initialized.  */
GPRAPI GPR_DLL void gpr_cv_broadcast(gpr_cv* cv);
...
/** Ensure that (*init_function)() has been called exactly once (for the
   ...
   finishes.*/
GPRAPI GPR_DLL void gpr_once_init(gpr_once* once, void (*init_function)(void));
...
/** Initialize *ev. */
GPRAPI GPR_DLL void gpr_event_init(gpr_event* ev);

/** Set *ev so that gpr_event_get() and gpr_event_wait() will return value.
   Requires:  *ev initialized; value != NULL; no prior or concurrent calls to
   gpr_event_set(ev, ...) since initialization.  */
GPRAPI GPR_DLL void gpr_event_set(gpr_event* ev, void* value);
...
/** Return the value set by gpr_event_set(ev, ...), or NULL if no such call has
   ...
   on most platforms.  */
GPRAPI GPR_DLL void* gpr_event_get(gpr_event* ev);
...
/** Wait until *ev is set by gpr_event_set(ev, ...), or abs_deadline is
   ...
   on most platforms.  */
GPRAPI GPR_DLL void* gpr_event_wait(gpr_event* ev, gpr_timespec abs_deadline);
...
GPRAPI GPR_DLL void gpr_ref_init(gpr_refcount* r, int n);

/** Increment the reference count *r.  Requires *r initialized. */
GPRAPI GPR_DLL void gpr_ref(gpr_refcount* r);

/** Increment the reference count *r.  Requires *r initialized.
   Crashes if refcount is zero */
GPRAPI GPR_DLL void gpr_ref_non_zero(gpr_refcount* r);

/** Increment the reference count *r by n.  Requires *r initialized, n > 0. */
GPRAPI GPR_DLL void gpr_refn(gpr_refcount* r, int n);

/** Decrement the reference count *r and return non-zero iff it has reached
   zero. .  Requires *r initialized. */
GPRAPI GPR_DLL int gpr_unref(gpr_refcount* r);

/** Return non-zero iff the reference count of *r is one, and thus is owned
   by exactly one object. */
GPRAPI GPR_DLL int gpr_ref_is_unique(gpr_refcount* r);
...
GPRAPI GPR_DLL void gpr_stats_init(gpr_stats_counter* c, intptr_t n);

/** *c += inc.  Requires: *c initialized. */
GPRAPI GPR_DLL void gpr_stats_inc(gpr_stats_counter* c, intptr_t inc);

/** Return *c.  Requires: *c initialized. */
GPRAPI GPR_DLL intptr_t gpr_stats_read(const gpr_stats_counter* c);
