 ...
 * contain.
 */
GPRAPI GPR_DLL void* gpr_malloc(size_t size);
/** like malloc, but zero all bytes before returning them */
GPRAPI GPR_DLL void* gpr_zalloc(size_t size);
/** free */
GPRAPI GPR_DLL void gpr_free(void* ptr);
/** realloc, never returns NULL */
GPRAPI GPR_DLL void* gpr_realloc(void* p, size_t size);
/** aligned malloc, never returns NULL, will align to alignment, which
 * must be a power of 2. */
GPRAPI GPR_DLL void* gpr_malloc_aligned(size_t size, size_t alignment);
/** free memory allocated by gpr_malloc_aligned */
GPRAPI GPR_DLL void gpr_free_aligned(void* ptr);
