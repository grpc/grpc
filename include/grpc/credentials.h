...
/** Channel certificate config object holds the channel's public certificates and
   associated private keys, as well as any CA certificates needed for server
   certificate validation (if applicable). Create using
   grpc_ssl_certificate_config_create(). */
typedef struct grpc_ssl_certificate_config
    grpc_ssl_certificate_config;

/** Creates a grpc_ssl_certificate_config object.
   - pem_roots_cert is the NULL-terminated string containing the PEM encoding of
     the server root certificates. This parameter may be NULL if the channel does
     not want the server to be authenticated with SSL.
   - pem_key_cert_pairs is an array private key / certificate chains of the
     channel. This parameter cannot be NULL.
   - num_key_cert_pairs indicates the number of items in the private_key_files
     and cert_chain_files parameters. It must be at least 1.
   - It is the caller's responsibility to free this object via
     grpc_ssl_certificate_config_destroy(). */
GRPCAPI grpc_ssl_certificate_config *
grpc_ssl_certificate_config_create(
    const char *pem_root_certs,
    const grpc_ssl_pem_key_cert_pair *pem_key_cert_pairs,
    size_t num_key_cert_pairs);

/** Destroys a grpc_ssl_certificate_config object. */
GRPCAPI void grpc_ssl_certificate_config_destroy(
...
/* Deprecated in favor of grpc_ssl_credentials_create_with_options.
...
/** Creates an options object using a certificate config fetcher. Use this
   method to reload the certificates and keys of the SSL channel without
...
/** Destroys a grpc_ssl_credentials_options object. */
GRPCAPI void grpc_ssl_credentials_options_destroy(
