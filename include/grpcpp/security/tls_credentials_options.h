...
typedef struct grpc_tls_credentials_options
 ...
 *
 *  The server authorization check arg contains all the info necessary
 * to schedule/cancel a server authorization check request. The callback
 * function must be called after finishing the schedule operation. See
 * the description of the grpc_tls_server_authorization_check_arg struct
 * in grpc_security.h for more details. **/
class TlsServerAuthorizationCheckArg {
 public:
  /** TlsServerAuthorizationCheckArg does not take ownership of the C
   * arg passed to the constructor. One must remember to free any memory
   * allocated to the C arg after using the setter functions below. **/
 ...
 * TlsServerAuthorizationCheckConfig instance. Refer to the definition
 * of the grpc_tls_server_authorization_check_config in
 * grpc_tls_credentials_options.h for more details on the expectations
 * of the member functions of the interface.
 * **/
 ...
 *  grps_tls_server_authorization_check_config. It is used for
 * experimental purposes for now and it is subject to change. **/
 ...
 public:
  ...
  int Schedule(TlsServerAuthorizationCheckArg* arg) const {
    if (server_authorization_check_interface_ == nullptr) {
      gpr_log(GPR_ERROR,
              "server authorization check interface is nullptr");
      if (arg != nullptr) {
...
// Base class of configurable options specified by users to configure
// their certain security features supported in TLS. It is used for
// experimental purposes for now and it is subject to change.
class TlsCredentialsOptions {
 public:
  // Constructor for base class TlsCredentialsOptions.
  //
  // @param certificate_provider the provider which fetches TLS
  // credentials that will be used in the TLS handshake
  TlsCredentialsOptions();
  // ---- Setters for member fields ----
  // Sets the certificate provider used to store root certs and identity
  // certs.
  void set_certificate_provider(
      std::shared_ptr<CertificateProviderInterface>
          certificate_provider);
  // Watches the updates of root certificates with name
  // |root_cert_name|. If used in TLS credentials, setting this field is
  // optional for both the client side and the server side. If this is
  // not set on the client side, we will use the root certificates
  // stored in the default system location, since client side must
  // provide root certificates in TLS(no matter single-side TLS or
  // mutual TLS). If this is not set on the server side, we will not
  // watch any root certificate updates, and assume no root certificates
  // needed for the server (in the one-side TLS scenario, the server is
  // not required to provide root certs). We don't support default root
  // certs on server side.
  void watch_root_certs();
  // Sets the name of root certificates being watched, if
  // |watch_root_certs| is called. If not set, an empty string will be
  // used as the name.
  //
  ...
  // |identity_cert_name|. If used in TLS credentials, it is required to
  // be set on the server side, and optional for the client side(in the
  // one-side TLS scenario, the client is not required to provide
  // identity certs).
  void watch_identity_key_cert_pairs();
  // Sets the name of identity key-cert pairs being watched, if
  // |watch_identity_key_cert_pairs| is called. If not set, an empty
  // string will be used as the name.
  //
  // @param identity_cert_name the name of identity key-cert pairs being
  // set.
  void set_identity_cert_name(const std::string& identity_cert_name);

  // ----- Getters for member fields ----
  // Get the internal c options. This function shall be used only
  // internally.
...
// certificate provider is not set, we will use the root certificates
// stored in the system default locations, and assume client won't
// provide any identity certificates(single side TLS). It is used for
// experimental purposes for now and it is subject to change.
class TlsChannelCredentialsOptions final
...
// It is used for experimental purposes for now and it is subject to
// change.
