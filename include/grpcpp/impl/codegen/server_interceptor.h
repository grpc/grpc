...
// factories can be provided to ServerBuilder which will be used to
// create a new vector of server interceptors per RPC. Server
// interceptor authors should create a subclass of
// ServerInterceptorFactorInterface which creates objects of their
// interceptors.
class ServerInterceptorFactoryInterface {
 public:
  virtual ~ServerInterceptorFactoryInterface() {}
  // Returns a pointer to an Interceptor object on successful creation,
  // nullptr otherwise. If nullptr is returned, this server interceptor
  // factory is ignored for the purposes of that RPC.
...
/// appears to an interceptor. It is created and owned by the library
/// and passed to the CreateServerInterceptor method of the
/// application's ServerInterceptorFactoryInterface implementation
class ServerRpcInfo {
 public:
  ...
  /// Return a pointer to the underlying ServerContext structure
  /// associated with the RPC to support features that apply to it
 ...
 private:
  ...
  void Unref() {
    if (GPR_UNLIKELY(ref_.fetch_sub(1, std::memory_order_acq_rel) ==
                     1)) {
      delete this;
    }
  }
