...
class ByteBuffer final {
 public:
  ...
  ByteBuffer(const Slice* slices, size_t nslices) {
    // The following assertions check that the representation of a
    // grpc::Slice is identical to that of a grpc_slice:  it has a
    // grpc_slice field, and nothing else.
    static_assert(
        std::is_same<decltype(slices[0].slice_), grpc_slice>::value,
        "Slice must have same representation as grpc_slice");
    static_assert(sizeof(Slice) == sizeof(grpc_slice),
                  "Slice must have same representation as grpc_slice");
    // The following assertions check that the representation of a
    // ByteBuffer is identical to grpc_byte_buffer*:  it has a
    // grpc_byte_buffer* field, and nothing else.
    static_assert(
        std::is_same<decltype(buffer_), grpc_byte_buffer*>::value,
        "ByteBuffer must have same representation as "
        "grpc_byte_buffer*");
    static_assert(sizeof(ByteBuffer) == sizeof(grpc_byte_buffer*),
                  "ByteBuffer must have same representation as "
                  "grpc_byte_buffer*");
    // The const_cast is legal if grpc_raw_byte_buffer_create() does no
    // more than its advertised side effect of increasing the reference
    // count of the slices it processes, and such an increase does not
    // affect the semantics seen by the caller of this constructor.
    buffer_ = g_core_codegen_interface->grpc_raw_byte_buffer_create(
        reinterpret_cast<grpc_slice*>(const_cast<Slice*>(slices)),
        nslices);
  }

  /// Constuct a byte buffer by referencing elements of existing buffer
  /// \a buf. Wrapper of core function grpc_byte_buffer_copy . This is
  /// not a deep copy; it is just a referencing. As a result, its
  /// performance is size-independent.
  ByteBuffer(const ByteBuffer& buf) : buffer_(nullptr) {
    operator=(buf);
  }
  ...
  /// a deep copy; it is just a referencing. As a result, its
  /// performance is size-independent.
  ByteBuffer& operator=(const ByteBuffer& buf) {
    ...
    if (buf.buffer_) {
      // then copy
      buffer_ =
          g_core_codegen_interface->grpc_byte_buffer_copy(buf.buffer_);
    }
    return *this;
  }

  // If this ByteBuffer's representation is a single flat slice, returns
  // a slice referencing that array.
  ...
  /// bbuf.Duplicate(); is equivalent to bbuf=bbuf; but is actually
  /// readable. This is not a deep copy; it is a referencing and its
  /// performance is size-independent.
  void Duplicate() {
  ...
  size_t Length() const {
    return buffer_ == nullptr
               ? 0
               : g_core_codegen_interface->grpc_byte_buffer_length(
                     buffer_);
  }
