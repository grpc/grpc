...
class ServerAsyncStreamingInterface {
 public:
  ...
  /// Request notification of the sending of initial metadata to the
  /// client. Completion will be notified by \a tag on the associated
  /// completion queue. This call is optional, but if it is used, it
  /// cannot be used concurrently with or after the \a Finish method.
  ///
...
class Service {
 ...
 protected:
  ...
  void RequestAsyncUnary(
    ...
    server_->RequestAsyncCall(methods_[idx].get(), context, stream,
                              call_cq, notification_cq, tag, request);
  }
  ...
  void MarkMethodAsync(int index) {
    // This does not have to be a hard error, however no one has
    // approached us with a use case yet. Please file an issue if you
  ...
  void MarkMethodRaw(int index) {
    ...
    methods_[idx]->SetServerApiType(
        internal::RpcServiceMethod::ApiType::RAW);
  }
  ...
  void MarkMethodGeneric(int index) {
    ...
    // believe you have one.
    size_t idx = static_cast<size_t>(index);
    GPR_CODEGEN_ASSERT(methods_[idx]->handler() != nullptr &&
                       "Cannot mark the method as 'generic' because it "
                       "has already been "
  ...
  void MarkMethodStreamed(int index,
    ...
    size_t idx = static_cast<size_t>(index);
    GPR_CODEGEN_ASSERT(
        methods_[idx] && methods_[idx]->handler() &&
        "Cannot mark an async or generic method Streamed");
    methods_[idx]->SetHandler(streamed_method);

    // From the server's point of view, streamed unary is a special
    // case of BIDI_STREAMING that has 1 read and 1 write, in that
    // order, and split server-side streaming is BIDI_STREAMING with 1
    // read and any number of writes, in that order.
  ...
  void MarkMethodCallback(int index, internal::MethodHandler* handler) {
    ...
    GPR_CODEGEN_ASSERT(methods_[idx].get() != nullptr &&
                       "Cannot mark the method as 'callback' because "
                       "it has already been "
                       "marked as 'generic'.");
  ...
  void MarkMethodRawCallback(int index,
    ...
    size_t idx = static_cast<size_t>(index);
    GPR_CODEGEN_ASSERT(methods_[idx].get() != nullptr &&
                       "Cannot mark the method as 'raw callback' "
