...
class CallbackUnaryHandler : public ::grpc::internal::MethodHandler {
 public:
  explicit CallbackUnaryHandler(std::function<ServerUnaryReactor*(
  ...
  void RunHandler(const HandlerParameter& param) final {
    ...
    if (param.status.ok()) {
      reactor =
          ::grpc::internal::CatchingReactorGetter<ServerUnaryReactor>(
              get_reactor_,
              static_cast<::grpc::CallbackServerContext*>(
                  param.server_context),
    ...
    if (reactor == nullptr) {
      // if deserialization or reactor creator failed, we need to fail
      // the call
      reactor =
          new (::grpc::g_core_codegen_interface->grpc_call_arena_alloc(
              param.call->call(), sizeof(UnimplementedUnaryReactor)))
              UnimplementedUnaryReactor(::grpc::Status(
  ...
  class ServerCallbackUnaryImpl : public ServerCallbackUnary {
   public:
    void Finish(::grpc::Status s) override {
      // A callback that only contains a call to MaybeDone can be run as
      // an inline callback regardless of whether or not OnDone is
      // inlineable because if the actual OnDone callback needs to be
      // scheduled, MaybeDone is responsible for dispatching to an
      // executor thread if needed. Thus, when setting up the
      // finish_tag_, we can set its own callback to inlineable.
      finish_tag_.Set(
          call_.call(),
          [this](bool) {
    ...
    void SendInitialMetadata() override {
      ...
      this->Ref();
      // The callback for this function should not be marked inline
      // because it is directly invoking a user-controlled reaction
      // (OnSendInitialMetadataDone). Thus it must be dispatched to an
      // executor thread. However, any OnDone needed after that can be
      // inlined because it is already running on an executor thread.
      meta_tag_.Set(
          call_.call(),
          [this](bool ok) {
   ...
   private:
    ...
    /// operations), maybe calls OnCancel if possible/needed, and maybe
    /// marks the completion of the RPC. This should be the last
    ...
    // reactor_ can always be loaded/stored with relaxed memory ordering
    // because its value is only set once, independently of other data
    // in the object, and the loads that use it will always actually
    // come provably later even though they are from different threads
    // since they are triggered by actions initiated only by the setting
    // up of the reactor_ variable. In a sense, it's a delayed "const":
    // it gets its value from the SetupReactor method (not the
    // constructor, so it's not a true const), but it doesn't change
    // after that and it only gets used by actions caused, directly or
    // indirectly, by that setup. This comment also applies to the
    // reactor_ variables of the other streaming objects in this file.
...
class CallbackClientStreamingHandler
 ...
 public:
  ...
  void RunHandler(const HandlerParameter& param) final {
    ...
    // Inlineable OnDone can be false in the CompletionOp callback
    // because there is no read reactor that has an inlineable OnDone;
    // this only applies to the DefaultReactor (which is unary).
    param.server_context->BeginCompletionOp(
        param.call,
        [reader](bool) {
    ...
    if (reactor == nullptr) {
      ...
      reactor =
          new (::grpc::g_core_codegen_interface->grpc_call_arena_alloc(
              param.call->call(),
              sizeof(UnimplementedReadReactor<RequestType>)))
              UnimplementedReadReactor<RequestType>(::grpc::Status(
                  ::grpc::StatusCode::UNIMPLEMENTED, ""));
    }
 ...
 private:
  ...
  class ServerCallbackReaderImpl
      : public ServerCallbackReader<RequestType> {
   public:
    void Finish(::grpc::Status s) override {
      // A finish tag with only MaybeDone can have its callback inlined
      // regardless even if OnDone is not inlineable because this
      // callback just checks a ref and then decides whether or not to
      // dispatch OnDone.
      finish_tag_.Set(
          call_.call(),
          [this](bool) {
    ...
    void SendInitialMetadata() override {
      GPR_CODEGEN_ASSERT(!ctx_->sent_initial_metadata_);
      this->Ref();
      // The callback for this function should not be inlined because it
      // invokes a user-controlled reaction, but any resulting OnDone
      // can be inlined in the executor to which this callback is
      // dispatched.
      meta_tag_.Set(
          call_.call(),
          [this](bool ok) {
   ...
   private:
    friend class CallbackClientStreamingHandler<RequestType,
                                                ResponseType>;
...
class CallbackServerStreamingHandler
 ...
 public:
  ...
  void RunHandler(const HandlerParameter& param) final {
    ...
    // Inlineable OnDone can be false in the CompletionOp callback
    // because there is no write reactor that has an inlineable OnDone;
    // this only applies to the DefaultReactor (which is unary).
    param.server_context->BeginCompletionOp(
        param.call,
        [writer](bool) {
    ...
    if (reactor == nullptr) {
      ...
      reactor =
          new (::grpc::g_core_codegen_interface->grpc_call_arena_alloc(
              ...
              UnimplementedWriteReactor<ResponseType>(::grpc::Status(
                  ::grpc::StatusCode::UNIMPLEMENTED, ""));
    }
 ...
 private:
  ...
  class ServerCallbackWriterImpl
      : public ServerCallbackWriter<ResponseType> {
   public:
    void Finish(::grpc::Status s) override {
      // A finish tag with only MaybeDone can have its callback inlined
      // regardless even if OnDone is not inlineable because this
    ...
    void WriteAndFinish(const ResponseType* resp,
                        ::grpc::WriteOptions options,
                        ::grpc::Status s) override {
      // This combines the write into the finish callback
      // TODO(vjpai): don't assert
      GPR_CODEGEN_ASSERT(
          finish_ops_.SendMessagePtr(resp, options).ok());
      Finish(std::move(s));
    }

   private:
    friend class CallbackServerStreamingHandler<RequestType,
                                                ResponseType>;
    ...
    auto* stream =
        new (::grpc::g_core_codegen_interface->grpc_call_arena_alloc(
            ...
            ServerCallbackReaderWriterImpl(
                ...
                param.call, param.call_requester);
    // Inlineable OnDone can be false in the CompletionOp callback
    // because there is no bidi reactor that has an inlineable OnDone;
    // this only applies to the DefaultReactor (which is unary).
    param.server_context->BeginCompletionOp(
        param.call,
        [stream](bool) {
  ...
  class ServerCallbackReaderWriterImpl
   ...
   private:
    ...
    void SetupReactor(
        ServerBidiReactor<RequestType, ResponseType>* reactor) {
      reactor_.store(reactor, std::memory_order_relaxed);
      // The callbacks for these functions should not be inlined because
      // they invoke user-controlled reactions, but any resulting
      // OnDones can be inlined in the executor to which a callback is
      // dispatched.
      write_tag_.Set(
          call_.call(),
          [this, reactor](bool ok) {
    ...
    void CallOnDone() override {
      ...
      this->~ServerCallbackReaderWriterImpl();  // explicitly call
                                                // destructor
      ::grpc::g_core_codegen_interface->grpc_call_unref(call);
      call_requester();
    }
