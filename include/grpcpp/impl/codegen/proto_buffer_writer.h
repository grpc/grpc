...
/// The principle is to give the proto layer one buffer of bytes at a
/// time that it can use to serialize the next portion of the message,
/// with the option to "backup" if more buffer is given than required at
/// the last buffer.
///
...
class ProtoBufferWriter
 ...
 public:
  ...
  ProtoBufferWriter(ByteBuffer* byte_buffer, int block_size,
                    int total_size)
    ...
    /// Create an empty raw byte buffer and look at its underlying slice
    /// buffer
    grpc_byte_buffer* bp =
        g_core_codegen_interface->grpc_raw_byte_buffer_create(nullptr,
                                                              0);
  ...
  /// Give the proto library the next buffer of bytes and its size. It
  /// is safe for the caller to write from data[0, size - 1].
  bool Next(void** data, int* size) override {
    ...
    if (have_backup_) {
      ...
      // When less than a whole block is needed, only allocate that
      // much. But make sure the allocated slice is not inlined.
      size_t allocate_length = remain > static_cast<size_t>(block_size_)
                                   ? block_size_
                                   : remain;
  ...
  /// Backup by \a count bytes because Next returned more bytes than
  /// needed (only used in the last buffer). \a count must be less than
  /// or equal too the last buffer returned from next.
  void BackUp(int count) override {
    ...
    /// 4. Mark that we still have the remaining part (for later
    /// use/unref)
    GPR_CODEGEN_ASSERT(count <=
                       static_cast<int>(GRPC_SLICE_LENGTH(slice_)));
    ...
    // It's dangerous to keep an inlined grpc_slice as the backup slice,
    // since on a following Next() call, a reference will be returned to
    // this slice via GRPC_SLICE_START_PTR, which will not be an address
    // held by slice_buffer_.
    have_backup_ = backup_slice_.refcount != nullptr;
    byte_count_ -= count;
  }

  /// Returns the total number of bytes written since this object was
  /// created.
  int64_t ByteCount() const override { return byte_count_; }

  // These protected members are needed to support internal
  // optimizations. they expose internal bits of grpc core that are NOT
  // stable. If you have a use case needs to use one of these functions,
 ...
 private:
  ...
  const int
      block_size_;  ///< size to alloc for each new \a grpc_slice needed
  const int total_size_;  ///< byte size of proto being serialized
  int64_t byte_count_;  ///< bytes written since this object was created
  grpc_slice_buffer* slice_buffer_;  ///< internal buffer of slices
                                     ///< holding the serialized data
  bool have_backup_;  ///< if we are holding a backup slice or not
  grpc_slice backup_slice_;  ///< holds space we can still write to, if
                             ///< the caller has called BackUp
