...
class Call final {
 public:
  Call() : call_hook_(nullptr), cq_(nullptr), call_(nullptr), max_receive_message_size_(-1) {}
  /** call is owned by the caller */
  Call(grpc_call* call, CallHook* call_hook, ::grpc::CompletionQueue* cq)
      : call_hook_(call_hook), cq_(cq), call_(call), max_receive_message_size_(-1) {}
  ...
  void PerformOps(CallOpSetInterface* ops) { call_hook_->PerformOpsOnCall(ops, this); }
  ...
  experimental::ClientRpcInfo* client_rpc_info() const { return client_rpc_info_; }

  experimental::ServerRpcInfo* server_rpc_info() const { return server_rpc_info_; }
