...
enum StatusCode {
  ...
  /// error-space
  /// that is not known in this address space. Also errors raised by
  /// APIs that
  ...
  /// problematic regardless of the state of the system (e.g., a
  /// malformed file
  /// name).
  INVALID_ARGUMENT = 3,
  ...
  /// response
  /// from a server could have been delayed long enough for the deadline
  /// to
  /// expire.
  DEADLINE_EXCEEDED = 4,
  ...
  /// required for
  /// the operation's execution. For example, directory to be deleted
  /// may be
  ...
  ///      they have first fixed up the directory by deleting files from
  ///      it.
  ...
  /// be fixed
  /// if the system state changes. For example, a 32-bit file system
  /// will
  /// generate INVALID_ARGUMENT if asked to read at an offset that is
  /// not in the
  ...
  /// error)
  /// when it applies so that callers who are iterating through a space
  /// can
  /// easily look for an OUT_OF_RANGE error to detect when they are
  /// done.
  OUT_OF_RANGE = 11,
  ...
  /// System has
  /// been broken. If you see one of these errors, Something is very
  /// broken.
  INTERNAL = 13,

  /// The service is currently unavailable. This is a most likely a
  /// transient
  /// condition and may be corrected by retrying with a backoff. Note
  /// that it is
