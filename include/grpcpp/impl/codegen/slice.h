...
class Slice final {
 public:
  ...
  Slice(grpc_slice slice, AddRef) : slice_(g_core_codegen_interface->grpc_slice_ref(slice)) {}
  ...
  explicit Slice(size_t len) : slice_(g_core_codegen_interface->grpc_slice_malloc(len)) {}
  ...
  Slice(const std::string& str)
      : slice_(g_core_codegen_interface->grpc_slice_from_copied_buffer(str.c_str(), str.length())) {
  }
  ...
  Slice(const Slice& other) : slice_(g_core_codegen_interface->grpc_slice_ref(other.slice_)) {}
  ...
  Slice(void* buf, size_t len, void (*destroy)(void*), void* user_data)
      : slice_(g_core_codegen_interface->grpc_slice_new_with_user_data(buf, len, destroy,
                                                                       user_data)) {}

  /// Specialization of above for common case where buf == user_data
  Slice(void* buf, size_t len, void (*destroy)(void*)) : Slice(buf, len, destroy, buf) {}
  ...
  grpc_slice c_slice() const { return g_core_codegen_interface->grpc_slice_ref(slice_); }
...
inline grpc::string_ref StringRefFromSlice(const grpc_slice* slice) {
  return grpc::string_ref(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(*slice)),
