...
/// The principle is to get one chunk of data at a time from the proto
/// layer, with options to backup (re-see some bytes) or skip (forward
/// past some bytes)
///
...
class ProtoBufferReader
 ...
 public:
  /// Constructs buffer reader from \a buffer. Will set \a status() to
  /// non ok if \a buffer is invalid (the internal buffer has not been
  /// initialized).
  ...
  bool Next(const void** data, int* size) override {
    ...
    /// If we have backed up previously, we need to return the backed-up
    /// slice
    if (backup_count_ > 0) {
      *data = GRPC_SLICE_START_PTR(*slice_) +
              GRPC_SLICE_LENGTH(*slice_) - backup_count_;
    ...
    if (!g_core_codegen_interface->grpc_byte_buffer_reader_peek(
            &reader_, &slice_)) {
      return false;
    }
    *data = GRPC_SLICE_START_PTR(*slice_);
  ...
  /// The proto library calls this to indicate that we should back up \a
  /// count bytes that have already been returned by the last call of
  /// Next. So do the backup and have that ready for a later Next.
  ...
  /// The proto library calls this to skip over \a count bytes.
 ...
 private:
  ...
  grpc_byte_buffer_reader
      reader_;         ///< internal object to read \a grpc_slice
