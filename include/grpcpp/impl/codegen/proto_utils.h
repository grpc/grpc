...
Status GenericSerialize(const grpc::protobuf::MessageLite& msg, ByteBuffer* bb, bool* own_buffer) {
  static_assert(std::is_base_of<protobuf::io::ZeroCopyOutputStream, ProtoBufferWriter>::value,
  ...
  *own_buffer = true;
  ...
  if (static_cast<size_t>(byte_size) <= GRPC_SLICE_INLINED_SIZE) {
    ...
    GPR_CODEGEN_ASSERT(slice.end() ==
                       msg.SerializeWithCachedSizesToArray(const_cast<uint8_t*>(slice.begin())));
...
Status GenericDeserialize(ByteBuffer* buffer, grpc::protobuf::MessageLite* msg) {
  static_assert(std::is_base_of<protobuf::io::ZeroCopyInputStream, ProtoBufferReader>::value,
...
class SerializationTraits<
    T, typename std::enable_if<std::is_base_of<grpc::protobuf::MessageLite, T>::value>::type> {
 public:
