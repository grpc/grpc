...
// TODO(vjpai): decide whether it is better for this to take a const
// lvalue
...
// constructed and then fired at exactly one point. There is no
// expectation that they can be reused without reconstruction.
...
class CallbackWithStatusTag : public grpc_completion_queue_functor {
 public:
  ...
  CallbackWithStatusTag(grpc_call* call, std::function<void(Status)> f,
    ...
    // A client-side callback should never be run inline since they will
    // always have work to do from the user application. So, set the
    // parent's inlineable field to false
    inlineable = false;
  }
...
/// CallbackWithSuccessTag can be reused multiple times, and will be
/// used in this fashion for streaming operations. As a result, it
/// shouldn't clear anything up until its destructor
class CallbackWithSuccessTag : public grpc_completion_queue_functor {
 public:
  ...
  // It should never be called on a tag that was constructed with
  // arguments or on a tag that has been Set before unless the tag has
  // been cleared. can_inline indicates that this particular callback
  // can be executed inline (without needing a thread hop) and is only
  // used for library-provided server callbacks.
 ...
 private:
  ...
  static void StaticRun(grpc_completion_queue_functor* cb, int ok) {
    static_cast<CallbackWithSuccessTag*>(cb)->Run(
        static_cast<bool>(ok));
  }
  void Run(bool ok) {
    void* ignored = ops_;
