...
// factories can be provided at channel creation which will be used to
// create a new vector of client interceptors per RPC. Client
// interceptor authors should create a subclass of
// ClientInterceptorFactorInterface which creates objects of their
// interceptors.
...
/// and passed to the CreateClientInterceptor method of the
/// application's ClientInterceptorFactoryInterface implementation
namespace experimental {
class ClientRpcInfo {
 public:
  // TODO(yashykt): Stop default-constructing ClientRpcInfo and remove
  // UNKNOWN
  ...
  /// Return an identifying suffix for the client stub, or nullptr if
  /// one wasn't specified.
  ...
  /// Return a pointer to the underlying ClientContext structure
 ...
 private:
  ...
  void RegisterInterceptors(
    ...
    // NOTE: The following is not a range-based for loop because it will
    // only
    //       iterate over a portion of the creators vector.
    for (auto it = creators.begin() + interceptor_pos;
         it != creators.end(); ++it) {
    ...
    if (internal::g_global_client_interceptor_factory != nullptr) {
      interceptors_.push_back(
          std::unique_ptr<experimental::Interceptor>(
...
// PLEASE DO NOT USE THIS. ALWAYS PREFER PER CHANNEL INTERCEPTORS OVER A
// GLOBAL INTERCEPTOR. IF USAGE IS ABSOLUTELY NECESSARY, PLEASE READ THE
// SAFETY NOTES. Registers a global client interceptor factory object,
// which is used for all RPCs made in this process. The application is
// responsible for maintaining the life of the object while gRPC
// operations are in progress. The global interceptor factory should
// only be registered once at the start of the process before any gRPC
