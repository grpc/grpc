...
class ClientContextTestPeer {
 public:
  ...
  /// Inject metadata to the ClientContext for the test. The test spouse
  ...
  void AddMetadata(const std::string& key, const std::string& value) {
    ...
    for (const auto& item : initial_metadata_storage_) {
 ...
 private:
  ...
  std::multimap<std::string, std::string> initial_metadata_storage_;
};
