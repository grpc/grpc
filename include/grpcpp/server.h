...
class Server : public ServerInterface, private GrpcLibraryCodegen {
 public:
  ...
  class GlobalCallbacks {
   public:
    ...
    /// Called before application callback for each synchronous server
    /// request
    virtual void PreSynchronousRequest(ServerContext* context) = 0;
    /// Called after application callback for each synchronous server
    /// request
    virtual void PostSynchronousRequest(ServerContext* context) = 0;
    /// Called before server is started.
    virtual void PreServerStart(Server* /*server*/) {}
    /// Called after a server port is added.
    virtual void AddPort(Server* /*server*/,
                         const std::string& /*addr*/,
                         ServerCredentials* /*creds*/, int /*port*/) {}
  };
  /// Set the global callback object. Can only be called once per
  /// application. Does not take ownership of callbacks, and expects the
  /// pointed to object to be alive until all server objects in the
  /// process have been destroyed. The same \a GlobalCallbacks object
  /// will be used throughout the application and is shared among all \a
  /// Server objects.
  static void SetGlobalCallbacks(GlobalCallbacks* callbacks);

  /// Returns a \em raw pointer to the underlying \a grpc_server
  /// instance. EXPERIMENTAL:  for internal/test use only
  ...
  /// NOTE: class experimental_type is not part of the public API of
  /// this class.
  ...
  /// NOTE: The function experimental() is not stable public API. It is
  /// a view to the experimental components of this class. It may be
 ...
 protected:
  ...
  /// instance.
  bool RegisterService(const std::string* addr,
  ...
  /// localhost:1234, 192.168.1.1:31416, [::1]:27182, etc.). \param
  /// creds The credentials associated with the server.
  ///
  /// \return bound port number on success, 0 on failure.
  ///
  /// \warning It is an error to call this method on an already started
  /// server.
  int AddListeningPort(const std::string& addr,
                       ServerCredentials* creds) override;

  /// NOTE: This is *NOT* a public API. The server constructors are
  /// supposed to be used by \a ServerBuilder class only. The
  /// constructor will be made 'private' very soon.
  ///
  /// Server constructors. To be used by \a ServerBuilder only.
  ///
  /// \param args The channel args
  ///
  /// \param sync_server_cqs The completion queues to use if the server
  /// is a synchronous server (or a hybrid server). The server polls for
  /// new RPCs on these queues
  ///
  /// \param min_pollers The minimum number of polling threads per
  /// server completion queue (in param sync_server_cqs) to use for
  /// listening to incoming requests (used only in case of sync server)
  ///
  /// \param max_pollers The maximum number of polling threads per
  /// server completion queue (in param sync_server_cqs) to use for
  /// listening to incoming requests (used only in case of sync server)
  ///
  /// \param sync_cq_timeout_msec The timeout to use when calling
  /// AsyncNext() on server completion queues passed via sync_server_cqs
  /// param.
 ...
 private:
  ...
  /// SyncRequestThreadManager is an implementation of ThreadManager.
  /// This class is responsible for polling for incoming RPCs and
  /// calling the RPC handlers. This is only used in case of a Sync
  /// server (i.e a server exposing a sync interface)
  ...
  /// instance.
  void RegisterAsyncGenericService(
      AsyncGenericService* service) override;

  /// Register a callback-based generic service. This call does not take
  /// ownership of theservice. The service must exist for the lifetime
  /// of the Server instance.
  void RegisterCallbackGenericService(
      CallbackGenericService* service) override;
  ...
  // Functions to manage the server shutdown ref count. Things that
  // increase the ref count are the running state of the server (take a
  // ref at start and drop it at shutdown) and each running callback
  // RPC.
  void Ref();
  ...
  // This should be destroyed after health_check_service_ and this
  // requirement is satisfied by declaring interceptor_creators_ before
  // health_check_service_. (C++ mandates that member objects be
  ...
  /// i.e. if the server has any services with sync methods. The server
  /// uses these completion queues to poll for new RPCs
  ...
  // callback_cq_ references the callbackable completion queue
  // associated with this server (if any). It is set on the first call
  // to CallbackCQ(). It is _not owned_ by the server; ownership belongs
  // with its internal shutdown callback tag (invoked when the CQ is
  // fully shutdown).
  std::atomic<CompletionQueue*> callback_cq_{nullptr};

  // List of CQs passed in by user that must be Shutdown only after
  // Server is Shutdown.  Even though this is only used with NDEBUG,
  // instantiate it in all cases since otherwise the size will be
  // inconsistent.
