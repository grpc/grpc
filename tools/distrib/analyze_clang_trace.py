    ...
    for path in trace_files:
        try:
            ...
            for event in data.get("traceEvents", []):
                if event.get("ph") == "X" and event.get("name", "").startswith(
                    "Instantiate"
                ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Bazel build with Clang time-trace and analyze template instantiation times."
    )
    parser.add_argument(
        "target",
        help="Bazel target to build, e.g., //test/core/util:sorted_pack_test",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=50,
        help="Number of top templates to display in the report (default 50)",
    )
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Skip the bazel build phase and analyze existing traces directly",
    )
    ...
    if not args.skip_build:
        print(
            f"--- 1. Building target {args.target} with profiling enabled ---"
        )
        try:
    ...
    if not trace_files:
        print(
            "No trace files found! Ensure compilation actually occurred.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    sorted_templates = sorted(
        template_times.items(), key=lambda item: item[1], reverse=True
    )
    ...
    print("-" * 100)
    print(
        f"Total instantiation time measured: {total_us_across_all / 1000.0:,.2f} ms"
    )
