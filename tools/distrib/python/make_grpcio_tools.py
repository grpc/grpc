...
CC_FILES={cc_files}
CC_FILES_WINDOWS_ONLY={cc_files_windows_only}
...
############ Begin of hard-coded file lists ############################
# TODO(b/529877304): Remove these hard coded lists and use yaml parser to get
# manifest from build_autogenerated.yaml, preprocessed_builds.yaml, etc.

ABSL_WINDOWS_ONLY_FILES = [
...
def _generate_deps_file_content():
    ...
    for name in cc_files_output:
        if name.endswith(".c") or name.endswith(".cc"):
            ...
            if filepath:
                if filepath in ABSL_WINDOWS_ONLY_FILES:
                    cc_files_windows_only.append(filepath)
                else:
    ...
    deps_file_content = DEPS_FILE_CONTENT.format(
        ...
        cc_files_windows_only=_pretty_print_list(sorted(cc_files_windows_only)),
