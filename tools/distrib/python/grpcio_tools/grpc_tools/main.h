...
// We declare `protoc_main` here since we want access to it from Cython
// as an extern but *without* triggering a dllimport declspec when on
// Windows.
...
struct ProtocError {
  ...
  ProtocError() {}
  ProtocError(std::string filename, int line, int column,
              std::string message)
      : filename(filename),
        line(line),
        column(column),
