...
def _commandfile_spawn(self, command, **kwargs):
    ...
    if use_ccache:
        ...
        for arg in command:
            ...
            # /Fo:
            #  Use a predictable path for ccache to avoid cache thrashing
            #  caused by randomized temp directories generated by setuptools.
        ...
        if predictable_fo_path:
            predictable_dir = os.path.dirname(predictable_fo_path)
            if not os.path.exists(predictable_dir):
                os.makedirs(predictable_dir)
    ...
    if use_ccache and predictable_fo_path and real_fo_path:
        # Copy file from predictable path to real path
        if os.path.exists(predictable_fo_path):
            real_dir = os.path.dirname(real_fo_path)
            if not os.path.exists(real_dir):
                os.makedirs(real_dir)
            shutil.copy(predictable_fo_path, real_fo_path)
