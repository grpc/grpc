...
test "$(bazel cquery 'somepath("//:grpc_unsecure", "//external:libssl")' --define=grpc_no_xds=true 2>/dev/null | wc -l)" -eq 0 || exit 1
test "$(bazel cquery 'somepath("//:grpc++_unsecure", "//external:libssl")' --define=grpc_no_xds=true 2>/dev/null | wc -l)" -eq 0 || exit 1
test "$(bazel query 'somepath("//:grpc++_codegen_proto", "//external:libssl")' 2>/dev/null | wc -l)" -eq 0 || exit 1
test "$(bazel cquery 'somepath("//test/cpp/microbenchmarks:helpers", "//external:libssl")' --define=grpc_no_xds=true 2>/dev/null | wc -l)" -eq 0 || exit 1
