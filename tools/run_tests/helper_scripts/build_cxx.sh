...
CMAKE_BUILD_TESTS_FLAG="-DgRPC_BUILD_TESTS=ON"
if [[ "$*" =~ "-DgRPC_BUILD_TESTS=OFF" ]]; then
  CMAKE_BUILD_TESTS_FLAG=""
fi

# TODO(yashykt/veblush): Remove workaround after fixing b/332425004 
if [ "${GRPC_RUNTESTS_ARCHITECTURE}" = "x86" ]; then
cmake "${CMAKE_BUILD_TESTS_FLAG}" -DCMAKE_BUILD_TYPE="${MSBUILD_CONFIG}" "$@" ../..
else
cmake -DgRPC_BUILD_GRPCPP_OTEL_PLUGIN=ON -DgRPC_ABSL_PROVIDER=package "${CMAKE_BUILD_TESTS_FLAG}" -DCMAKE_BUILD_TYPE="${MSBUILD_CONFIG}" -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX="${INSTALL_PATH}" "$@" ../..
fi

if [[ "$*" =~ "-DgRPC_BUILD_TESTS=OFF" ]]; then
# Just build default targets when gRPC_BUILD_TESTS is OFF.
# The default target builds grpc and grpc++ without tests safely in parallel.
