...
If "%GRPC_CMAKE_GENERATOR%" == "Ninja" (
  ...
  mkdir build
  cd build
  cmake -G "%GRPC_CMAKE_GENERATOR%" -DCMAKE_C_COMPILER="cl.exe" -DCMAKE_CXX_COMPILER="cl.exe" -DWITH_ABSEIL=ON -DBUILD_TESTING=OFF -DWITH_BENCHMARK=OFF -DCMAKE_BUILD_TYPE="%MSBUILD_CONFIG%" -DCMAKE_INSTALL_PREFIX="${INSTALL_PATH}" %* ..  || goto :error
  ninja -j%GRPC_RUN_TESTS_JOBS% install || goto :error
  ...
  mkdir build
  cd build
  cmake -G "%GRPC_CMAKE_GENERATOR%" -A "%GRPC_CMAKE_ARCHITECTURE%" %CMAKE_SYSTEM_VERSION_ARG% -DCMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE=x64 -DWITH_ABSEIL=ON -DBUILD_TESTING=OFF -DWITH_BENCHMARK=OFF -DCMAKE_INSTALL_PREFIX="${INSTALL_PATH}" %* .. || goto :error
