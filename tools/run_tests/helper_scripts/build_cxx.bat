...
If "%GRPC_CMAKE_GENERATOR%" == "Ninja" (
  ...
  set "CMAKE_BUILD_TESTS_FLAG=-DgRPC_BUILD_TESTS=ON"
  echo %* | find "-DgRPC_BUILD_TESTS=OFF" >nul
  if %errorlevel% == 0 set "CMAKE_BUILD_TESTS_FLAG="

  cmake -G "%GRPC_CMAKE_GENERATOR%" -DCMAKE_C_COMPILER="cl.exe" -DCMAKE_CXX_COMPILER="cl.exe" -DgRPC_BUILD_GRPCPP_OTEL_PLUGIN=ON -DgRPC_ABSL_PROVIDER=package %CMAKE_BUILD_TESTS_FLAG% -DCMAKE_BUILD_TYPE="%MSBUILD_CONFIG%" -DCMAKE_INSTALL_PREFIX="%INSTALL_PATH%" %* ../.. || goto :error
