...
If "%GRPC_CMAKE_GENERATOR%" == "Ninja" (
  ...
  mkdir build
  cd build
  cmake -G "%GRPC_CMAKE_GENERATOR%" -A "%GRPC_CMAKE_ARCHITECTURE%" %CMAKE_SYSTEM_VERSION_ARG% -DCMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE=x64 -DWITH_ABSEIL=ON -DBUILD_TESTING=OFF -DCMAKE_INSTALL_PREFIX="${INSTALL_PATH}" %* .. || goto :error
