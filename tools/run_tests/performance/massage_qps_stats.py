...
def massage_qps_stats(scenario_result):
  for stats in scenario_result["serverStats"] + scenario_result["clientStats"]:
    ...
    h = massage_qps_stats_helpers.histogram(core_stats, "call_combiner_thread_switch")
    stats["core_call_combiner_thread_switch"] = ",".join("%f" % x for x in h.buckets)
    stats["core_call_combiner_thread_switch_bkts"] = ",".join("%f" % x for x in h.boundaries)
    stats["core_call_combiner_thread_switch_50p"] = massage_qps_stats_helpers.percentile(h.buckets, 50, h.boundaries)
    stats["core_call_combiner_thread_switch_95p"] = massage_qps_stats_helpers.percentile(h.buckets, 95, h.boundaries)
    stats["core_call_combiner_thread_switch_99p"] = massage_qps_stats_helpers.percentile(h.buckets, 99, h.boundaries)
