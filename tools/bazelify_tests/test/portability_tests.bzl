...
def generate_run_tests_portability_tests(name):
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ...
            # Kokoro times out this test around 8746s (2h 25m)
            # set to 3h
            ["gcc_8", "--build_max_time=10800 --build_verbose_success --cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17", "tools/dockerfile/test/cxx_gcc_8_x64.current_version"],
