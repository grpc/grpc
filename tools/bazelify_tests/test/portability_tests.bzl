...
def generate_run_tests_portability_tests(name):
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ["gcc_10", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_gcc_12_x64.current_version"],
            ...
            ["clang_15", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DCMAKE_C_COMPILER=clang --cmake_configure_extra_args=-DCMAKE_CXX_COMPILER=clang++", "tools/dockerfile/test/cxx_clang_14_x64.current_version"],
