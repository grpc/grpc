...
def generate_run_tests_portability_tests(name):
    ...
    grpc_run_tests_harness_test(
        ...
        args = ["-l c -c dbg --build_only --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF"],
    ...
    for language in ["c", "c++"]:
        ...
        grpc_run_tests_harness_test(
            name = test_name,
            args = ["-l %s --config noexcept --build_only --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF" % language],
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ...
            ["gcc_14_cxx20", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=20 --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_gcc_14_x64.current_version"],
            ["gcc10.2_openssl102", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_SSL_PROVIDER=package --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_debian11_openssl102_x64.current_version"],
            ["gcc10.2_openssl111", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_SSL_PROVIDER=package --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_debian11_openssl111_x64.current_version"],
            ["gcc_12_openssl309", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_SSL_PROVIDER=package --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_debian12_openssl309_x64.current_version"],
            ["gcc_musl", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_alpine_x64.current_version"],
            ["clang_11", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DCMAKE_C_COMPILER=clang --cmake_configure_extra_args=-DCMAKE_CXX_COMPILER=clang++ --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_clang_11_x64.current_version"],
            ["clang_19_cxx23", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=23 --cmake_configure_extra_args=-DCMAKE_C_COMPILER=clang --cmake_configure_extra_args=-DCMAKE_CXX_COMPILER=clang++ --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_clang_19_x64.current_version"],
        ]
