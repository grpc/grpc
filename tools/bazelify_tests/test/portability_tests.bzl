...
def generate_run_tests_portability_tests(name):
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ...
            # Kokoro times out around 8746s (2h 25m), setting to 8400s (2h 20m) to get close, but not over so the logs can be collected.
            ["gcc_8", "--build_max_time=8400 --build_verbose_success --cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17", "tools/dockerfile/test/cxx_gcc_8_x64.current_version"],
