    ...
    for py_version in python3.9 python3.10 python3.11 python3.12 python3.13 python3.14; do
      if command -v "$py_version" >/dev/null 2>&1; then
        ...
        if [[ "$py_version" == "python3.12" || "$py_version" == "python3.13" || "$py_version" == "python3.14" ]]; then
          ...
          uv pip install --system --python "$py_version" -U 'cython==3.1.1' setuptools==65.4.1 six==1.16.0 wheel --break-system-packages
        else
          uv pip install --system --python "$py_version" -U 'cython==3.1.1' setuptools==65.4.1 six==1.16.0 wheel
        fi
      else
