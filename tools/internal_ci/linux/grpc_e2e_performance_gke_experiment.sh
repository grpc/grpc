...
GRPC_DOTNET_COMMIT="$(git ls-remote -h "https://github.com/${GRPC_DOTNET_REPO}.git" "${GRPC_DOTNET_GITREF}" | cut -f1)"
GRPC_GO_COMMIT="$(git ls-remote -h "https://github.com/${GRPC_GO_REPO}.gi" "${GRPC_GO_GITREF}" | cut -f1)"
GRPC_JAVA_COMMIT="$(git ls-remote -h "https://github.com/${GRPC_JAVA_REPO}.git" "${GRPC_JAVA_GITREF}" | cut -f1)"
TEST_INFRA_COMMIT="$(git ls-remote -h "https://github.com/${TEST_INFRA_REPO}.git" "${TEST_INFRA_GITREF}" | cut -f1)"
...
git checkout --detach "${TEST_INFRA_COMMIT}"
make all-tools
popd
