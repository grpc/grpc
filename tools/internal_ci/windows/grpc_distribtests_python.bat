...
@rem Consume 6GB of RAM in the background to starve the build processes
start python -c "import time; a = [bytearray(1024*1024) for _ in range(6144)]; print('Allocated 6GB'); time.sleep(3600)"

@rem Build all python windows artifacts
python tools/run_tests/task_runner.py -f artifact windows python %TASK_RUNNER_EXTRA_FILTERS% -j 5 --inner_jobs 4 -x build_artifacts_python/sponge_log.xml || set FAILED=true
...
@rem Export Windows Event Logs
if not exist artifacts mkdir artifacts
wevtutil epl System artifacts\system_logs.evtx
wevtutil epl Application artifacts\application_logs.evtx
