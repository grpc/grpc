...
VIRTUALENV_BIN=generate_projects_virtual_environment/bin
if [[ ! -d $VIRTUALENV_BIN ]]; then
  # These files are placed differently on Windows
  VIRTUALENV_BIN=generate_projects_virtual_environment/Scripts
fi
...
$VIRTUALENV_BIN/python tools/buildgen/generate_projects.py build_handwritten.yaml build_autogenerated.yaml $gen_build_files "$@"
