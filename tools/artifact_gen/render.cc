...
void RenderTemplate(const std::string& filename,
                    const nlohmann::json* build_yaml) {
  inja::Environment env{
      std::filesystem::path(absl::GetFlag(FLAGS_templates_dir) + "/" + filename)
          .parent_path()
          .string() +
      "/"};
  try {
...
void RenderAllTemplates(const nlohmann::json& build_yaml) {
  ...
  for (const auto& filename :
    ...
    threads.emplace_back([filename, build_yaml = &build_yaml]() {
      RenderTemplate(filename, build_yaml);
    });
  }
