...
with open('src/core/lib/config/config_vars.cc', 'w') as C:
    ...
    print(",".join("%s_(ParseConfigVar(load(%s), %s))" % (var_name(attr),c_str(name(attr)), DEFAULT_VALUE[attr['type']](attr['default'], name(attr))) for attr in attrs_in_packing_order), file=C)
    print("{}", file=C)
    print(file=C)
