...
with open('src/core/lib/gprpp/if_list.h', 'w') as H:
    ...
    for n in range(1, 64):
        print('', file=H)
        print("template <typename CheckArg, typename ActionArg, typename ActionFail, %s, %s> auto IfList(CheckArg input, ActionArg action_arg, ActionFail action_fail, %s, %s) {" % (
            ...
            ", ".join("Action%d action%d" % (i, i) for i in range(0, n)),
              ...
              file=H)
        for i in range(0, n):
            print("  if (check%d(input)) return action%d(action_arg);" % (i, i), file=H)
