...
class StatsDataGenerator:
  ...
  def _gen_bucket_code(self, shape):
    ...
    if done_trivial:
      ...
      while True:
        ...
        shift_data = find_ideal_shift(code_bounds[code_bounds_index:], 65536)
        if not shift_data:
          break
  ...
  def gen_stats_data_src(self, source_file_path):
    ...
    with open(source_file_path, "w") as C:
      ...
      print(
          "namespace { union DblUint { double dbl; uint64_t uint; }; }", file=C
      )
      ...
      print(
          ...
          % ",".join("%s{0}" % ctr.name for ctr in self._inst_map["Counter"]),
          file=C,
      )
      ...
      print(
          "    default: GPR_UNREACHABLE_CODE(return HistogramView());", file=C
      )
      for inst in self._inst_map["Histogram"]:
        ...
        print(
            ...
            % (
                inst.max,
                inst.buckets,
                histo_bucket_boundaries[self._shape(inst.max, inst.buckets)],
                inst.buckets,
                inst.name,
            ),
            file=C,
