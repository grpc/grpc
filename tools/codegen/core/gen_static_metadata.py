...
for i, elem in enumerate(all_strs):
    ...
    print('#define %s (grpc_static_slice_table()[%d])' % (
        mangle(elem).upper(), i), file=H)
print('', file=H)
print('', 'static constexpr uint8_t g_bytes[] = {%s};' % (','.join(
    '%d' % ord(c) for c in ''.join(all_strs))), file=C)
print('', file=C)
print('''
namespace grpc_core {
...
''', file=H)
print('grpc_slice_refcount grpc_core::StaticSliceRefcount::kStaticSubRefcount;', file=C)
print('''
namespace grpc_core {
...
print('StaticMetadata static_mdelem_table[GRPC_STATIC_MDELEM_COUNT] = {', file=C)
for idx, (a, b) in enumerate(all_elems):
    print('StaticMetadata(%s,%s, %d),' % (slice_def_for_ctx(
        str_idx(a)), slice_def_for_ctx(str_idx(b)), idx), file=C)
...
'''), file=C)
print(('grpc_mdelem '
             'static_mdelem_manifested[GRPC_STATIC_MDELEM_COUNT] = {'), file=C)
...
print(('  ((slice).refcount != NULL && (slice).refcount->GetType() == '
             'grpc_slice_refcount::Type::STATIC)'), file=H)
print('', file=H)
print('', file=C)
print('#define GRPC_STATIC_METADATA_INDEX(static_slice) \\', file=H)
print('(reinterpret_cast<grpc_core::StaticSliceRefcount*>((static_slice).refcount)->index)', file=H)
print('', file=H)
...
print(('extern uintptr_t '
             'grpc_static_mdelem_user_data[GRPC_STATIC_MDELEM_COUNT];'), file=H)

for i, elem in enumerate(all_elems):
    md_name = mangle(elem).upper()
    print('/* "%s": "%s" */' % elem, file=H)
    print(('#define %s (grpc_static_mdelem_manifested()[%d])' %
                 (md_name, i)), file=H)
print('', file=H)
...
print(('uintptr_t grpc_static_mdelem_user_data[GRPC_STATIC_MDELEM_COUNT] '
             '= {'), file=C)
print('  %s' % ','.join(
    '%d' % static_userdata.get(elem, 0) for elem in all_elems), file=C)
print('};', file=C)
print('', file=C)
...
print('static const uint16_t elem_keys[] = {%s};' % ','.join(
    '%d' % k for k in keys), file=C)
print('static const uint8_t elem_idxs[] = {%s};' % ','.join(
    '%d' % i for i in idxs), file=C)
print('', file=C)

print('grpc_mdelem grpc_static_mdelem_for_static_strings(intptr_t a, intptr_t b);', file=H)
print('grpc_mdelem grpc_static_mdelem_for_static_strings(intptr_t a, intptr_t b) {', file=C)
print('  if (a == -1 || b == -1) return GRPC_MDNULL;', file=C)
print('  uint32_t k = static_cast<uint32_t>(a * %d + b);' % len(all_strs), file=C)
print('  uint32_t h = elems_phash(k);', file=C)
print('  return h < GPR_ARRAY_SIZE(elem_keys) && elem_keys[h] == k && elem_idxs[h] != 255 ? GRPC_MAKE_MDELEM(&grpc_static_mdelem_table()[elem_idxs[h]].data(), GRPC_MDELEM_STORAGE_STATIC) : GRPC_MDNULL;', file=C)
print('}', file=C)
print('', file=C)
...
print('extern const uint8_t grpc_static_accept_encoding_metadata[%d];' % (
    1 << len(COMPRESSION_ALGORITHMS)), file=H)
print('const uint8_t grpc_static_accept_encoding_metadata[%d] = {' % (
    1 << len(COMPRESSION_ALGORITHMS)), file=C)
print('0,%s' % ','.join('%d' % md_idx(elem) for elem in compression_elems), file=C)
print('};', file=C)
print('', file=C)

print('#define GRPC_MDELEM_ACCEPT_ENCODING_FOR_ALGORITHMS(algs) (GRPC_MAKE_MDELEM(&grpc_static_mdelem_table()[grpc_static_accept_encoding_metadata[(algs)]].data(), GRPC_MDELEM_STORAGE_STATIC))', file=H)
print('', file=H)

print('extern const uint8_t grpc_static_accept_stream_encoding_metadata[%d];' % (
    1 << len(STREAM_COMPRESSION_ALGORITHMS)), file=H)
print('const uint8_t grpc_static_accept_stream_encoding_metadata[%d] = {' % (
    1 << len(STREAM_COMPRESSION_ALGORITHMS)), file=C)
print('0,%s' % ','.join(
    '%d' % md_idx(elem) for elem in stream_compression_elems), file=C)
print('};', file=C)

print('#define GRPC_MDELEM_ACCEPT_STREAM_ENCODING_FOR_ALGORITHMS(algs) (GRPC_MAKE_MDELEM(&grpc_static_mdelem_table()[grpc_static_accept_stream_encoding_metadata[(algs)]].data(), GRPC_MDELEM_STORAGE_STATIC))', file=H)
