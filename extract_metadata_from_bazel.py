...
def _get_target_metadata_from_bazel(target_name):
    ...
    lib_dict['deps_transitive'] = _extract_cc_deps(deps)
    return lib_dict


def _generate_build_metadata(lib_names):
    result = {}
    for lib_name in lib_names:
        result[lib_name] = _get_target_metadata_from_bazel(lib_name)

    # autodetect values of some properties
    for lib_name in lib_names:
        ...
        #lib_dict['gtest'] = depends_on_gtest
        
        # TODO: is any of these deps useful for autogenerating some properties?
        # @upb//
        # @zlib//
        # @com_google_absl
    ...
    for lib_name in lib_names:
        lib_dict = result[lib_name]
        lib_dict.update(_BUILD_EXTRA_METADATA.get(lib_name, {}))
    ...
    for lib_name in lib_names:
        to_name = _BUILD_EXTRA_METADATA.get(lib_name, {}).get('_RENAME', None)
        if to_name:
...
_BUILD_EXTRA_METADATA = {
    ...
    # TODO: autodetect gtest based on dependencies...
