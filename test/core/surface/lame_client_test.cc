...
void
verify_connectivity (void *arg, grpc_error * error)
{
  grpc_connectivity_state *state =
    static_cast < grpc_connectivity_state * >(arg);
...
void
test_transport_op (grpc_channel * channel)
{
...
main (int argc, char **argv)
{
  ...
  op++;
  error =
    grpc_call_start_batch (call, ops, (size_t) (op - ops), tag (2), nullptr);
  GPR_ASSERT (GRPC_CALL_OK == error);
