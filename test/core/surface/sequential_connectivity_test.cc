...
server_thread_func (void *args)
{
  ...
  grpc_event ev =
    grpc_completion_queue_next (a->cq, gpr_inf_future (GPR_CLOCK_REALTIME),
				nullptr);
...
static void
run_test (const test_fixture * fixture)
{
  ...
  for (size_t i = 0; i < NUM_CONNECTIONS; i++)
    {
      ...
      while ((state =
	      grpc_channel_check_connectivity_state (channels[i],
						     1)) !=
	     GRPC_CHANNEL_READY)
	{
...
static void
insecure_test_add_port (grpc_server * server, const char *addr)
{
...
static void
secure_test_add_port (grpc_server * server, const char *addr)
{
  grpc_ssl_pem_key_cert_pair pem_cert_key_pair = { test_server1_key,
    test_server1_cert
  };
  grpc_server_credentials *ssl_creds =
    grpc_ssl_server_credentials_create (nullptr, &pem_cert_key_pair, 1, 0,
					nullptr);
