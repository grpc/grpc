...
typedef struct test_fixture {
  ...
  // Have the creds here so all the channels will share the same one to
  // enabled subchannel sharing if needed.
...
static grpc_channel* create_test_channel(
    const char* addr, grpc_channel_credentials* creds,
  ...
  if (creds != nullptr) {
    channel =
        grpc_secure_channel_create(creds, addr, &channel_args, nullptr);
  } else {
    channel =
        grpc_insecure_channel_create(addr, &channel_args, nullptr);
  }
  return channel;
}
...
static void run_test(const test_fixture* fixture,
  ...
  grpc_core::Thread server_thread("grpc_server", server_thread_func,
                                  &sta);
  server_thread.Start();
  ...
  for (size_t i = 0; i < NUM_CONNECTIONS; i++) {
    channels[i] = create_test_channel(addr.c_str(), fixture->creds,
                                      share_subchannel);

    gpr_timespec connect_deadline =
        grpc_timeout_seconds_to_deadline(30);
    grpc_connectivity_state state;
    while ((state = grpc_channel_check_connectivity_state(
                channels[i], 1)) != GRPC_CHANNEL_READY) {
      grpc_channel_watch_connectivity_state(
          channels[i], state, connect_deadline, cq, nullptr);
      grpc_event ev = grpc_completion_queue_next(
          cq, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr);
      /* check that the watcher from "watch state" was free'd */
      GPR_ASSERT(grpc_channel_num_external_connectivity_watchers(
                     channels[i]) == 0);
      GPR_ASSERT(ev.type == GRPC_OP_COMPLETE);
      GPR_ASSERT(ev.tag == nullptr);
      GPR_ASSERT(ev.success == true);
  ...
  while (grpc_completion_queue_next(
             server_cq, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr)
...
static void secure_test_add_port(grpc_server* server,
  ...
  grpc_server_credentials* ssl_creds =
      grpc_ssl_server_credentials_create(nullptr, &pem_key_cert_pair, 1,
                                         0, nullptr);
