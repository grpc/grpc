...
TEST(ServerChttp2, AddSamePortTwice) {
  ...
  EXPECT_EQ(grpc_server_add_secure_http2_port(server, addr.c_str(), fake_creds), port);
  EXPECT_EQ(grpc_server_add_secure_http2_port(server, addr.c_str(), fake_creds), 0);
