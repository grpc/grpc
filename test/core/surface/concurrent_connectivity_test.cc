...
create_loop_destroy (void *addr)
{
  for (int i = 0; i < NUM_OUTER_LOOPS; ++i)
      ...
      for (int j = 0; j < NUM_INNER_LOOPS; ++j)
	{
	  ...
	  GPR_ASSERT (grpc_completion_queue_next (cq, poll_time, nullptr).
		      type == GRPC_OP_COMPLETE);
	  /* check that the watcher from "watch state" was free'd */
	  GPR_ASSERT (grpc_channel_num_external_connectivity_watchers (chan)
		      == 0);
	}
...
static void
on_connect (void *vargs, grpc_endpoint * tcp,
...
static void
done_pollset_shutdown (void *pollset, grpc_error * error)
{
...
run_concurrent_connectivity_test ()
{
  ...
  gpr_thd_join (server);
  {
    ...
    grpc_pollset_shutdown (args.pollset,
			   GRPC_CLOSURE_CREATE (done_pollset_shutdown,
						args.pollset,
...
void
watches_with_short_timeouts (void *addr)
{
