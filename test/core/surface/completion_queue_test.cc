...
static void test_no_op(void) {
  ...
  grpc_cq_polling_type polling_types[] = {GRPC_CQ_DEFAULT_POLLING, GRPC_CQ_NON_LISTENING,
                                          GRPC_CQ_NON_POLLING};
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(completion_types); i++) {
    for (size_t j = 0; j < GPR_ARRAY_SIZE(polling_types); j++) {
      ...
      shutdown_and_destroy(grpc_completion_queue_create(grpc_completion_queue_factory_lookup(&attr),
                                                        &attr, nullptr));
    }
  }
}
...
static void test_wait_empty(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    cc = grpc_completion_queue_create(grpc_completion_queue_factory_lookup(&attr), &attr, nullptr);
...
static void test_cq_tls_cache_full(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    GPR_ASSERT(grpc_completion_queue_thread_local_cache_flush(cc, &res_tag, &ok) == 1);
...
static void test_cq_tls_cache_empty(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    GPR_ASSERT(grpc_completion_queue_thread_local_cache_flush(cc, &res_tag, &ok) == 0);
    grpc_completion_queue_thread_local_cache_init(cc);
    GPR_ASSERT(grpc_completion_queue_thread_local_cache_flush(cc, &res_tag, &ok) == 0);
    shutdown_and_destroy(cc);
  }
...
static void test_shutdown_then_next_polling(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    cc = grpc_completion_queue_create(grpc_completion_queue_factory_lookup(&attr), &attr, nullptr);
    grpc_completion_queue_shutdown(cc);
    event = grpc_completion_queue_next(cc, gpr_inf_past(GPR_CLOCK_REALTIME), nullptr);
...
static void test_shutdown_then_next_with_timeout(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    grpc_completion_queue_shutdown(cc);
    event = grpc_completion_queue_next(cc, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr);
    GPR_ASSERT(event.type == GRPC_QUEUE_SHUTDOWN);
    grpc_completion_queue_destroy(cc);
  }
}
...
static void test_pluck(void) {
  ...
  for (size_t pidx = 0; pidx < GPR_ARRAY_SIZE(polling_types); pidx++) {
    ...
    for (i = 0; i < GPR_ARRAY_SIZE(tags); i++) {
      ev = grpc_completion_queue_pluck(cc, tags[i], gpr_inf_past(GPR_CLOCK_REALTIME), nullptr);
...
static void test_pluck_after_shutdown(void) {
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE(polling_types); i++) {
    ...
    cc = grpc_completion_queue_create(grpc_completion_queue_factory_lookup(&attr), &attr, nullptr);
    grpc_completion_queue_shutdown(cc);
    ev = grpc_completion_queue_pluck(cc, nullptr, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr);
    GPR_ASSERT(ev.type == GRPC_QUEUE_SHUTDOWN);
    grpc_completion_queue_destroy(cc);
  }
}
