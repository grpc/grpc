...
shutdown_and_destroy (grpc_completion_queue * cc)
{
  ...
  switch (grpc_get_cq_completion_type (cc))
    ...
    case GRPC_CQ_NEXT:
      {
	ev =
	  grpc_completion_queue_next (cc, gpr_inf_past (GPR_CLOCK_REALTIME),
				      nullptr);
	break;
      }
    case GRPC_CQ_PLUCK:
      {
	ev =
	  grpc_completion_queue_pluck (cc, create_test_tag (),
...
test_no_op (void)
{
  grpc_cq_completion_type completion_types[] =
    { GRPC_CQ_NEXT, GRPC_CQ_PLUCK };
...
test_pollset_conversion (void)
{
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (completion_types); i++)
      ...
      for (size_t j = 0; j < GPR_ARRAY_SIZE (polling_types); j++)
	{
	  ...
	  attr.cq_polling_type = polling_types[j];
	  cq =
	    grpc_completion_queue_create (grpc_completion_queue_factory_lookup
					  (&attr), &attr, nullptr);
...
test_wait_empty (void)
{
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (polling_types); i++)
	...
	grpc_completion_queue_create (grpc_completion_queue_factory_lookup
				      (&attr), &attr, nullptr);
      event =
	grpc_completion_queue_next (cc, gpr_now (GPR_CLOCK_REALTIME),
				    nullptr);
...
test_cq_end_op (void)
{
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (polling_types); i++)
	...
	grpc_completion_queue_create (grpc_completion_queue_factory_lookup
      ...
      grpc_cq_end_op (cc, tag, GRPC_ERROR_NONE, do_nothing_end_completion,
		      nullptr, &completion);
...
test_pluck (void)
{
  ...
  for (size_t pidx = 0; pidx < GPR_ARRAY_SIZE (polling_types); pidx++)
	...
	{
	  ev =
	    grpc_completion_queue_pluck (cc, tags[i],
					 gpr_inf_past (GPR_CLOCK_REALTIME),
					 nullptr);
	...
	{
	  ev =
	    grpc_completion_queue_pluck (cc,
					 tags[GPR_ARRAY_SIZE (tags) - i - 1],
					 gpr_inf_past (GPR_CLOCK_REALTIME),
					 nullptr);
...
static void
test_pluck_after_shutdown (void)
{
  grpc_cq_polling_type polling_types[] = {
    GRPC_CQ_DEFAULT_POLLING, GRPC_CQ_NON_LISTENING, GRPC_CQ_NON_POLLING
  };
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (polling_types); i++)
	...
	grpc_completion_queue_create (grpc_completion_queue_factory_lookup
      ...
      grpc_completion_queue_shutdown (cc);
      ev =
	grpc_completion_queue_pluck (cc, nullptr,
