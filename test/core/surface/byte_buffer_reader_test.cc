...
static void test_read_one_slice(void) {
  ...
  GPR_ASSERT(grpc_byte_buffer_reader_init(&reader, buffer) && "Couldn't init byte buffer reader");
...
static void test_byte_buffer_from_reader(void) {
  ...
  GPR_ASSERT(memcmp(GRPC_SLICE_START_PTR(buffer_from_reader->data.raw.slice_buffer.slices[0]),
                    "test", 4) == 0);
...
static void test_readall(void) {
  ...
  GPR_ASSERT(memcmp(&(GRPC_SLICE_START_PTR(slice_out)[512]), lotsa_bs, 1024) == 0);
