...
static void
test_read_corrupted_slice (void)
{
  grpc_slice slice;
  grpc_byte_buffer *buffer;
  grpc_byte_buffer_reader reader;
...
static void
read_compressed_slice (grpc_compression_algorithm algorithm,
		       size_t input_size)
{
...
test_byte_buffer_from_reader (void)
{
  ...
  GPR_ASSERT (memcmp
	      (GRPC_SLICE_START_PTR
	       (buffer_from_reader->data.raw.slice_buffer.slices[0]), "test",
	       4) == 0);
...
test_readall (void)
{
  ...
  GPR_ASSERT (memcmp
	      (&(GRPC_SLICE_START_PTR (slice_out)[512]), lotsa_bs,
	       1024) == 0);
