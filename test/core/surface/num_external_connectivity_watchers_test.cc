...
static void channel_idle_start_watch(grpc_channel* channel,
  ...
  gpr_timespec connect_deadline =
      grpc_timeout_milliseconds_to_deadline(1);
  GPR_ASSERT(grpc_channel_check_connectivity_state(channel, 0) ==
             GRPC_CHANNEL_IDLE);

  grpc_channel_watch_connectivity_state(
      channel, GRPC_CHANNEL_IDLE, connect_deadline, cq,
...
static void run_timeouts_test(const test_fixture* fixture) {
  ...
  grpc_init();
  std::string addr = grpc_core::JoinHostPort(
  ...
  /* start 3 watchers, see one time out, start another 3, and then see
   * them all time out */
  for (size_t i = 1; i <= 3; i++) {
    channel_idle_start_watch(channel, cq);
  }
...
static void run_channel_shutdown_before_timeout_test(
  ...
  /* start 1 watcher and then shut down the channel before the timer
   * goes off */
  GPR_ASSERT(grpc_channel_num_external_connectivity_watchers(channel) ==
             0);

  /* expecting a 30 second timeout to go off much later than the
   * shutdown. */
...
static grpc_channel* secure_test_create_channel(const char* addr) {
  ...
  grpc_channel_credentials* ssl_creds = grpc_ssl_credentials_create(
      test_root_cert, nullptr, nullptr, nullptr);
  ...
  grpc_channel* channel = grpc_secure_channel_create(
      ssl_creds, addr, new_client_args, nullptr);
  {
