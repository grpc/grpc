...
static void channel_idle_start_watch(grpc_channel* channel, grpc_completion_queue* cq) {
  ...
  GPR_ASSERT(grpc_channel_check_connectivity_state(channel, 0) == GRPC_CHANNEL_IDLE);

  grpc_channel_watch_connectivity_state(channel, GRPC_CHANNEL_IDLE, connect_deadline, cq,
...
static void run_timeouts_test(const test_fixture* fixture) {
  ...
  grpc_init();
  std::string addr = grpc_core::JoinHostPort("localhost", grpc_pick_unused_port_or_die());
  ...
  GPR_ASSERT(grpc_completion_queue_next(cq, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr).type ==
             GRPC_QUEUE_SHUTDOWN);
...
static grpc_channel* secure_test_create_channel(const char* addr) {
  ...
  const char* test_root_cert = reinterpret_cast<const char*> GRPC_SLICE_START_PTR(ca_slice);
  ...
  grpc_channel* channel = grpc_secure_channel_create(ssl_creds, addr, new_client_args, nullptr);
  {
