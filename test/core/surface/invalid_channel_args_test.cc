...
static int
ends_with (const char *src, const char *suffix)
{
...
static void
log_error_sink (gpr_log_func_args * args)
{
...
static void
verify_last_error (const char *message)
{
  if (message == nullptr)
    {
...
static char *
compose_error_string (const char *key, const char *message)
{
  char *ret;
...
static void
one_test (grpc_channel_args * args, char *expected_error_message)
{
  grpc_channel *chan =
...
static void
test_no_error_message (void)
{
  one_test (nullptr, nullptr);
}

static void
test_default_authority_type (void)
{
  grpc_arg client_arg;
  ...
  expected_error_message =
    compose_error_string (GRPC_ARG_DEFAULT_AUTHORITY,
			  " ignored: it must be a string");
  one_test (&client_args, expected_error_message);
}

static void
test_ssl_name_override_type (void)
{
  grpc_arg client_arg;
  grpc_channel_args client_args;
  char *expected_error_message;
...
static void
test_ssl_name_override_failed (void)
{
