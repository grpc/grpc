...
void test_register_method_fail(void) {
  ...
  method_old = grpc_server_register_method(
      server, "m2", "h2", GRPC_SRM_PAYLOAD_NONE,
      GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST);
  GPR_ASSERT(method_old != nullptr);
  method = grpc_server_register_method(server, "m2", "h2",
                                       GRPC_SRM_PAYLOAD_NONE, 0);
  GPR_ASSERT(method == nullptr);
  method = grpc_server_register_method(
...
void test_request_call_on_no_server_cq(void) {
  ...
  GPR_ASSERT(GRPC_CALL_ERROR_NOT_SERVER_COMPLETION_QUEUE ==
             grpc_server_request_call(server, nullptr, nullptr, nullptr,
                                      cc, cc, nullptr));
  GPR_ASSERT(GRPC_CALL_ERROR_NOT_SERVER_COMPLETION_QUEUE ==
...
void test_bind_server_twice(void) {
  ...
  GPR_ASSERT(0 == grpc_server_add_secure_http2_port(
                      server2, addr.c_str(), nullptr));
  GPR_ASSERT(port == grpc_server_add_insecure_http2_port(server1,
                                                         addr.c_str()));
  GPR_ASSERT(
      0 == grpc_server_add_insecure_http2_port(server2, addr.c_str()));
  grpc_server_credentials* fake_creds =
      grpc_fake_transport_security_server_credentials_create();
  GPR_ASSERT(0 == grpc_server_add_secure_http2_port(
                      server2, addr.c_str(), fake_creds));
...
void test_bind_server_to_addr(const char* host, bool secure) {
  ...
  if (secure) {
    ...
    GPR_ASSERT(
        grpc_server_add_insecure_http2_port(server, addr.c_str()));
  }
  grpc_completion_queue* cq =
      grpc_completion_queue_create_for_next(nullptr);
