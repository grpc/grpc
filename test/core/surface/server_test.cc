...
void
test_register_method_fail (void)
{
  ...
  method =
    grpc_server_register_method (server, "m", "h",
  ...
  GPR_ASSERT (method == nullptr);
  method =
    grpc_server_register_method (server, "m2", "h2",
				 GRPC_SRM_PAYLOAD_READ_INITIAL_BYTE_BUFFER,
				 GRPC_INITIAL_METADATA_IDEMPOTENT_REQUEST);
  GPR_ASSERT (method == nullptr);
...
void
test_request_call_on_no_server_cq (void)
{
  ...
  GPR_ASSERT (GRPC_CALL_ERROR_NOT_SERVER_COMPLETION_QUEUE ==
	      grpc_server_request_call (server, nullptr, nullptr, nullptr, cc,
					cc, nullptr));
  GPR_ASSERT (GRPC_CALL_ERROR_NOT_SERVER_COMPLETION_QUEUE ==
...
test_bind_server_twice (void)
{
  ...
  GPR_ASSERT (0 ==
	      grpc_server_add_secure_http2_port (server2, addr, nullptr));
  ...
  GPR_ASSERT (0 ==
	      grpc_server_add_secure_http2_port (server2, addr, fake_creds));
...
void
test_bind_server_to_addr (const char *host, bool secure)
{
  int port = grpc_pick_unused_port_or_die ();
  char *addr;
...
static void
test_bind_server_to_addrs (const char **addrs, size_t n)
{
