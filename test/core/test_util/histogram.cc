...
static size_t bucket_for(grpc_histogram* h, double x) {
  ...
  ABSL_CHECK(bucket < h->num_buckets);
  return bucket;
}
...
grpc_histogram* grpc_histogram_create(double resolution,
  ...
  ABSL_CHECK(resolution > 0.0);
  ABSL_CHECK(max_bucket_start > resolution);
  h->sum = 0.0;
  ...
  ABSL_CHECK_GT(h->num_buckets, 1u);
  ABSL_CHECK_LT(h->num_buckets, 100000000ul);
  h->buckets =
...
void grpc_histogram_merge_contents(grpc_histogram* histogram,
  ...
  size_t i;
  ABSL_CHECK(histogram->num_buckets == data_count);
  histogram->sum += sum;
