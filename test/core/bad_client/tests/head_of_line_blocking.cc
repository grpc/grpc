...
static const char prefix[] =
    ...
    // stream 1 headers: generated from server_registered_method.headers
    // in this directory
    ...
    // data frame for stream 1: advertise a 10000 byte payload (that we
    // won't fulfill)
    "\x00\x00\x05\x00\x00\x00\x00\x00\x01"
    "\x01\x00\x00\x27\x10"
    // stream 3 headers: generated from server_registered_method.headers
    // in this directory
    ...
    "\x10\x02te\x08trailers"
    "\x10\x0auser-agent\"bad-client grpc-c/0.12.0.0 (linux)"
    // data frame for stream 3: advertise a 10000 byte payload (that we
    // will fulfill)
