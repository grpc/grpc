...
static void
server_verifier (grpc_server * server, grpc_completion_queue * cq,
...
static void
server_verifier_sends_too_much_metadata (grpc_server * server,
...
static bool
client_validator (grpc_slice_buffer * incoming)
{
  for (size_t i = 0; i < incoming->count; ++i)
    {
      const char *s =
	(const char *) GRPC_SLICE_START_PTR (incoming->slices[i]);
