...
/* Test runner.
 *
 * Create a server, and for each arg in \a args send client_payload. For
 * each payload, run client_validator to make sure that the response is
 * as expected. Also execute \a server_validator in a separate thread to
 * assert that the bytes are handled as expected.
 *
 * The flags are only applicable to the last validator in the array.
...
/* A hack to let old tests work as before. In these tests, instead of an
 * array, the tests provide a single client_validator and payload
 */
...
#define GRPC_RUN_BAD_CLIENT_TEST(server_validator, client_validator,  \
  ...
  grpc_bad_client_arg COMBINE(bca, __LINE__) = {                      \
      client_validator, nullptr, payload, sizeof(payload) - 1};       \
  grpc_run_bad_client_test(server_validator, &COMBINE(bca, __LINE__), \
                           1, flags)

/* Helper validator functions */
/* Client side validator for connection preface from server. \a arg is
 * unused */
bool client_connection_preface_validator(grpc_slice_buffer* incoming,
                                         void* arg);

/* Client side validator for checking if reset stream is present at the
 * end of the buffer. \a arg is unused.
 */
bool rst_stream_client_validator(grpc_slice_buffer* incoming,
                                 void* arg);

/* Helper grpc_bad_client_arg arguments for direct use */
/* Sends a connection preface from the client with an empty settings
 * frame */
