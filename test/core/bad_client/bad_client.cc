...
void grpc_run_client_side_validator(grpc_bad_client_arg* arg, uint32_t flags,
  ...
  if (arg->client_payload_length < 4 * 1024) {
    ...
    // Add a debug log
    ABSL_LOG(INFO) << "TEST: " << hex;
    gpr_free(hex);
  } else {
    ABSL_LOG(INFO) << "TEST: (" << arg->client_payload_length
              << " byte long string)";
  }
  ...
  if (!(flags & GRPC_BAD_CLIENT_LARGE_REQUEST)) {
    ABSL_CHECK(gpr_event_wait(&done_write, grpc_timeout_seconds_to_deadline(5)));
  }
  ...
  if (sfd->client != nullptr) {
    ...
    if (arg->client_validator != nullptr) {
      ...
      while (true) {
        ...
        grpc_core::ExecCtx::Get()->Flush();
        do {
          ABSL_CHECK_GT(gpr_time_cmp(deadline, gpr_now(deadline.clock_type)), 0);
        ...
        ABSL_LOG(INFO) << "client validator failed; trying additional read "
...
void grpc_run_bad_client_test(
  ...
  ABSL_CHECK(grpc_core::Server::FromC(a.server)->HasOpenConnections());
  ...
  ABSL_CHECK(gpr_event_wait(&a.done_thd, grpc_timeout_seconds_to_deadline(5)));
  ...
  ABSL_CHECK(grpc_completion_queue_pluck(
...
bool rst_stream_client_validator(grpc_slice_buffer* incoming, void* /*arg*/) {
  ...
  grpc_slice_buffer_trim_end(incoming, kExpectedFrameLength,
                             &last_frame_buffer);
  ABSL_CHECK_EQ(last_frame_buffer.count, 1u);
  ...
  if (!success) {
    ABSL_LOG(INFO) << "client expected RST_STREAM frame, not found";
  }
...
void server_verifier_request_call(grpc_server* server,
  ...
  ABSL_CHECK_EQ(grpc_slice_str_cmp(call_details.method, "/foo/bar"), 0);
