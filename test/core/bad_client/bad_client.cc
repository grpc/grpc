...
static void
read_done (void *arg, grpc_error * error)
{
			  ...
			  size_t client_payload_length, uint32_t flags)
{
	  ...
	  while (true)
	    {
	      ...
	      GRPC_CLOSURE_INIT (&read_done_closure, read_done,
	      ...
	      do
		{
		  GPR_ASSERT (gpr_time_cmp
			      (deadline, gpr_now (deadline.clock_type)) > 0);
  ...
  GPR_ASSERT (gpr_event_wait
	      (&a.done_write, grpc_timeout_seconds_to_deadline (1)));
