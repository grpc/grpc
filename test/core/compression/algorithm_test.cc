...
static void test_algorithm_mesh(void) {
  ...
  for (i = 0; i < GRPC_COMPRESS_ALGORITHMS_COUNT; i++) {
    ...
    GPR_ASSERT(grpc_compression_algorithm_name(
        (grpc_compression_algorithm)i, &name));
    ...
    GPR_ASSERT(
        grpc_slice_eq(mdstr, grpc_compression_algorithm_slice(parsed)));
    ...
    if (parsed == 0) {
      ...
      GPR_ASSERT(
          grpc_slice_eq(GRPC_MDKEY(mdelem), GRPC_MDSTR_GRPC_ENCODING));
    } else {
      ...
      GPR_ASSERT(0 == memcmp(&name[stream_prefix_length],
                             GRPC_SLICE_START_PTR(value),
                             GRPC_SLICE_LENGTH(value)));
      GPR_ASSERT(grpc_slice_eq(GRPC_MDKEY(mdelem),
  ...
  /* test failure */
  GPR_ASSERT(GRPC_MDISNULL(grpc_compression_encoding_mdelem(
...
static void test_algorithm_failure(void) {
  ...
  GPR_ASSERT(grpc_compression_algorithm_name(
                 GRPC_COMPRESS_ALGORITHMS_COUNT, nullptr) == 0);
  GPR_ASSERT(grpc_compression_algorithm_name(
