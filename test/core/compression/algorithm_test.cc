  ...
  for (i = 0; i < GRPC_COMPRESS_ALGORITHMS_COUNT; i++)
    {
      ...
      GPR_ASSERT (grpc_slice_eq
		  (mdstr, grpc_compression_algorithm_slice (parsed)));
...
test_algorithm_failure (void)
{
  ...
  GPR_ASSERT (grpc_compression_algorithm_name
	      (static_cast < grpc_compression_algorithm >
	       (GRPC_COMPRESS_ALGORITHMS_COUNT + 1), nullptr) == 0);
  ...
  GPR_ASSERT (grpc_slice_eq
	      (grpc_compression_algorithm_slice
	       (GRPC_COMPRESS_ALGORITHMS_COUNT), grpc_empty_slice ()));
  GPR_ASSERT (grpc_slice_eq
	      (grpc_compression_algorithm_slice
	       (static_cast < grpc_compression_algorithm >
