...
static void
generate_random_payload (char *payload, size_t size)
{
  size_t i;
...
static bool
slice_buffer_equals_string (grpc_slice_buffer * buf, const char *str)
{
  size_t i;
...
test_stream_compression_simple_compress_decompress ()
{
  ...
  GPR_ASSERT (grpc_stream_decompress
	      (decompress_ctx, &relay, &sink, &output_size, ~(size_t) 0,
	       &end_of_context));
  ...
  ()
{
  ...
  GPR_ASSERT (grpc_stream_decompress
	      (decompress_ctx, &relay, &sink, &output_size, max_output_size,
	       &end_of_context));
  ...
  GPR_ASSERT (grpc_stream_decompress
	      (decompress_ctx, &relay, &sink, &output_size, remaining_size,
	       &end_of_context));
...
test_stream_compression_drop_context ()
{
  ...
  grpc_slice slice3 =
    grpc_slice_malloc (GRPC_SLICE_LENGTH (slice1) +
