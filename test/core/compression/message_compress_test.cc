...
static void assert_passthrough(grpc_slice value,
  ...
  GPR_ASSERT(
      grpc_compression_algorithm_name(algorithm, &algorithm_name) != 0);
  gpr_log(GPR_INFO,
...
static void test_bad_decompression_data_crc(void) {
  ...
  GPR_ASSERT(0 == grpc_msg_decompress(GRPC_COMPRESS_GZIP, &corrupted,
                                      &output));
...
static void test_bad_decompression_data_missing_trailer(void) {
  ...
  GPR_ASSERT(0 == grpc_msg_decompress(GRPC_COMPRESS_GZIP, &decompressed,
                                      &output));
...
static void test_bad_decompression_data_trailing_garbage(void) {
  ...
  grpc_core::ExecCtx exec_ctx;
  GPR_ASSERT(
      0 == grpc_msg_decompress(GRPC_COMPRESS_DEFLATE, &input, &output));
...
static void test_bad_compression_algorithm(void) {
  ...
  grpc_core::ExecCtx exec_ctx;
  was_compressed = grpc_msg_compress(GRPC_COMPRESS_ALGORITHMS_COUNT,
                                     &input, &output);
...
static void test_bad_decompression_algorithm(void) {
  ...
  grpc_core::ExecCtx exec_ctx;
  was_decompressed = grpc_msg_decompress(GRPC_COMPRESS_ALGORITHMS_COUNT,
                                         &input, &output);
