...
static void
assert_passthrough (grpc_slice value,
		    ...
		    compressability compress_result_check)
{
  ...
  GPR_ASSERT (grpc_compression_algorithm_name (algorithm, &algorithm_name) !=
	      0);
  gpr_log (GPR_INFO,
	   "assert_passthrough: value_length=%" PRIuPTR " value_hash=0x%08x "
	   "algorithm='%s' uncompressed_split='%s' compressed_split='%s'",
	   GRPC_SLICE_LENGTH (value),
	   gpr_murmur_hash3 (GRPC_SLICE_START_PTR (value),
			     GRPC_SLICE_LENGTH (value), 0), algorithm_name,
  ...
  grpc_split_slice_buffer (compressed_split_mode, &compressed_raw,
			   &compressed);

  {
    grpc_core::ExecCtx exec_ctx;
    GPR_ASSERT (grpc_msg_decompress
		(was_compressed ? algorithm : GRPC_COMPRESS_NONE, &compressed,
		 &output));
  }
...
test_tiny_data_compress (void)
{
  ...
  for (int i = 0; i < GRPC_COMPRESS_ALGORITHMS_COUNT; i++)
	...
	continue;
      grpc_core::ExecCtx exec_ctx;
      GPR_ASSERT (0 ==
		  grpc_msg_compress (static_cast <
				     grpc_compression_algorithm > (i), &input,
				     &output));
...
static void
test_bad_decompression_data_crc (void)
{
  grpc_slice_buffer input;
  ...
  GPR_ASSERT (0 ==
	      grpc_msg_decompress (GRPC_COMPRESS_GZIP, &corrupted, &output));
...
static void
test_bad_decompression_data_trailing_garbage (void)
{
  grpc_slice_buffer input;
  grpc_slice_buffer output;
  ...
  grpc_slice_buffer_add (&input,
			 ...
			 ("\x78\xda\x63\x60\x60\x60\x00\x00\x00\x04\x00\x01\x99",
			  13));

  /* try (and fail) to decompress the invalid compresed buffer */
  grpc_core::ExecCtx exec_ctx;
  GPR_ASSERT (0 ==
	      grpc_msg_decompress (GRPC_COMPRESS_DEFLATE, &input, &output));
...
static void
test_bad_decompression_data_stream (void)
{
  grpc_slice_buffer input;
  grpc_slice_buffer output;
  ...
  grpc_slice_buffer_init (&output);
  grpc_slice_buffer_add (&input,
			 grpc_slice_from_copied_buffer ("\x78\xda\xff\xff",
							4));
...
static void
test_bad_compression_algorithm (void)
{
  grpc_slice_buffer input;
  grpc_slice_buffer output;
  int was_compressed;
...
static void
test_bad_decompression_algorithm (void)
{
  grpc_slice_buffer input;
  grpc_slice_buffer output;
  ...
  for (i = 0; i < GRPC_COMPRESS_ALGORITHMS_COUNT; i++)
      ...
      for (j = 0; j < GPR_ARRAY_SIZE (uncompressed_split_modes); j++)
	{
	  for (k = 0; k < GPR_ARRAY_SIZE (compressed_split_modes); k++)
		...
		{
		  grpc_slice slice =
		    create_test_value (static_cast < test_value > (m));
		  assert_passthrough (slice,
				      static_cast <
				      ...
				      static_cast < grpc_slice_split_mode >
				      (k),
				      get_compressability (static_cast <
							   test_value > (m),
							   static_cast <
