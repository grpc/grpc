  ...
  for (i = 0; i < GPR_ARRAY_SIZE (valid_names); i++)
    {
      ...
      const int success =
	grpc_compression_algorithm_parse (grpc_slice_from_static_string
					  (valid_name), &algorithm);
...
static void
test_compression_algorithm_name (void)
{
  int success;
  const char *name;
  size_t i;
...
static void
test_compression_algorithm_for_level (void)
{
...

  {
    ...
    GPR_ASSERT (GRPC_COMPRESS_NONE ==
		...
		(GRPC_COMPRESS_LEVEL_NONE, accepted_encodings));
...
static void
test_compression_enable_disable_algorithm (void)
{
  ...
  for (algorithm = GRPC_COMPRESS_NONE;
       ...
       algorithm =
       static_cast < grpc_compression_algorithm > (static_cast <
						   int >(algorithm) + 1))
    {
