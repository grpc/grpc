...
// #include "envoy/service/auth/v3/external_auth.pb.h" // Commented out to fix
// build
 ...
 protected:
  ...
  RefCountedPtr<ExtAuthzFilter::Config> MakeConfig(
    ...
    auto channel_creds_config =
        MakeRefCounted<ChannelCredsConfig>("google_default", Json());
  ...
  RefCountedPtr<Blackboard> MakeBlackboard(
    ...
    auto client_server_target = std::make_unique<GrpcXdsServerTarget>(
        config->ext_authz->server_uri, channel_creds_config,
        call_creds_configs);

    auto client = MakeRefCounted<ExtAuthzClient>(
        transport_factory_, std::move(client_server_target));
    auto cache = MakeRefCounted<ExtAuthzFilter::ChannelCache>(
        client, config->ext_authz->xds_grpc_service);
...
TEST_F(ExtAuthzFilterTest, CreateSucceeds) {
  ...
  auto filter = ExtAuthzFilter::Create(
      ChannelArgs(),
      ChannelFilter::Args(/*instance_id=*/0, config, blackboard.get()));
...
TEST_F(ExtAuthzFilterTest, CheckAllowed) {
  ...
  auto unary_call =
      transport->WaitForUnaryCall("/envoy.service.auth.v3.Authorization/Check");
  ...
  unary_call->SendMessageToClient(
      "");  // Empty message is Valid CheckResponse with default OK status.
...
TEST_F(ExtAuthzFilterTest, CheckFailureAllow) {
  auto config =
      MakeConfig(/*instance_name=*/"ext_authz", /*server_uri=*/"server",
