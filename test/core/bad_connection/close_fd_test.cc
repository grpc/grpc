...
static void server_setup_transport(grpc_transport* transport) {
  ...
  g_ctx.server->core_server->SetupTransport(transport, nullptr,
                                            g_ctx.server->core_server->channel_args(), nullptr);
}

static void client_setup_transport(grpc_transport* transport) {
  grpc_core::ExecCtx exec_ctx;
  grpc_endpoint_add_to_pollset(g_ctx.ep->client, grpc_cq_pollset(g_ctx.client_cq));
  ...
  g_ctx.client =
      grpc_channel_create("socketpair-target", args, GRPC_CLIENT_DIRECT_CHANNEL, transport);
...
static void drain_cq(grpc_completion_queue* cq) {
  grpc_event event;
  do {
    event = grpc_completion_queue_next(cq, grpc_timeout_seconds_to_deadline(1), nullptr);
...
static void _test_close_before_server_recv(fd_type fdtype) {
  ...
  gpr_log(GPR_INFO, "Running test: test_close_%s_before_server_recv", fd_type_str(fdtype));
  test_init();
  ...
  call =
      grpc_channel_create_call(g_ctx.client, nullptr, GRPC_PROPAGATE_DEFAULTS, g_ctx.client_cq,
  ...
  error = grpc_server_request_call(g_ctx.server, &server_call, &call_details,
                                   &request_metadata_recv, g_ctx.bound_cq, g_ctx.cq, tag(101));
  GPR_ASSERT(GRPC_CALL_OK == error);
  event = grpc_completion_queue_next(g_ctx.cq, grpc_timeout_milliseconds_to_deadline(100), nullptr);
  ...
  error = grpc_call_start_batch(server_call, ops, static_cast<size_t>(op - ops), tag(102), nullptr);
  GPR_ASSERT(GRPC_CALL_OK == error);

  event = grpc_completion_queue_next(g_ctx.bound_cq, grpc_timeout_milliseconds_to_deadline(100),
                                     nullptr);
  ...
  event = grpc_completion_queue_next(g_ctx.client_cq, grpc_timeout_milliseconds_to_deadline(100),
                                     nullptr);
...
static void _test_close_before_server_send(fd_type fdtype) {
  ...
  grpc_byte_buffer* request_payload = grpc_raw_byte_buffer_create(&request_payload_slice, 1);
  grpc_byte_buffer* response_payload = grpc_raw_byte_buffer_create(&response_payload_slice, 1);
  gpr_log(GPR_INFO, "Running test: test_close_%s_before_server_send", fd_type_str(fdtype));
  test_init();
   ...
   * client. */
  close(fd);
  error = grpc_call_start_batch(server_call, ops, static_cast<size_t>(op - ops), tag(103), nullptr);
  GPR_ASSERT(GRPC_CALL_OK == error);
...
static void _test_close_before_client_send(fd_type fdtype) {
  ...
  grpc_byte_buffer* request_payload = grpc_raw_byte_buffer_create(&request_payload_slice, 1);
  grpc_byte_buffer* response_payload = grpc_raw_byte_buffer_create(&response_payload_slice, 1);
  gpr_log(GPR_INFO, "Running test: test_close_%s_before_client_send", fd_type_str(fdtype));
  test_init();
