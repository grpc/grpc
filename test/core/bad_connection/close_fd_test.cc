...
static void _test_close_before_server_recv(fd_type fdtype) {
  ...
  GRPC_CHECK_EQ(event.success, 1);
  GRPC_CHECK(event.tag == tag(101));
  ...
  if (event.type == GRPC_QUEUE_TIMEOUT) {
    GRPC_CHECK_EQ(event.success, 0);
    // status is not initialized
    GRPC_CHECK_EQ(status, GRPC_STATUS__DO_NOT_USE);
  } else {
    GRPC_CHECK(event.type == GRPC_OP_COMPLETE);
    GRPC_CHECK_EQ(event.success, 1);
    GRPC_CHECK(event.tag == tag(1));
