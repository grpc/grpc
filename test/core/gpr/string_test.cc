...
static void
expect_dump (const char *buf, size_t len, uint32_t flags, const char *result)
{
...
static void
test_pu32_succeed (const char *s, uint32_t want)
{
  uint32_t out;
  GPR_ASSERT (gpr_parse_bytes_to_uint32 (s, strlen (s), &out));
  GPR_ASSERT (out == want);
...
test_memrchr (void)
{
  ...
  GPR_ASSERT (0 ==
	      strcmp ((const char *) gpr_memrchr ("hello", 'h', 5), "hello"));
  GPR_ASSERT (0 ==
