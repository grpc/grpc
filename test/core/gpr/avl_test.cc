...
static long
int_compare (void *int1, void *int2, void *unused)
{
  return (*(int *) int1) - (*(int *) int2);
}

static void *
int_copy (void *p, void *unused)
{
  return box (*(int *) p);
}

static void
destroy (void *p, void *unused)
{
  gpr_free (p);
}
...
static void
check_get (gpr_avl avl, int key, int value)
{
  int *k = box (key);
...
static void
check_negget (gpr_avl avl, int key)
{
  int *k = box (key);
...
static gpr_avl
remove_int (gpr_avl avl, int key)
{
  int *k = box (key);
...
static void
test_stress (int amount_of_stress)
{
  int added[1024];
  int i, j;
  int deletions = 0;
  ...
  for (i = 1; deletions < amount_of_stress; i++)
      ...
      if (rand () < RAND_MAX / 2)
	{
	  added[idx] = i;
	  printf
	    ("avl = gpr_avl_add(avl, box(%d), box(%d), NULL); /* d=%d */\n",
	     idx, i, deletions);
