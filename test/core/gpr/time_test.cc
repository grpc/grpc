...
static void
i_to_s (intmax_t x, int base, int chars,
	void (*writer) (void *arg, const char *buf, size_t len), void *arg)
{
  char buf[64];
  char fmt[32];
	  ...
	  for (k = 1; k <= 10000000; k *= 10)
	    {
	      ...
	      if (gpr_time_cmp
		  (gpr_time_from_micros (sum * k, GPR_TIMESPAN), sumt) != 0)
		{
	      ...
	      if (gpr_time_cmp
		  (gpr_time_from_micros (diff * k, GPR_TIMESPAN), difft) != 0)
		{
		  fprintf (stderr, "i %d  j %d  diff %d    diff ", i, j,
			   diff);
		  fflush (stderr);
		  ts_to_s (sumt, &to_fp, stderr);
		  fprintf (stderr, "\n");
...
test_similar (void)
{
  ...
  GPR_ASSERT (1 == gpr_time_similar (gpr_time_from_micros (10, GPR_TIMESPAN),
				     ...
				     gpr_time_from_micros (10,
							   GPR_TIMESPAN)));
  GPR_ASSERT (1 ==
	      gpr_time_similar (gpr_time_from_micros (15, GPR_TIMESPAN),
				gpr_time_from_micros (10, GPR_TIMESPAN),
				gpr_time_from_micros (10, GPR_TIMESPAN)));
  GPR_ASSERT (0 ==
	      gpr_time_similar (gpr_time_from_micros (10, GPR_TIMESPAN),
				gpr_time_from_micros (25, GPR_TIMESPAN),
				gpr_time_from_micros (10, GPR_TIMESPAN)));
  GPR_ASSERT (0 ==
	      gpr_time_similar (gpr_time_from_micros (25, GPR_TIMESPAN),
				gpr_time_from_micros (10, GPR_TIMESPAN),
				gpr_time_from_micros (10, GPR_TIMESPAN)));
}

static void
test_convert_extreme (void)
{
  gpr_timespec realtime = { INT64_MAX, 1, GPR_CLOCK_REALTIME };
  gpr_timespec monotime =
    gpr_convert_clock_type (realtime, GPR_CLOCK_MONOTONIC);
  GPR_ASSERT (monotime.tv_sec == realtime.tv_sec);
  GPR_ASSERT (monotime.clock_type == GPR_CLOCK_MONOTONIC);
}

static void
test_cmp_extreme (void)
{
