...
static void i_to_s(intmax_t x, int base, int chars,
                   void (*writer)(void* arg, const char* buf,
                                  size_t len),
                   void* arg) {
  char buf[64];
  char fmt[32];
...
static void test_values(void) {
  ...
  for (i = -1; i > -1000 * 1000 * 1000; i *= 7) {
    x = gpr_time_from_micros(i, GPR_TIMESPAN);
    GPR_ASSERT(x.tv_sec * GPR_US_PER_SEC + x.tv_nsec / GPR_NS_PER_US ==
               i);
    x = gpr_time_from_nanos(i, GPR_TIMESPAN);
    GPR_ASSERT(x.tv_sec * GPR_NS_PER_SEC + x.tv_nsec == i);
    x = gpr_time_from_millis(i, GPR_TIMESPAN);
    GPR_ASSERT(x.tv_sec * GPR_MS_PER_SEC + x.tv_nsec / GPR_NS_PER_MS ==
               i);
  }
}
  ...
  for (i = -100; i <= 100; i++) {
    for (j = -100; j <= 100; j++) {
      for (k = 1; k <= 10000000; k *= 10) {
        ...
        if (gpr_time_cmp(gpr_time_from_micros(sum * k, GPR_TIMESPAN),
                         sumt) != 0) {
        ...
        if (gpr_time_cmp(gpr_time_from_micros(diff * k, GPR_TIMESPAN),
                         difft) != 0) {
