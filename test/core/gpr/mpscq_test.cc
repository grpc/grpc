  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (thds); i++)
    {
      ...
      GPR_ASSERT (gpr_thd_new
		  (&thds[i], "grpc_mt_test", test_thread, &ta[i], &options));
    }
  size_t num_done = 0;
  size_t spins = 0;
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (thds); i++)
    {
      ...
      ta[i].start = &start;
      GPR_ASSERT (gpr_thd_new
		  (&thds[i], "grpc_multipop_test", test_thread, &ta[i],
		   &options));
    }
  pull_args pa;
  pa.ta = ta;
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (pull_thds); i++)
    {
      ...
      gpr_thd_options_set_joinable (&options);
      GPR_ASSERT (gpr_thd_new
		  (&pull_thds[i], "grpc_multipop_pull", pull_thread, &pa,
		   &options));
    }
  gpr_event_set (&start, (void *) 1);
  for (size_t i = 0; i < GPR_ARRAY_SIZE (pull_thds); i++)
