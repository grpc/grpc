...
TEST(SliceBufferTest, TinyAddNotEnoughSpace) {
  ...
  // Exceed GRPC_SLICE_INLINED_SIZE which is at least 23.
  // Add 20 bytes, then 5 bytes.
  uint8_t* p1 = grpc_slice_buffer_tiny_add(&buf, 20);
  memset(p1, 'a', 20);
  ASSERT_EQ(buf.count, 1);
  ASSERT_EQ(buf.length, 20);

  // This one should fall back to a new slice.
  uint8_t* p2 = grpc_slice_buffer_tiny_add(&buf, 5);
  ASSERT_NE(p2, p1 + 20);
  memset(p2, 'b', 5);
  ...
  ASSERT_EQ(GRPC_SLICE_LENGTH(slice1), 20);
