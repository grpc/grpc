...
static void
test_vector (const char *raw, size_t raw_length, const char *encoded,
	     size_t encoded_length, const uint8_t * dict)
{
  ...
  char *encoded2raw_permissive_msg =
    grpc_dump_slice (encoded2raw_permissive_slice,
		     GPR_DUMP_HEX | GPR_DUMP_ASCII);
  gpr_log (GPR_DEBUG,
	   "Result:\nraw2encoded = %s\nencoded2raw = %s\nencoded2raw_permissive "
	   "= %s", raw2encoded_msg, encoded2raw_msg,
...
static void
test_nonconformant_vector (const char *encoded,
			   ...
			   const uint8_t * dict)
{
  ...
  grpc_slice permissive_unencoded_slice =
    grpc_slice_from_copied_buffer (permissive_unencoded,
				   permissive_unencoded_length);
  grpc_slice encoded_slice =
    grpc_slice_from_copied_buffer (encoded, encoded_length);
  grpc_slice encoded2raw_slice;
  GPR_ASSERT (!grpc_strict_percent_decode_slice (encoded_slice, dict,
