...
#define TEST_NONCONFORMANT_VECTOR(encoded, permissive_unencoded, dict)          \
  test_nonconformant_vector(encoded, sizeof(encoded) - 1, permissive_unencoded, \
...
static void test_vector(const char* raw, size_t raw_length, const char* encoded,
  ...
  grpc_slice encoded_slice = grpc_slice_from_copied_buffer(encoded, encoded_length);
  grpc_slice raw2encoded_slice = grpc_percent_encode_slice(raw_slice, dict);
  grpc_slice encoded2raw_slice;
  GPR_ASSERT(grpc_strict_percent_decode_slice(encoded_slice, dict, &encoded2raw_slice));
  grpc_slice encoded2raw_permissive_slice = grpc_permissive_percent_decode_slice(encoded_slice);

  char* raw2encoded_msg = grpc_dump_slice(raw2encoded_slice, GPR_DUMP_HEX | GPR_DUMP_ASCII);
  char* encoded2raw_msg = grpc_dump_slice(encoded2raw_slice, GPR_DUMP_HEX | GPR_DUMP_ASCII);
  char* encoded2raw_permissive_msg =
      grpc_dump_slice(encoded2raw_permissive_slice, GPR_DUMP_HEX | GPR_DUMP_ASCII);
  gpr_log(GPR_DEBUG,
...
static void test_nonconformant_vector(const char* encoded, size_t encoded_length,
                                      const char* permissive_unencoded,
                                      size_t permissive_unencoded_length, const uint8_t* dict) {
  char* permissive_unencoded_msg =
      gpr_dump(permissive_unencoded, permissive_unencoded_length, GPR_DUMP_HEX | GPR_DUMP_ASCII);
  char* encoded_msg = gpr_dump(encoded, encoded_length, GPR_DUMP_HEX | GPR_DUMP_ASCII);
  gpr_log(GPR_DEBUG, "Trial:\nraw = %s\nencoded = %s", permissive_unencoded_msg, encoded_msg);
  gpr_free(permissive_unencoded_msg);
  gpr_free(encoded_msg);

  grpc_slice permissive_unencoded_slice =
      grpc_slice_from_copied_buffer(permissive_unencoded, permissive_unencoded_length);
  grpc_slice encoded_slice = grpc_slice_from_copied_buffer(encoded, encoded_length);
  grpc_slice encoded2raw_slice;
  GPR_ASSERT(!grpc_strict_percent_decode_slice(encoded_slice, dict, &encoded2raw_slice));
  grpc_slice encoded2raw_permissive_slice = grpc_permissive_percent_decode_slice(encoded_slice);
  ...
  GPR_ASSERT(grpc_slice_eq(permissive_unencoded_slice, encoded2raw_permissive_slice));
...
int main(int argc, char** argv) {
  ...
  grpc_init();
  TEST_VECTOR("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~",
  ...
  TEST_NONCONFORMANT_VECTOR("%", "%", grpc_url_percent_encoding_unreserved_bytes);
  TEST_NONCONFORMANT_VECTOR("%A", "%A", grpc_url_percent_encoding_unreserved_bytes);
  TEST_NONCONFORMANT_VECTOR("%AG", "%AG", grpc_url_percent_encoding_unreserved_bytes);
  TEST_NONCONFORMANT_VECTOR("\0", "\0", grpc_url_percent_encoding_unreserved_bytes);
  grpc_shutdown();
  return 0;
}
