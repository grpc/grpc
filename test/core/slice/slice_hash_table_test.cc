...
static void
populate_entries (const test_entry * input, size_t num_entries,
...
static void
check_values (const test_entry * input, size_t num_entries,
...
static void
check_non_existent_value (const char *key_string,
			  grpc_slice_hash_table * table)
{
...
static void
destroy_string (void *value)
{
  gpr_free (value);
}

static grpc_slice_hash_table *
create_table_from_entries (const test_entry * test_entries,
			   size_t num_test_entries,
			   int (*value_cmp_fn) (void *, void *))
{
  ...
  grpc_slice_hash_table *table =
    grpc_slice_hash_table_create (num_test_entries, entries, destroy_string,
				  value_cmp_fn);
  gpr_free (entries);
  return table;
}
...
test_slice_hash_table_not_eq ()
{
  ...
  const size_t num_entries_b_smaller =
    GPR_ARRAY_SIZE (test_entries_b_smaller);
  grpc_slice_hash_table *table_b_smaller =
    create_table_from_entries (test_entries_b_smaller, num_entries_b_smaller,
			       value_cmp_fn);
  ...
  grpc_slice_hash_table *table_b_larger =
    create_table_from_entries (test_entries_b_larger, num_entries_b_larger,
			       value_cmp_fn);
