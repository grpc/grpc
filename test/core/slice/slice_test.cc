...
static void
test_slice_new_with_user_data (void)
{
  int marker = 0;
  uint8_t buf[2];
  grpc_slice slice;
...
static void
check_head_tail (grpc_slice slice, grpc_slice head, grpc_slice tail)
{
  ...
  GPR_ASSERT (0 ==
	      memcmp (GRPC_SLICE_START_PTR (slice) + GRPC_SLICE_LENGTH (head),
...
test_slice_from_copied_string_works (void)
{
  ...
  GPR_ASSERT (0 ==
	      memcmp (text, GRPC_SLICE_START_PTR (slice),
		      GRPC_SLICE_LENGTH (slice)));
  grpc_slice_unref (slice);
}

static void
test_slice_interning (void)
{
...
static void
test_static_slice_interning (void)
{
...
static void
test_static_slice_copy_interning (void)
{
