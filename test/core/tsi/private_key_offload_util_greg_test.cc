  ...
  void Sign(absl::string_view data_to_sign,
    ...
    if (padding == RSA_PKCS1_PADDING) {
      if (!EVP_PKEY_CTX_set_rsa_padding(pctx, RSA_PKCS1_PADDING)) {
        on_sign_complete(absl::InternalError("EVP_PKEY_CTX_set_rsa_padding failed"));
        return;
      }
    ...
    if (!EVP_DigestSignFinal(ctx.get(),
                             reinterpret_cast<uint8_t*>(sig.data()), &sig_len)) {
      on_sign_complete(absl::InternalError("EVP_DigestSignFinal failed"));
      return;
    }
    sig.resize(sig_len);
...
absl::Status Verify(EVP_PKEY* pkey, PrivateKeySigner::SignatureAlgorithm alg,
  ...
  if (EVP_DigestVerifyFinal(
          ctx.get(), reinterpret_cast<const uint8_t*>(sig.data()), sig.size())) {
...
TEST_P(BoringSslPrivateKeySignerTest, SignAndVerify) {
  const TestVector& param = GetParam();
//   ASSERT_TRUE(LoadFile(param.key_path, 0, &key_slice).ok());
  ...
  BoringSslPrivateKeySigner signer(std::move(key));
