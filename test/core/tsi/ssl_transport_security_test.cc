...
typedef struct ssl_key_cert_lib {
  ...
  bool use_root_store;
  char* root_cert;
  ssl_root_certs_store* root_store;
...
static void ssl_test_setup_handshakers(tsi_test_fixture* fixture) {
  ...
  GPR_ASSERT(
      tsi_create_ssl_client_handshaker_factory(
          client_key_cert_pair, key_cert_lib->root_cert,
          key_cert_lib->use_root_store ? key_cert_lib->root_store : nullptr,
          nullptr, (const char**)client_alpn_protocols,
...
static void ssl_test_destruct(tsi_test_fixture* fixture) {
  ...
  ssl_root_certs_store_destroy(key_cert_lib->root_store);
...
static tsi_test_fixture* ssl_tsi_test_fixture_create() {
  ...
  key_cert_lib->root_store =
      ssl_root_certs_store_create(key_cert_lib->root_cert);
...
void test_tsi_ssl_client_handshaker_factory_refcounting() {
  ...
  GPR_ASSERT(tsi_create_ssl_client_handshaker_factory(
                 nullptr, cert_chain, nullptr, nullptr, nullptr, 0,
