...
static void
ssl_test_setup_handshakers (tsi_test_fixture * fixture)
{
  ...
  GPR_ASSERT (tsi_create_ssl_server_handshaker_factory
	      (key_cert_lib->use_bad_server_cert ? key_cert_lib->
	       bad_server_pem_key_cert_pairs : key_cert_lib->
	       server_pem_key_cert_pairs,
	       key_cert_lib->use_bad_server_cert ? key_cert_lib->
	       bad_server_num_key_cert_pairs : key_cert_lib->
	       server_num_key_cert_pairs, key_cert_lib->root_cert,
	       ssl_fixture->force_client_auth, nullptr,
  ...
  GPR_ASSERT (tsi_ssl_server_handshaker_factory_create_handshaker
	      (ssl_fixture->server_handshaker_factory,
	       &server_handshaker) == TSI_OK);
  ssl_fixture->base.server_handshaker =
    tsi_create_adapter_handshaker (server_handshaker);
}

static void
check_alpn (ssl_tsi_test_fixture * ssl_fixture, const tsi_peer * peer)
{
  GPR_ASSERT (ssl_fixture != nullptr);
  GPR_ASSERT (ssl_fixture->alpn_lib != nullptr);
  ssl_alpn_lib *alpn_lib = ssl_fixture->alpn_lib;
...
check_basic_authenticated_peer_and_get_common_name (const tsi_peer * peer)
{
  ...
  GPR_ASSERT (memcmp
	      (cert_type_property->value.data, TSI_X509_CERTIFICATE_TYPE,
...
void
check_server0_peer (tsi_peer * peer)
{
  const tsi_peer_property *property =
  ...
  GPR_ASSERT (tsi_peer_get_property_by_name
	      (peer,
	       TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY) == nullptr);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "foo.test.google.com.au") ==
	      1);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "bar.test.google.com.au") ==
	      1);
...
static bool
check_subject_alt_name (tsi_peer * peer, const char *name)
{
...
void
check_server1_peer (tsi_peer * peer)
{
  const tsi_peer_property *property =
    check_basic_authenticated_peer_and_get_common_name (peer);
  ...
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "waterzooi.test.google.be") ==
	      1);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "foo.test.youtube.com") == 1);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "bar.foo.test.google.com") ==
	      0);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "test.google.fr") == 0);
  GPR_ASSERT (tsi_ssl_peer_matches_name (peer, "tartines.test.google.be") ==
	      0);
...
static void
check_client_peer (ssl_tsi_test_fixture * ssl_fixture, tsi_peer * peer)
{
  GPR_ASSERT (ssl_fixture != nullptr);
  GPR_ASSERT (ssl_fixture->alpn_lib != nullptr);
  ssl_alpn_lib *alpn_lib = ssl_fixture->alpn_lib;
...
static void
ssl_test_check_handshaker_peers (tsi_test_fixture * fixture)
{
  ssl_tsi_test_fixture *ssl_fixture = (ssl_tsi_test_fixture *) fixture;
  GPR_ASSERT (ssl_fixture != nullptr);
  GPR_ASSERT (ssl_fixture->key_cert_lib != nullptr);
...
static void
ssl_test_destruct (tsi_test_fixture * fixture)
{
  ssl_tsi_test_fixture *ssl_fixture = (ssl_tsi_test_fixture *) fixture;
  ...
  ssl_test_pem_key_cert_pair_destroy (key_cert_lib->
				      bad_client_pem_key_cert_pair);
  gpr_free (key_cert_lib->root_cert);
  gpr_free (key_cert_lib);
  /* Unreference others. */
  tsi_ssl_server_handshaker_factory_unref (ssl_fixture->
					   server_handshaker_factory);
  tsi_ssl_client_handshaker_factory_unref (ssl_fixture->
...
static char *
load_file (const char *dir_path, const char *file_name)
{
  char *file_path =
    static_cast <
    char
...
static tsi_test_fixture *
ssl_tsi_test_fixture_create ()
{
  ssl_tsi_test_fixture *ssl_fixture =
    static_cast <
    ssl_tsi_test_fixture * >(gpr_zalloc (sizeof (*ssl_fixture)));
  ...
  alpn_lib->server_alpn_protocols =
    static_cast <
    char **>(gpr_zalloc (sizeof (char *) * SSL_TSI_TEST_ALPN_NUM));
...
ssl_tsi_test_do_round_trip_for_all_configs ()
{
  ...
  for (unsigned int val = 0; val < TSI_TEST_NUM_OF_COMBINATIONS; val++)
      ...
      for (unsigned int ind = 0; ind < TSI_TEST_NUM_OF_ARGUMENTS; ind++)
	{
      ...
      ssl_fixture->base.config =
	tsi_test_frame_protector_config_create (bit_array[0], bit_array[1],
...
static void
ssl_tsi_test_handshaker_factory_destructor (tsi_ssl_handshaker_factory *
					    factory)
{
...
test_tsi_ssl_client_handshaker_factory_refcounting ()
{
  ...
  original_vtable = tsi_ssl_handshaker_factory_swap_vtable ((tsi_ssl_handshaker_factory *) client_handshaker_factory, &test_handshaker_factory_vtable);
...
test_tsi_ssl_server_handshaker_factory_refcounting ()
{
  ...
  handshaker_factory_destructor_called = false;
  original_vtable = tsi_ssl_handshaker_factory_swap_vtable ((tsi_ssl_handshaker_factory *) server_handshaker_factory, &test_handshaker_factory_vtable);
