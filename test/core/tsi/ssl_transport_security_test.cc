...
absl::Status void do_handshake_helper(SSL** out_client, SSL** out_server) {
  ...
  BIO* client_cert_bio(BIO_new_mem_buf(cert_pem.c_str()));
  X509* client_cert = PEM_read_bio_X509(client_cert_bio, /*x=*/nullptr,
                                        /*cb=*/nullptr, /*u=*/nullptr);
  BIO* client_key_bio(BIO_new_mem_buf(key_pem.c_str()));
  EVPPKEY* client_key = PEM_read_bio_PrivateKey(client_key_bio, /*x=*/nullptr,
                                                /*cb=*/nullptr, /*u=*/nullptr);

  BIO* server_cert_bio(BIO_new_mem_buf(cert_pem.c_str()));
  X509* server_cert = PEM_read_bio_X509(server_cert_bio, /*x=*/nullptr,
                                        /*cb=*/nullptr, /*u=*/nullptr);
  BIO* server_key_bio(BIO_new_mem_buf(key_pem.c_str()));
  EVPPKEY* server_key = PEM_read_bio_PrivateKey(server_key_bio, /*x=*/nullptr,
                                                /*cb=*/nullptr, /*u=*/nullptr);
...
void ssl_frame_protector_util_function_test_helper(
    ...
    size_t& peer_buffer_offset),
{
  ...
  GPR_ASSERT(ssl_protector_protect_util(
                 ...
                 buffer.data(), ssl, bio, &unprotected_bytes_size,
                 protected_output_frames.data(),
                 &protected_output_frames_size) == tsi_result::TSI_OK);

  // If |plaintext_size| is larger than the inner buffer size
  ...
  if (plaintext_size >= kMaxPlaintextBytesPerTlsRecord) {
    GPR_ASSERT(unprotected_bytes_size == kMaxPlaintextBytesPerTlsRecord);
  } else {
    GPR_ASSERT(unprotected_bytes_size == plaintext_size);
    ...
    GPR_ASSERT(ssl_protector_protect_flush_util(
                   ...
                   &still_pending_size) == tsi_result::TSI_OK);
    GPR_ASSERT(still_pending_size == 0);
  }

  GPR_ASSERT(protected_output_frames_size == expected_encrypted_bytes_size);
  // The first three bytes are always 0x17, 0x03, 0x03.
  GPR_ASSERT(protected_output_frames[0] == '\x17');
  GPR_ASSERT(protected_output_frames[1] == '\x03');
  GPR_ASSERT(protected_output_frames[2] == '\x03');
  // The next two bytes are the size of the record, which is 5 bytes less
  // than the size of the whole frame.
  GPR_ASSERT(CalculateRecordSizeFromHeader(protected_output_frames[3],
  ...
  GPR_ASSERT(ssl_protector_unprotect_util(
                 protected_output_frames.data(), peer_buffer.size(),
                 peer_buffer_offset, peer_buffer.data(), peer_ssl, peer_bio,
...
void ssl_tsi_test_frame_protector_util_functions() {
  ...
  for (std::size_t plaintext_size : {1, 1000, 16384, 17000}) {
    ...
    GPR_ASSERT(client_ssl != nullptr);
    GPR_ASSERT(server_ssl != nullptr);

    BIO* client_bio = nullptr;
    BIO* client_ssl_bio = nullptr;
    GPR_ASSERT(BIO_new_bio_pair(&client_bio, /*writebuf1=*/0, &client_ssl_bio,
                                /*writebuf2=*/0) == 1);
    SSL_set_bio(client_ssl, client_ssl_bio, client_ssl_bio);

    BIO* server_bio = nullptr;
    BIO* server_ssl_bio = nullptr;
    GPR_ASSERT(BIO_new_bio_pair(&server_bio, /*writebuf1=*/0, &server_ssl_bio,
                                /*writebuf2=*/0) == 1);
    SSL_set_bio(server_ssl, server_ssl_bio, server_ssl_bio);

    std::vector<uint8_t> client_buffer(kTls13RecordOverhead);
    size_t client_buffer_offset = 0;
    ...
    ssl_frame_protector_util_function_test_helper(
        plaintext_size, expected_size, client_ssl, client_bio, client_buffer,
        client_buffer_offset, server_ssl, server_bio, server_buffer,
        server_buffer_offset);
    ssl_frame_protector_util_function_test_helper(
        plaintext_size, expected_size, server_ssl, server_bio, server_buffer,
        server_buffer_offset, client_ssl, client_bio, client_buffer,
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < number_tls_versions; i++) {
    ...
    ssl_tsi_test_frame_protector_util_functions();
  }
  grpc_shutdown();
  return 0;
}
