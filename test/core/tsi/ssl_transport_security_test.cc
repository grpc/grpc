...
// OpenSSL 1.1 uses AES256 for encryption session ticket by default so
// specify different STEK size.
...
static void ssl_test_setup_handshakers(tsi_test_fixture* fixture) {
  ...
  if (alpn_lib->alpn_mode == ALPN_CLIENT_NO_SERVER ||
    ...
    client_options.num_alpn_protocols =
...
check_basic_authenticated_peer_and_get_common_name(
  ...
  GPR_ASSERT(memcmp(cert_type_property->value.data,
...
void check_server0_peer(tsi_peer* peer) {
  ...
  GPR_ASSERT(
      ...
      nullptr);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "foo.test.google.com.au") == 1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "bar.test.google.com.au") == 1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "BAR.TEST.GOOGLE.COM.AU") == 1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "Bar.Test.Google.Com.Au") == 1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "bAr.TeST.gOOgle.cOm.AU") == 1);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "bar.test.google.blah") ==
             0);
  GPR_ASSERT(tsi_ssl_peer_matches_name(
                 peer, "foo.bar.test.google.com.au") == 0);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "test.google.com.au") ==
             0);
  ...
  for (size_t i = 0; i < peer->property_count; i++) {
    ...
    if (strcmp(prop->name, property_name) == 0) {
      if (strlen(property_value) == prop->value.length &&
          memcmp(prop->value.data, property_value,
                 prop->value.length) == 0) {
        return true;
      }
    }
...
static bool check_subject_alt_name(tsi_peer* peer, const char* name) {
  return check_property(
      peer, TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY, name);
}
...
void check_server1_peer(tsi_peer* peer) {
  ...
  GPR_ASSERT(check_subject_alt_name(peer, "waterzooi.test.google.be") ==
             1);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "foo.test.google.fr") ==
             1);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "bar.test.google.fr") ==
             1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "waterzooi.test.google.be") == 1);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "foo.test.youtube.com") ==
             1);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "bar.foo.test.google.com") == 0);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "test.google.fr") == 0);
  GPR_ASSERT(
      tsi_ssl_peer_matches_name(peer, "tartines.test.google.be") == 0);
  GPR_ASSERT(tsi_ssl_peer_matches_name(peer, "tartines.youtube.com") ==
             0);
  tsi_peer_destruct(peer);
}
...
static void ssl_test_check_handshaker_peers(tsi_test_fixture* fixture) {
  ...
  // sends a bad certificate. In such a case, the server would fail the
  // TLS handshake and send an alert to the client as the first
  // application data message. In TLS 1.2, the client-side handshake
  // will fail if the client sends a bad certificate.
  //
  ...
  if (expect_client_success) {
    ...
    if (ssl_fixture->server_name_indication == nullptr ||
        strcmp(ssl_fixture->server_name_indication,
...
static void ssl_test_pem_key_cert_pair_destroy(
...
static void ssl_test_destruct(tsi_test_fixture* fixture) {
  ...
  for (size_t i = 0; i < key_cert_lib->bad_server_num_key_cert_pairs;
       i++) {
    ssl_test_pem_key_cert_pair_destroy(
...
static char* load_file(const char* dir_path, const char* file_name) {
  char* file_path = static_cast<char*>(gpr_zalloc(
      sizeof(char) * (strlen(dir_path) + strlen(file_name) + 1)));
...
void ssl_tsi_test_do_handshake_tiny_handshake_buffer() {
  ...
  // Handshake buffer is too small to hold both handshake messages and
  // the unused bytes.
...
void ssl_tsi_test_do_handshake_with_client_authentication_and_root_store() {
  gpr_log(GPR_INFO,
          "ssl_tsi_test_do_handshake_with_client_authentication_and_"
          "root_store");
  tsi_test_fixture* fixture = ssl_tsi_test_fixture_create();
  ssl_tsi_test_fixture* ssl_fixture =
      reinterpret_cast<ssl_tsi_test_fixture*>(fixture);
...
void ssl_tsi_test_do_handshake_with_server_name_indication_exact_domain() {
  gpr_log(GPR_INFO,
          "ssl_tsi_test_do_handshake_with_server_name_indication_exact_"
          "domain");
...
void ssl_tsi_test_do_handshake_with_server_name_indication_wild_star_domain() {
  gpr_log(GPR_INFO,
          "ssl_tsi_test_do_handshake_with_server_name_indication_wild_"
          "star_domain");
...
void ssl_tsi_test_do_round_trip_odd_buffer_size() {
  ...
  // 2. On 32-bit, the test is much slower (probably due to lack of
  // boringssl asm optimizations) so we only run a subset of tests to
  // avoid timeout.
  // 3. On Mac OS, we have slower testing machines so we only run a
  // subset of tests to avoid timeout.
  if (is_slow_build()) {
    size = 1;
  }
        ...
        for (size_t ind4 = 0; ind4 < size; ind4++) {
          for (size_t ind5 = 0; ind5 < size; ind5++) {
            ...
            tsi_test_frame_protector_config_set_buffer_size(
                ssl_fixture->base.config, odd_sizes[ind1],
...
void ssl_tsi_test_do_handshake_session_cache() {
  ...
  tsi_ssl_session_cache* session_cache =
      tsi_ssl_session_cache_create_lru(16);
...
static void ssl_tsi_test_handshaker_factory_destructor(
  ...
  if (original_vtable != nullptr &&
      original_vtable->destroy != nullptr) {
    original_vtable->destroy(factory);
  }
}

static tsi_ssl_handshaker_factory_vtable
...
void test_tsi_ssl_client_handshaker_factory_refcounting() {
  ...
  original_vtable = tsi_ssl_handshaker_factory_swap_vtable(
      reinterpret_cast<tsi_ssl_handshaker_factory*>(
  ...
  for (i = 0; i < 3; ++i) {
    GPR_ASSERT(tsi_ssl_client_handshaker_factory_create_handshaker(
                   client_handshaker_factory, "google.com",
                   &handshaker[i]) == TSI_OK);
  }
...
void test_tsi_ssl_server_handshaker_factory_refcounting() {
  ...
  for (i = 0; i < 3; ++i) {
    GPR_ASSERT(tsi_ssl_server_handshaker_factory_create_handshaker(
                   server_handshaker_factory, &handshaker[i]) ==
               TSI_OK);
  }
...
/* Attempting to create a handshaker factory with invalid parameters
...
void ssl_tsi_test_duplicate_root_certificates() {
  ...
  memcpy(dup_root_cert + strlen(root_cert), root_cert,
         strlen(root_cert));
  tsi_ssl_root_certs_store* root_store =
...
void ssl_tsi_test_extract_x509_subject_names() {
  ...
  char* cert =
      load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "multi-domain.pem");
  tsi_peer peer;
  GPR_ASSERT(tsi_ssl_extract_x509_subject_names_from_pem_cert(
                 cert, &peer) == TSI_OK);
  // tsi_peer should include one common name, one certificate, one
  // security level, ten SAN fields, two DNS SAN fields, three URI
  // fields, two email addresses and two IP addresses.
  ...
  const tsi_peer_property* property = tsi_peer_get_property_by_name(
      &peer, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY);
  GPR_ASSERT(property != nullptr);
  GPR_ASSERT(memcmp(property->value.data, expected_cn,
                    property->value.length) == 0);
  ...
  GPR_ASSERT(property != nullptr);
  GPR_ASSERT(
      memcmp(property->value.data, cert, property->value.length) == 0);
  // Check DNS
  ...
  GPR_ASSERT(
      check_subject_alt_name(&peer, "spiffe://foo.com/bar/baz") == 1);
  GPR_ASSERT(check_subject_alt_name(
                 &peer, "https://foo.test.domain.com/test") == 1);
  GPR_ASSERT(check_subject_alt_name(
                 &peer, "https://bar.test.domain.com/test") == 1);
...
void ssl_tsi_test_extract_cert_chain() {
  ...
  GPR_ASSERT(tsi_ssl_get_cert_chain_contents(
                 cert_chain, &chain_property) == TSI_OK);
