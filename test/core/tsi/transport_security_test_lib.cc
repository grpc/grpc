...
static void
notification_signal (tsi_test_fixture * fixture)
{
...
static void
notification_wait (tsi_test_fixture * fixture)
{
  gpr_mu_lock (&fixture->mu);
  while (!fixture->notified)
    {
      gpr_cv_wait (&fixture->cv, &fixture->mu,
...
static handshaker_args *
handshaker_args_create (tsi_test_fixture * fixture, bool is_client)
{
...
static void
handshaker_args_destroy (handshaker_args * args)
{
...
static void
setup_handshakers (tsi_test_fixture * fixture)
{
  GPR_ASSERT (fixture != nullptr);
...
static void
check_unused_bytes (tsi_test_fixture * fixture)
{
...
static void
check_handshake_results (tsi_test_fixture * fixture)
{
  GPR_ASSERT (fixture != nullptr);
  GPR_ASSERT (fixture->vtable != nullptr);
...
static void
send_bytes_to_peer (tsi_test_fixture * fixture,
		    const unsigned char *buf, size_t buf_size, bool is_client)
{
  GPR_ASSERT (fixture != nullptr);
  ...
  size_t *bytes_written =
    is_client ? &fixture->bytes_written_to_server_channel : &fixture->
    bytes_written_to_client_channel;
...
static void
maybe_append_unused_bytes (handshaker_args * args)
{
...
static void
receive_bytes_from_peer (tsi_test_fixture * fixture,
			 ...
			 bool is_client)
{
  ...
  size_t *bytes_written =
    is_client ? &fixture->bytes_written_to_client_channel : &fixture->
    bytes_written_to_server_channel;
  GPR_ASSERT (bytes_read != nullptr);
  GPR_ASSERT (bytes_written != nullptr);
  size_t to_read = *buf_size < *bytes_written - *bytes_read
    ? *buf_size : *bytes_written - *bytes_read;
...
static void
send_message_to_peer (tsi_test_fixture * fixture,
		      tsi_frame_protector * protector, bool is_client)
{
  ...
  unsigned char *protected_buffer =
    static_cast <
    unsigned char *>(gpr_zalloc (config->protected_buffer_size));
  size_t message_size =
  ...
  while (message_size > 0 && result == TSI_OK)
    {
      ...
      /* Do protect. */
      result =
	tsi_frame_protector_protect (protector, message_bytes,
	  ...
	  do
	    {
	      ...
	      result =
		tsi_frame_protector_protect_flush (protector,
						   protected_buffer,
						   &protected_buffer_size_to_send,
						   &still_pending_size);
	      GPR_ASSERT (result == TSI_OK);
...
static void
receive_message_from_peer (tsi_test_fixture * fixture,
			   ...
			   size_t * bytes_received, bool is_client)
{
  ...
  unsigned char *read_buffer =
    static_cast <
    unsigned char *>(gpr_zalloc (config->read_buffer_allocated_size));
  unsigned char *message_buffer =
    static_cast <
    unsigned char *>(gpr_zalloc (config->message_buffer_allocated_size));
  ...
  while (!done && result == TSI_OK)
      ...
      if (read_from_peer_size == 0)
	{
	  ...
	  receive_bytes_from_peer (fixture, &read_buffer,
				   &read_from_peer_size, is_client);
	  read_offset = 0;
	}
      if (read_from_peer_size == 0)
	{
      ...
      do
	{
	  ...
	  result =
	    tsi_frame_protector_unprotect (protector,
	  ...
	  if (message_buffer_size > 0)
	    {
	      memcpy (message + message_offset, message_buffer,
		      message_buffer_size);
...
grpc_error *
on_handshake_next_done (tsi_result result, void *user_data,
...
static void
on_handshake_next_done_wrapper (tsi_result result, void *user_data,
				const unsigned char *bytes_to_send,
				size_t bytes_to_send_size,
				tsi_handshaker_result * handshaker_result)
{
...
static bool
is_handshake_finished_properly (handshaker_args * args)
{
  GPR_ASSERT (args != nullptr);
  GPR_ASSERT (args->fixture != nullptr);
  tsi_test_fixture *fixture = args->fixture;
...
static void
do_handshaker_next (handshaker_args * args)
{
  /* Initialization. */
  GPR_ASSERT (args != nullptr);
  GPR_ASSERT (args->fixture != nullptr);
  ...
  do
    {
      ...
      receive_bytes_from_peer (args->fixture, &args->handshake_buffer,
	...
	{
	  args->error =
	    on_handshake_next_done (result, args, bytes_to_send,
				    bytes_to_send_size, handshaker_result);
...
void
tsi_test_do_handshake (tsi_test_fixture * fixture)
{
  /* Initializaiton. */
...
void
tsi_test_do_round_trip (tsi_test_fixture * fixture)
{
  /* Initialization. */
  GPR_ASSERT (fixture != nullptr);
  GPR_ASSERT (fixture->config != nullptr);
  ...
  GPR_ASSERT (tsi_handshaker_result_create_frame_protector
	      (fixture->client_result,
	       ...
	       0 ? nullptr : &client_max_output_protected_frame_size,
  ...
  GPR_ASSERT (tsi_handshaker_result_create_frame_protector
	      (fixture->server_result,
	       ...
	       0 ? nullptr : &server_max_output_protected_frame_size,
	       &server_frame_protector) == TSI_OK);
  /* Client sends a message to server. */
  send_message_to_peer (fixture, client_frame_protector,
			true /* is_client */ );
  unsigned char *server_received_message =
    static_cast <
    unsigned char *>(gpr_zalloc (TSI_TEST_DEFAULT_CHANNEL_SIZE));
  size_t server_received_message_size = 0;
  receive_message_from_peer (fixture, server_frame_protector,
  ...
  send_message_to_peer (fixture, server_frame_protector,
			false /* is_client */ );
  unsigned char *client_received_message =
    static_cast <
    unsigned char *>(gpr_zalloc (TSI_TEST_DEFAULT_CHANNEL_SIZE));
  size_t client_received_message_size = 0;
  receive_message_from_peer (fixture, client_frame_protector,
...
generate_random_message (size_t size)
{
  size_t i;
  unsigned char chars[] = "abcdefghijklmnopqrstuvwxyz1234567890";
  unsigned char *output =
    static_cast <
    unsigned char *>(gpr_zalloc (sizeof (unsigned char) * size));
  for (i = 0; i < size - 1; ++i)
    {
...
tsi_test_frame_protector_config_create (bool
					...
					bool
					use_default_handshake_buffer_size)
{
  tsi_test_frame_protector_config *config =
    static_cast <
    tsi_test_frame_protector_config * >(gpr_zalloc (sizeof (*config)));
  ...
  config->client_message_size = use_default_client_message
    ? TSI_TEST_BIG_MESSAGE_SIZE : TSI_TEST_SMALL_MESSAGE_SIZE;
  ...
  config->client_max_output_protected_frame_size =
    use_default_client_max_output_protected_frame_size
    ? 0 : TSI_TEST_SMALL_CLIENT_MAX_OUTPUT_PROTECTED_FRAME_SIZE;
  /* Set the value for server max_output_protected_frame_size.
     If it is 0, we pass NULL to tsi_handshaker_result_create_frame_protector(),
     which then uses default protected frame size for it. */
  config->server_max_output_protected_frame_size =
    use_default_server_max_output_protected_frame_size
    ? 0 : TSI_TEST_SMALL_SERVER_MAX_OUTPUT_PROTECTED_FRAME_SIZE;
  return config;
}
...
tsi_test_frame_protector_config_set_buffer_size
  (tsi_test_frame_protector_config * config,
   size_t read_buffer_allocated_size, size_t message_buffer_allocated_size,
...
void
tsi_test_frame_protector_config_destroy (tsi_test_frame_protector_config *
					 config)
{
  GPR_ASSERT (config != nullptr);
...
void
tsi_test_fixture_init (tsi_test_fixture * fixture)
{
  fixture->config =
    tsi_test_frame_protector_config_create (true, true, true, true, true,
					    true, true, true);
...
void
tsi_test_fixture_destroy (tsi_test_fixture * fixture)
{
  GPR_ASSERT (fixture != nullptr);
