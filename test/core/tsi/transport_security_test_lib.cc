...
static handshaker_args* handshaker_args_create(
  ...
  args->handshake_buffer = static_cast<unsigned char*>(
      gpr_zalloc(args->handshake_buffer_size));
...
static void send_bytes_to_peer(tsi_test_channel* test_channel,
  ...
  uint8_t* channel = is_client ? test_channel->server_channel
                               : test_channel->client_channel;
  GPR_ASSERT(channel != nullptr);
  size_t* bytes_written =
      is_client ? &test_channel->bytes_written_to_server_channel
                : &test_channel->bytes_written_to_client_channel;
  GPR_ASSERT(bytes_written != nullptr);
  GPR_ASSERT(*bytes_written + buf_size <=
...
static void receive_bytes_from_peer(tsi_test_channel* test_channel,
  ...
  uint8_t* channel = is_client ? test_channel->client_channel
                               : test_channel->server_channel;
  GPR_ASSERT(channel != nullptr);
  size_t* bytes_read =
      is_client ? &test_channel->bytes_read_from_client_channel
                : &test_channel->bytes_read_from_server_channel;
  size_t* bytes_written =
      is_client ? &test_channel->bytes_written_to_client_channel
...
void tsi_test_frame_protector_send_message_to_peer(
  ...
  unsigned char* protected_buffer = static_cast<unsigned char*>(
      gpr_zalloc(config->protected_buffer_size));
  size_t message_size = is_client ? config->client_message_size
...
void tsi_test_frame_protector_receive_message_from_peer(
  ...
  while (!done && result == TSI_OK) {
    ...
    if (read_from_peer_size == 0) {
      ...
      receive_bytes_from_peer(channel, &read_buffer,
    ...
    do {
      ...
      result = tsi_frame_protector_unprotect(
          protector, read_buffer + read_offset, &processed_size,
...
grpc_error_handle on_handshake_next_done(
    tsi_result result, void* user_data,
  ...
  if (result != TSI_OK) {
    ...
    return grpc_set_tsi_error_result(
        GRPC_ERROR_CREATE_FROM_STATIC_STRING("Handshake failed"),
        result);
  }
  /* Update handshaker result. */
  if (handshaker_result != nullptr) {
    tsi_handshaker_result** result_to_write =
        args->is_client ? &fixture->client_result
                        : &fixture->server_result;
  ...
  if (bytes_to_send_size > 0) {
    send_bytes_to_peer(fixture->channel, bytes_to_send,
                       bytes_to_send_size, args->is_client);
...
static void on_handshake_next_done_wrapper(
  ...
  handshaker_args* args = static_cast<handshaker_args*>(user_data);
  args->error =
      on_handshake_next_done(result, user_data, bytes_to_send,
...
static void do_handshaker_next(handshaker_args* args) {
  ...
  GPR_ASSERT(args->fixture != nullptr);
  tsi_test_fixture* fixture = args->fixture;
  tsi_handshaker* handshaker = args->is_client
  ...
  do {
    ...
    result = tsi_handshaker_next(
        ...
        const_cast<const unsigned char**>(&bytes_to_send),
        &bytes_to_send_size, &handshaker_result,
...
static void tsi_test_do_ping_pong(
    tsi_test_frame_protector_config* config, tsi_test_channel* channel,
  ...
  unsigned char* server_received_message = static_cast<unsigned char*>(
  ...
  unsigned char* client_received_message = static_cast<unsigned char*>(
      gpr_zalloc(TSI_TEST_DEFAULT_CHANNEL_SIZE));
...
void tsi_test_do_round_trip(tsi_test_fixture* fixture) {
  ...
  tsi_test_do_ping_pong(config, fixture->channel,
                        client_frame_protector, server_frame_protector);
...
static unsigned char* generate_random_message(size_t size) {
  ...
  unsigned char* output = static_cast<unsigned char*>(
      gpr_zalloc(sizeof(unsigned char) * size));
  for (i = 0; i < size - 1; ++i) {
...
tsi_test_frame_protector_config* tsi_test_frame_protector_config_create(
  ...
  /* Set the value for client max_output_protected_frame_size.
     If it is 0, we pass NULL to
     tsi_handshaker_result_create_frame_protector(), which then uses
     default protected frame size for it. */
  config->client_max_output_protected_frame_size =
