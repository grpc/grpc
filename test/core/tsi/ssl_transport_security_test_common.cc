...
void ssl_tsi_test_fixture_init(tsi_test_fixture* fixture) {
  ...
  key_cert_lib->server_pem_key_cert_pairs[1].private_key =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "server1.key");
  key_cert_lib->server_pem_key_cert_pairs[1].cert_chain =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "server1.pem");
  key_cert_lib->bad_server_pem_key_cert_pairs[0].private_key =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "badserver.key");
  key_cert_lib->bad_server_pem_key_cert_pairs[0].cert_chain =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "badserver.pem");
  key_cert_lib->client_pem_key_cert_pair.private_key =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "client.key");
  ...
  key_cert_lib->bad_client_pem_key_cert_pair.private_key =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "badclient.key");
  key_cert_lib->bad_client_pem_key_cert_pair.cert_chain =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "badclient.pem");
  key_cert_lib->root_cert =
      ssl_tsi_test_load_file(SSL_TSI_TEST_CREDENTIALS_DIR, "ca.pem");
...
void ssl_tsi_test_fixture_cleanup(tsi_test_fixture* fixture) {
  ...
  /* Unreference others. */
  if (ssl_fixture->server_handshaker_factory != nullptr) {
    tsi_ssl_server_handshaker_factory_unref(
        ssl_fixture->server_handshaker_factory);
  }
  if (ssl_fixture->client_handshaker_factory != nullptr) {
