...
static void fake_test_setup_handshakers(tsi_test_fixture* fixture) {
  fixture->client_handshaker = tsi_create_fake_handshaker(true /* is_client. */);
  fixture->server_handshaker = tsi_create_fake_handshaker(false /* is_client. */);
}
...
static void validate_handshaker_peers(tsi_handshaker_result* result) {
  ...
  GPR_ASSERT(memcmp(property->value.data, TSI_FAKE_CERTIFICATE_TYPE, property->value.length) == 0);
  property = tsi_peer_get_property_by_name(&peer, TSI_SECURITY_LEVEL_PEER_PROPERTY);
  GPR_ASSERT(property != nullptr);
  GPR_ASSERT(memcmp(property->value.data, TSI_FAKE_SECURITY_LEVEL, property->value.length) == 0);
...
static const struct tsi_test_fixture_vtable vtable = {
    fake_test_setup_handshakers, fake_test_check_handshaker_peers, fake_test_destruct};
        ...
        for (size_t ind4 = 0; ind4 < size; ind4++) {
          for (size_t ind5 = 0; ind5 < size; ind5++) {
            ...
            tsi_test_frame_protector_config_set_buffer_size(
                fake_fixture->base.config, odd_sizes[ind1], odd_sizes[ind2], odd_sizes[ind3],
