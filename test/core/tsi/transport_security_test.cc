...
typedef struct {
  ...
  /* Comma separated list of certificate names to match against. Any
     occurrence of '#' will be replaced with a null character before
     processing. */
...
/* Largely inspired from:
   ...
   TODO(jboeuf) uncomment test cases as we fix
...
const cert_name_test_entry cert_name_test_entries[] = {
    ...
    {0, "www.foo.com", "www.foo.com#", nullptr,
     nullptr}, /* # = null char. */
    ...
    /* Common name must not be used if subject alternative name was
       provided. */
    ...
    /* IDN tests */
    {1, "xn--poema-9qae5a.com.br", "xn--poema-9qae5a.com.br", nullptr,
     nullptr},
    ...
    /* Partial wildcards are disallowed, though RFC 2818 rules allow
       them. That is, forms such as baz*.example.net, *baz.example.net,
       and b*z.example.net should NOT match domains. Instead, the
       wildcard must always be the left-most label, and only a single
       label. */
    ...
    /* Wildcards should not be valid for public registry controlled
       domains, and unknown/unrecognized domains, at least three domain
    ...
    {1, "www.xn--poema-9qae5a.com.br", "*.xn--poema-9qae5a.com.br",
     nullptr, nullptr},
    {1, "test.example.xn--mgbaam7a8h", "*.example.xn--mgbaam7a8h",
     nullptr, nullptr},
    /*
    ...
    {0, "173.194.195.139", "foo.example.com", nullptr,
     "8.8.8.8,8.8.4.4"},
    {1, "173.194.195.139", "foo.example.com", nullptr,
...
name_list* name_list_add(const char* n) {
  name_list* result =
      static_cast<name_list*>(gpr_malloc(sizeof(name_list)));
  result->name = n;
  result->next = nullptr;
  return result;
}
...
static tsi_peer peer_from_cert_name_test_entry(
  ...
  GPR_ASSERT(tsi_construct_peer(
                 1 + dns_entries.name_count + ip_entries.name_count,
                 &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY,
                 entry->common_name, &peer.properties[0]) == TSI_OK);
  i = 1;
  while (nl != nullptr) {
    char* processed = processed_name(nl->name);
    GPR_ASSERT(tsi_construct_string_peer_property(
                   TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY,
                   processed, strlen(nl->name),
...
std::string cert_name_test_entry_to_string(
    const cert_name_test_entry* entry) {
...
static void test_peer_matches_name(void) {
  ...
  for (i = 0; i < GPR_ARRAY_SIZE(cert_name_test_entries); i++) {
    ...
    if (result != entry->expected) {
      gpr_log(GPR_ERROR, "%s",
              cert_name_test_entry_to_string(entry).c_str());
...
static void test_result_strings(void) {
  ...
  for (i = 0; i < GPR_ARRAY_SIZE(results); i++) {
    GPR_ASSERT(strcmp(results[i].str,
                      tsi_result_to_string(results[i].res)) == 0);
  }
  GPR_ASSERT(strcmp("UNKNOWN", tsi_result_to_string((tsi_result)42)) ==
             0);
}

static void test_protector_invalid_args(void) {
  GPR_ASSERT(tsi_frame_protector_protect(nullptr, nullptr, nullptr,
                                         nullptr, nullptr) ==
             TSI_INVALID_ARGUMENT);
  GPR_ASSERT(tsi_frame_protector_protect_flush(nullptr, nullptr,
                                               nullptr, nullptr) ==
             TSI_INVALID_ARGUMENT);
  GPR_ASSERT(tsi_frame_protector_unprotect(nullptr, nullptr, nullptr,
                                           nullptr, nullptr) ==
             TSI_INVALID_ARGUMENT);
}

static void test_handshaker_invalid_args(void) {
  GPR_ASSERT(tsi_handshaker_get_result(nullptr) ==
             TSI_INVALID_ARGUMENT);
  GPR_ASSERT(tsi_handshaker_extract_peer(nullptr, nullptr) ==
             TSI_INVALID_ARGUMENT);
  GPR_ASSERT(tsi_handshaker_create_frame_protector(
...
static void test_handshaker_invalid_state(void) {
  ...
  GPR_ASSERT(tsi_handshaker_extract_peer(h, &peer) ==
