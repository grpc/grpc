...
absl::Status DoHandshake(SSL** out_client, SSL** out_server) {
  ...
  EVP_PKEY* client_key = PEM_read_bio_PrivateKey(client_key_bio, /*x=*/nullptr,
                                                 /*cb=*/nullptr, /*u=*/nullptr);
  ...
  EVP_PKEY* server_key = PEM_read_bio_PrivateKey(server_key_bio, /*x=*/nullptr,
                                                 /*cb=*/nullptr, /*u=*/nullptr);
...
void SslFrameProtectorUtilFunctionTestHelper(
    const std::size_t plaintext_size,
    const std::size_t expected_encrypted_bytes_size, SSL* ssl, BIO* bio,
    std::vector<uint8_t>& buffer, std::size_t& buffer_offset, SSL* peer_ssl,
    BIO* peer_bio, std::vector<uint8_t>& peer_buffer,
    std::size_t& peer_buffer_offset) {
  ...
  EXPECT_EQ(ssl_protector_protect_util(
  ...
  if (plaintext_size >= kMaxPlaintextBytesPerTlsRecord) {
    ...
    EXPECT_EQ(unprotected_bytes_size, plaintext_size);
    ...
    EXPECT_EQ(ssl_protector_protect_flush_util(
  ...
  EXPECT_EQ(protected_output_frames_size, expected_encrypted_bytes_size);
  ...
  EXPECT_EQ(ssl_protector_unprotect_util(
                protected_output_frames.data(), peer_ssl, peer_bio,
...
TEST_P(FlowTest, TestWorkFlows) {
  SslFrameProtectorUtilFunctionTestHelper(
      ...
      server_bio, server_buffer, server_buffer_offset);
  SslFrameProtectorUtilFunctionTestHelper(
      GetParam().plaintext_size, GetParam().expected_encrypted_bytes_size,
      server_ssl, server_bio, server_buffer, server_buffer_offset, client_ssl,
      client_bio, client_buffer, client_buffer_offset);
}
