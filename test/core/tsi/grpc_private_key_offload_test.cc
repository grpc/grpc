...
namespace {
constexpr absl::string_view kSpiffeTestCredsRelativeDir =
    "test/core/tsi/test_creds/spiffe_end2end/";
 ...
 protected:
  class SslOffloadTsiTestFixture {
   public:
    explicit SslOffloadTsiTestFixture(OffloadParty offload_party,
      ...
      if (offload_party_ == OffloadParty::kClient) {
        ...
        default_signer_ =
            std::make_shared<BoringSslPrivateKeySigner>(std::move(pkey));
      } else if (offload_party_ == OffloadParty::kServer) {
        auto pkey = LoadPrivateKeyFromString(server_key_);
        default_signer_ =
            std::make_shared<BoringSslPrivateKeySigner>(std::move(pkey));
      }
   ...
   private:
    ...
    std::vector<tsi_ssl_pem_key_cert_pair>
        client_pem_key_cert_pairs_with_custom_signer_;
    std::vector<tsi_ssl_pem_key_cert_pair>
        server_pem_key_cert_pairs_with_default_signer_;
    std::vector<tsi_ssl_pem_key_cert_pair>
        client_pem_key_cert_pairs_with_default_signer_;
    std::shared_ptr<PrivateKeySigner> default_signer_;
...
TEST_P(PrivateKeyOffloadTest, OffloadFailsWithBadSignature) {
