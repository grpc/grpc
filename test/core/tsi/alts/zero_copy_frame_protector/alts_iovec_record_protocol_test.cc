...
static void randomly_slice(uint8_t* input, size_t input_length,
  ...
  *output = static_cast<iovec_t*>(
      gpr_malloc(*output_length * sizeof(iovec_t)));
  for (size_t i = 0; i < *output_length - 1; i++) {
    size_t slice_length = gsec_test_bias_random_uint32(
...
alts_iovec_record_protocol_test_fixture_create(bool rekey,
  ...
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 crypter, overflow_size, /*is_client=*/true,
                 integrity_only,
  ...
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(
                 ...
                 rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 crypter, overflow_size, /*is_client=*/false,
                 integrity_only,
...
alts_iovec_record_protocol_test_var_create() {
  ...
  var->header_buf =
      static_cast<uint8_t*>(gpr_malloc(var->header_length));
  ...
  var->protected_buf =
      static_cast<uint8_t*>(gpr_malloc(protected_buf_length));
...
static void integrity_only_random_seal_unseal(
    alts_iovec_record_protocol* sender,
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    ...
    status = alts_iovec_record_protocol_integrity_only_unprotect(
        receiver, var->data_iovec, var->data_iovec_length,
        var->header_iovec, var->tag_iovec, nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
    /* Makes sure data buffer has not been modified during
     * seal/unseal.  */
    GPR_ASSERT(memcmp(var->data_buf, var->dup_buf, var->data_length) ==
               0);
...
static void integrity_only_empty_seal_unseal(
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    ...
    grpc_status_code status =
        alts_iovec_record_protocol_integrity_only_protect(
            sender, nullptr, 0, var->header_iovec, var->tag_iovec,
            nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
    status = alts_iovec_record_protocol_integrity_only_unprotect(
        receiver, nullptr, 0, var->header_iovec, var->tag_iovec,
        nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
    alts_iovec_record_protocol_test_var_destroy(var);
  }
...
static void integrity_only_corrupted_data(
  ...
  gpr_free(error_message);
  revert_back_alter(var->header_buf + kZeroCopyFrameLengthFieldSize,
                    offset);
  /* Alter data.  */
...
static void integrity_only_protect_input_check(
  ...
  grpc_status_code status =
      alts_iovec_record_protocol_integrity_only_protect(
          rp, var->data_iovec, var->data_iovec_length, header_iovec,
  ...
  status = alts_iovec_record_protocol_integrity_only_protect(
      rp, var->data_iovec, var->data_iovec_length, var->header_iovec,
      tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
      "Tag is nullptr."));
  gpr_free(error_message);
...
static void privacy_integrity_empty_seal_unseal(
  ...
  alts_iovec_record_protocol_test_var* var =
      alts_iovec_record_protocol_test_var_create();
  size_t empty_payload_frame_size =
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    ...
    iovec_t data_iovec = {protected_buf + var->header_length,
                          var->tag_length};
    /* Seals and then unseals empty payload.  */
    grpc_status_code status =
        alts_iovec_record_protocol_privacy_integrity_protect(
    ...
    status = alts_iovec_record_protocol_privacy_integrity_unprotect(
        receiver, header_iovec, &data_iovec, 1, unprotected_iovec,
        nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
  }
...
static void privacy_integrity_unsync_seal_unseal(
  ...
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      receiver, header_iovec, &protected_iovec, 1,
...
static void privacy_integrity_corrupted_data(
  ...
  /* Alter frame length field.  */
  size_t offset =
      alter_random_byte(header_buf, kZeroCopyFrameLengthFieldSize);
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      receiver, header_iovec, &protected_iovec, 1,
      var->unprotected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
...
static void privacy_integrity_unprotect_input_check(
  ...
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      rp, header_iovec, &protected_iovec, 1, unprotected_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
...
static void record_protocol_wrong_mode(
   ...
   */
  status = alts_iovec_record_protocol_integrity_only_protect(
      privacy_integrity_protect_rp, var->data_iovec,
      var->data_iovec_length, var->header_iovec, var->tag_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
   ...
   */
  status = alts_iovec_record_protocol_integrity_only_unprotect(
      privacy_integrity_unprotect_rp, var->data_iovec,
      var->data_iovec_length, var->header_iovec, var->tag_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_FAILED_PRECONDITION, error_message,
   ...
   */
  status = alts_iovec_record_protocol_privacy_integrity_protect(
      integrity_only_protect_rp, var->data_iovec,
      var->data_iovec_length, var->protected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_FAILED_PRECONDITION, error_message,
...
static void integrity_seal_privacy_unseal(
  ...
  memcpy(var->protected_buf + var->data_length, var->tag_buf,
         var->tag_length);
  iovec_t protected_iovec = {var->protected_buf,
                             var->data_length + var->tag_length};
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      privacy_integrity_receiver, var->header_iovec, &protected_iovec,
      1, var->unprotected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INTERNAL, error_message,
      "Frame decryption failed."));
  gpr_free(error_message);
