...
static void randomly_slice(uint8_t* input, size_t input_length, iovec_t** output,
  ...
  *output = static_cast<iovec_t*>(gpr_malloc(*output_length * sizeof(iovec_t)));
  for (size_t i = 0; i < *output_length - 1; i++) {
    size_t slice_length = gsec_test_bias_random_uint32(static_cast<uint32_t>(input_length));
...
static size_t alter_random_byte(uint8_t* buf, size_t buf_length) {
  ...
  uint32_t offset = gsec_test_bias_random_uint32(static_cast<uint32_t>(buf_length));
  (*(buf + offset))++;
  return offset;
}
...
static alts_iovec_record_protocol_test_fixture* alts_iovec_record_protocol_test_fixture_create(
    bool rekey, bool integrity_only) {
  ...
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 ...
                 /*is_protect=*/true, &fixture->client_protect, nullptr) == GRPC_STATUS_OK);
  /* Create client record protocol for unprotect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 crypter, overflow_size, /*is_client=*/true, integrity_only,
                 /*is_protect=*/false, &fixture->client_unprotect, nullptr) == GRPC_STATUS_OK);
  /* Create server record protocol for protect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 crypter, overflow_size, /*is_client=*/false, integrity_only,
                 /*is_protect=*/true, &fixture->server_protect, nullptr) == GRPC_STATUS_OK);
  /* Create server record protocol for unprotect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_iovec_record_protocol_create(
                 crypter, overflow_size, /*is_client=*/false, integrity_only,
                 /*is_protect=*/false, &fixture->server_unprotect, nullptr) == GRPC_STATUS_OK);
...
static alts_iovec_record_protocol_test_var* alts_iovec_record_protocol_test_var_create() {
  ...
  randomly_slice(var->data_buf, var->data_length, &var->data_iovec, &var->data_iovec_length);
  /* Sets protected iovec.  */
  size_t protected_buf_length = var->header_length + var->data_length + var->tag_length;
...
static void integrity_only_random_seal_unseal(alts_iovec_record_protocol* sender,
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    alts_iovec_record_protocol_test_var* var = alts_iovec_record_protocol_test_var_create();
    /* Seals and then unseals.  */
    grpc_status_code status = alts_iovec_record_protocol_integrity_only_protect(
        sender, var->data_iovec, var->data_iovec_length, var->header_iovec, var->tag_iovec,
        nullptr);
    ...
    status = alts_iovec_record_protocol_integrity_only_unprotect(
        receiver, var->data_iovec, var->data_iovec_length, var->header_iovec, var->tag_iovec,
        nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
...
static void integrity_only_unsync_seal_unseal(alts_iovec_record_protocol* sender,
  ...
  alts_iovec_record_protocol_test_var* var = alts_iovec_record_protocol_test_var_create();
  grpc_status_code status = alts_iovec_record_protocol_integrity_only_protect(
      sender, var->data_iovec, var->data_iovec_length, var->header_iovec, var->tag_iovec, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  alts_iovec_record_protocol_test_var_destroy(var);
  /* Seals again.  */
  ...
  status = alts_iovec_record_protocol_integrity_only_unprotect(
      receiver, var->data_iovec, var->data_iovec_length, var->header_iovec, var->tag_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INTERNAL, error_message,
...
static void integrity_only_corrupted_data(alts_iovec_record_protocol* sender,
  ...
  char* error_message = nullptr;
  size_t offset = alter_random_byte(var->header_buf, kZeroCopyFrameLengthFieldSize);
  status = alts_iovec_record_protocol_integrity_only_unprotect(
      receiver, var->data_iovec, var->data_iovec_length, var->header_iovec, var->tag_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INTERNAL, error_message,
...
static void integrity_only_protect_input_check(alts_iovec_record_protocol* rp) {
  ...
  grpc_status_code status = alts_iovec_record_protocol_integrity_only_protect(
      rp, var->data_iovec, var->data_iovec_length, header_iovec, var->tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Header is nullptr."));
  gpr_free(error_message);
  ...
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Header length is incorrect."));
  gpr_free(error_message);
  /* Tag buffer is nullptr.  */
  iovec_t tag_iovec = {nullptr, var->tag_length};
  status = alts_iovec_record_protocol_integrity_only_protect(
      rp, var->data_iovec, var->data_iovec_length, var->header_iovec, tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Tag is nullptr."));
  gpr_free(error_message);
  ...
  status = alts_iovec_record_protocol_integrity_only_protect(
      rp, var->data_iovec, var->data_iovec_length, var->header_iovec, tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Tag length is incorrect."));
  gpr_free(error_message);
  alts_iovec_record_protocol_test_var_destroy(var);
}
...
static void privacy_integrity_random_seal_unseal(alts_iovec_record_protocol* sender,
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    alts_iovec_record_protocol_test_var* var = alts_iovec_record_protocol_test_var_create();
    /* Seals and then unseals.  */
    grpc_status_code status = alts_iovec_record_protocol_privacy_integrity_protect(
        sender, var->data_iovec, var->data_iovec_length, var->protected_iovec, nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
    iovec_t header_iovec = {var->protected_buf, var->header_length};
    gpr_free(var->data_iovec);
    /* Randomly slices protected buffer, excluding the header.  */
    randomly_slice(var->protected_buf + var->header_length, var->data_length + var->tag_length,
                   &var->data_iovec, &var->data_iovec_length);
    status = alts_iovec_record_protocol_privacy_integrity_unprotect(
        receiver, header_iovec, var->data_iovec, var->data_iovec_length, var->unprotected_iovec,
        nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
...
static void privacy_integrity_empty_seal_unseal(alts_iovec_record_protocol* sender,
  ...
  auto* protected_buf = static_cast<uint8_t*>(gpr_malloc(empty_payload_frame_size));
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
...
static void privacy_integrity_unsync_seal_unseal(alts_iovec_record_protocol* sender,
  ...
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      receiver, header_iovec, &protected_iovec, 1, var->unprotected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INTERNAL, error_message,
...
static void privacy_integrity_corrupted_data(alts_iovec_record_protocol* sender,
  ...
  /* Reverted protected data should be verified correctly.  */
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      receiver, header_iovec, &protected_iovec, 1, var->unprotected_iovec, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(var->data_buf, var->dup_buf, var->data_length) == 0);
  alts_iovec_record_protocol_test_var_destroy(var);
}
...
static void privacy_integrity_protect_input_check(alts_iovec_record_protocol* rp) {
  ...
  grpc_status_code status = alts_iovec_record_protocol_privacy_integrity_protect(
      rp, var->data_iovec, var->data_iovec_length, protected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Protected frame is nullptr."));
  gpr_free(error_message);
  ...
  status = alts_iovec_record_protocol_privacy_integrity_protect(
      rp, var->data_iovec, var->data_iovec_length, protected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Protected frame size is incorrect."));
  gpr_free(error_message);
  alts_iovec_record_protocol_test_var_destroy(var);
}
...
static void privacy_integrity_unprotect_input_check(alts_iovec_record_protocol* rp) {
  ...
  grpc_status_code status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      rp, header_iovec, &protected_iovec, 1, var->unprotected_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Header is nullptr."));
  gpr_free(error_message);
  ...
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Unprotected data size is incorrect."));
  gpr_free(error_message);
  alts_iovec_record_protocol_test_var_destroy(var);
}
...
static void record_protocol_wrong_mode(alts_iovec_record_protocol* integrity_only_protect_rp,
  ...
  status = alts_iovec_record_protocol_integrity_only_protect(
      privacy_integrity_protect_rp, var->data_iovec, var->data_iovec_length, var->header_iovec,
  ...
  status = alts_iovec_record_protocol_integrity_only_unprotect(
      privacy_integrity_unprotect_rp, var->data_iovec, var->data_iovec_length, var->header_iovec,
      var->tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_FAILED_PRECONDITION, error_message,
      "Integrity-only operations are not allowed for this object."));
  ...
  status = alts_iovec_record_protocol_privacy_integrity_protect(
      integrity_only_protect_rp, var->data_iovec, var->data_iovec_length, var->protected_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_FAILED_PRECONDITION, error_message,
  ...
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      integrity_only_unprotect_rp, var->header_iovec, var->data_iovec, var->data_iovec_length,
...
static void integrity_seal_privacy_unseal(alts_iovec_record_protocol* integrity_only_sender,
  ...
  status = alts_iovec_record_protocol_integrity_only_protect(
      integrity_only_sender, var->data_iovec, var->data_iovec_length, var->header_iovec,
  ...
  iovec_t protected_iovec = {var->protected_buf, var->data_length + var->tag_length};
  status = alts_iovec_record_protocol_privacy_integrity_unprotect(
      privacy_integrity_receiver, var->header_iovec, &protected_iovec, 1, var->unprotected_iovec,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INTERNAL, error_message,
                                                      "Frame decryption failed."));
  gpr_free(error_message);
...
static void privacy_seal_integrity_unseal(alts_iovec_record_protocol* privacy_integrity_sender,
  ...
  status = alts_iovec_record_protocol_privacy_integrity_protect(
      privacy_integrity_sender, var->data_iovec, var->data_iovec_length, var->protected_iovec,
      nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  /* Unseal with integrity-only unprotect.  */
  iovec_t header_iovec = {var->protected_buf, var->header_length};
  iovec_t data_iovec = {var->protected_buf + var->header_length, var->data_length};
  iovec_t tag_iovec = {var->protected_buf + var->header_length + var->data_length, var->tag_length};
  status = alts_iovec_record_protocol_integrity_only_unprotect(
      integrity_only_receiver, &data_iovec, 1, header_iovec, tag_iovec, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INTERNAL, error_message,
                                                      "Frame tag verification failed."));
  gpr_free(error_message);
  alts_iovec_record_protocol_test_var_destroy(var);
...
static void alts_iovec_record_protocol_random_seal_unseal_tests() {
  alts_iovec_record_protocol_test_fixture* fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/true);
  integrity_only_random_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  integrity_only_random_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  ...
  privacy_integrity_random_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  privacy_integrity_random_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
...
static void alts_iovec_record_protocol_empty_seal_unseal_tests() {
  alts_iovec_record_protocol_test_fixture* fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/true);
  integrity_only_empty_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  integrity_only_empty_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
  ...
  fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/false);
  privacy_integrity_empty_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  privacy_integrity_empty_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
...
static void alts_iovec_record_protocol_unsync_seal_unseal_tests() {
  alts_iovec_record_protocol_test_fixture* fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/true);
  integrity_only_unsync_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  integrity_only_unsync_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
  ...
  fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/false);
  privacy_integrity_unsync_seal_unseal(fixture->client_protect, fixture->server_unprotect);
  privacy_integrity_unsync_seal_unseal(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
...
static void alts_iovec_record_protocol_corrupted_data_tests() {
  alts_iovec_record_protocol_test_fixture* fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/true);
  integrity_only_corrupted_data(fixture->client_protect, fixture->server_unprotect);
  integrity_only_corrupted_data(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
  ...
  fixture = alts_iovec_record_protocol_test_fixture_create(
      /*rekey=*/false, /*integrity_only=*/false);
  privacy_integrity_corrupted_data(fixture->client_protect, fixture->server_unprotect);
  privacy_integrity_corrupted_data(fixture->server_protect, fixture->client_unprotect);
  alts_iovec_record_protocol_test_fixture_destroy(fixture);
...
static void alts_iovec_record_protocol_mix_operations_tests() {
  ...
  record_protocol_wrong_mode(fixture_1->client_protect, fixture_1->client_unprotect,
                             fixture_2->client_protect, fixture_2->client_unprotect);
  integrity_seal_privacy_unseal(fixture_1->client_protect, fixture_2->server_unprotect);
  privacy_seal_integrity_unseal(fixture_2->client_protect, fixture_1->server_unprotect);
