...
static void alter_random_byte(grpc_slice_buffer* sb) {
  ...
  uint32_t offset = gsec_test_bias_random_uint32(static_cast<uint32_t>(sb->length));
...
static alts_grpc_record_protocol_test_fixture* test_fixture_integrity_only_create(bool rekey,
  ...
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_grpc_integrity_only_record_protocol_create(crypter, 8, /*is_client=*/true,
                                                             /*is_protect=*/true, extra_copy,
                                                             &fixture->client_protect) == TSI_OK);
  /* Create client record protocol for unprotect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_grpc_integrity_only_record_protocol_create(crypter, 8, /*is_client=*/true,
                                                             /*is_protect=*/false, extra_copy,
                                                             &fixture->client_unprotect) == TSI_OK);
  /* Create server record protocol for protect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_grpc_integrity_only_record_protocol_create(crypter, 8, /*is_client=*/false,
                                                             /*is_protect=*/true, extra_copy,
...
static alts_grpc_record_protocol_test_fixture* test_fixture_privacy_integrity_create(bool rekey) {
  ...
  GPR_ASSERT(alts_grpc_privacy_integrity_record_protocol_create(
                 crypter, 8, /*is_client=*/true, /*is_protect=*/true, &fixture->client_protect) ==
             TSI_OK);
  /* Create client record protocol for unprotect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  ...
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
  GPR_ASSERT(alts_grpc_privacy_integrity_record_protocol_create(
                 crypter, 8, /*is_client=*/false, /*is_protect=*/true, &fixture->server_protect) ==
             TSI_OK);
  /* Create server record protocol for unprotect.  */
  GPR_ASSERT(gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength,
                                              rekey, &crypter, nullptr) == GRPC_STATUS_OK);
...
static alts_grpc_record_protocol_test_var* alts_grpc_record_protocol_test_var_create() {
  alts_grpc_record_protocol_test_var* var = static_cast<alts_grpc_record_protocol_test_var*>(
  ...
  for (size_t i = 0; i < var->original_sb.count; i++) {
    grpc_slice_buffer_add(&var->duplicate_sb, grpc_slice_ref(var->original_sb.slices[i]));
  }
  return var;
}
...
static void random_seal_unseal(alts_grpc_record_protocol* sender,
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    alts_grpc_record_protocol_test_var* var = alts_grpc_record_protocol_test_var_create();
    /* Seals and then unseals.  */
    size_t data_length = var->original_sb.length;
    tsi_result status =
        alts_grpc_record_protocol_protect(sender, &var->original_sb, &var->protected_sb);
    GPR_ASSERT(status == TSI_OK);
    GPR_ASSERT(var->protected_sb.length == data_length + var->header_length + var->tag_length);
    status =
        alts_grpc_record_protocol_unprotect(receiver, &var->protected_sb, &var->unprotected_sb);
    GPR_ASSERT(status == TSI_OK);
    GPR_ASSERT(are_slice_buffers_equal(&var->unprotected_sb, &var->duplicate_sb));
...
static void empty_seal_unseal(alts_grpc_record_protocol* sender,
  ...
  for (size_t i = 0; i < kSealRepeatTimes; i++) {
    ...
    tsi_result status =
        alts_grpc_record_protocol_protect(sender, &var->original_sb, &var->protected_sb);
    GPR_ASSERT(status == TSI_OK);
    GPR_ASSERT(var->protected_sb.length == var->header_length + var->tag_length);
    status =
        alts_grpc_record_protocol_unprotect(receiver, &var->protected_sb, &var->unprotected_sb);
    GPR_ASSERT(status == TSI_OK);
    GPR_ASSERT(are_slice_buffers_equal(&var->unprotected_sb, &var->duplicate_sb));
...
static void unsync_seal_unseal(alts_grpc_record_protocol* sender,
  ...
  alts_grpc_record_protocol_test_var* var = alts_grpc_record_protocol_test_var_create();
  /* Seals once.  */
  status = alts_grpc_record_protocol_protect(sender, &var->original_sb, &var->protected_sb);
  GPR_ASSERT(status == TSI_OK);
  grpc_slice_buffer_reset_and_unref_internal(&var->protected_sb);
  /* Seals again.  */
  status = alts_grpc_record_protocol_protect(sender, &var->duplicate_sb, &var->protected_sb);
  GPR_ASSERT(status == TSI_OK);
  /* Unseals the second frame.  */
  status = alts_grpc_record_protocol_unprotect(receiver, &var->protected_sb, &var->unprotected_sb);
...
static void input_check(alts_grpc_record_protocol* rp) {
  ...
  status = alts_grpc_record_protocol_protect(rp, &var->original_sb, &var->protected_sb);
  GPR_ASSERT(status == TSI_OK);
  status = alts_grpc_record_protocol_unprotect(rp, nullptr, &var->unprotected_sb);
  GPR_ASSERT(status == TSI_INVALID_ARGUMENT);
  ...
  grpc_slice_buffer_move_first(&var->protected_sb, var->header_length + var->tag_length - 1,
                               &temp_sb);
  status = alts_grpc_record_protocol_unprotect(rp, &temp_sb, &var->unprotected_sb);
  GPR_ASSERT(status == TSI_INVALID_ARGUMENT);
...
int main(int argc, char** argv) {
  ...
  grpc_init();
  alts_grpc_record_protocol_tests(&test_fixture_integrity_only_no_rekey_no_extra_copy_create);
  alts_grpc_record_protocol_tests(&test_fixture_integrity_only_rekey_create);
  alts_grpc_record_protocol_tests(&test_fixture_integrity_only_extra_copy_create);
