...
bool grpc_gcp_handshaker_resp_set_peer_rpc_versions(grpc_gcp_HandshakerResp* resp, upb_arena* arena,
  ...
  grpc_gcp_HandshakerResult* result = grpc_gcp_HandshakerResp_mutable_result(resp, arena);
  grpc_gcp_RpcProtocolVersions* upb_versions =
      grpc_gcp_HandshakerResult_mutable_peer_rpc_versions(result, arena);
  grpc_gcp_RpcProtocolVersions_assign_from_struct(upb_versions, arena, &versions);
  return true;
}
...
grpc_gcp_HandshakerReq* grpc_gcp_handshaker_req_decode(grpc_slice slice, upb_arena* arena) {
  ...
  grpc_gcp_HandshakerReq* resp =
      grpc_gcp_HandshakerReq_parse(reinterpret_cast<char*>(buf), buf_size, arena);
  if (!resp) {
...
static bool handshaker_identity_equals(const grpc_gcp_Identity* l_id,
  ...
  if ((grpc_gcp_Identity_has_service_account(l_id) !=
      ...
      (grpc_gcp_Identity_has_hostname(l_id) != grpc_gcp_Identity_has_hostname(r_id))) {
    return false;
  }
  ...
  if (grpc_gcp_Identity_has_service_account(l_id)) {
    ...
    if (!upb_strview_eql(grpc_gcp_Identity_hostname(l_id), grpc_gcp_Identity_hostname(r_id))) {
      return false;
    }
  }
  return true;
...
static bool handshaker_rpc_versions_equals(const grpc_gcp_RpcProtocolVersions* l_version,
...
bool grpc_gcp_handshaker_resp_equals(const grpc_gcp_HandshakerResp* l_resp,
  ...
  return upb_strview_eql(grpc_gcp_HandshakerResp_out_frames(l_resp),
         ...
         (grpc_gcp_HandshakerResp_bytes_consumed(l_resp) ==
          grpc_gcp_HandshakerResp_bytes_consumed(l_resp)) &&
         grpc_gcp_handshaker_resp_result_equals(grpc_gcp_HandshakerResp_result(l_resp),
                                                grpc_gcp_HandshakerResp_result(r_resp)) &&
         grpc_gcp_handshaker_resp_status_equals(grpc_gcp_HandshakerResp_status(l_resp),
...
bool grpc_gcp_handshaker_resp_result_equals(const grpc_gcp_HandshakerResult* l_result,
  ...
  return upb_strview_eql(grpc_gcp_HandshakerResult_application_protocol(l_result),
         ...
         handshaker_identity_equals(grpc_gcp_HandshakerResult_peer_identity(l_result),
                                    grpc_gcp_HandshakerResult_peer_identity(r_result)) &&
         handshaker_identity_equals(grpc_gcp_HandshakerResult_local_identity(l_result),
         ...
         handshaker_rpc_versions_equals(grpc_gcp_HandshakerResult_peer_rpc_versions(l_result),
...
bool grpc_gcp_handshaker_resp_status_equals(const grpc_gcp_HandshakerStatus* l_status,
  ...
  return (grpc_gcp_HandshakerStatus_code(l_status) == grpc_gcp_HandshakerStatus_code(r_status)) &&
