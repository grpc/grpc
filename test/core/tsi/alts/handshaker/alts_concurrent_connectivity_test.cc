...
grpc_channel* create_secure_channel_for_test(
  ...
  grpc_channel_credentials* channel_creds =
      grpc_alts_credentials_create_customized(
          alts_options, fake_handshake_server_addr,
          true /* enable_untrusted_alts */);
  grpc_alts_credentials_options_destroy(alts_options);
  // The main goal of these tests are to stress concurrent ALTS
  // handshakes, so we prevent subchnannel sharing.
  ...
  grpc_channel* channel = grpc_secure_channel_create(
      channel_creds, server_addr, channel_args, nullptr);
...
class FakeHandshakeServer {
 public:
  explicit FakeHandshakeServer(bool check_num_concurrent_rpcs) {
    ...
    // TODO(apolcyn): when removing the global concurrent handshake
    // limiting queue, set MAX_CONCURRENT_STREAMS on this server.
...
class TestServer {
 public:
  explicit TestServer()
    ...
    GPR_ASSERT(grpc_server_add_secure_http2_port(
        server_, server_addr_.c_str(), server_creds));
    grpc_server_credentials_release(server_creds);
    grpc_server_start(server_);
    gpr_log(GPR_DEBUG, "Start TestServer %p. listen on %s", this,
            server_addr_.c_str());
    server_thd_ =
        absl::make_unique<std::thread>(PollUntilShutdown, this);
  }
 ...
 private:
  ...
  // so that we avoid competing for ALTS handshake server resources
  // (e.g. available HTTP2 streams on a globally shared handshaker
  // subchannel) with clients that are trying to do mutual ALTS
  // handshakes with this server (which could "deadlock" mutual
  // handshakes).
...
class ConnectLoopRunner {
 public:
  explicit ConnectLoopRunner(
      ...
      int reconnect_backoff_ms)
      : server_address_(
            grpc_core::UniquePtr<char>(gpr_strdup(server_address))),
        fake_handshake_server_addr_(grpc_core::UniquePtr<char>(
            gpr_strdup(fake_handshake_server_addr))),
    ...
    for (size_t i = 0; i < self->loops_; i++) {
      ...
      while (state != self->expected_connectivity_states_) {
        if (self->expected_connectivity_states_ ==
          ...
          ASSERT_EQ(self->expected_connectivity_states_,
                    GRPC_CHANNEL_READY);
        }
        grpc_channel_watch_connectivity_state(
            channel, state, gpr_inf_future(GPR_CLOCK_REALTIME), cq,
            nullptr);
        grpc_event ev =
        ...
        if (self->expected_connectivity_states_ ==
          ...
          // Detect a race in state checking: if the
          // watch_connectivity_state completed from prior state
          // "connecting", this could be because the channel momentarily
          // entered state "transient failure", which is what we want.
          // However, if the channel immediately re-enters "connecting"
          // state, then the new state check might still result in
          // "connecting". A continuous repeat of this can cause this
          // loop to never terminate in time. So take this scenario to
          // indicate that the channel momentarily entered transient
          // failure.
          break;
        }
      }
      ...
      gpr_log(GPR_DEBUG, "runner:%p connect_loop finished loop %ld",
              self, i);
    }
  }
...
// Perform a few ALTS handshakes sequentially (using the fake,
// in-process ALTS handshake server).
...
TEST(AltsConcurrentConnectivityTest,
     TestConcurrentClientServerHandshakes) {
  FakeHandshakeServer fake_handshake_server(
      true /* check num concurrent rpcs */);
  // Test
  {
    ...
    std::vector<std::unique_ptr<ConnectLoopRunner>>
        connect_loop_runners;
    gpr_log(GPR_DEBUG,
    ...
    for (size_t i = 0; i < num_concurrent_connects; i++) {
      connect_loop_runners.push_back(
          absl::make_unique<ConnectLoopRunner>(
              test_server.address(), fake_handshake_server.address(),
 ...
 public:
  ...
  enum class AcceptMode {
    kWaitForClientToSendFirstBytes,  // useful for emulating ALTS based
                                     // grpc servers
    kEagerlySendSettings,  // useful for emulating insecure grpc servers
                           // (e.g. ALTS handshake servers)
  };

  explicit FakeTcpServer(
      AcceptMode accept_mode,
      const std::function<ProcessReadResult(int, int, int)>&
          process_read_cb)
    ...
    if (fcntl(accept_socket_, F_SETFL, O_NONBLOCK) != 0) {
      gpr_log(GPR_ERROR, "Failed to set O_NONBLOCK on socket: %d",
              errno);
      abort();
    }
    sockaddr_in6 addr;
    ...
    if (bind(accept_socket_, reinterpret_cast<const sockaddr*>(&addr),
             sizeof(addr)) != 0) {
      gpr_log(GPR_ERROR, "Failed to bind socket to [::1]:%d : %d",
              port_, errno);
      abort();
    }
    if (listen(accept_socket_, 100)) {
      gpr_log(GPR_ERROR,
              "Failed to listen on socket bound to [::1]:%d : %d",
              port_, errno);
      abort();
    }
    gpr_event_init(&stop_ev_);
    run_server_loop_thd_ =
        absl::make_unique<std::thread>(RunServerLoop, this);
  }
  ...
  static ProcessReadResult CloseSocketUponReceivingBytesFromPeer(
    ...
    if (bytes_received_size < 0 && read_error != EAGAIN &&
      ...
      gpr_log(GPR_ERROR,
              "Failed to receive from peer socket: %d. errno: %d", s,
              errno);
      abort();
    }
    if (bytes_received_size >= 0) {
      gpr_log(GPR_DEBUG,
              "Fake TCP server received %d bytes from peer socket: %d. "
              "Close "
              "the "
              "connection.",
  ...
  static ProcessReadResult CloseSocketUponCloseFromPeer(
      int bytes_received_size, int read_error, int s) {
    if (bytes_received_size < 0 && read_error != EAGAIN &&
        read_error != EWOULDBLOCK) {
      gpr_log(GPR_ERROR,
    ...
    if (bytes_received_size == 0) {
      ...
      gpr_log(GPR_DEBUG,
              "Fake TCP server received 0 bytes from peer socket: %d. "
              "Close "
              "the "
              "connection.",
              s);
   ...
   public:
    ...
    void MaybeContinueSendingSettings() {
      ...
      if (total_bytes_sent_ < int(kEmptyHttp2SettingsFrame.size())) {
        int bytes_to_send =
            kEmptyHttp2SettingsFrame.size() - total_bytes_sent_;
        int bytes_sent = send(
            fd_, kEmptyHttp2SettingsFrame.data() + total_bytes_sent_,
            bytes_to_send, 0);
  ...
  //      and reads from them if so, processing the sockets as
  //      configured.
    ...
    while (!gpr_event_get(&self->stop_ev_)) {
      ...
      if (p != -1) {
        ...
        if (fcntl(p, F_SETFL, O_NONBLOCK) != 0) {
          gpr_log(GPR_ERROR,
                  "Failed to set O_NONBLOCK on peer socket:%d errno:%d",
                  p, errno);
          abort();
        }
      ...
      while (it != peers.end()) {
        ...
        ProcessReadResult r = self->process_read_cb_(
            bytes_received_size, errno, peer->fd());
        if (r == CLOSE_SOCKET) {
          it = peers.erase(it);
        } else {
 ...
 * from the remote peer, after having earlier sent the first bytes of
 * the ALTS handshake to the peer, i.e. after getting into the middle of
 * a handshake. */
TEST(
  ...
  // getting cancelled. Because there isn't explicit synchronization
  // between an ALTS handshake client's RECV_STATUS op completing after
  // call cancellation, and the corresponding fake handshake server's
  // sync method handler returning, enforcing a limit on the number of
  // active RPCs at the fake handshake server would be inherently racey.
  FakeHandshakeServer fake_handshake_server(
      false /* check num concurrent rpcs */);
  // The fake_backend_server emulates a secure (ALTS based) gRPC
  // backend. So it waits for the client to send the first bytes.
      ...
      FakeTcpServer::CloseSocketUponReceivingBytesFromPeer);
  {
    ...
    for (size_t i = 0; i < num_concurrent_connects; i++) {
      connect_loop_runners.push_back(
          absl::make_unique<ConnectLoopRunner>(
              fake_backend_server.address(),
              fake_handshake_server.address(),
              10 /* per connect deadline seconds */, 3 /* loops */,
              GRPC_CHANNEL_TRANSIENT_FAILURE /* expected connectivity
                                                states */
              ,
              0 /* reconnect_backoff_ms unset */));
    }
    ...
    if (gpr_time_cmp(gpr_now(GPR_CLOCK_MONOTONIC), test_deadline) > 0) {
      gpr_log(
          GPR_ERROR,
          "Exceeded test deadline. ALTS handshakes might not be "
          "failing "
 ...
 * fail fast when the ALTS handshake server fails incoming handshakes
 * fast. */
TEST(
    AltsConcurrentConnectivityTest,
  ...
  FakeTcpServer fake_handshake_server(
      ...
      FakeTcpServer::CloseSocketUponReceivingBytesFromPeer);
  // The fake_backend_server emulates a secure (ALTS based) server, so
  // wait for the client to send the first bytes.
  FakeTcpServer fake_backend_server(
      FakeTcpServer::AcceptMode::kWaitForClientToSendFirstBytes,
...
/* This test is intended to make sure that ALTS handshakes correctly
 * fail fast when the ALTS handshake server is non-responsive, in which
 * case the overall connection deadline kicks in. */
TEST(AltsConcurrentConnectivityTest,
     TestHandshakeFailsFastWhenHandshakeServerHangsAfterAccepting) {
  // fake_handshake_server emulates an insecure server, so send settings
  // first. It will be unresponsive for the rest of the connection,
  // though.
  FakeTcpServer fake_handshake_server(
      FakeTcpServer::AcceptMode::kEagerlySendSettings,
      FakeTcpServer::CloseSocketUponCloseFromPeer);
  // fake_backend_server emulates an ALTS based server, so wait for the
  // client to send the first bytes.
  FakeTcpServer fake_backend_server(
      FakeTcpServer::AcceptMode::kWaitForClientToSendFirstBytes,
      FakeTcpServer::CloseSocketUponCloseFromPeer);
  {
    ...
    if (gpr_time_cmp(gpr_now(GPR_CLOCK_MONOTONIC), test_deadline) > 0) {
      gpr_log(GPR_ERROR,
              "Exceeded test deadline. ALTS handshakes might not be "
              "failing "
              "fast when the handshake server is non-response timeout "
              "occurs");
      abort();
    }
  }
