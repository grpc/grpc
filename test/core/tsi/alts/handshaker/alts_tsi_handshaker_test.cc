...
using grpc_core::internal::alts_handshaker_client_get_recv_buffer_addr_for_testing;
using grpc_core::internal::alts_handshaker_client_on_status_received_for_testing;
...
static grpc_byte_buffer* generate_handshaker_response(alts_handshaker_response_type type) {
  ...
  switch (type) {
    ...
    case CLIENT_START:
    case SERVER_START:
      grpc_gcp_HandshakerResp_set_out_frames(resp,
                                             upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_OUT_FRAME));
      break;
    case CLIENT_NEXT:
      grpc_gcp_HandshakerResp_set_out_frames(resp,
                                             upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_OUT_FRAME));
      grpc_gcp_HandshakerResp_set_bytes_consumed(resp,
                                                 strlen(ALTS_TSI_HANDSHAKER_TEST_CONSUMED_BYTES));
      result = grpc_gcp_HandshakerResp_mutable_result(resp, arena.ptr());
      peer_identity = grpc_gcp_HandshakerResult_mutable_peer_identity(result, arena.ptr());
      grpc_gcp_Identity_attributes_set(
          peer_identity, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_ATTRIBUTES_KEY),
          upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_ATTRIBUTES_VALUE), arena.ptr());
      grpc_gcp_Identity_set_service_account(
          peer_identity, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_IDENTITY));
      grpc_gcp_HandshakerResult_set_key_data(result,
                                             upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_KEY_DATA));
      ...
      local_identity = grpc_gcp_HandshakerResult_mutable_local_identity(result, arena.ptr());
      grpc_gcp_Identity_set_service_account(
          local_identity, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_LOCAL_IDENTITY));
      ...
      break;
    case SERVER_NEXT:
      grpc_gcp_HandshakerResp_set_bytes_consumed(resp, strlen(ALTS_TSI_HANDSHAKER_TEST_OUT_FRAME));
      result = grpc_gcp_HandshakerResp_mutable_result(resp, arena.ptr());
      peer_identity = grpc_gcp_HandshakerResult_mutable_peer_identity(result, arena.ptr());
      grpc_gcp_Identity_attributes_set(
          peer_identity, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_ATTRIBUTES_KEY),
...
static void on_client_start_success_cb(tsi_result status, void* user_data,
  ...
  GPR_ASSERT(memcmp(bytes_to_send, ALTS_TSI_HANDSHAKER_TEST_OUT_FRAME, bytes_to_send_size) == 0);
  GPR_ASSERT(result == nullptr);
  /* Validate peer identity. */
  tsi_peer peer;
  GPR_ASSERT(tsi_handshaker_result_extract_peer(result, &peer) == TSI_INVALID_ARGUMENT);
  /* Validate frame protector. */
  tsi_frame_protector* protector = nullptr;
  GPR_ASSERT(tsi_handshaker_result_create_frame_protector(result, nullptr, &protector) ==
             TSI_INVALID_ARGUMENT);
  /* Validate unused bytes. */
  const unsigned char* unused_bytes = nullptr;
  size_t unused_bytes_size = 0;
  GPR_ASSERT(tsi_handshaker_result_get_unused_bytes(result, &unused_bytes, &unused_bytes_size) ==
             TSI_INVALID_ARGUMENT);
...
static void on_client_next_success_cb(tsi_result status, void* user_data,
  ...
  GPR_ASSERT(tsi_handshaker_result_create_zero_copy_grpc_protector(result, nullptr,
                                                                   &zero_copy_protector) == TSI_OK);
  size_t actual_max_frame_size;
  tsi_zero_copy_grpc_protector_max_frame_size(zero_copy_protector, &actual_max_frame_size);
  ...
  GPR_ASSERT(memcmp(ALTS_TSI_HANDSHAKER_TEST_PEER_IDENTITY, peer.properties[1].value.data,
                    peer.properties[1].value.length) == 0);
  /* Validate alts context. */
  upb::Arena context_arena;
  grpc_gcp_AltsContext* ctx = grpc_gcp_AltsContext_parse(
      peer.properties[3].value.data, peer.properties[3].value.length, context_arena.ptr());
  ...
  GPR_ASSERT(memcmp(ALTS_TSI_HANDSHAKER_TEST_APPLICATION_PROTOCOL, application_protocol.data,
                    application_protocol.size) == 0);
  GPR_ASSERT(memcmp(ALTS_TSI_HANDSHAKER_TEST_RECORD_PROTOCOL, record_protocol.data,
                    record_protocol.size) == 0);
  GPR_ASSERT(memcmp(ALTS_TSI_HANDSHAKER_TEST_PEER_IDENTITY, peer_account.data, peer_account.size) ==
             0);
  GPR_ASSERT(
      memcmp(ALTS_TSI_HANDSHAKER_TEST_LOCAL_IDENTITY, local_account.data, local_account.size) == 0);
  ...
  while (peer_attributes_entry != nullptr) {
    upb_strview key = grpc_gcp_AltsContext_PeerAttributesEntry_key(
        const_cast<grpc_gcp_AltsContext_PeerAttributesEntry*>(peer_attributes_entry));
    upb_strview val = grpc_gcp_AltsContext_PeerAttributesEntry_value(
        const_cast<grpc_gcp_AltsContext_PeerAttributesEntry*>(peer_attributes_entry));
    GPR_ASSERT(
        upb_strview_eql(key, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_ATTRIBUTES_KEY)));
    GPR_ASSERT(
        upb_strview_eql(val, upb_strview_makez(ALTS_TSI_HANDSHAKER_TEST_PEER_ATTRIBUTES_VALUE)));
    peer_attributes_entry = grpc_gcp_AltsContext_peer_attributes_nextmutable(ctx, &iter);
  }
  /* Validate security level. */
  GPR_ASSERT(memcmp(ALTS_TSI_HANDSHAKER_TEST_SECURITY_LEVEL, peer.properties[4].value.data,
                    peer.properties[4].value.length) == 0);
  tsi_peer_destruct(&peer);
  /* Validate unused bytes. */
  const unsigned char* bytes = nullptr;
  size_t bytes_size = 0;
  GPR_ASSERT(tsi_handshaker_result_get_unused_bytes(result, &bytes, &bytes_size) == TSI_OK);
  GPR_ASSERT(bytes_size == strlen(ALTS_TSI_HANDSHAKER_TEST_REMAIN_BYTES));
  GPR_ASSERT(memcmp(bytes, ALTS_TSI_HANDSHAKER_TEST_REMAIN_BYTES, bytes_size) == 0);
  /* Validate frame protector. */
  tsi_frame_protector* protector = nullptr;
  GPR_ASSERT(tsi_handshaker_result_create_frame_protector(result, nullptr, &protector) == TSI_OK);
...
static void on_server_next_success_cb(tsi_result status, void* user_data,
  ...
  // max frame size.
  tsi_zero_copy_grpc_protector* zero_copy_protector = nullptr;
  size_t user_specified_max_frame_size = ALTS_TSI_HANDSHAKER_TEST_MAX_FRAME_SIZE;
  GPR_ASSERT(tsi_handshaker_result_create_zero_copy_grpc_protector(
                 result, &user_specified_max_frame_size, &zero_copy_protector) == TSI_OK);
  size_t actual_max_frame_size;
  tsi_zero_copy_grpc_protector_max_frame_size(zero_copy_protector, &actual_max_frame_size);
...
static tsi_result mock_client_start(alts_handshaker_client* client) {
  ...
  alts_handshaker_client_check_fields_for_testing(client, on_client_start_success_cb, nullptr, true,
                                                  nullptr);
...
static tsi_result mock_server_start(alts_handshaker_client* client, grpc_slice* bytes_received) {
  ...
  alts_handshaker_client_check_fields_for_testing(client, on_server_start_success_cb, nullptr, true,
                                                  nullptr);
...
static tsi_result mock_next(alts_handshaker_client* client, grpc_slice* bytes_received) {
  ...
  alts_tsi_handshaker* handshaker = alts_handshaker_client_get_handshaker_for_testing(client);
  ...
  alts_handshaker_client_check_fields_for_testing(client, cb, nullptr, true, bytes_received);
  GPR_ASSERT(bytes_received != nullptr);
  GPR_ASSERT(memcmp(GRPC_SLICE_START_PTR(*bytes_received), ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES,
                    GRPC_SLICE_LENGTH(*bytes_received)) == 0);
  /* Populate handshaker response for next request. */
  grpc_slice out_frame = grpc_slice_from_static_string(ALTS_TSI_HANDSHAKER_TEST_OUT_FRAME);
  grpc_byte_buffer** recv_buffer_ptr =
      alts_handshaker_client_get_recv_buffer_addr_for_testing(client);
...
static alts_handshaker_client_vtable vtable = {mock_client_start, mock_server_start, mock_next,
...
static tsi_handshaker* create_test_handshaker(bool is_client) {
  ...
  alts_tsi_handshaker_create(options, "target_name", ALTS_HANDSHAKER_SERVICE_URL_FOR_TESTING,
                             is_client, nullptr, &handshaker, 0);
  alts_tsi_handshaker* alts_handshaker = reinterpret_cast<alts_tsi_handshaker*>(handshaker);
...
static void check_handshaker_next_invalid_input() {
  ...
  GPR_ASSERT(tsi_handshaker_next(nullptr, nullptr, 0, nullptr, nullptr, nullptr,
                                 check_must_not_be_called, nullptr) == TSI_INVALID_ARGUMENT);
  /* Check nullptr callback. */
  GPR_ASSERT(tsi_handshaker_next(handshaker, nullptr, 0, nullptr, nullptr, nullptr, nullptr,
...
static void check_handshaker_next_success() {
   ...
   * correctness check.
   */
  tsi_handshaker* client_handshaker = create_test_handshaker(true /* is_client */);
  tsi_handshaker* server_handshaker = create_test_handshaker(false /* is_client */);
  /* Client start. */
  GPR_ASSERT(tsi_handshaker_next(client_handshaker, nullptr, 0, nullptr, nullptr, nullptr,
                                 on_client_start_success_cb, nullptr) == TSI_ASYNC);
  ...
  wait(&tsi_to_caller_notification);
  /* Server start. */
  GPR_ASSERT(tsi_handshaker_next(server_handshaker, nullptr, 0, nullptr, nullptr, nullptr,
                                 on_server_start_success_cb, nullptr) == TSI_ASYNC);
  wait(&tsi_to_caller_notification);
  /* Server next. */
...
static void check_handshaker_next_with_shutdown() {
  ...
  GPR_ASSERT(tsi_handshaker_next(handshaker, nullptr, 0, nullptr, nullptr, nullptr,
                                 on_client_start_success_cb, nullptr) == TSI_ASYNC);
  wait(&tsi_to_caller_notification);
  ...
  GPR_ASSERT(
      tsi_handshaker_next(handshaker, (const unsigned char*)ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES,
                          ...
                          on_client_next_success_cb, nullptr) == TSI_HANDSHAKE_SHUTDOWN);
  /* Cleanup. */
  run_tsi_handshaker_destroy_with_exec_ctx(handshaker);
}
...
static void check_handshaker_next_failure() {
  ...
  /* Client start. */
  GPR_ASSERT(tsi_handshaker_next(client_handshaker, nullptr, 0, nullptr, nullptr, nullptr,
                                 check_must_not_be_called, nullptr) == TSI_INTERNAL_ERROR);
  /* Server start. */
  GPR_ASSERT(tsi_handshaker_next(server_handshaker, nullptr, 0, nullptr, nullptr, nullptr,
                                 check_must_not_be_called, nullptr) == TSI_INTERNAL_ERROR);
  /* Server next. */
  GPR_ASSERT(tsi_handshaker_next(server_handshaker,
                                 (const unsigned char*)ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES,
                                 strlen(ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES), nullptr, nullptr,
                                 nullptr, check_must_not_be_called, nullptr) == TSI_INTERNAL_ERROR);
  /* Client next. */
  GPR_ASSERT(tsi_handshaker_next(client_handshaker,
                                 (const unsigned char*)ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES,
                                 strlen(ALTS_TSI_HANDSHAKER_TEST_RECV_BYTES), nullptr, nullptr,
...
static void check_handle_response_nullptr_handshaker() {
   ...
   */
  tsi_handshaker* handshaker = create_test_handshaker(true /* is_client */);
  tsi_handshaker_next(handshaker, nullptr, 0, nullptr, nullptr, nullptr, on_client_start_success_cb,
                      nullptr);
  alts_tsi_handshaker* alts_handshaker = reinterpret_cast<alts_tsi_handshaker*>(handshaker);
  grpc_slice slice = grpc_empty_slice();
  grpc_byte_buffer* recv_buffer = grpc_raw_byte_buffer_create(&slice, 1);
  alts_handshaker_client* client = alts_tsi_handshaker_get_client_for_testing(alts_handshaker);
  /* Check nullptr handshaker. */
  alts_handshaker_client_set_fields_for_testing(client, nullptr, on_invalid_input_cb, nullptr,
  ...
  {
    grpc_core::ExecCtx exec_ctx;
    alts_handshaker_client_on_status_received_for_testing(client, GRPC_STATUS_OK, GRPC_ERROR_NONE);
  }
  /* Cleanup. */
  grpc_slice_unref(slice);
  run_tsi_handshaker_destroy_with_exec_ctx(handshaker);
...
static void check_handle_response_nullptr_recv_bytes() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_invalid_input_cb,
                                                nullptr, nullptr, GRPC_STATUS_OK);
  alts_handshaker_client_handle_response(client, true);
  alts_handshaker_client_ref_for_testing(client);
  {
    grpc_core::ExecCtx exec_ctx;
...
static void check_handle_response_failed_grpc_call_to_handshaker_service() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_failed_grpc_call_cb,
                                                nullptr, recv_buffer, GRPC_STATUS_UNKNOWN);
  alts_handshaker_client_handle_response(client, true);
  alts_handshaker_client_ref_for_testing(client);
  {
    grpc_core::ExecCtx exec_ctx;
    alts_handshaker_client_on_status_received_for_testing(client, GRPC_STATUS_UNKNOWN,
                                                          GRPC_ERROR_NONE);
  }
  /* Cleanup. */
  grpc_slice_unref(slice);
...
static void check_handle_response_failed_recv_message_from_handshaker_service() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_failed_grpc_call_cb,
                                                nullptr, recv_buffer, GRPC_STATUS_OK);
...
static void check_handle_response_invalid_resp() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_invalid_resp_cb,
                                                nullptr, recv_buffer, GRPC_STATUS_OK);
  alts_handshaker_client_handle_response(client, true);
  alts_handshaker_client_ref_for_testing(client);
  {
...
static void check_handle_response_success(void* /*unused*/) {
  ...
  {
    grpc_core::ExecCtx exec_ctx;
    alts_handshaker_client_on_status_received_for_testing(cb_event, GRPC_STATUS_OK,
                                                          GRPC_ERROR_NONE);
  }
  /* Server start. */
  wait(&caller_to_tsi_notification);
...
static void check_handle_response_failure() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_failed_resp_cb, nullptr,
                                                recv_buffer, GRPC_STATUS_OK);
...
static void check_handle_response_after_shutdown() {
  ...
  alts_handshaker_client_set_fields_for_testing(client, alts_handshaker, on_shutdown_resp_cb,
                                                nullptr, recv_buffer, GRPC_STATUS_OK);
  alts_handshaker_client_handle_response(client, true);
  alts_handshaker_client_ref_for_testing(client);
  {
...
void check_handshaker_next_fails_after_shutdown() {
  ...
  cb_event = nullptr;
  /* Tests. */
  grpc_core::Thread thd("alts_tsi_handshaker_test", &check_handle_response_with_shutdown, nullptr);
  thd.Start();
...
void check_handshaker_success() {
  ...
  notification_init(&tsi_to_caller_notification);
  /* Tests. */
  grpc_core::Thread thd("alts_tsi_handshaker_test", &check_handle_response_success, nullptr);
  thd.Start();
