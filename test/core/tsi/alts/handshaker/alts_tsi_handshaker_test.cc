...
TEST(AltsTsiHandshakerTest, SetUnusedBytesConsumedExceedsRecvBytes) {
  // A malformed or malicious handshaker service response may report consuming
  // more bytes than were actually received. bytes_consumed larger than the
  // recv_bytes length must not underflow the size_t computation of the
  // unused-bytes size (which would lead to a huge allocation / OOB read).
  upb::Arena arena;
  grpc_gcp_HandshakerResp* resp = grpc_gcp_HandshakerResp_new(arena.ptr());
  ...
  // Report one more byte consumed than is available in recv_bytes.
