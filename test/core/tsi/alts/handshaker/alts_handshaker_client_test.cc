...
/**
 * Validate if grpc operation data is correctly populated with the
 * fields of ALTS handshaker client.
 */
...
static grpc_gcp_HandshakerReq* deserialize_handshaker_req(
  ...
  grpc_gcp_HandshakerReq* req =
      grpc_gcp_handshaker_req_decode(slice, arena);
...
/**
 * A mock grpc_caller used to check if client_start, server_start, and
 * next operations correctly handle invalid arguments. It should not be
 * called.
 */
 ...
 * operation. It checks if the client_start handshaker request is
 * populated with correct handshake_security_protocol,
 * application_protocol, record_protocol and max_frame_size, and op is
 * correctly populated.
 */
static grpc_call_error check_client_start_success(
  ...
  GPR_ASSERT(alts_handshaker_client_get_closure_for_testing(client) ==
             closure);
  ...
  upb_strview const* application_protocols =
      grpc_gcp_StartClientHandshakeReq_application_protocols(
          client_start, nullptr);
  GPR_ASSERT(
      upb_strview_eql(application_protocols[0],
  ...
  validate_target_identities(target_identities,
  ...
  GPR_ASSERT(
      grpc_gcp_StartClientHandshakeReq_max_frame_size(client_start) ==
 ...
 * operation. It checks if the server_start handshaker request is
 * populated with correct handshake_security_protocol,
 * application_protocol, record_protocol and max_frame_size, and op is
 * correctly populated.
 */
static grpc_call_error check_server_start_success(
    grpc_call* /*call*/, const grpc_op* op, size_t nops,
    grpc_closure* closure) {
  // RECV_STATUS ops are asserted to always succeed
  if (is_recv_status_op(op, nops)) {
  ...
  upb_strview const* record_protocols =
      grpc_gcp_ServerHandshakeParameters_record_protocols(value,
                                                          nullptr);
  GPR_ASSERT(upb_strview_eql(record_protocols[0],
                             upb_strview_makez(ALTS_RECORD_PROTOCOL)));
  ...
  GPR_ASSERT(
      grpc_gcp_StartServerHandshakeReq_max_frame_size(server_start) ==
      ALTS_HANDSHAKER_CLIENT_TEST_MAX_FRAME_SIZE);
  GPR_ASSERT(validate_op(client, op, nops, true /* is_start */));
  return GRPC_CALL_OK;
}
...
/**
 * A mock grpc_caller used to check correct execution of next operation.
 * It checks if the next handshaker request is populated with correct
 * information, and op is correctly populated.
 */
...
/**
 * A mock grpc_caller used to check if client_start, server_start, and
 * next operations correctly handle the situation when the grpc call
 * made to the handshaker service fails.
 */
...
static alts_handshaker_client_test_config* create_config() {
  ...
  config->out_frame = grpc_slice_from_static_string(
...
static void schedule_request_invalid_arg_test() {
  ...
  /* Tests. */
  alts_handshaker_client_set_grpc_caller_for_testing(
      config->client, check_must_not_be_called);
  /* Check client_start. */
  {
  ...
  }
  {
    grpc_core::ExecCtx exec_ctx;
    GPR_ASSERT(
        alts_handshaker_client_next(nullptr, &config->out_frame) ==
        TSI_INVALID_ARGUMENT);
  }
  /* Check shutdown. */
...
static void schedule_request_success_test() {
  ...
  alts_handshaker_client_set_grpc_caller_for_testing(
      config->client, check_next_success);
  {
    grpc_core::ExecCtx exec_ctx;
  ...
  alts_handshaker_client_set_grpc_caller_for_testing(
      config->server, check_next_success);
  {
    grpc_core::ExecCtx exec_ctx;
    GPR_ASSERT(alts_handshaker_client_next(
                   config->server, &config->out_frame) == TSI_OK);
...
static void schedule_request_grpc_call_failure_test() {
  ...
  alts_handshaker_client_set_grpc_caller_for_testing(
      config->client, check_grpc_call_failure);
  {
    grpc_core::ExecCtx exec_ctx;
    ...
    alts_handshaker_client_set_cb_for_testing(
        config->client, tsi_cb_assert_tsi_internal_error);
    alts_handshaker_client_start_client(config->client);
  }
  /* Check server_start failure. */
  alts_handshaker_client_set_grpc_caller_for_testing(
      config->server, check_grpc_call_failure);
  {
    grpc_core::ExecCtx exec_ctx;
    // TODO(apolcyn): go back to asserting TSI_INTERNAL_ERROR as return
    // value instead of callback status, after removing the global
    // queue in https://github.com/grpc/grpc/pull/20722
    alts_handshaker_client_set_cb_for_testing(
        config->server, tsi_cb_assert_tsi_internal_error);
    alts_handshaker_client_start_server(config->server,
                                        &config->out_frame);
  }
  {
    grpc_core::ExecCtx exec_ctx;
