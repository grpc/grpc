...
static void alts_crypter_test_random_seal_unseal(
  ...
  size_t num_overhead_bytes =
      alts_crypter_num_overhead_bytes(server_seal);
...
static void alts_crypter_test_multiple_random_seal_unseal(
  ...
  grpc_status_code status = alts_crypter_process_in_place(
      client_seal, data_buffer1, protected_data_size, size1, &size1,
      nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size1 == protected_data_size);
  status = alts_crypter_process_in_place(client_seal, data_buffer2,
                                         protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size2 == protected_data_size);
  status = alts_crypter_process_in_place(server_unseal, data_buffer1,
                                         protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer1, duplicate_buffer1, data_size) == 0);
  GPR_ASSERT(size1 == data_size);
  status = alts_crypter_process_in_place(server_unseal, data_buffer2,
                                         protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  ...
  /* Server seal and client unseal */
  status = alts_crypter_process_in_place(server_seal, data_buffer1,
                                         protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size1 == protected_data_size);
  status = alts_crypter_process_in_place(server_seal, data_buffer2,
                                         protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size2 == protected_data_size);
  status = alts_crypter_process_in_place(client_unseal, data_buffer1,
                                         protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer1, duplicate_buffer1, data_size) == 0);
  GPR_ASSERT(size1 == data_size);
  status = alts_crypter_process_in_place(client_unseal, data_buffer2,
                                         protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer2, duplicate_buffer2, data_size) == 0);
...
static void alts_crypter_test_corrupted_unseal(
  ...
  gsec_test_copy_and_alter_random_byte(
      data_buffer, &corrupted_data_buffer, protected_data_size);
  status = alts_crypter_process_in_place(
      server_unseal, corrupted_data_buffer, protected_data_size, size,
      &size, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_FAILED_PRECONDITION, error_message,
      "Checking tag failed"));
  GPR_ASSERT(memcmp(corrupted_data_buffer, zero_buffer, data_size) ==
             0);
  ...
  size = data_size;
  gsec_test_random_bytes(data_buffer, data_size);
  status = alts_crypter_process_in_place(client_seal, data_buffer,
                                         protected_data_size, size,
                                         &size, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size == protected_data_size);
  gsec_test_copy(data_buffer, &corrupted_data_buffer,
                 protected_data_size);
...
static void alts_crypter_test_unsync_seal_unseal(
    alts_crypter* server_seal, alts_crypter* server_unseal,
    alts_crypter* client_seal, alts_crypter* client_unseal) {
  size_t data_size = gsec_test_bias_random_uint32(1024) + 1;
  size_t num_overhead_bytes =
...
static void alts_crypter_test_input_sanity_check(
    alts_crypter* crypter_seal, alts_crypter* crypter_unseal) {
  size_t data_size = gsec_test_bias_random_uint32(1024) + 1;
  size_t num_overhead_bytes =
      alts_crypter_num_overhead_bytes(crypter_seal);
  size_t protected_data_size = data_size + num_overhead_bytes;
  auto* data_buffer =
      static_cast<uint8_t*>(gpr_malloc(protected_data_size));
  gsec_test_random_bytes(data_buffer, data_size);
  ...
  /* Crypter is nullptr. */
  grpc_status_code status = alts_crypter_process_in_place(
      nullptr, data_buffer, protected_data_size, size, &size,
      &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
  ...
  size = data_size;
  status = alts_crypter_process_in_place(crypter_seal, nullptr,
                                         protected_data_size, size,
                                         &size, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
...
static void create_random_alts_seal_crypter(
  ...
  alts_unseal_crypter_create(*server_crypter_unseal,
                             /*is_client=*/false, overflow_size,
                             server_unseal, nullptr);
  gpr_free(key);
}

static void destroy_random_alts_seal_crypter(
    alts_crypter* server_seal, alts_crypter* server_unseal,
    alts_crypter* client_seal, alts_crypter* client_unseal) {
...
static void alts_crypter_do_generic_tests() {
  ...
  create_random_alts_seal_crypter(
      &server_seal, &server_unseal, &client_seal, &client_unseal,
      &server_crypter_seal, &server_crypter_unseal,
      &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_random_seal_unseal(server_seal, server_unseal,
                                       client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal,
                                   client_seal, client_unseal);
  ...
  create_random_alts_seal_crypter(
      ...
      &server_crypter_seal, &server_crypter_unseal,
      &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_corrupted_unseal(server_seal, server_unseal,
                                     client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal,
                                   client_seal, client_unseal);
  ...
  create_random_alts_seal_crypter(
      ...
      &server_crypter_seal, &server_crypter_unseal,
      &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_unsync_seal_unseal(server_seal, server_unseal,
                                       client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal,
                                   client_seal, client_unseal);
