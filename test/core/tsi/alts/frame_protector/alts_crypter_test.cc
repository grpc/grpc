...
static void alts_crypter_test_random_seal_unseal(alts_crypter* server_seal,
  ...
  status = alts_crypter_process_in_place(server_unseal, data_buffer, protected_data_size, size,
                                         &size, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer, duplicate_buffer, data_size) == 0);
  GPR_ASSERT(size == data_size);
  /* Server seal and client unseal */
  status = alts_crypter_process_in_place(server_seal, data_buffer, protected_data_size, size, &size,
                                         nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size == protected_data_size);
  status = alts_crypter_process_in_place(client_unseal, data_buffer, protected_data_size, size,
                                         &size, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer, duplicate_buffer, data_size) == 0);
  GPR_ASSERT(size == data_size);
...
static void alts_crypter_test_multiple_random_seal_unseal(alts_crypter* server_seal,
  ...
  uint8_t* data_buffer1 = static_cast<uint8_t*>(gpr_malloc(protected_data_size));
  uint8_t* data_buffer2 = static_cast<uint8_t*>(gpr_malloc(protected_data_size));
  ...
  status = alts_crypter_process_in_place(client_seal, data_buffer2, protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size2 == protected_data_size);
  status = alts_crypter_process_in_place(server_unseal, data_buffer1, protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer1, duplicate_buffer1, data_size) == 0);
  GPR_ASSERT(size1 == data_size);
  status = alts_crypter_process_in_place(server_unseal, data_buffer2, protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  ...
  /* Server seal and client unseal */
  status = alts_crypter_process_in_place(server_seal, data_buffer1, protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size1 == protected_data_size);
  status = alts_crypter_process_in_place(server_seal, data_buffer2, protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size2 == protected_data_size);
  status = alts_crypter_process_in_place(client_unseal, data_buffer1, protected_data_size, size1,
                                         &size1, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer1, duplicate_buffer1, data_size) == 0);
  GPR_ASSERT(size1 == data_size);
  status = alts_crypter_process_in_place(client_unseal, data_buffer2, protected_data_size, size2,
                                         &size2, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(data_buffer2, duplicate_buffer2, data_size) == 0);
  GPR_ASSERT(size2 == data_size);
...
static void alts_crypter_test_corrupted_unseal(alts_crypter* server_seal,
  ...
  gsec_test_copy_and_alter_random_byte(data_buffer, &corrupted_data_buffer, protected_data_size);
  status = alts_crypter_process_in_place(server_unseal, corrupted_data_buffer, protected_data_size,
                                         size, &size, &error_message);
  ...
  size = data_size;
  gsec_test_random_bytes(data_buffer, data_size);
  status = alts_crypter_process_in_place(client_seal, data_buffer, protected_data_size, size, &size,
                                         nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(size == protected_data_size);
...
static void alts_crypter_test_input_sanity_check(alts_crypter* crypter_seal,
  ...
  /* Crypter is nullptr. */
  grpc_status_code status = alts_crypter_process_in_place(nullptr, data_buffer, protected_data_size,
                                                          size, &size, &error_message);
  ...
  size = data_size;
  status = alts_crypter_process_in_place(crypter_seal, nullptr, protected_data_size, size, &size,
                                         &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "data is nullptr."));
  gpr_free(error_message);

  /* Seal data size is 0. */
  size = 0;
  status = alts_crypter_process_in_place(crypter_seal, data_buffer, protected_data_size, size,
                                         &size, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "data_size is zero."));
  gpr_free(error_message);

  /* Seal data buffer has a size smaller than the required. */
  size = data_size;
  status = alts_crypter_process_in_place(crypter_seal, data_buffer, protected_data_size - 1, size,
                                         &size, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
  ...
  size = data_size;
  status = alts_crypter_process_in_place(crypter_unseal, nullptr, protected_data_size, size, &size,
                                         &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "data is nullptr."));
  gpr_free(error_message);
  ...
  size = 0;
  status = alts_crypter_process_in_place(crypter_unseal, data_buffer, protected_data_size, size,
                                         &size, &error_message);
  GPR_ASSERT(
      gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
...
static void create_random_alts_seal_crypter(
    ...
    alts_crypter** client_unseal, gsec_aead_crypter** server_crypter_seal,
    gsec_aead_crypter** server_crypter_unseal, gsec_aead_crypter** client_crypter_seal,
  ...
  gsec_aes_gcm_aead_crypter_create(key, key_length, kAesGcmNonceLength, kAesGcmTagLength, rekey,
  ...
  alts_seal_crypter_create(*client_crypter_seal, /*is_client=*/true, overflow_size, client_seal,
                           nullptr);
  alts_unseal_crypter_create(*client_crypter_unseal, /*is_client=*/true, overflow_size,
                             client_unseal, nullptr);
  alts_seal_crypter_create(*server_crypter_seal, /*is_client=*/false, overflow_size, server_seal,
                           nullptr);
  alts_unseal_crypter_create(*server_crypter_unseal, /*is_client=*/false, overflow_size,
                             server_unseal, nullptr);
  gpr_free(key);
}

static void destroy_random_alts_seal_crypter(alts_crypter* server_seal, alts_crypter* server_unseal,
                                             alts_crypter* client_seal,
                                             alts_crypter* client_unseal) {
...
static void alts_crypter_do_generic_tests() {
  alts_crypter *server_seal = nullptr, *server_unseal = nullptr, *client_seal = nullptr,
               *client_unseal = nullptr;
  gsec_aead_crypter *server_crypter_seal = nullptr, *server_crypter_unseal = nullptr,
                    *client_crypter_seal = nullptr, *client_crypter_unseal = nullptr;
  /* Random seal and unseal tests */
  create_random_alts_seal_crypter(&server_seal, &server_unseal, &client_seal, &client_unseal,
                                  &server_crypter_seal, &server_crypter_unseal,
                                  &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_random_seal_unseal(server_seal, server_unseal, client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal, client_seal, client_unseal);

  create_random_alts_seal_crypter(&server_seal, &server_unseal, &client_seal, &client_unseal,
                                  &server_crypter_seal, &server_crypter_unseal,
                                  &client_crypter_seal, &client_crypter_unseal, /*rekey=*/true);
  alts_crypter_test_random_seal_unseal(server_seal, server_unseal, client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal, client_seal, client_unseal);
  ...
  create_random_alts_seal_crypter(&server_seal, &server_unseal, &client_seal, &client_unseal,
                                  &server_crypter_seal, &server_crypter_unseal,
                                  &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_multiple_random_seal_unseal(server_seal, server_unseal, client_seal,
                                                client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal, client_seal, client_unseal);
  ...
  create_random_alts_seal_crypter(&server_seal, &server_unseal, &client_seal, &client_unseal,
                                  &server_crypter_seal, &server_crypter_unseal,
                                  &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_corrupted_unseal(server_seal, server_unseal, client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal, client_seal, client_unseal);
  ...
  create_random_alts_seal_crypter(&server_seal, &server_unseal, &client_seal, &client_unseal,
                                  &server_crypter_seal, &server_crypter_unseal,
                                  &client_crypter_seal, &client_crypter_unseal, /*rekey=*/false);
  alts_crypter_test_unsync_seal_unseal(server_seal, server_unseal, client_seal, client_unseal);
  destroy_random_alts_seal_crypter(server_seal, server_unseal, client_seal, client_unseal);
