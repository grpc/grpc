...
static bool do_bytes_represent_client(alts_counter* ctr, unsigned char* /*counter*/, size_t size) {
...
static void alts_counter_test_input_sanity_check(size_t counter_size, size_t overflow_size) {
  ...
  grpc_status_code status = alts_counter_create(true, 0, overflow_size, &ctr, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_details, "counter_size is invalid."));
  gpr_free(error_details);

  /* Invalid overflow size. */
  status = alts_counter_create(true, counter_size, 0, &ctr, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_details, "overflow_size is invalid."));
  gpr_free(error_details);

  /* alts_counter is nullptr. */
  status = alts_counter_create(true, counter_size, overflow_size, nullptr, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_details, "crypter_counter is nullptr."));
  gpr_free(error_details);

  status = alts_counter_create(true, counter_size, overflow_size, &ctr, &error_details);
  ...
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_details, "is_overflow is nullptr."));
  gpr_free(error_details);
  alts_counter_destroy(ctr);
}

static void alts_counter_test_overflow_full_range(bool is_client, size_t counter_size,
                                                  size_t overflow_size) {
  alts_counter* ctr = nullptr;
  char* error_details = nullptr;
  grpc_status_code status =
      alts_counter_create(is_client, counter_size, overflow_size, &ctr, &error_details);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  unsigned char* expected = static_cast<unsigned char*>(gpr_zalloc(counter_size));
  if (is_client) {
    expected[counter_size - 1] = 0x80;
  }
  /* Do a single iteration to ensure the counter is initialized as expected. */
  GPR_ASSERT(do_bytes_represent_client(ctr, alts_counter_get_counter(ctr), counter_size) ==
             is_client);
  GPR_ASSERT(memcmp(alts_counter_get_counter(ctr), expected, counter_size) == 0);
  bool is_overflow = false;
  GPR_ASSERT(alts_counter_increment(ctr, &is_overflow, &error_details) == GRPC_STATUS_OK);
...
static void alts_counter_test_overflow_single_increment(bool is_client, size_t counter_size,
                                                        size_t overflow_size) {
  alts_counter* ctr = nullptr;
  char* error_details = nullptr;
  grpc_status_code status =
  ...
  GPR_ASSERT(do_bytes_represent_client(ctr, alts_counter_get_counter(ctr), counter_size) ==
             is_client);
  GPR_ASSERT(memcmp(expected, alts_counter_get_counter(ctr), counter_size) == 0);
  bool is_overflow = false;
  GPR_ASSERT(alts_counter_increment(ctr, &is_overflow, &error_details) == GRPC_STATUS_OK);
  GPR_ASSERT(!is_overflow);
  GPR_ASSERT(do_bytes_represent_client(ctr, alts_counter_get_counter(ctr), counter_size) ==
...
int main(int /*argc*/, char** /*argv*/) {
  ...
  alts_counter_test_overflow_full_range(true, kSmallCounterSize, kSmallOverflowSize);
  alts_counter_test_overflow_full_range(false, kSmallCounterSize, kSmallOverflowSize);
  alts_counter_test_overflow_single_increment(true, kGcmCounterSize, kGcmOverflowSize);
