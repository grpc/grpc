...
static void alts_counter_test_input_sanity_check(size_t counter_size,
  ...
  status =
      alts_counter_create(true, counter_size, 0, &ctr, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_details,
  ...
  status = alts_counter_create(true, counter_size, overflow_size,
                               nullptr, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_details,
  ...
  status =
      alts_counter_increment(nullptr, &is_overflow, &error_details);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_details,
      "crypter_counter is nullptr."));
  gpr_free(error_details);
...
static void alts_counter_test_overflow_full_range(
    bool is_client, size_t counter_size, size_t overflow_size) {
  alts_counter* ctr = nullptr;
  char* error_details = nullptr;
  ...
  /* Do a single iteration to ensure the counter is initialized as
   * expected. */
  GPR_ASSERT(do_bytes_represent_client(ctr,
                                       alts_counter_get_counter(ctr),
                                       counter_size) == is_client);
  GPR_ASSERT(memcmp(alts_counter_get_counter(ctr), expected,
                    counter_size) == 0);
  bool is_overflow = false;
  GPR_ASSERT(alts_counter_increment(ctr, &is_overflow,
                                    &error_details) == GRPC_STATUS_OK);
  GPR_ASSERT(!is_overflow);
  /**
   * The counter can return 2^{overflow_size * 8} counters. The
   * high-order bit is fixed to the client/server. The last call will
   * yield a useable counter, but overflow the counter object.
   */
  ...
  for (ind = 1; ind < iterations - 1; ind++) {
    GPR_ASSERT(do_bytes_represent_client(ctr,
                                         ...
                                         counter_size) == is_client);
    GPR_ASSERT(
        alts_counter_increment(ctr, &is_overflow, &error_details) ==
        GRPC_STATUS_OK);
    GPR_ASSERT(!is_overflow);
  }
  GPR_ASSERT(do_bytes_represent_client(ctr,
...
static void alts_counter_test_overflow_single_increment(
  ...
  GPR_ASSERT(do_bytes_represent_client(ctr,
                                       alts_counter_get_counter(ctr),
                                       counter_size) == is_client);
  GPR_ASSERT(memcmp(expected, alts_counter_get_counter(ctr),
                    counter_size) == 0);
  bool is_overflow = false;
  GPR_ASSERT(alts_counter_increment(ctr, &is_overflow,
                                    &error_details) == GRPC_STATUS_OK);
...
int main(int /*argc*/, char** /*argv*/) {
  alts_counter_test_input_sanity_check(kGcmCounterSize,
                                       kGcmOverflowSize);
