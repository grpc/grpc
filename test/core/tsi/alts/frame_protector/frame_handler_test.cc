...
static size_t frame_length(size_t payload_length) { return payload_length + kFrameHeaderSize; }

static frame_handler* create_frame_handler() {
  frame_handler* handler = static_cast<frame_handler*>(gpr_malloc(sizeof(frame_handler)));
...
static void frame(frame_handler* handler, unsigned char* payload, size_t payload_length,
                  size_t write_length) {
  handler->buffer_size = frame_length(payload_length);
  handler->buffer = static_cast<unsigned char*>(gpr_malloc(handler->buffer_size));
  GPR_ASSERT(alts_reset_frame_writer(handler->writer, payload, payload_length));
  size_t offset = 0;
  while (offset < handler->buffer_size && !alts_is_frame_writer_done(handler->writer)) {
    size_t bytes_written = GPR_MIN(write_length, handler->buffer_size - offset);
    GPR_ASSERT(alts_write_frame_bytes(handler->writer, handler->buffer + offset, &bytes_written));
...
static size_t deframe(frame_handler* handler, unsigned char* bytes, size_t read_length) {
  ...
  size_t offset = 0;
  while (offset < handler->buffer_size && !alts_is_frame_reader_done(handler->reader)) {
    size_t bytes_read = GPR_MIN(read_length, handler->buffer_size - offset);
    GPR_ASSERT(alts_read_frame_bytes(handler->reader, handler->buffer + offset, &bytes_read));
    offset += bytes_read;
  }
...
static void frame_n_deframe(frame_handler* handler, unsigned char* payload, size_t payload_length,
                            size_t write_length, size_t read_length) {
  frame(handler, payload, payload_length, write_length);
  unsigned char* bytes = static_cast<unsigned char*>(gpr_malloc(kFrameHandlerTestBufferSize));
...
static void frame_handler_test_frame_deframe() {
  ...
  frame_n_deframe(handler, payload, payload_length, frame_length(payload_length),
...
static void frame_handler_test_null_writer_byte_length() {
  ...
  frame_handler* handler = create_frame_handler();
  GPR_ASSERT(alts_reset_frame_writer(handler->writer, payload, payload_length));
  GPR_ASSERT(!alts_write_frame_bytes(handler->writer, handler->buffer, nullptr));
  destroy_frame_handler(handler);
}
...
static void frame_handler_test_null_writer_bytes() {
  ...
  frame_handler* handler = create_frame_handler();
  GPR_ASSERT(alts_reset_frame_writer(handler->writer, payload, payload_length));
  GPR_ASSERT(!alts_write_frame_bytes(handler->writer, nullptr, &payload_length));
  destroy_frame_handler(handler);
}
...
static void frame_handler_test_bad_frame_length() {
  ...
  GPR_ASSERT(!alts_read_frame_bytes(handler->reader, handler->buffer, &bytes_read));
  GPR_ASSERT(alts_is_frame_reader_done(handler->reader));
...
static void frame_handler_test_unsupported_message_type() {
  ...
  frame_handler* handler = create_frame_handler();
  frame(handler, payload, payload_length, payload_length);
  memset(handler->buffer + kFrameLengthFieldSize, 0x00, kFrameMessageTypeFieldSize);
  unsigned char* bytes = static_cast<unsigned char*>(gpr_malloc(kFrameHandlerTestBufferSize));
  GPR_ASSERT(alts_reset_frame_reader(handler->reader, bytes));
  size_t bytes_read = handler->buffer_size;
  GPR_ASSERT(!alts_read_frame_bytes(handler->reader, handler->buffer, &bytes_read));
