...
static void gsec_randomly_slice(uint8_t* input, size_t input_length,
  ...
  *output = static_cast<struct iovec*>(
      malloc(*output_length * sizeof(**output)));
  size_t i;
...
static void gsec_test_random_encrypt_decrypt(gsec_aead_crypter* crypter,
  ...
  gsec_assert_ok(
      gsec_aead_crypter_encrypt(
          crypter, nonce, nonce_length, aad, aad_length, message,
  ...
  gsec_aead_crypter_max_plaintext_length(
      crypter, ciphertext_bytes_written, &plaintext_length, nullptr);
  uint8_t* plaintext =
  ...
  /**
   * The returned plaintext will be zeroed if there was an
   * authentication error.
   */
  uint8_t* zero_message =
      static_cast<uint8_t*>(gpr_zalloc(plaintext_length));
  if (tag_length >= kTestMinTagLengthForCorruption) {
    ...
    if (nonce_length > 0) {
      ...
      gsec_test_copy_and_alter_random_byte(nonce, &corrupt_nonce,
                                           nonce_length);
      ...
      GPR_ASSERT(memcmp(zero_message, plaintext, plaintext_length) ==
                 0);
...
static void gsec_test_multiple_random_encrypt_decrypt(
    gsec_aead_crypter* crypter, size_t* aad_lengths,
    size_t* message_lengths, size_t count) {
  GPR_ASSERT(crypter != nullptr);
  size_t nonce_length, tag_length;
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    gsec_aead_crypter_max_ciphertext_and_tag_length(
        ...
        nullptr);
    ciphertext_and_tags[ind] = static_cast<uint8_t*>(
        gpr_malloc(ciphertext_and_tag_lengths[ind]));
    ...
    GPR_ASSERT(status == GRPC_STATUS_OK);
    GPR_ASSERT(message_length + tag_length ==
               ciphertext_and_tag_lengths[ind]);
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    size_t message_length =
        (message_lengths == nullptr) ? 0 : message_lengths[ind];
    gsec_aead_crypter_max_plaintext_length(
        crypter, ciphertext_bytes_writtens[ind],
        &(plaintext_lengths[ind]), nullptr);
    plaintexts[ind] =
        static_cast<uint8_t*>(gpr_malloc(plaintext_lengths[ind]));
    grpc_status_code status = gsec_aead_crypter_decrypt(
        crypter, nonces[ind], nonce_length, aads[ind], aad_length,
    ...
    GPR_ASSERT(memcmp(messages[ind], plaintexts[ind], message_length) ==
               0);
  }
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    gsec_randomly_slice(aads[ind], aad_length, &aad_vecs,
                        &aad_vecs_length);
    size_t message_length =
        (message_lengths == nullptr) ? 0 : message_lengths[ind];
    ...
    gsec_assert_ok(gsec_aead_crypter_encrypt_iovec(
                   ...
                   error_details);
    GPR_ASSERT(memcmp(ciphertext_and_tags[ind],
                      another_ciphertext_vec.iov_base,
                      ciphertext_length) == 0);
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    uint8_t* decrypted =
        static_cast<uint8_t*>(malloc(decrypted_length));
    ...
    GPR_ASSERT(memcmp(decrypted_vec.iov_base, messages[ind],
                      message_length) == 0);
    free(decrypted);
    free(aad_vecs);
...
static void gsec_test_multiple_encrypt_decrypt(
  ...
  gsec_test_multiple_random_encrypt_decrypt(crypter, aad_lengths,
                                            nullptr, count);
  gsec_test_multiple_random_encrypt_decrypt(crypter, nullptr,
                                            message_lengths, count);
...
static void gsec_test_encryption_failure(gsec_aead_crypter* crypter) {
  ...
  /* nullptr nonce */
  grpc_status_code status = gsec_aead_crypter_encrypt(
      crypter, nullptr, nonce_length, aad, aad_length, message,
      message_length, ciphertext_and_tag, ciphertext_and_tag_length,
  ...
  gsec_assert_ok(
      gsec_aead_crypter_encrypt(
          crypter, nonce, nonce_length, nullptr, 0, message,
          message_length, ciphertext_and_tag, ciphertext_and_tag_length,
          &ciphertext_bytes_written, &error_message),
      error_message);
  ...
  status = gsec_aead_crypter_encrypt(
      crypter, nonce, nonce_length, aad, aad_length, message,
      message_length, ciphertext_and_tag, ciphertext_and_tag_length - 1,
      &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
...
static void gsec_test_decryption_failure(gsec_aead_crypter* crypter) {
  ...
  grpc_status_code status = gsec_aead_crypter_encrypt(
      crypter, nonce, nonce_length, aad, aad_length, message,
      message_length, ciphertext_and_tag, ciphertext_and_tag_length,
      &ciphertext_bytes_written, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(ciphertext_bytes_written == ciphertext_and_tag_length);
  ...
  /* nullptr nonce */
  status = gsec_aead_crypter_decrypt(
      crypter, nullptr, nonce_length, aad, aad_length,
  ...
  /* Big nonce */
  status = gsec_aead_crypter_decrypt(
      crypter, nonce, nonce_length + 1, aad, aad_length,
      ciphertext_and_tag, ciphertext_and_tag_length, plaintext,
      plaintext_length, &plaintext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
  ...
  /* nullptr aad */
  status = gsec_aead_crypter_decrypt(
      crypter, nonce, nonce_length, nullptr, aad_length,
      ciphertext_and_tag, ciphertext_and_tag_length, plaintext,
      plaintext_length, &plaintext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message,
  ...
  if (tag_length > 0) {
    status = gsec_aead_crypter_decrypt(
        crypter, nonce, nonce_length, aad, aad_length,
        ciphertext_and_tag, tag_length - 1, plaintext, plaintext_length,
        &plaintext_bytes_written, &error_message);
  ...
  status = gsec_aead_crypter_decrypt(
      crypter, nonce, nonce_length, aad, aad_length, ciphertext_and_tag,
      ciphertext_and_tag_length, plaintext, plaintext_length, nullptr,
      &error_message);
...
static void gsec_test_encrypt_decrypt_test_vector(
  ...
  size_t ciphertext_and_tag_length, ciphertext_bytes_written = 0;
  gsec_aead_crypter_max_ciphertext_and_tag_length(
      crypter, test_vector->plaintext_length,
      &ciphertext_and_tag_length, nullptr);
  uint8_t* ciphertext_and_tag_bytes =
      static_cast<uint8_t*>(gpr_malloc(ciphertext_and_tag_length));
  grpc_status_code status = gsec_aead_crypter_encrypt(
      crypter, test_vector->nonce, test_vector->nonce_length,
      test_vector->aad, test_vector->aad_length, test_vector->plaintext,
      test_vector->plaintext_length, ciphertext_and_tag_bytes,
      ciphertext_and_tag_length, &ciphertext_bytes_written, nullptr);

  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(ciphertext_bytes_written == ciphertext_and_tag_length);
  GPR_ASSERT(memcmp(test_vector->ciphertext_and_tag,
  ...
  size_t plaintext_length, plaintext_bytes_written = 0;
  gsec_aead_crypter_max_plaintext_length(
      crypter, ciphertext_and_tag_length, &plaintext_length, nullptr);
...
static void gsec_aead_malloc_test_vector(
    gsec_aead_test_vector** test_vector, const uint8_t* key,
    size_t key_length, const uint8_t* nonce, size_t nonce_length,
    const uint8_t* aad, size_t aad_length, const uint8_t* plaintext,
    size_t plaintext_length, const uint8_t* ciphertext_and_tag,
    size_t ciphertext_and_tag_length) {
  *test_vector = static_cast<gsec_aead_test_vector*>(
  ...
  gsec_test_copy(plaintext, &((*test_vector)->plaintext),
                 plaintext_length);
  gsec_test_copy(ciphertext_and_tag,
                 &((*test_vector)->ciphertext_and_tag),
                 ciphertext_and_tag_length);
}
...
static void gsec_test_create_random_aes_gcm_crypter(
  ...
  gsec_aes_gcm_aead_crypter_create(key, key_length, nonce_length,
...
static void gsec_test_do_vector_tests_rekey_nist() {
  ...
  //
  // Key expanded by setting expandedKey = (key||(key ^ {0x01, ..,
  // 0x01})||key ^ {0x02,..,0x02}))[0:44].
  ...
  uint8_t aad_0[1] = {};
  uint8_t key_0[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  ...
  uint8_t ciphertext_0[] = {0x85, 0xE8, 0x73, 0xE0, 0x2,  0xF6,
                            0xEB, 0xDC, 0x40, 0x60, 0x95, 0x4E,
                            0xB8, 0x67, 0x55, 0x8};
  vec = {nonce_0, aad_0, key_0, plaintext_0, ciphertext_0,
         12,      0,     44,    0,           16};
  ...
  uint8_t nonce_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                       0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
  uint8_t aad_1[1] = {};
  uint8_t key_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                     0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x1,
                     0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1,
                     0x1, 0x1, 0x1, 0x1, 0x1, 0x2, 0x2, 0x2, 0x2,
                     0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2};
  ...
  uint8_t ciphertext_1[] = {
      0x51, 0xE9, 0xA8, 0xCB, 0x23, 0xCA, 0x25, 0x12, 0xC8, 0x25, 0x6A,
      0xFF, 0xF8, 0xE7, 0x2D, 0x68, 0x1A, 0xCA, 0x19, 0xA1, 0x14, 0x8A,
      0xC1, 0x15, 0xE8, 0x3D, 0xF4, 0x88, 0x8C, 0xC0, 0xD,  0x11};
  vec = {nonce_1, aad_1, key_1, plaintext_1, ciphertext_1,
         12,      0,     44,    16,          32};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  uint8_t ciphertext_2[] = {
      0x10, 0x18, 0xED, 0x5A, 0x14, 0x2,  0xA8, 0x65, 0x16, 0xD6,
      0x57, 0x6D, 0x70, 0xB2, 0xFF, 0xCC, 0xCA, 0x26, 0x1B, 0x94,
      0xDF, 0x88, 0xB5, 0x8F, 0x53, 0xB6, 0x4D, 0xFB, 0xA4, 0x35,
      0xD1, 0x8B, 0x2F, 0x6E, 0x3B, 0x78, 0x69, 0xF9, 0x35, 0x3D,
      0x4A, 0xC8, 0xCF, 0x9,  0xAF, 0xB1, 0x66, 0x3D, 0xAA, 0x7B,
      0x40, 0x17, 0xE6, 0xFC, 0x2C, 0x17, 0x7C, 0xC,  0x8,  0x7C,
      0xD,  0xF1, 0x16, 0x21, 0x29, 0x95, 0x22, 0x13, 0xCE, 0xE1,
      0xBC, 0x6E, 0x9C, 0x84, 0x95, 0xDD, 0x70, 0x5E, 0x1F, 0x3D};
  vec = {nonce_2, aad_2, key_2, plaintext_2, ciphertext_2,
         12,      0,     44,    64,          80};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  uint8_t key_3[] = {
      ...
      0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_3[] = {
      0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59,
      0x9,  0xC5, 0xAF, 0xF5, 0x26, 0x9A, 0x86, 0xA7, 0xA9, 0x53,
      0x15, 0x34, 0xF7, 0xDA, 0x2E, 0x4C, 0x30, 0x3D, 0x8A, 0x31,
      0x8A, 0x72, 0x1C, 0x3C, 0xC,  0x95, 0x95, 0x68, 0x9,  0x53,
      0x2F, 0xCF, 0xE,  0x24, 0x49, 0xA6, 0xB5, 0x25, 0xB1, 0x6A,
      0xED, 0xF5, 0xAA, 0xD,  0xE6, 0x57, 0xBA, 0x63, 0x7B, 0x39};
  ...
  vec = {nonce_3, aad_3, key_3, plaintext_3, ciphertext_3,
         12,      20,    44,    60,          76};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from adapted NIST test vector 4 for KDF counter boundary
  // (flip nonce bit 15)
  ...
  vec = {nonce_5, aad_5, key_5, plaintext_5, ciphertext_5,
         12,      20,    44,    60,          76};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  vec = {nonce_6, aad_6, key_6, plaintext_6, ciphertext_6,
         12,      20,    44,    60,          76};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  vec = {nonce_7, aad_7, key_7, plaintext_7, ciphertext_7,
         12,      20,    44,    60,          76};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
}
...
static void gsec_test_do_vector_tests_rekey_ieee() {
  ...
  uint8_t aad_8[] = {
      0xD6, 0x9,  0xB1, 0xF0, 0x56, 0x63, 0x7A, 0xD,  0x46, 0xDF,
  ...
  uint8_t key_8[] = {
      0xAD, 0x7A, 0x2B, 0xD0, 0x3E, 0xAC, 0x83, 0x5A, 0x6F, 0x62, 0xF,
  ...
  uint8_t ciphertext_8[] = {0x3E, 0xA0, 0xB5, 0x84, 0xF3, 0xC8,
                            0x5E, 0x93, 0xF9, 0x32, 0xE,  0xA5,
                            0x91, 0x69, 0x9E, 0xFB};
  vec = {nonce_8, aad_8, key_8, plaintext_8, ciphertext_8,
         12,      70,    44,    0,           16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  uint8_t aad_9[] = {
      ...
      0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x0,  0x1};
  uint8_t key_9[] = {
      0xE3, 0xC0, 0x8A, 0x8F, 0x6,  0xC6, 0xE3, 0xAD, 0x95, 0xA7, 0x5,
  ...
  uint8_t ciphertext_9[] = {0x29, 0x4E, 0x2,  0x8B, 0xF1, 0xFE,
                            0x6F, 0x14, 0xC4, 0xE8, 0xF7, 0x30,
                            0x5C, 0x93, 0x3E, 0xB5};
  vec = {nonce_9, aad_9, key_9, plaintext_9, ciphertext_9,
         12,      70,    44,    0,           16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
  ...
  uint8_t plaintext_10[] = {
      ...
      0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x0,  0x2};
  ...
  uint8_t aad_12[] = {
      0xE2, 0x1,  0x6,  0xD7, 0xCD, 0xD,  0xF0, 0x76, 0x1E, 0x8D,
      0xCD, 0x3D, 0x88, 0xE5, 0x40, 0x0,  0x76, 0xD4, 0x57, 0xED,
      0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
      0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
      0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
      0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
  ...
  uint8_t key_12[] = {
      0x7,  0x1B, 0x11, 0x3B, 0xC,  0xA7, 0x43, 0xFE, 0xCC, 0xCF, 0x3D,
  ...
  uint8_t ciphertext_12[] = {0x58, 0x83, 0x7A, 0x10, 0x56, 0x2B,
                             0xF,  0x1F, 0x8E, 0xDB, 0xE5, 0x8C,
                             0xA5, 0x58, 0x11, 0xD3};
  ...
  uint8_t aad_13[] = {
      ...
      0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x0,  0x3};
  uint8_t key_13[] = {
      0x69, 0x1D, 0x3E, 0xE9, 0x9,  0xD7, 0xF5, 0x41, 0x67, 0xFD, 0x1C,
  ...
  uint8_t ciphertext_13[] = {0xC2, 0x72, 0x2F, 0xF6, 0xCA, 0x29,
                             0xA2, 0x57, 0x71, 0x8A, 0x52, 0x9D,
                             0x1F, 0xC,  0x6A, 0x3B};
  ...
  uint8_t ciphertext_14[] = {
      0xFD, 0x96, 0xB7, 0x15, 0xB9, 0x3A, 0x13, 0x34, 0x6A, 0xF5,
      0x1E, 0x8A, 0xCD, 0xF7, 0x92, 0xCD, 0xC7, 0xB2, 0x68, 0x6F,
      0x85, 0x74, 0xC7, 0xE,  0x6B, 0xC,  0xBF, 0x16, 0x29, 0x1D,
      0xED, 0x42, 0x7A, 0xD7, 0x3F, 0xEC, 0x48, 0xCD, 0x29, 0x8E,
      0x5,  0x28, 0xA1, 0xF4, 0xC6, 0x44, 0xA9, 0x49, 0xFC, 0x31,
      0xDC, 0x92, 0x79, 0x70, 0x6D, 0xDB, 0xA3, 0x3F};
  ...
  uint8_t ciphertext_15[] = {
      0xB6, 0x8F, 0x63, 0x0,  0xC2, 0xE9, 0xAE, 0x83, 0x3B, 0xDC,
      0x7,  0xE,  0x24, 0x2,  0x1A, 0x34, 0x77, 0x11, 0x8E, 0x78,
      0xCC, 0xF8, 0x4E, 0x11, 0xA4, 0x85, 0xD8, 0x61, 0x47, 0x6C,
      0x30, 0xF,  0x17, 0x53, 0x53, 0xD5, 0xCD, 0xF9, 0x20, 0x8,
      0xA4, 0xF8, 0x78, 0xE6, 0xCC, 0x35, 0x77, 0x76, 0x80, 0x85,
      0xC5, 0xA,  0xE,  0x98, 0xFD, 0xA6, 0xCB, 0xB8};
  ...
  uint8_t aad_16[] = {
      0x84, 0xC5, 0xD5, 0x13, 0xD2, 0xAA, 0xF6, 0xE5, 0xBB, 0xD2, 0x72,
      0x77, 0x88, 0xE5, 0x23, 0x0,  0x89, 0x32, 0xD6, 0x12, 0x7C, 0xFD,
      0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6, 0x8,  0x0,  0xF,  0x10, 0x11,
      ...
      0x3E, 0x3F, 0x0,  0x5};
  uint8_t key_16[] = {
      0x1,  0x3F, 0xE0, 0xB,  0x5F, 0x11, 0xBE, 0x7F, 0x86, 0x6D, 0xC,
  ...
  uint8_t ciphertext_16[] = {0xCC, 0xA2, 0xE,  0xEC, 0xDA, 0x62,
                             0x83, 0xF0, 0x9B, 0xB3, 0x54, 0x3D,
                             0xD9, 0x9E, 0xDB, 0x9B};
  ...
  uint8_t ciphertext_17[] = {0xB2, 0x32, 0xCC, 0x1D, 0xA5, 0x11,
                             0x7B, 0xF1, 0x50, 0x3,  0x73, 0x4F,
                             0xA5, 0x99, 0xD2, 0x71};
  ...
  uint8_t key_20[] = {
      0x88, 0xEE, 0x8,  0x7F, 0xD9, 0x5D, 0xA9, 0xFB, 0xF6, 0x72, 0x5A,
  ...
  uint8_t ciphertext_20[] = {0x81, 0x3F, 0xE,  0x63, 0xF,  0x96,
                             0xFB, 0x2D, 0x3,  0xF,  0x58, 0xD8,
                             0x3F, 0x5C, 0xDF, 0xD0};
  ...
  uint8_t ciphertext_21[] = {0x77, 0xE5, 0xA4, 0x4C, 0x21, 0xEB,
                             0x7,  0x18, 0x8A, 0xAC, 0xBD, 0x74,
                             0xD1, 0x98, 0xE,  0x97};
  ...
  uint8_t ciphertext_22[] = {
      0x95, 0x8E, 0xC3, 0xF6, 0xD6, 0xA,  0xFE, 0xDA, 0x99, 0xEF,
      0xD8, 0x88, 0xF1, 0x75, 0xE5, 0xFC, 0xD4, 0xC8, 0x7B, 0x9B,
      0xCC, 0x5C, 0x2F, 0x54, 0x26, 0x25, 0x3A, 0x8B, 0x50, 0x62,
      0x96, 0xC8, 0xC4, 0x33, 0x9,  0xAB, 0x2A, 0xDB, 0x59, 0x39,
      0x46, 0x25, 0x41, 0xD9, 0x5E, 0x80, 0x81, 0x1E, 0x4,  0xE7,
      0x6,  0xB1, 0x49, 0x8F, 0x2C, 0x40, 0x7C, 0x7F, 0xB2, 0x34,
      0xF8, 0xCC, 0x1,  0xA6, 0x47, 0x55, 0xE,  0xE6, 0xB5, 0x57,
      0xB3, 0x5A, 0x7E, 0x39, 0x45, 0x38, 0x18, 0x21, 0xF4};
  ...
  uint8_t ciphertext_23[] = {
      0xB4, 0x4D, 0x7,  0x20, 0x11, 0xCD, 0x36, 0xD2, 0x72, 0xA9,
      0xB7, 0xA9, 0x8D, 0xB9, 0xAA, 0x90, 0xCB, 0xC5, 0xC6, 0x7B,
      0x93, 0xDD, 0xCE, 0x67, 0xC8, 0x54, 0x50, 0x32, 0x14, 0xE2,
      0xE8, 0x96, 0xEC, 0x7E, 0x9D, 0xB6, 0x49, 0xED, 0x4B, 0xCF,
      0x6F, 0x85, 0xA,  0xAC, 0x2,  0x23, 0xD0, 0xCF, 0x92, 0xC8,
      0x3D, 0xB8, 0x7,  0x95, 0xC3, 0xA1, 0x7E, 0xCC, 0x12, 0x48,
      0xBB, 0x0,  0x59, 0x17, 0x12, 0xB1, 0xAE, 0x71, 0xE2, 0x68,
      0x16, 0x41, 0x96, 0x25, 0x21, 0x62, 0x81, 0xB,  0x0};
...
static void gsec_test_do_vector_tests_nist() {
  ...
  const uint8_t test_vector_1_key[] = {
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t test_vector_1_nonce[] = {0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00};
  ...
  gsec_aead_malloc_test_vector(
      ...
      0, test_vector_1_plaintext, 0, test_vector_1_ciphertext_and_tag,
  ...
  const uint8_t test_vector_2_key[] = {
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t test_vector_2_nonce[] = {0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00};
  const uint8_t test_vector_2_aad[1] = {};
  const uint8_t test_vector_2_plaintext[] = {
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  const uint8_t test_vector_3_key[] = {
      0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c,
      0x6d, 0x6a, 0x8f, 0x94, 0x67, 0x30, 0x83, 0x08};
  const uint8_t test_vector_3_nonce[] = {0xca, 0xfe, 0xba, 0xbe,
                                         0xfa, 0xce, 0xdb, 0xad,
                                         0xde, 0xca, 0xf8, 0x88};
  ...
  const uint8_t test_vector_3_ciphertext_and_tag[] = {
      ...
      0x47, 0x3f, 0x59, 0x85, 0x4d, 0x5c, 0x2a, 0xf3, 0x27, 0xcd,
  ...
  const uint8_t test_vector_4_key[] = {
      0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c,
      0x6d, 0x6a, 0x8f, 0x94, 0x67, 0x30, 0x83, 0x08};
  const uint8_t test_vector_4_nonce[] = {0xca, 0xfe, 0xba, 0xbe,
                                         0xfa, 0xce, 0xdb, 0xad,
                                         0xde, 0xca, 0xf8, 0x88};
  const uint8_t test_vector_4_aad[] = {
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_4_aad) / sizeof(uint8_t),
      test_vector_4_plaintext,
...
static void gsec_test_do_vector_tests_ieee() {
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_5_aad) / sizeof(uint8_t),
      test_vector_5_plaintext, 0, test_vector_5_ciphertext_and_tag,
  ...
  const uint8_t test_vector_6_nonce[] = {0x12, 0x15, 0x35, 0x24,
                                         0xc0, 0x89, 0x5e, 0x81,
                                         0xb2, 0xc2, 0x84, 0x65};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_6_aad) / sizeof(uint8_t),
      test_vector_6_plaintext, 0, test_vector_6_ciphertext_and_tag,
  ...
  const uint8_t test_vector_7_key[] = {
      0xad, 0x7a, 0x2b, 0xd0, 0x3e, 0xac, 0x83, 0x5a,
      0x6f, 0x62, 0x0f, 0xdc, 0xb5, 0x06, 0xb3, 0x45};

  const uint8_t test_vector_7_nonce[] = {0x12, 0x15, 0x35, 0x24,
                                         0xc0, 0x89, 0x5e, 0x81,
                                         0xb2, 0xc2, 0x84, 0x65};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_7_aad) / sizeof(uint8_t),
      test_vector_7_plaintext,
  ...
  const uint8_t test_vector_8_key[] = {
      ...
      0x57, 0xb2, 0x3f, 0x75, 0x48, 0x3c, 0xe3, 0x30, 0x21, 0xa9, 0xc7,
      0x2b, 0x70, 0x25, 0x66, 0x62, 0x04, 0xc6, 0x9c, 0x0b, 0x72};
  const uint8_t test_vector_8_nonce[] = {0x12, 0x15, 0x35, 0x24,
                                         0xc0, 0x89, 0x5e, 0x81,
                                         0xb2, 0xc2, 0x84, 0x65};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_8_aad) / sizeof(uint8_t),
      test_vector_8_plaintext,
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_9_aad) / sizeof(uint8_t),
      test_vector_9_plaintext, 0, test_vector_9_ciphertext_and_tag,
  ...
  const uint8_t test_vector_10_nonce[] = {0xf0, 0x76, 0x1e, 0x8d,
                                          0xcd, 0x3d, 0x00, 0x01,
                                          0x76, 0xd4, 0x57, 0xed};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_10_key) / sizeof(uint8_t),
      test_vector_10_nonce,
      sizeof(test_vector_10_nonce) / sizeof(uint8_t),
      test_vector_10_aad, sizeof(test_vector_10_aad) / sizeof(uint8_t),
  ...
  const uint8_t test_vector_11_key[] = {
      0x07, 0x1b, 0x11, 0x3b, 0x0c, 0xa7, 0x43, 0xfe,
      0xcc, 0xcf, 0x3d, 0x05, 0x1f, 0x73, 0x73, 0x82};
  const uint8_t test_vector_11_nonce[] = {0xf0, 0x76, 0x1e, 0x8d,
                                          0xcd, 0x3d, 0x00, 0x01,
                                          0x76, 0xd4, 0x57, 0xed};
  ...
  const uint8_t test_vector_11_ciphertext_and_tag[] = {
      0x13, 0xb4, 0xc7, 0x2b, 0x38, 0x9d, 0xc5, 0x01, 0x8e, 0x72,
      0xa1, 0x71, 0xdd, 0x85, 0xa5, 0xd3, 0x75, 0x22, 0x74, 0xd3,
      0xa0, 0x19, 0xfb, 0xca, 0xed, 0x09, 0xa4, 0x25, 0xcd, 0x9b,
      0x2e, 0x1c, 0x9b, 0x72, 0xee, 0xe7, 0xc9, 0xde, 0x7d, 0x52,
      0xb3, 0xf3, 0xd6, 0xa5, 0x28, 0x4f, 0x4a, 0x6d, 0x3f, 0xe2,
      0x2a, 0x5d, 0x6c, 0x2b, 0x96, 0x04, 0x94, 0xc3};
  gsec_aead_malloc_test_vector(
      &test_vector_11, test_vector_11_key,
      sizeof(test_vector_11_key) / sizeof(uint8_t),
      test_vector_11_nonce,
      sizeof(test_vector_11_nonce) / sizeof(uint8_t),
      test_vector_11_aad, sizeof(test_vector_11_aad) / sizeof(uint8_t),
      test_vector_11_plaintext,
  ...
  const uint8_t test_vector_12_key[] = {
      ...
      0xa0, 0xb5, 0xd7, 0x69, 0x08, 0x1f, 0x2b, 0xde, 0x1a, 0xee, 0x65,
      0x5f, 0xdb, 0xab, 0x80, 0xbd, 0x52, 0x95, 0xae, 0x6b, 0xe7};
  const uint8_t test_vector_12_nonce[] = {0xf0, 0x76, 0x1e, 0x8d,
                                          0xcd, 0x3d, 0x00, 0x01,
                                          0x76, 0xd4, 0x57, 0xed};
  ...
  const uint8_t test_vector_12_ciphertext_and_tag[] = {
      0xc1, 0x62, 0x3f, 0x55, 0x73, 0x0c, 0x93, 0x53, 0x30, 0x97,
      0xad, 0xda, 0xd2, 0x56, 0x64, 0x96, 0x61, 0x25, 0x35, 0x2b,
      0x43, 0xad, 0xac, 0xbd, 0x61, 0xc5, 0xef, 0x3a, 0xc9, 0x0b,
      0x5b, 0xee, 0x92, 0x9c, 0xe4, 0x63, 0x0e, 0xa7, 0x9f, 0x6c,
      0xe5, 0x19, 0x12, 0xaf, 0x39, 0xc2, 0xd1, 0xfd, 0xc2, 0x05,
      0x1f, 0x8b, 0x7b, 0x3c, 0x9d, 0x39, 0x7e, 0xf2};
  gsec_aead_malloc_test_vector(
      &test_vector_12, test_vector_12_key,
      sizeof(test_vector_12_key) / sizeof(uint8_t),
      test_vector_12_nonce,
      sizeof(test_vector_12_nonce) / sizeof(uint8_t),
      test_vector_12_aad, sizeof(test_vector_12_aad) / sizeof(uint8_t),
      test_vector_12_plaintext,
  ...
  const uint8_t test_vector_13_key[] = {
      0x01, 0x3f, 0xe0, 0x0b, 0x5f, 0x11, 0xbe, 0x7f,
      0x86, 0x6d, 0x0c, 0xbb, 0xc5, 0x5a, 0x7a, 0x90};
  const uint8_t test_vector_13_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9,
                                          0xe3, 0x37, 0x24, 0xc6,
                                          0x89, 0x32, 0xd6, 0x12};
  const uint8_t test_vector_13_aad[] = {
      0x84, 0xc5, 0xd5, 0x13, 0xd2, 0xaa, 0xf6, 0xe5, 0xbb, 0xd2, 0x72,
      0x77, 0x88, 0xe5, 0x23, 0x00, 0x89, 0x32, 0xd6, 0x12, 0x7c, 0xfd,
      0xe9, 0xf9, 0xe3, 0x37, 0x24, 0xc6, 0x08, 0x00, 0x0f, 0x10, 0x11,
      0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c,
      0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
      0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x31, 0x32,
      0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d,
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_13_key) / sizeof(uint8_t),
      test_vector_13_nonce,
      sizeof(test_vector_13_nonce) / sizeof(uint8_t),
      test_vector_13_aad, sizeof(test_vector_13_aad) / sizeof(uint8_t),
  ...
  const uint8_t test_vector_14_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9,
                                          0xe3, 0x37, 0x24, 0xc6,
                                          0x89, 0x32, 0xd6, 0x12};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_14_key) / sizeof(uint8_t),
      test_vector_14_nonce,
      sizeof(test_vector_14_nonce) / sizeof(uint8_t),
      test_vector_14_aad, sizeof(test_vector_14_aad) / sizeof(uint8_t),
  ...
  const uint8_t test_vector_15_key[] = {
      0x01, 0x3f, 0xe0, 0x0b, 0x5f, 0x11, 0xbe, 0x7f,
      0x86, 0x6d, 0x0c, 0xbb, 0xc5, 0x5a, 0x7a, 0x90};
  const uint8_t test_vector_15_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9,
                                          0xe3, 0x37, 0x24, 0xc6,
                                          0x89, 0x32, 0xd6, 0x12};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_15_key) / sizeof(uint8_t),
      test_vector_15_nonce,
      sizeof(test_vector_15_nonce) / sizeof(uint8_t),
      test_vector_15_aad, sizeof(test_vector_15_aad) / sizeof(uint8_t),
      test_vector_15_plaintext,
  ...
  const uint8_t test_vector_16_key[] = {
      ...
      0x6a, 0xc4, 0x3f, 0xb3, 0x60, 0x9a, 0xc1, 0xc8, 0x0f, 0xee, 0x1b,
      0x62, 0x44, 0x97, 0xef, 0x94, 0x2e, 0x2f, 0x79, 0xa8, 0x23};
  const uint8_t test_vector_16_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9,
                                          0xe3, 0x37, 0x24, 0xc6,
                                          0x89, 0x32, 0xd6, 0x12};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_16_key) / sizeof(uint8_t),
      test_vector_16_nonce,
      sizeof(test_vector_16_nonce) / sizeof(uint8_t),
      test_vector_16_aad, sizeof(test_vector_16_aad) / sizeof(uint8_t),
      test_vector_16_plaintext,
  ...
  const uint8_t test_vector_17_key[] = {
      0x88, 0xee, 0x08, 0x7f, 0xd9, 0x5d, 0xa9, 0xfb,
      0xf6, 0x72, 0x5a, 0xa9, 0xd7, 0x57, 0xb0, 0xcd};
  const uint8_t test_vector_17_nonce[] = {0x7a, 0xe8, 0xe2, 0xca,
                                          0x4e, 0xc5, 0x00, 0x01,
                                          0x2e, 0x58, 0x49, 0x5c};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_17_key) / sizeof(uint8_t),
      test_vector_17_nonce,
      sizeof(test_vector_17_nonce) / sizeof(uint8_t),
      test_vector_17_aad, sizeof(test_vector_17_aad) / sizeof(uint8_t),
  ...
  const uint8_t test_vector_18_nonce[] = {0x7a, 0xe8, 0xe2, 0xca,
                                          0x4e, 0xc5, 0x00, 0x01,
                                          0x2e, 0x58, 0x49, 0x5c};
  ...
  gsec_aead_malloc_test_vector(
      ...
      sizeof(test_vector_18_key) / sizeof(uint8_t),
      test_vector_18_nonce,
      sizeof(test_vector_18_nonce) / sizeof(uint8_t),
      test_vector_18_aad, sizeof(test_vector_18_aad) / sizeof(uint8_t),
  ...
  const uint8_t test_vector_19_key[] = {
      0x88, 0xee, 0x08, 0x7f, 0xd9, 0x5d, 0xa9, 0xfb,
      0xf6, 0x72, 0x5a, 0xa9, 0xd7, 0x57, 0xb0, 0xcd};
  const uint8_t test_vector_19_nonce[] = {0x7a, 0xe8, 0xe2, 0xca,
                                          0x4e, 0xc5, 0x00, 0x01,
                                          0x2e, 0x58, 0x49, 0x5c};
  ...
  const uint8_t test_vector_19_ciphertext_and_tag[] = {
      0xc3, 0x1f, 0x53, 0xd9, 0x9e, 0x56, 0x87, 0xf7, 0x36, 0x51,
      0x19, 0xb8, 0x32, 0xd2, 0xaa, 0xe7, 0x07, 0x41, 0xd5, 0x93,
      0xf1, 0xf9, 0xe2, 0xab, 0x34, 0x55, 0x77, 0x9b, 0x07, 0x8e,
      0xb8, 0xfe, 0xac, 0xdf, 0xec, 0x1f, 0x8e, 0x3e, 0x52, 0x77,
      0xf8, 0x18, 0x0b, 0x43, 0x36, 0x1f, 0x65, 0x12, 0xad, 0xb1,
      0x6d, 0x2e, 0x38, 0x54, 0x8a, 0x2c, 0x71, 0x9d, 0xba, 0x72,
      0x28, 0xd8, 0x40, 0x88, 0xf8, 0x75, 0x7a, 0xdb, 0x8a, 0xa7,
      0x88, 0xd8, 0xf6, 0x5a, 0xd6, 0x68, 0xbe, 0x70, 0xe7};
  gsec_aead_malloc_test_vector(
      &test_vector_19, test_vector_19_key,
      sizeof(test_vector_19_key) / sizeof(uint8_t),
      test_vector_19_nonce,
      sizeof(test_vector_19_nonce) / sizeof(uint8_t),
      test_vector_19_aad, sizeof(test_vector_19_aad) / sizeof(uint8_t),
      test_vector_19_plaintext,
  ...
  const uint8_t test_vector_20_key[] = {
      ...
      0xb8, 0x9e, 0x5c, 0x15, 0x51, 0x1f, 0xce, 0xd9, 0x21, 0x64, 0x90,
      0xfb, 0x1c, 0x1a, 0x2c, 0xaa, 0x0f, 0xfe, 0x04, 0x07, 0xe5};
  const uint8_t test_vector_20_nonce[] = {0x7a, 0xe8, 0xe2, 0xca,
                                          0x4e, 0xc5, 0x00, 0x01,
                                          0x2e, 0x58, 0x49, 0x5c};
  ...
  const uint8_t test_vector_20_ciphertext_and_tag[] = {
      0xba, 0x8a, 0xe3, 0x1b, 0xc5, 0x06, 0x48, 0x6d, 0x68, 0x73,
      0xe4, 0xfc, 0xe4, 0x60, 0xe7, 0xdc, 0x57, 0x59, 0x1f, 0xf0,
      0x06, 0x11, 0xf3, 0x1c, 0x38, 0x34, 0xfe, 0x1c, 0x04, 0xad,
      0x80, 0xb6, 0x68, 0x03, 0xaf, 0xcf, 0x5b, 0x27, 0xe6, 0x33,
      0x3f, 0xa6, 0x7c, 0x99, 0xda, 0x47, 0xc2, 0xf0, 0xce, 0xd6,
      0x8d, 0x53, 0x1b, 0xd7, 0x41, 0xa9, 0x43, 0xcf, 0xf7, 0xa6,
      0x71, 0x3b, 0xd0, 0x26, 0x11, 0xcd, 0x7d, 0xaa, 0x01, 0xd6,
      0x1c, 0x5c, 0x88, 0x6d, 0xc1, 0xa8, 0x17, 0x01, 0x07};
  gsec_aead_malloc_test_vector(
      &test_vector_20, test_vector_20_key,
      sizeof(test_vector_20_key) / sizeof(uint8_t),
      test_vector_20_nonce,
      sizeof(test_vector_20_nonce) / sizeof(uint8_t),
      test_vector_20_aad, sizeof(test_vector_20_aad) / sizeof(uint8_t),
      test_vector_20_plaintext,
