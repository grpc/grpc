...
static void gsec_randomly_slice(uint8_t* input, size_t input_length, struct iovec** output,
  ...
  *output = static_cast<struct iovec*>(malloc(*output_length * sizeof(**output)));
  size_t i;
...
static void gsec_test_random_encrypt_decrypt(gsec_aead_crypter* crypter, size_t aad_length,
  ...
  uint8_t* ciphertext_and_tag = static_cast<uint8_t*>(gpr_malloc(ciphertext_and_tag_length));
  ...
  gsec_aead_crypter_max_plaintext_length(crypter, ciphertext_bytes_written, &plaintext_length,
                                         nullptr);
  uint8_t* plaintext = static_cast<uint8_t*>(gpr_malloc(plaintext_length));
  grpc_status_code status = gsec_aead_crypter_decrypt(
      crypter, nonce, nonce_length, aad, aad_length, ciphertext_and_tag, ciphertext_bytes_written,
      plaintext, plaintext_length, &plaintext_bytes_written, nullptr);
  ...
  if (tag_length >= kTestMinTagLengthForCorruption) {
    ...
    if (nonce_length > 0) {
      ...
      status =
          gsec_aead_crypter_decrypt(crypter, corrupt_nonce, nonce_length, aad, aad_length,
                                    ciphertext_and_tag, ciphertext_bytes_written, plaintext,
                                    plaintext_length, &plaintext_bytes_written, &error_message);

      GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_FAILED_PRECONDITION,
                                                          "Checking tag failed.", error_message));
    ...
    gsec_test_copy_and_alter_random_byte(ciphertext_and_tag, &corrupt_ciphertext_and_tag,
                                         ciphertext_and_tag_length);
    status =
        gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, aad, aad_length,
                                  corrupt_ciphertext_and_tag, ciphertext_bytes_written, plaintext,
                                  plaintext_length, &plaintext_bytes_written, &error_message);

    GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_FAILED_PRECONDITION,
                                                        error_message, "Checking tag failed"));
    GPR_ASSERT(plaintext_bytes_written == 0);
    GPR_ASSERT(memcmp(zero_message, plaintext, plaintext_length) == 0);
    gpr_free(error_message);
    ...
    plaintext_bytes_written = 0;
    gsec_test_copy(ciphertext_and_tag, &corrupt_ciphertext_and_tag, ciphertext_and_tag_length);
...
static void gsec_test_multiple_random_encrypt_decrypt(gsec_aead_crypter* crypter,
                                                      size_t* aad_lengths, size_t* message_lengths,
                                                      size_t count) {
  GPR_ASSERT(crypter != nullptr);
  size_t nonce_length, tag_length;
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    size_t message_length = (message_lengths == nullptr) ? 0 : message_lengths[ind];
  ...
  size_t* ciphertext_and_tag_lengths = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  size_t* ciphertext_bytes_writtens = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  size_t* plaintext_lengths = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  size_t* plaintext_bytes_writtens = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  uint8_t** ciphertext_and_tags = static_cast<uint8_t**>(gpr_malloc(sizeof(uint8_t*) * count));
  uint8_t** plaintexts = static_cast<uint8_t**>(gpr_malloc(sizeof(uint8_t*) * count));
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    size_t message_length = (message_lengths == nullptr) ? 0 : message_lengths[ind];
    gsec_aead_crypter_max_ciphertext_and_tag_length(crypter, message_length,
                                                    &(ciphertext_and_tag_lengths[ind]), nullptr);
    ciphertext_and_tags[ind] = static_cast<uint8_t*>(gpr_malloc(ciphertext_and_tag_lengths[ind]));
    grpc_status_code status = gsec_aead_crypter_encrypt(
        crypter, nonces[ind], nonce_length, aads[ind], aad_length, messages[ind], message_length,
        ciphertext_and_tags[ind], ciphertext_and_tag_lengths[ind],
        &(ciphertext_bytes_writtens[ind]), nullptr);
    GPR_ASSERT(status == GRPC_STATUS_OK);
    GPR_ASSERT(message_length + tag_length == ciphertext_and_tag_lengths[ind]);
    GPR_ASSERT(ciphertext_bytes_writtens[ind] == ciphertext_and_tag_lengths[ind]);
  }
  /* Do Decryption */
  for (ind = 0; ind < count; ind++) {
    size_t aad_length = (aad_lengths == nullptr) ? 0 : aad_lengths[ind];
    size_t message_length = (message_lengths == nullptr) ? 0 : message_lengths[ind];
    gsec_aead_crypter_max_plaintext_length(crypter, ciphertext_bytes_writtens[ind],
                                           &(plaintext_lengths[ind]), nullptr);
    plaintexts[ind] = static_cast<uint8_t*>(gpr_malloc(plaintext_lengths[ind]));
    grpc_status_code status = gsec_aead_crypter_decrypt(
        crypter, nonces[ind], nonce_length, aads[ind], aad_length, ciphertext_and_tags[ind],
        ciphertext_bytes_writtens[ind], plaintexts[ind], plaintext_lengths[ind],
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    gsec_randomly_slice(messages[ind], message_length, &message_vecs, &message_vecs_length);

    size_t ciphertext_length = ciphertext_and_tag_lengths[ind];
    uint8_t* another_ciphertext = static_cast<uint8_t*>(malloc(ciphertext_length));
    struct iovec another_ciphertext_vec = {another_ciphertext, ciphertext_length};

    char* error_details = nullptr;
    size_t ciphertext_bytes_written = 0;
    gsec_assert_ok(gsec_aead_crypter_encrypt_iovec(crypter, nonces[ind], nonce_length, aad_vecs,
                                                   aad_vecs_length, message_vecs,
                                                   message_vecs_length, another_ciphertext_vec,
                                                   &ciphertext_bytes_written, &error_details),
                   error_details);
    GPR_ASSERT(
        memcmp(ciphertext_and_tags[ind], another_ciphertext_vec.iov_base, ciphertext_length) == 0);
  ...
  for (ind = 0; ind < count; ind++) {
    ...
    gsec_randomly_slice(ciphertext_and_tags[ind], ciphertext_bytes_writtens[ind], &ciphertext_vecs,
    ...
    GPR_ASSERT(memcmp(decrypted_vec.iov_base, messages[ind], message_length) == 0);
    free(decrypted);
    free(aad_vecs);
...
static void gsec_test_multiple_encrypt_decrypt(gsec_aead_crypter* crypter) {
  ...
  size_t* aad_lengths = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  size_t* message_lengths = static_cast<size_t*>(gpr_malloc(sizeof(size_t) * count));
  size_t ind;
  for (ind = 0; ind < count; ind++) {
  ...
  gsec_test_multiple_random_encrypt_decrypt(crypter, aad_lengths, message_lengths, count);
  gsec_test_multiple_random_encrypt_decrypt(crypter, aad_lengths, nullptr, count);
  gsec_test_multiple_random_encrypt_decrypt(crypter, nullptr, message_lengths, count);
...
static void gsec_test_encryption_failure(gsec_aead_crypter* crypter) {
  ...
  /* nullptr nonce */
  grpc_status_code status = gsec_aead_crypter_encrypt(
      crypter, nullptr, nonce_length, aad, aad_length, message, message_length, ciphertext_and_tag,
      ciphertext_and_tag_length, &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Nonce buffer is nullptr."));
  gpr_free(error_message);

  /* Big nonce */
  status = gsec_aead_crypter_encrypt(crypter, nonce, nonce_length + 1, aad, aad_length, message,
                                     message_length, ciphertext_and_tag, ciphertext_and_tag_length,
                                     &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Nonce buffer has the wrong length."));
  gpr_free(error_message);
  ...
  /* nullptr aad */
  status = gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, nullptr, aad_length, message,
                                     message_length, ciphertext_and_tag, ciphertext_and_tag_length,
                                     &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "aad is nullptr."));
  gpr_free(error_message);

  /* nullptr aad with zero length */
  gsec_assert_ok(
      gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, nullptr, 0, message, message_length,
                                ciphertext_and_tag, ciphertext_and_tag_length,
                                &ciphertext_bytes_written, &error_message),
      error_message);

  /* nullptr plaintext */
  status = gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, aad, aad_length, nullptr,
                                     message_length, ciphertext_and_tag, ciphertext_and_tag_length,
                                     &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "plaintext is nullptr."));
  gpr_free(error_message);

  /* nullptr ciphertext */
  status = gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, aad, aad_length, message,
                                     message_length, nullptr, ciphertext_and_tag_length,
                                     &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "ciphertext is nullptr."));
  gpr_free(error_message);

  /* Short ciphertext */
  status = gsec_aead_crypter_encrypt(
      crypter, nonce, nonce_length, aad, aad_length, message, message_length, ciphertext_and_tag,
      ciphertext_and_tag_length - 1, &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message,
  ...
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "bytes_written is nullptr."));
  gpr_free(error_message);

  /* nullptr plaintext/ciphertext encrypt with zero length */
  gsec_assert_ok(gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, aad, aad_length, nullptr,
                                           0, ciphertext_and_tag, ciphertext_and_tag_length,
                                           &ciphertext_bytes_written, &error_message),
                 error_message);
...
static void gsec_test_decryption_failure(gsec_aead_crypter* crypter) {
  ...
  char* error_message;
  /* nullptr nonce */
  status = gsec_aead_crypter_decrypt(crypter, nullptr, nonce_length, aad, aad_length,
                                     ciphertext_and_tag, ciphertext_and_tag_length, plaintext,
                                     plaintext_length, &plaintext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "Nonce buffer is nullptr."));
  gpr_free(error_message);
  ...
  /* Big nonce */
  status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length + 1, aad, aad_length,
                                     ciphertext_and_tag, ciphertext_and_tag_length, plaintext,
                                     plaintext_length, &plaintext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(
      status, GRPC_STATUS_INVALID_ARGUMENT, error_message, "Nonce buffer has the wrong length."));
  ...
  /* nullptr aad */
  status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, nullptr, aad_length,
                                     ciphertext_and_tag, ciphertext_and_tag_length, plaintext,
                                     plaintext_length, &plaintext_bytes_written, &error_message);
  GPR_ASSERT(gsec_test_expect_compare_code_and_substr(status, GRPC_STATUS_INVALID_ARGUMENT,
                                                      error_message, "aad is nullptr."));
  ...
  /* nullptr aad with zero length */
  status = gsec_aead_crypter_encrypt(crypter, nonce, nonce_length, nullptr, 0, message,
                                     message_length, ciphertext_and_tag, ciphertext_and_tag_length,
                                     &ciphertext_bytes_written, &error_message);
  GPR_ASSERT(status == GRPC_STATUS_OK);

  status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, nullptr, 0, ciphertext_and_tag,
  ...
  /* Small ciphertext */
  if (tag_length > 0) {
    status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, aad, aad_length,
                                       ciphertext_and_tag, tag_length - 1, plaintext,
                                       plaintext_length, &plaintext_bytes_written, &error_message);
  ...
  /* nullptr ciphertext */
  status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, aad, aad_length, nullptr,
                                     ciphertext_and_tag_length, plaintext, plaintext_length,
                                     &plaintext_bytes_written, &error_message);
  ...
  /* nullptr plaintext */
  status = gsec_aead_crypter_decrypt(crypter, nonce, nonce_length, aad, aad_length,
                                     ciphertext_and_tag, ciphertext_and_tag_length, nullptr,
                                     plaintext_length, &plaintext_bytes_written, &error_message);
  ...
  status = gsec_aead_crypter_decrypt(
      crypter, nonce, nonce_length, aad, aad_length, ciphertext_and_tag, ciphertext_and_tag_length,
      plaintext, plaintext_length - 1, &plaintext_bytes_written, &error_message);
...
static void gsec_test_encrypt_decrypt_test_vector(gsec_aead_crypter* crypter,
  ...
  size_t ciphertext_and_tag_length, ciphertext_bytes_written = 0;
  gsec_aead_crypter_max_ciphertext_and_tag_length(crypter, test_vector->plaintext_length,
                                                  &ciphertext_and_tag_length, nullptr);
  uint8_t* ciphertext_and_tag_bytes = static_cast<uint8_t*>(gpr_malloc(ciphertext_and_tag_length));
  grpc_status_code status = gsec_aead_crypter_encrypt(
      crypter, test_vector->nonce, test_vector->nonce_length, test_vector->aad,
      test_vector->aad_length, test_vector->plaintext, test_vector->plaintext_length,
      ciphertext_and_tag_bytes, ciphertext_and_tag_length, &ciphertext_bytes_written, nullptr);
  ...
  size_t plaintext_length, plaintext_bytes_written = 0;
  gsec_aead_crypter_max_plaintext_length(crypter, ciphertext_and_tag_length, &plaintext_length,
                                         nullptr);
  uint8_t* plaintext_bytes = static_cast<uint8_t*>(gpr_malloc(plaintext_length));
  status = gsec_aead_crypter_decrypt(crypter, test_vector->nonce, test_vector->nonce_length,
                                     test_vector->aad, test_vector->aad_length,
                                     test_vector->ciphertext_and_tag,
                                     test_vector->ciphertext_and_tag_length, plaintext_bytes,
                                     plaintext_length, &plaintext_bytes_written, nullptr);
  GPR_ASSERT(status == GRPC_STATUS_OK);
  GPR_ASSERT(memcmp(test_vector->plaintext, plaintext_bytes, plaintext_bytes_written) == 0);
...
static void gsec_test_get_crypter_from_test_vector(gsec_aead_crypter** crypter,
  ...
  GPR_ASSERT(key_length == kAes128GcmKeyLength || key_length == kAes256GcmKeyLength ||
  ...
  gsec_aes_gcm_aead_crypter_create(test_vector->key, key_length, nonce_length, tag_length, rekey,
                                   crypter, nullptr);
}
...
static void gsec_aead_malloc_test_vector(gsec_aead_test_vector** test_vector, const uint8_t* key,
                                         size_t key_length, const uint8_t* nonce,
                                         size_t nonce_length, const uint8_t* aad, size_t aad_length,
                                         const uint8_t* plaintext, size_t plaintext_length,
                                         const uint8_t* ciphertext_and_tag,
                                         size_t ciphertext_and_tag_length) {
  *test_vector = static_cast<gsec_aead_test_vector*>(gpr_malloc(sizeof(gsec_aead_test_vector)));
...
static void gsec_test_create_random_aes_gcm_crypter(gsec_aead_crypter** crypter, size_t key_length,
                                                    size_t nonce_length, size_t tag_length,
                                                    bool rekey) {
  uint8_t* key;
  gsec_test_random_array(&key, key_length);
  gsec_aes_gcm_aead_crypter_create(key, key_length, nonce_length, tag_length, rekey, crypter,
                                   nullptr);
  gpr_free(key);
}
...
static void gsec_test_get_random_aes_gcm_crypters(gsec_aead_crypter*** crypters) {
  *crypters =
  ...
  gsec_test_create_random_aes_gcm_crypter(&((*crypters)[0]), kAes128GcmKeyLength,
                                          kAesGcmNonceLength, kAesGcmTagLength, /*rekey=*/false);
  gsec_test_create_random_aes_gcm_crypter(&((*crypters)[1]), kAes256GcmKeyLength,
                                          kAesGcmNonceLength, kAesGcmTagLength, /*rekey=*/false);
  gsec_test_create_random_aes_gcm_crypter(&((*crypters)[2]), kAes128GcmRekeyKeyLength,
                                          kAesGcmNonceLength, kAesGcmTagLength, /*rekey=*/true);
}
...
static void gsec_test_do_vector_tests_rekey_nist() {
  ...
  uint8_t nonce_0[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
  uint8_t aad_0[1] = {};
  uint8_t key_0[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                     0x0, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1,
                     0x1, 0x1, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2};
  ...
  uint8_t nonce_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
  uint8_t aad_1[1] = {};
  uint8_t key_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                     0x0, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1,
                     0x1, 0x1, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2};
  uint8_t plaintext_1[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                           0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
  uint8_t ciphertext_1[] = {0x51, 0xE9, 0xA8, 0xCB, 0x23, 0xCA, 0x25, 0x12, 0xC8, 0x25, 0x6A,
                            0xFF, 0xF8, 0xE7, 0x2D, 0x68, 0x1A, 0xCA, 0x19, 0xA1, 0x14, 0x8A,
                            0xC1, 0x15, 0xE8, 0x3D, 0xF4, 0x88, 0x8C, 0xC0, 0xD,  0x11};
  vec = {nonce_1, aad_1, key_1, plaintext_1, ciphertext_1, 12, 0, 44, 16, 32};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from NIST test vector 3
  uint8_t nonce_2[] = {0xCA, 0xFE, 0xBA, 0xBE, 0xFA, 0xCE, 0xDB, 0xAD, 0xDE, 0xCA, 0xF8, 0x88};
  uint8_t aad_2[1] = {};
  uint8_t key_2[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_2[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,
  ...
  uint8_t ciphertext_2[] = {0x10, 0x18, 0xED, 0x5A, 0x14, 0x2,  0xA8, 0x65, 0x16, 0xD6, 0x57, 0x6D,
  ...
  uint8_t nonce_3[] = {0xCA, 0xFE, 0xBA, 0xBE, 0xFA, 0xCE, 0xDB, 0xAD, 0xDE, 0xCA, 0xF8, 0x88};
  uint8_t aad_3[] = {0xFE, 0xED, 0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED,
                     0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xAB, 0xAD, 0xDA, 0xD2};
  uint8_t key_3[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_3[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,  0xC5,
  ...
  uint8_t ciphertext_3[] = {0x10, 0x18, 0xED, 0x5A, 0x14, 0x2,  0xA8, 0x65, 0x16, 0xD6, 0x57,
  ...
  // nonce bit 15)
  uint8_t nonce_4[] = {0xCA, 0x7E, 0xBA, 0xBE, 0xFA, 0xCE, 0xDB, 0xAD, 0xDE, 0xCA, 0xF8, 0x88};
  uint8_t aad_4[] = {0xFE, 0xED, 0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED,
                     0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xAB, 0xAD, 0xDA, 0xD2};
  uint8_t key_4[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_4[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,  0xC5,
                           0xAF, 0xF5, 0x26, 0x9A, 0x86, 0xA7, 0xA9, 0x53, 0x15, 0x34, 0xF7, 0xDA,
                           0x2E, 0x4C, 0x30, 0x3D, 0x8A, 0x31, 0x8A, 0x72, 0x1C, 0x3C, 0xC,  0x95,
                           0x95, 0x68, 0x9,  0x53, 0x2F, 0xCF, 0xE,  0x24, 0x49, 0xA6, 0xB5, 0x25,
                           0xB1, 0x6A, 0xED, 0xF5, 0xAA, 0xD,  0xE6, 0x57, 0xBA, 0x63, 0x7B, 0x39};
  uint8_t ciphertext_4[] = {0xE6, 0x50, 0xD3, 0xC0, 0xFB, 0x87, 0x93, 0x27, 0xF2, 0xD0, 0x32,
  ...
  // Derived from adapted NIST test vector 4 for KDF counter boundary (flip
  // nonce bit 16)
  uint8_t nonce_5[] = {0xCA, 0xFE, 0xBB, 0xBE, 0xFA, 0xCE, 0xDB, 0xAD, 0xDE, 0xCA, 0xF8, 0x88};
  uint8_t aad_5[] = {0xFE, 0xED, 0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED,
                     0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xAB, 0xAD, 0xDA, 0xD2};
  uint8_t key_5[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_5[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,  0xC5,
                           0xAF, 0xF5, 0x26, 0x9A, 0x86, 0xA7, 0xA9, 0x53, 0x15, 0x34, 0xF7, 0xDA,
                           0x2E, 0x4C, 0x30, 0x3D, 0x8A, 0x31, 0x8A, 0x72, 0x1C, 0x3C, 0xC,  0x95,
                           0x95, 0x68, 0x9,  0x53, 0x2F, 0xCF, 0xE,  0x24, 0x49, 0xA6, 0xB5, 0x25,
                           0xB1, 0x6A, 0xED, 0xF5, 0xAA, 0xD,  0xE6, 0x57, 0xBA, 0x63, 0x7B, 0x39};
  uint8_t ciphertext_5[] = {0xC0, 0x12, 0x1E, 0x6C, 0x95, 0x4D, 0x7,  0x67, 0xF9, 0x66, 0x30,
  ...
  // Derived from adapted NIST test vector 4 for KDF counter boundary (flip
  // nonce bit 63)
  uint8_t nonce_6[] = {0xCA, 0xFE, 0xBA, 0xBE, 0xFA, 0xCE, 0xDB, 0x2D, 0xDE, 0xCA, 0xF8, 0x88};
  uint8_t aad_6[] = {0xFE, 0xED, 0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED,
                     0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xAB, 0xAD, 0xDA, 0xD2};
  uint8_t key_6[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_6[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,  0xC5,
                           0xAF, 0xF5, 0x26, 0x9A, 0x86, 0xA7, 0xA9, 0x53, 0x15, 0x34, 0xF7, 0xDA,
                           0x2E, 0x4C, 0x30, 0x3D, 0x8A, 0x31, 0x8A, 0x72, 0x1C, 0x3C, 0xC,  0x95,
                           0x95, 0x68, 0x9,  0x53, 0x2F, 0xCF, 0xE,  0x24, 0x49, 0xA6, 0xB5, 0x25,
                           0xB1, 0x6A, 0xED, 0xF5, 0xAA, 0xD,  0xE6, 0x57, 0xBA, 0x63, 0x7B, 0x39};
  uint8_t ciphertext_6[] = {0x8A, 0xF3, 0x7E, 0xA5, 0x68, 0x4A, 0x4D, 0x81, 0xD4, 0xFD, 0x81,
  ...
  // Derived from adapted NIST test vector 4 for KDF counter boundary (flip
  // nonce bit 64)
  uint8_t nonce_7[] = {0xCA, 0xFE, 0xBA, 0xBE, 0xFA, 0xCE, 0xDB, 0xAD, 0xDF, 0xCA, 0xF8, 0x88};
  uint8_t aad_7[] = {0xFE, 0xED, 0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED,
                     0xFA, 0xCE, 0xDE, 0xAD, 0xBE, 0xEF, 0xAB, 0xAD, 0xDA, 0xD2};
  uint8_t key_7[] = {0xFE, 0xFF, 0xE9, 0x92, 0x86, 0x65, 0x73, 0x1C, 0x6D, 0x6A, 0x8F,
                     0x94, 0x67, 0x30, 0x83, 0x8,  0xFF, 0xFE, 0xE8, 0x93, 0x87, 0x64,
                     0x72, 0x1D, 0x6C, 0x6B, 0x8E, 0x95, 0x66, 0x31, 0x82, 0x9,  0xFC,
                     0xFD, 0xEB, 0x90, 0x84, 0x67, 0x71, 0x1E, 0x6F, 0x68, 0x8D, 0x96};
  uint8_t plaintext_7[] = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x6,  0xE5, 0xA5, 0x59, 0x9,  0xC5,
                           0xAF, 0xF5, 0x26, 0x9A, 0x86, 0xA7, 0xA9, 0x53, 0x15, 0x34, 0xF7, 0xDA,
                           0x2E, 0x4C, 0x30, 0x3D, 0x8A, 0x31, 0x8A, 0x72, 0x1C, 0x3C, 0xC,  0x95,
                           0x95, 0x68, 0x9,  0x53, 0x2F, 0xCF, 0xE,  0x24, 0x49, 0xA6, 0xB5, 0x25,
                           0xB1, 0x6A, 0xED, 0xF5, 0xAA, 0xD,  0xE6, 0x57, 0xBA, 0x63, 0x7B, 0x39};
  uint8_t ciphertext_7[] = {0xFB, 0xD5, 0x28, 0x44, 0x8D, 0x3,  0x46, 0xBF, 0xA8, 0x78, 0x63,
...
static void gsec_test_do_vector_tests_rekey_ieee() {
  ...
  uint8_t nonce_8[] = {0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81, 0xB2, 0xC2, 0x84, 0x65};
  uint8_t aad_8[] = {0xD6, 0x9,  0xB1, 0xF0, 0x56, 0x63, 0x7A, 0xD,  0x46, 0xDF, 0x99, 0x8D,
                     0x88, 0xE5, 0x22, 0x2A, 0xB2, 0xC2, 0x84, 0x65, 0x12, 0x15, 0x35, 0x24,
                     0xC0, 0x89, 0x5E, 0x81, 0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14,
                     0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
                     0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
                     0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x0,  0x1};
  uint8_t key_8[] = {0xAD, 0x7A, 0x2B, 0xD0, 0x3E, 0xAC, 0x83, 0x5A, 0x6F, 0x62, 0xF,
                     0xDC, 0xB5, 0x6,  0xB3, 0x45, 0xAC, 0x7B, 0x2A, 0xD1, 0x3F, 0xAD,
                     0x82, 0x5B, 0x6E, 0x63, 0xE,  0xDD, 0xB4, 0x7,  0xB2, 0x44, 0xAF,
                     0x78, 0x29, 0xD2, 0x3C, 0xAE, 0x81, 0x58, 0x6D, 0x60, 0xD,  0xDE};
  ...
  uint8_t nonce_9[] = {0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81, 0xB2, 0xC2, 0x84, 0x65};
  uint8_t aad_9[] = {0xD6, 0x9,  0xB1, 0xF0, 0x56, 0x63, 0x7A, 0xD,  0x46, 0xDF, 0x99, 0x8D,
                     0x88, 0xE5, 0x22, 0x2A, 0xB2, 0xC2, 0x84, 0x65, 0x12, 0x15, 0x35, 0x24,
                     0xC0, 0x89, 0x5E, 0x81, 0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14,
                     0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
                     0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
                     0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x0,  0x1};
  uint8_t key_9[] = {0xE3, 0xC0, 0x8A, 0x8F, 0x6,  0xC6, 0xE3, 0xAD, 0x95, 0xA7, 0x5,
                     0x57, 0xB2, 0x3F, 0x75, 0x48, 0x3C, 0xE3, 0x30, 0x21, 0xA9, 0xC7,
                     0x2B, 0x70, 0x25, 0x66, 0x62, 0x4,  0xC6, 0x9C, 0xB,  0x72, 0xE1,
                     0xC2, 0x88, 0x8D, 0x4,  0xC4, 0xE1, 0xAF, 0x97, 0xA5, 0x7,  0x55};
  ...
  uint8_t nonce_10[] = {0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81, 0xB2, 0xC2, 0x84, 0x65};
  uint8_t aad_10[] = {0xD6, 0x9,  0xB1, 0xF0, 0x56, 0x63, 0x7A, 0xD,  0x46, 0xDF,
                      0x99, 0x8D, 0x88, 0xE5, 0x2E, 0x0,  0xB2, 0xC2, 0x84, 0x65,
                      0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81};
  uint8_t key_10[] = {0xAD, 0x7A, 0x2B, 0xD0, 0x3E, 0xAC, 0x83, 0x5A, 0x6F, 0x62, 0xF,
                      0xDC, 0xB5, 0x6,  0xB3, 0x45, 0xAC, 0x7B, 0x2A, 0xD1, 0x3F, 0xAD,
                      0x82, 0x5B, 0x6E, 0x63, 0xE,  0xDD, 0xB4, 0x7,  0xB2, 0x44, 0xAF,
                      0x78, 0x29, 0xD2, 0x3C, 0xAE, 0x81, 0x58, 0x6D, 0x60, 0xD,  0xDE};
  uint8_t plaintext_10[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
                            0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24,
                            0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30,
                            0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x0,  0x2};
  uint8_t ciphertext_10[] = {0xDB, 0x3D, 0x25, 0x71, 0x9C, 0x6B, 0xA,  0x3C, 0xA6, 0x14, 0x5C,
  ...
  vec = {nonce_10, aad_10, key_10, plaintext_10, ciphertext_10, 12, 28, 44, 48, 64};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.2.2 60-byte crypt
  uint8_t nonce_11[] = {0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81, 0xB2, 0xC2, 0x84, 0x65};
  uint8_t aad_11[] = {0xD6, 0x9,  0xB1, 0xF0, 0x56, 0x63, 0x7A, 0xD,  0x46, 0xDF,
                      0x99, 0x8D, 0x88, 0xE5, 0x2E, 0x0,  0xB2, 0xC2, 0x84, 0x65,
                      0x12, 0x15, 0x35, 0x24, 0xC0, 0x89, 0x5E, 0x81};
  uint8_t key_11[] = {0xE3, 0xC0, 0x8A, 0x8F, 0x6,  0xC6, 0xE3, 0xAD, 0x95, 0xA7, 0x5,
                      0x57, 0xB2, 0x3F, 0x75, 0x48, 0x3C, 0xE3, 0x30, 0x21, 0xA9, 0xC7,
                      0x2B, 0x70, 0x25, 0x66, 0x62, 0x4,  0xC6, 0x9C, 0xB,  0x72, 0xE1,
                      0xC2, 0x88, 0x8D, 0x4,  0xC4, 0xE1, 0xAF, 0x97, 0xA5, 0x7,  0x55};
  uint8_t plaintext_11[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
                            0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24,
                            0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30,
                            0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x0,  0x2};
  uint8_t ciphertext_11[] = {0x16, 0x41, 0xF2, 0x8E, 0xC1, 0x3A, 0xFC, 0xC8, 0xF7, 0x90, 0x33,
  ...
  vec = {nonce_11, aad_11, key_11, plaintext_11, ciphertext_11, 12, 28, 44, 48, 64};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.3.1 60-byte auth
  uint8_t nonce_12[] = {0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D, 0x0, 0x1, 0x76, 0xD4, 0x57, 0xED};
  uint8_t aad_12[] = {0xE2, 0x1,  0x6,  0xD7, 0xCD, 0xD,  0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D,
  ...
  uint8_t key_12[] = {0x7,  0x1B, 0x11, 0x3B, 0xC,  0xA7, 0x43, 0xFE, 0xCC, 0xCF, 0x3D,
                      0x5,  0x1F, 0x73, 0x73, 0x82, 0x6,  0x1A, 0x10, 0x3A, 0xD,  0xA6,
                      0x42, 0xFF, 0xCD, 0xCE, 0x3C, 0x4,  0x1E, 0x72, 0x72, 0x83, 0x5,
                      0x19, 0x13, 0x39, 0xE,  0xA5, 0x41, 0xFC, 0xCE, 0xCD, 0x3F, 0x7};
  ...
  vec = {nonce_12, aad_12, key_12, plaintext_12, ciphertext_12, 12, 68, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.3.2 60-byte auth
  uint8_t nonce_13[] = {0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D, 0x0, 0x1, 0x76, 0xD4, 0x57, 0xED};
  uint8_t aad_13[] = {0xE2, 0x1,  0x6,  0xD7, 0xCD, 0xD,  0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D,
                      0x88, 0xE5, 0x40, 0x0,  0x76, 0xD4, 0x57, 0xED, 0x8,  0x0,  0xF,  0x10,
                      0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C,
                      0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28,
                      0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
                      0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x0,  0x3};
  uint8_t key_13[] = {0x69, 0x1D, 0x3E, 0xE9, 0x9,  0xD7, 0xF5, 0x41, 0x67, 0xFD, 0x1C,
                      0xA0, 0xB5, 0xD7, 0x69, 0x8,  0x1F, 0x2B, 0xDE, 0x1A, 0xEE, 0x65,
                      0x5F, 0xDB, 0xAB, 0x80, 0xBD, 0x52, 0x95, 0xAE, 0x6B, 0xE7, 0x6B,
                      0x1F, 0x3C, 0xEB, 0xB,  0xD5, 0xF7, 0x43, 0x65, 0xFF, 0x1E, 0xA2};
  ...
  vec = {nonce_13, aad_13, key_13, plaintext_13, ciphertext_13, 12, 68, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.4.1 54-byte crypt
  uint8_t nonce_14[] = {0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D, 0x0, 0x1, 0x76, 0xD4, 0x57, 0xED};
  uint8_t aad_14[] = {0xE2, 0x1,  0x6,  0xD7, 0xCD, 0xD,  0xF0, 0x76, 0x1E, 0x8D,
                      0xCD, 0x3D, 0x88, 0xE5, 0x4C, 0x2A, 0x76, 0xD4, 0x57, 0xED};
  uint8_t key_14[] = {0x7,  0x1B, 0x11, 0x3B, 0xC,  0xA7, 0x43, 0xFE, 0xCC, 0xCF, 0x3D,
                      0x5,  0x1F, 0x73, 0x73, 0x82, 0x6,  0x1A, 0x10, 0x3A, 0xD,  0xA6,
                      0x42, 0xFF, 0xCD, 0xCE, 0x3C, 0x4,  0x1E, 0x72, 0x72, 0x83, 0x5,
                      0x19, 0x13, 0x39, 0xE,  0xA5, 0x41, 0xFC, 0xCE, 0xCD, 0x3F, 0x7};
  uint8_t plaintext_14[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
  ...
  uint8_t ciphertext_14[] = {0xFD, 0x96, 0xB7, 0x15, 0xB9, 0x3A, 0x13, 0x34, 0x6A, 0xF5, 0x1E, 0x8A,
  ...
  vec = {nonce_14, aad_14, key_14, plaintext_14, ciphertext_14, 12, 20, 44, 42, 58};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.4.2 54-byte crypt
  uint8_t nonce_15[] = {0xF0, 0x76, 0x1E, 0x8D, 0xCD, 0x3D, 0x0, 0x1, 0x76, 0xD4, 0x57, 0xED};
  uint8_t aad_15[] = {0xE2, 0x1,  0x6,  0xD7, 0xCD, 0xD,  0xF0, 0x76, 0x1E, 0x8D,
                      0xCD, 0x3D, 0x88, 0xE5, 0x4C, 0x2A, 0x76, 0xD4, 0x57, 0xED};
  uint8_t key_15[] = {0x69, 0x1D, 0x3E, 0xE9, 0x9,  0xD7, 0xF5, 0x41, 0x67, 0xFD, 0x1C,
                      0xA0, 0xB5, 0xD7, 0x69, 0x8,  0x1F, 0x2B, 0xDE, 0x1A, 0xEE, 0x65,
                      0x5F, 0xDB, 0xAB, 0x80, 0xBD, 0x52, 0x95, 0xAE, 0x6B, 0xE7, 0x6B,
                      0x1F, 0x3C, 0xEB, 0xB,  0xD5, 0xF7, 0x43, 0x65, 0xFF, 0x1E, 0xA2};
  uint8_t plaintext_15[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                            0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22,
                            0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,
                            0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x0,  0x4};
  uint8_t ciphertext_15[] = {0xB6, 0x8F, 0x63, 0x0,  0xC2, 0xE9, 0xAE, 0x83, 0x3B, 0xDC, 0x7,  0xE,
  ...
  vec = {nonce_15, aad_15, key_15, plaintext_15, ciphertext_15, 12, 20, 44, 42, 58};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.5.1 65-byte auth
  uint8_t nonce_16[] = {0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6, 0x89, 0x32, 0xD6, 0x12};
  uint8_t aad_16[] = {0x84, 0xC5, 0xD5, 0x13, 0xD2, 0xAA, 0xF6, 0xE5, 0xBB, 0xD2, 0x72, 0x77,
  ...
  uint8_t key_16[] = {0x1,  0x3F, 0xE0, 0xB,  0x5F, 0x11, 0xBE, 0x7F, 0x86, 0x6D, 0xC,
                      0xBB, 0xC5, 0x5A, 0x7A, 0x90, 0x0,  0x3E, 0xE1, 0xA,  0x5E, 0x10,
                      0xBF, 0x7E, 0x87, 0x6C, 0xD,  0xBA, 0xC4, 0x5B, 0x7B, 0x91, 0x3,
                      0x3D, 0xE2, 0x9,  0x5D, 0x13, 0xBC, 0x7D, 0x84, 0x6F, 0xE,  0xB9};
  ...
  vec = {nonce_16, aad_16, key_16, plaintext_16, ciphertext_16, 12, 81, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.5.2 65-byte auth
  uint8_t nonce_17[] = {0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6, 0x89, 0x32, 0xD6, 0x12};
  uint8_t aad_17[] = {0x84, 0xC5, 0xD5, 0x13, 0xD2, 0xAA, 0xF6, 0xE5, 0xBB, 0xD2, 0x72, 0x77,
                      0x88, 0xE5, 0x23, 0x0,  0x89, 0x32, 0xD6, 0x12, 0x7C, 0xFD, 0xE9, 0xF9,
                      0xE3, 0x37, 0x24, 0xC6, 0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14,
                      0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
                      0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
                      0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
                      0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x0,  0x5};
  uint8_t key_17[] = {0x83, 0xC0, 0x93, 0xB5, 0x8D, 0xE7, 0xFF, 0xE1, 0xC0, 0xDA, 0x92,
                      0x6A, 0xC4, 0x3F, 0xB3, 0x60, 0x9A, 0xC1, 0xC8, 0xF,  0xEE, 0x1B,
                      0x62, 0x44, 0x97, 0xEF, 0x94, 0x2E, 0x2F, 0x79, 0xA8, 0x23, 0x81,
                      0xC2, 0x91, 0xB7, 0x8F, 0xE5, 0xFD, 0xE3, 0xC2, 0xD8, 0x90, 0x68};
  ...
  vec = {nonce_17, aad_17, key_17, plaintext_17, ciphertext_17, 12, 81, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE  2.6.1 61-byte crypt
  uint8_t nonce_18[] = {0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6, 0x89, 0x32, 0xD6, 0x12};
  uint8_t aad_18[] = {0x84, 0xC5, 0xD5, 0x13, 0xD2, 0xAA, 0xF6, 0xE5, 0xBB, 0xD2,
                      0x72, 0x77, 0x88, 0xE5, 0x2F, 0x0,  0x89, 0x32, 0xD6, 0x12,
                      0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6};
  uint8_t key_18[] = {0x1,  0x3F, 0xE0, 0xB,  0x5F, 0x11, 0xBE, 0x7F, 0x86, 0x6D, 0xC,
                      0xBB, 0xC5, 0x5A, 0x7A, 0x90, 0x0,  0x3E, 0xE1, 0xA,  0x5E, 0x10,
                      0xBF, 0x7E, 0x87, 0x6C, 0xD,  0xBA, 0xC4, 0x5B, 0x7B, 0x91, 0x3,
                      0x3D, 0xE2, 0x9,  0x5D, 0x13, 0xBC, 0x7D, 0x84, 0x6F, 0xE,  0xB9};
  uint8_t plaintext_18[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
                            0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
                            0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
                            0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
                            0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x0,  0x6};
  uint8_t ciphertext_18[] = {0xFF, 0x19, 0x10, 0xD3, 0x5A, 0xD7, 0xE5, 0x65, 0x78, 0x90, 0xC7,
  ...
  vec = {nonce_18, aad_18, key_18, plaintext_18, ciphertext_18, 12, 28, 44, 49, 65};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.6.2 61-byte crypt
  uint8_t nonce_19[] = {0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6, 0x89, 0x32, 0xD6, 0x12};
  uint8_t aad_19[] = {0x84, 0xC5, 0xD5, 0x13, 0xD2, 0xAA, 0xF6, 0xE5, 0xBB, 0xD2,
                      0x72, 0x77, 0x88, 0xE5, 0x2F, 0x0,  0x89, 0x32, 0xD6, 0x12,
                      0x7C, 0xFD, 0xE9, 0xF9, 0xE3, 0x37, 0x24, 0xC6};
  uint8_t key_19[] = {0x83, 0xC0, 0x93, 0xB5, 0x8D, 0xE7, 0xFF, 0xE1, 0xC0, 0xDA, 0x92,
                      0x6A, 0xC4, 0x3F, 0xB3, 0x60, 0x9A, 0xC1, 0xC8, 0xF,  0xEE, 0x1B,
                      0x62, 0x44, 0x97, 0xEF, 0x94, 0x2E, 0x2F, 0x79, 0xA8, 0x23, 0x81,
                      0xC2, 0x91, 0xB7, 0x8F, 0xE5, 0xFD, 0xE3, 0xC2, 0xD8, 0x90, 0x68};
  uint8_t plaintext_19[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
                            0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20,
                            0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
                            0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
                            0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x0,  0x6};
  uint8_t ciphertext_19[] = {0xD,  0xB4, 0xCF, 0x95, 0x6B, 0x5F, 0x97, 0xEC, 0xA4, 0xEA, 0xB8,
  ...
  vec = {nonce_19, aad_19, key_19, plaintext_19, ciphertext_19, 12, 28, 44, 49, 65};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.7.1 79-byte crypt
  uint8_t nonce_20[] = {0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x0, 0x1, 0x2E, 0x58, 0x49, 0x5C};
  uint8_t aad_20[] = {0x68, 0xF2, 0xE7, 0x76, 0x96, 0xCE, 0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x88,
                      0xE5, 0x41, 0x0,  0x2E, 0x58, 0x49, 0x5C, 0x8,  0x0,  0xF,  0x10, 0x11, 0x12,
                      0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F,
                      0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
                      0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39,
                      0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46,
                      0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x0,  0x7};
  uint8_t key_20[] = {0x88, 0xEE, 0x8,  0x7F, 0xD9, 0x5D, 0xA9, 0xFB, 0xF6, 0x72, 0x5A,
                      0xA9, 0xD7, 0x57, 0xB0, 0xCD, 0x89, 0xEF, 0x9,  0x7E, 0xD8, 0x5C,
                      0xA8, 0xFA, 0xF7, 0x73, 0x5B, 0xA8, 0xD6, 0x56, 0xB1, 0xCC, 0x8A,
                      0xEC, 0xA,  0x7D, 0xDB, 0x5F, 0xAB, 0xF9, 0xF4, 0x70, 0x58, 0xAB};
  uint8_t plaintext_20[1] = {};
  uint8_t ciphertext_20[] = {0x81, 0x3F, 0xE,  0x63, 0xF,  0x96, 0xFB, 0x2D,
                             0x3,  0xF,  0x58, 0xD8, 0x3F, 0x5C, 0xDF, 0xD0};
  vec = {nonce_20, aad_20, key_20, plaintext_20, ciphertext_20, 12, 87, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.7.2 79-byte crypt
  uint8_t nonce_21[] = {0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x0, 0x1, 0x2E, 0x58, 0x49, 0x5C};
  uint8_t aad_21[] = {0x68, 0xF2, 0xE7, 0x76, 0x96, 0xCE, 0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x88,
                      0xE5, 0x41, 0x0,  0x2E, 0x58, 0x49, 0x5C, 0x8,  0x0,  0xF,  0x10, 0x11, 0x12,
                      0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F,
                      0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C,
                      0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39,
                      0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46,
                      0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x0,  0x7};
  uint8_t key_21[] = {0x4C, 0x97, 0x3D, 0xBC, 0x73, 0x64, 0x62, 0x16, 0x74, 0xF8, 0xB5,
                      0xB8, 0x9E, 0x5C, 0x15, 0x51, 0x1F, 0xCE, 0xD9, 0x21, 0x64, 0x90,
                      0xFB, 0x1C, 0x1A, 0x2C, 0xAA, 0xF,  0xFE, 0x4,  0x7,  0xE5, 0x4E,
                      0x95, 0x3F, 0xBE, 0x71, 0x66, 0x60, 0x14, 0x76, 0xFA, 0xB7, 0xBA};
  uint8_t plaintext_21[1] = {};
  uint8_t ciphertext_21[] = {0x77, 0xE5, 0xA4, 0x4C, 0x21, 0xEB, 0x7, 0x18,
                             0x8A, 0xAC, 0xBD, 0x74, 0xD1, 0x98, 0xE, 0x97};
  vec = {nonce_21, aad_21, key_21, plaintext_21, ciphertext_21, 12, 87, 44, 0, 16};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.8.1 61-byte crypt
  uint8_t nonce_22[] = {0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x0, 0x1, 0x2E, 0x58, 0x49, 0x5C};
  uint8_t aad_22[] = {0x68, 0xF2, 0xE7, 0x76, 0x96, 0xCE, 0x7A, 0xE8, 0xE2, 0xCA,
                      0x4E, 0xC5, 0x88, 0xE5, 0x4D, 0x0,  0x2E, 0x58, 0x49, 0x5C};
  uint8_t key_22[] = {0x88, 0xEE, 0x8,  0x7F, 0xD9, 0x5D, 0xA9, 0xFB, 0xF6, 0x72, 0x5A,
                      0xA9, 0xD7, 0x57, 0xB0, 0xCD, 0x89, 0xEF, 0x9,  0x7E, 0xD8, 0x5C,
                      0xA8, 0xFA, 0xF7, 0x73, 0x5B, 0xA8, 0xD6, 0x56, 0xB1, 0xCC, 0x8A,
                      0xEC, 0xA,  0x7D, 0xDB, 0x5F, 0xAB, 0xF9, 0xF4, 0x70, 0x58, 0xAB};
  uint8_t plaintext_22[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                            0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22,
                            0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,
  ...
  uint8_t ciphertext_22[] = {0x95, 0x8E, 0xC3, 0xF6, 0xD6, 0xA,  0xFE, 0xDA, 0x99, 0xEF, 0xD8, 0x88,
  ...
  vec = {nonce_22, aad_22, key_22, plaintext_22, ciphertext_22, 12, 20, 44, 63, 79};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);

  // Derived from IEEE 2.8.2 61-byte crypt
  uint8_t nonce_23[] = {0x7A, 0xE8, 0xE2, 0xCA, 0x4E, 0xC5, 0x0, 0x1, 0x2E, 0x58, 0x49, 0x5C};
  uint8_t aad_23[] = {0x68, 0xF2, 0xE7, 0x76, 0x96, 0xCE, 0x7A, 0xE8, 0xE2, 0xCA,
                      0x4E, 0xC5, 0x88, 0xE5, 0x4D, 0x0,  0x2E, 0x58, 0x49, 0x5C};
  uint8_t key_23[] = {0x4C, 0x97, 0x3D, 0xBC, 0x73, 0x64, 0x62, 0x16, 0x74, 0xF8, 0xB5,
                      0xB8, 0x9E, 0x5C, 0x15, 0x51, 0x1F, 0xCE, 0xD9, 0x21, 0x64, 0x90,
                      0xFB, 0x1C, 0x1A, 0x2C, 0xAA, 0xF,  0xFE, 0x4,  0x7,  0xE5, 0x4E,
                      0x95, 0x3F, 0xBE, 0x71, 0x66, 0x60, 0x14, 0x76, 0xFA, 0xB7, 0xBA};
  uint8_t plaintext_23[] = {0x8,  0x0,  0xF,  0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                            0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22,
                            0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,
                            0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
                            0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43,
                            0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x0,  0x8};
  uint8_t ciphertext_23[] = {0xB4, 0x4D, 0x7,  0x20, 0x11, 0xCD, 0x36, 0xD2, 0x72, 0xA9, 0xB7, 0xA9,
  ...
  vec = {nonce_23, aad_23, key_23, plaintext_23, ciphertext_23, 12, 20, 44, 63, 79};
  gsec_test_verify_crypter_on_test_vector(&vec, /*rekey=*/true);
}
...
static void gsec_test_do_vector_tests_nist() {
  ...
  const uint8_t test_vector_1_key[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  const uint8_t test_vector_1_ciphertext_and_tag[] = {0x58, 0xe2, 0xfc, 0xce, 0xfa, 0x7e,
                                                      0x30, 0x61, 0x36, 0x7f, 0x1d, 0x57,
                                                      0xa4, 0xe7, 0x45, 0x5a};
  gsec_aead_malloc_test_vector(&test_vector_1, test_vector_1_key,
  ...
  const uint8_t test_vector_2_key[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t test_vector_2_nonce[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t test_vector_2_aad[1] = {};
  const uint8_t test_vector_2_plaintext[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  gsec_aead_malloc_test_vector(
      &test_vector_2, test_vector_2_key, sizeof(test_vector_2_key) / sizeof(uint8_t),
      test_vector_2_nonce, sizeof(test_vector_2_nonce) / sizeof(uint8_t), test_vector_2_aad, 0,
      test_vector_2_plaintext, sizeof(test_vector_2_plaintext) / sizeof(uint8_t),
      test_vector_2_ciphertext_and_tag, sizeof(test_vector_2_ciphertext_and_tag) / sizeof(uint8_t));
  ...
  const uint8_t test_vector_3_key[] = {0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c,
  ...
  const uint8_t test_vector_3_ciphertext_and_tag[] = {
      ...
      0x3d, 0x58, 0xe0, 0x91, 0x47, 0x3f, 0x59, 0x85, 0x4d, 0x5c, 0x2a, 0xf3, 0x27, 0xcd,
      0x64, 0xa6, 0x2c, 0xf3, 0x5a, 0xbd, 0x2b, 0xa6, 0xfa, 0xb4};
  gsec_aead_malloc_test_vector(
      &test_vector_3, test_vector_3_key, sizeof(test_vector_3_key) / sizeof(uint8_t),
      test_vector_3_nonce, sizeof(test_vector_3_nonce) / sizeof(uint8_t), test_vector_3_aad, 0,
      test_vector_3_plaintext, sizeof(test_vector_3_plaintext) / sizeof(uint8_t),
      test_vector_3_ciphertext_and_tag, sizeof(test_vector_3_ciphertext_and_tag) / sizeof(uint8_t));
  ...
  const uint8_t test_vector_4_key[] = {0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c,
                                       0x6d, 0x6a, 0x8f, 0x94, 0x67, 0x30, 0x83, 0x08};
  const uint8_t test_vector_4_nonce[] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce,
                                         0xdb, 0xad, 0xde, 0xca, 0xf8, 0x88};
  const uint8_t test_vector_4_aad[] = {0xfe, 0xed, 0xfa, 0xce, 0xde, 0xad, 0xbe, 0xef, 0xfe, 0xed,
                                       0xfa, 0xce, 0xde, 0xad, 0xbe, 0xef, 0xab, 0xad, 0xda, 0xd2};
  ...
  const uint8_t test_vector_4_ciphertext_and_tag[] = {
      ...
      0x6a, 0x0a, 0xac, 0x97, 0x3d, 0x58, 0xe0, 0x91, 0x5b, 0xc9, 0x4f, 0xbc, 0x32,
      0x21, 0xa5, 0xdb, 0x94, 0xfa, 0xe9, 0x5a, 0xe7, 0x12, 0x1a, 0x47};
  gsec_aead_malloc_test_vector(
      &test_vector_4, test_vector_4_key, sizeof(test_vector_4_key) / sizeof(uint8_t),
      test_vector_4_nonce, sizeof(test_vector_4_nonce) / sizeof(uint8_t), test_vector_4_aad,
      sizeof(test_vector_4_aad) / sizeof(uint8_t), test_vector_4_plaintext,
      sizeof(test_vector_4_plaintext) / sizeof(uint8_t), test_vector_4_ciphertext_and_tag,
...
static void gsec_test_do_vector_tests_ieee() {
  ...
  const uint8_t test_vector_5_key[] = {0xad, 0x7a, 0x2b, 0xd0, 0x3e, 0xac, 0x83, 0x5a,
  ...
  const uint8_t test_vector_5_ciphertext_and_tag[] = {0xf0, 0x94, 0x78, 0xa9, 0xb0, 0x90,
                                                      0x07, 0xd0, 0x6f, 0x46, 0xe9, 0xb6,
                                                      0xa1, 0xda, 0x25, 0xdd};
  gsec_aead_malloc_test_vector(
      &test_vector_5, test_vector_5_key, sizeof(test_vector_5_key) / sizeof(uint8_t),
      test_vector_5_nonce, sizeof(test_vector_5_nonce) / sizeof(uint8_t), test_vector_5_aad,
      sizeof(test_vector_5_aad) / sizeof(uint8_t), test_vector_5_plaintext, 0,
      test_vector_5_ciphertext_and_tag, sizeof(test_vector_5_ciphertext_and_tag) / sizeof(uint8_t));
  ...
  const uint8_t test_vector_6_key[] = {0xe3, 0xc0, 0x8a, 0x8f, 0x06, 0xc6, 0xe3, 0xad,
  ...
  const uint8_t test_vector_6_ciphertext_and_tag[] = {0x2f, 0x0b, 0xc5, 0xaf, 0x40, 0x9e,
                                                      0x06, 0xd6, 0x09, 0xea, 0x8b, 0x7d,
                                                      0x0f, 0xa5, 0xea, 0x50};
  gsec_aead_malloc_test_vector(
      &test_vector_6, test_vector_6_key, sizeof(test_vector_6_key) / sizeof(uint8_t),
      test_vector_6_nonce, sizeof(test_vector_6_nonce) / sizeof(uint8_t), test_vector_6_aad,
      sizeof(test_vector_6_aad) / sizeof(uint8_t), test_vector_6_plaintext, 0,
      test_vector_6_ciphertext_and_tag, sizeof(test_vector_6_ciphertext_and_tag) / sizeof(uint8_t));
  ...
  const uint8_t test_vector_7_key[] = {0xad, 0x7a, 0x2b, 0xd0, 0x3e, 0xac, 0x83, 0x5a,
                                       0x6f, 0x62, 0x0f, 0xdc, 0xb5, 0x06, 0xb3, 0x45};

  const uint8_t test_vector_7_nonce[] = {0x12, 0x15, 0x35, 0x24, 0xc0, 0x89,
                                         0x5e, 0x81, 0xb2, 0xc2, 0x84, 0x65};
  const uint8_t test_vector_7_aad[] = {0xd6, 0x09, 0xb1, 0xf0, 0x56, 0x63, 0x7a, 0x0d, 0x46, 0xdf,
  ...
  const uint8_t test_vector_7_ciphertext_and_tag[] = {
      0x70, 0x1a, 0xfa, 0x1c, 0xc0, 0x39, 0xc0, 0xd7, 0x65, 0x12, 0x8a, 0x66, 0x5d,
      0xab, 0x69, 0x24, 0x38, 0x99, 0xbf, 0x73, 0x18, 0xcc, 0xdc, 0x81, 0xc9, 0x93,
      0x1d, 0xa1, 0x7f, 0xbe, 0x8e, 0xdd, 0x7d, 0x17, 0xcb, 0x8b, 0x4c, 0x26, 0xfc,
      0x81, 0xe3, 0x28, 0x4f, 0x2b, 0x7f, 0xba, 0x71, 0x3d, 0x4f, 0x8d, 0x55, 0xe7,
      0xd3, 0xf0, 0x6f, 0xd5, 0xa1, 0x3c, 0x0c, 0x29, 0xb9, 0xd5, 0xb8, 0x80};
  gsec_aead_malloc_test_vector(
      &test_vector_7, test_vector_7_key, sizeof(test_vector_7_key) / sizeof(uint8_t),
      test_vector_7_nonce, sizeof(test_vector_7_nonce) / sizeof(uint8_t), test_vector_7_aad,
      sizeof(test_vector_7_aad) / sizeof(uint8_t), test_vector_7_plaintext,
      sizeof(test_vector_7_plaintext) / sizeof(uint8_t), test_vector_7_ciphertext_and_tag,
  ...
  const uint8_t test_vector_8_key[] = {0xe3, 0xc0, 0x8a, 0x8f, 0x06, 0xc6, 0xe3, 0xad,
                                       0x95, 0xa7, 0x05, 0x57, 0xb2, 0x3f, 0x75, 0x48,
                                       0x3c, 0xe3, 0x30, 0x21, 0xa9, 0xc7, 0x2b, 0x70,
                                       0x25, 0x66, 0x62, 0x04, 0xc6, 0x9c, 0x0b, 0x72};
  const uint8_t test_vector_8_nonce[] = {0x12, 0x15, 0x35, 0x24, 0xc0, 0x89,
                                         0x5e, 0x81, 0xb2, 0xc2, 0x84, 0x65};
  const uint8_t test_vector_8_aad[] = {0xd6, 0x09, 0xb1, 0xf0, 0x56, 0x63, 0x7a, 0x0d, 0x46, 0xdf,
                                       0x99, 0x8d, 0x88, 0xe5, 0x2e, 0x00, 0xb2, 0xc2, 0x84, 0x65,
                                       0x12, 0x15, 0x35, 0x24, 0xc0, 0x89, 0x5e, 0x81};
  ...
  const uint8_t test_vector_8_ciphertext_and_tag[] = {
      0xe2, 0x00, 0x6e, 0xb4, 0x2f, 0x52, 0x77, 0x02, 0x2d, 0x9b, 0x19, 0x92, 0x5b,
      0xc4, 0x19, 0xd7, 0xa5, 0x92, 0x66, 0x6c, 0x92, 0x5f, 0xe2, 0xef, 0x71, 0x8e,
      0xb4, 0xe3, 0x08, 0xef, 0xea, 0xa7, 0xc5, 0x27, 0x3b, 0x39, 0x41, 0x18, 0x86,
      0x0a, 0x5b, 0xe2, 0xa9, 0x7f, 0x56, 0xab, 0x78, 0x36, 0x5c, 0xa5, 0x97, 0xcd,
      0xbb, 0x3e, 0xdb, 0x8d, 0x1a, 0x11, 0x51, 0xea, 0x0a, 0xf7, 0xb4, 0x36};
  gsec_aead_malloc_test_vector(
      &test_vector_8, test_vector_8_key, sizeof(test_vector_8_key) / sizeof(uint8_t),
      test_vector_8_nonce, sizeof(test_vector_8_nonce) / sizeof(uint8_t), test_vector_8_aad,
      sizeof(test_vector_8_aad) / sizeof(uint8_t), test_vector_8_plaintext,
      sizeof(test_vector_8_plaintext) / sizeof(uint8_t), test_vector_8_ciphertext_and_tag,
  ...
  const uint8_t test_vector_9_key[] = {0x07, 0x1b, 0x11, 0x3b, 0x0c, 0xa7, 0x43, 0xfe,
  ...
  const uint8_t test_vector_9_ciphertext_and_tag[] = {0x0c, 0x01, 0x7b, 0xc7, 0x3b, 0x22,
                                                      0x7d, 0xfc, 0xc9, 0xba, 0xfa, 0x1c,
                                                      0x41, 0xac, 0xc3, 0x53};
  gsec_aead_malloc_test_vector(
      &test_vector_9, test_vector_9_key, sizeof(test_vector_9_key) / sizeof(uint8_t),
      test_vector_9_nonce, sizeof(test_vector_9_nonce) / sizeof(uint8_t), test_vector_9_aad,
      sizeof(test_vector_9_aad) / sizeof(uint8_t), test_vector_9_plaintext, 0,
      test_vector_9_ciphertext_and_tag, sizeof(test_vector_9_ciphertext_and_tag) / sizeof(uint8_t));
  ...
  const uint8_t test_vector_10_key[] = {0x69, 0x1d, 0x3e, 0xe9, 0x09, 0xd7, 0xf5, 0x41,
  ...
  const uint8_t test_vector_10_ciphertext_and_tag[] = {0x35, 0x21, 0x7c, 0x77, 0x4b, 0xbc,
                                                       0x31, 0xb6, 0x31, 0x66, 0xbc, 0xf9,
                                                       0xd4, 0xab, 0xed, 0x07};
  gsec_aead_malloc_test_vector(&test_vector_10, test_vector_10_key,
                               ...
                               sizeof(test_vector_10_aad) / sizeof(uint8_t),
                               test_vector_10_plaintext, 0, test_vector_10_ciphertext_and_tag,
  ...
  const uint8_t test_vector_11_key[] = {0x07, 0x1b, 0x11, 0x3b, 0x0c, 0xa7, 0x43, 0xfe,
                                        0xcc, 0xcf, 0x3d, 0x05, 0x1f, 0x73, 0x73, 0x82};
  const uint8_t test_vector_11_nonce[] = {0xf0, 0x76, 0x1e, 0x8d, 0xcd, 0x3d,
                                          0x00, 0x01, 0x76, 0xd4, 0x57, 0xed};
  const uint8_t test_vector_11_aad[] = {0xe2, 0x01, 0x06, 0xd7, 0xcd, 0x0d, 0xf0, 0x76, 0x1e, 0x8d,
                                        0xcd, 0x3d, 0x88, 0xe5, 0x4c, 0x2a, 0x76, 0xd4, 0x57, 0xed};
  ...
  gsec_aead_malloc_test_vector(
      &test_vector_11, test_vector_11_key, sizeof(test_vector_11_key) / sizeof(uint8_t),
      test_vector_11_nonce, sizeof(test_vector_11_nonce) / sizeof(uint8_t), test_vector_11_aad,
      sizeof(test_vector_11_aad) / sizeof(uint8_t), test_vector_11_plaintext,
      sizeof(test_vector_11_plaintext) / sizeof(uint8_t), test_vector_11_ciphertext_and_tag,
  ...
  const uint8_t test_vector_12_key[] = {0x69, 0x1d, 0x3e, 0xe9, 0x09, 0xd7, 0xf5, 0x41,
                                        0x67, 0xfd, 0x1c, 0xa0, 0xb5, 0xd7, 0x69, 0x08,
                                        0x1f, 0x2b, 0xde, 0x1a, 0xee, 0x65, 0x5f, 0xdb,
                                        0xab, 0x80, 0xbd, 0x52, 0x95, 0xae, 0x6b, 0xe7};
  const uint8_t test_vector_12_nonce[] = {0xf0, 0x76, 0x1e, 0x8d, 0xcd, 0x3d,
                                          0x00, 0x01, 0x76, 0xd4, 0x57, 0xed};
  const uint8_t test_vector_12_aad[] = {0xe2, 0x01, 0x06, 0xd7, 0xcd, 0x0d, 0xf0, 0x76, 0x1e, 0x8d,
                                        0xcd, 0x3d, 0x88, 0xe5, 0x4c, 0x2a, 0x76, 0xd4, 0x57, 0xed};
  ...
  const uint8_t test_vector_12_ciphertext_and_tag[] = {
      0xc1, 0x62, 0x3f, 0x55, 0x73, 0x0c, 0x93, 0x53, 0x30, 0x97, 0xad, 0xda, 0xd2, 0x56, 0x64,
      0x96, 0x61, 0x25, 0x35, 0x2b, 0x43, 0xad, 0xac, 0xbd, 0x61, 0xc5, 0xef, 0x3a, 0xc9, 0x0b,
      0x5b, 0xee, 0x92, 0x9c, 0xe4, 0x63, 0x0e, 0xa7, 0x9f, 0x6c, 0xe5, 0x19, 0x12, 0xaf, 0x39,
      0xc2, 0xd1, 0xfd, 0xc2, 0x05, 0x1f, 0x8b, 0x7b, 0x3c, 0x9d, 0x39, 0x7e, 0xf2};
  gsec_aead_malloc_test_vector(
      &test_vector_12, test_vector_12_key, sizeof(test_vector_12_key) / sizeof(uint8_t),
      test_vector_12_nonce, sizeof(test_vector_12_nonce) / sizeof(uint8_t), test_vector_12_aad,
      sizeof(test_vector_12_aad) / sizeof(uint8_t), test_vector_12_plaintext,
      sizeof(test_vector_12_plaintext) / sizeof(uint8_t), test_vector_12_ciphertext_and_tag,
  ...
  const uint8_t test_vector_13_key[] = {0x01, 0x3f, 0xe0, 0x0b, 0x5f, 0x11, 0xbe, 0x7f,
  ...
  const uint8_t test_vector_13_aad[] = {
      0x84, 0xc5, 0xd5, 0x13, 0xd2, 0xaa, 0xf6, 0xe5, 0xbb, 0xd2, 0x72, 0x77, 0x88, 0xe5,
      0x23, 0x00, 0x89, 0x32, 0xd6, 0x12, 0x7c, 0xfd, 0xe9, 0xf9, 0xe3, 0x37, 0x24, 0xc6,
      0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,
      0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28,
      0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36,
      0x37, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f, 0x00, 0x05};
  const uint8_t test_vector_13_plaintext[1] = {};
  const uint8_t test_vector_13_ciphertext_and_tag[] = {0x21, 0x78, 0x67, 0xe5, 0x0c, 0x2d,
                                                       0xad, 0x74, 0xc2, 0x8c, 0x3b, 0x50,
                                                       0xab, 0xdf, 0x69, 0x5a};
  gsec_aead_malloc_test_vector(&test_vector_13, test_vector_13_key,
                               ...
                               sizeof(test_vector_13_aad) / sizeof(uint8_t),
                               test_vector_13_plaintext, 0, test_vector_13_ciphertext_and_tag,
  ...
  const uint8_t test_vector_14_key[] = {0x83, 0xc0, 0x93, 0xb5, 0x8d, 0xe7, 0xff, 0xe1,
  ...
  const uint8_t test_vector_14_ciphertext_and_tag[] = {0x6e, 0xe1, 0x60, 0xe8, 0xfa, 0xec,
                                                       0xa4, 0xb3, 0x6c, 0x86, 0xb2, 0x34,
                                                       0x92, 0x0c, 0xa9, 0x75};
  gsec_aead_malloc_test_vector(&test_vector_14, test_vector_14_key,
                               ...
                               sizeof(test_vector_14_aad) / sizeof(uint8_t),
                               test_vector_14_plaintext, 0, test_vector_14_ciphertext_and_tag,
  ...
  const uint8_t test_vector_15_key[] = {0x01, 0x3f, 0xe0, 0x0b, 0x5f, 0x11, 0xbe, 0x7f,
                                        0x86, 0x6d, 0x0c, 0xbb, 0xc5, 0x5a, 0x7a, 0x90};
  const uint8_t test_vector_15_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9, 0xe3, 0x37,
                                          0x24, 0xc6, 0x89, 0x32, 0xd6, 0x12};
  const uint8_t test_vector_15_aad[] = {0x84, 0xc5, 0xd5, 0x13, 0xd2, 0xaa, 0xf6, 0xe5, 0xbb, 0xd2,
                                        0x72, 0x77, 0x88, 0xe5, 0x2f, 0x00, 0x89, 0x32, 0xd6, 0x12,
                                        0x7c, 0xfd, 0xe9, 0xf9, 0xe3, 0x37, 0x24, 0xc6};
  const uint8_t test_vector_15_plaintext[] = {
      0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
      0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26,
      0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x31, 0x32, 0x33,
      0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b, 0x00, 0x06};
  const uint8_t test_vector_15_ciphertext_and_tag[] = {
      0x3a, 0x4d, 0xe6, 0xfa, 0x32, 0x19, 0x10, 0x14, 0xdb, 0xb3, 0x03, 0xd9, 0x2e,
      0xe3, 0xa9, 0xe8, 0xa1, 0xb5, 0x99, 0xc1, 0x4d, 0x22, 0xfb, 0x08, 0x00, 0x96,
      0xe1, 0x38, 0x11, 0x81, 0x6a, 0x3c, 0x9c, 0x9b, 0xcf, 0x7c, 0x1b, 0x9b, 0x96,
      0xda, 0x80, 0x92, 0x04, 0xe2, 0x9d, 0x0e, 0x2a, 0x76, 0x42, 0xbf, 0xd3, 0x10,
      0xa4, 0x83, 0x7c, 0x81, 0x6c, 0xcf, 0xa5, 0xac, 0x23, 0xab, 0x00, 0x39, 0x88};
  gsec_aead_malloc_test_vector(
      &test_vector_15, test_vector_15_key, sizeof(test_vector_15_key) / sizeof(uint8_t),
      test_vector_15_nonce, sizeof(test_vector_15_nonce) / sizeof(uint8_t), test_vector_15_aad,
      sizeof(test_vector_15_aad) / sizeof(uint8_t), test_vector_15_plaintext,
      sizeof(test_vector_15_plaintext) / sizeof(uint8_t), test_vector_15_ciphertext_and_tag,
  ...
  const uint8_t test_vector_16_key[] = {0x83, 0xc0, 0x93, 0xb5, 0x8d, 0xe7, 0xff, 0xe1,
                                        0xc0, 0xda, 0x92, 0x6a, 0xc4, 0x3f, 0xb3, 0x60,
                                        0x9a, 0xc1, 0xc8, 0x0f, 0xee, 0x1b, 0x62, 0x44,
                                        0x97, 0xef, 0x94, 0x2e, 0x2f, 0x79, 0xa8, 0x23};
  const uint8_t test_vector_16_nonce[] = {0x7c, 0xfd, 0xe9, 0xf9, 0xe3, 0x37,
                                          0x24, 0xc6, 0x89, 0x32, 0xd6, 0x12};
  const uint8_t test_vector_16_aad[] = {0x84, 0xc5, 0xd5, 0x13, 0xd2, 0xaa, 0xf6, 0xe5, 0xbb, 0xd2,
                                        0x72, 0x77, 0x88, 0xe5, 0x2f, 0x00, 0x89, 0x32, 0xd6, 0x12,
                                        0x7c, 0xfd, 0xe9, 0xf9, 0xe3, 0x37, 0x24, 0xc6};
  ...
  const uint8_t test_vector_16_ciphertext_and_tag[] = {
      0x11, 0x02, 0x22, 0xff, 0x80, 0x50, 0xcb, 0xec, 0xe6, 0x6a, 0x81, 0x3a, 0xd0,
      0x9a, 0x73, 0xed, 0x7a, 0x9a, 0x08, 0x9c, 0x10, 0x6b, 0x95, 0x93, 0x89, 0x16,
      0x8e, 0xd6, 0xe8, 0x69, 0x8e, 0xa9, 0x02, 0xeb, 0x12, 0x77, 0xdb, 0xec, 0x2e,
      0x68, 0xe4, 0x73, 0x15, 0x5a, 0x15, 0xa7, 0xda, 0xee, 0xd4, 0xa1, 0x0f, 0x4e,
      0x05, 0x13, 0x9c, 0x23, 0xdf, 0x00, 0xb3, 0xaa, 0xdc, 0x71, 0xf0, 0x59, 0x6a};
  gsec_aead_malloc_test_vector(
      &test_vector_16, test_vector_16_key, sizeof(test_vector_16_key) / sizeof(uint8_t),
      test_vector_16_nonce, sizeof(test_vector_16_nonce) / sizeof(uint8_t), test_vector_16_aad,
      sizeof(test_vector_16_aad) / sizeof(uint8_t), test_vector_16_plaintext,
      sizeof(test_vector_16_plaintext) / sizeof(uint8_t), test_vector_16_ciphertext_and_tag,
  ...
  const uint8_t test_vector_17_key[] = {0x88, 0xee, 0x08, 0x7f, 0xd9, 0x5d, 0xa9, 0xfb,
  ...
  const uint8_t test_vector_17_aad[] = {
      0x68, 0xf2, 0xe7, 0x76, 0x96, 0xce, 0x7a, 0xe8, 0xe2, 0xca, 0x4e, 0xc5, 0x88, 0xe5, 0x41,
      0x00, 0x2e, 0x58, 0x49, 0x5c, 0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
      ...
      0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x00, 0x07};
  const uint8_t test_vector_17_plaintext[1] = {};
  const uint8_t test_vector_17_ciphertext_and_tag[] = {0x07, 0x92, 0x2b, 0x8e, 0xbc, 0xf1,
                                                       0x0b, 0xb2, 0x29, 0x75, 0x88, 0xca,
                                                       0x4c, 0x61, 0x45, 0x23};
  gsec_aead_malloc_test_vector(&test_vector_17, test_vector_17_key,
                               ...
                               sizeof(test_vector_17_aad) / sizeof(uint8_t),
                               test_vector_17_plaintext, 0, test_vector_17_ciphertext_and_tag,
  ...
  const uint8_t test_vector_18_key[] = {0x4c, 0x97, 0x3d, 0xbc, 0x73, 0x64, 0x62, 0x16,
  ...
  const uint8_t test_vector_18_ciphertext_and_tag[] = {0x00, 0xbd, 0xa1, 0xb7, 0xe8, 0x76,
                                                       0x08, 0xbc, 0xbf, 0x47, 0x0f, 0x12,
                                                       0x15, 0x7f, 0x4c, 0x07};
  gsec_aead_malloc_test_vector(&test_vector_18, test_vector_18_key,
                               ...
                               sizeof(test_vector_18_aad) / sizeof(uint8_t),
                               test_vector_18_plaintext, 0, test_vector_18_ciphertext_and_tag,
  ...
  const uint8_t test_vector_19_key[] = {0x88, 0xee, 0x08, 0x7f, 0xd9, 0x5d, 0xa9, 0xfb,
                                        0xf6, 0x72, 0x5a, 0xa9, 0xd7, 0x57, 0xb0, 0xcd};
  const uint8_t test_vector_19_nonce[] = {0x7a, 0xe8, 0xe2, 0xca, 0x4e, 0xc5,
                                          0x00, 0x01, 0x2e, 0x58, 0x49, 0x5c};
  const uint8_t test_vector_19_aad[] = {0x68, 0xf2, 0xe7, 0x76, 0x96, 0xce, 0x7a, 0xe8, 0xe2, 0xca,
  ...
  const uint8_t test_vector_19_plaintext[] = {
      ...
      0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x00, 0x08};
  const uint8_t test_vector_19_ciphertext_and_tag[] = {
      0xc3, 0x1f, 0x53, 0xd9, 0x9e, 0x56, 0x87, 0xf7, 0x36, 0x51, 0x19, 0xb8, 0x32, 0xd2,
      0xaa, 0xe7, 0x07, 0x41, 0xd5, 0x93, 0xf1, 0xf9, 0xe2, 0xab, 0x34, 0x55, 0x77, 0x9b,
      0x07, 0x8e, 0xb8, 0xfe, 0xac, 0xdf, 0xec, 0x1f, 0x8e, 0x3e, 0x52, 0x77, 0xf8, 0x18,
      0x0b, 0x43, 0x36, 0x1f, 0x65, 0x12, 0xad, 0xb1, 0x6d, 0x2e, 0x38, 0x54, 0x8a, 0x2c,
      0x71, 0x9d, 0xba, 0x72, 0x28, 0xd8, 0x40, 0x88, 0xf8, 0x75, 0x7a, 0xdb, 0x8a, 0xa7,
      0x88, 0xd8, 0xf6, 0x5a, 0xd6, 0x68, 0xbe, 0x70, 0xe7};
  gsec_aead_malloc_test_vector(
      &test_vector_19, test_vector_19_key, sizeof(test_vector_19_key) / sizeof(uint8_t),
      test_vector_19_nonce, sizeof(test_vector_19_nonce) / sizeof(uint8_t), test_vector_19_aad,
      sizeof(test_vector_19_aad) / sizeof(uint8_t), test_vector_19_plaintext,
      sizeof(test_vector_19_plaintext) / sizeof(uint8_t), test_vector_19_ciphertext_and_tag,
  ...
  const uint8_t test_vector_20_key[] = {0x4c, 0x97, 0x3d, 0xbc, 0x73, 0x64, 0x62, 0x16,
                                        0x74, 0xf8, 0xb5, 0xb8, 0x9e, 0x5c, 0x15, 0x51,
                                        0x1f, 0xce, 0xd9, 0x21, 0x64, 0x90, 0xfb, 0x1c,
                                        0x1a, 0x2c, 0xaa, 0x0f, 0xfe, 0x04, 0x07, 0xe5};
  const uint8_t test_vector_20_nonce[] = {0x7a, 0xe8, 0xe2, 0xca, 0x4e, 0xc5,
                                          0x00, 0x01, 0x2e, 0x58, 0x49, 0x5c};
  const uint8_t test_vector_20_aad[] = {0x68, 0xf2, 0xe7, 0x76, 0x96, 0xce, 0x7a, 0xe8, 0xe2, 0xca,
                                        0x4e, 0xc5, 0x88, 0xe5, 0x4d, 0x00, 0x2e, 0x58, 0x49, 0x5c};
  ...
  const uint8_t test_vector_20_ciphertext_and_tag[] = {
      0xba, 0x8a, 0xe3, 0x1b, 0xc5, 0x06, 0x48, 0x6d, 0x68, 0x73, 0xe4, 0xfc, 0xe4, 0x60,
      0xe7, 0xdc, 0x57, 0x59, 0x1f, 0xf0, 0x06, 0x11, 0xf3, 0x1c, 0x38, 0x34, 0xfe, 0x1c,
      0x04, 0xad, 0x80, 0xb6, 0x68, 0x03, 0xaf, 0xcf, 0x5b, 0x27, 0xe6, 0x33, 0x3f, 0xa6,
      0x7c, 0x99, 0xda, 0x47, 0xc2, 0xf0, 0xce, 0xd6, 0x8d, 0x53, 0x1b, 0xd7, 0x41, 0xa9,
      0x43, 0xcf, 0xf7, 0xa6, 0x71, 0x3b, 0xd0, 0x26, 0x11, 0xcd, 0x7d, 0xaa, 0x01, 0xd6,
      0x1c, 0x5c, 0x88, 0x6d, 0xc1, 0xa8, 0x17, 0x01, 0x07};
  gsec_aead_malloc_test_vector(
      &test_vector_20, test_vector_20_key, sizeof(test_vector_20_key) / sizeof(uint8_t),
      test_vector_20_nonce, sizeof(test_vector_20_nonce) / sizeof(uint8_t), test_vector_20_aad,
      sizeof(test_vector_20_aad) / sizeof(uint8_t), test_vector_20_plaintext,
      sizeof(test_vector_20_plaintext) / sizeof(uint8_t), test_vector_20_ciphertext_and_tag,
