...
/**
 * This method returns a uint32 that's not quite uniformly random, but
 ...
 * - source_len: the length of source buffer to be copied from its
 * beginning.
 */
...
/**
 * This method copies data from a source to a destination buffer, and
 * flips one byte in the destination buffer randomly.
 *
 * - src: a source buffer.
 * - des: a destination buffer.
 * - length: the length of source buffer to be copied from its
 * beginning.
 */
...
/**
 * This method compares two grpc_status_code values, and verifies if one
 ...
 *
 * If both checks succeed, the method returns 1 and otherwise, it
 * returns 0.
 */
