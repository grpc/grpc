...
json_writer_output_char (void *userdata, char c)
{
  json_writer_userdata *state =
    static_cast < json_writer_userdata * >(userdata);
...
check_string (json_reader_userdata * state, size_t needed)
{
  ...
  state->scratchpad =
    static_cast <
    char *>(gpr_realloc (state->scratchpad, state->allocated + needed));
...
json_reader_string_clear (void *userdata)
{
  json_reader_userdata *state =
    static_cast < json_reader_userdata * >(userdata);
...
json_reader_container_begins (void *userdata, grpc_json_type type)
{
  ...
  stacked_container *container =
    static_cast <
    stacked_container * >(gpr_malloc (sizeof (stacked_container)));
