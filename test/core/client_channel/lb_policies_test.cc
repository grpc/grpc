...
test_spec_create (size_t num_iters, size_t num_servers)
{
  ...
  spec->kill_at =
    static_cast < int **>(gpr_malloc (sizeof (int *) * num_iters));
  spec->revive_at =
    static_cast < int **>(gpr_malloc (sizeof (int *) * num_iters));
...
kill_server (const servers_fixture * f, size_t i)
{
  ...
  grpc_server_shutdown_and_notify (f->servers[i], f->shutdown_cq,
				   tag (10000));
  GPR_ASSERT (grpc_completion_queue_pluck
	      (f->shutdown_cq, tag (10000), n_millis_time (5000),
...
static void
revive_server (const servers_fixture * f, request_data * rdata, size_t i)
{
  int got_port;
  ...
  GPR_ASSERT ((got_port =
	       grpc_server_add_insecure_http2_port (f->servers[i],
						    f->
						    servers_hostports[i])) >
	      0);
...
setup_servers (const char *server_host,
	       request_data * rdata, const size_t num_servers)
{
  servers_fixture *f =
  ...
  f->server_calls =
    static_cast <
    grpc_call ** >(gpr_malloc (sizeof (grpc_call *) * num_servers));
  ...
  f->servers =
    static_cast <
    grpc_server ** >(gpr_malloc (sizeof (grpc_server *) * num_servers));
  f->servers_hostports =
...
static request_sequences
request_sequences_create (size_t n)
{
...
static void
request_sequences_destroy (const request_sequences * rseqs)
{
...
static request_sequences
perform_request (servers_fixture * f,
		 grpc_channel * client,
		 request_data * rdata, const test_spec * spec)
{
  ...
  const request_sequences sequences =
    request_sequences_create (spec->num_iters);
  ...
  for (iter_num = 0; iter_num < spec->num_iters; iter_num++)
	...
	&rdata->trailing_metadata_recv;
      ...
      op++;
      GPR_ASSERT (GRPC_CALL_OK ==
		  grpc_call_start_batch (c, ops, (size_t) (op - ops), tag (1),
					 nullptr));
	     ...
	     GRPC_QUEUE_TIMEOUT)
	{
	  ...
	  gpr_log (GPR_DEBUG,
		   "EVENT: success:%d, type:%d, tag:%d iter:%" PRIuPTR,
      ...
      if (s_idx >= 0)
	{
	  ...
	  op++;
	  GPR_ASSERT (GRPC_CALL_OK ==
		      grpc_call_start_batch (f->server_calls[s_idx], ops,
					     (size_t) (op - ops), tag (102),
					     nullptr));
	  ...
	  GPR_ASSERT (0 ==
		      grpc_slice_str_cmp (rdata->call_details[s_idx].method,
					  "/foo"));
	  GPR_ASSERT (0 ==
		      grpc_slice_str_cmp (rdata->call_details[s_idx].host,
					  "foo.test.google.fr"));
	  ...
	  GPR_ASSERT (GRPC_CALL_OK ==
		      grpc_server_request_call (f->servers[s_idx],
						&f->server_calls[s_idx],
						&rdata->call_details[s_idx],
						&f->
						request_metadata_recv[s_idx],
						f->cq, f->cq,
						tag (1000 + (int) s_idx)));
	}
      else
	{			/* no response from server */
      ...
      GPR_ASSERT (grpc_completion_queue_next
		  (f->cq,
		   grpc_timeout_milliseconds_to_deadline (RETRY_TIMEOUT),
...
static grpc_call **
perform_multirequest (servers_fixture * f,
		      grpc_channel * client, size_t concurrent_calls)
{
  grpc_call **calls;
  grpc_op ops[6];
  grpc_op *op;
  ...
  calls =
    static_cast <
    grpc_call ** >(gpr_malloc (sizeof (grpc_call *) * concurrent_calls));
  for (i = 0; i < f->num_servers; i++)
    {
      kill_server (f, i);
    }
  ...
  for (i = 0; i < concurrent_calls; i++)
    {
      calls[i] =
	grpc_channel_create_call (client, nullptr, GRPC_PROPAGATE_DEFAULTS,
				  f->cq,
				  grpc_slice_from_static_string ("/foo"),
...
run_spec (const test_spec * spec)
{
  ...
  servers_hostports_str =
    gpr_strjoin_sep ((const char **) f->servers_hostports, f->num_servers,
		     ",", nullptr);
  ...
  gpr_log (GPR_INFO, "Testing '%s' with servers=%s client=%s",
	   spec->description, servers_hostports_str, client_hostport);

  const request_sequences sequences =
    perform_request (f, client, &rdata, spec);
...
static grpc_channel *
create_client (const servers_fixture * f)
{
  grpc_channel *client;
  char *client_hostport;
  char *servers_hostports_str;
...
test_ping ()
{
  ...
  while (state != GRPC_CHANNEL_READY)
    {
      grpc_channel_watch_connectivity_state (client, state,
...
static void
test_pending_calls (size_t concurrent_calls)
{
  size_t i;
  grpc_call **calls;
  grpc_channel *client;
...
test_get_channel_info ()
{
  ...
  channel =
    grpc_insecure_channel_create ("ipv4:127.0.0.1:1234", args, nullptr);
  {
...
static void
print_failed_expectations (const int *expected_connection_sequence,
...
static void
verify_vanilla_round_robin (const servers_fixture * f,
			    grpc_channel * client,
			    const request_sequences * sequences,
			    const size_t num_iters)
{
...
static void
verify_vanishing_floor_round_robin (const servers_fixture * f,
				    grpc_channel * client,
				    const request_sequences * sequences,
				    const size_t num_iters)
{
...
static void
verify_total_carnage_round_robin (const servers_fixture * f,
				  grpc_channel * client,
				  const request_sequences * sequences,
				  const size_t num_iters)
{
...
static void
verify_partial_carnage_round_robin (const servers_fixture * f,
				    grpc_channel * client,
				    const request_sequences * sequences,
				    const size_t num_iters)
{
  ...
  grpc_connectivity_state actual =
    static_cast < grpc_connectivity_state >
    (sequences->connectivity_states[0]);
...
static void
dump_array (const char *desc, const int *data, const size_t count)
{
  gpr_strvec s;
  char *tmp;
  size_t i;
...
static void
verify_rebirth_round_robin (const servers_fixture * f,
			    grpc_channel * client,
			    const request_sequences * sequences,
			    const size_t num_iters)
{
  dump_array ("actual_connection_sequence", sequences->connections,
	      num_iters);
  ...
  if (!found_failure_status)
      ...
      for (size_t i = 0; i < num_iters; i++)
	{
	  gpr_log (GPR_ERROR, "[%d]: %s", (int) i,
		   grpc_connectivity_state_name (static_cast <
						 grpc_connectivity_state >
						 (sequences->
						  connectivity_states[i])));
	}
    }
}
