		    ...
		    grpc_closure * on_done, grpc_resolved_addresses ** addrs)
{
  ...
  if (g_fail_resolution)
  ...
  else
    {
      ...
      *addrs =
	static_cast <
	grpc_resolved_addresses * >(gpr_malloc (sizeof (**addrs)));
      (*addrs)->naddrs = 1;
      (*addrs)->addrs =
	static_cast <
	grpc_resolved_address * >(gpr_malloc (sizeof (*(*addrs)->addrs)));
...
my_dns_lookup_ares (const char *dns_server, const char *addr,
...
create_resolver (const char *name)
{
...
static void
on_done (void *ev, grpc_error * error)
{
...
static bool
wait_loop (int deadline_seconds, gpr_event * ev)
{
...
static void
call_resolver_next_now_lock_taken (void *arg, grpc_error * error_unused)
{
...
static void
call_resolver_next_after_locking (grpc_resolver * resolver,
				  grpc_channel_args ** result,
				  grpc_closure * on_complete)
{
...
main (int argc, char **argv)
{
...

  {
    ...
    gpr_event ev1;
    gpr_event_init (&ev1);
    call_resolver_next_after_locking (resolver, &result,
