...
build_fake_resolver (grpc_combiner * combiner,
...
on_resolution_cb (void *arg, grpc_error * error)
{
  ...
  GPR_ASSERT (grpc_lb_addresses_cmp
	      (actual_lb_addresses, expected_lb_addresses) == 0);
...
test_fake_resolver ()
{
  ...
  grpc_resolver *resolver =
    build_fake_resolver (combiner, response_generator);
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (uris); ++i)
    {
      grpc_lb_addresses_set_address_from_uri (addresses, i, uris[i],
					      is_balancer[i],
					      balancer_names[i], nullptr);
  ...
  grpc_uri *uris_update[] =
    { grpc_uri_parse ("ipv4:192.168.1.0:31416", true) };
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (uris_update); ++i)
    {
      grpc_lb_addresses_set_address_from_uri (addresses_update, i,
					      uris_update[i],
					      is_balancer_update[i],
					      balancer_names_update[i],
					      nullptr);
