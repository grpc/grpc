...
// Feature mask supports call credentials with a minimum security level
// of GRPC_PRIVACY_AND_INTEGRITY.
#define FEATURE_MASK_SUPPORTS_PER_CALL_CREDENTIALS 4
// Feature mask supports call credentials with a minimum security level
// of GRPC_SECURTITY_NONE.
...
struct grpc_end2end_test_config {
  ...
  /* Which features are supported by this fixture. See feature flags
   * above. */
  uint32_t feature_mask;

  /* If the call host is setup by the fixture (for example, via the
   * GRPC_SSL_TARGET_NAME_OVERRIDE_ARG channel arg), which value should
   * the test expect to find in call_details.host */
