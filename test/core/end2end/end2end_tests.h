...
#define GRPC_HTTP2_PH2_TEST_SUITE "Ph2Client"
...
struct CoreTestConfiguration {
  ...
  // To enable sepcific test suites pass a `|` separated list to
  // include_test_suites.
...
#define SKIP_IF_DISABLED_IN_CONFIG(include_suite, include_test, exclude_test, \
                                   suite, test)                               \
  if (!IsTestEnabledInConfig(include_suite, include_test, exclude_test,       \
                             suite, test)) {                                  \
