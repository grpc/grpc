...
struct CallbackContext {
  ...
  explicit CallbackContext(
      void (*cb)(grpc_completion_queue_functor* functor, int success)) {
...
static void child_thread(void* arg) {
  ...
  ev = grpc_completion_queue_next(
      ce->cq, gpr_inf_future(GPR_CLOCK_MONOTONIC), nullptr);
...
static void test_connectivity(grpc_end2end_test_config config) {
  ...
  grpc_channel_watch_connectivity_state(
      f.client, GRPC_CHANNEL_IDLE, grpc_timeout_seconds_to_deadline(3),
      f.cq, tag(2));
  ...
  grpc_channel_watch_connectivity_state(
      f.client, GRPC_CHANNEL_CONNECTING,
  ...
  grpc_channel_watch_connectivity_state(
      f.client, GRPC_CHANNEL_READY, grpc_timeout_seconds_to_deadline(3),
      f.cq, tag(5));
...
static void cb_watch_connectivity(
    grpc_completion_queue_functor* functor, int success) {
...
static void test_watch_connectivity_cq_callback(
  ...
  cq = grpc_completion_queue_create_for_callback(
      &cb_shutdown_ctx.functor, nullptr);
  ...
  /* we just check that the callback was executed once notifying a
   * connection transition */
  GPR_ASSERT(gpr_event_wait(&cb_ctx.finished,
                            gpr_inf_future(GPR_CLOCK_MONOTONIC)) !=
             nullptr);

  /* shutdown, since shutdown cb might be executed in a background
   * thread we actively wait till is executed. */
