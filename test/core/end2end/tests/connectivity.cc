...
child_thread (void *arg)
{
  ...
  ev =
    grpc_completion_queue_next (ce->cq, gpr_inf_future (GPR_CLOCK_MONOTONIC),
				nullptr);
...
test_connectivity (grpc_end2end_test_config config)
{
  ...
  GPR_ASSERT (gpr_thd_new
	      (&thdid, "grpc_connectivity", child_thread, &ce, &thdopt));
...
void
connectivity (grpc_end2end_test_config config)
{
