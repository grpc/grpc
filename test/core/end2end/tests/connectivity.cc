...
struct CallbackContext {
  ...
  gpr_event finished;
  explicit CallbackContext(void (*cb)(grpc_completion_queue_functor* functor, int success)) {
...
static void child_thread(void* arg) {
  ...
  ev = grpc_completion_queue_next(ce->cq, gpr_inf_future(GPR_CLOCK_MONOTONIC), nullptr);
...
static void test_connectivity(grpc_end2end_test_config config) {
  ...
  arg_array[0].key = const_cast<char*>("grpc.testing.fixed_reconnect_backoff_ms");
  ...
  grpc_channel_watch_connectivity_state(f.client, GRPC_CHANNEL_IDLE, gpr_now(GPR_CLOCK_MONOTONIC),
                                        f.cq, tag(1));
  ...
  grpc_channel_watch_connectivity_state(f.client, GRPC_CHANNEL_IDLE,
                                        grpc_timeout_seconds_to_deadline(3), f.cq, tag(2));
  ...
  grpc_channel_watch_connectivity_state(f.client, GRPC_CHANNEL_CONNECTING,
                                        grpc_timeout_seconds_to_deadline(3), f.cq, tag(3));
  ...
  while (state != GRPC_CHANNEL_READY) {
    grpc_channel_watch_connectivity_state(f.client, state, grpc_timeout_seconds_to_deadline(3),
                                          f.cq, tag(4));
  ...
  grpc_channel_watch_connectivity_state(f.client, GRPC_CHANNEL_READY,
                                        grpc_timeout_seconds_to_deadline(3), f.cq, tag(5));
...
static void cb_shutdown(grpc_completion_queue_functor* functor, int /*success*/) {
  CallbackContext* cb_ctx = reinterpret_cast<CallbackContext*>(functor);
...
static void test_watch_connectivity_cq_callback(grpc_end2end_test_config config) {
  ...
  cq = grpc_completion_queue_create_for_callback(&cb_shutdown_ctx.functor, nullptr);

  /* start watching for any change, cb is immediately called
   * and no dead lock should be raised */
  grpc_channel_watch_connectivity_state(f.client, GRPC_CHANNEL_IDLE,
                                        grpc_timeout_seconds_to_deadline(3), cq, &cb_ctx.functor);

  /* we just check that the callback was executed once notifying a connection
   * transition */
  GPR_ASSERT(gpr_event_wait(&cb_ctx.finished, gpr_inf_future(GPR_CLOCK_MONOTONIC)) != nullptr);

  /* shutdown, since shutdown cb might be executed in a background thread
   * we actively wait till is executed. */
  grpc_completion_queue_shutdown(cq);
  gpr_event_wait(&cb_shutdown_ctx.finished, gpr_inf_future(GPR_CLOCK_MONOTONIC));
