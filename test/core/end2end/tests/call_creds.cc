...
static grpc_end2end_test_fixture begin_test(grpc_end2end_test_config config, const char* test_name,
  ...
  gpr_log(GPR_INFO, "Running test: %s%s/%s", test_name,
          use_secure_call_creds ? "_with_secure_call_creds" : "_with_insecure_call_creds",
          config.name);
  f = config.create_fixture(nullptr, nullptr);
  config.init_client(&f, nullptr);
  if (fail_server_auth_check) {
    grpc_arg fail_auth_arg = {
        GRPC_ARG_STRING, const_cast<char*>(FAIL_AUTH_CHECK_SERVER_ARG_NAME), {nullptr}};
...
static void request_response_with_payload_and_call_creds(const char* test_name,
  ...
  if (use_secure_call_creds) {
    ...
    creds = grpc_md_only_test_credentials_create(fake_md_key, fake_md_value, false);
  }
  ...
  switch (mode) {
    ...
    case OVERRIDE:
      ...
      if (use_secure_call_creds) {
        creds = grpc_google_iam_credentials_create(overridden_iam_token, overridden_iam_selector,
                                                   nullptr);
      } else {
        creds = grpc_md_only_test_credentials_create(overridden_fake_md_key,
                                                     overridden_fake_md_value, false);
      }
      GPR_ASSERT(creds != nullptr);
      GPR_ASSERT(grpc_call_set_credentials(c, creds) == GRPC_CALL_OK);
      break;
    ...
    switch (mode) {
      case NONE:
        if (use_secure_call_creds) {
          ...
          GPR_ASSERT(contains_metadata(&request_metadata_recv, fake_md_key, fake_md_value));
        }
        break;
      case OVERRIDE:
        if (use_secure_call_creds) {
          ...
          GPR_ASSERT(contains_metadata(&request_metadata_recv, overridden_fake_md_key,
...
static void test_request_response_with_payload_and_call_creds(grpc_end2end_test_config config,
  ...
  request_response_with_payload_and_call_creds("test_request_response_with_payload_and_call_creds",
                                               config, NONE, use_secure_call_creds);
}
...
static void test_request_response_with_payload_and_overridden_call_creds(
  ...
  request_response_with_payload_and_call_creds(
      "test_request_response_with_payload_and_overridden_call_creds", config, OVERRIDE,
...
static void test_request_response_with_payload_and_deleted_call_creds(
    grpc_end2end_test_config config, bool use_secure_call_creds) {
  request_response_with_payload_and_call_creds(
      "test_request_response_with_payload_and_deleted_call_creds", config, DESTROY,
      use_secure_call_creds);
}
...
static void test_request_response_with_payload_fail_to_send_call_creds(
    grpc_end2end_test_config config, bool use_secure_call_creds) {
  request_response_with_payload_and_call_creds(
      "test_request_response_with_payload_fail_to_send_call_creds", config, FAIL,
      use_secure_call_creds);
}
...
static void test_request_with_server_rejecting_client_creds(grpc_end2end_test_config config) {
  ...
  f = begin_test(config, "test_request_with_server_rejecting_client_creds", false, 1);
  cqv = cq_verifier_create(f.cq);
...
void call_creds(grpc_end2end_test_config config) {
  ...
  if (config.feature_mask & FEATURE_MASK_SUPPORTS_PER_CALL_CREDENTIALS_LEVEL_INSECURE) {
  ...
  if (config.feature_mask & FEATURE_MASK_SUPPORTS_PER_CALL_CREDENTIALS ||
      config.feature_mask & FEATURE_MASK_SUPPORTS_PER_CALL_CREDENTIALS_LEVEL_INSECURE) {
