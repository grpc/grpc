...
begin_test (grpc_end2end_test_config config,
	    const char *test_name, int fail_server_auth_check)
{
...
static gpr_timespec
n_seconds_from_now (int n)
{
  return grpc_timeout_seconds_to_deadline (n);
}

static gpr_timespec
five_seconds_from_now (void)
{
...
static void
request_response_with_payload_and_call_creds (const char *test_name,
					      ...
					      override_mode mode)
{
  ...
  c =
    grpc_channel_create_call (f.client, nullptr, GRPC_PROPAGATE_DEFAULTS,
			      ...
			      ("foo.test.google.fr:1234", config), deadline,
			      nullptr);
  GPR_ASSERT (c);
  creds =
  ...
  switch (mode)
    ...
    case OVERRIDE:
      ...
      creds =
	grpc_google_iam_credentials_create (overridden_iam_token,
...
static void
test_request_response_with_payload_and_call_creds (grpc_end2end_test_config
						   config)
{
...
static void
test_request_with_server_rejecting_client_creds (grpc_end2end_test_config
						 config)
{
  grpc_op ops[6];
  grpc_op *op;
  ...
  f =
    begin_test (config, "test_request_with_server_rejecting_client_creds", 1);
  cqv = cq_verifier_create (f.cq);
...
void
call_creds (grpc_end2end_test_config config)
{
