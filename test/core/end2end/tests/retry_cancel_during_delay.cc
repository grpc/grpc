...
static void test_retry_cancel_during_delay(
  ...
  std::string name =
      absl::StrCat("retry_cancel_during_delay/", mode.name);
  grpc_end2end_test_fixture f =
  ...
  gpr_log(GPR_INFO, "Expect completion before: %s",
          absl::FormatTime(grpc_core::ToAbslTime(expect_finish_before))
              .c_str());
  GPR_ASSERT(gpr_time_cmp(gpr_now(GPR_CLOCK_MONOTONIC),
                          expect_finish_before) < 0);
