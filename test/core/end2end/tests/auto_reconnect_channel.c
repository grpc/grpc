...
static void fail_the_first_rpc_after_restarting_server(
...
static void auto_reconnect_channel_test(grpc_end2end_test_config config) {
  ...
  /* TODO(roth): Remove this step after the retry code is finished. If we can
     reconnect the sub-channel automatically, the first rpc after restarting
     the server should not fail. */
  fail_the_first_rpc_after_restarting_server(config, &f, cqv);
