...
static void simple_request_body(grpc_end2end_test_config config, grpc_end2end_test_fixture f) {
  ...
  int was_cancelled = 2;
  char* peer;
  grpc_stats_data* before = static_cast<grpc_stats_data*>(gpr_malloc(sizeof(grpc_stats_data)));
  grpc_stats_data* after = static_cast<grpc_stats_data*>(gpr_malloc(sizeof(grpc_stats_data)));
