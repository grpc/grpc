...
static void test_retry_exceeds_buffer_size_in_delay(grpc_end2end_test_config config) {
  grpc_call* c;
  grpc_call* s;
  grpc_op ops[6];
  grpc_op* op;
  ...
  grpc_end2end_test_fixture f =
      begin_test(config, "retry_exceeds_buffer_size_in_delay", &client_args, nullptr);
