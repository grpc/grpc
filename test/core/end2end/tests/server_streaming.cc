...
static void test_server_streaming(grpc_end2end_test_config config, int num_messages) {
  grpc_end2end_test_fixture f = begin_test(config, "test_server_streaming", nullptr, nullptr);
  grpc_call* c;
  grpc_call* s;
  ...
  while (true) {
    ...
    op->reserved = nullptr;
    op++;
    error = grpc_call_start_batch(c, ops, static_cast<size_t>(op - ops), tag(102), nullptr);
    GPR_ASSERT(GRPC_CALL_OK == error);
    CQ_MAYBE_EXPECT_COMPLETION(cqv, tag(1), true, &seen_status);
