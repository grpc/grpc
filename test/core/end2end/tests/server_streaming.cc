...
/* Client requests status along with the initial metadata. Server
 * streams messages and ends with a non-OK status. Client reads after
 * server is done writing, and expects to get the status after the
 * messages. */
static void test_server_streaming(grpc_end2end_test_config config,
  ...
  op->reserved = nullptr;
  op++;
  // Client requests status early but should not receive status till all
