...
static void
test_cacheable_request_response_with_metadata_and_payload
  (grpc_end2end_test_config config)
{
  ...
  if (config.feature_mask & FEATURE_MASK_SUPPORTS_REQUEST_PROXYING)
    ...
    {
      GPR_ASSERT (GRPC_INITIAL_METADATA_CACHEABLE_REQUEST & call_details.
		  flags);
    }
...
void
simple_cacheable_request (grpc_end2end_test_config config)
{
  test_cacheable_request_response_with_metadata_and_payload (config);
}

void
simple_cacheable_request_pre_init (void)
{
}
