...
static void
request_for_disabled_algorithm (grpc_end2end_test_config config,
				const char *test_name,
				uint32_t send_flags_bitmask,
				grpc_compression_algorithm
				algorithm_to_disable,
				grpc_compression_algorithm
				...
				grpc_metadata * client_metadata)
{
  ...
  client_args =
    grpc_channel_args_set_compression_algorithm (nullptr,
						 requested_client_compression_algorithm);
  server_args =
    grpc_channel_args_set_compression_algorithm (nullptr, GRPC_COMPRESS_NONE);
  {
    ...
    server_args =
      grpc_channel_args_compression_algorithm_set_state (&server_args,
							 algorithm_to_disable,
							 false);
  }
  ...
  GPR_ASSERT (grpc_compression_algorithm_name
	      (algorithm_to_disable, &algo_name));
...
static void
request_with_payload_template (grpc_end2end_test_config config,
			       const char *test_name,
			       ...
			       bool send_message_before_initial_metadata)
{
  ...
  for (int i = 0; i < 2; i++)
      ...
      if (i > 0 || !send_message_before_initial_metadata)
	{
	  ...
	  op++;
	  error =
	    grpc_call_start_batch (c, ops, (size_t) (op - ops), tag (2),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
	...
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (102),
      ...
      op++;
      error =
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (103),
			       nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      op++;
      error =
	grpc_call_start_batch (c, ops, (size_t) (op - ops), tag (3), nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      GPR_ASSERT (byte_buffer_eq_string
		  (response_payload_recv, response_str));
  ...
  op++;
  error =
    grpc_call_start_batch (c, ops, (size_t) (op - ops), tag (4), nullptr);
  GPR_ASSERT (GRPC_CALL_OK == error);
  ...
  (grpc_end2end_test_config config)
{
  request_with_payload_template (config,
				 "test_invoke_request_with_exceptionally_uncompressed_payload",
				 GRPC_WRITE_NO_COMPRESS, GRPC_COMPRESS_GZIP,
				 GRPC_COMPRESS_GZIP, GRPC_COMPRESS_NONE,
...
static void
test_invoke_request_with_uncompressed_payload (grpc_end2end_test_config
					       config)
{
  request_with_payload_template (config,
...
test_invoke_request_with_compressed_payload (grpc_end2end_test_config config)
{
  request_with_payload_template (config,
				 ...
				 GRPC_COMPRESS_GZIP, GRPC_COMPRESS_GZIP,
				 nullptr, false,
				 /* ignored */ GRPC_COMPRESS_LEVEL_NONE,
				 false);
}
...
static void
test_invoke_request_with_server_level (grpc_end2end_test_config config)
{
  request_with_payload_template (config,
				 "test_invoke_request_with_server_level", 0,
				 GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
				 GRPC_COMPRESS_NONE,
				 GRPC_COMPRESS_NONE /* ignored */ ,
				 nullptr, true, GRPC_COMPRESS_LEVEL_HIGH,
				 false);
}
  ...
  (grpc_end2end_test_config config)
{
  ...
  request_with_payload_template (config,
				 "test_invoke_request_with_compressed_payload_md_override_1",
				 0, GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
				 GRPC_COMPRESS_GZIP, GRPC_COMPRESS_NONE,
  ...
  request_with_payload_template (config,
				 "test_invoke_request_with_compressed_payload_md_override_2",
				 0, GRPC_COMPRESS_DEFLATE, GRPC_COMPRESS_NONE,
				 GRPC_COMPRESS_GZIP, GRPC_COMPRESS_NONE,
				 &gzip_compression_override, false,
				 /*ignored */ GRPC_COMPRESS_LEVEL_NONE,
				 false);
  ...
  request_with_payload_template (config,
				 "test_invoke_request_with_compressed_payload_md_override_3",
				 0, GRPC_COMPRESS_DEFLATE, GRPC_COMPRESS_NONE,
				 GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
...
static void
test_invoke_request_with_disabled_algorithm (grpc_end2end_test_config config)
{
...
void
compressed_payload (grpc_end2end_test_config config)
{
...
void
compressed_payload_pre_init (void)
{
}
