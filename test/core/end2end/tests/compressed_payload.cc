...
static grpc_end2end_test_fixture begin_test(grpc_end2end_test_config config, const char* test_name,
  ...
  gpr_log(GPR_INFO, "Running test: %s%s/%s", test_name,
          decompress_in_core ? "" : "_with_decompression_disabled", config.name);
...
static void request_for_disabled_algorithm(
    grpc_end2end_test_config config, const char* test_name, uint32_t send_flags_bitmask,
    grpc_compression_algorithm algorithm_to_disable,
    grpc_compression_algorithm requested_client_compression_algorithm,
    grpc_status_code expected_error, grpc_metadata* client_metadata, bool decompress_in_core) {
  grpc_call* c;
  grpc_call* s;
  ...
  server_args =
      grpc_channel_args_set_channel_default_compression_algorithm(nullptr, GRPC_COMPRESS_NONE);
  server_args =
      grpc_channel_args_compression_algorithm_set_state(&server_args, algorithm_to_disable, false);
  if (!decompress_in_core) {
    grpc_arg disable_decompression_in_core_arg = grpc_channel_arg_integer_create(
    ...
    client_args =
        grpc_channel_args_copy_and_add(client_args, &disable_decompression_in_core_arg, 1);
    server_args =
        grpc_channel_args_copy_and_add(server_args, &disable_decompression_in_core_arg, 1);
    grpc_channel_args_destroy(old_client_args);
    grpc_channel_args_destroy(old_server_args);
  }

  f = begin_test(config, test_name, client_args, server_args, decompress_in_core);
...
static void request_with_payload_template_inner(
    grpc_end2end_test_config config, const char* test_name, uint32_t client_send_flags_bitmask,
    ...
    grpc_compression_algorithm expected_algorithm_from_server, grpc_metadata* client_init_metadata,
  ...
  GPR_ASSERT(GPR_BITCOUNT(grpc_call_test_only_get_encodings_accepted_by_peer(s)) ==
             GRPC_COMPRESS_ALGORITHMS_COUNT);
  GPR_ASSERT(
      GPR_BITGET(grpc_call_test_only_get_encodings_accepted_by_peer(s), GRPC_COMPRESS_NONE) != 0);
  GPR_ASSERT(GPR_BITGET(grpc_call_test_only_get_encodings_accepted_by_peer(s),
                        GRPC_COMPRESS_DEFLATE) != 0);
  GPR_ASSERT(
      GPR_BITGET(grpc_call_test_only_get_encodings_accepted_by_peer(s), GRPC_COMPRESS_GZIP) != 0);
  memset(ops, 0, sizeof(ops));
  op = ops;
  op->op = GRPC_OP_SEND_INITIAL_METADATA;
  op->data.send_initial_metadata.count = 0;
  if (set_server_level) {
    ...
    op->data.send_initial_metadata.maybe_compression_level.level = server_compression_level;
  }
  op->flags = 0;
  op->reserved = nullptr;
  op++;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    GPR_ASSERT(request_payload_recv->data.raw.compression ==
               (decompress_in_core ? GRPC_COMPRESS_NONE : expected_algorithm_from_client));
    ...
    if (server_compression_level > GRPC_COMPRESS_LEVEL_NONE) {
      ...
      GPR_ASSERT(response_payload_recv->data.raw.compression ==
                 (decompress_in_core ? GRPC_COMPRESS_NONE : expected_algorithm_from_server));
    }
...
static void request_with_payload_template(
  ...
  request_with_payload_template_inner(
      config, test_name, client_send_flags_bitmask, default_client_channel_compression_algorithm,
      default_server_channel_compression_algorithm, expected_algorithm_from_client,
      expected_algorithm_from_server, client_init_metadata, set_server_level,
      server_compression_level, send_message_before_initial_metadata, false);
  request_with_payload_template_inner(
      config, test_name, client_send_flags_bitmask, default_client_channel_compression_algorithm,
      default_server_channel_compression_algorithm, expected_algorithm_from_client,
      expected_algorithm_from_server, client_init_metadata, set_server_level,
      server_compression_level, send_message_before_initial_metadata, true);
}

static void test_invoke_request_with_exceptionally_uncompressed_payload(
    grpc_end2end_test_config config) {
  request_with_payload_template(config,
...
static void test_invoke_request_with_uncompressed_payload(grpc_end2end_test_config config) {
  request_with_payload_template(config, "test_invoke_request_with_uncompressed_payload", 0,
...
static void test_invoke_request_with_compressed_payload(grpc_end2end_test_config config) {
  request_with_payload_template(config, "test_invoke_request_with_compressed_payload", 0,
...
static void test_invoke_request_with_server_level(grpc_end2end_test_config config) {
  request_with_payload_template(config, "test_invoke_request_with_server_level", 0,
                                GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
                                GRPC_COMPRESS_NONE /* ignored */, nullptr, true,
                                GRPC_COMPRESS_LEVEL_HIGH, false);
}
...
static void test_invoke_request_with_compressed_payload_md_override(
  ...
  identity_compression_override.value = grpc_slice_from_static_string("identity");
  ...
  request_with_payload_template(config, "test_invoke_request_with_compressed_payload_md_override_1",
                                0, GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE, GRPC_COMPRESS_GZIP,
                                GRPC_COMPRESS_NONE, &gzip_compression_override, false,
                                /*ignored*/ GRPC_COMPRESS_LEVEL_NONE, false);

  /* Channel default DEFLATE, call override to GZIP */
  request_with_payload_template(config, "test_invoke_request_with_compressed_payload_md_override_2",
                                0, GRPC_COMPRESS_DEFLATE, GRPC_COMPRESS_NONE, GRPC_COMPRESS_GZIP,
                                GRPC_COMPRESS_NONE, &gzip_compression_override, false,
                                /*ignored*/ GRPC_COMPRESS_LEVEL_NONE, false);

  /* Channel default DEFLATE, call override to NONE (aka IDENTITY) */
  request_with_payload_template(config, "test_invoke_request_with_compressed_payload_md_override_3",
                                0, GRPC_COMPRESS_DEFLATE, GRPC_COMPRESS_NONE, GRPC_COMPRESS_NONE,
...
static void test_invoke_request_with_disabled_algorithm(grpc_end2end_test_config config) {
  request_for_disabled_algorithm(config, "test_invoke_request_with_disabled_algorithm", 0,
