...
static void request_for_disabled_algorithm(
  ...
  GPR_ASSERT(grpc_compression_algorithm_name(algorithm_to_disable,
                                             &algo_name));
  std::string expected_details = absl::StrFormat(
      "Compression algorithm '%s' is disabled.", algo_name);
  /* and we expect a specific reason for it */
  GPR_ASSERT(0 ==
             grpc_slice_str_cmp(details, expected_details.c_str()));
...
static void request_with_payload_template_inner(
  ...
  GPR_ASSERT(
      GPR_BITGET(grpc_call_test_only_get_encodings_accepted_by_peer(s),
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (i > 0 || !send_message_before_initial_metadata) {
      ...
      error = grpc_call_start_batch(
          c, ops, static_cast<size_t>(op - ops), tag(2), nullptr);
      GPR_ASSERT(GRPC_CALL_OK == error);
    ...
    GPR_ASSERT(
        byte_buffer_eq_string(request_payload_recv, request_str));
    ...
    GPR_ASSERT(
        byte_buffer_eq_string(response_payload_recv, response_str));
