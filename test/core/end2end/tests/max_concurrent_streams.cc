...
static void test_max_concurrent_streams(grpc_end2end_test_config config) {
  ...
  GPR_ASSERT(c1);
  c2 = grpc_channel_create_call(f.client, nullptr, GRPC_PROPAGATE_DEFAULTS, f.cq,
                                grpc_slice_from_static_string("/beta"), nullptr, deadline, nullptr);
  GPR_ASSERT(c2);

  GPR_ASSERT(GRPC_CALL_OK == grpc_server_request_call(f.server, &s1, &call_details,
  ...
  op++;
  op->op = GRPC_OP_RECV_INITIAL_METADATA;
  op->data.recv_initial_metadata.recv_initial_metadata = &initial_metadata_recv1;
  op->flags = 0;
  op->reserved = nullptr;
  op++;
  ...
  while (!got_client_start || !got_server_start) {
    ev = grpc_completion_queue_next(f.cq, grpc_timeout_seconds_to_deadline(3), nullptr);
  ...
  GPR_ASSERT(GRPC_CALL_OK == grpc_server_request_call(f.server, &s2, &call_details,
                                                      &request_metadata_recv, f.cq, f.cq,
                                                      tag(201)));
...
static void test_max_concurrent_streams_with_timeout_on_first(grpc_end2end_test_config config) {
  grpc_end2end_test_fixture f;
  grpc_arg server_arg;
  grpc_channel_args server_args;
  grpc_call* c1;
  ...
  f = begin_test(config, "test_max_concurrent_streams_with_timeout_on_first", nullptr,
                 &server_args);
  cqv = cq_verifier_create(f.cq);
  ...
  /* start two requests - ensuring that the second is not accepted until
     the first completes */
  c1 = grpc_channel_create_call(f.client, nullptr, GRPC_PROPAGATE_DEFAULTS, f.cq,
  ...
  op++;
  op->op = GRPC_OP_RECV_INITIAL_METADATA;
  op->data.recv_initial_metadata.recv_initial_metadata = &initial_metadata_recv2;
  op->flags = 0;
  op->reserved = nullptr;
  op++;
  error = grpc_call_start_batch(c2, ops, static_cast<size_t>(op - ops), tag(402), nullptr);
...
static void test_max_concurrent_streams_with_timeout_on_second(grpc_end2end_test_config config) {
  grpc_end2end_test_fixture f;
  grpc_arg server_arg;
  grpc_channel_args server_args;
  grpc_call* c1;
  ...
  f = begin_test(config, "test_max_concurrent_streams_with_timeout_on_second", nullptr,
                 &server_args);
  cqv = cq_verifier_create(f.cq);
