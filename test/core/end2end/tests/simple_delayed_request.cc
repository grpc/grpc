...
static void
simple_delayed_request_body (grpc_end2end_test_config config,
			     grpc_end2end_test_fixture * f,
			     grpc_channel_args * client_args,
			     grpc_channel_args * server_args, long delay_us)
{
  grpc_call *c;
  grpc_call *s;
...
test_simple_delayed_request_short (grpc_end2end_test_config config)
{
  ...
  gpr_log (GPR_INFO, "Running test: %s/%s",
	   "test_simple_delayed_request_short", config.name);
...
test_simple_delayed_request_long (grpc_end2end_test_config config)
{
  ...
  gpr_log (GPR_INFO, "Running test: %s/%s",
	   "test_simple_delayed_request_long", config.name);
  f = config.create_fixture (nullptr, nullptr);
...
void
simple_delayed_request (grpc_end2end_test_config config)
{
...
void
simple_delayed_request_pre_init (void)
{
}
