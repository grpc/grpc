...
static void test_retry(CoreTestConfiguration config) {
  grpc_call* c;
  grpc_call* s;
  grpc_op ops[6];
  grpc_op* op;
  ...
  CoreTestFixture f = begin_test(config, "retry", &client_args, nullptr);
...
void retry(CoreTestConfiguration config) {
