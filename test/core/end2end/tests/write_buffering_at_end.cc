...
/* Client sends a request with payload, server reads then returns
 * status. */
static void test_invoke_request_with_payload(
  ...
  grpc_end2end_test_fixture f = begin_test(
      config, "test_invoke_request_with_payload", nullptr, nullptr);
  ...
  /* recv message should not succeed yet - it's buffered at the client
   * still */
  memset(ops, 0, sizeof(ops));
  op = ops;
  ...
  GPR_ASSERT(
      byte_buffer_eq_string(request_payload_recv1, "hello world"));
