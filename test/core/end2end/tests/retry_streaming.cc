...
static void test_retry_streaming(grpc_end2end_test_config config) {
  ...
  grpc_byte_buffer* request2_payload = grpc_raw_byte_buffer_create(&request2_payload_slice, 1);
  grpc_byte_buffer* request3_payload = grpc_raw_byte_buffer_create(&request3_payload_slice, 1);
  ...
  grpc_arg args[] = {
      ...
      grpc_channel_arg_integer_create(
          const_cast<char*>(GRPC_ARG_MAX_CHANNEL_TRACE_EVENT_MEMORY_PER_NODE), 1024 * 8),
  ...
  grpc_end2end_test_fixture f = begin_test(config, "retry_streaming", &client_args, nullptr);
  ...
  c = grpc_channel_create_call(f.client, nullptr, GRPC_PROPAGATE_DEFAULTS, f.cq,
                               ...
                               nullptr);
  grpc_core::channelz::ChannelNode* channelz_channel = grpc_channel_get_channelz_node(f.client);
  ...
  GPR_ASSERT(byte_buffer_eq_slice(request2_payload_recv, request2_payload_slice));
  ...
  op->data.recv_message.recv_message = &request2_payload_recv;
  op++;
  error = grpc_call_start_batch(s, ops, static_cast<size_t>(op - ops), tag(203), nullptr);
  GPR_ASSERT(GRPC_CALL_OK == error);
  ...
  op->data.send_status_from_server.status_details = &status_details;
  op++;
  error = grpc_call_start_batch(s, ops, static_cast<size_t>(op - ops), tag(205), nullptr);
  GPR_ASSERT(GRPC_CALL_OK == error);
  ...
  GPR_ASSERT(byte_buffer_eq_slice(request2_payload_recv, request2_payload_slice));
  grpc_byte_buffer_destroy(request2_payload_recv);
  GPR_ASSERT(byte_buffer_eq_slice(request3_payload_recv, request3_payload_slice));
