...
// the basic retry test, except that the client sends two messages on
// the call before the initial attempt fails.
// FIXME: We should also test the case where the retry is committed
// after replaying 1 of 2 previously-completed send_message ops.
// However, there's no way to trigger that from an end2end test, because
// the replayed ops happen under the hood -- they are not surfaced to
// the C-core API, and therefore we have no way to inject the commit at
// the right point.
static void test_retry_streaming(grpc_end2end_test_config config) {
  ...
  grpc_slice request2_payload_slice =
      grpc_slice_from_static_string("bar");
  grpc_slice request3_payload_slice =
      grpc_slice_from_static_string("baz");
  grpc_slice response_payload_slice =
      grpc_slice_from_static_string("quux");
