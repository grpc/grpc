...
static grpc_end2end_test_fixture begin_test(grpc_end2end_test_config config, const char* test_name,
  ...
  gpr_log(GPR_INFO, "Running test: %s/%s/request_status_early=%s", test_name, config.name,
...
static void test(grpc_end2end_test_config config, bool request_status_early,
  ...
  grpc_byte_buffer* response_payload1 = grpc_raw_byte_buffer_create(&response_payload1_slice, 1);
  grpc_slice response_payload2_slice = grpc_slice_from_copied_string("world");
  grpc_byte_buffer* response_payload2 = grpc_raw_byte_buffer_create(&response_payload2_slice, 1);
  grpc_end2end_test_fixture f =
      begin_test(config, "streaming_error_response", nullptr, nullptr, request_status_early);
