...
//   so no retry is done
static void test_retry_recv_trailing_metadata_error(grpc_end2end_test_config config) {
  grpc_call* c;
  grpc_call* s;
  grpc_op ops[6];
  grpc_op* op;
  ...
  grpc_end2end_test_fixture f =
      begin_test(config, "retry_recv_trailing_metadata_error", &client_args, nullptr);
...
class InjectStatusFilter {
 ...
 public:
  class CallData {
   ...
   private:
    CallData() {
      GRPC_CLOSURE_INIT(&recv_trailing_metadata_ready_, RecvTrailingMetadataReady, this, nullptr);
    }
    ...
    static void RecvTrailingMetadataReady(void* arg, grpc_error_handle /*error*/) {
      ...
      grpc_core::Closure::Run(
          ...
          grpc_error_set_int(GRPC_ERROR_CREATE_FROM_STATIC_STRING("injected error"),
...
bool MaybeAddFilter(grpc_channel_stack_builder* builder, void* /*arg*/) {
  ...
  // Install filter.
  return grpc_channel_stack_builder_prepend_filter(builder, &InjectStatusFilter::kFilterVtable,
                                                   nullptr, nullptr);
}
