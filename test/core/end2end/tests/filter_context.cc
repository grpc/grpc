...
static void test_request(grpc_end2end_test_config config) {
  ...
  grpc_end2end_test_fixture f = begin_test(config, "filter_context", nullptr, nullptr);
...
static void start_transport_stream_op_batch(grpc_call_element* elem,
  ...
  gpr_log(GPR_INFO, "start_transport_stream_op_batch(): context=%p", batch->payload->context);
...
static const grpc_channel_filter test_filter = {start_transport_stream_op_batch,
...
static void init_plugin(void) {
  ...
  grpc_channel_init_register_stage(GRPC_CLIENT_SUBCHANNEL, INT_MAX, maybe_add_filter,
                                   const_cast<grpc_channel_filter*>(&test_filter));
...
void filter_context_pre_init(void) { grpc_register_plugin(init_plugin, destroy_plugin); }
