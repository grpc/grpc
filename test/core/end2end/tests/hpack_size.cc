		     ...
		     grpc_end2end_test_fixture f, size_t index)
{
  ...
  extra_metadata[0].value =
    grpc_slice_from_static_string (hobbits[index % GPR_ARRAY_SIZE (hobbits)]
				   [0]);
...
static void
test_size (grpc_end2end_test_config config, int encode_size, int decode_size)
{
  size_t i;
  grpc_end2end_test_fixture f;
  grpc_arg server_arg;
...
void
hpack_size (grpc_end2end_test_config config)
{
