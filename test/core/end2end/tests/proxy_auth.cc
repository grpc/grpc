...
static void test_invoke_proxy_auth(grpc_end2end_test_config config) {
  ...
  grpc_end2end_test_fixture f = begin_test(config, "test_invoke_proxy_auth", &client_args, nullptr);
  simple_request_body(config, f);
  end_test(&f);
  config.tear_down_data(&f);
}

void proxy_auth(grpc_end2end_test_config config) { test_invoke_proxy_auth(config); }
