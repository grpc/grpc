...
static void
test_max_age_forcibly_close (grpc_end2end_test_config config)
{
  ...
  gpr_timespec expect_shutdown_time = grpc_timeout_milliseconds_to_deadline ((int) (MAX_CONNECTION_AGE_MS * MAX_CONNECTION_AGE_JITTER_MULTIPLIER) + MAX_CONNECTION_AGE_GRACE_MS + IMMEDIATE_SHUTDOWN_GRACE_TIME_MS);
...
static void
test_max_age_gracefully_close (grpc_end2end_test_config config)
{
  grpc_end2end_test_fixture f = config.create_fixture (nullptr, nullptr);
  cq_verifier *cqv = cq_verifier_create (f.cq);
  grpc_arg server_a[3];
...
void
max_connection_age (grpc_end2end_test_config config)
{
...
void
max_connection_age_pre_init (void)
{
}
