...
/* The amount of time we wait for the connection to time out, but after
   it the connection should not use up its grace period. It should be a
   number between MAX_CONNECTION_AGE_MS and MAX_CONNECTION_AGE_MS +
   MAX_CONNECTION_AGE_GRACE_MS */
#define CQ_MAX_CONNECTION_AGE_WAIT_TIME_S 1
/* The amount of time we wait after the connection reaches its max age,
   it should be shorter than CALL_DEADLINE_S -
...
static void test_max_age_forcibly_close(
  ...
  server_a[1].key =
      const_cast<char*>(GRPC_ARG_MAX_CONNECTION_AGE_GRACE_MS);
  ...
  gpr_timespec deadline =
      grpc_timeout_seconds_to_deadline(CALL_DEADLINE_S);
  grpc_op ops[6];
  grpc_op* op;
  ...
  gpr_timespec expect_shutdown_time =
  ...
  /* After the channel reaches its max age, we still do nothing here.
     And wait for it to use up its max age grace period. */
  CQ_EXPECT_COMPLETION(cqv, tag(1), true);
  cq_verify(cqv);

  gpr_timespec channel_shutdown_time = gpr_now(GPR_CLOCK_MONOTONIC);
  GPR_ASSERT(gpr_time_cmp(channel_shutdown_time, expect_shutdown_time) <
             0);
  ...
  /* The connection should be closed immediately after the max age grace
     period, the in-progress RPC should fail. */
...
static void test_max_age_gracefully_close(
  ...
  /* The connection is shutting down gracefully. In-progress rpc should
     not be
  ...
  /* The connection is closed gracefully with goaway, the rpc should
     still be completed. */
