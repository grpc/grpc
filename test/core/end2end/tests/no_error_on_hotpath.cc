...
static void test_no_error_on_hotpath_one_request(grpc_end2end_test_config config) {
  grpc_end2end_test_fixture f;
...
static void test_no_error_on_hotpath_10_requests(grpc_end2end_test_config config) {
  int i;
  grpc_end2end_test_fixture f =
      begin_test(config, "test_no_error_on_hotpath_in_one_request", nullptr, nullptr);
  // First RPC is not considered the hotpath, since there are lots of things to
  // set up.
  simple_request_body(config, f);
  grpc_disable_error_creation();
