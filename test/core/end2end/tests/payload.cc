...
static void
request_response_with_payload (grpc_end2end_test_config config,
			       grpc_end2end_test_fixture f)
{
  ...
  GPR_ASSERT (byte_buffer_eq_slice
	      (request_payload_recv, request_payload_slice));
  GPR_ASSERT (byte_buffer_eq_slice
	      (response_payload_recv, response_payload_slice));
...
{
  grpc_end2end_test_fixture f =
    begin_test (config, "test_invoke_request_response_with_payload", nullptr,
		nullptr);
...
test_invoke_10_request_response_with_payload (grpc_end2end_test_config config)
{
  int i;
  grpc_end2end_test_fixture f =
    begin_test (config, "test_invoke_10_request_response_with_payload",
		nullptr, nullptr);
  for (i = 0; i < 10; i++)
    {
      request_response_with_payload (config, f);
...
void
payload (grpc_end2end_test_config config)
{
