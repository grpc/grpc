...
static grpc_end2end_test_fixture begin_test(grpc_end2end_test_config config, const char* test_name,
  ...
  gpr_log(GPR_INFO, "Running test: %s/%s/%s/%s", test_name, config.name, mode.name,
...
static void test_cancel_after_round_trip(grpc_end2end_test_config config, cancellation_mode mode,
  ...
  grpc_byte_buffer* response_payload1 = grpc_raw_byte_buffer_create(&response_payload_slice, 1);
  grpc_byte_buffer* response_payload2 = grpc_raw_byte_buffer_create(&response_payload_slice, 1);
  int was_cancelled = 2;
  ...
  grpc_end2end_test_fixture f =
      begin_test(config, "cancel_after_round_trip", mode, use_service_config, args, nullptr);
  cq_verifier* cqv = cq_verifier_create(f.cq);

  gpr_timespec deadline =
      use_service_config ? gpr_inf_future(GPR_CLOCK_MONOTONIC) : five_seconds_from_now();
...
void cancel_after_round_trip(grpc_end2end_test_config config) {
  ...
  for (i = 0; i < GPR_ARRAY_SIZE(cancellation_modes); i++) {
    test_cancel_after_round_trip(config, cancellation_modes[i], false /* use_service_config */);
    if (config.feature_mask & FEATURE_MASK_SUPPORTS_CLIENT_CHANNEL &&
      ...
      test_cancel_after_round_trip(config, cancellation_modes[i], true /* use_service_config */);
    }
  }
}
