...
static void shutdown_server(grpc_end2end_test_fixture* f) {
  ...
  GPR_ASSERT(
      grpc_completion_queue_pluck(f->shutdown_cq, tag(1000), five_seconds_from_now(), nullptr)
...
static void test_keepalive_timeout(grpc_end2end_test_config config) {
  ...
  grpc_channel_args keepalive_args = {GPR_ARRAY_SIZE(keepalive_arg_elems), keepalive_arg_elems};

  grpc_end2end_test_fixture f = begin_test(config, "keepalive_timeout", &keepalive_args, nullptr);
...
static void test_read_delays_keepalive(grpc_end2end_test_config config) {
  ...
  grpc_channel_args keepalive_args = {GPR_ARRAY_SIZE(keepalive_arg_elems), keepalive_arg_elems};
  grpc_end2end_test_fixture f =
      begin_test(config, "test_read_delays_keepalive", &keepalive_args, nullptr);
  /* Disable ping ack to trigger the keepalive timeout */
  grpc_set_disable_ping_ack(true);
  grpc_call* c;
  grpc_call* s;
