...
static grpc_end2end_test_fixture begin_test(grpc_end2end_test_config config, const char* test_name,
  ...
  grpc_arg fake_security_name_override = {GRPC_ARG_STRING,
                                          const_cast<char*>(GRPC_SSL_TARGET_NAME_OVERRIDE_ARG),
                                          {const_cast<char*>("foo.test.google.fr:1234")}};
  grpc_channel_args* new_client_args =
      grpc_channel_args_copy_and_add(client_args, &fake_security_name_override, 1);
...
static void test_invoke_simple_request(grpc_end2end_test_config config) {
  ...
  c = grpc_channel_create_call(
      f.client, nullptr, GRPC_PROPAGATE_DEFAULTS, f.cq, grpc_slice_from_static_string("/foo"),
      get_host_override_slice("foo.test.google.fr:1234", config), deadline, nullptr);
  GPR_ASSERT(c);
...
void call_host_override(grpc_end2end_test_config config) { test_invoke_simple_request(config); }
