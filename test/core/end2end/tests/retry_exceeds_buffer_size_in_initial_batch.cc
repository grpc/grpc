...
static void test_retry_exceeds_buffer_size_in_initial_batch(grpc_end2end_test_config config) {
  grpc_call* c;
  grpc_call* s;
  grpc_op ops[6];
  grpc_op* op;
  ...
  grpc_arg args[] = {
      ...
      grpc_channel_arg_string_create(
          ...
          const_cast<char*>("{\n"
                            ...
                            "  } ]\n"
                            "}")),
      grpc_channel_arg_integer_create(const_cast<char*>(GRPC_ARG_PER_RPC_RETRY_BUFFER_SIZE), 2),
  };
  grpc_channel_args client_args = {GPR_ARRAY_SIZE(args), args};
  grpc_end2end_test_fixture f =
      begin_test(config, "retry_exceeds_buffer_size_in_initial_batch", &client_args, nullptr);
