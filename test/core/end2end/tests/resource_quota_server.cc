...
void
resource_quota_server (grpc_end2end_test_config config)
{
...
#define SERVER_END_BASE_TAG 0x4000
  ...
  grpc_slice *details =
    (grpc_slice *) malloc (sizeof (grpc_slice) * NUM_CALLS);
  ...
  for (int i = 0; i < NUM_CALLS; i++)
    {
      error =
	grpc_server_request_call (f.server, &server_calls[i],
				  &call_details[i], &request_metadata_recv[i],
				  f.cq, f.cq,
  ...
  for (int i = 0; i < NUM_CALLS; i++)
	...
	&trailing_metadata_recv[i];
      ...
      op++;
      error =
	grpc_call_start_batch (client_calls[i], ops, (size_t) (op - ops),
  ...
  while (pending_client_calls + pending_server_recv_calls +
      ...
      else if (ev_tag < SERVER_START_BASE_TAG)
	{
	  ...
	  switch (status[call_id])
	    ...
	    case GRPC_STATUS_OK:
	      break;
	    default:
	      gpr_log (GPR_ERROR, "Unexpected status code: %d",
		       status[call_id]);
	      abort ();
	    }
      ...
      else if (ev_tag < SERVER_RECV_BASE_TAG)
	{
	  ...
	  op++;
	  error =
	    grpc_call_start_batch (server_calls[call_id], ops,
				   (size_t) (op - ops),
				   tag (SERVER_RECV_BASE_TAG + call_id),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      else if (ev_tag < SERVER_END_BASE_TAG)
	{
	  ...
	  error =
	    grpc_call_start_batch (server_calls[call_id], ops,
				   (size_t) (op - ops),
				   tag (SERVER_END_BASE_TAG + call_id),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
...
void
resource_quota_server_pre_init (void)
{
}
