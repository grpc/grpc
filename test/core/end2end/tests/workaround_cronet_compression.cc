...
static void
test_workaround_cronet_compression (grpc_end2end_test_config config)
{
  for (uint32_t i = 0; i < workaround_configs_num; i++)
    {
      request_with_payload_template (config,
				     ...
				     0, GRPC_COMPRESS_GZIP,
				     GRPC_COMPRESS_GZIP, GRPC_COMPRESS_GZIP,
				     workaround_configs[i].
				     expected_algorithm_from_server, nullptr,
				     false,
...
void
workaround_cronet_compression (grpc_end2end_test_config config)
{
...
void
workaround_cronet_compression_pre_init (void)
{
}
