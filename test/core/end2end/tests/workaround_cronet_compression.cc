...
static void request_with_payload_template(
  ...
  if (user_agent_override) {
    ...
    client_args =
        grpc_channel_args_copy_and_add(client_args_old, &arg, 1);
...
static workaround_cronet_compression_config workaround_configs[] = {
    ...
    {const_cast<char*>("grpc-objc/1.3.0-dev grpc-c/3.0.0-dev (ios; "
                       "cronet_http; gentle)"),
     GRPC_COMPRESS_NONE},
    {const_cast<char*>(
...
static void test_workaround_cronet_compression(
  ...
  for (uint32_t i = 0; i < workaround_configs_num; i++) {
    request_with_payload_template(
        ...
        workaround_configs[i].expected_algorithm_from_server, nullptr,
        false,
        /* ignored */ GRPC_COMPRESS_LEVEL_NONE,
        workaround_configs[i].user_agent_override, true);
    request_with_payload_template(
        config,
        "test_invoke_request_with_compressed_payload_with_compression_"
        "disabled",
