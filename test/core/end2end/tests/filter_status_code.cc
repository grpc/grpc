...
static void
test_request (grpc_end2end_test_config config)
{
  grpc_call *c;
  grpc_call *s;
  grpc_end2end_test_fixture f =
  ...
  c =
    grpc_channel_create_call (f.client, nullptr, GRPC_PROPAGATE_DEFAULTS,
			      ...
			      get_host_override_slice ("foo.test.google.fr",
						       config), deadline,
			      nullptr);
  GPR_ASSERT (c);
  gpr_mu_lock (&g_mu);
...
static void
client_destroy_call_elem (grpc_call_element * elem,
...
static void
server_destroy_call_elem (grpc_call_element * elem,
			  const grpc_call_final_info * final_info,
			  grpc_closure * ignored)
{
  final_status_data *data = (final_status_data *) elem->call_data;
...
static bool
maybe_add_filter (grpc_channel_stack_builder * builder, void *arg)
{
  ...
  if (g_enable_filter)
	...
	grpc_channel_stack_builder_create_iterator_at_last (builder);
      ...
      const bool retval =
	grpc_channel_stack_builder_add_filter_before (it, filter, nullptr,
						      nullptr);
...
void
filter_status_code (grpc_end2end_test_config config)
{
...
void
filter_status_code_pre_init (void)
{
