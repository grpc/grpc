...
static void request_for_disabled_algorithm(
  ...
  GPR_ASSERT(grpc_compression_algorithm_name(algorithm_to_disable,
                                             &algo_name));
  std::string expected_details = absl::StrCat(
      "Compression algorithm '", algo_name, "' is disabled.");
...
static void test_invoke_request_with_compressed_payload_md_override(
  ...
  request_with_payload_template(
      ...
      /*ignored*/ GRPC_COMPRESS_LEVEL_NONE, false, false,
      GRPC_COMPRESS_NONE);
...
void stream_compression_compressed_payload(
    grpc_end2end_test_config config) {
