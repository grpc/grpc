...
shutdown_server (grpc_end2end_test_fixture * f)
{
  ...
  grpc_event ev =
    grpc_completion_queue_next (f->cq, grpc_timeout_seconds_to_deadline (5),
				nullptr);
...
static void
test_max_message_length_on_request (grpc_end2end_test_config config,
				    bool send_limit,
				    ...
				    bool use_string_json_value)
{
  ...
  GPR_ASSERT (0 ==
	      grpc_slice_str_cmp (call_details.method, "/service/method"));
...
static void
test_max_message_length_on_response (grpc_end2end_test_config config,
				     bool send_limit,
				     bool use_service_config,
				     bool use_string_json_value)
{
  ...
  if (use_service_config)
    {
      ...
      arg.type = GRPC_ARG_STRING;
      arg.key = const_cast < char *>(GRPC_ARG_SERVICE_CONFIG);
      arg.value.string = const_cast < char *>(use_string_json_value
					      ? "{\n"
					      "  \"methodConfig\": [ {\n"
					      "    \"name\": [\n"
					      "      { \"service\": \"service\", \"method\": \"method\" }\n"
...
void
max_message_length (grpc_end2end_test_config config)
{
...
void
max_message_length_pre_init (void)
{
}
