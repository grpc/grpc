...
static void test_cancel_after_accept(grpc_end2end_test_config config, cancellation_mode mode,
  ...
  grpc_end2end_test_fixture f =
      begin_test(config, "cancel_after_accept", mode, use_service_config, args, nullptr);
...
void cancel_after_accept(grpc_end2end_test_config config) {
  ...
  for (i = 0; i < GPR_ARRAY_SIZE(cancellation_modes); i++) {
    test_cancel_after_accept(config, cancellation_modes[i], false /* use_service_config */);
    if (config.feature_mask & FEATURE_MASK_SUPPORTS_CLIENT_CHANNEL &&
      ...
      test_cancel_after_accept(config, cancellation_modes[i], true /* use_service_config */);
    }
  }
}
