...
static void
done_write (void *arg, grpc_error * error)
{
...
{
  grpc_slice slice =
    grpc_slice_from_copied_buffer (state.response_payload,
...
static void
handle_read (void *arg, grpc_error * error)
{
  GPR_ASSERT (error == GRPC_ERROR_NONE);
...
static void
on_connect (void *arg, grpc_endpoint * tcp,
...
static void
start_rpc (int target_port, grpc_status_code expected_status,
	   const char *expected_detail)
{
  ...
  state.channel =
    grpc_insecure_channel_create (state.target, nullptr, nullptr);
  grpc_slice host = grpc_slice_from_static_string ("localhost");
  state.call =
    grpc_channel_create_call (state.channel, nullptr, GRPC_PROPAGATE_DEFAULTS,
			      state.cq,
  ...
  op++;
  error =
    grpc_call_start_batch (state.call, ops, (size_t) (op - ops), tag (1),
			   nullptr);
...
static void
run_test (const char *response_payload,
	  size_t response_payload_length,
	  grpc_status_code expected_status, const char *expected_detail)
{
...
main (int argc, char **argv)
{
  ...
  run_test (HTTP2_RESP (204), sizeof (HTTP2_RESP (204)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (204));

  run_test (HTTP2_RESP (206), sizeof (HTTP2_RESP (206)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (206));

  run_test (HTTP2_RESP (304), sizeof (HTTP2_RESP (304)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (304));

  run_test (HTTP2_RESP (400), sizeof (HTTP2_RESP (400)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (400));

  run_test (HTTP2_RESP (404), sizeof (HTTP2_RESP (404)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (404));

  run_test (HTTP2_RESP (500), sizeof (HTTP2_RESP (500)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (500));

  /* status not defined in hpack static table */
  run_test (HTTP2_RESP (401), sizeof (HTTP2_RESP (401)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (401));

  run_test (HTTP2_RESP (403), sizeof (HTTP2_RESP (403)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (403));

  run_test (HTTP2_RESP (502), sizeof (HTTP2_RESP (502)) - 1,
	    GRPC_STATUS_CANCELLED, HTTP2_DETAIL_MSG (502));
