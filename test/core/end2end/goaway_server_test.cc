...
static void (*iomgr_resolve_address) (const char *addr,
				      const char *default_port,
				      grpc_pollset_set * interested_parties,
				      grpc_closure * on_done,
				      grpc_resolved_addresses ** addresses);

static grpc_ares_request *(*iomgr_dns_lookup_ares) (const char *dns_server,
						    const char *addr,
						    const char *default_port,
						    grpc_pollset_set *
						    interested_parties,
		    ...
		    bool check_grpclb, char **service_config_json)
{
  ...
  if (g_resolve_port < 0)
	...
	static_cast <
	struct sockaddr_in *>(gpr_zalloc (sizeof (struct sockaddr_in)));
      sa->sin_family = AF_INET;
      sa->sin_addr.s_addr = htonl (0x7f000001);
      sa->sin_port = htons ((uint16_t) g_resolve_port);
      grpc_lb_addresses_set_address (*lb_addrs, 0, sa, sizeof (*sa), false,
				     nullptr, nullptr);
      gpr_free (sa);
      gpr_mu_unlock (&g_mu);
    }
...
main (int argc, char **argv)
{
  ...
  GPR_ASSERT (GRPC_CALL_OK ==
	      grpc_server_request_call (server1, &server_call1,
  ...
  GPR_ASSERT (GRPC_CALL_OK ==
	      grpc_server_request_call (server2, &server_call2,
