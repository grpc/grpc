...
static void
end (input_stream * inp)
{
  inp->cur = inp->end;
}

static char *
read_string (input_stream * inp, bool * special)
{
  char *str = nullptr;
  size_t cap = 0;
  size_t sz = 0;
...
static void
read_buffer (input_stream * inp, char **buffer, size_t * length,
	     bool * special)
{
...
static grpc_slice
maybe_intern (grpc_slice s, bool intern)
{
...
static grpc_slice
read_string_like_slice (input_stream * inp)
{
  bool special;
...
static grpc_slice
read_buffer_like_slice (input_stream * inp)
{
  char *buffer;
  size_t length;
  bool special;
...
static int
read_int (input_stream * inp)
{
  return (int) read_uint32 (inp);
}

static grpc_channel_args *
read_args (input_stream * inp)
{
  size_t n = next_byte (inp);
  grpc_arg *args =
    static_cast < grpc_arg * >(gpr_malloc (sizeof (*args) * n));
...
static void
cred_artifact_ctx_finish (cred_artifact_ctx * ctx)
{
...
static const char *
read_cred_artifact (cred_artifact_ctx * ctx, input_stream * inp,
...
read_ssl_channel_creds (input_stream * inp)
{
  ...
  const char *root_certs =
    read_cred_artifact (&ctx, inp, builtin_root_certs,
			GPR_ARRAY_SIZE (builtin_root_certs));
  const char *private_key =
    read_cred_artifact (&ctx, inp, builtin_private_keys,
			GPR_ARRAY_SIZE (builtin_private_keys));
  ...
  grpc_channel_credentials *creds = grpc_ssl_credentials_create (root_certs,
								 private_key
								 != nullptr
								 && certs !=
								 nullptr ?
...
read_call_creds (input_stream * inp, int depth)
{
  if (depth > 64)
    {
  ...
  switch (next_byte (inp))
	...
	const char *auth_selector =
	  read_cred_artifact (&ctx, inp, nullptr, 0);
	grpc_call_credentials *out = auth_token == nullptr
	  || auth_selector ==
	  nullptr ? nullptr : grpc_google_iam_credentials_create (auth_token,
								  auth_selector,
								  nullptr);
	cred_artifact_ctx_finish (&ctx);
	return out;
...
static grpc_channel_credentials *
read_channel_creds (input_stream * inp)
{
  switch (next_byte (inp))
    {
    case 0:
...
static bool
is_eof (input_stream * inp)
{
  return inp->cur == inp->end;
}
...
static void
finish_resolve (void *arg, grpc_error * error)
{
  ...
  if (error == GRPC_ERROR_NONE && 0 == strcmp (r->addr, "server"))
      ...
      else if (r->lb_addrs != nullptr)
	{
	  ...
	  grpc_lb_addresses_set_address (lb_addrs, 0, nullptr, 0, false,
					 nullptr, nullptr);
	  *r->lb_addrs = lb_addrs;
	}
		    ...
		    grpc_resolved_addresses ** addresses)
{
  ...
  r->addrs = addresses;
  r->lb_addrs = nullptr;
  grpc_timer_init (&r->timer,
		   GPR_MS_PER_SEC + grpc_core::ExecCtx::Get ()->Now (),
		   GRPC_CLOSURE_CREATE (finish_resolve, r,
...
extern void (*grpc_tcp_client_connect_impl) (grpc_closure * closure,
					     grpc_endpoint ** ep,
					     grpc_pollset_set *
					     interested_parties,
...
static void
do_connect (void *arg, grpc_error * error)
{
...
static void
sched_connect (grpc_closure * closure, grpc_endpoint ** ep,
	       gpr_timespec deadline)
{
  ...
  future_connect *fc =
    static_cast < future_connect * >(gpr_malloc (sizeof (*fc)));
  fc->closure = closure;
  fc->ep = ep;
  fc->deadline = deadline;
  grpc_timer_init (&fc->timer,
		   GPR_MS_PER_SEC + grpc_core::ExecCtx::Get ()->Now (),
		   GRPC_CLOSURE_CREATE (do_connect, fc,
					grpc_schedule_on_exec_ctx));
}

static void
my_tcp_client_connect (grpc_closure * closure, grpc_endpoint ** ep,
		       grpc_pollset_set * interested_parties,
...
static validator *
create_validator (void (*validate) (void *arg, bool success), void *arg)
{
...
static void
assert_success_and_decrement (void *counter, bool success)
{
  GPR_ASSERT (success);
  --*(int *) counter;
}
...
static void
decrement (void *counter, bool success)
{
  --*(int *) counter;
}
...
static connectivity_watch *
make_connectivity_watch (gpr_timespec s, int *counter)
{
...
static void
validate_connectivity_watch (void *p, bool success)
{
...
typedef enum
{ ROOT, CLIENT, SERVER, PENDING_SERVER } call_state_type;
...
static call_state *
new_call (call_state * sibling, call_state_type type)
{
...
static call_state *
maybe_delete_call_state (call_state * call)
{
  call_state *next = call->next;

  if (call->call != nullptr)
    return next;
  if (call->pending_ops != 0)
    return next;
...
static void
add_to_free (call_state * call, void *p)
{
...
static grpc_slice *
add_slice_to_unref (call_state * call, grpc_slice s)
{
...
static void
read_metadata (input_stream * inp, size_t * count,
  ...
  if (*count)
    {
      *metadata =
	static_cast <
	grpc_metadata * >(gpr_malloc (*count * sizeof (**metadata)));
...
static void
finished_request_call (void *csp, bool success)
{
...
static void
finished_batch (void *p, bool success)
{
...
static validator *
make_finished_batch_validator (call_state * cs, uint8_t has_ops)
{
...
LLVMFuzzerTestOneInput (const uint8_t * data, size_t size)
{
  ...
  while (!is_eof (&inp) || g_channel != nullptr || g_server != nullptr ||
      ...
      if (is_eof (&inp))
	{
	  ...
	  if (g_server != nullptr)
	    {
	      if (!server_shutdown)
		{
		  grpc_server_shutdown_and_notify (g_server, cq,
						   create_validator
	  ...
	  g_now =
	    gpr_time_add (g_now, gpr_time_from_seconds (1, GPR_TIMESPAN));
	}
	...
	case 1:
	  {
	    g_now =
	      gpr_time_add (g_now,
			    gpr_time_from_micros (read_uint32 (&inp),
						  GPR_TIMESPAN));
	    break;
	  }
	...
	case 2:
	  {
		...
		g_channel =
		  grpc_insecure_channel_create (target_uri, args, nullptr);
		GPR_ASSERT (g_channel != nullptr);
		{
		  grpc_core::ExecCtx exec_ctx;
		...
		if (st != GRPC_CHANNEL_SHUTDOWN)
		  {
		    gpr_timespec deadline =
		      gpr_time_add (gpr_now (GPR_CLOCK_REALTIME),
				    gpr_time_from_micros (read_uint32 (&inp),
							  GPR_TIMESPAN));
		    grpc_channel_watch_connectivity_state (g_channel, st,
							   deadline, cq,
							   create_validator
	...
	case 10:
	  {
	    ...
	    if (g_active_call->type != ROOT)
	      {
		if (g_active_call->call == nullptr
		    || g_active_call->type == CLIENT)
		  {
		    end (&inp);
		    break;
		  }
		...
		cs->call =
		  grpc_channel_create_call (g_channel, parent_call,
					    propagation_mask, cq, method,
	...
	case 12:
	  {
	    ...
	    grpc_op *ops =
	      static_cast <
	      grpc_op * >(gpr_malloc (sizeof (grpc_op) * num_ops));
	    if (num_ops > 0)
		...
		switch (next_byte (&inp))
		  ...
		  case GRPC_OP_SEND_STATUS_FROM_SERVER:
		    ...
		    read_metadata (&inp,
				   &op->data.send_status_from_server.
				   trailing_metadata_count,
				   &op->data.send_status_from_server.
				   trailing_metadata, g_active_call);
	    ...
	    if (ok)
	      {
		validator *v =
		  make_finished_batch_validator (g_active_call, has_ops);
		g_active_call->pending_ops++;
		grpc_call_error error =
		  grpc_call_start_batch (g_active_call->call, ops, num_ops, v,
					 nullptr);
	...
	case 19:
	  {
	    ...
	    grpc_call_error error =
	      grpc_server_request_call (g_server, &cs->call,
					&cs->call_details,
					&cs->recv_initial_metadata, cq, cq,
					v);
	    if (error != GRPC_CALL_OK)
	      {
		v->validate (v->arg, false);
	...
	case 22:
	  {
		...
		g_channel =
		  grpc_secure_channel_create (creds, target_uri, args,
					      nullptr);
		GPR_ASSERT (g_channel != nullptr);
		{
		  grpc_core::ExecCtx exec_ctx;
  ...
  GPR_ASSERT (grpc_completion_queue_next
	      (cq, gpr_inf_past (GPR_CLOCK_REALTIME),
