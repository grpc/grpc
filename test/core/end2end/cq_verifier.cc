...
static int
has_metadata (const grpc_metadata * md, size_t count, const char *key,
	      const char *value)
{
  size_t i;
  for (i = 0; i < count; i++)
...
int
contains_metadata (grpc_metadata_array * array, const char *key,
		   const char *value)
{
...
static int
has_metadata_slices (const grpc_metadata * md, size_t count,
...
int
contains_metadata_slices (grpc_metadata_array * array, grpc_slice key,
			  grpc_slice value)
{
...
static grpc_slice
merge_slices (grpc_slice * slices, size_t nslices)
{
  size_t i;
  size_t len = 0;
  uint8_t *cursor;
...
int
raw_byte_buffer_eq_slice (grpc_byte_buffer * rbb, grpc_slice b)
{
  grpc_slice a;
  int ok;
...
int
byte_buffer_eq_slice (grpc_byte_buffer * bb, grpc_slice b)
{
...
static bool
is_probably_integer (void *p)
{
  return ((uintptr_t) p) < 1000000;
}
...
void
cq_verify_empty_timeout (cq_verifier * v, int timeout_sec)
{
  gpr_timespec deadline = gpr_time_add (gpr_now (GPR_CLOCK_REALTIME),
					gpr_time_from_seconds (timeout_sec,
							       GPR_TIMESPAN));
  grpc_event ev;
...
{
  cq_verify_empty_timeout (v, 1);
}

static void
add (cq_verifier * v, const char *file, int line,
...
void
cq_expect_completion (cq_verifier * v, const char *file, int line, void *tag,
		      bool success)
{
