...
struct Expectation {
  Expectation(const char* f, int l, grpc_completion_type t, void* tag_arg, bool check_success_arg,
              int success_arg, bool* seen_arg)
      : file(f),
        line(l),
        type(t),
        tag(tag_arg),
...
static int has_metadata(const grpc_metadata* md, size_t count, const char* key, const char* value) {
  size_t i;
  for (i = 0; i < count; i++) {
    if (0 == grpc_slice_str_cmp(md[i].key, key) && 0 == grpc_slice_str_cmp(md[i].value, value)) {
      return 1;
    }
  }
  return 0;
...
static int has_metadata_slices(const grpc_metadata* md, size_t count, grpc_slice key,
                               grpc_slice value) {
  size_t i;
  for (i = 0; i < count; i++) {
...
int raw_byte_buffer_eq_slice(grpc_byte_buffer* rbb, grpc_slice b) {
  ...
  a = merge_slices(rbb->data.raw.slice_buffer.slices, rbb->data.raw.slice_buffer.count);
...
int byte_buffer_eq_slice(grpc_byte_buffer* bb, grpc_slice b) {
  if (bb->data.raw.compression > GRPC_COMPRESS_NONE) {
    ...
    GPR_ASSERT(grpc_msg_decompress(
        grpc_compression_algorithm_to_message_compression_algorithm(bb->data.raw.compression),
        &bb->data.raw.slice_buffer, &decompressed_buffer));
    grpc_byte_buffer* rbb =
        grpc_raw_byte_buffer_create(decompressed_buffer.slices, decompressed_buffer.count);
...
static bool is_probably_integer(void* p) { return reinterpret_cast<uintptr_t>(p) < 1000000; }
...
std::string ExpectationString(const Expectation& e) {
  ...
  if (is_probably_integer(e.tag)) {
    out = absl::StrFormat("tag(%" PRIdPTR ") ", reinterpret_cast<intptr_t>(e.tag));
  } else {
...
static void fail_no_event_received(cq_verifier* v) {
  gpr_log(GPR_ERROR, "no event received, but expected:%s", ExpectationsString(*v).c_str());
  abort();
}
...
void cq_verify(cq_verifier* v, int timeout_sec) {
  ...
  while (!v->expectations.empty()) {
    ...
    gpr_log(GPR_ERROR, "cq returned unexpected event: %s", grpc_event_string(&ev).c_str());
...
void cq_verify_empty_timeout(cq_verifier* v, int timeout_sec) {
  gpr_timespec deadline =
      gpr_time_add(gpr_now(GPR_CLOCK_REALTIME), gpr_time_from_seconds(timeout_sec, GPR_TIMESPAN));
  grpc_event ev;
  ...
  if (ev.type != GRPC_QUEUE_TIMEOUT) {
    gpr_log(GPR_ERROR, "unexpected event (expected nothing): %s", grpc_event_string(&ev).c_str());
    abort();
  }
}
...
void cq_maybe_expect_completion(cq_verifier* v, const char* file, int line, void* tag, bool success,
                                bool* seen) {
  v->maybe_expectations.emplace_back(file, line, GRPC_OP_COMPLETE, tag, true /* check_success */,
                                     success, seen);
}

static void add(cq_verifier* v, const char* file, int line, grpc_completion_type type, void* tag,
                bool check_success, bool success) {
  v->expectations.emplace_back(file, line, type, tag, check_success, success, nullptr);
}
