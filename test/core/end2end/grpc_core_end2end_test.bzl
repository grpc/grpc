...
def grpc_core_end2end_test_suite(name, config_src, deps = [], shard_count = 1, enable_fuzzing = True, tags = [], flaky = False):
    ...
    for test in _TESTS:
        grpc_cc_test(
            name = "%s_%s_test" % (name, test),
            srcs = [config_src, "tests/%s.cc" % test],
            external_deps = [
                "absl/log:log",
                "gtest",
                "gtest_main",
