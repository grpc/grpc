...
gpr_timespec five_seconds_time() {
  return grpc_timeout_seconds_to_deadline(5);
}

grpc_server* server_create(grpc_completion_queue* cq,
  ...
  grpc_ssl_pem_key_cert_pair pem_cert_key_pair = {server_key,
                                                  server_cert};
  ...
  GPR_ASSERT(grpc_server_add_secure_http2_port(server, server_addr,
                                               server_creds));
...
grpc_channel* client_create(const char* server_addr,
  ...
  GPR_ASSERT(GRPC_LOG_IF_ERROR(
      "load_file", grpc_load_file(CLIENT_KEY_PATH, 1, &key_slice)));
  const char* ca_cert =
      reinterpret_cast<const char*> GRPC_SLICE_START_PTR(ca_slice);
  ...
  grpc_channel_args* client_args = grpc_channel_args_copy_and_add(
      nullptr, args, GPR_ARRAY_SIZE(args));

  grpc_channel* client = grpc_secure_channel_create(
      client_creds, server_addr, client_args, nullptr);
