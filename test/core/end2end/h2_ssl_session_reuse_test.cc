...
grpc_server* server_create(grpc_completion_queue* cq, const char* server_addr) {
  ...
  ABSL_CHECK(grpc_server_add_http2_port(server, server_addr, server_creds));
...
void do_round_trip(grpc_completion_queue* cq, grpc_server* server,
  ...
  ABSL_CHECK_NE(property, nullptr);
  if (expect_session_reuse) {
    ABSL_CHECK_EQ(strcmp(property->value, "true"), 0);
  } else {
    ABSL_CHECK_EQ(strcmp(property->value, "false"), 0);
  }
