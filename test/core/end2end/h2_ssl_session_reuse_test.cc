...
grpc_server* server_create(grpc_completion_queue* cq, const char* server_addr) {
  ...
  GRPC_CHECK(grpc_server_add_http2_port(server, server_addr, server_creds));
...
void do_round_trip(grpc_completion_queue* cq, grpc_server* server,
  ...
  GRPC_CHECK_NE(property, nullptr);
  if (expect_session_reuse) {
    GRPC_CHECK_EQ(std::string(property->value), "true");
  } else {
    GRPC_CHECK_EQ(std::string(property->value), "false");
  }
