...
static void
log_resolved_addrs (const char *label, const char *hostname)
{
...
void
test_connect (const char *server_host, const char *client_host, int port,
	      int expect_ok)
{
  ...
  GPR_ASSERT ((got_port =
	       grpc_server_add_insecure_http2_port (server,
						    server_hostport)) > 0);
  if (port == 0)
    {
      port = got_port;
  ...
  if (client_host[0] == 'i')
    {
      ...
      uri_slice =
	grpc_slice_new ((char *) client_host, strlen (client_host),
			do_nothing);
      grpc_slice_buffer_init (&uri_parts);
      grpc_slice_split (uri_slice, ",", &uri_parts);
      hosts_with_port =
	static_cast <
	char **>(gpr_malloc (sizeof (char *) * uri_parts.count));
  ...
  gpr_log (GPR_INFO, "Testing with server=%s client=%s (expecting %s)",
	   server_hostport, client_hostport,
  ...
  if (expect_ok)
    {
      ...
      op++;
      error =
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (102),
			       nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      GPR_ASSERT (0 ==
		  grpc_slice_str_cmp (call_details.host,
				      "foo.test.google.fr"));
...
main (int argc, char **argv)
{
  ...
  for (grpc_forbid_dualstack_sockets_for_testing = 0;
       ...
       grpc_forbid_dualstack_sockets_for_testing++)
    {
      ...
      test_connect ("0.0.0.0", "ipv6:[::ffff:127.0.0.1],[::ffff:127.0.0.2]",
		    0, 1);
