...
static void server_setup_transport(void* ts,
  ...
  grpc_end2end_test_fixture* f =
      static_cast<grpc_end2end_test_fixture*>(ts);
  grpc_core::ExecCtx exec_ctx;
  grpc_endpoint_pair* sfd =
      static_cast<grpc_endpoint_pair*>(f->fixture_data);
  grpc_endpoint_add_to_pollset(sfd->server, grpc_cq_pollset(f->cq));
  grpc_error_handle error = f->server->core_server->SetupTransport(
      transport, nullptr, f->server->core_server->channel_args(),
      nullptr);
...
static void client_setup_transport(void* ts,
  ...
  grpc_channel_args* args = grpc_channel_args_copy_and_add(
      cs->client_args, &authority_arg, 1);
  ...
  if (cs->f->client != nullptr) {
    ...
    GRPC_ERROR_UNREF(error);
    cs->f->client = grpc_lame_client_channel_create(nullptr, status,
                                                    "lame channel");
    grpc_transport_destroy(transport);
  }
}
...
static grpc_end2end_test_fixture chttp2_create_fixture_socketpair(
  ...
  grpc_endpoint_pair* sfd = static_cast<grpc_endpoint_pair*>(
      gpr_malloc(sizeof(grpc_endpoint_pair)));
...
static void chttp2_init_client_socketpair(
  ...
  cs.f = f;
  transport =
      grpc_create_chttp2_transport(client_args, sfd->client, true);
...
static void chttp2_init_server_socketpair(
  ...
  transport =
      grpc_create_chttp2_transport(server_args, sfd->server, false);
