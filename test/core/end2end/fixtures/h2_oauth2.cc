...
find_metadata (const grpc_metadata * md,
	       size_t md_count, const char *key, const char *value)
{
  size_t i;
...
static void
process_oauth2_success (void *state, grpc_auth_context * ctx,
...
static void
process_oauth2_failure (void *state, grpc_auth_context * ctx,
			const grpc_metadata * md, size_t md_count,
			grpc_process_auth_metadata_done_cb cb,
			void *user_data)
{
