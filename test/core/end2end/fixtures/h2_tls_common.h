  ...
  switch (provider_type) {
    case SecurityPrimitives::ProviderType::STATIC_PROVIDER: {
      ...
      grpc_tls_identity_pairs_add_pair(client_pairs, private_key.c_str(),
                                       identity_cert.c_str());
      ffd->client_provider = grpc_tls_certificate_provider_in_memory_create();
      GPR_ASSERT(grpc_tls_certificate_provider_in_memory_set_root_cert(
                     ffd->client_provider, root_cert.c_str(), nullptr) ==
                 GRPC_STATUS_OK);
      GPR_ASSERT(grpc_tls_certificate_provider_in_memory_set_key_cert_pairs(
                     ffd->client_provider, client_pairs, nullptr) ==
                 GRPC_STATUS_OK);
      ...
      grpc_tls_identity_pairs_add_pair(server_pairs, private_key.c_str(),
                                       identity_cert.c_str());
      ffd->server_provider = grpc_tls_certificate_provider_in_memory_create();
      GPR_ASSERT(grpc_tls_certificate_provider_in_memory_set_root_cert(
                     ffd->server_provider, root_cert.c_str(), nullptr) ==
                 GRPC_STATUS_OK);
      GPR_ASSERT(grpc_tls_certificate_provider_in_memory_set_key_cert_pairs(
                     ffd->server_provider, server_pairs, nullptr) ==
                 GRPC_STATUS_OK);
