			   ...
			   grpc_channel_args * server_args)
{
  ...
  grpc_end2end_proxy *proxy =
    (grpc_end2end_proxy *) gpr_malloc (sizeof (*proxy));
  ...
  GPR_ASSERT (gpr_thd_new
	      (&proxy->thd, "grpc_end2end_proxy", thread_main, proxy, &opt));
  ...
  if (!pc->proxy->shutdown)
	...
	pc->p2s_initial_metadata.metadata;
      ...
      err = grpc_call_start_batch (pc->c2p, &op, 1,
				   new_closure (on_c2p_sent_initial_metadata,
						pc), nullptr);
...
on_new_call (void *arg, int success)
{
  ...
  if (success)
	...
	&pc->p2s_initial_metadata;
      ...
      err = grpc_call_start_batch (pc->p2s, &op, 1,
				   new_closure (on_p2s_recv_initial_metadata,
						pc), nullptr);
      GPR_ASSERT (err == GRPC_CALL_OK);
	...
	pc->c2p_initial_metadata.metadata;
      ...
      err = grpc_call_start_batch (pc->p2s, &op, 1,
				   new_closure (on_p2s_sent_initial_metadata,
						pc), nullptr);
      GPR_ASSERT (err == GRPC_CALL_OK);
	...
	&pc->p2s_trailing_metadata;
      ...
      err =
	grpc_call_start_batch (pc->p2s, &op, 1,
			       new_closure (on_p2s_status, pc), nullptr);
      GPR_ASSERT (err == GRPC_CALL_OK);
	...
	grpc_call_start_batch (pc->c2p, &op, 1,
			       new_closure (on_c2p_closed, pc), nullptr);
      GPR_ASSERT (err == GRPC_CALL_OK);
