...
grpc_end2end_proxy* grpc_end2end_proxy_create(const grpc_end2end_proxy_def* def,
  ...
  grpc_end2end_proxy* proxy =
      reinterpret_cast<grpc_end2end_proxy*>(gpr_malloc(sizeof(*proxy)));
...
static closure* new_closure(void (*func)(void* arg, int success), void* arg) {
  closure* cl = reinterpret_cast<closure*>(gpr_malloc(sizeof(*cl)));
  cl->func = func;
  cl->arg = arg;
  return cl;
}
...
static void shutdown_complete(void* arg, int success) {
  grpc_end2end_proxy* proxy = reinterpret_cast<grpc_end2end_proxy*>(arg);
  proxy->shutdown = 1;
...
static void on_c2p_sent_initial_metadata(void* arg, int success) {
  proxy_call* pc = reinterpret_cast<proxy_call*>(arg);
...
static void on_new_call(void* arg, int success) {
  ...
  if (success) {
    grpc_op op;
    memset(&op, 0, sizeof(op));
    proxy_call* pc = reinterpret_cast<proxy_call*>(gpr_malloc(sizeof(*pc)));
  ...
  for (;;) {
    ...
    switch (ev.type) {
      ...
      case GRPC_OP_COMPLETE:
        cl = reinterpret_cast<closure*>(ev.tag);
