...
grpc_end2end_proxy* grpc_end2end_proxy_create(
  ...
  gpr_log(GPR_DEBUG, "PROXY ADDR:%s BACKEND:%s",
          proxy->proxy_port.c_str(), proxy->server_port.c_str());

  proxy->cq = grpc_completion_queue_create_for_next(nullptr);
  proxy->server =
      def->create_server(proxy->proxy_port.c_str(), server_args);
  ...
  grpc_channel_args* proxy_client_args =
      grpc_channel_args_copy_and_add_and_remove(
          client_args, &arg_to_remove, 1, &arg_to_add, 1);
  proxy->client =
  ...
  proxy->thd =
      grpc_core::Thread("grpc_end2end_proxy", thread_main, proxy);
  proxy->thd.Start();
...
static closure* new_closure(void (*func)(void* arg, int success),
                            void* arg) {
...
void grpc_end2end_proxy_destroy(grpc_end2end_proxy* proxy) {
  grpc_server_shutdown_and_notify(
      proxy->server, proxy->cq, new_closure(shutdown_complete, proxy));
  proxy->thd.Join();
...
static void on_p2s_recv_initial_metadata(void* arg, int /*success*/) {
  ...
  if (!pc->proxy->shutdown && !grpc_call_is_trailers_only(pc->p2s)) {
    ...
    err = grpc_call_start_batch(
        pc->c2p, &op, 1, new_closure(on_c2p_sent_initial_metadata, pc),
        nullptr);
...
static void on_p2s_sent_message(void* arg, int success) {
  ...
  if (!pc->proxy->shutdown && success) {
    ...
    err = grpc_call_start_batch(
        pc->c2p, &op, 1, new_closure(on_c2p_recv_msg, pc), nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
  }
...
static void on_c2p_recv_msg(void* arg, int success) {
  ...
  if (!pc->proxy->shutdown && success) {
    if (pc->c2p_msg != nullptr) {
      ...
      err = grpc_call_start_batch(
          pc->p2s, &op, 1, new_closure(on_p2s_sent_close, pc), nullptr);
      GPR_ASSERT(err == GRPC_CALL_OK);
    }
  } else {
    if (pc->c2p_msg != nullptr) {
...
static void on_c2p_sent_message(void* arg, int success) {
  ...
  if (!pc->proxy->shutdown && success) {
    ...
    err = grpc_call_start_batch(
        pc->p2s, &op, 1, new_closure(on_p2s_recv_msg, pc), nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
  }
...
static void on_p2s_recv_msg(void* arg, int success) {
  ...
  if (!pc->proxy->shutdown && success && pc->p2s_msg) {
    ...
    err = grpc_call_start_batch(
        pc->c2p, &op, 1, new_closure(on_c2p_sent_message, pc), nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
  } else {
    grpc_byte_buffer_destroy(pc->p2s_msg);
  }
...
static void on_p2s_status(void* arg, int success) {
  proxy_call* pc = static_cast<proxy_call*>(arg);
  grpc_op op[2];  // Possibly send empty initial metadata also if
                  // trailers-only
  grpc_call_error err;
...
static void on_new_call(void* arg, int success) {
  ...
  if (success) {
    ...
    err = grpc_call_start_batch(
        pc->p2s, &op, 1, new_closure(on_p2s_recv_initial_metadata, pc),
        nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
    ...
    op.data.send_initial_metadata.count =
        pc->c2p_initial_metadata.count;
    op.data.send_initial_metadata.metadata =
        pc->c2p_initial_metadata.metadata;
    refpc(pc, "on_p2s_sent_initial_metadata");
    err = grpc_call_start_batch(
        pc->p2s, &op, 1, new_closure(on_p2s_sent_initial_metadata, pc),
        nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
    ...
    err = grpc_call_start_batch(
        pc->p2s, &op, 1, new_closure(on_p2s_status, pc), nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
    ...
    err = grpc_call_start_batch(
        pc->c2p, &op, 1, new_closure(on_c2p_closed, pc), nullptr);
    GPR_ASSERT(err == GRPC_CALL_OK);
