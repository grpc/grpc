...
static void
proxy_connection_ref (proxy_connection * conn, const char *reason)
{
...
static void
proxy_connection_unref (proxy_connection * conn, const char *reason)
{
  if (gpr_unref (&conn->refcount))
      ...
      if (conn->server_endpoint != nullptr)
	{
      ...
      grpc_slice_buffer_destroy_internal (&conn->
...
static void
proxy_connection_failed (proxy_connection * conn,
...
static void
on_client_write_done (void *arg, grpc_error * error)
{
...
static void
on_server_write_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
...
// the backend server.
static void
on_client_read_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
  if (error != GRPC_ERROR_NONE)
    {
...
static void
on_server_read_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
  if (error != GRPC_ERROR_NONE)
    {
...
static void
on_write_response_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
  conn->client_is_writing = false;
  if (error != GRPC_ERROR_NONE)
...
// CONNECT request.
static void
on_server_connect_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
  if (error != GRPC_ERROR_NONE)
    {
 ...
 */
static bool
proxy_auth_header_matches (char *proxy_auth_header_val, char *expected_cred)
{
...
static void
on_read_request_done (void *arg, grpc_error * error)
{
  proxy_connection *conn = (proxy_connection *) arg;
  ...
  for (size_t i = 0; i < conn->client_read_buffer.count; ++i)
	...
	{
	  error =
	    grpc_http_parser_parse (&conn->http_parser,
  ...
  const grpc_arg *proxy_auth_arg =
    grpc_channel_args_find (conn->proxy->channel_args,
  ...
  if (proxy_auth_arg != nullptr && proxy_auth_arg->type == GRPC_ARG_STRING)
	...
	{
	  if (strcmp (conn->http_request.hdrs[i].key, "Proxy-Authorization")
	      == 0)
	    {
	      client_authenticated =
		proxy_auth_header_matches (conn->http_request.hdrs[i].value,
...
static void
on_accept (void *arg, grpc_endpoint * endpoint,
	   ...
	   grpc_tcp_server_acceptor * acceptor)
{
  ...
  GRPC_CLOSURE_INIT (&conn->on_server_connect_done, on_server_connect_done,
		     conn, grpc_combiner_scheduler (conn->proxy->combiner));
...
grpc_end2end_http_proxy_create (grpc_channel_args * args)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_end2end_http_proxy *proxy =
  ...
  GPR_ASSERT (gpr_thd_new
	      (&proxy->thd, "grpc_http_proxy", thread_main, proxy, &opt));
  return proxy;
}

static void
destroy_pollset (void *arg, grpc_error * error)
{
...
void
grpc_end2end_http_proxy_destroy (grpc_end2end_http_proxy * proxy)
{
...
const char *
grpc_end2end_http_proxy_get_proxy_name (grpc_end2end_http_proxy * proxy)
{
