...
grpc_end2end_test_fixture grpc_end2end_local_chttp2_create_fixture_fullstack() {
  ...
  grpc_end2end_local_fullstack_fixture_data* ffd = new grpc_end2end_local_fullstack_fixture_data();
  memset(&f, 0, sizeof(f));
  f.fixture_data = ffd;
...
void grpc_end2end_local_chttp2_init_server_fullstack(grpc_end2end_test_fixture* f,
  ...
  GPR_ASSERT(grpc_server_add_secure_http2_port(f->server, ffd->localaddr.c_str(), creds));
