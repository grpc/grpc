...
/*
 * Check if server should fail auth check. If it is true, a different
 * metadata processor will be installed that always fails in processing
 * client's metadata.
 */
...
void grpc_end2end_local_chttp2_init_server_fullstack(
  ...
  if (fail_server_auth_check(server_args)) {
    grpc_auth_metadata_processor processor = {process_auth_failure,
                                              nullptr, nullptr};
    grpc_server_credentials_set_auth_metadata_processor(creds,
                                                        processor);
  }
  GPR_ASSERT(grpc_server_add_secure_http2_port(
      f->server, ffd->localaddr.c_str(), creds));
