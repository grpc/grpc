...
void chttp2_init_client_fullstack(grpc_end2end_test_fixture* f, grpc_channel_args* client_args) {
  ...
  f->client = grpc_insecure_channel_create(ffd->localaddr.c_str(), client_args, nullptr);
  GPR_ASSERT(f->client);
}
...
void chttp2_init_server_fullstack(grpc_end2end_test_fixture* f, grpc_channel_args* server_args) {
  ...
  GPR_ASSERT(grpc_server_add_insecure_http2_port(f->server, ffd->localaddr.c_str()));
