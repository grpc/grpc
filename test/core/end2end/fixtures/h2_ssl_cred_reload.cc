...
static grpc_ssl_certificate_config_reload_status ssl_server_certificate_config_callback(
  ...
  fullstack_secure_fixture_data* ffd = static_cast<fullstack_secure_fixture_data*>(user_data);
  if (!ffd->server_credential_reloaded) {
    ...
    *config = grpc_ssl_server_certificate_config_create(ca_cert, &pem_key_cert_pair, 1);
...
static grpc_end2end_test_fixture chttp2_create_fixture_secure_fullstack_tls1_2(
    grpc_channel_args* client_args, grpc_channel_args* server_args) {
  return chttp2_create_fixture_secure_fullstack(client_args, server_args, grpc_tls_version::TLS1_2);
}

static grpc_end2end_test_fixture chttp2_create_fixture_secure_fullstack_tls1_3(
    grpc_channel_args* client_args, grpc_channel_args* server_args) {
  return chttp2_create_fixture_secure_fullstack(client_args, server_args, grpc_tls_version::TLS1_3);
}
...
static void chttp2_init_client_secure_fullstack(grpc_end2end_test_fixture* f,
  ...
  fullstack_secure_fixture_data* ffd = static_cast<fullstack_secure_fixture_data*>(f->fixture_data);
  f->client = grpc_secure_channel_create(creds, ffd->localaddr.c_str(), client_args, nullptr);
...
static void chttp2_init_client_simple_ssl_secure_fullstack(grpc_end2end_test_fixture* f,
  ...
  if (f != nullptr && ssl_creds != nullptr) {
    ...
    grpc_ssl_credentials* creds = reinterpret_cast<grpc_ssl_credentials*>(ssl_creds);
...
static void chttp2_init_server_simple_ssl_secure_fullstack(grpc_end2end_test_fixture* f,
  ...
  grpc_ssl_server_credentials_options* options =
      grpc_ssl_server_credentials_create_options_using_config_fetcher(
          GRPC_SSL_DONT_REQUEST_CLIENT_CERTIFICATE, ssl_server_certificate_config_callback,
          f->fixture_data);
  grpc_server_credentials* ssl_creds = grpc_ssl_server_credentials_create_with_options(options);
  if (f != nullptr && ssl_creds != nullptr) {
    // Set the min and max TLS version.
    grpc_ssl_server_credentials* creds = reinterpret_cast<grpc_ssl_server_credentials*>(ssl_creds);
    fullstack_secure_fixture_data* ffd =
        static_cast<fullstack_secure_fixture_data*>(f->fixture_data);
    creds->set_min_tls_version(ffd->tls_version);
    creds->set_max_tls_version(ffd->tls_version);
...
static grpc_end2end_test_config configs[] = {
    {"chttp2/simple_ssl_fullstack_tls1_2",
     FEATURE_MASK_SUPPORTS_DELAYED_CONNECTION | FEATURE_MASK_SUPPORTS_PER_CALL_CREDENTIALS |
         FEATURE_MASK_SUPPORTS_CLIENT_CHANNEL | FEATURE_MASK_SUPPORTS_AUTHORITY_HEADER,
     "foo.test.google.fr", chttp2_create_fixture_secure_fullstack_tls1_2,
     chttp2_init_client_simple_ssl_secure_fullstack, chttp2_init_server_simple_ssl_secure_fullstack,
