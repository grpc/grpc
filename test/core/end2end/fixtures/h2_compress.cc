...
static grpc_end2end_test_fixture chttp2_create_fixture_fullstack_compression(
  ...
  fullstack_compression_fixture_data* ffd = new fullstack_compression_fixture_data();
...
void chttp2_init_client_fullstack_compression(grpc_end2end_test_fixture* f,
  ...
  ffd->client_args_compression =
      grpc_channel_args_set_channel_default_compression_algorithm(client_args, GRPC_COMPRESS_GZIP);
  f->client =
      grpc_insecure_channel_create(ffd->localaddr.c_str(), ffd->client_args_compression, nullptr);
}
...
void chttp2_init_server_fullstack_compression(grpc_end2end_test_fixture* f,
  ...
  ffd->server_args_compression =
      grpc_channel_args_set_channel_default_compression_algorithm(server_args, GRPC_COMPRESS_GZIP);
  if (f->server) {
...
static grpc_end2end_test_config configs[] = {
    {"chttp2/fullstack_compression",
     ...
     nullptr, chttp2_create_fixture_fullstack_compression, chttp2_init_client_fullstack_compression,
     chttp2_init_server_fullstack_compression, chttp2_tear_down_fullstack_compression},
};
