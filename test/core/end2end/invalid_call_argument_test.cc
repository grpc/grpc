...
static void prepare_test(int is_client) {
  ...
  if (is_client) {
    ...
    g_state.chan = grpc_insecure_channel_create("nonexistant:54321", nullptr, nullptr);
    grpc_slice host = grpc_slice_from_static_string("nonexistant");
    g_state.call = grpc_channel_create_call(g_state.chan, nullptr, GRPC_PROPAGATE_DEFAULTS,
                                            g_state.cq, grpc_slice_from_static_string("/Foo"),
                                            &host, g_state.deadline, nullptr);
  } else {
    ...
    grpc_server_add_insecure_http2_port(g_state.server, server_hostport.c_str());
    grpc_server_start(g_state.server);
    server_hostport = grpc_core::JoinHostPort("localhost", port);
    g_state.chan = grpc_insecure_channel_create(server_hostport.c_str(), nullptr, nullptr);
    ...
    GPR_ASSERT(GRPC_CALL_OK == grpc_server_request_call(g_state.server, &g_state.server_call,
...
static void cleanup_test() {
  ...
  while (grpc_completion_queue_next(g_state.cq, gpr_inf_future(GPR_CLOCK_REALTIME), nullptr).type !=
         GRPC_QUEUE_SHUTDOWN) {
  }
...
static void test_non_null_reserved_on_start_batch() {
  ...
  prepare_test(1);
  GPR_ASSERT(GRPC_CALL_ERROR == grpc_call_start_batch(g_state.call, nullptr, 0, nullptr, tag(1)));
  cleanup_test();
}
...
static void test_non_null_reserved_on_op() {
  ...
  op->reserved = tag(2);
  op++;
  GPR_ASSERT(GRPC_CALL_ERROR == grpc_call_start_batch(g_state.call, g_state.ops,
                                                      (size_t)(op - g_state.ops), tag(1), nullptr));
  cleanup_test();
}
...
static void test_send_initial_metadata_more_than_once() {
  ...
  GPR_ASSERT(GRPC_CALL_ERROR_TOO_MANY_OPERATIONS ==
             grpc_call_start_batch(g_state.call, g_state.ops, (size_t)(op - g_state.ops), tag(1),
                                   nullptr));
  cleanup_test();
}
...
static void test_too_many_metadata() {
  ...
  op->reserved = nullptr;
  op++;
  GPR_ASSERT(GRPC_CALL_ERROR_INVALID_METADATA == grpc_call_start_batch(g_state.call, g_state.ops,
...
static void test_send_null_message() {
  ...
  op->reserved = nullptr;
  op++;
  GPR_ASSERT(GRPC_CALL_ERROR_INVALID_MESSAGE == grpc_call_start_batch(g_state.call, g_state.ops,
                                                                      (size_t)(op - g_state.ops),
                                                                      tag(1), nullptr));
  cleanup_test();
}
...
static void test_send_server_status_from_client() {
  ...
  op->reserved = nullptr;
  op++;
  GPR_ASSERT(GRPC_CALL_ERROR_NOT_ON_CLIENT == grpc_call_start_batch(g_state.call, g_state.ops,
                                                                    (size_t)(op - g_state.ops),
                                                                    tag(1), nullptr));
  cleanup_test();
}
...
static void test_receive_initial_metadata_twice_at_client() {
  ...
  op->data.recv_initial_metadata.recv_initial_metadata = &g_state.initial_metadata_recv;
  op->flags = 0;
  op->reserved = nullptr;
  op++;
  GPR_ASSERT(GRPC_CALL_OK == grpc_call_start_batch(g_state.call, g_state.ops,
...
static void test_recv_status_on_client_twice() {
  ...
  op->data.recv_status_on_client.trailing_metadata = &g_state.trailing_metadata_recv;
...
static void test_send_close_from_client_on_server() {
  ...
  GPR_ASSERT(GRPC_CALL_ERROR_NOT_ON_SERVER ==
             grpc_call_start_batch(g_state.server_call, g_state.ops, (size_t)(op - g_state.ops),
                                   tag(2), nullptr));
  cleanup_test();
}
...
static void test_too_many_trailing_metadata() {
  ...
  op = g_state.ops;
  op->op = GRPC_OP_SEND_STATUS_FROM_SERVER;
  op->data.send_status_from_server.trailing_metadata_count = static_cast<size_t>(INT_MAX) + 1;
  op->data.send_status_from_server.status = GRPC_STATUS_UNIMPLEMENTED;
  grpc_slice status_details = grpc_slice_from_static_string("xyz");
  op->data.send_status_from_server.status_details = &status_details;
  op->flags = 0;
