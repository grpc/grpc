...
TEST(ConnectivityStateName, Basic) {
  ...
  EXPECT_STREQ("TRANSIENT_FAILURE", ConnectivityStateName(GRPC_CHANNEL_TRANSIENT_FAILURE));
...
class Watcher : public ConnectivityStateWatcherInterface {
 public:
  Watcher(int* count, grpc_connectivity_state* output, absl::Status* status,
      ...
      : count_(count), output_(output), status_(status), destroyed_(destroyed) {}
...
TEST(StateTracker, SetAndGetState) {
  ConnectivityStateTracker tracker("xxx", GRPC_CHANNEL_CONNECTING, absl::Status());
  ...
  EXPECT_TRUE(tracker.status().ok());
  absl::Status transient_failure_status(absl::StatusCode::kUnavailable, "status for testing");
  tracker.SetState(GRPC_CHANNEL_TRANSIENT_FAILURE, transient_failure_status, "reason");
...
TEST(StateTracker, NotificationUponAddingWatcher) {
  ...
  tracker.AddWatcher(GRPC_CHANNEL_IDLE, MakeOrphanable<Watcher>(&count, &state, &status));
  EXPECT_EQ(count, 1);
...
TEST(StateTracker, NotificationUponAddingWatcherWithTransientFailure) {
  ...
  absl::Status status;
  absl::Status transient_failure_status(absl::StatusCode::kUnavailable, "status for testing");
  ConnectivityStateTracker tracker("xxx", GRPC_CHANNEL_TRANSIENT_FAILURE, transient_failure_status);
  tracker.AddWatcher(GRPC_CHANNEL_IDLE, MakeOrphanable<Watcher>(&count, &state, &status));
  EXPECT_EQ(count, 1);
...
TEST(StateTracker, NotificationUponStateChange) {
  ...
  EXPECT_TRUE(status.ok());
  absl::Status transient_failure_status(absl::StatusCode::kUnavailable, "status for testing");
  tracker.SetState(GRPC_CHANNEL_TRANSIENT_FAILURE, transient_failure_status, "whee");
  EXPECT_EQ(count, 1);
  EXPECT_EQ(state, GRPC_CHANNEL_TRANSIENT_FAILURE);
  EXPECT_EQ(status, transient_failure_status);
}
...
TEST(StateTracker, SubscribeThenUnsubscribe) {
  ...
  ConnectivityStateTracker tracker("xxx", GRPC_CHANNEL_IDLE);
  ConnectivityStateWatcherInterface* watcher = new Watcher(&count, &state, &status, &destroyed);
  tracker.AddWatcher(GRPC_CHANNEL_IDLE, OrphanablePtr<ConnectivityStateWatcherInterface>(watcher));
...
TEST(StateTracker, AddWhenAlreadyShutdown) {
  ...
  absl::Status status;
  bool destroyed = false;
  ConnectivityStateTracker tracker("xxx", GRPC_CHANNEL_SHUTDOWN, absl::Status());
  ConnectivityStateWatcherInterface* watcher = new Watcher(&count, &state, &status, &destroyed);
  tracker.AddWatcher(GRPC_CHANNEL_IDLE, OrphanablePtr<ConnectivityStateWatcherInterface>(watcher));
  EXPECT_TRUE(destroyed);
  EXPECT_EQ(count, 1);
...
TEST(StateTracker, DoNotNotifyShutdownAtDestructionIfAlreadyInShutdown) {
  ...
  absl::Status status;
  {
    ...
    tracker.AddWatcher(GRPC_CHANNEL_SHUTDOWN, MakeOrphanable<Watcher>(&count, &state, &status));
    // No initial notification, since we started the watch from the
    // current state.
    EXPECT_EQ(count, 0);
    EXPECT_EQ(state, GRPC_CHANNEL_SHUTDOWN);
...
int main(int argc, char** argv) {
  ...
  grpc_init();
  grpc_core::testing::grpc_tracer_enable_flag(&grpc_core::grpc_connectivity_state_trace);
