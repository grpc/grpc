...
static void
must_succeed (void *arg, grpc_error * error)
{
...
static void
must_fail (void *arg, grpc_error * error)
{
  GPR_ASSERT (error != GRPC_ERROR_NONE);
  GPR_ASSERT (arg == THE_ARG);
  g_counter++;
...
static void
test_connectivity_state_name (void)
{
  gpr_log (GPR_DEBUG, "test_connectivity_state_name");
  GPR_ASSERT (0 ==
	      strcmp (grpc_connectivity_state_name (GRPC_CHANNEL_IDLE),
		      "IDLE"));
  GPR_ASSERT (0 ==
	      strcmp (grpc_connectivity_state_name (GRPC_CHANNEL_CONNECTING),
		      "CONNECTING"));
  GPR_ASSERT (0 ==
	      strcmp (grpc_connectivity_state_name (GRPC_CHANNEL_READY),
		      "READY"));
  GPR_ASSERT (0 ==
	      strcmp (grpc_connectivity_state_name
		      (GRPC_CHANNEL_TRANSIENT_FAILURE), "TRANSIENT_FAILURE"));
  GPR_ASSERT (0 ==
	      strcmp (grpc_connectivity_state_name (GRPC_CHANNEL_SHUTDOWN),
		      "SHUTDOWN"));
}
...
static void
test_subscribe_then_unsubscribe (void)
{
  grpc_connectivity_state_tracker tracker;
  ...
  g_counter = 0;
  grpc_connectivity_state_init (&tracker, GRPC_CHANNEL_IDLE, "xxx");
  GPR_ASSERT (grpc_connectivity_state_notify_on_state_change
...
static void
test_subscribe_then_destroy (void)
{
  grpc_connectivity_state_tracker tracker;
  grpc_closure *closure =
...
static void
test_subscribe_with_failure_then_destroy (void)
{
  grpc_connectivity_state_tracker tracker;
  grpc_closure *closure =
    GRPC_CLOSURE_CREATE (must_fail, THE_ARG, grpc_schedule_on_exec_ctx);
  ...
  GPR_ASSERT (0 ==
	      grpc_connectivity_state_notify_on_state_change (&tracker,
							      &state,
							      closure));
  grpc_core::ExecCtx::Get ()->Flush ();
  GPR_ASSERT (state == GRPC_CHANNEL_SHUTDOWN);
...
main (int argc, char **argv)
{
  ...
  grpc_init ();
  grpc_core::testing::
    grpc_tracer_enable_flag (&grpc_connectivity_state_trace);
