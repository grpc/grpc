...
// A gRPC server, running in its own thread.
	...
	void Start ()
	{
	  ...
	  grpc_arg arg;
	    ...
	    arg.key =
	    const_cast < char *>(GRPC_ARG_SERVER_HANDSHAKE_TIMEOUT_MS);
	    arg.value.integer = 1000;
	  grpc_channel_args args = { 1, &arg };
	    server_ = grpc_server_create (&args, nullptr);
	    ASSERT_TRUE (grpc_server_add_insecure_http2_port
			 (server_, address_));
	    cq_ = grpc_completion_queue_create_for_next (nullptr);
	    grpc_server_register_completion_queue (server_, cq_, nullptr);
	    grpc_server_start (server_);
	    thread_.
	    reset (new std::thread (std::bind (&ServerThread::Serve, this)));
	}
	...
	void Serve ()
	{
	  ...
	  grpc_event ev =
	    grpc_completion_queue_next (cq_,
...
// closes, and then terminates.
	...
	explicit Client (const char
			 *server_address):server_address_ (server_address)
	{
	}
	...
	void Connect ()
	{
	  ...
	  grpc_error *error =
	    grpc_blocking_resolve_address (server_address_, "80",
					   &server_addresses);
	  ...
	  grpc_tcp_client_connect (state.closure (), &endpoint_, pollset_set,
				   ...
				   1000);
	  ASSERT_TRUE (PollUntilDone (&state,
	...
	void Shutdown ()
	{
	  ...
	  grpc_pollset_shutdown (pollset_,
				 GRPC_CLOSURE_CREATE (&Client::PollsetDestroy,
						      pollset_,
	...
	bool PollUntilDone (EventState * state, grpc_millis deadline)
	{
	  while (true)
		...
		return true;
	      if (grpc_core::ExecCtx::Get ()->Now () >= deadline)
		return false;
	    }
	}
