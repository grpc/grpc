...
/* test add & lookup */
static void
test_basic_add_find (uint32_t n)
{
  grpc_chttp2_stream_map map;
  uint32_t i;
  size_t got;
...
static void
verify_for_each (void *user_data, uint32_t stream_id, void *ptr)
{
...
static void
check_delete_evens (grpc_chttp2_stream_map * map, uint32_t n)
{
...
/* add a bunch of keys, delete the even ones, and make sure the map is
   consistent */
static void
test_delete_evens_sweep (uint32_t n)
{
  grpc_chttp2_stream_map map;
  uint32_t i;
...
/* add a bunch of keys, delete the even ones immediately, and make sure the map
   is consistent */
static void
test_delete_evens_incremental (uint32_t n)
{
  grpc_chttp2_stream_map map;
  uint32_t i;
...
static void
test_periodic_compaction (uint32_t n)
{
  grpc_chttp2_stream_map map;
  uint32_t i;
  uint32_t del;
