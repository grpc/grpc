...
TEST_F(Http2ServerTransportTest, TestServerStreamFlowControlWindowUpdate) {
  // Verifies that the server autonomously sends stream flow control tokens for
  // a stream when receiving DATA frames, even when keepalives, pings, and
  // server application writes are disabled or absent. With initial window size
  // configured to 8192, sending 8192 bytes fully depletes the stream flow
  // control window to 0 tokens while keeping the transport flow control window
  // above 50%, isolating the stream window update mechanism.
  ExecCtx ctx;
  ...
  // Step 2: Exchange settings with the server, expecting the server to
  // advertise INITIAL_WINDOW_SIZE = 8192 in its SETTINGS frame.
  ...
  // Step 5: Expect the server to autonomously send a WINDOW_UPDATE frame
  // replenishing the stream flow control window.
  step->ThenExpectWrite({
