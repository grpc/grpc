...
TEST_F(Http2ServerTransportTest, TestBdpProbeEnabledWithoutAck) {
  ExecCtx ctx;
  InitTransport(GetChannelArgs()
  ...
  step2->ThenExpectWrite([&, step2](SliceBuffer& buffer) {
    uint64_t opaque_id =
        VerifyPingFrameAndReturnOpaqueId(buffer, /*is_ack=*/false);
    // We dont not read a PING ACK, because that will trigger CompleteBdpPing
    // which is a very time sensitive API and not suitable this test suite.
