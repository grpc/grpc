...
static void verify_frames(grpc_slice_buffer& output, bool header_is_eof) {
  ...
  for (i = 0; i < output.count;) {
    ...
    if (first_frame && type != GRPC_CHTTP2_FRAME_HEADER) {
      ...
      g_failure = 1;
    } else if (first_frame && header_is_eof && !(flags & GRPC_CHTTP2_DATA_FLAG_END_STREAM)) {
      gpr_log(GPR_ERROR, "missing END_STREAM flag in HEADER frame");
      g_failure = 1;
    }
    if (is_closed && (type == GRPC_CHTTP2_FRAME_DATA || type == GRPC_CHTTP2_FRAME_HEADER)) {
      gpr_log(GPR_ERROR, "stream is closed; new frame headers and data are not allowed");
      g_failure = 1;
    }
    if (end_header &&
        (type == GRPC_CHTTP2_FRAME_HEADER || type == GRPC_CHTTP2_FRAME_CONTINUATION)) {
      gpr_log(GPR_ERROR,
              "frame header is ended; new headers and continuations are not "
              "allowed");
      g_failure = 1;
    }
    if (in_header && (type == GRPC_CHTTP2_FRAME_DATA || type == GRPC_CHTTP2_FRAME_HEADER)) {
      gpr_log(GPR_ERROR, "parsing frame header; new headers and data are not allowed");
      g_failure = 1;
    }
    if (flags & ~(GRPC_CHTTP2_DATA_FLAG_END_STREAM | GRPC_CHTTP2_DATA_FLAG_END_HEADERS)) {
...
static void verify(const verify_params params, const char* expected, size_t nheaders, ...) {
  ...
  size_t i;
  va_list l;
  grpc_linked_mdelem* e = static_cast<grpc_linked_mdelem*>(gpr_malloc(sizeof(*e) * nheaders));
  ...
  for (i = 0; i < nheaders; i++) {
    ...
    e[i].md =
        grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string(key)), value_slice);
  }
  e[0].prev = nullptr;
  ...
  if (cap_to_delete == num_to_delete) {
    ...
    to_delete = static_cast<void**>(gpr_realloc(to_delete, sizeof(*to_delete) * cap_to_delete));
  }
...
static void test_basic_headers() {
  ...
  for (i = 0; i < 350; i++) {
    verify(params, "000003 0104 deadbeef c0 bf be", 3, "a", "a", "b", "c", "a", "d");
  }
...
static void verify_continuation_headers(const char* key, const char* value, bool is_eof) {
  ...
  grpc_linked_mdelem* e = static_cast<grpc_linked_mdelem*>(gpr_malloc(sizeof(*e)));
  grpc_metadata_batch b;
  grpc_metadata_batch_init(&b);
  e[0].md = elem;
  e[0].prev = nullptr;
...
static void test_decode_table_overflow() {
  ...
  for (i = 0; i < 29; i++) {
    ...
    if (i == 0) {
      ...
      std::string expect = absl::StrFormat("00000a 0104 deadbeef 3fe107 40 02%02x%02x 02%02x%02x",
                                           key[0], key[1], value[0], value[1]);
      verify(params, expect.c_str(), 1, key, value);
    } else {
      std::string expect = absl::StrFormat("000008 0104 deadbeef %02x 40 02%02x%02x 02%02x%02x",
...
static void verify_table_size_change_match_elem_size(const char* key, const char* value,
...
static void test_encode_header_size() {
  ...
  verify_table_size_change_match_elem_size("true-binary-bin", "I_am_true_binary_value", true);
}

static void test_interned_key_indexed() {
  int i;
  verify_params params = {false, false, true};
  verify(params, "000009 0104 deadbeef 40 0161 0162 0f2f 0163", 2, "a", "b", "a", "c");
  for (i = 0; i < 10; i++) {
    verify(params, "000008 0104 deadbeef 0f2f 0162 0f2f 0163", 2, "a", "b", "a", "c");
  }
}
