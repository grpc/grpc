...
/* verify that the output frames that are generated by encoding the
   stream have sensible type and flags values */
static void verify_frames(grpc_slice_buffer& output,
  ...
  for (i = 0; i < output.count;) {
    ...
    while (merged_length <
           frame_size + 9) {  // including 9 byte frame header
      grpc_slice* slice = &output.slices[i++];
      merged_length += GRPC_SLICE_LENGTH(*slice);
    }
    ...
    if (is_closed && (type == GRPC_CHTTP2_FRAME_DATA ||
      ...
      gpr_log(GPR_ERROR,
              "stream is closed; new frame headers and data are not "
              "allowed");
      g_failure = 1;
    }
    if (end_header && (type == GRPC_CHTTP2_FRAME_HEADER ||
      ...
      gpr_log(GPR_ERROR,
              "frame header is ended; new headers and continuations "
              "are not "
              "allowed");
      g_failure = 1;
    }
    ...
    if (flags & GRPC_CHTTP2_DATA_FLAG_END_STREAM) {
      ...
      if (type == GRPC_CHTTP2_FRAME_CONTINUATION) {
        gpr_log(GPR_ERROR,
                "unexpected END_STREAM flag in CONTINUATION frame");
        g_failure = 1;
      }
    }
  }
...
static void verify(const verify_params params, const char* expected,
  ...
  size_t i;
  va_list l;
  grpc_linked_mdelem* e = static_cast<grpc_linked_mdelem*>(
      gpr_malloc(sizeof(*e) * nheaders));
  ...
  grpc_chttp2_encode_header(&g_compressor, nullptr, 0, &b, &hopt,
                            &output);
  ...
  if (!grpc_slice_eq(merged, expect)) {
    char* expect_str =
        grpc_dump_slice(expect, GPR_DUMP_HEX | GPR_DUMP_ASCII);
    char* got_str =
        grpc_dump_slice(merged, GPR_DUMP_HEX | GPR_DUMP_ASCII);
...
static void test_basic_headers() {
  ...
  verify(params, "000001 0104 deadbeef be", 1, "a", "a");
  verify(params, "000006 0104 deadbeef be 40 0162 0163", 2, "a", "a",
         "b", "c");
  ...
  for (i = 0; i < 350; i++) {
    verify(params, "000003 0104 deadbeef c0 bf be", 3, "a", "a", "b",
           "c", "a", "d");
  }

  verify(params, "000006 0104 deadbeef c0 00 016b 0176", 2, "a", "a",
         "k", "v");
...
static void test_decode_table_overflow() {
  // Decrease the default table size to make decode table overflow
  // easier.
  ...
  /* if the above passes, then we must have just knocked this pair out
     of the decoder stack, and so we'll be forced to re-encode it */
  verify(params, "000007 0104 deadbeef 40 026161 026261", 1, "aa",
         "ba");
}

static void verify_table_size_change_match_elem_size(
    const char* key, const char* value, bool use_true_binary) {
  grpc_slice_buffer output;
  grpc_mdelem elem = grpc_mdelem_from_slices(
      grpc_slice_intern(grpc_slice_from_static_string(key)),
      grpc_slice_intern(grpc_slice_from_static_string(value)));
  size_t elem_size =
      grpc_chttp2_get_size_in_hpack_table(elem, use_true_binary);
...
static void test_encode_header_size() {
  ...
  verify_table_size_change_match_elem_size(
      "true-binary-bin", "I_am_true_binary_value", true);
}

static void test_interned_key_indexed() {
  int i;
  verify_params params = {false, false, true};
  verify(params, "000009 0104 deadbeef 40 0161 0162 0f2f 0163", 2, "a",
         "b", "a", "c");
  for (i = 0; i < 10; i++) {
    verify(params, "000008 0104 deadbeef 0f2f 0162 0f2f 0163", 2, "a",
           "b", "a", "c");
  }
}
