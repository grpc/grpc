...
grpc_status_code PerformCall(grpc_channel* channel, grpc_server* server,
  ...
  // Start a call
  c = grpc_channel_create_call(channel, nullptr,
  ...
  cq_verify(cqv);
  grpc_call_cancel_with_status(s, GRPC_STATUS_PERMISSION_DENIED,
                               "test status", nullptr);
...
// Test that sending a lot of RPCs that are cancelled by the server
// doesn't result in too many pings due to the pings sent by BDP.
TEST(TooManyPings,
     TestLotsOfServerCancelledRpcsDoesntGiveTooManyPings) {
  grpc_completion_queue* cq =
  ...
  // perform an RPC
  gpr_log(GPR_INFO,
          "Performing %d total RPCs and expecting them all to receive "
          "status "
          "PERMISSION_DENIED (%d)",
  ...
  for (auto itr = statuses_and_counts.begin();
       itr != statuses_and_counts.end(); itr++) {
    if (itr->first != GRPC_STATUS_PERMISSION_DENIED) {
      num_not_cancelled += itr->second;
    }
    gpr_log(GPR_INFO, "%d / %d RPCs received status code: %d",
            itr->second, kNumTotalRpcs, itr->first);
  }
  if (num_not_cancelled > 0) {
    gpr_log(GPR_ERROR,
            "Expected all RPCs to receive status PERMISSION_DENIED "
            "(%d) but %d "
...
// Perform a simple RPC where the client makes a request, and both the
// client and server continue reading so that gRPC can send and receive
// keepalive pings.
grpc_status_code PerformWaitingCall(grpc_channel* channel,
                                    grpc_server* server,
                                    grpc_completion_queue* cq) {
  grpc_call* c;
  grpc_call* s;
  ...
  CQ_EXPECT_COMPLETION(cqv, tag(101), 1);
  cq_verify(cqv);
  // Since the server is configured to allow only a single ping strike,
  // it would take 3 pings to trigger the GOAWAY frame with
  // "too_many_pings" from the server. (The second ping from the client
  // would be the first bad ping sent too quickly leading to a ping
  // strike and the third ping would lead to the GOAWAY.) If the client
  // settings match with the server's settings, there won't be a bad
  // ping, and the call will end due to the deadline expiring instead.
  CQ_EXPECT_COMPLETION(cqv, tag(1), 1);
...
void VerifyChannelReady(grpc_channel* channel,
                        grpc_completion_queue* cq) {
  grpc_connectivity_state state = grpc_channel_check_connectivity_state(
      channel, 1 /* try_to_connect */);
  while (state != GRPC_CHANNEL_READY) {
    grpc_channel_watch_connectivity_state(
        channel, state, grpc_timeout_seconds_to_deadline(5), cq,
        nullptr);
...
void VerifyChannelDisconnected(grpc_channel* channel,
                               grpc_completion_queue* cq) {
  // Verify channel gets disconnected. Use a ping to make sure that
  // clients tries sending/receiving bytes if the channel is connected.
  grpc_channel_ping(channel, cq, reinterpret_cast<void*>(2000),
                    nullptr);
...
class KeepaliveThrottlingTest : public ::testing::Test {
 protected:
  // Starts the server and makes sure that the channel is able to get
  // connected.
  grpc_server* ServerStart(const char* addr,
                           grpc_completion_queue* cq) {
    // Set up server channel args to expect pings at an interval of 5
    // seconds and use a single ping strike
    ...
    grpc_channel_args server_channel_args = {
        GPR_ARRAY_SIZE(server_args), server_args};
    // Create server
    grpc_server* server =
...
TEST_F(KeepaliveThrottlingTest, KeepaliveThrottlingMultipleChannels) {
  ...
  // We need 3 GOAWAY frames to throttle the keepalive time from 1
  // second to 8 seconds (> 5sec).
  for (int i = 0; i < 3; i++) {
  ...
  gpr_log(GPR_INFO,
          "Client keepalive time %d should now be in sync with the "
          "server settings",
          expected_keepalive_time_sec);
  EXPECT_EQ(PerformWaitingCall(channel, server, cq),
  ...
  gpr_log(GPR_INFO,
          "Now testing second channel sharing the same subchannel");
...
// Tests that when new subchannels are created due to a change in
// resolved addresses, the new subchannels use the updated keepalive
// time.
TEST_F(KeepaliveThrottlingTest, NewSubchannelsUseUpdatedKeepaliveTime) {
  ...
  // create a single channel with multiple subchannels with a keepalive
  // ping interval of 1 second. To get finer control on subchannel
  // connection times, we are using pick_first instead of round_robin
  // and using the fake resolver response generator to switch between
  // the two.
  ...
  grpc_channel_args client_channel_args = {GPR_ARRAY_SIZE(client_args),
                                           client_args};
  grpc_channel* channel = grpc_insecure_channel_create(
      "fake:///", &client_channel_args, nullptr);
  // For a single subchannel 3 GOAWAYs would be sufficient to increase
  // the keepalive time from 1 second to beyond 5 seconds. Even though
  // we are alternating between two subchannels, 3 GOAWAYs should still
  // be enough since the channel should start all new transports with
  // the new keepalive value (even those from a different subchannel).
  int expected_keepalive_time_sec = 1;
  for (int i = 0; i < 3; i++) {
    gpr_log(GPR_INFO, "Expected keepalive time : %d",
            expected_keepalive_time_sec);
    ...
    // ExecCtx::Flush() might not be enough to make sure that the
    // resolver result has been propagated, so sleep for a bit.
    grpc_core::ExecCtx::Get()->Flush();
    gpr_sleep_until(grpc_timeout_seconds_to_deadline(1));
    EXPECT_EQ(
        PerformWaitingCall(channel, i % 2 == 0 ? server1 : server2, cq),
        GRPC_STATUS_UNAVAILABLE);
    expected_keepalive_time_sec *= 2;
  }
  gpr_log(GPR_INFO,
...
// Tests that when a channel has multiple subchannels and receives a
// GOAWAY with "too_many_pings" on one of them, all subchannels start
// any new transports with an updated keepalive time.
TEST_F(KeepaliveThrottlingTest,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    grpc_server* server = ServerStart(
        i % 2 == 0 ? server_address1.c_str() : server_address2.c_str(),
        cq);
...
// Perform a simple RPC where the client makes a request expecting a
// response with payload.
void PerformCallWithResponsePayload(grpc_channel* channel,
                                    grpc_server* server,
                                    grpc_completion_queue* cq) {
  grpc_slice response_payload_slice =
      grpc_slice_from_static_string("hello");
...
TEST(TooManyPings, BdpPingNotSentWithoutReceiveSideActivity) {
  ...
  grpc_channel* channel = grpc_insecure_channel_create(
      server_address.c_str(), &client_channel_args, nullptr);
  VerifyChannelReady(channel, cq);
  EXPECT_EQ(TransportCounter::count(),
            2 /* one each for server and client */);
  cq_verifier* cqv = cq_verifier_create(cq);
  // Channel should be able to send two pings without disconnect if
  // there was no BDP sent.
  ...
  // Send two more pings to verify. The second ping should cause a
  // disconnect. If BDP was not sent, the second ping would not cause a
  // disconnect.
