...
class TestServer {
 public:
  explicit TestServer(grpc_completion_queue* cq,
    ...
    ABSL_CHECK(grpc_server_add_http2_port(server_, address_.c_str(), server_creds));
  ...
  ~TestServer() {
    ...
    ABSL_CHECK(event.success);
    ABSL_CHECK(event.tag == this);
...
class TransportCounter {
 public:
  void InitCallback() {
    ...
    ++num_created_;
    ++num_live_;
    ABSL_LOG(INFO) << "TransportCounter num_created_=" << num_created_
...
void EnsureConnectionsArentLeaked(grpc_completion_queue* cq) {
  ABSL_LOG(INFO) << "The channel has been destroyed, wait for it to shut down and "
               "close...";
  ...
  if (g_transport_counter->num_created() < 2) {
    ABSL_LOG(ERROR) << "g_transport_counter->num_created() == "
  ...
  for (;;) {
    ...
    if (gpr_time_cmp(gpr_now(GPR_CLOCK_MONOTONIC), overall_deadline) > 0) {
      ABSL_LOG(INFO) << "g_transport_counter->num_live() never returned 0. "
                   "It's likely this test has triggered a connection leak.";
      ABSL_CHECK(0);
    }
    ABSL_LOG(INFO) << "g_transport_counter->num_live() returned " << live_transports
