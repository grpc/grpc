...
//
// Parses `buffer` into the sequence of HTTP/2 frames that it holds.
//
// `buffer` is consumed. This is intended for use inside a ThenExpectWrite or
// ThenPerformRead callback, where the framework discards the buffer once the
// callback returns.
//
// On a malformed or truncated buffer this reports a test failure and returns
// the frames parsed so far, so that EXPECT_THAT still produces a useful diff.
inline std::vector<Http2Frame> ParseFrames(SliceBuffer& buffer) {
  std::vector<Http2Frame> frames;
  while (buffer.Length() > 0) {
    if (buffer.Length() < kFrameHeaderSize) {
      ADD_FAILURE() << "Truncated HTTP/2 frame header. Expected at least "
                    << static_cast<int>(kFrameHeaderSize) << " bytes, found "
                    << buffer.Length();
      break;
    }
    uint8_t header_bytes[kFrameHeaderSize];
    buffer.MoveFirstNBytesIntoBuffer(kFrameHeaderSize, header_bytes);
    const Http2FrameHeader header = Http2FrameHeader::Parse(header_bytes);
    if (buffer.Length() < header.length) {
      ADD_FAILURE() << "Truncated HTTP/2 frame payload for "
                    << header.ToString() << ". Found only " << buffer.Length()
                    << " bytes";
      break;
    }
    SliceBuffer payload;
    buffer.MoveFirstNBytesIntoSliceBuffer(header.length, payload);
    http2::ValueOrHttp2Status<Http2Frame> frame =
        ParseFramePayload(header, std::move(payload));
    if (!frame.IsOk()) {
      ADD_FAILURE() << "Failed to parse " << header.ToString() << " : "
                    << frame.DebugString();
      break;
    }
...
// Matchers over the Http2Frame variant.
//
// Only add a matcher for a frame type that has a field a test cannot
// reasonably predict. WINDOW_UPDATE has one (the increment is a flow control
// accounting artifact) and PING has one (the opaque id is generated by the
// transport). Every other frame type is fully determined by the test, so
// assert on it with the frame's own operator==, for example
// ::testing::VariantWith<Http2RstStreamFrame>(expected_frame).

inline ::testing::Matcher<const Http2Frame&> IsWindowUpdateFrame(
    ::testing::Matcher<uint32_t> stream_id,
    ::testing::Matcher<uint32_t> increment = ::testing::_) {
  return ::testing::VariantWith<Http2WindowUpdateFrame>(::testing::AllOf(
      ::testing::Field("stream_id", &Http2WindowUpdateFrame::stream_id,
                       stream_id),
      ::testing::Field("increment", &Http2WindowUpdateFrame::increment,
                       increment)));
}

inline ::testing::Matcher<const Http2Frame&> IsPingFrame(
    ::testing::Matcher<bool> ack,
    ::testing::Matcher<uint64_t> opaque = ::testing::_) {
  return ::testing::VariantWith<Http2PingFrame>(::testing::AllOf(
