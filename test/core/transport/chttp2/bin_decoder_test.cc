...
static grpc_slice base64_decode_with_length(const char* s,
  ...
  grpc_slice ss = grpc_slice_from_copied_string(s);
  grpc_slice out =
      grpc_chttp2_base64_decode_with_length(ss, output_length);
  grpc_slice_unref_internal(ss);
  return out;
}
...
int main(int argc, char** argv) {
  ...
  grpc_init();
  {
    ...
    /* ENCODE_AND_DECODE tests grpc_chttp2_base64_decode_with_length(),
       which takes encoded base64 strings without pad chars, but output
       length is required. */
    ...
    EXPECT_SLICE_EQ("\xc0\xc1\xc2\xc3\xc4\xc5",
                    base64_decode("wMHCw8TF"));
