...
absl::Status ValidateFrame(I... i) {
  ...
  auto error_type = frame.GetErrorType();
  if (error_type == http2::Http2Status::Http2ErrorType::kStreamError) {
    return frame.GetAbslStreamError();
  }
  return frame.GetAbslConnectionError();
}
