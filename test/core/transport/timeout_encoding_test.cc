...
static void
assert_encodes_as (grpc_millis ts, const char *s)
{
...
static void
assert_decodes_as (const char *buffer, grpc_millis expected)
{
  grpc_millis got;
  uint32_t hash = gpr_murmur_hash3 (buffer, strlen (buffer), 0);
  gpr_log (GPR_INFO, "check decoding '%s' (hash=0x%x)", buffer, hash);
  GPR_ASSERT (1 ==
	      grpc_http2_decode_timeout (grpc_slice_from_static_string
					 (buffer), &got));
  if (got != expected)
    {
      gpr_log (GPR_ERROR, "got:'%" PRIdPTR "' != expected:'%" PRIdPTR "'",
	       got, expected);
      abort ();
    }
}
...
void
decode_suite (char ext, grpc_millis (*answer) (int64_t x))
{
...
static grpc_millis
millis_from_nanos (int64_t x)
{
  return (grpc_millis) (x / GPR_NS_PER_MS + (x % GPR_NS_PER_MS != 0));
}

static grpc_millis
millis_from_micros (int64_t x)
{
  return (grpc_millis) (x / GPR_US_PER_MS + (x % GPR_US_PER_MS != 0));
}

static grpc_millis
millis_from_millis (int64_t x)
{
  return (grpc_millis) x;
}

static grpc_millis
millis_from_seconds (int64_t x)
{
...
static void
assert_decoding_fails (const char *s)
{
  grpc_millis x;
  GPR_ASSERT (0 ==
	      grpc_http2_decode_timeout (grpc_slice_from_static_string (s),
					 &x));
}
