...
static void assert_decodes_as(const char* buffer,
  ...
  if (got != expected) {
    gpr_log(GPR_ERROR, "got:'%" PRId64 "' != expected:'%" PRId64 "'",
            got, expected);
    abort();
  }
}
...
void decode_suite(char ext, grpc_millis (*answer)(int64_t x)) {
  long test_vals[] = {1,       12,      123,      1234,      12345,
                      123456,  1234567, 12345678, 123456789, 98765432,
                      9876543, 987654,  98765,    9876,      987,
                      98,      9};
...
static grpc_millis millis_from_nanos(int64_t x) {
  return static_cast<grpc_millis>(x / GPR_NS_PER_MS +
                                  (x % GPR_NS_PER_MS != 0));
}
static grpc_millis millis_from_micros(int64_t x) {
  return static_cast<grpc_millis>(x / GPR_US_PER_MS +
                                  (x % GPR_US_PER_MS != 0));
}
...
void test_decoding(void) {
  ...
  assert_decodes_as("1000000000S",
                    millis_from_seconds(1000 * 1000 * 1000));
...
static void assert_decoding_fails(const char* s) {
  grpc_millis x;
  GPR_ASSERT(0 == grpc_http2_decode_timeout(
                      grpc_slice_from_static_string(s), &x));
}
