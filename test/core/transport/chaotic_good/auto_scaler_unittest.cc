...
TEST(ChooseWorstTailLatencyTest, WorksForClient) {
  absl::flat_hash_map<uint32_t, Metrics> metrics;
  metrics[0] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  metrics[1] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  metrics[2] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  metrics[3] = Metrics(RandomDigest(150, 10), RandomDigest(100, 10));
  metrics[4] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
...
TEST(ChooseWorstTailLatencyTest, WorksForServer) {
  ...
  metrics[1] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  metrics[2] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  metrics[3] = Metrics(RandomDigest(100, 10), RandomDigest(150, 10));
  metrics[4] = Metrics(RandomDigest(100, 10), RandomDigest(100, 10));
  EXPECT_EQ(ChooseWorstTailLatency(std::move(metrics)), 3);
}
