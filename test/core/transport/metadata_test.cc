...
static grpc_slice
maybe_intern (grpc_slice in, bool intern)
{
...
static grpc_slice
maybe_dup (grpc_slice in, bool dup)
{
...
static void
test_create_metadata (bool intern_keys, bool intern_values)
{
  ...
  m1 =
    grpc_mdelem_from_slices (maybe_intern
			     (grpc_slice_from_static_string ("a"),
			      intern_keys),
			     maybe_intern (grpc_slice_from_static_string
					   ("b"), intern_values));
  m2 =
    grpc_mdelem_from_slices (maybe_intern
			     (grpc_slice_from_static_string ("a"),
...
static void
test_create_many_ephemeral_metadata (bool intern_keys, bool intern_values)
{
  ...
  for (i = 0; i < MANY; i++)
    {
      ...
      GRPC_MDELEM_UNREF (grpc_mdelem_from_slices
			 (maybe_intern
			  (grpc_slice_from_static_string ("a"), intern_keys),
			  maybe_intern (grpc_slice_from_copied_string
					(buffer), intern_values)));
    }
...
test_create_many_persistant_metadata (void)
{
  ...
  for (i = 0; i < MANY; i++)
	...
	grpc_mdelem_from_slices (grpc_slice_intern
				 (grpc_slice_from_static_string ("a")),
				 grpc_slice_intern
				 (grpc_slice_from_static_string (buffer)));
    }
  /* verify phase */
  for (i = 0; i < MANY; i++)
    {
...
static void
test_spin_creating_the_same_thing (bool intern_keys, bool intern_values)
{
  gpr_log (GPR_INFO,
...
static void
test_identity_laws (bool intern_keys, bool intern_values)
{
...
test_things_stick_around (void)
{
  ...
  for (i = 0; i < nstrs; i++)
	...
	{
	  gpr_asprintf (&buffer,
			"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx%" PRIuPTR "x",
			shuf[j]);
...
static void
verify_ascii_header_size (const char *key, const char *value,
...
static void
verify_binary_header_size (const char *key, const uint8_t * value,
			   ...
			   bool intern_value)
{
  grpc_mdelem elem =
    grpc_mdelem_from_slices (maybe_intern
			     (grpc_slice_from_static_string (key),
			      intern_key),
			     maybe_intern (grpc_slice_from_static_buffer
					   (value, value_len),
					   intern_value));
...
static void
test_mdelem_sizes_in_hpack (bool intern_key, bool intern_value)
{
...
static void
test_copied_static_metadata (bool dup_key, bool dup_value)
{
