...
static void test_create_metadata(bool intern_keys, bool intern_values) {
  ...
  gpr_log(GPR_INFO, "test_create_metadata: intern_keys=%d intern_values=%d", intern_keys,
          intern_values);

  grpc_init();
  grpc_core::ExecCtx exec_ctx;
  m1 = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
                               maybe_intern(grpc_slice_from_static_string("b"), intern_values));
  m2 = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
                               maybe_intern(grpc_slice_from_static_string("b"), intern_values));
  m3 = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
...
static void test_create_many_ephemeral_metadata(bool intern_keys, bool intern_values) {
  char buffer[GPR_LTOA_MIN_BUFSIZE];
  long i;

  gpr_log(GPR_INFO, "test_create_many_ephemeral_metadata: intern_keys=%d intern_values=%d",
...
static void test_create_many_persistant_metadata(void) {
  char buffer[GPR_LTOA_MIN_BUFSIZE];
  long i;
  grpc_mdelem* created = static_cast<grpc_mdelem*>(gpr_malloc(sizeof(grpc_mdelem) * MANY));
  grpc_mdelem md;
  ...
  for (i = 0; i < MANY; i++) {
    gpr_ltoa(i, buffer);
    created[i] = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string("a")),
                                         grpc_slice_intern(grpc_slice_from_static_string(buffer)));
  }
  /* verify phase */
  for (i = 0; i < MANY; i++) {
    gpr_ltoa(i, buffer);
    md = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string("a")),
                                 grpc_slice_intern(grpc_slice_from_static_string(buffer)));
...
static void test_spin_creating_the_same_thing(bool intern_keys, bool intern_values) {
  gpr_log(GPR_INFO, "test_spin_creating_the_same_thing: intern_keys=%d intern_values=%d",
          intern_keys, intern_values);
  ...
  grpc_mdelem a, b, c;
  GRPC_MDELEM_UNREF(
      a = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
                                  maybe_intern(grpc_slice_from_static_string("b"), intern_values)));
  GRPC_MDELEM_UNREF(
      b = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
                                  maybe_intern(grpc_slice_from_static_string("b"), intern_values)));
  GRPC_MDELEM_UNREF(
      c = grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string("a"), intern_keys),
                                  maybe_intern(grpc_slice_from_static_string("b"), intern_values)));
...
static void test_identity_laws(bool intern_keys, bool intern_values) {
  gpr_log(GPR_INFO, "test_identity_laws: intern_keys=%d intern_values=%d", intern_keys,
          intern_values);
...
static void test_things_stick_around(void) {
  size_t i, j;
  size_t nstrs = 1000;
  grpc_slice* strs = static_cast<grpc_slice*>(gpr_malloc(sizeof(grpc_slice) * nstrs));
  ...
  for (i = 0; i < nstrs; i++) {
    std::string buffer = absl::StrFormat("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx%" PRIuPTR "x", i);
  ...
  for (i = 0; i < nstrs; i++) {
    ...
    for (j = i + 1; j < nstrs; j++) {
      std::string buffer =
          absl::StrFormat("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx%" PRIuPTR "x", shuf[j]);
...
static void test_user_data_works(void) {
  ...
  *ud2 = 2;
  md = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string("abc")),
...
static void verify_ascii_header_size(const char* key, const char* value, bool intern_key,
                                     bool intern_value) {
  grpc_mdelem elem =
      grpc_mdelem_from_slices(maybe_intern(grpc_slice_from_static_string(key), intern_key),
...
static void verify_binary_header_size(const char* key, const uint8_t* value, size_t value_len,
                                      bool intern_key, bool intern_value) {
  grpc_mdelem elem = grpc_mdelem_from_slices(
      maybe_intern(grpc_slice_from_static_string(key), intern_key),
      maybe_intern(grpc_slice_from_static_buffer(value, value_len), intern_value));
...
static void test_mdelem_sizes_in_hpack(bool intern_key, bool intern_value) {
  gpr_log(GPR_INFO, "test_mdelem_size: intern_key=%d intern_value=%d", intern_key, intern_value);
  grpc_init();
  grpc_core::ExecCtx exec_ctx;
  ...
  verify_ascii_header_size("hello", "worldxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", intern_key,
                           intern_value);
  verify_ascii_header_size(":scheme", "http", intern_key, intern_value);

  for (uint8_t i = 0; i < BUFFER_SIZE; i++) {
    verify_binary_header_size("hello-bin", binary_value, i, intern_key, intern_value);
  }
...
static void test_copied_static_metadata(bool dup_key, bool dup_value) {
  gpr_log(GPR_INFO, "test_static_metadata: dup_key=%d dup_value=%d", dup_key, dup_value);
  grpc_init();
  grpc_core::ExecCtx exec_ctx;

  for (size_t i = 0; i < GRPC_STATIC_MDELEM_COUNT; i++) {
    grpc_mdelem p = GRPC_MAKE_MDELEM(&grpc_static_mdelem_table()[i], GRPC_MDELEM_STORAGE_STATIC);
    grpc_mdelem q = grpc_mdelem_from_slices(maybe_dup(GRPC_MDKEY(p), dup_key),
...
static void test_grpc_metadata_batch_get_value_with_absent_key(void) {
  ...
  absl::optional<absl::string_view> value =
      grpc_metadata_batch_get_value(&metadata, "absent_key", &concatenated_value);
...
static void test_grpc_metadata_batch_get_value_returns_one_value(void) {
  ...
  storage.md = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string(kKey)),
                                       grpc_slice_intern(grpc_slice_from_static_string(kValue)));
  GPR_ASSERT(grpc_metadata_batch_link_head(&metadata, &storage) == GRPC_ERROR_NONE);
  std::string concatenated_value;
  absl::optional<absl::string_view> value =
...
static void test_grpc_metadata_batch_get_value_returns_multiple_values(void) {
  ...
  storage1.md = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string(kKey)),
                                        grpc_slice_intern(grpc_slice_from_static_string(kValue1)));
  GPR_ASSERT(grpc_metadata_batch_link_tail(&metadata, &storage1) == GRPC_ERROR_NONE);
  grpc_linked_mdelem storage2;
  storage2.md = grpc_mdelem_from_slices(grpc_slice_intern(grpc_slice_from_static_string(kKey)),
                                        grpc_slice_intern(grpc_slice_from_static_string(kValue2)));
  GPR_ASSERT(grpc_metadata_batch_link_tail(&metadata, &storage2) == GRPC_ERROR_NONE);
  std::string concatenated_value;
  absl::optional<absl::string_view> value =
      grpc_metadata_batch_get_value(&metadata, kKey, &concatenated_value);
  GPR_ASSERT(value.has_value());
