...
static void
test_request_succeeds (grpc_slice_split_mode split_mode,
...
static void
test_succeeds (grpc_slice_split_mode split_mode,
  ...
  if (expect_body != nullptr)
    {
      ...
      GPR_ASSERT (0 ==
		  memcmp (expect_body, response.body, response.body_length));
    }
  else
    {
...
static void
test_fails (grpc_slice_split_mode split_mode, const char *response_text)
{
  grpc_http_parser parser;
  grpc_slice input_slice = grpc_slice_from_copied_string (response_text);
  size_t num_slices;
...
static void
test_request_fails (grpc_slice_split_mode split_mode,
		    const char *request_text)
{
  grpc_http_parser parser;
  grpc_slice input_slice = grpc_slice_from_copied_string (request_text);
  ...
  for (i = 0; i < GPR_ARRAY_SIZE (split_modes); i++)
    {
      ...
      test_request_succeeds (split_modes[i],
			     ...
			     "abc", NULL);
      test_request_succeeds (split_modes[i], "GET / HTTP/1.0\n" "\n" "xyz",
			     "GET", GRPC_HTTP_HTTP10, "/", "xyz", NULL);
      ...
      tmp1 =
	static_cast <
	char *>(gpr_malloc (2 * GRPC_HTTP_PARSER_MAX_HEADER_LENGTH));
