...
static void
on_finish (void *arg, grpc_error * error)
{
  const char *expect =
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR ("pollset_kick",
				 grpc_pollset_kick
				 (grpc_polling_entity_pollset (&g_pops),
				  nullptr)));
  gpr_mu_unlock (g_mu);
}
...
test_get (int port)
{
  ...
  grpc_httpcli_get (&g_context, &g_pops, resource_quota, &req,
		    n_seconds_time (15), GRPC_CLOSURE_CREATE (on_finish,
							      &response,
  ...
  while (!g_done)
    {
      ...
      GPR_ASSERT (GRPC_LOG_IF_ERROR
		  ("pollset_work",
		   grpc_pollset_work (grpc_polling_entity_pollset (&g_pops),
...
test_post (int port)
{
  ...
  grpc_httpcli_post (&g_context, &g_pops, resource_quota, &req, "hello", 5,
		     n_seconds_time (15), GRPC_CLOSURE_CREATE (on_finish,
							       &response,
							       grpc_schedule_on_exec_ctx),
		     &response);
...
static void
destroy_pops (void *p, grpc_error * error)
{
...
main (int argc, char **argv)
{
  ...
  grpc_init ();
  {
	...
	args[0] = gpr_strdup (argv[1]);
      ...
      {
	arg_shift = 1;
	gpr_asprintf (&args[0], "%s/../../tools/distrib/python_wrapper.sh",
		      root);
	gpr_asprintf (&args[1], "%s/../../test/core/http/test_server.py",
		      root);
      }
