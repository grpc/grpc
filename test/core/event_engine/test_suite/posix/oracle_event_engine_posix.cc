...
std::string ReadBytes(int sockfd, int& saved_errno, int num_expected_bytes) {
  ...
  do {
    ...
    if (saved_errno == EAGAIN &&
      ...
      ABSL_CHECK_OK(BlockUntilReadable(sockfd));
...
int WriteBytes(int sockfd, int& saved_errno, std::string write_bytes) {
  ...
  do {
    ...
    if (saved_errno == EAGAIN && ret < static_cast<int>(write_bytes.length())) {
      ABSL_CHECK_GE(ret, 0);
      ABSL_CHECK_OK(BlockUntilWritable(sockfd));
...
void PosixOracleEndpoint::ProcessReadOperations() {
  ABSL_LOG(INFO) << "Starting thread to process read ops ...";
  while (true) {
  ...
  ABSL_LOG(INFO) << "Shutting down read ops thread ...";
}

void PosixOracleEndpoint::ProcessWriteOperations() {
  ABSL_LOG(INFO) << "Starting thread to process write ops ...";
  while (true) {
  ...
  ABSL_LOG(INFO) << "Shutting down write ops thread ...";
}
...
absl::Status PosixOracleListener::Start() {
  grpc_core::MutexLock lock(&mu_);
  ABSL_CHECK(!listener_fds_.empty());
...
PosixOracleListener::~PosixOracleListener() {
  ...
  ABSL_CHECK(write(pipefd_[1], kStopMessage, strlen(kStopMessage)) != -1);
  serve_.Join();
  on_shutdown_(absl::OkStatus());
}

void PosixOracleListener::HandleIncomingConnections() {
  ABSL_LOG(INFO) << "Starting accept thread ...";
  ABSL_CHECK(!listener_fds_.empty());
  ...
  while (true) {
    ...
    for (int i = 0; i < nfds - 1; i++) {
      ...
      if (client_sock_fd < 0) {
        ABSL_LOG(ERROR) << "Error accepting new connection: "
  ...
  ABSL_LOG(INFO) << "Shutting down accept thread ...";
  gpr_free(pfds);
}
