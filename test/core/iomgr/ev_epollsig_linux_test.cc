...
static void
test_fd_init (test_fd * tfds, int *fds, int num_fds)
{
  int i;
  int r;
...
static void
test_fd_cleanup (test_fd * tfds, int num_fds)
{
  int release_fd;
  int i;
  ...
  for (i = 0; i < num_fds; i++)
    {
      ...
      grpc_fd_orphan (tfds[i].fd, nullptr, &release_fd,
...
static void
test_pollset_init (test_pollset * pollsets, int num_pollsets)
{
  int i;
...
static void
test_pollset_cleanup (test_pollset * pollsets, int num_pollsets)
{
  ...
  for (i = 0; i < NUM_POLLSETS; i++)
    {
      GPR_ASSERT (grpc_are_polling_islands_equal
		  (expected_pi,
		   grpc_pollset_get_polling_island (pollsets[i].pollset)));
    }
  ...
  while (thread_wakeups < 1000000)
    {
      ...
      GPR_ASSERT (GRPC_LOG_IF_ERROR ("pollset_work",
				     grpc_pollset_work (shared->pollset,
							&worker,
...
static void
test_threading_wakeup (void *arg, grpc_error * error)
{
  threading_shared *shared = static_cast < threading_shared * >(arg);
  ++shared->wakeups;
  ++thread_wakeups;
...
test_threading (void)
{
  ...
  grpc_wakeup_fd fd;
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("wakeup_fd_init", grpc_wakeup_fd_init (&fd)));
  shared.wakeup_fd = &fd;
  ...
  shared.wakeups = 0;
  {
    ...
    grpc_fd_notify_on_read (shared.wakeup_desc,
			    GRPC_CLOSURE_INIT (&shared.on_wakeup,
					       test_threading_wakeup, &shared,
					       grpc_schedule_on_exec_ctx));
  }
  ...
  grpc_wakeup_fd_destroy (&fd);
  {
    ...
    grpc_pollset_shutdown (shared.pollset,
			   GRPC_CLOSURE_CREATE (destroy_pollset,
						shared.pollset,
						grpc_schedule_on_exec_ctx));
  }
