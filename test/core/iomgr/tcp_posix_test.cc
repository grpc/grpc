...
static void
read_cb (void *user_data, grpc_error * error)
{
...
large_read_test (size_t slice_size)
{
  ...
  grpc_channel_args args = { GPR_ARRAY_SIZE (a), a };
  ep =
    grpc_tcp_create (grpc_fd_create (sv[1], "large_read_test"), &args,
		     "test");
  grpc_endpoint_add_to_pollset (ep, g_pollset);
...
static void
write_done (void *user_data /* write_socket_state */ ,
	    grpc_error * error)
{
...
void
on_fd_released (void *arg, grpc_error * errors)
{
  int *done = (int *) arg;
  *done = 1;
  GPR_ASSERT (GRPC_LOG_IF_ERROR
...
static void
release_fd_test (size_t num_bytes, size_t slice_size)
{
  int sv[2];
  grpc_endpoint *ep;
  struct read_socket_state state;
