...
static void create_inet_sockets(int sv[2]) {
  ...
  int ret;
  do {
    ret = connect(client, reinterpret_cast<sockaddr*>(&addr), sizeof(sockaddr_in));
...
static size_t count_slices(grpc_slice* slices, size_t nslices, int* current_data) {
  size_t num_bytes = 0;
  unsigned i, j;
  unsigned char* buf;
  for (i = 0; i < nslices; ++i) {
...
static void read_cb(void* user_data, grpc_error_handle error) {
  ...
  gpr_log(GPR_INFO, "Read %" PRIuPTR " bytes of %" PRIuPTR, read_bytes, state->target_read_bytes);
...
static void read_test(size_t num_bytes, size_t slice_size) {
  ...
  gpr_log(GPR_INFO, "Read test of size %" PRIuPTR ", slice size %" PRIuPTR, num_bytes, slice_size);
...
static void large_read_test(size_t slice_size) {
  ...
  grpc_channel_args args = {GPR_ARRAY_SIZE(a), a};
  ep = grpc_tcp_create(grpc_fd_create(sv[1], "large_read_test", false), &args, "test");
  grpc_endpoint_add_to_pollset(ep, g_pollset);
...
static void write_done(void* user_data /* write_socket_state */, grpc_error_handle error) {
  GPR_ASSERT(error == GRPC_ERROR_NONE);
...
void timestamps_verifier(void* arg, grpc_core::Timestamps* ts, grpc_error_handle error) {
  GPR_ASSERT(error == GRPC_ERROR_NONE);
...
static void write_test(size_t num_bytes, size_t slice_size, bool collect_timestamps) {
  int sv[2];
  grpc_endpoint* ep;
  ...
  gpr_log(GPR_INFO, "Start write test with %" PRIuPTR " bytes, slice size %" PRIuPTR, num_bytes,
          slice_size);
  ...
  a[0].type = GRPC_ARG_INTEGER, a[0].value.integer = static_cast<int>(slice_size);
  grpc_channel_args args = {GPR_ARRAY_SIZE(a), a};
  ep = grpc_tcp_create(grpc_fd_create(sv[1], "write_test", collect_timestamps), &args, "test");
  grpc_endpoint_add_to_pollset(ep, g_pollset);
  ...
  grpc_endpoint_write(
      ...
      grpc_event_engine_can_track_errors() && collect_timestamps ? &done_timestamps : nullptr);
  drain_socket_blocking(sv[0], num_bytes, num_bytes);
  exec_ctx.Flush();
  gpr_mu_lock(g_mu);
  for (;;) {
    grpc_pollset_worker* worker = nullptr;
    if (state.write_done && (!(grpc_event_engine_can_track_errors() && collect_timestamps) ||
...
static void release_fd_test(size_t num_bytes, size_t slice_size) {
  ...
  gpr_log(GPR_INFO, "Release fd read_test of size %" PRIuPTR ", slice size %" PRIuPTR, num_bytes,
          slice_size);
  ...
  while (state.read_bytes < state.target_read_bytes) {
    grpc_pollset_worker* worker = nullptr;
    GPR_ASSERT(GRPC_LOG_IF_ERROR("pollset_work", grpc_pollset_work(g_pollset, &worker, deadline)));
    gpr_log(GPR_DEBUG, "wakeup: read=%" PRIdPTR " target=%" PRIdPTR, state.read_bytes,
...
static grpc_endpoint_test_fixture create_fixture_tcp_socketpair(size_t slice_size) {
  int sv[2];
  grpc_endpoint_test_fixture f;
  grpc_core::ExecCtx exec_ctx;

  create_sockets(sv);
  grpc_resource_quota* resource_quota = grpc_resource_quota_create("tcp_posix_test_socketpair");
  grpc_arg a[1];
  a[0].key = const_cast<char*>(GRPC_ARG_TCP_READ_CHUNK_SIZE);
  a[0].type = GRPC_ARG_INTEGER;
  a[0].value.integer = static_cast<int>(slice_size);
  grpc_channel_args args = {GPR_ARRAY_SIZE(a), a};
  f.client_ep = grpc_tcp_create(grpc_fd_create(sv[0], "fixture:client", false), &args, "test");
  f.server_ep = grpc_tcp_create(grpc_fd_create(sv[1], "fixture:server", false), &args, "test");
