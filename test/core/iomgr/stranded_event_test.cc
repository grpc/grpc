...
void StartCall(TestCall* test_call) {
  ...
  grpc_call_error error = grpc_call_start_batch(
      test_call->call, ops, static_cast<size_t>(op - ops), tag,
      nullptr);
...
void ReceiveInitialMetadata(TestCall* test_call,
...
class TestServer {
 public:
  explicit TestServer() {
    ...
    address_ = grpc_core::JoinHostPort("127.0.0.1",
 ...
 private:
  void AcceptThread() {
    ...
    grpc_call_error error =
        grpc_server_request_call(server_, &call, &call_details,
...
grpc_core::Resolver::Result BuildResolverResponse(
  ...
  for (const auto& address_str : addresses) {
    absl::StatusOr<grpc_core::URI> uri =
        grpc_core::URI::Parse(address_str);
    if (!uri.ok()) {
...
TEST(Pollers, TestReadabilityNotificationsDontGetStrandedOnOneCq) {
   ...
   * exceeds the value of MAX_EPOLL_EVENTS_HANDLED_EACH_POLL_CALL (16),
   * which is enough to repro a bug at time of writing. */
  ...
  const std::string kSharedUnconnectableAddress =
      grpc_core::JoinHostPort("127.0.0.1",
                              grpc_pick_unused_port_or_die());
  ...
  // Instantiate servers inline here, so that we get port allocation out
  // of the way and don't depend on it during the actual test. It can
  // sometimes take time to allocate kNumCalls ports from the port
  // server, and we don't want to hit test timeouts because of that.
  ...
  for (int i = 0; i < kNumCalls; i++) {
    ...
    threads.push_back(std::thread([kSharedUnconnectableAddress,
                                   ...
                                   &ping_pong_round_mu,
                                   &ping_pong_round_cv, test_server]() {
      ...
      // Start a call, and ensure that round_robin load balancing is
      // configured
      ...
      EXPECT_EQ(std::string(lb_policy_name), "round_robin")
          << "not using round robin; this test has a low chance of "
             "hitting the "
      ...
      gpr_log(GPR_DEBUG,
              "now receive status on call with server address:%s",
              test_server->address().c_str());
      FinishCall(test_call.get());
      GPR_ASSERT(test_call->status.has_value());
      GPR_ASSERT(test_call->status.value() ==
  ...
  for (size_t i = 1; i <= kNumMessagePingPongsPerCall; i++) {
    {
      ...
      ping_pong_round++;
      ping_pong_round_cv.SignalAll();
      gpr_log(GPR_DEBUG, "initiate ping pong round: %ld",
              ping_pong_round);
    }
  }
