...
static void
set_event_to_true (void *value, grpc_error * error)
{
  gpr_event_set (static_cast < gpr_event * >(value), (void *) 1);
}

static void
test_execute_one (void)
{
...
static void
check_one (void *a, grpc_error * error)
{
...
execute_many_loop (void *a)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE (set_event_to_true, &args->done,
					   grpc_combiner_scheduler (args->
								    lock)),
		      GRPC_ERROR_NONE);
}

static void
test_execute_many (void)
{
  ...
  for (size_t i = 0; i < GPR_ARRAY_SIZE (thds); i++)
    {
      ...
      GPR_ASSERT (gpr_thd_new
		  (&thds[i], "grpc_execute_many", execute_many_loop, &ta[i],
		   &options));
    }
  for (size_t i = 0; i < GPR_ARRAY_SIZE (thds); i++)
    {
...
static void
in_finally (void *arg, grpc_error * error)
{
  gpr_event_set (&got_in_finally, (void *) 1);
}

static void
add_finally (void *arg, grpc_error * error)
{
...
test_execute_finally (void)
{
  ...
  GRPC_CLOSURE_SCHED (GRPC_CLOSURE_CREATE
		      (add_finally, lock, grpc_combiner_scheduler (lock)),
		      GRPC_ERROR_NONE);
  grpc_core::ExecCtx::Get ()->Flush ();
