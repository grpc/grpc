...
static void
on_start (grpc_fd * emfd, void *user_data)
{
  g_number_of_starts++;
}

static bool
on_read (grpc_fd * emfd)
{
...
static void
on_write (grpc_fd * emfd, void *user_data,
...
static void
on_fd_orphaned (grpc_fd * emfd, grpc_closure * closure, void *user_data)
{
...
static int
test_socket_factory_socket (grpc_socket_factory * factory, int domain,
...
static int
test_socket_factory_bind (grpc_socket_factory * factory, int sockfd,
...
static int
test_socket_factory_compare (grpc_socket_factory * a, grpc_socket_factory * b)
{
  return GPR_ICMP (a, b);
}

static void
test_socket_factory_destroy (grpc_socket_factory * factory)
{
  test_socket_factory *f = (test_socket_factory *) factory;
  gpr_free (f);
}
...
static test_socket_factory *
test_socket_factory_create (void)
{
  test_socket_factory *factory =
    static_cast <
    test_socket_factory * >(gpr_malloc (sizeof (test_socket_factory)));
...
static void
test_no_op_with_port_and_socket_factory (void)
{
  grpc_pollset_init (g_pollset, &g_mu);
  g_number_of_orphan_calls = 0;
  grpc_core::ExecCtx exec_ctx;
