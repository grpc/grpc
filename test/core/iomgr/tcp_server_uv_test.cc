...
static void tcp_connect(const struct sockaddr* remote,
...
static void test_connect(unsigned n) {
  ...
  GPR_ASSERT(
      (uv_ip6_addr("::", svr_port, (struct sockaddr_in6*)addr)) == 0);
  ...
  for (i = 0; i < n; i++) {
    ...
    tcp_connect((struct sockaddr*)addr, (socklen_t)resolved_addr.len,
                &result);
