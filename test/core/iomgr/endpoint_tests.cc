...
/*
   ...
   All tests which write data into an endpoint write i%256 into byte i,
   which is verified by readers.

   In general there are a few interesting things to vary which may lead
   to exercising different codepaths in an implementation:
...
size_t count_slices(grpc_slice* slices, size_t nslices,
                    int* current_data) {
  size_t num_bytes = 0;
  size_t i;
  size_t j;
...
static grpc_endpoint_test_fixture begin_test(
    grpc_endpoint_test_config config, const char* test_name,
    size_t slice_size) {
...
static grpc_slice* allocate_blocks(size_t num_bytes, size_t slice_size,
  ...
  size_t nslices =
      num_bytes / slice_size + (num_bytes % slice_size ? 1 : 0);
  grpc_slice* slices = static_cast<grpc_slice*>(
      gpr_malloc(sizeof(grpc_slice) * nslices));
  ...
  *num_blocks = nslices;
  ...
  for (i = 0; i < nslices; ++i) {
    slices[i] = grpc_slice_malloc(
        slice_size > num_bytes_left ? num_bytes_left : slice_size);
...
static void read_scheduler(void* data, grpc_error_handle /* error */) {
  ...
  grpc_endpoint_read(state->read_ep, &state->incoming,
                     &state->done_read,
                     /*urgent=*/false);
}
...
static void read_and_write_test_read_handler(void* data,
  ...
  if (state->bytes_read == state->target_bytes ||
    ...
    /* We perform many reads one after another. If grpc_endpoint_read
     * and the read_handler are both run inline, we might end up growing
     * the stack
...
static void read_and_write_test_write_handler(void* data,
  ...
  if (error == GRPC_ERROR_NONE) {
    ...
    if (state->current_write_size != 0) {
      slices = allocate_blocks(state->current_write_size, 8192,
                               &nslices, &state->current_write_data);
      grpc_slice_buffer_reset_and_unref(&state->outgoing);
      grpc_slice_buffer_addn(&state->outgoing, slices, nslices);
      /* We perform many writes one after another. If
       * grpc_endpoint_write and the write_handler are both run inline,
       * we might end up growing the stack beyond the limit. Schedule
       * the write on ExecCtx to avoid this. */
...
static void read_and_write_test(grpc_endpoint_test_config config,
  ...
  gpr_log(GPR_DEBUG,
          "num_bytes=%" PRIuPTR " write_size=%" PRIuPTR
  ...
  GRPC_CLOSURE_INIT(&state.done_read, read_and_write_test_read_handler,
                    &state, grpc_schedule_on_exec_ctx);
  GRPC_CLOSURE_INIT(&state.write_scheduler, write_scheduler, &state,
                    grpc_schedule_on_exec_ctx);
  GRPC_CLOSURE_INIT(&state.done_write,
  ...
  /* NOTE: Sets up initial conditions so we can have the same write
     handler for the first iteration as for later iterations. It does
     the right thing even when bytes_written is unsigned. */
  state.bytes_written -= state.current_write_size;
...
static void multiple_shutdown_test(grpc_endpoint_test_config config) {
  ...
  grpc_slice_buffer_add(&slice_buffer,
