...
size_t
count_slices (grpc_slice * slices, size_t nslices, int *current_data)
{
  size_t num_bytes = 0;
  size_t i;
  size_t j;
		 ...
		 size_t * num_blocks, uint8_t * current_data)
{
  ...
  grpc_slice *slices =
    (grpc_slice *) gpr_malloc (sizeof (grpc_slice) * nslices);
...
static void
read_and_write_test_read_handler (void *data, grpc_error * error)
{
...
static void
read_and_write_test_write_handler (void *data, grpc_error * error)
{
  struct read_and_write_test_state *state =
    (struct read_and_write_test_state *) data;
		     ...
		     size_t slice_size, bool shutdown)
{
  ...
  gpr_log (GPR_DEBUG,
	   "num_bytes=%" PRIuPTR " write_size=%" PRIuPTR " slice_size=%"
	   PRIuPTR " shutdown=%d", num_bytes, write_size, slice_size,
	   shutdown);
  ...
  if (shutdown)
  ...
  else
    {
      gpr_log (GPR_INFO,
	       ...
	       " bytes, slice size %" PRIuPTR, num_bytes, slice_size);
    }
...
static void
inc_on_failure (void *arg, grpc_error * error)
{
  gpr_mu_lock (g_mu);
  *(int *) arg += (error != GRPC_ERROR_NONE);
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("kick", grpc_pollset_kick (g_pollset, nullptr)));
  gpr_mu_unlock (g_mu);
}

static void
wait_for_fail_count (int *fail_count, int want_fail_count)
{
  grpc_core::ExecCtx::Get ()->Flush ();
  gpr_mu_lock (g_mu);
  grpc_millis deadline =
...
static void
multiple_shutdown_test (grpc_endpoint_test_config config)
{
  grpc_endpoint_test_fixture f =
...
void
grpc_endpoint_tests (grpc_endpoint_test_config config,
