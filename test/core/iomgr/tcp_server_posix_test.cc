...
static void
on_connect_result_init (on_connect_result * result)
{
...
static void
on_connect_result_set (on_connect_result * result,
		       const grpc_tcp_server_acceptor * acceptor)
{
  ...
  result->server_fd =
    grpc_tcp_server_port_fd (result->server, acceptor->port_index,
			     acceptor->fd_index);
}

static void
server_weak_ref_shutdown (void *arg, grpc_error * error)
{
...
static void
server_weak_ref_init (server_weak_ref * weak_ref)
{
  weak_ref->server = nullptr;
...
static void
server_weak_ref_set (server_weak_ref * weak_ref, grpc_tcp_server * server)
{
...
static void
test_addr_init_str (test_addr * addr)
{
  char *str = nullptr;
  if (grpc_sockaddr_to_string (&str, &addr->addr, 0) != -1)
    {
      size_t str_len;
      memcpy (addr->str, str,
	      (str_len = strnlen (str, sizeof (addr->str) - 1)));
      addr->str[str_len] = '\0';
      gpr_free (str);
    }
  else
...
static void
on_connect (void *arg, grpc_endpoint * tcp, grpc_pollset * pollset,
	    grpc_tcp_server_acceptor * acceptor)
{
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("pollset_kick", grpc_pollset_kick (g_pollset, nullptr)));
  gpr_mu_unlock (g_mu);
}
...
static grpc_error *
tcp_connect (const test_addr * remote, on_connect_result * result)
{
...
static void
test_connect (size_t num_connects,
	      ...
	      test_addrs * dst_addrs, bool test_dst_addrs)
{
  ...
  gpr_log (GPR_INFO,
	   ...
	   (unsigned long) (channel_args !=
			    nullptr ? channel_args->num_args : 0),
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR ("grpc_tcp_server_add_port",
				 grpc_tcp_server_add_port (s, &resolved_addr,
							   &svr_port)));
  ...
  GPR_ASSERT (grpc_tcp_server_add_port (s, &resolved_addr1, &port) ==
	      GRPC_ERROR_NONE && port == svr1_port);
      ...
      for (port_num = 0; port_num < num_ports; ++port_num)
	{
	  ...
	  for (fd_num = 0; fd_num < num_fds; ++fd_num)
	      ...
	      for (connect_num = 0; connect_num < num_connects; ++connect_num)
		{
		  ...
		  on_connect_result_init (&result);
		  GPR_ASSERT (GRPC_LOG_IF_ERROR
			      ("tcp_connect", tcp_connect (&dst, &result)));
		  ...
		  GPR_ASSERT (grpc_tcp_server_port_fd
			      (s, result.port_index,
			       result.fd_index) == result.server_fd);
		}
	    }
