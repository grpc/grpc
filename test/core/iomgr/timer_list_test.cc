...
static void add_test(void) {
  ...
  for (i = 0; i < 10; i++) {
    grpc_timer_init(&exec_ctx, &timers[i], start + 10,
                    GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(i),
...
void destruction_test(void) {
  ...
  grpc_timer_init(&exec_ctx, &timers[0], 100,
                  GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(0),
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&exec_ctx, &timers[1], 3,
                  GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(1),
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&exec_ctx, &timers[2], 100,
                  GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(2),
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&exec_ctx, &timers[3], 3,
                  GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(3),
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&exec_ctx, &timers[4], 1,
                  GRPC_CLOSURE_CREATE(cb, reinterpret_cast<void*>(4),
                                      grpc_schedule_on_exec_ctx));
  exec_ctx.now = 2;
  GPR_ASSERT(grpc_timer_check(&exec_ctx, nullptr) == GRPC_TIMERS_FIRED);
  grpc_exec_ctx_finish(&exec_ctx);
