...
static void cb(void* arg, grpc_error_handle error) {
  cb_called[reinterpret_cast<intptr_t>(arg)]
...
static void add_test(void) {
  ...
  GPR_ASSERT(grpc_timer_check(nullptr) ==
...
/* Cleans up a list with pending timers that simulate
   long-running-services. This test does the following: 1) Simulates
   grpc server start time to 25 days in the past (completed in `main`
   using TestOnlyGlobalInit()) 2) Creates 4 timers - one with a deadline
   25 days in the future, one just 3 milliseconds in future, one way out
   in the future, and one using the grpc_timespec_to_millis_round_up
   function to compute a deadline of 25 days in the future 3) Simulates
   4 milliseconds of elapsed time by changing `now` (cached at step 1)
   to `now+4` 4) Shuts down the timer list
...
void long_running_service_cleanup_test(void) {
  ...
  grpc_timer_init(&timers[0], now + kMillisIn25Days,
                  GRPC_CLOSURE_CREATE(cb, (void*)(intptr_t)0,
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&timers[1], now + 3,
                  GRPC_CLOSURE_CREATE(cb, (void*)(intptr_t)1,
                                      grpc_schedule_on_exec_ctx));
  grpc_timer_init(&timers[2], GRPC_MILLIS_INF_FUTURE - 1,
                  GRPC_CLOSURE_CREATE(cb, (void*)(intptr_t)2,
                                      grpc_schedule_on_exec_ctx));
  ...
  /* grpc_timespec_to_millis_round_up is how users usually compute a
    millisecond input value into grpc_timer_init, so we mimic that
    behavior here */
  grpc_timer_init(&timers[3],
                  grpc_timespec_to_millis_round_up(deadline_spec),
                  GRPC_CLOSURE_CREATE(cb, (void*)(intptr_t)3,
                                      grpc_schedule_on_exec_ctx));
...
int main(int argc, char** argv) {
  ...
  /* Begin long running service tests */
  {
    ...
    gpr_timespec new_start = gpr_time_sub(
