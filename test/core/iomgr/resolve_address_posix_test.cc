...
{
  return
    grpc_timespec_to_millis_round_up (grpc_timeout_seconds_to_deadline
				      (seconds));
}

static void
actually_poll (void *argsp)
{
...
static void
poll_pollset_until_request_done (args_struct * args)
{
...
static void
must_succeed (void *argsp, grpc_error * err)
{
  args_struct *args = static_cast < args_struct * >(argsp);
...
static void
must_fail (void *argsp, grpc_error * err)
{
  args_struct *args = static_cast < args_struct * >(argsp);
  GPR_ASSERT (err != GRPC_ERROR_NONE);
  gpr_atm_rel_store (&args->done_atm, 1);
...
test_unix_socket (void)
{
  ...
  grpc_resolve_address ("unix:/path/name", nullptr, args.pollset_set,
...
static void
test_unix_socket_path_name_too_long (void)
{
  grpc_core::ExecCtx exec_ctx;
  args_struct args;
  args_init (&args);
  ...
  poll_pollset_until_request_done (&args);
  grpc_resolve_address (path_name, nullptr, args.pollset_set,
