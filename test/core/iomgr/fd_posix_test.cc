		     ...
		     bool success)
{
  ...
  server *sv = se->sv;
  grpc_fd_orphan (se->em_fd, nullptr, nullptr, false /* already_closed */ ,
		  "a");
  gpr_free (se);
...
listen_shutdown_cb (void *arg /*server */ , int success)
{
  ...
  grpc_fd_orphan (sv->em_fd, nullptr, nullptr, false /* already_closed */ ,
		  "b");
	   ...
	   grpc_error * error)
{
  ...
  fd =
    accept (grpc_fd_wrapped_fd (listen_em_fd), (struct sockaddr *) &ss,
	    &slen);
  GPR_ASSERT (fd >= 0);
...
client_session_shutdown_cb (void *arg /*client */ , int success)
{
  ...
  grpc_fd_orphan (cl->em_fd, nullptr, nullptr, false /* already_closed */ ,
		  "c");
  cl->done = 1;
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("pollset_kick", grpc_pollset_kick (g_pollset, nullptr)));
...
static void
first_read_callback (void *arg /* fd_change_data */ ,
		     grpc_error * error)
{
...
static void
second_read_callback (void *arg /* fd_change_data */ ,
		      grpc_error * error)
{
  fd_change_data *fdc = static_cast < fd_change_data * >(arg);
