...
static void test_set_get_int() {
  grpc_error_handle error =
      GRPC_ERROR_CREATE_FROM_STATIC_STRING("Test");
  GPR_ASSERT(error);
  intptr_t i = 0;
...
static void test_set_get_str() {
  ...
  GPR_ASSERT(
      !grpc_error_get_str(error, GRPC_ERROR_STR_TSI_ERROR, &str));

  GPR_ASSERT(grpc_error_get_str(error, GRPC_ERROR_STR_FILE, &str));
  GPR_ASSERT(strstr((char*)GRPC_SLICE_START_PTR(str),
                    "error_test.c"));  // __FILE__ expands differently
                                       // on Windows. All should at
                                       // least contain error_test.c

  GPR_ASSERT(
      grpc_error_get_str(error, GRPC_ERROR_STR_DESCRIPTION, &str));
  ...
  GPR_ASSERT(
      grpc_error_get_str(error, GRPC_ERROR_STR_GRPC_MESSAGE, &str));
  GPR_ASSERT(!strncmp((char*)GRPC_SLICE_START_PTR(str),
                      "longer message", GRPC_SLICE_LENGTH(str)));
...
static void test_copy_and_unref() {
  ...
  grpc_slice str;
  GPR_ASSERT(
      grpc_error_get_str(error1, GRPC_ERROR_STR_GRPC_MESSAGE, &str));
  GPR_ASSERT(!strncmp((char*)GRPC_SLICE_START_PTR(str), "message",
                      GRPC_SLICE_LENGTH(str)));

  // error 1 has two refs
  GRPC_ERROR_REF(error1);
  // this gives error3 a ref to the new error, and decrements error1 to
  // one ref
  grpc_error_handle error3 =
      grpc_error_set_str(error1, GRPC_ERROR_STR_SYSCALL,
                         grpc_slice_from_static_string("syscall"));
  GPR_ASSERT(error3 !=
             error1);  // should not be the same because of extra ref
  GPR_ASSERT(
      grpc_error_get_str(error3, GRPC_ERROR_STR_GRPC_MESSAGE, &str));
  GPR_ASSERT(!strncmp((char*)GRPC_SLICE_START_PTR(str), "message",
                      GRPC_SLICE_LENGTH(str)));
...
static void test_create_referencing() {
  ...
  grpc_error_handle parent =
      GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING("Parent", &child,
                                                       1);
  GPR_ASSERT(parent);
...
static void test_create_referencing_many() {
  ...
  children[2] =
      grpc_error_set_str(GRPC_ERROR_CREATE_FROM_STATIC_STRING("Child3"),
                         GRPC_ERROR_STR_GRPC_MESSAGE,
                         grpc_slice_from_static_string("message 3"));

  grpc_error_handle parent =
      GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING("Parent",
                                                       children, 3);
  GPR_ASSERT(parent);
...
static void test_overflow() {
  grpc_error_handle error =
      GRPC_ERROR_CREATE_FROM_STATIC_STRING("Overflow");

  for (size_t i = 0; i < 150; ++i) {
    error = grpc_error_add_child(
        error, GRPC_ERROR_CREATE_FROM_STATIC_STRING("Child"));
  }
  ...
  GPR_ASSERT(i == 5);
  GPR_ASSERT(
      !grpc_error_get_int(error, GRPC_ERROR_INT_GRPC_STATUS, &i));
