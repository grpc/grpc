...
create_test_elements (size_t num_elements)
{
  grpc_timer *elems =
    static_cast <
    grpc_timer * >(gpr_malloc (num_elements * sizeof (grpc_timer)));
  size_t i;
...
static int
contains (grpc_timer_heap * pq, grpc_timer * el)
{
  size_t i;
...
static void
check_valid (grpc_timer_heap * pq)
{
  size_t i;
  for (i = 0; i < pq->timer_count; ++i)
    {
...
static elem_struct *
search_elems (elem_struct * elems, size_t count, bool inserted)
{
  size_t *search_order =
...
test2 (void)
{
  ...
  elem_struct *elems =
    static_cast <
    elem_struct * >(gpr_malloc (elems_size * sizeof (elem_struct)));
