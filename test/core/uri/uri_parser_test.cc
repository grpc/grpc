...
static void TestSucceeds(absl::string_view uri_text, absl::string_view scheme,
...
TEST(URIParserTest, BasicExamplesAreParsedCorrectly) {
  ...
  TestSucceeds("zookeeper://127.0.0.1:2181/foo/bar", "zookeeper", "127.0.0.1:2181", "/foo/bar", {},
               {}, "");
  TestSucceeds("dns:foo.com#fragment-all-the-things", "dns", "", "foo.com", {}, {},
               "fragment-all-the-things");
  TestSucceeds("http://localhost:8080/whatzit?mi_casa=su_casa", "http", "localhost:8080",
               "/whatzit", {{"mi_casa", "su_casa"}}, {{"mi_casa", "su_casa"}}, "");
  TestSucceeds("http://localhost:8080/whatzit?1=2#buckle/my/shoe", "http", "localhost:8080",
               "/whatzit", {{"1", "2"}}, {{"1", "2"}}, "buckle/my/shoe");
}
...
TEST(URIParserTest, UncommonValidExamplesAreParsedCorrectly) {
  ...
  TestSucceeds("http:?legit#twice", "http", "", "", {{"legit", ""}}, {{"legit", ""}}, "twice");
...
TEST(URIParserTest, VariousKeyValueAndNonKVQueryParamsAreParsedCorrectly) {
  TestSucceeds("http://foo/path?a&b=B&c=&#frag", "http", "foo", "/path",
               {{"c", ""}, {"a", ""}, {"b", "B"}}, {{"a", ""}, {"b", "B"}, {"c", ""}}, "frag");
}

TEST(URIParserTest, ParserTreatsFirstEqualSignAsKVDelimiterInQueryString) {
  TestSucceeds("http://localhost:8080/?too=many=equals&are=present=here#fragged", "http",
               "localhost:8080", "/", {{"are", "present=here"}, {"too", "many=equals"}},
               {{"too", "many=equals"}, {"are", "present=here"}}, "fragged");
  TestSucceeds("http://auth/path?foo=bar=baz&foobar===", "http", "auth", "/path",
               {{"foo", "bar=baz"}, {"foobar", "=="}}, {{"foo", "bar=baz"}, {"foobar", "=="}}, "");
}

TEST(URIParserTest, RepeatedQueryParamsAreSupportedInOrderedPairsButDeduplicatedInTheMap) {
  absl::StatusOr<grpc_core::URI> uri = grpc_core::URI::Parse("http://foo/path?a=2&a=1&a=3");
  ASSERT_TRUE(uri.ok());
  ...
  ASSERT_THAT(uri->query_parameter_pairs(), ElementsAre(grpc_core::URI::QueryParam{"a", "2"},
...
TEST(URIParserTest, QueryParamMapRemainsValiditAfterMovingTheURI) {
  ...
  {
    absl::StatusOr<grpc_core::URI> uri = grpc_core::URI::Parse("http://foo/path?a=2&b=1&c=3");
    ASSERT_TRUE(uri.ok());
...
TEST(URIParserTest, QueryParamMapRemainsValidAfterCopyingTheURI) {
  ...
  grpc_core::URI uri_copy;
  {
    absl::StatusOr<grpc_core::URI> del_uri = grpc_core::URI::Parse("http://foo/path?a=2&b=1&c=3");
...
TEST(URIParserTest, NonKeyValueQueryStringsWork) {
  TestSucceeds("http://www.google.com?yay-i'm-using-queries", "http", "www.google.com", "",
               {{"yay-i'm-using-queries", ""}}, {{"yay-i'm-using-queries", ""}}, "");
}

TEST(URIParserTest, IPV6StringsAreParsedCorrectly) {
  TestSucceeds("ipv6:[2001:db8::1%252]:12345", "ipv6", "", "[2001:db8::1%2]:12345", {}, {}, "");
  TestSucceeds("ipv6:[fe80::90%eth1.sky1]:6010", "ipv6", "", "[fe80::90%eth1.sky1]:6010", {}, {},
               "");
}
