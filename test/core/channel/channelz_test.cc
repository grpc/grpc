...
class CallCountingHelperPeer {
 public:
  explicit CallCountingHelperPeer(CallCountingHelper* node)
      : node_(node) {}
...
void ValidateGetTopChannels(size_t expected_channels) {
  ...
  // This check will naturally have to change when we support
  // pagination. tracked: https://github.com/grpc/grpc/issues/16019.
...
class ChannelFixture {
 public:
  explicit ChannelFixture(int max_tracer_event_memory = 0) {
    ...
    grpc_channel_args client_args = {GPR_ARRAY_SIZE(client_a),
                                     client_a};
    channel_ = grpc_insecure_channel_create("fake_target", &client_args,
                                            nullptr);
  }
...
class ServerFixture {
 public:
  explicit ServerFixture(int max_tracer_event_memory = 0) {
    ...
    grpc_channel_args server_args = {GPR_ARRAY_SIZE(server_a),
                                     server_a};
...
void ValidateChildInteger(const Json::Object& object,
...
void ValidateCounters(const std::string& json_str,
  ...
  ValidateChildInteger(data.object_value(), "callsStarted",
                       args.calls_started);
  ValidateChildInteger(data.object_value(), "callsFailed",
                       args.calls_failed);
...
void ValidateServer(ServerNode* server,
                    const ValidateChannelDataArgs& args) {
...
class ChannelzRegistryBasedTest
...
TEST_F(ChannelzRegistryBasedTest, GetTopChannelsUuidCheck) {
  ...
  std::vector<intptr_t> uuids =
      GetUuidListFromArray(array.array_value());
...
TEST_F(ChannelzRegistryBasedTest, InternalChannelTest) {
  ...
  grpc_channel* internal_channel = grpc_insecure_channel_create(
...
TEST(ChannelzServerTest, BasicServerAPIFunctionality) {
  ...
  ServerNode* channelz_server =
      server.server()->core_server->channelz_node();
