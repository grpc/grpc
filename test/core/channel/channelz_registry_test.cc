...
TEST_F(ChannelzRegistryTest, UuidsAreIncreasing) {
  ...
  for (size_t i = 1; i < channelz_channels.size(); ++i) {
    EXPECT_LT(channelz_channels[i - 1]->uuid(),
...
TEST_F(ChannelzRegistryTest, TestUnregistration) {
  ...
  // These channels will stay in the registry for the duration of the
  // test.
  ...
  {
    // These channels will unregister themselves at the end of this
    // block.
  ...
  // Check that the even channels are present and the odd channels are
  // not.
  for (int i = 0; i < kLoopIterations; i++) {
    RefCountedPtr<BaseNode> retrieved =
  ...
  // sure that the unregistration didn't leave the registry in a weird
  // state.
