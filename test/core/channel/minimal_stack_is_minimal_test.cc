...
main (int argc, char **argv)
{
  ...
  errors += CHECK_STACK ("chttp2", nullptr, GRPC_SERVER_CHANNEL, "server",
			 ...
			 "message_compress", "connected", NULL);
  errors +=
    CHECK_STACK (nullptr, nullptr, GRPC_CLIENT_CHANNEL, "client-channel",
		 NULL);
...
static int
check_stack (const char *file, int line, const char *transport_name,
      ...
      for (size_t i = 0; i < channel_args->num_args; i++)
	{
	  ...
	  switch (channel_args->args[i].type)
	    ...
	    case GRPC_ARG_STRING:
	      gpr_strvec_add (&v,
			      gpr_strdup (channel_args->args[i].value.
					  string));
	      break;
      ...
      gpr_log (file, line, GPR_LOG_SEVERITY_ERROR,
	       ...
	       transport_name,
	       grpc_channel_stack_type_string (static_cast <
