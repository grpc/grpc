...
/*******************************************************************************
 * This test verifies that various stack configurations result in the
 * set of filters that we expect.
 *
 * This is akin to a golden-file test, and suffers the same
 * disadvantages and advantages: it reflects that the code as written
 * has not been modified - and valid code modifications WILL break this
 * test and it will need updating.
 *
 * The intent therefore is to allow code reviewers to more easily catch
 * changes that perturb the generated list of channel filters in
 * different configurations and assess whether such a change is correct
 * and desirable.
 */
...
static int check_stack(const char* file, int line,
                       const char* transport_name,
                       grpc_channel_args* init_args,
                       unsigned channel_stack_type, ...);

// arguments: const char *transport_name   - the name of the transport
// type to
//                                           simulate
//            grpc_channel_args *init_args - channel args to pass down
//            grpc_channel_stack_type channel_stack_type - the archetype
//            of
//                                           channel stack to create
//            variadic arguments - the (in-order) expected list of
//            channel
//                                 filters to instantiate, terminated
//                                 with NULL
...
int main(int argc, char** argv) {
  ...
  errors += CHECK_STACK("unknown", &minimal_stack_args,
                        GRPC_CLIENT_DIRECT_CHANNEL, "authority",
                        "connected", NULL);
  errors += CHECK_STACK("unknown", &minimal_stack_args,
                        GRPC_CLIENT_SUBCHANNEL, "authority",
                        "connected", NULL);
  errors +=
      CHECK_STACK("unknown", &minimal_stack_args, GRPC_SERVER_CHANNEL,
                  "server", "connected", NULL);
  errors += CHECK_STACK("chttp2", &minimal_stack_args,
                        GRPC_CLIENT_DIRECT_CHANNEL, "authority",
                        "http-client", "message_decompress",
                        "connected", NULL);
  errors +=
      CHECK_STACK("chttp2", &minimal_stack_args, GRPC_CLIENT_SUBCHANNEL,
                  ...
                  "connected", NULL);
  errors += CHECK_STACK("chttp2", &minimal_stack_args,
                        GRPC_SERVER_CHANNEL, "server", "http-server",
                        "message_decompress", "connected", NULL);
  errors += CHECK_STACK(nullptr, &minimal_stack_args,
                        GRPC_CLIENT_CHANNEL, "client-channel", NULL);
  ...
  errors +=
      CHECK_STACK("unknown", nullptr, GRPC_SERVER_CHANNEL, "server",
  ...
  errors += CHECK_STACK("chttp2", nullptr, GRPC_CLIENT_SUBCHANNEL,
                        "authority", "message_size", "http-client",
                        "message_decompress", "message_compress",
                        "connected", NULL);
  errors += CHECK_STACK("chttp2", nullptr, GRPC_SERVER_CHANNEL,
                        "server", "message_size", "deadline",
                        "http-server", "message_decompress",
                        "message_compress", "connected", NULL);
...
static int check_stack(const char* file, int line,
  ...
  if (got != expect) {
      ...
      switch (channel_args->args[i].type) {
        ...
        case GRPC_ARG_POINTER: {
          value = absl::StrFormat(
              "%p", channel_args->args[i].value.pointer.p);
          break;
        }
      }
      parts.push_back(
          absl::StrCat(channel_args->args[i].key, "=", value));
    }
    std::string args_str =
    ...
    gpr_log(
        ...
        args_str.c_str());
    gpr_log(file, line, GPR_LOG_SEVERITY_ERROR, "EXPECTED: %s",
            expect.c_str());
    gpr_log(file, line, GPR_LOG_SEVERITY_ERROR, "GOT:      %s",
            got.c_str());
    result = 1;
  }
