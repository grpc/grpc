...
size_t GetSizeofTraceEvent() {
  return sizeof(ChannelTrace::TraceEvent);
}
...
void ValidateChannelTraceData(const Json& json,
  ...
  size_t num_events_logged = static_cast<size_t>(strtol(
      num_events_logged_json.string_value().c_str(), nullptr, 0));
...
void ValidateChannelTraceCustom(ChannelTrace* tracer,
  ...
  grpc::testing::ValidateChannelTraceProtoJsonTranslation(
      json_str.c_str());
  ValidateChannelTraceData(json, num_events_logged,
                           num_events_expected);
}

void ValidateChannelTrace(ChannelTrace* tracer,
                          size_t num_events_logged) {
  ValidateChannelTraceCustom(tracer, num_events_logged,
                             num_events_logged);
}
...
// Tests basic ChannelTrace functionality like construction, adding
// trace, and lookups by uuid.
...
// subchannles. This exercises the ref/unref patterns since the parent
// tracer and this function will both hold refs to the subchannel.
TEST(ChannelTracerTest, ComplexTest) {
  ...
  RefCountedPtr<ChannelNode> sc1 = MakeRefCounted<ChannelNode>(
      "fake_target", kEventListMemoryLimit, 0);
...
// subchannels have connections. Ensures that everything lives as long
// as it should then gets deleted.
...
TEST(ChannelTracerTest, TestSmallMemoryLimit) {
  grpc_core::ExecCtx exec_ctx;
  // doesn't make sense, but serves a testing purpose for the channel
  // tracing bookkeeping. All tracing events added should will get
...
TEST(ChannelTracerTest, TestEviction) {
  ...
  // at this point the list is full, and each subsequent enntry will
  // cause an eviction.
  for (int i = 1; i <= kNumEvents; ++i) {
    AddSimpleTrace(&tracer);
...
TEST(ChannelTracerTest, TestMultipleEviction) {
  ...
  // at this point the list is full, and each subsequent enntry will
  // cause an eviction. We will now add in a trace event that has a
  // copied string. This uses more memory, so it will cause a double
  // eviciction
  tracer.AddTraceEvent(
      ChannelTrace::Severity::Info,
...
TEST(ChannelTracerTest, TestTotalEviction) {
  ...
  grpc_slice huge_slice =
      grpc_slice_malloc(kTraceEventSize * (kNumEvents + 1));
