...
TEST(ZTraceCollectorTest, SingleTraceWorks) {
  ...
  ztrace->Run({{"memory_cap", std::to_string(1024 * 1024 * 1024)},
               {"test_arg", "test_value"}},
...
TEST(ZTraceCollectorTest, MultipleTracesWork) {
  ...
  ztrace1->Run({{"memory_cap", std::to_string(1024 * 1024 * 1024)},
                {"test_arg", "test_value"}},
               grpc_event_engine::experimental::GetDefaultEventEngine(),
  ...
  ztrace2->Run({{"memory_cap", std::to_string(1024 * 1024 * 1024)},
                {"test_arg", "test_value"}},
               grpc_event_engine::experimental::GetDefaultEventEngine(),
...
TEST(ZTraceCollectorTest, EarlyTerminationWorks) {
  ...
  std::vector<std::string> result;
  auto ztrace = collector.MakeZTrace();
  ztrace->Run({{"test_arg", "test_value"}},
              grpc_event_engine::experimental::GetDefaultEventEngine(),
              [&n, &result](absl::StatusOr<std::optional<std::string>> r) {
                if (!r.ok()) {
                  n.Notify();
...
struct ExhaustionResult {
  ExhaustionResult(std::unique_ptr<ZTrace> ztrace)
...
TEST(ZTraceCollectorTest, ExhaustionTest) {
  ...
  for (size_t i = 0; i < 10000; i++) {
    ...
    CHECK(r->result.ok());
    r->ztrace->Run({{"test_arg", "test_value"}},
                   grpc_event_engine::experimental::GetDefaultEventEngine(),
