...
secure_endpoint_create_fixture_tcp_socketpair (size_t slice_size,
					       ...
					       bool use_zero_copy_protector)
{
  ...
  tsi_zero_copy_grpc_protector *fake_read_zero_copy_protector =
    ...
    ? tsi_create_fake_zero_copy_grpc_protector (nullptr) : nullptr;
  ...
  if (leftover_nslices == 0)
	  ...
	  while (message_size > 0)
	    {
	      ...
	      result =
		tsi_frame_protector_protect (fake_write_protector,
					     message_bytes,
      ...
      do
	{
	  size_t protected_buffer_size_to_send = buffer_size;
	  result =
	    tsi_frame_protector_protect_flush (fake_write_protector, cur,
					       &protected_buffer_size_to_send,
					       &still_pending_size);
	  GPR_ASSERT (result == TSI_OK);
	  cur += protected_buffer_size_to_send;
      ...
      encrypted_leftover = grpc_slice_from_copied_buffer ((const char *)
							  encrypted_buffer,
							  total_buffer_size -
							  buffer_size);
      f.client_ep =
	grpc_secure_endpoint_create (fake_read_protector,
				     fake_read_zero_copy_protector,
...
{
  return secure_endpoint_create_fixture_tcp_socketpair (slice_size, nullptr,
							0, false);
}

static grpc_endpoint_test_fixture
secure_endpoint_create_fixture_tcp_socketpair_noleftover_zero_copy (size_t
								    slice_size)
{
  return secure_endpoint_create_fixture_tcp_socketpair (slice_size, nullptr,
							0, true);
...
static grpc_endpoint_test_fixture
secure_endpoint_create_fixture_tcp_socketpair_leftover_zero_copy (size_t
								  slice_size)
{
  grpc_slice s =
    grpc_slice_from_copied_string ("hello world 12345678900987654321");
...
static void
inc_call_ctr (void *arg, grpc_error * error)
{
  ++*(int *) arg;
}
...
static void
destroy_pollset (void *p, grpc_error * error)
{
