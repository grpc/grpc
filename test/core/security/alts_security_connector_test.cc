...
static void test_invalid_input_failure() {
  grpc_core::RefCountedPtr<grpc_auth_context> ctx = grpc_alts_auth_context_from_tsi_peer(nullptr);
...
static void test_empty_certificate_type_failure() {
  ...
  grpc_core::RefCountedPtr<grpc_auth_context> ctx = grpc_alts_auth_context_from_tsi_peer(&peer);
  GPR_ASSERT(ctx == nullptr);
...
static void test_missing_rpc_protocol_versions_property_failure() {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_ALTS_SERVICE_ACCOUNT_PEER_PROPERTY, "alice", &peer.properties[1]) == TSI_OK);
  grpc_core::RefCountedPtr<grpc_auth_context> ctx = grpc_alts_auth_context_from_tsi_peer(&peer);
  GPR_ASSERT(ctx == nullptr);
  tsi_peer_destruct(&peer);
}
...
static void test_missing_security_level_property_failure() {
  ...
  grpc_gcp_rpc_protocol_versions_set_max(&peer_versions, GRPC_PROTOCOL_VERSION_MAX_MAJOR,
                                         GRPC_PROTOCOL_VERSION_MAX_MINOR);
  grpc_gcp_rpc_protocol_versions_set_min(&peer_versions, GRPC_PROTOCOL_VERSION_MIN_MAJOR,
                                         GRPC_PROTOCOL_VERSION_MIN_MINOR);
  grpc_slice serialized_peer_versions;
  GPR_ASSERT(grpc_gcp_rpc_protocol_versions_encode(&peer_versions, &serialized_peer_versions));

  GPR_ASSERT(tsi_construct_string_peer_property(
                 TSI_ALTS_RPC_VERSIONS,
                 reinterpret_cast<char*>(GRPC_SLICE_START_PTR(serialized_peer_versions)),
                 GRPC_SLICE_LENGTH(serialized_peer_versions), &peer.properties[2]) == TSI_OK);
  grpc_core::RefCountedPtr<grpc_auth_context> ctx = grpc_alts_auth_context_from_tsi_peer(&peer);
  GPR_ASSERT(ctx == nullptr);
...
static void test_unknown_peer_property_failure() {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_CERTIFICATE_TYPE_PEER_PROPERTY,
                                                             TSI_ALTS_CERTIFICATE_TYPE,
                                                             &peer.properties[0]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring("unknown", "alice",
...
static bool test_identity(const grpc_auth_context* ctx, const char* expected_property_name,
  ...
  if (strncmp(prop->value, expected_identity, prop->value_length) != 0) {
    gpr_log(GPR_ERROR, "Expected peer identity %s and got got %s.", expected_identity, prop->value);
    return false;
  }
  return true;
}
...
static void test_alts_peer_to_auth_context_success() {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property(
                 ...
                 GRPC_SLICE_LENGTH(serialized_alts_ctx), &peer.properties[4]) == TSI_OK);
  grpc_core::RefCountedPtr<grpc_auth_context> ctx = grpc_alts_auth_context_from_tsi_peer(&peer);
  GPR_ASSERT(ctx != nullptr);
  GPR_ASSERT(test_identity(ctx.get(), TSI_ALTS_SERVICE_ACCOUNT_PEER_PROPERTY, "alice"));
