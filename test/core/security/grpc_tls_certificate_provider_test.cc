...
class GrpcTlsCertificateProviderTest : public ::testing::Test {
 protected:
  ...
  // CredentialInfo contains the parameters when calling
  // OnCertificatesChanged of a watcher. When OnCertificatesChanged is
  // invoked, we will push a CredentialInfo to the cert_update_queue of
  // state_, and check in each test if the status updates are correct.
  struct CredentialInfo {
    std::string root_certs;
    PemKeyCertPairList key_cert_pairs;
    CredentialInfo(std::string root, PemKeyCertPairList key_cert)
        : root_certs(std::move(root)),
  ...
  // ErrorInfo contains the parameters when calling OnError of a
  // watcher. When OnError is invoked, we will push a ErrorInfo to the
  // error_queue of state_, and check in each test if the status updates
  // are correct.
  struct ErrorInfo {
  ...
  struct WatcherState {
    ...
    std::deque<CredentialInfo> GetCredentialQueue() {
      // We move the data member value so the data member will be
      // re-initiated with size 0, and ready for the next check.
      MutexLock lock(&mu);
   ...
   public:
    ...
    void OnCertificatesChanged(
      ...
      state_->cert_update_queue.emplace_back(
          std::move(updated_root), std::move(updated_identity));
    }
    ...
    void OnError(grpc_error_handle root_cert_error,
      ...
      if (root_cert_error != GRPC_ERROR_NONE) {
        ...
        GPR_ASSERT(grpc_error_get_str(root_cert_error,
                                      GRPC_ERROR_STR_DESCRIPTION,
                                      &root_error_slice));
        root_error_str =
            std::string(StringViewFromSlice(root_error_slice));
      }
  ...
  WatcherState* MakeWatcher(
    ...
    // TlsCertificatesTestWatcher ctor takes a pointer to the
    // WatcherState. It sets WatcherState::watcher to point to itself.
    // The TlsCertificatesTestWatcher dtor will set
    // WatcherState::watcher back to nullptr to indicate that it's been
    // destroyed.
  ...
  // Use a std::list<> here to avoid the address invalidation caused by
  // internal reallocation of std::vector<>.
...
TEST_F(GrpcTlsCertificateProviderTest,
       StaticDataCertificateProviderCreation) {
  ...
  EXPECT_THAT(watcher_state_3->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  "", MakeCertKeyPairs(private_key_.c_str(),
                                       cert_chain_.c_str()))));
...
// The following tests write credential data to temporary files to test
// the transition behavior of the provider.
TEST_F(GrpcTlsCertificateProviderTest,
  ...
  // Wait 2 seconds for the provider's refresh thread to read the
  // updated files.
...
TEST_F(
  ...
  WatcherState* watcher_state_1 =
      MakeWatcher(provider.distributor(), kCertName, kCertName);
  // The initial data is all good, so we expect to have successful
  // credential updates.
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  root_cert_, MakeCertKeyPairs(private_key_.c_str(),
  ...
  // We have no ideas on how many errors we will receive, so we only
  // check once.
  EXPECT_THAT(
      watcher_state_1->GetErrorQueue(),
...
TEST_F(
  ...
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  root_cert_, MakeCertKeyPairs(private_key_.c_str(),
                                               cert_chain_.c_str()))));
  // Delete root TmpFile object, which will remove the corresponding
  // file.
  tmp_root_cert.reset();
  // Wait 2 seconds for the provider's refresh thread to read the
  // deleted files.
...
TEST_F(
  ...
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  root_cert_, MakeCertKeyPairs(private_key_.c_str(),
                                               cert_chain_.c_str()))));
  // Delete identity TmpFile objects, which will remove the
  // corresponding files.
  tmp_identity_key.reset();
  tmp_identity_cert.reset();
  // Wait 2 seconds for the provider's refresh thread to read the
...
TEST_F(GrpcTlsCertificateProviderTest,
       FailedKeyCertMatchOnEmptyPrivateKey) {
...
TEST_F(GrpcTlsCertificateProviderTest,
       FailedKeyCertMatchOnEmptyCertificate) {
  absl::StatusOr<bool> status =
...
TEST_F(GrpcTlsCertificateProviderTest,
       FailedKeyCertMatchOnInvalidCertFormat) {
  absl::StatusOr<bool> status = PrivateKeyAndCertificateMatch(
      private_key_2_, "invalid_certificate");
  EXPECT_FALSE(status.ok());
  EXPECT_EQ(status.status().code(), absl::StatusCode::kInvalidArgument);
...
TEST_F(GrpcTlsCertificateProviderTest,
  ...
  absl::StatusOr<bool> status = PrivateKeyAndCertificateMatch(
      "invalid_private_key", cert_chain_2_);
  EXPECT_EQ(status.status().code(), absl::StatusCode::kInvalidArgument);
  EXPECT_EQ(status.status().message(),
...
TEST_F(GrpcTlsCertificateProviderTest,
       FailedKeyCertMatchOnInvalidPair) {
  absl::StatusOr<bool> status =
