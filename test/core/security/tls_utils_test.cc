...
TEST(XdsSanMatchingTest, EmptySansList) {
  ...
  EXPECT_FALSE(VerifySubjectAlternativeNames(
...
TEST(XdsSanMatchingTest, EmptyMatchersList) {
  ...
  EXPECT_TRUE(VerifySubjectAlternativeNames(sans.data(), sans.size(), {}));
}
...
TEST(XdsSanMatchingTest, ExactMatchDns) {
  ...
  EXPECT_TRUE(VerifySubjectAlternativeNames(sans.data(), sans.size(),
                                            {"a.example.com"}));
  EXPECT_FALSE(VerifySubjectAlternativeNames(sans.data(), sans.size(),
                                             {"b.example.com"}));
  sans = {"b.example.com."};
  EXPECT_FALSE(VerifySubjectAlternativeNames(sans.data(), sans.size(),
                                             {"a.example.com."}));
...
TEST(XdsSanMatchingTest, ExactMatchMultipleSansMultipleMatchers) {
  std::vector<const char*> sans = {"a.example.com", "foo.example.com",
                                   "b.example.com"};
  EXPECT_TRUE(VerifySubjectAlternativeNames(
      sans.data(), sans.size(),
...
TEST(XdsSanMatchingTest, WildCardMatch) {
  ...
  EXPECT_FALSE(VerifySubjectAlternativeNames(sans.data(), sans.size(),
                                             {".example.com"}));
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"example.com"}));
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"foo.bar.com"}));
}

TEST(XdsSanMatchingTest, WildCardDoesNotMatchSingleLabelDomain) {
  std::vector<const char*> sans = {"*"};
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"abc"}));
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"abc.com."}));
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"bar.baz.com"}));
  sans = {"*."};
  EXPECT_FALSE(
      VerifySubjectAlternativeNames(sans.data(), sans.size(), {"abc"}));
  EXPECT_FALSE(
