...
static grpc_call_credentials* create_sts_creds(
  ...
  if (strlen(json_file_path) == 0) {
    ...
    auto status = grpc::experimental::StsCredentialsOptionsFromJson(
        reinterpret_cast<const char*>(
            GRPC_SLICE_START_PTR(sts_options_slice)),
        &options);
...
int main(int argc, char** argv) {
  ...
  gpr_cmdline_add_string(cl, "json_sts_options",
                         "File path of the json sts options. If the "
                         "path is empty, the program "
                         "will attempt to use the $STS_CREDENTIALS "
  ...
  if (json_sts_options_file_path != nullptr &&
    ...
    gpr_log(GPR_ERROR,
            "--json_sts_options and --json_refresh_token are mutually "
            "exclusive.");
    exit(1);
  }
  ...
  if (use_gce) {
    if (json_sts_options_file_path != nullptr ||
        json_refresh_token_file_path != nullptr) {
      gpr_log(GPR_INFO,
              "Ignoring json refresh token or sts options to get a "
              "token from "
    ...
    gpr_log(GPR_ERROR,
            "Missing --gce, --json_sts_options, or "
            "--json_refresh_token option.");
    exit(1);
  }
