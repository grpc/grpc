...
static grpc_call_credentials* create_sts_creds(const char* json_file_path) {
  ...
  if (strlen(json_file_path) == 0) {
    ...
    GPR_ASSERT(
        GRPC_LOG_IF_ERROR("load_file", grpc_load_file(json_file_path, 1, &sts_options_slice)));
    auto status = grpc::experimental::StsCredentialsOptionsFromJson(
        reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(sts_options_slice)), &options);
  ...
  grpc_sts_credentials_options opts = grpc::experimental::StsCredentialsCppToCoreOptions(options);
...
int main(int argc, char** argv) {
  ...
  gpr_cmdline_add_string(cl, "json_refresh_token", "File path of the json refresh token.",
  ...
  gpr_cmdline_add_flag(cl, "gce", "Get a token from the GCE metadata server (only works in GCE).",
                       &use_gce);
  gpr_cmdline_parse(cl, argc, argv);

  grpc_init();

  if (json_sts_options_file_path != nullptr && json_refresh_token_file_path != nullptr) {
    gpr_log(GPR_ERROR, "--json_sts_options and --json_refresh_token are mutually exclusive.");
    exit(1);
  }
  ...
  if (use_gce) {
    ...
    gpr_log(GPR_ERROR, "Missing --gce, --json_sts_options, or --json_refresh_token option.");
    exit(1);
  }
