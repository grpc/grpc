...
void
create_jwt (const char *json_key_file_path, const char *service_url,
	    const char *scope)
{
  ...
  key =
    grpc_auth_json_key_create_from_string ((const char *)
  ...
  jwt =
    grpc_jwt_encode_and_sign (&key,
			      service_url ==
			      nullptr ? GRPC_JWT_OAUTH2_AUDIENCE :
			      service_url, grpc_max_auth_token_lifetime (),
			      scope);
...
main (int argc, char **argv)
{
  ...
  gpr_cmdline_add_string (cl, "scope",
			  ...
			  "exclusive with service_url", &scope);
