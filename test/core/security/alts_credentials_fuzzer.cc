...
static void read_target_service_accounts(input_stream* inp,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (service_account != nullptr) {
      grpc_alts_credentials_client_options_add_target_service_account(options, service_account);
      gpr_free(service_account);
    }
  }
  // Added to improve code coverage.
  grpc_alts_credentials_client_options_add_target_service_account(options, nullptr);
  grpc_alts_credentials_client_options_add_target_service_account(nullptr,
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  while (inp.cur != inp.end) {
    ...
    char* handshaker_service_url = grpc_fuzzer_get_next_byte(&inp) & 0x01
    ...
    if (grpc_fuzzer_get_next_byte(&inp) & 0x01) {
      ...
      grpc_alts_credentials_options* options = grpc_alts_credentials_server_options_create();
      grpc_server_credentials* cred = grpc_alts_server_credentials_create_customized(
          options, handshaker_service_url, enable_untrusted_alts);
      if (!enable_untrusted_alts && !is_on_gcp) {
        GPR_ASSERT(cred == nullptr);
      } else {
