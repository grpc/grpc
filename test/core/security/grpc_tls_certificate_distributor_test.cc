...
class GrpcTlsCertificateDistributorTest : public ::testing::Test {
 protected:
  ...
  // CallbackStatus contains the parameters when calling
  // watch_status_callback_ of the distributor. When a particular
  // callback is invoked, we will push a CallbackStatus to a
  // callback_queue_, and check in each test if the status updates are
  // correct.
  struct CallbackStatus {
    std::string cert_name;
    bool root_being_watched;
    bool identity_being_watched;
    CallbackStatus(std::string name, bool root_watched,
                   bool identity_watched)
  ...
  void SetUp() override {
    distributor_.SetWatchStatusCallback(
        [this](std::string cert_name, bool root_being_watched,
               bool identity_being_watched) {
          callback_queue_.emplace_back(std::move(cert_name),
                                       root_being_watched,
...
TEST_F(GrpcTlsCertificateDistributorTest, UpdateCredentialsOnAnySide) {
  ...
  // SetKeyMaterials should trigger watcher's OnCertificatesChanged
  // method.
  distributor_.SetKeyMaterials(
  ...
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  ...
                  MakeCertKeyPairs(kIdentityCert1PrivateKey,
                                   kIdentityCert1Contents))));
  // Set root certs should trigger watcher's OnCertificatesChanged
  // again.
  distributor_.SetKeyMaterials(kCertName1, kRootCert2Contents,
                               absl::nullopt);
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  kRootCert2Contents,
                  MakeCertKeyPairs(kIdentityCert1PrivateKey,
                                   kIdentityCert1Contents))));
  // Set identity certs should trigger watcher's OnCertificatesChanged
  // again.
  distributor_.SetKeyMaterials(
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  // Push credential updates to kRootCert1Name and check if the status
  // works as expected.
  distributor_.SetKeyMaterials(kRootCert1Name, kRootCert1Contents,
                               absl::nullopt);
  ...
  // Push credential updates to kIdentityCert1Name and check if the
  ...
  EXPECT_THAT(GetCallbackQueue(),
              ::testing::ElementsAre(
                  CallbackStatus(kRootCert2Name, false, false),
                  CallbackStatus(kIdentityCert1Name, false, false)));
}

TEST_F(GrpcTlsCertificateDistributorTest,
       SameRootNameDiffIdentityName) {
  // Register watcher 1.
  WatcherState* watcher_state_1 =
      MakeWatcher(kRootCert1Name, kIdentityCert1Name);
  EXPECT_THAT(GetCallbackQueue(),
  ...
  EXPECT_THAT(GetCallbackQueue(),
              ::testing::ElementsAre(
                  CallbackStatus(kIdentityCert2Name, false, true)));
  // Push credential updates to kRootCert1Name and check if the status
  // works as expected.
  distributor_.SetKeyMaterials(kRootCert1Name, kRootCert1Contents,
                               absl::nullopt);
  ...
  EXPECT_THAT(GetCallbackQueue(),
              ::testing::ElementsAre(
                  CallbackStatus(kRootCert1Name, false, false),
                  CallbackStatus(kIdentityCert2Name, false, false)));
}

TEST_F(GrpcTlsCertificateDistributorTest,
       AddAndCancelFirstWatcherForSameRootAndIdentityCertName) {
  // Register watcher 1 watching kCertName1 for both root and identity
  // certs.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  EXPECT_THAT(GetCallbackQueue(), ::testing::ElementsAre(CallbackStatus(
                                      kCertName1, true, true)));
  // Push credential updates to kCertName1 and check if the status works
  // as expected.
  distributor_.SetKeyMaterials(
      kCertName1, kRootCert1Contents,
      MakeCertKeyPairs(kIdentityCert1PrivateKey,
...
TEST_F(
  ...
  WatcherState* watcher_state_1 =
      MakeWatcher(kCertName1, absl::nullopt);
  EXPECT_THAT(GetCallbackQueue(), ::testing::ElementsAre(CallbackStatus(
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  // Register watcher 1 and watcher 2 watching kCertName1 for root and
  // identity certs.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  EXPECT_THAT(GetCallbackQueue(), ::testing::ElementsAre(CallbackStatus(
                                      kCertName1, true, true)));
...
TEST_F(GrpcTlsCertificateDistributorTest, SetKeyMaterialsInCallback) {
  ...
  // Start 1000 threads that will register a watcher to a new cert name,
  // verify the key materials being set, and then cancel the watcher, to
  // make sure the lock mechanism in the distributor is safe.
...
TEST_F(GrpcTlsCertificateDistributorTest,
       WatchACertInfoWithValidCredentials) {
  // Push credential updates to kCertName1.
  distributor_.SetKeyMaterials(
      kCertName1, kRootCert1Contents,
      MakeCertKeyPairs(kIdentityCert1PrivateKey,
  ...
  // Register watcher 2.
  WatcherState* watcher_state_2 =
      MakeWatcher(kRootCert2Name, absl::nullopt);
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  // Register watcher 1.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  // Calling SetErrorForCert on both cert names should only call one
  // OnError on watcher 1.
  ...
  EXPECT_THAT(watcher_state_1->GetErrorQueue(),
              ::testing::ElementsAre(
                  ErrorInfo(kErrorMessage, kIdentityErrorMessage)));
...
TEST_F(GrpcTlsCertificateDistributorTest,
       SetErrorForCertForRootOrIdentity) {
  // Register watcher 1.
  WatcherState* watcher_state_1 =
      MakeWatcher(kCertName1, absl::nullopt);
  ...
  EXPECT_THAT(watcher_state_1->GetErrorQueue(),
              ::testing::ElementsAre());
  // Calling SetErrorForCert on both names should still get one OnError
  // call.
  distributor_.SetErrorForCert(
      kCertName1,
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kRootErrorMessage),
  ...
  WatcherState* watcher_state_2 =
      MakeWatcher(absl::nullopt, kCertName1);
  // Calling SetErrorForCert on identity name should only call one
  // OnError on watcher 2.
  distributor_.SetErrorForCert(
      kCertName1, absl::nullopt,
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kIdentityErrorMessage));
  EXPECT_THAT(
      watcher_state_2->GetErrorQueue(),
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  distributor_.SetErrorForCert(
      ...
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kRootErrorMessage),
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kIdentityErrorMessage));
  // Register watcher 1 for kCertName1 as root and kCertName2 as
  // identity.
  ...
  EXPECT_THAT(watcher_state_1->GetErrorQueue(),
              ::testing::ElementsAre(ErrorInfo(kRootErrorMessage, "")));
  // Calling SetErrorForCert on kCertName2 should trigger OnError with
  // both errors, because kCertName1 also has error.
  distributor_.SetErrorForCert(
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  distributor_.SetErrorForCert(
      ...
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kRootErrorMessage),
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kIdentityErrorMessage));
  // Register watcher 1 for kCertName2 as root and kCertName1 as
  // identity.
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  EXPECT_THAT(
      ...
      ::testing::ElementsAre(ErrorInfo("", kIdentityErrorMessage)));
  CancelWatch(watcher_state_1);
  // Register watcher 2 for kCertName2 as identity and a non-existing
  // name kRootCert1Name as root.
  WatcherState* watcher_state_2 =
      MakeWatcher(kRootCert1Name, kCertName2);
  // Should not trigger OnError.
  EXPECT_THAT(watcher_state_2->GetErrorQueue(),
              ::testing::ElementsAre());
  // Calling SetErrorForCert on kCertName2 should trigger OnError.
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  WatcherState* watcher_state_2 =
      MakeWatcher(kCertName2, kIdentityCert1Name);
  // Should not trigger OnError.
  EXPECT_THAT(watcher_state_2->GetErrorQueue(),
              ::testing::ElementsAre());
  // Calling SetErrorForCert on kCertName2 should trigger OnError.
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  // Register watcher 1.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  // watcher 1 should receive both the old credentials and the error
  // right away.
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  kRootCert1Contents,
...
TEST_F(GrpcTlsCertificateDistributorTest,
  ...
  // Register watcher 1.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  // watcher 1 should only receive credential updates without any error,
  // because the previous error is wiped out by a successful update.
  EXPECT_THAT(watcher_state_1->GetCredentialQueue(),
              ::testing::ElementsAre(CredentialInfo(
                  kRootCert1Contents,
                  MakeCertKeyPairs(kIdentityCert1PrivateKey,
...
TEST_F(GrpcTlsCertificateDistributorTest,
       WatchCertInfoThenInvokeSetError) {
  // Register watcher 1.
  WatcherState* watcher_state_1 = MakeWatcher(kCertName1, kCertName1);
  // Register watcher 2.
  WatcherState* watcher_state_2 =
      MakeWatcher(kRootCert1Name, absl::nullopt);
  // Register watcher 3.
  WatcherState* watcher_state_3 =
...
TEST_F(GrpcTlsCertificateDistributorTest,
       WatchErroredCertInfoBySetError) {
  // Register watcher 1 watching kCertName1 as root.
  WatcherState* watcher_state_1 =
      MakeWatcher(kCertName1, absl::nullopt);
  // Register watcher 2 watching kCertName2 as identity.
  WatcherState* watcher_state_2 =
      MakeWatcher(absl::nullopt, kCertName2);
  // Call SetError and then cancel all watchers.
  distributor_.SetError(
      GRPC_ERROR_CREATE_FROM_STATIC_STRING(kErrorMessage));
  CancelWatch(watcher_state_1);
  CancelWatch(watcher_state_2);
  // Register watcher 3 watching kCertName1 as root and kCertName2 as
  // identity should not get the error updates.
  WatcherState* watcher_state_3 = MakeWatcher(kCertName1, kCertName2);
  EXPECT_THAT(watcher_state_3->GetErrorQueue(),
              ::testing::ElementsAre());
  CancelWatch(watcher_state_3);
  // Register watcher 4 watching kCertName2 as root and kCertName1 as
  // identity should not get the error updates.
  WatcherState* watcher_state_4 = MakeWatcher(kCertName2, kCertName1);
  EXPECT_THAT(watcher_state_4->GetErrorQueue(),
              ::testing::ElementsAre());
