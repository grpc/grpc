...
static int check_transport_security_type(const grpc_auth_context* ctx) {
  grpc_auth_property_iterator it =
      grpc_auth_context_find_properties_by_name(ctx, GRPC_TRANSPORT_SECURITY_TYPE_PROPERTY_NAME);
...
static int check_peer_property(const tsi_peer* peer, const tsi_peer_property* expected) {
  ...
  for (i = 0; i < peer->property_count; i++) {
    ...
    if ((strcmp(prop->name, expected->name) == 0) &&
        ...
        (memcmp(prop->value.data, expected->value.data, expected->value.length) == 0)) {
      return 1;
    }
  }
  return 0; /* Not found... */
...
static int check_ssl_peer_equivalence(const tsi_peer* original, const tsi_peer* reconstructed) {
  ...
  for (i = 0; i < original->property_count; i++) {
    ...
    if ((strcmp(prop->name, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY) == 0) ||
        (strcmp(prop->name, TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY) == 0) ||
...
static void test_check_security_level() {
  GPR_ASSERT(grpc_check_security_level(GRPC_PRIVACY_AND_INTEGRITY, GRPC_PRIVACY_AND_INTEGRITY) ==
             true);
  GPR_ASSERT(grpc_check_security_level(GRPC_PRIVACY_AND_INTEGRITY, GRPC_INTEGRITY_ONLY) == true);
  GPR_ASSERT(grpc_check_security_level(GRPC_PRIVACY_AND_INTEGRITY, GRPC_SECURITY_NONE) == true);
  GPR_ASSERT(grpc_check_security_level(GRPC_INTEGRITY_ONLY, GRPC_PRIVACY_AND_INTEGRITY) == false);
  GPR_ASSERT(grpc_check_security_level(GRPC_INTEGRITY_ONLY, GRPC_INTEGRITY_ONLY) == true);
  GPR_ASSERT(grpc_check_security_level(GRPC_INTEGRITY_ONLY, GRPC_SECURITY_NONE) == true);
  GPR_ASSERT(grpc_check_security_level(GRPC_SECURITY_NONE, GRPC_PRIVACY_AND_INTEGRITY) == false);
  GPR_ASSERT(grpc_check_security_level(GRPC_SECURITY_NONE, GRPC_INTEGRITY_ONLY) == false);
  GPR_ASSERT(grpc_check_security_level(GRPC_SECURITY_NONE, GRPC_SECURITY_NONE) == true);
}
...
static void test_unauthenticated_ssl_peer(void) {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_CERTIFICATE_TYPE_PEER_PROPERTY,
...
static int check_identity(const grpc_auth_context* ctx, const char* expected_property_name,
  ...
  for (i = 0; i < num_identities; i++) {
    ...
    if (strncmp(prop->value, expected_identities[i], prop->value_length) != 0) {
      gpr_log(GPR_ERROR, "Expected peer identity %s and got %s.", expected_identities[i],
              prop->value);
      return 0;
    }
  }
...
static int check_x509_cn(const grpc_auth_context* ctx, const char* expected_cn) {
  grpc_auth_property_iterator it =
      grpc_auth_context_find_properties_by_name(ctx, GRPC_X509_CN_PROPERTY_NAME);
  const grpc_auth_property* prop = grpc_auth_property_iterator_next(&it);
  if (prop == nullptr) {
...
static int check_x509_pem_cert(const grpc_auth_context* ctx, const char* expected_pem_cert) {
  grpc_auth_property_iterator it =
  ...
  if (strncmp(prop->value, expected_pem_cert, prop->value_length) != 0) {
    gpr_log(GPR_ERROR, "Expected pem cert %s and got %s", expected_pem_cert, prop->value);
    return 0;
  }
  if (grpc_auth_property_iterator_next(&it) != nullptr) {
...
static int check_x509_pem_cert_chain(const grpc_auth_context* ctx,
  ...
  grpc_auth_property_iterator it =
      grpc_auth_context_find_properties_by_name(ctx, GRPC_X509_PEM_CERT_CHAIN_PROPERTY_NAME);
  const grpc_auth_property* prop = grpc_auth_property_iterator_next(&it);
  if (prop == nullptr) {
  ...
  if (strncmp(prop->value, expected_pem_cert_chain, prop->value_length) != 0) {
    gpr_log(GPR_ERROR, "Expected pem cert chain %s and got %s", expected_pem_cert_chain,
            prop->value);
    return 0;
  }
  if (grpc_auth_property_iterator_next(&it) != nullptr) {
...
static int check_sans(const grpc_auth_context* ctx, const char* expected_property_name,
  ...
  for (const auto& property_value : expected_property_values) {
    const grpc_auth_property* prop = grpc_auth_property_iterator_next(&it);
    if (prop == nullptr) {
      gpr_log(GPR_ERROR, "Expected value %s not found.", property_value.c_str());
      return 0;
    }
    if (strncmp(prop->value, property_value.c_str(), prop->value_length) != 0) {
      gpr_log(GPR_ERROR, "Expected peer %s and got %s.", property_value.c_str(), prop->value);
      return 0;
    }
  }
  if (grpc_auth_property_iterator_next(&it) != nullptr) {
    gpr_log(GPR_ERROR, "Expected only %zu property values.", expected_property_values.size());
    return 0;
  }
  return 1;
}

static int check_spiffe_id(const grpc_auth_context* ctx, const char* expected_spiffe_id,
                           bool expect_spiffe_id) {
  grpc_auth_property_iterator it =
      grpc_auth_context_find_properties_by_name(ctx, GRPC_PEER_SPIFFE_ID_PROPERTY_NAME);
  const grpc_auth_property* prop = grpc_auth_property_iterator_next(&it);
  ...
  if (strncmp(prop->value, expected_spiffe_id, prop->value_length) != 0) {
    gpr_log(GPR_ERROR, "Expected SPIFFE ID %s but got %s.", expected_spiffe_id, prop->value);
    return 0;
  }
  if (grpc_auth_property_iterator_next(&it) != nullptr) {
...
static void test_cn_only_ssl_peer_to_auth_context(void) {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_CERTIFICATE_TYPE_PEER_PROPERTY,
                                                             ...
                                                             &peer.properties[0]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY, expected_cn, &peer.properties[1]) ==
             TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_PEM_CERT_PROPERTY, expected_pem_cert, &peer.properties[2]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_SECURITY_LEVEL_PEER_PROPERTY,
                 tsi_security_level_to_string(TSI_PRIVACY_AND_INTEGRITY),
                 &peer.properties[3]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_PEM_CERT_CHAIN_PROPERTY,
  ...
  GPR_ASSERT(ctx != nullptr);
  GPR_ASSERT(grpc_auth_context_peer_is_authenticated(ctx.get()));
  GPR_ASSERT(check_identity(ctx.get(), GRPC_X509_CN_PROPERTY_NAME, &expected_cn, 1));
  GPR_ASSERT(check_transport_security_type(ctx.get()));
...
static void test_cn_and_one_san_ssl_peer_to_auth_context(void) {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_PEM_CERT_PROPERTY, expected_pem_cert, &peer.properties[3]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_SECURITY_LEVEL_PEER_PROPERTY,
                 tsi_security_level_to_string(TSI_PRIVACY_AND_INTEGRITY),
                 &peer.properties[4]) == TSI_OK);
  ...
  GPR_ASSERT(ctx != nullptr);
  GPR_ASSERT(grpc_auth_context_peer_is_authenticated(ctx.get()));
  GPR_ASSERT(check_identity(ctx.get(), GRPC_X509_SAN_PROPERTY_NAME, &expected_san, 1));
  GPR_ASSERT(check_transport_security_type(ctx.get()));
  GPR_ASSERT(check_x509_cn(ctx.get(), expected_cn));
  GPR_ASSERT(check_x509_pem_cert(ctx.get(), expected_pem_cert));
  GPR_ASSERT(check_x509_pem_cert_chain(ctx.get(), expected_pem_cert_chain));
...
static void test_cn_and_multiple_sans_ssl_peer_to_auth_context(void) {
  ...
  const char* expected_pem_cert_chain = "pem_cert1_chain";
  size_t i;
  GPR_ASSERT(tsi_construct_peer(5 + GPR_ARRAY_SIZE(expected_sans), &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_CERTIFICATE_TYPE_PEER_PROPERTY,
                                                             TSI_X509_CERTIFICATE_TYPE,
                                                             &peer.properties[0]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
  ...
  for (i = 0; i < GPR_ARRAY_SIZE(expected_sans); i++) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                   TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY, expected_sans[i],
  ...
  GPR_ASSERT(ctx != nullptr);
  GPR_ASSERT(grpc_auth_context_peer_is_authenticated(ctx.get()));
  GPR_ASSERT(check_identity(ctx.get(), GRPC_X509_SAN_PROPERTY_NAME, expected_sans,
...
static void test_cn_and_multiple_sans_and_others_ssl_peer_to_auth_context(void) {
  ...
  const char* expected_sans[] = {"san1", "san2", "san3"};
  size_t i;
  GPR_ASSERT(tsi_construct_peer(7 + GPR_ARRAY_SIZE(expected_sans), &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_CERTIFICATE_TYPE_PEER_PROPERTY,
                                                             TSI_X509_CERTIFICATE_TYPE,
                                                             &peer.properties[0]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring("foo", "bar", &peer.properties[1]) ==
             TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY, expected_cn, &peer.properties[2]) ==
             TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring("chapi", "chapo",
                                                             &peer.properties[3]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_PEM_CERT_PROPERTY, expected_pem_cert, &peer.properties[4]) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_SECURITY_LEVEL_PEER_PROPERTY,
                 tsi_security_level_to_string(TSI_PRIVACY_AND_INTEGRITY),
                 &peer.properties[5]) == TSI_OK);
...
static void test_dns_peer_to_auth_context(void) {
  ...
  for (size_t i = 0; i < expected_dns.size(); ++i) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_DNS_PEER_PROPERTY,
...
static void test_uri_peer_to_auth_context(void) {
  ...
  for (size_t i = 0; i < expected_uri.size(); ++i) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_URI_PEER_PROPERTY,
                                                               expected_uri[i].c_str(),
                                                               &peer.properties[i]) == TSI_OK);
  }
  grpc_core::RefCountedPtr<grpc_auth_context> ctx =
      grpc_ssl_peer_to_auth_context(&peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
...
static void test_email_peer_to_auth_context(void) {
  ...
  for (size_t i = 0; i < expected_emails.size(); ++i) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_EMAIL_PEER_PROPERTY,
                                                               expected_emails[i].c_str(),
                                                               &peer.properties[i]) == TSI_OK);
  }
  grpc_core::RefCountedPtr<grpc_auth_context> ctx =
      grpc_ssl_peer_to_auth_context(&peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
  GPR_ASSERT(ctx != nullptr);
  GPR_ASSERT(check_sans(ctx.get(), GRPC_PEER_EMAIL_PROPERTY_NAME, expected_emails));
  tsi_peer_destruct(&peer);
  ctx.reset(DEBUG_LOCATION, "test");
}
...
static void test_ip_peer_to_auth_context(void) {
  tsi_peer peer;
  const std::vector<std::string> expected_ips = {"128.128.128.128", "255.255.255.255"};
  GPR_ASSERT(tsi_construct_peer(expected_ips.size(), &peer) == TSI_OK);
  for (size_t i = 0; i < expected_ips.size(); ++i) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_IP_PEER_PROPERTY,
                                                               expected_ips[i].c_str(),
                                                               &peer.properties[i]) == TSI_OK);
  }
  grpc_core::RefCountedPtr<grpc_auth_context> ctx =
      grpc_ssl_peer_to_auth_context(&peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
...
static void test_spiffe_id_peer_to_auth_context(void) {
  ...
  size_t i;
  GPR_ASSERT(tsi_construct_peer(invalid_spiffe_id.size(), &invalid_peer) == TSI_OK);
  ...
  grpc_core::RefCountedPtr<grpc_auth_context> invalid_ctx =
      grpc_ssl_peer_to_auth_context(&invalid_peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(TSI_X509_URI_PEER_PROPERTY,
                                                             valid_spiffe_id.c_str(),
                                                             &valid_peer.properties[0]) == TSI_OK);
  grpc_core::RefCountedPtr<grpc_auth_context> valid_ctx =
      grpc_ssl_peer_to_auth_context(&valid_peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
  ...
  std::vector<std::string> multiple_spiffe_id = {"spiffe://foo.bar.com/wl", "https://xyz",
                                                 "spiffe://foo.bar.com/wl2"};
  GPR_ASSERT(tsi_construct_peer(multiple_spiffe_id.size(), &multiple_peer) == TSI_OK);
  ...
  grpc_core::RefCountedPtr<grpc_auth_context> multiple_ctx =
      grpc_ssl_peer_to_auth_context(&multiple_peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
  ...
  std::vector<std::string> multiple_uri = {"spiffe://foo.bar.com/wl", "https://xyz",
                                           "ssh://foo.bar.com/"};
  GPR_ASSERT(tsi_construct_peer(multiple_uri.size(), &multiple_uri_peer) == TSI_OK);
  for (i = 0; i < multiple_spiffe_id.size(); i++) {
    GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
  ...
  grpc_core::RefCountedPtr<grpc_auth_context> multiple_uri_ctx =
      grpc_ssl_peer_to_auth_context(&multiple_uri_peer, GRPC_SSL_TRANSPORT_SECURITY_TYPE);
...
static grpc_ssl_roots_override_result override_roots_permanent_failure(char** /*pem_root_certs*/) {
...
class TestDefaultSslRootStore : public DefaultSslRootStore {
 public:
  static grpc_slice ComputePemRootCertsForTesting() { return ComputePemRootCerts(); }
};
...
static void test_default_ssl_roots(void) {
  ...
  FILE* roots_env_var_file = gpr_tmpfile("test_roots_for_env_var", &roots_env_var_file_path);
  ...
  grpc_slice roots = grpc_core::TestDefaultSslRootStore::ComputePemRootCertsForTesting();
  ...
  /* Now let's set the env var: We should get the contents pointed value
     instead. */
  GPR_GLOBAL_CONFIG_SET(grpc_default_ssl_roots_file_path, roots_env_var_file_path);
  ...
  const tsi_ssl_root_certs_store* root_store = grpc_core::TestDefaultSslRootStore::GetRootStore();
...
static void test_peer_alpn_check(void) {
  ...
  GPR_ASSERT(tsi_construct_peer(1, &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property("wrong peer property name", alpn, strlen(alpn),
                                                &peer.properties[0]) == TSI_OK);
  ...
  // property value.
  GPR_ASSERT(tsi_construct_peer(1, &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property(TSI_SSL_ALPN_SELECTED_PROTOCOL, wrong_alpn,
                                                strlen(wrong_alpn), &peer.properties[0]) == TSI_OK);
  ...
  // value.
  GPR_ASSERT(tsi_construct_peer(1, &peer) == TSI_OK);
  GPR_ASSERT(tsi_construct_string_peer_property(TSI_SSL_ALPN_SELECTED_PROTOCOL, alpn, strlen(alpn),
                                                &peer.properties[0]) == TSI_OK);
