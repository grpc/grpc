...
static int
check_peer_property (const tsi_peer * peer,
...
static int
check_ssl_peer_equivalence (const tsi_peer * original,
  ...
  for (i = 0; i < original->property_count; i++)
    {
	  ...
	  ||
	  (strcmp
	   (prop->name, TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY) == 0)
	  || (strcmp (prop->name, TSI_X509_PEM_CERT_PROPERTY) == 0))
	{
	  if (!check_peer_property (reconstructed, prop))
	    return 0;
	}
    }
  return 1;
...
static int
check_x509_pem_cert (const grpc_auth_context * ctx,
...
override_roots_success (char **pem_root_certs)
{
...
static grpc_ssl_roots_override_result
override_roots_permanent_failure (char **pem_root_certs)
{
  return GRPC_SSL_ROOTS_OVERRIDE_FAIL_PERMANENTLY;
}

static void
test_default_ssl_roots (void)
{
