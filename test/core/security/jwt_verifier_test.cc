...
/* Trick our JWT library into issuing a JWT with
...
static const char good_jwk_set[] =
    ...
    "   \"n\": "
    "\"4S8myegefIeRdynuYkSqBYaOLDvU19cHKC56RIqGjrkXFoZuydIz1IxACpWTtDas"
    "b4jQ6mxP"
...
static const char good_google_email_keys_part1[] =
    ...
    "CERTIFICATE-----"
    "\\nMIICATCCAWoCCQDEywLhxvHjnDANBgkqhkiG9w0BAQsFADBFMQswCQYDVQQGEwJ"
    "B\\nVTET"
...
static void test_jwt_issuer_email_domain(void) {
  ...
  /* This is not a very good parser but make sure we do not crash on
     these weird inputs. */
...
static void test_claims_success(void) {
  ...
  GPR_ASSERT(
      strcmp(grpc_jwt_claims_audience(claims), "https://foo.com") == 0);
  GPR_ASSERT(strcmp(grpc_jwt_claims_issuer(claims), "blah.foo.com") ==
             0);
  GPR_ASSERT(strcmp(grpc_jwt_claims_subject(claims),
                    "juju@blah.foo.com") == 0);
...
static void test_expired_claims_failure(void) {
  ...
  GPR_ASSERT(gpr_time_cmp(grpc_jwt_claims_issued_at(claims), exp_iat) ==
             0);
  GPR_ASSERT(
      gpr_time_cmp(grpc_jwt_claims_expires_at(claims), exp_exp) == 0);
  GPR_ASSERT(
      gpr_time_cmp(grpc_jwt_claims_not_before(claims), exp_nbf) == 0);
...
static char* json_key_str(const char* last_part) {
  size_t result_len = strlen(json_key_str_part1) +
                      strlen(json_key_str_part2) + strlen(last_part);
...
static void on_verification_success(void* user_data,
  ...
  GPR_ASSERT(
      strcmp(grpc_jwt_claims_audience(claims), expected_audience) == 0);
  grpc_jwt_claims_destroy(claims);
}
...
static void test_jwt_verifier_google_email_issuer_success(void) {
  ...
  char* jwt = nullptr;
  char* key_str =
      json_key_str(json_key_str_part3_for_google_email_issuer);
  grpc_auth_json_key key =
      grpc_auth_json_key_create_from_string(key_str);
  gpr_free(key_str);
  GPR_ASSERT(grpc_auth_json_key_is_valid(&key));
  grpc_httpcli_set_override(httpcli_get_google_keys_for_email,
                            httpcli_post_should_not_be_called);
  jwt = grpc_jwt_encode_and_sign(&key, expected_audience,
                                 expected_lifetime, nullptr);
...
static void test_jwt_verifier_custom_email_issuer_success(void) {
  ...
  grpc_jwt_verifier* verifier =
      grpc_jwt_verifier_create(&custom_mapping, 1);
  char* jwt = nullptr;
  char* key_str =
      json_key_str(json_key_str_part3_for_custom_email_issuer);
  grpc_auth_json_key key =
      grpc_auth_json_key_create_from_string(key_str);
  gpr_free(key_str);
  GPR_ASSERT(grpc_auth_json_key_is_valid(&key));
...
static int httpcli_get_openid_config(
  ...
  *response = http_response(200, gpr_strdup(good_openid_config));
  ...
  GPR_ASSERT(
      strcmp(request->http.path, GRPC_OPENID_CONFIG_URL_SUFFIX) == 0);
...
static void on_verification_key_retrieval_error(
...
static void on_verification_bad_signature(
    void* user_data, grpc_jwt_verifier_status status,
    grpc_jwt_claims* claims) {
