...
static void
test_jwt_issuer_email_domain (void)
{
...
test_claims_success (void)
{
  ...
  grpc_slice s =
    grpc_slice_from_copied_string (claims_without_time_constraint);
  grpc_json *json =
    grpc_json_parse_string_with_len ((char *) GRPC_SLICE_START_PTR (s),
				     GRPC_SLICE_LENGTH (s));
...
static void
test_bad_subject_claims_failure (void)
{
  grpc_jwt_claims *claims;
			   ...
			   grpc_httpcli_response * response)
{
  *response = http_response (200, gpr_strdup (good_openid_config));
  ...
  GPR_ASSERT (strcmp (request->http.path, GRPC_OPENID_CONFIG_URL_SUFFIX) ==
	      0);
