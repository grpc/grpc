...
class TlsSecurityConnectorTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    grpc_slice ca_slice_1, ca_slice_0, cert_slice_1, key_slice_1,
        cert_slice_0, key_slice_0;
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file", grpc_load_file(CA_CERT_PATH, 1, &ca_slice_1)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file", grpc_load_file(CLIENT_CERT_PATH, 1, &ca_slice_0)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file",
        grpc_load_file(SERVER_CERT_PATH_1, 1, &cert_slice_1)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file",
        grpc_load_file(SERVER_KEY_PATH_1, 1, &key_slice_1)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file",
        grpc_load_file(SERVER_CERT_PATH_0, 1, &cert_slice_0)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "load_file",
        grpc_load_file(SERVER_KEY_PATH_0, 1, &key_slice_0)));
    root_cert_1_ =
        std::string(grpc_core::StringViewFromSlice(ca_slice_1));
    root_cert_0_ =
        std::string(grpc_core::StringViewFromSlice(ca_slice_0));
...
class TlsTestCertificateProvider
...
TEST_F(TlsSecurityConnectorTest,
  ...
  EXPECT_EQ(tls_connector->KeyCertPairListForTesting(),
            identity_pairs_0_);
  distributor->SetKeyMaterials(kRootCertName, root_cert_1_,
                               absl::nullopt);
  distributor->SetKeyMaterials(kIdentityCertName, absl::nullopt,
                               identity_pairs_1_);
  EXPECT_NE(tls_connector->ClientHandshakerFactoryForTesting(),
...
TEST_F(TlsSecurityConnectorTest,
  ...
  grpc_core::RefCountedPtr<grpc_channel_security_connector>
      root_connector = root_credential->create_security_connector(
          nullptr, "some_target", nullptr, &root_new_args);
  EXPECT_NE(root_connector, nullptr);
  grpc_core::TlsChannelSecurityConnector* tls_root_connector =
      static_cast<grpc_core::TlsChannelSecurityConnector*>(
          root_connector.get());
  EXPECT_NE(tls_root_connector->ClientHandshakerFactoryForTesting(),
            nullptr);
...
TEST_F(
  ...
  EXPECT_NE(tls_root_connector->ClientHandshakerFactoryForTesting(),
            nullptr);
  EXPECT_EQ(tls_root_connector->KeyCertPairListForTesting(),
            identity_pairs_0_);
  // If we have a root update, we shouldn't receive them in security
  // connector, since we claimed to use default system roots.
  distributor->SetKeyMaterials(kRootCertName, root_cert_1_,
                               absl::nullopt);
  EXPECT_NE(tls_root_connector->ClientHandshakerFactoryForTesting(),
            nullptr);
...
TEST_F(TlsSecurityConnectorTest,
  ...
  EXPECT_EQ(tls_connector->ClientHandshakerFactoryForTesting(),
            nullptr);
  EXPECT_EQ(tls_connector->RootCertsForTesting(), root_cert_0_);
  // After updating the root certs, the client_handshaker_factory_
  // should be updated.
  distributor->SetKeyMaterials(kIdentityCertName, absl::nullopt,
                               identity_pairs_0_);
  EXPECT_NE(tls_connector->ClientHandshakerFactoryForTesting(),
...
TEST_F(TlsSecurityConnectorTest,
  ...
  EXPECT_EQ(tls_connector->KeyCertPairListForTesting(),
            identity_pairs_0_);
  // Calling SetErrorForCert on distributor shouldn't invalidate the
  // previous valid credentials.
...
TEST_F(TlsSecurityConnectorTest,
  ...
  auto connector = grpc_core::TlsChannelSecurityConnector::
...
TEST_F(TlsSecurityConnectorTest,
       CreateChannelSecurityConnectorFailNoOptions) {
  grpc_core::RefCountedPtr<grpc_tls_credentials_options> options =
      grpc_core::MakeRefCounted<grpc_tls_credentials_options>();
  grpc_core::RefCountedPtr<TlsCredentials> credential =
      grpc_core::MakeRefCounted<TlsCredentials>(options);
  auto connector = grpc_core::TlsChannelSecurityConnector::
      CreateTlsChannelSecurityConnector(credential, nullptr, nullptr,
...
TEST_F(TlsSecurityConnectorTest, TlsCheckHostNameSuccess) {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 ...
                 target_name, &peer.properties[0]) == TSI_OK);
...
TEST_F(TlsSecurityConnectorTest, TlsCheckHostNameFail) {
  ...
  GPR_ASSERT(tsi_construct_string_peer_property_from_cstring(
                 TSI_X509_SUBJECT_ALTERNATIVE_NAME_PEER_PROPERTY,
                 another_name, &peer.properties[0]) == TSI_OK);
  grpc_error_handle error =
      grpc_core::internal::TlsCheckHostName(target_name, &peer);
  tsi_peer_destruct(&peer);
...
TEST_F(TlsSecurityConnectorTest,
  ...
  grpc_core::TlsServerSecurityConnector* tls_connector =
      static_cast<grpc_core::TlsServerSecurityConnector*>(
          connector.get());
  EXPECT_NE(tls_connector->ServerHandshakerFactoryForTesting(),
            nullptr);
  EXPECT_EQ(tls_connector->RootCertsForTesting(), root_cert_0_);
  EXPECT_EQ(tls_connector->KeyCertPairListForTesting(),
            identity_pairs_0_);
...
// Note that on server side, we don't have tests watching root certs
// only, because in TLS, the identity certs should always be presented.
// If we don't provide, it will try to load certs from some default
// system locations, and will hence fail on some systems.
TEST_F(TlsSecurityConnectorTest,
...
TEST_F(TlsSecurityConnectorTest,
  ...
  EXPECT_EQ(tls_connector->ServerHandshakerFactoryForTesting(),
            nullptr);
  EXPECT_EQ(tls_connector->KeyCertPairListForTesting(),
            identity_pairs_0_);
  // After updating the root certs, the server_handshaker_factory_
  // should be updated.
  distributor->SetKeyMaterials(kRootCertName, root_cert_0_,
                               absl::nullopt);
  EXPECT_NE(tls_connector->ServerHandshakerFactoryForTesting(),
...
TEST_F(TlsSecurityConnectorTest,
  ...
  auto connector = grpc_core::TlsServerSecurityConnector::
...
TEST_F(TlsSecurityConnectorTest,
       CreateServerSecurityConnectorFailNoOptions) {
  grpc_core::RefCountedPtr<grpc_tls_credentials_options> options =
      grpc_core::MakeRefCounted<grpc_tls_credentials_options>();
  grpc_core::RefCountedPtr<TlsServerCredentials> credential =
      grpc_core::MakeRefCounted<TlsServerCredentials>(options);
  auto connector = grpc_core::TlsServerSecurityConnector::
      CreateTlsServerSecurityConnector(credential, nullptr);
  EXPECT_EQ(connector, nullptr);
}
