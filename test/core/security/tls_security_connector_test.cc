...
class TlsSecurityConnectorTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    GPR_ASSERT(GRPC_LOG_IF_ERROR("load_file", grpc_load_file(CA_CERT_PATH, 1, &ca_slice_1)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR("load_file", grpc_load_file(CLIENT_CERT_PATH, 1, &ca_slice_0)));
    GPR_ASSERT(
        GRPC_LOG_IF_ERROR("load_file", grpc_load_file(SERVER_CERT_PATH_1, 1, &cert_slice_1)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR("load_file", grpc_load_file(SERVER_KEY_PATH_1, 1, &key_slice_1)));
    GPR_ASSERT(
        GRPC_LOG_IF_ERROR("load_file", grpc_load_file(SERVER_CERT_PATH_0, 1, &cert_slice_0)));
    GPR_ASSERT(GRPC_LOG_IF_ERROR("load_file", grpc_load_file(SERVER_KEY_PATH_0, 1, &key_slice_0)));
    ...
    std::string identity_key_1 = std::string(grpc_core::StringViewFromSlice(key_slice_1));
    std::string identity_key_0 = std::string(grpc_core::StringViewFromSlice(key_slice_0));
    std::string identity_cert_1 = std::string(grpc_core::StringViewFromSlice(cert_slice_1));
    std::string identity_cert_0 = std::string(grpc_core::StringViewFromSlice(cert_slice_0));
...
TEST_F(TlsSecurityConnectorTest, SystemRootsWhenCreateChannelSecurityConnector) {
  ...
  grpc_core::TlsChannelSecurityConnector* tls_root_connector =
      static_cast<grpc_core::TlsChannelSecurityConnector*>(root_connector.get());
...
TEST_F(TlsSecurityConnectorTest, RootCertsObtainedWhenCreateChannelSecurityConnector) {
  grpc_core::RefCountedPtr<grpc_tls_certificate_distributor> distributor =
      grpc_core::MakeRefCounted<grpc_tls_certificate_distributor>();
  distributor->SetKeyMaterials(kRootCertName, root_cert_0_, absl::nullopt);
  distributor->SetKeyMaterials(kIdentityCertName, absl::nullopt, identity_pairs_0_);
...
TEST_F(TlsSecurityConnectorTest, DistributorHasErrorForChannelSecurityConnector) {
  ...
  // valid credentials.
  distributor->SetErrorForCert(kRootCertName, GRPC_ERROR_CREATE_FROM_STATIC_STRING(kErrorMessage),
                               absl::nullopt);
...
TEST_F(TlsSecurityConnectorTest, CreateChannelSecurityConnectorFailNoTargetName) {
  grpc_core::RefCountedPtr<grpc_tls_credentials_options> options =
      grpc_core::MakeRefCounted<grpc_tls_credentials_options>();
  grpc_core::RefCountedPtr<TlsCredentials> credential =
      grpc_core::MakeRefCounted<TlsCredentials>(options);
...
TEST_F(TlsSecurityConnectorTest, CreateChannelSecurityConnectorFailNoCredentials) {
  auto connector = grpc_core::TlsChannelSecurityConnector::CreateTlsChannelSecurityConnector(
      nullptr, grpc_core::MakeRefCounted<grpc_tls_credentials_options>(), nullptr, kTargetName,
      nullptr, nullptr);
  EXPECT_EQ(connector, nullptr);
}
...
TEST_F(TlsSecurityConnectorTest, TlsCheckHostNameSuccess) {
  ...
  grpc_error_handle error = grpc_core::internal::TlsCheckHostName(target_name, &peer);
...
TEST_F(TlsSecurityConnectorTest, IdentityCertsObtainedWhenCreateServerSecurityConnector) {
  ...
  grpc_core::TlsServerSecurityConnector* tls_identity_connector =
      static_cast<grpc_core::TlsServerSecurityConnector*>(identity_connector.get());
...
TEST_F(TlsSecurityConnectorTest, CreateServerSecurityConnectorFailNoCredentials) {
  auto connector = grpc_core::TlsServerSecurityConnector::CreateTlsServerSecurityConnector(
...
TEST_F(TlsSecurityConnectorTest, CreateServerSecurityConnectorFailNoOptions) {
  ...
  grpc_core::RefCountedPtr<TlsServerCredentials> credential =
      grpc_core::MakeRefCounted<TlsServerCredentials>(options);
  auto connector =
      grpc_core::TlsServerSecurityConnector::CreateTlsServerSecurityConnector(credential, nullptr);
  EXPECT_EQ(connector, nullptr);
}
