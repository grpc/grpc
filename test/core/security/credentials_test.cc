...
grpc_mock_channel_credentials_create (const grpc_channel_credentials_vtable *
				      vtable)
{
...
static void
test_add_to_empty_md_array (void)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_credentials_mdelem_array md_array;
  memset (&md_array, 0, sizeof (md_array));
...
static void
test_add_abunch_to_md_array (void)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_credentials_mdelem_array md_array;
  memset (&md_array, 0, sizeof (md_array));
...
test_oauth2_token_fetcher_creds_parsing_ok (void)
{
  ...
  GPR_ASSERT (grpc_slice_str_cmp (GRPC_MDKEY (token_md), "authorization") ==
	      0);
...
static void
check_metadata (const expected_md * expected,
		grpc_credentials_mdelem_array * md_array)
{
  for (size_t i = 0; i < md_array->size; ++i)
	...
	{
	  if (0 ==
	      grpc_slice_str_cmp (GRPC_MDKEY (md_array->md[j]),
				  expected[i].key))
	    {
...
static void
check_request_metadata (void *arg, grpc_error * error)
{
  ...
  if (state->expected_error == GRPC_ERROR_NONE)
  ...
  else
    {
      ...
      GPR_ASSERT (grpc_error_get_str
		  (error, GRPC_ERROR_STR_DESCRIPTION, &actual_error));
  ...
  grpc_pollset_set_destroy (grpc_polling_entity_pollset_set
			    (&state->pollent));
  gpr_free (state);
}

static request_metadata_state *
make_request_metadata_state (grpc_error * expected_error,
			     const expected_md * expected,
			     size_t expected_size)
{
...
static void
run_request_metadata_test (grpc_call_credentials * creds,
...
check_channel_oauth2_create_security_connector (grpc_channel_credentials * c,
...
static void
test_channel_oauth2_composite_creds (void)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_channel_args *new_args;
  ...
  GPR_ASSERT (grpc_channel_credentials_create_security_connector
	      (channel_oauth2_creds, nullptr, nullptr, nullptr,
	       &new_args) == GRPC_SECURITY_OK);
  grpc_channel_credentials_release (channel_oauth2_creds);
}

static void
test_oauth2_google_iam_composite_creds (void)
{
  grpc_core::ExecCtx exec_ctx;
  expected_md emd[] = {
...
static void
test_channel_oauth2_google_iam_composite_creds (void)
{
  grpc_core::ExecCtx exec_ctx;
  grpc_channel_args *new_args;
  grpc_channel_credentials_vtable vtable = {
...
test_compute_engine_creds_failure (void)
{
  grpc_core::ExecCtx exec_ctx;
  request_metadata_state *state =
    make_request_metadata_state (GRPC_ERROR_CREATE_FROM_STATIC_STRING
...
static grpc_service_account_jwt_access_credentials *
creds_as_jwt (grpc_call_credentials * creds)
{
...
test_jwt_creds_lifetime (void)
{
  ...
  jwt_creds =
    grpc_service_account_jwt_access_credentials_create (json_key_string,
							...
							nullptr);
  GPR_ASSERT (gpr_time_cmp
	      (creds_as_jwt (jwt_creds)->jwt_lifetime, token_lifetime) == 0);
  grpc_call_credentials_release (jwt_creds);
...
test_jwt_creds_signing_failure (void)
{
  ...
  request_metadata_state *state =
    make_request_metadata_state (GRPC_ERROR_CREATE_FROM_STATIC_STRING
				 ("Could not generate JWT."), nullptr,
				 0);
  grpc_call_credentials *creds =
    grpc_service_account_jwt_access_credentials_create (json_key_string,
							grpc_max_auth_token_lifetime
...
static void
test_google_default_creds_refresh_token (void)
{
  grpc_core::ExecCtx exec_ctx;
...
static int
default_creds_gce_detection_httpcli_get_success_override (const
...
static char *
null_well_known_creds_path_getter (void)
{
  return nullptr;
}

static void
test_google_default_creds_gce (void)
{
  grpc_core::ExecCtx exec_ctx;
  expected_md emd[] = {
    {"authorization", "Bearer ya29.AHES6ZRN3-HlhAPya30GnW_bHSb_"}
...
static int
default_creds_gce_detection_httpcli_get_failure_override (const
							  grpc_httpcli_request
							  * request,
							  grpc_millis
							  deadline,
...
static void
test_no_google_default_creds (void)
{
  grpc_flush_cached_google_default_credentials ();
  gpr_setenv (GRPC_GOOGLE_CREDENTIALS_ENV_VAR, "");	/* Reset. */
  grpc_override_well_known_credentials_path_getter
...
static int
plugin_get_metadata_success (void *state, grpc_auth_metadata_context context,
...
static int
plugin_get_metadata_failure (void *state, grpc_auth_metadata_context context,
			     grpc_credentials_plugin_metadata_cb cb,
			     void *user_data,
			     grpc_metadata
			     creds_md
...
test_metadata_plugin_success (void)
{
  ...
  request_metadata_state *md_state =
    make_request_metadata_state (GRPC_ERROR_NONE, plugin_md,
				 GPR_ARRAY_SIZE (plugin_md));
...
test_metadata_plugin_failure (void)
{
  ...
  request_metadata_state *md_state =
    make_request_metadata_state (GRPC_ERROR_CREATE_FROM_COPIED_STRING
				 (expected_error), nullptr, 0);
...
static void
test_get_well_known_google_credentials_file_path (void)
{
  char *path;
...
static void
test_channel_creds_duplicate_without_call_creds (void)
{
  grpc_core::ExecCtx exec_ctx;
...
static void
test_auth_metadata_context (void)
{
