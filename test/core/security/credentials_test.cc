...
TEST(CredentialsTest, TestGoogleDefaultCredsAuthKey) {
  ...
  creds->Unref();
  grpc_core::SetEnv(GRPC_GOOGLE_CREDENTIALS_ENV_VAR, ""); /* Reset. */
...
TEST(CredentialsTest, TestGetWellKnownGoogleCredentialsFilePath) {
  auto home = grpc_core::GetEnv("HOME");
  ...
  grpc_core::SetEnv("HOME", "/fake/home/for/bazel");
...
TEST(CredentialsTest, TestAwsExternalAccountCredsSuccessPathRegionEnvKeysUrl) {
  ExecCtx exec_ctx;
  grpc_core::SetEnv("AWS_REGION", "test_regionz");
  auto credential_source =
      Json::Parse(valid_aws_external_account_creds_options_credential_source);
  GPR_ASSERT(credential_source.ok());
  ExternalAccountCredentials::Options options = {
  ...
  ExecCtx::Get()->Flush();
  HttpRequest::SetOverride(nullptr, nullptr, nullptr);
  grpc_core::UnsetEnv("AWS_REGION");
}

TEST(CredentialsTest,
     TestAwsExternalAccountCredsSuccessPathDefaultRegionEnvKeysUrl) {
  ExecCtx exec_ctx;
  grpc_core::SetEnv("AWS_DEFAULT_REGION", "test_regionz");
  auto credential_source =
      Json::Parse(valid_aws_external_account_creds_options_credential_source);
  GPR_ASSERT(credential_source.ok());
  ExternalAccountCredentials::Options options = {
  ...
  ExecCtx::Get()->Flush();
  HttpRequest::SetOverride(nullptr, nullptr, nullptr);
  grpc_core::UnsetEnv("AWS_DEFAULT_REGION");
}
...
TEST(CredentialsTest,
  ...
  grpc_core::SetEnv("AWS_REGION", "test_regionz");
  grpc_core::SetEnv("AWS_DEFAULT_REGION", "ERROR_REGION");
  auto credential_source =
      Json::Parse(valid_aws_external_account_creds_options_credential_source);
  GPR_ASSERT(credential_source.ok());
  ExternalAccountCredentials::Options options = {
...
TEST(CredentialsTest, TestAwsExternalAccountCredsSuccessPathRegionUrlKeysEnv) {
  ExecCtx exec_ctx;
  grpc_core::SetEnv("AWS_ACCESS_KEY_ID", "test_access_key_id");
  grpc_core::SetEnv("AWS_SECRET_ACCESS_KEY", "test_secret_access_key");
  grpc_core::SetEnv("AWS_SESSION_TOKEN", "test_token");
  auto credential_source =
      Json::Parse(valid_aws_external_account_creds_options_credential_source);
  GPR_ASSERT(credential_source.ok());
  ExternalAccountCredentials::Options options = {
  ...
  ExecCtx::Get()->Flush();
  HttpRequest::SetOverride(nullptr, nullptr, nullptr);
  grpc_core::UnsetEnv("AWS_ACCESS_KEY_ID");
  grpc_core::UnsetEnv("AWS_SECRET_ACCESS_KEY");
  grpc_core::UnsetEnv("AWS_SESSION_TOKEN");
}
