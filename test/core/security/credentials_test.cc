  ...
  for (const std::pair<std::string, std::string>& kv :
       absl::flat_hash_map<std::string, std::string>(
           {{"resource", "resource"},
            ...
            {"subject_token_type", test_signed_jwt_token_type}})) {
    const auto it = url->query_parameters().find(kv.first);
    GPR_ASSERT(it != url->query_parameters().end());
    if (it->second != kv.second) {
      gpr_log(GPR_ERROR, "%s!=%s", it->second.c_str(), kv.second.c_str());
    }
    GPR_ASSERT(it->second == kv.second);
  }
