...
set_socket_nonblocking (thread_args * args)
{
  ...
  if (!GRPC_LOG_IF_ERROR ("Unable to set write socket nonblocking",
			  grpc_set_socket_nonblocking (args->fds.write_fd,
						       1)))
    {
      return -1;
    }
...
static void
print_histogram (grpc_histogram * histogram)
{
...
static int
connect_client (struct sockaddr *addr, socklen_t len)
{
...
int
create_socket (const char *socket_type, fd_pair * client_fds,
	       fd_pair * server_fds)
{
...
static int
run_benchmark (const char *socket_type, thread_args * client_args,
...
static int
run_all_benchmarks (size_t msg_size)
{
  int error = 0;
  size_t i;
