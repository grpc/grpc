...
TEST_F(SecureEndpointReadCoalescingTest, ReadHintSmallerThanFirstFrame) {
  ...
  // We should only get the first 5 bytes because that's what the hint asked
  // for.
...
TEST_F(SecureEndpointReadCoalescingTest, ReadHintSpansFrames) {
  ...
  // We should get the full first frame, and the first two bytes of the second
  // frame because that's what the hint asked for.
  EXPECT_EQ(result, "frame1fr");
}
