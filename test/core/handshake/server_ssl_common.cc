...
server_thread (void *arg)
{
  ...
  grpc_server_credentials *ssl_creds =
    grpc_ssl_server_credentials_create (ca_cert, &pem_key_cert_pair, 1, 0,
					nullptr);
...
bool
server_ssl_test (const char *alpn_list[], unsigned int alpn_list_len,
		 const char *alpn_expected)
{
  ...
  GPR_ASSERT (gpr_thd_new
	      (&thdid, "grpc_ssl_test", server_thread, &port, &thdopt));
  ...
  GPR_ASSERT (SSL_CTX_set_alpn_protos (ctx, alpn_protos, alpn_protos_len) ==
	      0);
