...
static int
alpn_select_cb (SSL * ssl, const uint8_t ** out, uint8_t * out_len,
...
static bool
client_ssl_test (char *server_alpn_preferred)
{
  bool success = true;
  ...
  GPR_ASSERT (gpr_thd_new
	      (&thdid, "grpc_client_ssl_test", server_thread, &args,
	       &thdopt));
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR ("load_file",
				 grpc_load_file (SSL_CERT_PATH, 1,
						 &cert_slice)));
  ...
  while (state != GRPC_CHANNEL_READY && retries-- > 0)
    {
      grpc_channel_watch_connectivity_state (channel, state,
      ...
      state =
	grpc_channel_check_connectivity_state (channel,
					       0 /* try_to_connect */ );
    }
