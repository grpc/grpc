...
static int create_socket(int* out_port) {
  ...
  *out_port = -1;
  ...
  if (bind(s, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) <
      0) {
  ...
  if (getsockname(s, reinterpret_cast<struct sockaddr*>(&addr),
                  &addr_len) != 0 ||
      addr_len > sizeof(addr)) {
    perror("getsockname");
...
static int alpn_select_cb(SSL* /*ssl*/, const uint8_t** out,
  ...
  *out_len = static_cast<uint8_t>(
  ...
  // Validate that the ALPN list includes "h2" and "grpc-exp", that
  // "grpc-exp" precedes "h2".
  ...
  while (inp < in_end) {
    ...
    if (length == strlen("grpc-exp") &&
        strncmp(inp, "grpc-exp", length) == 0) {
      grpc_exp_seen = true;
      GPR_ASSERT(!h2_seen);
    }
...
static void ssl_server_info_callback(const SSL* ssl, int where,
                                     int ret) {
  if (ret == 0) {
...
// https://wiki.openssl.org/index.php/Simple_TLS_Server and the gRPC
// core internals in src/core/tsi/ssl_transport_security.c.
static void server_thread(void* arg) {
  ...
  // Enable automatic curve selection. This is a NO-OP when using
...
// establishes a TLS handshake via the core library to the server. The
// TLS server validates ALPN aspects of the handshake and supplies the
// protocol specified in the server_alpn_preferred argument to the
// client.
static bool client_ssl_test(char* server_alpn_preferred) {
  ...
  // Find a port we can bind to. Retries added to handle flakes in port
  // server and port picking.
  int port = -1;
  int server_socket = -1;
  ...
  server_args args = {server_socket, server_alpn_preferred,
                      &ssl_library_info};
  bool ok;
  grpc_core::Thread thd("grpc_client_ssl_test", server_thread, &args,
                        &ok);
  GPR_ASSERT(ok);
  thd.Start();
  ...
  // Establish a channel pointing at the TLS server. Since the gRPC
  // runtime is lazy, this won't necessarily establish a connection yet.
  ...
  grpc_channel* channel = grpc_secure_channel_create(
      ssl_creds, target.c_str(), &grpc_args, nullptr);
  GPR_ASSERT(channel);

  // Initially the channel will be idle, the
  // grpc_channel_check_connectivity_state triggers an attempt to
  // connect.
  GPR_ASSERT(grpc_channel_check_connectivity_state(
                 channel, 1 /* try_to_connect */) == GRPC_CHANNEL_IDLE);

  // Wait a bounded number of times for the channel to be ready. When
  // the channel is ready, the initial TLS handshake will have
  // successfully completed and we know that the client's ALPN list
  ...
  while (state != GRPC_CHANNEL_READY && retries-- > 0) {
    grpc_channel_watch_connectivity_state(
        channel, state, grpc_timeout_seconds_to_deadline(3), cq,
        nullptr);
...
int main(int argc, char* argv[]) {
  ...
  // Handshake succeeeds when the server has grpc-exp as the ALPN
  // preference.
  GPR_ASSERT(client_ssl_test(const_cast<char*>("grpc-exp")));
  // Handshake succeeeds when the server has h2 as the ALPN preference.
  // This covers legacy gRPC servers which don't support grpc-exp.
  GPR_ASSERT(client_ssl_test(const_cast<char*>("h2")));
  // Handshake fails when the server uses a fake protocol as its ALPN
  // preference. This validates the client is correctly validating ALPN
  // returns and sanity checks the client_ssl_test.
