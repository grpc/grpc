...
static int verify_callback(const char* target_host, const char* target_pem,
  ...
  if (target_host != nullptr) {
    GPR_ASSERT((callback_target_host = static_cast<char*>(
                    malloc(strlen(target_host) + 1))) != nullptr);
    strcpy(callback_target_host, target_host);
  }
  if (target_pem != nullptr) {
    GPR_ASSERT((callback_target_pem = static_cast<char*>(
                    malloc(strlen(target_pem) + 1))) != nullptr);
    strcpy(callback_target_pem, target_pem);
  }
...
int main(int argc, char* argv[]) {
  ...
  free(callback_target_host);
  free(callback_target_pem);
