...
static void server_thread(void* arg) {
  ...
  // set, sleeping between polls. The total time spent (deadline *
  // retries) should be strictly greater than the client retry limit so
  // that the client will always timeout first.
  int retries = 60;
...
// This test launches a minimal TLS grpc server on a separate thread and
// then establishes a TLS handshake via the core library to the server.
// The client uses the supplied verify options.
static bool verify_peer_options_test(
  ...
  bool ok;
  grpc_core::Thread thd("grpc_client_ssl_test", server_thread, &port,
                        &ok);
  GPR_ASSERT(ok);
  thd.Start();
  ...
  // successfully completed. The total time spent on the client side
  // (retries * deadline) should be greater than the server side time
  // limit.
  int retries = 10;
...
static int verify_callback(const char* target_host,
                           const char* target_pem, void* userdata) {
...
int main(int argc, char* argv[]) {
  ...
  // Load the server's cert so that we can assert it gets passed to the
  // callback
  ...
  // If the callback returns non-zero, initializing the channel should
  // fail.
