...
static void test_grpc_parse_ipv6_parity_with_getaddrinfo(
    const char* target,
    const struct sockaddr_in6 result_from_getaddrinfo) {
  ...
  // Compare the sockaddr returned from gRPC's ipv6 resolver with that
  ...
  GPR_ASSERT(memcmp(&result_from_grpc_parser->sin6_addr,
                    &result_from_getaddrinfo.sin6_addr,
                    sizeof(in6_addr)) == 0);
  ...
  // TODO(unknown): compare sin6_flow_info fields? parse_ipv6 zero's
  // this field as is. Cleanup
}
...
struct sockaddr_in6 resolve_with_gettaddrinfo(const char* uri_text) {
  ...
  for (struct addrinfo* resp = result; resp != nullptr;
       resp = resp->ai_next) {
...
int main(int argc, char** argv) {
  ...
  // Per RFC 3493, an interface index is a "small positive integer
  // starts at 1". Probe candidate interface index numbers until we find
  // one that the system recognizes, and then use that for the test.
  ...
  std::string target = absl::StrFormat("ipv6:[fe80::1234%%%s]:12345",
                                       arbitrary_interface_name);
  struct sockaddr_in6 result_from_getaddrinfo =
      resolve_with_gettaddrinfo(target.c_str());
  // Run the test
  gpr_log(GPR_DEBUG,
          "Run test_grpc_parse_ipv6_parity_with_getaddrinfo with "
          "target: %s",
          target.c_str());
