...
TEST(FusedFilterTest, ClientFilterTest) {
  ...
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<TestFusedFilter> filter,
  ...
  RunSuccessfulPromise<ServerMetadata>(
      call.OnServerInitialMetadata(std::move(server_metadata_handle), filter.get()));
  RunSuccessfulPromise<ClientMetadata>(
      call.OnClientInitialMetadata(std::move(client_metadata_handle), filter.get()));
  call.OnServerTrailingMetadata(*server_trailing_metadata_handle, filter.get());
