...
static size_t
bucket_for_unchecked (grpc_histogram * h, double x)
{
...
static size_t
bucket_for (grpc_histogram * h, double x)
{
  size_t bucket =
...
static double
bucket_start (grpc_histogram * h, double x)
{
  return pow (h->multiplier, x);
}

grpc_histogram *
grpc_histogram_create (double resolution, double max_bucket_start)
{
...
void
grpc_histogram_destroy (grpc_histogram * h)
{
  gpr_free (h->buckets);
  gpr_free (h);
}
...
void
grpc_histogram_add (grpc_histogram * h, double x)
{
  h->sum += x;
...
int
grpc_histogram_merge (grpc_histogram * dst, const grpc_histogram * src)
{
...
void
grpc_histogram_merge_contents (grpc_histogram * dst, const uint32_t * data,
...
static double
threshold_for_count_below (grpc_histogram * h, double count_below)
{
  double count_so_far;
  double lower_bound;
  double upper_bound;
...
double
grpc_histogram_percentile (grpc_histogram * h, double percentile)
{
  return threshold_for_count_below (h, h->count * percentile / 100.0);
}

double
grpc_histogram_mean (grpc_histogram * h)
{
...
double
grpc_histogram_stddev (grpc_histogram * h)
{
  return sqrt (grpc_histogram_variance (h));
}

double
grpc_histogram_variance (grpc_histogram * h)
{
  if (h->count == 0)
    return 0.0;
...
double
grpc_histogram_maximum (grpc_histogram * h)
{
  return h->max_seen;
}

double
grpc_histogram_minimum (grpc_histogram * h)
{
  return h->min_seen;
}

double
grpc_histogram_count (grpc_histogram * h)
{
  return h->count;
}

double
grpc_histogram_sum (grpc_histogram * h)
{
  return h->sum;
}

double
grpc_histogram_sum_of_squares (grpc_histogram * h)
{
  return h->sum_of_squares;
}

const uint32_t *
grpc_histogram_get_contents (grpc_histogram * h, size_t * size)
{
  *size = h->num_buckets;
  return h->buckets;
}
