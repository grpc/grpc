...
/** Simple command line parser.
   ...
   Supports flags that can be specified as -foo, --foo, --no-foo,
   -no-foo, etc And integers, strings that can be specified as -foo=4,
   -foo blah, etc
   ...
   int main(int argc, char **argv) {
     ...
     gpr_cmdline_add_int(cl, "verbose", "Produce verbose output?",
   &verbose); gpr_cmdline_parse(cl, argc, argv);
...
/** Construct a command line parser: takes a short description of the
   tool doing the parsing */
gpr_cmdline* gpr_cmdline_create(const char* description);
/** Add an integer parameter, with a name (used on the command line) and
   some helpful text (used in the command usage) */
...
/** Parse the command line; returns 1 on success, on failure either dies
   (by default) or returns 0 if gpr_cmdline_set_survive_failure() has
   been called */
