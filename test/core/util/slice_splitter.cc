...
void
grpc_split_slices (grpc_slice_split_mode mode, grpc_slice * src_slices,
  ...
  switch (mode)
      ...
      for (i = 0; i < src_slice_count; i++)
	{
	  for (j = 0; j < GRPC_SLICE_LENGTH (src_slices[i]); j++)
	    {
	      (*dst_slices)[length] =
		grpc_slice_sub (src_slices[i], j, j + 1);
	      length++;
	    }
	}
      break;
...
void
grpc_split_slice_buffer (grpc_slice_split_mode mode, grpc_slice_buffer * src,
			 grpc_slice_buffer * dst)
{
...
grpc_slice
grpc_slice_merge (grpc_slice * slices, size_t nslices)
{
  uint8_t *out = nullptr;
  size_t length = 0;
  size_t capacity = 0;
  ...
  for (i = 0; i < nslices; i++)
	...
	{
	  capacity =
	    GPR_MAX (capacity * 2, GRPC_SLICE_LENGTH (slices[i]) + length);
