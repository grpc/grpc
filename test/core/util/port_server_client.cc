...
static void
destroy_pops_and_shutdown (void *p, grpc_error * error)
{
  grpc_pollset *pollset =
    grpc_polling_entity_pollset ((grpc_polling_entity *) p);
  grpc_pollset_destroy (pollset);
  gpr_free (pollset);
}

static void
freed_port_from_server (void *arg, grpc_error * error)
{
...
void
grpc_free_port_using_server (int port)
{
...
static void
got_port_from_server (void *arg, grpc_error * error)
{
  size_t i;
  int port = 0;
...
int
grpc_pick_port_using_server (void)
{
  grpc_httpcli_context context;
  grpc_httpcli_request req;
  portreq pr;
