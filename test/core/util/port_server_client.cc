...
void grpc_free_port_using_server(int port) {
  ...
  grpc_init();
  {
    ...
    auto httpcli_request = grpc_core::HttpCliRequest::Get(
        &pr.pops, grpc_core::ResourceQuota::Default(), &req, absl::make_unique<grpc_core::HttpCliRequest::PlaintextHttpCliHandshakerFactory>(),
...
static void got_port_from_server(void* arg, grpc_error_handle error) {
  ...
  if (failed) {
    ...
    pr->response = {};
    pr->httpcli_request = grpc_core::HttpCliRequest::Get(
        &pr->pops, grpc_core::ResourceQuota::Default(), &req, absl::make_unique<grpc_core::HttpCliRequest::PlaintextHttpCliHandshakerFactory>(),
        grpc_core::ExecCtx::Get()->Now() + 30 * GPR_MS_PER_SEC,
