...
print_current_stack ()
{
  ...
  CaptureStackBackTraceType func =
    (CaptureStackBackTraceType) (GetProcAddress
...
#define MAX_CALLERS 62
  ...
  symbol =
    (SYMBOL_INFOW *) calloc (sizeof (SYMBOL_INFOW) + 256 * sizeof (wchar_t),
			     1);
  ...
  for (unsigned int i = 0; i < frames; i++)
    {
      ...
      fwprintf (stderr, L"*** %d: %016I64X %ls - %016I64X\n", i,
		(DWORD64) callers_stack[i], symbol->Name,
		(DWORD64) symbol->Address);
      fflush (stderr);
    }
...
static void
print_stack_from_context (CONTEXT c)
{
...
#error "Platform not supported!"
  ...
  while (StackWalk (imageType, process, thread, &s, &c, 0,
	...
	SymFromAddrW (process, (DWORD64) (s.AddrPC.Offset), 0, symbol);
      fwprintf (stderr, L"*** %016I64X %ls - %016I64X\n",
		(DWORD64) (s.AddrPC.Offset),
		has_symbol ? symbol->Name : L"<<no symbol>>",
		(DWORD64) symbol->Address);
      fflush (stderr);
    }
...
static void
crash_handler (int signum, siginfo_t * info, void *data)
{
...
gpr_timespec
grpc_timeout_seconds_to_deadline (int64_t time_s)
{
...
gpr_timespec
grpc_timeout_milliseconds_to_deadline (int64_t time_ms)
{
  return gpr_time_add (gpr_now (GPR_CLOCK_MONOTONIC),
