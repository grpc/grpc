...
static void
te_read (grpc_endpoint * ep, grpc_slice_buffer * slices, grpc_closure * cb)
{
...
static void
maybe_call_write_cb_locked (trickle_endpoint * te)
{
...
static void
te_write (grpc_endpoint * ep, grpc_slice_buffer * slices, grpc_closure * cb)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
  gpr_mu_lock (&te->mu);
...
static void
te_add_to_pollset (grpc_endpoint * ep, grpc_pollset * pollset)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
...
static void
te_add_to_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset_set)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
...
static void
te_delete_from_pollset_set (grpc_endpoint * ep,
...
static void
te_shutdown (grpc_endpoint * ep, grpc_error * why)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
  gpr_mu_lock (&te->mu);
...
te_get_resource_user (grpc_endpoint * ep)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
...
static void
te_finish_write (void *arg, grpc_error * error)
{
...
grpc_endpoint *
grpc_trickle_endpoint_create (grpc_endpoint * wrap, double bytes_per_second)
{
...
static double
ts2dbl (gpr_timespec s)
{
  return (double) s.tv_sec + 1e-9 * (double) s.tv_nsec;
}

size_t
grpc_trickle_endpoint_trickle (grpc_endpoint * ep)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
  gpr_mu_lock (&te->mu);
...
size_t
grpc_trickle_get_backlog (grpc_endpoint * ep)
{
  trickle_endpoint *te = (trickle_endpoint *) ep;
  gpr_mu_lock (&te->mu);
  size_t backlog = te->write_buffer.length;
