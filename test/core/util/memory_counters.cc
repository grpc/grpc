...
static void* guard_malloc(size_t size) {
  ...
  ptr = reinterpret_cast<size_t*>(g_old_allocs.malloc_fn(size + sizeof(size)));
  *ptr++ = size;
  return ptr;
}

static void* guard_realloc(void* vptr, size_t size) {
  size_t* ptr = reinterpret_cast<size_t*>(vptr);
  if (vptr == nullptr) {
  ...
  ptr = reinterpret_cast<size_t*>(
      g_old_allocs.realloc_fn(ptr, size + sizeof(size)));
  *ptr++ = size;
  return ptr;
}
