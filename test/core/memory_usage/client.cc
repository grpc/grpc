...
static void
init_ping_pong_request (int call_idx)
{
  ...
  calls[call_idx].call =
    grpc_channel_create_call (channel, nullptr, GRPC_PROPAGATE_DEFAULTS, cq,
			      ...
			      ("/Reflector/reflectUnary"), &hostname,
...
static void
finish_ping_pong_request (int call_idx)
{
...
send_snapshot_request (int call_idx, grpc_slice call_type)
{
  grpc_metadata_array_init (&calls[call_idx].initial_metadata_recv);
  grpc_metadata_array_init (&calls[call_idx].trailing_metadata_recv);
  ...
  GPR_ASSERT (GRPC_CALL_OK ==
	      grpc_call_start_batch (calls[call_idx].call, snapshot_ops,
...
main (int argc, char **argv)
{
  ...
  do
    {
      event = grpc_completion_queue_next (cq,
