...
static void
request_call_unary (int call_idx)
{
  if (call_idx == (int) (sizeof (calls) / sizeof (fling_call)))
    {
      gpr_log (GPR_INFO,
	       "Used all call slots (10000) on server. Server exit.");
      _exit (0);
    }
...
static void
send_initial_metadata_unary (void *tag)
{
  ...
  GPR_ASSERT (GRPC_CALL_OK ==
	      grpc_call_start_batch ((*(fling_call *) tag).call, metadata_ops,
				     1, tag, nullptr));
}

static void
send_status (void *tag)
{
  ...
  GPR_ASSERT (GRPC_CALL_OK ==
	      grpc_call_start_batch ((*(fling_call *) tag).call, &status_op,
				     1, tag, nullptr));
}

static void
send_snapshot (void *tag, struct grpc_memory_counters *snapshot)
{
  grpc_op *op;
	...
	case GRPC_OP_COMPLETE:
	  switch (s->state)
	    ...
	    case FLING_SERVER_BATCH_SEND_STATUS_FLING_CALL:
	      for (int k = 0;
		   k < (int) (sizeof (calls) / sizeof (fling_call)); ++k)
		{
