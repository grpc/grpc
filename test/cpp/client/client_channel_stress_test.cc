  ...
  grpc
{
    ...
    testing
  {
	...
	kNumBalancers = 5;
      const size_t
	kNumClientThreads = 100;
      const int
	kResolutionUpdateIntervalMs = 50;
      const int
	kServerlistUpdateIntervalMs = 10;
      const int
	kTestDurationSec = 30;

      using
	BackendServiceImpl = TestServiceImpl;
	...
	explicit
	BalancerServiceImpl (const std::vector < int >&all_backend_ports):
	...
	LoadBalanceResponse
	BuildRandomResponseForBackends ()
	{
	  ...
	  size_t
	    num_drop_entry = std::rand () % (all_backend_ports_.size () + 1);
	  std::vector < int >
	    random_backend_indices;
	  for (size_t i = 0; i < num_non_drop_entry; ++i)
	    {
	      random_backend_indices.push_back (std::rand () %
						all_backend_ports_.size ());
	    }
	  for (size_t i = 0; i < num_drop_entry; ++i)
	    {
	      random_backend_indices.push_back (-1);
	    }
	  std::shuffle (random_backend_indices.begin (),
	...
	KeepSendingRequests ()
	{
	  ...
	  while (!shutdown_)
		...
		context;
	      context.
		set_deadline (grpc_timeout_milliseconds_to_deadline (1000));
	      EchoRequest
		request;
	...
	Start ()
	{
	  ...
	  for (size_t i = 0; i < kNumBackends; ++i)
	    {
	      ...
	      backend_servers_.emplace_back (ServerThread <
					     BackendServiceImpl > ("backend",
								   server_host_,
								   backends_.
								   back ().
								   get ()));
	  ...
	  for (size_t i = 0; i < kNumBalancers; ++i)
	    {
	      balancers_.
		emplace_back (new BalancerServiceImpl (backend_ports));
	      balancer_servers_.emplace_back (ServerThread <
					      BalancerServiceImpl >
	  ...
	  for (size_t i = 0; i < kNumClientThreads; ++i)
	    {
	      client_threads_.
		emplace_back (std::
			      thread (&ClientChannelStressTest::
				      KeepSendingRequests, this));
	    }
	}
