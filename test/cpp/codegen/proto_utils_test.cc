...
// Provide access to GrpcBufferWriter internals.
    ...
    {
    public:
      explicit GrpcBufferWriterPeer (internal::GrpcBufferWriter *
				     writer):writer_ (writer)
      {
      }
      bool have_backup () const
      {
	return writer_->have_backup_;
      }
...
// Set backup_size to 0 to indicate no backup is needed.
	...
	for (int i = 2; i < (int) GRPC_SLICE_INLINED_SIZE; i++)
      ...
      TEST (WriterTest, SmallBlockTinyBackup)
      {
	BufferWriterTest (64, 256, 1);
      }

      TEST (WriterTest, SmallBlockNoBackup)
      {
	BufferWriterTest (64, 256, 0);
      }

      TEST (WriterTest, SmallBlockFullBackup)
      {
	BufferWriterTest (64, 256, 64);
      }

      TEST (WriterTest, LargeBlockTinyBackup)
      {
	BufferWriterTest (4096, 8192, 1);
      }

      TEST (WriterTest, LargeBlockNoBackup)
      {
	BufferWriterTest (4096, 8192, 0);
      }

      TEST (WriterTest, LargeBlockFullBackup)
      {
	BufferWriterTest (4096, 8192, 4096);
      }

      TEST (WriterTest, LargeBlockLargeBackup)
      {
