...
namespace {
void Usage() {
  using grpc_sleuth::GetPrintFn;
  GetPrintFn()(std::string("Sleuth version ") +
               std::to_string(grpc_sleuth::kSleuthVersion) + "\n");
  GetPrintFn()("Usage: sleuth <tool> [args...]\n");
  for (const auto& tool : grpc_sleuth::ToolRegistry::Get()->GetMetadata()) {
    GetPrintFn()(std::string("  ") + std::string(tool.name) + " " +
                 std::string(tool.args) + ": " + std::string(tool.description) +
                 "\n");
  }
  GetPrintFn()("Run 'sleuth --help' for more information.\n");
}
}  // namespace
...
int RunSleuth(int argc, char** argv,
              std::function<void(std::string)> print_fn) {
  ...
  if (tool == nullptr) {
    GetPrintFn()(std::string("Unknown tool: ") + tool_name + "\n");
    Usage();
    return 1;
  }
  ...
  if (!tool_args.ok()) {
    GetPrintFn()(std::string("Argument parsing failed: ") +
                 tool_args.status().ToString() + "\n");
    return 1;
  }
  auto status = tool(*tool_args.value());
  if (!status.ok()) {
    GetPrintFn()(std::string("Tool failed: ") + status.ToString() + "\n");
    return 1;
  }
  return 0;
}
