...
def generate_resolver_component_tests():
    ...
    for unsecure_build_config_suffix in ["_unsecure", ""]:
        ...
        for enable_event_engine_dns_experiment in ["", "_enable_event_engine_dns_experiment"]:
            grpc_cc_test(
                name = "resolver_component_tests_runner_invoker%s%s" % (unsecure_build_config_suffix, enable_event_engine_dns_experiment),
                srcs = [
                ...
                ] + ["--extra_args=--grpc_experiments=event_engine_dns" if len(enable_event_engine_dns_experiment) > 0 else ""],
