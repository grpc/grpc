...
namespace
{
  ...
  public:
    ...
    bool
    operator== (const GrpcLBAddress & other) const
    {
      return
	this->
	is_balancer == other.is_balancer && this->address == other.address;
    }
  ...
  vector < GrpcLBAddress > ParseExpectedAddrs (std::string expected_addrs)
  {
	...
	size_t
	  next_semicolon = expected_addrs.find (";");
	bool
	  is_balancer =
  ...
  void
  PollPollsetUntilRequestDone (ArgsStruct * args)
  {
    gpr_timespec
      deadline = NSecondDeadline (10);
    while (true)
      {
	bool
	  done = gpr_atm_acq_load (&args->done_atm) != 0;
	if (done)
	  {
	    break;
	  }
	gpr_timespec
	  time_left = gpr_time_sub (deadline, gpr_now (GPR_CLOCK_REALTIME));
  ...
  void
  CheckServiceConfigResultLocked (grpc_channel_args * channel_args,
				  ArgsStruct * args)
  {
    const grpc_arg *
      service_config_arg =
  ...
  void
  CheckLBPolicyResultLocked (grpc_channel_args * channel_args,
			     ArgsStruct * args)
  {
    const grpc_arg *
      lb_policy_arg =
  ...
  void
  CheckResolverResultLocked (void *argsp, grpc_error * err)
  {
    ArgsStruct *
    ...
    grpc_lb_addresses *
      addresses = (grpc_lb_addresses *) channel_arg->value.pointer.p;
    ...
    for (size_t i = 0; i < addresses->num_addresses; i++)
      {
	grpc_lb_address
	  addr = addresses->addresses[i];
	char *
	  str;
	grpc_sockaddr_to_string (&str, &addr.address, 1 /* normalize */ );
	gpr_log (GPR_INFO, "%s", str);
	found_lb_addrs.
	  emplace_back (GrpcLBAddress (std::string (str), addr.is_balancer));
	gpr_free (str);
      }
  ...
  TEST (ResolverComponentTest, TestResolvesRelevantRecords)
  {
    ...
    args.expected_service_config_string =
      FLAGS_expected_chosen_service_config;
