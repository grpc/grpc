...
ABSL_FLAG(
    ...
    "Child process test binaries are located differently based on this "
    "flag. ");
...
void InvokeResolverComponentTestsRunner(
  ...
  if (WIFEXITED(status)) {
    ...
    gpr_log(GPR_INFO,
            "Resolver component test test-runner ended with unknown "
            "status %d",
            status);
    abort();
  }
...
int main(int argc, char** argv) {
  ...
  if (absl::GetFlag(FLAGS_running_under_bazel)) {
    GPR_ASSERT(!absl::GetFlag(
                    FLAGS_grpc_test_directory_relative_to_test_srcdir)
                    .empty());
    // Use bazel's TEST_SRCDIR environment variable to locate the "test
    // data" binaries.
    ...
    // Invoke bazel's executeable links to the .sh and .py scripts
    // (don't use the .sh and .py suffixes) to make sure that we're
    // using bazel's test environment.
    grpc::testing::InvokeResolverComponentTestsRunner(
        ...
        bin_dir + "/utils/dns_resolver",
        bin_dir + "/utils/tcp_connect");
    gpr_free(test_srcdir);
  } else {
    // Get the current binary's directory relative to repo root to
    // invoke the correct build config (asan/tsan/dbg, etc.).
    std::string const bin_dir = my_bin.substr(0, my_bin.rfind('/'));
    // Invoke the .sh and .py scripts directly where they are in source
    // code.
    grpc::testing::InvokeResolverComponentTestsRunner(
