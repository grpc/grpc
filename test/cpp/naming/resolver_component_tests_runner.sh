...
do
  ...
  RETRY=0
  dig A dummy-health-check.resolver-tests-version-5.grpctestingexp. @localhost -p "$FLAGS_test_dns_server_port" +tries=1 +timeout=1 | grep 123.123.123.123 || RETRY=1
  dig A dummy-health-check.resolver-tests-version-5.grpctestingexp. @localhost -p "$FLAGS_test_dns_server_port" +tries=1 +timeout=1 +tcp | grep 123.123.123.123 || RETRY=1
  if [[ "$RETRY" == 0 ]]; then
    break
  fi;
  sleep 0.1
...
$FLAGS_test_bin_path \
  --target_name='ipv4-config-causing-fallback-to-tcp.resolver-tests-version-5.grpctestingexp.' \
  --expected_addrs='1.2.3.4:443,False' \
