...
do
  ...
  RETRY=0
  dig A dummy-health-check.resolver-tests-version-2.grpctestingexp. @localhost -p "$FLAGS_test_dns_server_port" +tries=1 +timeout=1 | grep 123.123.123.123 || RETRY=1
  dig A dummy-health-check.resolver-tests-version-2.grpctestingexp. @localhost -p "$FLAGS_test_dns_server_port" +tries=1 +timeout=1 +tcp | grep 123.123.123.123 || RETRY=1
  if [[ "$RETRY" == 0 ]]; then
    break
  fi;
  sleep 0.1
