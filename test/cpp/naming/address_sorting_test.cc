...
EventEngine::ResolvedAddress TestAddressToGrpcResolvedAddress(
  ...
  if (test_addr.family == AF_INET) {
    ...
    return EventEngine::ResolvedAddress(
        reinterpret_cast<sockaddr*>(&in_dest),
    ...
    return EventEngine::ResolvedAddress(
        reinterpret_cast<sockaddr*>(&in6_dest),
...
class MockSourceAddrFactory : public address_sorting_source_addr_factory {
 public:
  ...
  bool GetSourceAddr(const address_sorting_address* dest_addr,
    ...
    sockaddr dest_addr_sockaddr;
    memcpy(&dest_addr_sockaddr, dest_addr, dest_addr->len);
    EventEngine::ResolvedAddress dest_addr_as_resolved_addr(&dest_addr_sockaddr,
                                                            dest_addr->len);
    ...
    EventEngine::ResolvedAddress source_addr_as_resolved_addr =
        TestAddressToGrpcResolvedAddress(it->second);
    memcpy(source_addr->addr, source_addr_as_resolved_addr.address(),
           source_addr_as_resolved_addr.size());
    source_addr->len = source_addr_as_resolved_addr.size();
    return true;
  }
...
std::vector<EventEngine::ResolvedAddress> BuildLbAddrInputs(
...
void VerifyLbAddrOutputs(
  ...
  for (size_t i = 0; i < addresses.size(); ++i) {
    std::string ip_addr_str = ResolvedAddressToString(addresses[i]).value();
...
TEST_F(AddressSortingTest, TestDepriotizesUnreachableAddresses) {
  ...
  VerifyLbAddrOutputs(sorted_addrs, {
                                        "1.2.3.4:443",
                                        "5.6.7.8:443",
                                    });
}
...
TEST_F(AddressSortingTest,
  ...
  auto sorted_addrs = grpc_event_engine::experimental::SortAddresses(lb_addrs);
  ASSERT_EQ(sorted_addrs.size(), 2);
  EXPECT_EQ(ResolvedAddressToString(sorted_addrs[0]).value(), "[::1]:443");
  ...
  EXPECT_THAT(
      ...
      ::testing::Contains(ResolvedAddressToString(lb_addrs[1]).value()));
}
...
TEST_F(AddressSortingTest, TestStableSortV4CompatAndSiteLocalAddresses) {
  ...
  EXPECT_EQ(ResolvedAddressToString(sorted_addrs[0]).value(),
            "[fec0::2000]:443");
  // We've observed some inet_ntop implementations have special representations
  // of IPv4-compatible IPv6 addresses, and others represent them as normal
  // IPv6 addresses. For the purposes of this test, we don't care which
  ...
  EXPECT_THAT(
      acceptable_addresses,
      ::testing::Contains(ResolvedAddressToString(sorted_addrs[1]).value()));
}
