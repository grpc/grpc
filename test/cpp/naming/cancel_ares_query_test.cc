...
gpr_timespec FiveSecondsFromNow(void) { return grpc_timeout_seconds_to_deadline(5); }
  ...
  while (true) {
    ...
    GRPC_LOG_IF_ERROR(
        ...
        grpc_pollset_work(args->pollset, &worker,
                          grpc_timespec_to_millis_round_up(gpr_inf_future(GPR_CLOCK_REALTIME))));
...
class AssertFailureResultHandler : public grpc_core::Resolver::ResultHandler {
 public:
  ...
  void ReturnResult(grpc_core::Resolver::Result /*result*/) override { GPR_ASSERT(false); }
...
void TestCancelActiveDNSQuery(ArgsStruct* args) {
  ...
  std::string client_target = absl::StrFormat(
      "dns://[::1]:%d/dont-care-since-wont-be-resolved.test.com:1234", fake_dns_port);
...
void TestCancelDuringActiveQuery(cancellation_test_query_timeout_setting query_timeout_setting) {
  ...
  std::string client_target = absl::StrFormat(
      "dns://[::1]:%d/dont-care-since-wont-be-resolved.test.com:1234", fake_dns_port);
  gpr_log(GPR_DEBUG, "TestCancelActiveDNSQuery. query timeout setting: %d", query_timeout_setting);
  ...
  if (query_timeout_setting == NONE) {
    ...
    arg.value.integer = 1;  // Set this shorter than the call deadline so that it goes off.
  ...
  grpc_channel* client = grpc_insecure_channel_create(client_target.c_str(), client_args, nullptr);
  grpc_completion_queue* cq = grpc_completion_queue_create_for_next(nullptr);
  cq_verifier* cqv = cq_verifier_create(cq);
  grpc_call* call = grpc_channel_create_call(client, nullptr, GRPC_PROPAGATE_DEFAULTS, cq,
                                             grpc_slice_from_static_string("/foo"), nullptr,
                                             rpc_deadline, nullptr);
  GPR_ASSERT(call);
  ...
  grpc_call_error error =
      grpc_call_start_batch(call, ops_base, static_cast<size_t>(op - ops_base), Tag(1), nullptr);
...
TEST_F(CancelDuringAresQuery, TestHitDeadlineAndDestroyChannelDuringAresResolutionIsGraceful) {
