...
namespace grpc
{
  namespace
  {
    ...
    static grpc_slice build_response_payload_slice (const char *host,
						    int *ports, size_t nports,
      ...
      for (size_t i = 0; i < nports; i++)
	{
      ...
      return grpc_slice_from_copied_buffer (enc_resp.data (),
					    enc_resp.size ());
    }
    ...
    static void start_lb_server (server_fixture * sf, int *ports,
				 size_t nports, int update_delay_ms)
    {
      grpc_call *s;
      ...
      cq_verify (cqv);
      gpr_log (GPR_INFO, "LB Server[%s](%s) after tag 200",
      ...
      op++;
      error =
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (202),
			       nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      CQ_EXPECT_COMPLETION (cqv, tag (202), 1);
      cq_verify (cqv);
      gpr_log (GPR_INFO, "LB Server[%s](%s) after RECV_MSG",
	       sf->servers_hostport, sf->balancer_name);
	...
	grpc_byte_buffer_reader_readall (&bbr);
      ...
      op++;
      error =
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (201),
			       nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      gpr_log (GPR_INFO, "LB Server[%s](%s) after tag 201",
	       sf->servers_hostport, sf->balancer_name);
      ...
      for (int i = 0; i < 2; i++)
	{
	  if (i == 0)
	  ...
	  else
	    {
	      ...
	      response_payload_slice =
		build_response_payload_slice ("127.0.0.1",
					      ports + (nports / 2),
					      (nports + 1) / 2 /* ceil */ ,
					      ""
	  ...
	  op++;
	  error =
	    grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (203),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      gpr_log (GPR_INFO, "LB Server[%s](%s) shutting down",
	       sf->servers_hostport, sf->balancer_name);
      ...
      op++;
      error =
	grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (204),
			       nullptr);
      GPR_ASSERT (GRPC_CALL_OK == error);
      ...
      cq_verify (cqv);
      gpr_log (GPR_INFO,
	       "LB Server[%s](%s) after tag 204. All done. LB server out",
	       sf->servers_hostport, sf->balancer_name);
      ...
      while (true)
	{
	  ...
	  ev =
	    grpc_completion_queue_next (sf->cq,
					grpc_timeout_seconds_to_deadline (60),
					nullptr);
	  if (!ev.success)
	    {
	      gpr_log (GPR_INFO, "Server[%s] being torn down",
		       sf->servers_hostport);
	      cq_verifier_destroy (cqv);
	      grpc_metadata_array_destroy (&request_metadata_recv);
	      grpc_call_details_destroy (&call_details);
	  ...
	  gpr_log (GPR_INFO, "Server[%s] after tag 100",
		   sf->servers_hostport);
	  ...
	  op++;
	  error =
	    grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (101),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
	  gpr_log (GPR_INFO, "Server[%s] after tag 101",
		   sf->servers_hostport);

	  bool exit = false;
	  grpc_slice response_payload_slice =
	    grpc_slice_from_copied_string (PAYLOAD);
	  while (!exit)
	    {
	      op = ops;
	      op->op = GRPC_OP_RECV_MESSAGE;
		...
		{
		  gpr_log (GPR_INFO,
			   "Server[%s] forced to shutdown. Call #%d",
			   sf->servers_hostport, sf->num_calls_serviced);
		  exit = true;
		}
	      ...
	      if (!exit)
		{
		  ...
		  op++;
		  error =
		    grpc_call_start_batch (s, ops, (size_t) (op - ops),
					   tag (103), nullptr);
		  GPR_ASSERT (GRPC_CALL_OK == error);
		  ev =
		    grpc_completion_queue_next (sf->cq,
	  ...
	  gpr_log (GPR_INFO, "Server[%s] OUT OF THE LOOP",
		   sf->servers_hostport);
	  grpc_slice_unref (response_payload_slice);
	  ...
	  op++;
	  error =
	    grpc_call_start_batch (s, ops, (size_t) (op - ops), tag (104),
				   nullptr);
	  GPR_ASSERT (GRPC_CALL_OK == error);
	...
	grpc_slice_from_static_string ("foo.test.google.fr:1234");
      c =
	grpc_channel_create_call (cf->client, nullptr,
				  GRPC_PROPAGATE_DEFAULTS, cf->cq,
...
#define BALANCERS_NAME "lb.name"
			      ...
			      client_fixture * cf)
    {
      ...
      gpr_asprintf (&expected_target_names, "%s;%s", backends_name,
		    BALANCERS_NAME);
	...
	grpc_fake_resolver_response_generator_create ();
      ...
      grpc_lb_addresses_set_address_from_uri (addresses, 0, lb_uri, true,
					      lb_server->balancer_name,
					      nullptr);
      grpc_uri_destroy (lb_uri);
      gpr_free (lb_uri_str);

      gpr_asprintf (&cf->server_uri, "fake:///%s",
		    lb_server->servers_hostport);
	...
	grpc_channel_args_copy_and_add (nullptr, new_args,
	...
	grpc_fake_transport_security_credentials_create ();
      cf->client =
	grpc_secure_channel_create (fake_creds, cf->server_uri, args,
				    nullptr);
	...
	grpc_fake_transport_security_server_credentials_create ();
      ...
      GPR_ASSERT ((assigned_port =
		   grpc_server_add_secure_http2_port (sf->server,
						      sf->servers_hostport,
						      server_creds)) > 0);
...
#define LB_TOKEN_PREFIX "token"
      ...
      for (int i = 0; i < NUM_BACKENDS; ++i)
	{
	  ...
	  gpr_thd_new (&tf.lb_backends[i].tid, "grpclb_backend",
		       fork_backend_server, &tf.lb_backends[i], &options);
	}

      tf.lb_server.lb_token_prefix = LB_TOKEN_PREFIX;
      tf.lb_server.balancer_name = BALANCERS_NAME;
      setup_server ("127.0.0.1", &tf.lb_server);
      gpr_thd_new (&tf.lb_server.tid, "grpclb_server", fork_lb_server,
		   &tf.lb_server, &options);
...
// Batch 2 is sent after a delay of \a lb_server_update_delay_ms milliseconds.
    ...
    TEST (GrpclbTest, Updates)
    {
      ...
      GPR_ASSERT (tf_result.lb_backends[0].num_calls_serviced +
		  tf_result.lb_backends[1].num_calls_serviced == 1);
      GPR_ASSERT (tf_result.lb_backends[2].num_calls_serviced +
		  tf_result.lb_backends[3].num_calls_serviced > 0);
      ...
      for (int i = 0; i < 4; i++)
	{
      ...
      GPR_ASSERT (tf_result.lb_backends[0].num_calls_serviced +
		  tf_result.lb_backends[1].num_calls_serviced >= 2);
      GPR_ASSERT (tf_result.lb_backends[2].num_calls_serviced +
		  tf_result.lb_backends[3].num_calls_serviced > 0);
