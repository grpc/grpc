...
namespace grpc
{
  namespace
  {
      ...
      else
	{
      ...
      GPR_ASSERT (inet_ntop (af, (void *) pb_ip.bytes, ip_str, 46) !=
		  nullptr);
      return ip_str;
    }
	...
	grpc_grpclb_request_create (service_name.c_str ());
	...
	grpc_grpclb_initial_response_parse (encoded_slice);
      ...
      EXPECT_EQ (c_initial_response->client_stats_report_interval.seconds,
		 123);
	...
	grpc_slice_from_copied_buffer (encoded_response.data (),
	...
	grpc_grpclb_response_parse_serverlist (encoded_slice);
      ...
      EXPECT_STREQ (c_serverlist->servers[0]->load_balance_token,
		    "rate_limting");
      EXPECT_TRUE (c_serverlist->servers[0]->drop);
      EXPECT_TRUE (c_serverlist->servers[1]->has_ip_address);

      EXPECT_EQ (PackedStringToIp (c_serverlist->servers[1]->ip_address),
		 "10.0.0.1");
      EXPECT_EQ (c_serverlist->servers[1]->port, 54321);
      EXPECT_STREQ (c_serverlist->servers[1]->load_balance_token,
		    "load_balancing");
