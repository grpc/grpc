...
bool ProtoReflectionDescriptorDatabase::FindFileByName(const string& filename,
  ...
  if (response.message_response_case() ==
    ...
    if (error.error_code() == StatusCode::NOT_FOUND) {
      gpr_log(GPR_INFO, "NOT_FOUND from server for FindFileByName(%s)", filename.c_str());
    } else {
      gpr_log(GPR_INFO,
              "Error on FindFileByName(%s)\n\tError code: %d\n"
              "\tError Message: %s",
              filename.c_str(), error.error_code(), error.error_message().c_str());
    }
  } else {
    gpr_log(GPR_INFO,
  ...
  if (response.message_response_case() ==
    ...
    if (error.error_code() == StatusCode::NOT_FOUND) {
      ...
      gpr_log(GPR_INFO,
              ...
              "\tError code: %d\n\tError Message: %s",
              symbol_name.c_str(), error.error_code(), error.error_message().c_str());
    }
  } else {
    gpr_log(GPR_INFO,
...
bool ProtoReflectionDescriptorDatabase::FindFileContainingExtension(
    const string& containing_type, int field_number, protobuf::FileDescriptorProto* output) {
  ...
  if (response.message_response_case() ==
    ...
    if (error.error_code() == StatusCode::NOT_FOUND) {
      ...
      gpr_log(GPR_INFO, "NOT_FOUND from server for FindFileContainingExtension(%s, %d)",
...
bool ProtoReflectionDescriptorDatabase::FindAllExtensionNumbers(const string& extendee_type,
  ...
  if (response.message_response_case() ==
      ServerReflectionResponse::MessageResponseCase::kAllExtensionNumbersResponse) {
    ...
    *output = std::vector<int>(number.begin(), number.end());
    ...
    if (error.error_code() == StatusCode::NOT_FOUND) {
      ...
      gpr_log(GPR_INFO,
              ...
              "\tError code: %d\n\tError Message: %s",
              extendee_type.c_str(), error.error_code(), error.error_message().c_str());
    }
  }
  return false;
}
...
bool ProtoReflectionDescriptorDatabase::GetServices(std::vector<std::string>* output) {
  ...
  if (response.message_response_case() ==
    ...
    gpr_log(GPR_INFO, "Error on GetServices() response type\n\tExpecting: %d\n\tReceived: %d",
            ServerReflectionResponse::MessageResponseCase::kListServicesResponse,
            response.message_response_case());
  }
  return false;
...
protobuf::FileDescriptorProto ProtoReflectionDescriptorDatabase::ParseFileDescriptorProtoResponse(
...
bool ProtoReflectionDescriptorDatabase::DoOneRequest(const ServerReflectionRequest& request,
                                                     ServerReflectionResponse& response) {
  bool success = false;
  stream_mutex_.lock();
