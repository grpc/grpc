...
// Match the user input method string to the full_name from method descriptor.
      bool MethodNameMatch (const grpc::string & full_name,
	...
	parser_->LogError (oss.str ());
      ...
      {
	std::
	  cerr << "warning " << filename << " " << line << " " << column <<
	  " " << message << std::endl;
      }
    ...
    ProtoFileParser::ProtoFileParser (std::shared_ptr < grpc::Channel >
				      channel,
				      const grpc::string & proto_path,
				      const grpc::
				      string & protofiles):has_error_ (false),
	...
	{
	  reflection_db_.
	    reset (new grpc::ProtoReflectionDescriptorDatabase (channel));
      ...
      if (!protofiles.empty ())
	{
	  ...
	  importer_.
	    reset (new protobuf::compiler::
		   Importer (&source_tree_, error_printer_.get ()));
		  ...
		  for (int i = 0; i < file_desc->service_count (); i++)
		    {
		      ...
		      known_services.insert (file_desc->service (i)->
					     full_name ());
		    }
		}
	      else
	  ...
	  file_db_.
	    reset (new protobuf::
		   DescriptorPoolDatabase (*importer_->pool ()));
	}
      ...
      else
	{
	  desc_db_.
	    reset (new protobuf::
		   MergedDescriptorDatabase (reflection_db_.get (),
					     file_db_.get ()));
	}
    ...
    grpc::string ProtoFileParser::GetFullMethodName (const grpc::
						     string & method)
    {
      has_error_ = false;
      ...
      if (known_methods_.find (method) != known_methods_.end ())
	{
      ...
      for (auto it = service_desc_list_.begin ();
	   it != service_desc_list_.end (); it++)
	{
    ...
    grpc::string ProtoFileParser::GetFormattedMethodName (const grpc::
							  string & method)
    {
      has_error_ = false;
    ...
    grpc::string ProtoFileParser::GetMessageTypeFromMethod (const grpc::
							    string & method,
							    bool is_request)
    {
      has_error_ = false;
    ...
    bool ProtoFileParser::IsStreaming (const grpc::string & method,
				       bool is_request)
    {
      has_error_ = false;
    ...
    grpc::string ProtoFileParser::GetSerializedProtoFromMethod (const grpc::
								string &
								method,
								const grpc::
								string &
	...
	GetMessageTypeFromMethod (method, is_request);
      if (has_error_)
	{
	  return "";
	}
    ...
    grpc::string ProtoFileParser::GetTextFormatFromMethod (const grpc::
							   string & method,
							   const grpc::
							   string &
							   serialized_proto,
      ...
      if (has_error_)
	{
      ...
      return GetTextFormatFromMessageType (message_type_name,
					   serialized_proto);
    }

    grpc::string ProtoFileParser::
      GetSerializedProtoFromMessageType (const grpc::
      ...
      if (!desc)
	{
      ...
      std::unique_ptr < grpc::protobuf::Message >
	msg (dynamic_factory_->GetPrototype (desc)->New ());
      bool ok =
	protobuf::TextFormat::ParseFromString (text_format_proto, msg.get ());
      if (!ok)
	{
    ...
    grpc::string ProtoFileParser::GetTextFormatFromMessageType (const grpc::
								string &
								message_type_name,
								const grpc::
								string &
