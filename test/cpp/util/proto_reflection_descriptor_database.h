...
class ProtoReflectionDescriptorDatabase : public protobuf::DescriptorDatabase {
 public:
  ...
  explicit ProtoReflectionDescriptorDatabase(const std::shared_ptr<grpc::Channel>& channel);
  ...
  bool FindFileContainingExtension(const string& containing_type, int field_number,
                                   protobuf::FileDescriptorProto* output) override;
 ...
 private:
  typedef ClientReaderWriter<grpc::reflection::v1alpha::ServerReflectionRequest,
  ...
  bool DoOneRequest(const grpc::reflection::v1alpha::ServerReflectionRequest& request,
