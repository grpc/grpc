...
// to feed a DescriptorPool instance.
  ...
  {
  public:
    explicit ProtoReflectionDescriptorDatabase (std::unique_ptr <
						reflection::v1alpha::
						ServerReflection::Stub >
						stub);

    explicit ProtoReflectionDescriptorDatabase (std::shared_ptr <
  ...
  private:
    typedef ClientReaderWriter <
      ...
      grpc::reflection::v1alpha::ServerReflectionResponse > ClientStream;

    const protobuf::
      FileDescriptorProto ParseFileDescriptorProtoResponse (const grpc::
							    string &
							    byte_fd_proto);

    void AddFileFromResponse (const grpc::reflection::v1alpha::
			      FileDescriptorResponse & response);

    const std::shared_ptr < ClientStream > GetStream ();

    bool DoOneRequest (const grpc::reflection::v1alpha::
      ...
      std::unordered_set < int >>missing_extensions_;
      std::unordered_map < string,
      std::vector < int >>cached_extension_numbers_;
