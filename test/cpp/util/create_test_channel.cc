...
namespace grpc
{
  ...
  namespace
  {
	...
	GetChannelCredentials (grpc::ChannelArguments * args) override
      {
...
// Register ssl with non-test roots type to the credentials provider.
    void AddProdSslType ()
    {
      testing::GetCredentialsProvider ()->
	AddSecureType (kProdTlsCredentialsType,
...
//   CreateTestChannel("", "ssl", "test.google.com:443", true, creds);
						 ...
						 args)
  {
    ...
    if (cred_type.empty ())
      {
	return CreateCustomChannel (server, InsecureChannelCredentials (),
				    args);
      }
	...
	if (use_prod_roots)
	  {
	    ...
	    channel_creds =
	      testing::GetCredentialsProvider ()->
	      GetChannelCredentials (kProdTlsCredentialsType, &channel_args);
	...
	else
	  {
	    ...
	    channel_creds =
	      testing::GetCredentialsProvider ()->
	      GetChannelCredentials (testing::kTlsCredentialsType,
				     &channel_args);
	  }
	...
	if (creds.get ())
	  {
	    channel_creds =
	      CompositeChannelCredentials (channel_creds, creds);
	  }
	...
	channel_creds =
	  testing::GetCredentialsProvider ()->
	  GetChannelCredentials (cred_type, &channel_args);
	GPR_ASSERT (channel_creds != nullptr);
...
// Shortcut for end2end and interop tests.
  std::shared_ptr < Channel > CreateTestChannel (const grpc::string & server,
						 bool enable_ssl)
  {
    return CreateTestChannel (server, "foo.test.google.fr", enable_ssl,
			      false);
  }
						 ...
						 CallCredentials > &creds)
  {
    ...
    std::shared_ptr < ChannelCredentials > channel_creds =
      testing::GetCredentialsProvider ()->
      GetChannelCredentials (credential_type, &channel_args);
    GPR_ASSERT (channel_creds != nullptr);
    if (creds.get ())
      {
