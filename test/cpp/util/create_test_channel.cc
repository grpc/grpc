...
ABSL_FLAG(
    std::string, grpc_test_use_grpclb_with_child_policy, "",
...
void AddProdSslType() {
  testing::GetCredentialsProvider()->AddSecureType(
      ...
      std::unique_ptr<testing::CredentialTypeProvider>(
          new SslCredentialProvider));
}

void MaybeSetCustomChannelArgs(grpc::ChannelArguments* args) {
  if (!absl::GetFlag(FLAGS_grpc_test_use_grpclb_with_child_policy)
           .empty()) {
    args->SetString(
        "grpc.service_config",
...
// When cred_type is 'ssl', if server is empty, override_hostname is
// used to create channel. Otherwise, connect to server and override
// hostname if override_hostname is provided. When cred_type is not
// 'ssl', override_hostname is ignored. Set use_prod_root to true to use
// the SSL root for connecting to google. In this case, path to the
// roots pem file must be set via environment variable
...
std::shared_ptr<Channel> CreateTestChannel(
  ...
  return CreateTestChannel(server, "foo.test.google.fr", security_type,
                           false);
}
...
std::shared_ptr<Channel> CreateTestChannel(
  ...
  return ::grpc::CreateCustomChannel(server, channel_creds,
                                     channel_args);
}
...
std::shared_ptr<Channel> CreateTestChannel(
  ...
  if (cred_type.empty()) {
    if (interceptor_creators.empty()) {
      ...
      return experimental::CreateCustomChannelWithInterceptors(
          ...
          std::move(interceptor_creators));
             ...
             testing::kTlsCredentialsType) {  // cred_type == "ssl"
    if (use_prod_roots) {
      gpr_once_init(&g_once_init_add_prod_ssl_provider,
                    &AddProdSslType);
      channel_creds =
          testing::GetCredentialsProvider()->GetChannelCredentials(
    ...
    if (interceptor_creators.empty()) {
      ...
      return experimental::CreateCustomChannelWithInterceptors(
          server, channel_creds, channel_args,
          std::move(interceptor_creators));
    }
  }
}
...
std::shared_ptr<Channel> CreateTestChannel(
  ...
  std::string credential_type =
      security_type == testing::ALTS
          ...
          : (security_type == testing::TLS
                 ? testing::kTlsCredentialsType
