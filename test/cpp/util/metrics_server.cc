...
namespace grpc
{
  namespace testing
  {
    ...
    QpsGauge::QpsGauge ():start_time_ (gpr_now (GPR_CLOCK_REALTIME)),
      num_queries_ (0)
    {
    }
    ...
    std::shared_ptr < QpsGauge >
      MetricsServiceImpl::CreateQpsGauge (const grpc::string & name,
...
// on the returned server instance.
    std::unique_ptr < grpc::Server >
      MetricsServiceImpl::StartServer (int port)
    {
