...
CliCall::CliCall(const std::shared_ptr<grpc::Channel>& channel,
  ...
  call_->StartCall(CoreTestFixture::tag(1));
  void* got_tag;
  bool ok;
  cq_.Next(&got_tag, &ok);
  GPR_ASSERT(ok);
...
void CliCall::Write(const std::string& request) {
  ...
  call_->Write(send_buffer, CoreTestFixture::tag(2));
  cq_.Next(&got_tag, &ok);
  GPR_ASSERT(ok);
}
...
bool CliCall::Read(std::string* response,
  ...
  call_->Read(&recv_buffer, CoreTestFixture::tag(3));
...
void CliCall::WritesDone() {
  ...
  call_->WritesDone(CoreTestFixture::tag(4));
  cq_.Next(&got_tag, &ok);
  GPR_ASSERT(ok);
}
...
bool CliCall::ReadAndMaybeNotifyWrite(
  ...
  while (got_tag != CoreTestFixture::tag(3)) {
    gpr_mu_lock(&write_mu_);
    write_done_ = true;
    ...
    if (got_tag == CoreTestFixture::tag(2)) {
      GPR_ASSERT(ok);
    }
  }
  ...
  if (!cq_result || !ok) {
    ...
    if (!ok) {
      ...
      if (!write_done_) {
        cq_.Next(&got_tag, &ok);
        GPR_ASSERT(got_tag != CoreTestFixture::tag(2));
        write_done_ = true;
        gpr_cv_signal(&write_cv_);
      }
      gpr_mu_unlock(&write_mu_);
...
Status CliCall::Finish(IncomingMetadataContainer* server_trailing_metadata) {
  ...
  call_->Finish(&status, CoreTestFixture::tag(5));
  cq_.Next(&got_tag, &ok);
  GPR_ASSERT(ok);
