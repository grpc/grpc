...
namespace grpc
{
  ...
  CliCall::CliCall (std::shared_ptr < grpc::Channel > channel, const grpc::string & method, const OutgoingMetadataContainer & metadata):stub_ (new grpc::
	   GenericStub
	   (channel))
    {
      ...
      if (!metadata.empty ())
	{
	  for (OutgoingMetadataContainer::const_iterator iter =
	       metadata.begin (); iter != metadata.end (); ++iter)
	    {
	...
	{
	  response->append (reinterpret_cast <
			    const char *>(slices[i].begin ()),
			    slices[i].size ());
	}
    ...
    bool CliCall::ReadAndMaybeNotifyWrite (grpc::string * response,
					   IncomingMetadataContainer *
    ...
    Status CliCall::Finish (IncomingMetadataContainer *
			    server_trailing_metadata)
    {
      void *got_tag;
      bool ok;
