...
namespace grpc
{
  namespace testing
  {
	...
	bool CallMethod (int argc, const char **argv,
			 const CliCredentials & cred,
			 GrpcToolOutputCallback callback);
	bool ListServices (int argc, const char **argv,
			   const CliCredentials & cred,
			   GrpcToolOutputCallback callback);
	bool PrintType (int argc, const char **argv,
			const CliCredentials & cred,
			GrpcToolOutputCallback callback);
	...
	// callback);
	bool ParseMessage (int argc, const char **argv,
			   const CliCredentials & cred,
			   GrpcToolOutputCallback callback);
	bool ToText (int argc, const char **argv, const CliCredentials & cred,
		     GrpcToolOutputCallback callback);
	bool ToBinary (int argc, const char **argv,
		       const CliCredentials & cred,
		       GrpcToolOutputCallback callback);
	...
	const grpc::string cred_usage_;
      ...
      template < typename T >
	std::function < bool (GrpcTool *, int, const char **,
			      const CliCredentials &,
			      GrpcToolOutputCallback) > BindWith5Args (T
								       &&
								       func)
      {
	return std::bind (std::forward < T > (func), std::placeholders::_1,
      ...
      template < typename T > size_t ArraySize (T & a)
      {
	return ((sizeof (a) / sizeof (*(a))) /
		static_cast < size_t > (!(sizeof (a) % sizeof (*(a)))));
      }

      void ParseMetadataFlag (std::multimap < grpc::string,
			      grpc::string > *client_metadata)
      {
	...
	for (size_t i = 0; i < fields.size (); i += 2)
	  {
	    client_metadata->insert (std::pair < grpc::string,
				     grpc::string > (fields[i],
						     fields[i + 1]));
	  }
      }
	...
	fprintf (stderr, "%s\n", message.c_str ());
	grpc::string pair;
	for (typename T::const_iterator iter = m.begin (); iter != m.end ();
	     ++iter)
	  {
	    pair.clear ();
      ...
      void ReadResponse (CliCall * call, const grpc::string & method_name,
			 GrpcToolOutputCallback callback,
			 ProtoFileParser * parser, gpr_mu * parser_mu,
			 bool print_mode)
      {
	grpc::string serialized_response_proto;
	std::multimap < grpc::string_ref,
	  grpc::string_ref > server_initial_metadata;
	...
	const char *command;
	  std::function < bool (GrpcTool *, int, const char **,
				const CliCredentials &,
				GrpcToolOutputCallback) > function;
	int min_args;
	int max_args;
      };
      ...
      if (cmd != nullptr)
	{
	  ...
	  const bool ok =
	    cmd->function (&grpc_tool, argc, argv, cred, callback);
	  return ok ? 0 : 1;
	}
      else
	{
  ...
  GrpcTool::GrpcTool ():print_command_usage_ (false),
      usage_exit_status_ (0)
    {
    }
      ...
      if (argc < 2)
	{
	  ...
	  if (FLAGS_l)
	    ...
	    {
	      for (auto it = service_list.begin (); it != service_list.end ();
		   it++)
		{
      ...
      else
	{
	  ...
	  if (service != nullptr)
		...
		{
		  output =
		    FLAGS_l ? DescribeService (service) :
	...
	grpc::CreateChannel (server_address, cred.GetCredentials ());
      ...
      parser.
	reset (new grpc::testing::
	       ProtoFileParser (FLAGS_remotedb ? channel : nullptr,
      ...
      else
	{
	  formatted_method_name =
	    parser->GetFormattedMethodName (method_name);
	}
      ...
      if (parser->IsStreaming (method_name, true /* is_request */ ))
	{
	  ...
	  while (!request_text.empty () ||
	      ...
	      if (!request_text.empty ())
		{
		  if (FLAGS_binary_input)
			...
			parser->GetSerializedProtoFromMethod (method_name,
							      request_text,
							      true
							      /* is_request */
							      );
	  ...
	  std::multimap < grpc::string_ref,
	    grpc::string_ref > server_trailing_metadata;
	...
	{			// parser->IsStreaming(method_name, true /* is_request */)
		     ...
		     (!input_stream->eof () && getline (*input_stream, line)))
		{
		  if (!request_text.empty ())
			...
			server_initial_metadata, server_trailing_metadata;
		      CliCall call (channel, formatted_method_name,
				    client_metadata);
	  ...
	  if (argc == 3)
		...
		{
		  std::ifstream input_file (FLAGS_infile,
					    std::ios::in | std::ios::binary);
	  ...
	  for (bool receive_initial_metadata = true;
	       call.Read (&serialized_response_proto,
			  receive_initial_metadata ? &server_initial_metadata
			  : nullptr); receive_initial_metadata = false)
	    {
	      if (!FLAGS_binary_output)
		{
		  serialized_response_proto =
	...
	{
	  serialized_request_proto =
	    parser->GetSerializedProtoFromMessageType (type_name,
						       message_text);
	  if (parser->HasError ())
	    {
	      return false;
      ...
      else
	{
	  grpc::string output_text =
	    parser->GetTextFormatFromMessageType (type_name,
						  serialized_request_proto);
	  if (parser->HasError ())
	    {
	      return false;
    ...
    bool GrpcTool::ToText (int argc, const char **argv,
			   const CliCredentials & cred,
			   GrpcToolOutputCallback callback)
    {
      CommandUsage ("Convert binary message to text\n"
    ...
    bool GrpcTool::ToBinary (int argc, const char **argv,
			     const CliCredentials & cred,
			     GrpcToolOutputCallback callback)
    {
      CommandUsage ("Convert text message to binary\n"
