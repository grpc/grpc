...
// Send a rpc with a normal stub and then a CliCall. Verify they match.
	...
	s = stub_->Echo (&context, request, &response);
      ...
      const
	grpc::string
      kMethod ("/grpc.testing.EchoTestService/Echo");
	...
	grpc::string_ref > server_initial_metadata, server_trailing_metadata;
      client_metadata.insert (std::pair < grpc::string,
			      grpc::string > ("key1", "val1"));
      Status
	s2 = CliCall::Call (channel_, kMethod, request_bin, &response_bin,
