...
#define ECHO_TEST_SERVICE_DESCRIPTION                                  \
  ...
  "  rpc Echo(grpc.testing.EchoRequest) returns "                      \
  "(grpc.testing.EchoResponse) "                                       \
  "{}\n"                                                               \
  "  rpc Echo1(grpc.testing.EchoRequest) returns "                     \
  "(grpc.testing.EchoResponse) "                                       \
  "{}\n"                                                               \
  "  rpc Echo2(grpc.testing.EchoRequest) returns "                     \
  "(grpc.testing.EchoResponse) "                                       \
  "{}\n"                                                               \
  ...
  "(grpc.testing.StringValue) {}\n"                                    \
  "  rpc CheckClientInitialMetadata(grpc.testing.SimpleRequest) "      \
  "returns "                                                           \
  ...
  "(grpc.testing.EchoResponse) {}\n"                                   \
  "  rpc UnimplementedBidi(stream grpc.testing.EchoRequest) returns "  \
  "(stream "                                                           \
  "grpc.testing.EchoResponse) {}\n"                                    \
  "}\n"                                                                \
  "\n"
...
class TestServiceImpl
 ...
 public:
  ...
  // If deadline set, timeout + current time should be an upper bound
  // for it
  Status CheckDeadlineUpperBound(ServerContext* context,
    ...
    // Returning string instead of bool to avoid using embedded messages
    // in proto3
    response->set_message(
        seconds.count() <= absl::GetFlag(FLAGS_timeout) ? "true"
                                                        : "false");
    return Status::OK;
  }
...
class GrpcToolTest : public ::testing::Test {
 protected:
  ...
  // SetUpServer cannot be used with EXPECT_EXIT.
  // grpc_pick_unused_port_or_die() uses atexit() to free chosen ports,
  // and it will spawn a new thread in resolve_address_posix.c:192 at
  // exit time.
...
TEST_F(GrpcToolTest, ListCommand) {
  ...
  EXPECT_TRUE(
      0 == GrpcToolMainLib(ArraySize(argv), argv, TestCliCredentials(),
                           std::bind(PrintStream, &output_stream,
                                     std::placeholders::_1)));
  EXPECT_TRUE(0 ==
              strcmp(output_stream.str().c_str(),
...
TEST_F(GrpcToolTest, TypeCommand) {
  // Test input "grpc_cli type localhost:<port>
  ...
  const grpc::protobuf::Descriptor* desc =
      grpc::protobuf::DescriptorPool::generated_pool()
          ->FindMessageTypeByName("grpc.testing.EchoRequest");
...
TEST_F(GrpcToolTest, TypeNotFound) {
  ...
  EXPECT_TRUE(
      1 == GrpcToolMainLib(ArraySize(argv), argv, TestCliCredentials(),
                           std::bind(PrintStream, &output_stream,
                                     std::placeholders::_1)));
  ShutdownServer();
}
...
TEST_F(GrpcToolTest, CallCommandJsonInput) {
  // Test input "grpc_cli call localhost:<port> Echo "{ \"message\":
  // \"Hello\"}"
  std::stringstream output_stream;
...
TEST_F(GrpcToolTest, CallCommandRequestStreamJsonInput) {
  // Test input: grpc_cli call localhost:<port> RequestStream "{
  // \"message\":
  // \"Hello0\"}"
  std::stringstream output_stream;
...
TEST_F(GrpcToolTest, CallCommandBidiStream) {
  // Test input: grpc_cli call localhost:<port> BidiStream "message:
  // 'Hello0'"
  std::stringstream output_stream;
...
TEST_F(GrpcToolTest, ParseCommand) {
  // Test input "grpc_cli parse localhost:<port>
  ...
  // Parse text message to binary message and then parse it back to text
  // message
  output_stream.str(std::string());
  output_stream.clear();
  ...
  absl::SetFlag(&FLAGS_binary_output, false);
  EXPECT_TRUE(0 ==
              GrpcToolMainLib(5, argv, TestCliCredentials(),
                              std::bind(PrintStream, &output_stream,
                                        std::placeholders::_1)));
...
TEST_F(GrpcToolTest, TooManyArguments) {
  // Test input "grpc_cli call localhost:<port> Echo Echo "message:
  // 'Hello'"
  std::stringstream output_stream;
...
TEST_F(GrpcToolTest, ConfiguringDefaultServiceConfig) {
  ...
  const std::string server_address = SetUpServer();
  const char* argv[] = {"grpc_cli", "ls", server_address.c_str()};
  // Just check that the tool is still operational when
  // --default_service_config is configured. This particular service
  // config is in reality redundant with the channel's default
  // configuration.
  absl::SetFlag(&FLAGS_l, false);
