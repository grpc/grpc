  ...
  grpc
{
    ...
    testing
  {
    ...
    namespace
    {
      ...
      const int
	kServerDefaultResponseStreamsToSend = 3;
	  ...
	  override
	{
      ...
      bool
      PrintStream (std::stringstream * ss, const grpc::string & output)
      {
	(*ss) << output;
	return
	  true;
	  ...
	  override
	{
	  ...
	  context->
	  AddTrailingMetadata ("trailing_key", "trailing_value");
	  response->set_message (request->message ());
	  return Status::OK;
	}

	Status
	RequestStream (ServerContext * context,
	...
	Status
	ResponseStream (ServerContext * context, const EchoRequest * request,
	...
	server_->Shutdown ();
      ...
      void
      ExitWhenError (int argc, const char **argv, const CliCredentials & cred,
		     GrpcToolOutputCallback callback)
      {
	int
	  result = GrpcToolMainLib (argc, argv, cred, callback);
	if (result)
	  {
	    exit (result);
	  }
    ...
    TEST_F (GrpcToolTest, NoCommand)
    {
      ...
      EXPECT_EXIT (GrpcToolMainLib
		   (ArraySize (argv), argv, TestCliCredentials (),
		    std::bind (PrintStream, &output_stream,
			       std::placeholders::_1)),::testing::
		   ExitedWithCode (1), "No command specified\n" USAGE_REGEX);
      // No output
    ...
    TEST_F (GrpcToolTest, InvalidCommand)
    {
      ...
      EXPECT_EXIT (GrpcToolMainLib
		   (ArraySize (argv), argv, TestCliCredentials (),
		    std::bind (PrintStream, &output_stream,
			       std::placeholders::_1)),::testing::
		   ExitedWithCode (1), "Invalid command 'abc'\n" USAGE_REGEX);
      // No output
      EXPECT_TRUE (0 == output_stream.tellp ());
    }
    ...
    TEST_F (GrpcToolTest, ListCommand)
    {
      ...
      const
	grpc::string
	server_address = SetUpServer ();
      const char *
      argv[] = { "grpc_cli", "ls", server_address.c_str () };

      FLAGS_l = false;
      EXPECT_TRUE (0 ==
		   GrpcToolMainLib (ArraySize (argv), argv,
				    TestCliCredentials (),
				    std::bind (PrintStream, &output_stream,
					       std::placeholders::_1)));
      EXPECT_TRUE (0 ==
	...
	server_address = SetUpServer ();
      const char *
	argv[] = { "grpc_cli", "type", server_address.c_str (),
	"grpc.testing.EchoRequest"
      };
	...
	server_address = SetUpServer ();
      const char *
	argv[] = { "grpc_cli", "call", server_address.c_str (), "Echo",
	"message: 'Hello'"
      };
	...
	server_address = SetUpServer ();
      const char *
	argv[] = { "grpc_cli", "call", server_address.c_str (),
	...
	server_address = SetUpServer ();
      const char *
	argv[] = { "grpc_cli", "parse", server_address.c_str (),
    ...
    TEST_F (GrpcToolTest, TooManyArguments)
    {
      ...
      std::stringstream output_stream;
      const char *
	argv[] = { "grpc_cli", "call", "localhost:10000",
    ...
    TEST_F (GrpcToolTest, CallCommandWithBadMetadata)
    {
      ...
      const char *
	argv[] = { "grpc_cli", "call", "localhost:10000", "Echo",
	"message: 'Hello'"
      };
	...
	EXPECT_EXIT (GrpcToolMainLib
		     (ArraySize (argv), argv, TestCliCredentials (),
		      std::bind (PrintStream, &output_stream,
				 std::placeholders::_1)),::testing::
		     ExitedWithCode (1), ".*Failed to parse metadata flag.*");
      }
