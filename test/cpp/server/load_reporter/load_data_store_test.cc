...
class LoadDataStoreTest : public ::testing::Test {
 public:
  ...
  // Check whether per_balancer_stores contains a store which was
  // originally created for <hostname, lb_id, and load_key>.
...
TEST_F(LoadDataStoreTest, AssignToSelf) {
  ...
  EXPECT_TRUE(PerBalancerStoresContains(
      load_data_store, assigned_stores, kHostname1, kLbId1, kLoadKey1));
}
...
TEST_F(LoadDataStoreTest, ReassignOrphanStores) {
  ...
  EXPECT_TRUE(PerBalancerStoresContains(load_data_store,
                                        assigned_to_lb_id_1, kHostname1,
                                        kLbId1, kLoadKey1));
  EXPECT_TRUE(PerBalancerStoresContains(load_data_store,
                                        assigned_to_lb_id_1, kHostname1,
                                        kLbId2, kLoadKey1));
  ...
  EXPECT_TRUE(PerBalancerStoresContains(load_data_store,
                                        assigned_to_lb_id_3, kHostname1,
                                        kLbId1, kLoadKey1));
  EXPECT_TRUE(PerBalancerStoresContains(load_data_store,
                                        assigned_to_lb_id_3, kHostname1,
                                        kLbId2, kLoadKey1));
  ...
  EXPECT_FALSE(
      PerBalancerStoresContains(load_data_store, assigned_to_lb_id_4,
  ...
  EXPECT_TRUE(PerBalancerStoresContains(load_data_store,
                                        assigned_to_lb_id_4, kHostname2,
                                        kLbId4, kLoadKey1));
}
...
TEST_F(LoadDataStoreTest, OrphanAssignmentIsSticky) {
  ...
  for (size_t i = 0; i < num_lb_ids; ++i) {
    load_data_store.ReportStreamCreated(kHostname1, FormatLbId(i),
                                        kLoadKey1);
  ...
  for (const auto& lb_id : active_lb_ids) {
    if (PerBalancerStoresContains(
            load_data_store,
            load_data_store.GetAssignedStores(kHostname1, lb_id),
  ...
  // Close 10 more stream, skipping the assigned_lb_id. The assignment
  // of orphaned_lb_id shouldn't change.
...
TEST_F(LoadDataStoreTest, HostTemporarilyLoseAllStreams) {
  ...
  auto store_lb_id_1 =
      load_data_store.FindPerBalancerStore(kHostname1, kLbId1);
  ...
  // Detailed load data won't be kept when the PerBalancerStore is
  // suspended.
...
TEST_F(LoadDataStoreTest, OneStorePerLbId) {
  LoadDataStore load_data_store;
  EXPECT_EQ(load_data_store.FindPerBalancerStore(kHostname1, kLbId1),
            nullptr);
  EXPECT_EQ(
      load_data_store.FindPerBalancerStore(kHostname1, kInvalidLbId),
      nullptr);
  EXPECT_EQ(load_data_store.FindPerBalancerStore(kHostname2, kLbId2),
            nullptr);
  EXPECT_EQ(load_data_store.FindPerBalancerStore(kHostname2, kLbId3),
            nullptr);
  ...
  auto store_invalid_lb_id_1 =
      load_data_store.FindPerBalancerStore(kHostname1, kInvalidLbId);
  // Two stores will be created: one is for the stream; the other one is
  // for kInvalidLbId.
  ...
  auto store_lb_id_3 =
      load_data_store.FindPerBalancerStore(kHostname2, kLbId3);
...
TEST_F(LoadDataStoreTest, ExactlyOnceAssignment) {
  ...
  for (size_t i = num_close; i < num_create; ++i) {
    for (auto assigned_store : *load_data_store.GetAssignedStores(
             kHostname1, FormatLbId(i))) {
      EXPECT_TRUE(
          reported_lb_ids.insert(assigned_store->lb_id()).second);
    }
  }
...
TEST_F(LoadDataStoreTest, UnknownBalancerIdTracking) {
  ...
  load_data_store.MergeRow(
      kHostname1, LoadRecordKey(kLbId2, kLbTag1, kUser1, kClientIp1),
      v2);
  EXPECT_TRUE(load_data_store.IsTrackedUnknownBalancerId(kLbId2));
  LoadRecordValue v3(952);
  load_data_store.MergeRow(
      kHostname2, LoadRecordKey(kLbId3, kLbTag1, kUser1, kClientIp1),
      v3);
  ...
  auto store_lb_id_1 =
      load_data_store.FindPerBalancerStore(kHostname1, kLbId1);
  EXPECT_EQ(store_lb_id_1->load_record_map().size(), 1U);
  EXPECT_EQ(store_lb_id_1->load_record_map()
                .find(kKey1)
                ->second.start_count(),
            v1.start_count());
  ...
  EXPECT_EQ(load_data_store.FindPerBalancerStore(kHostname1, kLbId2),
            nullptr);
  EXPECT_EQ(load_data_store.FindPerBalancerStore(kHostname2, kLbId3),
            nullptr);
  ...
  EXPECT_EQ(store_lb_id_1->load_record_map()
            ...
            v1.start_count());
  EXPECT_EQ(
      store_lb_id_1->load_record_map().find(kKey1)->second.ok_count(),
      v4.ok_count());
...
TEST_F(PerBalancerStoreTest, DataAggregation) {
  ...
  EXPECT_FALSE(
      per_balancer_store.IsNumCallsInProgressChangedSinceLastReport());
  EXPECT_EQ(per_balancer_store.GetNumCallsInProgressForReport(),
  ...
  EXPECT_EQ(value_for_key1.start_count(),
            v1.start_count() + v3.start_count());
  EXPECT_EQ(value_for_key1.ok_count(), v1.ok_count() + v3.ok_count());
  EXPECT_EQ(value_for_key1.error_count(),
            v1.error_count() + v3.error_count());
  EXPECT_EQ(value_for_key1.bytes_sent(),
            v1.bytes_sent() + v3.bytes_sent());
  EXPECT_EQ(value_for_key1.bytes_recv(),
            v1.bytes_recv() + v3.bytes_recv());
  EXPECT_EQ(value_for_key1.latency_ms(),
            v1.latency_ms() + v3.latency_ms());
  EXPECT_EQ(value_for_key1.call_metrics().size(), 2U);
  EXPECT_EQ(
      value_for_key1.call_metrics().find(kMetric1)->second.num_calls(),
      v1.call_metrics().find(kMetric1)->second.num_calls() +
          v3.call_metrics().find(kMetric1)->second.num_calls());
  EXPECT_EQ(
      value_for_key1.call_metrics()
          .find(kMetric1)
          ->second.total_metric_value(),
      v1.call_metrics().find(kMetric1)->second.total_metric_value() +
          v3.call_metrics()
              .find(kMetric1)
              ->second.total_metric_value());
  EXPECT_EQ(
      value_for_key1.call_metrics().find(kMetric2)->second.num_calls(),
  ...
  EXPECT_EQ(
      value_for_key2.call_metrics().find(kMetric1)->second.num_calls(),
      v2.call_metrics().find(kMetric1)->second.num_calls());
  EXPECT_EQ(
      value_for_key2.call_metrics()
          .find(kMetric1)
          ->second.total_metric_value(),
      v2.call_metrics().find(kMetric1)->second.total_metric_value());
  EXPECT_EQ(
      value_for_key2.call_metrics().find(kMetric2)->second.num_calls(),
