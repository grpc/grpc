...
namespace grpc
{
  namespace testing
  {
	...
	return false;
      ...
      {
	register_service_ = true;
      }

      bool init_server_is_called ()
      {
	return init_server_is_called_;
      }
      bool finish_is_called ()
      {
	return finish_is_called_;
      }
	...
	std::unique_ptr < TestServerBuilderPlugin >
	  plugin (new TestServerBuilderPlugin ());
	if (register_service_)
	...
	return;
      already_here = true;
      ::grpc::ServerBuilder::
	InternalAddPluginFactory (&CreateTestServerBuilderPlugin);
    }
...
// be added using ServerBuilder::SetOption().
	...
	if (GetParam ())
	  ...
	  {
	    // Add ServerBuilder plugin using ServerBuilder::SetOption()
	    std::unique_ptr < InsertPluginServerBuilderOption >
	      option (new InsertPluginServerBuilderOption ());
    ...
    INSTANTIATE_TEST_CASE_P (ServerBuilderPluginTest,
			     ServerBuilderPluginTest,::testing::Values (false,
									true));
