...
static gpr_timespec now_impl(gpr_clock_type clock) {
  ...
  ABSL_CHECK_GE(ts.tv_nsec, 0);
  ABSL_CHECK_LT(ts.tv_nsec, GPR_NS_PER_SEC);
  gpr_mu_lock(&g_mu);
...
TEST(TimespecTest, GrpcNegativeMillisToTimespec) {
  ...
  ABSL_CHECK(ts.tv_sec = -2);
  ABSL_CHECK(ts.tv_nsec = 5e8);
  ABSL_CHECK_EQ(ts.clock_type, GPR_CLOCK_MONOTONIC);
}
