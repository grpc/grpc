  ...
  grpc
{
    ...
    testing
  {
    namespace
    {
      ...
      bool
      CheckIsLocalhost (const grpc::string & addr)
      {
	const
	  grpc::string
	kIpv6 ("ipv6:[::1]:");
	const
	  grpc::string
	kIpv4MappedIpv6 ("ipv6:[::ffff:127.0.0.1]:");
	const
	  grpc::string
	kIpv4 ("ipv4:127.0.0.1:");
	return
	  addr.
	substr (0, kIpv4.size ()) == kIpv4 ||
	  addr.substr (0, kIpv4MappedIpv6.size ()) == kIpv4MappedIpv6 ||
	  addr.substr (0, kIpv6.size ()) == kIpv6;
      }

      const char
	kTestCredsPluginErrorMsg[] = "Could not find plugin metadata.";
	...
	Status
	GetMetadata (grpc::string_ref service_url,
	  ...
	  override
	{
	  ...
	  if (is_successful_)
	    {
	      metadata->
		insert (std::make_pair (metadata_key_, metadata_value_));
	      return
		Status::OK;
	    }
	  else
	...
	TestAuthMetadataProcessor (bool is_blocking):
	is_blocking_ (is_blocking)
	{
	}
	...
	std::shared_ptr <
	  CallCredentials >
	GetCompatibleClientCreds ()
	{
	  return MetadataCredentialsFromPlugin (std::unique_ptr <
						MetadataCredentialsPlugin >
						(new
						 TestMetadataCredentialsPlugin
						 (TestMetadataCredentialsPlugin::
	...
	Status
	Process (const InputMetadata & auth_metadata, AuthContext * context,
	  ...
	  override
	{
	  ...
	  auto
	    auth_md =
	    auth_metadata.
	    find (TestMetadataCredentialsPlugin::kGoodMetadataKey);
	  EXPECT_NE (auth_md, auth_metadata.end ());
	  string_ref
	    auth_md_value = auth_md->second;
	  if (auth_md_value == kGoodGuy)
		...
		insert (std::
			make_pair (string
				   (auth_md->first.data (),
				    auth_md->first.length ()),
				   string (auth_md->second.data (),
					   auth_md->second.length ())));
	      return Status::OK;
	    }
	  else
	    {
	      return Status (StatusCode::UNAUTHENTICATED,
			     ...
			     string (auth_md_value.data (),
				     auth_md_value.length ()));
	    }
	}
	...
	bool
      ...
      const char
	TestAuthMetadataProcessor::kGoodGuy[] = "Dr Jekyll";
      const char
	TestAuthMetadataProcessor::kIdentityPropName[] = "novel identity";
	...
	Status
	Echo (ServerContext * server_context, const EchoRequest * request,
	...
	TestScenario
      {
      public:
	TestScenario (bool proxy, bool inproc_stub,
	...
	void
	StartServer (const std::shared_ptr < AuthMetadataProcessor >
		     &processor)
	{
	  int
	...
	void
	RestartServer (const std::shared_ptr < AuthMetadataProcessor >
		       &processor)
	{
	  if (is_server_started_)
	    {
	...
	void
	BuildAndStartServer (const std::shared_ptr < AuthMetadataProcessor >
			     &processor)
	{
	  ServerBuilder
	    builder;
	  ...
	  builder.
	    SetSyncServerOption (ServerBuilder::SyncServerOption::NUM_CQS, 4);
	  builder.
	    SetSyncServerOption (ServerBuilder::SyncServerOption::
				 CQ_TIMEOUT_MSEC, 10);
	...
	virtual void
	ConfigureServerBuilder (ServerBuilder * builder)
	{
	...
	ResetChannel ()
	{
	  ...
	  if (!GetParam ().inproc)
	    {
	      channel_ =
		CreateCustomChannel (server_address_.str (), channel_creds,
				     args);
	    }
	  else
	    {
	...
	ResetStub ()
	{
	  ...
	  if (GetParam ().use_proxy)
		...
		builder;
	      ...
	      builder.
		SetSyncServerOption (ServerBuilder::SyncServerOption::NUM_CQS,
				     4);
	      builder.
		SetSyncServerOption (ServerBuilder::SyncServerOption::
				     CQ_TIMEOUT_MSEC, 10);
	...
	int
      ...
      static void
      SendRpc (grpc::testing::EchoTestService::Stub * stub, int num_rpcs,
...
// by calling ServerContext::TryCancel()
	...
	void
	TestRequestStreamServerCancel (ServerTryCancelRequestPhase
				       server_try_cancel,
				       int num_msgs_to_send)
	{
	  ...
	  context.
	  AddMetadata (kServerTryCancelRequest,
		       grpc::to_string (server_try_cancel));

	  auto
	    stream = stub_->RequestStream (&context, &response);
	...
	// NOTE: Do not call this function with server_try_cancel == DO_NOT_CANCEL.
	void
	TestResponseStreamServerCancel (ServerTryCancelRequestPhase
					server_try_cancel)
	{
	  ResetStub ();
	  EchoRequest
	  ...
	  request.set_message ("hello");
	  auto
	    stream = stub_->ResponseStream (&context, request);
	...
	// NOTE: Do not call this function with server_try_cancel == DO_NOT_CANCEL.
	void
	TestBidiStreamServerCancel (ServerTryCancelRequestPhase
				    server_try_cancel, int num_messages)
	{
	  ResetStub ();
	  EchoRequest
	    request;
...
// but before returning to the client
	...
	const
	  auto &
	  trailing_metadata = context.GetServerTrailingMetadata ();
	auto
	  iter = trailing_metadata.find ("user-agent");
	EXPECT_TRUE (iter != trailing_metadata.end ());
	grpc::string expected_prefix = user_agent_prefix_ + " grpc-c++/";
	EXPECT_TRUE (iter->second.starts_with (expected_prefix)) << iter->
	  second;
      }
	...
	request.set_message ("hello");
	EXPECT_TRUE (stream->
		     Write (request, WriteOptions ().set_write_through ()));
	EXPECT_TRUE (stream->
		     Write (request, WriteOptions ().set_write_through ()));
	stream->WritesDone ();
	Status
	...
	for (int i = 0; i < kServerDefaultResponseStreamsToSend; ++i)
	  {
	    ...
	    EXPECT_EQ (response.message (),
		       request.message () + grpc::to_string (i));
	  }
...
// The two stubs are created on the same channel.
	...
	std::unique_ptr < grpc::testing::duplicate::EchoTestService::Stub >
	  dup_pkg_stub (grpc::testing::duplicate::EchoTestService::
			NewStub (channel_));
	ClientContext
	  context2;
	...
	EXPECT_TRUE (s.ok ());
      ...
      void
      CancelRpc (ClientContext * context, int delay_us,
...
// reading 10 requests.
	...
	EXPECT_EQ (s.error_code (), StatusCode::CANCELLED);
      ...
      void
      ReaderThreadFunc (ClientReaderWriter < EchoRequest,
			EchoResponse > *stream, gpr_event * ev)
      {
	EchoResponse
	  resp;
...
// Takes 10s.
	...
	auto
	  channel =
	  CreateChannel (server_address.str (),
			 InsecureChannelCredentials ());
	// Start IDLE
...
// Talking to a non-existing service.
	...
	ClientContext
	  context;
	Status
	  s = stub->Unimplemented (&context, request, &response);
...
// This is an example of setting error details.
	...
	request.mutable_param ()->set_echo_metadata (true);
	DebugInfo *
	  info = request.mutable_param ()->mutable_debug_info ();
	...
	EXPECT_FALSE (s.ok ());
	auto
	  trailers = context.GetServerTrailingMetadata ();
	EXPECT_EQ (1u, trailers.count (kDebugInfoTrailerKey));
	auto
	  iter = trailers.find (kDebugInfoTrailerKey);
	...
	expected_status.back ().
	  set_binary_error_details ("text error details");
	expected_status.emplace_back ();
	expected_status.back ().set_code (13);	// INTERNAL
	expected_status.back ().set_error_message ("text error message");
	expected_status.back ().
	  set_binary_error_details ("\x0\x1\x2\x3\x4\x5\x6\x8\x9\xA\xB");
	...
	for (auto iter = expected_status.begin ();
	     iter != expected_status.end (); ++iter)
	  {
	    ...
	    request.set_message ("Hello");
	    auto *
	      error = request.mutable_param ()->mutable_expected_error ();
	    ...
	    EXPECT_TRUE (context.debug_error_string ().find ("created") !=
			 std::string::npos);
	    EXPECT_TRUE (context.debug_error_string ().find ("file") !=
			 std::string::npos);
	    EXPECT_TRUE (context.debug_error_string ().find ("line") !=
			 std::string::npos);
	    EXPECT_TRUE (context.debug_error_string ().find ("status") !=
			 std::string::npos);
	    EXPECT_TRUE (context.debug_error_string ().find ("13") !=
			 std::string::npos);
	  }
      }
...
// Ask server to echo back the deadline it sees.
	...
	EXPECT_LE (response.param ().request_deadline () -
		   sent_deadline.tv_sec, 1);
	EXPECT_GE (response.param ().request_deadline () -
		   sent_deadline.tv_sec, -1);
      }
...
// Ask server to echo back the deadline it sees. The rpc has no deadline.
	...
	ClientContext
	  context;
	Status
	  s = stub_->Unimplemented (&context, request, &response);
	EXPECT_FALSE (s.ok ());
...
//////////////////////////////////////////////////////////////////////////
	...
	SecureEnd2endTest ()
	{
	  ...
	  GPR_ASSERT (GetParam ().credentials_type !=
	...
	EXPECT_TRUE (s.ok ());
      ...
      bool
      MetadataContains (const std::multimap < grpc::string_ref,
	...
	for (std::multimap < grpc::string_ref,
	     grpc::string_ref >::const_iterator iter = metadata.begin ();
	...
	return count == 1;
      ...
      {
	auto *
	  processor = new TestAuthMetadataProcessor (true);
	...
	request.mutable_param ()->
	  set_expected_client_identity (TestAuthMetadataProcessor::kGoodGuy);
	request.mutable_param ()->
	  set_expected_transport_security_type (GetParam ().credentials_type);
	  ...
	  set_credentials (MetadataCredentialsFromPlugin
			   (std::unique_ptr < MetadataCredentialsPlugin >
			    (new
			     TestMetadataCredentialsPlugin
			     (TestMetadataCredentialsPlugin::kBadMetadataKey,
			      "Does not matter, will fail the key is invalid.",
			      false, true))));
	request.set_message ("Hello");
	...
	EXPECT_EQ (s.error_message (),
		   grpc::
		   string ("Getting metadata from plugin failed with error: ")
      ...
      {
	auto *
	  processor = new TestAuthMetadataProcessor (false);
	StartServer (std::shared_ptr < AuthMetadataProcessor > (processor));
	ResetStub ();
	EchoRequest
	  request;
	...
	ClientContext
	  context;
	const char
	  kMetadataKey1[] = "call-creds-key1";
	const char
	  kMetadataKey2[] = "call-creds-key2";
	const char
	  kMetadataVal1[] = "call-creds-val1";
	const char
	  kMetadataVal2[] = "call-creds-val2";

	context.
	  set_credentials (CompositeCallCredentials
			   (MetadataCredentialsFromPlugin
			    (std::unique_ptr < MetadataCredentialsPlugin >
			     (new
			      TestMetadataCredentialsPlugin (kMetadataKey1,
							     kMetadataVal1,
							     true, true))),
	...
	EchoResponse
	  response;
	request.set_message ("Hello");
	request.mutable_param ()->set_check_auth_context (GetParam ().
							  credentials_type ==
							  kTlsCredentialsType);
	request.mutable_param ()->
	  set_expected_transport_security_type (GetParam ().credentials_type);
	...
	if (GetParam ().credentials_type == kTlsCredentialsType)
	  {
	    ...
	    EXPECT_EQ ("192.168.1.3",
		       ToString (auth_ctx->GetPeerIdentity ()[3]));
	  }
      }
	...
	return scenarios;
      ...
      INSTANTIATE_TEST_CASE_P (End2end,
			       End2endTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (false, true, true, true)));

      INSTANTIATE_TEST_CASE_P (End2endServerTryCancel,
			       End2endServerTryCancelTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (false, true, true, true)));

      INSTANTIATE_TEST_CASE_P (ProxyEnd2end,
			       ProxyEnd2endTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (true, true, true, true)));

      INSTANTIATE_TEST_CASE_P (SecureEnd2end,
			       SecureEnd2endTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (false, false, true, false)));

      INSTANTIATE_TEST_CASE_P (ResourceQuotaEnd2end,
			       ResourceQuotaEnd2endTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (false, true, true, true)));
