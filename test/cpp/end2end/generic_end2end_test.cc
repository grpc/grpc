  ...
  grpc
{
    ...
    testing
  {
	...
	return (void *) (intptr_t) i;
      ...
      void
      verify_ok (CompletionQueue * cq, int i, bool expect_ok)
      {
	bool
	  ok;
	void *
	...
	void
	SendRpc (int num_rpcs, bool check_deadline, gpr_timespec deadline)
	{
	  const
	    grpc::string
	  kMethodName ("/grpc.cpp.test.util.EchoTestService/Echo");
	  for (int i = 0; i < num_rpcs; i++)
		...
		srv_ctx;
	      ...
	      send_request.
		set_message ("Hello world. Hello world. Hello world.");
		...
		generic_stub_->Call (&cli_ctx, kMethodName, &cli_cq_,
				     tag (1));
	      client_ok (1);
		...
		SerializeToByteBuffer (&send_request);
	      ...
	      if (check_deadline)
		{
		  EXPECT_TRUE (gpr_time_similar
			       (deadline, srv_ctx.raw_deadline (),
		...
		recv_buffer;
	      ...
	      client_ok (8);
	      EXPECT_TRUE (ParseFromByteBuffer
			   (&recv_buffer, &recv_response));
...
// One ping, one pong.
	...
	const
	  grpc::string
	kMethodName ("/grpc.cpp.test.util.EchoTestService/BidiStream");
	EchoRequest
	  send_request;
	EchoRequest
	  recv_request;
