...
TEST_F(GenericEnd2endTest, SequentialUnaryRpcs) {
  ...
  for (int i = 0; i < num_rpcs; i++) {
    ...
    call->Finish(&cli_recv_buffer, &recv_status, grpc_core::CqVerifier::tag(1));
    std::thread client_check([this] { client_ok(1); });

    generic_service_.RequestCall(&srv_ctx, &stream, srv_cq_.get(),
                                 srv_cq_.get(), grpc_core::CqVerifier::tag(4));
    request_call.join();
    EXPECT_EQ(server_host_, srv_ctx.host().substr(0, server_host_.length()));
    EXPECT_EQ(kMethodName, srv_ctx.method());

    ByteBuffer srv_recv_buffer;
    stream.Read(&srv_recv_buffer, grpc_core::CqVerifier::tag(5));
    server_ok(5);
    ...
    stream.Write(*srv_send_buffer, grpc_core::CqVerifier::tag(6));
    server_ok(6);
