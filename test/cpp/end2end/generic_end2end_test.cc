...
class GenericEnd2endTest : public ::testing::Test {
 protected:
  ...
  void SendRpc(int num_rpcs) { SendRpc(num_rpcs, false, gpr_inf_future(GPR_CLOCK_MONOTONIC)); }
  ...
  void DriveCompletionQueue() {
...

    {
      ...
      if (!shutting_down_) {
        generic_service_.RequestCall(&server_context, &reader_writer, srv_cq_.get(), srv_cq_.get(),
      ...
      while (srv_cq_->Next(reinterpret_cast<void**>(&event), &ok)) {
        ...
        switch (event) {
          case Event::kCallReceived:
            reader_writer.Finish(::grpc::Status(::grpc::StatusCode::UNIMPLEMENTED, "go away"),
...
TEST_F(GenericEnd2endTest, SequentialUnaryRpcs) {
  ...
  for (int i = 0; i < num_rpcs; i++) {
    ...
    std::unique_ptr<ByteBuffer> cli_send_buffer = SerializeToByteBuffer(&send_request);
    std::thread request_call([this]() { server_ok(4); });
    ...
    send_response.set_message(recv_request.message());
    std::unique_ptr<ByteBuffer> srv_send_buffer = SerializeToByteBuffer(&send_response);
...
TEST_F(GenericEnd2endTest, Deadline) {
  ResetStub();
  SendRpc(1, true,
          gpr_time_add(gpr_now(GPR_CLOCK_MONOTONIC), gpr_time_from_seconds(10, GPR_TIMESPAN)));
}
...
TEST_F(GenericEnd2endTest, ShortDeadline) {
  ...
  cli_ctx.set_deadline(
      gpr_time_add(gpr_now(GPR_CLOCK_MONOTONIC), gpr_time_from_micros(500, GPR_TIMESPAN)));
