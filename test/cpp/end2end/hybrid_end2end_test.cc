...
// Handlers to handle async request at a server. To be run in a separate thread.
	...
	service->RequestEcho (&srv_ctx, &recv_request, &response_writer, cq,
			      cq, tag (1));
	Verify (cq, 1, true);
	...
	Verify (cq, 2, true);
      ...
      template < class Service >
	void HandleClientStreaming (Service * service,
	...
	do
	  {
	    i++;
	    send_response.mutable_message ()->append (recv_request.
						      message ());
	...
	Verify (cq, 100, true);
      ...
      template < class Service >
	void HandleServerStreaming (Service * service,
				    ServerCompletionQueue * cq)
      {
	ServerContext srv_ctx;
	EchoRequest recv_request;
	...
	service->RequestResponseStream (&srv_ctx, &recv_request, &srv_stream,
					cq, cq, tag (1));
	Verify (cq, 1, true);
	...
	Verify (cq, 4, true);
      ...
      void HandleGenericRequestStream (GenericServerAsyncReaderWriter *
				       stream, CompletionQueue * cq)
      {
	ByteBuffer recv_buffer;
	EchoRequest recv_request;
	EchoResponse send_response;
...
// Request and handle one generic call.
	...
	if (srv_ctx.method () == "/grpc.testing.EchoTestService/Echo")
	  ...
	  {			// other methods not handled yet.
	    gpr_log (GPR_ERROR, "method: %s", srv_ctx.method ().c_str ());
	    GPR_ASSERT (0);
	  }
      }

      class TestServiceImplDupPkg:public::grpc::testing::duplicate::
	EchoTestService::Service
      {
      public:
	...
	void SetUpServer (::grpc::Service * service1,::grpc::Service *
			  service2, AsyncGenericService * generic_service,
	...
	void SendEcho ()
	{
	  ...
	  Status recv_status =
	    stub_->Echo (&cli_ctx, send_request, &recv_response);
	...
	void SendEchoToDupService ()
	{
	  std::shared_ptr < Channel > channel =
	    CreateChannel (server_address_.str (),
			   InsecureChannelCredentials ());
	  auto stub =
	    grpc::testing::duplicate::EchoTestService::NewStub (channel);
	  EchoRequest send_request;
	  EchoResponse recv_response;
	  ClientContext cli_ctx;
	  cli_ctx.set_wait_for_ready (true);
	  send_request.set_message ("Hello");
	  Status recv_status =
	    stub->Echo (&cli_ctx, send_request, &recv_response);
	  EXPECT_EQ (send_request.message () + "_dup",
		     recv_response.message ());
	  EXPECT_TRUE (recv_status.ok ());
	}
	...
	grpc::testing::UnimplementedEchoService::
	  Service unimplemented_service_;
	...
	std::ostringstream server_address_;
      ...
      {
	typedef EchoTestService::WithAsyncMethod_Echo < TestServiceImpl >
	  SType;
	SType service;
	...
	typedef EchoTestService::WithAsyncMethod_RequestStream <
	  EchoTestService::WithAsyncMethod_Echo < TestServiceImpl >> SType;
	SType service;
	SetUpServer (&service, nullptr, nullptr);
	ResetStub ();
	std::thread echo_handler_thread (HandleEcho < SType >, &service,
					 cqs_[0].get (), false);
	std::thread request_stream_handler_thread (HandleClientStreaming <
						   SType >, &service,
						   cqs_[1].get ());
	TestAllMethods ();
	echo_handler_thread.join ();
	...
	SetUpServer (&service, nullptr, nullptr);
	ResetStub ();
	std::thread response_stream_handler_thread (HandleServerStreaming <
						    SType >, &service,
						    cqs_[0].get ());
	std::thread request_stream_handler_thread (HandleClientStreaming <
						   SType >, &service,
...
// Add a second service with one sync method.
      TEST_F (HybridEnd2endTest,
	      AsyncRequestStreamResponseStream_SyncDupService)
      {
	typedef EchoTestService::WithAsyncMethod_RequestStream <
	  EchoTestService::WithAsyncMethod_ResponseStream < TestServiceImpl >>
	  SType;
...
// Add a second service with one sync streamed unary method.
      class StreamedUnaryDupPkg:public duplicate::EchoTestService::
	WithStreamedUnaryMethod_Echo < TestServiceImplDupPkg >
      {
      public:
	Status StreamedEcho (ServerContext * context,
			     ServerUnaryStreamer < EchoRequest,
			     EchoResponse > *stream) override
	{
	  EchoRequest req;
	  EchoResponse resp;
	  uint32_t next_msg_sz;
	    stream->NextMessageSize (&next_msg_sz);
	    gpr_log (GPR_INFO, "Streamed Unary Next Message Size is %u",
		     next_msg_sz);
...
// Add a second service that is fully Streamed Unary
      class FullyStreamedUnaryDupPkg:public duplicate::EchoTestService::
...
// Add a second service with one sync split server streaming method.
      class SplitResponseStreamDupPkg:public duplicate::
	EchoTestService::WithSplitStreamingMethod_ResponseStream <
	TestServiceImplDupPkg >
      {
      public:
	Status StreamedResponseStream (ServerContext * context,
				       ServerSplitStreamer < EchoRequest,
				       EchoResponse > *stream) override
	{
	  EchoRequest req;
	  EchoResponse resp;
	  uint32_t next_msg_sz;
	    stream->NextMessageSize (&next_msg_sz);
	    gpr_log (GPR_INFO, "Split Streamed Next Message Size is %u",
		     next_msg_sz);
	    GPR_ASSERT (stream->Read (&req));
	  for (int i = 0; i < kServerDefaultResponseStreamsToSend; i++)
	    {
	      resp.set_message (req.message () + grpc::to_string (i) +
				"_dup");
	      GPR_ASSERT (stream->Write (resp));
	    }
	  return Status::OK;
...
// Add a second service that is fully split server streamed
      class FullySplitStreamedDupPkg:public duplicate::EchoTestService::
...
// Add a second service that is fully server streamed
      class FullyStreamedDupPkg:public duplicate::EchoTestService::
	StreamedService
      {
      public:
	Status StreamedEcho (ServerContext * context,
...
// Add a second service with one async method.
      TEST_F (HybridEnd2endTest,
	      AsyncRequestStreamResponseStream_AsyncDupService)
      {
	typedef EchoTestService::WithAsyncMethod_RequestStream <
	  EchoTestService::WithAsyncMethod_ResponseStream < TestServiceImpl >>
	  SType;
	...
	std::thread request_stream_handler_thread (HandleClientStreaming <
						   SType >, &service,
						   cqs_[1].get ());
	std::thread echo_handler_thread (HandleEcho <
					 duplicate::EchoTestService::
					 AsyncService >, &dup_service,
					 cqs_[2].get (), true);
	TestAllMethods ();
	SendEchoToDupService ();
	response_stream_handler_thread.join ();
	...
	ResetStub ();
	std::thread generic_handler_thread (HandleGenericCall,
					    &generic_service, cqs_[0].get ());
	TestAllMethods ();
	generic_handler_thread.join ();
      ...
      {
	typedef EchoTestService::WithAsyncMethod_RequestStream <
	  EchoTestService::WithGenericMethod_Echo < TestServiceImpl >> SType;
	SType service;
	AsyncGenericService generic_service;
	SetUpServer (&service, nullptr, &generic_service);
	ResetStub ();
...
// Add a second service with one async method.
      TEST_F (HybridEnd2endTest,
	      GenericEchoAsyncRequestStream_AsyncDupService)
      {
	typedef EchoTestService::WithAsyncMethod_RequestStream <
	  EchoTestService::WithGenericMethod_Echo < TestServiceImpl >> SType;
	SType service;
	...
	std::thread generic_handler_thread (HandleGenericCall,
					    &generic_service, cqs_[0].get ());
	std::thread generic_handler_thread2 (HandleGenericCall,
					     &generic_service,
					     cqs_[1].get ());
	std::thread response_stream_handler_thread (HandleServerStreaming <
						    SType >, &service,
