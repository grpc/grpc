...
class SpiffeGenericEnd2endTest : public ::testing::Test {
 protected:
  SpiffeGenericEnd2endTest() : server_host_("localhost") {}
  ...
  void ResetStub() {
    std::shared_ptr<Channel> channel = grpc::CreateChannel(
        server_address_.str(), SpiffeTestChannelCredentials());
...
TEST_F(SpiffeGenericEnd2endTest, SequentialUnaryRpcs) {
  ResetStub();
...
// One ping, one pong.
TEST_F(SpiffeGenericEnd2endTest, SimpleBidiStreaming) {
  ResetStub();
...
TEST_F(SpiffeGenericEnd2endTest, Deadline) {
  ResetStub();
  SendRpc(1, true,
...
TEST_F(SpiffeGenericEnd2endTest, ShortDeadline) {
  ResetStub();
