...
TEST_P(XdsExtProcEnd2endTest, ServerTrailingMetadataHookFailure) {
  grpc_core::g_test_ext_proc_server_trailing_metadata_modifier =
      [](grpc_metadata_batch* /*metadata*/) {
        return absl::InvalidArgumentError("injected trailing error");
      };
...
TEST_P(XdsExtProcEnd2endTest, ServerToClientMessageHookFailure) {
  grpc_core::g_test_ext_proc_server_to_client_message_modifier =
      [](grpc_core::MessageHandle* /*message*/) {
        return absl::InvalidArgumentError("injected s2c message error");
      };
  ...
  EXPECT_FALSE(status.ok());
  EXPECT_EQ(status.error_code(), grpc::StatusCode::INVALID_ARGUMENT);
  EXPECT_EQ(status.error_message(), "injected s2c message error");
}
