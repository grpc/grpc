 ...
 protected:
  ParameterizedClientInterceptorsEnd2endTest() {
    ...
    if (GetParam().channel_type() == ChannelType::kHttpChannel) {
      ...
      int flags;
      ABSL_CHECK_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv_), 0);
      flags = fcntl(sv_[0], F_GETFL, 0);
      ABSL_CHECK_EQ(fcntl(sv_[0], F_SETFL, flags | O_NONBLOCK), 0);
      flags = fcntl(sv_[1], F_GETFL, 0);
      ABSL_CHECK_EQ(fcntl(sv_[1], F_SETFL, flags | O_NONBLOCK), 0);
      ABSL_CHECK(grpc_set_socket_no_sigpipe_if_possible(sv_[0]) == absl::OkStatus());
      ABSL_CHECK(grpc_set_socket_no_sigpipe_if_possible(sv_[1]) == absl::OkStatus());
      server_ = builder.BuildAndStart();
...
int main(int argc, char** argv) {
  ...
  ABSL_CHECK(grpc_wait_until_shutdown(10));
  return ret;
}
