...
// same thing as the default one.
	...
	public::grpc::health::v1::Health::Service
      ...
      public:
	Status
	Check (ServerContext * context, const HealthCheckRequest * request,
	       HealthCheckResponse * response)
	  override
	{
	  std::lock_guard < std::mutex > lock (mu_);
	  auto
	    iter = status_map_.find (request->service ());
	...
	void
	SetStatus (const grpc::string & service_name,
	...
	void
	SetAll (HealthCheckResponse::ServingStatus status)
	{
	  std::lock_guard < std::mutex > lock (mu_);
...
// also serves as an example of how to override the default service.
	...
	HealthCheckServiceInterface
      ...
      public:
	explicit
	CustomHealthCheckService (HealthCheckServiceImpl * impl):
	impl_ (impl)
	{
	  impl_->SetStatus ("", HealthCheckResponse::SERVING);
	}
	void
	SetServingStatus (const grpc::string & service_name, bool serving)
	  override
	{
	  impl_->SetStatus (service_name,
			    serving ? HealthCheckResponse::
			    SERVING : HealthCheckResponse::NOT_SERVING);
	}

	void
	SetServingStatus (bool serving)
	  override
	{
	...
	HealthCheckServiceImpl * impl_;	// not owned
      ...
      void
      LoopCompletionQueue (ServerCompletionQueue * cq)
      {
	void *
	  tag;
	bool
	...
	void
	SetUpServer (bool register_sync_test_service, bool add_async_cq,
		     ...
		     std::unique_ptr < HealthCheckServiceInterface > service)
	{
	  ...
	  bool
	    register_sync_health_service_impl =
	  ...
	  if (explicit_health_service)
		...
		option (new
			HealthCheckServiceServerBuilderOption (std::
							       move
							       (service)));
	...
	SendHealthCheckRpc (const grpc::string & service_name,
			    const Status & expected_status,
			    HealthCheckResponse::
			    ServingStatus expected_serving_status)
	{
	  HealthCheckRequest
	    request;
	  ...
	  ClientContext
	    context;
	  Status
	    s = hc_stub_->Check (&context, request, &response);
	...
	{
	  HealthCheckServiceInterface *
	    service = server_->GetHealthCheckService ();
	  EXPECT_TRUE (service != nullptr);
	  const
	    grpc::string
	  kHealthyService ("healthy_service");
	  const
	    grpc::string
	  kUnhealthyService ("unhealthy_service");
	  const
	    grpc::string
	  kNotRegisteredService ("not_registered");
	...
	HealthCheckServiceInterface *
	  default_service = server_->GetHealthCheckService ();
...
// Provide an explicit override of health checking service interface.
	...
	std::unique_ptr < HealthCheckServiceInterface >
	  override_service (new
			    CustomHealthCheckService
			    (&health_check_service_impl_));
	HealthCheckServiceInterface *
	  underlying_service = override_service.get ();
