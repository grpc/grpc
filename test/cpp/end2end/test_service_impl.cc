...
// the response in seconds.
      void
      MaybeEchoDeadline (ServerContext * context, const EchoRequest * request,
	...
	if (request->has_param () && request->param ().echo_deadline ())
	  {
	    ...
	    response->
	    mutable_param ()->
	    set_request_deadline (deadline.tv_sec);
	  }
      }

      void
      CheckServerAuthContext (const ServerContext * context,
			      const grpc::
			      string & expected_transport_security_type,
	...
	if (expected_client_identity.empty ())
	  ...
	  {
	    auto
	      identity = auth_ctx->GetPeerIdentity ();
    ...
    Status
    TestServiceImpl::Echo (ServerContext * context,
      ...
      if (request->has_param () && request->param ().has_expected_error ())
	{
	  const
	    auto &
	    error = request->param ().expected_error ();
	...
	GetIntValueFromMetadata (kServerTryCancelRequest,
				 context->client_metadata (), DO_NOT_CANCEL);
      ...
      if (request->has_param () && request->param ().echo_metadata ())
	{
	  const
	    ...
	    grpc::string_ref,
	  grpc::string_ref > &
	    client_metadata = context->client_metadata ();
	  for (std::multimap < grpc::string_ref,
	       grpc::string_ref >::const_iterator iter =
	       client_metadata.begin (); iter != client_metadata.end ();
	       ++iter)
	    {
	   ...
	   request->param ().check_auth_context ()))
	{
	  CheckServerAuthContext (context,
				  request->param ().
				  expected_transport_security_type (),
				  request->param ().
	  ...
	  && request->param ().response_message_length () > 0)
	{
	  response->
	    set_message (grpc::
			 string (request->param ().response_message_length (),
				 '\0'));
	}
...
// Unimplemented is left unimplemented to test the returned error.
    ...
    Status
    TestServiceImpl::RequestStream (ServerContext * context,
	...
	GetIntValueFromMetadata (kServerTryCancelRequest,
      ...
      int
	cancel_after_reads =
	GetIntValueFromMetadata (kServerCancelAfterReads,
      ...
      if (server_try_cancel == CANCEL_DURING_PROCESSING)
	{
	  server_try_cancel_thd =
	    new std::thread (&TestServiceImpl::ServerTryCancel, this,
			     context);
	}
...
// TODO(yangg) make it generic by adding a parameter into EchoRequest
    Status
    TestServiceImpl::ResponseStream (ServerContext * context,
				     const EchoRequest * request,
	...
	GetIntValueFromMetadata (kServerTryCancelRequest,
      ...
      int
	server_coalescing_api =
	GetIntValueFromMetadata (kServerUseCoalescingApi,
				 context->client_metadata (), 0);
    ...
    Status
    TestServiceImpl::BidiStream (ServerContext * context,
				 ServerReaderWriter < EchoResponse,
				 EchoRequest > *stream)
    {
      // If server_try_cancel is set in the metadata, the RPC is cancelled by the
      // server by calling ServerContext::TryCancel() depending on the value:
      ...
      if (server_try_cancel == CANCEL_DURING_PROCESSING)
	{
      ...
      int
	server_write_last =
	GetIntValueFromMetadata (kServerFinishAfterNReads,
				 context->client_metadata (), 0);
    ...
    int
    TestServiceImpl::GetIntValueFromMetadata (const char *key,
					      const std::multimap <
					      grpc::string_ref,
    ...
    void
    TestServiceImpl::ServerTryCancel (ServerContext * context)
    {
      EXPECT_FALSE (context->IsCancelled ());
      context->TryCancel ();
