...
class ChannelzServerTest : public ::testing::Test {
 public:
  ...
  void ConfigureProxy(size_t num_backends) {
    backend_servers_.resize(num_backends);
    backend_ports_.resize(num_backends);
    backend_services_.resize(num_backends);
    for (size_t i = 0; i < num_backends; ++i) {
      // create a new backend.
      backend_ports_[i] = grpc_pick_unused_port_or_die();
      ServerBuilder backend_builder;
      grpc::string backend_server_address =
          "localhost:" + to_string(backend_ports_[i]);
      backend_builder.AddListeningPort(backend_server_address,
                                       InsecureServerCredentials());
      backend_services_[i].reset(new TestServiceImpl);
      // ensure that the backend itself has channelz disabled.
      backend_builder.AddChannelArgument(GRPC_ARG_ENABLE_CHANNELZ, 0);
      backend_builder.RegisterService(backend_services_[i].get());
      backend_servers_[i] = backend_builder.BuildAndStart();
  ...
  void ResetStubs() {
    ...
    ChannelArguments args;
    // disable channels. We only want to focus on proxy to backend outbound.
 ...
 protected:
  ...
  std::vector<std::unique_ptr<TestServiceImpl>> backend_services_;

  // ensure channel server is linked in.
  channelz::ChannelzServerPlugin plugin_;
};
