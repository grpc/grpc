  ...
  grpc
{
    ...
    testing
  {
    ...
    public:
      static void
      BidiStream_Sender (ServerReaderWriter < EchoResponse,
			 EchoRequest > *stream, gpr_atm * should_exit)
      {
	EchoResponse
	  response;
	response.
	set_message (kLargeString);
	while (gpr_atm_acq_load (should_exit) == static_cast < gpr_atm > (0))
      ...
      Status
      BidiStream (ServerContext * context,
	...
	std::thread sender (std::
			    bind (&TestServiceImpl::BidiStream_Sender, stream,
				  &should_exit));
	...
	ServerBuilder
	  builder;
	builder.
	AddListeningPort (server_address_.str (),
    ...
    static void
    Drainer (ClientReaderWriter < EchoRequest, EchoResponse > *reader)
    {
      EchoResponse
	response;
