...
TEST_F(RawEnd2EndTest, PureAsyncService) {
  ...
  response_writer.Finish(send_response_, Status::OK, CoreTestFixture::tag(3));
...
TEST_F(RawEnd2EndTest, RawServerClientStreaming) {
  ...
  cli_stream->Write(send_request_, CoreTestFixture::tag(3));
  srv_stream.Read(&recv_request_buffer_, CoreTestFixture::tag(4));
  Verifier().Expect(3, true).Expect(4, true).Verify(cq_.get());
  ParseFromByteBuffer(&recv_request_buffer_, &recv_request_);
  EXPECT_EQ(send_request_.message(), recv_request_.message());

  cli_stream->Write(send_request_, CoreTestFixture::tag(5));
  srv_stream.Read(&recv_request_buffer_, CoreTestFixture::tag(6));
  Verifier().Expect(5, true).Expect(6, true).Verify(cq_.get());

  ParseFromByteBuffer(&recv_request_buffer_, &recv_request_);
  EXPECT_EQ(send_request_.message(), recv_request_.message());
  cli_stream->WritesDone(CoreTestFixture::tag(7));
  srv_stream.Read(&recv_request_buffer_, CoreTestFixture::tag(8));
  Verifier().Expect(7, true).Expect(8, false).Verify(cq_.get());

  ParseFromByteBuffer(&recv_request_buffer_, &recv_request_);
  send_response_.set_message(recv_request_.message());
  SerializeToByteBufferInPlace(&send_response_, &send_response_buffer_);
  srv_stream.Finish(send_response_buffer_, Status::OK, CoreTestFixture::tag(9));
  cli_stream->Finish(&recv_status_, CoreTestFixture::tag(10));
...
TEST_F(RawEnd2EndTest, RawServerServerStreaming) {
  ...
  send_response_.set_message(recv_request_.message());
  SerializeToByteBufferInPlace(&send_response_, &send_response_buffer_);
  srv_stream.Write(send_response_buffer_, CoreTestFixture::tag(3));
  cli_stream->Read(&recv_response_, CoreTestFixture::tag(4));
  Verifier().Expect(3, true).Expect(4, true).Verify(cq_.get());
  EXPECT_EQ(send_response_.message(), recv_response_.message());

  srv_stream.Write(send_response_buffer_, CoreTestFixture::tag(5));
  cli_stream->Read(&recv_response_, CoreTestFixture::tag(6));
  Verifier().Expect(5, true).Expect(6, true).Verify(cq_.get());
  EXPECT_EQ(send_response_.message(), recv_response_.message());

  srv_stream.Finish(Status::OK, CoreTestFixture::tag(7));
  cli_stream->Read(&recv_response_, CoreTestFixture::tag(8));
  Verifier().Expect(7, true).Expect(8, false).Verify(cq_.get());

  cli_stream->Finish(&recv_status_, CoreTestFixture::tag(9));
