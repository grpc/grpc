...
// - Handling of LB servers restart, including reconnection after
// backing-off
//   retries.
// - Destruction of load balanced channel (and therefore of grpclb
// instance)
//   while:
//   1) the internal LB call is still active. This should work by virtue
//   of the weak reference the LB call holds. The call should be
//   terminated as part of the grpclb shutdown process. 2) the retry
//   timer is active. Again, the weak reference it holds should prevent
//   a premature call to \a glb_destroy.
...
class BackendServiceImpl : public BackendService {
 public:
  ...
  Status Echo(ServerContext* context, const EchoRequest* request,
    ...
    const auto status =
        TestServiceImpl::Echo(context, request, response);
...
class BalancerServiceImpl : public BalancerService {
 public:
  using Stream =
      ServerReaderWriter<LoadBalanceResponse, LoadBalanceRequest>;
  using ResponseDelayPair = std::pair<LoadBalanceResponse, int>;

  explicit BalancerServiceImpl(
  ...
  Status BalanceLoad(ServerContext* context, Stream* stream) override {
    ...
    }
    {
      ...
      // TODO(juanlishen): Initial response should always be the first
      // response.
      ...
      if (client_load_reporting_interval_seconds_ > 0) {
        ...
        while (stream->Read(&request)) {
          gpr_log(GPR_INFO,
                  "LB[%p]: received client load report message '%s'",
          ...
          for (const auto& drop_token_count :
            ...
            load_report.drop_token_counts[drop_token_count
                                              .load_balance_token()] =
                drop_token_count.num_calls();
          }
          // We need to acquire the lock here in order to prevent the
          // notify_one below from firing before its corresponding wait
          // is executed.
          grpc::internal::MutexLock lock(&mu_);
  ...
  void add_response(const LoadBalanceResponse& response,
                    int send_after_ms) {
    grpc::internal::MutexLock lock(&mu_);
  ...
  ClientStats WaitForLoadReport() {
    ...
    if (load_report_queue_.empty()) {
      ...
      grpc::internal::WaitUntil(load_report_cond_, &mu_, [this] {
        return !load_report_queue_.empty();
      });
...
class GrpclbEnd2endTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    for (size_t i = 0; i < num_backends_; ++i) {
      backends_.emplace_back(
          new ServerThread<BackendServiceImpl>("backend"));
  ...
  void ResetStub(int fallback_timeout = 0,
    ...
    // TODO(dgq): templatize tests to run everything using both secure
    // and insecure channel credentials.
  ...
  grpc_core::Resolver::Result MakeResolverResult(
    ...
    grpc_arg arg =
        CreateGrpclbBalancerAddressesArg(&balancer_addresses);
  ...
  void SetNextResolution(
    ...
    grpc_core::Resolver::Result result =
        MakeResolverResult(balancer_address_data, backend_address_data,
                           service_config_json);
  ...
  LoadBalanceResponse BuildResponseForBackends(
    ...
    for (const int& backend_port : backend_ports) {
      auto* server = response.mutable_server_list()->add_servers();
      server->set_ip_address(ipv6_only_
                                 ? Ip6ToPackedString("::1")
  ...
  Status SendRpc(EchoResponse* response = nullptr,
                 int timeout_ms = 1000, bool wait_for_ready = false,
  ...
  void CheckRpcSendOk(const size_t times = 1,
    ...
    for (size_t i = 0; i < times; ++i) {
      ...
      const Status status =
          SendRpc(&response, timeout_ms, wait_for_ready);
  ...
  struct ServerThread {
    ...
    void Start(const std::string& server_host) {
      gpr_log(GPR_INFO, "starting %s server on port %d", type_.c_str(),
              port_);
      GPR_ASSERT(!running_);
      running_ = true;
      service_.Start();
      ...
      thread_ = absl::make_unique<std::thread>(std::bind(
          &ServerThread::Serve, this, server_host, &mu, &cond));
      cond.Wait(&mu);
  ...
  std::vector<std::unique_ptr<ServerThread<BackendServiceImpl>>>
      backends_;
  std::vector<std::unique_ptr<ServerThread<BalancerServiceImpl>>>
      balancers_;
...
TEST_F(SingleBalancerTest, UsePickFirstChildPolicy) {
  ...
  CheckRpcSendOk(kNumRpcs, 1000 /* timeout_ms */,
                 true /* wait_for_ready */);
  balancers_[0]->service_.NotifyDoneWithServerlists();
...
TEST_F(SingleBalancerTest, SwapChildPolicy) {
  ...
  CheckRpcSendOk(kNumRpcs, 1000 /* timeout_ms */,
                 true /* wait_for_ready */);
  // Check that every backend saw the same number of requests.  This
  // verifies that we used round_robin.
  for (size_t i = 0; i < backends_.size(); ++i) {
...
TEST_F(SingleBalancerTest, SameBackendListedMultipleTimes) {
  ...
  ScheduleResponseForBalancer(0, BuildResponseForBackends(ports, {}),
                              0);
...
TEST_F(SingleBalancerTest, InitiallyEmptyServerlist) {
  ...
  ScheduleResponseForBalancer(
      0, BuildResponseForBackends(GetBackendPorts(), {}),
      kServerlistDelayMs);
...
TEST_F(SingleBalancerTest, Fallback) {
  ...
  // Wait until the serverlist reception has been processed and all
  // backends in the serverlist are reachable.
  for (size_t i = kNumBackendsInResolution; i < backends_.size(); ++i) {
    WaitForBackend(i);
  }
...
TEST_F(SingleBalancerTest, FallbackUpdate) {
  ...
  for (size_t i = kNumBackendsInResolution;
       i < kNumBackendsInResolution + kNumBackendsInResolutionUpdate;
       ++i) {
    backend_addresses.emplace_back(
        AddressData{backends_[i]->port_, ""});
  }
  ...
  // The resolution update is used: each backend in the resolution
  // update should have gotten one request.
  for (size_t i = 0; i < kNumBackendsInResolution; ++i) {
    EXPECT_EQ(0U, backends_[i]->service_.request_count());
  }
  for (size_t i = kNumBackendsInResolution;
...
TEST_F(SingleBalancerTest,
  ...
  // First two backends are fallback, last two are pointed to by
  // balancer.
  const size_t kNumFallbackBackends = 2;
  const size_t kNumBalancerBackends =
  ...
  for (size_t i = 0; i < balancers_.size(); ++i) {
    balancer_addresses.emplace_back(
        AddressData{balancers_[i]->port_, ""});
  }
  SetNextResolution(balancer_addresses, backend_addresses);
  ScheduleResponseForBalancer(
      0,
  ...
  // Stop balancer.  RPCs should continue going to backends from
  // balancer.
...
TEST_F(SingleBalancerTest, FallbackEarlyWhenBalancerChannelFails) {
  ...
  SetNextResolution(balancer_addresses, backend_addresses);
  // Send RPC with deadline less than the fallback timeout and make sure
  // it succeeds.
...
TEST_F(SingleBalancerTest,
       FallbackControlledByBalancer_BeforeFirstServerlist) {
  const int kFallbackTimeoutMs = 10000 * grpc_test_slowdown_factor();
  ResetStub(kFallbackTimeoutMs);
  // Return one balancer and one fallback backend.
  std::vector<AddressData> balancer_addresses;
...
TEST_F(SingleBalancerTest,
       FallbackControlledByBalancer_AfterFirstServerlist) {
  ...
  backend_addresses.emplace_back(AddressData{backends_[0]->port_, ""});
  SetNextResolution(balancer_addresses, backend_addresses);
  // Balancer initially sends serverlist, then tells client to fall
  // back, then sends the serverlist again. The serverlist points to
  // backend 1.
...
TEST_F(SingleBalancerTest, ServiceNameFromLbPolicyConfig) {
  ...
  // We need to wait for all backends to come online.
  WaitForAllBackends();
  EXPECT_EQ(balancers_[0]->service_.service_names().back(),
            "test_service");
}
...
TEST_F(UpdatesTest, UpdateBalancersButKeepUsingOriginalBalancer) {
  ...
  ScheduleResponseForBalancer(
      0, BuildResponseForBackends(first_backend, {}), 0);
  ScheduleResponseForBalancer(
      1, BuildResponseForBackends(second_backend, {}), 0);
  ...
  // The current LB call is still working, so grpclb continued using it
  // to the first balancer, which doesn't assign the second backend.
  EXPECT_EQ(0U, backends_[1]->service_.request_count());
...
// Send an update with the same set of LBs as the one in SetUp() in
// order to verify that the LB channel inside grpclb keeps the initial
// connection (which by definition is also present in the update).
TEST_F(UpdatesTest, UpdateBalancersRepeated) {
  ...
  // grpclb continued using the original LB call to the first balancer,
...
TEST_F(UpdatesTest, UpdateBalancersDeadUpdate) {
  ...
  EXPECT_EQ(1U, balancers_[0]->service_.request_count());
  EXPECT_EQ(1U, balancers_[0]->service_.response_count());
  // The second balancer, published as part of the first update, may end
  // up getting two requests (that is, 1 <= #req <= 2) if the LB call
  // retry timer firing races with the arrival of the update containing
  // the second balancer.
...
TEST_F(UpdatesTest, ReresolveDeadBackend) {
  ResetStub(500);
  // The first resolution contains the addresses of a balancer that
  // never responds, and a fallback backend.
  std::vector<AddressData> balancer_addresses;
  balancer_addresses.emplace_back(
      AddressData{balancers_[0]->port_, ""});
  std::vector<AddressData> backend_addresses;
  ...
  // The re-resolution result will contain the addresses of the same
  // balancer and a new fallback backend.
  balancer_addresses.clear();
  balancer_addresses.emplace_back(
      AddressData{balancers_[0]->port_, ""});
  backend_addresses.clear();
  ...
  // Wait until re-resolution has finished, as signaled by the second
...
// TODO(juanlishen): Should be removed when the first response is always
// the initial response. Currently, if client load reporting is not
// enabled, the balancer doesn't send initial response. When the backend
// shuts down, an unexpected re-resolution will happen. This test
// configuration is a workaround for test ReresolveDeadBalancer.
...
TEST_F(UpdatesWithClientLoadReportingTest, ReresolveDeadBalancer) {
  ...
  EXPECT_EQ(1U, balancers_[0]->service_.request_count());
  EXPECT_EQ(1U, balancers_[0]->service_.response_count());
  // After balancer 0 is killed, we restart an LB call immediately
  // (because we disconnect to a previously connected balancer).
  // Although we will cancel this call when the re-resolution update is
  // done and another LB call restart is needed, this old call may still
  // succeed reaching the LB server if re-resolution is slow. So
  // balancer 1 may have received 2 requests and sent 2 responses.
  EXPECT_GE(balancers_[1]->service_.request_count(), 1U);
  EXPECT_GE(balancers_[1]->service_.response_count(), 1U);
  EXPECT_LE(balancers_[1]->service_.request_count(), 2U);
  EXPECT_LE(balancers_[1]->service_.response_count(), 2U);
...
TEST_F(SingleBalancerTest, Drop) {
  ...
  const int num_of_drop_addresses =
      num_of_drop_by_rate_limiting_addresses +
  ...
  size_t num_drops = 0;
  for (size_t i = 0; i < kNumRpcsPerAddress * num_total_addresses;
       ++i) {
    EchoResponse response;
...
TEST_F(SingleBalancerTest, DropAll) {
  ...
  // First call succeeds.
  CheckRpcSendOk();
  // But eventually, the update with only dropped servers is processed
  // and calls fail.
  Status status;
  do {
...
class SingleBalancerWithClientLoadReportingTest
    : public GrpclbEnd2endTest {
 public:
  SingleBalancerWithClientLoadReportingTest()
      : GrpclbEnd2endTest(4, 1, 3) {}
};
...
TEST_F(SingleBalancerWithClientLoadReportingTest, BalancerRestart) {
  ...
  // last serverlist we received from the balancer before it was shut
  // down.
  ResetBackendCounters();
...
TEST_F(SingleBalancerWithClientLoadReportingTest, Drop) {
  ...
  std::tie(num_warmup_ok, num_warmup_failure, num_warmup_drops) =
      WaitForAllBackends(
          num_total_addresses /* num_requests_multiple_of */);
  ...
  EXPECT_EQ(kNumRpcsPerAddress * num_total_addresses +
                num_total_warmup_requests,
            client_stats.num_calls_started);
  EXPECT_EQ(kNumRpcsPerAddress * num_total_addresses +
                num_total_warmup_requests,
            client_stats.num_calls_finished);
  ...
  EXPECT_EQ(kNumRpcsPerAddress * num_backends_ + num_warmup_ok,
            client_stats.num_calls_finished_known_received);
  // The number of warmup request is a multiple of the number of
  // addresses. Therefore, all addresses in the scheduled balancer
  // response are hit the same number of times.
