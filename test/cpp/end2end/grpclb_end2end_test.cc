  ...
  grpc
{
    ...
    testing
  {
    ...
    template < typename ServiceType > class CountedService:public
	ServiceType
      {
      public:
	...
	size_t
      ...
      using
	BackendService = CountedService < TestServiceImpl >;
      using
	BalancerService = CountedService < LoadBalancer::Service >;
	  ...
	  override
	{
	  IncreaseRequestCount ();
	  const auto
	    status = TestServiceImpl::Echo (context, request, response);
	  IncreaseResponseCount ();
	  return status;
	}
	...
	return grpc::string (reinterpret_cast < const char *>(&ip4),
			     sizeof (ip4));
      }
	...
	ClientStats &
	operator+= (const ClientStats & other)
	{
	...
	using
	ResponseDelayPair = std::pair < LoadBalanceResponse, int >;

	explicit
	BalancerServiceImpl (int client_load_reporting_interval_seconds):
	...
	Status
	BalanceLoad (ServerContext * context, Stream * stream)
	  override
	{
	...
	for (const auto & response_and_delay:responses_and_delays)
	    ...
	    }
	  {
	    ...
	    if (shutdown_)
	      goto done;
	    serverlist_cond_.wait (lock,[this]
				   {
				   return serverlist_ready_;
				   }
	    );
	  ...
	  if (client_load_reporting_interval_seconds_ > 0)
	    {
	      request.Clear ();
	      stream->Read (&request);
	      gpr_log (GPR_INFO, "LB[%p]: recv client load report msg: '%s'",
...
request.client_stats ().calls_finished_with_drop ())
		{
		  client_stats_.drop_token_counts[drop_token_count.
						  load_balance_token ()] +=
	...
	void
	add_response (const LoadBalanceResponse & response, int send_after_ms)
	{
	  std::unique_lock < std::mutex > lock (mu_);
	  responses_and_delays_.
	    push_back (std::make_pair (response, send_after_ms));
	}
	...
	static LoadBalanceResponse
	BuildResponseForBackends (const std::vector < int >&backend_ports,
				  const std::map < grpc::string,
				  size_t > &drop_token_counts)
	{
	  LoadBalanceResponse
	    response;
	...
	{
	  std::unique_lock < std::mutex > lock (mu_);
	  load_report_cond_.wait (lock,[this]
				  {
				  return load_report_ready_;
				  }
	  );
	...
	NotifyDoneWithServerlists ()
	{
      ...
      private:
	void
	SendResponse (Stream * stream, const LoadBalanceResponse & response,
		      int delay_ms)
	{
	  ...
	  override
	{
	  for (size_t i = 0; i < backends_.size (); ++i)
	    {
	      if (backends_[i]->Shutdown ())
		backend_servers_[i].Shutdown ();
	    }
	  for (size_t i = 0; i < balancers_.size (); ++i)
	    {
	      if (balancers_[i]->Shutdown ())
	  ...
	  for (size_t i = 0; i < balancer_servers_.size (); ++i)
	    {
	      addresses.emplace_back (AddressData
				      {
				      balancer_servers_[i].port_, true, ""}
	      );
	    }
	...
	void
	ResetStub (int fallback_timeout = 0)
	{
	  ChannelArguments
	    args;
	  ...
	  channel_ =
	    CreateCustomChannel (uri.str (), InsecureChannelCredentials (),
				 args);
	...
	void
	SendRpcAndCount (int *num_total, int *num_ok, int *num_failure,
			 int *num_drops)
	{
	  const Status
	    status = SendRpc ();
	...
	WaitForAllBackends (int num_requests_multiple_of = 1)
	{
	  int
	    num_ok = 0;
	  int
	...
	void
	WaitForBackend (size_t backend_idx)
	{
	  do
	    {
	      CheckRpcSendOk ();
	...
	void
	SetNextResolution (const std::vector < AddressData > &address_data)
	{
	  ...
	  for (size_t i = 0; i < address_data.size (); ++i)
	    {
	      char *
		lb_uri_str;
	      gpr_asprintf (&lb_uri_str, "ipv4:127.0.0.1:%d",
			    address_data[i].port);
	      grpc_uri *
		lb_uri = grpc_uri_parse (lb_uri_str, true);
	  ...
	  grpc_arg
	    fake_addresses = grpc_lb_addresses_create_channel_arg (addresses);
	  grpc_channel_args
	  fake_result = { 1, &fake_addresses };
	...
	const
	std::vector < int >
	GetBackendPorts (const size_t start_index = 0) const
	{
	  std::vector < int >
	    backend_ports;
	...
	void
	ScheduleResponseForBalancer (size_t i,
	...
	Status
	SendRpc (EchoResponse * response = nullptr, int timeout_ms = 1000)
	{
	  const bool
	    local_response = (response == nullptr);
	  if (local_response)
	  ...
	  ClientContext
	    context;
	  context.
	    set_deadline (grpc_timeout_milliseconds_to_deadline (timeout_ms));
	  Status
	...
	void
	CheckRpcSendOk (const size_t times = 1)
	{
	...
	template < typename T > struct ServerThread
	{
	  explicit
	  ServerThread (const grpc::string & type,
	  ...
	  service_ (service)
	  {
	    ...
	    gpr_log (GPR_INFO, "starting %s server on port %d",
		     type_.c_str (), port_);
	    ...
	    reset (new std::
		   thread (std::
			   bind (&ServerThread::Start, this, server_host, &mu,
				 &cond)));
	    cond.
	    wait (lock);
	  ...
	  void
	  Start (const grpc::string & server_host, std::mutex * mu,
	...
	SetNextResolutionAllBalancers ();
	const int
	  kServerlistDelayMs = 500 * grpc_test_slowdown_factor ();
	const int
	  kCallDeadlineMs = 1000 * grpc_test_slowdown_factor ();
	...
	SetNextResolutionAllBalancers ();
	const int
	  kFallbackTimeoutMs = 200 * grpc_test_slowdown_factor ();
	const int
	  kServerlistDelayMs = 500 * grpc_test_slowdown_factor ();
	const size_t
	  kNumBackendInResolution = backends_.size () / 2;

	ResetStub (kFallbackTimeoutMs);
	std::vector < AddressData > addresses;
	addresses.emplace_back (AddressData
				{
				balancer_servers_[0].port_, true, ""});
	for (size_t i = 0; i < kNumBackendInResolution; ++i)
	  {
	    addresses.emplace_back (AddressData
				    {
				    backend_servers_[i].port_, false, ""}
	    );
	  }
	SetNextResolution (addresses);
	...
	const int
	  kServerlistDelayMs = 500 * grpc_test_slowdown_factor ();
	const size_t
	  kNumBackendInResolution = backends_.size () / 3;
	const size_t
	  kNumBackendInResolutionUpdate = backends_.size () / 3;
	...
	for (size_t i =
	     kNumBackendInResolution + kNumBackendInResolutionUpdate;
	     i < backends_.size (); ++i)
	  {
	    EXPECT_EQ (0U, backend_servers_[i].service_->request_count ());
	  }
	...
	ScheduleResponseForBalancer (0,
				     BalancerServiceImpl::
				     BuildResponseForBackends (first_backend,
							       {
							       }), 0);
	ScheduleResponseForBalancer (1,
				     BalancerServiceImpl::
	...
	addresses.emplace_back (AddressData
				{
				balancer_servers_[1].port_, true, ""});
...
// by definition is also present in the update).
	...
	addresses.emplace_back (AddressData
				{
				balancer_servers_[2].port_, true, ""});
	gpr_log (GPR_INFO, "========= ABOUT TO UPDATE 1 ==========");
	SetNextResolution (addresses);
	gpr_log (GPR_INFO, "========= UPDATE 1 DONE ==========");
	...
	balancers_[0]->NotifyDoneWithServerlists ();
	if (balancers_[0]->Shutdown ())
	  balancer_servers_[0].Shutdown ();
	...
	const size_t
	  kNumRpcsPerAddress = 100;
	const int
	  num_of_drop_by_rate_limiting_addresses = 1;
	const int
	  num_of_drop_by_load_balancing_addresses = 2;
	const int
	  num_of_drop_addresses = num_of_drop_by_rate_limiting_addresses +
	  num_of_drop_by_load_balancing_addresses;
	const int
	  num_total_addresses = num_backends_ + num_of_drop_addresses;
	ScheduleResponseForBalancer (0,
				     BalancerServiceImpl::
				     BuildResponseForBackends (GetBackendPorts
							       (),
	...
	const int
	  num_of_drop_by_rate_limiting_addresses = 1;
	const int
	  num_of_drop_by_load_balancing_addresses = 1;
	ScheduleResponseForBalancer (0,
				     BalancerServiceImpl::
	...
	EXPECT_THAT (client_stats.
		     drop_token_counts,::testing::ElementsAre ());
      }
	...
	SetNextResolutionAllBalancers ();
	const size_t
	  kNumRpcsPerAddress = 3;
	const int
	  num_of_drop_by_rate_limiting_addresses = 2;
	const int
	  num_of_drop_by_load_balancing_addresses = 1;
	const int
	  num_of_drop_addresses = num_of_drop_by_rate_limiting_addresses +
	...
	int
	  num_warmup_ok = 0;
	int
	  num_warmup_failure = 0;
	int
	  num_warmup_drops = 0;
	std::tie (num_warmup_ok, num_warmup_failure, num_warmup_drops) =
	  WaitForAllBackends (num_total_addresses
			      /* num_requests_multiple_of */ );
	const int
	...
	EXPECT_EQ (kNumRpcsPerAddress * num_total_addresses +
		   num_total_warmup_requests, client_stats.num_calls_started);
	EXPECT_EQ (kNumRpcsPerAddress * num_total_addresses +
		   num_total_warmup_requests,
		   client_stats.num_calls_finished);
	EXPECT_EQ (0U,
	...
	const int
	  num_times_drop_addresses_hit =
	  num_warmup_drops / num_of_drop_addresses;
	EXPECT_THAT (client_stats.
		     drop_token_counts,::testing::ElementsAre (::testing::
							       Pair
							       ("load_balancing",
								(kNumRpcsPerAddress
								 +
