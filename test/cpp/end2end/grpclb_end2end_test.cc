...
class BalancerServiceImpl : public BalancerService {
 public:
  ...
  void Shutdown() {
    ...
    ShutdownStream();
    ABSL_LOG(INFO) << "LB[" << this << "]: shut down";
  }
 ...
 private:
  ...
  Status BalanceLoad(ServerContext* context, Stream* stream) override {
    ABSL_LOG(INFO) << "LB[" << this << "]: BalanceLoad";
    {
      grpc_core::MutexLock lock(&mu_);
      if (shutdown_) {
        ABSL_LOG(INFO) << "LB[" << this << "]: shutdown at stream start";
        return Status::OK;
      }
    }
    ...
    if (!stream->Read(&request)) {
      ABSL_LOG(INFO) << "LB[" << this << "]: stream read returned false";
      return Status::OK;
    }
    ...
    IncreaseRequestCount();
    ABSL_LOG(INFO) << "LB[" << this << "]: received initial message '"
              << request.DebugString() << "'";
    ...
    auto thread_cleanup = absl::MakeCleanup([&]() {
      ABSL_LOG(INFO) << "shutting down reader thread";
      reader_shutdown.Notify();
      ABSL_LOG(INFO) << "joining reader thread";
      reader.join();
      ABSL_LOG(INFO) << "joining reader thread complete";
    });
    ...
    while (true) {
      ...
      ABSL_LOG(INFO) << "LB[" << this
                << "]: Sending response: " << response->DebugString();
      IncreaseResponseCount();
      stream->Write(*response);
    }
    ABSL_LOG(INFO) << "LB[" << this << "]: done";
    return Status::OK;
  }
  ...
  void ReadThread(Stream* stream, absl::Notification* shutdown) {
    ...
    while (!shutdown->HasBeenNotified() && stream->Read(&request)) {
      ABSL_LOG(INFO) << "LB[" << this << "]: received client load report message '"
                << request.DebugString() << "'";
...
class GrpclbEnd2endTest : public ::testing::Test {
 protected:
  ...
  std::tuple<int, int, int> WaitForAllBackends(
    ...
    ABSL_LOG(INFO) << "Waiting for backends [" << start_index << ", " << stop_index
              << ")";
    ...
    ABSL_LOG(INFO) << "Performed " << num_total
  ...
  void SetNextResolutionFromEndpoints(
    ...
    ABSL_CHECK_OK(result.service_config);
...
TEST_F(GrpclbEnd2endTest, UsePickFirstChildPolicy) {
  ...
  for (size_t i = 0; i < backends_.size(); ++i) {
    if (backends_[i]->service().request_count() > 0) {
      ABSL_LOG(INFO) << "backend " << i << " saw traffic";
...
TEST_F(GrpclbEnd2endTest,
  ...
  // use it.
  ABSL_LOG(INFO) << "========= ABOUT TO UPDATE 1 ==========";
  SetNextResolution({balancer2->port()});
  ABSL_LOG(INFO) << "========= UPDATE 1 DONE ==========";
...
TEST_F(GrpclbEnd2endTest,
  ...
  ABSL_LOG(INFO) << "========= SHUTTING DOWN BALANCER CALL ==========";
  balancer_->service().ShutdownStream();
  ABSL_LOG(INFO) << "========= DONE SHUTTING DOWN BALANCER CALL ==========";
...
TEST_F(GrpclbEnd2endTest, BalancerDiesThenSwitchToNewBalancer) {
  ...
  ABSL_LOG(INFO) << "********** ABOUT TO KILL BALANCER *************";
  balancer_->Shutdown();
  ABSL_LOG(INFO) << "********** KILLED BALANCER *************";
  ...
  ABSL_LOG(INFO) << "========= BEFORE THIRD BATCH ==========";
  CheckRpcSendOk(10);
  ABSL_LOG(INFO) << "========= DONE WITH THIRD BATCH ==========";
...
TEST_F(GrpclbEnd2endTest, ReresolveDeadBackendWhileInFallback) {
  ...
  EXPECT_EQ(10U, backends_[0]->service().request_count());
  // Kill backend 0.
  ABSL_LOG(INFO) << "********** ABOUT TO KILL BACKEND 0 *************";
  backends_[0]->Shutdown();
  ABSL_LOG(INFO) << "********** KILLED BACKEND 0 *************";
...
TEST_F(GrpclbEnd2endTest, ReresolveWhenBalancerCallFails) {
  ...
  EXPECT_EQ(0U, balancer2->service().response_count());
  // Kill balancer 0.
  ABSL_LOG(INFO) << "********** ABOUT TO KILL BALANCER 0 *************";
  balancer_->Shutdown();
  ABSL_LOG(INFO) << "********** KILLED BALANCER 0 *************";
  // This should trigger a re-resolution.
  EXPECT_TRUE(response_generator_->WaitForReresolutionRequest(
      absl::Seconds(5 * grpc_test_slowdown_factor())));
  ABSL_LOG(INFO) << "********** SAW RE-RESOLUTION REQUEST *************";
