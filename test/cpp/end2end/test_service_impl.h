...
class TestServiceSignaller {
 public:
  ...
  void ServerWaitToContinue() {
    std::unique_lock<std::mutex> lock(mu_);
    cv_server_continue_.wait(
        lock, [this] { return server_should_continue_; });
  }
 ...
 public:
  ...
  Status Echo(ServerContext* context, const EchoRequest* request,
    ...
    if (server_try_cancel > DO_NOT_CANCEL) {
      ...
      // called, the 'request' message is already read from the client.
      // So the scenarios in server_try_cancel don't make much sense.
      // Just cancel the RPC as long as server_try_cancel is not
      // DO_NOT_CANCEL
    ...
    if (request->has_param() &&
      ...
      response->set_message(std::string(
          request->param().response_message_length(), '\0'));
    }
  ...
  Status CheckClientInitialMetadata(ServerContext* context,
    ...
    EXPECT_EQ(
        internal::MetadataMatchCount(context->client_metadata(),
                                     kCheckClientInitialMetadataKey,
                                     kCheckClientInitialMetadataVal),
        1);
    EXPECT_EQ(1u, context->client_metadata().count(
                      kCheckClientInitialMetadataKey));
    return Status::OK;
  }
  ...
  Status RequestStream(ServerContext* context,
    ...
    //   CANCEL_DURING_PROCESSING: The RPC is cancelled while the server
    //   is reading messages from the client CANCEL_AFTER_PROCESSING:
    //   The RPC is cancelled after the server reads all the messages
    //   from the client
    int server_try_cancel = internal::GetIntValueFromMetadata(
        kServerTryCancelRequest, context->client_metadata(),
        DO_NOT_CANCEL);
    ...
    if (server_try_cancel == CANCEL_DURING_PROCESSING) {
      server_try_cancel_thd = new std::thread(
          [context] { internal::ServerTryCancel(context); });
    }
  ...
  Status ResponseStream(ServerContext* context,
    ...
    // the value:
    //   CANCEL_BEFORE_PROCESSING: The RPC is cancelled before the
    //   server writes any messages to the client
    //   CANCEL_DURING_PROCESSING: The RPC is cancelled while the server
    //   is writing messages to the client CANCEL_AFTER_PROCESSING: The
    //   RPC is cancelled after the server writes all the messages to
    //   the client
    int server_try_cancel = internal::GetIntValueFromMetadata(
        kServerTryCancelRequest, context->client_metadata(),
        DO_NOT_CANCEL);
  ...
  Status BidiStream(
    ...
    // the value:
    //   CANCEL_BEFORE_PROCESSING: The RPC is cancelled before the
    //   server reads/ writes any messages from/to the client
    //   CANCEL_DURING_PROCESSING: The RPC is cancelled while the server
    //   is reading/writing messages from/to the client
    ...
    // kServerFinishAfterNReads suggests after how many reads, the
    // server should write the last message and send status (coalesced
    // using WriteLast)
