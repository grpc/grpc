...
const auto
  original_tcp_connect_fn = grpc_tcp_client_connect_impl;
  ...
  grpc
{
    ...
    testing
  {
    namespace
    {
      ...
      gpr_atm
	g_connection_delay_ms;

      void
      tcp_client_connect_with_delay (grpc_closure * closure,
				     grpc_endpoint ** ep,
      ...
      {
	const int
	  delay_ms = gpr_atm_acq_load (&g_connection_delay_ms);
	if (delay_ms > 0)
	  {
	    gpr_sleep_until (grpc_timeout_milliseconds_to_deadline
			     (delay_ms));
	  }
	original_tcp_connect_fn (closure, ep, interested_parties,
...
// every call to the Echo RPC.
	...
	void
	StartServers (size_t num_servers,
	...
	void
	SetNextResolution (const std::vector < int >&ports)
	{
	  grpc_core::ExecCtx exec_ctx;
	  grpc_lb_addresses *
	    addresses = grpc_lb_addresses_create (ports.size (), nullptr);
	...
	for (const auto & server:servers_)
	    ports.push_back (server->port_);
	  return ports;
	}

	void
	ResetStub (const std::vector < int >&ports,
		   ...
		   ChannelArguments args = ChannelArguments ())
	{
	  ...
	  args.SetPointer (GRPC_ARG_FAKE_RESOLVER_RESPONSE_GENERATOR,
			   response_generator_);
	  channel_ =
	    CreateCustomChannel ("fake:///", InsecureChannelCredentials (),
				 args);
	  stub_ = grpc::testing::EchoTestService::NewStub (channel_);
	}

	bool
	SendRpc (EchoResponse * response = nullptr)
	{
	  const bool
	...
	struct ServerData
	{
	  ...
	  explicit
	  ServerData (const grpc::string & server_host, int port = 0)
	  {
	    ...
	    reset (new std::
		   thread (std::
			   bind (&ServerData::Start, this, server_host, &mu,
				 &cond)));
	    cond.
	    wait (lock,[this]
		  {
	...
	void
	WaitForServer (size_t server_idx)
	{
	  do
	    {
	      CheckRpcSendOk ();
	...
	for (const auto & server:servers_)
	    {
	      if (server->service_.request_count () == 0)
		return false;
	    }
	  return true;
	}
	...
	void
	UpdateConnectionOrder (const std::vector < std::unique_ptr <
			       ServerData >> &servers,
			       std::vector < int >*connection_order)
	{
	  for (size_t i = 0; i < servers.size (); ++i)
	      ...
	      if (servers[i]->service_.request_count () == 1)
		{
		  ...
		  const auto
		    it =
		    std::find (connection_order->begin (),
	...
	for (size_t i = 0; i < servers_.size (); ++i)
	  {
	    const int
	      request_count = servers_[i]->service_.request_count ();
	...
	ChannelArguments
	  args;
	constexpr int
	  kInitialBackOffMs = 100;
	args.SetInt (GRPC_ARG_INITIAL_RECONNECT_BACKOFF_MS,
		     kInitialBackOffMs);
	const
	std::vector < int >
	ports = { grpc_pick_unused_port_or_die () };
	const gpr_timespec
	  t0 = gpr_now (GPR_CLOCK_MONOTONIC);
	ResetStub (ports, "pick_first", args);
	SetNextResolution (ports);
	// The channel won't become connected (there's no server).
	ASSERT_FALSE (channel_->
		      WaitForConnected (grpc_timeout_milliseconds_to_deadline
					(kInitialBackOffMs * 2)));
	...
	ASSERT_TRUE (channel_->
		     WaitForConnected (grpc_timeout_milliseconds_to_deadline
				       (kInitialBackOffMs * 2)));
	const gpr_timespec
	  t1 = gpr_now (GPR_CLOCK_MONOTONIC);
	const grpc_millis
	  waited_ms = gpr_time_to_millis (gpr_time_sub (t1, t0));
	gpr_log (GPR_DEBUG, "Waited %ld milliseconds", waited_ms);
	// We should have waited at least kInitialBackOffMs. We substract one to
	// account for test and precision accuracy drift.
	EXPECT_GE (waited_ms, kInitialBackOffMs - 1);
	// But not much more.
	EXPECT_GT (gpr_time_cmp
		   (grpc_timeout_milliseconds_to_deadline
		    (kInitialBackOffMs * 1.10), t1), 0);
      }
	...
	for (size_t i = 0; i < servers_.size (); ++i)
      ...
      for (const bool force_creation:{
	     true, false})
	  {
	    grpc_subchannel_index_test_only_set_force_creation
	      (force_creation);
	    gpr_log (GPR_INFO, "Force subchannel creation: %d",
		     force_creation);
	...
	const gpr_timespec
	  deadline = grpc_timeout_seconds_to_deadline (5);
	gpr_timespec
	...
	const auto
	  pre_death = servers_[0]->service_.request_count ();
	...
	for (int i = 0; i < 10 * kNumServers; ++i)
	  CheckRpcSendOk ();
	const auto
	  post_death = servers_[0]->service_.request_count ();
