  ...
  grpc
{
    ...
    testing
  {
...
#ifdef GRPC_POSIX_SOCKET
      static int
      maybe_assert_non_blocking_poll (struct pollfd *pfds, nfds_t nfds,
				      int timeout)
      {
	...
	PollOverride
      ...
      public:
	explicit
	PollingOverrider (bool allow_blocking):
	...
	Verifier & ExpectMaybe (int i, bool expect_ok, bool * seen)
	{
	  if (!*seen)
	    {
	      maybe_expectations_[tag (i)] = MaybeExpect
	      {
	      expect_ok, seen};
	    }
	  return *this;
	...
	int
	Next (CompletionQueue * cq, bool ignore_ok)
	{
	  bool
	    ok;
	  void *
	...
	template < typename T >
	  CompletionQueue::NextStatus DoOnceThenAsyncNext (CompletionQueue *
							   cq, void **got_tag,
							   bool * ok,
							   T deadline,
							   std::function <
	...
	void
	Verify (CompletionQueue * cq, bool ignore_ok)
	{
	  GPR_ASSERT (!expectations_.empty ()
		      || !maybe_expectations_.empty ());
		  ...
		  if (spin_)
		    {
		      for (;;)
			{
			  GPR_ASSERT (std::chrono::system_clock::now () <
				      deadline);
			  auto
			    r =
			    cq->AsyncNext (&got_tag, &ok,
			  ...
			  if (r == CompletionQueue::GOT_EVENT)
			    break;
			  gpr_log (GPR_ERROR,
				   "unexpected result from AsyncNext");
			  abort ();
			}
		    }
		  else
		...
		std::function < void (void) > lambda)
	{
	  if (expectations_.empty ())
	      ...
	      if (spin_)
		{
		  while (std::chrono::system_clock::now () < deadline)
		    {
		      EXPECT_EQ (DoOnceThenAsyncNext
				 (cq, &got_tag, &ok, deadline, lambda),
				 CompletionQueue::TIMEOUT);
		    }
		}
	      else
	      ...
	      while (!expectations_.empty ())
		{
		  ...
		  if (spin_)
		      ...
		      for (;;)
			{
			  ...
			  auto
			    r =
			    DoOnceThenAsyncNext (cq, &got_tag, &ok,
						 gpr_time_0
						 (GPR_CLOCK_REALTIME),
						 lambda);
			  if (r == CompletionQueue::TIMEOUT)
      ...
      private:
	void
	GotTag (void *got_tag, bool ok, bool ignore_ok)
	{
	  auto
	    it = expectations_.find (got_tag);
	  if (it != expectations_.end ())
	    ...
	    {
	      auto
		it2 = maybe_expectations_.find (got_tag);
	...
	bool
      ...
      bool
      plugin_has_sync_methods (std::unique_ptr < ServerBuilderPlugin >
			       &plugin)
      {
...
// that needs to be tested here.
      class
	ServerBuilderSyncPluginDisabler:
	public::grpc::ServerBuilderOption
      {
      public:
	void
	UpdateArguments (ChannelArguments * arg)
	  override
	{
	}

	void
	UpdatePlugins (std::vector < std::unique_ptr <
		       ServerBuilderPlugin >> *plugins)
	  override
	{
	...
	TestScenario
      {
      public:
	TestScenario (bool non_block, bool inproc_stub,
		      const grpc::string & creds_type, bool hcs,
	...
	const
      ...
      static
	std::ostream &
      operator<< (std::ostream & out, const TestScenario & scenario)
      {
	...
	{
	  poll_overrider_.
	    reset (new PollingOverrider (!GetParam ().disable_blocking));
	...
	BuildAndStartServer ()
	{
	  ...
	  service_.
	    reset (new grpc::testing::EchoTestService::AsyncService ());
	  ...
	  std::unique_ptr < ServerBuilderOption >
	    sync_plugin_disabler (new ServerBuilderSyncPluginDisabler ());
	...
	ResetStub ()
	{
	  ...
	  std::shared_ptr < Channel > channel =
	    !(GetParam ().inproc) ? CreateCustomChannel (server_address_.
							 str (),
							 channel_creds,
							 args) : server_->
	    InProcessChannel (args);
	  stub_ = grpc::testing::EchoTestService::NewStub (channel);
	}

	void
	SendRpc (int num_rpcs)
	{
	  for (int i = 0; i < num_rpcs; i++)
		...
		cli_ctx;
	      ServerContext
		srv_ctx;
	      grpc::ServerAsyncResponseWriter < EchoResponse >
		response_writer (&srv_ctx);

	      send_request.set_message (GetParam ().message_content);
	      std::unique_ptr < ClientAsyncResponseReader <
		EchoResponse >> response_reader (stub_->
						 AsyncEcho (&cli_ctx,
							    send_request,
							    cq_.get ()));

	      service_->RequestEcho (&srv_ctx, &recv_request,
				     &response_writer, cq_.get (), cq_.get (),
				     tag (2));

	      Verifier (GetParam ().disable_blocking).Expect (2,
							      true).
		Verify (cq_.get ());
	      ...
	      Verifier (GetParam ().disable_blocking).Expect (3,
							      true).Expect (4,
									    true).
		Verify (cq_.get ());
	...
	int
	  poller_slowdown_factor = 1;
	// It needs 2 pollset_works to reconnect the channel with polling engine
	// "poll"
	char *
	  s = gpr_getenv ("GRPC_POLL_STRATEGY");
...
// We do not need to protect notify because the use is synchronized.
      void
      ServerWait (Server * server, int *notify)
      {
	server->Wait ();
	*notify = 1;
      }
...
// Test a simple RPC using the async version of Next
	...
	std::unique_ptr < ClientAsyncResponseReader <
	  EchoResponse >> response_reader (stub_->
					   AsyncEcho (&cli_ctx, send_request,
						      cq_.get ()));

	std::chrono::system_clock::time_point time_now (std::chrono::
							system_clock::now ());
	std::chrono::system_clock::time_point time_limit (std::chrono::
							  system_clock::
							  now () +
							  std::chrono::
							  seconds (10));
	...
	response_writer.Finish (send_response, Status::OK, tag (3));
	response_reader->Finish (&recv_response, &recv_status, tag (4));
	Verifier (GetParam ().disable_blocking).Expect (3, true).Expect (4,
									 true).
...
// Test a simple RPC using the async version of Next
	...
	auto
	  resp_writer_ptr = &response_writer;
	auto
	  lambda_2 =[&, this, resp_writer_ptr] (){
	  gpr_log (GPR_ERROR, "CALLED");
	  service_->RequestEcho (&srv_ctx, &recv_request, resp_writer_ptr,
				 cq_.get (), cq_.get (), tag (2));
	};
	...
	auto
	  recv_resp_ptr = &recv_response;
	auto
	  status_ptr = &recv_status;
	send_response.set_message (recv_request.message ());
	auto
	  lambda_3 =[&, this, resp_writer_ptr, send_response] (){
...
// Two pings and a final pong.
	...
	std::unique_ptr < ClientAsyncWriter <
	  EchoRequest >> cli_stream (stub_->
				     AsyncRequestStream (&cli_ctx,
							 &recv_response,
							 cq_.get (),
							 tag (1)));
	...
	Verifier (GetParam ().disable_blocking).Expect (2, true).Expect (1,
									 true).
	  Verify (cq_.get ());
	...
	Verifier (GetParam ().disable_blocking).Expect (5, true).Expect (6,
									 true).
	  Verify (cq_.get ());
	...
	Verifier (GetParam ().disable_blocking).Expect (7, true).Expect (8,
									 false).
	  Verify (cq_.get ());
	...
	Verifier (GetParam ().disable_blocking).Expect (9, true).Expect (10,
									 true).
	  Verify (cq_.get ());
...
// Two pings and a final pong.
	...
	Verifier (GetParam ().disable_blocking).ExpectUnless (3, true,
							      seen3).
	  Expect (4, true).Verify (cq_.get ());
	...
	Verifier (GetParam ().disable_blocking).Expect (7,
							false).Verify (cq_.
								       get
								       ());
	...
	Verifier (GetParam ().disable_blocking).Expect (8, true).Expect (9,
									 true).
	  Verify (cq_.get ());
...
// One ping, two pongs.
	...
	std::unique_ptr < ClientAsyncReader <
	  EchoResponse >> cli_stream (stub_->
				      AsyncResponseStream (&cli_ctx,
							   send_request,
							   cq_.get (),
							   tag (1)));
	...
	Verifier (GetParam ().disable_blocking).Expect (1, true).Expect (2,
									 true).
	  Verify (cq_.get ());
	EXPECT_EQ (send_request.message (), recv_request.message ());
	...
	cli_stream->Finish (&recv_status, tag (9));
	Verifier (GetParam ().disable_blocking).Expect (9,
							true).Verify (cq_.
								      get ());
...
// One ping, two pongs. Using WriteAndFinish API
	...
	Verifier (GetParam ().disable_blocking).Expect (8,
							true).Verify (cq_.
								      get ());
...
// One ping, one pong.
	...
	ServerContext
	  srv_ctx;
	ServerAsyncReaderWriter < EchoResponse,
	  EchoRequest > srv_stream (&srv_ctx);

	send_request.set_message (GetParam ().message_content);
	std::unique_ptr < ClientAsyncReaderWriter < EchoRequest,
	  EchoResponse >> cli_stream (stub_->
				      AsyncBidiStream (&cli_ctx, cq_.get (),
						       tag (1)));
...
// One ping, one pong. Using server:WriteAndFinish api
	...
	Verifier (GetParam ().disable_blocking).Expect (5,
							false).Verify (cq_.
								       get
								       ());
	...
	srv_stream.WriteAndFinish (send_response, WriteOptions (), Status::OK,
				   tag (6));
	cli_stream->Read (&recv_response, tag (7));
	Verifier (GetParam ().disable_blocking).Expect (6, true).Expect (7,
									 true).
	  Verify (cq_.get ());
	EXPECT_EQ (send_response.message (), recv_response.message ());
...
// Metadata tests
	...
	EXPECT_EQ (meta1.second,
		   ToString (client_initial_metadata.find (meta1.first)->
			     second));
	EXPECT_EQ (meta2.second,
		   ToString (client_initial_metadata.find (meta2.first)->
			     second));
	EXPECT_EQ (meta3.second,
		   ToString (client_initial_metadata.find (meta3.first)->
			     second));
	EXPECT_GE (client_initial_metadata.size (),
	...
	Verifier (GetParam ().disable_blocking).Expect (4,
							true).Verify (cq_.
								      get ());
	auto
	  server_initial_metadata = cli_ctx.GetServerInitialMetadata ();
	EXPECT_EQ (meta1.second,
		   ToString (server_initial_metadata.find (meta1.first)->
			     second));
	EXPECT_EQ (meta2.second,
		   ToString (server_initial_metadata.find (meta2.first)->
			     second));
	EXPECT_EQ (static_cast < size_t > (2),
		   server_initial_metadata.size ());
	...
	Verifier (GetParam ().disable_blocking).Expect (4, true).Expect (5,
									 true).
	  Verify (cq_.get ());
	...
	EXPECT_TRUE (recv_status.ok ());
	auto
	  server_trailing_metadata = cli_ctx.GetServerTrailingMetadata ();
	EXPECT_EQ (meta1.second,
		   ToString (server_trailing_metadata.find (meta1.first)->
			     second));
	EXPECT_EQ (meta2.second,
		   ToString (server_trailing_metadata.find (meta2.first)->
			     second));
	EXPECT_EQ (static_cast < size_t > (2),
		   server_trailing_metadata.size ());
      }
	...
	send_request.set_message (GetParam ().message_content);
	std::pair < grpc::string, grpc::string > meta1 ("key1", "val1");
	std::pair < grpc::string, grpc::string > meta2 ("key2-bin",
							grpc::
							string
	...
	auto
	  server_initial_metadata = cli_ctx.GetServerInitialMetadata ();
	EXPECT_EQ (meta3.second,
		   ToString (server_initial_metadata.find (meta3.first)->
			     second));
	EXPECT_EQ (meta4.second,
		   ToString (server_initial_metadata.find (meta4.first)->
			     second));
	EXPECT_GE (server_initial_metadata.size (),
		   static_cast < size_t > (2));
	...
	EXPECT_EQ (meta5.second,
		   ToString (server_trailing_metadata.find (meta5.first)->
			     second));
	EXPECT_EQ (meta6.second,
		   ToString (server_trailing_metadata.find (meta6.first)->
			     second));
	EXPECT_GE (server_trailing_metadata.size (),
		   static_cast < size_t > (2));
      }
...
// Server uses AsyncNotifyWhenDone API to check for normal finish
	...
	auto
	  ...
	  GetCredentialsProvider ()->GetChannelCredentials (GetParam ().
							    ...
							    &args);
	std::shared_ptr < Channel > channel =
	  !(GetParam ().inproc) ? CreateCustomChannel (server_address_.str (),
						       channel_creds,
						       args) : server_->
	  InProcessChannel (args);
	std::unique_ptr < grpc::testing::UnimplementedEchoService::Stub >
	  stub;
	...
	send_request.set_message (GetParam ().message_content);
	std::unique_ptr < ClientAsyncResponseReader <
	  EchoResponse >> response_reader (stub->
					   AsyncUnimplemented (&cli_ctx,
							       send_request,
							       cq_.get ()));
...
// API to check for cancellation
	...
	//   client)
	void
	TestClientStreamingServerCancel (ServerTryCancelRequestPhase
					 server_try_cancel)
	{
	  ResetStub ();
	  ...
	  std::unique_ptr <
	    ...
	    EchoRequest >>
	  cli_stream (stub_->
		      AsyncRequestStream (&cli_ctx, &recv_response, &cli_cq,
					  tag (1)));

	  // On the server, request to be notified of 'RequestStream' calls
	  // and receive the 'RequestStream' call just made by the client
	  srv_ctx.
	  AsyncNotifyWhenDone (tag (11));
	  service_->
	  RequestRequestStream (&srv_ctx, &srv_stream, cq_.get (), cq_.get (),
				tag (2));
	  std::thread
	  t1 ([this, &cli_cq]
	      {
	      Verifier (GetParam ().disable_blocking).Expect (1,
							      true).
	      Verify (&cli_cq);});
	  Verifier (GetParam ().disable_blocking).Expect (2,
							  true).Verify (cq_.
	  ...
	  bool
	    expected_server_cq_result = true;
	  bool
	    expected_client_cq_result = true;

	  if (server_try_cancel == CANCEL_BEFORE_PROCESSING)
	    {
	      srv_ctx.TryCancel ();
	      Verifier (GetParam ().disable_blocking).Expect (11,
							      true).
		Verify (cq_.get ());
	      EXPECT_TRUE (srv_ctx.IsCancelled ());
	  ...
	  std::
	    thread
	    cli_thread ([&cli_cq, &cli_stream, &expected_client_cq_result,
			...
			send_request.set_message ("Ping " +
						  grpc::to_string (tag_idx));
			cli_stream->Write (send_request, tag (tag_idx));
			Verifier (GetParam ().disable_blocking).
			Expect (tag_idx,
				expected_client_cq_result).Verify (&cli_cq,
								   ignore_client_cq_result);}
			cli_stream->WritesDone (tag (6));
			// Ignore ok on WritesDone since cancel can affect it
			Verifier (GetParam ().disable_blocking).Expect (6,
									expected_client_cq_result).
			Verify (&cli_cq, ignore_client_cq_result);});

	  bool
	    ignore_cq_result = false;
	  bool
	    want_done_tag = false;
	  std::thread * server_try_cancel_thd = nullptr;

	  auto
	    verif = Verifier (GetParam ().disable_blocking);
	  ...
	  for (int tag_idx = 6; tag_idx <= 8; tag_idx++)
	    {
	      ...
	      int
		got_tag =
		verif.Expect (tag_idx,
			      expected_server_cq_result).Next (cq_.get (),
							       ignore_cq_result);
	      GPR_ASSERT ((got_tag == tag_idx)
	      ...
	      if (got_tag == 11)
		{
		  ...
		  EXPECT_EQ (verif.Next (cq_.get (), ignore_cq_result),
			     tag_idx);
		}
	    }
	  ...
	  cli_stream->Finish (&recv_status, tag (10));
	  Verifier (GetParam ().disable_blocking).Expect (10,
							  true).
	    Verify (&cli_cq);
	...
	//   client)
	void
	TestServerStreamingServerCancel (ServerTryCancelRequestPhase
					 server_try_cancel)
	{
	  ResetStub ();
	  ...
	  service_->RequestResponseStream (&srv_ctx, &recv_request,
					   &srv_stream, cq_.get (),
					   cq_.get (), tag (2));
	  ...
	  for (int tag_idx = 3; tag_idx <= 5; tag_idx++)
	    {
	      ...
	      int
		got_tag =
		verif.Expect (tag_idx, expected_cq_result).Next (cq_.get (),
								 ignore_cq_result);
	      GPR_ASSERT ((got_tag == tag_idx)
			  || (got_tag == 11 && want_done_tag));
	      if (got_tag == 11)
	...
	//   client)
	void
	TestBidiStreamingServerCancel (ServerTryCancelRequestPhase
				       server_try_cancel)
	{
	  ResetStub ();
	  ...
	  if (server_try_cancel == CANCEL_BEFORE_PROCESSING)
	      ...
	      do
		{
		  ...
		  GPR_ASSERT (((got_tag == 3) && !tag_3_done)
			      || (got_tag == 11));
		  if (got_tag == 3)
		    {
		      tag_3_done = true;
	  ...
	  got_tag =
	    tag_3_done ? 3 : verif.Next (cq_.get (), ignore_cq_result);
	...
	auto
	  ...
	  return GetCredentialsProvider ()->
	    GetChannelCredentials (kInsecureCredentialsType,
				   nullptr) != nullptr;
	};
	...
	// https://github.com/grpc/grpc/issues/11223 is resolved
	...
	)
	  {
		...
		if (insec_ok ())
		  {
		    scenarios.emplace_back (false, true,
	...
	return scenarios;
      ...
      INSTANTIATE_TEST_CASE_P (AsyncEnd2end,
			       AsyncEnd2endTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (true, true, 1024)));
      INSTANTIATE_TEST_CASE_P (AsyncEnd2endServerTryCancel,
			       AsyncEnd2endServerTryCancelTest,::testing::
			       ValuesIn (CreateTestScenarios
					 (false, false, 0)));
