...
using ::envoy::extensions::filters::network::http_connection_manager::
    v3::HttpConnectionManager;
using ::envoy::extensions::filters::network::http_connection_manager::
    v3::HttpFilter;
using ::envoy::extensions::transport_sockets::tls::v3::
 ...
 public:
  ...
  explicit ClientStats(const ClusterStats& cluster_stats)
    ...
    for (const auto& input_locality_stats :
      ...
      locality_stats_.emplace(
          input_locality_stats.locality().sub_zone(),
...
class AdsServiceImpl
 ...
 public:
  ...
  struct EdsResourceArgs {
    struct Endpoint {
      explicit Endpoint(
          int port, HealthStatus health_status = HealthStatus::UNKNOWN,
          int lb_weight = 1)
          : port(port),
  ...
  void UnsetResource(const std::string& type_url,
                     const std::string& name) {
    grpc_core::MutexLock lock(&ads_mu_);
    ResourceTypeState& resource_type_state = resource_map_[type_url];
    ++resource_type_state.resource_type_version;
    ResourceState& resource_state =
        resource_type_state.resource_name_map[name];
    resource_state.resource_type_version =
        resource_type_state.resource_type_version;
    resource_state.resource.reset();
    gpr_log(GPR_INFO,
            "ADS[%p]: Unsetting %s resource %s; resource_type_version "
            "now %u",
            this, type_url.c_str(), name.c_str(),
            resource_type_state.resource_type_version);
    for (SubscriptionState* subscription :
         resource_state.subscriptions) {
      subscription->update_queue->emplace_back(type_url, name);
    }
  }

  void SetResource(google::protobuf::Any resource,
    ...
    gpr_log(GPR_INFO,
            "ADS[%p]: Updating %s resource %s; resource_type_version "
            "now %u",
            this, type_url.c_str(), name.c_str(),
            resource_type_state.resource_type_version);
    for (SubscriptionState* subscription :
  ...
  void SetEdsResource(const ClusterLoadAssignment& assignment) {
    ...
    SetResource(std::move(resource), kEdsTypeUrl,
 ...
 private:
  // A queue of resource type/name pairs that have changed since the
  // client subscribed to them.
  using UpdateQueue =
      std::deque<std::pair<std::string /* type url */,
                           std::string /* resource name */>>;

  // A struct representing a client's subscription to a particular
  // resource.
  struct SubscriptionState {
    // The queue upon which to place updates when the resource is
    // updated.
    UpdateQueue* update_queue;
  };

  // A struct representing the a client's subscription to all the
  // resources.
  ...
  using ResourceMap =
      std::map<std::string /* type_url */, ResourceTypeState>;

  template <class RpcApi, class DiscoveryRequest,
            class DiscoveryResponse>
  class RpcService : public RpcApi::Service {
   public:
    using Stream =
        ServerReaderWriter<DiscoveryResponse, DiscoveryRequest>;
    ...
    Status StreamAggregatedResources(ServerContext* context,
      ...
      gpr_log(GPR_INFO, "ADS[%p]: StreamAggregatedResources starts",
              this);
      ...
      // that the parent won't be destroyed until this stream is
      // complete.
      ...
      // subscribed to them.
      UpdateQueue update_queue;
      // Resources that the client will be subscribed to keyed by
      // resource type url.
      ...
      std::thread reader(std::bind(&RpcService::BlockingRead, this,
                                   stream, &requests, &stream_closed));
      // Main loop to process requests and updates.
      while (true) {
        // Boolean to keep track if the loop received any work to do: a
        // request or an update; regardless whether a response was
        // actually sent out.
        bool did_work = false;
        ...
        // Look for updates and decide what to handle.
        {
          ...
          if (!update_queue.empty()) {
            ...
            ProcessUpdate(resource_type, resource_name,
                          &subscription_map, &sent_state, &response);
          }
        }
        if (response.has_value()) {
          gpr_log(GPR_INFO, "ADS[%p]: Sending update response: %s",
                  this, response->DebugString().c_str());
          stream->Write(response.value());
        }
      ...
      gpr_log(GPR_INFO, "ADS[%p]: StreamAggregatedResources done",
              this);
   ...
   private:
    ...
    void ProcessRequest(const DiscoveryRequest& request,
      ...
      if (request.response_nonce().empty()) {
        ...
        int client_nonce;
        GPR_ASSERT(
            absl::SimpleAtoi(request.response_nonce(), &client_nonce));
        // Ignore requests with stale nonces.
        if (client_nonce < sent_state->nonce) return;
        // Check for ACK or NACK.
        auto it = parent_->resource_type_response_state_.find(
            v3_resource_type);
        if (it != parent_->resource_type_response_state_.end()) {
          if (!request.has_error_detail()) {
            ...
            gpr_log(GPR_INFO,
                    "ADS[%p]: client ACKed resource_type=%s version=%s",
                    this, request.type_url().c_str(),
            ...
            gpr_log(GPR_INFO,
                    "ADS[%p]: client NACKed resource_type=%s "
                    "version=%s: %s",
                    this, request.type_url().c_str(),
                    request.version_info().c_str(),
      ...
      for (const std::string& resource_name :
        ...
        if (parent_->MaybeSubscribe(v3_resource_type, resource_name,
            ...
            ClientNeedsResourceUpdate(
                resource_type_state, resource_state,
                sent_state->resource_type_version)) {
          gpr_log(GPR_INFO,
                  "ADS[%p]: Sending update for type=%s name=%s", this,
          ...
          gpr_log(GPR_INFO,
                  "ADS[%p]: client does not need update for type=%s "
                  "name=%s",
                  this, request.type_url().c_str(),
                  resource_name.c_str());
        }
    ...
    // Completing the building a DiscoveryResponse by adding common
    // information for all resources and by adding all subscribed
    // resources for LDS and CDS.
    ...
    static void CheckBuildVersion(
        const ::envoy::service::discovery::v3::
            DiscoveryRequest& /*request*/) {}
  ...
  bool MaybeSubscribe(const std::string& resource_type,
                      ...
                      ResourceState* resource_state,
                      UpdateQueue* update_queue) {
    // The update_queue will be null if we were not previously
    // subscribed.
  ...
  void ProcessUnsubscriptions(
    ...
    for (auto it = subscription_name_map->begin();
      ...
      gpr_log(GPR_INFO,
              "ADS[%p]: Unsubscribe to type=%s name=%s state=%p", this,
              resource_type.c_str(), resource_name.c_str(),
              &subscription_state);
  ...
  std::map<std::string /*resource_type*/, int>
      resource_type_min_versions_;
  // An instance data member containing the current state of all
  // resources. Note that an entry will exist whenever either of the
  // following is true:
  // - The resource exists (i.e., has been created by SetResource() and
  // has not
...
class LrsServiceImpl
 ...
 public:
  ...
  size_t request_count() {
    return v2_rpc_service_.request_count() +
           v3_rpc_service_.request_count();
  }

  size_t response_count() {
    return v2_rpc_service_.response_count() +
           v3_rpc_service_.response_count();
  }
 ...
 private:
  template <class RpcApi, class LoadStatsRequest,
            class LoadStatsResponse>
  class RpcService : public CountedService<typename RpcApi::Service> {
   public:
    using Stream =
        ServerReaderWriter<LoadStatsResponse, LoadStatsRequest>;
    ...
    Status StreamLoadStats(ServerContext* /*context*/,
      ...
      // Take a reference of the LrsServiceImpl object, reference will
      // go out of scope after this method exits.
      ...
      if (stream->Read(&request)) {
        CountedService<
            typename RpcApi::Service>::IncreaseRequestCount();
        ...
        CountedService<
            typename RpcApi::Service>::IncreaseResponseCount();
        // Wait for report.
        request.Clear();
        while (stream->Read(&request)) {
          gpr_log(GPR_INFO,
                  "LRS[%p]: received client load report message: %s",
...
class TestType {
 public:
  enum FilterConfigSetup {
    ...
    // Enable the fault injection filter in LDS, but override the filter
    // config
    // in route.
    kRouteOverride,
  };
  ...
  FilterConfigSetup filter_config_setup() const {
  ...
  std::string AsString() const {
    std::string retval =
        (use_fake_resolver_ ? "FakeResolver" : "XdsResolver");
 ...
 private:
  ...
  FilterConfigSetup filter_config_setup_ =
      kHTTPConnectionManagerOriginal;
...
std::string ReadFile(const char* file_path) {
  ...
  GPR_ASSERT(GRPC_LOG_IF_ERROR("load_file",
                               grpc_load_file(file_path, 0, &slice)));
  std::string file_contents(grpc_core::StringViewFromSlice(slice));
  grpc_slice_unref(slice);
  return file_contents;
}

grpc_core::PemKeyCertPairList ReadTlsIdentityPair(
    const char* key_path, const char* cert_path) {
  return grpc_core::PemKeyCertPairList{grpc_core::PemKeyCertPair(
      ReadFile(key_path), ReadFile(cert_path))};
}

// Based on StaticDataCertificateProvider, but provides alternate
// certificates if the certificate name is not empty.
class FakeCertificateProvider final
          ...
          if (it == cert_data_map_.end()) {
            grpc_error_handle error =
                GRPC_ERROR_CREATE_FROM_COPIED_STRING(
                    absl::StrCat(
                        "No certificates available for cert_name \"",
                        cert_name, "\"")
                        .c_str());
            distributor_->SetErrorForCert(cert_name,
            ...
            distributor_->SetKeyMaterials(
                cert_name, std::move(root_certificate),
...
std::shared_ptr<ChannelCredentials> CreateTlsFallbackCredentials() {
  ...
  grpc_tls_credentials_options_set_certificate_provider(
      options, grpc_core::MakeRefCounted<
...
class NoOpHttpFilter : public grpc_core::XdsHttpFilterImpl {
 public:
  ...
  GenerateFilterConfig(upb_strview /* serialized_filter_config */,
    ...
    return grpc_core::XdsHttpFilterImpl::FilterConfig{
        name_, grpc_core::Json()};
  }
...
// There is slight difference between time fetched by GPR and by C++
// system clock API. It's unclear if they are using the same syscall,
// but we do know GPR round the number at millisecond-level. This
// creates a 1ms difference, which could cause flake.
...
// Returns the number of RPCs needed to pass error_tolerance
// at 99.99994% chance. Rolling dices in drop/fault-injection generates
// a binomial distribution (if our code is not horribly wrong). Let's
// make "n" the number of samples, "p" the probability. If we have np>5
// & n(1-p)>5, we can approximately treat the binomial distribution as a
// normal distribution.
//
// For normal distribution, we can easily look up how many standard
// deviation we need to reach 99.995%. Based on Wiki's table
// https://en.wikipedia.org/wiki/68%E2%80%9395%E2%80%9399.7_rule, we
// need 5.00 sigma (standard deviation) to cover the probability area
// of 99.99994%. In another word, for a sample with size "n" probability
// "p" error-tolerance "k", we want the error always land within 5.00
// sigma. The sigma of binominal distribution and be computed as
// sqrt(np(1-p)). Hence, we have the equation:
//
//   kn <= 5.00 * sqrt(np(1-p))
size_t ComputeIdealNumRpcs(double p, double error_tolerance) {
  GPR_ASSERT(p >= 0 && p <= 1);
  size_t num_rpcs = ceil(p * (1 - p) * 5.00 * 5.00 / error_tolerance /
                         error_tolerance);
  gpr_log(GPR_INFO,
          "Sending %" PRIuPTR
...
class XdsEnd2endTest : public ::testing::TestWithParam<TestType> {
 protected:
  ...
  void SetUp() override {
    ...
    ipv6_only_ =
        !localhost_resolves_to_ipv4 && localhost_resolves_to_ipv6;
    ...
    default_listener_.mutable_api_listener()
        ->mutable_api_listener()
    ...
    for (size_t i = 0; i < num_balancers_; ++i) {
      ...
      balancers_.back()->ads_service()->SetCdsResource(
          default_cluster_);
    }
    ...
    logical_dns_cluster_resolver_response_generator_ =
        grpc_core::MakeRefCounted<
            grpc_core::FakeResolverResponseGenerator>();
    lb_channel_response_generator_ = grpc_core::MakeRefCounted<
        grpc_core::FakeResolverResponseGenerator>();
    ...
    if (GetParam().bootstrap_source() ==
      ...
      gpr_setenv("GRPC_XDS_BOOTSTRAP_CONFIG", GetParam().use_v2()
                                                  ? kBootstrapFileV2
                                                  : kBootstrapFileV3);
    } else if (GetParam().bootstrap_source() ==
               TestType::kBootstrapFromFile) {
    ...
    if (GetParam().bootstrap_source() !=
      ...
      // Make sure each test creates a new XdsClient instance rather
      // than reusing the one from the previous test.  This avoids
      // spurious failures caused when a load reporting test runs after
      // a non-load reporting test and the XdsClient is still talking to
      // the old LRS server, which fails because it's not expecting the
      // client to connect.  It also ensures that each test can
      // independently set the global channel args for the xDS channel.
  ...
  std::shared_ptr<Channel> CreateChannel(
    ...
    if (GetParam().bootstrap_source() ==
      ...
      if (xds_channel_args == nullptr)
        xds_channel_args = &xds_channel_args_;
      args.SetPointerWithVtable(
    ...
    args.SetPointerWithVtable(
        ...
        &grpc_core::FakeResolverResponseGenerator::
            kChannelArgPointerVtable);
    std::string uri =
        absl::StrCat(GetParam().use_fake_resolver() ? "fake" : "xds",
                     ":///", server_name);
    std::shared_ptr<ChannelCredentials> channel_creds =
        GetParam().use_xds_credentials()
            ? experimental::XdsCredentials(
  ...
  struct RpcOptions {
    ...
    RpcOptions& set_client_cancel_after_us(
        int rpc_client_cancel_after_us) {
  ...
  void ResetBackendCounters(size_t start_index = 0,
                            size_t stop_index = 0) {
    ...
    for (int port : ports) {
      absl::StatusOr<grpc_core::URI> lb_uri =
          grpc_core::URI::Parse(absl::StrCat(
              ipv6_only_ ? "ipv6:[::1]:" : "ipv4:127.0.0.1:", port));
  ...
  std::string CreateMetadataValueThatHashesToBackendPort(int port) {
    return absl::StrCat(ipv6_only_ ? "[::1]" : "127.0.0.1", ":", port,
                        "_0");
  }

  std::string CreateMetadataValueThatHashesToBackend(int index) {
    return CreateMetadataValueThatHashesToBackendPort(
        backends_[index]->port());
  }
  ...
  void SetNextResolution(const std::vector<int>& ports,
    ...
    if (!GetParam().use_fake_resolver())
      return;  // Not used with xds resolver.
  ...
  void SetNextResolutionForLbChannelAllBalancers(
    ...
    SetNextResolutionForLbChannel(ports, service_config_json,
                                  expected_targets, response_generator);
  }
  ...
  void SetNextResolutionForLbChannel(
    ...
    if (expected_targets != nullptr) {
      ...
      result.args = grpc_channel_args_copy_and_add(
          nullptr, &expected_targets_arg, 1);
    }
    if (response_generator == nullptr) {
  ...
  Status SendRpc(const RpcOptions& rpc_options = RpcOptions(),
    ...
    switch (rpc_options.service) {
      case SERVICE_ECHO:
        status = SendRpcMethod(&stub_, rpc_options, &context, request,
                               response);
        break;
      case SERVICE_ECHO1:
        status = SendRpcMethod(&stub1_, rpc_options, &context, request,
                               response);
        break;
      case SERVICE_ECHO2:
        status = SendRpcMethod(&stub2_, rpc_options, &context, request,
                               response);
        break;
    }
  ...
  CreateEndpointsForBackends(
    ...
    for (size_t i = start_index; i < stop_index; ++i) {
      endpoints.emplace_back(
          CreateEndpoint(i, health_status, lb_weight));
    }
    return endpoints;
  }
  ...
  ClusterLoadAssignment BuildEdsResource(
    ...
    for (const auto& locality : args.locality_list) {
      ...
      endpoints->mutable_load_balancing_weight()->set_value(
          locality.lb_weight);
      ...
      for (size_t i = 0; i < locality.endpoints.size(); ++i) {
        ...
        if (locality.endpoints.size() > i &&
            locality.endpoints[i].health_status !=
                HealthStatus::UNKNOWN) {
          lb_endpoints->set_health_status(
              locality.endpoints[i].health_status);
        }
        if (locality.endpoints.size() > i &&
    ...
    if (!args.drop_categories.empty()) {
      ...
      for (const auto& p : args.drop_categories) {
        ...
        auto* drop_percentage =
            drop_overload->mutable_drop_percentage();
  ...
  void SetRouteConfiguration(int idx,
                             const RouteConfiguration& route_config) {
    if (GetParam().enable_rds_testing()) {
      balancers_[idx]->ads_service()->SetRdsResource(route_config);
    } else {
  ...
  class ServerThread {
   public:
    ...
    void Start() {
      ...
      // notify_one by ServerThread::Serve from firing before the wait
      // below is hit.
    ...
    void Serve(grpc_core::Mutex* mu, grpc_core::CondVar* cond) {
      ...
      if (use_xds_enabled_server_) {
        ...
        if (GetParam().bootstrap_source() ==
            TestType::kBootstrapFromChannelArg) {
          builder.SetOption(
              absl::make_unique<XdsChannelArgsServerBuilderOption>(
                  test_obj_));
        }
    ...
    bool use_xds_enabled_server() const {
      return use_xds_enabled_server_;
    }
   ...
   private:
    class XdsChannelArgsServerBuilderOption
     ...
     public:
      explicit XdsChannelArgsServerBuilderOption(
      ...
      void UpdatePlugins(
          std::vector<
              std::unique_ptr<grpc::ServerBuilderPlugin>>* /*plugins*/)
          override {}
   ...
   public:
    explicit BalancerServerThread(
        ...
        : ServerThread(test_obj),
          ...
          lrs_service_(
              new LrsServiceImpl(client_load_reporting_interval)) {}
...
// Tests that the balancer sends the correct response to the client, and
// the client sends RPCs to the backends using the default child policy.
TEST_P(BasicTest, Vanilla) {
  ...
  EXPECT_EQ((GetParam().use_fake_resolver()
                 ? "xds_cluster_resolver_experimental"
...
// Tests that RPCs will be blocked until a non-empty serverlist is
// received.
TEST_P(BasicTest, InitiallyEmptyServerlist) {
  ...
  CheckRpcSendOk(1, RpcOptions()
                        .set_timeout_ms(kCallDeadlineMs)
                        .set_wait_for_ready(true));
  const auto ellapsed_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          system_clock::now() - t0);
  // but eventually, the LB sends a serverlist update that allows the
  // call to proceed. The call delay must be larger than the delay in
  // sending the populated serverlist but under the call's deadline
  // (which is enforced by the call's deadline).
  EXPECT_GT(ellapsed_ms.count(), kServerlistDelayMs);
  delayed_resource_setter.join();
}

// Tests that RPCs will fail with UNAVAILABLE instead of
// DEADLINE_EXCEEDED if all the servers are unreachable.
TEST_P(BasicTest, AllServersUnreachableFailFast) {
  // Set Rpc timeout to 5 seconds to ensure there is enough time
  // for communication with the xDS server to take place upon test start
  // up.
  ...
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  const Status status =
      SendRpc(RpcOptions().set_timeout_ms(kRpcTimeoutMs));
  // The error shouldn't be DEADLINE_EXCEEDED because timeout is set to
  // 5 seconds, and we should disocver in that time that the target
  // backend is down.
  EXPECT_EQ(StatusCode::UNAVAILABLE, status.error_code());
}

// Tests that RPCs fail when the backends are down, and will succeed
// again after the backends are restarted.
TEST_P(BasicTest, BackendsRestart) {
  ...
  ShutdownAllBackends();
  // Sending multiple failed requests instead of just one to ensure that
  // the client notices that all backends are down before we restart
  // them. If we didn't do this, then a single RPC could fail here due
  // to the race condition between the LB pick and the GOAWAY from the
  // chosen backend being shut down, which would not actually prove that
  // the client noticed that all of the backends are down. Then, when we
  // send another request below (which we expect to succeed), if the
  // callbacks happen in the wrong order, the same race condition could
  // happen again due to the client not yet having noticed that the
  // backends were all down.
  CheckRpcSendFailure(num_backends_);
  // Restart all backends.  RPCs should start succeeding again.
  StartAllBackends();
  CheckRpcSendOk(
      1, RpcOptions().set_timeout_ms(2000).set_wait_for_ready(true));
}
...
TEST_P(XdsResolverOnlyTest,
       ResourceTypeVersionPersistsAcrossStreamRestarts) {
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  AdsServiceImpl::EdsResourceArgs args({
...
TEST_P(XdsResolverOnlyTest, ChangeClusters) {
  ...
  SetListenerAndRouteConfiguration(0, default_listener_,
                                   new_route_config);
...
// Tests that we restart all xDS requests when we reestablish the ADS
// call.
...
TEST_P(XdsResolverOnlyTest,
       CircuitBreakingMultipleChannelsShareCallCounter) {
  constexpr size_t kMaxConcurrentRequests = 10;
  // Populate new EDS resources.
  AdsServiceImpl::EdsResourceArgs args({
      {"locality0", CreateEndpointsForBackends(0, 1)},
  ...
  auto response_generator2 = grpc_core::MakeRefCounted<
      grpc_core::FakeResolverResponseGenerator>();
  auto lb_response_generator2 = grpc_core::MakeRefCounted<
      grpc_core::FakeResolverResponseGenerator>();
  grpc_arg xds_arg =
      grpc_core::FakeResolverResponseGenerator::MakeChannelArg(
  ...
  SetNextResolutionForLbChannelAllBalancers();
  SetNextResolutionForLbChannelAllBalancers(
      nullptr, nullptr, lb_response_generator2.get());
...
TEST_P(XdsResolverOnlyTest, ClusterChangeAfterAdsCallFails) {
  ...
  //      xds_cluster_resolver policy that the resource no longer
  //      exists.
  //
...
TEST_P(GlobalXdsClientTest, MultipleBadResources) {
  ...
  SetNextResolutionForLbChannelAllBalancers();
  CheckRpcSendFailure();
  // Need to create a second channel to subscribe to a second LDS
  // resource.
  ...
  EXPECT_EQ(response_state.state,
...
TEST_P(SecureNamingTest, TargetNameIsUnexpected) {
  ...
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  // Make sure that we blow up (via abort() from the security connector)
  // when the name from the balancer doesn't match expectations.
  ASSERT_DEATH_IF_SUPPORTED({ CheckRpcSendOk(); }, "");
}

using LdsTest = BasicTest;

// Tests that LDS client should send a NACK if there is no API listener
// in the Listener in the LDS response.
...
// Tests that LDS client should send a NACK if the route_specifier in
// the http_connection_manager is neither inlined route_config nor RDS.
TEST_P(LdsTest, WrongRouteSpecifier) {
  ...
  EXPECT_EQ(response_state.state,
            AdsServiceImpl::ResponseState::NACKED);
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr("HttpConnectionManager neither has "
                                   "inlined route_config nor RDS."));
}
...
// Tests that LDS client should send a NACK if the rds message in the
// http_connection_manager has a config_source field that does not
// specify ADS.
...
TEST_P(LdsTest, RejectsUnparseableHttpFilterType) {
  ...
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr("filter config for type "
                                   ...
                                   "router.v3.Router failed to parse"));
}
...
TEST_P(LdsTest, RejectsHttpFiltersNotSupportedOnClients) {
  ...
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr(
                  "Filter grpc.testing.server_only_http_filter is not "
...
// Tests that LDS client should send an ACK upon correct LDS response
// (with inlined RDS result).
TEST_P(LdsRdsTest, Vanilla) {
  ...
  EXPECT_EQ(RouteConfigurationResponseState(0).state,
            AdsServiceImpl::ResponseState::ACKED);
  // Make sure we actually used the RPC service for the right version of
  // xDS.
...
// Tests that LDS client ACKs but fails if matching domain can't be
// found in the LDS response.
...
// Tests that LDS client should choose the virtual host with matching
// domain if multiple virtual hosts exist in the LDS response.
...
// Tests that LDS client should choose the last route in the virtual
// host if multiple routes exist in the LDS response.
...
// Tests that LDS client should send a NACK if route has an action other
// than RouteAction in the LDS response.
...
TEST_P(LdsRdsTest,
       RouteActionWeightedTargetHasIncorrectTotalWeightSet) {
  ...
  route1->mutable_match()->set_prefix(
      "/grpc.testing.EchoTest1Service/");
  auto* weighted_cluster1 = route1->mutable_route()
...
TEST_P(LdsRdsTest, RouteActionWeightedClusterHasZeroTotalWeight) {
  ...
  EXPECT_EQ(response_state.state,
            AdsServiceImpl::ResponseState::NACKED);
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr("RouteAction weighted_cluster has "
                                   "no valid clusters specified."));
}

TEST_P(LdsRdsTest,
       RouteActionWeightedTargetClusterHasEmptyClusterName) {
  const size_t kWeight75 = 75;
  RouteConfiguration route_config = default_route_config_;
  auto* route1 =
      route_config.mutable_virtual_hosts(0)->mutable_routes(0);
...
TEST_P(LdsRdsTest, RouteHeaderMatchInvalidRegex) {
  ...
  EXPECT_EQ(response_state.state,
            AdsServiceImpl::ResponseState::NACKED);
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr("header matcher: Invalid regex "
                                   "string specified in matcher."));
}
...
TEST_P(LdsRdsTest, RouteHeaderMatchInvalidRange) {
  ...
  EXPECT_EQ(response_state.state,
            AdsServiceImpl::ResponseState::NACKED);
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr("header matcher: Invalid range "
                                   "specifier specified: end cannot be "
                                   "smaller than start."));
}

// Tests that LDS client should choose the default route (with no
// matching specified) after unable to find a match with previous
// routes.
TEST_P(LdsRdsTest, XdsRoutingPathMatching) {
  ...
  route2->mutable_match()->set_path(
      "/grpc.testing.EchoTest2Service/Echo2");
  route2->mutable_route()->set_cluster(kNewCluster2Name);
  auto* route3 =
      new_route_config.mutable_virtual_hosts(0)->add_routes();
  route3->mutable_match()->set_path(
      "/grpc.testing.EchoTest3Service/Echo3");
  ...
  EXPECT_EQ(kNumEcho1Rpcs,
            backends_[2]->backend_service1()->request_count());
  EXPECT_EQ(0, backends_[2]->backend_service2()->request_count());
  EXPECT_EQ(0, backends_[3]->backend_service()->request_count());
  EXPECT_EQ(0, backends_[3]->backend_service1()->request_count());
  EXPECT_EQ(kNumEcho2Rpcs,
            backends_[3]->backend_service2()->request_count());
}
...
TEST_P(LdsRdsTest, XdsRoutingWeightedCluster) {
  ...
  const size_t kNumEchoRpcs =
      10;  // RPCs that will go to a fixed backend.
  const size_t kWeight75 = 75;
  ...
  weighted_cluster1->set_name(kNewCluster1Name);
  weighted_cluster1->mutable_weight()->set_value(kWeight75);
  auto* weighted_cluster2 = route1->mutable_route()
                                ->mutable_weighted_clusters()
                                ->add_clusters();
  weighted_cluster2->set_name(kNewCluster2Name);
  weighted_cluster2->mutable_weight()->set_value(kWeight25);
  // Cluster with weight 0 will not be used.
  auto* weighted_cluster3 = route1->mutable_route()
                                ->mutable_weighted_clusters()
                                ->add_clusters();
  ...
  const int weight_25_request_count =
      backends_[2]->backend_service1()->request_count();
  gpr_log(GPR_INFO,
          "target_75 received %d rpcs and target_25 received %d rpcs",
          weight_75_request_count, weight_25_request_count);
  EXPECT_THAT(
      static_cast<double>(weight_75_request_count) / kNumEcho1Rpcs,
      ::testing::DoubleNear(kWeight75Percent, kErrorTolerance));
  EXPECT_THAT(
      static_cast<double>(weight_25_request_count) / kNumEcho1Rpcs,
...
TEST_P(LdsRdsTest, RouteActionWeightedTargetDefaultRoute) {
  ...
  gpr_log(GPR_INFO,
          ...
          weight_75_request_count, weight_25_request_count);
  EXPECT_THAT(
      static_cast<double>(weight_75_request_count) / kNumEchoRpcs,
      ::testing::DoubleNear(kWeight75Percent, kErrorTolerance));
  EXPECT_THAT(
      static_cast<double>(weight_25_request_count) / kNumEchoRpcs,
      ::testing::DoubleNear(kWeight25Percent, kErrorTolerance));
}
...
TEST_P(LdsRdsTest, XdsRoutingWeightedClusterUpdateWeights) {
  ...
  EXPECT_EQ(0, backends_[3]->backend_service1()->request_count());
  EXPECT_THAT(static_cast<double>(weight_50_request_count_1) /
                  kNumEcho1Rpcs5050,
              ::testing::DoubleNear(kWeight50Percent, kErrorTolerance));
  EXPECT_THAT(static_cast<double>(weight_50_request_count_2) /
                  kNumEcho1Rpcs5050,
              ::testing::DoubleNear(kWeight50Percent, kErrorTolerance));
}
...
TEST_P(LdsRdsTest, XdsRoutingWeightedClusterUpdateClusters) {
  ...
  EXPECT_THAT(
      static_cast<double>(weight_25_request_count) / kNumEcho1Rpcs7525,
      ::testing::DoubleNear(kWeight25Percent, kErrorTolerance));
  // Change Route Configurations: new set of clusters with different
  // weights.
  weighted_cluster1->mutable_weight()->set_value(kWeight50);
  weighted_cluster2->set_name(kNewCluster2Name);
  weighted_cluster2->mutable_weight()->set_value(kWeight50);
...
TEST_P(LdsRdsTest, XdsRoutingClusterUpdateClustersWithPickingDelays) {
  ...
  new_cluster.mutable_eds_cluster_config()->set_service_name(
      kNewEdsServiceName);
  balancers_[0]->ads_service()->SetCdsResource(new_cluster);
  // Bring down the current backend: 0, this will delay route picking
  // time, resulting in un-committed RPCs.
  ShutdownBackend(0);
  ...
  default_route->mutable_route()->set_cluster(kNewClusterName);
  SetRouteConfiguration(0, new_route_config);
  // Wait for RPCs to go to the new backend: 1, this ensures that the
  // client has processed the update.
  WaitForBackend(1, WaitForBackendOptions()
                        .set_reset_counters(false)
                        .set_allow_failures(true));
  // Bring up the previous backend: 0, this will allow the delayed RPC
  // to finally call on_call_committed upon completion.
  StartBackend(0);
  sending_rpc.join();
  // Make sure RPCs go to the correct backend:
...
TEST_P(LdsRdsTest, XdsRoutingApplyXdsTimeout) {
  ...
  route2->mutable_route()->set_cluster(kNewCluster2Name);
  max_stream_duration =
      route2->mutable_route()->mutable_max_stream_duration();
  duration = max_stream_duration->mutable_max_stream_duration();
  duration->set_seconds(kTimeoutMaxStreamDurationSecond);
  duration->set_nanos(kTimeoutNano);
...
TEST_P(LdsRdsTest,
       XdsRoutingApplyApplicationTimeoutWhenXdsTimeoutExplicit0) {
...
TEST_P(LdsRdsTest,
       XdsRoutingApplyApplicationTimeoutWhenHttpTimeoutExplicit0) {
  const int64_t kTimeoutApplicationSecond = 4;
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  // Populate new EDS resources.
...
TEST_P(LdsRdsTest, XdsRoutingHeadersMatchingSpecialCasesToIgnore) {
  ...
  // Verify that only the default backend got RPCs since all previous
  // routes were mismatched.
  EXPECT_EQ(kNumEchoRpcs,
            backends_[0]->backend_service()->request_count());
  EXPECT_EQ(0, backends_[1]->backend_service()->request_count());
  const auto response_state = RouteConfigurationResponseState(0);
...
TEST_P(LdsRdsTest, XdsRoutingRuntimeFractionMatching) {
  ...
  const int matched_backend_count =
      backends_[1]->backend_service()->request_count();
  EXPECT_THAT(
      static_cast<double>(default_backend_count) / kNumRpcs,
      ::testing::DoubleNear(1 - kRouteMatchPercent, kErrorTolerance));
  EXPECT_THAT(
      static_cast<double>(matched_backend_count) / kNumRpcs,
...
// Test that we NACK filters without configs in FilterConfig in
// VirtualHost.
TEST_P(LdsRdsTest,
       RejectsHttpFilterWithoutConfigInFilterConfigInVirtualHost) {
  if (GetParam().use_v2()) return;  // Filters supported in v3 only.
  RouteConfiguration route_config = default_route_config_;
  auto* per_filter_config = route_config.mutable_virtual_hosts(0)
                                ->mutable_typed_per_filter_config();
...
TEST_P(LdsRdsTest,
       IgnoresOptionalHttpFilterWithoutConfigInVirtualHost) {
  if (GetParam().use_v2()) return;  // Filters supported in v3 only.
  RouteConfiguration route_config = default_route_config_;
  auto* per_filter_config = route_config.mutable_virtual_hosts(0)
                                ->mutable_typed_per_filter_config();
...
TEST_P(LdsRdsTest,
       RejectsHttpFilterWithoutConfigInFilterConfigInRoute) {
  if (GetParam().use_v2()) return;  // Filters supported in v3 only.
  RouteConfiguration route_config = default_route_config_;
  auto* per_filter_config = route_config.mutable_virtual_hosts(0)
                                ->mutable_routes(0)
...
TEST_P(LdsRdsTest, RejectsUnknownHttpFilterTypeInClusterWeight) {
  ...
  auto* per_filter_config =
      cluster_weight->mutable_typed_per_filter_config();
  (*per_filter_config)["unknown"].PackFrom(Listener());
  SetListenerAndRouteConfiguration(0, default_listener_, route_config);
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
...
TEST_P(LdsRdsTest,
       IgnoresOptionalUnknownHttpFilterTypeInClusterWeight) {
  if (GetParam().use_v2()) return;  // Filters supported in v3 only.
  RouteConfiguration route_config = default_route_config_;
  auto* cluster_weight = route_config.mutable_virtual_hosts(0)
                             ->mutable_routes(0)
...
// Test that we ignore optional filters without configs in
// ClusterWeight.
TEST_P(LdsRdsTest,
       IgnoresOptionalHttpFilterWithoutConfigInClusterWeight) {
  if (GetParam().use_v2()) return;  // Filters supported in v3 only.
  RouteConfiguration route_config = default_route_config_;
  auto* cluster_weight = route_config.mutable_virtual_hosts(0)
                             ->mutable_routes(0)
...
TEST_P(CdsTest, LogicalDNSClusterType) {
  ...
  // Set Logical DNS result
  {
    ...
    grpc_core::Resolver::Result result;
    result.addresses =
        CreateAddressListFromPortList(GetBackendPorts(1, 2));
...
TEST_P(CdsTest, AggregateClusterEdsToLogicalDns) {
  ...
  // Set Logical DNS result
  {
    ...
    grpc_core::Resolver::Result result;
    result.addresses =
        CreateAddressListFromPortList(GetBackendPorts(2, 3));
    logical_dns_cluster_resolver_response_generator_->SetResponse(
        std::move(result));
  }
  // Wait for traffic to go to backend 1.
...
// Test that CDS client should send a NACK if cluster type is Logical
// DNS but the feature is not yet supported.
...
// Test that CDS client should send a NACK if cluster type is AGGREGATE
// but the feature is not yet supported.
...
// Tests that CDS client should send a NACK if the eds_config in CDS
// response is other than ADS.
...
// Tests that CDS client should send a NACK if the lb_policy in CDS
// response is other than ROUND_ROBIN.
...
// Tests that CDS client should send a NACK if the lrs_server in CDS
// response is other than SELF.
...
// Tests that ring hash policy that hashes using channel id ensures all
// RPCs to go 1 particular backend.
...
// Tests that ring hash policy that hashes using a header value can
// spread RPCs across all the backends.
TEST_P(CdsTest, RingHashHeaderHashing) {
  ...
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  SetNextResolutionForLbChannelAllBalancers();
  // Note each type of RPC will contains a header value that will always
  // be hashed to a specific backend as the header value matches the
  // value used to create the entry in the ring.
  std::vector<std::pair<std::string, std::string>> metadata = {
  ...
  const auto rpc_options1 =
      RpcOptions().set_metadata(std::move(metadata1));
  const auto rpc_options2 =
      RpcOptions().set_metadata(std::move(metadata2));
  const auto rpc_options3 =
      RpcOptions().set_metadata(std::move(metadata3));
...
// Tests that ring hash policy that hashes using a header value and
// regex rewrite to aggregate RPCs to 1 backend.
...
TEST_P(CdsTest, RingHashNoHashPolicy) {
  ...
  CheckRpcSendOk(kNumRpcs);
  const int request_count_1 =
      backends_[0]->backend_service()->request_count();
  const int request_count_2 =
      backends_[1]->backend_service()->request_count();
  EXPECT_THAT(
      static_cast<double>(request_count_1) / kNumRpcs,
      ::testing::DoubleNear(kDistribution50Percent, kErrorTolerance));
  EXPECT_THAT(
      static_cast<double>(request_count_2) / kNumRpcs,
      ::testing::DoubleNear(kDistribution50Percent, kErrorTolerance));
  gpr_unsetenv("GRPC_XDS_EXPERIMENTAL_ENABLE_RING_HASH");
}
...
TEST_P(CdsTest,
       RingHashContinuesPastTerminalPolicyThatDoesNotProduceResult) {
  gpr_setenv("GRPC_XDS_EXPERIMENTAL_ENABLE_RING_HASH", "true");
  auto cluster = default_cluster_;
  cluster.set_lb_policy(Cluster::RING_HASH);
  balancers_[0]->ads_service()->SetCdsResource(cluster);
...
// Test random hash is used when header hashing specified a header field
...
TEST_P(CdsTest, RingHashUnsupportedHashPolicyDefaultToRandomHashing) {
  ...
  auto* route =
      new_route_config.mutable_virtual_hosts(0)->mutable_routes(0);
  auto* hash_policy_unsupported_1 =
      route->mutable_route()->add_hash_policy();
  hash_policy_unsupported_1->mutable_cookie()->set_name("cookie");
  auto* hash_policy_unsupported_2 =
      route->mutable_route()->add_hash_policy();
  hash_policy_unsupported_2->mutable_connection_properties()
      ->set_source_ip(true);
  auto* hash_policy_unsupported_3 =
      route->mutable_route()->add_hash_policy();
...
// Tests that ring hash policy that hashes using a random value can
// spread RPCs across all the backends according to locality weight.
TEST_P(CdsTest,
       RingHashRandomHashingDistributionAccordingToEndpointWeight) {
  gpr_setenv("GRPC_XDS_EXPERIMENTAL_ENABLE_RING_HASH", "true");
  const size_t kWeight1 = 1;
  const size_t kWeight2 = 2;
  const size_t kWeightTotal = kWeight1 + kWeight2;
  const double kWeight33Percent =
      static_cast<double>(kWeight1) / kWeightTotal;
  const double kWeight66Percent =
...
TEST_P(
  ...
  const size_t kWeightTotal = kWeight1 + kWeight2;
  const double kWeight20Percent =
      static_cast<double>(kWeight1) / kWeightTotal;
  const double kWeight80Percent =
      static_cast<double>(kWeight2) / kWeightTotal;
  const double kErrorTolerance = 0.05;
  const uint32_t kRpcTimeoutMs = 10000;
  const size_t kNumRpcs =
...
// Tests round robin is not implacted by the endpoint weight, and that
// the localities in a locality map are picked according to their
// weights.
TEST_P(CdsTest, RingHashEndpointWeightDoesNotImpactWeightedRoundRobin) {
  ...
  // ADS response contains 2 localities, each of which contains 1
  // backend.
  AdsServiceImpl::EdsResourceArgs args({
      {"locality0",
  ...
  // Send kNumRpcs RPCs.
  CheckRpcSendOk(kNumRpcs);
  // The locality picking rates should be roughly equal to the
  // expectation.
  ...
  const double locality_picked_rate_1 =
      ...
      kNumRpcs;
  EXPECT_THAT(
      locality_picked_rate_0,
      ::testing::DoubleNear(kLocalityWeightRate0, kErrorTolerance));
  EXPECT_THAT(
      locality_picked_rate_1,
      ::testing::DoubleNear(kLocalityWeightRate1, kErrorTolerance));
}

// Tests that ring hash policy that hashes using a fixed string ensures
// all RPCs to go 1 particular backend; and that subsequent hashing
// policies are ignored due to the setting of terminal.
TEST_P(CdsTest, RingHashFixedHashingTerminalPolicy) {
  ...
  hash_policy->set_terminal(true);
  auto* hash_policy_to_be_ignored =
      route->mutable_route()->add_hash_policy();
  hash_policy_to_be_ignored->mutable_header()->set_header_name(
      "random_string");
  SetListenerAndRouteConfiguration(0, default_listener_,
                                   new_route_config);
  AdsServiceImpl::EdsResourceArgs args({
...
// (tho it is not possible to catch the connecting state before moving
// to ready)
...
// Test that when the first pick is down leading to a transient failure,
// we will move on to the next ring hash entry.
...
// Test that when a backend goes down, we will move on to the next
// subchannel (with a lower priority).  When the backend comes back up,
// traffic will move back.
...
TEST_P(CdsTest, RingHashAllFailReattempt) {
  ...
  EXPECT_TRUE(
      channel_->WaitForConnected(grpc_timeout_milliseconds_to_deadline(
          kConnectionTimeoutMilliseconds)));
  gpr_unsetenv("GRPC_XDS_EXPERIMENTAL_ENABLE_RING_HASH");
}

// Test that when all backends are down and then up, we may pick a TF
// backend and we will then jump to ready backend.
TEST_P(CdsTest, RingHashTransientFailureSkipToAvailableReady) {
  ...
  // backend 0. So by purposely bring down backend 0 and bring up
  // another backend, this will ensure Picker's first choice of backend
  // 0 will fail and it will
  ...
  // unused ports to fill the ring, it is almost guaranteed that the
  // Picker will go through some non-READY entries and skip them as per
  // design.
  ShutdownBackend(0);
  CheckRpcSendFailure(1, rpc_options);
  StartBackend(1);
...
// Test we nack when ring hash policy has invalid hash function
// (something other than XX_HASH.
...
class XdsSecurityTest : public BasicTest {
 protected:
  ...
  void SetUp() override {
    ...
    // server certs after https://github.com/grpc/grpc/pull/24876 is
    // merged
  ...
  // Sends CDS updates with the new security configuration and verifies
  // that after propagation, this new configuration is used for
  // connections. If \a identity_instance_name and \a root_instance_name
  // are both empty, connections are expected to use fallback
  // credentials.
  void UpdateAndVerifyXdsSecurityConfiguration(
    ...
    if (!identity_instance_name.empty() ||
      ...
      if (!san_matchers.empty()) {
        ...
        for (const auto& san_matcher : san_matchers) {
          *validation_context->add_match_subject_alt_names() =
              san_matcher;
        }
      }
    ...
    balancers_[0]->ads_service()->SetCdsResource(cluster);
    // The updates might take time to have an effect, so use a retry
    // loop.
    ...
    for (; num_tries < kRetryCount; num_tries++) {
      ...
      if (test_expects_failure) {
        // Restart the servers to force a reconnection so that
        // previously connected subchannels are not used for the RPC.
        ShutdownBackend(0);
        StartBackend(0);
        if (SendRpc().ok()) {
          gpr_log(GPR_ERROR,
        ...
        WaitForBackend(
            0, WaitForBackendOptions().set_allow_failures(true));
        Status status = SendRpc();
        if (!status.ok()) {
          gpr_log(GPR_ERROR,
                  "RPC failed. code=%d message=%s Trying again.",
        ...
        if (backends_[0]->backend_service()->last_peer_identity() !=
          ...
          gpr_log(
              ...
              absl::StrJoin(
                  ...
                  ",")
                  .c_str(),
              absl::StrJoin(expected_authenticated_identity, ",")
                  .c_str());
          continue;
        }
      }
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithExactSanMatcher) {
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {server_san_exact_},
      authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithPrefixSanMatcher) {
  ...
  g_fake1_cert_data_map = &fake1_cert_map;
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {server_san_prefix_},
      authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithSuffixSanMatcher) {
  ...
  g_fake1_cert_data_map = &fake1_cert_map;
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {server_san_suffix_},
      authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithContainsSanMatcher) {
  ...
  g_fake1_cert_data_map = &fake1_cert_map;
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {server_san_contains_},
      authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithRegexSanMatcher) {
  ...
  g_fake1_cert_data_map = &fake1_cert_map;
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {server_san_regex_},
      authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithSanMatchersUpdate) {
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "", {bad_san_1_, bad_san_2_},
      {}, true /* failure */);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "",
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithIdentityPluginUpdate) {
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      ...
      authenticated_identity_);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin2", "", {server_san_exact_},
      fallback_authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
  g_fake2_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest, TestMtlsConfigurationWithBothPluginsUpdated) {
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      ...
      authenticated_identity_);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin2", "good", "fake_plugin2", "good",
...
TEST_P(XdsSecurityTest,
       TestMtlsConfigurationWithRootCertificateNameUpdate) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}},
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      ...
      authenticated_identity_);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "bad", "fake_plugin1", "", {server_san_regex_},
      {}, true /* failure */);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(XdsSecurityTest,
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      ...
      authenticated_identity_);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "bad", {server_san_exact_},
      {}, true /* failure */);
  g_fake1_cert_data_map = nullptr;
}
...
TEST_P(
  ...
  UpdateAndVerifyXdsSecurityConfiguration(
      ...
      authenticated_identity_);
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "", "fake_plugin1", "good", {server_san_exact_},
      fallback_authenticated_identity_);
  g_fake1_cert_data_map = nullptr;
}

TEST_P(XdsSecurityTest,
       TestMtlsConfigurationWithBothCertificateNamesUpdated) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}},
      {"bad", {bad_root_cert_, bad_identity_pair_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
  UpdateAndVerifyXdsSecurityConfiguration(
      "fake_plugin1", "bad", "fake_plugin1", "bad",
...
TEST_P(XdsSecurityTest,
       TestTlsConfigurationWithRootCertificateNameUpdate) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}},
      {"bad", {bad_root_cert_, bad_identity_pair_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
...
TEST_P(XdsSecurityTest, TestFileWatcherCertificateProvider) {
  UpdateAndVerifyXdsSecurityConfiguration(
      "file_plugin", "", "file_plugin", "", {server_san_exact_},
      authenticated_identity_);
}
...
TEST_P(XdsEnabledServerTest, HttpFilterNotSupportedOnServer) {
  ...
  EXPECT_THAT(response_state.error_message,
              ::testing::HasSubstr(
                  "Filter grpc.testing.client_only_http_filter is not "
    ...
    if (!identity_instance_name.empty()) {
      ...
      if (!root_instance_name.empty()) {
        ...
        downstream_tls_context.mutable_common_tls_context()
            ...
            ->mutable_validation_context_certificate_provider_instance()
            ->set_certificate_name(std::string(root_certificate_name));
        downstream_tls_context.mutable_require_client_certificate()
            ->set_value(require_client_certificates);
      }
      transport_socket->mutable_typed_config()->PackFrom(
...
TEST_P(XdsServerSecurityTest,
       TestMtlsWithIdentityCertificateNameUpdate) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}},
      {"good", {root_cert_, identity_pair_2_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
...
TEST_P(XdsServerSecurityTest,
       TestMtlsNotRequiringButProvidingClientCerts) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
...
TEST_P(XdsServerSecurityTest,
       TestMtlsNotRequiringAndNotProvidingClientCerts) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
  SetLdsUpdate("fake_plugin1", "", "fake_plugin1", "", false);
...
TEST_P(XdsServerSecurityTest,
       TestTlsWithIdentityCertificateNameUpdate) {
  FakeCertificateProvider::CertDataMap fake1_cert_map = {
      {"", {root_cert_, identity_pair_}},
      {"good", {root_cert_, identity_pair_2_}}};
  g_fake1_cert_data_map = &fake1_cert_map;
...
class XdsEnabledServerStatusNotificationTest
...
// This test verifies that the resource getting deleted when already
// serving results in future connections being dropped.
TEST_P(XdsEnabledServerStatusNotificationTest,
...
TEST_P(XdsEnabledServerStatusNotificationTest,
  ...
  for (int i = 0; i < 5; i++) {
    ...
    backends_[0]->notifier()->WaitOnServingStatusChange(
        ...
        grpc::StatusCode::OK);
    SendRpc([this]() { return CreateInsecureChannel(); }, {}, {});
    // Deleting the resource will make the server start rejecting
    // connections
    UnsetLdsUpdate();
    backends_[0]->notifier()->WaitOnServingStatusChange(
        absl::StrCat(ipv6_only_ ? "[::1]:" : "127.0.0.1:",
...
TEST_P(XdsEnabledServerStatusNotificationTest,
       ExistingRpcsOnResourceDeletion) {
  // Send a valid LDS update to get the server to start listening
  SetValidLdsUpdate();
  backends_[0]->notifier()->WaitOnServingStatusChange(
      absl::StrCat(ipv6_only_ ? "[::1]:" : "127.0.0.1:",
  ...
  struct StreamingRpc {
    ...
    ClientContext context;
    std::unique_ptr<ClientReaderWriter<EchoRequest, EchoResponse>>
        stream;
  ...
  for (int i = 0; i < kNumChannels; i++) {
    ...
    streaming_rpcs[i].stub = grpc::testing::EchoTestService::NewStub(
  ...
  for (int i = 0; i < kNumChannels; i++) {
    ...
    new_context.set_deadline(
        grpc_timeout_milliseconds_to_deadline(1000));
    EXPECT_FALSE(streaming_rpcs[i]
                     .stub->Echo(&new_context, request, &response)
                     .ok());
  }
}
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  socket_address->set_address(ipv6_only_ ? "::1" : "127.0.0.1");
  socket_address->set_port_value(backends_[0]->port());
  // Add filter chain with destination port that should never get
  // matched
  auto* filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  filter_chain->mutable_filter_chain_match()
      ...
      ->set_value(8080);
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // RPC should fail since no matching filter chain was found and no
  // default filter chain is configured.
  SendRpc([this]() { return CreateInsecureChannel(); }, {}, {},
          true /* test_expects_failure */);
}

TEST_P(XdsServerFilterChainMatchTest,
       FilterChainsWithServerNamesDontMatch) {
  Listener listener;
  listener.set_name(absl::StrCat(
      "grpc/server?xds.resource.listening_address=",
      ipv6_only_ ? "[::1]:" : "127.0.0.1:", backends_[0]->port()));
  ...
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  filter_chain->mutable_filter_chain_match()->add_server_names(
      "server_name");
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // RPC should fail since no matching filter chain was found and no
  // default filter chain is configured.
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  socket_address->set_address(ipv6_only_ ? "::1" : "127.0.0.1");
  socket_address->set_port_value(backends_[0]->port());
  // Add filter chain with transport protocol "tls" that should never
  // match
  auto* filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  socket_address->set_address(ipv6_only_ ? "::1" : "127.0.0.1");
  socket_address->set_port_value(backends_[0]->port());
  // Add filter chain with application protocol that should never get
  // matched
  auto* filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  filter_chain->mutable_filter_chain_match()->set_transport_protocol(
      "raw_buffer");
  // Add another filter chain with no transport protocol set but
  // application protocol set (fails match)
  ...
  filter_chain->mutable_filter_chain_match()->add_application_protocols(
      "h2");
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // A successful RPC proves that filter chains that mention
  // "raw_buffer" as the transport protocol are chosen as the best match
  // in the round.
  SendRpc([this]() { return CreateInsecureChannel(); }, {}, {});
}

TEST_P(
    XdsServerFilterChainMatchTest,
  ...
  socket_address->set_address(ipv6_only_ ? "::1" : "127.0.0.1");
  socket_address->set_port_value(backends_[0]->port());
  // Add filter chain with prefix range (length 4 and 16) but with
  // server name mentioned. (Prefix range is matched first.)
  auto* filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  auto* prefix_range =
  ...
  filter_chain->mutable_filter_chain_match()->add_server_names(
      "server_name");
  // Add filter chain with two prefix ranges (length 8 and 24). Since 24
  ...
  // Add another filter chain with a non-matching prefix range (with
  // length 30)
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  ...
  filter_chain->mutable_filter_chain_match()->add_server_names(
      "server_name");
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // A successful RPC proves that the filter chain with the longest
  // matching prefix range was the best match.
  SendRpc([this]() { return CreateInsecureChannel(); }, {}, {});
}
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  // Note that backends_[0]->port() will never be a match for the source
  // port because it is already being used by a backend.
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  filter_chain->mutable_filter_chain_match()->set_source_type(
  ...
  filter_chain->mutable_filter_chain_match()->add_source_ports(
      backends_[0]->port());
  // Add filter chain with the default source type (ANY) but bad source
  // port.
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  socket_address->set_address(ipv6_only_ ? "::1" : "127.0.0.1");
  socket_address->set_port_value(backends_[0]->port());
  // Add filter chain with source prefix range (length 16) but with a
  // bad source port mentioned. (Prefix range is matched first.) Note
  // that backends_[0]->port() will never be a match for the source port
  // because it is already being used by a backend.
  auto* filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  auto* source_prefix_range = filter_chain->mutable_filter_chain_match()
                                  ->add_source_prefix_ranges();
  source_prefix_range->set_address_prefix(ipv6_only_ ? "::1"
                                                     : "127.0.0.1");
  source_prefix_range->mutable_prefix_len()->set_value(4);
  source_prefix_range = filter_chain->mutable_filter_chain_match()
                            ->add_source_prefix_ranges();
  source_prefix_range->set_address_prefix(ipv6_only_ ? "::1"
                                                     : "127.0.0.1");
  ...
  filter_chain->mutable_filter_chain_match()->add_source_ports(
      backends_[0]->port());
  // Add filter chain with two source prefix ranges (length 8 and 24).
  // Since 24 is the highest match, it should be chosen.
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  source_prefix_range = filter_chain->mutable_filter_chain_match()
  ...
  // Add another filter chain with a non-matching source prefix range
  // (with length 30) and bad source port
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  source_prefix_range = filter_chain->mutable_filter_chain_match()
  ...
  filter_chain->mutable_filter_chain_match()->add_source_ports(
      backends_[0]->port());
  // Add another filter chain with no source prefix range mentioned and
  // bad source port
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  ...
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // A successful RPC proves that the filter chain with the longest
  // matching source prefix range was the best match.
  SendRpc([this]() { return CreateInsecureChannel(); }, {}, {});
}
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  // Since we don't know which port will be used by the channel, just
  // add all ports except for 0.
  ...
  transport_socket->mutable_typed_config()->PackFrom(
      downstream_tls_context);
  balancers_[0]->ads_service()->SetLdsResource(listener);
  // A successful RPC proves that the filter chain with matching source
  // port was chosen.
  SendRpc([this]() { return CreateInsecureChannel(); }, {}, {});
}
...
TEST_P(XdsServerFilterChainMatchTest, DuplicateMatchNacked) {
  ...
  EXPECT_THAT(
      ...
      ::testing::HasSubstr("Duplicate matching rules detected when "
                           "adding filter chain: {}"));
}

TEST_P(XdsServerFilterChainMatchTest,
       DuplicateMatchOnPrefixRangesNacked) {
  Listener listener;
  listener.set_name(absl::StrCat(
      "grpc/server?xds.resource.listening_address=",
      ipv6_only_ ? "[::1]:" : "127.0.0.1:", backends_[0]->port()));
  ...
  if (ipv6_only_) {
    EXPECT_THAT(
        balancers_[0]
            ...
            .error_message,
        ::testing::HasSubstr(
            "Duplicate matching rules detected when adding filter "
            "chain: "
            "{prefix_ranges={{address_prefix=[::]:0, prefix_len=16}, "
            "{address_prefix=[::]:0, prefix_len=32}}}"));
  } else {
    EXPECT_THAT(balancers_[0]
                    ->ads_service()
                    ->lds_response_state()
                    .error_message,
                ::testing::HasSubstr(
                    "Duplicate matching rules detected when adding "
                    "filter chain: "
                    "{prefix_ranges={{address_prefix=127.0.0.0:0, "
                    "prefix_len=16}, "
...
TEST_P(XdsServerFilterChainMatchTest,
       DuplicateMatchOnTransportProtocolNacked) {
  Listener listener;
  listener.set_name(absl::StrCat(
      "grpc/server?xds.resource.listening_address=",
      ipv6_only_ ? "[::1]:" : "127.0.0.1:", backends_[0]->port()));
...
TEST_P(XdsServerFilterChainMatchTest,
       DuplicateMatchOnLocalSourceTypeNacked) {
  Listener listener;
  listener.set_name(absl::StrCat(
      "grpc/server?xds.resource.listening_address=",
      ipv6_only_ ? "[::1]:" : "127.0.0.1:", backends_[0]->port()));
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  filter_chain->mutable_filter_chain_match()->set_source_type(
      FilterChainMatch::EXTERNAL);
  // Add a duplicate filter chain with the same external source type
  // entry
  filter_chain = listener.add_filter_chains();
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
...
TEST_P(XdsServerFilterChainMatchTest,
  ...
  filter_chain->add_filters()->mutable_typed_config()->PackFrom(
      HttpConnectionManager());
  auto* prefix_range = filter_chain->mutable_filter_chain_match()
                           ->add_source_prefix_ranges();
  prefix_range->set_address_prefix(ipv6_only_ ? "::1" : "127.0.0.1");
  prefix_range->mutable_prefix_len()->set_value(16);
  prefix_range = filter_chain->mutable_filter_chain_match()
                     ->add_source_prefix_ranges();
  prefix_range->set_address_prefix(ipv6_only_ ? "::1" : "127.0.0.1");
  prefix_range->mutable_prefix_len()->set_value(24);
  ...
  if (ipv6_only_) {
    EXPECT_THAT(balancers_[0]
                ...
                ::testing::HasSubstr(
                    "Duplicate matching rules detected when adding "
                    "filter chain: "
                    "{source_prefix_ranges={{address_prefix=[::]:0, "
                    "prefix_len=16}, "
                    "{address_prefix=[::]:0, prefix_len=32}}}"));
  } else {
    EXPECT_THAT(
...
TEST_P(XdsServerFilterChainMatchTest,
       DuplicateMatchOnSourcePortNacked) {
  Listener listener;
  listener.set_name(absl::StrCat(
      "grpc/server?xds.resource.listening_address=",
      ipv6_only_ ? "[::1]:" : "127.0.0.1:", backends_[0]->port()));
...
// types, to make sure that there are no cut-and-paste errors in the
// code that cause us to look at data for the wrong resource type.  So
// we add this test to make sure that the EDS resource name defaults to
// the cluster name if not specified in the CDS resource.
...
// Tests that the localities in a locality map are picked according to
// their weights.
...
// Tests that the locality map can work properly even when it contains a
// large number of localities.
TEST_P(LocalityMapTest, StressTest) {
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  const size_t kNumLocalities = 100;
  const uint32_t kRpcTimeoutMs = 5000;
  // The first ADS response contains kNumLocalities localities, each of
  // which contains backend 0.
  AdsServiceImpl::EdsResourceArgs args;
  ...
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  // The second ADS response contains 1 locality, which contains
  // backend 1.
  args = AdsServiceImpl::EdsResourceArgs({
      {"locality0", CreateEndpointsForBackends(1, 2)},
  });
  ...
  // Wait until backend 0 is ready, before which kNumLocalities
  // localities are received and handled by the xds policy.
  WaitForBackend(0, WaitForBackendOptions().set_reset_counters(false),
                 RpcOptions().set_timeout_ms(kRpcTimeoutMs));
  EXPECT_EQ(0U, backends_[1]->backend_service()->request_count());
  // Wait until backend 1 is ready, before which kNumLocalities
  // localities are removed by the xds policy.
  WaitForBackend(1);
  delayed_resource_setter.join();
}

// Tests that the localities in a locality map are picked correctly
// after update (addition, modification, deletion).
TEST_P(LocalityMapTest, UpdateMap) {
  ...
  const double kTotalLocalityWeight0 = std::accumulate(
      kLocalityWeights0.begin(), kLocalityWeights0.end(), 0);
  ...
  // Delete the first locality, keep the second locality, change the
  // third locality's weight from 4 to 2, and add a new locality with
  // weight 6.
  const std::vector<int> kLocalityWeights1 = {3, 2, 6};
  const double kTotalLocalityWeight1 = std::accumulate(
      kLocalityWeights1.begin(), kLocalityWeights1.end(), 0);
  ...
  // The picking rates of the first 3 backends should be roughly equal
  // to the expectation.
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    EXPECT_THAT(
        locality_picked_rates[i],
        ::testing::AllOf(::testing::Ge(locality_weight_rate_0[i] *
                                       (1 - kErrorTolerance)),
                         ::testing::Le(locality_weight_rate_0[i] *
                                       (1 + kErrorTolerance))));
  }
  args = AdsServiceImpl::EdsResourceArgs({
      {"locality1", CreateEndpointsForBackends(1, 2), 3},
  ...
  // Wait until the locality update has been processed, as signaled by
  // backend 3 receiving a request.
  WaitForAllBackends(3, 4);
  ...
  // The picking rates of the last 3 backends should be roughly equal to
  // the expectation.
  ...
  for (size_t i = 1; i < 4; ++i) {
    ...
    EXPECT_THAT(
        locality_picked_rates[i],
        ::testing::AllOf(::testing::Ge(locality_weight_rate_1[i] *
                                       (1 - kErrorTolerance)),
                         ::testing::Le(locality_weight_rate_1[i] *
                                       (1 + kErrorTolerance))));
  }
}
...
// Localities with the highest priority are used when multiple priority
// exist.
TEST_P(FailoverTest, ChooseHighestPriority) {
  ...
  AdsServiceImpl::EdsResourceArgs args({
      {"locality0", CreateEndpointsForBackends(0, 1),
       kDefaultLocalityWeight, 1},
      {"locality1", CreateEndpointsForBackends(1, 2),
       kDefaultLocalityWeight, 2},
      {"locality2", CreateEndpointsForBackends(2, 3),
       kDefaultLocalityWeight, 3},
      {"locality3", CreateEndpointsForBackends(3, 4),
       kDefaultLocalityWeight, 0},
  });
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
...
TEST_P(FailoverTest, DoesNotUseLocalityWithNoEndpoints) {
  ...
  AdsServiceImpl::EdsResourceArgs args({
      ...
      {"locality1", CreateEndpointsForBackends(),
       kDefaultLocalityWeight, 0},
  });
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
...
// If a locality with higher priority than the current one becomes
// ready, switch to it.
...
// The first update only contains unavailable priorities. The second
// update contains available priorities.
...
// Tests that after the localities' priorities are updated, we still
// choose the highest READY priority with the updated localities.
...
TEST_P(FailoverTest,
  ...
  // - Priority 1 is locality 1, containing backends 1 and 2, which are
  // up.
  ShutdownBackend(0);
  AdsServiceImpl::EdsResourceArgs args({
      {"locality0", CreateEndpointsForBackends(0, 1),
       kDefaultLocalityWeight, 0},
      {"locality1", CreateEndpointsForBackends(1, 3),
       kDefaultLocalityWeight, 1},
  });
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  ...
  args = AdsServiceImpl::EdsResourceArgs({
      {"locality0", CreateEndpointsForBackends(0, 1),
       kDefaultLocalityWeight, 0},
      {"locality1", CreateEndpointsForBackends(1, 4),
       kDefaultLocalityWeight, 0},
  });
  std::thread delayed_resource_setter(
      std::bind(&BasicTest::SetEdsResourceWithDelay, this, 0,
  ...
  // so we will create priority 1.  Backends 1 and 2 should have
  // traffic, but backend 3 should not.
  WaitForAllBackends(1, 3,
                     WaitForBackendOptions().set_reset_counters(false));
  EXPECT_EQ(0UL, backends_[3]->backend_service()->request_count());
  // When backend 3 gets traffic, we know the second update has been
  // seen.
  WaitForBackend(3);
...
TEST_P(DropTest, Vanilla) {
  ...
  const double kDropRateForThrottle =
      kDropPerMillionForThrottle / 1000000.0;
  ...
  for (size_t i = 0; i < kNumRpcs; ++i) {
    ...
    if (!status.ok() && status.error_message() ==
                            "Call dropped by load balancing policy") {
      ++num_drops;
    } else {
      EXPECT_TRUE(status.ok()) << "code=" << status.error_code()
  ...
  const double seen_drop_rate =
      static_cast<double>(num_drops) / kNumRpcs;
  EXPECT_THAT(seen_drop_rate,
              ::testing::DoubleNear(kDropRateForLbAndThrottle,
                                    kErrorTolerance));
}
...
TEST_P(DropTest, Update) {
  ...
  balancers_[0]->ads_service()->SetEdsResource(
      BuildEdsResource(args, DefaultEdsServiceName()));
  // Wait until the drop rate increases to the middle of the two
  // configs, which implies that the update has been in effect.
...
// Tests that the old LB call is still used after the balancer address
// update as long as that call is still alive.
TEST_P(BalancerUpdateTest,
       UpdateBalancersButKeepUsingOriginalBalancer) {
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  AdsServiceImpl::EdsResourceArgs args(
      {{"locality0", CreateEndpointsForBackends(0, 1)}});
  ...
  // The current LB call is still working, so xds continued using it to
  // the first balancer, which doesn't assign the second backend.
  EXPECT_EQ(0U, backends_[1]->backend_service()->request_count());
  // The ADS service of balancer 0 sent at least 1 response.
  EXPECT_GT(balancers_[0]->ads_service()->eds_response_state().state,
            AdsServiceImpl::ResponseState::NOT_SENT);
...
// address updates as long as that call is still alive. Send an update
// with the same set of LBs as the one in SetUp() in order to verify
// that the LB channel inside xds keeps the initial connection (which by
// definition is also present in the update).
TEST_P(BalancerUpdateTest, Repeated) {
  ...
  // xds continued using the original LB call to the first balancer,
  // which doesn't assign the second backend.
  EXPECT_EQ(0U, backends_[1]->backend_service()->request_count());
  ports.clear();
  ports.emplace_back(balancers_[0]->port());
  ports.emplace_back(balancers_[1]->port());
...
// Tests that if the balancer is down, the RPCs will still be sent to
// the backends according to the last balancer response, until a new
// balancer is reachable.
TEST_P(BalancerUpdateTest, DeadUpdate) {
  ...
  SetNextResolutionForLbChannel({balancers_[1]->port()});
  gpr_log(GPR_INFO, "========= UPDATE 1 DONE ==========");
  // Wait until update has been processed, as signaled by the second
  // backend receiving a request. In the meantime, the client continues
  // to be serviced (by the first backend) without interruption.
  EXPECT_EQ(0U, backends_[1]->backend_service()->request_count());
  WaitForBackend(1);
...
TEST_P(ClientLoadReportingTest, Vanilla) {
  ...
  // TODO(juanlishen): Partition the backends after multiple localities
  // is tested.
  AdsServiceImpl::EdsResourceArgs args({
      {"locality0", CreateEndpointsForBackends()},
  });
  ...
  EXPECT_EQ(
      (kNumRpcsPerAddress + kNumFailuresPerAddress) * num_backends_ +
          num_ok + num_failure,
...
// Tests that if the balancer restarts, the client load report contains
// the stats before and after the restart correctly.
...
TEST_P(ClientLoadReportingWithDropTest, Vanilla) {
  ...
  EXPECT_THAT(
      static_cast<double>(
          client_stats.dropped_requests(kThrottleDropType)) /
...
class FaultInjectionTest : public XdsEnd2endTest {
 public:
  ...
  // Builds a Listener with Fault Injection filter config. If the
  // http_fault is nullptr, then assign an empty filter config. This
  // filter config is required to enable the fault injection features.
  static Listener BuildListenerWithFaultInjection(
      const HTTPFault& http_fault = HTTPFault()) {
    HttpConnectionManager http_connection_manager;
    Listener listener;
    listener.set_name(kServerName);
    HttpFilter* fault_filter =
        http_connection_manager.add_http_filters();
    fault_filter->set_name("envoy.fault");
    fault_filter->mutable_typed_config()->PackFrom(http_fault);
    HttpFilter* router_filter =
        http_connection_manager.add_http_filters();
  ...
  void SetFilterConfig(HTTPFault& http_fault) {
    switch (GetParam().filter_config_setup()) {
      ...
      case TestType::FilterConfigSetup::
          kHTTPConnectionManagerOriginal: {
        Listener listener = BuildListenerWithFaultInjection(http_fault);
        SetListenerAndRouteConfiguration(0, listener,
                                         default_route_config_);
      }
    };
  }
...
TEST_P(FaultInjectionTest, XdsFaultInjectionAlwaysAbort) {
  ...
  auto* abort_percentage =
      http_fault.mutable_abort()->mutable_percentage();
...
TEST_P(FaultInjectionTest, XdsFaultInjectionPercentageAbort) {
  ...
  const double kErrorTolerance = 0.05;
  const size_t kNumRpcs =
      ComputeIdealNumRpcs(kAbortRate, kErrorTolerance);
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  // Create an EDS resource
  AdsServiceImpl::EdsResourceArgs args({
  ...
  const double seen_abort_rate =
      static_cast<double>(num_aborted) / kNumRpcs;
...
TEST_P(FaultInjectionTest, XdsFaultInjectionPercentageDelay) {
  const uint32_t kRpcTimeoutMilliseconds =
      grpc_test_slowdown_factor() * 3000;
  ...
  const double kErrorTolerance = 0.05;
  const size_t kNumRpcs =
      ComputeIdealNumRpcs(kDelayRate, kErrorTolerance);
  ...
  // Construct the fault injection filter config
  HTTPFault http_fault;
  auto* delay_percentage =
      http_fault.mutable_delay()->mutable_percentage();
  ...
  const double seen_delay_rate =
      static_cast<double>(num_delayed) / kNumRpcs;
...
TEST_P(FaultInjectionTest,
       XdsFaultInjectionAlwaysDelayPercentageAbort) {
  const uint32_t kAbortPercentagePerHundred = 50;
  const double kAbortRate = kAbortPercentagePerHundred / 100.0;
  const uint32_t kFixedDelaySeconds = 1;
  const uint32_t kRpcTimeoutMilliseconds =
      100 * 1000;  // 100s should not reach
  ...
  // Config fault injection via different setup
  SetFilterConfig(http_fault);
  // Allow the channel to connect to one backends, so the herd of queued
  // RPCs won't be executed on the same ExecCtx object and using the
  // cached Now() value, which causes millisecond level delay error.
  channel_->WaitForConnected(grpc_timeout_milliseconds_to_deadline(
      kConnectionTimeoutMilliseconds));
  // Send kNumRpcs RPCs and count the aborts.
  int num_aborted = 0;
  RpcOptions rpc_options =
      RpcOptions().set_timeout_ms(kRpcTimeoutMilliseconds);
  std::vector<ConcurrentRpc> rpcs =
      SendConcurrentRpcs(stub_.get(), kNumRpcs, rpc_options);
  for (auto& rpc : rpcs) {
...
// This test and the above test apply different denominators to delay
// and abort. This ensures that we are using the right denominator for
// each injected fault in our code.
TEST_P(FaultInjectionTest,
...
TEST_P(FaultInjectionTest, XdsFaultInjectionMaxFault) {
  ...
  const uint32_t kNumRpcs =
      30;  // kNumRpcs should be bigger than kMaxFault
 ...
 public:
  ...
  void SetUp() override {
    ...
    std::string admin_server_address =
        absl::StrCat(ipv6_only_ ? "[::1]:" : "127.0.0.1:",
                     admin_server_thread_->port());
    admin_channel_ = grpc::CreateChannel(
        admin_server_address,
        std::make_shared<SecureChannelCredentials>(
            grpc_fake_transport_security_credentials_create()));
    csds_stub_ = envoy::service::status::v3::
        ClientStatusDiscoveryService::NewStub(admin_channel_);
...
MATCHER_P4(EqNode, id, user_agent_name, user_agent_version,
           client_features, "equals Node") {
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(id, arg.id(), result_listener);
  ok &= ::testing::ExplainMatchResult(
      user_agent_name, arg.user_agent_name(), result_listener);
  ok &= ::testing::ExplainMatchResult(
      user_agent_version, arg.user_agent_version(), result_listener);
  ok &= ::testing::ExplainMatchResult(
      client_features, arg.client_features(), result_listener);
  return ok;
}
...
MATCHER_P2(EqListenersConfigDump, version_info, dynamic_listeners,
  ...
  ok &= ::testing::ExplainMatchResult(::testing::ElementsAre(),
                                      arg.static_listeners(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(version_info, arg.version_info(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(
      dynamic_listeners, arg.dynamic_listeners(), result_listener);
  return ok;
}
...
MATCHER_P2(EqDynamicListenerState, version_info, listener,
  ...
  ok &= ::testing::ExplainMatchResult(version_info, arg.version_info(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(listener, arg.listener(),
                                      result_listener);
  return ok;
}
...
MATCHER_P2(EqListener, name, api_listener, "equals Listener") {
  bool ok = true;
  ok &=
      ::testing::ExplainMatchResult(name, arg.name(), result_listener);
  ok &= ::testing::ExplainMatchResult(
...
MATCHER_P(EqRoutesConfigDump, dynamic_route_configs,
  ...
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(::testing::ElementsAre(),
                                      arg.static_route_configs(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(dynamic_route_configs,
                                      arg.dynamic_route_configs(),
                                      result_listener);
  return ok;
}
...
MATCHER_P2(EqClustersConfigDump, version_info, dynamic_active_clusters,
  ...
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(
      ::testing::ElementsAre(), arg.static_clusters(), result_listener);
  ok &= ::testing::ExplainMatchResult(::testing::ElementsAre(),
                                      arg.dynamic_warming_clusters(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(version_info, arg.version_info(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(dynamic_active_clusters,
                                      arg.dynamic_active_clusters(),
                                      result_listener);
  return ok;
}
...
MATCHER_P(EqEndpoint, port, "equals Endpoint") {
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(
      port, arg.address().socket_address().port_value(),
      result_listener);
  return ok;
}

MATCHER_P2(EqLocalityLbEndpoints, port, weight,
           "equals LocalityLbEndpoints") {
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(
      ::testing::ElementsAre(::testing::Property(
...
MATCHER_P2(EqUpdateFailureState, details, version_info,
  ...
  bool ok = true;
  ok &= ::testing::ExplainMatchResult(details, arg.details(),
                                      result_listener);
  ok &= ::testing::ExplainMatchResult(version_info, arg.version_info(),
                                      result_listener);
  return ok;
...
MATCHER_P(UnpackListener, matcher, "is a Listener") {
  ...
  return ::testing::ExplainMatchResult(matcher, config,
                                       result_listener);
}

MATCHER_P(UnpackRouteConfiguration, matcher,
...
MATCHER_P(UnpackHttpConnectionManager, matcher,
...
MATCHER_P(UnpackClusterLoadAssignment, matcher,
...
MATCHER_P5(EqDynamicListener, name, version_info, client_status,
           api_listener_matcher, error_state,
  ...
  ok &=
      ::testing::ExplainMatchResult(name, arg.name(), result_listener);
  ok &= ::testing::ExplainMatchResult(
      client_status, arg.client_status(), result_listener);
...
MATCHER_P5(EqDynamicRouteConfig, name, version_info, client_status,
  ...
  if (client_status == ClientResourceStatus::REQUESTED ||
    ...
    ok &= ::testing::ExplainMatchResult(
        UnpackRouteConfiguration(
            EqRouteConfiguration(name, cluster_name)),
        arg.route_config(), result_listener);
  }
  ok &= ::testing::ExplainMatchResult(error_state, arg.error_state(),
                                      result_listener);
...
MATCHER_P4(EqDynamicCluster, name, version_info, client_status,
           error_state, "equals DynamicCluster") {
  bool ok = true;
...
MATCHER_P6(EqDynamicEndpointConfig, name, version_info, client_status,
           port, weight, error_state, "equals DynamicEndpointConfig") {
  bool ok = true;
  if (client_status == ClientResourceStatus::REQUESTED ||
      client_status == ClientResourceStatus::DOES_NOT_EXIST) {
    ok &= ::testing::ExplainMatchResult(
...
MATCHER_P2(EqNoRdsHCM, route_configuration_name, cluster_name,
  ...
  return ::testing::ExplainMatchResult(
      UnpackHttpConnectionManager(
          EqHttpConnectionManagerNotRds(EqRouteConfiguration(
              route_configuration_name, cluster_name))),
      arg, result_listener);
}
...
TEST_P(ClientStatusDiscoveryServiceTest, XdsConfigDumpVanilla) {
  ...
  gpr_log(GPR_INFO, "xDS config dump: %s",
          csds_response.DebugString().c_str());
  ...
  if (GetParam().enable_rds_testing()) {
    ...
    route_config_dump_matcher =
        EqRoutesConfigDump(::testing::ElementsAre(
            EqDynamicRouteConfig(kDefaultRouteConfigurationName, "1",
          ...
          ::testing::Property(
              ...
              EqEndpointsConfigDump(
                  ::testing::ElementsAre(EqDynamicEndpointConfig(
                      kDefaultEdsServiceName, "1",
                      ClientResourceStatus::ACKED, backends_[0]->port(),
                      kDefaultLocalityWeight, ::testing::_))))));
}

TEST_P(ClientStatusDiscoveryServiceTest, XdsConfigDumpEmpty) {
  // The CSDS service should not fail if XdsClient is not initialized or
  // there is no working xDS configs.
  FetchCsdsResponse();
}
            ...
            EqListenersConfigDump(
                ...
                ::testing::ElementsAre(EqDynamicListener(
                    ...
                    EqUpdateFailureState(
                        ::testing::HasSubstr("Listener has neither "
                                             "address nor ApiListener"),
                        "2")))))));
    if (ok) return;  // TEST PASSED!
    gpr_sleep_until(grpc_timeout_milliseconds_to_deadline(
        kFetchIntervalMilliseconds));
  }
...
TEST_P(ClientStatusDiscoveryServiceTest, XdsConfigDumpEndpointError) {
  ...
  endpoint->mutable_address()->mutable_socket_address()->set_port_value(
      1 << 1);
    ...
    bool ok = ::testing::Value(
        ...
        ::testing::Contains(::testing::Property(
            ...
            EqEndpointsConfigDump(
                ::testing::ElementsAre(EqDynamicEndpointConfig(
                    kDefaultEdsServiceName, "1",
                    ClientResourceStatus::NACKED, backends_[0]->port(),
...
TEST_P(ClientStatusDiscoveryServiceTest,
       XdsConfigDumpListenerRequested) {
...
TEST_P(ClientStatusDiscoveryServiceTest,
  ...
  // The VirtualHost must match the domain name, otherwise will cause
...
TEST_P(CsdsShortAdsTimeoutTest, XdsConfigDumpListenerDoesNotExist) {
  int kTimeoutMillisecond =
      1000000;  // 1000s wait for the transient failure.
  balancers_[0]->ads_service()->UnsetResource(kLdsTypeUrl, kServerName);
  CheckRpcSendFailure(1,
                      RpcOptions().set_timeout_ms(kTimeoutMillisecond),
                      grpc::UNAVAILABLE);
...
// Run with all combinations of xds/fake resolver and enabling load
// reporting.
...
// Don't run with load reporting or v2 or RDS, since they are irrelevant
// to the tests.
INSTANTIATE_TEST_SUITE_P(
    XdsTest, SecureNamingTest,
...
// Not enabling load reporting or RDS, since those are irrelevant to
// these tests.
INSTANTIATE_TEST_SUITE_P(
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        TestType().set_use_fake_resolver().set_use_xds_credentials()),
    &TestTypeName);
...
// XdsResolverLoadReprtingOnlyTest depends on XdsResolver and load
// reporting.
INSTANTIATE_TEST_SUITE_P(
