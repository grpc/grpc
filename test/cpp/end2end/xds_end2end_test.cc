...
// Test that LDS client should reject route which has invalid regex.
TEST_P(LdsRdsTest, RouteMatchInvalidRegex) {
  gpr_setenv("GRPC_XDS_EXPERIMENTAL_ROUTING", "true");
...
TEST_P(LdsRdsTest, XdsRoutingRegexMatching) {
  gpr_setenv("GRPC_XDS_EXPERIMENTAL_ROUTING", "true");
  const char* kNewCluster1Name = "new_cluster_1";
  const char* kNewCluster2Name = "new_cluster_2";
  const size_t kNumEcho1Rpcs = 10;
  ...
  route1->mutable_match()->mutable_safe_regex()->set_regex(".*[1-1].*");
  route1->mutable_route()->set_cluster(kNewCluster1Name);
  auto* route2 = new_route_config.mutable_virtual_hosts(0)->add_routes();
  // Will match "/grpc.testing.EchoTest2Service/"
  route2->mutable_match()->mutable_safe_regex()->set_regex(".*[2-2].*");
  route2->mutable_route()->set_cluster(kNewCluster2Name);
  auto* default_route = new_route_config.mutable_virtual_hosts(0)->add_routes();
  default_route->mutable_match()->set_prefix("");
  default_route->mutable_route()->set_cluster(kDefaultResourceName);
