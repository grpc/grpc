...
namespace grpc
{
  namespace testing
  {
    ...
    class SynchronousClient:public ClientImpl < BenchmarkService::Stub,
      SimpleRequest >
    {
    public:
      SynchronousClient (const ClientConfig & config):ClientImpl <
	BenchmarkService::Stub, SimpleRequest > (config, BenchmarkStubCreator)
      {
	num_threads_ =
	...
	SetupLoadTest (config, num_threads_);
      ...
      virtual bool ThreadFuncImpl (HistogramEntry * entry,
				   size_t thread_idx) = 0;
	...
	if (!closed_loop_)
	  {
		...
		if (gpr_time_cmp (next_issue_time, one_sec_delay) <= 0)
		...
		else
		  {
		    ...
		    if (gpr_atm_acq_load (&thread_pool_done_) != static_cast <
			gpr_atm > (0))
		      {
			return false;
		      }
    ...
    {
    public:
      SynchronousUnaryClient (const ClientConfig &
			      config):SynchronousClient (config)
      {
  ...
  template < class StreamType > class SynchronousStreamingClient:public
      SynchronousClient
    {
    public:
      SynchronousStreamingClient (const ClientConfig &
				  config):SynchronousClient (config),
	context_ (num_threads_), stream_ (num_threads_),
	stream_mu_ (num_threads_), shutdown_ (num_threads_),
	...
	if (!s.ok ())
	  {
	    ...
	    if (!shutdown_[thread_idx].val)
	      {
		gpr_log (GPR_ERROR,
			 "Stream %" PRIuPTR " received an error %s",
    ...
    class SynchronousStreamingPingPongClient final:public
      SynchronousStreamingClient < grpc::ClientReaderWriter < SimpleRequest,
      SimpleResponse >>
    {
    public:
      SynchronousStreamingPingPongClient (const ClientConfig &
					  config):SynchronousStreamingClient
	(config)
      {
      }
    ...
    class SynchronousStreamingFromClientClient final:public
      SynchronousStreamingClient < grpc::ClientWriter < SimpleRequest >>
    {
    public:
      SynchronousStreamingFromClientClient (const ClientConfig &
					    config):SynchronousStreamingClient
	(config), last_issue_ (num_threads_)
      {
      }
	...
	if (!shutdown_[thread_idx].val)
	  {
	    stream_[thread_idx] =
	      stub->StreamingFromClient (&context_[thread_idx],
	...
	GPR_TIMER_SCOPE ("SynchronousStreamingFromClientClient::ThreadFunc",
			 0);
	if (stream_[thread_idx]->Write (request_))
	  {
    ...
    class SynchronousStreamingFromServerClient final:public
      SynchronousStreamingClient < grpc::ClientReader < SimpleResponse >>
    {
    public:
      SynchronousStreamingFromServerClient (const ClientConfig &
					    config):SynchronousStreamingClient
	(config), last_recv_ (num_threads_)
      {
      }
	...
	return true;
      ...
      bool ThreadFuncImpl (HistogramEntry * entry, size_t thread_idx) override
      {
	GPR_TIMER_SCOPE ("SynchronousStreamingFromServerClient::ThreadFunc",
			 0);
    ...
    class SynchronousStreamingBothWaysClient final:public
      SynchronousStreamingClient < grpc::ClientReaderWriter < SimpleRequest,
      SimpleResponse >>
    {
    public:
      SynchronousStreamingBothWaysClient (const ClientConfig &
					  config):SynchronousStreamingClient
	(config)
      {
      }
	...
	if (!shutdown_[thread_idx].val)
	  {
	    stream_[thread_idx] =
	      stub->StreamingBothWays (&context_[thread_idx]);
	  }
	else
	  {
	    return false;
    ...
    std::unique_ptr < Client >
      CreateSynchronousClient (const ClientConfig & config)
    {
      switch (config.rpc_type ())
	{
	case UNARY:
	  return std::unique_ptr < Client >
	    (new SynchronousUnaryClient (config));
	case STREAMING:
	  return std::unique_ptr < Client >
