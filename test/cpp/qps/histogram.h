...
namespace grpc
{
  namespace testing
  {
    ...
    public:
      ...
      Histogram ():impl_ (grpc_histogram_create (default_resolution (),
	...
	other.impl_ = nullptr;
      ...
      {
	grpc_histogram_merge (impl_, h.impl_);
      }
      void Add (double value)
      {
	grpc_histogram_add (impl_, value);
      }
	...
	return grpc_histogram_percentile (impl_, pctile);
      ...
      double Count () const
      {
	return grpc_histogram_count (impl_);
      }
	...
	p->set_count (grpc_histogram_count (impl_));
      ...
      {
	grpc_histogram_merge_contents (impl_, &*p.bucket ().begin (),
				       p.bucket_size (), p.min_seen (),
				       p.max_seen (), p.sum (),
