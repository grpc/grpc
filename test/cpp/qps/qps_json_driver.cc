...
ABSL_FLAG(double, initial_search_value, 0.0,
          "initial parameter value to start the search with (i.e. "
          "lower bound)");
...
ABSL_FLAG(std::string, json_file_out, "",
...
ABSL_FLAG(std::string, per_worker_credential_types, "",
          "A map of QPS worker addresses to credential types. When "
          "creating a "
          "channel to a QPS worker's driver port, the qps_json_driver "
          "first checks "
          "if the 'name:port' string is in the map, and it uses the "
          "corresponding "
          "credential type if so. If the QPS worker's 'name:port' "
          "string is not "
          "in the map, then the driver -> worker channel will be "
          "created with "
          "the credentials specified in --credential_type. The value "
          "of this flag "
          "is a semicolon-separated list of map entries, where each "
          "map entry is "
          "a comma-separated pair.");
ABSL_FLAG(bool, run_inproc, false,
...
ABSL_FLAG(
    ...
    "end of the benchmark run. If 0, this periodic collection is "
    "disabled.");
...
ConstructPerWorkerCredentialTypesMap() {
  // Parse a list of the form: "addr1,cred_type1;addr2,cred_type2;..."
  // into a map.
  std::string remaining =
      absl::GetFlag(FLAGS_per_worker_credential_types);
  ...
  while (!remaining.empty()) {
    ...
    if (next_semicolon == std::string::npos) {
      ...
      remaining =
          remaining.substr(next_semicolon + 1, std::string::npos);
    }
    ...
    std::string cred_type =
        next_entry.substr(comma + 1, std::string::npos);
...
static double SearchOfferedLoad(
    double initial_offered_load, double targeted_cpu_load,
    Scenario* scenario,
    const std::map<std::string, std::string>&
        per_worker_credential_types,
    bool* success) {
  ...
  double current_cpu_load =
      GetCpuLoad(scenario, current_offered_load,
  ...
  double targeted_offered_load = BinarySearch(
      scenario, targeted_cpu_load, current_offered_load / 2,
...
static bool QpsDriver() {
  ...
  auto per_worker_credential_types =
      ConstructPerWorkerCredentialTypesMap();
  if (scfile) {
    // Read the json data from disk
    FILE* json_file =
        fopen(absl::GetFlag(FLAGS_scenarios_file).c_str(), "r");
    ...
    if (absl::GetFlag(FLAGS_search_param).empty()) {
      ...
      if (absl::GetFlag(FLAGS_search_param) == "offered_load") {
        ...
        double targeted_offered_load = SearchOfferedLoad(
            absl::GetFlag(FLAGS_initial_search_value),
            absl::GetFlag(FLAGS_targeted_cpu_load), scenario,
            per_worker_credential_types, &success);
        gpr_log(GPR_INFO, "targeted_offered_load %f",
