...
ABSL_FLAG(std::string, search_param, "",
          ...
          "added.");
ABSL_FLAG(double, initial_search_value, 0.0,
...
ABSL_FLAG(int32_t, median_latency_collection_interval_millis, 0,
...
static std::map<std::string, std::string> ConstructPerWorkerCredentialTypesMap() {
  ...
  while (!remaining.empty()) {
    ...
    if (out.find(addr) != out.end()) {
      gpr_log(GPR_ERROR, "Found duplicate addr in per_worker_credential_types.");
      abort();
    }
    out[addr] = cred_type;
  }
...
static std::unique_ptr<ScenarioResult> RunAndReport(
    const Scenario& scenario, const std::map<std::string, std::string>& per_worker_credential_types,
    bool* success) {
  std::cerr << "RUNNING SCENARIO: " << scenario.name() << "\n";
  auto result = RunScenario(
      scenario.client_config(), scenario.num_clients(), scenario.server_config(),
      scenario.num_servers(), scenario.warmup_seconds(), scenario.benchmark_seconds(),
      !absl::GetFlag(FLAGS_run_inproc) ? scenario.spawn_local_worker_count() : -2,
      absl::GetFlag(FLAGS_qps_server_target_override), absl::GetFlag(FLAGS_credential_type),
      per_worker_credential_types, absl::GetFlag(FLAGS_run_inproc),
...
static double GetCpuLoad(Scenario* scenario, double offered_load,
                         ...
                         bool* success) {
  scenario->mutable_client_config()->mutable_load_params()->mutable_poisson()->set_offered_load(
      offered_load);
  auto result = RunAndReport(*scenario, per_worker_credential_types, success);
  return result->summary().server_cpu_usage();
}

static double BinarySearch(Scenario* scenario, double targeted_cpu_load, double low, double high,
                           const std::map<std::string, std::string>& per_worker_credential_types,
                           bool* success) {
  while (low <= high * (1 - absl::GetFlag(FLAGS_error_tolerance))) {
    double mid = low + (high - low) / 2;
    double current_cpu_load = GetCpuLoad(scenario, mid, per_worker_credential_types, success);
...
static double SearchOfferedLoad(
    ...
    const std::map<std::string, std::string>& per_worker_credential_types, bool* success) {
  std::cerr << "RUNNING SCENARIO: " << scenario->name() << "\n";
  double current_offered_load = initial_offered_load;
  double current_cpu_load =
      GetCpuLoad(scenario, current_offered_load, per_worker_credential_types, success);
  ...
  while (*success && (current_cpu_load < targeted_cpu_load)) {
    ...
    current_cpu_load =
        GetCpuLoad(scenario, current_offered_load, per_worker_credential_types, success);
    gpr_log(GPR_DEBUG, "Binary Search: current_offered_load  %.0f", current_offered_load);
  }

  double targeted_offered_load =
      BinarySearch(scenario, targeted_cpu_load, current_offered_load / 2, current_offered_load,
...
static bool QpsDriver() {
  ...
  if ((!scfile && !scjson && !absl::GetFlag(FLAGS_quit)) ||
      (scfile && (scjson || absl::GetFlag(FLAGS_quit))) || (scjson && absl::GetFlag(FLAGS_quit))) {
    gpr_log(GPR_ERROR,
  ...
  if (scfile) {
    ...
    return RunQuit(absl::GetFlag(FLAGS_credential_type), per_worker_credential_types);
  }
    ...
    if (absl::GetFlag(FLAGS_search_param).empty()) {
      ...
      if (absl::GetFlag(FLAGS_search_param) == "offered_load") {
        ...
        double targeted_offered_load = SearchOfferedLoad(
            absl::GetFlag(FLAGS_initial_search_value), absl::GetFlag(FLAGS_targeted_cpu_load),
            scenario, per_worker_credential_types, &success);
        gpr_log(GPR_INFO, "targeted_offered_load %f", targeted_offered_load);
        GetCpuLoad(scenario, targeted_offered_load, per_worker_credential_types, &success);
      } else {
