...
DEFINE_string (search_param, "",
	       ...
	       "added.");
DEFINE_double (initial_search_value, 0.0,
...
namespace grpc
{
  namespace testing
  {
    ...
    static std::unique_ptr < ScenarioResult >
      RunAndReport (const Scenario & scenario, bool * success)
    {
    ...
    static double GetCpuLoad (Scenario * scenario, double offered_load,
			      bool * success)
    {
      scenario->mutable_client_config ()->mutable_load_params ()->
	mutable_poisson ()->set_offered_load (offered_load);
	      ...
	      if (FLAGS_search_param == "offered_load")
		{
		  ...
		  double targeted_offered_load =
		    SearchOfferedLoad (FLAGS_initial_search_value,
				       FLAGS_targeted_cpu_load, scenario,
				       &success);
