  ...
  namespace testing
  {
    ...
    public:
      AsyncQpsServerTest (const ServerConfig & config,
			  std::function < void (ServerBuilder *,
						ServiceType *) >
			  register_service,
			  std::function < void (ServiceType *,
						ServerContextType *,
						RequestType *,
			  ...
			  std::function < grpc::Status (const PayloadConfig &,
							RequestType *,
							ResponseType *) >
			  process_rpc):Server (config)
      {
	...
	auto process_rpc_bound =
	  std::bind (process_rpc, config.payload_config (),
		     std::placeholders::_1,
		     std::placeholders::_2);
	...
	for (int i = 0; i < 5000; i++)
	  {
		...
		if (request_unary_function)
		  {
		    auto request_unary =
		      std::bind (request_unary_function, &async_service_,
				 std::placeholders::_1,
		    ...
		    contexts_.
		      emplace_back (new
				    ServerRpcContextUnaryImpl (request_unary,
							       process_rpc_bound));
		  }
		if (request_streaming_function)
		  {
		    auto request_streaming =
		      std::bind (request_streaming_function, &async_service_,
		...
		if (request_streaming_from_client_function)
		  {
		    auto request_streaming_from_client =
		      std::bind (request_streaming_from_client_function,
				 &async_service_,
				 std::placeholders::_1, std::placeholders::_2,
				 srv_cqs_[j].get (),
				 srv_cqs_[j].get (), std::placeholders::_3);
		    contexts_.
		      emplace_back (new
				    ...
				    (request_streaming_from_client,
				     process_rpc_bound));
		  }
		if (request_streaming_from_server_function)
		  {
		    auto request_streaming_from_server =
		      std::bind (request_streaming_from_server_function,
				 &async_service_,
				 std::placeholders::_1, std::placeholders::_2,
	...
	std::thread shutdown_thread (&AsyncQpsServerTest::ShutdownThreadFunc,
				     this);
	...
	while (srv_cqs_[cq_[thread_idx]]->DoThenAsyncNext ([&, ctx, ok,
							    mu_ptr] ()
							   {
							   ctx->lock ();
							   if (!ctx->
	...
	{
	public:
	  ServerRpcContextUnaryImpl (std::function <
				     void (ServerContextType *, RequestType *,
				     ...
				     grpc::Status (RequestType *,
						   ResponseType *) >
				     invoke_method):srv_ctx_ (new
							      ServerContextType),
	    next_state_ (&ServerRpcContextUnaryImpl::invoker),
	    request_method_ (request_method), invoke_method_ (invoke_method),
	...
	private:
	  ...
	  std::function < void (ServerContextType *, RequestType *,
				grpc::ServerAsyncResponseWriter <
				ResponseType > *, void *) > request_method_;
	  std::function < grpc::Status (RequestType *,
					ResponseType *) > invoke_method_;
	...
	{
	public:
	  ServerRpcContextStreamingImpl (std::function <
					 void (ServerContextType *,
	  ...
	  {
	    request_method_ (srv_ctx_.get (), &stream_,
			     AsyncQpsServerTest::tag (this));
	  }
	...
	private:
	  ...
	  {
	    return false;	/* reset the context */
	  }
	...
	class ServerRpcContextStreamingFromClientImpl final:public
	  ServerRpcContext
	{
	public:
	  ServerRpcContextStreamingFromClientImpl (std::function <
						   void (ServerContextType *,
							 grpc::
							 ServerAsyncReader <
							 ResponseType,
						   ...
						   Status (RequestType *,
							   ResponseType *) >
						   invoke_method):srv_ctx_
	    (new ServerContextType),
	    next_state_ (&ServerRpcContextStreamingFromClientImpl::
			 request_done), request_method_ (request_method),
	    invoke_method_ (invoke_method), stream_ (srv_ctx_.get ())
	  {
	    request_method_ (srv_ctx_.get (), &stream_,
			     AsyncQpsServerTest::tag (this));
	  }
	  ...
	  void Reset () override
	  {
	    ...
	    req_ = RequestType ();
	    stream_ =
	      grpc::ServerAsyncReader < ResponseType,
	      RequestType > (srv_ctx_.get ());

	    // Then request the method
	    next_state_ =
	      &ServerRpcContextStreamingFromClientImpl::request_done;
	    request_method_ (srv_ctx_.get (), &stream_,
			     AsyncQpsServerTest::tag (this));
	  }
	...
	private:
	  ...
	  bool read_done (bool ok)
	  {
		...
		// finish the stream
		next_state_ =
		  &ServerRpcContextStreamingFromClientImpl::finish_done;
	...
	class ServerRpcContextStreamingFromServerImpl final:public
	  ServerRpcContext
	{
	public:
	  ServerRpcContextStreamingFromServerImpl (std::function <
						   void (ServerContextType *,
							 RequestType *,
							 grpc::
							 ServerAsyncWriter <
						   ...
						   invoke_method):srv_ctx_
	    (new ServerContextType),
	    next_state_ (&ServerRpcContextStreamingFromServerImpl::
			 request_done), request_method_ (request_method),
	    invoke_method_ (invoke_method), stream_ (srv_ctx_.get ())
	  {
	  ...
	  void Reset () override
	  {
	    ...
	    req_ = RequestType ();
	    stream_ =
	      grpc::ServerAsyncWriter < ResponseType > (srv_ctx_.get ());

	    // Then request the method
	    next_state_ =
	      &ServerRpcContextStreamingFromServerImpl::request_done;
	    request_method_ (srv_ctx_.get (), &req_, &stream_,
			     AsyncQpsServerTest::tag (this));
	  }
	...
	private:
	  bool request_done (bool ok)
	  {
	    ...
	    next_state_ =
	      &ServerRpcContextStreamingFromServerImpl::write_done;
	    stream_.Write (response_, AsyncQpsServerTest::tag (this));
	    return true;
	  }
	  ...
	  bool write_done (bool ok)
	  {
		...
		next_state_ =
		  &ServerRpcContextStreamingFromServerImpl::finish_done;
		stream_.Finish (Status::OK, AsyncQpsServerTest::tag (this));
	      }
	    return true;
	  }
	...
	std::vector < std::unique_ptr <
	  grpc::ServerCompletionQueue >> srv_cqs_;
	std::vector < int >cq_;
	  ...
	  CreateAsyncServer (const ServerConfig & config)
	{
	  return std::unique_ptr < Server > (new AsyncQpsServerTest <
					     ...
					     grpc::ServerContext > (config,
								    ...
								    &BenchmarkService::
								    AsyncService::
								    RequestStreamingFromClient,
								    &BenchmarkService::
								    AsyncService::
								    RequestStreamingFromServer,
								    &BenchmarkService::
								    AsyncService::
								    RequestStreamingBothWays,
								    ProcessSimpleRPC));
	}
	std::unique_ptr < Server >
	  CreateAsyncGenericServer (const ServerConfig & config)
	{
	  return std::unique_ptr < Server > (new AsyncQpsServerTest <
					     ...
					     (config, RegisterGenericService,
					      nullptr,
					      &grpc::AsyncGenericService::
					      RequestCall, nullptr, nullptr,
					      nullptr, ProcessGenericRPC));
	}
