...
namespace grpc
{
  namespace testing
  {
    ...
    void CompositeReporter::
      ReportQueriesPerCpuSec (const ScenarioResult & result)
    {
      for (size_t i = 0; i < reporters_.size (); ++i)
	{
      ...
      for (int i = 0; i < result.client_stats_size (); i++)
	{
	  if (result.client_stats (i).has_core_stats ())
	    {
	      ReportCoreStats ("CLIENT", i,
			       result.client_stats (i).core_stats ());
	    }
	}
      for (int i = 0; i < result.server_stats_size (); i++)
	{
	  if (result.server_stats (i).has_core_stats ())
	    {
	      ReportCoreStats ("SERVER", i,
			       result.server_stats (i).core_stats ());
	    }
	}
    }
	...
	{
	  gpr_log (GPR_DEBUG,
		   "%s[%d].%s = %.1lf/%.1lf/%.1lf (50/95/99%%-ile)", name,
		   idx, grpc_stats_histogram_name[i],
		   grpc_stats_histo_percentile (&data,
						(grpc_stats_histograms) i,
						50),
		   grpc_stats_histo_percentile (&data,
						(grpc_stats_histograms) i,
						95),
