  ...
  grpc
{
    ...
    testing
  {
    static
      std::string
    get_host (const std::string & worker)
    {
      char *
	host;
      char *
    ...
    static
      deque <
      string >
    get_workers (const string & env_name)
    {
      deque < string > out;
      char *
	env = gpr_getenv (env_name.c_str ());
      if (!env)
	{
	  env = gpr_strdup ("");
	}
...
// helpers for postprocess_scenario_result
    static double
    WallTime (ClientStats s)
    {
      return s.time_elapsed ();
    }
    static double
    SystemTime (ClientStats s)
    {
      return s.time_system ();
    }
    static double
    UserTime (ClientStats s)
    {
      return s.time_user ();
    }
    static double
    CliPollCount (ClientStats s)
    {
      return s.cq_poll_count ();
    }
    static double
    SvrPollCount (ServerStats s)
    {
      return s.cq_poll_count ();
    }
    static double
    ServerWallTime (ServerStats s)
    {
      return s.time_elapsed ();
    }
    static double
    ServerSystemTime (ServerStats s)
    {
      return s.time_system ();
    }
    static double
    ServerUserTime (ServerStats s)
    {
      return s.time_user ();
    }
    static double
    ServerTotalCpuTime (ServerStats s)
    {
      return s.total_cpu_time ();
    }
    static double
    ServerIdleCpuTime (ServerStats s)
    {
...
// Postprocess ScenarioResult and populate result summary.
    static void
    postprocess_scenario_result (ScenarioResult * result)
    {
      Histogram
	histogram;
      ...
      auto
	time_estimate = average (result->client_stats (), WallTime);
      auto
	qps = histogram.Count () / time_estimate;
      auto
	qps_per_server_core = qps / sum (result->server_cores (), Cores);

      result->mutable_summary ()->set_qps (qps);
      result->mutable_summary ()->
	set_qps_per_server_core (qps_per_server_core);
      ...
      result->mutable_summary ()->set_latency_999 (histogram.
						   Percentile (99.9));

      auto
	server_system_time = 100.0 *
	sum (result->server_stats (), ServerSystemTime) /
	sum (result->server_stats (), ServerWallTime);
      auto
	server_user_time =
	100.0 * sum (result->server_stats (),
		     ServerUserTime) / sum (result->server_stats (),
					    ServerWallTime);

      auto
	client_system_time =
	100.0 * sum (result->client_stats (),
		     SystemTime) / sum (result->client_stats (), WallTime);
      auto
	client_user_time = 100.0 * sum (result->client_stats (), UserTime) /
      ...
      if (result->request_results_size () > 0)
	{
      ...
      result->mutable_summary ()->
	set_client_polls_per_request (sum
				      (result->client_stats (),
				       CliPollCount) / histogram.Count ());
      result->mutable_summary ()->
	set_server_polls_per_request (sum
				      (result->server_stats (),
				       SvrPollCount) / histogram.Count ());
    ...
    std::unique_ptr < ScenarioResult >
      RunScenario (const ClientConfig & initial_client_config,
		   size_t num_clients,
      ...
      if (run_inproc)
	{
      ...
      auto
	alloc_context =[](list < ClientContext > *contexts) {
	...
	result_client_config;
      const ServerConfig
	result_server_config = initial_server_config;
	...
	(!run_inproc) ? get_workers ("QPS_WORKERS") : deque < string > ();
      ClientConfig
	client_config = initial_client_config;
      ...
      for (int i = 0; i < abs (spawn_local_worker_count); i++)
	{
	  ...
	  local_workers.
	    emplace_back (new QpsWorker (driver_port, 0, credential_type));
	...
	unique_ptr <
      ...
      std::unordered_map < string, std::deque < int >>
	hosts_cores;
      ChannelArguments
	channel_args;
      ...
      for (size_t i = 0; i < num_servers; i++)
	{
	  ...
	  if (!run_inproc)
	    {
	      servers[i].stub =
		WorkerService::
		NewStub (CreateChannel
			 (workers[i],
			  GetCredentialsProvider ()->
			  GetChannelCredentials (credential_type,
						 &channel_args)));
	    }
	  else
	    {
	      servers[i].stub =
		WorkerService::NewStub (local_workers[i]->
					InProcessChannel (channel_args));
	    }
	  ...
	  *args.mutable_setup () = server_config;
	  ...
	  if (!servers[i].stream->Read (&init_status))
	    {
	      gpr_log (GPR_ERROR, "Server %zu did not yield initial status",
		       i);
	    }
	  if (qps_server_target_override.length () > 0)
		...
		cli_target;
	      ...
	      gpr_join_host_port (&cli_target, host.c_str (),
				  init_status.port ());
	      client_config.add_server_targets (cli_target);
	      gpr_free (cli_target);
	    }
      ...
      for (size_t i = 0; i < num_clients; i++)
	{
	  ...
	  if (!run_inproc)
	    ...
	    {
	      clients[i].stub =
		WorkerService::NewStub (local_workers[i + num_servers]->
					InProcessChannel (channel_args));
	    }
	  ClientConfig
	  ...
	  gpr_log (GPR_DEBUG,
		   "Client %" PRIdPTR " gets %" PRIdPTR " channels", i,
		   num_channels);
      ...
      for (size_t i = 0; i < num_clients; i++)
	{
	  ...
	  if (!clients[i].stream->Read (&init_status))
	    {
	      gpr_log (GPR_ERROR, "Client %zu did not yield initial status",
		       i);
	    }
	}
      ...
      for (size_t i = 0; i < num_clients; i++)
	{
      ...
      gpr_timespec
	start = gpr_now (GPR_CLOCK_REALTIME);
      gpr_sleep_until (gpr_time_add
		       (start,
			gpr_time_from_seconds (warmup_seconds,
					       GPR_TIMESPAN)));
      ...
      for (size_t i = 0; i < num_clients; i++)
	{
      ...
      gpr_sleep_until (gpr_time_add (start,
				     gpr_time_from_seconds (warmup_seconds +
							    benchmark_seconds,
							    GPR_TIMESPAN)));
      ...
      for (size_t i = 0; i < num_clients; i++)
	{
      ...
      for (std::unordered_map < int, int64_t >::iterator it =
	   merged_statuses.begin (); it != merged_statuses.end (); ++it)
	{
	  RequestResultCount *
	    rrc = result->add_request_results ();
    ...
    bool
    RunQuit (const grpc::string & credential_type)
    {
      // Get client, server lists
      bool
	result = true;
      auto
	workers = get_workers ("QPS_WORKERS");
      if (workers.size () == 0)
	{
	  return false;
	}
      ...
      for (size_t i = 0; i < workers.size (); i++)
	{
	  ...
	  Status
	    s = stub->QuitWorker (&ctx, dummy, &dummy);
	  if (!s.ok ())
	    {
	      gpr_log (GPR_ERROR,
