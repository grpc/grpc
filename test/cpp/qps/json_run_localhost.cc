...
static SubProcess *
  g_driver;
static SubProcess *
  g_workers[kNumWorkers];

template < class T > std::string as_string (const T & val)
{
...
static void
LogStatus (int status, const char *label)
{
  if (WIFEXITED (status))
    {
...
main (int argc, char **argv)
{
  ...
  for (int i = 0; i < kNumWorkers; i++)
	...
	port = grpc_pick_unused_port_or_die ();
      ...
      {
      bin_dir + "/qps_worker", "-driver_port", as_string (port)};
  ...
  for (int i = 0; i < kNumWorkers; ++i)
	...
	{
	  const int
	    worker_status = g_workers[i]->Join ();
	  if (worker_status != 0)
	    {
