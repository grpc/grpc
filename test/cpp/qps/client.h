...
namespace grpc
{
  namespace testing
  {
    ...
    template < class RequestType > class ClientRequestCreator
    {
    public:
    ...
    template <> class ClientRequestCreator < SimpleRequest >
    {
    public:
	...
	if (payload_config.has_bytebuf_params ())
	...
	else if (payload_config.has_simple_params ())
	  {
	    ...
	    req->set_response_size (payload_config.simple_params ().
				    resp_size ());
	    req->mutable_payload ()->
	      set_type (grpc::testing::PayloadType::COMPRESSABLE);
    ...
    template <> class ClientRequestCreator < ByteBuffer >
    {
    public:
      ClientRequestCreator (ByteBuffer * req,
			    const PayloadConfig & payload_config)
      {
	if (payload_config.has_bytebuf_params ())
	  {
    ...
    inline void MergeStatusHistogram (const StatusHistogram & from,
				      StatusHistogram * to)
    {
      for (StatusHistogram::const_iterator it = from.begin ();
	   it != from.end (); ++it)
	{
    ...
    {
    public:
      Client ():timer_ (new UsageTimer),
	interarrival_timer_ (),
	started_requests_ (false), last_reset_poll_count_ (0)
      {
	...
	if (reset)
	  {
	    ...
	    std::vector < StatusHistogram >
	      to_merge_status (threads_.size ());
	...
	return stats;
      ...
      {
	gpr_atm_rel_store (&thread_pool_done_,
			   static_cast < gpr_atm > (true));
	...
	switch (load.load_case ())
	  ...
	  case LoadParams::kPoisson:
	    random_dist.
	      reset (new
		     ExpDist (load.poisson ().offered_load () / num_threads));
	    break;
	  default:
	    GPR_ASSERT (false);
	  }
	...
	if (!random_dist)
	...
	else
	  {
	    ...
	    for (size_t i = 0; i < num_threads; i++)
	      {
		next_time_.push_back (gpr_time_add (now,
						    gpr_time_from_nanos
						    (interarrival_timer_.
						     next (i),
						     GPR_TIMESPAN)));
	...
	next_time_[thread_idx] =
	  gpr_time_add (next_time_[thread_idx],
			gpr_time_from_nanos (interarrival_timer_.
					     next (thread_idx),
					     GPR_TIMESPAN));
	return result;
      }
	...
	return closed_loop_ ? std::function < gpr_timespec () > ()
      ...
      {
      public:
	Thread (Client * client, size_t idx):client_ (client), idx_ (idx),
	...
	{
	  int wait_loop = 0;
	  while (!gpr_event_wait (&client_->start_requests_,
				  gpr_time_add (gpr_now (GPR_CLOCK_REALTIME),
						gpr_time_from_seconds (20,
								       GPR_TIMESPAN))))
	    {
	      gpr_log (GPR_INFO,
		       "%" PRIdPTR ": Waiting for benchmark to start (%d)",
		       idx_, wait_loop);
	      wait_loop++;
	    }
  ...
  template < class StubType, class RequestType > class ClientImpl:public
      Client
    {
    public:
      ClientImpl (const ClientConfig & config,
		  ...
		  create_stub):cores_ (gpr_cpu_num_cores ()),
	create_stub_ (create_stub)
      {
	for (int i = 0; i < config.client_channels (); i++)
	  {
	    channels_.emplace_back (config.
				    server_targets (i %
						    config.
						    server_targets_size ()),
    ...
    protected:
      ...
      {
      public:
	ClientChannelInfo (const grpc::string & target,
			   ...
			   int shard)
	{
	  ...
	  if (target.find (inproc_pfx) != 0)
	  ...
	  else
	    {
	      ...
	      channel_ =
		(*g_inproc_servers)[srv_num]->InProcessChannel (args);
	      is_inproc_ = true;
	    }
	...
	std::unique_ptr < std::thread > WaitForReady ()
	{
      ...
      private:
	void set_channel_args (const ClientConfig & config,
			       ChannelArguments * args)
	{
	...
	bool is_inproc_;
      ...
      std::function < std::unique_ptr < StubType > (const std::shared_ptr <
						    Channel >
						    &) >create_stub_;
    };

    std::unique_ptr < Client >
      CreateSynchronousClient (const ClientConfig & args);
