...
static void
BM_NoOpExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
...
static void
BM_WellFlushed (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
...
static void
BM_ClosureInitAgainstExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_closure
  ...
  while (state.KeepRunning ())
    {
      benchmark::
	DoNotOptimize (GRPC_CLOSURE_INIT
...
static void
BM_ClosureInitAgainstCombiner (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
...
static void
BM_ClosureRunOnExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_closure
...
static void
BM_ClosureCreateAndRun (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  while (state.KeepRunning ())
    {
      GRPC_CLOSURE_RUN (GRPC_CLOSURE_CREATE
...
static void
BM_ClosureInitAndRun (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  ...
  while (state.KeepRunning ())
    {
      GRPC_CLOSURE_RUN (GRPC_CLOSURE_INIT
			(&c, DoNothing, nullptr, grpc_schedule_on_exec_ctx),
			GRPC_ERROR_NONE);
    }
...
static void
BM_ClosureSchedOnExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_closure
...
static void
BM_ClosureSched2OnExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_closure
...
static void
BM_ClosureSched3OnExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_closure
...
static void
BM_AcquireMutex (benchmark::State & state)
{
  TrackCounters
    track_counters;
...
static void
BM_TryAcquireMutex (benchmark::State & state)
{
  TrackCounters
    track_counters;
  // for comparison with the combiner stuff below
...
static void
BM_AcquireSpinlock (benchmark::State & state)
{
  TrackCounters
    track_counters;
  // for comparison with the combiner stuff below
  gpr_spinlock
    mu = GPR_SPINLOCK_INITIALIZER;
  grpc_core::ExecCtx exec_ctx;
  while (state.KeepRunning ())
    {
...
static void
BM_TryAcquireSpinlock (benchmark::State & state)
{
  TrackCounters
    track_counters;
  // for comparison with the combiner stuff below
...
static void
BM_ClosureSchedOnCombiner (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
...
static void
BM_ClosureSched2OnCombiner (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
...
static void
BM_ClosureSched3OnCombiner (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
...
static void
BM_ClosureSched2OnTwoCombiners (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
    combiner1 = grpc_combiner_create ();
  grpc_combiner *
...
static void
BM_ClosureSched4OnTwoCombiners (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_combiner *
...
public:
  ...
  void
  ScheduleFirstAgainstDifferentScheduler (grpc_closure_scheduler * scheduler)
  {
...
private:
  ...
  static void
  Step (void *arg, grpc_error * error)
  {
    Rescheduler *
      self = static_cast < Rescheduler * >(arg);
...
static void
BM_ClosureReschedOnExecCtx (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  Rescheduler
  r (state, grpc_schedule_on_exec_ctx);
  r.ScheduleFirst ();
...
static void
BM_ClosureReschedOnCombiner (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  grpc_combiner *
    combiner = grpc_combiner_create ();
  Rescheduler
  r (state, grpc_combiner_scheduler (combiner));
  r.ScheduleFirst ();
  grpc_core::ExecCtx::Get ()->Flush ();
  GRPC_COMBINER_UNREF (combiner, "finished");
...
static void
BM_ClosureReschedOnCombinerFinally (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  grpc_combiner *
    combiner = grpc_combiner_create ();
  Rescheduler
  r (state, grpc_combiner_finally_scheduler (combiner));
  r.ScheduleFirstAgainstDifferentScheduler (grpc_combiner_scheduler
					    (combiner));
  grpc_core::ExecCtx::Get ()->Flush ();
  GRPC_COMBINER_UNREF (combiner, "finished");
