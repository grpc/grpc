...
void
TrackCounters::Finish (benchmark::State & state)
{
...
void
TrackCounters::AddLabel (const grpc::string & label)
{
  labels_.push_back (label);
}

void
TrackCounters::AddToLabel (std::ostream & out, benchmark::State & state)
{
  ...
  for (int i = 0; i < GRPC_STATS_HISTOGRAM_COUNT; i++)
	...
	benchmark::
	Counter (grpc_stats_histo_percentile
		 (&stats, (grpc_stats_histograms) i, 50.0));
      std::ostringstream tail_ss;
      tail_ss << grpc_stats_histogram_name[i] << "-99p";
      state.counters[tail_ss.str ()] =
	benchmark::
	Counter (grpc_stats_histo_percentile
		 (&stats, (grpc_stats_histograms) i, 99.0));
    }
