...
static void BM_InitCancelTimer(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    GRPC_CLOSURE_INIT(
        &timer_closure->closure, [](void* /*args*/, grpc_error_handle /*err*/) {}, nullptr,
        grpc_schedule_on_exec_ctx);
    grpc_timer_init(&timer_closure->timer, GRPC_MILLIS_INF_FUTURE, &timer_closure->closure);
...
static void BM_TimerBatch(benchmark::State& state) {
  ...
  const grpc_millis start = reverse ? GRPC_MILLIS_INF_FUTURE : GRPC_MILLIS_INF_FUTURE - kTimerCount;
  const grpc_millis end = reverse ? GRPC_MILLIS_INF_FUTURE - kTimerCount : GRPC_MILLIS_INF_FUTURE;
