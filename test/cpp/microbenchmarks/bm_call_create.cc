...
void
BM_Zalloc (benchmark::State & state)
{
...
BENCHMARK (BM_Zalloc)->Arg (64)->Arg (128)->Arg (256)->Arg (512)->Arg (1024)->
Arg (1536)->Arg (2048)->Arg (3072)->Arg (4096)->Arg (5120)->Arg (6144)->
Arg (7168);
...
public:
  InsecureChannel ():
  BaseChannelFixture (grpc_insecure_channel_create
...
template < class Fixture > static void
BM_CallCreateDestroy (benchmark::State & state)
{
  TrackCounters
    track_counters;
  Fixture
  ...
  void *
    method_hdl = grpc_channel_register_call (fixture.channel (), "/foo/bar",
					     nullptr, nullptr);
  while (state.KeepRunning ())
    {
      grpc_call_unref (grpc_channel_create_registered_call
		       (fixture.channel (), nullptr, GRPC_PROPAGATE_DEFAULTS,
			cq, method_hdl, deadline, nullptr));
    }
...
static void
BM_LameChannelCallCreateCpp (benchmark::State & state)
{
  TrackCounters
    track_counters;
  auto
  ...
  while (state.KeepRunning ())
    {
      ...
      auto
	reader = stub->AsyncEcho (&cli_ctx, send_request, &cq);
...
static void
BM_LameChannelCallCreateCore (benchmark::State & state)
{
  TrackCounters
    track_counters;
  ...
  void *
    rc =
    grpc_channel_register_call (channel, "/grpc.testing.EchoTestService/Echo",
				nullptr, nullptr);
  while (state.KeepRunning ())
    {
      GPR_TIMER_SCOPE ("BenchmarkCycle", 0);
      grpc_call *
	call =
	grpc_channel_create_registered_call (channel, nullptr,
					     GRPC_PROPAGATE_DEFAULTS, cq, rc,
					     gpr_inf_future
					     (GPR_CLOCK_REALTIME), nullptr);
      grpc_metadata_array_init (&initial_metadata_recv);
      grpc_metadata_array_init (&trailing_metadata_recv);
      grpc_byte_buffer *
	request_payload_send =
...
static void
BM_LameChannelCallCreateCoreSeparateBatch (benchmark::State & state)
{
  TrackCounters
    track_counters;
...
static void
FilterDestroy (void *arg, grpc_error * error)
{
  gpr_free (arg);
}

static void
DoNothing (void *arg, grpc_error * error)
{
}
...
private:
  static void
  NoRef (grpc_client_channel_factory * factory)
  {
  }
  static void
  NoUnref (grpc_client_channel_factory * factory)
  {
  }
  static grpc_subchannel *
  CreateSubchannel (grpc_client_channel_factory * factory,
		    const grpc_subchannel_args * args)
  {
    return nullptr;
  }
  static grpc_channel *
  CreateClientChannel (grpc_client_channel_factory * factory,
		       const char *target,
...
const grpc_client_channel_factory_vtable
  FakeClientChannelFactory::vtable_ = {
  NoRef, NoUnref, CreateSubchannel, CreateClientChannel
};

static grpc_arg
StringArg (const char *key, const char *value)
{
  grpc_arg a;
...
template < const grpc_channel_filter *kFilter, uint32_t kFlags > struct Fixture
{
...
/* implementation of grpc_transport_get_endpoint */
  grpc_endpoint *GetEndpoint (grpc_transport * self)
  {
    return nullptr;
  }
...
public:
  ...
  public:
    ...
    void Finish ()
    {
      grpc_metadata_batch_destroy (&batch_);
    }
...
template < class Fixture, class TestOp > static void
BM_IsolatedFilter (benchmark::State & state)
{
  ...
  args.
    push_back (grpc_client_channel_factory_create_channel_arg
  ...
  size_t channel_size =
    grpc_channel_stack_size (filters.size () == 0 ? nullptr : &filters[0],
			     filters.size ());
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("channel_stack_init",
	       grpc_channel_stack_init (1, FilterDestroy, channel_stack,
					&filters[0], filters.size (),
					&channel_args,
					fixture.
					flags & REQUIRES_TRANSPORT ?
					&dummy_transport::
					dummy_transport : nullptr, "CHANNEL",
					channel_stack)));
  grpc_core::ExecCtx::Get ()->Flush ();
  grpc_call_stack *call_stack =
    static_cast <
    grpc_call_stack * >(gpr_zalloc (channel_stack->call_stack_size));
  ...
  while (state.KeepRunning ())
    {
      GPR_TIMER_SCOPE ("BenchmarkCycle", 0);
      GRPC_ERROR_UNREF (grpc_call_stack_init
			(channel_stack, 1, DoNothing, nullptr, &call_args));
...
BENCHMARK_TEMPLATE (BM_IsolatedFilter, ClientDeadlineFilter,
		    SendEmptyMetadata);
typedef
Fixture < &
...
BENCHMARK_TEMPLATE (BM_IsolatedFilter, ServerDeadlineFilter,
		    SendEmptyMetadata);
typedef
Fixture < &
  ...
  isolated_call_filter
{
  ...
  static void
  StartTransportStreamOp (grpc_call_element * elem,
			  grpc_transport_stream_op_batch * op)
  {
    call_data *
      calld = static_cast < call_data * >(elem->call_data);
  ...
  static void
  StartTransportOp (grpc_channel_element * elem, grpc_transport_op * op)
  {
  ...
  static grpc_error *
  InitCallElem (grpc_call_element * elem, const grpc_call_element_args * args)
  {
    call_data *
      calld = static_cast < call_data * >(elem->call_data);
  ...
  static void
  SetPollsetOrPollsetSet (grpc_call_element * elem,
			  grpc_polling_entity * pollent)
  {
  }

  static void
  DestroyCallElem (grpc_call_element * elem,
		   const grpc_call_final_info * final_info,
		   grpc_closure * then_sched_closure)
  {
  ...
  grpc_error *
  InitChannelElem (grpc_channel_element * elem,
		   grpc_channel_element_args * args)
  {
    return GRPC_ERROR_NONE;
  }
  ...
  void
  DestroyChannelElem (grpc_channel_element * elem)
  {
  }

  void
  GetChannelInfo (grpc_channel_element * elem,
		  const grpc_channel_info * channel_info)
  {
  }
...
public:
  IsolatedCallFixture ()
  {
    grpc_channel_stack_builder *
      builder = grpc_channel_stack_builder_create ();
    ...
    GPR_ASSERT (grpc_channel_stack_builder_append_filter
		(builder, &isolated_call_filter::isolated_call_filter,
		 nullptr, nullptr));
    {
      grpc_core::ExecCtx exec_ctx;
      channel_ =
	grpc_channel_create_with_builder (builder, GRPC_CLIENT_CHANNEL);
    }
    cq_ = grpc_completion_queue_create_for_next (nullptr);
  }

  void
  Finish (benchmark::State & state)
  {
...
static void
BM_IsolatedCall_NoOp (benchmark::State & state)
{
  IsolatedCallFixture
    fixture;
  gpr_timespec
...
static void
BM_IsolatedCall_Unary (benchmark::State & state)
{
  IsolatedCallFixture
    fixture;
  gpr_timespec
  ...
  grpc_byte_buffer *
    send_message = grpc_raw_byte_buffer_create (&slice, 1);
  grpc_byte_buffer *
  ...
  while (state.KeepRunning ())
    {
      ...
      grpc_call *
	call =
	grpc_channel_create_registered_call (fixture.channel (), nullptr,
...
static void
BM_IsolatedCall_StreamingSend (benchmark::State & state)
{
  IsolatedCallFixture
    fixture;
  gpr_timespec
