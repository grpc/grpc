...
public:
  DummyEndpoint ()
  {
    static const grpc_endpoint_vtable
      my_vtable = { read,
      write,
      add_to_pollset,
      add_to_pollset_set,
...
private:
  ...
  void
  QueueRead (grpc_slice_buffer * slices, grpc_closure * cb)
  {
  ...
  static void
  read (grpc_endpoint * ep, grpc_slice_buffer * slices, grpc_closure * cb)
  {
    static_cast < DummyEndpoint * >(ep)->QueueRead (slices, cb);
  }

  static void
  write (grpc_endpoint * ep, grpc_slice_buffer * slices, grpc_closure * cb)
  {
    GRPC_CLOSURE_SCHED (cb, GRPC_ERROR_NONE);
  }

  static grpc_workqueue *
  get_workqueue (grpc_endpoint * ep)
  {
    return nullptr;
  }

  static void
  add_to_pollset (grpc_endpoint * ep, grpc_pollset * pollset)
  {
  }

  static void
  add_to_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset)
  {
  }

  static void
  delete_from_pollset_set (grpc_endpoint * ep, grpc_pollset_set * pollset)
  {
  }

  static void
  shutdown (grpc_endpoint * ep, grpc_error * why)
  {
  ...
  get_resource_user (grpc_endpoint * ep)
  {
    return static_cast < DummyEndpoint * >(ep)->ru_;
  }
  static char *
  get_peer (grpc_endpoint * ep)
  {
    return gpr_strdup ("test");
  }
  static int
  get_fd (grpc_endpoint * ep)
  {
    return 0;
  }
};
...
template < class F >
  grpc_closure * MakeOnceClosure (F f, grpc_closure_scheduler * sched =
				  grpc_schedule_on_exec_ctx)
{
...
public:
  ...
  void Init (benchmark::State & state)
  {
    ...
    grpc_transport_init_stream (f_->transport (),
				static_cast < grpc_stream * >(stream_),
				&refcount_, nullptr, arena_);
  }
...
static void
BM_StreamCreateDestroy (benchmark::State & state)
{
...
template < class Metadata > static void
BM_StreamCreateSendInitialMetadataDestroy (benchmark::State & state)
{
  TrackCounters track_counters;
  grpc_core::ExecCtx exec_ctx;
  Fixture f (grpc::ChannelArguments (), true);
  ...
  for (size_t i = 0; i < elems.size (); i++)
    {
      GPR_ASSERT (GRPC_LOG_IF_ERROR
		  ("addmd",
		   grpc_metadata_batch_add_tail (&b, &storage[i], elems[i])));
    }
  ...
  start = MakeClosure ([&](grpc_error * error)
		       ...
		       send_initial_metadata = &b; s.Op (&op);}
  );
...
static void
BM_TransportEmptyOp (benchmark::State & state)
{
  TrackCounters track_counters;
  grpc_core::ExecCtx exec_ctx;
  Fixture f (grpc::ChannelArguments (), true);
...
static void
BM_TransportStreamSend (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  Fixture
  f (grpc::ChannelArguments (), true);
  auto
    s = std::unique_ptr < Stream > (new Stream (&f));
  s->Init (state);
...
#define SLICE_FROM_BUFFER(s) grpc_slice_from_static_buffer(s, sizeof(s) - 1)
     ...
     static grpc_slice
     CreateIncomingDataSlice (size_t length, size_t frame_size)
{
  std::queue < char >
    unframed;
...
static void
BM_TransportStreamRecv (benchmark::State & state)
{
  TrackCounters
    track_counters;
  grpc_core::ExecCtx exec_ctx;
  ...
  grpc_byte_stream *
    recv_stream;
  grpc_slice
    incoming_data = CreateIncomingDataSlice (state.range (0), 16384);
  ...
  std::unique_ptr < Closure > c = MakeClosure ([&](grpc_error * error)
					       ...
					       op.payload->recv_message.
					       recv_message_ready =
					       drain_start.get (); s.Op (&op);
					       f.
					       PushInput (grpc_slice_ref
							  (incoming_data));}
  );
  ...
  drain_start = MakeClosure ([&](grpc_error * error)
			     ...
			     {
			     GPR_ASSERT (!state.KeepRunning ()); return;}
  ...
  drain = MakeClosure ([&](grpc_error * error)
		       ...
		       while (grpc_byte_stream_next
			      (recv_stream, recv_stream->length - received,
			       drain_continue.get ())
			      && GRPC_ERROR_NONE ==
			      grpc_byte_stream_pull (recv_stream, &recv_slice)
			      && (received +=
				  GRPC_SLICE_LENGTH (recv_slice),
    ...
    drain_continue = MakeClosure ([&](grpc_error * error)
				  {
				  grpc_byte_stream_pull (recv_stream,
							 &recv_slice);
				  received += GRPC_SLICE_LENGTH (recv_slice);
				  grpc_slice_unref_internal (recv_slice);
				  GRPC_CLOSURE_RUN (drain.get (),
						    GRPC_ERROR_NONE);}
    );
    ...
    op.on_complete = c.get ();
    s.Op (&op);
    f.PushInput (SLICE_FROM_BUFFER ("\x00\x00\x00\x04\x00\x00\x00\x00\x00"
				    // Generated using:
