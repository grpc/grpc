...
typedef
  std::unique_ptr <
grpc_error, ErrorDeleter > ErrorPtr;

static void
BM_ErrorCreateFromStatic (benchmark::State & state)
{
...
static void
BM_ErrorCreateFromCopied (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
      GRPC_ERROR_UNREF (GRPC_ERROR_CREATE_FROM_COPIED_STRING
			("Error not inline"));
    }
  track_counters.Finish (state);
}
...
static void
BM_ErrorCreateAndSetStatus (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
...
static void
BM_ErrorCreateAndSetIntAndStr (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
      GRPC_ERROR_UNREF (grpc_error_set_str
			(grpc_error_set_int
			 (GRPC_ERROR_CREATE_FROM_STATIC_STRING
			  ...
			  (intptr_t) 0), GRPC_ERROR_STR_RAW_BYTES,
			 grpc_slice_from_static_string ("raw bytes")));
    }
  track_counters.Finish (state);
}
...
static void
BM_ErrorCreateAndSetIntLoop (benchmark::State & state)
{
  TrackCounters track_counters;
...
static void
BM_ErrorCreateAndSetStrLoop (benchmark::State & state)
{
  TrackCounters track_counters;
  grpc_error *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Error");
...
static void
BM_ErrorRefUnref (benchmark::State & state)
{
  TrackCounters track_counters;
  grpc_error *error = GRPC_ERROR_CREATE_FROM_STATIC_STRING ("Error");
  while (state.KeepRunning ())
...
static void
BM_ErrorUnrefNone (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
...
static void
BM_ErrorGetIntFromNoError (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
      intptr_t value;
      grpc_error_get_int (GRPC_ERROR_NONE, GRPC_ERROR_INT_GRPC_STATUS,
			  &value);
    }
  track_counters.Finish (state);
}
...
static void
BM_ErrorGetMissingInt (benchmark::State & state)
{
  TrackCounters track_counters;
  ErrorPtr
...
static void
BM_ErrorGetPresentInt (benchmark::State & state)
{
  TrackCounters track_counters;
  ErrorPtr
    error (grpc_error_set_int
...
private:
  ...
  ErrorPtr error_
  {
  GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING ("Error", nested_errors_,
						      1)};
};

template < class Fixture > static void
BM_ErrorStringOnNewError (benchmark::State & state)
{
  TrackCounters track_counters;
  while (state.KeepRunning ())
    {
...
template < class Fixture > static void
BM_ErrorStringRepeatedly (benchmark::State & state)
{
  TrackCounters track_counters;
  Fixture fixture;
  while (state.KeepRunning ())
...
template < class Fixture > static void
BM_ErrorGetStatus (benchmark::State & state)
{
  TrackCounters track_counters;
  Fixture fixture;
...
template < class Fixture > static void
BM_ErrorGetStatusCode (benchmark::State & state)
{
  TrackCounters track_counters;
  Fixture fixture;
  grpc_core::ExecCtx exec_ctx;
...
template < class Fixture > static void
BM_ErrorHttpError (benchmark::State & state)
{
  TrackCounters track_counters;
  Fixture fixture;
  grpc_core::ExecCtx exec_ctx;
  while (state.KeepRunning ())
    {
      ...
      grpc_error_get_status (fixture.error (), fixture.deadline (), nullptr,
...
template < class Fixture > static void
BM_HasClearGrpcStatus (benchmark::State & state)
{
  TrackCounters track_counters;
  Fixture fixture;
  while (state.KeepRunning ())
