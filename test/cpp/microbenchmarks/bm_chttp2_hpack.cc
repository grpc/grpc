  ...
  static void Prepare(grpc_metadata_batch* b) {
    ...
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
        "addmd", b->Append(grpc_mdelem_from_slices(
                     ...
                     grpc_slice_intern(grpc_slice_from_static_string(
                         "foo.test.google.fr:1234"))))));
    b->Set(grpc_core::GrpcTraceBinMetadata(), grpc_core::Slice(grpc_core::StaticSlice::FromStaticString(
            "\x00\x01\x02\x03\x04\x05\x06\x07\x08"
                                          ...
                                          "\x30")));
    b->Set(grpc_core::GrpcTagsBinMetadata(), grpc_core::Slice(grpc_core::StaticSlice::FromStaticString(
            "\x00\x01\x02\x03\x04\x05\x06\x07\x08"
                                          "\x09\x0a\x0b\x0c\x0d\x0e\x0f"
                                          "\x10\x11\x12\x13")));
    GPR_ASSERT(GRPC_LOG_IF_ERROR(
