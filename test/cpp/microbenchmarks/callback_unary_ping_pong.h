...
inline void SendCallbackUnaryPingPong(benchmark::State* state, ClientContext* cli_ctx,
                                      EchoRequest* request, EchoResponse* response,
                                      EchoTestService::Stub* stub_, bool* done, std::mutex* mu,
  ...
  stub_->async()->Echo(cli_ctx, request, response,
                       [state, cli_ctx, request, response, stub_, done, mu, cv](Status s) {
                         ...
                         if (state->KeepRunning()) {
                           ...
                           SendCallbackUnaryPingPong(state, cli_ctx, request, response, stub_, done,
                                                     mu, cv);
                         } else {
...
static void BM_CallbackUnaryPingPong(benchmark::State& state) {
  ...
  if (state.KeepRunning()) {
    ...
    SendCallbackUnaryPingPong(&state, &cli_ctx, &request, &response, stub_.get(), &done, &mu, &cv);
  }
