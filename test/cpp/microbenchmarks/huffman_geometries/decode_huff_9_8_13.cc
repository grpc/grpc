// Copyright 2023 gRPC authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// This file is autogenerated: see
// tools/codegen/core/gen_huffman_decompressor.cc

#include <grpc/support/port_platform.h>

#include "test/cpp/microbenchmarks/huffman_geometries/decode_huff_9_8_13.h"
namespace grpc_core {
namespace geometry_9_8_13 {
const uint8_t HuffDecoderCommon::table2_0_emit_[10] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74};
const uint8_t HuffDecoderCommon::table2_0_ops_[32] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x26, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table3_0_emit_[36] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74, 0x20, 0x25,
    0x2d, 0x2e, 0x2f, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3d, 0x41,
    0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e, 0x70, 0x72, 0x75};
const uint8_t HuffDecoderCommon::table3_0_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x2a, 0x2e,
    0x32, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a, 0x4e, 0x52, 0x56, 0x5a,
    0x5e, 0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76, 0x7a, 0x7e, 0x82, 0x86,
    0x8a, 0x8e, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table4_0_emit_[22] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74, 0x20,
    0x25, 0x2d, 0x2e, 0x2f, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};
const uint8_t HuffDecoderCommon::table4_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x06, 0x01, 0x01, 0x01,
    0x0a, 0x01, 0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01,
    0x01, 0x16, 0x01, 0x01, 0x01, 0x1a, 0x01, 0x01, 0x01, 0x1e, 0x01,
    0x01, 0x01, 0x22, 0x01, 0x01, 0x01, 0x26, 0x01, 0x2a, 0x01, 0x2e,
    0x01, 0x32, 0x01, 0x36, 0x01, 0x3a, 0x01, 0x3e, 0x01, 0x42, 0x01,
    0x46, 0x01, 0x4a, 0x01, 0x4e, 0x01, 0x52, 0x01, 0x56};
const uint8_t HuffDecoderCommon::table4_1_emit_[46] = {
    0x3d, 0x41, 0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e, 0x70,
    0x72, 0x75, 0x3a, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a,
    0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56,
    0x57, 0x59, 0x6a, 0x6b, 0x71, 0x76, 0x77, 0x78, 0x79, 0x7a};
const uint8_t HuffDecoderCommon::table4_1_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x01, 0x2a,
    0x01, 0x2e, 0x01, 0x32, 0x01, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a,
    0x4e, 0x52, 0x56, 0x5a, 0x5e, 0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76,
    0x7a, 0x7e, 0x82, 0x86, 0x8a, 0x8e, 0x92, 0x96, 0x9a, 0x9e, 0xa2,
    0xa6, 0xaa, 0xae, 0xb2, 0xb6, 0x01, 0x01, 0x01, 0x00};
const uint8_t* const HuffDecoderCommon::table4_emit_[2] = {
    table4_0_emit_,
    table4_1_emit_,
};
const uint8_t* const HuffDecoderCommon::table4_ops_[2] = {
    table4_0_ops_,
    table4_1_ops_,
};
const uint8_t HuffDecoderCommon::table5_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table5_1_emit_[52] = {
    0x3d, 0x41, 0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e,
    0x70, 0x72, 0x75, 0x3a, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48,
    0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53,
    0x54, 0x55, 0x56, 0x57, 0x59, 0x6a, 0x6b, 0x71, 0x76, 0x77, 0x78,
    0x79, 0x7a, 0x26, 0x2a, 0x2c, 0x3b, 0x58, 0x5a};
const uint8_t HuffDecoderCommon::table5_1_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table5_emit_[2] = {
    table4_0_emit_,
    table5_1_emit_,
};
const uint8_t* const HuffDecoderCommon::table5_ops_[2] = {
    table5_0_ops_,
    table5_1_ops_,
};
const uint8_t HuffDecoderCommon::table1_0_emit_[2] = {0x30, 0x31};
const uint16_t HuffDecoderCommon::table1_0_ops_[32] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085,
    0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085};
const uint8_t HuffDecoderCommon::table1_1_emit_[2] = {0x32, 0x61};
const uint8_t HuffDecoderCommon::table1_2_emit_[2] = {0x63, 0x65};
const uint8_t HuffDecoderCommon::table1_3_emit_[2] = {0x69, 0x6f};
const uint8_t HuffDecoderCommon::table1_4_emit_[2] = {0x73, 0x74};
const uint8_t HuffDecoderCommon::table1_5_emit_[4] = {0x20, 0x25, 0x2d, 0x2e};
const uint16_t HuffDecoderCommon::table1_5_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086,
    0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106,
    0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186};
const uint8_t HuffDecoderCommon::table1_6_emit_[4] = {0x2f, 0x33, 0x34, 0x35};
const uint8_t HuffDecoderCommon::table1_7_emit_[4] = {0x36, 0x37, 0x38, 0x39};
const uint8_t HuffDecoderCommon::table1_8_emit_[4] = {0x3d, 0x41, 0x5f, 0x62};
const uint8_t HuffDecoderCommon::table1_9_emit_[4] = {0x64, 0x66, 0x67, 0x68};
const uint8_t HuffDecoderCommon::table1_10_emit_[4] = {0x6c, 0x6d, 0x6e, 0x70};
const uint8_t HuffDecoderCommon::table1_11_emit_[6] = {0x72, 0x75, 0x3a,
                                                       0x42, 0x43, 0x44};
const uint16_t HuffDecoderCommon::table1_11_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0207, 0x0207, 0x0207, 0x0207, 0x0287, 0x0287, 0x0287, 0x0287};
const uint8_t HuffDecoderCommon::table1_12_emit_[8] = {0x45, 0x46, 0x47, 0x48,
                                                       0x49, 0x4a, 0x4b, 0x4c};
const uint16_t HuffDecoderCommon::table1_12_ops_[32] = {
    0x0007, 0x0007, 0x0007, 0x0007, 0x0087, 0x0087, 0x0087, 0x0087,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0207, 0x0207, 0x0207, 0x0207, 0x0287, 0x0287, 0x0287, 0x0287,
    0x0307, 0x0307, 0x0307, 0x0307, 0x0387, 0x0387, 0x0387, 0x0387};
const uint8_t HuffDecoderCommon::table1_13_emit_[8] = {0x4d, 0x4e, 0x4f, 0x50,
                                                       0x51, 0x52, 0x53, 0x54};
const uint8_t HuffDecoderCommon::table1_14_emit_[8] = {0x55, 0x56, 0x57, 0x59,
                                                       0x6a, 0x6b, 0x71, 0x76};
const uint8_t HuffDecoderCommon::table1_15_emit_[10] = {
    0x77, 0x78, 0x79, 0x7a, 0x26, 0x2a, 0x2c, 0x3b, 0x58, 0x5a};
const uint16_t HuffDecoderCommon::table1_15_ops_[32] = {
    0x0007, 0x0007, 0x0007, 0x0007, 0x0087, 0x0087, 0x0087, 0x0087,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0208, 0x0208, 0x0288, 0x0288, 0x0308, 0x0308, 0x0388, 0x0388,
    0x0408, 0x0408, 0x0488, 0x0488, 0x0019, 0x0029, 0x0039, 0x0049};
const uint8_t* const HuffDecoderCommon::table1_emit_[16] = {
    table1_0_emit_,  table1_1_emit_,  table1_2_emit_,  table1_3_emit_,
    table1_4_emit_,  table1_5_emit_,  table1_6_emit_,  table1_7_emit_,
    table1_8_emit_,  table1_9_emit_,  table1_10_emit_, table1_11_emit_,
    table1_12_emit_, table1_13_emit_, table1_14_emit_, table1_15_emit_,
};
const uint16_t* const HuffDecoderCommon::table1_ops_[16] = {
    table1_0_ops_,  table1_0_ops_,  table1_0_ops_,  table1_0_ops_,
    table1_0_ops_,  table1_5_ops_,  table1_5_ops_,  table1_5_ops_,
    table1_5_ops_,  table1_5_ops_,  table1_5_ops_,  table1_11_ops_,
    table1_12_ops_, table1_12_ops_, table1_12_ops_, table1_15_ops_,
};
const uint8_t HuffDecoderCommon::table12_0_inner_[5] = {0x01, 0x02, 0x06, 0x0a,
                                                        0x00};
const uint8_t HuffDecoderCommon::table12_0_outer_[8] = {0, 1, 2, 3, 0, 0, 0, 4};
const uint8_t HuffDecoderCommon::table13_0_emit_[9] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b, 0x5d, 0x7e};
const uint8_t HuffDecoderCommon::table13_0_inner_[11] = {
    0x01, 0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x00};
const uint8_t HuffDecoderCommon::table13_0_outer_[16] = {
    0, 0, 0, 1, 0, 2, 0, 3, 4, 5, 6, 7, 8, 9, 0, 10};
const uint8_t HuffDecoderCommon::table14_0_emit_[11] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b, 0x5d, 0x7e, 0x5e, 0x7d};
const uint8_t HuffDecoderCommon::table14_0_ops_[32] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01,
    0x06, 0x01, 0x01, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01, 0x16,
    0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x26, 0x2a, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table15_0_emit_[14] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b,
    0x5d, 0x7e, 0x5e, 0x7d, 0x3c, 0x60, 0x7b};
const uint8_t HuffDecoderCommon::table15_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x06, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0a, 0x01,
    0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01, 0x01, 0x16,
    0x01, 0x01, 0x01, 0x1a, 0x01, 0x01, 0x01, 0x1e, 0x01, 0x01, 0x01,
    0x22, 0x01, 0x26, 0x01, 0x2a, 0x2e, 0x32, 0x36, 0x00};
const uint8_t HuffDecoderCommon::table16_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table10_0_emit_[1] = {0x7c};
const uint16_t HuffDecoderCommon::table10_0_ops_[32] = {
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002};
const uint8_t HuffDecoderCommon::table10_2_emit_[1] = {0x23};
const uint16_t HuffDecoderCommon::table10_2_ops_[32] = {
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003};
const uint8_t HuffDecoderCommon::table10_3_emit_[1] = {0x3e};
const uint8_t HuffDecoderCommon::table10_4_emit_[2] = {0x00, 0x24};
const uint16_t HuffDecoderCommon::table10_4_ops_[32] = {
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084,
    0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084, 0x0084};
const uint8_t HuffDecoderCommon::table10_5_emit_[2] = {0x40, 0x5b};
const uint8_t HuffDecoderCommon::table10_6_emit_[2] = {0x5d, 0x7e};
const uint8_t HuffDecoderCommon::table10_7_emit_[5] = {0x5e, 0x7d, 0x3c, 0x60,
                                                       0x7b};
const uint16_t HuffDecoderCommon::table10_7_ops_[32] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085,
    0x0106, 0x0106, 0x0106, 0x0106, 0x0186, 0x0186, 0x0186, 0x0186,
    0x0206, 0x0206, 0x0206, 0x0206, 0x0018, 0x0028, 0x0038, 0x0048};
const uint8_t* const HuffDecoderCommon::table10_emit_[8] = {
    table10_0_emit_, table10_0_emit_, table10_2_emit_, table10_3_emit_,
    table10_4_emit_, table10_5_emit_, table10_6_emit_, table10_7_emit_,
};
const uint16_t* const HuffDecoderCommon::table10_ops_[8] = {
    table10_0_ops_, table10_0_ops_, table10_2_ops_, table10_2_ops_,
    table10_4_ops_, table10_4_ops_, table10_4_ops_, table10_7_ops_,
};
const uint8_t HuffDecoderCommon::table17_0_emit_[5] = {0x5c, 0xc3, 0xd0, 0x80,
                                                       0x82};
const uint8_t HuffDecoderCommon::table17_0_inner_[5] = {0x02, 0x06, 0x0a, 0x0f,
                                                        0x13};
const uint8_t HuffDecoderCommon::table20_0_emit_[6] = {0x83, 0xa2, 0xb8,
                                                       0xc2, 0xe0, 0xe2};
const uint8_t HuffDecoderCommon::table20_0_inner_[7] = {0x02, 0x06, 0x0a, 0x0e,
                                                        0x12, 0x16, 0x01};
const uint8_t HuffDecoderCommon::table19_0_emit_[10] = {
    0x83, 0xa2, 0xb8, 0xc2, 0xe0, 0xe2, 0x99, 0xa1, 0xa7, 0xac};
const uint8_t HuffDecoderCommon::table19_0_inner_[10] = {
    0x03, 0x0b, 0x13, 0x1b, 0x23, 0x2b, 0x34, 0x3c, 0x44, 0x4c};
const uint8_t HuffDecoderCommon::table22_0_emit_[9] = {
    0xb0, 0xb1, 0xb3, 0xd1, 0xd8, 0xd9, 0xe3, 0xe5, 0xe6};
const uint8_t HuffDecoderCommon::table22_0_inner_[10] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x01};
const uint8_t HuffDecoderCommon::table21_0_emit_[23] = {
    0xb0, 0xb1, 0xb3, 0xd1, 0xd8, 0xd9, 0xe3, 0xe5, 0xe6, 0x81, 0x84, 0x85,
    0x86, 0x88, 0x92, 0x9a, 0x9c, 0xa0, 0xa3, 0xa4, 0xa9, 0xaa, 0xad};
const uint8_t HuffDecoderCommon::table21_0_ops_[32] = {
    0x04, 0x04, 0x0c, 0x0c, 0x14, 0x14, 0x1c, 0x1c, 0x24, 0x24, 0x2c,
    0x2c, 0x34, 0x34, 0x3c, 0x3c, 0x44, 0x44, 0x4d, 0x55, 0x5d, 0x65,
    0x6d, 0x75, 0x7d, 0x85, 0x8d, 0x95, 0x9d, 0xa5, 0xad, 0xb5};
const uint8_t HuffDecoderCommon::table24_0_emit_[12] = {
    0xb2, 0xb5, 0xb9, 0xba, 0xbb, 0xbd, 0xbe, 0xc4, 0xc6, 0xe4, 0xe8, 0xe9};
const uint8_t HuffDecoderCommon::table24_0_ops_[32] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x26, 0x2a,
    0x2e, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table25_0_emit_[41] = {
    0xb2, 0xb5, 0xb9, 0xba, 0xbb, 0xbd, 0xbe, 0xc4, 0xc6, 0xe4, 0xe8,
    0xe9, 0x01, 0x87, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8f, 0x93, 0x95,
    0x96, 0x97, 0x98, 0x9b, 0x9d, 0x9e, 0xa5, 0xa6, 0xa8, 0xae, 0xaf,
    0xb4, 0xb6, 0xb7, 0xbc, 0xbf, 0xc5, 0xe7, 0xef};
const uint8_t HuffDecoderCommon::table25_0_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x01, 0x2a,
    0x01, 0x2e, 0x32, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a, 0x4e, 0x52,
    0x56, 0x5a, 0x5e, 0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76, 0x7a, 0x7e,
    0x82, 0x86, 0x8a, 0x8e, 0x92, 0x96, 0x9a, 0x9e, 0xa2, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table26_0_emit_[53] = {
    0xb2, 0xb5, 0xb9, 0xba, 0xbb, 0xbd, 0xbe, 0xc4, 0xc6, 0xe4, 0xe8,
    0xe9, 0x01, 0x87, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8f, 0x93, 0x95,
    0x96, 0x97, 0x98, 0x9b, 0x9d, 0x9e, 0xa5, 0xa6, 0xa8, 0xae, 0xaf,
    0xb4, 0xb6, 0xb7, 0xbc, 0xbf, 0xc5, 0xe7, 0xef, 0x09, 0x8e, 0x90,
    0x91, 0x94, 0x9f, 0xab, 0xce, 0xd7, 0xe1, 0xec, 0xed};
const uint8_t HuffDecoderCommon::table26_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table27_0_emit_[57] = {
    0xb2, 0xb5, 0xb9, 0xba, 0xbb, 0xbd, 0xbe, 0xc4, 0xc6, 0xe4, 0xe8, 0xe9,
    0x01, 0x87, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8f, 0x93, 0x95, 0x96, 0x97,
    0x98, 0x9b, 0x9d, 0x9e, 0xa5, 0xa6, 0xa8, 0xae, 0xaf, 0xb4, 0xb6, 0xb7,
    0xbc, 0xbf, 0xc5, 0xe7, 0xef, 0x09, 0x8e, 0x90, 0x91, 0x94, 0x9f, 0xab,
    0xce, 0xd7, 0xe1, 0xec, 0xed, 0xc7, 0xcf, 0xea, 0xeb};
const uint8_t HuffDecoderCommon::table27_0_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table28_0_emit_[4] = {0xb2, 0xb5, 0xb9, 0xba};
const uint8_t HuffDecoderCommon::table28_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x06, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x0a, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0e};
const uint8_t HuffDecoderCommon::table28_1_emit_[4] = {0xbb, 0xbd, 0xbe, 0xc4};
const uint8_t HuffDecoderCommon::table28_2_emit_[4] = {0xc6, 0xe4, 0xe8, 0xe9};
const uint8_t HuffDecoderCommon::table28_3_emit_[8] = {0x01, 0x87, 0x89, 0x8a,
                                                       0x8b, 0x8c, 0x8d, 0x8f};
const uint8_t HuffDecoderCommon::table28_3_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x06, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x0a, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0e, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x16, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x1a, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x1e};
const uint8_t HuffDecoderCommon::table28_4_emit_[8] = {0x93, 0x95, 0x96, 0x97,
                                                       0x98, 0x9b, 0x9d, 0x9e};
const uint8_t HuffDecoderCommon::table28_5_emit_[8] = {0xa5, 0xa6, 0xa8, 0xae,
                                                       0xaf, 0xb4, 0xb6, 0xb7};
const uint8_t HuffDecoderCommon::table28_6_emit_[11] = {
    0xbc, 0xbf, 0xc5, 0xe7, 0xef, 0x09, 0x8e, 0x90, 0x91, 0x94, 0x9f};
const uint8_t HuffDecoderCommon::table28_6_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x06, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x0a, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0e, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01, 0x01, 0x16,
    0x01, 0x01, 0x01, 0x1a, 0x01, 0x01, 0x01, 0x1e, 0x01, 0x01, 0x01,
    0x22, 0x01, 0x01, 0x01, 0x26, 0x01, 0x01, 0x01, 0x2a};
const uint8_t HuffDecoderCommon::table28_7_emit_[25] = {
    0xab, 0xce, 0xd7, 0xe1, 0xec, 0xed, 0xc7, 0xcf, 0xea,
    0xeb, 0xc0, 0xc1, 0xc8, 0xc9, 0xca, 0xcd, 0xd2, 0xd5,
    0xda, 0xdb, 0xee, 0xf0, 0xf2, 0xf3, 0xff};
const uint8_t HuffDecoderCommon::table28_7_ops_[64] = {
    0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x06, 0x01, 0x01, 0x01,
    0x0a, 0x01, 0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01,
    0x01, 0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x2a,
    0x2e, 0x32, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a, 0x4e, 0x52, 0x56,
    0x5a, 0x5e, 0x62, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t* const HuffDecoderCommon::table28_emit_[8] = {
    table28_0_emit_, table28_1_emit_, table28_2_emit_, table28_3_emit_,
    table28_4_emit_, table28_5_emit_, table28_6_emit_, table28_7_emit_,
};
const uint8_t* const HuffDecoderCommon::table28_ops_[8] = {
    table28_0_ops_, table28_0_ops_, table28_0_ops_, table28_3_ops_,
    table28_3_ops_, table28_3_ops_, table28_6_ops_, table28_7_ops_,
};
const uint8_t HuffDecoderCommon::table29_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table29_3_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table29_6_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table29_7_emit_[44] = {
    0xab, 0xce, 0xd7, 0xe1, 0xec, 0xed, 0xc7, 0xcf, 0xea, 0xeb, 0xc0,
    0xc1, 0xc8, 0xc9, 0xca, 0xcd, 0xd2, 0xd5, 0xda, 0xdb, 0xee, 0xf0,
    0xf2, 0xf3, 0xff, 0xcb, 0xcc, 0xd3, 0xd4, 0xd6, 0xdd, 0xde, 0xdf,
    0xf1, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe};
const uint8_t HuffDecoderCommon::table29_7_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table29_emit_[8] = {
    table28_0_emit_, table28_1_emit_, table28_2_emit_, table28_3_emit_,
    table28_4_emit_, table28_5_emit_, table28_6_emit_, table29_7_emit_,
};
const uint8_t* const HuffDecoderCommon::table29_ops_[8] = {
    table29_0_ops_, table29_0_ops_, table29_0_ops_, table29_3_ops_,
    table29_3_ops_, table29_3_ops_, table29_6_ops_, table29_7_ops_,
};
const uint8_t HuffDecoderCommon::table30_0_emit_[2] = {0xb2, 0xb5};
const uint8_t HuffDecoderCommon::table30_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table30_1_emit_[2] = {0xb9, 0xba};
const uint8_t HuffDecoderCommon::table30_2_emit_[2] = {0xbb, 0xbd};
const uint8_t HuffDecoderCommon::table30_3_emit_[2] = {0xbe, 0xc4};
const uint8_t HuffDecoderCommon::table30_4_emit_[2] = {0xc6, 0xe4};
const uint8_t HuffDecoderCommon::table30_5_emit_[2] = {0xe8, 0xe9};
const uint8_t HuffDecoderCommon::table30_6_emit_[4] = {0x01, 0x87, 0x89, 0x8a};
const uint8_t HuffDecoderCommon::table30_7_emit_[4] = {0x8b, 0x8c, 0x8d, 0x8f};
const uint8_t HuffDecoderCommon::table30_8_emit_[4] = {0x93, 0x95, 0x96, 0x97};
const uint8_t HuffDecoderCommon::table30_9_emit_[4] = {0x98, 0x9b, 0x9d, 0x9e};
const uint8_t HuffDecoderCommon::table30_10_emit_[4] = {0xa5, 0xa6, 0xa8, 0xae};
const uint8_t HuffDecoderCommon::table30_11_emit_[4] = {0xaf, 0xb4, 0xb6, 0xb7};
const uint8_t HuffDecoderCommon::table30_12_emit_[4] = {0xbc, 0xbf, 0xc5, 0xe7};
const uint8_t HuffDecoderCommon::table30_13_emit_[7] = {0xef, 0x09, 0x8e, 0x90,
                                                        0x91, 0x94, 0x9f};
const uint8_t HuffDecoderCommon::table30_13_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table30_14_emit_[10] = {
    0xab, 0xce, 0xd7, 0xe1, 0xec, 0xed, 0xc7, 0xcf, 0xea, 0xeb};
const uint8_t HuffDecoderCommon::table30_14_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table30_15_emit_[63] = {
    0xc0, 0xc1, 0xc8, 0xc9, 0xca, 0xcd, 0xd2, 0xd5, 0xda, 0xdb, 0xee,
    0xf0, 0xf2, 0xf3, 0xff, 0xcb, 0xcc, 0xd3, 0xd4, 0xd6, 0xdd, 0xde,
    0xdf, 0xf1, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xfa, 0xfb, 0xfc, 0xfd,
    0xfe, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0b, 0x0c, 0x0e,
    0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x17, 0x18, 0x19, 0x1a,
    0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0xdc, 0xf9};
const uint8_t HuffDecoderCommon::table30_15_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table30_emit_[16] = {
    table30_0_emit_,  table30_1_emit_,  table30_2_emit_,  table30_3_emit_,
    table30_4_emit_,  table30_5_emit_,  table30_6_emit_,  table30_7_emit_,
    table30_8_emit_,  table30_9_emit_,  table30_10_emit_, table30_11_emit_,
    table30_12_emit_, table30_13_emit_, table30_14_emit_, table30_15_emit_,
};
const uint8_t* const HuffDecoderCommon::table30_ops_[16] = {
    table30_0_ops_, table30_0_ops_,  table30_0_ops_,  table30_0_ops_,
    table30_0_ops_, table30_0_ops_,  table29_0_ops_,  table29_0_ops_,
    table29_0_ops_, table29_0_ops_,  table29_0_ops_,  table29_0_ops_,
    table29_0_ops_, table30_13_ops_, table30_14_ops_, table30_15_ops_,
};
const uint8_t HuffDecoderCommon::table31_0_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table31_6_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table31_13_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table31_14_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table31_15_ops_[256] = {
    };
const uint8_t* const HuffDecoderCommon::table31_emit_[16] = {
    table30_0_emit_,  table30_1_emit_,  table30_2_emit_,  table30_3_emit_,
    table30_4_emit_,  table30_5_emit_,  table30_6_emit_,  table30_7_emit_,
    table30_8_emit_,  table30_9_emit_,  table30_10_emit_, table30_11_emit_,
    table30_12_emit_, table30_13_emit_, table30_14_emit_, table30_15_emit_,
};
const uint8_t* const HuffDecoderCommon::table31_ops_[16] = {
    table31_0_ops_, table31_0_ops_,  table31_0_ops_,  table31_0_ops_,
    table31_0_ops_, table31_0_ops_,  table31_6_ops_,  table31_6_ops_,
    table31_6_ops_, table31_6_ops_,  table31_6_ops_,  table31_6_ops_,
    table31_6_ops_, table31_13_ops_, table31_14_ops_, table31_15_ops_,
};
const uint8_t HuffDecoderCommon::table23_0_emit_[1] = {0xb2};
const uint16_t HuffDecoderCommon::table23_0_ops_[128] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005};
const uint8_t HuffDecoderCommon::table23_2_emit_[1] = {0xb5};
const uint8_t HuffDecoderCommon::table23_4_emit_[1] = {0xb9};
const uint8_t HuffDecoderCommon::table23_6_emit_[1] = {0xba};
const uint8_t HuffDecoderCommon::table23_8_emit_[1] = {0xbb};
const uint8_t HuffDecoderCommon::table23_10_emit_[1] = {0xbd};
const uint8_t HuffDecoderCommon::table23_12_emit_[1] = {0xbe};
const uint8_t HuffDecoderCommon::table23_14_emit_[1] = {0xc4};
const uint8_t HuffDecoderCommon::table23_16_emit_[1] = {0xc6};
const uint8_t HuffDecoderCommon::table23_18_emit_[1] = {0xe4};
const uint8_t HuffDecoderCommon::table23_20_emit_[1] = {0xe8};
const uint8_t HuffDecoderCommon::table23_22_emit_[1] = {0xe9};
const uint8_t HuffDecoderCommon::table23_24_emit_[1] = {0x01};
const uint16_t HuffDecoderCommon::table23_24_ops_[128] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006};
const uint8_t HuffDecoderCommon::table23_25_emit_[1] = {0x87};
const uint8_t HuffDecoderCommon::table23_26_emit_[1] = {0x89};
const uint8_t HuffDecoderCommon::table23_27_emit_[1] = {0x8a};
const uint8_t HuffDecoderCommon::table23_28_emit_[1] = {0x8b};
const uint8_t HuffDecoderCommon::table23_29_emit_[1] = {0x8c};
const uint8_t HuffDecoderCommon::table23_30_emit_[1] = {0x8d};
const uint8_t HuffDecoderCommon::table23_31_emit_[1] = {0x8f};
const uint8_t HuffDecoderCommon::table23_32_emit_[1] = {0x93};
const uint8_t HuffDecoderCommon::table23_33_emit_[1] = {0x95};
const uint8_t HuffDecoderCommon::table23_34_emit_[1] = {0x96};
const uint8_t HuffDecoderCommon::table23_35_emit_[1] = {0x97};
const uint8_t HuffDecoderCommon::table23_36_emit_[1] = {0x98};
const uint8_t HuffDecoderCommon::table23_37_emit_[1] = {0x9b};
const uint8_t HuffDecoderCommon::table23_38_emit_[1] = {0x9d};
const uint8_t HuffDecoderCommon::table23_39_emit_[1] = {0x9e};
const uint8_t HuffDecoderCommon::table23_40_emit_[1] = {0xa5};
const uint8_t HuffDecoderCommon::table23_41_emit_[1] = {0xa6};
const uint8_t HuffDecoderCommon::table23_42_emit_[1] = {0xa8};
const uint8_t HuffDecoderCommon::table23_43_emit_[1] = {0xae};
const uint8_t HuffDecoderCommon::table23_44_emit_[1] = {0xaf};
const uint8_t HuffDecoderCommon::table23_45_emit_[1] = {0xb4};
const uint8_t HuffDecoderCommon::table23_46_emit_[1] = {0xb6};
const uint8_t HuffDecoderCommon::table23_47_emit_[1] = {0xb7};
const uint8_t HuffDecoderCommon::table23_48_emit_[1] = {0xbc};
const uint8_t HuffDecoderCommon::table23_49_emit_[1] = {0xbf};
const uint8_t HuffDecoderCommon::table23_50_emit_[1] = {0xc5};
const uint8_t HuffDecoderCommon::table23_51_emit_[1] = {0xe7};
const uint8_t HuffDecoderCommon::table23_52_emit_[1] = {0xef};
const uint8_t HuffDecoderCommon::table23_53_emit_[2] = {0x09, 0x8e};
const uint16_t HuffDecoderCommon::table23_53_ops_[128] = {
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007, 0x0007,
    0x0007, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027, 0x0027,
    0x0027, 0x0027};
const uint8_t HuffDecoderCommon::table23_54_emit_[2] = {0x90, 0x91};
const uint8_t HuffDecoderCommon::table23_55_emit_[2] = {0x94, 0x9f};
const uint8_t HuffDecoderCommon::table23_56_emit_[2] = {0xab, 0xce};
const uint8_t HuffDecoderCommon::table23_57_emit_[2] = {0xd7, 0xe1};
const uint8_t HuffDecoderCommon::table23_58_emit_[2] = {0xec, 0xed};
const uint8_t HuffDecoderCommon::table23_59_emit_[4] = {0xc7, 0xcf, 0xea, 0xeb};
const uint16_t HuffDecoderCommon::table23_59_ops_[128] = {
    0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008,
    0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008,
    0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0008,
    0x0008, 0x0008, 0x0008, 0x0008, 0x0008, 0x0028, 0x0028, 0x0028, 0x0028,
    0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028,
    0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028,
    0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028, 0x0028,
    0x0028, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048,
    0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048,
    0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048,
    0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0048, 0x0068, 0x0068, 0x0068,
    0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068,
    0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068,
    0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068, 0x0068,
    0x0068, 0x0068};
const uint8_t HuffDecoderCommon::table23_60_emit_[8] = {0xc0, 0xc1, 0xc8, 0xc9,
                                                        0xca, 0xcd, 0xd2, 0xd5};
const uint16_t HuffDecoderCommon::table23_60_ops_[128] = {
    0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009,
    0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0029, 0x0029,
    0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029,
    0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0049, 0x0049, 0x0049, 0x0049,
    0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049,
    0x0049, 0x0049, 0x0049, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069,
    0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069,
    0x0069, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089,
    0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x00a9,
    0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9,
    0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00c9, 0x00c9, 0x00c9,
    0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9,
    0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9,
    0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9, 0x00e9,
    0x00e9, 0x00e9};
const uint8_t HuffDecoderCommon::table23_61_emit_[9] = {
    0xda, 0xdb, 0xee, 0xf0, 0xf2, 0xf3, 0xff, 0xcb, 0xcc};
const uint16_t HuffDecoderCommon::table23_61_ops_[128] = {
    0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009,
    0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0009, 0x0029, 0x0029,
    0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0029,
    0x0029, 0x0029, 0x0029, 0x0029, 0x0029, 0x0049, 0x0049, 0x0049, 0x0049,
    0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049, 0x0049,
    0x0049, 0x0049, 0x0049, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069,
    0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069, 0x0069,
    0x0069, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089,
    0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x0089, 0x00a9,
    0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9,
    0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00a9, 0x00c9, 0x00c9, 0x00c9,
    0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00c9,
    0x00c9, 0x00c9, 0x00c9, 0x00c9, 0x00ea, 0x00ea, 0x00ea, 0x00ea, 0x00ea,
    0x00ea, 0x00ea, 0x00ea, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a,
    0x010a, 0x010a};
const uint8_t HuffDecoderCommon::table23_62_emit_[16] = {
    0xd3, 0xd4, 0xd6, 0xdd, 0xde, 0xdf, 0xf1, 0xf4,
    0xf5, 0xf6, 0xf7, 0xf8, 0xfa, 0xfb, 0xfc, 0xfd};
const uint16_t HuffDecoderCommon::table23_62_ops_[128] = {
    0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x002a,
    0x002a, 0x002a, 0x002a, 0x002a, 0x002a, 0x002a, 0x002a, 0x004a, 0x004a,
    0x004a, 0x004a, 0x004a, 0x004a, 0x004a, 0x004a, 0x006a, 0x006a, 0x006a,
    0x006a, 0x006a, 0x006a, 0x006a, 0x006a, 0x008a, 0x008a, 0x008a, 0x008a,
    0x008a, 0x008a, 0x008a, 0x008a, 0x00aa, 0x00aa, 0x00aa, 0x00aa, 0x00aa,
    0x00aa, 0x00aa, 0x00aa, 0x00ca, 0x00ca, 0x00ca, 0x00ca, 0x00ca, 0x00ca,
    0x00ca, 0x00ca, 0x00ea, 0x00ea, 0x00ea, 0x00ea, 0x00ea, 0x00ea, 0x00ea,
    0x00ea, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a, 0x010a,
    0x012a, 0x012a, 0x012a, 0x012a, 0x012a, 0x012a, 0x012a, 0x012a, 0x014a,
    0x014a, 0x014a, 0x014a, 0x014a, 0x014a, 0x014a, 0x014a, 0x016a, 0x016a,
    0x016a, 0x016a, 0x016a, 0x016a, 0x016a, 0x016a, 0x018a, 0x018a, 0x018a,
    0x018a, 0x018a, 0x018a, 0x018a, 0x018a, 0x01aa, 0x01aa, 0x01aa, 0x01aa,
    0x01aa, 0x01aa, 0x01aa, 0x01aa, 0x01ca, 0x01ca, 0x01ca, 0x01ca, 0x01ca,
    0x01ca, 0x01ca, 0x01ca, 0x01ea, 0x01ea, 0x01ea, 0x01ea, 0x01ea, 0x01ea,
    0x01ea, 0x01ea};
const uint8_t HuffDecoderCommon::table23_63_emit_[33] = {
    0xfe, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0b, 0x0c, 0x0e,
    0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x17, 0x18, 0x19, 0x1a,
    0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0xdc, 0xf9, 0x0a, 0x0d, 0x16};
const uint16_t HuffDecoderCommon::table23_63_ops_[128] = {
    0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x002b,
    0x002b, 0x002b, 0x002b, 0x004b, 0x004b, 0x004b, 0x004b, 0x006b, 0x006b,
    0x006b, 0x006b, 0x008b, 0x008b, 0x008b, 0x008b, 0x00ab, 0x00ab, 0x00ab,
    0x00ab, 0x00cb, 0x00cb, 0x00cb, 0x00cb, 0x00eb, 0x00eb, 0x00eb, 0x00eb,
    0x010b, 0x010b, 0x010b, 0x010b, 0x012b, 0x012b, 0x012b, 0x012b, 0x014b,
    0x014b, 0x014b, 0x014b, 0x016b, 0x016b, 0x016b, 0x016b, 0x018b, 0x018b,
    0x018b, 0x018b, 0x01ab, 0x01ab, 0x01ab, 0x01ab, 0x01cb, 0x01cb, 0x01cb,
    0x01cb, 0x01eb, 0x01eb, 0x01eb, 0x01eb, 0x020b, 0x020b, 0x020b, 0x020b,
    0x022b, 0x022b, 0x022b, 0x022b, 0x024b, 0x024b, 0x024b, 0x024b, 0x026b,
    0x026b, 0x026b, 0x026b, 0x028b, 0x028b, 0x028b, 0x028b, 0x02ab, 0x02ab,
    0x02ab, 0x02ab, 0x02cb, 0x02cb, 0x02cb, 0x02cb, 0x02eb, 0x02eb, 0x02eb,
    0x02eb, 0x030b, 0x030b, 0x030b, 0x030b, 0x032b, 0x032b, 0x032b, 0x032b,
    0x034b, 0x034b, 0x034b, 0x034b, 0x036b, 0x036b, 0x036b, 0x036b, 0x038b,
    0x038b, 0x038b, 0x038b, 0x03ab, 0x03ab, 0x03ab, 0x03ab, 0x03cd, 0x03ed,
    0x040d, 0x001d};
const uint8_t* const HuffDecoderCommon::table23_emit_[64] = {
    table23_0_emit_,  table23_0_emit_,  table23_2_emit_,  table23_2_emit_,
    table23_4_emit_,  table23_4_emit_,  table23_6_emit_,  table23_6_emit_,
    table23_8_emit_,  table23_8_emit_,  table23_10_emit_, table23_10_emit_,
    table23_12_emit_, table23_12_emit_, table23_14_emit_, table23_14_emit_,
    table23_16_emit_, table23_16_emit_, table23_18_emit_, table23_18_emit_,
    table23_20_emit_, table23_20_emit_, table23_22_emit_, table23_22_emit_,
    table23_24_emit_, table23_25_emit_, table23_26_emit_, table23_27_emit_,
    table23_28_emit_, table23_29_emit_, table23_30_emit_, table23_31_emit_,
    table23_32_emit_, table23_33_emit_, table23_34_emit_, table23_35_emit_,
    table23_36_emit_, table23_37_emit_, table23_38_emit_, table23_39_emit_,
    table23_40_emit_, table23_41_emit_, table23_42_emit_, table23_43_emit_,
    table23_44_emit_, table23_45_emit_, table23_46_emit_, table23_47_emit_,
    table23_48_emit_, table23_49_emit_, table23_50_emit_, table23_51_emit_,
    table23_52_emit_, table23_53_emit_, table23_54_emit_, table23_55_emit_,
    table23_56_emit_, table23_57_emit_, table23_58_emit_, table23_59_emit_,
    table23_60_emit_, table23_61_emit_, table23_62_emit_, table23_63_emit_,
};
const uint16_t* const HuffDecoderCommon::table23_ops_[64] = {
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_0_ops_,  table23_0_ops_,  table23_0_ops_,  table23_0_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_24_ops_, table23_24_ops_, table23_24_ops_,
    table23_24_ops_, table23_53_ops_, table23_53_ops_, table23_53_ops_,
    table23_53_ops_, table23_53_ops_, table23_53_ops_, table23_59_ops_,
    table23_60_ops_, table23_61_ops_, table23_62_ops_, table23_63_ops_,
};
}  // namespace geometry_9_8_13
}  // namespace grpc_core
