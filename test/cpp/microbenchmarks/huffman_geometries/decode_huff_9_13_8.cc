// Copyright 2023 gRPC authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// This file is autogenerated: see
// tools/codegen/core/gen_huffman_decompressor.cc

#include <grpc/support/port_platform.h>

#include "test/cpp/microbenchmarks/huffman_geometries/decode_huff_9_13_8.h"
namespace grpc_core {
namespace geometry_9_13_8 {
const uint8_t HuffDecoderCommon::table2_0_emit_[10] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74};
const uint8_t HuffDecoderCommon::table2_0_ops_[32] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x26, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table3_0_emit_[36] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74, 0x20, 0x25,
    0x2d, 0x2e, 0x2f, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3d, 0x41,
    0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e, 0x70, 0x72, 0x75};
const uint8_t HuffDecoderCommon::table3_0_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x2a, 0x2e,
    0x32, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a, 0x4e, 0x52, 0x56, 0x5a,
    0x5e, 0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76, 0x7a, 0x7e, 0x82, 0x86,
    0x8a, 0x8e, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table4_0_emit_[22] = {
    0x30, 0x31, 0x32, 0x61, 0x63, 0x65, 0x69, 0x6f, 0x73, 0x74, 0x20,
    0x25, 0x2d, 0x2e, 0x2f, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};
const uint8_t HuffDecoderCommon::table4_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x06, 0x01, 0x01, 0x01,
    0x0a, 0x01, 0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01,
    0x01, 0x16, 0x01, 0x01, 0x01, 0x1a, 0x01, 0x01, 0x01, 0x1e, 0x01,
    0x01, 0x01, 0x22, 0x01, 0x01, 0x01, 0x26, 0x01, 0x2a, 0x01, 0x2e,
    0x01, 0x32, 0x01, 0x36, 0x01, 0x3a, 0x01, 0x3e, 0x01, 0x42, 0x01,
    0x46, 0x01, 0x4a, 0x01, 0x4e, 0x01, 0x52, 0x01, 0x56};
const uint8_t HuffDecoderCommon::table4_1_emit_[46] = {
    0x3d, 0x41, 0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e, 0x70,
    0x72, 0x75, 0x3a, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a,
    0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56,
    0x57, 0x59, 0x6a, 0x6b, 0x71, 0x76, 0x77, 0x78, 0x79, 0x7a};
const uint8_t HuffDecoderCommon::table4_1_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x01, 0x2a,
    0x01, 0x2e, 0x01, 0x32, 0x01, 0x36, 0x3a, 0x3e, 0x42, 0x46, 0x4a,
    0x4e, 0x52, 0x56, 0x5a, 0x5e, 0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76,
    0x7a, 0x7e, 0x82, 0x86, 0x8a, 0x8e, 0x92, 0x96, 0x9a, 0x9e, 0xa2,
    0xa6, 0xaa, 0xae, 0xb2, 0xb6, 0x01, 0x01, 0x01, 0x00};
const uint8_t* const HuffDecoderCommon::table4_emit_[2] = {
    table4_0_emit_,
    table4_1_emit_,
};
const uint8_t* const HuffDecoderCommon::table4_ops_[2] = {
    table4_0_ops_,
    table4_1_ops_,
};
const uint8_t HuffDecoderCommon::table5_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table5_1_emit_[52] = {
    0x3d, 0x41, 0x5f, 0x62, 0x64, 0x66, 0x67, 0x68, 0x6c, 0x6d, 0x6e,
    0x70, 0x72, 0x75, 0x3a, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48,
    0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53,
    0x54, 0x55, 0x56, 0x57, 0x59, 0x6a, 0x6b, 0x71, 0x76, 0x77, 0x78,
    0x79, 0x7a, 0x26, 0x2a, 0x2c, 0x3b, 0x58, 0x5a};
const uint8_t HuffDecoderCommon::table5_1_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table5_emit_[2] = {
    table4_0_emit_,
    table5_1_emit_,
};
const uint8_t* const HuffDecoderCommon::table5_ops_[2] = {
    table5_0_ops_,
    table5_1_ops_,
};
const uint8_t HuffDecoderCommon::table1_0_emit_[2] = {0x30, 0x31};
const uint16_t HuffDecoderCommon::table1_0_ops_[32] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085,
    0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085, 0x0085};
const uint8_t HuffDecoderCommon::table1_1_emit_[2] = {0x32, 0x61};
const uint8_t HuffDecoderCommon::table1_2_emit_[2] = {0x63, 0x65};
const uint8_t HuffDecoderCommon::table1_3_emit_[2] = {0x69, 0x6f};
const uint8_t HuffDecoderCommon::table1_4_emit_[2] = {0x73, 0x74};
const uint8_t HuffDecoderCommon::table1_5_emit_[4] = {0x20, 0x25, 0x2d, 0x2e};
const uint16_t HuffDecoderCommon::table1_5_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086,
    0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106, 0x0106,
    0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186, 0x0186};
const uint8_t HuffDecoderCommon::table1_6_emit_[4] = {0x2f, 0x33, 0x34, 0x35};
const uint8_t HuffDecoderCommon::table1_7_emit_[4] = {0x36, 0x37, 0x38, 0x39};
const uint8_t HuffDecoderCommon::table1_8_emit_[4] = {0x3d, 0x41, 0x5f, 0x62};
const uint8_t HuffDecoderCommon::table1_9_emit_[4] = {0x64, 0x66, 0x67, 0x68};
const uint8_t HuffDecoderCommon::table1_10_emit_[4] = {0x6c, 0x6d, 0x6e, 0x70};
const uint8_t HuffDecoderCommon::table1_11_emit_[6] = {0x72, 0x75, 0x3a,
                                                       0x42, 0x43, 0x44};
const uint16_t HuffDecoderCommon::table1_11_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086, 0x0086,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0207, 0x0207, 0x0207, 0x0207, 0x0287, 0x0287, 0x0287, 0x0287};
const uint8_t HuffDecoderCommon::table1_12_emit_[8] = {0x45, 0x46, 0x47, 0x48,
                                                       0x49, 0x4a, 0x4b, 0x4c};
const uint16_t HuffDecoderCommon::table1_12_ops_[32] = {
    0x0007, 0x0007, 0x0007, 0x0007, 0x0087, 0x0087, 0x0087, 0x0087,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0207, 0x0207, 0x0207, 0x0207, 0x0287, 0x0287, 0x0287, 0x0287,
    0x0307, 0x0307, 0x0307, 0x0307, 0x0387, 0x0387, 0x0387, 0x0387};
const uint8_t HuffDecoderCommon::table1_13_emit_[8] = {0x4d, 0x4e, 0x4f, 0x50,
                                                       0x51, 0x52, 0x53, 0x54};
const uint8_t HuffDecoderCommon::table1_14_emit_[8] = {0x55, 0x56, 0x57, 0x59,
                                                       0x6a, 0x6b, 0x71, 0x76};
const uint8_t HuffDecoderCommon::table1_15_emit_[10] = {
    0x77, 0x78, 0x79, 0x7a, 0x26, 0x2a, 0x2c, 0x3b, 0x58, 0x5a};
const uint16_t HuffDecoderCommon::table1_15_ops_[32] = {
    0x0007, 0x0007, 0x0007, 0x0007, 0x0087, 0x0087, 0x0087, 0x0087,
    0x0107, 0x0107, 0x0107, 0x0107, 0x0187, 0x0187, 0x0187, 0x0187,
    0x0208, 0x0208, 0x0288, 0x0288, 0x0308, 0x0308, 0x0388, 0x0388,
    0x0408, 0x0408, 0x0488, 0x0488, 0x0019, 0x0029, 0x0039, 0x0049};
const uint8_t* const HuffDecoderCommon::table1_emit_[16] = {
    table1_0_emit_,  table1_1_emit_,  table1_2_emit_,  table1_3_emit_,
    table1_4_emit_,  table1_5_emit_,  table1_6_emit_,  table1_7_emit_,
    table1_8_emit_,  table1_9_emit_,  table1_10_emit_, table1_11_emit_,
    table1_12_emit_, table1_13_emit_, table1_14_emit_, table1_15_emit_,
};
const uint16_t* const HuffDecoderCommon::table1_ops_[16] = {
    table1_0_ops_,  table1_0_ops_,  table1_0_ops_,  table1_0_ops_,
    table1_0_ops_,  table1_5_ops_,  table1_5_ops_,  table1_5_ops_,
    table1_5_ops_,  table1_5_ops_,  table1_5_ops_,  table1_11_ops_,
    table1_12_ops_, table1_12_ops_, table1_12_ops_, table1_15_ops_,
};
const uint8_t HuffDecoderCommon::table12_0_inner_[5] = {0x01, 0x02, 0x06, 0x0a,
                                                        0x00};
const uint8_t HuffDecoderCommon::table12_0_outer_[8] = {0, 1, 2, 3, 0, 0, 0, 4};
const uint8_t HuffDecoderCommon::table13_0_emit_[9] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b, 0x5d, 0x7e};
const uint8_t HuffDecoderCommon::table13_0_inner_[11] = {
    0x01, 0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x00};
const uint8_t HuffDecoderCommon::table13_0_outer_[16] = {
    0, 0, 0, 1, 0, 2, 0, 3, 4, 5, 6, 7, 8, 9, 0, 10};
const uint8_t HuffDecoderCommon::table14_0_emit_[11] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b, 0x5d, 0x7e, 0x5e, 0x7d};
const uint8_t HuffDecoderCommon::table14_0_ops_[32] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01,
    0x06, 0x01, 0x01, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01, 0x16,
    0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x26, 0x2a, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table15_0_emit_[14] = {
    0x7c, 0x23, 0x3e, 0x00, 0x24, 0x40, 0x5b,
    0x5d, 0x7e, 0x5e, 0x7d, 0x3c, 0x60, 0x7b};
const uint8_t HuffDecoderCommon::table15_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x06, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0a, 0x01,
    0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x12, 0x01, 0x01, 0x01, 0x16,
    0x01, 0x01, 0x01, 0x1a, 0x01, 0x01, 0x01, 0x1e, 0x01, 0x01, 0x01,
    0x22, 0x01, 0x26, 0x01, 0x2a, 0x2e, 0x32, 0x36, 0x00};
const uint8_t HuffDecoderCommon::table16_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table17_0_ops_[256] = {
    };
const uint8_t HuffDecoderCommon::table18_0_emit_[0] = {

};
const uint8_t HuffDecoderCommon::table18_0_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01};
const uint8_t HuffDecoderCommon::table18_1_emit_[1] = {0x7c};
const uint8_t HuffDecoderCommon::table18_1_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02};
const uint8_t HuffDecoderCommon::table18_2_emit_[1] = {0x23};
const uint8_t HuffDecoderCommon::table18_3_emit_[1] = {0x3e};
const uint8_t HuffDecoderCommon::table18_4_emit_[2] = {0x00, 0x24};
const uint8_t HuffDecoderCommon::table18_4_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x06};
const uint8_t HuffDecoderCommon::table18_5_emit_[2] = {0x40, 0x5b};
const uint8_t HuffDecoderCommon::table18_6_emit_[2] = {0x5d, 0x7e};
const uint8_t HuffDecoderCommon::table18_7_emit_[5] = {0x5e, 0x7d, 0x3c, 0x60,
                                                       0x7b};
const uint8_t HuffDecoderCommon::table18_7_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x06, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0a, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x0e, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x12, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t* const HuffDecoderCommon::table18_emit_[8] = {
    table18_0_emit_, table18_1_emit_, table18_2_emit_, table18_3_emit_,
    table18_4_emit_, table18_5_emit_, table18_6_emit_, table18_7_emit_,
};
const uint8_t* const HuffDecoderCommon::table18_ops_[8] = {
    table18_0_ops_, table18_1_ops_, table18_1_ops_, table18_1_ops_,
    table18_4_ops_, table18_4_ops_, table18_4_ops_, table18_7_ops_,
};
const uint8_t HuffDecoderCommon::table19_8_emit_[1] = {0x00};
const uint8_t HuffDecoderCommon::table19_9_emit_[1] = {0x24};
const uint8_t HuffDecoderCommon::table19_10_emit_[1] = {0x40};
const uint8_t HuffDecoderCommon::table19_11_emit_[1] = {0x5b};
const uint8_t HuffDecoderCommon::table19_12_emit_[1] = {0x5d};
const uint8_t HuffDecoderCommon::table19_13_emit_[1] = {0x7e};
const uint8_t HuffDecoderCommon::table19_14_emit_[2] = {0x5e, 0x7d};
const uint8_t HuffDecoderCommon::table19_15_emit_[6] = {0x3c, 0x60, 0x7b,
                                                        0x5c, 0xc3, 0xd0};
const uint8_t HuffDecoderCommon::table19_15_ops_[64] = {
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x06, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x0a, 0x0e, 0x12, 0x16, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t* const HuffDecoderCommon::table19_emit_[16] = {
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_1_emit_,
    table18_0_emit_,  table18_2_emit_,  table18_0_emit_,  table18_3_emit_,
    table19_8_emit_,  table19_9_emit_,  table19_10_emit_, table19_11_emit_,
    table19_12_emit_, table19_13_emit_, table19_14_emit_, table19_15_emit_,
};
const uint8_t* const HuffDecoderCommon::table19_ops_[16] = {
    table18_0_ops_, table18_0_ops_, table18_0_ops_, table18_1_ops_,
    table18_0_ops_, table18_1_ops_, table18_0_ops_, table18_1_ops_,
    table18_1_ops_, table18_1_ops_, table18_1_ops_, table18_1_ops_,
    table18_1_ops_, table18_1_ops_, table18_4_ops_, table19_15_ops_,
};
const uint8_t HuffDecoderCommon::table20_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table20_3_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table20_14_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table20_15_emit_[14] = {
    0x3c, 0x60, 0x7b, 0x5c, 0xc3, 0xd0, 0x80,
    0x82, 0x83, 0xa2, 0xb8, 0xc2, 0xe0, 0xe2};
const uint8_t HuffDecoderCommon::table20_15_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table20_emit_[16] = {
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_1_emit_,
    table18_0_emit_,  table18_2_emit_,  table18_0_emit_,  table18_3_emit_,
    table19_8_emit_,  table19_9_emit_,  table19_10_emit_, table19_11_emit_,
    table19_12_emit_, table19_13_emit_, table19_14_emit_, table20_15_emit_,
};
const uint8_t* const HuffDecoderCommon::table20_ops_[16] = {
    table20_0_ops_, table20_0_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_3_ops_, table20_0_ops_,  table20_3_ops_,
    table20_3_ops_, table20_3_ops_, table20_3_ops_,  table20_3_ops_,
    table20_3_ops_, table20_3_ops_, table20_14_ops_, table20_15_ops_,
};
const uint8_t HuffDecoderCommon::table21_28_emit_[1] = {0x5e};
const uint8_t HuffDecoderCommon::table21_29_emit_[1] = {0x7d};
const uint8_t HuffDecoderCommon::table21_30_emit_[2] = {0x3c, 0x60};
const uint8_t HuffDecoderCommon::table21_31_emit_[25] = {
    0x7b, 0x5c, 0xc3, 0xd0, 0x80, 0x82, 0x83, 0xa2, 0xb8,
    0xc2, 0xe0, 0xe2, 0x99, 0xa1, 0xa7, 0xac, 0xb0, 0xb1,
    0xb3, 0xd1, 0xd8, 0xd9, 0xe3, 0xe5, 0xe6};
const uint8_t HuffDecoderCommon::table21_31_ops_[128] = {
    };
const uint8_t* const HuffDecoderCommon::table21_emit_[32] = {
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_0_emit_,
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_1_emit_,
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_2_emit_,
    table18_0_emit_,  table18_0_emit_,  table18_0_emit_,  table18_3_emit_,
    table18_0_emit_,  table19_8_emit_,  table18_0_emit_,  table19_9_emit_,
    table18_0_emit_,  table19_10_emit_, table18_0_emit_,  table19_11_emit_,
    table18_0_emit_,  table19_12_emit_, table18_0_emit_,  table19_13_emit_,
    table21_28_emit_, table21_29_emit_, table21_30_emit_, table21_31_emit_,
};
const uint8_t* const HuffDecoderCommon::table21_ops_[32] = {
    table20_0_ops_, table20_0_ops_, table20_0_ops_,  table20_0_ops_,
    table20_0_ops_, table20_0_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_0_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_0_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_3_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_3_ops_, table20_0_ops_,  table20_3_ops_,
    table20_0_ops_, table20_3_ops_, table20_0_ops_,  table20_3_ops_,
    table20_3_ops_, table20_3_ops_, table20_14_ops_, table21_31_ops_,
};
const uint16_t HuffDecoderCommon::table10_0_ops_[128] = {
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002, 0x0002,
    0x0002, 0x0002};
const uint16_t HuffDecoderCommon::table10_16_ops_[128] = {
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003};
const uint16_t HuffDecoderCommon::table10_32_ops_[128] = {
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004, 0x0004,
    0x0004, 0x0004};
const uint16_t HuffDecoderCommon::table10_56_ops_[128] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0005, 0x0005};
const uint8_t HuffDecoderCommon::table10_60_emit_[1] = {0x3c};
const uint16_t HuffDecoderCommon::table10_60_ops_[128] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0006, 0x0006};
const uint8_t HuffDecoderCommon::table10_61_emit_[1] = {0x60};
const uint8_t HuffDecoderCommon::table10_62_emit_[1] = {0x7b};
const uint8_t HuffDecoderCommon::table10_63_emit_[50] = {
    0x5c, 0xc3, 0xd0, 0x80, 0x82, 0x83, 0xa2, 0xb8, 0xc2, 0xe0,
    0xe2, 0x99, 0xa1, 0xa7, 0xac, 0xb0, 0xb1, 0xb3, 0xd1, 0xd8,
    0xd9, 0xe3, 0xe5, 0xe6, 0x81, 0x84, 0x85, 0x86, 0x88, 0x92,
    0x9a, 0x9c, 0xa0, 0xa3, 0xa4, 0xa9, 0xaa, 0xad, 0xb2, 0xb5,
    0xb9, 0xba, 0xbb, 0xbd, 0xbe, 0xc4, 0xc6, 0xe4, 0xe8, 0xe9};
const uint16_t HuffDecoderCommon::table10_63_ops_[128] = {
    0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x000a, 0x020a,
    0x020a, 0x020a, 0x020a, 0x020a, 0x020a, 0x020a, 0x020a, 0x040a, 0x040a,
    0x040a, 0x040a, 0x040a, 0x040a, 0x040a, 0x040a, 0x060b, 0x060b, 0x060b,
    0x060b, 0x080b, 0x080b, 0x080b, 0x080b, 0x0a0b, 0x0a0b, 0x0a0b, 0x0a0b,
    0x0c0b, 0x0c0b, 0x0c0b, 0x0c0b, 0x0e0b, 0x0e0b, 0x0e0b, 0x0e0b, 0x100b,
    0x100b, 0x100b, 0x100b, 0x120b, 0x120b, 0x120b, 0x120b, 0x140b, 0x140b,
    0x140b, 0x140b, 0x160c, 0x160c, 0x180c, 0x180c, 0x1a0c, 0x1a0c, 0x1c0c,
    0x1c0c, 0x1e0c, 0x1e0c, 0x200c, 0x200c, 0x220c, 0x220c, 0x240c, 0x240c,
    0x260c, 0x260c, 0x280c, 0x280c, 0x2a0c, 0x2a0c, 0x2c0c, 0x2c0c, 0x2e0c,
    0x2e0c, 0x300d, 0x320d, 0x340d, 0x360d, 0x380d, 0x3a0d, 0x3c0d, 0x3e0d,
    0x400d, 0x420d, 0x440d, 0x460d, 0x480d, 0x4a0d, 0x4c0d, 0x4e0d, 0x500d,
    0x520d, 0x540d, 0x560d, 0x580d, 0x5a0d, 0x5c0d, 0x5e0d, 0x600d, 0x620d,
    0x001d, 0x002d, 0x003d, 0x004d, 0x005d, 0x006d, 0x007d, 0x008d, 0x009d,
    0x00ad, 0x00bd, 0x00cd, 0x00dd, 0x00ed, 0x00fd, 0x010d, 0x011d, 0x012d,
    0x013d, 0x014d};
const uint8_t* const HuffDecoderCommon::table10_emit_[64] = {
    table18_1_emit_,  table18_1_emit_,  table18_1_emit_,  table18_1_emit_,
    table18_1_emit_,  table18_1_emit_,  table18_1_emit_,  table18_1_emit_,
    table18_1_emit_,  table18_1_emit_,  table18_1_emit_,  table18_1_emit_,
    table18_1_emit_,  table18_1_emit_,  table18_1_emit_,  table18_1_emit_,
    table18_2_emit_,  table18_2_emit_,  table18_2_emit_,  table18_2_emit_,
    table18_2_emit_,  table18_2_emit_,  table18_2_emit_,  table18_2_emit_,
    table18_3_emit_,  table18_3_emit_,  table18_3_emit_,  table18_3_emit_,
    table18_3_emit_,  table18_3_emit_,  table18_3_emit_,  table18_3_emit_,
    table19_8_emit_,  table19_8_emit_,  table19_8_emit_,  table19_8_emit_,
    table19_9_emit_,  table19_9_emit_,  table19_9_emit_,  table19_9_emit_,
    table19_10_emit_, table19_10_emit_, table19_10_emit_, table19_10_emit_,
    table19_11_emit_, table19_11_emit_, table19_11_emit_, table19_11_emit_,
    table19_12_emit_, table19_12_emit_, table19_12_emit_, table19_12_emit_,
    table19_13_emit_, table19_13_emit_, table19_13_emit_, table19_13_emit_,
    table21_28_emit_, table21_28_emit_, table21_29_emit_, table21_29_emit_,
    table10_60_emit_, table10_61_emit_, table10_62_emit_, table10_63_emit_,
};
const uint16_t* const HuffDecoderCommon::table10_ops_[64] = {
    table10_0_ops_,  table10_0_ops_,  table10_0_ops_,  table10_0_ops_,
    table10_0_ops_,  table10_0_ops_,  table10_0_ops_,  table10_0_ops_,
    table10_0_ops_,  table10_0_ops_,  table10_0_ops_,  table10_0_ops_,
    table10_0_ops_,  table10_0_ops_,  table10_0_ops_,  table10_0_ops_,
    table10_16_ops_, table10_16_ops_, table10_16_ops_, table10_16_ops_,
    table10_16_ops_, table10_16_ops_, table10_16_ops_, table10_16_ops_,
    table10_16_ops_, table10_16_ops_, table10_16_ops_, table10_16_ops_,
    table10_16_ops_, table10_16_ops_, table10_16_ops_, table10_16_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_32_ops_, table10_32_ops_, table10_32_ops_, table10_32_ops_,
    table10_56_ops_, table10_56_ops_, table10_56_ops_, table10_56_ops_,
    table10_60_ops_, table10_60_ops_, table10_60_ops_, table10_63_ops_,
};
const uint8_t HuffDecoderCommon::table39_0_emit_[15] = {
    0xc0, 0xc1, 0xc8, 0xc9, 0xca, 0xcd, 0xd2, 0xd5,
    0xda, 0xdb, 0xee, 0xf0, 0xf2, 0xf3, 0xff};
const uint8_t HuffDecoderCommon::table39_0_inner_[16] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e,
    0x22, 0x26, 0x2a, 0x2e, 0x32, 0x36, 0x3a, 0x01};
const uint8_t HuffDecoderCommon::table38_0_emit_[17] = {
    0xc0, 0xc1, 0xc8, 0xc9, 0xca, 0xcd, 0xd2, 0xd5, 0xda,
    0xdb, 0xee, 0xf0, 0xf2, 0xf3, 0xff, 0xcb, 0xcc};
const uint8_t HuffDecoderCommon::table38_0_ops_[32] = {
    0x04, 0x04, 0x0c, 0x0c, 0x14, 0x14, 0x1c, 0x1c, 0x24, 0x24, 0x2c,
    0x2c, 0x34, 0x34, 0x3c, 0x3c, 0x44, 0x44, 0x4c, 0x4c, 0x54, 0x54,
    0x5c, 0x5c, 0x64, 0x64, 0x6c, 0x6c, 0x74, 0x74, 0x7d, 0x85};
const uint8_t HuffDecoderCommon::table42_0_emit_[6] = {0xec, 0xed, 0xc7,
                                                       0xcf, 0xea, 0xeb};
const uint8_t HuffDecoderCommon::table42_0_inner_[6] = {0x02, 0x06, 0x0b,
                                                        0x0f, 0x13, 0x17};
const uint8_t HuffDecoderCommon::table45_0_emit_[17] = {
    0xd3, 0xd4, 0xd6, 0xdd, 0xde, 0xdf, 0xf1, 0xf4, 0xf5,
    0xf6, 0xf7, 0xf8, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe};
const uint8_t HuffDecoderCommon::table45_0_ops_[32] = {
    0x02, 0x06, 0x0a, 0x0e, 0x12, 0x16, 0x1a, 0x1e, 0x22, 0x26, 0x2a,
    0x2e, 0x32, 0x36, 0x3a, 0x3e, 0x42, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00};
const uint8_t HuffDecoderCommon::table46_0_emit_[46] = {
    0xd3, 0xd4, 0xd6, 0xdd, 0xde, 0xdf, 0xf1, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8,
    0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
    0x0b, 0x0c, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x17, 0x18,
    0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0xdc, 0xf9};
const uint8_t HuffDecoderCommon::table46_0_ops_[64] = {
    0x01, 0x02, 0x01, 0x06, 0x01, 0x0a, 0x01, 0x0e, 0x01, 0x12, 0x01,
    0x16, 0x01, 0x1a, 0x01, 0x1e, 0x01, 0x22, 0x01, 0x26, 0x01, 0x2a,
    0x01, 0x2e, 0x01, 0x32, 0x01, 0x36, 0x01, 0x3a, 0x01, 0x3e, 0x01,
    0x42, 0x46, 0x4a, 0x4e, 0x52, 0x56, 0x5a, 0x5e, 0x62, 0x66, 0x6a,
    0x6e, 0x72, 0x76, 0x7a, 0x7e, 0x82, 0x86, 0x8a, 0x8e, 0x92, 0x96,
    0x9a, 0x9e, 0xa2, 0xa6, 0xaa, 0xae, 0xb2, 0xb6, 0x00};
const uint8_t HuffDecoderCommon::table47_0_ops_[128] = {
    };
const uint8_t HuffDecoderCommon::table44_0_emit_[4] = {0xd3, 0xd4, 0xd6, 0xdd};
const uint16_t HuffDecoderCommon::table44_0_ops_[32] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0025, 0x0025, 0x0025, 0x0025, 0x0025, 0x0025, 0x0025, 0x0025,
    0x0045, 0x0045, 0x0045, 0x0045, 0x0045, 0x0045, 0x0045, 0x0045,
    0x0065, 0x0065, 0x0065, 0x0065, 0x0065, 0x0065, 0x0065, 0x0065};
const uint8_t HuffDecoderCommon::table44_1_emit_[4] = {0xde, 0xdf, 0xf1, 0xf4};
const uint8_t HuffDecoderCommon::table44_2_emit_[4] = {0xf5, 0xf6, 0xf7, 0xf8};
const uint8_t HuffDecoderCommon::table44_3_emit_[4] = {0xfa, 0xfb, 0xfc, 0xfd};
const uint8_t HuffDecoderCommon::table44_4_emit_[7] = {0xfe, 0x02, 0x03, 0x04,
                                                       0x05, 0x06, 0x07};
const uint16_t HuffDecoderCommon::table44_4_ops_[32] = {
    0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005, 0x0005,
    0x0026, 0x0026, 0x0026, 0x0026, 0x0046, 0x0046, 0x0046, 0x0046,
    0x0066, 0x0066, 0x0066, 0x0066, 0x0086, 0x0086, 0x0086, 0x0086,
    0x00a6, 0x00a6, 0x00a6, 0x00a6, 0x00c6, 0x00c6, 0x00c6, 0x00c6};
const uint8_t HuffDecoderCommon::table44_5_emit_[8] = {0x08, 0x0b, 0x0c, 0x0e,
                                                       0x0f, 0x10, 0x11, 0x12};
const uint16_t HuffDecoderCommon::table44_5_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0026, 0x0026, 0x0026, 0x0026,
    0x0046, 0x0046, 0x0046, 0x0046, 0x0066, 0x0066, 0x0066, 0x0066,
    0x0086, 0x0086, 0x0086, 0x0086, 0x00a6, 0x00a6, 0x00a6, 0x00a6,
    0x00c6, 0x00c6, 0x00c6, 0x00c6, 0x00e6, 0x00e6, 0x00e6, 0x00e6};
const uint8_t HuffDecoderCommon::table44_6_emit_[8] = {0x13, 0x14, 0x15, 0x17,
                                                       0x18, 0x19, 0x1a, 0x1b};
const uint8_t HuffDecoderCommon::table44_7_emit_[10] = {
    0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0xdc, 0xf9, 0x0a, 0x0d, 0x16};
const uint16_t HuffDecoderCommon::table44_7_ops_[32] = {
    0x0006, 0x0006, 0x0006, 0x0006, 0x0026, 0x0026, 0x0026, 0x0026,
    0x0046, 0x0046, 0x0046, 0x0046, 0x0066, 0x0066, 0x0066, 0x0066,
    0x0086, 0x0086, 0x0086, 0x0086, 0x00a6, 0x00a6, 0x00a6, 0x00a6,
    0x00c6, 0x00c6, 0x00c6, 0x00c6, 0x00e8, 0x0108, 0x0128, 0x0018};
const uint8_t* const HuffDecoderCommon::table44_emit_[8] = {
    table44_0_emit_, table44_1_emit_, table44_2_emit_, table44_3_emit_,
    table44_4_emit_, table44_5_emit_, table44_6_emit_, table44_7_emit_,
};
const uint16_t* const HuffDecoderCommon::table44_ops_[8] = {
    table44_0_ops_, table44_0_ops_, table44_0_ops_, table44_0_ops_,
    table44_4_ops_, table44_5_ops_, table44_5_ops_, table44_7_ops_,
};
}  // namespace geometry_9_13_8
}  // namespace grpc_core
