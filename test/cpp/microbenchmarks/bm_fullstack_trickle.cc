...
ABSL_FLAG(int32_t, warmup_max_time_seconds, 10, "Maximum number of seconds to run warmup loop");
...
class TrickledCHTTP2 : public EndpointPairFixture {
 public:
  TrickledCHTTP2(Service* service, bool streaming, size_t req_size, size_t resp_size,
                 size_t kilobits_per_second, grpc_passthru_endpoint_stats* stats)
      : EndpointPairFixture(service, MakeEndpoints(kilobits_per_second, stats),
                            FixtureConfiguration()),
        stats_(stats) {
    if (absl::GetFlag(FLAGS_log)) {
      std::ostringstream fn;
      fn << "trickle." << (streaming ? "streaming" : "unary") << "." << req_size << "." << resp_size
         << "." << kilobits_per_second << ".csv";
      log_ = absl::make_unique<std::ofstream>(fn.str().c_str());
      write_csv(log_.get(), "t", "iteration", "client_backlog", "server_backlog", "client_t_stall",
                "client_s_stall", "server_t_stall", "server_s_stall", "client_t_remote",
                "server_t_remote", "client_t_announced", "server_t_announced",
                "client_s_remote_delta", "server_s_remote_delta", "client_s_local_delta",
                "server_s_local_delta", "client_s_announced_delta", "server_s_announced_delta",
                "client_peer_iws", "client_local_iws", "client_sent_iws", "client_acked_iws",
                "server_peer_iws", "server_local_iws", "server_sent_iws", "server_acked_iws",
                "client_queued_bytes", "server_queued_bytes");
    }
  }
  ...
  void AddToLabel(std::ostream& out, benchmark::State& state) override {
    out << " writes/iter:"
        << (static_cast<double>(stats_->num_writes) / static_cast<double>(state.iterations()))
        << " cli_transport_stalls/iter:"
        << (static_cast<double>(client_stats_.streams_stalled_due_to_transport_flow_control) /
            static_cast<double>(state.iterations()))
        << " cli_stream_stalls/iter:"
        << (static_cast<double>(client_stats_.streams_stalled_due_to_stream_flow_control) /
            static_cast<double>(state.iterations()))
        << " svr_transport_stalls/iter:"
        << (static_cast<double>(server_stats_.streams_stalled_due_to_transport_flow_control) /
            static_cast<double>(state.iterations()))
        << " svr_stream_stalls/iter:"
        << (static_cast<double>(server_stats_.streams_stalled_due_to_stream_flow_control) /
            static_cast<double>(state.iterations()));
  }

  void Log(int64_t iteration) GPR_ATTRIBUTE_NO_TSAN {
    auto now = gpr_time_sub(gpr_now(GPR_CLOCK_MONOTONIC), start_);
    grpc_chttp2_transport* client = reinterpret_cast<grpc_chttp2_transport*>(client_transport_);
    grpc_chttp2_transport* server = reinterpret_cast<grpc_chttp2_transport*>(server_transport_);
    ...
    grpc_chttp2_stream* server_stream =
        server->stream_map.count == 1
            ...
            : nullptr;
    write_csv(log_.get(), static_cast<double>(now.tv_sec) + 1e-9 * static_cast<double>(now.tv_nsec),
              ...
              client->flow_control->remote_window_, server->flow_control->remote_window_,
              client->flow_control->announced_window_, server->flow_control->announced_window_,
              ...
              client_stream ? client_stream->flow_control->announced_window_delta_ : -1,
              server_stream ? server_stream->flow_control->announced_window_delta_ : -1,
              client->settings[GRPC_PEER_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
              ...
              client->settings[GRPC_ACKED_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
              server->settings[GRPC_PEER_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
              ...
              server->settings[GRPC_ACKED_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
  ...
  void Step(bool update_stats) {
    ...
    inc_time();
    size_t client_backlog = grpc_trickle_endpoint_trickle(endpoint_pair_.client);
    size_t server_backlog = grpc_trickle_endpoint_trickle(endpoint_pair_.server);

    if (update_stats) {
      UpdateStats(reinterpret_cast<grpc_chttp2_transport*>(client_transport_), &client_stats_,
                  client_backlog);
      UpdateStats(reinterpret_cast<grpc_chttp2_transport*>(server_transport_), &server_stats_,
                  server_backlog);
    }
  }
 ...
 private:
  ...
  static grpc_endpoint_pair MakeEndpoints(size_t kilobits, grpc_passthru_endpoint_stats* stats) {
  ...
  void UpdateStats(grpc_chttp2_transport* t, Stats* s, size_t backlog) GPR_ATTRIBUTE_NO_TSAN {
    if (backlog == 0) {
...
static void TrickleCQNext(TrickledCHTTP2* fixture, void** t, bool* ok, int64_t iteration) {
  while (true) {
    fixture->Log(iteration);
    switch (fixture->cq()->AsyncNext(t, ok, gpr_inf_past(GPR_CLOCK_MONOTONIC))) {
...
static void BM_PumpStreamServerToClient_Trickle(benchmark::State& state) {
  ...
  std::unique_ptr<TrickledCHTTP2> fixture(new TrickledCHTTP2(
      &service, true, state.range(0) /* req_size */, state.range(0) /* resp_size */,
      state.range(1) /* bw in kbit/s */, grpc_passthru_endpoint_stats_create()));
  {
    ...
    for (int i = 0;
         i < GPR_MAX(absl::GetFlag(FLAGS_warmup_iterations),
                     absl::GetFlag(FLAGS_warmup_megabytes) * 1024 * 1024 / (14 + state.range(0)));
         i++) {
      inner_loop(true);
...
static void StreamingTrickleArgs(benchmark::internal::Benchmark* b) {
  for (int i = 1; i <= 128 * 1024 * 1024; i *= 8) {
    for (int j = 64; j <= 128 * 1024 * 1024; j *= 8) {
      double expected_time = static_cast<double>(14 + i) / (125.0 * static_cast<double>(j));
...
static void BM_PumpUnbalancedUnary_Trickle(benchmark::State& state) {
  EchoTestService::AsyncService service;
  std::unique_ptr<TrickledCHTTP2> fixture(new TrickledCHTTP2(
      &service, false, state.range(0) /* req_size */, state.range(1) /* resp_size */,
      state.range(2) /* bw in kbit/s */, grpc_passthru_endpoint_stats_create()));
...
static void UnaryTrickleArgs(benchmark::internal::Benchmark* b) {
  for (int bw = 64; bw <= 128 * 1024 * 1024; bw *= 16) {
    ...
    for (int i = 64; i <= 128 * 1024 * 1024; i *= 64) {
      double expected_time = static_cast<double>(14 + i) / (125.0 * static_cast<double>(bw));
      if (expected_time > 2.0) continue;
      b->Args({i, 1, bw});
      b->Args({1, i, bw});
      b->Args({i, i, bw});
