...
DEFINE_int32 (warmup_megabytes, 1,
	      "Number of megabytes to pump before collecting flow control stats");
DEFINE_int32 (warmup_iterations, 100,
...
namespace grpc
{
  namespace testing
  {
    ...
    template < class A0 > static void write_csv (std::ostream * out, A0 && a0)
    {
      if (!out)
	return;
      (*out) << a0 << "\n";
    ...
    public:
      TrickledCHTTP2 (Service * service, bool streaming, size_t req_size,
		      size_t resp_size,
		      size_t
		      kilobits_per_second):EndpointPairFixture (service,
								MakeEndpoints
								(kilobits_per_second),
	...
	if (FLAGS_log)
	  {
	    ...
	    write_csv (log_.get (), "t", "iteration", "client_backlog",
		       ...
		       "server_t_remote", "client_t_announced",
		       "server_t_announced", "client_s_remote_delta",
		       "server_s_remote_delta", "client_s_local_delta",
		       "server_s_local_delta", "client_s_announced_delta",
		       "server_s_announced_delta", "client_peer_iws",
		       "client_local_iws", "client_sent_iws",
		       "client_acked_iws", "server_peer_iws",
		       "server_local_iws", "server_sent_iws",
		       "server_acked_iws", "client_queued_bytes",
		       "server_queued_bytes");
	  }
      }
	...
	out << " writes/iter:"
	  ...
	  << ((double) client_stats_.
	      streams_stalled_due_to_stream_flow_control /
	      (double) state.
	      iterations ()) << " svr_transport_stalls/iter:" << ((double)
								  server_stats_.
								  ...
								  iterations
								  ()) <<
	  " svr_stream_stalls/iter:" << ((double) server_stats_.
					 streams_stalled_due_to_stream_flow_control
					 / (double) state.iterations ());
      }
	...
	write_csv (log_.get (),
		   ...
		   settings[GRPC_ACKED_SETTINGS]
		   [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE],
		   client_stream ? client_stream->flow_controlled_buffer.
		   length : 0,
		   server_stream ? server_stream->flow_controlled_buffer.
		   length : 0);
      }
	...
	if (update_stats)
	  {
	    UpdateStats ((grpc_chttp2_transport *) client_transport_,
			 &client_stats_, client_backlog);
	    UpdateStats ((grpc_chttp2_transport *) server_transport_,
...
// force library initialization
      ...
      while (true)
	{
	  ...
	  switch (fixture->cq ()->
		  AsyncNext (t, ok, gpr_inf_past (GPR_CLOCK_MONOTONIC)))
	    {
    ...
    static void BM_PumpStreamServerToClient_Trickle (benchmark::State & state)
    {
      ...
      std::unique_ptr < TrickledCHTTP2 >
	fixture (new
		 TrickledCHTTP2 (&service, true,
				 state.range (0) /* req_size */ ,
				 state.range (0) /* resp_size */ ,
				 state.range (1) /* bw in kbit/s */ ));
      {
	...
	for (int i = 0;
	     i < GPR_MAX (FLAGS_warmup_iterations,
			  FLAGS_warmup_megabytes * 1024 * 1024 / (14 +
								  state.
								  range (0)));
	     i++)
	  {
      ...
      for (int i = 1; i <= 128 * 1024 * 1024; i *= 8)
	{
	  for (int j = 64; j <= 128 * 1024 * 1024; j *= 8)
		...
		static_cast < double >(14 + i) / (125.0 * static_cast <
						  double >(j));
	      if (expected_time > 2.0)
		continue;
	      b->Args (
			{
			i, j}
    ...
    static void BM_PumpUnbalancedUnary_Trickle (benchmark::State & state)
    {
      ...
      std::unique_ptr < TrickledCHTTP2 >
	fixture (new
		 TrickledCHTTP2 (&service, false,
				 state.range (0) /* req_size */ ,
				 state.range (1) /* resp_size */ ,
				 state.range (2) /* bw in kbit/s */ ));
