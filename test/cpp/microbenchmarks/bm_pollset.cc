...
static void
shutdown_ps (void *ps, grpc_error * error)
{
  grpc_pollset_destroy (static_cast < grpc_pollset * >(ps));
}

static void
BM_CreateDestroyPollset (benchmark::State & state)
{
  TrackCounters
    track_counters;
  size_t
    ps_sz = grpc_pollset_size ();
  grpc_pollset *
...
#ifdef GRPC_LINUX_MULTIPOLL_WITH_EPOLL
static void
BM_PollEmptyPollset_SpeedOfLight (benchmark::State & state)
{
...
BENCHMARK (BM_PollEmptyPollset_SpeedOfLight)->Args (
						     {
						     1, 0}
...
static void
BM_PollEmptyPollset (benchmark::State & state)
{
  TrackCounters
    track_counters;
  size_t
    ps_sz = grpc_pollset_size ();
  grpc_pollset *
    ps = static_cast < grpc_pollset * >(gpr_zalloc (ps_sz));
  gpr_mu *
    mu;
  grpc_pollset_init (ps, &mu);
  grpc_core::ExecCtx exec_ctx;
...
static void
BM_PollAddFd (benchmark::State & state)
{
  TrackCounters
    track_counters;
  size_t
  ...
  GPR_ASSERT (GRPC_LOG_IF_ERROR
	      ("wakeup_fd_init", grpc_wakeup_fd_init (&wakeup_fd)));
  grpc_fd *
    fd = grpc_fd_create (wakeup_fd.read_fd, "xxx");
  while (state.KeepRunning ())
    {
  ...
  Closure * MakeClosure (F f, grpc_closure_scheduler * scheduler)
{
  struct C:
    ...
    Closure
  {
    C (F f, grpc_closure_scheduler * scheduler):
    f_ (f)
    {
      GRPC_CLOSURE_INIT (this, C::cbfn, this, scheduler);
    }
    static void
    cbfn (void *arg, grpc_error * error)
    {
      C *
	p = static_cast < C * >(arg);
      p->f_ ();
...
#ifdef GRPC_LINUX_MULTIPOLL_WITH_EPOLL
static void
BM_SingleThreadPollOneFd_SpeedOfLight (benchmark::State & state)
{
  // equivalent to BM_PollEmptyPollset, but just use the OS primitives to guage
  // what the speed of light would be if we abstracted perfectly
  TrackCounters
...
static void
BM_SingleThreadPollOneFd (benchmark::State & state)
{
  TrackCounters
    track_counters;
  size_t
  ...
  grpc_fd *
    wakeup = grpc_fd_create (wakeup_fd.read_fd, "wakeup_read");
  grpc_pollset_add_fd (ps, wakeup);
  bool
    done = false;
  Closure *
    continue_closure = MakeClosure ([&](){
				    GRPC_ERROR_UNREF
				    (grpc_wakeup_fd_consume_wakeup
				     (&wakeup_fd)); if (!state.KeepRunning ())
				    {
				    done = true; return;}
				    GRPC_ERROR_UNREF (grpc_wakeup_fd_wakeup
						      (&wakeup_fd));
				    grpc_fd_notify_on_read (wakeup,
							    continue_closure);}
				    , grpc_schedule_on_exec_ctx);
  ...
  while (!done)
    {
      GRPC_ERROR_UNREF (grpc_pollset_work
			(ps, nullptr, GRPC_MILLIS_INF_FUTURE));
    }
  grpc_fd_orphan (wakeup, nullptr, nullptr, false /* already_closed */ ,
		  "done");
  wakeup_fd.read_fd = 0;
  grpc_closure
    shutdown_ps_closure;
