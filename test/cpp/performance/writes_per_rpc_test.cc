...
namespace grpc
{
  namespace testing
  {
	...
	/* add server endpoint to server_ */
	{
	  ...
	  grpc_server_setup_transport (server_->c_server (), transport,
				       nullptr, server_args);
    ...
    {
    public:
      InProcessCHTTP2 (Service * service):EndpointPairFixture (service,
							       MakeEndpoints
							       ())
      {
      }
    ...
    static double UnaryPingPong (int request_size, int response_size)
    {
      ...
      std::unique_ptr < InProcessCHTTP2 >
	fixture (new InProcessCHTTP2 (&service));
	...
	reinterpret_cast <
	  ServerEnv * >(server_env_buffer + sizeof (ServerEnv))
      };
      ...
      service.RequestEcho (&server_env[1]->ctx, &server_env[1]->recv_request,
			   ...
			   fixture->cq (), tag (1));
      std::unique_ptr < EchoTestService::Stub >
	stub (EchoTestService::NewStub (fixture->channel ()));
