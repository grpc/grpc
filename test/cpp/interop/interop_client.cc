...
void UnaryCompressionChecks(
  ...
  if (request->response_compressed().value()) {
    if (received_compression == GRPC_COMPRESS_NONE) {
      ...
      gpr_log(GPR_ERROR,
              "Failure: Requested compression but got uncompressed "
              "response "
              "from server.");
      abort();
    }
    ...
    // Didn't request compression -> make sure the response is
    // uncompressed
    GPR_ASSERT(!(inspector.WasCompressed()));
  }
}
...
InteropClient::ServiceStub::ServiceStub(
  ...
  // If new_stub_every_call is false, then this is our chance to
  // initialize stub_. (see Get())
...
InteropClient::ServiceStub::GetUnimplementedServiceStub() {
  if (unimplemented_service_stub_ == nullptr) {
    unimplemented_service_stub_ =
        UnimplementedService::NewStub(channel_);
  }
...
InteropClient::InteropClient(ChannelCreationFunc channel_creation_func,
    ...
    : serviceStub_(std::move(channel_creation_func),
                   new_stub_every_test_case),
      do_not_abort_on_transient_failures_(
          do_not_abort_on_transient_failures) {}

bool InteropClient::AssertStatusOk(
    const Status& s, const std::string& optional_debug_string) {
  if (s.ok()) {
    return true;
  }

  // Note: At this point, s.error_code is definitely not StatusCode::OK
  // (we already checked for s.ok() above). So, the following will call
  // abort() (unless s.error_code() corresponds to a transient failure
  // and 'do_not_abort_on_transient_failures' is true)
...
bool InteropClient::DoEmpty() {
  ...
  Status s =
      serviceStub_.Get()->EmptyCall(&context, request, &response);
...
bool InteropClient::PerformLargeUnary(
  ...
  request->mutable_payload()->set_body(payload.c_str(),
                                       kLargeRequestSize);
  ...
  Status s =
      serviceStub_.Get()->UnaryCall(&context, *request, response);
  if (!AssertStatusOk(s, context.debug_error_string())) {
    return false;
  }
...
bool InteropClient::DoComputeEngineCreds(
  ...
  gpr_log(GPR_DEBUG, "Got oauth_scope %s",
...
bool InteropClient::DoOauth2AuthToken(const std::string& username,
                                      const std::string& oauth_scope) {
  gpr_log(GPR_DEBUG,
          "Sending a unary rpc with raw oauth2 access token "
          "credentials ...");
  SimpleRequest request;
  SimpleResponse response;
  request.set_fill_username(true);
...
bool InteropClient::DoGoogleDefaultCredentials(
  ...
  GPR_ASSERT(response.username().c_str() == default_service_account);
  gpr_log(GPR_DEBUG,
          "Large unary rpc with GoogleDefaultCredentials done.");
  return true;
}
...
bool InteropClient::DoClientCompressedUnary() {
  ...
  probe_req.mutable_payload()->set_body(
      std::string(kLargeRequestSize, '\0'));

  gpr_log(GPR_DEBUG, "Sending probe for compressed unary request.");
  const Status s = serviceStub_.Get()->UnaryCall(&probe_context,
                                                 probe_req, &probe_res);
  if (s.error_code() != grpc::StatusCode::INVALID_ARGUMENT) {
    // The server isn't able to evaluate incoming compression, making
    // the rest of this test moot.
  ...
  for (size_t i = 0; i < compressions.size(); i++) {
    std::string log_suffix = absl::StrFormat(
        "(compression=%s)", compressions[i] ? "true" : "false");
...
bool InteropClient::DoServerCompressedUnary() {
  ...
  for (size_t i = 0; i < compressions.size(); i++) {
    ...
    gpr_log(GPR_DEBUG,
            "Sending unary request for compressed response %s.",
            log_suffix.c_str());
    SimpleRequest request;
    SimpleResponse response;
...
// Either abort() (unless do_not_abort_on_transient_failures_ is true)
// or return false
...
bool InteropClient::DoRequestStreaming() {
  ...
  for (size_t i = 0; i < request_stream_sizes.size(); ++i) {
    ...
    if (!stream->Write(request)) {
      gpr_log(GPR_ERROR,
              "DoRequestStreaming(): stream->Write() failed");
      return TransientFailureOrAbort();
    }
...
bool InteropClient::DoResponseStreaming() {
  ...
  if (i < response_stream_sizes.size()) {
    // stream->Read() failed before reading all the expected messages.
    // This is most likely due to connection failure.
    gpr_log(GPR_ERROR,
...
bool InteropClient::DoClientCompressedStreaming() {
  ...
  std::unique_ptr<ClientWriter<StreamingInputCallRequest>> probe_stream(
      serviceStub_.Get()->StreamingInputCall(&probe_context,
                                             &probe_res));
  ...
  gpr_log(GPR_DEBUG,
          "Sending streaming request with compression enabled");
  if (!stream->Write(request)) {
    gpr_log(GPR_ERROR, "%s(): stream->Write() failed", __func__);
    return TransientFailureOrAbort();
  }
  ...
  gpr_log(GPR_DEBUG,
          "Sending streaming request with compression disabled");
...
bool InteropClient::DoServerCompressedStreaming() {
  ...
  for (size_t i = 0; i < sizes.size(); i++) {
    ...
    gpr_log(GPR_DEBUG, "Sending request streaming rpc %s.",
            log_suffix.c_str());

    ResponseParameters* const response_parameter =
        request.add_response_parameters();
    response_parameter->mutable_compressed()->set_value(
        compressions[i]);
...
bool InteropClient::DoResponseStreamingWithSlowConsumer() {
  ...
  if (i < kNumResponseMessages) {
    gpr_log(GPR_ERROR,
            "DoResponseStreamingWithSlowConsumer(): Responses read "
            "(i=%d) is "
            "less than the expected messages (i.e kNumResponseMessages "
            "= %d)",
...
bool InteropClient::DoHalfDuplex() {
  ...
  for (unsigned int i = 0; i < response_stream_sizes.size(); ++i) {
    ...
    if (!stream->Write(request)) {
      gpr_log(GPR_ERROR, "DoHalfDuplex(): stream->Write() failed. i=%d",
              i);
      return TransientFailureOrAbort();
    }
  }
  ...
  if (i < response_stream_sizes.size()) {
    ...
    // This is most likely due to a connection failure
    gpr_log(GPR_ERROR,
            "DoHalfDuplex(): Responses read (i=%d) are less than the "
            "expected "
...
bool InteropClient::DoPingPong() {
  ...
  for (unsigned int i = 0; i < request_stream_sizes.size(); ++i) {
    ...
    if (!stream->Write(request)) {
      gpr_log(GPR_ERROR, "DoPingPong(): stream->Write() failed. i: %d",
              i);
      return TransientFailureOrAbort();
    }

    if (!stream->Read(&response)) {
      gpr_log(GPR_ERROR, "DoPingPong(): stream->Read() failed. i:%d",
              i);
      return TransientFailureOrAbort();
    }
...
bool InteropClient::DoCancelAfterFirstResponse() {
  ...
  if (!stream->Read(&response)) {
    gpr_log(GPR_ERROR,
            "DoCancelAfterFirstResponse(): stream->Read failed");
    return TransientFailureOrAbort();
  }
...
bool InteropClient::DoCacheableUnary() {
  ...
  SimpleRequest request;
  request.mutable_payload()->set_body(timestamp.c_str(),
                                      timestamp.size());
  ...
  Status s1 = serviceStub_.Get()->CacheableUnaryCall(&context1, request,
                                                     &response1);
  ...
  Status s2 = serviceStub_.Get()->CacheableUnaryCall(&context2, request,
                                                     &response2);
  ...
  // Check that the body is same for both requests. It will be the same
  // if the second response is a cached copy of the first response
  ...
  request1.mutable_payload()->set_body(timestamp.c_str(),
                                       timestamp.size());
  ClientContext context3;
  SimpleResponse response3;
  context3.set_cacheable(true);
  context3.AddMetadata("x-user-ip", "1.2.3.4");
  Status s3 = serviceStub_.Get()->CacheableUnaryCall(
      &context3, request1, &response3);
...
bool InteropClient::DoPickFirstUnary() {
  ...
  for (int i = 0; i < rpcCount; i++) {
    ...
    if (response.server_id() != server_id) {
      gpr_log(GPR_ERROR,
              "#%d rpc hits server_id %s, expect server_id %s", i,
...
bool InteropClient::DoCustomMetadata() {
  ...
  const std::string kInitialMetadataValue(
...
void InteropClient::PerformSoakTest(
  ...
  for (size_t i = 0; i < results.size(); i++) {
    ...
    if (!success) {
      gpr_log(GPR_DEBUG,
              "soak iteration: %ld elapsed_ms: %d failed: %s", i,
              elapsed_ms, debug_string.c_str());
      total_failures++;
    } else {
      gpr_log(GPR_DEBUG, "soak iteration: %ld elapsed_ms: %d succeeded",
              i, elapsed_ms);
    }
  }
  ...
  double latency_ms_worst =
      grpc_histogram_maximum(latencies_ms_histogram);
  grpc_histogram_destroy(latencies_ms_histogram);
  if (iterations_ran < soak_iterations) {
    gpr_log(GPR_ERROR,
            "soak test consumed all %d seconds of time and quit early, "
            "only "
            ...
            "Some or all of the iterations that did run were "
            "unexpectedly slow. "
            "See breakdown above for which iterations succeeded, "
            "failed, and "
            "why for more info.",
            overall_timeout_seconds, iterations_ran, soak_iterations,
            total_failures, max_failures, latency_ms_median,
    ...
    gpr_log(GPR_ERROR,
            ...
            "failed, and "
            "why for more info.",
            soak_iterations, total_failures, max_failures,
...
bool InteropClient::DoRpcSoakTest(
  ...
  PerformSoakTest(false /* reset channel per iteration */,
...
bool InteropClient::DoChannelSoakTest(
  ...
  gpr_log(GPR_DEBUG,
          ...
          soak_iterations);
  GPR_ASSERT(soak_iterations > 0);
  PerformSoakTest(true /* reset channel per iteration */,
                  soak_iterations, max_failures,
                  max_acceptable_per_iteration_latency_ms,
                  overall_timeout_seconds);
...
bool InteropClient::DoLongLivedChannelTest(int32_t soak_iterations,
  ...
  if (num_failures == 0) {
    ...
    gpr_log(GPR_DEBUG,
            "long_lived_channel test failed with %d rpc failures.",
            num_failures);
    return false;
  }
}
...
bool InteropClient::DoUnimplementedMethod() {
  ...
  Status s = serviceStub_.Get()->UnimplementedCall(&context, request,
                                                   &response);
