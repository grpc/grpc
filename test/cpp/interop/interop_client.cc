...
// The same value is defined by the Java client.
      const std::vector < int >request_stream_sizes =
	{ 27182, 8, 1828, 45904 };
      const std::vector < int >response_stream_sizes =
	{ 31415, 9, 2653, 58979 };
      ...
      void UnaryCompressionChecks (const InteropClientContextInspector &
				   inspector, const SimpleRequest * request,
				   const SimpleResponse * response)
      {
	...
	if (request->response_compressed ().value ())
	  {
	    ...
	    GPR_ASSERT (inspector.
			GetMessageFlags () & GRPC_WRITE_INTERNAL_COMPRESS);
	  }
	else
	  {
  ...
  InteropClient::ServiceStub::ServiceStub (std::shared_ptr < Channel > channel, bool new_stub_every_call):channel_ (channel),
    ...
    void InteropClient::ServiceStub::Reset (std::shared_ptr < Channel >
					    channel)
    {
      channel_ = channel;
  ...
  InteropClient::InteropClient (std::shared_ptr < Channel > channel, bool new_stub_every_test_case, bool do_not_abort_on_transient_failures):serviceStub_ (channel, new_stub_every_test_case),
    ...
    bool InteropClient::DoComputeEngineCreds (const grpc::
					      string &
    ...
    bool InteropClient::DoClientCompressedUnary ()
    {
      ...
      probe_req.mutable_payload ()->
	set_body (grpc::string (kLargeRequestSize, '\0'));

      gpr_log (GPR_DEBUG, "Sending probe for compressed unary request.");
      const Status s =
	serviceStub_.Get ()->UnaryCall (&probe_context, probe_req,
					&probe_res);
      ...
      for (size_t i = 0; i < compressions.size (); i++)
	{
	  ...
	  if (!PerformLargeUnary
	      (&request, &response, UnaryCompressionChecks))
	    {
...
// false
    ...
    bool InteropClient::DoRequestStreaming ()
    {
      ...
      std::unique_ptr < ClientWriter <
	StreamingInputCallRequest >> stream (serviceStub_.Get ()->
					     StreamingInputCall (&context,
								 &response));
	...
	StreamingOutputCallResponse >> stream (serviceStub_.Get ()->
					       StreamingOutputCall (&context,
								    request));
    ...
    bool InteropClient::DoClientCompressedStreaming ()
    {
      ...
      std::unique_ptr < ClientWriter <
	StreamingInputCallRequest >> probe_stream (serviceStub_.Get ()->
						   StreamingInputCall
						   (&probe_context,
						    &probe_res));
      ...
      for (size_t i = 0; i < sizes.size (); i++)
	{
	  ...
	  ResponseParameters *const response_parameter =
	    request.add_response_parameters ();
	  response_parameter->mutable_compressed ()->
	    set_value (compressions[i]);
      ...
      while (stream->Read (&response))
	{
	  // Payload size checks.
	  GPR_ASSERT (response.payload ().body () ==
		      grpc::string (request.response_parameters (k).size (),
				    '\0'));
      ...
      for (unsigned int i = 0; i < response_stream_sizes.size (); ++i)
	{
	  ...
	  if (!stream->Write (request))
	    {
	      gpr_log (GPR_ERROR,
		       "DoHalfDuplex(): stream->Write() failed. i=%d", i);
	      return TransientFailureOrAbort ();
	    }
	}
      stream->WritesDone ();
      ...
      if (i < response_stream_sizes.size ())
	{
	  ...
	  gpr_log (GPR_ERROR,
		   ...
		   "number of messages response_stream_sizes.size() (%"
		   PRIuPTR ")", i, response_stream_sizes.size ());
	  return TransientFailureOrAbort ();
	}
      ...
      for (unsigned int i = 0; i < request_stream_sizes.size (); ++i)
	{
	  ...
	  if (!stream->Write (request))
	    {
	      gpr_log (GPR_ERROR,
		       "DoPingPong(): stream->Write() failed. i: %d", i);
	      return TransientFailureOrAbort ();
	    }
	...
	serviceStub_.Get ()->UnaryCall (&context, request, &response);
      if (!AssertStatusCode
	  (s, grpc::StatusCode::UNKNOWN, context.debug_error_string ()))
	{
	  return false;
	}
	...
	std::to_string ((long long unsigned) ts.tv_nsec);
      SimpleRequest request;
      ...
      Status s1 =
	serviceStub_.Get ()->CacheableUnaryCall (&context1, request,
						 &response1);
      if (!AssertStatusOk (s1, context1.debug_error_string ()))
	{
      ...
      Status s2 =
	serviceStub_.Get ()->CacheableUnaryCall (&context2, request,
						 &response2);
      if (!AssertStatusOk (s2, context2.debug_error_string ()))
	{
      ...
      Status s3 =
	serviceStub_.Get ()->CacheableUnaryCall (&context3, request1,
						 &response3);
      if (!AssertStatusOk (s3, context3.debug_error_string ()))
	{
      ...
      GPR_ASSERT (response3.payload ().body () !=
		  response1.payload ().body ());
      return true;
    }
    ...
    bool InteropClient::DoCustomMetadata ()
    {
      ...
      const grpc::
	string kInitialMetadataValue ("test_initial_metadata_value");
      const grpc::
	string kEchoTrailingBinMetadataKey ("x-grpc-test-echo-trailing-bin");
	...
	GPR_ASSERT (grpc::
		    string (iter->second.begin (),
			    iter->second.end ()) == kTrailingBinValue);
