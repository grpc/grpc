...
Http2Client::ServiceStub::ServiceStub(const std::shared_ptr<Channel>& channel) : channel_(channel) {
...
Http2Client::Http2Client(const std::shared_ptr<Channel>& channel)
    : serviceStub_(channel), channel_(channel), defaultRequest_(BuildDefaultRequest()) {}
...
bool Http2Client::AssertStatusCode(const Status& s, StatusCode expected_code) {
  ...
  gpr_log(GPR_ERROR, "Error status code: %d (expected: %d), message: %s", s.error_code(),
...
void Http2Client::MaxStreamsWorker(const std::shared_ptr<grpc::Channel>& /*channel*/) {
  SimpleResponse response;
  AssertStatusCode(SendUnaryCall(&response), grpc::StatusCode::OK);
  GPR_ASSERT(response.payload().body() == std::string(kLargeResponseSize, '\0'));
}
...
bool Http2Client::DoMaxStreams() {
  ...
  for (int i = 0; i < 10; i++) {
    test_threads.emplace_back(std::thread(&Http2Client::MaxStreamsWorker, this, channel_));
  }
...
int main(int argc, char** argv) {
  ...
  snprintf(host_port, host_port_buf_size, "%s:%d", absl::GetFlag(FLAGS_server_host).c_str(),
  ...
  GPR_ASSERT(channel->WaitForConnected(
      gpr_time_add(gpr_now(GPR_CLOCK_REALTIME), gpr_time_from_seconds(300, GPR_TIMESPAN))));
  ...
  if (absl::GetFlag(FLAGS_test_case) == "rst_after_header") {
    ...
    const char* testcases[] = {"goaway",         "max_streams",      "ping",
