...
namespace grpc
{
  namespace testing
  {
    ...
    Http2Client::ServiceStub::ServiceStub (std::shared_ptr < Channel >
  ...
  Http2Client::Http2Client (std::shared_ptr < Channel > channel):serviceStub_ (channel),
      channel_ (channel),
    ...
    void Http2Client::MaxStreamsWorker (std::shared_ptr < grpc::Channel >
					channel)
    {
      SimpleResponse response;
      AssertStatusCode (SendUnaryCall (&response), grpc::StatusCode::OK);
      ...
      for (int i = 0; i < 10; i++)
	{
	  test_threads.
	    emplace_back (std::
			  thread (&Http2Client::MaxStreamsWorker, this,
				  channel_));
	}
