...
DEFINE_int32 (num_channels_per_server, 1,
...
static int
  log_level = GPR_LOG_SEVERITY_DEBUG;
...
void
TestLogFunction (gpr_log_func_args * args)
{
...
TestCaseType
GetTestTypeFromName (const grpc::string & test_name)
{
  TestCaseType
...
bool
ParseCommaDelimitedString (const grpc::string & comma_delimited_str,
			   std::vector < grpc::string > &tokens)
{
  size_t
    bpos = 0;
  size_t
    epos = grpc::string::npos;
...
bool
ParseTestCasesString (const grpc::string & test_cases,
		      std::vector < std::pair < TestCaseType, int >>&tests)
{
  ...
  for (auto it = tokens.begin (); it != tokens.end (); it++)
      ...
      if (colon_pos == grpc::string::npos)
	{
      ...
      int
	weight = std::stoi (it->substr (colon_pos + 1));
      TestCaseType
	test_case = GetTestTypeFromName (test_name);
...
void
LogParameterInfo (const std::vector < grpc::string > &addresses,
...
main (int argc, char **argv)
{
  ...
  std::vector < std::pair < TestCaseType, int >>
    tests;
  ...
  for (auto it = server_addresses.begin (); it != server_addresses.end ();
	   ...
	   channel_idx++)
	{
	  ...
	  std::shared_ptr < grpc::Channel > channel =
	    grpc::CreateTestChannel (*it, FLAGS_server_host_override,
	  ...
	  for (int stub_idx = 0; stub_idx < FLAGS_num_stubs_per_channel;
	       stub_idx++)
	    {
	      clients.
		emplace_back (new
			      StressTestInteropClient (++thread_idx, *it,
						       channel, test_selector,
						       FLAGS_test_duration_secs,
		...
		is_already_created = false;
	      ...
	      test_threads.
		emplace_back (std::
			      thread (&StressTestInteropClient::MainLoop,
				      clients.back ().get (),
