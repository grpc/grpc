...
const char
  kEchoInitialMetadataKey[] = "x-grpc-test-echo-initial";
const char
  kEchoTrailingBinMetadataKey[] = "x-grpc-test-echo-trailing-bin";
const char
  kEchoUserAgentKey[] = "x-grpc-test-echo-useragent";

void
MaybeEchoMetadata (ServerContext * context)
{
  const
    auto &
  ...
  auto
    iter = client_metadata.find (kEchoInitialMetadataKey);
  if (iter != client_metadata.end ())
    {
      context->AddInitialMetadata (kEchoInitialMetadataKey,
				   grpc::string (iter->second.begin (),
						 iter->second.end ()));
    }
...
bool
SetPayload (int size, Payload * payload)
{
  std::unique_ptr < char[] >
  body (new char[size] ());
...
bool
CheckExpectedCompression (const ServerContext & context,
			  const bool compression_expected)
{
  const InteropServerContextInspector
  inspector (context);
  const grpc_compression_algorithm
    received_compression = inspector.GetCallCompressionAlgorithm ();
...
public:
			     ...
			     SimpleResponse * response)
  {
    ...
    response->mutable_payload ()->set_body (timestamp.c_str (),
					    timestamp.size ());
  ...
  Status StreamingOutputCall (ServerContext * context,
			      const StreamingOutputCallRequest * request,
			      ServerWriter < StreamingOutputCallResponse >
			      *writer)
  {
  ...
  Status StreamingInputCall (ServerContext * context,
			     ServerReader < StreamingInputCallRequest >
			     *reader, StreamingInputCallResponse * response)
  {
			 ...
			 StreamingOutputCallRequest > *stream)
  {
	...
	if (request.response_parameters_size () != 0)
	  {
	    response.mutable_payload ()->set_type (request.payload ().
						   type ());
	    response.mutable_payload ()->
	      set_body (grpc::
			string (request.response_parameters (0).size (),
				'\0'));
	    int time_us;
	    if ((time_us =
			 ...
			 StreamingOutputCallRequest > *stream)
  {
    ...
    for (unsigned int i = 0; write_success && i < requests.size (); i++)
      {
	response.mutable_payload ()->set_type (requests[i].payload ().
					       type ());
	...
	response.mutable_payload ()->
	  set_body (grpc::
		    string (requests[i].response_parameters (0).size (),
			    '\0'));
	write_success = stream->Write (response);
      }
    if (write_success)
...
void
grpc::testing::interop::RunServer (std::shared_ptr < ServerCredentials >
				   creds)
{
...
void
grpc::testing::interop::RunServer (std::shared_ptr < ServerCredentials >
				   creds, const int port,
				   ServerStartedCondition *
				   server_started_condition)
{
  GPR_ASSERT (port != 0);
