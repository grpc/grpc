...
typedef std::function<std::shared_ptr<Channel>(void)>
    ChannelCreationFunc;

class InteropClient {
 public:
  /// If new_stub_every_test_case is true, a new TestService::Stub
  /// object is created for every test case If
  /// do_not_abort_on_transient_failures is true, abort() is not called
  ...
  // Verifies Unicode and Whitespace is correctly processed in status
  // message.
  ...
  // all requests are sent to one server despite multiple servers are
  // resolved
  bool DoPickFirstUnary();

  // The following interop test are not yet part of the interop spec,
  // and are not implemented cross-language. They are considered
  // experimental for now, but at some point in the future, might be
  // codified and implemented in all languages
  ...
  bool DoGoogleDefaultCredentials(
 ...
 private:
  class ServiceStub {
   ...
   private:
    ...
    std::unique_ptr<UnimplementedService::Stub>
        unimplemented_service_stub_;
    std::shared_ptr<Channel> channel_;
    bool new_stub_every_call_;  // If true, a new stub is returned by
                                // every Get() call
  };

  bool PerformLargeUnary(SimpleRequest* request,
