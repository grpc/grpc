...
def run_cmd(args, desc, env=None, cwd=None):
    ...
    if env:
        proc_env.update(env)
    res = subprocess.run(args, env=proc_env, cwd=cwd, capture_output=True, text=True)
    if res.returncode != 0:
...
def start_proc(args, env, desc, cwd=None):
    ...
    proc_env = os.environ.copy()
    proc_env.update(env)
    return subprocess.Popen(args, env=proc_env, cwd=cwd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def verify_spans(spans_file):
    ...
    for s in all_spans:
        print(f"  Span: '{s.get('name')}' (TraceID: {s.get('trace_id')}, SpanID: {s.get('span_id')}, ParentID: {s.get('parent_span_id')})")

    # 1. Assert all spans share the same Trace ID
    trace_id = client_span.get("trace_id")
    if attempt_span.get("trace_id") != trace_id:
        print(f"Assertion Failed: Attempt span Trace ID mismatch. Expected {trace_id}, got {attempt_span.get('trace_id')}")
        return False
    if server_span.get("trace_id") != trace_id:
        print(f"Assertion Failed: Server span Trace ID mismatch. Expected {trace_id}, got {server_span.get('trace_id')}")
        return False
    ...
    if attempt_span.get("parent_span_id") != client_span.get("span_id"):
        print(f"Assertion Failed: Attempt span is not child of Client span. Expected parent {client_span.get('span_id')}, got {attempt_span.get('parent_span_id')}")
        return False
    # Server Span must be a child of Attempt Span
    if server_span.get("parent_span_id") != attempt_span.get("span_id"):
        print(f"Assertion Failed: Server span is not child of Attempt span. Expected parent {attempt_span.get('span_id')}, got {server_span.get('parent_span_id')}")
        return False
    ...
    if prev_attempts is None:
        print("Assertion Failed: Attribute 'previous-rpc-attempts' not found on Attempt span.")
        return False
    if int(prev_attempts) != 0:
        print(f"Assertion Failed: Attribute 'previous-rpc-attempts' value mismatch. Expected 0, got {prev_attempts}")
        return False
    if trans_retry is None:
        print("Assertion Failed: Attribute 'transparent-retry' not found on Attempt span.")
        return False
    if trans_retry is not False:
        print(f"Assertion Failed: Attribute 'transparent-retry' value mismatch. Expected False, got {trans_retry}")
        return False
    ...
    if "Outbound message" not in attempt_events and "Outbound message" not in client_events:
        print("Assertion Failed: Event 'Outbound message' not found on Client or Attempt span.")
        return False
    if "Inbound message" not in attempt_events and "Inbound message" not in client_events:
        print("Assertion Failed: Event 'Inbound message' not found on Client or Attempt span.")
        return False

    server_events = [e.get("name") for e in server_span.get("events", [])]
    if "Inbound message" not in server_events:
        print("Assertion Failed: Event 'Inbound message' not found on Server span.")
        return False
    if "Outbound message" not in server_events:
        print("Assertion Failed: Event 'Outbound message' not found on Server span.")
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Build and run OpenTelemetry Interop tests locally")
    parser.add_argument("--skip_build", action="store_true", help="Skip building targets with Bazel/Gradle")
    parser.add_argument("--client", choices=["c++", "java", "python"], default="c++", help="Client language (c++, java, or python)")
    parser.add_argument("--server", choices=["c++", "java", "python"], default="c++", help="Server language (c++, java, or python)")
    args = parser.parse_args()

    if not args.skip_build:
        if args.client == "c++" or args.server == "c++":
            run_cmd(["./tools/bazel", "build", "--macos_minimum_os=11.0", "//test/cpp/interop:interop_client", "//test/cpp/interop:interop_server", "//test/cpp/interop:otlp_collector"], "Building C++ interop targets")
        if args.client == "java" or args.server == "java":
            run_cmd(["./gradlew", ":grpc-interop-testing:installDist", "-x", "test"], "Building Java interop targets", cwd="../grpc-java")
        if args.client == "python" or args.server == "python":
            run_cmd(["./tools/bazel", "build", "--macos_minimum_os=11.0", "//src/python/grpcio_tests/tests/interop:client", "//src/python/grpcio_tests/tests/interop:server_bin"], "Building Python interop targets")
    ...
    try:
        ...
        collector_proc = start_proc(
            ["./bazel-bin/test/cpp/interop/otlp_collector", f"--port={collector_port}", f"--file={spans_file}"],
            {},
            "OTLP Collector"
        )
        ...
        if args.server == "c++":
            server_proc = start_proc(
                ["./bazel-bin/test/cpp/interop/interop_server", f"--port={server_port}", "--enable_opentelemetry=true"],
                env,
                "C++ Interop Server"
            )
        elif args.server == "java":
            server_proc = start_proc(
                ["../grpc-java/interop-testing/build/install/grpc-interop-testing/bin/test-server", f"--port={server_port}", "--use_tls=false", "--enable_opentelemetry=true"],
                env,
                "Java Interop Server"
            )
        elif args.server == "python":
            server_proc = start_proc(
                ["./bazel-bin/src/python/grpcio_tests/tests/interop/server_bin", f"--port={server_port}", "--use_tls=false", "--enable_opentelemetry=true"],
                env,
        ...
        if args.client == "c++":
            ...
            ], "Running C++ Interop Client", env=env)
            ...
            client_res = run_cmd([
                ...
                "--use_tls=false",
                "--enable_opentelemetry=true"
            ], "Running Java Interop Client", env=env)
            ...
            client_res = run_cmd([
                ...
                "--use_tls=false",
                "--enable_opentelemetry=true"
            ], "Running Python Interop Client", env=env)
