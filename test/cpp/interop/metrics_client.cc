...
// Do not log anything
void
BlackholeLogger (gpr_log_func_args * args)
{
}
...
bool
PrintMetrics (std::unique_ptr < MetricsService::Stub > stub, bool total_only,
	      int deadline_secs)
{
  ...
  std::unique_ptr < grpc::ClientReader <
    GaugeResponse >> reader (stub->GetAllGauges (&context, message));
  ...
  while (reader->Read (&gauge_response))
	...
	{
	  std::cout << "Gauge '" << gauge_response.
	    name () << "' is not long valued" << std::endl;
	}
    }
...
main (int argc, char **argv)
{
  ...
  std::shared_ptr < grpc::Channel >
    channel (grpc::
	     CreateChannel (FLAGS_metrics_server_address,
