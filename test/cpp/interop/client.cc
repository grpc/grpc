...
ABSL_FLAG(bool, use_test_ca, false,
...
ABSL_FLAG(std::string, test_case, "large_unary",
          ...
          // TODO(veblush): Replace the help message with the following
          // full message once Abseil fixes the flag-help compiler error
          // on Windows. (b/171659833)
          /*
          "cancel_after_begin : cancel stream after starting it;\n"
          "cancel_after_first_response: cancel on first response;\n"
          "channel_soak: sends 'soak_iterations' rpcs, rebuilds channel
          each time;\n" "client_compressed_streaming : compressed
          request streaming with " "client_compressed_unary : single
          compressed request;\n" "client_streaming : request streaming
          with single response;\n" "compute_engine_creds: large_unary
          with compute engine auth;\n" "custom_metadata: server will
          echo custom metadata;\n" "empty_stream : bi-di stream with no
          request/response;\n" "empty_unary : empty (zero bytes) request
          and response;\n" "google_default_credentials: large unary
          using GDC;\n" "half_duplex : half-duplex streaming;\n"
          "jwt_token_creds: large_unary with JWT token auth;\n"
          "large_unary : single request and (large) response;\n"
          "long_lived_channel: sends large_unary rpcs over a long-lived
          channel;\n" "oauth2_auth_token: raw oauth2 access token
          auth;\n" "per_rpc_creds: raw oauth2 access token on a single
          rpc;\n" "ping_pong : full-duplex streaming;\n" "response
          streaming;\n" "rpc_soak: 'sends soak_iterations' large_unary
          rpcs;\n" "server_compressed_streaming : single request with
          compressed " "server_compressed_unary : single compressed
          response;\n" "server_streaming : single request with response
          streaming;\n" "slow_consumer : single request with response
          streaming with " "slow client consumer;\n"
          "special_status_message: verify Unicode and whitespace in
          status message;\n" "status_code_and_message: verify status
          code & message;\n" "timeout_on_sleeping_server: deadline
          exceeds on stream;\n" "unimplemented_method: client calls an
          unimplemented method;\n" "unimplemented_service: client calls
          an unimplemented service;\n"
          */
);
...
ABSL_FLAG(
    bool, do_not_abort_on_transient_failures, false,
...
// alphanumeric characters and dashes in keys, and any character but
// semicolons in values. Convert keys to lowercase. On failure, log an
...
bool ParseAdditionalMetadataFlag(
  ...
  while (start_pos < flag.length()) {
    ...
    if (colon_pos == std::string::npos) {
      gpr_log(GPR_ERROR,
              "Couldn't parse metadata flag: extra characters at end "
              "of flag");
      return false;
    }
    ...
    constexpr char alphanum_and_hyphen[] =
        ...
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    if (key.find_first_not_of(alphanum_and_hyphen) !=
        std::string::npos) {
      gpr_log(
          GPR_ERROR,
...
int main(int argc, char** argv) {
  ...
  if (absl::GetFlag(FLAGS_additional_metadata).empty()) {
    ...
    std::multimap<std::string, std::string> additional_metadata;
    if (!ParseAdditionalMetadataFlag(
  ...
  actions["client_streaming"] = std::bind(
      &grpc::testing::InteropClient::DoRequestStreaming, &client);
  actions["server_streaming"] = std::bind(
      &grpc::testing::InteropClient::DoResponseStreaming, &client);
  ...
  actions["slow_consumer"] =
      std::bind(&grpc::testing::InteropClient::
  ...
  actions["cancel_after_begin"] = std::bind(
      &grpc::testing::InteropClient::DoCancelAfterBegin, &client);
  ...
  actions["pick_first_unary"] = std::bind(
      &grpc::testing::InteropClient::DoPickFirstUnary, &client);
  if (absl::GetFlag(FLAGS_use_tls)) {
    ...
    actions["jwt_token_creds"] =
        std::bind(&grpc::testing::InteropClient::DoJwtTokenCreds,
                  &client, GetServiceAccountJsonKey());
    actions["oauth2_auth_token"] =
        std::bind(&grpc::testing::InteropClient::DoOauth2AuthToken,
                  &client, absl::GetFlag(FLAGS_default_service_account),
                  absl::GetFlag(FLAGS_oauth_scope));
  ...
  if (absl::GetFlag(FLAGS_custom_credentials_type) ==
    ...
    actions["google_default_credentials"] = std::bind(
        &grpc::testing::InteropClient::DoGoogleDefaultCredentials,
        &client, absl::GetFlag(FLAGS_default_service_account));
  }
  actions["status_code_and_message"] = std::bind(
      &grpc::testing::InteropClient::DoStatusWithMessage, &client);
  actions["special_status_message"] = std::bind(
      &grpc::testing::InteropClient::DoSpecialStatusMessage, &client);
  actions["custom_metadata"] = std::bind(
      &grpc::testing::InteropClient::DoCustomMetadata, &client);
  actions["unimplemented_method"] = std::bind(
      &grpc::testing::InteropClient::DoUnimplementedMethod, &client);
  actions["unimplemented_service"] = std::bind(
      &grpc::testing::InteropClient::DoUnimplementedService, &client);
  actions["cacheable_unary"] = std::bind(
      &grpc::testing::InteropClient::DoCacheableUnary, &client);
  ...
  actions["long_lived_channel"] =
      std::bind(&grpc::testing::InteropClient::DoLongLivedChannelTest,
                &client, absl::GetFlag(FLAGS_soak_iterations),
  ...
  if (absl::GetFlag(FLAGS_test_case) == "all") {
  ...
  } else if (actions.find(absl::GetFlag(FLAGS_test_case)) !=
             actions.end()) {
    ...
    gpr_log(GPR_ERROR,
            "Unsupported test case %s. Valid options are\n%s",
