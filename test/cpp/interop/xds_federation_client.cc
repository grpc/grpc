...
ABSL_FLAG(std::string, credentials_types, "",
          "Comma-separated list of credentials, each entry is used for the server of the corresponding index in server_uris. Supported values: compute_engine_channel_creds, INSECURE_CREDENTIALS");
...
ABSL_FLAG(int32_t, soak_overall_timeout_seconds, 10,
          "The overall number of seconds after which a soak test should stop and fail, if the desired number of iterations have not yet completed.");
ABSL_FLAG(int32_t, soak_min_time_ms_between_rpcs, 0,
          "The minimum time in milliseconds between consecutive RPCs in a soak test (rpc_soak or channel_soak), useful for limiting QPS");
  ...
  for (int i = 0; i < uris.size(); i++) {
    threads.push_back(std::thread([](){
      ...
      if (test_case == "rpc_soak") {
        ...
        gpr_log(GPR_ERROR, "Invalid test case, must be either rpc_soak or channel_soak");
        GPR_ASSERT(0);
      }
    }));
  }
