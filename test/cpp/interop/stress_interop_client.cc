...
namespace grpc
{
  namespace testing
  {
    ...
    using std::vector;
      ...
      WeightedRandomTestSelector::WeightedRandomTestSelector (const vector <
							      pair <
							      TestCaseType,
							      int
							      >>&tests):tests_
...
// passed in the constructror
  ...
  StressTestInteropClient::StressTestInteropClient (int test_id, const grpc::string & server_address, std::shared_ptr < Channel > channel, const WeightedRandomTestSelector & test_selector, long test_duration_secs, long sleep_duration_ms, bool do_not_abort_on_transient_failures):test_id_ (test_id),
    ...
    void StressTestInteropClient::MainLoop (std::shared_ptr < QpsGauge >
					    qps_gauge)
    {
      ...
      else
	{
	  test_end_time =
	    gpr_time_add (gpr_now (GPR_CLOCK_REALTIME),
			  gpr_time_from_seconds (test_duration_secs_,
						 GPR_TIMESPAN));
	}
      ...
      while (gpr_time_cmp (gpr_now (GPR_CLOCK_REALTIME), test_end_time) < 0)
	{
	  ...
	  if (sleep_duration_ms_ > 0)
		...
		gpr_time_add (gpr_now (GPR_CLOCK_REALTIME),
			      gpr_time_from_millis (sleep_duration_ms_,
						    GPR_TIMESPAN));
