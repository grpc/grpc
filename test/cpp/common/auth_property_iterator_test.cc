  ...
  grpc
{
  namespace
  {
	...
	EXPECT_EQ (1,
		   grpc_auth_context_set_peer_identity_property_name (ctx_,
								      "name"));
      }
      void
      TearDown ()
	...
	c_iter = grpc_auth_context_property_iterator (ctx_);
      const grpc_auth_property *
	property = grpc_auth_property_iterator_next (&c_iter);
      TestAuthPropertyIterator
