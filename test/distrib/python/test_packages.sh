...
then
  ...
  ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?[0-9]*.whl)
  TOOLS_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?tools[_-]?[0-9]*.whl)
  ...
  ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?[0-9]*.tar.gz)
  TOOLS_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?tools[_-]?[0-9]*.tar.gz)
  OBSERVABILITY_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?observability[_-]?[0-9]*.tar.gz)
fi

HEALTH_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?health[_-]?checking[_-]?[0-9]*.tar.gz)
REFLECTION_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?reflection[_-]?[0-9]*.tar.gz)
TESTING_ARCHIVES=("$EXTERNAL_GIT_ROOT"/input_artifacts/grpcio[_-]?testing[_-]?[0-9]*.tar.gz)
