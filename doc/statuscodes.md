...
gRPC uses a set of well defined status codes as part of the RPC API. All RPCs started at a client return  a `status` object composed of an integer `code` and an ASCII string `message`. The server-side can choose the status it returns for a given RPC.
